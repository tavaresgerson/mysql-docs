...
* Valores hexadecimais são tratados como strings binárias se não forem comparados a um número.
...
* Em todos os outros casos, os argumentos são comparados como números de ponto flutuante (dupla precisão). Por exemplo, uma comparação de operandos de string e número ocorre como uma comparação de números de ponto flutuante.
...
Para comparações de uma coluna de texto com um número, o MySQL não pode usar um índice na coluna para pesquisar o valor rapidamente. Se *`str_col`* for uma coluna de texto indexada, o índice não pode ser usado ao realizar a pesquisa na seguinte declaração:

```
SELECT * FROM tbl_name WHERE str_col=1;
```

A razão para isso é que existem muitas strings diferentes que podem converter para o valor `1`, como `'1'`, `' 1'` ou `'1a'`.

Outro problema pode surgir ao comparar uma coluna de texto com o inteiro `0`. Considere a tabela `t1` criada e preenchida como mostrado aqui:
...
Quando o `0` é citado, a declaração é bem-sucedida, mas a tabela criada não contém nenhuma linha porque não havia nenhuma correspondente a `'0'`, como mostrado aqui:
...
Este é um problema conhecido, que se deve ao fato de que o modo rigoroso não é aplicado ao processamento de `SELECT`. Veja também Modo SQL Rigoroso.

As comparações entre números de ponto flutuante e valores de inteiro grande são aproximadas porque o inteiro é convertido para ponto flutuante de dupla precisão antes da comparação, o que não é capaz de representar exatamente todos os inteiros de 64 bits. Por exemplo, o valor inteiro 253 + 1 não é representável como um float e é arredondado para 253 ou 253 + 2 antes de uma comparação com float, dependendo da plataforma.

Para ilustrar, apenas a primeira das seguintes comparações compara valores iguais, mas ambas retornam verdadeiro (1):
...
Para obter mais informações sobre comparações em ponto flutuante, consulte a Seção B.3.4.8, “Problemas com Valores em Ponto Flutuante”.
...
Como as conversões produzidas por essa biblioteca diferem em alguns casos dos resultados sem `dtoa`, existe a possibilidade de incompatibilidades em aplicações que dependem de resultados anteriores. Por exemplo, aplicações que dependem de um resultado exato específico de conversões anteriores podem precisar de ajustes para acomodar uma precisão adicional.

A biblioteca `dtoa` fornece conversões com as seguintes propriedades. *`D`* representa um valor com uma representação `DECIMAL` - DECIMAL, NUMERIC") ou de string, e *`F`* representa um número de ponto flutuante no formato binário nativo (IEEE).

* A conversão *`F`* -> *`D`* é feita com a melhor precisão possível, retornando *`D`* como a string mais curta que gera *`F`* ao ser lida de volta e arredondada para o valor mais próximo no formato binário nativo, conforme especificado pelo IEEE.
...
A conversão implícita de um valor numérico ou temporal para string produz um valor que tem um conjunto de caracteres e uma ordenação determinados pelas variáveis de sistema `character_set_connection` e `collation_connection`. (Essas variáveis são comumente definidas com `SET NAMES`. Para informações sobre conjuntos de caracteres de conexão, consulte a Seção 12.4, “Conjuntos de caracteres de conexão e ordenações”).

Isso significa que tal conversão resulta em uma string de caracteres (não binária) (um valor `CHAR`, `VARCHAR` ou `LONGTEXT`), exceto no caso em que o conjunto de caracteres de conexão é definido como `binary`. Nesse caso, o resultado da conversão é uma string binária (um valor `BINARY`, `VARBINARY` ou `LONGBLOB`).

Para expressões numéricas, as observações anteriores sobre a *avaliação* da expressão se aplicam de maneira um pouco diferente para a *atribuição* da expressão; por exemplo, em uma declaração como esta:

```
CREATE TABLE t SELECT integer_expr;
```

Neste caso, a tabela na coluna resultante da expressão tem o tipo `INT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") ou `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") dependendo do comprimento da expressão numérica. Se o comprimento máximo da expressão não cabe em um `INT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT"), `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") é usado em vez disso. O comprimento é obtido a partir do valor `max_length` do metadado do conjunto de resultados da consulta (`SELECT`) (consulte a API C Estrutura de dados básicas de dados). Isso significa que você pode forçar um `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") em vez de `INT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") usando uma expressão suficientemente longa:
