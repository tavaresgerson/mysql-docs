...
<table frame="box" rules="all" summary="Uma referência que lista operadores de comparação."><col style="width: 28%"/><col style="width: 71%"/><thead><tr><th>Nome</th> <th>Descrição</th> </tr></thead><tbody><tr><td><a class="link" href="operadores-de-comparação.html#operador-maior-que"><code>&gt;</code></a></td> <td>Operador maior que </td> </tr><tr><td><a class="link" href="operadores-de-comparação.html#operador-maior-que-ou-igual"><code>&gt;=</code></a></td> <td>Operador maior que ou igual </td> </tr><tr><td><a class="link" href="operadores-de-comparação.html#operador-menor-que"><code>&lt;</code></a></td> <td>Operador menor que </td> </tr><tr><td><a class="link" href="operadores-de-comparação.html#operador-diferente-de"><code>&lt;&gt;</code>, <code class="literal">!=</code></a></td> <td>Operador diferente de </td> </tr><tr><td><a class="link" href="operadores-de-comparação.html#operador-menor-que-ou-igual"><code>&lt;=</code></a></td> <td>Operador menor que ou igual </td> </tr><tr><td><a class="link" href="operadores-de-comparação.html#operador-igual-a"><code>&lt;=&gt;</code></a></td> <td>Operador igual-a, NULL-safe </td> </tr><tr><td><a class="link" href="operadores-de-comparação.html#operador-igual"><code>=</code></a></td> <td>Operador igual </td> </tr><tr><td><a class="link" href="operadores-de-comparação.html#operador-entre"><code>BETWEEN ... AND ...</code></a></td> <td>Verificar se um valor está dentro de uma faixa de valores </td> </tr><tr><td><a class="link" href="funções-de-comparações.html#coalesce"><code>COALESCE()</code></a></td> <td>Retornar o primeiro argumento que não é NULL </td> </tr><tr><td><a class="link" href="operadores-de-comparações.html#existe"><code>EXISTS()</code></a></td> <td>Verificar se o resultado de uma consulta contém alguma linha </td> </tr><tr><td><a class="link" href="operadores-de-comparações.html#maior"><code>GREATEST()</code></a></td> <td>Retornar o maior argumento </td> </tr><tr><td><a class="link" href="operadores-de-comparações.html#pertencente"><code>IN()</code></a></td> <td>Verificar se um valor está dentro de um conjunto de valores </td> </tr><tr><td><a class="link" href="funções-de-comparações.html#intervalo"><code>INTERVAL()</code></a></td> <td>Retornar o índice do argumento que é menor que o primeiro argumento </td> </tr><tr><td><a class="link" href="operadores-de-comparações.html#igual"><code>IS</code></a></td> <td>Testar um valor contra um booleano </td> </tr><tr><td><a class="link" href="operadores-de-comparações.html#diferente-de"><code>IS NOT</code></a></td> <td>Testar um valor contra um booleano </td> </tr><tr><td><a class="link" href="operadores-de-comparações.html#não-NULL"><code>IS NOT NULL</code></a></td> <td>Testar se o argumento é NULL </td> </tr><tr><td><a class="link" href="operadores-de-comparações.html#NULL"><code>IS NULL</code></a></td> <td>Testar se o argumento é NULL </td> </tr><tr><td><a class="link" href="funções-de-comparações.html#isnull"><code>ISNULL()</code></a></td> <td>Testar se o argumento é NULL </td> </tr><tr><td><a class="link" href="funções-de-comparações.html#menor"><code>LEAST()</code></a></td> <td>Retornar o menor argumento </td> </tr><tr><td><a class="link" href="funções-de-comparações.html#semelhante"><code>LIKE</code></a></td> <td>Busca simples de padrões </td> 

As operações de comparação resultam em um valor de `1` (`TRUE`), `0` (`FALSE`) ou `NULL`. Essas operações funcionam tanto com operandos escalares quanto com operandos de linha. As strings são automaticamente convertidas em números e os números em strings conforme necessário.
...
Em versões anteriores do MySQL, ao avaliar uma expressão contendo `LEAST()` ou `GREATEST()`, o servidor tentava adivinhar o contexto em que a função foi usada e forçar os argumentos da função ao tipo de dados do conjunto da expressão. Por exemplo, os argumentos de `LEAST("11", "45", "2")` são avaliados e ordenados como strings, de modo que essa expressão retorne `"11"`.
...
Por padrão, as comparações de string não são sensíveis ao caso e usam o conjunto de caracteres atual. O padrão é `utf8mb4`.
...
* `GREATEST(value1,value2,...)`
  ...
  Com dois ou mais argumentos, retorna o argumento maior (com o valor máximo). Os argumentos são comparados usando as mesmas regras que para `LEAST()`.
...
Você nunca deve misturar valores citados e não citados em uma lista `IN()`, pois as regras de comparação para valores citados (como strings) e valores não citados (como números) diferem. A mistura de tipos pode, portanto, levar a resultados inconsistentes. Por exemplo, não escreva uma expressão `IN()` assim:
...
Para cumprir com o padrão SQL, `IN()` retorna `NULL` não apenas se a expressão do lado esquerdo for `NULL`, mas também se não for encontrado nenhum correspondente na lista e uma das expressões na lista for `NULL`.
...
* `IS boolean_value`
  ...
  Testará um valor contra um valor booleano, onde *`boolean_value`* pode ser `TRUE`, `FALSE` ou `UNKNOWN`.
...
* `IS NULL`
  ...
  + Se a variável `sql_auto_is_null` for definida como 1, após uma instrução que insere com sucesso um valor gerado automaticamente `AUTO_INCREMENT`, você pode encontrar esse valor emitindo uma instrução do seguinte formato:
...
Veja Obtenção de Valores de Autoincremento e a descrição da opção `FLAG_AUTO_IS_NULL` nos Parâmetros de Conexão do Connector/ODBC.
...
* `ISNULL(expr)`
  ...
  Se *`expr`* for `NULL`, o `ISNULL()` retorna `1`, caso contrário, ele retorna `0`.
  ...
  O `ISNULL()` pode ser usado em vez de `=` para verificar se um valor é `NULL`. (Comparar um valor com `NULL` usando `=` sempre resulta em `NULL`.)
...
* `LEAST(value1,value2,...)`
  ...
  Com dois ou mais argumentos, retorna o menor (valor mínimo) argumento. Os argumentos são comparados seguindo as regras a seguir:

  + Se algum argumento for `NULL`, o resultado é `NULL`. Não é necessário fazer uma comparação.

  + Se todos os argumentos forem de valor inteiro, eles são comparados como inteiros.

  + Se pelo menos um argumento for de valor de ponto flutuante, eles são comparados como valores de ponto flutuante. Caso contrário, se pelo menos um argumento for de valor `DECIMAL` - DECIMAL, NUMERIC")`, eles são comparados como valores `DECIMAL` - DECIMAL, NUMERIC")`.

  + Se os argumentos compreendem uma mistura de números e strings, eles são comparados como strings.

  + Se algum argumento for uma string não binária (caractere), os argumentos são comparados como strings não binárias.

  + Em todos os outros casos, os argumentos são comparados como strings binárias.

  O tipo de retorno de `LEAST()` é o tipo agregado dos tipos de argumentos da comparação.
