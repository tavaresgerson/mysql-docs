...
As funções desta seção retornam atributos de valores JSON.
...
* `JSON_TYPE(json_val)`
  ...
  Para um resultado não `NULL`, não `erro`, a seguinte lista descreve os possíveis valores de retorno de `JSON_TYPE()`:
...
- `DATETIME`: Escalares `DATETIME` e `TIMESTAMP` MySQL

- `DATE`: Escalares `DATE` MySQL

- `TIME`: Escalares `TIME` MySQL

Tipos de string:

- `STRING`: Escalares de tipo caractere `utf8mb3` MySQL: `CHAR`, `VARCHAR`, `TEXT`, `ENUM` e `SET`

Tipos binários:

- `BLOB`: Escalares de tipo binário MySQL, incluindo `BINARY`, `VARBINARY`, `BLOB` e `BIT`
