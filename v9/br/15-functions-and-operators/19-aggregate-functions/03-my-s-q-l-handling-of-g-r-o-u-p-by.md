...
O SQL:1999 e versões posteriores permitem tais não agregados como recurso opcional T301, desde que sejam funcionalmente dependentes das colunas `GROUP BY`: Se houver uma relação entre `name` e `custid`, a consulta é legal. Esse seria o caso, por exemplo, se `custid` fosse uma chave primária de `customers`.
...
A consulta é válida se `nome` for uma chave primária de `t` ou uma coluna `NOT NULL` única. Nesses casos, o MySQL reconhece que a coluna selecionada é funcionalmente dependente de uma coluna de agrupamento. Por exemplo, se `nome` for uma chave primária, seu valor determina o valor de `endereço` porque cada grupo tem apenas um valor da chave primária e, portanto, apenas uma linha. Como resultado, não há aleatoriedade na escolha do valor de `endereço` em um grupo e não há necessidade de rejeitar a consulta.

A consulta é inválida se `nome` não for uma chave primária de `t` ou uma coluna `NOT NULL` única. Nesse caso, não pode ser inferida uma dependência funcional e ocorre um erro:
...
Se você souber que, *para um conjunto de dados específico*, cada valor de `nome` de fato determina o valor de `endereço`, `endereço` é efetivamente funcionalmente dependente de `nome`. Para dizer ao MySQL para aceitar a consulta, você pode usar a função `ANY_VALUE()`:
...
O exemplo anterior é bastante simples, no entanto. Em particular, é improvável que você agrupe em uma única coluna de chave primária, pois cada grupo conterá apenas uma linha. Para exemplos adicionais que demonstram a dependência funcional em consultas mais complexas, consulte a Seção 14.19.4, “Detecção de Dependência Funcional”.

Se uma consulta tiver funções agregadas e nenhuma cláusula `GROUP BY`, ela não pode ter colunas não agregadas na lista `SELECT`, na condição `HAVING` ou na lista `ORDER BY` com `ONLY_FULL_GROUP_BY` habilitada:
...
Sem `GROUP BY`, há um único grupo e ele é não determinístico, que valor de `nome` escolher para o grupo. Aqui, também, `ANY_VALUE()` pode ser usado, se não for importante qual valor de `nome` o MySQL escolher:
...
Para ordenar o resultado, os duplicados devem ser eliminados primeiro. Mas, para isso, devemos manter a primeira linha ou a terceira? Essa escolha arbitrária influencia o valor retido de `c3`, o que, por sua vez, influencia a ordenação e torna-a também arbitrária. Para evitar esse problema, uma consulta que tem `DISTINCT` e `ORDER BY` é rejeitada como inválida se qualquer expressão `ORDER BY` não satisfazer pelo menos uma dessas condições:
...
Outra extensão do MySQL ao SQL padrão permite referências na cláusula `HAVING` para expressões aliadas na lista de seleção. Por exemplo, a seguinte consulta retorna os valores de `name` que ocorrem apenas uma vez na tabela `orders`:
...
O SQL padrão permite apenas expressões de coluna nas cláusulas `GROUP BY`, então uma declaração como esta é inválida porque `FLOOR(value/100)` é uma expressão não de coluna:
...
Na presença de uma expressão não-coluna na cláusula `GROUP BY`, o MySQL reconhece a igualdade entre essa expressão e as expressões na lista de seleção. Isso significa que, com o modo SQL `ONLY_FULL_GROUP_BY` habilitado, a consulta que contém `GROUP BY id, FLOOR(value/100)` é válida porque a mesma expressão `FLOOR()` ocorre na lista de seleção. No entanto, o MySQL não tenta reconhecer a dependência funcional em expressões não-coluna `GROUP BY`, então a consulta seguinte é inválida com `ONLY_FULL_GROUP_BY` habilitado, mesmo que a terceira expressão selecionada seja uma fórmula simples da coluna `id` e a expressão `FLOOR()` na cláusula `GROUP BY`:
