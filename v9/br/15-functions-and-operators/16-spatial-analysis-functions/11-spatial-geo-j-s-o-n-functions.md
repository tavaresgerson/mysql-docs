...
<table summary="Ferramentas de opção para a função ST_AsGeoJSON()."""><col style="width: 10%"/><col style="width: 90%"/><thead><tr> <th>Valor da opção</th> <th>Significado</th> </tr></thead><tbody><tr> <td>0</td> <td>Sem opções. Este é o padrão se <em><code>options</code></em> não for especificado.</td> </tr><tr> <td>1</td> <td>Adicionar uma caixa de delimitação à saída.</td> </tr><tr> <td>2</td> <td>Adicionar uma URN de CRS em formato curto à saída. O formato padrão é um formato curto (<code>EPSG:<em><code>srid</code></em></code>). Se o SRID for 0, a URN de CRS é <code>MySQL:0</code>.</td> </tr><tr> <td>4</td> <td>Adicionar uma URN de CRS em formato longo (<code>urn:ogc:def:crs:EPSG::<em><code>srid</code></em></code>). Esta opção substitui a opção 2. Por exemplo, os valores das opções 5 e 7 significam a mesma coisa (adicionar uma caixa de delimitação e uma URN de CRS em formato longo). Se o SRID for 0, a URN de CRS é <code>urn:ogc:def:crs:MySQL::0</code>.</td> </tr></tbody></table>```
