...
* Se qualquer argumento tiver uma longitude ou latitude fora do intervalo, ocorre um erro:
  ...
  Os intervalos mostrados são em graus. Os limites exatos de intervalo diferem ligeiramente devido à aritmética de ponto flutuante.
...
A string resultante não tem mais de *`max_length`* caracteres, que tem um limite superior de 100. A string pode ser mais curta que *`max_length`* caracteres porque o algoritmo que cria o valor geohash continua até criar uma string que seja uma representação exata da localização ou *`max_length`* caracteres, o que ocorrer primeiro.
...
Retorna a latitude de um valor de string geohash, como um número de precisão dupla no intervalo [−90, 90].

A função de decodificação `ST_LatFromGeoHash()` lê no máximo 433 caracteres do argumento *`geohash_str`*. Isso representa o limite superior de informações na representação interna dos valores de coordenadas. Caracteres após o 433º são ignorados, mesmo que sejam ilegalmente inválidos e produzam um erro.
...
Retorna a longitude de um valor de string geohash, como um número de precisão dupla no intervalo [−180, 180].
...
* `ST_PointFromGeoHash(geohash_str, srid)`
  ...
  Retorna um valor `POINT` contendo o valor decodificado do geohash, dado um valor de string geohash.
