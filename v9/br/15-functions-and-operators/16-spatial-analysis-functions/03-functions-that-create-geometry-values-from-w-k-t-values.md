...
Funções como `ST_GeomFromText()` que aceitam argumentos de coleção de geometria WKT entendem tanto a sintaxe padrão `'GEOMETRYCOLLECTION EMPTY'` do OpenGIS quanto a sintaxe não padrão `'GEOMETRYCOLLECTION()'` do MySQL. Funções como `ST_AsWKT()` que produzem valores WKT produzem a sintaxe padrão `'GEOMETRYCOLLECTION EMPTY'`:
...
* Se qualquer argumento de geometria for `NULL` ou não for uma geometria bem formada sintaticamente, ou se o argumento SRID for `NULL`, o valor de retorno é `NULL`.
...
[`ST_LineFromText(wkt [, srid [, opções]])`](gis-wkt-functions.html#function_st-linefromtext), 

  Construi um valor `LineString` usando sua representação WKT e SRID.

  Essas funções tratam seus argumentos conforme descrito na introdução desta seção.

[`ST_MLineFromText(wkt [, srid [, opções]])`](gis-wkt-functions.html#function_st-mlinefromtext), 
