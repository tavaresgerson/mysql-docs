...
* Seção 20.7.4, “Compressão de Mensagens”
...
* `group_replication_set_communication_protocol()`
  ...
  Atualizar a versão do protocolo de comunicação da Replicação de Grupo de um grupo para que membros em versões anteriores possam se juntar, ou atualizar a versão do protocolo de comunicação da Replicação de Grupo de um grupo após a atualização do servidor MySQL em todos os membros. O privilégio `GROUP_REPLICATION_ADMIN` é necessário para usar esta função, e todos os membros do grupo existentes devem estar online quando você emitir a declaração, sem perda da maioria.
