### 14.18.2 Funções Usadas com Identificadores Globais de Transação (GTIDs)

As funções descritas nesta seção são usadas com replicação baseada em GTIDs. É importante lembrar que todas essas funções aceitam representações de cadeias de caracteres de conjuntos de GTIDs como argumentos. Como tal, os conjuntos de GTIDs devem ser sempre citados quando usados com eles. Veja Conjuntos de GTIDs para mais informações.

A união de dois conjuntos de GTIDs é simplesmente suas representações como cadeias de caracteres, unidas por uma vírgula interposta. Em outras palavras, você pode definir uma função muito simples para obter a união de dois conjuntos de GTIDs, semelhante à criada aqui:
...
* `GTID_SUBSET(set1,set2)`
  ...
  Dados dois conjuntos de identificadores globais de transação *`set1`* e *`set2`*, retorna true se todos os GTIDs em *`set1`* também estiverem em *`set2`*. Retorna `NULL` se *`set1`* ou *`set2`* for `NULL`. Retorna false caso contrário.

  Os conjuntos de GTIDs usados com esta função são representados como cadeias de caracteres, conforme mostrado nos seguintes exemplos:
...
Dado dois conjuntos de identificadores de transação global *`set1`* e *`set2`*, retorna apenas aqueles GTIDs de *`set1`* que não estão em *`set2`*. Retorna `NULL` se *`set1`* ou *`set2`* for `NULL`.

Todos os conjuntos de GTID usados com essa função são representados como strings e devem ser entre aspas, como mostrado nesses exemplos:
...
Os conjuntos de GTID usados com essa função são representados como strings e, portanto, devem ser entre aspas, conforme mostrado no exemplo a seguir:
  ...
  O `gtid_mode` não pode ser alterado para OFF enquanto qualquer cliente estiver usando essa função para esperar que os GTIDs sejam aplicados.