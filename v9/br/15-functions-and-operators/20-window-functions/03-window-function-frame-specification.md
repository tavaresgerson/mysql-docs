### 14.20.3 Especificação do Quadro da Função Janela
...
* Definindo um quadro para ser todas as linhas da partição do início até a linha atual, você pode calcular totais acumulados para cada linha.

* Definindo um quadro para se estender *`N`* linhas em ambos os lados da linha atual, você pode calcular médias móveis.

A seguinte consulta demonstra o uso de quadros móveis para calcular totais acumulados dentro de cada grupo de valores `level` ordenados temporalmente, bem como médias móveis calculadas a partir da linha atual e das linhas que a precedem e a seguem imediatamente:
...
Para a coluna `running_average`, não há linha de quadro que preceida a primeira ou que siga a última. Nesses casos, `AVG()` calcula a média das linhas disponíveis.

As funções agregadas usadas como funções de janela operam em linhas no quadro da linha atual, assim como essas funções de janela não agregadas:
...
O SQL padrão especifica que funções de janela que operam em toda a partição não devem ter uma cláusula de quadro. O MySQL permite uma cláusula de quadro para tais funções, mas ignora-a. Essas funções usam toda a partição, mesmo que um quadro seja especificado:
...
A cláusula de quadro, se fornecida, tem a seguinte sintaxe:
...
Na ausência de uma cláusula de quadro, o quadro padrão depende se uma cláusula `ORDER BY` está presente, conforme descrito mais adiante nesta seção.

O valor *`frame_units`* indica o tipo de relação entre a linha atual e as linhas do quadro:

* `ROWS`: O quadro é definido pelas posições iniciais e finais das linhas. Os desvios são diferenças nos números de linha em relação ao número de linha atual.

* `RANGE`: O quadro é definido pelas linhas dentro de uma faixa de valores. Os desvios são diferenças nos valores das linhas em relação ao valor da linha atual.
...
Com a sintaxe `BETWEEN`, *`frame_start`* não deve ocorrer depois de *`frame_end`*.
...
Como a estrutura padrão difere dependendo da presença ou ausência de `ORDER BY`, adicionar `ORDER BY` a uma consulta para obter resultados determinísticos pode alterar os resultados. (Por exemplo, os valores produzidos por `SUM()` podem mudar.) Para obter os mesmos resultados, mas ordenados por `ORDER BY`, forneça uma especificação explícita da estrutura a ser usada independentemente de `ORDER BY` estar presente.

O significado de uma especificação de estrutura pode não ser óbvio quando o valor da linha atual é `NULL`. Supondo que esse seja o caso, esses exemplos ilustram como várias especificações de estrutura se aplicam:

* `ORDER BY X ASC RANGE BETWEEN 10 FOLLOWING AND 15 FOLLOWING`

  A estrutura começa em `NULL` e termina em `NULL`, incluindo assim apenas as linhas com valor `NULL`.

* `ORDER BY X ASC RANGE BETWEEN 10 FOLLOWING AND UNBOUNDED FOLLOWING`

  A estrutura começa em `NULL` e termina no final da partição. Como uma ordenação `ASC` coloca valores `NULL` primeiro, a estrutura é toda a partição.

* `ORDER BY X DESC RANGE BETWEEN 10 FOLLOWING AND UNBOUNDED FOLLOWING`

  A estrutura começa em `NULL` e termina no final da partição. Como uma ordenação `DESC` coloca valores `NULL` por último, a estrutura é apenas os valores `NULL`.
...
O quadro começa no início da partição e para nas linhas com o valor `NULL`. Como um `ASC` ordena os valores `NULL` primeiro, o quadro é composto apenas pelos valores `NULL`.