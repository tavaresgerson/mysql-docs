...
O padrão SQL impõe uma restrição às funções de janela, proibi-las de serem usadas em instruções `UPDATE` ou `DELETE` para atualizar linhas. O uso dessas funções em uma subconsulta dessas instruções (para selecionar linhas) é permitido.
