...
* As linhas de consulta relacionadas à linha atual sobre as quais ocorre a avaliação da função compõem a janela para a linha atual.
...
As funções de janela são permitidas apenas na lista de seleção e na cláusula `ORDER BY`. As linhas do resultado da consulta são determinadas pela cláusula `FROM`, após o processamento de `WHERE`, `GROUP BY` e `HAVING`, e a execução da janela ocorre antes de `ORDER BY`, `LIMIT` e `SELECT DISTINCT`.
...
* No segundo caso, *`window_name`* é o nome de uma especificação de janela definida por uma cláusula `WINDOW` em outro lugar da consulta. Para detalhes, consulte a Seção 14.20.4, “Janelas Nomeadas”.
...
O SQL padrão exige que a cláusula `PARTITION BY` seja seguida apenas por nomes de colunas. Uma extensão do MySQL permite expressões, não apenas nomes de colunas. Por exemplo, se uma tabela contém uma coluna `TIMESTAMP` chamada `ts`, o SQL padrão permite `PARTITION BY ts`, mas não `PARTITION BY HOUR(ts)`, enquanto o MySQL permite ambos.

* *`order_clause`*: Uma cláusula `ORDER BY` indica como as linhas devem ser ordenadas em cada partição. As linhas que são iguais de acordo com a cláusula `ORDER BY` são consideradas iguais. Se `ORDER BY` for omitido, as linhas da partição não serão ordenadas, sem ordem de processamento implícita, e todas as linhas da partição serão iguais.
  ...
  *`order_clause`* tem esta sintaxe:
  ...
  Cada expressão `ORDER BY` pode ser opcionalmente seguida por `ASC` ou `DESC` para indicar a direção da ordenação. O padrão é `ASC` se nenhuma direção for especificada. Valores `NULL` são ordenados primeiro em ordenações ascendentes, último em ordenações descendentes.
