...
Se você estiver usando o **gdb** no Linux, você deve instalar um arquivo `.gdb`, com as seguintes informações, no seu diretório atual:
...
Inclua a saída anterior em um relatório de bug, que você pode enviar seguindo as instruções na Seção 1.6, “Como relatar bugs ou problemas”.
...
Se você estiver usando a interface Perl `DBI`, você pode ativar as informações de depuração usando o método `trace` ou configurando a variável de ambiente `DBI_TRACE`.