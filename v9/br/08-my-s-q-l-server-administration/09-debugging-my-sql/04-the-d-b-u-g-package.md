...
<table frame="all" summary="Descrição das flags permitidas para as opções de depuração <code>debug_options</code>."><col style="width: 8%"/><col style="width: 92%"/><thead><tr> <th><p> Flag </p></th> <th><p> Descrição </p></th> </tr></thead><tbody><tr> <td><p> <code class="literal">d</code> </p></td> <td><p> Habilitar a saída das macros <code>DBUG_<em class="replaceable"><code>XXX</code></em></code> para o estado atual. Pode ser seguido por uma lista de palavras-chave, que habilita a saída apenas para as macros DBUG com essa palavra-chave. Uma lista vazia de palavras-chave habilita a saída para todas as macros. </p><p> No MySQL, as palavras-chave comuns de macros de depuração para habilitar são <code class="literal">enter</code>, <code class="literal">exit</code>, <code class="literal">error</code>, <code class="literal">warning</code>, <code class="literal">info</code> e <code class="literal">loop</code>. </p></td> </tr><tr> <td><p> <code class="literal">D</code> </p></td> <td><p> Aguardar após cada linha de saída do depurador. O argumento é o atraso, em décimos de segundo, sujeito às capacidades da máquina. Por exemplo, <code class="literal">D,20</code> especifica um atraso de dois segundos. </p></td> </tr><tr> <td><p> <code class="literal">f</code> </p></td> <td><p> Limitar a depuração, rastreamento e perfilagem para a lista de funções nomeadas. Uma lista vazia habilita todas as funções. As flags <code class="literal">d</code> ou <code class="literal">t</code> ainda devem ser dadas; essa flag limita apenas suas ações se estiverem habilitadas. </p></td> </tr><tr> <td><p> <code class="literal">F</code> </p></td> <td><p> Identificar o nome do arquivo fonte para cada linha de saída de depuração ou rastreamento. </p></td> </tr><tr> <td><p> <code class="literal">i</code> </p></td> <td><p> Identificar o processo com o PID ou ID de thread para cada linha de saída de depuração ou rastreamento. </p></td> </tr><tr> <td><p> <code class="literal">L</code> </p></td> <td><p> Identificar o número da linha do arquivo fonte para cada linha de saída de depuração ou rastreamento. </p></td> </tr><tr> <td><p> <code class="literal">n</code> </p></td> <td><p> Imprimir a profundidade de aninhamento atual da função para cada linha de saída de depuração ou rastreamento. </p></td> </tr><tr> <td><p> <code class="literal">N</code> </p></td> <td><p> Numerar cada linha de saída de depuração. </p></td> </tr><tr> <td><p> <code class="literal">o</code> </p></td> <td><p> Rediritar a saída do depurador para o arquivo especificado. A saída padrão é <code class="literal">stderr</code>. </p></td> </tr><tr> <td><p> <code class="literal">O</code> </p></td> <td><p> Como <code class="literal">o</code>, mas o arquivo é realmente descarregado entre cada escrita. Quando necessário, o arquivo é fechado e reaberto entre cada escrita. </p></td> </tr><tr> <td><p> <code class="literal">a</code> </p></td> <td><p> Como <code class="literal">o</code>, mas abre para adição. </p></td> </tr><tr> <td><p> <code class="literal">A</code> </p></td> <td><p> Como <code class="literal">O</code>, mas abre para adição. </p></td> </tr><tr> <td><p> <code class="literal">p</code> </p></td> <td><p> Limitar as ações do depurador a processos especificados. Um processo deve ser identificado com a macro <code class="literal">DBUG_PROCESS</code> e deve corresponder a um no list para que as ações de depuração ou rastreamento ocorram. </p></td> </tr><tr> <td><p> <code class="literal">P</code> </p></td> <td><p> Imprimir o nome atual do processo para cada linha de saída de depuração ou rastreamento. </p></td> </tr><tr> <td><p>
