...
Neste contexto, “console” significa `stderr`, a saída de erro padrão. Isso é seu terminal ou janela de console, a menos que a saída de erro padrão tenha sido redirecionada para um destino diferente.
...
* Se `--console` for fornecido, o destino padrão é a console. (`--console` tem precedência sobre `--log-error` se ambos forem fornecidos, e os seguintes itens sobre `--log-error` não se aplicam.)

* Se `--log-error` não for fornecido ou for fornecido sem nomear um arquivo, o destino padrão é um arquivo chamado `host_name.err` no diretório de dados, a menos que a opção `--pid-file` seja especificada. Nesse caso, o nome do arquivo é o nome do arquivo de PID com um sufixo de `.err` no diretório de dados.
...
Em sistemas Unix e Unix-like, o **mysqld** usa a opção `--log-error` para determinar se o destino padrão do log de erro é a console ou um arquivo, e, se for um arquivo, o nome do arquivo:

* Se `--log-error` não for fornecido, o destino padrão é a console.
...
##### Como o Destino Padrão do Log de Erro Afeta os Canais de Log

Após o servidor interpretar as opções de configuração do destino do log de erro, ele define a variável de sistema `log_error` para indicar o destino padrão do log de erro. Os componentes dos canais de log podem basear seu próprio destino de saída no valor de `log_error`, ou determinar seu destino de forma independente de `log_error`.

Se `log_error` for `stderr`, o destino padrão do log de erro é a console, e os canais de log que baseiam seu destino de saída no destino padrão também escrevem na console:
...
Se `log_error` não for `stderr`, o destino padrão do log de erro é um arquivo e `log_error` indica o nome do arquivo. Os canais de log que baseiam seu destino de saída no destino padrão baseiam o nome de arquivo de saída no nome desse arquivo. (Um canal pode usar exatamente esse nome, ou pode usar alguma variação dele.) Suponha que o valor de `log_error` seja `file_name`. Então, os canais de log usam o nome assim:

* `log_sink_internal`, `log_sink_test`: Esses sinks escrevem em *`file_name`*.

* `log_sink_json`: Instâncias consecutivas desse sink nomeadas no valor `log_error_services` escrevem em arquivos nomeados *`file_name`* mais um sufixo numerado `.NN.json`: `file_name.00.json`, `file_name.01.json`, e assim por diante.
