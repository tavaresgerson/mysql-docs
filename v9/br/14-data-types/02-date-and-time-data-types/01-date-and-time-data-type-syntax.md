...
* `DATE`
  ...
  Uma data. O intervalo compatível é `'1000-01-01'` a `'9999-12-31'`. O MySQL exibe valores de `DATE` no formato `'YYYY-MM-DD'`, mas permite a atribuição de valores a colunas `DATE` usando strings ou números.

* `DATETIME[(fsp)]`

  Uma combinação de data e hora. O intervalo compatível é `'1000-01-01 00:00:00.000000'` a `'9999-12-31 23:59:59.499999'`. O MySQL exibe valores de `DATETIME` no formato `'YYYY-MM-DD hh:mm:ss[.fraction]'`, mas permite a atribuição de valores a colunas `DATETIME` usando strings ou números.
...
A menos que especificado de outra forma, a primeira coluna `TIMESTAMP` em uma tabela é definida para ser automaticamente configurada com a data e hora da última modificação, se não tiver sido explicitamente atribuído um valor. Isso torna o `TIMESTAMP` útil para registrar o timestamp de uma operação de `INSERT` ou `UPDATE`. Você também pode configurar qualquer coluna `TIMESTAMP` para a data e hora atuais, atribuindo-lhe um valor `NULL`, a menos que tenha sido definido com o atributo `NULL` para permitir valores `NULL`.
...
* `TIME[(fsp)]`
  ...
  Um horário. O intervalo é `'-838:59:59.000000'` a `'838:59:59.000000'`. O MySQL exibe valores `TIME` no formato `'hh:mm:ss[.fraction]'`, mas permite a atribuição de valores a colunas `TIME` usando strings ou números.

  Um valor opcional *`fsp`* no intervalo de 0 a 6 pode ser dado para especificar a precisão de frações de segundo. Um valor de 0 significa que não há parte fracionária. Se omitido, a precisão padrão é 0.
...
* `YEAR[(4)]`
  ...
  Para informações adicionais sobre o formato de exibição e a interpretação dos valores de entrada do tipo `YEAR`, consulte a Seção 13.2.4, “O Tipo YEAR”.
...
As funções agregadas `SUM()` e `AVG()` não funcionam com valores temporais. (Eles convertem os valores em números, perdendo tudo após o primeiro caractere não numérico.) Para contornar esse problema, converta para unidades numéricas, realize a operação agregada e converta de volta para um valor temporal. Exemplos:
