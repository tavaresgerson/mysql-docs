...
* Quando uma coluna de ponto flutuante ou ponto fixo recebe um valor que excede a faixa implícita pela precisão e escala especificadas (ou padrão), o MySQL armazena o valor que representa o ponto final correspondente dessa faixa.
...
Com o modo SQL rigoroso habilitado, ocorre um erro de saída de faixa:
...
Para que a operação seja bem-sucedida neste caso, converta o valor para não-negativo;
...
Se ocorrer um estouro, isso depende da faixa dos operandos, então outra maneira de lidar com a expressão anterior é usar aritmética de valor exato porque os valores `DECIMAL` - `DECIMAL`, `NUMERIC") têm uma faixa maior do que os inteiros:
...
A subtração entre valores inteiros, onde um é do tipo `UNSIGNED`, produz um resultado não-negativo por padrão. Se o resultado de outra forma fosse negativo, ocorre um erro:
