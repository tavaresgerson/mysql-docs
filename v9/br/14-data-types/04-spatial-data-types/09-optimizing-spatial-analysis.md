...
`MyISAM` e `InnoDB` suportam tanto índices `SPATIAL` quanto não `SPATIAL`. Outros motores de armazenamento suportam índices não `SPATIAL`, conforme descrito na Seção 15.1.18, “Instrução CREATE INDEX”.