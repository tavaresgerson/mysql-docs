...
As tabelas `InnoDB` permitem valores `SRID` para SRS cartesianas e geográficas. As tabelas `MyISAM` permitem valores `SRID` para SRS cartesianas.
...
Para outros exemplos que mostram como usar tipos de dados espaciais no MySQL, veja a Seção 13.4.6, “Criação de Colunas Espaciais”. Para informações sobre sistemas de referência espaciais, veja a Seção 13.4.5, “Suporte a Sistemas de Referência Espacial”.