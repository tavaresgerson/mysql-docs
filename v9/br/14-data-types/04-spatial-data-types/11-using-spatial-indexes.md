...
O otimizador investiga se índices espaciais disponíveis podem ser usados na busca de consultas que utilizam uma função como `MBRContains()` ou `MBRWithin()` na cláusula `WHERE`. A consulta a seguir encontra todos os objetos que estão no retângulo dado:
