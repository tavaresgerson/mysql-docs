...
* Para um caractere BMP, `utf16` e `ucs2` têm características de armazenamento idênticas: mesmos valores de código, mesma codificação, mesma extensão.
...
Como `utf16` suporta surrogados e `ucs2` não, há uma verificação de validade que se aplica apenas em `utf16`: Você não pode inserir um surrogato superior sem um inferior, ou vice-versa. Por exemplo:
...
Como o MySQL deve permitir o pior caso (onde um único caractere requer quatro bytes), o comprimento máximo de uma coluna ou índice `utf16` é apenas metade do comprimento máximo para uma coluna ou índice `ucs2`. Por exemplo, o comprimento máximo de uma chave de índice de uma tabela `MEMORY` é de 3072 bytes, então essas instruções criam tabelas com os índices mais longos permitidos para colunas `ucs2` e `utf16`:
