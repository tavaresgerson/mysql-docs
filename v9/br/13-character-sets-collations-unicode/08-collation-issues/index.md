## 12.8 Problemas de Colagem

12.8.1 Uso do COLLATE em Instruções SQL

12.8.2 Precedência da Cláusula COLLATE

12.8.3 Compatibilidade entre Conjuntos de Caracteres e Colagem

12.8.4 Coercibilidade da Colagem em Expressões

12.8.5 A Colagem Binária Comparada às Colagens _bin

12.8.6 Exemplos do Efeito da Colagem

12.8.7 Uso da Colagem em Pesquisas no INFORMATION_SCHEMA

As seções a seguir discutem vários aspectos das colagens de conjuntos de caracteres.