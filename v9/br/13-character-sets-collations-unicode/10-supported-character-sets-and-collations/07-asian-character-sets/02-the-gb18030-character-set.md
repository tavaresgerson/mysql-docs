...
No MySQL, o conjunto de caracteres `gb18030` corresponde ao *Conjunto de Caracteres Codificado Chinês GB 18030-2005*, que é o conjunto de caracteres oficial da República Popular da China (RPC).
...
* Suporta a conversão para cima e para baixo para todos os pontos de código GB18030. A dobra de maiúsculas definida pelo Unicode também é suportada (com base no `CaseFolding-6.3.0.txt`).
...
* As buscas por ligaduras também correspondem às ligaduras maiúsculas ao usar a collation `gb18030_unicode_520_ci`.
...
* O comprimento mínimo de multibytes é 1 e o máximo é 4. O conjunto de caracteres determina o comprimento de uma sequência usando os primeiros 1 ou 2 bytes.

##### Cotações suportadas

* `gb18030_bin`: Uma cotação binária.
* `gb18030_chinese_ci`: A cotação padrão, que suporta o Pinyin. A ordenação de caracteres não chineses é baseada na ordem da chave de ordenação original. A chave de ordenação original é `GB(UPPER(ch))` se `UPPER(ch)` existir. Caso contrário, a chave de ordenação original é `GB(ch)`. Os caracteres chineses são ordenados de acordo com a cotação Pinyin definida no Repositório de Dados de Localidade Comum Unicode (CLDR 24). Os caracteres não chineses são ordenados antes dos caracteres chineses, com exceção de `GB+FE39FE39`, que é o ponto máximo de código.

* `gb18030_unicode_520_ci`: Uma cotação Unicode. Use esta cotação se precisar garantir que as ligaduras sejam ordenadas corretamente.