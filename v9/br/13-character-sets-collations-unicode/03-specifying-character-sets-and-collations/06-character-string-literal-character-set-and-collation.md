...
* Se tanto *`_charset_name`* quanto `COLLATE collation_name` forem especificados, o conjunto de caracteres *`charset_name`* e a cotação *`collation_name`* são usados. *`collation_name`* deve ser uma cotação permitida para *`charset_name`*.

* Se *`_charset_name`* for especificado, mas `COLLATE` não for especificado, o conjunto de caracteres *`charset_name`* e sua cotação padrão são usados. Para ver a cotação padrão para cada conjunto de caracteres, use a declaração `SHOW CHARACTER SET` ou consulte a tabela `INFORMATION_SCHEMA` `CHARACTER_SETS`.

* Se *`_charset_name`* não for especificado, mas `COLLATE collation_name` for especificado, o conjunto de caracteres padrão da conexão, dado pela variável de sistema `character_set_connection`, e a collation *`collation_name`* são usados. *`collation_name`* deve ser uma collation permitida para o conjunto de caracteres padrão da conexão.

* Caso contrário (nem *`_charset_name`* nem `COLLATE collation_name` são especificados), o conjunto de caracteres padrão da conexão e a collation, dadas pelas variáveis de sistema `character_set_connection` e `collation_connection`, são usadas.
...
Um introduzir indica o conjunto de caracteres para a string seguinte, mas não altera a forma como o analisador processa escapamentos dentro da string. Os escapamentos são sempre interpretados pelo analisador de acordo com o conjunto de caracteres dado por `character_set_connection`.
