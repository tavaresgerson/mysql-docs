...
Suponha que a tabela `t` tenha uma coluna não binária chamada `col1`, definida como `CHAR(50) CHARACTER SET latin1`, mas você deseja convertê-la para usar `utf8mb4` para que possa armazenar valores de muitos idiomas. A seguinte instrução realiza isso:
...
Esse procedimento exige que a tabela não tenha sido modificada já com instruções como `INSERT` ou `UPDATE` após uma atualização para o MySQL 4.1 ou superior. Nesse caso, o MySQL armazenaria novos valores na coluna usando `latin1`, e a coluna contaria com uma mistura de valores `sjis` e `latin1` e não pode ser convertida corretamente.
...
Para converter todas as colunas de caracteres em uma tabela, a instrução `ALTER TABLE ... CONVERT TO CHARACTER SET charset` pode ser útil. Veja a Seção 15.1.11, “Instrução ALTER TABLE”.

Observação

As instruções `ALTER TABLE` que fazem alterações nos conjuntos de caracteres ou colorações de tabela ou coluna devem ser executadas usando `ALGORITHM=COPY`. Para mais informações, consulte a Seção 17.12.1, “Operações DDL Online”.