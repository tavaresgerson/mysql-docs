...
* `UNICODE`: A expressão pode conter caracteres no intervalo Unicode `U+0000` a `U+10FFFF`. Isso inclui caracteres na faixa da Plana Multilíngue Básica (BMP) (`U+0000` a `U+FFFF`) e caracteres suplementares fora da faixa da BMP (`U+10000` a `U+10FFFF`).
...
* Uma coluna com o conjunto de caracteres `ascii` tem repertório `ASCII` devido ao seu conjunto de caracteres. Na tabela a seguir, `c1` tem repertório `ASCII`:
  ...
  O seguinte exemplo ilustra como o repertório permite que um resultado seja determinado em um caso em que ocorre um erro sem repertório:
...
* Funções com um argumento de string herdam o repertório de seu argumento. O resultado de `UPPER(_utf8mb4'abc')` tem repertório `ASCII` porque seu argumento tem repertório `ASCII`. (Apesar de `_utf8mb4` introduzir, a string `'abc'` não contém nenhum caractere fora do intervalo ASCII.)
