...
4. Se você deseja uma correspondência semelhante para outros conjuntos de caracteres Unicode, adicione outros elementos `<collation>`. Por exemplo, para definir `ucs2_phone_ci`, adicione um elemento `<collation>` ao elemento `<charset nome="ucs2">`. Lembre-se de que cada correspondência deve ter seu próprio ID único.

5. Reinicie o servidor e use esta declaração para verificar se a correspondência está presente:
...
Agora, teste a correspondência para garantir que ela tenha as propriedades desejadas.

Crie uma tabela contendo alguns números de telefone de amostra usando a nova correspondência:
