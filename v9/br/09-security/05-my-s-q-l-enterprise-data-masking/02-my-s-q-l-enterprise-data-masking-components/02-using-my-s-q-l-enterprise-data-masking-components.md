...
**Mascaramento do Número da Conta Principal do Cartão de Pagamento.** As funções de mascaramento fornecem mascaramento estrito e relaxado dos números da conta principal.
...
**Mascaramento do Número de Segurança Social dos EUA.** `mask_ssn()` mascara todos, exceto os últimos quatro dígitos do número:
...
**Mascaramento do Número de Seguro Nacional do Reino Unido.** `mask_uk_nin()` mascara todos, exceto os primeiros dois dígitos do número:
...
Várias funções de componente geram valores aleatórios. Esses valores podem ser usados para testes, simulação e assim por diante.
...
Como não pode ser garantido que o número gerado não tenha sido atribuído, o resultado de `gen_rnd_canada_sin()` nunca deve ser exibido (exceto possivelmente em testes). Para exibição em aplicativos voltados para o usuário, sempre empregue uma função de mascaramento, como `mask_canada_sin()`, conforme mostrado aqui:
...
`gen_rnd_email()` retorna um endereço de e-mail aleatório com um número especificado de dígitos para as partes do nome e sobrenome no domínio especificado, `mynet.com` no exemplo a seguir:
...
`gen_rnd_iban()` retorna um número escolhido de um intervalo não utilizado para números legítimos:
...
`gen_rnd_pan()` retorna um Número Principal de Conta (PAN) de cartão de pagamento aleatório.

Como não pode ser garantido que o número gerado não tenha sido atribuído a uma conta de pagamento legítima, o resultado de `gen_rnd_pan()` nunca deve ser exibido, exceto para fins de teste. Para exibição em aplicativos, sempre empregue uma função de mascaramento, como `mask_pan()` ou `mask_pan_relaxed()`. Mostramos o uso dessa última função com `gen_rnd_pan()` aqui:
...
`gen_rnd_ssn()` retorna um Número de Segurança Social dos EUA aleatório, cuja primeira parte é escolhida de um intervalo não utilizado para números legítimos:
...
`gen_rnd_uk_nin()` retorna um Número de Seguro Nacional (NIN) do Reino Unido.

Como não pode ser garantido que o número gerado não tenha sido atribuído, o resultado de `gen_rnd_uk_nin()` nunca deve ser exibido (exceto possivelmente em testes). Para exibição em aplicativos voltados para o usuário, sempre empregue uma função de mascaramento, como `mask_uk_nin()`, conforme mostrado aqui:
...
`gen_rnd_us_phone()` retorna um número de telefone aleatório dos EUA com o código de área 555 e que não seja usado para números legítimos:
...
`gen_rnd_uuid()` retorna um número escolhido de uma faixa que não seja usada para identificadores legítimos:
...
O MySQL Enterprise Data Masking permite que dicionários sejam usados como fontes de valores aleatórios chamados *termos*. Para usar um dicionário, ele deve primeiro ser adicionado à tabela `masking_dictionaries` do sistema e receber um nome. Os dicionários são lidos da tabela e carregados para o cache durante a inicialização dos componentes (no início do servidor). Os termos podem então ser adicionados, removidos e selecionados dos dicionários e usados como valores aleatórios ou como substituições para outros valores.
...
Além disso, suponha que um dicionário chamado `US_Cities` contenha esses nomes de cidades nos Estados Unidos:
...
A função `gen_blocklist()` permite que um termo de um dicionário seja substituído por um termo de outro dicionário, o que resulta em mascaramento por substituição. Seus argumentos são o termo a ser substituído, o dicionário em que o termo aparece e o dicionário a partir do qual será escolhido um substituto. Por exemplo, para substituir uma cidade dos EUA por uma cidade alemã, ou vice-versa, use `gen_blocklist()` da seguinte forma:
...
Agora, o representante só vê o que é necessário, e a privacidade do cliente é preservada.
