...
Esta seção descreve como usar o comando **openssl** para configurar os arquivos de chave RSA que permitem que o MySQL suporte a troca segura de senhas em conexões não criptografadas para contas autenticadas pelos plugins `sha256_password` (desatualizado) e `caching_sha2_password`.
...
Para criar os arquivos de chave privada e pública RSA, execute esses comandos enquanto estiver logado na conta do sistema usada para executar o servidor MySQL, para que os arquivos pertençam a essa conta:
