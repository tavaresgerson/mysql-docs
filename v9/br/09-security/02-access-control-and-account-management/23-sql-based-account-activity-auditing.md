...
Nesses contextos, o controle de privilégios é feito contra a conta `DEFINER` e `CURRENT_USER()` refere-se àquela conta, não à conta do cliente que invocou a rotina ou visual armazenada ou que causou o disparo a ser ativado. Para determinar o usuário que está invocando, você pode chamar a função `USER()`, que retorna um valor indicando o nome real do usuário fornecido pelo cliente e o host a partir do qual o cliente se conectou. No entanto, esse valor não corresponde necessariamente diretamente a uma conta na tabela `user`, porque o valor de `USER()` nunca contém caracteres de mascaramento, enquanto os valores de conta (como retornados por `CURRENT_USER()`) podem conter caracteres de mascaramento de nome de usuário e host.
...
A parte do nome do host de uma conta também pode conter caracteres de mascaramento. Se o nome do host contiver um caractere de padrão `'%'` ou `'_'` ou usar a notação de máscara de rede, a conta pode ser usada para clientes se conectando a partir de vários hosts e o valor de `CURRENT_USER()` não indica qual deles. Por exemplo, a conta `'user2'@'%.example.com'` pode ser usada por `user2` para se conectar a partir de qualquer host no domínio `example.com`. Se `user2` se conecta a partir de `remote.example.com`, `USER()` e `CURRENT_USER()` retornam valores diferentes:
...
Se um aplicativo precisar invocar `USER()` para auditoria de usuários (por exemplo, se estiver realizando auditoria a partir de triggers), mas também precisar associar o valor `USER()` a uma conta na tabela `user`, é necessário evitar contas que contenham caracteres especiais na coluna `User` ou `Host`. Especificamente, não é permitido que `User` esteja vazio (o que cria uma conta de usuário anônimo) e não é permitido que haja caracteres de padrão ou notação de máscara de rede nos valores de `Host`. Todas as contas devem ter um valor de `User` não vazio e um valor literal de `Host`.

Em relação aos exemplos anteriores, as contas `''@'localhost'` e `'user2'@'%.example.com'` devem ser alteradas para não usar caracteres especiais:
...
Se `user2` precisar se conectar a vários hosts no domínio `example.com`, deve haver uma conta separada para cada host.
