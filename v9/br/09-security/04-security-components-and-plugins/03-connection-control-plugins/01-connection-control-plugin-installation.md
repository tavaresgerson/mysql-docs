...
O nome base do arquivo da biblioteca do plugin é `connection_control`. O sufixo do nome do arquivo difere conforme a plataforma (por exemplo, `.so` para sistemas Unix e similares, `.dll` para Windows).
...
Para verificar a instalação do plugin, examine a tabela do Schema de Informações `PLUGINS` ou use a instrução `SHOW PLUGINS` (consulte a Seção 7.6.2, “Obtendo Informações de Plugins do Servidor”). Por exemplo:
...
Se quiser impedir que o servidor seja executado sem um plugin de controle de conexão específico, use um valor de opção de `FORCE` ou `FORCE_PLUS_PERMANENT` para forçar o falhanço do início do servidor se o plugin não se inicializar com sucesso.
...
* O atraso é zero até `connection_control_failed_connections_threshold` tentativas consecutivas de conexão falha.

* Posteriormente, o servidor adiciona um atraso crescente para tentativas consecutivas subsequentes, até que uma conexão bem-sucedida ocorra. Os atrasos não ajustados começam em 1000 milissegundos (1 segundo) e aumentam em 1000 milissegundos por tentativa. Ou seja, uma vez que o atraso é ativado para uma conta, os atrasos não ajustados para tentativas falhas subsequentes são de 1000 milissegundos, 2000 milissegundos, 3000 milissegundos e assim por diante.
...
* Uma vez que o atraso é ativado para uma conta, a primeira conexão bem-sucedida subsequente também experimenta um atraso, mas a contagem de falhas é redefinida para conexões subsequentes.
...
* Se `connection_control_min_connection_delay` e `connection_control_max_connection_delay` forem 1000 e 20000, os atrasos ajustados são os mesmos dos atrasos não ajustados, até um máximo de 20000 milissegundos. As quatro e subsequentes tentativas de conexão falhadas são atrasadas por 1000 milissegundos, 2000 milissegundos, 3000 milissegundos e assim por diante.

* Se `connection_control_min_connection_delay` e `connection_control_max_connection_delay` forem 1500 e 20000, os atrasos ajustados para as quatro e subsequentes tentativas de conexão falhadas são 1500 milissegundos, 2000 milissegundos, 3000 milissegundos e assim por diante, até um máximo de 20000 milissegundos.

* Se `connection_control_min_connection_delay` e `connection_control_max_connection_delay` forem 2000 e 3000, os atrasos ajustados para as quatro e subsequentes tentativas de conexão falhadas são 2000 milissegundos, 2000 milissegundos e 3000 milissegundos, com todas as subsequentes tentativas de conexão falhadas também atrasadas por 3000 milissegundos.
...
As variáveis de sistema `connection_control_min_connection_delay` e `connection_control_max_connection_delay` têm valores mínimos e máximos de 1000 e 2.147.483.647. Além disso, a faixa permitida de valores de cada variável também depende do valor atual da outra:
...
Para os casos descritos acima, a tentativa de conexão corresponde a alguma entrada de `mysql.user`, e se a solicitação é bem-sucedida ou não depende se o cliente fornece as credenciais de autenticação corretas. Por exemplo, se o cliente apresentar uma senha incorreta, a tentativa de conexão falha.
...
O servidor mantém informações sobre quais hosts do cliente podem se conectar ao servidor (essencialmente a união dos valores de host para entradas `mysql.user`). Se um cliente tentar se conectar a partir de qualquer outro host, o servidor rejeita a tentativa em uma fase inicial da configuração da conexão:
...
* A tabela `INFORMATION_SCHEMA` `CONNECTION_CONTROL_FAILED_LOGIN_ATTEMPTS` fornece informações sobre o número atual de tentativas de conexão falhas consecutivas por conta (combinação de usuário/host). Isso contabiliza todas as tentativas falhas, independentemente de elas terem sido adiadas ou não.
