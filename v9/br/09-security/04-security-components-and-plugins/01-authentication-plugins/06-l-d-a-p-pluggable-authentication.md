#### 8.4.1.6 Autenticação Pluggable LDAP

Nota

A autenticação pluggable LDAP é uma extensão incluída na Edição Empresarial do MySQL, um produto comercial. Para obter mais informações sobre produtos comerciais, consulte <https://www.mysql.com/products/>.
...
**Tabela 8.19 Nomes de plugins e bibliotecas para autenticação LDAP simples**

<table summary="Nomes dos plugins e arquivos de biblioteca usados para autenticação de senha LDAP simples."><col style="width: 30%"/><col style="width: 70%"/><thead><tr> <th>Plugin ou Arquivo</th> <th>Nome do Plugin ou Arquivo</th> </tr></thead><tbody><tr> <td>Nome do plugin do lado do servidor</td> <td><code class="literal">authentication_ldap_simple</code></td> </tr><tr> <td>Nome do plugin do lado do cliente</td> <td><code class="literal">mysql_clear_password</code></td> </tr><tr> <td>Nome do arquivo de biblioteca</td> <td><code class="filename">authentication_ldap_simple.so</code></td> </tr></tbody></table>

**Tabela 8.20 Nomes de plugins e bibliotecas para autenticação LDAP baseada em SASL**

<table summary="Nomes dos plugins e arquivos de biblioteca usados para autenticação de senha LDAP baseada em SASL."><col style="width: 30%"/><col style="width: 70%"/><thead><tr> <th>Plugin ou Arquivo</th> <th>Nome do Plugin ou Arquivo</th> </tr></thead><tbody><tr> <td>Nome do plugin do lado do servidor</td> <td><code class="literal">authentication_ldap_sasl</code></td> </tr><tr> <td>Nome do plugin do lado do cliente</td> <td><code class="literal">authentication_ldap_sasl_client</code></td> </tr><tr> <td>Nomes dos arquivos de biblioteca</td> <td><code class="filename">authentication_ldap_sasl.so</code>, <code class="filename">authentication_ldap_sasl_client.so</code></td> </tr></tbody></table>

Os arquivos de biblioteca incluem apenas os plugins de autenticação `authentication_ldap_XXX`. O plugin `mysql_clear_password` do lado do cliente é incorporado à biblioteca de cliente `libmysqlclient`.

Cada plugin LDAP do lado do servidor funciona com um plugin do lado do cliente específico:

* O plugin de autenticação `authentication_ldap_simple` do lado do servidor realiza a autenticação LDAP simples. Para conexões por contas que utilizam este plugin, os programas cliente utilizam o plugin `mysql_clear_password` do lado do cliente, que envia a senha ao servidor como texto claro. Não é utilizada hashing ou criptografia de senhas, portanto, é recomendada uma conexão segura entre o cliente MySQL e o servidor para evitar a exposição da senha.

* O plugin de autenticação `authentication_ldap_sasl` do lado do servidor realiza a autenticação LDAP baseada em SASL. Para conexões por contas que utilizam este plugin, os programas cliente utilizam o plugin `authentication_ldap_sasl_client` do lado do cliente. Os plugins LDAP SASL do lado do cliente e do servidor utilizam mensagens SASL para a transmissão segura de credenciais dentro do protocolo LDAP, para evitar o envio do texto claro da senha entre o cliente MySQL e o servidor.

Nas plataformas Microsoft Windows, os plugins do lado do servidor e do lado do cliente são suportados para autenticação LDAP baseada em SASL.

Os plugins de autenticação LDAP do lado do servidor são incluídos apenas na Edição Empresarial do MySQL. Eles não estão incluídos nas distribuições comunitárias do MySQL. O plugin LDAP SASL do lado do cliente está incluído em todas as distribuições, incluindo as distribuições comunitárias, e, como mencionado anteriormente, o plugin `mysql_clear_password` do lado do cliente está embutido na biblioteca de clientes `libmysqlclient`, que também está incluída em todas as distribuições. Isso permite que clientes de qualquer distribuição se conectem a um servidor que tenha o plugin do lado do servidor apropriado carregado.

As seções a seguir fornecem informações de instalação e uso específicas para a autenticação de autenticação plugável LDAP:

* Pré-requisitos para a autenticação pluggable LDAP
...
* Definindo tempos de espera para a autenticação pluggable LDAP
...
* Preferência de grupo de autenticação LDAP e especificação de mapeamento
...
Para informações gerais sobre a autenticação pluggable no MySQL, consulte a Seção 8.2.17, “Autenticação pluggable”. Para informações sobre o plugin `mysql_clear_password`, consulte a Seção 8.4.1.3, “Autenticação pluggable de texto claro no lado do cliente”. Para informações sobre usuários proxy, consulte a Seção 8.2.19, “Usuários proxy”.

Nota

Se o seu sistema suportar o PAM e permitir o LDAP como método de autenticação PAM, outra maneira de usar o LDAP para a autenticação de usuários do MySQL é usar o plugin `authentication_pam` no lado do servidor. Consulte a Seção 8.4.1.4, “Autenticação pluggable PAM”.

##### Pré-requisitos para a autenticação pluggable LDAP

Para usar a autenticação pluggable LDAP para o MySQL, esses pré-requisitos devem ser atendidos:
...
Esta seção fornece uma visão geral de como o MySQL e o LDAP trabalham juntos para autenticar usuários do MySQL. Para exemplos que mostram como configurar contas do MySQL para usar plugins de autenticação LDAP específicos, consulte Usar Autenticação Legível (LDAP) Pluggable. Para informações sobre os métodos de autenticação disponíveis para os plugins LDAP, consulte Métodos de Autenticação LDAP.

O cliente se conecta ao servidor MySQL, fornecendo o nome de usuário e uma senha do usuário do cliente do MySQL:

* Para autenticação LDAP simples, os plugins do lado do cliente e do lado do servidor comunicam a senha em texto claro. Uma conexão segura entre o cliente e o servidor do MySQL é recomendada para evitar a exposição da senha.

* Para autenticação LDAP baseada em SASL, os plugins do lado do cliente e do servidor evitam enviar a senha em texto claro entre o cliente e o servidor MySQL. Por exemplo, os plugins podem usar mensagens SASL para transmissão segura de credenciais dentro do protocolo LDAP. Para o método de autenticação GSSAPI, os plugins do lado do cliente e do servidor comunicam-se de forma segura usando Kerberos sem usar diretamente mensagens LDAP.
...
* Se a conta MySQL nomear um nome de DN (Distinguished Name) de usuário LDAP, a autenticação LDAP usa esse valor e a senha LDAP fornecida pelo cliente. (Para associar um DN de usuário LDAP a uma conta MySQL, inclua uma cláusula `BY` que especifique uma string de autenticação na declaração `CREATE USER` que cria a conta.)
...
* Se a entrada LDAP não tiver nenhum atributo de grupo, o plugin de autenticação retorna o nome do usuário cliente como o nome do usuário autenticado.
...
Para que o plugin seja utilizável pelo servidor, os arquivos da biblioteca de plugins devem estar localizados no diretório do plugin MySQL (o diretório nomeado pela variável de sistema `plugin_dir`). Se necessário, configure a localização do diretório do plugin definindo o valor de `plugin_dir` na inicialização do servidor.

Os nomes de base dos arquivos da biblioteca de plugins do lado do servidor são `authentication_ldap_simple` e `authentication_ldap_sasl`. O sufixo do nome do arquivo difere de acordo com a plataforma (por exemplo, `.so` para sistemas Unix e Unix-like, `.dll` para Windows).
...
Para carregar os plugins na inicialização do servidor, use as opções `--plugin-load-add` para nomear os arquivos de biblioteca que os contêm. Com esse método de carregamento de plugins, as opções devem ser fornecidas toda vez que o servidor for iniciado. Além disso, especifique valores para quaisquer variáveis de sistema fornecidas pelo plugin que você deseja configurar.

Cada plugin LDAP do lado do servidor expõe um conjunto de variáveis de sistema que permitem que sua operação seja configurada. A maioria dessas variáveis é opcional, mas você deve definir as variáveis que especificam o host do servidor LDAP (para que o plugin saiba onde se conectar) e o nome distinto da base para operações de vinculação LDAP (para limitar o escopo das pesquisas e obter pesquisas mais rápidas). Para obter detalhes sobre todas as variáveis de sistema LDAP, consulte a Seção 8.4.1.13, “Variáveis do Sistema do Sistema de Autenticação Conectable”.
...
Após instalar os plugins em tempo de execução, as variáveis de sistema que eles exibem ficam disponíveis e você pode adicionar configurações para eles ao seu arquivo `my.cnf` para configurar os plugins para reinicializações subsequentes. Por exemplo:
...
Para definir e persistir cada valor em tempo de execução em vez de no início, use essas instruções:
...
`SET PERSIST` define um valor para a instância MySQL em execução. Ele também salva o valor, fazendo com que ele seja transferido para reinicializações subsequentes do servidor. Para alterar um valor para a instância MySQL em execução sem que ele seja transferido para reinicializações subsequentes, use a palavra-chave `GLOBAL` em vez de `PERSIST`. Veja a Seção 15.7.6.1, “Sintaxe SET para Atribuição de Variáveis”.
...
Para associar contas MySQL a um plugin de autenticação legível por LDAP, veja Usar Autenticação Legível por LDAP.

Observações Adicionais para SELinux

Em sistemas que executam EL6 ou EL e têm SELinux habilitado, as alterações na política SELinux são necessárias para habilitar os plugins de autenticação legível por LDAP do MySQL para se comunicarem com o serviço LDAP:
...
##### Desinstalação da Autenticação Legível por LDAP
...
* Se você instalou os plugins em tempo de execução usando `INSTALL PLUGIN`, eles permanecem instalados após reinicializações do servidor. Para desinstalá-los, use `UNINSTALL PLUGIN`:
...
Além disso, remova do seu arquivo `my.cnf` quaisquer opções de inicialização que definam variáveis de sistema relacionadas aos plugins LDAP. Se você usou `SET PERSIST` para persistir variáveis de sistema LDAP, use `RESET PERSIST` para remover as configurações.

##### LDAP Pluggable Authentication e ldap.conf

Para instalações que usam o OpenLDAP, o arquivo `ldap.conf` fornece configurações globais para clientes LDAP. Opções podem ser definidas neste arquivo para afetar clientes LDAP, incluindo os plugins de autenticação LDAP. O OpenLDAP usa opções de configuração nesta ordem de precedência:
...
* Configuração TLS: Variáveis de sistema estão disponíveis para habilitar TLS e controlar a configuração da CA, como `authentication_ldap_simple_tls` e `authentication_ldap_simple_ca_path` para autenticação LDAP simples, e `authentication_ldap_sasl_tls` e `authentication_ldap_sasl_ca_path` para autenticação LDAP SASL.

* Referência LDAP. Veja Referência de Busca LDAP.

Para obter mais informações sobre `ldap.conf`, consulte a página de manual `ldap.conf(5)`.

##### Configuração de Limites de Tempo para Autenticação Conectada LDAP

Para que as contas do MySQL se conectem a um servidor MySQL usando autenticação conectada LDAP, o servidor LDAP deve estar disponível e operacional. A interação entre o servidor MySQL e o servidor LDAP envolve dois passos. Primeiro, o servidor MySQL estabelece uma conexão com o servidor LDAP via TCP. Segundo, o servidor MySQL envia uma solicitação de vinculação LDAP sobre a conexão com o servidor LDAP e aguarda uma resposta antes de autenticar a conta. Se qualquer um dos passos falhar, a conta do MySQL não pode se conectar ao servidor MySQL.

Limites de tempo de curta duração que substituem os valores de limite de tempo de um sistema hospedeiro são aplicados por padrão tanto aos passos de conexão quanto de resposta. Em todos os casos, o usuário da conta recebe uma notificação de que sua tentativa de se conectar ao MySQL foi negada se o limite de tempo expirar. O registro no lado do cliente e no lado do servidor pode fornecer informações adicionais. No lado do cliente, defina a seguinte variável de ambiente para elevar o nível de detalhe e, em seguida, reinicie o cliente MySQL:
...
As seguintes variáveis de sistema suportam limites de tempo padrão para autenticação baseada em SASL e LDAP simples apenas em plataformas Linux.

**Tabela 8.21 Variáveis de sistema para autenticação baseada em SASL e LDAP simples**

<table summary="Variáveis do sistema usadas para autenticação de senha LDAP com base em SASL e simples."><col style="width: 75%"/><col style="width: 25%"/><thead><tr> <th>Nome da Variável do Sistema</th> <th>Valor de Timeout Padrão</th> </tr></thead><tbody><tr> <td><a class="link" href="pluggable-authentication-system-variables.html#sysvar_authentication_ldap_sasl_connect_timeout"><code class="literal">authentication_ldap_sasl_connect_timeout</code></a></td> <td>30 segundos</td> </tr><tr> <td><a class="link" href="pluggable-authentication-system-variables.html#sysvar_authentication_ldap_sasl_response_timeout"><code class="literal">authentication_ldap_sasl_response_timeout</code></a></td> <td>30 segundos</td> </tr><tr> <td><a class="link" href="pluggable-authentication-system-variables.html#sysvar_authentication_ldap_simple_connect_timeout"><code class="literal">authentication_ldap_simple_connect_timeout</code></a></td> <td>30 segundos</td> </tr><tr> <td><a class="link" href="pluggable-authentication-system-variables.html#sysvar_authentication_ldap_simple_response_timeout"><code class="literal">authentication_ldap_simple_response_timeout</code></a></td> <td>30 segundos</td> </tr></tbody></table>

Os valores de timeout para autenticação LDAP são ajustáveis no início e durante o runtime do servidor. Se você definir um timeout como zero usando uma dessas variáveis, você efetivamente desativa-o e o servidor MySQL retorna ao timeout padrão do sistema do host.
...
* O pool de conexões está em uso (especificamente, a variável de sistema `authentication_ldap_sasl_max_pool_size` tem um valor maior que zero, o que permite o pool).

##### Usando Autenticação LDAP Pluggable

Esta seção descreve como habilitar as contas do MySQL para se conectarem ao servidor MySQL usando a autenticação LDAP pluggable. Assume-se que o servidor está sendo executado com os plugins do lado do servidor apropriados habilitados, conforme descrito em Instalando Autenticação LDAP Pluggable, e que os plugins do lado do cliente apropriados estão disponíveis no host do cliente.

Esta seção não descreve a configuração ou administração do LDAP. Você deve estar familiarizado com esses tópicos.

Os dois plugins do lado do servidor LDAP trabalham cada um com um plugin do lado do cliente específico:

* O plugin `authentication_ldap_simple` do lado do servidor realiza a autenticação LDAP simples. Para conexões por contas que usam este plugin, os programas do cliente usam o plugin do lado do cliente `mysql_clear_password`, que envia a senha para o servidor como texto claro. Não é usada hash ou criptografia de senha, portanto, uma conexão segura entre o cliente MySQL e o servidor é recomendada para evitar a exposição da senha.
...
* Deve haver uma entrada de diretório LDAP para cada usuário a ser autenticado.

* Deve haver uma conta de usuário MySQL que especifique um plugin de autenticação LDAP no lado do servidor e, opcionalmente, nomeie o nome distinto do usuário LDAP (DN) associado. (Para associar um DN de usuário LDAP a uma conta MySQL, inclua uma cláusula `BY` na instrução `CREATE USER` que cria a conta.) Se uma conta não especificar uma string LDAP, a autenticação LDAP usa o nome de usuário especificado pelo cliente para encontrar a entrada LDAP.

* Os programas cliente se conectam usando o método de conexão apropriado para o plugin de autenticação no lado do servidor que a conta MySQL usa. Para autenticação LDAP, as conexões requerem o nome de usuário MySQL e a senha LDAP. Além disso, para contas que usam o plugin `authentication_ldap_simple` no lado do servidor, inicie programas cliente com a opção `--enable-cleartext-plugin` para habilitar o plugin `mysql_clear_password` no lado do cliente.
...
Para configurar uma conta MySQL para autenticação LDAP simples, use uma instrução `CREATE USER` para especificar o plugin `authentication_ldap_simple`, opcionalmente incluindo o nome distinto do usuário LDAP (DN), conforme mostrado aqui:
...
A string de autenticação especificada na cláusula `BY` não inclui a senha LDAP. Isso deve ser fornecido pelo usuário cliente no momento da conexão.
...
O plugin de autenticação `mysql_clear_password` do lado do cliente deixa a senha intacta, então os programas cliente enviam-na ao servidor MySQL como texto claro. Isso permite que a senha seja passada como está para o servidor LDAP. Uma senha em texto claro é necessária para usar a biblioteca LDAP do lado do servidor sem SASL, mas pode ser um problema de segurança em algumas configurações. Essas medidas minimizam o risco:

* Para tornar menos provável o uso acidental do plugin `mysql_clear_password`, os clientes MySQL devem habilitá-lo explicitamente (por exemplo, com a opção `--enable-cleartext-plugin`). Veja a Seção 8.4.1.3, “Autenticação com Pluggable Authentication com Texto Aberto do Lado do Cliente”.
...
2. A tentativa de conexão corresponde à conta `'betsy'@'localhost'`. O plugin LDAP do lado do servidor encontra que essa conta tem uma string de autenticação de `'uid=betsy_ldap,ou=People,dc=example,dc=com'` para nomear o DN do usuário LDAP. O plugin envia essa string e a senha do LDAP para o servidor LDAP.

3. O servidor LDAP encontra a entrada LDAP para `betsy_ldap` e a senha corresponde, então a autenticação LDAP é bem-sucedida.

4. A entrada LDAP não tem nenhum atributo de grupo, então o plugin do lado do servidor retorna o nome do usuário do cliente (`betsy`) como o usuário autenticado. Esse é o mesmo nome de usuário fornecido pelo cliente, então não ocorre nenhum encaminhamento e a sessão do cliente usa a conta `'betsy'@'localhost'` para verificação de privilégios.

Se a instrução `CREATE USER` não contiver nenhuma cláusula `BY` para especificar o nome distinto LDAP `betsy_ldap`, as tentativas de autenticação usariam o nome de usuário fornecido pelo cliente (neste caso, `betsy`). Na ausência de uma entrada LDAP para `betsy`, a autenticação falharia.
...
##### Autenticação LDAP com Encaminhamento

Os plugins de autenticação LDAP suportam o encaminhamento, permitindo que um usuário se conecte ao servidor MySQL como um usuário, mas assuma os privilégios de um usuário diferente. Esta seção descreve o suporte básico ao proxy de plugins LDAP. Os plugins LDAP também suportam a especificação da preferência do grupo e do mapeamento do usuário proxy; consulte Especificação de Preferência e Mapeamento de Preferência de Grupo de Autenticação LDAP.

A implementação de encaminhamento descrita aqui é baseada no uso de valores de atributos de grupo LDAP para mapear usuários MySQL que se autenticam usando LDAP para outras contas MySQL que definem conjuntos diferentes de privilégios. Os usuários não se conectam diretamente através das contas que definem os privilégios. Em vez disso, eles se conectam através de uma conta proxy padrão autenticada com LDAP, de modo que todos os logins externos sejam mapeados para as contas MySQL encaminhadas que possuem os privilégios. Qualquer usuário que se conecte usando a conta proxy é mapeado para uma dessas contas MySQL encaminhadas, cujos privilégios determinam as operações de banco de dados permitidas ao usuário externo.
...
No momento da conexão, os valores do atributo de grupo se tornam os nomes dos usuários autenticados, portanto, eles nomeiam as contas `accounting` e `front_office` proxy.

* Os exemplos assumem o uso da autenticação SASL LDAP. Faça os ajustes apropriados para a autenticação LDAP simples.

Crie a conta MySQL proxy padrão:
...
A definição da conta proxy não tem a cláusula `AS 'auth_string'` para nomear um DN de usuário LDAP. Assim:

* Quando um cliente se conecta, o nome do usuário do cliente se torna o nome do usuário LDAP para ser pesquisado.
...
Se a instalação do MySQL tiver usuários anônimos, eles podem conflitar com o usuário proxy padrão. Para obter mais informações sobre esse problema e maneiras de lidar com ele, consulte Conflitos de Usuário Proxy Padrão e Usuário Anônimo.
...
As contas proxy usam o plugin de autenticação `mysql_no_login` para impedir que os clientes usem as contas para fazer login diretamente no servidor MySQL. Em vez disso, espera-se que os usuários que se autenticam usando LDAP usem a conta proxy padrão `''@'%'`. (Isso assume que o plugin `mysql_no_login` está instalado. Para instruções, consulte Seção 8.4.1.8, “Autenticação Pluggable No-Login”.) Para métodos alternativos de proteção das contas proxy contra o uso direto, consulte Proteger Login Direto em Contas Proxy.
...
1. O servidor autentica a conexão usando a conta de proxy padrão `''@'%'`, para o usuário cliente `basha`.
...
4. O usuário autenticado difere do nome do usuário cliente `basha`, com o resultado de que `basha` é tratado como um proxy para `accounting`, e `basha` assume os privilégios da conta `accounting` proxy. A consulta a seguir retorna a saída conforme mostrado:
...
Isso demonstra que `basha` usa os privilégios concedidos à conta MySQL `accounting` proxy, e que a proxy é realizada através da conta de proxy usuário padrão.
...
1. O servidor autentica a conexão usando a conta de proxy padrão `''@'%'`, para o usuário cliente `basil`.
...
4. O usuário autenticado difere do nome do usuário cliente `basil`, com o resultado de que `basil` é tratado como um proxy para `front_office`, e `basil` assume os privilégios da conta `front_office` proxy. A consulta a seguir retorna a saída conforme mostrado:
...
Isso demonstra que `basil` usa os privilégios concedidos à conta MySQL `front_office` proxy, e que a proxy é realizada através da conta de proxy usuário padrão.
...
Este exemplo de código demonstra como configurar e usar a autenticação LDAP para diferentes usuários e grupos. A conta de proxy padrão `''@'%'` é usada para autenticar os usuários `basha` e `basil`.

Como descrito na Autenticação LDAP com Proxy, o proxy de autenticação básica LDAP funciona com o princípio de que o plugin usa o primeiro nome de grupo retornado pelo servidor LDAP como o nome da conta de usuário proxy do MySQL. Essa capacidade simples não permite especificar nenhuma preferência sobre qual nome de grupo usar se o servidor LDAP retornar vários nomes de grupo, ou especificar qualquer nome diferente do nome de grupo como o nome do usuário proxy.

Para contas do MySQL que usam autenticação LDAP, a string de autenticação pode especificar as seguintes informações para permitir maior flexibilidade de proxy:

* Uma lista de grupos em ordem de preferência, de modo que o plugin use o primeiro nome de grupo na lista que corresponde a um grupo retornado pelo servidor LDAP.

* Um mapeamento de nomes de grupo para nomes de usuários proxy, de modo que um nome de grupo, ao ser correspondido, possa fornecer um nome especificado para usar como o usuário proxy. Isso fornece uma alternativa para usar o nome de grupo como o nome do usuário proxy.

Considere a seguinte definição de conta proxy MySQL:

```
  "my group name"="my user name"
  ```

A string de autenticação tem um sufixo DN de usuário `ou=People,dc=example,dc=com` prefixado pelo caractere `+`. Assim, como descrito na Autenticação LDAP Sufixos DN de Usuário, o DN completo do usuário é construído a partir do sufixo do DN de usuário conforme especificado, mais o nome do usuário do cliente como o atributo `uid`.
...
Assumindo que o plugin encontra uma correspondência de nome de grupo, ele realiza a mapeamento desse nome de grupo para o nome de usuário proxy do MySQL, se houver. Para a conta de proxy do exemplo, o mapeamento ocorre da seguinte forma:

* Se o nome de grupo correspondente for `grp1` ou `grp3`, esses são associados na string de autenticação com os nomes de usuário `usera` e `userc`, respectivamente. O plugin usa o nome de usuário associado correspondente como o nome de usuário proxy.

* Se o nome de grupo correspondente for `grp2`, não há um nome de usuário associado na string de autenticação. O plugin usa `grp2` como o nome de usuário proxy.

Se o servidor LDAP retornar um grupo no formato DN, o plugin LDAP analisa o DN do grupo para extrair o nome do grupo dele.

Para especificar a preferência do grupo LDAP e as informações de mapeamento, esses princípios se aplicam:

* Comece a parte de preferência do grupo e mapeamento da string de autenticação com o caractere prefixo `#`.
...
+ Para um item especificado como `group_name=user_name` (com um nome de usuário), o nome do grupo é mapeado para o nome do usuário, que é usado como o nome do usuário proxy do MySQL.

+ Para um item especificado como *`group_name`* (sem nome de usuário), o nome do grupo é usado como o nome do usuário proxy do MySQL.

* Para citar um nome de grupo ou usuário que contenha caracteres especiais, como espaço, rode-o com caracteres de aspas duplas (`"`). Por exemplo, se um item tiver nomes de grupo e usuário de `meu nome de grupo` e `meu nome de usuário`, ele deve ser escrito em uma mapeo de grupo usando aspas:
  ...
  Se um item tiver nomes de grupo e usuário de `meu_grupo_nome` e `meu_nome_usuario` (que não contêm caracteres especiais), ele pode, mas não precisa, ser escrito usando aspas. Qualquer um dos seguintes é válido:
...
* Para escapar um caractere, anteceda-o com uma barra invertida (`\`). Isso é útil, particularmente, para incluir uma aspas literal ou barra invertida, que, de outra forma, não são incluídas literalmente.

* O DN do usuário não precisa estar presente na string de autenticação, mas, se estiver presente, ele deve preceder a parte de preferência e mapeamento do grupo. O DN do usuário pode ser fornecido como o DN completo do usuário ou como um sufixo de DN do usuário com um caractere de prefixo `+`. (Veja Sufixos de DN de Usuário de Autenticação LDAP.)
...
Esta string de autenticação da conta não tem `+` no início, então é considerada o valor completo do DN do usuário:
...
Esta string de autenticação da conta tem `+` no início, então é considerada apenas parte do DN do usuário:
...
O cliente se conecta com o nome do usuário especificado na conta (`accounting`), que, neste caso, é usado como o atributo `uid`, juntamente com a string de autenticação, para construir o DN do usuário: `uid=accounting,ou=People,dc=example,dc=com`
...
A discussão a seguir fornece informações sobre os requisitos de configuração para usar o método GSSAPI. Assume-se familiaridade com os conceitos e operação do Kerberos. A lista a seguir define brevemente vários termos comuns do Kerberos. Você também pode achar a seção Glossário de [RFC 4120](https://tools.ietf.org/html/rfc4120) útil.
...
* TGT: O bilhete de concessão de acesso; apresentado ao TGS para obter bilhetes de serviço para acesso ao serviço.
...
* O Active Directory inclui ambos os servidores, com autenticação Kerberos habilitada por padrão no servidor LDAP do Active Directory.

* O OpenLDAP fornece um servidor LDAP, mas pode ser necessário um servidor KDC separado, com configuração adicional de Kerberos necessária.

O Kerberos também deve estar disponível no host do cliente. O cliente entra em contato com o AS usando uma senha para obter um TGT. O cliente, em seguida, usa o TGT para obter acesso do TGS a outros serviços, como o LDAP.

As seções a seguir discutem as etapas de configuração para usar GSSAPI/Kerberos para autenticação SASL LDAP no MySQL:

* Verificar a disponibilidade de Kerberos e LDAP
...
* Uma principal chamada `bredon@MYSQL.LOCAL` está registrada no KDC. (Em uma discussão posterior, esse nome de principal também é associado à conta MySQL que autentica-se ao servidor MySQL usando GSSAPI/Kerberos.)
...
3. Verifique se a autenticação Kerberos funciona corretamente:
   ...
   1. Use **kinit** para autenticar-se no Kerberos:
      ...
      O comando autentica para o principal Kerberos chamado `bredon@MYSQL.LOCAL`. Insira a senha do principal quando o comando solicitar. O KDC retorna um TGT que é armazenado em cache no lado do cliente para uso por outras aplicações que reconhecem o Kerberos.
...
Supondo que o servidor LDAP seja acessível através do Kerberos como descrito acima, configure o plugin de autenticação LDAP SASL do lado do servidor para usar o método de autenticação GSSAPI/Kerberos. (Para informações gerais sobre a instalação do plugin LDAP, consulte Instalando o LDAP Pluggable Authentication.) Aqui está um exemplo de configurações relacionadas ao plugin que o arquivo `my.cnf` do servidor pode conter:

```
  uid=bredon,dc=MYSQL,dc=LOCAL
  ```

Essas configurações de arquivos de opção configuram o plugin LDAP SASL da seguinte forma:
...
* `authentication_ldap_sasl_auth_method_name` deve ser definido como `GSSAPI` para usar o método de autenticação SASL LDAP GSSAPI/Kerberos.
...
* `authentication_ldap_sasl_bind_root_dn` e `authentication_ldap_sasl_bind_root_pwd` configuram o DN raiz e a senha para a capacidade de busca de grupos. Essa capacidade é necessária, mas os usuários podem não ter privilégios para fazer buscas. Nesse caso, é necessário fornecer informações sobre o DN raiz:

  + Na opção DN, `admin` deve ser o nome de uma conta LDAP administrativa que tenha privilégios para realizar buscas de usuários.

  + Na opção senha, *`password`* deve ser a senha da conta `admin`.

* `authentication_ldap_sasl_bind_base_dn` indica o caminho do base de DN do usuário, para que as buscas busquem usuários no domínio `MYSQL.LOCAL`.

* `authentication_ldap_sasl_user_search_attr` especifica um atributo padrão de busca do Active Directory, `sAMAccountName`. Esse atributo é usado em buscas para corresponder a nomes de logon; os valores dos atributos não são os mesmos dos valores do DN do usuário.

###### Crie uma Conta MySQL que Use GSSAPI/Kerberos para Autenticação LDAP

A autenticação do MySQL usando o plugin de autenticação LDAP SASL com o método GSSAPI/Kerberos é baseada em um usuário que é um principal Kerberos. A discussão a seguir usa um principal chamado `bredon@MYSQL.LOCAL` como esse usuário, que deve ser registrado em vários lugares:
...
```
  ...
  No Active Directory (que usa o Kerberos como método de autenticação padrão), a criação de um usuário cria tanto o principal Kerberos quanto a entrada do LDAP.

* O DBA do MySQL deve criar uma conta que tenha o nome do principal Kerberos como nome de usuário e que autentique usando o plugin SASL LDAP do lado do servidor para GSSAPI/Kerberos.

Suponha que o principal Kerberos e a entrada do LDAP tenham sido registrados pelos administradores de serviço apropriados, e que, conforme descrito anteriormente em Instalar o LDAP Pluggable Authentication e Configurar o Plugin de Autenticação SASL LDAP do Lado do Servidor para GSSAPI/Kerberos, o servidor MySQL tenha sido iniciado com configurações apropriadas para o plugin SASL LDAP do lado do servidor. O DBA do MySQL então cria uma conta MySQL que corresponde ao nome do principal Kerberos, incluindo o nome do domínio.

Observação

O plugin SASL LDAP usa um DN de usuário constante para a autenticação Kerberos e ignora qualquer DN de usuário configurado pelo MySQL. Isso tem certas implicações:

* Para qualquer conta MySQL que use a autenticação GSSAPI/Kerberos, a string de autenticação nas instruções `CREATE USER` ou `ALTER USER` não deve conter DN de usuário, pois ele não tem efeito.

* Como a string de autenticação não contém DN de usuário, ela deve conter informações de mapeamento de grupo, para permitir que o usuário seja tratado como um usuário proxy mapeado para o usuário desejado proxy. Para informações sobre proxy com o plugin de autenticação LDAP, consulte Autenticação LDAP com Proxy.
...
Observe atentamente a citação para o nome da conta do proxy na primeira declaração `CREATE USER` e na declaração `GRANT PROXY`:
...
* Para a autenticação LDAP Kerberos, a parte do usuário do nome da conta inclui o domínio principal, então `'bredon@MYSQL.LOCAL'` é citado como um único valor. Como não há parte de host dada, o nome completo da conta do MySQL usa o padrão de `'%'` como parte do host: `'bredon@MYSQL.LOCAL'@'%'`

Observação

Ao criar uma conta que autentica usando o plugin de autenticação LDAP SASL `authentication_ldap_sasl` com o método de autenticação GSSAPI/Kerberos, a declaração `CREATE USER` inclui o reino como parte do nome do usuário. Isso difere da criação de contas que usam o plugin `authentication_kerberos` Kerberos. Para tais contas, a declaração `CREATE USER` não inclui o reino como parte do nome do usuário. Em vez disso, especifique o reino como a string de autenticação na cláusula `BY`. Veja Como criar uma conta do MySQL que usa autenticação Kerberos.

A conta proxy usa o plugin de autenticação `mysql_no_login` para impedir que os clientes usem a conta para fazer login diretamente no servidor MySQL. Em vez disso, espera-se que os usuários que se autentiquem usando LDAP usem a conta de proxy `bredon@MYSQL.LOCAL`. (Isso assume que o plugin `mysql_no_login` está instalado. Para instruções, consulte a Seção 8.4.1.8, “Autenticação Pluggable sem Login”.) Para métodos alternativos de proteção de contas proxy contra uso direto, consulte Como Proteger Logins Diretos em Contas Proxy.

###### Use a Conta MySQL para Conectar ao Servidor MySQL

Após configurar uma conta MySQL que se autentica usando GSSAPI/Kerberos, os clientes podem usá-la para se conectar ao servidor MySQL. A autenticação Kerberos pode ocorrer antes ou no momento da invocação do programa cliente MySQL:

* Antes de invocar o programa cliente MySQL, o usuário do cliente pode obter um TGT do KDC independentemente do MySQL. Por exemplo, o usuário do cliente pode usar **kinit** para se autenticar no Kerberos fornecendo um nome de principal Kerberos e a senha da principal:

  ```
  mysql --default-auth=authentication_ldap_sasl_client
  ```

  O TGT resultante é armazenado em cache e torna-se disponível para uso por outras aplicações que reconhecem o Kerberos, como programas que usam o plugin de autenticação LDAP SASL do lado do cliente. Neste caso, o programa cliente MySQL se autentica no servidor MySQL usando o TGT, então invoque o cliente sem especificar um nome de usuário ou senha:
  ...
  Como descrito anteriormente, quando o TGT está em cache, as opções de nome de usuário e senha não são necessárias no comando do cliente. Se o comando incluir-as de qualquer forma, elas são tratadas da seguinte maneira:

  + Se o comando incluir um nome de usuário, a autenticação falha se esse nome não corresponder ao nome da principal no TGT.

+ Se o comando incluir uma senha, o plugin do lado do cliente a ignora. Como a autenticação é baseada no TGT, ela pode ser bem-sucedida *mesmo que a senha fornecida pelo usuário esteja incorreta*. Por essa razão, o plugin emite um aviso se um TGT válido for encontrado, o que faz com que a senha seja ignorada.

* Se o cache do Kerberos não contiver TGT, o próprio plugin de autenticação SASL LDAP do lado do cliente pode obter o TGT do KDC. Inicie o cliente com as opções para o nome e a senha do principal Kerberos associado à conta MySQL (insira o comando em uma única linha, em seguida, insira a senha do principal quando solicitado):
...
* Se o cache do Kerberos não contiver TGT e o comando do cliente não especificar o nome do principal como o nome do usuário, a autenticação falha.

Se você não tiver certeza se um TGT existe, pode usar o **klist** para verificar.

A autenticação ocorre da seguinte forma:

1. O cliente usa o TGT para se autenticar usando o Kerberos.
2. O servidor encontra a entrada LDAP para o principal e usa-a para autenticar a conexão para a conta de proxy MySQL `bredon@MYSQL.LOCAL`.

3. As informações de mapeamento de grupo na string de autenticação da conta de proxy (`'#krb_grp=proxied_krb_user'`) indicam que o usuário autenticado proxy deve ser `proxied_krb_user`.

4. `bredon@MYSQL.LOCAL` é tratado como um proxy para `proxied_krb_user`, e a seguinte consulta retorna a saída conforme mostrado:
...
```
   ...
   O valor `CURRENT_USER()` é o nome completo da conta de usuário proxy, que consiste na parte de usuário `proxied_krb_user` e na parte `%` do host.
...
Isso demonstra que o encaminhamento ocorre através da conta de usuário `bredon@MYSQL.LOCAL` do proxy, e que `bredon@MYSQL.LOCAL` assume os privilégios concedidos à conta de usuário `proxied_krb_user` do usuário encaminhado.

Uma vez obtida a TGT, ela é armazenada no lado do cliente e pode ser usada até expirar sem precisar ser especificada novamente a senha. No entanto, independentemente de como a TGT foi obtida, o plugin do lado do cliente a usa para adquirir ingressos de serviço e se comunicar com o plugin do lado do servidor.

Observação

Quando o próprio plugin de autenticação do lado do cliente obtém a TGT, o usuário do cliente pode não querer que a TGT seja reutilizada. Como descrito no Parâmetros de configuração do cliente para autenticação LDAP, o arquivo local `/etc/krb5.conf` pode ser usado para fazer com que o plugin do lado do cliente destrua a TGT quando estiver pronto para isso.

O plugin do lado do servidor não tem acesso à própria TGT ou à senha Kerberos usada para obtê-la.
