...
As variáveis do sistema `audit_log_xxx_policy` são usadas para o modo de registro de auditoria obsoleto (ver Seção 8.4.6.10, “Filtragem do Registro de Auditoria no Modo Legado”). Com a filtragem de registro de auditoria baseada em regras, essas variáveis permanecem visíveis (por exemplo, usando `SHOW VARIABLES`), mas as alterações nelas não têm efeito, a menos que você escreva filtros que contenham construções que as refiram.

A lista a seguir descreve as variáveis predefinidas permitidas para os itens `variable`:

* `audit_log_connection_policy_value`

  Esta variável corresponde ao valor da variável do sistema `audit_log_connection_policy`. O valor é um inteiro não assinado. A Tabela 8.40, “Valores de `audit_log_connection_policy_value`”, mostra os valores permitidos e os valores correspondentes de `audit_log_connection_policy`.

  **Tabela 8.40 Valores de `audit_log_connection_policy_value`**
...
* `audit_log_policy_value`
  ...
  Esta variável corresponde ao valor da variável de sistema `audit_log_policy`. O valor é um inteiro não signicante. A Tabela 8.41, “Valores de `audit_log_policy_value`”, mostra os valores permitidos e os valores correspondentes de `audit_log_policy`.

  **Tabela 8.41 Valores de `audit_log_policy_value`**
...
* `audit_log_statement_policy_value`
  ...
  Esta variável corresponde ao valor da variável de sistema `audit_log_statement_policy`. O valor é um inteiro não negativo. A tabela 8.42, “Valores de `audit_log_statement_policy_value`”, mostra os valores permitidos e os valores correspondentes de `audit_log_statement_policy`.

  **Tabela 8.42 Valores de `audit_log_statement_policy_value`**
