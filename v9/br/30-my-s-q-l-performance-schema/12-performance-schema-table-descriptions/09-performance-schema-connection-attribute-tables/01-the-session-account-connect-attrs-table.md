#### 29.12.9.1 A tabela `session_account_connect_attrs`
...
A tabela `session_account_connect_attrs` tem os seguintes índices:

* Chave primária em (`PROCESSLIST_ID`, `ATTR_NAME`)

O `TRUNCATE TABLE` não é permitido para a tabela `session_account_connect_attrs`.