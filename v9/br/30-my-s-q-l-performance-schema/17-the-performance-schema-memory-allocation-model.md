## Modelo de Alocação de Memória do Schema de Desempenho
...
* Nunca libera memória durante a operação do servidor (embora possa ser reciclada)

* Libera toda a memória usada na desligada
...
Vários parâmetros de dimensionamento do Schema de Desempenho são autoscalados e não precisam ser configurados explicitamente, a menos que você queira estabelecer um limite explícito na alocação de memória:
...
Para um parâmetro autoscalado, a configuração funciona da seguinte forma:

* Com o valor definido como -1 (o padrão), o parâmetro é autoscalado:
...
* Com o valor definido como *`N`* > 0:
  ...
  + Uma vez que o tamanho do buffer atinja *`N`*, mais memória não é alocada. Os dados coletados pelo Schema de Desempenho para este buffer são perdidos e quaisquer contagem de "instância perdida" correspondente são incrementadas.
