...
As tabelas `INNODB_CMPMEM` e `INNODB_CMPMEM_RESET` fornecem informações de status sobre as páginas compactadas dentro do pool de buffer do `InnoDB`.

As tabelas `INNODB_CMPMEM` e `INNODB_CMPMEM_RESET` têm as seguintes colunas:
...
* `BUFFER_POOL_INSTANCE`
  ...
  Um identificador único para a instância do pool de buffer.
...
* `RELOCATION_OPS`
  ...
  O número de vezes que um bloco do tamanho `PAGE_SIZE` foi realocado. O sistema buddy pode realocar o "vizinho de amigo" alocado de um bloco liberado quando ele tenta formar um bloco liberado maior. Ler a tabela `INNODB_CMPMEM_RESET` reescreve esse contador.

* `RELOCATION_TIME`

  O tempo total em microsegundos usado para realocar blocos do tamanho `PAGE_SIZE`. Ler a tabela `INNODB_CMPMEM_RESET` reescreve esse contador.
...
* Use essas tabelas para medir a eficácia da compressão de tabelas do `InnoDB` no seu banco de dados.
