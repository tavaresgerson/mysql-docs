...
A instrução `EXPLAIN` produz informações extras (extendidas) que não fazem parte da saída do `EXPLAIN`, mas podem ser visualizadas ao emitir uma instrução `SHOW WARNINGS` após `EXPLAIN`. As informações extendidas estão disponíveis para as instruções `SELECT`, `DELETE`, `INSERT`, `REPLACE` e `UPDATE`.
...
As informações extendidas exibidas com uma instrução `SHOW WARNINGS` após `EXPLAIN` são produzidas apenas para instruções `SELECT`. O `SHOW WARNINGS` exibe um resultado vazio para outras instruções explicáveis (`DELETE`, `INSERT`, `REPLACE` e `UPDATE`).
...
* `<in_optimizer>(fragmento da consulta)`
  ...
  Este é um objeto interno do otimizador sem significado para o usuário.

* `<index_lookup>(fragmento da consulta)`

  O fragmento da consulta é processado usando uma busca por índice para encontrar linhas qualificadoras.
...
* `<materialize>(fragmento da consulta)`
  ...
  A materialização da subconsulta é usada.
...
* `<primary_index_lookup>(fragmento da consulta)`
  ...
  O fragmento da consulta é processado usando uma busca por chave primária para encontrar linhas qualificadoras.
