...
O otimizador pode reescrevê-la como uma subconsulta `EXISTS` correlacionada:
...
A materialização de subconsultas usando uma tabela temporária evita essas reescritas e permite executar a subconsulta apenas uma vez, em vez de uma vez por linha da consulta externa.

Para que a materialização de subconsultas seja usada no MySQL, o sinalizador `materialization` da variável de sistema `optimizer_switch` deve estar habilitado. (Veja a Seção 10.9.2, “Otimizações Alternativas”). Com o sinalizador `materialization` habilitado, a materialização se aplica aos predicados da subconsulta que aparecem em qualquer lugar (na lista de seleção, `WHERE`, `ON`, `GROUP BY`, `HAVING` ou `ORDER BY`), para predicados que caibam em qualquer um desses casos de uso:
...
* O predicado tem essa forma quando há uma única expressão externa *`oe`* e uma expressão interna *`ie`*. As expressões podem ser opcionais.
...
Um exemplo em que a materialização de subconsulta não é usada é a seguinte consulta, onde `t2.b` é uma coluna opcional:
...
O uso do `EXPLAIN` com uma consulta fornece alguma indicação de se o otimizador usa a materialização de subconsulta:

* Comparado à execução de consultas que não utilizam materialização, `select_type` pode mudar de `SUBQUERY DEPENDENT` para `SUBQUERY`. Isso indica que, para uma subconsulta que seria executada uma vez por linha externa, a materialização permite que a subconsulta seja executada apenas uma vez.

* Para uma saída de `EXPLAIN` estendida, o texto exibido por uma consulta `SHOW WARNINGS` subsequente inclui `materialize` e `materialized-subquery`.

O MySQL também pode aplicar a materialização de subconsulta a uma instrução `UPDATE` ou `DELETE` de uma única tabela que utiliza um predicado de subconsulta `[NOT] IN` ou `[NOT] EXISTS`, desde que a instrução não utilize `ORDER BY` ou `LIMIT`, e que a materialização de subconsulta seja permitida por uma dica do otimizador ou pela configuração `optimizer_switch`.