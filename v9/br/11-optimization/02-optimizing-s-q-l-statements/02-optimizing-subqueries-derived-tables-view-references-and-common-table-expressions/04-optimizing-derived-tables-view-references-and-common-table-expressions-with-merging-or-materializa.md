...
Com a materialização, `derived_t1` e `derived_t2` são tratados como tabelas separadas dentro de suas respetivas consultas.

O otimizador lida com tabelas derivadas, referências de visualização e expressões de tabela comuns da mesma forma: Evita a materialização desnecessária sempre que possível, o que permite empurrar condições da consulta externa para as tabelas derivadas e produzir planos de execução mais eficientes. (Para um exemplo, consulte a Seção 10.2.2.2, “Otimizando Subconsultas com Materialização”.)
...
Os seguintes meios estão disponíveis para influenciar se o otimizador tenta combinar tabelas derivadas, referências de visualizações e expressões de tabela comuns no bloco de consulta externa:

* As dicas de otimização `MERGE` e `NO_MERGE` podem ser usadas. Elas se aplicam assumindo que nenhuma outra regra impeça a combinação. Veja a Seção 10.9.3, “Dicas de Otimização”.

* Da mesma forma, você pode usar a bandeira `derived_merge` da variável de sistema `optimizer_switch`. Veja a Seção 10.9.2, “Otimizações Desconectables”. Por padrão, a bandeira está habilitada para permitir a combinação. Desabilitar a bandeira impede a combinação e evita erros `ER_UPDATE_TABLE_USED`.

A bandeira `derived_merge` também se aplica a visualizações que não contêm a cláusula `ALGORITHM`. Assim, se um erro `ER_UPDATE_TABLE_USED` ocorrer para uma referência de visualização que usa uma expressão equivalente à subconsulta, adicionar `ALGORITHM=TEMPTABLE` à definição da visualização impede a combinação e tem precedência sobre o valor `derived_merge`.

* É possível desabilitar a combinação usando em subconsulta quaisquer construções que impeçam a combinação, embora essas não sejam tão explícitas em seu efeito na materialização. Construções que impedem a combinação são as mesmas para tabelas derivadas, expressões de tabela comuns e referências de visualizações:
...
Se o otimizador escolher a estratégia de materialização em vez de combinar para uma tabela derivada, ele trata a consulta da seguinte forma:
...
Se o processo de otimização processar `t1` primeiro e a cláusula `WHERE` produzir um resultado vazio, a junção deve ser necessariamente vazia e a tabela derivada não precisa ser materializada.
...
A cláusula `ALGORITHM` para `CREATE VIEW` não afeta a materialização de nenhuma cláusula `WITH`") que antecede a instrução `SELECT` na definição da vista. Considere a seguinte declaração:

```
CREATE ALGORITHM={TEMPTABLE|MERGE} VIEW v1 AS WITH ... SELECT ...
```

O valor `ALGORITHM` afeta a materialização apenas do `SELECT`, e não da cláusula `WITH`")".
