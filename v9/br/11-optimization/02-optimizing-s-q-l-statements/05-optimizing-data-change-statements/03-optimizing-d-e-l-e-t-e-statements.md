...
Para excluir todas as linhas de uma tabela `MyISAM`, `TRUNCATE TABLE tbl_name` é mais rápido do que `DELETE FROM tbl_name`. As operações de truncar não são seguras em transações; um erro ocorre ao tentar uma delas durante uma transação ativa ou bloqueio de tabela ativo. Consulte a Seção 15.1.42, “Instrução TRUNCATE TABLE”.