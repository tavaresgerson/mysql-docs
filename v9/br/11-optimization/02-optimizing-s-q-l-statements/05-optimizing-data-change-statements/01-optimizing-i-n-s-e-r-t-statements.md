#### 10.2.5.1 Otimizando Instruções de Inserção

Para otimizar a velocidade de inserção, combine muitas operações pequenas em uma única operação grande. Idealmente, faça uma única conexão, envie os dados para muitas novas linhas de uma só vez e adiaste todas as atualizações de índice e verificações de consistência até o final.
...
* Consulte a Seção 10.5.5, “Carregamento de Dados em Massa para Tabelas InnoDB”, para dicas específicas para tabelas `InnoDB`.

* Consulte a Seção 10.6.2, “Carregamento de Dados em Massa para Tabelas MyISAM”, para dicas específicas para tabelas `MyISAM`.