...
Sem filtragem condicional, a saída do `EXPLAIN` é semelhante a esta:
...
Normalmente, o otimizador não calcula o efeito de filtragem de condições (redução do número de linhas da linha prefixada) para a última tabela unida, porque não há a próxima tabela para passar as linhas. Uma exceção ocorre para `EXPLAIN`: Para fornecer mais informações, o efeito de filtragem é calculado para todas as tabelas unidas, incluindo a última.
...
* Da mesma forma, se não houver informações sobre o histograma de uma coluna, gere um histograma (veja a Seção 10.9.6, “Estatísticas do Otimizador”).

* Mude a ordem da junção. As maneiras de realizar isso incluem dicas de otimizador de ordem de junção (veja a Seção 10.9.3, “Dicas do Otimizador”), `STRAIGHT_JOIN` imediatamente após o `SELECT` e o operador de junção `STRAIGHT_JOIN`.
