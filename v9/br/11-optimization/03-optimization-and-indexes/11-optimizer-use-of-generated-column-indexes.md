### 10.3.11 Uso do Optimizer de Índices de Colunas Geradas
...
* A coluna gerada deve ser definida como uma expressão que contenha pelo menos uma chamada de função ou um dos operadores mencionados no item anterior. A expressão não pode consistir em uma referência simples a outra coluna. Por exemplo, `gc INT AS (f1) STORED` consiste apenas em uma referência de coluna, então índices em `gc` não são considerados.
...
* Para comparações de strings com colunas geradas indexadas que calculam um valor a partir de uma função JSON que retorna uma string entre aspas, é necessário `JSON_UNQUOTE()` na definição da coluna para remover as aspas extras do valor da função. (Para comparação direta de uma string com o resultado da função, o comparador JSON remove as aspas, mas isso não ocorre para buscas de índice.) Por exemplo, em vez de escrever uma definição de coluna assim:
  ...
  Com a definição deste último, o otimizador pode detectar uma correspondência para ambas essas comparações:
...
Sem a função `JSON_UNQUOTE()` na definição da coluna, o otimizador detecta uma correspondência apenas para a primeira dessas comparações.

* Se o otimizador escolher o índice errado, uma dica de índice pode ser usada para desabilitar ele e forçar o otimizador a fazer uma escolha diferente.