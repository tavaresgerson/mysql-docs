...
As dicas de índice fornecem ao otimizador informações sobre como escolher índices durante o processamento de consultas. As dicas de índice, descritas aqui, diferem das dicas do otimizador, descritas na Seção 10.9.3, “Dicas do otimizador”. As dicas de índice e do otimizador podem ser usadas separadamente ou juntas.
...
O MySQL 9.5 suporta as dicas de otimização de nível de índice `JOIN_INDEX`, `GROUP_INDEX`, `ORDER_INDEX` e `INDEX`, que são equivalentes e destinadas a substituir as dicas de índice `FORCE INDEX`, bem como as dicas de otimização `NO_JOIN_INDEX`, `NO_GROUP_INDEX`, `NO_ORDER_INDEX` e `NO_INDEX`, que são equivalentes e destinadas a substituir as dicas de índice `IGNORE INDEX`. Portanto, você deve esperar que `USE INDEX`, `FORCE INDEX` e `IGNORE INDEX` sejam descontinuados em uma futura versão do MySQL e, em algum momento depois disso, serem removidos completamente.

Essas dicas de otimização de nível de índice são suportadas tanto em declarações `DELETE` de uma única tabela quanto em declarações `DELETE` de várias tabelas.

Para obter mais informações, consulte Dicas de otimização de nível de índice.

Cada dica requer nomes de índice, não de colunas. Para se referir a uma chave primária, use o nome `PRIMARY`. Para ver os nomes dos índices de uma tabela, use a declaração `SHOW INDEX` ou a tabela `STATISTICS` do Schema de Informações.

Um valor de *`index_name`* não precisa ser o nome completo de um índice. Pode ser um prefixo inequívoco de um nome de índice. Se um prefixo for ambíguo, ocorre um erro.
...
No entanto, há um erro em misturar `USE INDEX` e `FORCE INDEX` para a mesma tabela:
...
1. `{USE|FORCE} INDEX` é aplicado se presente. (Se não, o conjunto de índices determinado pelo otimizador é usado.)
...
Para buscas `FULLTEXT`, as dicas de índice funcionam da seguinte forma:

* Para buscas no modo de linguagem natural, as dicas de índice são ignoradas silenciosamente. Por exemplo, `IGNORE INDEX(i1)` é ignorado sem aviso e o índice ainda é usado.

* Para buscas no modo booleano, as dicas de índice com `FOR ORDER BY` ou `FOR GROUP BY` são ignoradas silenciosamente. As dicas de índice com `FOR JOIN` ou sem modificador `FOR` são respeitadas. Em contraste com a forma como as dicas se aplicam para buscas não `FULLTEXT`, a dica é usada para todas as fases da execução da consulta (encontrar linhas e recuperação, agrupamento e ordenação). Isso é verdadeiro mesmo se a dica for dada para um índice não `FULLTEXT`.
