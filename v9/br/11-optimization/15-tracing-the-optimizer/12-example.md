...
Agora podemos examinar o registro, cujo primeiro campo (`QUERY`), contendo a declaração original a ser rastreada, é mostrado aqui:
...
O campo `TRACE` começa mostrando que a execução da declaração é composta por etapas discretas, como esta:
...
A saída mostrada agora exibe a consulta como ela é usada para preparar a junção; todas as colunas (campos) foram resolvidas para suas tabelas e bancos de dados, e cada `SELECT` é anotado com um número de sequência, o que pode ser útil ao estudar subconsultas.
...
Como foi demonstrado na segunda parte da análise de intervalo, não é possível usar `GROUP_MIN_MAX` porque ele aceita apenas uma tabela, e temos duas na junção. Isso significa que não é possível acessar o intervalo.

O otimizador estima que ler a primeira tabela e aplicar quaisquer condições necessárias a ela resulta em 20 linhas:
...
Agora que a ordem das tabelas está fixa, podemos dividir a condição `WHERE` em partes que podem ser testadas precocemente (pushdown das condições na árvore de junção):
