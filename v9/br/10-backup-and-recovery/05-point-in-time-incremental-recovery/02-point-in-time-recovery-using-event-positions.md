...
Como exemplo, suponha que por volta das 20:06:00 de 11 de março de 2020, uma instrução SQL foi executada que excluiu uma tabela. Você pode realizar uma recuperação em ponto no tempo para restaurar o servidor até seu estado imediatamente antes da exclusão da tabela. Aqui estão alguns passos de amostra para isso:

1. Restaure o último backup completo criado antes do ponto no tempo de interesse (chame-o de `tp`, que é 20:06:00 de 11 de março de 2020 em nosso exemplo). Quando terminar, anote a posição do registro binário até a qual você restaurou o servidor para uso posterior e reinicie o servidor.

   Nota

   Embora a última posição do registro binário recuperada também seja exibida pelo InnoDB após a restauração e o reinício do servidor, isso *não* é um meio confiável para obter a posição final do log da sua restauração, pois podem ter havido eventos de DDL e alterações não do InnoDB que ocorreram após o tempo refletido pela posição exibida. Sua ferramenta de backup e restauração deve fornecer a você a última posição do registro binário para sua recuperação: por exemplo, se você estiver usando **mysqlbinlog** para a tarefa, verifique a posição de parada do replay do registro binário; se você estiver usando MySQL Enterprise Backup, a última posição do registro binário foi salva em seu backup. Veja Recuperação em Ponto no Tempo.
...
3. Aplique os eventos no arquivo de log binário ao servidor, começando com a posição do log que você encontrou no passo 1 (assumindo que é 155) e terminando na posição que você encontrou no passo 2 que está *antes* do seu momento de interesse (que é 232):
...
4. Além da recuperação do ponto no tempo que foi concluída, se você também quiser reexecutar todas as instruções *após* seu ponto no tempo de interesse, use **mysqlbinlog** novamente para aplicar todos os eventos após `tp` ao servidor. Observamos no passo 2 que, após a instrução que queríamos pular, o log está na posição 355; podemos usá-lo para a opção `--start-position`, para que quaisquer instruções após a posição sejam incluídas:
...
Seu banco de dados foi restaurado à última instrução registrada no arquivo de log binário, mas com o evento selecionado pulado.