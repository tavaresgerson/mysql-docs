...
Em casos em que você precisa transferir bancos de dados entre diferentes arquiteturas, você pode usar o **mysqldump** para criar um arquivo contendo instruções SQL. Em seguida, você pode transferir o arquivo para a outra máquina e alimentá-lo como entrada para o cliente **mysql**.

Use **mysqldump --help** para ver quais opções estão disponíveis.

Observação

Se GTIDs estiverem em uso no servidor onde você cria o dump (`gtid_mode=ON`), por padrão, o **mysqldump** inclui o conteúdo do conjunto `gtid_executed` no dump para transferi-lo para a nova máquina. Os resultados podem variar dependendo das versões do MySQL Server envolvidas. Verifique a descrição da opção **mysqldump's** `--set-gtid-purged` para descobrir o que acontece com as versões que você está usando e como alterar o comportamento se o resultado do comportamento padrão não for adequado para sua situação.
...
Em seguida, transfira os arquivos no *`DUMPDIR`* para um diretório correspondente na máquina de destino e carregue os arquivos no MySQL lá:
...
Não se esqueça de copiar o banco de dados `mysql` porque é lá que as tabelas de concessão são armazenadas. Você pode precisar executar comandos como usuário `root` do MySQL na nova máquina até que o banco de dados `mysql` esteja no lugar.
