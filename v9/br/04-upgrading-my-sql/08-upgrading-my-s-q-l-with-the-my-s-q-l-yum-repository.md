...
2. ### Atualizando o MySQL
   ...
   Atualize os componentes do MySQL usando comandos padrão do yum (ou dnf), como MySQL Server:
...
Após atualizar o MySQL usando o repositório Yum, as aplicações compiladas com versões mais antigas das bibliotecas cliente compartilhadas devem continuar funcionando.

*Se você recompilar as aplicações e vincular dinamicamente com as bibliotecas atualizadas:* Como é típico com novas versões de bibliotecas compartilhadas, onde há diferenças ou adições na versão dos símbolos entre as bibliotecas mais novas e as mais antigas (por exemplo, entre as bibliotecas cliente compartilhadas padrão 9.5 mais novas e algumas versões mais antigas — anteriores ou variantes — das bibliotecas compartilhadas fornecidas nativamente pelos repositórios de software das distribuições Linux, ou de outras fontes), quaisquer aplicações compiladas usando as bibliotecas compartilhadas mais novas e atualizadas exigem essas bibliotecas atualizadas nos sistemas onde as aplicações são implantadas. Como esperado, se essas bibliotecas não estiverem no lugar, as aplicações que requerem as bibliotecas compartilhadas falharão. Por essa razão, certifique-se de implantar os pacotes para as bibliotecas compartilhadas do MySQL nesses sistemas. Para fazer isso, adicione o repositório Yum do MySQL aos sistemas (veja Adicionando o repositório Yum do MySQL) e instale as bibliotecas compartilhadas mais recentes usando as instruções fornecidas em Instalando Produtos e Componentes Adicionais do MySQL com Yum.