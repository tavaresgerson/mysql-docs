## 2.6 Instalando o MySQL usando a Rede Unbreakable Linux (ULN)

O Linux suporta várias soluções diferentes para instalar o MySQL, conforme descrito na Seção 2.5, “Instalando o MySQL no Linux”. Um dos métodos, descrito nesta seção, é a instalação a partir da Rede Unbreakable Linux (ULN) da Oracle. Você pode encontrar informações sobre o Oracle Linux e a ULN em <http://linux.oracle.com/>.

Para usar a ULN, você precisa obter um login da ULN e registrar a máquina usada para a instalação na ULN. Isso é descrito em detalhes na [FAQ da ULN](https://linux.oracle.com/uln_faq.html). A página também descreve como instalar e atualizar pacotes.
