...
2. A opção `WITH_SSL` do **CMake** determina qual biblioteca SSL usar para compilar o MySQL (veja a Seção 2.8.7, “Opções de Configuração de Código-Fonte do MySQL”). O padrão é `-DWITH_SSL=system`, que usa o OpenSSL. Para tornar isso explícito, especifique essa opção. Por exemplo:
   ...
   Pacotes de sistema alternativos do OpenSSL são suportados usando `WITH_SSL=openssl11` no EL7 ou `WITH_SSL=openssl3` no EL8. Os plugins de autenticação, como LDAP e Kerberos, são desabilitados, pois não suportam essas versões alternativas do OpenSSL.
...
Para obter informações adicionais, consulte a Seção 8.3.1, “Configurando o MySQL para usar conexões criptografadas”.