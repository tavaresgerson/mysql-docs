...
As informações aqui aplicam-se principalmente se você instalou o MySQL usando a versão `noinstall`, ou se deseja configurar e testar o MySQL manualmente em vez de usar o MySQL Configurator.

Os exemplos nessas seções assumem que o MySQL está instalado na localização padrão de `C:\Program Files\MySQL\MySQL Server 9.5`. Ajuste os nomes de caminho mostrados nos exemplos se o MySQL estiver instalado em um local diferente.
...
Você deve ver mensagens semelhantes às seguintes ao iniciar o servidor (os nomes dos caminhos e tamanhos podem variar). As mensagens `pronto para conexões` indicam que o servidor está pronto para atender conexões de clientes.
...
Se você omitir a opção `--console`, o servidor escreverá a saída de diagnóstico no log de erro no diretório de dados (`C:\Program Files\MySQL\MySQL Server 9.5\data` por padrão). O log de erro é o arquivo com a extensão `.err` e pode ser configurado usando a opção `--log-error`.
