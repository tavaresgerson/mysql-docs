## 11.2 Objetos de Esquema Nomes

11.2.1 Limites de Comprimento dos Identificadores

11.2.2 Qualificadores de Identificadores

11.2.3 Sensibilidade ao Caso dos Identificadores

11.2.4 Mapeamento de Identificadores para Nomes de Arquivos

11.2.5 Análise e Resolução de Nomes de Funções

Certos objetos dentro do MySQL, incluindo banco de dados, tabela, índice, coluna, alias, visualização, procedimento armazenado, partição, espaço de tabelas, grupo de recursos e outros nomes de objetos são conhecidos como identificadores. Esta seção descreve a sintaxe permitida para identificadores no MySQL. A seção 11.2.1, “Limites de Comprimento dos Identificadores”, indica o comprimento máximo de cada tipo de identificador. A seção 11.2.3, “Sensibilidade ao Caso dos Identificadores”, descreve quais tipos de identificadores são sensíveis ao caso e sob quais condições.
...
Internacionalmente, os identificadores são convertidos e armazenados como Unicode (UTF-8). Os caracteres Unicode permitidos nos identificadores são aqueles na Planilha Multilíngue de Básico (BMP). Caracteres suplementares não são permitidos. Os identificadores podem, portanto, conter esses caracteres:
...
* O uso do sinal de dólar como o primeiro caractere no nome não citado de um banco de dados, tabela, visualização, coluna, programa armazenado ou alias é desaconselhado, incluindo tais nomes usados com qualificadores (veja a Seção 11.2.2, “Qualificadores de Identificador”). Um identificador não citado que começa com um sinal de dólar não pode conter quaisquer caracteres adicionais de sinal de dólar. Caso contrário, o sinal de dólar inicial é permitido, mas aciona uma mensagem de depreciação.

O sinal de dólar ainda pode ser usado como o caractere inicial de tal identificador sem produzir a mensagem de alerta, quando é citado de acordo com as regras dadas mais adiante nesta seção.
...
Em outras partes da declaração, as referências citadas ao alias devem usar aspas para identificar o nome ou a referência é tratada como uma literal de string.
