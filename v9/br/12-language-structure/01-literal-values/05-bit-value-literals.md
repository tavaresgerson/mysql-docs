...
Os literais de valor de bit são escritos usando a notação `b'val'` ou `0bval`. *`val`* é um valor binário escrito usando zeros e uns. A letra inicial de qualquer `b` não importa. Uma inicial `0b` é sensível a maiúsculas e minúsculas e não pode ser escrita como `0B`.
...
Um literal de valor de bit pode ter um introduzir opcional de conjunto de caracteres e cláusula `COLLATE`, para designá-lo como uma string que usa um conjunto de caracteres e ordenação particulares:
...
Para literais de bits, as operações de bits são consideradas contexto numérico, mas as operações de bits permitem argumentos de string numérica ou binária no MySQL 9.5 e versões posteriores. Para especificar explicitamente o contexto de string binária para literais de bits, use um introducer `_binary` para pelo menos um dos argumentos:
