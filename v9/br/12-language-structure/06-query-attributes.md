...
* Um aplicativo produz páginas que geram consultas de banco de dados, e para cada consulta, deve rastrear o URL da página que a gerou.
...
O MySQL suporta essas capacidades sem o uso de soluções alternativas, como comentários formatados especialmente incluídos nas strings de consulta. O restante desta seção descreve como usar o suporte a atributos de consulta, incluindo os pré-requisitos que devem ser atendidos.
...
* Se várias operações de definição de atributo ocorrerem antes de enviar uma instrução para o servidor para execução, a operação de definição mais recente se aplica e substitui os atributos definidos em operações anteriores.
...
* Os atributos não estão disponíveis para instruções preparadas com `PREPARE`.

* A função `mysql_query_attribute_string()` não pode ser usada em instruções DDL.

* Os atributos não são replicados. As instruções que invocam a função `mysql_query_attribute_string()` não obterão o mesmo valor em todos os servidores.

### Pré-requisitos para Usar Atributos de Consulta

Para acessar atributos de consulta dentro de instruções SQL para as quais atributos foram definidos, o componente `query_attributes` deve estar instalado. Faça isso usando a seguinte instrução:

```
INSTALL COMPONENT "file://component_query_attributes";
```

A instalação do componente é uma operação única que não precisa ser feita por inicialização de servidor. `INSTALL COMPONENT` carrega o componente e também o registra na tabela `mysql.component` do sistema para que ele seja carregado durante as inicializações subsequentes do servidor.
...
Para desinstalar o componente `query_attributes`, use a seguinte instrução:

```
UNINSTALL COMPONENT "file://component_query_attributes";
```

`UNINSTALL COMPONENT` desliga o componente e o desregistra da tabela `mysql.component` do sistema para que ele não seja carregado durante as inicializações subsequentes do servidor.
