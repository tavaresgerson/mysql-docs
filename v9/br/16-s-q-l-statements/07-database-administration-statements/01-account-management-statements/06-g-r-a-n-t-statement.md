...
Os caracteres curingas `_` e `%` são permitidos ao especificar nomes de banco de dados em declarações `GRANT` que concedem privilégios ao nível do banco de dados (`GRANT ... ON db_name.*`). Isso significa, por exemplo, que, para usar um caractere `_` como parte de um nome de banco de dados, especifique-o usando o caractere de escape `\` como `_` na declaração `GRANT`, para evitar que o usuário possa acessar bancos de dados adicionais que correspondam ao padrão de curinga (por exemplo, `GRANT ... ON `foo_bar`.* TO ...`).
...
<table><thead><tr> <th>Privilegio</th> <th>Significado e Níveis de Concedi­do</th> </tr></thead><tbody><tr> <td><code>ALL [PRIVILEGES]</code></td> <td>Concede todos os privilégios no nível de acesso especificado, exceto <code>GRANT OPTION</code> e <code>PROXY</code>.</td> </tr><tr> <td><code>ALTER</code></td> <td>Habilita o uso de <code>ALTER TABLE</code>. Níveis: Global, banco de dados, tabela.</td> </tr><tr> <td><code>ALTER ROUTINE</code></td> <td>Habilita a alteração ou remoção de rotinas armazenadas. Níveis: Global, banco de dados.</td> </tr><tr> <td><code>CREATE</code></td> <td>Habilita a criação de bancos de dados e tabelas. Níveis: Global, banco de dados, tabela.</td> </tr><tr> <td><code>CREATE ROLE</code></td> <td>Habilita a criação de um papel. Nível: Global.</td> </tr><tr> <td><code>CREATE ROUTINE</code></td> <td>Habilita a criação de rotinas armazenadas. Níveis: Global, banco de dados.</td> </tr><tr> <td><code>CREATE TABLESPACE</code></td> <td>Habilita a criação de espaços de tabelas e grupos de arquivos de log. Nível: Global.</td> </tr><tr> <td><code>CREATE TEMPORARY TABLES</code></td> <td>Habilita o uso de <code>CREATE TEMPORARY TABLE</code>. Níveis: Global, banco de dados.</td> </tr><tr> <td><code>CREATE USER</code></td> <td>Habilita o uso de <code>CREATE USER</code>, <code>DROP USER</code>, <code>RENAME USER</code>, e <code>REVOKE ALL PRIVILEGES</code>. Nível: Global.</td> </tr><tr> <td><code>CREATE VIEW</code></td> <td>Habilita a criação ou alteração de visualizações. Níveis: Global, banco de dados, tabela.</td> </tr><tr> <td><code>DELETE</code></td> <td>Habilita o uso de <code>DELETE</code>. N

**Tabela 15.12 Prerrogativas Dinâmicas Permitidas para GRANTE e REVOGAR**

<table><thead><tr> <th>Privilegio</th> <th>Significado e Níveis de Concedibilidade</th> </tr></thead><tbody><tr> <td><code>APPLICATION_PASSWORD_ADMIN</code></td> <td>Habilitar administração de senha dupla. Nível: Global.</td> </tr><tr> <td><code>AUDIT_ABORT_EXEMPT</code></td> <td>Permitir consultas bloqueadas pelo filtro do log de auditoria. Nível: Global.</td> </tr><tr> <td><code>AUDIT_ADMIN</code></td> <td>Habilitar a configuração do log de auditoria. Nível: Global.</td> </tr><tr> <td><code>AUTHENTICATION_POLICY_ADMIN</code></td> <td>Habilitar a administração da política de autenticação. Nível: Global.</td> </tr><tr> <td><code>BACKUP_ADMIN</code></td> <td>Habilitar a administração de backups. Nível: Global.</td> </tr><tr> <td><code>BINLOG_ADMIN</code></td> <td>Habilitar o controle do log binário. Nível: Global.</td> </tr><tr> <td><code>BINLOG_ENCRYPTION_ADMIN</code></td> <td>Habilitar a ativação e desativação da criptografia do log binário. Nível: Global.</td> </tr><tr> <td><code>CLONE_ADMIN</code></td> <td>Habilitar a administração de clones. Nível: Global.</td> </tr><tr> <td><code>CONNECTION_ADMIN</code></td> <td>Habilitar o controle do limite/restrição de conexões. Nível: Global.</td> </tr><tr> <td><code>ENCRYPTION_KEY_ADMIN</code></td> <td>Habilitar a rotação da chave <code>InnoDB</code>. Nível: Global.</td> </tr><tr> <td><code>FIREWALL_ADMIN</code></td> <td>Habilitar a administração das regras de firewall, qualquer usuário. Nível: Global.</td> </tr><tr> <td><code>FIREWALL_EXEMPT</code></td> <td>Eximir o usuário das restrições do firewall. Nível: Global.</td> </tr><tr> <td><code>FIREWALL_USER</code></td> <td>Habilitar a administração das regras de firewall, para o próprio usuário. Nível: Global.</td> </tr><tr> <td><code>FLUSH_OPTIMIZER_COSTS</code></td> <td>Habilitar a recarga dos custos do otimizador. Nível: Global.</td> </tr><tr> <td><code>FLUSH_STATUS</code></td> <td>Habilitar o envio do indicador de status. Nível: Global.</td> </tr><tr> <td><code>FLUSH_TABLES</code></td> <td>Habilitar o esvaziamento das tabelas. Nível: Global.</td> </tr><tr> <td><a class
