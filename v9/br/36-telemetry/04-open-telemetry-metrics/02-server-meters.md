...
Os medidores também podem ser configurados a partir da linha de comando, no início do servidor ou de um arquivo de configuração.

Para habilitar, ou desabilitar, um medidor, ou alterar a frequência do medidor, use a seguinte sintaxe de parâmetro de linha de comando:

```
        --performance-schema-meter='meterName=frequency:integer,enabled:ON|OFF'
```

O seguinte exemplo habilita o medidor `mysql.inno` e define a frequência para 50:
