...
Uma exceção se aplica ao arquivo de log de erro se você iniciar o **mysqld** usando **mysqld_safe**, que não respeita `UMASK`: **mysqld_safe** pode criar o arquivo de log de erro se ele não existir antes de iniciar o **mysqld**, e **mysqld_safe** usa um umask configurado para um valor rigoroso de `0137`. Se isso não for adequado, crie o arquivo de erro manualmente com o modo de acesso desejado antes de executar **mysqld_safe**.
