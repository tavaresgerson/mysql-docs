...
* Mesmo que você use `lower_case_table_names=2` (que permite que o MySQL lembre do caso usado para bancos de dados e nomes de tabelas), o MySQL não lembra do caso usado para nomes de banco de dados para a função `DATABASE()` ou dentro dos vários logs (em sistemas que não são sensíveis ao caso).
...
+ Instruções `CREATE TABLE ... SELECT` ou `INSERT ... SELECT` que inserem valores nulos ou `NULL` em uma coluna `AUTO_INCREMENT`.
...
A maneira mais fácil de evitar esse problema é adicionar uma cláusula `ORDER BY` às consultas não determinísticas mencionadas anteriormente para garantir que as linhas sejam sempre armazenadas ou modificadas na mesma ordem. O uso do formato de registro baseado em linhas ou misto também evita o problema.
...
* **mysqlbinlog** não exclui arquivos temporários deixados após uma instrução `LOAD DATA`. Veja a Seção 6.6.9, “mysqlbinlog — Ferramenta para Processar Arquivos de Log Binários”.
...
* Em uma declaração `UPDATE`, as colunas são atualizadas da esquerda para a direita. Se você se referir a uma coluna atualizada, você receberá o valor atualizado em vez do valor original. Por exemplo, a seguinte declaração incrementa `KEY` em `2`, **não** `1`:

  ```
  mysql> UPDATE tbl_name SET KEY=KEY+1,KEY=KEY+1;
  ```

* Você pode referenciar múltiplas tabelas temporárias na mesma consulta, mas não pode referenciar uma determinada tabela temporária mais de uma vez. Por exemplo, o seguinte não funciona:
...
Observe que isso acontece apenas para consultas que não têm as colunas `ORDER BY` no resultado.
...
* Uma declaração `UPDATE` que envolve uma tabela temporária com uma junção em uma tabela não temporária com um gatilho definido nela pode resultar em um erro, mesmo que a declaração de atualização leia apenas a tabela não temporária, nos seguintes casos:
