...
Os operadores aritméticos na seguinte expressão são usados para pesquisar países com um PIB per capita superior a $30000. As condições de busca podem incluir operadores aritméticos e a maioria das funções do MySQL.

Nota

Sete documentos na coleção `countryinfo` têm um valor de população de zero. Portanto, mensagens de aviso aparecem no final do resultado.
...
Para especificar o número de documentos incluídos em um conjunto de resultados, adicione o método `limit()` com um valor ao método `find()`. A consulta a seguir retorna os primeiros cinco documentos na coleção `countryinfo`.
...
Por exemplo, a consulta a seguir ordena todos os documentos pelo campo IndepYear e, em seguida, retorna os primeiros oito documentos em ordem decrescente.
...
* Consulte CollectionFindFunction para a definição completa da sintaxe.