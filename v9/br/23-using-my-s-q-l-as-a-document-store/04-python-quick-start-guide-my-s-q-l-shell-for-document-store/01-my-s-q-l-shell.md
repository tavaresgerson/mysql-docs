...
Este guia rápido assume um certo nível de familiaridade com o MySQL Shell. A seção a seguir é uma visão geral de alto nível, consulte a documentação do MySQL Shell para mais informações. O MySQL Shell é uma interface de script unificada para o MySQL Server. Ele suporta script em JavaScript e Python. O JavaScript é o modo de processamento padrão.
...
Uma vez que o MySQL Shell seja aberto, o prompt `mysql-js>` indica que o idioma ativo para esta sessão é JavaScript. Para alternar o MySQL Shell para o modo Python, use o comando `\py`.
...
* As teclas **seta para a esquerda** e **seta para a direita** movem-se horizontalmente dentro da linha de entrada atual.
...
Digite `\help` seguido do nome de um comando para obter ajuda detalhada sobre um comando individual do MySQL Shell. Por exemplo, para visualizar a ajuda sobre o comando `\connect`, execute:
...
* Veja Execução de código interativo para uma explicação sobre como a execução de código interativa funciona no MySQL Shell.
