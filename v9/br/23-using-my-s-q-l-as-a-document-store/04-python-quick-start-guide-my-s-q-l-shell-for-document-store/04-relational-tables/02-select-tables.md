...
Para especificar uma ordem dos resultados, adicione o método `order_by()` ao método `select()`. Passe ao método `order_by()` uma lista de uma ou mais colunas para ordenar e, opcionalmente, o atributo `desc` (descrescente) ou `asc` (crescente), conforme apropriado. A ordem crescente é o tipo de ordem padrão.
