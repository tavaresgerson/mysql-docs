...
Devido a esse comportamento, problemas podem ocorrer ocasionalmente quando não é fornecido um valor para uma opção que espera um. Considere o seguinte exemplo, onde um usuário se conecta a um servidor MySQL que está em execução no host `tonfisk` como usuário `jon`:
...
Neste caso, o **mysql** não conseguiu encontrar um valor seguindo a opção `--user` porque nada veio depois dela na linha de comando. No entanto, se você omitir o valor para uma opção que *não* é a última opção a ser usada, você obtém um erro diferente que você pode não estar esperando:
...
O servidor tentou iniciar usando `/usr/local/mysql/var/tonfisk.err` como o log de erro, mas então desligou. Examinar as últimas linhas desse arquivo mostra a razão:
...
Quando o cliente **mysql** lê esse arquivo, essas entradas são parseadas como `--host` `--user` ou `--host=--user`, com o resultado mostrado aqui:
...
Especificar uma opção que requer um valor sem um valor em um arquivo de opções faz com que o servidor abortem com um erro.