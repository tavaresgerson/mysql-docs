...
Especifique `--verbose` ou `-v` duas vezes para exibir também os tipos de dados e alguns metadados para cada coluna, e eventos de log informativos, como eventos de log de consulta de linha de registro, se a variável de sistema `binlog_rows_query_log_events` estiver definida como `TRUE`. A saída contém um comentário adicional após cada mudança de coluna:
...
Você não deve suprimir as declarações `BINLOG` se pretende reexecutar a saída do **mysqlbinlog**.

As declarações SQL produzidas pelo `--verbose` para eventos de linha são muito mais legíveis do que as declarações `BINLOG` correspondentes. No entanto, elas não correspondem exatamente às declarações SQL originais que geraram os eventos. As seguintes limitações se aplicam:

* Os nomes originais das colunas são perdidos e substituídos por `@N`, onde *`N`* é um número de coluna.

* A informação sobre o conjunto de caracteres não está disponível no log binário, o que afeta a exibição das colunas de texto:
...
A interpretação adequada dos eventos de linha requer as informações da descrição do formato no início do log binário. Como o **mysqlbinlog** não sabe antecipadamente se o resto do log contém eventos de linha, por padrão, ele exibe o evento de descrição de formato usando uma declaração `BINLOG` na parte inicial da saída.

Se o log binário não for conhecido por conter eventos que exijam uma declaração `BINLOG` (ou seja, sem eventos de linha), a opção `--base64-output=NEVER` pode ser usada para evitar que esse cabeçalho seja escrito.