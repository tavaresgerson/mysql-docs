...
Fazer backup e restaurar bancos de dados de um Clã NDB pode ser feito usando a funcionalidade nativa do `NDB` encontrada no cliente de gerenciamento do Clã NDB e no programa **ndb_restore** incluído na distribuição do Clã NDB. Para mais informações, veja a Seção 25.6.8, “Backup Online do Clã NDB”, e a Seção 25.5.23, “ndb_restore — Restaurar um Backup de um Clã NDB”. Você também pode usar a funcionalidade padrão do MySQL fornecida para esse propósito no **mysqldump** e no servidor MySQL. Veja a Seção 6.5.4, “mysqldump — Um Programa de Backup de Banco de Dados”, para mais informações.
