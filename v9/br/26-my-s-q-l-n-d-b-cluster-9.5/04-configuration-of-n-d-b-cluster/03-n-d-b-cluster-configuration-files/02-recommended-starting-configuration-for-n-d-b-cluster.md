...
**Opções my.cnf requeridas para nós SQL.** Servidores MySQL que atuam como nós SQL do NDB Cluster devem sempre ser iniciados com as opções `--ndbcluster` e `--ndb-connectstring`, seja na linha de comando ou no `my.cnf`.