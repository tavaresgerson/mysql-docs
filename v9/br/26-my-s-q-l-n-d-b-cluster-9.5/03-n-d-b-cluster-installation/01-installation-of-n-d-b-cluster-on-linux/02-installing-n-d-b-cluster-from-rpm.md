...
<table><col style="width: 50%"/><col style="width: 50%"/><thead><tr> <th>Componente</th> <th>Descrição</th> </tr></thead><tbody><tr> <td><code class="literal">auto-installer</code> (DESAPARECIDO)</td> <td>Programa de Auto-Instalador do NDB Cluster; consulte <a class="ulink" href="/doc/refman/8.0/pt-BR/mysql-cluster-installer.html" target="_blank">O Auto-Instalador do NDB Cluster (NÃO MAIS APOIADO)</a>, para uso</td> </tr><tr> <td><code class="literal">client</code></td> <td>Programas de cliente MySQL e <code class="literal">NDB</code>; inclui o cliente <a class="link" href="mysql.html" title="6.5.1 mysql — O Cliente de Linha de Comando MySQL"><span class="command"><strong>mysql</strong></span></a>, o cliente <a class="link" href="mysql-cluster-programs-ndb-mgm.html" title="25.5.5 ndb_mgm — O Cliente de Gerenciamento do NDB Cluster"><span class="command"><strong>ndb_mgm</strong></span></a> e outras ferramentas de cliente</td> </tr><tr> <td><code class="literal">common</code></td> <td>Informações sobre o conjunto de caracteres e mensagens de erro necessárias pelo servidor MySQL</td> </tr><tr> <td><code class="literal">data-node</code></td> <td>Binários do nó de dados do NDB Cluster (<a class="link" href="mysql-cluster-programs-ndbd.html" title="25.5.1 ndbd — O Daemon do Nó de Dados do NDB Cluster"><span class="command"><strong>ndbd</strong></span></a>) e <a class="link" href="mysql-cluster-programs-ndbmtd.html" title="25.5.3 ndbmtd — O Daemon de Nó de Dados do NDB Cluster (Multi-Thread)"><span class="command"><strong>ndbmtd</strong></span></a> e outros binários de nó de dados</td> </tr><tr> <td><code class="literal">devel</code></td> <td>Cabeçalhos e arquivos de biblioteca necessários para o desenvolvimento de aplicativos do cliente MySQL</td> </tr><tr> <td><code class="literal">embedded</code></td> <td>Servidor MySQL embutido</td> </tr><tr> <td><code class="literal">embedded-compat</code></td> <td>Servidor MySQL embutido compatível com versões anteriores</td> </tr><tr> <td><code class="literal">embedded-devel</code></td> <td>Cabeçalhos e outros arquivos necessários para o desenvolvimento de aplicativos para servidores MySQL embutidos</td> </tr><tr> <td><code class="literal">java</code></td> <td>Arquivos JAR necessários para o suporte a aplicativos ClusterJ</td> </tr><tr> <td><code class="literal">libs</code></td> <td>Bibliotecas de cliente MySQL</td> </tr><tr> <td><code class="literal">libs-compat</code></td> <td>Bibliotecas de cliente MySQL compatíveis com versões anteriores</td> </tr><tr> <td><code class="literal">management-server</code></td> <td>O servidor de gerenciamento do NDB Cluster (<a class="link" href="mysql-cluster-programs-ndb-mgmd.html" title="25.5.4 ndb_mgmd — O Daemon do Servidor de Gerenciamento do NDB Cluster"><span class="command"><strong>ndb_mgmd</strong></span></a>)</td> </tr><tr> <td><code class="literal">memcached</code></td> <td>Arquivos necessários para suportar o <code class="literal">ndbmemcache</code></td> </tr><tr> <td><code class="literal">minimal-debuginfo</code></td> <td>Informações de depuração para o pacote server-minimal; útil ao desenvolver aplicativos que utilizam este pacote ou ao depurar este pacote</td> </tr><tr> <td><code class="literal">ndbclient</code></td> <td>Biblioteca de cliente <code class="literal">NDB</code> para executar aplicativos da API e API MGM do NDB (<code class="literal">libndbclient</code>))</td> </tr><tr> <td><code class="literal">ndbclient-devel</code></td> <td>Cabeçalhos e outros arquivos necessários para o desenvolvimento de aplicativos da API e API MGM do NDB</td> </tr><tr> <td><code class="literal">server</code></td> <td>O servidor MySQL (<a class="link" href="mysqld.html" title="6.3.1 mysqld — O Servidor MySQL"><span class="command"><strong>mysqld</strong></span></a>) com suporte ao mecanismo de armazenamento <code class="literal">NDB</code> incluído, e os programas de
...
Além disso, o RPM `client` deve ser instalado para fornecer o cliente de gerenciamento **ndb\_mgm** em pelo menos um nó de gerenciamento. Você também pode querer instalá-lo nos nós SQL, para ter **mysql** e outros programas de cliente MySQL disponíveis nesses. Discutimos a instalação dos nós por tipo mais adiante nesta seção.
...
Isso instala o binário do servidor MySQL (**mysqld**), com suporte ao mecanismo de armazenamento **NDB**, no diretório `/usr/sbin`. Ele também instala todos os arquivos de suporte necessários do servidor MySQL e programas úteis do servidor MySQL, incluindo os scripts de inicialização **mysql.server** e **mysqld\_safe** (em `/usr/share/mysql` e `/usr/bin`, respectivamente). O instalador RPM deve cuidar automaticamente de questões de configuração geral (como a criação do usuário e grupo `mysql`, se necessário).
...
Isso instala o binário do servidor MySQL (**mysqld**) com suporte ao mecanismo de armazenamento **NDB** no diretório `/usr/sbin`, além de todos os arquivos de suporte necessários ao MySQL Server. Também instala os scripts de inicialização **mysql.server** e **mysqld\_safe** (em `/usr/share/mysql` e `/usr/bin`, respectivamente). O instalador RPM deve cuidar automaticamente de questões de configuração geral (como a criação do usuário e grupo `mysql`, se necessário).
...
Embora este RPM instale muitos outros arquivos, apenas o binário do servidor de gerenciamento **ndb\_mgmd** (no diretório `/usr/sbin`) é realmente necessário para rodar um nó de gerenciamento. O RPM `server` também instala **ndb\_mgm**, o cliente de gerenciamento **NDB**.
