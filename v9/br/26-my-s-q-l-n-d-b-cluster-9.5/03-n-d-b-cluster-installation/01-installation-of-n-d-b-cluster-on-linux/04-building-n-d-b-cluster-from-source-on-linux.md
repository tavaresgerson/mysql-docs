...
**Nodos de gerenciamento.** Ao compilar a partir da fonte e executar o **make install** padrão, os binários do servidor de gerenciamento e do cliente de gerenciamento (**ndb\_mgmd** e **ndb\_mgm**) podem ser encontrados em `/usr/local/mysql/bin`. Apenas o **ndb\_mgmd** deve estar presente no host do nó de gerenciamento; no entanto, também é uma boa ideia ter o **ndb\_mgm** presente na mesma máquina. Nenhum desses executáveis requer uma localização específica no sistema de arquivos da máquina host.
