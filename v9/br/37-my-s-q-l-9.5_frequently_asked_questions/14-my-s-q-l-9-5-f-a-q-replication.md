    ...
    <div class="col-md-8">
      ...
      <ul>
        ...
        <li><strong>A.14.6</strong>: Como posso usar a replicação para melhorar o desempenho do meu sistema?
          <p> Configure uma fonte como fonte e direcione todos os escritos para ela. Em seguida, configure tantas réplicas quanto o orçamento e os racks permitirem, e distribua as leituras entre a fonte e as réplicas. Você também pode iniciar as réplicas com a opção <a class="ulink" href="/doc/refman/8.0/en/innodb-parameters.html#option_mysqld_innodb" target="_top"><code>--skip-innodb</code></a> na opção, habilitar a variável de sistema <a class="link" href="server-system-variables.html#sysvar_low_priority_updates"><code>low_priority_updates</code></a> e definir a variável de sistema <a class="link" href="server-system-variables.html#sysvar_delay_key_write"><code>delay_key_write</code></a> para <code>ALL</code> para obter melhorias de desempenho na replica no lado do cliente. Nesse caso, a replica usa tabelas <code>MyISAM</code> não transactionais em vez de tabelas <code>InnoDB</code> para obter mais velocidade ao eliminar o overhead transactionário. </p>
