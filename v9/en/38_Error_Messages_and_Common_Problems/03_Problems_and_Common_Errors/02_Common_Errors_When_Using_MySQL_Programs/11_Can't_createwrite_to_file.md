...
One fix is to start [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") with the
[`--tmpdir`](server-options.html#option_mysqld_tmpdir) option or to add the
...
[`tmpdir`](server-system-variables.html#sysvar_tmpdir) directory.

Check also the error code that you get with
[**perror**](perror.html "6.8.1 perror — Display MySQL Error Message Information"). One reason the server cannot write
