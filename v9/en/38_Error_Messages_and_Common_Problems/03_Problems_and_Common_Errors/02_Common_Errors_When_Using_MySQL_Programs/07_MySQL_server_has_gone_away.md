...
<table summary="MySQL server has gone away error codes and a description of each code."><col style="width: 35%"/><col style="width: 65%"/><thead><tr>
<th>Error Code</th>
<th>Description</th>
</tr></thead><tbody><tr>
<td><a class="ulink" href="/doc/mysql-errors/9.5/en/client-error-reference.html#error_cr_server_gone_error" target="_top"><code class="literal">CR_SERVER_GONE_ERROR</code></a></td>
<td>The client couldn't send a question to the server.</td>
</tr><tr>
<td><a class="ulink" href="/doc/mysql-errors/9.5/en/client-error-reference.html#error_cr_server_lost" target="_top"><code class="literal">CR_SERVER_LOST</code></a></td>
...
setting the [`wait_timeout`](server-system-variables.html#sysvar_wait_timeout)
variable when you start [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"). See
...
* You (or the db administrator) has killed the running
  thread with a [`KILL`](kill.html "15.7.8.4 KILL Statement")
  statement or a [**mysqladmin kill**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") command.
...
* You got a timeout from the TCP/IP connection on the client
  ...
  [`mysql_options(...,
  MYSQL_OPT_READ_TIMEOUT,...)`](/doc/c-api/9.5/en/mysql-options.html) or
  [`mysql_options(...,
  MYSQL_OPT_WRITE_TIMEOUT,...)`](/doc/c-api/9.5/en/mysql-options.html). In this case
...
* You are using a Windows client and the server had dropped
  ...
  [`wait_timeout`](server-system-variables.html#sysvar_wait_timeout) expired)
  ...
  [`mysql_ping()`](/doc/c-api/9.5/en/mysql-ping.html) on the
  connection if there has been a long time since the last
  query (this is what Connector/ODBC does) or set
  [`wait_timeout`](server-system-variables.html#sysvar_wait_timeout) on the
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") server so high that it in
  practice never times out.

* You can also get these errors if you send a query to the
  server that is incorrect or too large. If
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") receives a packet that is too
  ...
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") columns), you can
  ...
  in [Section B.3.2.8, “Packet Too Large”](packet-too-large.html "B.3.2.8 Packet Too Large").

  An [`INSERT`](insert.html "15.2.7 INSERT Statement") or
  [`REPLACE`](replace.html "15.2.12 REPLACE Statement") statement that
...
executing [**mysqladmin version**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") and examining
the server's uptime. If the client connection was broken
because [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") crashed and restarted, you
should concentrate on finding the reason for the crash. Start
by checking whether issuing the query again kills the server
again. See [Section B.3.3.3, “What to Do If MySQL Keeps Crashing”](crashing.html "B.3.3.3 What to Do If MySQL Keeps Crashing").

You can obtain more information about lost connections by
starting [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") with the
...
* If a specific query kills [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") and
  the tables involved were checked with
  [`CHECK TABLE`](check-table.html "15.7.3.2 CHECK TABLE Statement") before you ran
...
* What is the value of the
  [`wait_timeout`](server-system-variables.html#sysvar_wait_timeout) system
  variable in the MySQL server? ([**mysqladmin
  variables**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") gives you the value of this variable.)

* Have you tried to run [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") with the
...
See also [Section B.3.2.9, “Communication Errors and Aborted Connections”](communication-errors.html "B.3.2.9 Communication Errors and Aborted Connections"), and
