...
change it, see [Section 15.7.1.1, “ALTER USER Statement”](alter-user.html "15.7.1.1 ALTER USER Statement"), and
...
2. Stop the MySQL server if it is running. Locate the
   ...
   `kill` (not `kill -9`)
   to the [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") process. Use the actual
...
password using the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client (but this
...
1. Stop the MySQL server if necessary, then restart it with
   ...
   [`SET PASSWORD`](set-password.html "15.7.1.10 SET PASSWORD Statement"). Because
   ...
   [`skip_networking`](server-system-variables.html#sysvar_skip_networking). On
...
2. Connect to the MySQL server using the
   [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client; no password is
