...
Error messages can originate on the server side or the client side, and each error message includes an error code, SQLSTATE value, and message string, as described in Section B.1, “Error Message Sources and Elements”. For lists of server-side, client-side, and global (shared between server and clients) errors, see MySQL 9.5 Error Message Reference.
...
* [`SHOW ENGINE INNODB STATUS`](show-engine.html "15.7.7.17 SHOW ENGINE Statement") statement output includes information about the most recent foreign key error if a `CREATE TABLE` statement for an `InnoDB` table fails.
...
For descriptions of the client library error functions, see MySQL 9.5 C API Developer Guide.
