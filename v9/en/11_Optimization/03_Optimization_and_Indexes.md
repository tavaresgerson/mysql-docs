...
* They exist only for backward compatibility (for example, the ability to perform a dump in MySQL 8.4 and restore in MySQL 9.4). Because `SPATIAL` indexes on columns that are not SRID-restricted are of no use to the optimizer, each such column should be modified:
...
For additional information about column indexes, see Section 15.1.18, “CREATE INDEX Statement”.
...
For additional information about index prefixes, see Section 15.1.18, “CREATE INDEX Statement”.
...
+ 1` with the name of the generated column that matches the expression. That is also apparent in the rewritten query available in the extended `EXPLAIN` information displayed by [`SHOW WARNINGS`](show-warnings.html "15.7.7.43 SHOW WARNINGS Statement"):
