...
[Section 13.4.10, “Creating Spatial Indexes”](creating-spatial-indexes.html "13.4.10 Creating Spatial Indexes")). The optimizer
...
(Prior to MySQL 9.5, the optimizer performs
...
* They are updated for table modifications
  ...
  [`DELETE`](delete.html "15.2.2 DELETE Statement"), and so forth).
...
* They exist only for backward compatibility (for example, the
  ability to perform a dump in MySQL
  8.4 and restore in MySQL
  9.4). Because `SPATIAL`
  indexes on columns that are not SRID-restricted are of no
  use to the optimizer, each such column should be modified:
