...
* All threads share the [`MyISAM`](myisam-storage-engine.html "18.2 The MyISAM Storage Engine")
  key buffer. The
  ...
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") columns). A
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") column requires five
  to eight bytes plus the length of the
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") data. The
...
* If an internal in-memory temporary table becomes too large
  ...
  storage engine. Reaching this limit also increments
  ...
  described in [Section 10.4.4, “Internal Temporary Table Use in MySQL”](internal-temporary-tables.html "10.4.4 Internal Temporary Table Use in MySQL").

  For [`MEMORY`](memory-storage-engine.html "18.3 The MEMORY Storage Engine") tables explicitly
  created with [`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement"),
  only the
  [`max_heap_table_size`](server-system-variables.html#sysvar_max_heap_table_size)
...
* The [MySQL Performance
  Schema](performance-schema.html "Chapter 29 MySQL Performance Schema") is a feature for monitoring MySQL server
  ...
  [Section 29.17, “The Performance Schema Memory-Allocation Model”](performance-schema-memory-model.html "29.17 The Performance Schema Memory-Allocation Model").
...
* All joins are executed in a single pass, and most joins
  ...
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") columns are stored on
  disk.
...
* For each table having [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types")
  columns, a buffer is enlarged dynamically to read in
  larger [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") values. If you
  scan a table, the buffer grows as large as the largest
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") value.
...
* A [`FLUSH TABLES`](flush.html#flush-tables) statement or
  [**mysqladmin flush-tables**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") command closes
...
* In a replication topology, the following settings affect
  ...
  + The [`rpl_read_size`](replication-options-replica.html#sysvar_rpl_read_size)
...
report that [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") uses a lot of memory.
...
`swap -s`. We test [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
