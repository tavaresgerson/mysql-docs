...
[Section 17.16, “InnoDB Integration with MySQL Performance Schema”](innodb-performance-schema.html "17.16 InnoDB Integration with MySQL Performance Schema") for information
...
* Controlling the types of data change operations for which
  ...
  [Configuring Change Buffering](innodb-change-buffer.html#innodb-change-buffer-configuration "Configuring Change Buffering").
  Enabling change buffering can give better performance on
  IO-bound workloads, but can cause issues during recovery,
  bulk load, or during buffer pool resizing. Having it
  disabled (default as of MySQL 8.4) helps ensure stability
  even if it may lower performance.
...
* Turning the adaptive hash indexing feature on and off using
  ...
  option. See [Section 17.5.3, “Adaptive Hash Index”](innodb-adaptive-hash.html "17.5.3 Adaptive Hash Index") for more
...
* Controlling how much I/O `InnoDB` performs
  ...
  [Section 17.8.7, “Configuring InnoDB I/O Capacity”](innodb-configuring-io-capacity.html "17.8.7 Configuring InnoDB I/O Capacity"). You might
...
* Controlling the algorithm that determines when
  ...
  [Section 17.8.3.5, “Configuring Buffer Pool Flushing”](innodb-buffer-pool-flushing.html "17.8.3.5 Configuring Buffer Pool Flushing"). The algorithm
...
* Adjusting log files to a size that makes sense for
  ...
  [recovery](glossary.html#glos_crash_recovery "crash recovery") process.
  In particular, scanning the
  [redo log](glossary.html#glos_redo_log "redo log") and applying
...
* Increasing the maximum number of concurrent transactions,
  ...
  databases. See [Section 17.6.6, “Undo Logs”](innodb-undo-logs.html "17.6.6 Undo Logs").

* Moving purge operations (a type of garbage collection) into
  a background thread. See
  [Section 17.8.9, “Purge Configuration”](innodb-purge-configuration.html "17.8.9 Purge Configuration"). To effectively
