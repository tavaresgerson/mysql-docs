...
TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") statement.
...
[`COLUMN_STATISTICS`](information-schema-column-statistics-table.html "28.3.11 The INFORMATION_SCHEMA COLUMN_STATISTICS Table") has these
columns:
...
* `HISTOGRAM`: A
  [`JSON`](json.html "13.5 The JSON Data Type") value describing the
...
[`JSON`](json.html "13.5 The JSON Data Type") objects to permit
...
* `histogram-type`: The histogram type:
  ...
  + `singleton`: One bucket represents one
    ...
    in the [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement")
...
* `number-of-buckets-specified`: The number
  ...
  TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") statement that generated the histogram.
...
* `collation-id`: The collation ID for the
  ...
  Schema [`COLLATIONS`](information-schema-collations-table.html "28.3.6 The INFORMATION_SCHEMA COLLATIONS Table") table.

To extract particular values from the histogram objects, you can
use [`JSON`](json.html "13.5 The JSON Data Type") operations. For example:
...
[`ABS()`](mathematical-functions.html#function_abs) and
[`FLOOR()`](mathematical-functions.html#function_floor):
...
[Section 10.2.1.2, “Range Optimization”](range-optimization.html "10.2.1.2 Range Optimization")). In this case, histogram
...
TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") to regenerate the histogram statistics, then run
the query again.

Alternatively, to disable histogram statistics, use
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") to drop them. A
different method of disabling histogram statistics is to turn
off the
[`condition_fanout_filter`](switchable-optimizations.html#optflag_condition-fanout-filter) flag
...
visible using [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement"). Consider
...
of an index, and [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") shows
