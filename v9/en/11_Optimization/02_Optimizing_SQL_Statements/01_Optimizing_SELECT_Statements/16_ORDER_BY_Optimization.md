...
Previously (MySQL 9.4 and lower),
`GROUP BY` sorted implicitly under certain
conditions. In MySQL 9.5, that no longer
...
[`sort_buffer_size`](server-system-variables.html#sysvar_sort_buffer_size) to larger
...
as necessary, up to `sort_buffer_size`
bytes.)
