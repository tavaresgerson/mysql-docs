...
MySQL 9.5 supports transformation of general
...
IN`](comparison-operators.html#operator_not-in). See [Section 15.2.15.3, “Subqueries with ANY, IN, or SOME”](any-in-some-subqueries.html "15.2.15.3 Subqueries with ANY, IN, or SOME"),
and [Section 15.2.15.4, “Subqueries with ALL”](all-subqueries.html "15.2.15.4 Subqueries with ALL"), for more information.)

A subquery which qualifies for the transformation is converted
to a derived table as follows:

* For `=ANY` and
  `<>ALL` predicates:

  1. The subquery is converted into a derived table.
  2. The selected expression from the subquery is converted
     from *`expr`* to
     [`MIN(expr)`](aggregate-functions.html#function_min)
     or
     [`MAX(expr)`](aggregate-functions.html#function_max),
     depending on the actual predicate.

  3. Select [`COUNT(*)`](aggregate-functions.html#function_count) from
     the subquery.
...
* For other predicates:
  ...
  1. The subquery is converted into a derived table.
  2. The selected expression from the subquery is selected
     from the derived table.

  3. Unless the selected expression is distinct, duplicate
     elimination is performed.

  4. If the selected expression is nullable, and processing
     of nulls is necessary, select an indication of whether
     the subquery contains any nulls.

Note

When the [`semijoin`](switchable-optimizations.html#optflag_semijoin)
...
The outer query block must contain at least one table, and may
not contain more than 60 tables.
...
The query expression for the subquery cannot contain any of
the following:

* A set operation (any of
  [`UNION`](union.html "15.2.18 UNION Clause"),
  [`INTERSECT`](intersect.html "15.2.8 INTERSECT Clause"), or
  [`EXCEPT`](except.html "15.2.4 EXCEPT Clause")).

* An explicit grouping, except when the resolver is able to
  eliminate grouping.
...
conditions with enclosing [`AND`](logical-operators.html#operator_and)
operators.
