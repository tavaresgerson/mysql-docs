...
* [Table Columns](data-size.html#data-size-table-columns "Table Columns")
* [Row Format](data-size.html#data-size-row-format "Row Format")
* [Indexes](data-size.html#data-size-indexes "Indexes")
* [Joins](data-size.html#data-size-joins "Joins")
* [Normalization](data-size.html#data-size-normalization "Normalization")
...
* Use the most efficient (smallest) data types possible.
  ...
  [`MEDIUMINT`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") is often a better
  choice than [`INT`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") because a
  [`MEDIUMINT`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") column uses 25%
  less space.
...
* `InnoDB` tables are created using the
  ...
  [`CHAR`](char.html "13.3.2 The CHAR and VARCHAR Types") column storage when
...
* To minimize space even further by storing table data in
  ...
  [**myisampack**](myisampack.html "6.6.6 myisampack — Generate Compressed, Read-Only MyISAM Tables") command on an existing
  `MyISAM` table.
...
* For `MyISAM` tables, if you do not have
  ...
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") columns), a fixed-size
  row format is used. This is faster but may waste some
  space. See [Section 18.2.3, “MyISAM Table Storage Formats”](myisam-table-formats.html "18.2.3 MyISAM Table Storage Formats"). You can
  hint that you want to have fixed length rows even if you
  have [`VARCHAR`](char.html "13.3.2 The CHAR and VARCHAR Types") columns with
  the [`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") option
  `ROW_FORMAT=FIXED`.
...
* If it is very likely that a long string column has a
  ...
  column (see [Section 15.1.18, “CREATE INDEX Statement”](create-index.html "15.1.18 CREATE INDEX Statement")). Shorter
