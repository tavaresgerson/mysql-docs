...
* Evaluation of [`UNION`](union.html "15.2.18 UNION Clause")
  statements, with some exceptions described later.

* Evaluation of some views, such those that use the
  `TEMPTABLE` algorithm,
  [`UNION`](union.html "15.2.18 UNION Clause"), or aggregation.
...
* To evaluate
  [`INSERT ...
  SELECT`](insert-select.html "15.2.7.1 INSERT ... SELECT Statement") statements that select from and insert into
  the same table, MySQL creates an internal temporary table to
  hold the rows from the
  [`SELECT`](select.html "15.2.13 SELECT Statement"), then inserts those
...
* Evaluation of window functions (see
  [Section 14.20, “Window Functions”](window-functions.html "14.20 Window Functions")) uses temporary tables as
  necessary.

To determine whether a statement requires a temporary table, use
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") and check the
`Extra` column to see whether it says
`Using temporary` (see
[Section 10.8.1, “Optimizing Queries with EXPLAIN”](using-explain.html "10.8.1 Optimizing Queries with EXPLAIN")). `EXPLAIN`
does not necessarily say `Using temporary` for
derived or materialized temporary tables. For statements that
use window functions, [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement")
...
* Presence of a [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") or
  [`TEXT`](blob.html "13.3.4 The BLOB and TEXT Types") column in the table. The
  `TempTable` storage engine, which is the
  default storage engine for in-memory internal temporary
  tables in MySQL 9.5, supports binary large
  object types. See
...
* Presence of any string column with a maximum length larger
  ...
  strings) in the [`SELECT`](select.html "15.2.13 SELECT Statement") list,
  if [`UNION`](union.html "15.2.18 UNION Clause") or
  [`UNION ALL`](union.html "15.2.18 UNION Clause")
  is used.

* The [`SHOW COLUMNS`](show-columns.html "15.7.7.6 SHOW COLUMNS Statement") and
  [`DESCRIBE`](describe.html "15.8.1 DESCRIBE Statement") statements use
...
[`UNION`](union.html "15.2.18 UNION Clause") statements that meet
...
executed. [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") and optimizer
...
efficient storage for [`VARCHAR`](char.html "13.3.2 The CHAR and VARCHAR Types")
and [`VARBINARY`](binary-varbinary.html "13.3.3 The BINARY and VARBINARY Types") columns, and
other binary large object types.

The following variables control `TempTable`
storage engine limits and behavior:

* [`tmp_table_size`](server-system-variables.html#sysvar_tmp_table_size): Defines
  the maximum size of any individual in-memory internal
  temporary table created using the
  `TempTable` storage engine. When the
  limit determined by
  [`tmp_table_size`](server-system-variables.html#sysvar_tmp_table_size) is
  reached, MySQL automatically converts the in-memory
  internal temporary table to an `InnoDB`
  on-disk internal temporary table. The default value is
  16777216 bytes (16 MiB).

  The [`tmp_table_size`](server-system-variables.html#sysvar_tmp_table_size) limit
  is intended to prevent individual queries from consuming
  an inordinate amount of global
  `TempTable` resources, which can affect
  the performance of concurrent queries that require such
  resources. Global `TempTable` resources
  are controlled by
  [`temptable_max_ram`](server-system-variables.html#sysvar_temptable_max_ram) and
  [`temptable_max_mmap`](server-system-variables.html#sysvar_temptable_max_mmap).

  If [`tmp_table_size`](server-system-variables.html#sysvar_tmp_table_size) is less
  than [`temptable_max_ram`](server-system-variables.html#sysvar_temptable_max_ram),
  it is not possible for an in-memory temporary table to use
  more than [`tmp_table_size`](server-system-variables.html#sysvar_tmp_table_size).
  If [`tmp_table_size`](server-system-variables.html#sysvar_tmp_table_size) is
  greater than the sum of
  [`temptable_max_ram`](server-system-variables.html#sysvar_temptable_max_ram) and
  [`temptable_max_mmap`](server-system-variables.html#sysvar_temptable_max_mmap), an
  in-memory temporary table cannot use more than the sum of
  the [`temptable_max_ram`](server-system-variables.html#sysvar_temptable_max_ram) and
  [`temptable_max_mmap`](server-system-variables.html#sysvar_temptable_max_mmap)
  limits.
...
* [`temptable_max_ram`](server-system-variables.html#sysvar_temptable_max_ram):
  ...
  [`temptable_max_ram`](server-system-variables.html#sysvar_temptable_max_ram) is 3%
  of the total memory available on the server, with a
  minimum of 1 GB and a maximum of 4 GB.

  Note

  [`temptable_max_ram`](server-system-variables.html#sysvar_temptable_max_ram) does
...
* [`temptable_max_mmap`](server-system-variables.html#sysvar_temptable_max_mmap): Sets
  the maximum amount of memory the
  `TempTable` storage engine is permitted
  ...
  directory ([`tmpdir`](server-system-variables.html#sysvar_tmpdir)).
  `temptable_max_mmap = 0` disables
...
* Temporary files are created in the directory defined by
  the [`tmpdir`](server-system-variables.html#sysvar_tmpdir) variable.

* Temporary files are deleted immediately after they are
  created and opened, and therefore do not remain visible in
  the [`tmpdir`](server-system-variables.html#sysvar_tmpdir) directory. The
...
([`internal_tmp_mem_storage_engine=MEMORY`](server-system-variables.html#sysvar_internal_tmp_mem_storage_engine)),
...
[`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement"). For such tables,
...
MySQL 9.5 uses only the
[`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") storage engine for on-disk
internal temporary tables. (The
[`MYISAM`](myisam-storage-engine.html "18.2 The MyISAM Storage Engine") storage engine is no
...
`TempTable` storage engine allocates space
...
in [Internal Temporary Table Storage Engine](internal-temporary-tables.html#internal-temporary-tables-engines "Internal Temporary Table Storage Engine").
...
When an internal temporary table is converted from in-memory
to on-disk, the server increments system status variables to
track these changes:
...
* [`TempTable_count_hit_max_ram`](server-status-variables.html#statvar_TempTable_count_hit_max_ram)
  ...
  is reached. This is specific to the
  `TempTable` storage engine, and is a
  global status variable.

* [`Count_hit_tmp_table_size`](server-status-variables.html#statvar_Count_hit_tmp_table_size)
  is incremented under these conditions:

  + `TempTable` storage engine: if the
    [`tmp_table_size`](server-system-variables.html#sysvar_tmp_table_size) limit
    is reached.

  + `MEMORY` storage engine: if the
    smaller limit value of
    [`tmp_table_size`](server-system-variables.html#sysvar_tmp_table_size) or
