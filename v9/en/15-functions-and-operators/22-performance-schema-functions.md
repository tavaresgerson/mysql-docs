...
<table frame="box" rules="all" summary="A reference that lists Performance Schema functions."><col style="width: 28%"/><col style="width: 71%"/><thead><tr><th>Name</th> <th>Description</th> </tr></thead><tbody><tr><td><code class="literal">FORMAT_BYTES()</code></td> <td> Convert byte count to value with units </td> </tr><tr><td><code class="literal">FORMAT_PICO_TIME()</code></td> <td> Convert time in picoseconds to value with units </td> </tr><tr><td><code class="literal">PS_CURRENT_THREAD_ID()</code></td> <td> Performance Schema thread ID for current thread </td> </tr><tr><td><code class="literal">PS_THREAD_ID()</code></td> <td> Performance Schema thread ID for given thread </td> </tr></tbody></table>
