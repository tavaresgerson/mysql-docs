...
<table frame="box" rules="all" summary="A reference that lists vector functions."><col style="width: 28%"/><col style="width: 71%"/><thead><tr><th>Name</th> <th>Description</th> </tr></thead><tbody><tr><td><code class="literal">DISTANCE()</code></td> <td> Calculates the distance between two vectors per the specified method </td> </tr><tr><td><code class="literal">STRING_TO_VECTOR()</code></td> <td> Get the binary value of the VECTOR column represented by a conforming string </td> </tr><tr><td><code class="literal">VECTOR_DIM()</code></td> <td> Get the length of a vector (that is, the number of entries it contains) </td> </tr><tr><td><code class="literal">VECTOR_TO_STRING()</code></td> <td> Get the string representation of a VECTOR column, given its value as a binary string </td> </tr></tbody></table>
