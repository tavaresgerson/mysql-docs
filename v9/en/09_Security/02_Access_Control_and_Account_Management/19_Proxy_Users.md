...
[Section 8.2.17, “Pluggable Authentication”](pluggable-authentication.html "8.2.17 Pluggable Authentication"). For information about
specific plugins, see [Section 8.4.1, “Authentication Plugins”](authentication-plugins.html "8.4.1 Authentication Plugins").
For information about writing authentication plugins that support
proxy users, see
[Implementing Proxy User Support in Authentication Plugins](/doc/extending-mysql/9.5/en/writing-authentication-plugins-proxy-users.html).
...
privileges. See [Section 8.2.10, “Using Roles”](roles.html "8.2.10 Using Roles").
...
* The account for the external proxy user must be set up to be
  ...
  [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") to change its
  plugin.
...
* The account for the proxied user must exist and be granted
  ...
  [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statements for this.
...
* The proxy user account must have the
  ...
  [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statement for this.
...
* For a client connecting to the proxy account to be treated
  ...
  [`PROXY`](privileges-provided.html#priv_proxy) privilege held by the
  proxy user.
...
When proxying occurs, the [`USER()`](information-functions.html#function_user)
and [`CURRENT_USER()`](information-functions.html#function_current-user) functions can
...
In the [`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") statement that
...
* Include the `ACCOUNT LOCK` option when you
  create the account. See [Section 15.7.1.3, “CREATE USER Statement”](create-user.html "15.7.1.3 CREATE USER Statement"). With
...
The [`PROXY`](privileges-provided.html#priv_proxy) privilege is needed to
...
The corresponding [`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") syntax
is:
...
MySQL [`GRANT`](grant.html "15.7.1.6 GRANT Statement") and
[`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") syntax extensions work as
usual. Examples:
...
The [`PROXY`](privileges-provided.html#priv_proxy) privilege can be
...
* By *`proxied_user`* for itself: The
  value of [`USER()`](information-functions.html#function_user) must exactly
  match [`CURRENT_USER()`](information-functions.html#function_current-user) and
...
[`PROXY ... WITH GRANT
OPTION`](privileges-provided.html#priv_proxy) privilege for `''@''`, that
...
`''@''` can authenticate as
`developer` (because `''@''`
has the [`PROXY`](privileges-provided.html#priv_proxy) privilege to do so)
...
[`GRANT`](grant.html "15.7.1.6 GRANT Statement") statements, not shown.) The
[`USER()`](information-functions.html#function_user) and
[`CURRENT_USER()`](information-functions.html#function_current-user) functions return
these values:
...
users by default. `sha256_password` can request
that the MySQL server itself map proxy users according to
granted proxy privileges. If the
...
USER`](create-user.html "15.7.1.3 CREATE USER Statement"), then grant it the
[`PROXY`](privileges-provided.html#priv_proxy) privilege to a single other
account to be treated as the proxied user. When the server
receives a successful connection request for the proxy user, it
finds that the user has the [`PROXY`](privileges-provided.html#priv_proxy)
...
* The server does not proxy to or from an anonymous user, even
  if the associated [`PROXY`](privileges-provided.html#priv_proxy)
...
* [`proxy_user`](server-system-variables.html#sysvar_proxy_user): This value is
...
* [`external_user`](server-system-variables.html#sysvar_external_user): Sometimes
