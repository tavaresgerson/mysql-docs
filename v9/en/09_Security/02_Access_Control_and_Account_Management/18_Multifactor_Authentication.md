...
MySQL includes support for multifactor authentication. This
capability includes forms of MFA that require up to three
authentication values. That is, MySQL account management supports
accounts that use 2FA or 3FA, in addition to the existing 1FA
support.
...
* [Elements of Multifactor Authentication Support](multifactor-authentication.html#multifactor-authentication-elements "Elements of Multifactor Authentication Support")
...
* [`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") and
  [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") have syntax
...
* To enable authentication to the MySQL server using accounts
  ...
  [`--password3`](connection-options.html#option_general_password3) options that
  ...
  [`mysql_options4()`](/doc/c-api/9.5/en/mysql-options4.html) C API
  function enables the same capability.

* The server-side `authentication_webauthn`
  plugin enables authentication using devices. This
  server-side, device-based authentication plugin is included
  only in MySQL Enterprise Edition distributions. It is not included in MySQL
  community distributions. However, the client-side
  `authentication_webauthn_client` plugin is
  ...
  `authentication_webauthn` to authenticate
  on a server that has that plugin loaded. See
...
* `authentication_webauthn` also enables
...
* Multifactor authentication can use non-WebAuthn MySQL
  authentication methods, the WebAuthn authentication method,
...
The value of
[`authentication_policy`](server-system-variables.html#sysvar_authentication_policy) is a list
...
[`authentication_policy`](server-system-variables.html#sysvar_authentication_policy) value
...
The default
[`authentication_policy`](server-system-variables.html#sysvar_authentication_policy) value is
...
[`authentication_policy`](server-system-variables.html#sysvar_authentication_policy) value can
...
[`authentication_policy`](server-system-variables.html#sysvar_authentication_policy) example
values and the policy established by each.

**Table 8.11 Example authentication\_policy Values**

<table summary="Example authentication_policy values and their meanings."><col style="width: 50%"/><col style="width: 50"/><thead><tr>
...
plugins (such as `caching_sha2_password`) for
factor 2 or 3.
...
described in [Section 8.4.1.6, “LDAP Pluggable Authentication”](ldap-pluggable-authentication.html "8.4.1.6 LDAP Pluggable Authentication"),
...
[`--password1`](connection-options.html#option_general_password1),
[`--password2`](connection-options.html#option_general_password2), and
[`--password3`](connection-options.html#option_general_password3) options that permit
up to three passwords to be specified. These options are similar
to the [`--password`](connection-options.html#option_general_password) option in that
...
[`--password2`](mysql-command-options.html#option_mysql_password2) options.
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") prompts for each password in turn:
...
[`ALTER USER
user ADD
factor`](alter-user.html "15.7.1.1 ALTER USER Statement") syntax. Both methods
are shown below:
...
[`ALTER USER
user DROP
factor`](alter-user.html "15.7.1.1 ALTER USER Statement") syntax permits
dropping a factor. The following example drops the third factor
(`authentication_webauthn`) that was added in
...
Use [`SHOW CREATE USER`](show-create-user.html "15.7.7.14 SHOW CREATE USER Statement") to view the
