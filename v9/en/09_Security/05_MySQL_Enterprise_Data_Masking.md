...
MySQL Enterprise Data Masking is an extension included in MySQL Enterprise Edition, a commercial product. To learn more about commercial products, see <https://www.mysql.com/products/>.

Note

This feature was known previously as “MySQL Enterprise Data Masking and De-Identification”.

MySQL Enterprise Data Masking provides the following data masking and de-identification capabilities:
...
MySQL Enterprise Data Masking was implemented originally in MySQL as a plugin library. As of MySQL 9.5, MySQL Enterprise Edition also provides components to access data masking and de-identification capabilities. For information about the similarities and differences, see Table 8.46, “Comparison Between Data-Masking Components and Plugin Elements”.

If you are using MySQL Enterprise Data Masking for the first time, consider installing the components for access to the ongoing enhancements only available with component infrastructure.
...
Masking and de-identification are core to regulatory compliance, so MySQL Enterprise Data Masking can help application developers satisfy privacy requirements:
...
The following sections describe the elements of MySQL Enterprise Data Masking, discuss how to install and use it, and provide reference information for its elements.
...
Components provide expanded access to MySQL Enterprise Data Masking functionality. Previously, MySQL implemented masking and de-identification capabilities as a plugin library file containing a plugin and several loadable functions. Before you begin the component installation, remove the `data_masking` plugin and all of its loadable functions to avoid conflicts. For instructions, see Section 8.5.3.1, “MySQL Enterprise Data Masking Plugin Installation”.
...
To remove MySQL Enterprise Data Masking when using the `mysql` system schema:
...
Because it cannot be guaranteed that the number generated has not been assigned, the result of `gen_rnd_canada_sin()` should never be displayed (except possibly in testing). For display in user-facing applications, always employ a masking function such as `mask_canada_sin()`, as shown here:
...
MySQL Enterprise Data Masking is based on a plugin library that implements these elements:
...
#### 8.5.3.1 MySQL Enterprise Data Masking Plugin Installation
...
To install the MySQL Enterprise Data Masking plugin and functions, use the `INSTALL PLUGIN` and `CREATE FUNCTION` statements, adjusting the `.so` suffix for your platform as necessary:
...
#### 8.5.3.2 Using the MySQL Enterprise Data Masking Plugin

Before using MySQL Enterprise Data Masking, install it according to the instructions provided at Section 8.5.3.1, “MySQL Enterprise Data Masking Plugin Installation”.
...
#### 8.5.3.4 MySQL Enterprise Data Masking Plugin Function Descriptions

The MySQL Enterprise Data Masking plugin library includes several functions, which may be grouped into these categories:
...
If a MySQL Enterprise Data Masking function is invoked from within the **mysql** client, binary string results display using hexadecimal notation, depending on the value of the `--binary-as-hex`. For more information about that option, see Section 6.5.1, “mysql — The MySQL Command-Line Client”.
