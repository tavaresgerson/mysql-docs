...
MySQL client programs may elect to resume a prior SSL session,
provided that the server has the session in its runtime cache.
This section describes the conditions that are favorable for SSL
session reuse, the server variables used for managing and
monitoring the session cache, and the client command-line options
for storing and reusing session data.

* [Server-Side Runtime Configuration and Monitoring for SSL Session Reuse](reusing-ssl-sessions.html#ssl-session-cache-server-configuration "Server-Side Runtime Configuration and Monitoring for SSL Session Reuse")
* [Client-Side Configuration for SSL Session Reuse](reusing-ssl-sessions.html#ssl-session-data-client-configuration "Client-Side Configuration for SSL Session Reuse")
...
[SSL Session Reuse](/doc/c-api/9.5/en/c-api-ssl-session-reuse.html)).
...
<table summary="The system variables that define caching for session reuse and the corresponding status variables that expose active session-cache values."><col style="width: 45%"/><col style="width: 55%"/><thead><tr>
...
1. Set each cache-related system variable that should be
   ...
   see [System Variable Assignment](set-variable.html#set-variable-system-variables "System Variable Assignment").
...
1. Invoke [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") to establish an encrypted
   connection to a server running MySQL 9.5.
...
3. From the prompt of your command interpreter, invoke any
   ...
   and then [**mysqlshow**](mysqlshow.html "6.5.6 mysqlshow — Display Database, Table, and Column Information") client:
   ...
   To confirm whether [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") reused a
   session, see the output from the `status`
   command. If the currently active [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client")
   connection did resume the session, the status information
   includes `SSL session reused: true`.

In addition to [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") and
[**mysqlshow**](mysqlshow.html "6.5.6 mysqlshow — Display Database, Table, and Column Information"), SSL session reuse applies to
[**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program"), [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files"),
...
[**mysqlimport**](mysqlimport.html "6.5.5 mysqlimport — A Data Import Program"), [**mysqlslap**](mysqlslap.html "6.5.7 mysqlslap — A Load Emulation Client"),
**mysqltest**,
[**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility"), and
...
[`--ssl-session-data`](connection-options.html#option_general_ssl-session-data) but the
...
[`--ssl-session-data`](connection-options.html#option_general_ssl-session-data) . If the
