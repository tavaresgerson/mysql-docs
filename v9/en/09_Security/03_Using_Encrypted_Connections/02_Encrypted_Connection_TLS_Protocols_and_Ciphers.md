...
MySQL 9.5 supports the TLSv1.2 and TLSv1.3
...
MySQL 9.5 does not support the old TLSv1 and
TLSv1.1 protocols.
...
* Change the system-wide host configuration to permit
  ...
  have an `/etc/ssl/openssl.cnf` file that
  ...
  `None` makes the system more permissive.
...
therefore as shown in [MySQL Server TLS Protocol Default Settings](/doc/refman/8.0/en/encrypted-connection-protocols-ciphers.html#tls-support-defaults).
...
* `SOURCE_TLS_VERSION`: The replica does not
...
* For regular source/replica replication connections, where
  ...
  `SOURCE_SSL_CIPHER` option for the
  [`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
  statement. See
  [Section 19.3.1, “Setting Up Replication to Use Encrypted Connections”](replication-encrypted-connections.html "19.3.1 Setting Up Replication to Use Encrypted Connections").
...
variables [`--tls-ciphersuites`](server-system-variables.html#sysvar_tls_ciphersuites)
...
* For regular source/replica replication connections, where
  ...
  `SOURCE_TLS_CIPHERSUITES` option for the
  [`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
  statement. See
  [Section 19.3.1, “Setting Up Replication to Use Encrypted Connections”](replication-encrypted-connections.html "19.3.1 Setting Up Replication to Use Encrypted Connections").
