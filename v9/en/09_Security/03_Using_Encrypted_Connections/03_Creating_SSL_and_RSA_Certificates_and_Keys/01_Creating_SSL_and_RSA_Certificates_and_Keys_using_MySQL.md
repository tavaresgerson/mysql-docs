...
Server autogeneration helps lower the barrier to using SSL by
making it easier to generate the required files. However,
certificates generated by this method are self-signed, which
may not be very secure. After you gain experience using these,
consider obtaining certificate and key material from a
...
variable is enabled, no SSL options are specified, and the
...
SSL and RSA files created automatically by the server have
...
* SSL files created by the server are valid for ten years
