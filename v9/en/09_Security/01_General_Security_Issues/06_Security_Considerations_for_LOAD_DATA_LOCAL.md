...
The [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") statement loads a
...
DATA`](load-data.html "15.2.9 LOAD DATA Statement") has two potential security issues:

* Because [`LOAD DATA
  LOCAL`](load-data.html "15.2.9 LOAD DATA Statement") is an SQL statement, parsing occurs on the
  ...
  [`LOAD DATA
  LOCAL`](load-data.html "15.2.9 LOAD DATA Statement"), so a more fundamental issue is that clients
...
* In a Web environment where the clients are connecting from a
  ...
  [`LOAD DATA
  LOCAL`](load-data.html "15.2.9 LOAD DATA Statement") to read any files that the Web server process
...
using the
[`--ssl-mode=VERIFY_IDENTITY`](connection-options.html#option_general_ssl-mode) option
...
To avoid [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") issues, clients
...
* [Enabling or Disabling Local Data Loading Capability](load-data-local-security.html#load-data-local-configuration "Enabling or Disabling Local Data Loading Capability")
* [Restricting Files Permitted for Local Data Loading](load-data-local-security.html#load-data-local-permitted-files "Restricting Files Permitted for Local Data Loading")
* [MySQL Shell and Local Data Loading](load-data-local-security.html#load-data-local-shell "MySQL Shell and Local Data Loading")
...
* On the server side:
  ...
  + The [`local_infile`](server-system-variables.html#sysvar_local_infile) system
    variable controls server-side `LOCAL`
    capability. Depending on the
    [`local_infile`](server-system-variables.html#sysvar_local_infile) setting,
    the server refuses or permits local data loading by
    clients that request local data loading.

  + By default,
    [`local_infile`](server-system-variables.html#sysvar_local_infile) is
    disabled. (This is a change from previous versions of
    MySQL.) To cause the server to refuse or permit
    [`LOAD DATA
    LOCAL`](load-data.html "15.2.9 LOAD DATA Statement") statements explicitly (regardless of how
    client programs and libraries are configured at build
    time or runtime), start [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") with
    [`local_infile`](server-system-variables.html#sysvar_local_infile) disabled
    or enabled.
    [`local_infile`](server-system-variables.html#sysvar_local_infile) can also
    be set at runtime.

* On the client side:

  + The [`ENABLED_LOCAL_INFILE`](source-configuration-options.html#option_cmake_enabled_local_infile)
    **CMake** option controls the compiled-in
    default `LOCAL` capability for the
    MySQL client library (see
    [Section 2.8.7, “MySQL Source-Configuration Options”](source-configuration-options.html "2.8.7 MySQL Source-Configuration Options")). Clients
  ...
  + By default, the client library in MySQL binary
    ...
    with [`ENABLED_LOCAL_INFILE`](source-configuration-options.html#option_cmake_enabled_local_infile)
  ...
  + For client programs that use the C API, local data
    ...
    [`mysql_options()`](/doc/c-api/9.5/en/mysql-options.html) C API
    function to disable or enable the
    `MYSQL_OPT_LOCAL_INFILE` option. See
    [mysql\_options()](/doc/c-api/9.5/en/mysql-options.html).

  + For the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client, local data
    loading capability is determined by the default compiled
    ...
    [`--local-infile[=1]`](mysql-command-options.html#option_mysql_local-infile) option.

  + For the [**mysqlimport**](mysqlimport.html "6.5.5 mysqlimport — A Data Import Program") client, local
    data loading is not used by default. To disable or
    enable it explicitly, use the
    [`--local=0`](mysqlimport.html#option_mysqlimport_local) or
    [`--local[=1]`](mysqlimport.html#option_mysqlimport_local) option.

  + If you use
    [`LOAD DATA
    LOCAL`](load-data.html "15.2.9 LOAD DATA Statement") in Perl scripts or other programs that
    ...
    using the
    [`loose-`](option-modifiers.html "6.2.2.4 Program Option Modifiers")
    prefix:
...
[`LOAD DATA
LOCAL`](load-data.html "15.2.9 LOAD DATA Statement") statement receives the following error message:
...
[`mysql_options()`](/doc/c-api/9.5/en/mysql-options.html) C API function
(see [mysql\_options()](/doc/c-api/9.5/en/mysql-options.html)).
...
[`mysql_options()`](/doc/c-api/9.5/en/mysql-options.html) options as
follows:

* The [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client has a
  ...
  [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") uses the option value to set the
...
* [**mysqlimport**](mysqlimport.html "6.5.5 mysqlimport — A Data Import Program") sets
...
[`LOAD DATA LOCAL
INFILE`](load-data.html "15.2.9 LOAD DATA Statement") statements to upload data, so the
...
[MySQL Shell Utilities](/doc/mysql-shell/9.5/en/mysql-shell-utilities.html).