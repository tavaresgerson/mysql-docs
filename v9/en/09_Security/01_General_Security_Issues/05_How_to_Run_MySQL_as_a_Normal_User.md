...
RPM packages, the MySQL server [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") should be
...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") can be started and run by any user.
However, you should avoid running the server as the Unix
`root` user for security reasons. To change
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") to run as a normal unprivileged Unix
...
3. Start the server as user *`user_name`*.
   Another alternative is to start [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") as
   the Unix `root` user and use the
   [`--user=user_name`](server-options.html#option_mysqld_user)
   option. [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") starts, then switches to run
...
machine can run the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client with a
