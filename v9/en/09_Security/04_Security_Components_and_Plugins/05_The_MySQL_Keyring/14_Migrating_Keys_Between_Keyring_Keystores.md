...
* Migration from one keyring component to another: The
  [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") utility provides
  this capability.
...
* [Key Migration Using the mysql\_migrate\_keyring Utility](keyring-key-migration.html#keyring-key-migration-using-mysql-migrate-keyring "Key Migration Using the mysql_migrate_keyring Utility")
...
* Pausing keyring operations during an online migration is
  ...
  setting its global
  [`keyring_operations`](keyring-system-variables.html#sysvar_keyring_operations) system
  ...
  + If an online migration operation exits abnormally (for
    ...
    for
    [`keyring_operations`](keyring-system-variables.html#sysvar_keyring_operations) to
...
* To indicate the source keyring plugin and the destination
  ...
  + [`--keyring-migration-destination`](keyring-options.html#option_mysqld_keyring-migration-destination):
  ...
  + [`--keyring-migration-from-component`](keyring-options.html#option_mysqld_keyring-migration-from-component):
...
* For an online migration, some running server currently is
  ...
  + [`--keyring-migration-host`](keyring-options.html#option_mysqld_keyring-migration-host):
  ...
  + [`--keyring-migration-user`](keyring-options.html#option_mysqld_keyring-migration-user),
  ...
  + [`--keyring-migration-port`](keyring-options.html#option_mysqld_keyring-migration-port):
    For TCP/IP connections, the port number to connect to
    on the running server.

  + [`--keyring-migration-socket`](keyring-options.html#option_mysqld_keyring-migration-socket):
...
* Other server options might be required; other non-keyring
  ...
  options is by using
  [`--defaults-file`](option-file-options.html#option_general_defaults-file) to name an
  option file that contains the required options.

  + The migration server must not start up with its own
    keyring. This means that
    [`--defaults-file`](option-file-options.html#option_general_defaults-file) must
    not point to the same options file that is used to
    start the running server if it cantains a line such as
  ...
  + If migrating from a plugin to a component, the
    ...
    directory. Although, the component configuration (for
    example, `component_keyring_file.cnf`
    in the plugin directory) should be present in the
    `bin` directory, so that the new
...
* The user who invokes a server in key-migration mode must
  ...
  unless the [`--user`](server-options.html#option_mysqld_user) option is
...
* The user a server in key-migration mode runs as must have
  ...
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") normally runs as the
  ...
  a
  [`--user=user_name`](server-options.html#option_mysqld_user)
...
2. (Online migration only) Disable
   [`keyring_operations`](keyring-system-variables.html#sysvar_keyring_operations) on the
   running server.

3. Load the keyring plugin or component libraries for the
   source and destination keystores.

4. Copy keys from the source keystore to the destination.
5. Unload the keyring plugin or component libraries for the
   source and destination keystores.
...
[**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility"), determine the key
...
* To indicate the source and destination keyring components
  ...
  + [`--component-dir`](mysql-migrate-keyring.html#option_mysql_migrate_keyring_component-dir):
    ...
    [`plugin_dir`](server-system-variables.html#sysvar_plugin_dir) system
  ...
  [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") must support them
  both.
...
* For an online migration, some running server currently is
  ...
  [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") can connect to it
  ...
  + [`--host`](mysql-migrate-keyring.html#option_mysql_migrate_keyring_host):
    The host where the running server is located. This is
    always the local host because
    [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") can migrate
    keys only between keystores managed by local
    components.

  + [`--user`](mysql-migrate-keyring.html#option_mysql_migrate_keyring_user),
    [`--password`](mysql-migrate-keyring.html#option_mysql_migrate_keyring_password):
    The account credentials to use to connect to the
    running server.

  + [`--port`](mysql-migrate-keyring.html#option_mysql_migrate_keyring_port):
    For TCP/IP connections, the port number to connect to
    on the running server.

  + [`--socket`](mysql-migrate-keyring.html#option_mysql_migrate_keyring_socket):
    For Unix socket file or Windows named pipe
    connections, the socket file or named pipe to connect
    to on the running server.
...
Start [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") with options
indicating the source and destination keystores and whether
the migration is offline or online, possibly with other
options. Keep the following considerations in mind:

* The user who invokes
  [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") must not be the
  `root` operating system user.

* The user who invokes
  [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") must have
  permission to read and write any local keyring files, such
  as the data file for a file-based plugin.

  If you invoke [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility")
  ...
  operation. Suppose that [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") normally
  runs as the `mysql` operating system
  user, but you invoke
  [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") while logged in
  as `isabel`. Any new directories or files
  created by [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") are
  ...
  [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") as the
...
[**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") like this (enter the
...
[**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") performs a migration
...
1. Connect to each running server manually and set
   [`keyring_operations=OFF`](keyring-system-variables.html#sysvar_keyring_operations).
...
2. Use a migration server or
   [**mysql\_migrate\_keyring**](mysql-migrate-keyring.html "6.6.8 mysql_migrate_keyring — Keyring Key Migration Utility") to perform an
   offline key migration for each paused server.

3. Connect to each running server manually and set
   [`keyring_operations=ON`](keyring-system-variables.html#sysvar_keyring_operations).
