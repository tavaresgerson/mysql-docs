...
The `component_keyring_hashicorp` component
...
The `component_keyring_hashicorp` keyring
...
[Keyring
Components Versus Keyring Plugins](keyring-component-plugin-comparison.html "8.4.5.1 Keyring Components Versus Keyring Plugins").

The `keyring_hashicorp` components supports the
...
To install `component_keyring_hashicorp`, use
the general instructions found in
[Section 8.4.5.2, “Keyring Component Installation”](keyring-component-installation.html "8.4.5.2 Keyring Component Installation"), together with
the configuration information specific to
`component_keyring_hashicorp` found here.
...
* [Certificate and Key Preparation](keyring-hashicorp-component.html#keyring-hashicorp-component-certificate-configuration "Certificate and Key Preparation")
* [HashiCorp Vault Setup](keyring-hashicorp-component.html#keyring-hashicorp-component-vault-configuration "HashiCorp Vault Setup")
* [component\_keyring\_hashicorp Configuration](keyring-hashicorp-component.html#keyring-hashicorp-component-configuration "component_keyring_hashicorp Configuration")
* [Migration from HashiCorp Vault Plugin](keyring-hashicorp-component.html#keyring-hashicorp-component-migration-from-plugin "Migration from HashiCorp Vault Plugin")

##### Certificate and Key Preparation

The `keyring_hashicorp` component requires a
...
[HashiCorp Vault Setup](keyring-hashicorp-component.html#keyring-hashicorp-component-vault-configuration "HashiCorp Vault Setup").)
The instructions as shown apply to Linux platforms and may
require adjustment for other platforms.
...
`component_keyring_hashicorp` component.
...
##### component\_keyring\_hashicorp Configuration

When it initializes,
`component_keyring_hashicorp` reads a
component global configuration file named
`component_keyring_hashicorp.cnf` in
[`plugin_dir`](server-system-variables.html#sysvar_plugin_dir). If this file
contains `{"read_local_config": true}`, the
...
`component_keyring_hashicorp.cnf`) in the
MySQL data directory
([`datadir`](server-system-variables.html#sysvar_datadir)).

If the component does not find the global configuration file
or (in cases where it looks for one) a local configuration
file, it cannot start.
...
`component_keyring_hashicorp.cnf` are shown
in the following table:

<table summary="component_keyring_hashicorp configuration items."><col style="width: 15%"/><col style="width: 20%"/><col style="width: 10%"/><col style="width: 20%"/><col style="width: 15%"/><col style="width: 20%"/><thead><tr>
<th scope="col">Configuration Item</th>
<th scope="col">Description</th>
<th scope="col">Required</th>
<th scope="col">Valid Values</th>
<th scope="col">Default</th>
<th scope="col">Corresponding System Variable</th>
</tr></thead><tbody><tr>
<th scope="row"><code class="literal">auth_mode</code></th>
<td>Whether to use AppRole or token-based authentication.</td>
<td>No.</td>
<td><code class="literal">approle</code>, <code class="literal">token</code></td>
<td><code class="literal">approle</code></td>
<td><span class="emphasis"><em>None</em></span></td>
</tr><tr>
<th scope="row"><code class="literal">auth_path</code></th>
<td>Authentication path where AppRole authentication is enabled within the
                HashiCorp Vault server.</td>
<td>No.</td>
<td>Valid Unix-style file system path; cannot be empty.</td>
...
<th scope="row"><code class="literal">ca_path</code></th>
<td>Path to local file (accessible to MySQL server) containing properly
                formatted TLS certificate authority.</td>
<td>No</td>
<td>File system path valid for the system hosting the MySQL server. Path
                must be absolute.</td>
<td><span class="emphasis"><em>Empty string</em></span></td>
...
<th scope="row"><code class="literal">caching</code></th>
<td>Whether to enable in-memory key cache. If enabled,
                <code class="literal">component_keyring_hashicorp</code> populates
                it during initialization; only the key list is
                populated.</td>
<td>No.</td>
<td><code class="literal">ON</code>, <code class="literal">OFF</code></td>
...
<th scope="row"><code class="literal">role_id</code></th>
<td>The HashiCorp Vault AppRole authentication role ID.</td>
<td>Yes, when <code class="literal">auth_mode</code> is <code class="literal">approle</code>.</td>
<td>Must be in valid UUID format.</td>
<td><span class="emphasis"><em>Empty string</em></span></td>
...
<th scope="row"><code class="literal">secret_id</code></th>
<td>The HashiCorp Vault AppRole authentication secret ID, in UUID format.</td>
<td>Yes, when <code class="literal">auth_mode</code> is <code class="literal">approle</code>.</td>
<td>Must be in valid UUID format.</td>
<td><span class="emphasis"><em>Empty string</em></span></td>
...
<th scope="row"><code class="literal">server_url</code></th>
<td>URL of HashiCorp Vault server.</td>
<td>No.</td>
<td>A valid URL beginning with <code class="literal">https://</code>.</td>
...
<th scope="row"><code class="literal">store_path</code></th>
<td>Path within HashiCorp Vault server which is writeable when appropriate
                AppRole credentials are provided by
                <code class="literal">component_keyring_hashicorp</code>. To
                specify credentials, set <code class="literal">role_id</code> and
                <code class="literal">secret_id</code> (see
                <a class="xref" href="keyring-hashicorp-component.html#keyring-hashicorp-component-configuration" title="component_keyring_hashicorp Configuration">component_keyring_hashicorp Configuration</a>).</td>
<td>Yes.</td>
<td><span class="emphasis"><em>Empty string</em></span></td>
<td>Valid Unix-style file system path; cannot be empty.</td>
...
<th scope="row"><code class="literal">token_path</code></th>
<td>Path to token file.</td>
<td>Yes, when <code class="literal">auth_mode</code> is <code class="literal">token</code>.</td>
<td><span class="emphasis"><em>None</em></span></td>
<td>File system path valid for the system hosting the MySQL server.</td>
<td><span class="emphasis"><em>None</em></span></td>
</tr></tbody></table>

Note

`component_keyring_hashicorp` does not
support any of the system variables provided by the
`keyring_hashicorp` plugin; the latter are
...
`component_keyring_hashicorp` must be loaded
using a manifest file `mysqld.my` (see
[Section 8.4.5.2, “Keyring Component Installation”](keyring-component-installation.html "8.4.5.2 Keyring Component Installation")). This file
...
A `mysqld.my` or other manifest file may
...
`component_keyring_hashicorp.cnf` file
...
This example shows a sample configuration file for a setup
...
`/v1/`). If HashiCorp develops new protocol
versions, it may be necessary to change
`/v1/` to something else in your
configuration.
...
values to `role_id` and
`secret_id`. The setup procedure also results
in the store path `/v1/kv/mysql`, which is
used for `store_path` in
`component_keyring_hashicorp.cnf`.

##### Migration from HashiCorp Vault Plugin

To migrate from the HashiCorp Vault keyring plugin to the
HashiCorp Vault keyring component, it is necessary to set up
loading of the component, create a component configuration
equivalent to that used by the plugin, stop loading of the
plugin, and remove any references to the plugin or its
associated system variables from all configuration files. You
can accomplish these tasks by performing the steps shown here:

1. Create or modify a local or global manifest file
   `mysqld.my` (see
   ...
   content of the file must include (completely) the
   `components` item shown here:
...
2. Obtain the values of any of the following startup options
   that you find in the MySQL server's
   `my.conf` file:

   * [`--keyring-hashicorp-auth-path`](keyring-system-variables.html#sysvar_keyring_hashicorp_auth_path)
   * [`--keyring-hashicorp-server-url`](keyring-system-variables.html#sysvar_keyring_hashicorp_server_url)
   * [`--keyring-hashicorp-role-id`](keyring-system-variables.html#sysvar_keyring_hashicorp_role_id)
   * [`--keyring-hashicorp-secret-id`](keyring-system-variables.html#sysvar_keyring_hashicorp_secret_id)
   * [`--keyring-hashicorp-store-path`](keyring-system-variables.html#sysvar_keyring_hashicorp_store_path)
   * [`--keyring-hashicorp-caching`](keyring-system-variables.html#sysvar_keyring_hashicorp_caching)

   Note

   MySQL Components are not compatible with
   [`--early-plugin-load`](server-options.html#option_mysqld_early-plugin-load), so
   this value is not needed by
...
3. Write a component configuration file
   `component_keyring_hashicorp.cnf` (see
   [component\_keyring\_hashicorp Configuration](keyring-hashicorp-component.html#keyring-hashicorp-component-configuration "component_keyring_hashicorp Configuration"))
   which sets each of the configuration items to the value
   obtained in the previous step for its equivalent system
...
4. Perform any migration of keys that might be required. See
...
5. Uninstall the plugin using [`UNINSTALL
...
6. Remove any references to the plugin in
   ...
   specific to the HashiCorp keyring plugin (equivalent
   options listed previously). Variables which have been
   persisted (saved to `mysqld-auto.cnf`)
   must be removed from the server's configuration using
   [`RESET PERSIST`](reset-persist.html "15.7.8.7 RESET PERSIST Statement").

7. Restart [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") to cause the changes to
   take effect.