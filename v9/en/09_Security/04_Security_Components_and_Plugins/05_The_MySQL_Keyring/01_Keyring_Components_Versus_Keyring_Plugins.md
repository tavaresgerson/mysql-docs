...
* Keyring plugin loading uses the
  ...
  (deprecated). Keyring component loading uses a manifest.
...
* Keyring components have fewer restrictions than keyring
  ...
  `component_keyring_oci` can generate keys
  of type `AES` with a size of 16, 24, or
  32 bytes only.
...
* Keyring components support secure storage for persisted
  ...
  See [Persisting Sensitive System Variables](persisted-system-variables.html#persisted-system-variables-sensitive "Persisting Sensitive System Variables").