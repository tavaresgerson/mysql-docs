...
DATA`](load-data.html "15.2.9 LOAD DATA Statement") are not logged.
...
* New-style XML format
  ([`audit_log_format=NEW`](audit-log-reference.html#sysvar_audit_log_format)): An
  XML format that has better compatibility with Oracle Audit
  Vault than old-style XML format. MySQL 9.5 uses
  new-style XML format by default.

* Old-style XML format
  ([`audit_log_format=OLD`](audit-log-reference.html#sysvar_audit_log_format)): The
...
If you change [`audit_log_format`](audit-log-reference.html#sysvar_audit_log_format),
it is recommended that you also change
[`audit_log_file`](audit-log-reference.html#sysvar_audit_log_file). For example, if
you set [`audit_log_format`](audit-log-reference.html#sysvar_audit_log_format) to
`JSON`, set
...
* `<COMMAND_CLASS>`
  ...
  *`xxx`* is
  `drop_table` and
  `select` for [`DROP
  TABLE`](drop-table.html "15.1.37 DROP TABLE Statement") and [`SELECT`](select.html "15.2.13 SELECT Statement")
...
* `<MYSQL_VERSION>`
  ...
  [`VERSION()`](information-functions.html#function_version) function or
...
* `<OS_LOGIN>`
  ...
  the [`external_user`](server-system-variables.html#sysvar_external_user) system
  variable (see [Section 8.2.19, “Proxy Users”](proxy-users.html "8.2.19 Proxy Users")).
...
* `<PROXY_USER>`
  ...
  [Section 8.2.19, “Proxy Users”](proxy-users.html "8.2.19 Proxy Users")). The value is empty if user
...
* `<STATUS>`
  ...
  the value of the
  [`mysql_errno()`](/doc/c-api/9.5/en/mysql-errno.html) C API
...
* `<STATUS_CODE>`
  ...
  the [`mysql_errno()`](/doc/c-api/9.5/en/mysql-errno.html) C API
...
([`audit_log_format=JSON`](audit-log-reference.html#sysvar_audit_log_format)), the
log file contents form a [`JSON`](json.html "13.5 The JSON Data Type")
array with each array element representing an audited event as
a [`JSON`](json.html "13.5 The JSON Data Type") hash of key-value pairs.
...
[Adding Query Statistics for Outlier Detection](audit-log-logging-configuration.html#audit-log-query-statistics "Adding Query Statistics for Outlier Detection"). For the
`bytes_sent` and
...
or a [`JSON`](json.html "13.5 The JSON Data Type") hash. For items that
...
* `account`
  ...
  the [`CURRENT_USER()`](information-functions.html#function_current-user) function
...
* `bytes_received`
  ...
  [`log_slow_extra`](server-system-variables.html#sysvar_log_slow_extra) must be
  set to `ON`.
