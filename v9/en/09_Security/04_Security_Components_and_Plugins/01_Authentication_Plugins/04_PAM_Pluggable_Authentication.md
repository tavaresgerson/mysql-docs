...
* If you installed the plugin at runtime using an
  ...
  it, use [`UNINSTALL PLUGIN`](uninstall-plugin.html "15.7.4.6 UNINSTALL PLUGIN Statement"):
...
[`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") statement, use the
...
* No proxy users. This uses PAM only to check login names
  ...
  *`user_name`* must be the external
  ...
  to 32 characters (see [Section 8.2.3, “Grant Tables”](grant-tables.html "8.2.3 Grant Tables")),
...
* The PAM service name corresponds to the authentication
  ...
  [`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") statement for
...
4. Use the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") command-line client to
...
7. Use the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") command-line client to
   connect to the MySQL server as `antonio`.
...
Setting `AUTHENTICATION_PAM_LOG=1` (or some
other arbitrary value) does *not* include
