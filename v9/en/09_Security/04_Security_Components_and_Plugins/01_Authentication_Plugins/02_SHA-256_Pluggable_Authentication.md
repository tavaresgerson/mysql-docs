...
see [Section 8.4.1.1, “Caching SHA-2 Pluggable Authentication”](caching-sha2-pluggable-authentication.html "8.4.1.1 Caching SHA-2 Pluggable Authentication").
...
default authentication plugin in MySQL 9.5 and
...
<table summary="Names for the plugins and library file used for SHA-256 password authentication."><col style="width: 30%"/><col style="width: 70%"/><thead><tr>
...
(The `IDENTIFIED WITH` clause is not needed
if `sha256_password` is the default plugin;
...
* The [`Rsa_public_key`](server-status-variables.html#statvar_Rsa_public_key)
...
3. Restart the server, then connect to it and check the
   [`Rsa_public_key`](server-status-variables.html#statvar_Rsa_public_key) status
...
[`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") time). The plugin
