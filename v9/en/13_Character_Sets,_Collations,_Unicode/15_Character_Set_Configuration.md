...
[`--collation-server`](server-system-variables.html#sysvar_collation_server) options when you
start the server. See [Section 7.1.7, “Server Command Options”](server-options.html "7.1.7 Server Command Options"). The
...
set, use the [`SHOW COLLATION`](show-collation.html "15.7.7.5 SHOW COLLATION Statement")
...
* If your program uses an incorrect path to determine where the
  ...
  [client]
  character-sets-dir="C:/Program Files/MySQL/MySQL Server 9.5/share/charsets"
  ```
...
* If the character set is a complex character set that cannot be
  ...
  [Section 12.14.4, “Adding a UCA Collation to a Unicode Character Set”](adding-collation-unicode-uca.html "12.14.4 Adding a UCA Collation to a Unicode Character Set").
...
[`character_set_system`](server-system-variables.html#sysvar_character_set_system) differs from
[`character_set_server`](server-system-variables.html#sysvar_character_set_server) or
[`character_set_client`](server-system-variables.html#sysvar_character_set_client), and you
...
[`--default-character-set=system_character_set`](mysql-command-options.html#option_mysql_default-character-set)—that
