...
the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client or other MySQL client
application, as shown in the example that follows.

1. We create a log file group named `lg_1`
   using [`CREATE LOGFILE GROUP`](create-logfile-group.html "15.1.20 CREATE LOGFILE GROUP Statement").
   ...
   `undo_1.log` to `lg_1`
   in this [`CREATE LOGFILE GROUP`](create-logfile-group.html "15.1.20 CREATE LOGFILE GROUP Statement")
   statement:
   ...
   GROUP`](alter-logfile-group.html "15.1.8 ALTER LOGFILE GROUP Statement") statement:
   ...
   * Every [`CREATE LOGFILE
     GROUP`](create-logfile-group.html "15.1.20 CREATE LOGFILE GROUP Statement") and [`ALTER LOGFILE
     GROUP`](alter-logfile-group.html "15.1.8 ALTER LOGFILE GROUP Statement") statement must include an
     `ENGINE` option. The only permitted
     values for this option are
     [`NDBCLUSTER`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") and
   ...
   * When you add an undo log file to a log file group using
     ...
     *`node_id`* is the node ID of the
     ...
     LOGFILE GROUP`](alter-logfile-group.html "15.1.8 ALTER LOGFILE GROUP Statement") statement just shown creates 4
   ...
   * See [Section 15.1.20, “CREATE LOGFILE GROUP Statement”](create-logfile-group.html "15.1.20 CREATE LOGFILE GROUP Statement"), and
     [Section 15.1.8, “ALTER LOGFILE GROUP Statement”](alter-logfile-group.html "15.1.8 ALTER LOGFILE GROUP Statement"), for more
...
2. Now we can create a tablespace—an abstract container
   ...
   The [`CREATE TABLESPACE`](create-tablespace.html "15.1.25 CREATE TABLESPACE Statement")
   statement creates a tablespace `ts_1` with
   the data file `data_1.dat`, and
   associates `ts_1` with log file group
   `lg_1`. The [`ALTER
   TABLESPACE`](alter-tablespace.html "15.1.12 ALTER TABLESPACE Statement") adds the second data file
   (`data_2.dat`).
   ...
   * When you add a data file to a tablespace using
     ...
     *`node_id`* is the node ID of the
     ...
     TABLESPACE`](alter-tablespace.html "15.1.12 ALTER TABLESPACE Statement") statement just shown creates 4 data
   ...
   * [`CREATE TABLESPACE`](create-tablespace.html "15.1.25 CREATE TABLESPACE Statement")
     ...
     `NDB` tablespaces,
     [`ALTER TABLESPACE`](alter-tablespace.html "15.1.12 ALTER TABLESPACE Statement") accepts
     an `ENGINE` clause only for
     `ALTER TABLESPACE ... ADD DATAFILE`;
     `ENGINE` is rejected for any other
     `ALTER TABLESPACE` statement. For
     `NDB` tablespaces, the only permitted
     values for the `ENGINE` option are
     `NDBCLUSTER` and
     `NDB`.
   ...
   * For more information about the
     ...
     statements, see [Section 15.1.25, “CREATE TABLESPACE Statement”](create-tablespace.html "15.1.25 CREATE TABLESPACE Statement"), and
...
3. Now it is possible to create a table whose unindexed columns
   ...
   [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine to use
   ...
   [`DELETE`](delete.html "15.2.2 DELETE Statement") statements on it just
   ...
   TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") statement. `STORAGE DISK`
   causes the column to be stored on disk, and `STORAGE
   MEMORY` causes in-memory storage to be used. See
   [Section 15.1.24, “CREATE TABLE Statement”](create-table.html "15.1.24 CREATE TABLE Statement"), for more information.

You can obtain information about the `NDB` disk
data files and undo log files just created by querying the
[`FILES`](information-schema-files-table.html "28.3.15 The INFORMATION_SCHEMA FILES Table") table in the
...
[`SHOW CREATE TABLE`](show-create-table.html "15.7.7.12 SHOW CREATE TABLE Statement") confirms that
...
You can see using [**ndb\_desc**](mysql-cluster-programs-ndb-desc.html "25.5.9 ndb_desc — Describe NDB Tables") that the indexed
