...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") NDB Cluster handler performs the
following actions:
...
The [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") on which the copying `ALTER
TABLE` statement is issued takes a metadata lock, but
this is in effect only on that [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"). Other
...
`ALGORITHM=INSTANT` is not supported by NDB
9.5.

Regardless of the algorithm used, the [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") for online `ALTER
TABLE` operations. That syntax has since been removed.

Operations that add and drop indexes on variable-width columns of
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables occur online. Online
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table alterations made in an NDB
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables. Online renaming of
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables is also supported.

Disk-based columns cannot be added to
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables online. This means that,
if you wish to add an in-memory column to an
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table that uses a table-level
...
option on [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables. This can be
...
requires an [`OPTIMIZE TABLE`](optimize-table.html "15.7.3.4 OPTIMIZE TABLE Statement") or null
[`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") statement. For more
information, see [Section 25.6.7, “Adding NDB Cluster Data Nodes Online”](mysql-cluster-online-add-node.html "25.6.7 Adding NDB Cluster Data Nodes Online").
...
Online [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement"),
[`CREATE INDEX`](create-index.html "15.1.18 CREATE INDEX Statement"), or
[`DROP INDEX`](drop-index.html "15.1.31 DROP INDEX Statement") statements that add
columns or add or drop indexes are subject to the following
limitations:

* A given online [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") can
...
* The table being altered is not locked with respect to API
  ...
  [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") `ADD
  COLUMN`, `ADD INDEX`, or
  `DROP INDEX` operation (or
  [`CREATE INDEX`](create-index.html "15.1.18 CREATE INDEX Statement") or
  [`DROP INDEX`](drop-index.html "15.1.31 DROP INDEX Statement") statement) is run.
...
* The table to be altered must have an explicit primary key; the
  ...
  [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine is not
...
* The tablespace used by the table cannot be changed online.
  ...
  TABLESPACE=new_tablespace`](alter-table.html "15.1.11 ALTER TABLE Statement")
...
[`TEXT`](blob.html "13.3.4 The BLOB and TEXT Types") type, and must meet the
following criteria:
...
For online [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") operations
on [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables, fixed-format columns
...
[`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") statements using
`ALGORITHM=INPLACE`.
