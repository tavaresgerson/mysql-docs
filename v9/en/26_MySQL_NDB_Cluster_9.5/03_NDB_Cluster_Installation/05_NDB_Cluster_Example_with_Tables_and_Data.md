...
* For a table to be replicated in the cluster, it must use the
  [`NDBCLUSTER`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine. To
  ...
  storage engine, use [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement")
...
* Every [`NDBCLUSTER`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table has a
  primary key. If no primary key is defined by the user when a
  table is created, the [`NDBCLUSTER`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5")
...
output of [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program"), you can open the SQL
...
[`INSERT`](insert.html "15.2.7 INSERT Statement") statements necessary to
...
[`NDBCLUSTER`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine for this
...
tables, and then use [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") to
...
then use the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") command-line client to read
...
on the SQL node, use [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") on the
...
Running [`SELECT`](select.html "15.2.13 SELECT Statement") queries on the SQL
...
between nodes. See
[Section 25.6.19.2, “NDB Cluster and MySQL Privileges”](mysql-cluster-security-mysql-privileges.html "25.6.19.2 NDB Cluster and MySQL Privileges"), for
more information.
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables. It is important to
...
might execute the [`SELECT`](select.html "15.2.13 SELECT Statement") statement
