...
1. There are two different methods by which the backup may be
   ...
   * **Method A.**
     ...
     *`management_host`* is the IP
     ...
     [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") management server for
   ...
   * **Method B.**
     ...
     [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") management client as
     part of the [`START
...
2. Copy the cluster backup files to the replica that is being
   ...
   *is* necessary that the replication process
   ...
   can do this by starting the replica with
...
3. Create any databases on the replica cluster that are present
   ...
   A [`CREATE DATABASE`](create-database.html "15.1.14 CREATE DATABASE Statement") (or
   [`CREATE
   SCHEMA`](create-database.html "15.1.14 CREATE DATABASE Statement")) statement corresponding to each database
...
7. Using `@latest` as the epoch value obtained
   ...
   [`SHOW BINARY LOG STATUS`](show-binary-log-status.html "15.7.7.1 SHOW BINARY LOG STATUS Statement") on the
...
8. Using the values obtained in the previous step, you can now
   ...
   [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client. Use the following
