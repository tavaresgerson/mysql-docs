...
3. Set up the replica to use the source. Using the
   ...
   *`replica_password`* is the password
   ...
   statement, see [Section 15.4.2.2, “CHANGE REPLICATION SOURCE TO Statement”](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement").
...
4. If the source cluster is already in use, you can create a
   ...
   *`dump_file`* is the name of the file
   ...
   [`--skip-replica-start`](replication-options-replica.html#option_mysqld_skip-replica-start). Once
...
5. Ensure that each MySQL server acting as a replication source
   ...
   Use [`--ndb-replica-batch-size`](mysql-cluster-options-variables.html#sysvar_ndb_replica_batch_size) to
