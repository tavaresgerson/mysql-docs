...
1. Start the MySQL replication source server by issuing this
   ...
   `NDB` tables explicitly, using the
   [`--ndb-log-bin`](mysql-cluster-options-variables.html#sysvar_ndb_log_bin) option.
...
2. Start the MySQL replica server as shown here:
   ...
   [`START REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement has
...
[`replica_allow_batching`](mysql-cluster-options-variables.html#sysvar_replica_allow_batching) on the
replicas' [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") processes. Normally,
...
an appropriate [`SHOW VARIABLES`](show-variables.html "15.7.7.42 SHOW VARIABLES Statement")
