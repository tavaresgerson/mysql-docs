...
1. Obtain the time of the most recent global checkpoint (GCP).
   ...
   `mysql.ndb_apply_status` table whose server
   IDs have a match in the `IGNORE_SERVER_IDS`
   list from the [`CHANGE REPLICATION SOURCE
   TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement used to prepare this replicas's
   ...
   of [`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement"). We
...
2. Using the information obtained from the query shown in Step 1,
   ...
   one [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") instance to access user
   ...
   You must ensure that the replica [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
   is started with
   [`--replica-skip-errors=ddl_exist_errors`](replication-options-replica.html#sysvar_replica_skip_errors)
   before executing [`START
   REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement"). Otherwise, replication may stop with
