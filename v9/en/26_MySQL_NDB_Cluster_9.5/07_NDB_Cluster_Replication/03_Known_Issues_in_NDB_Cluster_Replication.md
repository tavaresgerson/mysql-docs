...
[`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement"), and indicates
...
See [Section 25.7.8, “Implementing Failover with NDB Cluster Replication”](mysql-cluster-replication-failover.html "25.7.8 Implementing Failover with NDB Cluster Replication"), for more
...
* All SQL nodes acting as sources and replicas are started with
  the system variable
  [`log_replica_updates`](replication-options-binary-log.html#sysvar_log_replica_updates) enabled.
...
[`log_replica_updates`](replication-options-binary-log.html#sysvar_log_replica_updates) system
...
The [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine uses
...
[`replica_exec_mode`](replication-options-replica.html#sysvar_replica_exec_mode) to
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables without primary keys can
still occur, due to the possibility of duplicate rows being
inserted in such cases. For this reason, it is highly
recommended that all [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables
...
of unique key columns of [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables
could result in duplicate-key errors when replicated. This issue
is solved for replication between
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables by deferring unique key
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5"). Thus, updates of unique keys
when replicating from [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") to a
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table such as
...
Every [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table is implicitly
...
[`RESET BINARY LOGS AND GTIDS`](reset-binary-logs-and-gtids.html "15.4.1.2 RESET BINARY LOGS AND GTIDS Statement") and
[`RESET REPLICA`](reset-replica.html "15.4.2.3 RESET REPLICA Statement") statements to clear
the invalid `ndb_binlog_index` and
`ndb_apply_status` tables, respectively.

**Replication from NDB to other storage engines.**
It is possible to replicate an [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5")
...
* Multi-source and circular replication are not supported
  ...
  [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine for this to
  work).
...
engines.**
For replication from [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") to a
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") and a different storage engine.
(An NDB Cluster database *can* simultaneously
replicate to multiple NDB Cluster databases.) If the source uses
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables, it is still possible to
...
* **Change the storage engine used for the mysql.ndb\_apply\_status table.**
  ...
  [`ALTER TABLE
  mysql.ndb_apply_status ENGINE=MyISAM`](alter-table.html "15.1.11 ALTER TABLE Statement") on the
  replica. It is safe to do this when using a storage engine
  other than [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") on the replica,
...
storage engine other than [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5"), the
...
9.5; this includes management nodes, data nodes,
and API or SQL nodes.

Note

In NDB 9.5, you can disable IPv6 support in the
