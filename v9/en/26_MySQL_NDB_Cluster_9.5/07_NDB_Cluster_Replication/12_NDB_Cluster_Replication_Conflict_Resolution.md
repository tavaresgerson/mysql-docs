...
its type cannot be [`TIMESTAMP`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types"), as
...
[Matching with wildcards](mysql-cluster-replication-schema.html#ndb-replication-wildcards "Matching with wildcards"), for information about
...
* On the source writing the binary logs, you must determine
  ...
  by applying the [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") startup option
  [`--ndb-log-updated-only`](mysql-cluster-options-variables.html#option_mysqld_ndb-log-updated-only)
  ...
  (such as [`TEXT`](blob.html "13.3.4 The BLOB and TEXT Types") or
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") columns),
  [`--ndb-log-updated-only`](mysql-cluster-options-variables.html#option_mysqld_ndb-log-updated-only) can
  ...
  See
  [Section 19.5.1.21, “Replication and max\_allowed\_packet”](replication-features-max-allowed-packet.html "19.5.1.21 Replication and max_allowed_packet"),
...
* When using `NDB$MAX_INS()` or
  ...
  `NDB$MAX_INS()` or
  `NDB$MAX_DEL_WIN_INS()`, an incoming write
...
`NDB$MAX_DELETE_WIN()`,
`NDB$MAX_INS()`, and
`NDB$MAX_DEL_WIN_INS()` for timestamp-based
...
[`TIMESTAMP`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types"); instead, its data type
should be [`INT`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")
([`INTEGER`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")) or
[`BIGINT`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT"). The
...
with the
[`--ndb-log-update-as-write`](mysql-cluster-options-variables.html#option_mysqld_ndb-log-update-as-write) option
for [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"), as described later in this
section.
...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"), or start a new
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") instance with different logging
options.
...
* [NDB$MAX\_INS()](mysql-cluster-replication-conflict-resolution.html#mysql-cluster-replication-ndb-max-ins "NDB$MAX_INS()")
* [NDB$MAX\_DEL\_WIN\_INS()](mysql-cluster-replication-conflict-resolution.html#mysql-cluster-replication-ndb-max-del-win-ins "NDB$MAX_DEL_WIN_INS()")
...
[`TimeBetweenEpochs`](mysql-cluster-ndbd-definition.html#ndbparam-ndbd-timebetweenepochs), or
...
section (see
[NDB$OLD()](mysql-cluster-replication-conflict-resolution.html#mysql-cluster-replication-ndb-old "NDB$OLD()")). You must
...
table (see [ndb\_replication Table](mysql-cluster-replication-schema.html#ndb-replication-ndb-replication "ndb_replication Table")).
...
* Tables having [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") or
...
all” rule (see
[NDB$EPOCH()](mysql-cluster-replication-conflict-resolution.html#mysql-cluster-replication-ndb-epoch "NDB$EPOCH()")) but with
...
[`--ndb-log-transaction-id`](mysql-cluster-options-variables.html#option_mysqld_ndb-log-transaction-id) set to
...
[`ndb_conflict_role`](mysql-cluster-options-variables.html#sysvar_ndb_conflict_role) system
...
status variables, or the
`CONFLICT_REFLECTED_OP_PREPARE_COUNT` and
`CONFLICT_REFLECTED_OP_DISCARD_COUNT` columns
of the
[`ndb_replication_applier_status`](performance-schema-ndb-replication-applier-status-table.html "29.12.12.3 The ndb_replication_applier_status Table")
Performance Schema table. The number of changes applied is
simply the difference between these two values (note that
...
corresponding to each [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table for
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine. (An example
...
[`Ndb_conflict_fn_epoch`](mysql-cluster-options-variables.html#statvar_Ndb_conflict_fn_epoch) system
status variable, or by checking the
`CONFLICT_FN_EPOCH` column of the Performance
Schema
[`ndb_replication_applier_status`](performance-schema-ndb-replication-applier-status-table.html "29.12.12.3 The ndb_replication_applier_status Table")
table.
...
[`Ndb_conflict_fn_epoch2`](mysql-cluster-options-variables.html#statvar_Ndb_conflict_fn_epoch2) and
...
[`Ndb_conflict_fn_max`](mysql-cluster-options-variables.html#statvar_Ndb_conflict_fn_max) provides a
...
[`Ndb_conflict_fn_max_ins`](mysql-cluster-options-variables.html#statvar_Ndb_conflict_fn_max_ins) tracks
the number of times that “greater timestamp wins”
handling has been applied to write operations (using
`NDB$MAX_INS()`); a count of the number of
times that “same timestamp wins” handling of writes
...
[`Ndb_conflict_fn_old`](mysql-cluster-options-variables.html#statvar_Ndb_conflict_fn_old). In
addition to incrementing
[`Ndb_conflict_fn_old`](mysql-cluster-options-variables.html#statvar_Ndb_conflict_fn_old), the
primary key of the row that was not used is inserted into an
exceptions table, as
explained elsewhere in this section.

Each of the status variables referenced in the preceding
paragraphs has an equivalent column in the Performance Schema
[`ndb_replication_applier_status`](performance-schema-ndb-replication-applier-status-table.html "29.12.12.3 The ndb_replication_applier_status Table")
table. See the description of this table for more information.
See also [Section 25.4.3.9.3, “NDB Cluster Status Variables”](mysql-cluster-options-variables.html#mysql-cluster-status-variables "25.4.3.9.3 NDB Cluster Status Variables").

#### Examples

The following examples assume that you have already a working
NDB Cluster replication setup, as described in
[Section 25.7.5, “Preparing the NDB Cluster for Replication”](mysql-cluster-replication-preparation.html "25.7.5 Preparing the NDB Cluster for Replication"), and
...
2. On the source, perform this
   ...
   on a specific [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") only, use the actual
   server ID.
...
**NDB$OLD() example.**
Suppose an [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table such as the
...
1. First—and *prior* to creating
   ...
   the
   [`mysql.ndb_replication`](mysql-cluster-replication-schema.html#ndb-replication-ndb-replication "ndb_replication Table")
   table, as shown here:
...
*A*) as a `READ_ROW`
operation (see
[Conflict Resolution Exceptions Table](mysql-cluster-replication-conflict-resolution.html#conflict-resolution-exceptions-table "Conflict Resolution Exceptions Table"), for a
...
tables `t1` and `t2` in
database `test`, and that we wish to use
...
[ndb\_replication Table](mysql-cluster-replication-schema.html#ndb-replication-ndb-replication "ndb_replication Table"), for other
possible values.
...
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client connected to the source, and
`mysqlR>` to indicate a
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client running on the replica.
...
[`INSERT`](insert.html "15.2.7 INSERT Statement") statements on the source.
...
in the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client on the replica:
