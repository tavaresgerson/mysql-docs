...
* [In-Place Upgrade](upgrade-binary-package.html#upgrade-procedure-inplace "In-Place Upgrade")
* [Logical Upgrade](upgrade-binary-package.html#upgrade-procedure-logical "Logical Upgrade")
* [MySQL Cluster Upgrade](upgrade-binary-package.html#upgrading-cluster "MySQL Cluster Upgrade")
...
[**mysqld\_safe**](mysqld-safe.html "6.3.2 mysqld_safe — MySQL Server Startup Script") is not installed. In such
...
1. Review the information in
   [Section 3.1, “Before You Begin”](upgrade-before-you-begin.html "3.1 Before You Begin").
...
3. If you use XA transactions with `InnoDB`,
   run [`XA
   RECOVER`](xa-statements.html "15.3.8.1 XA Transaction SQL Statements") before upgrading to check for uncommitted
   XA transactions. If results are returned, either commit or
   rollback the XA transactions by issuing an
   [`XA
   COMMIT`](xa-statements.html "15.3.8.1 XA Transaction SQL Statements") or
   [`XA
   ROLLBACK`](xa-statements.html "15.3.8.1 XA Transaction SQL Statements") statement.

4. If you normally run your MySQL server configured with
...
6. Upgrade the MySQL binaries or packages. If upgrading a
   binary installation, unpack the new MySQL binary
   distribution package. See
   [Obtain and Unpack the Distribution](binary-installation.html#binary-installation-unpack "Obtain and Unpack the Distribution"). For
   package-based installations, install the new packages.

7. Start the MySQL 9.5 server, using the existing
   ...
   tablespaces, use the
   [`--early-plugin-load`](server-options.html#option_mysqld_early-plugin-load) option to
   load the keyring plugin.

   Note

   The [`--early-plugin-load`](server-options.html#option_mysqld_early-plugin-load)
   server startup option is deprecated, and subject to
   removal in a future version of MySQL. See the description
   of this option for more information.

   When you start the MySQL 9.5 server, it
   ...
   after upgrading from MySQL 9.4 to MySQL
   9.5, you may notice that tables no longer have
   `.frm` files.

   If this step fails, the server reverts all changes to the
   data directory. In this case, you should remove all redo log
   files, start your MySQL 9.4 server on the same
   data directory, and fix the cause of any errors. Then
   perform another slow shutdown of the 9.4
   server and start the MySQL 9.5 server to try
   again.
...
8. In the previous step, the server upgrades the data
   ...
   9.5, and examines all user databases for
...
[**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program"), installing the new MySQL server,
...
MySQL 9.5 release, perform the steps described in
[Section 3.6, “Preparing Your Installation for Upgrade”](upgrade-prerequisites.html "3.6 Preparing Your Installation for Upgrade").
...
2. Export your existing data from the previous MySQL
   ...
   Use the [`--routines`](mysqldump.html#option_mysqldump_routines) and
   [`--events`](mysqldump.html#option_mysqldump_events) options with
   [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") (as shown above) if your
   ...
   [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") utility provided with MySQL
   5.7.9 or higher to create your dump files. The
   [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") utility provided in earlier
   releases uses incorrect syntax for generated column
   definitions (Bug #20769542). You can use the Information
   Schema [`COLUMNS`](information-schema-columns-table.html "28.3.8 The INFORMATION_SCHEMA COLUMNS Table") table to
...
4. Install MySQL 9.5. For installation
   instructions, see [Chapter 2, *Installing MySQL*](installing.html "Chapter 2 Installing MySQL").

5. Initialize a new data directory, as described in
   [Section 2.9.1, “Initializing the Data Directory”](data-directory-initialization.html "2.9.1 Initializing the Data Directory"). For
   example:
...
6. Start the MySQL 9.5 server, using the new data
...
8. Load the previously created dump file into the new MySQL
   ...
   [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") issues DML instructions for
...
9. Perform any remaining upgrade operations:
   ...
   [`--upgrade=FORCE`](server-options.html#option_mysqld_upgrade) option to
   ...
   Upon restart with
   [`--upgrade=FORCE`](server-options.html#option_mysqld_upgrade), the server
   makes any changes required in the `mysql`
   system schema between MySQL 9.4 and MySQL
   9.5, so that you can take advantage of new
   privileges or capabilities. It also brings the Performance
   Schema, `INFORMATION_SCHEMA`, and
   `sys` schema up to date for MySQL
   9.5, and examines all user schemas for
   incompatibilities with the current version of MySQL.
...
[In-Place Upgrade](upgrade-binary-package.html#upgrade-procedure-inplace "In-Place Upgrade"), for use if you are
...
1. Import the data dictionary.
   ...
   [`--upgrade=MINIMAL`](server-options.html#option_mysqld_upgrade) option to
...
2. Upgrade the system tables by restarting each individual
   [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") without the
   [`--upgrade=MINIMAL`](server-options.html#option_mysqld_upgrade) option.