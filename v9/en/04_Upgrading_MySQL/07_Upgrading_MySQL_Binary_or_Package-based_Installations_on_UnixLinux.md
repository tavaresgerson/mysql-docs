...
3. If you use XA transactions with `InnoDB`, run [`XA RECOVER`](xa-statements.html "15.3.8.1 XA Transaction SQL Statements") before upgrading to check for uncommitted XA transactions. If results are returned, either commit or rollback the XA transactions by issuing an [`XA COMMIT`](xa-statements.html "15.3.8.1 XA Transaction SQL Statements") or [`XA ROLLBACK`](xa-statements.html "15.3.8.1 XA Transaction SQL Statements") statement.
...
7. Start the MySQL 9.5 server, using the existing data directory. For example:
   ...
   The `--early-plugin-load` server startup option is deprecated, and subject to removal in a future version of MySQL. See the description of this option for more information.

   When you start the MySQL 9.5 server, it automatically detects whether data dictionary tables are present. If not, the server creates them in the data directory, populates them with metadata, and then proceeds with its normal startup sequence. During this process, the server upgrades metadata for all database objects, including databases, tablespaces, system and user tables, views, and stored programs (stored procedures and functions, triggers, and Event Scheduler events). The server also removes files that previously were used for metadata storage. For example, after upgrading from MySQL 9.4 to MySQL 9.5, you may notice that tables no longer have `.frm` files.

   If this step fails, the server reverts all changes to the data directory. In this case, you should remove all redo log files, start your MySQL 9.4 server on the same data directory, and fix the cause of any errors. Then perform another slow shutdown of the 9.4 server and start the MySQL 9.5 server to try again.

8. In the previous step, the server upgrades the data dictionary as necessary, making any changes required in the `mysql` system database between MySQL 9.4 and MySQL 9.5, so that you can take advantage of new privileges or capabilities. It also brings the Performance Schema, `INFORMATION_SCHEMA`, and `sys` databases up to date for MySQL 9.5, and examines all user databases for incompatibilities with the current version of MySQL.
...
To identify incompatibilities before upgrading to the latest MySQL 9.5 release, perform the steps described in Section 3.6, “Preparing Your Installation for Upgrade”.
...
4. Install MySQL 9.5. For installation instructions, see Chapter 2, *Installing MySQL*.
...
6. Start the MySQL 9.5 server, using the new data directory. For example:
...
9. Perform any remaining upgrade operations:
   ...
   Upon restart with `--upgrade=FORCE`, the server makes any changes required in the `mysql` system schema between MySQL 9.4 and MySQL 9.5, so that you can take advantage of new privileges or capabilities. It also brings the Performance Schema, `INFORMATION_SCHEMA`, and `sys` schema up to date for MySQL 9.5, and examines all user schemas for incompatibilities with the current version of MySQL.
