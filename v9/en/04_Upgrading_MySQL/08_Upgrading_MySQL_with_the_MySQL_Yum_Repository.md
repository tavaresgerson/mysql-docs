...
[Section 2.5.1, “Installing MySQL on Linux Using the MySQL Yum Repository”](linux-installation-yum-repo.html "2.5.1 Installing MySQL on Linux Using the MySQL Yum Repository"), for a list), you
...
* An innovation series, such as MySQL
  9.5, is in a separate track than an
  LTS series, such as MySQL 8.4. The LTS
  series is active by default.
...
1. ### Selecting a Target Series
   ...
   latest version in the release track you have chosen during
   installation (see [Selecting a Release Series](linux-installation-yum-repo.html#yum-repo-select-series "Selecting a Release Series") for
   details), which means, for example, a 8.0.x
   installation is *not* updated to a
   8.4.x release automatically. To update to
   another release series, you must first disable the
   subrepository for the series that has been selected (by
   default, or by yourself) and enable the subrepository for your
   target series. To do that, see the general instructions given
   in [Selecting a Release Series](linux-installation-yum-repo.html#yum-repo-select-series "Selecting a Release Series") for editing the
   subrepository entries in the
   `/etc/yum.repos.d/mysql-community.repo`
   file.

   As a general rule, to upgrade from one bugfix series to
   another, go to the next bugfix series rather than skipping a
   bugfix series. For example, if you are currently running MySQL
   5.7 and wish to upgrade to MySQL
   8.4, upgrade to MySQL 8.0
   first before upgrading to MySQL 8.4. For
   ...
   * For important information about upgrading from MySQL
     ...
     [Upgrading
     from MySQL 5.7 to
     8.0](/doc/refman/8.0/en/upgrading-from-previous-series.html).

   * For important information about upgrading from MySQL
     8.0 to 8.4, see
     [Upgrading
     from MySQL 8.0 to
     8.4](/doc/refman/8.4/en/upgrading-from-previous-series.html).

   * In-place downgrading of MySQL is not supported by the
     MySQL Yum repository. Follow the instructions in
     [Chapter 4, *Downgrading MySQL*](downgrading.html "Chapter 4 Downgrading MySQL").

2. ### Upgrading MySQL

   Upgrade MySQL components using standard yum (or dnf) commands,
...
9.5 shared client libraries and some
...
(see [Adding the MySQL Yum Repository](linux-installation-yum-repo.html#yum-repo-setup "Adding the MySQL Yum Repository")) and install the latest
shared libraries using the instructions given in
[Installing Additional MySQL Products and Components with Yum](linux-installation-yum-repo.html#yum-install-components "Installing Additional MySQL Products and Components with Yum").