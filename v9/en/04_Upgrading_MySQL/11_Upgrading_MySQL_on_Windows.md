...
To upgrade MySQL on Windows, either [download and execute the latest MySQL Server MSI](windows-upgrading.html#windows-upgrading-msi "Upgrading MySQL with MSI") or [use the Windows ZIP archive distribution](windows-upgrading.html#windows-upgrading-zip-distribution "Upgrading MySQL Using the Windows ZIP Distribution").

Note

Unlike MySQL 9.5, MySQL 8.0 uses MySQL Installer to install and upgrade MySQL Server along with most other MySQL products; but MySQL Installer is not available with MySQL 8.1 and higher. However, the configuration functionality used in MySQL Installer is available as of MySQL 8.1 using Section 2.3.2, “Configuration: Using MySQL Configurator” that is bundled with both the MSI and Zip archive.
...
Download and execute the latest MSI. Although upgrading between release series is not directly supported, the "Custom Setup" option allows defining an installation location as otherwise the MSI installs to the standard location, such as `C:\Program Files\MySQL\MySQL Server 9.5\`.

Execute [MySQL Configurator](mysql-configurator.html "2.3.2 Configuration: Using MySQL Configurator") to configure your installation.
...
2. If the server is running, stop it. If the server is installed as a service, stop the service with the following command from the command prompt:
   ...
   If you are not running the MySQL server as a service, use **mysqladmin** to stop it. For example, before upgrading from MySQL 9.4 to 9.5, use **mysqladmin** from MySQL 9.4 as follows:
