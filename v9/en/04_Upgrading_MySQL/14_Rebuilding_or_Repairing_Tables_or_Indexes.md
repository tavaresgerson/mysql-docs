...
To rebuild a table with [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement"), use a “null” alteration; that is, an `ALTER TABLE` statement that “changes” the table to use the storage engine that it already has. For example, if `t1` is an `InnoDB` table, use this statement:
