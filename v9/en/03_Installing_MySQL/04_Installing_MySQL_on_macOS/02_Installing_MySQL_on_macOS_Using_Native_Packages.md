...
1. Download the disk image (`.dmg`) file (the
   ...
   For example, for MySQL server 9.5.0 it might be
   named
   `mysql-9.5.0-macos-10.13-x86_64.pkg`.
...
6. Define a password for the root user, and also toggle whether
   MySQL Server should start after the configuration step is
   complete.

7. Summary is the final step and references
...
`mysql-9.5.0-macos10.15-x86_64.dmg`
installs MySQL into
`/usr/local/mysql-9.5.0-macos10.15-x86_64/` with a symlink to
