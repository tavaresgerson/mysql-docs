...
systems, see [Section 2.3.4, “Troubleshooting a Microsoft Windows MySQL Server Installation”](windows-troubleshooting.html "2.3.4 Troubleshooting a Microsoft Windows MySQL Server Installation").

If you have problems starting the server, here are some things
to try:

* Check the [error log](glossary.html#glos_error_log "error log") to
  see why the server does not start. Log files are located in
  the [data
  directory](glossary.html#glos_data_directory "data directory") (typically `C:\Program
  Files\MySQL\MySQL Server 9.5\data` on
...
* Specify any special options needed by the storage engines
  ...
  [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5")), be sure that you have
  them configured the way you want before starting the server.
  If you are using `InnoDB` tables, see
  [Section 17.8, “InnoDB Configuration”](innodb-configuration.html "17.8 InnoDB Configuration") for guidelines and
  [Section 17.14, “InnoDB Startup Options and System Variables”](innodb-parameters.html "17.14 InnoDB Startup Options and System Variables") for option syntax.
...
* Make sure that the server knows where to find the
  [data directory](glossary.html#glos_data_directory "data directory").
  The [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") server uses this directory as
  ...
  settings are, invoke [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") with the
  [`--verbose`](server-options.html#option_mysqld_verbose) and
  [`--help`](server-options.html#option_mysqld_help) options. If the data
  directory is located somewhere else on your system, specify
  that location with the
  [`--datadir`](server-system-variables.html#sysvar_datadir) option to
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") or [**mysqld\_safe**](mysqld-safe.html "6.3.2 mysqld_safe — MySQL Server Startup Script"),
  on the command line or in an option file. Otherwise, the
  server does not work properly. As an alternative to the
  [`--datadir`](server-system-variables.html#sysvar_datadir) option, you can
  specify [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") the location of the base
  directory under which MySQL is installed with the
  [`--basedir`](server-system-variables.html#sysvar_basedir), and
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") looks for the
  `data` directory there.

  To check the effect of specifying path options, invoke
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") with those options followed by the
  [`--verbose`](server-options.html#option_mysqld_verbose) and
  [`--help`](server-options.html#option_mysqld_help) options. For example,
  if you change location to the directory where
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") is installed and then run the
  ...
  [`--datadir`](server-system-variables.html#sysvar_datadir) as well, but
  [`--verbose`](server-options.html#option_mysqld_verbose) and
  [`--help`](server-options.html#option_mysqld_help) must be the last
  options.

  Once you determine the path settings you want, start the
  server without [`--verbose`](server-options.html#option_mysqld_verbose) and
  [`--help`](server-options.html#option_mysqld_help).

  If [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") is currently running, you can
...
* Make sure that the server can access the
  ...
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"), this means that the privileges of
  ...
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") to access the directories it uses
...
* Verify that the network interfaces the server wants to use
  ...
  other program (perhaps another [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
  server) is using the TCP/IP port or Unix socket file that
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") is trying to use:
  ...
  another [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") server running. If so,
  shut down the server before starting
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") again. (If another server is
  ...
  the TCP/IP port that [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") is trying to
  use. Track down what program this is and disable it, or tell
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") to listen to a different port with
  the [`--port`](server-options.html#option_mysqld_port) option. In this
...
* If you cannot get [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") to start, try to
  make a trace file to find the problem by using the
  [`--debug`](server-options.html#option_mysqld_debug) option. See
