...
[Section 2.3.1, “Choosing an Installation Package”](windows-choosing-package.html "2.3.1 Choosing an Installation Package").

Note

MySQL 9.5 Server requires the Microsoft Visual C++
...
1. Download the MSI from <https://dev.mysql.com/downloads/> and
   ...
   Microsoft Windows Start menu under the `MySQL`
   group.

2. Upon completion, the installation wizard prompts to execute
   [MySQL Configurator](mysql-configurator.html "2.3.2 Configuration: Using MySQL Configurator"). Execute it
...
9.5` for installations using the MSI,
although the MSI Custom setup type allows using
a different location. If you use the ZIP archive method to install
MySQL, install it there are elsewhere, such as
`C:\mysql`. Regardless, the layout of the
...
<td><a class="link" href="mysqld.html" title="6.3.1 mysqld — The MySQL Server"><span class="command"><strong>mysqld</strong></span></a> server, client, and utility programs</td>
<td></td>
</tr><tr>
<th scope="row"><code class="filename">%PROGRAMDATA%\MySQL\MySQL Server 9.5\</code></th>
...
<td>With the MSI, use the <code class="literal">Custom</code> type to include this
          optional component.</td>
</tr><tr>
...
Use the standard
[msiexec](https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/msiexec#install-options)
options for a silent installation. This example includes
`/i` for a normal installation,
`/qn` to not show a GUI and to avoid user
interaction, and `/lv` to write verbose
...
see [Section 2.3.3.8, “Starting MySQL as a Windows Service”](windows-start-service.html "2.3.3.8 Starting MySQL as a Windows Service").

To accommodate the [`RESTART`](restart.html "15.7.8.8 RESTART Statement") statement,
...
case, there are two [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") processes. If
[`RESTART`](restart.html "15.7.8.8 RESTART Statement") capability is not required,
the server can be started with the
[`--no-monitor`](server-options.html#option_mysqld_no-monitor) option. See
...
platform, see [Section 2.3.6, “Windows Platform Restrictions”](windows-restrictions.html "2.3.6 Windows Platform Restrictions").
...
* To use MySQL server with .NET applications, you must have the
  ...
  configuration instructions, see [MySQL Connector/NET Developer Guide](/doc/connector-net/en/).
...
choosing a package, see [Section 2.3.1, “Choosing an Installation Package”](windows-choosing-package.html "2.3.1 Choosing an Installation Package").
...
* **MySQL and Virus Checking
  ...
  ([`datadir`](server-system-variables.html#sysvar_datadir)) used to store your
  ...
  [`tmpdir`](server-options.html#option_mysqld_tmpdir) parameter to your
  `my.ini` configuration file. For more
  information, see [Section 2.3.3.2, “Creating an Option File”](windows-create-option-file.html "2.3.3.2 Creating an Option File").