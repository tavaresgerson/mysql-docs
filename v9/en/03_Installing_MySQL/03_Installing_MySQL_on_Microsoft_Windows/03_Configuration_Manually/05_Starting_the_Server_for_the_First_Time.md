...
Files\MySQL\MySQL Server 9.5`. Adjust the
...
If you omit the [`--console`](server-options.html#option_mysqld_console) option,
the server writes diagnostic output to the error log in the data
directory (`C:\Program Files\MySQL\MySQL Server
9.5\data` by default). The error log is
the file with the `.err` extension, and may
be set using the [`--log-error`](server-options.html#option_mysqld_log-error)
option.
