...
* From the MySQL software repositories:
  ...
  + The MySQL SLES repository (see
    [Section 2.5.3, “Using the MySQL SLES Repository”](linux-installation-sles-repo.html "2.5.3 Using the MySQL SLES Repository") for
    details).
...
<td>The source code RPM looks similar to
            mysql-community-9.5.0-1.el7.src.rpm, depending
            on selected OS</td>
</tr><tr>
...
<td><code class="literal">sl5</code></td>
...
* The `mysql` user is created using the
  ...
  server host (see
  [Creating
  the mysql User and Group](/doc/mysql-secure-deployment-guide/en/secure-deployment-install.html#secure-deployment-mysql-user) for details). To switch to
...
`/usr/lib64/mysql/plugin/debug` and is
configurable with [`plugin_dir`](server-system-variables.html#sysvar_plugin_dir).
