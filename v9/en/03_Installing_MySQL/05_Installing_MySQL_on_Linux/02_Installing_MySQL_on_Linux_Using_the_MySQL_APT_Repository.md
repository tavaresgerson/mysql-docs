...
This section provides guidance on installing MySQL using the MySQL
APT repository.
...
The following instructions assume that no version of MySQL
(whether distributed by Oracle or other parties) has already
been installed on your system; if that is not the case, follow
the instructions given in
[Replacing a Native Distribution of MySQL Using the MySQL APT Repository](linux-installation-apt-repo.html#repo-qg-apt-replacing "Replacing a Native Distribution of MySQL Using the MySQL APT Repository") or
[Replacing a MySQL Server Installed by a Direct deb Package Download](linux-installation-apt-repo.html#repo-qg-apt-replace-direct "Replacing a MySQL Server Installed by a Direct deb Package Download") instead.
...
2. Select and download the release package for your Linux
   distribution.

   Although this is not required for each update, it does update
   MySQL repository information to include the current
   information, which includes adding a new LTS series.

3. Install the downloaded release package with the following
   command, replacing
   *`version-specific-package-name`* with
   the name of the downloaded package (preceded by its path, if
   you are not running the command inside the folder where the
   package is):
   ...
   For example, for version *`w.x.y-z`* of
   ...
   Note that the same package works on all supported Debian and
   Ubuntu platforms.

4. During the installation of the package, you will be asked to
   choose the versions of the MySQL server and other components
   (for example, the MySQL Workbench) that you want to install.
   If you are not sure which version to choose, do not change the
   default options selected for you. You can also choose
   none if you do not want a particular
   component to be installed. After making the choices for all
   components, choose Ok to finish the
   configuration and installation of the release package.

   Note

   The innovation track, which includes MySQL
   9.5, contains "-innovation-" in the
   component name.

   You can always change your choices for the versions later; see
   [Selecting a Major Release Version](linux-installation-apt-repo.html#repo-qg-apt-select-series "Selecting a Major Release Version") for instructions.

5. Update package information from the MySQL APT repository with
   the following command (*this step is
   mandatory*):
...
[Appendix A: Adding and Configuring the MySQL APT Repository Manually](linux-installation-apt-repo.html#repo-qg-apt-repo-manual-setup "Appendix A: Adding and Configuring the MySQL APT Repository Manually") for details.

Note

Once the MySQL APT repository is enabled on your system, you
will no longer be able to install any MySQL packages from your
platform's native software repositories until the MySQL APT
repository is disabled.

Note

Once the MySQL APT repository is enabled on your system, any
system-wide upgrade by the **apt-get upgrade**
command will automatically upgrade the MySQL packages on your
system and also replace any native MySQL packages you installed
from your Linux distribution's software repository, if APT finds
replacements for them from within the MySQL APT repository.
...
configuration package (see [Adding the MySQL Apt Repository](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository")).
...
you want. Make your selection and choose Ok.
...
when you use the **apt-get install** command next
time.
...
During the installation, you are asked to supply a password for
the root user for your MySQL installation.

Important

Make sure you remember the root password you set. Users who
want to set a password later can leave the
password field blank in the dialogue box
and just press Ok; in that case, root
access to the server will be authenticated by
[Section 8.4.1.10, “Socket Peer-Credential Pluggable Authentication”](socket-pluggable-authentication.html "8.4.1.10 Socket Peer-Credential Pluggable Authentication") for
connections using a Unix socket file. You can set the root
password later using the program
[**mysql\_secure\_installation**](mysql-secure-installation.html "6.4.2 mysql_secure_installation — Improve MySQL Installation Security").

#### Starting and Stopping the MySQL Server

The MySQL server is started automatically after installation. You
can check the status of the MySQL server with the following
command:
...
[Adding the MySQL Apt Repository](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository") for instructions), first, use the
...
1. Add the MySQL APT repository to your system's repository list
   and choose the major release series you want (see
   [Adding the MySQL Apt Repository](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository") for instructions).

2. Update package information from the MySQL APT repository with
   the following command (*this step is
   mandatory*):
...
4. Download the source code for the major components of MySQL and
   ...
   `deb` packages for installing the various
   MySQL components are created.

5. Pick the `deb` packages for the MySQL
   ...
   * Preconfigure the MySQL server package with the following
     ...
     [Installing MySQL with APT](linux-installation-apt-repo.html#repo-qg-apt-installing "Installing MySQL with APT") above. You
...
[Starting and Stopping the MySQL Server](linux-installation-apt-repo.html#repo-qg-apt-starting-and-stopping-server "Starting and Stopping the MySQL Server").
...
* Before performing any upgrade to MySQL, follow carefully the
  ...
  instructions discussed there, *it is especially
...
* The following instructions assume that MySQL has been
  ...
  [Replacing a Native Distribution of MySQL Using the MySQL APT Repository](linux-installation-apt-repo.html#repo-qg-apt-replacing "Replacing a Native Distribution of MySQL Using the MySQL APT Repository") or
  [Replacing a MySQL Server Installed by a Direct deb Package Download](linux-installation-apt-repo.html#repo-qg-apt-replace-direct "Replacing a MySQL Server Installed by a Direct deb Package Download") instead. Also
...
1. Make sure you already have the MySQL APT repository on your
   system's repository list (see [Adding the MySQL Apt Repository](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository")
   for instructions).

2. Make sure you have the most up-to-date package information on
...
3. Note that, by default, the MySQL APT repository will update
   MySQL to the release series you have selected when you were
   [adding the MySQL APT repository
   to your system](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository"). If you want to upgrade to another
   release series, select it by following the steps given in
   [Selecting a Major Release Version](linux-installation-apt-repo.html#repo-qg-apt-select-series "Selecting a Major Release Version").

   As a general rule, to upgrade from one release series to
   another, go to the next series rather than skipping a series.
   For example, if you are currently running MySQL 5.7 and wish
   to upgrade to a newer series, upgrade to MySQL 8.0 first
   before upgrading to 8.4.

   Important

   In-place downgrading of MySQL is not supported by the
   MySQL APT repository. Follow the instructions in
...
4. Upgrade MySQL by the following command:
   ...
   MySQL package, use the same **apt-get install**
   ...
   If you perform a system-wide upgrade using **apt-get
   upgrade**, only the MySQL library and development
   packages are upgraded with newer versions (if available). To
   upgrade other components including the server, client, test
   suite, etc., use the **apt-get install**
   command.
...
1. ##### Backing Up Your Database
   ...
   repository. See [Chapter 9, *Backup and Recovery*](backup-and-recovery.html "Chapter 9 Backup and Recovery") for
   instructions.
...
2. ##### Adding the MySQL APT Repository and Selecting a Release Series
   ...
   instructions given in [Adding the MySQL Apt Repository](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository").
...
3. ##### Replacing the Native Distribution by an APT Update
   ...
   [Step 4](linux-installation-apt-repo.html#apt-repo-upgrade-step4) in
   [Upgrading MySQL with the MySQL APT Repository](linux-installation-apt-repo.html#repo-qg-apt-upgrading "Upgrading MySQL with the MySQL APT Repository").
...
**dpkg -P** command might impact the newly
...
Download page](https://dev.mysql.com/downloads/) or from the MySQL APT repository. The
...
`deb` packages and now want to replace the
installation using the ones from the MySQL APT repository, follow
these steps:

1. Back up your database. See
   [Chapter 9, *Backup and Recovery*](backup-and-recovery.html "Chapter 9 Backup and Recovery") for instructions.

2. Follow the steps given previously for
   [adding the MySQL APT
   repository](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository").
...
5. If needed, restore the data on the new MySQL installation. See
   [Chapter 9, *Backup and Recovery*](backup-and-recovery.html "Chapter 9 Backup and Recovery") for instructions.
...
replacing **`package-name`** with the name of the
...
[Installing Additional MySQL Products and Components with APT](linux-installation-apt-repo.html#apt-repo-install-components "Installing Additional MySQL Products and Components with APT") for more details):
...
command (see [Replacing the Native Distribution by an APT Update](linux-installation-apt-repo.html#repo-qg-apt-replace-native "Replacing the Native Distribution by an APT Update") for more
details).
...
systems (see [Adding the MySQL Apt Repository](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository")) and installing the
...
* The MySQL APT repository supports installation of MySQL NDB Cluster on
  ...
  [Installing NDB Cluster Using .deb Files](/doc/refman/5.7/en/mysql-cluster-install-debian.html).
...
1. ##### Adding the MySQL APT Repository for MySQL NDB Cluster
   ...
   Follow the steps in [Adding the MySQL Apt Repository](linux-installation-apt-repo.html#apt-repo-setup "Adding the MySQL Apt Repository") to add
   ...
   choose “MySQL Server & Cluster”; when asked
   ...
   [Selecting a Major Release Version](linux-installation-apt-repo.html#repo-qg-apt-select-series "Selecting a Major Release Version") to select MySQL NDB Cluster
   for installation. When you are asked which MySQL product you
   want to configure, choose “MySQL Server &
   Cluster”; when asked which version you wish to receive,
   choose
   “mysql-cluster-*`x`*.*`y`*.”
...
2. ##### Installing MySQL NDB Cluster
   ...
   * Install the components for SQL nodes:
     ...
     password given in [Installing MySQL with APT](linux-installation-apt-repo.html#repo-qg-apt-installing "Installing MySQL with APT")
...
3. ##### Configuring and Starting MySQL NDB Cluster
   ...
   See [Section 25.3.3, “Initial Configuration of NDB Cluster”](mysql-cluster-install-configuration.html "25.3.3 Initial Configuration of NDB Cluster") on
...
[Adding the MySQL Apt Repository for MySQL NDB Cluster](linux-installation-apt-repo.html#apt-repo-setup-cluster "Adding the MySQL APT Repository for MySQL NDB Cluster")), follow the same steps
given in [Installing Additional MySQL Products and Components with APT](linux-installation-apt-repo.html#apt-repo-install-components "Installing Additional MySQL Products and Components with APT").
...
* Download the MySQL GPG Public key (see
  [Section 2.1.4.2, “Signature Checking Using GnuPG”](checking-gpg-signature.html "2.1.4.2 Signature Checking Using GnuPG") on how to do that)
...
* Alternatively, you can download the GPG key to your APT
  ...
  `A8D3785C`, as shown above. For earlier
  MySQL releases, the keyID is `3A79BD29`.
  Using an incorrect key can cause a key verification error.

* Create a file named
  `/etc/apt/sources.list.d/mysql.list`, and
  ...
  + Choose the appropriate version name for the version of
    your system; examples include “bookworm”
    (for Debian 12) and “jammy” (for Ubuntu
    22.04).

  + For installing the MySQL server, client, and database
    common files, choose “mysql-8.4”,
    “mysql-8.0”, or
    “mysql-innovation” (for MySQL
    9.5) according to the
    [MySQL series](mysql-releases.html "1.3 MySQL Releases: Innovation and LTS") you
    want. To switch to another release series later, come
    back and adjust the entry with your new choice. This
    also includes access to tools such as MySQL Router and
    MySQL Shell.
  ...
  + Include “mysql-tools” to install a
    connector.

  For example, on the Ubuntu 22.04 platform use these lines in
  your `mysql.list` files to install MySQL
  8.4 and the latest MySQL Connectors from the MySQL APT
  repository:
...
[Installing MySQL with APT](linux-installation-apt-repo.html#repo-qg-apt-installing "Installing MySQL with APT") or
[Installing Additional MySQL Products and Components with APT](linux-installation-apt-repo.html#apt-repo-install-components "Installing Additional MySQL Products and Components with APT").