...
* The second number (**0**) is the minor version number. The minor version number does not change for an LTS series, but it does change for an [Innovation series](glossary.html#glos_innovation_series "Innovation Series").
...
For SUSE Linux Enterprise Server (SLES) platforms, MySQL can be installed using the [MySQL SLES Repository](https://dev.mysql.com/downloads/repo/suse/). See Section 2.5.3, “Using the MySQL SLES Repository” for details.
