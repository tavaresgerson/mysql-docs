...
* **Primary Keys.**
  ...
  SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement to
  `ON` for a Group Replication channel.
  ...
  you set `sql_require_primary_key=ON` or
...
* **Replica Updates Logged.**
  Set
  [`log_replica_updates=ON`](replication-options-binary-log.html#sysvar_log_replica_updates) if
  it is not already enabled. (In MySQL 9.5,
...
* **Binary Log Row Format.**
  Set [`binlog_format=ROW`](replication-options-binary-log.html#sysvar_binlog_format) if
  necessary; in MySQL 9.5, this is the default.
  ...
  setting for `REQUIRE_ROW_FORMAT` is
...
* **Multithreaded Appliers.**
  ...
  multithreaded by default. The default is 4 parallel
  applier threads; up to 1024 parallel applier threads can
  be specified. For a multithreaded replica, the following
  is also required:
...
* **Detached XA transactions.**
  MySQL 9.5 and later supports detached XA
