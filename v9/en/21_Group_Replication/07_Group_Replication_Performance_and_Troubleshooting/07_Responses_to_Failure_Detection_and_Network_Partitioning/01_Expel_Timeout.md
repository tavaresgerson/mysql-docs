...
* If a suspect member becomes active only after the suspicion
  ...
  to make the member try to rejoin the group automatically at
  this point. This feature is active by default in MySQL
  9.5, and the member makes three auto-rejoin
  attempts. If the auto-rejoin procedure does not succeed or
  is not attempted, the expelled member then follows the exit
  action specified by
