...
1. If `READ_ONLY` is the exit action, the
   ...
   `ERROR` in the view of the group.
...
2. If `OFFLINE_MODE` is the exit action, the
   ...
   [`SUPER`](privileges-provided.html#priv_super) privilege. This exit
   ...
   member's status is displayed as `ERROR` in
   the view of the group (not `OFFLINE`, which
...
recommend that
[`super_read_only=ON`](server-system-variables.html#sysvar_super_read_only) is set in
