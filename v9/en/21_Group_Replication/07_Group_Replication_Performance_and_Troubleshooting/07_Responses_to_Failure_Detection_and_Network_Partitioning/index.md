...
1. In MySQL 9.5, when a suspicion is created, a
   waiting period of 5 seconds is added before the suspicion
...
2. If an expelled member resumes communication and realises that
   it was expelled, it automatically makes three attempts to
   rejoin the group (with 5 minutes between each attempt); if
   this auto-rejoin procedure does not work, it then stops trying
   to rejoin the group.
...
increasing the expel timeout and auto-rejoin attempts. While a
member is undergoing any of the default behaviors described
previously, although it does not accept writes, reads can still be
performed if the member is still communicating with clients, with
an increasing likelihood of stale reads over time. If avoiding
stale reads is a higher priority for you than avoiding operator
intervention, consider reducing the expel timeout and auto-rejoin
...
example, in a group of 5 servers (S1, S2, S3, S4, S5), if there is
...
unreachable, therefore the group cannot achieve quorum. For a
detailed description of this scenario, see
[Section 20.7.8, “Handling a Network Partition and Loss of Quorum”](group-replication-network-partitioning.html "20.7.8 Handling a Network Partition and Loss of Quorum"). In this
