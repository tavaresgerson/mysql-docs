...
If you want to use remote cloning operations for distributed
...
[`BACKUP_ADMIN`](privileges-provided.html#priv_backup-admin) permission to the
...
plugin, see [Section 7.6.6, “The Clone Plugin”](clone-plugin.html "7.6.6 The Clone Plugin") . Detailed
prerequisites for a remote cloning operation are covered in
[Section 7.6.6.3, “Cloning Remote Data”](clone-plugin-remote.html "7.6.6.3 Cloning Remote Data") . For Group Replication,
...
* If distributed recovery is configured to use SSL
  ...
  [`clone_ssl_key`](clone-plugin-options-variables.html#sysvar_clone_ssl_key)) to match
...
* To invoke the remote cloning operation on the recipient,
  ...
  the [`CLONE_ADMIN`](privileges-provided.html#priv_clone-admin) privilege,
...
* As the clone user on the donor for the remote cloning
  ...
  in [Section 20.2.1.3, “User Credentials For Distributed Recovery”](group-replication-user-credentials.html "20.2.1.3 User Credentials For Distributed Recovery")).
  You must therefore give the
  [`BACKUP_ADMIN`](privileges-provided.html#priv_backup-admin) privilege to
...
* If you use [`START
  GROUP_REPLICATION`](start-group-replication.html "15.4.3.1 START GROUP_REPLICATION Statement") to specify the replication
  user credentials on a server that previously supplied the
  user credentials using [`CHANGE
  REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement"), ensure that you remove
  the user credentials from the replication metadata
  repositories before any remote cloning operations take
  place. Also ensure that
...
[`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed) sets of the
...
[`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged) sets of the
...
[Section 7.6.6.10, “Monitoring Cloning Operations”](clone-plugin-monitoring.html "7.6.6.10 Monitoring Cloning Operations") for information on
...
GROUP_REPLICATION`](start-group-replication.html "15.4.3.1 START GROUP_REPLICATION Statement") manually again following this
restart. If
...
SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement, they are transferred to and
used by the joining member after cloning, and they must be
valid there. With stored credentials, all group members that
...
GROUP_REPLICATION`](start-group-replication.html "15.4.3.1 START GROUP_REPLICATION Statement") statement, these are used to start
...
[Section 20.6.3, “Securing Distributed Recovery Connections”](group-replication-distributed-recovery-securing.html "20.6.3 Securing Distributed Recovery Connections"),
and use [`START GROUP_REPLICATION`](start-group-replication.html "15.4.3.1 START GROUP_REPLICATION Statement")
to supply them instead.

If a `PRIVILEGE_CHECKS_USER` account has been
used to help secure the replication appliers (see
[Section 19.3.3.2, “Privilege Checks For Group Replication Channels”](replication-privilege-checks-gr.html "19.3.3.2 Privilege Checks For Group Replication Channels")), the
`PRIVILEGE_CHECKS_USER` account and related
settings from the donor are cloned to the joining member. If
the joining member is set to start Group Replication on boot,
it automatically uses the account for privilege checks on the
...
You can initiate a cloning operation manually if the operation
involves a group member on which Group Replication is running,
provided that the cloning operation does not remove and
replace the data on the recipient. The statement to initiate
the cloning operation must therefore include the `DATA
