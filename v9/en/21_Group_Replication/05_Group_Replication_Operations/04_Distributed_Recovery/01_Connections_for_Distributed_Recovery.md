...
* For distributed recovery, by default, group members offer
  their standard SQL client connection to joining members, as
  specified by [`hostname`](server-system-variables.html#sysvar_hostname) and
  [`port`](server-system-variables.html#sysvar_port). If an alternative
  port number is specified by
  [`report_port`](replication-options-replica.html#sysvar_report_port), that one is
  used instead.

* Group members may instead advertise an alternative list of
  distributed recovery endpoints as dedicated client
  connections for joining members, allowing you to control
  distributed recovery traffic separately from connections by
  regular client users of the member. A member transmits the
  list of distributed recovery endpoints specified by
...
[`hostname`](server-system-variables.html#sysvar_hostname) system variable. It
...
Schema table
[`replication_group_members`](performance-schema-replication-group-members-table.html "29.12.11.18 The replication_group_members Table"). If
...
[`report_host`](replication-options-replica.html#sysvar_report_host) system variable
...
specified by the [`port`](server-system-variables.html#sysvar_port),
[`report_port`](replication-options-replica.html#sysvar_report_port), or
[`admin_port`](server-system-variables.html#sysvar_admin_port) system variable.
The server must listen for TCP/IP connections on these ports.
If you specify the
[`admin_port`](server-system-variables.html#sysvar_admin_port), the replication
...
[`admin_port`](server-system-variables.html#sysvar_admin_port) keeps distributed
...
[`START GROUP_REPLICATION`](start-group-replication.html "15.4.3.1 START GROUP_REPLICATION Statement")
...
You can optionally configure compression for distributed
recovery by the method of state transfer from a donor's
binary log. Compression can benefit distributed recovery where
network bandwidth is limited and the donor has to transfer
many transactions to the joining member. The
...
[Section 20.2.1.3, “User Credentials For Distributed Recovery”](group-replication-user-credentials.html "20.2.1.3 User Credentials For Distributed Recovery"). For
...
([`clone_ssl_ca`](clone-plugin-options-variables.html#sysvar_clone_ssl_ca),
[`clone_ssl_cert`](clone-plugin-options-variables.html#sysvar_clone_ssl_cert), and
...
RSA key pairs are used for password exchange. In this case,
either use the
