...
GROUP_REPLICATION`](stop-group-replication.html "15.4.3.2 STOP GROUP_REPLICATION Statement") statement or system shutdown, the
...
the [`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed) GTID set, and
...
1. For each group member in turn, in any order:
   ...
   1. Connect a client to the group member. If Group Replication
      ...
      GROUP_REPLICATION`](stop-group-replication.html "15.4.3.2 STOP GROUP_REPLICATION Statement") statement and wait for Group
      Replication to stop.
   ...
   4. Collect the following information from the group member:
      ...
      * The contents of the
        [`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed) GTID
...
2. When you have collected the transaction sets from all the
   ...
   transactions ([`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed))
...
5. Add each of the other members back into the group, in any
   ...
   GROUP_REPLICATION`](start-group-replication.html "15.4.3.1 START GROUP_REPLICATION Statement") statement on each of them:
