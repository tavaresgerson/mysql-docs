...
[Section 20.8.3.3, “Group Replication Online Upgrade Methods”](group-replication-online-upgrade-methods.html "20.8.3.3 Group Replication Online Upgrade Methods"). The
...
* Disable Group Replication from starting up automatically so
  ...
  [`group_replication_start_on_boot=0`](group-replication-system-variables.html#sysvar_group_replication_start_on_boot).

  Important

  If an upgraded member has [`group_replication_start_on_boot=1`](group-replication-system-variables.html#sysvar_group_replication_start_on_boot) then it could
...
* Stop the member, for example using [**mysqladmin
  shutdown**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") or the
  [`SHUTDOWN`](shutdown.html "15.7.8.9 SHUTDOWN Statement") statement. Any other
  members in the group continue running.

* Upgrade the member, using the in-place or provisioning
  approach. See [Chapter 3, *Upgrading MySQL*](upgrading.html "Chapter 3 Upgrading MySQL") for details. When
  restarting the upgraded member, because
  [`group_replication_start_on_boot`](group-replication-system-variables.html#sysvar_group_replication_start_on_boot) is set to 0, Group
...
* Connect to the upgraded member and issue
  [`START GROUP_REPLICATION`](start-group-replication.html "15.4.3.1 START GROUP_REPLICATION Statement"). This
...
joins with [`super_read_only=on`](server-system-variables.html#sysvar_super_read_only).
...
When all members of a group have been upgraded to the same
