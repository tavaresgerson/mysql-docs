...
* `host` is the
  [`hostname`](server-system-variables.html#sysvar_hostname) address of the
...
* `port` is the
  [`port`](server-system-variables.html#sysvar_port) number of the donor
  MySQL server instance. (The X Protocol port specified by
  [`mysqlx_port`](x-plugin-options-system-variables.html#sysvar_mysqlx_port) is not
...
* `[REQUIRE [NO] SSL]` explicitly specifies
  ...
  [Configuring an Encrypted Connection for Cloning](clone-plugin-remote.html#clone-plugin-remote-ssl "Configuring an Encrypted Connection for Cloning").
...
* On the donor, the clone user requires the
  [`BACKUP_ADMIN`](privileges-provided.html#priv_backup-admin) privilege for
...
* On the recipient, the clone user requires the
  [`CLONE_ADMIN`](privileges-provided.html#priv_clone-admin) privilege for
  replacing recipient data, blocking DDL on the recipient
  during the cloning operation, and automatically restarting
  the server. The [`CLONE_ADMIN`](privileges-provided.html#priv_clone-admin)
  privilege includes
...
[`CLONE
INSTANCE`](clone.html "15.7.5 CLONE Statement") statement is executed:

* The donor and recipient must be the same MySQL server
  series, such as 8.4.0 and 8.4.11. To determine the MySQL
  server version, issue the following query:
...
* `InnoDB` permits creating some tablespace
  ...
  [`FILES`](information-schema-files-table.html "28.3.15 The INFORMATION_SCHEMA FILES Table") table to identify
...
* Plugins that are active on the donor, including any
  ...
  [`SHOW PLUGINS`](show-plugins.html "15.7.7.28 SHOW PLUGINS Statement") statement or
...
* The same [`innodb_page_size`](innodb-parameters.html#sysvar_innodb_page_size)
  ...
  VARIABLES`](show-variables.html "15.7.7.42 SHOW VARIABLES Statement") syntax.
...
* If cloning encrypted or page-compressed data, the donor
  ...
  use them, see
  [Section 7.6.6.5, “Cloning Encrypted Data”](clone-plugin-encrypted-data.html "7.6.6.5 Cloning Encrypted Data"), and
  [Section 7.6.6.6, “Cloning Compressed Data”](clone-plugin-compressed-data.html "7.6.6.6 Cloning Compressed Data"). To
...
* There must be no other cloning operation running. Only a
  ...
  [`clone_status`](performance-schema-clone-status-table.html "29.12.19.1 The clone_status Table") table. See
  [Monitoring Cloning Operations using Performance Schema Clone Tables](clone-plugin-monitoring.html#clone-plugin-monitoring-performance-schema-clone-tables "Monitoring Cloning Operations using Performance Schema Clone Tables").
...
* Undo tablespace file names on the donor must be unique.
  ...
  are not permitted for this reason. An error is reported if
  duplicate undo tablespace file names are encountered
  during a cloning operation.

  To view undo tablespace file names on the donor to ensure
  that they are unique, query the
  ...
  files, see [Section 17.6.3.4, “Undo Tablespaces”](innodb-undo-tablespaces.html "17.6.3.4 Undo Tablespaces").
...
* By default, the recipient MySQL server instance is
  ...
  [Monitoring Cloning Operations using Performance Schema Clone Tables](clone-plugin-monitoring.html#clone-plugin-monitoring-performance-schema-clone-tables "Monitoring Cloning Operations using Performance Schema Clone Tables").)
  The [`RESTART`](restart.html "15.7.8.8 RESTART Statement") statement has
  the same monitoring process requirement. For more
  information, see [Section 15.7.8.8, “RESTART Statement”](restart.html "15.7.8.8 RESTART Statement"). This
...
See [Remote Cloning Prerequisites](clone-plugin-remote.html#clone-remote-prerequisites "Remote Cloning Prerequisites").

1. Login to the donor MySQL server instance with an
   administrative user account.

   1. Create a clone user with the
      [`BACKUP_ADMIN`](privileges-provided.html#priv_backup-admin) privilege.
...
2. Login to the recipient MySQL server instance with an
   ...
   1. Create a clone user with the
      [`CLONE_ADMIN`](privileges-provided.html#priv_clone-admin) privilege.
...
3. Log on to the recipient MySQL server instance as the clone
   ...
   and execute the
   [`CLONE
   INSTANCE`](clone.html "15.7.5 CLONE Statement") statement.
...
[Cloning Remote Data](clone-plugin-remote.html#clone-plugin-remote-example "Cloning Remote Data") with one
exception: The [`CLONE
INSTANCE`](clone.html "15.7.5 CLONE Statement") statement must include the `DATA
...
refer to [Section 8.3.1, “Configuring MySQL to Use Encrypted Connections”](using-encrypted-connections.html "8.3.1 Configuring MySQL to Use Encrypted Connections") for
...
2. Configure the following SSL options on the recipient MySQL
   ...
   * [`clone_ssl_key`](clone-plugin-options-variables.html#sysvar_clone_ssl_key)
...
3. To require that an encrypted connection is used, include
   ...
   [`CLONE`](clone.html "15.7.5 CLONE Statement") statement on the
   recipient.
