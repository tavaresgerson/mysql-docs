...
* Plugins for authenticating attempts by clients to connect to
  ...
  protocols. See [Section 8.2.17, “Pluggable Authentication”](pluggable-authentication.html "8.2.17 Pluggable Authentication").
...
* A connection control plugin that enables administrators to
  ...
  [Section 8.4.3, “Connection Control Plugins”](connection-control-plugin.html "8.4.3 Connection Control Plugins").

  The Connection Control plugin is deprecated, and subject to
  removal in a future MySQL release. Users are encouraged to
  migrate to the Connection Control component; see
  [Section 8.4.2, “The Connection Control Component”](connection-control-component.html "8.4.2 The Connection Control Component"), for more
  information.
...
* Semisynchronous replication plugins implement an interface to
  ...
  See [Section 19.4.10, “Semisynchronous Replication”](replication-semisync.html "19.4.10 Semisynchronous Replication").
...
* MySQL Enterprise Edition includes a thread pool plugin that manages connection
  ...
  connections. See [Section 7.6.3, “MySQL Enterprise Thread Pool”](thread-pool.html "7.6.3 MySQL Enterprise Thread Pool").

* MySQL Enterprise Edition includes an audit plugin for monitoring and logging of
  connection and query activity. See [Section 8.4.6, “MySQL Enterprise Audit”](audit-log.html "8.4.6 MySQL Enterprise Audit").
...
* Query rewrite plugins examine statements received by MySQL
  ...
  them. See [Section 7.6.4, “The Rewriter Query Rewrite Plugin”](rewriter-query-rewrite-plugin.html "7.6.4 The Rewriter Query Rewrite Plugin"), and
...
* Version Tokens enables creation of and synchronization around
  ...
  functions. See [Version Tokens](/doc/refman/8.4/en/version-tokens.html).

* Keyring plugins provide secure storage for sensitive
  information. See [Section 8.4.5, “The MySQL Keyring”](keyring.html "8.4.5 The MySQL Keyring").
...
* X Plugin extends MySQL Server to be able to function as a
  ...
  as a document store. See [Section 22.5, “X Plugin”](x-plugin.html "22.5 X Plugin").

* Clone permits cloning `InnoDB` data from a
  local or remote MySQL server instance. See
  [Section 7.6.6, “The Clone Plugin”](clone-plugin.html "7.6.6 The Clone Plugin").
...
plugins, see [The MySQL Plugin API](/doc/extending-mysql/9.5/en/plugin-api.html).