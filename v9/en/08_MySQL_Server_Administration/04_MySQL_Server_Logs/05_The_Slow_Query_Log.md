...
[`min_examined_row_limit`](server-system-variables.html#sysvar_min_examined_row_limit) rows to be
...
[**mysqldumpslow**](mysqldumpslow.html "6.6.10 mysqldumpslow — Summarize Slow Query Log Files") command to process a slow query
...
time. [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") writes a statement to the slow
...
[`long_query_time`](server-system-variables.html#sysvar_long_query_time) are 0 and 10,
...
be changed using
[`log_slow_admin_statements`](server-system-variables.html#sysvar_log_slow_admin_statements) and
...
[`--slow_query_log[={0|1}]`](server-system-variables.html#sysvar_slow_query_log). With no
argument or an argument of 1,
[`--slow_query_log`](server-system-variables.html#sysvar_slow_query_log) enables the log.
...
[`log_output`](server-system-variables.html#sysvar_log_output) system variable (as
described in [Section 7.4.1, “Selecting General Query Log and Slow Query Log Output Destinations”](log-destinations.html "7.4.1 Selecting General Query Log and Slow Query Log Output Destinations")).

Note

If you specify the `TABLE` log destination,
see [Log Tables and “Too many open files” Errors](log-destinations.html#log-destinations-tables-open-files "Log Tables and “Too many open files” Errors").
...
[`slow_query_log`](server-system-variables.html#sysvar_slow_query_log) and
[`slow_query_log_file`](server-system-variables.html#sysvar_slow_query_log_file) system
variables. Set [`slow_query_log`](server-system-variables.html#sysvar_slow_query_log)
to 0 to disable the log or to 1 to enable it. Set
[`slow_query_log_file`](server-system-variables.html#sysvar_slow_query_log_file) to specify
...
use the [`--log-short-format`](server-options.html#option_mysqld_log-short-format)
option.
...
2. The query must have taken at least
   [`long_query_time`](server-system-variables.html#sysvar_long_query_time) seconds, or
   [`log_queries_not_using_indexes`](server-system-variables.html#sysvar_log_queries_not_using_indexes)
...
The [`log_timestamps`](server-system-variables.html#sysvar_log_timestamps) system
...
zone with [`CONVERT_TZ()`](date-and-time-functions.html#function_convert-tz) or by
setting the session [`time_zone`](server-system-variables.html#sysvar_time_zone)
system variable.
...
([`binlog_format=ROW`](replication-options-binary-log.html#sysvar_binlog_format)), these
...
[`binlog_format=STATEMENT`](replication-options-binary-log.html#sysvar_binlog_format) is set,
or when [`binlog_format=MIXED`](replication-options-binary-log.html#sysvar_binlog_format) is
set and the statement is logged in statement format. Slow
queries that are logged in row format when
[`binlog_format=MIXED`](replication-options-binary-log.html#sysvar_binlog_format) is set, or
that are logged when
[`binlog_format=ROW`](replication-options-binary-log.html#sysvar_binlog_format) is set, are
not added to the replica's slow query log, even if
[`log_slow_replica_statements`](replication-options-replica.html#sysvar_log_slow_replica_statements) is
enabled.
...
[`log_output`](server-system-variables.html#sysvar_log_output) system variable. If
...
Enabling the [`log_slow_extra`](server-system-variables.html#sysvar_log_slow_extra)
system variable causes the server to write the following extra
fields to `FILE` output in addition to those
just listed (`TABLE` output is unaffected).
Some field descriptions refer to status variable names. Consult
the status variable descriptions for more information. However,
in the slow query log, the counters are per-statement values,
not cumulative per-session values.
...
* `Bytes_received:
  ...
  The [`Bytes_received`](server-status-variables.html#statvar_Bytes_received) value
  for the statement.

* `Bytes_sent: N`

  The [`Bytes_sent`](server-status-variables.html#statvar_Bytes_sent) value for
  the statement.

* `Read_first: N`

  The [`Handler_read_first`](server-status-variables.html#statvar_Handler_read_first)
  value for the statement.

* `Read_last: N`

  The [`Handler_read_last`](server-status-variables.html#statvar_Handler_read_last)
  value for the statement.

* `Read_key: N`

  The [`Handler_read_key`](server-status-variables.html#statvar_Handler_read_key) value
  for the statement.

* `Read_next: N`

  The [`Handler_read_next`](server-status-variables.html#statvar_Handler_read_next)
  value for the statement.

* `Read_prev: N`

  The [`Handler_read_prev`](server-status-variables.html#statvar_Handler_read_prev)
  value for the statement.

* `Read_rnd: N`

  The [`Handler_read_rnd`](server-status-variables.html#statvar_Handler_read_rnd) value
  for the statement.

* `Read_rnd_next:
  N`

  The [`Handler_read_rnd_next`](server-status-variables.html#statvar_Handler_read_rnd_next)
  value for the statement.

* `Sort_merge_passes:
  N`

  The [`Sort_merge_passes`](server-status-variables.html#statvar_Sort_merge_passes)
  value for the statement.

* `Sort_range_count:
  N`

  The [`Sort_range`](server-status-variables.html#statvar_Sort_range) value for
  the statement.

* `Sort_rows: N`

  The [`Sort_rows`](server-status-variables.html#statvar_Sort_rows) value for
  the statement.

* `Sort_scan_count:
  N`

  The [`Sort_scan`](server-status-variables.html#statvar_Sort_scan) value for
  the statement.
...
* `Created_tmp_tables:
  ...
  The [`Created_tmp_tables`](server-status-variables.html#statvar_Created_tmp_tables)
  value for the statement.
...
[`log_slow_extra`](server-system-variables.html#sysvar_log_slow_extra). Log file
...
statement that includes a timestamp, which indicates when the
slow statement began executing.

Passwords in statements written to the slow query log are
rewritten by the server not to occur literally in plain text.
See [Section 8.1.2.3, “Passwords and Logging”](password-logging.html "8.1.2.3 Passwords and Logging").

Statements that cannot be parsed (due, for example, to syntax
