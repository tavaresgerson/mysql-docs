...
* System variables may affect log sinks. See
  [System Variables That Affect Error Log Format](error-log-format.html#error-log-format-system-variables "System Variables That Affect Error Log Format").

For names and descriptions of the fields in error events, see
[Section 7.4.2.3, “Error Event Fields”](error-log-event-fields.html "7.4.2.3 Error Event Fields"). For all log sinks, the
thread ID included in error log messages is that of the thread
within [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") responsible for writing the
...
* [log\_sink\_internal Output Format](error-log-format.html#error-log-format-output-format-for-log-sink-internal "log_sink_internal Output Format")
* [log\_sink\_json Output Format](error-log-format.html#error-log-format-output-format-for-log-sink-json "log_sink_json Output Format")
* [log\_sink\_syseventlog Output Format](error-log-format.html#error-log-format-output-format-for-log-sink-syseventlog "log_sink_syseventlog Output Format")
...
The `ts` (timestamp) key is unique to the
JSON-format log sink. The value is an integer indicating
milliseconds since the epoch (`'1970-01-01
00:00:00'` UTC).
...
* The server buffers log events (rather than formatted log
  messages), which enables it to apply configuration
  settings to those events retroactively, after the settings
  are known, with the result that flushed messages use the
  configured settings, not the defaults. Also, messages are
  flushed to all configured sinks, not just the default
  sink.
  ...
  Although this behavior uses the defaults, it is preferable
  to losing messages when exceptional conditions occur.
