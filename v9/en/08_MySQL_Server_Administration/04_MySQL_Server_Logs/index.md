...
<table summary="MySQL Server log types and the information written to each log."><col style="width: 30%"/><col style="width: 70%"/><thead><tr>
<th>Log Type</th>
...
<td>DDL logs</td>
<td>Atomic DDL operations performed by DDL statements</td>
</tr></tbody></table>

By default, no logs are enabled, except the error log on Windows.
For information about DDL log behavior, see
[Viewing DDL Logs](atomic-ddl.html#atomic-ddl-view-logs "Viewing DDL Logs"). The following log-specific
...
[`FLUSH LOGS`](flush.html#flush-logs) statement; execute
[**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") with a `flush-logs`
or `refresh` argument; or execute
[**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") with a
[`--flush-logs`](mysqldump.html#option_mysqldump_flush-logs) option. See
...
[Section 6.5.4, “mysqldump — A Database Backup Program”](mysqldump.html "6.5.4 mysqldump — A Database Backup Program"). In addition, the binary log is flushed
...
[Section 7.4.1, “Selecting General Query Log and Slow Query Log Output Destinations”](log-destinations.html "7.4.1 Selecting General Query Log and Slow Query Log Output Destinations"), [Section 7.4.3, “The General Query Log”](query-log.html "7.4.3 The General Query Log"), and
...
of old log files, see [Section 7.4.6, “Server Log Maintenance”](log-file-maintenance.html "7.4.6 Server Log Maintenance").
