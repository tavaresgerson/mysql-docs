...
`zstd` algorithm, and then written to the
...
BINLOG EVENTS`](show-binlog-events.html "15.7.7.3 SHOW BINLOG EVENTS Statement") and [`SHOW RELAYLOG
EVENTS`](show-relaylog-events.html "15.7.7.35 SHOW RELAYLOG EVENTS Statement") statements.
...
tables using the `NDB` storage engine at run
time using the
...
`--binlog-transaction-compression` on the
...
transaction compression is not in use. In case the
compressed transaction packet exceeds the maximum
packet size used in replication, which is 1 GiB, the source
server writes the transaction uncompressed, so
that it can be sent in smaller pieces.
...
For the [`SHOW BINLOG EVENTS`](show-binlog-events.html "15.7.7.3 SHOW BINLOG EVENTS Statement") and
...
such as [`START
REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") with the `UNTIL` clause,
[`SOURCE_POS_WAIT()`](replication-functions-synchronization.html#function_source-pos-wait), and
[`sql_replica_skip_counter`](replication-options-replica.html#sysvar_sql_replica_skip_counter), you
...
[`sql_replica_skip_counter`](replication-options-replica.html#sysvar_sql_replica_skip_counter), a
...
`SOURCE_ZSTD_COMPRESSION_LEVEL` options of
the [`CHANGE REPLICATION SOURCE
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement, or the
