...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") servers that they manage themselves. (For
...
9.4 and one from MySQL 9.5, to see how
...
[Section 6.2.2, “Specifying Program Options”](program-options.html "6.2.2 Specifying Program Options"). To see the values used by a given
instance, connect to it and execute a [`SHOW
VARIABLES`](show-variables.html "15.7.7.42 SHOW VARIABLES Statement") statement.
...
* [`--port=port_num`](server-options.html#option_mysqld_port)
  ...
  [`--port`](server-options.html#option_mysqld_port) controls the port number
  for TCP/IP connections. Alternatively, if the host has multiple
  network addresses, you can set the
  [`bind_address`](server-system-variables.html#sysvar_bind_address) system variable to
  cause each server to listen to a different address.

* [`--socket={file_name|pipe_name}`](server-options.html#option_mysqld_socket)

  [`--socket`](server-options.html#option_mysqld_socket) controls the Unix socket
...
[`--socket`](server-options.html#option_mysqld_socket) and
[`--port`](server-options.html#option_mysqld_port) options. Suppose that you
...
directory. [**mysqld\_safe**](mysqld-safe.html "6.3.2 mysqld_safe — MySQL Server Startup Script") determines the proper
[`--basedir`](server-system-variables.html#sysvar_basedir) option to pass to
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"), and you need specify only the
[`--socket`](mysqld-safe.html#option_mysqld_safe_socket) and
[`--port`](mysqld-safe.html#option_mysqld_safe_port) options to
...
[`--defaults-file`](option-file-options.html#option_general_defaults-file) option is useful for
this purpose.