...
The discussion here uses [**mysqld\_safe**](mysqld-safe.html "6.3.2 mysqld_safe — MySQL Server Startup Script") to
...
Assume that an existing 9.4 server is configured for
the default TCP/IP port number (3306) and Unix socket file
(`/tmp/mysql.sock`). To configure a new
9.5.0 server to have different operating parameters,
...
[`CMAKE_INSTALL_PREFIX`](source-configuration-options.html#option_cmake_cmake_install_prefix) value should
...
[**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") defaults to using a Unix socket file
...
[`--port`](server-options.html#option_mysqld_port) option values, and pass a
[`--datadir=dir_name`](server-system-variables.html#sysvar_datadir)
option to [**mysqld\_safe**](mysqld-safe.html "6.3.2 mysqld_safe — MySQL Server Startup Script") so that the server uses a
different data directory.

Alternatively, put the options for each server in a different
option file, then start each server using a
[`--defaults-file`](option-file-options.html#option_general_defaults-file) option that
...
[Section 6.9, “Environment Variables”](environment-variables.html "6.9 Environment Variables"), includes a list of other
environment variables you can use to affect MySQL programs.

On Unix, the [**mysqld\_multi**](mysqld-multi.html "6.3.4 mysqld_multi — Manage Multiple MySQL Servers") script provides
