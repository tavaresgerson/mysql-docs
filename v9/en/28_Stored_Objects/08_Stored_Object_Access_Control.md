...
* [The DEFINER Attribute](stored-objects-security.html#stored-objects-security-definer "The DEFINER Attribute")
* [The SQL SECURITY Characteristic](stored-objects-security.html#stored-objects-security-sql-security "The SQL SECURITY Characteristic")
* [Examples](stored-objects-security.html#stored-objects-security-examples "Examples")
* [Orphan Stored Objects](stored-objects-security.html#stored-objects-security-orphan-objects "Orphan Stored Objects")
* [Risk-Minimization Guidelines](stored-objects-security.html#stored-objects-security-guidelines "Risk-Minimization Guidelines")
...
* If you have the
  [`SET_ANY_DEFINER`](privileges-provided.html#priv_set-any-definer) privilege,
  you can specify any account as the
  `DEFINER` attribute. If the account does
  not exist, a warning is generated. Additionally, to set a
  stored object `DEFINER` attribute to an
...
* Otherwise, the only permitted account is your own, specified
  ...
  [`CURRENT_USER()`](information-functions.html#function_current-user). You cannot
...
* A stored object that executes in definer security context
  ...
  object (for example, [`EXECUTE`](privileges-provided.html#priv_execute)
  to call a stored procedure or
  [`SELECT`](privileges-provided.html#priv_select) to select from a
...
Any user who has the [`EXECUTE`](privileges-provided.html#priv_execute)
privilege for `p1` can invoke it with a
[`CALL`](call.html "15.2.1 CALL Statement") statement. However, when
...
[`EXECUTE`](privileges-provided.html#priv_execute) privilege for
`p1` as well as the
[`UPDATE`](privileges-provided.html#priv_update) privilege for the table
...
lacks the [`EXECUTE`](privileges-provided.html#priv_execute) privilege for
`p2` or the
[`UPDATE`](privileges-provided.html#priv_update) privilege for the table
`t1`.
...
* An existing stored object can become orphaned through
  execution of a [`DROP USER`](drop-user.html "15.7.1.5 DROP USER Statement")
  statement that drops the object `DEFINER`
  account, or a [`RENAME USER`](rename-user.html "15.7.1.7 RENAME USER Statement")
...
* [`DROP USER`](drop-user.html "15.7.1.5 DROP USER Statement") fails with an error
...
* [`RENAME USER`](rename-user.html "15.7.1.7 RENAME USER Statement") fails with an
  error if any account to be renamed is named as the
  `DEFINER` attribute for any stored object.
  (That is, the statement fails if renaming an account would
  cause a stored object to become orphaned.)

* [`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") fails with an
...
* To identify which `DEFINER` values exist in
  ...
  [`ALTER EVENT`](alter-event.html "15.1.3 ALTER EVENT Statement") or
  [`ALTER VIEW`](alter-view.html "15.1.13 ALTER VIEW Statement") to directly modify
...
* To identify which objects have a given
  ...
  For the [`ROUTINES`](information-schema-routines-table.html "28.3.36 The INFORMATION_SCHEMA ROUTINES Table") table, the
...
* Definer-context objects should be written keeping in mind
  ...
  + A stored routine cannot be referenced by a user who does
    not have the [`EXECUTE`](privileges-provided.html#priv_execute)
    privilege for it.

  + A view cannot be referenced by a user who does not have
    the appropriate privilege for it
    ([`SELECT`](privileges-provided.html#priv_select) to select from
    it, [`INSERT`](privileges-provided.html#priv_insert) to insert into
    it, and so forth).
...
* By default, when a routine with the `SQL SECURITY
  ...
  [`CREATE
  FUNCTION`](create-function.html "15.1.16 CREATE FUNCTION Statement") statement is issued. For stored programs,
  if execution should occur with roles different from the
  default, the program body can execute
  [`SET ROLE`](set-role.html "15.7.1.11 SET ROLE Statement") to activate the
