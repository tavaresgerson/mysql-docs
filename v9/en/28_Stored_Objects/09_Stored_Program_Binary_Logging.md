...
* Replicated statements executed on a replica are processed by
  the replica's applier thread. Unless you implement
  replication privilege checks (see
  [Section 19.3.3, “Replication Privilege Checks”](replication-privilege-checks.html "19.3.3 Replication Privilege Checks")), the applier
...
[Section 7.4.4, “The Binary Log”](binary-log.html "7.4.4 The Binary Log").) If the binary log is not enabled,
replication is not possible, nor is the binary log available for
data recovery. Binary logging is enabled by default, and is
disabled only if you start the server with
...
[`CALL`](call.html "15.2.1 CALL Statement") statement is not logged. For
...
([`binlog_format=MIXED`](replication-options-binary-log.html#sysvar_binlog_format)) uses
...
* To create or alter a stored function, you must have the
  [`SET_ANY_DEFINER`](privileges-provided.html#priv_set-any-definer) privilege, in
  addition to the [`CREATE ROUTINE`](privileges-provided.html#priv_create-routine)
  ...
  [`SET_ANY_DEFINER`](privileges-provided.html#priv_set-any-definer) might be
  required regardless of whether binary logging is enabled. See
...
* When you create a stored function, you must declare either
  ...
  FUNCTION`](create-function.html "15.1.16 CREATE FUNCTION Statement") statement to be accepted, at least one of
  ...
  This function uses [`UUID()`](miscellaneous-functions.html#function_uuid),
...
* When you attempt to execute a stored function, if
  ...
  [`log_bin_trust_function_creators=1`](replication-options-binary-log.html#sysvar_log_bin_trust_function_creators)
...
* To relax the preceding conditions on function creation (that
  you must have the [`SUPER`](privileges-provided.html#priv_super)
  ...
  [`log_bin_trust_function_creators`](replication-options-binary-log.html#sysvar_log_bin_trust_function_creators)
  does not apply. [`SUPER`](privileges-provided.html#priv_super) is not
...
* For information about built-in functions that may be unsafe
  ...
  [Section 19.5.1, “Replication Features and Issues”](replication-features.html "19.5.1 Replication Features and Issues").
...
exception: [`CREATE TRIGGER`](create-trigger.html "15.1.26 CREATE TRIGGER Statement") does not
...
[`UUID()`](miscellaneous-functions.html#function_uuid) function is nondeterministic
...
TRIGGER`](create-trigger.html "15.1.26 CREATE TRIGGER Statement") if you do not have the required privileges. On
...
* The server writes [`CREATE EVENT`](create-event.html "15.1.15 CREATE EVENT Statement"),
  ...
  [`DROP PROCEDURE`](drop-procedure.html "15.1.34 DROP PROCEDURE and DROP FUNCTION Statements"), and
  [`DROP FUNCTION`](drop-function.html "15.1.30 DROP FUNCTION Statement") statements to the
  binary log.

* A stored function invocation is logged as a
  [`SELECT`](select.html "15.2.13 SELECT Statement") statement if the
  ...
  statements. For example, [`SELECT`](select.html "15.2.13 SELECT Statement")
  statements are not written to the binary log, but a
  [`SELECT`](select.html "15.2.13 SELECT Statement") might invoke a stored
  ...
  When the [`SELECT`](select.html "15.2.13 SELECT Statement") statement
  executes, the function `f1()` is invoked
  three times. Two of those invocations insert a row, and MySQL
  logs a [`SELECT`](select.html "15.2.13 SELECT Statement") statement for
  ...
  The server also logs a [`SELECT`](select.html "15.2.13 SELECT Statement")
  statement for a stored function invocation when the function
  invokes a stored procedure that causes an error. In this case,
  the server writes the [`SELECT`](select.html "15.2.13 SELECT Statement")
...
* Logging stored function invocations rather than the statements
  ...
  + Statements executed on a replica are processed by the
    ...
    privilege checks (see
    [Section 19.3.3, “Replication Privilege Checks”](replication-privilege-checks.html "19.3.3 Replication Privilege Checks")), the
    applier thread has full privileges.

  The implication is that although a user must have the
  [`CREATE ROUTINE`](privileges-provided.html#priv_create-routine) privilege to
  ...
  The [`CREATE FUNCTION`](create-function.html "15.1.16 CREATE FUNCTION Statement") and
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statements are written
  ...
  [`SUPER`](privileges-provided.html#priv_super) privilege, in addition to
  the usual [`CREATE ROUTINE`](privileges-provided.html#priv_create-routine)
  privilege that is required. Similarly, to use
  [`ALTER FUNCTION`](alter-function.html "15.1.4 ALTER FUNCTION Statement"), you must have
  the [`SUPER`](privileges-provided.html#priv_super) privilege in addition
  to the [`ALTER ROUTINE`](privileges-provided.html#priv_alter-routine) privilege.
  Without the [`SUPER`](privileges-provided.html#priv_super) privilege, an
  error occurs:
  ...
  [`SUPER`](privileges-provided.html#priv_super) privilege (for example,
  if all users with the [`CREATE
  ROUTINE`](privileges-provided.html#priv_create-routine) privilege on your system are experienced
...
* The use of replication privilege checks recommended regardless
  of any choices you make about privileges for function
  creators. Replication privilege checks can be set up to ensure
  that only expected and relevant operations are authorized for
  the replication channel. For instructions to do this, see
...
* Stored procedure calls are logged at the statement level
  rather than at the [`CALL`](call.html "15.2.1 CALL Statement") level.
  That is, the server does not log the
  [`CALL`](call.html "15.2.1 CALL Statement") statement, it logs those
  ...
  + A statement to be logged might contain references to local
    ...
    *`var_name`* is the local variable
    ...
    [`NAME_CONST()`](miscellaneous-functions.html#function_name-const) has a value of
    *`var_value`*, and a
    “name” of
    *`var_name`*. Thus, if you invoke
    ...
    [`NAME_CONST()`](miscellaneous-functions.html#function_name-const) enables a
    ...
    The use of [`NAME_CONST()`](miscellaneous-functions.html#function_name-const) can
    result in a problem for
    [`CREATE TABLE
    ... SELECT`](create-table.html "15.1.24 CREATE TABLE Statement") statements when the source column
    expressions refer to local variables. Converting these
    references to [`NAME_CONST()`](miscellaneous-functions.html#function_name-const)
  ...
  + Procedure calls can occur within a committed or
    ...
    transaction,
    [`BEGIN`](commit.html "15.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements")
    and [`COMMIT`](commit.html "15.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements") statements are
...
* A stored procedure call is *not* written to
  ...
  [`DO`](do.html "15.2.3 DO Statement") statement (if it occurs
