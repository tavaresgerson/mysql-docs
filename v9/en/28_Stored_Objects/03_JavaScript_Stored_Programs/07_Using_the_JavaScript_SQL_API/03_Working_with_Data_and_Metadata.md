...
The function `print_result()`, shown here,
takes a result set
([`SqlResult`](srjsapi-sqlresult.html "27.3.6.3 SqlResult Object")) as input:
...
If `query` is the text of a valid SQL
...
`print_result()` prints its output to
`stdout`. This includes the names of the
...
[`nextResult()`](srjsapi-sqlresult.html#srjsapi-sqlresult-nextresult), and
...
This section describes basic error handling in MySQL
JavaScript stored programs, when using the SQL API.
...
catch` blocks, in which case execution proceeds. If
...
Executing [`SHOW WARNINGS`](show-warnings.html "15.7.7.43 SHOW WARNINGS Statement") after a
...
query is aborted (`CTRL-C`), the stored
program stops executing immediately and produces an error.
Likewise, out of memory errors cannot be handled within
JavaScript routines.

An SQL statement that causes errors that are not handled
within the stored program passes them back to the client. To
observe this, we create a stored procedure using the following
SQL statement:
...
Now we call `jssp_simple_error()`, passing to
...
You can choose to handle SQL errors in JavaScript instead,
using try-catch syntax, like this:
...
`jssp_catch_errors()` is one which attempts
