...
API client library](/doc/c-api/9.5/en/) for C programs,
[MySQL Connector/J](/doc/connector-j/en/) for Java
programs, and [MySQL Connector/NET](/doc/connector-net/en/) for
...
[C API Prepared Statement Interface](/doc/c-api/9.5/en/c-api-prepared-statement-interface.html). Other language
...
being the
[`mysqli`
extension](http://php.net/mysqli), available in PHP 5.0 and later.
...
* You can use it from any program that can send SQL statements to
  the server to be executed, such as the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client")
  client program.
...
* [`PREPARE`](prepare.html "15.5.1 PREPARE Statement") prepares a statement for
  execution (see [Section 15.5.1, “PREPARE Statement”](prepare.html "15.5.1 PREPARE Statement")).

* [`EXECUTE`](execute.html "15.5.2 EXECUTE Statement") executes a prepared
  statement (see [Section 15.5.2, “EXECUTE Statement”](execute.html "15.5.2 EXECUTE Statement")).

* [`DEALLOCATE PREPARE`](deallocate-prepare.html "15.5.3 DEALLOCATE PREPARE Statement") releases a
  prepared statement (see [Section 15.5.3, “DEALLOCATE PREPARE Statement”](deallocate-prepare.html "15.5.3 DEALLOCATE PREPARE Statement")).
...
[`max_prepared_stmt_count`](server-system-variables.html#sysvar_max_prepared_stmt_count) system
...
`CREATE TABLE ... START TRANSACTION` is not
...
* Statements containing any reference to the
  [`warning_count`](server-system-variables.html#sysvar_warning_count) or
  [`error_count`](server-system-variables.html#sysvar_error_count) system variable.
...
event DDL. Attempting to use a placeholder in such a statement is
rejected by [`PREPARE`](prepare.html "15.5.1 PREPARE Statement") with
ERROR 6413 (HY000): Dynamic parameters can only be used
in DML statements. Instead, you can do this in a
reusable fashion is to assemble the text containing the event SQL in
the body of a stored procedure, passing any variable parts of the
SQL statement as `IN` parameters to the stored
procedure; then you can prepare the assembled text with a
[`PREPARE`](prepare.html "15.5.1 PREPARE Statement") statement (also within the
body of the stored procedure), then invoke the procedure using the
desired parameter values. See [Section 15.1.15, “CREATE EVENT Statement”](create-event.html "15.1.15 CREATE EVENT Statement"), for an
example.

In prepared [`CALL`](call.html "15.2.1 CALL Statement") statements used with
[`PREPARE`](prepare.html "15.5.1 PREPARE Statement") and
[`EXECUTE`](execute.html "15.5.2 EXECUTE Statement"), placeholder support for
`OUT` and `INOUT` parameters is
available beginning with MySQL 9.5. See
[Section 15.2.1, “CALL Statement”](call.html "15.2.1 CALL Statement"), for an example and a workaround for earlier
...
[`PREPARE`](prepare.html "15.5.1 PREPARE Statement") cannot itself be a
...
[`mysql_stmt_prepare()`](/doc/c-api/9.5/en/mysql-stmt-prepare.html) C API function
to prepare a [`PREPARE`](prepare.html "15.5.1 PREPARE Statement"),
[`EXECUTE`](execute.html "15.5.2 EXECUTE Statement"), or
[`DEALLOCATE PREPARE`](deallocate-prepare.html "15.5.3 DEALLOCATE PREPARE Statement") statement.
...
executed with [`PREPARE`](prepare.html "15.5.1 PREPARE Statement") and
[`EXECUTE`](execute.html "15.5.2 EXECUTE Statement"). The statement for a cursor
...
To write C programs that use the [`CALL`](call.html "15.2.1 CALL Statement")
SQL statement to execute stored procedures that contain prepared
statements, the `CLIENT_MULTI_RESULTS` flag must be
enabled. This is because each [`CALL`](call.html "15.2.1 CALL Statement")
...
[`mysql_real_connect()`](/doc/c-api/9.5/en/mysql-real-connect.html), either
...
information, see [Section 15.2.1, “CALL Statement”](call.html "15.2.1 CALL Statement").