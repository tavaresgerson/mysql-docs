...
The [`IF`](if.html "15.6.5.2 IF Statement") statement for stored
programs implements a basic conditional construct.

Note

There is also an [`IF()`](flow-control-functions.html#function_if)
...
[`IF`](if.html "15.6.5.2 IF Statement") statement can have
`THEN`, `ELSE`, and
...
constructs, including other [`IF`](if.html "15.6.5.2 IF Statement")
statements. Each [`IF`](if.html "15.6.5.2 IF Statement") must be
...
In this example, the inner [`IF`](if.html "15.6.5.2 IF Statement") is
