...
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") generates table
statistics:

* [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") without any
  ...
  [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") for key
  distribution analysis is equivalent to using
  [**myisamchk --analyze**](myisamchk.html "6.6.4 myisamchk — MyISAM Table-Maintenance Utility").

* [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") with the
...
This statement requires [`SELECT`](privileges-provided.html#priv_select)
and [`INSERT`](privileges-provided.html#priv_insert) privileges for the
table.

[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") works with
`InnoDB`, `NDB`, and
`MyISAM` tables. It does not work with views.

If the [`innodb_read_only`](innodb-parameters.html#sysvar_innodb_read_only) system
...
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") is supported for
partitioned tables, and you can use `ALTER TABLE ...
ANALYZE PARTITION` to analyze one or more partitions;
for more information, see [Section 15.1.11, “ALTER TABLE Statement”](alter-table.html "15.1.11 ALTER TABLE Statement"), and
...
TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") statements to the binary log so that they
...
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") returns a result
set with the columns shown in the following table.

<table summary="Columns of the ANALYZE TABLE result set."><col style="width: 15%"/><col style="width: 60%"/><thead><tr>
<th>Column</th>
<th>Value</th>
</tr></thead><tbody><tr>
...
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") without either
`HISTOGRAM` clause performs a key
distribution analysis and stores the distribution for the
...
[`SHOW INDEX`](show-index.html "15.7.7.24 SHOW INDEX Statement") statement or the
`INFORMATION_SCHEMA`
[`STATISTICS`](information-schema-statistics-table.html "28.3.40 The INFORMATION_SCHEMA STATISTICS Table") table. See
[Section 15.7.7.24, “SHOW INDEX Statement”](show-index.html "15.7.7.24 SHOW INDEX Statement"), and
...
For `InnoDB` tables,
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") determines index
...
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") could produce
different numbers. This makes [`ANALYZE
TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") fast on `InnoDB` tables but
not 100% accurate because it does not take all rows into
account.

You can make the
[statistics](glossary.html#glos_statistics "statistics") collected by
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") more precise and
more stable by enabling
[`innodb_stats_persistent`](innodb-parameters.html#sysvar_innodb_stats_persistent), as
explained in [Section 17.8.10.1, “Configuring Persistent Optimizer Statistics Parameters”](innodb-persistent-stats.html "17.8.10.1 Configuring Persistent Optimizer Statistics Parameters"). When
[`innodb_stats_persistent`](innodb-parameters.html#sysvar_innodb_stats_persistent) is
enabled, it is important to run [`ANALYZE
TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") after major changes to index column data, as
statistics are not recalculated periodically (such as after a
server restart).

If [`innodb_stats_persistent`](innodb-parameters.html#sysvar_innodb_stats_persistent) is
...
`InnoDB`, see
[Section 17.8.10.1, “Configuring Persistent Optimizer Statistics Parameters”](innodb-persistent-stats.html "17.8.10.1 Configuring Persistent Optimizer Statistics Parameters"), and
...
[`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement"). In the few cases
that [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") does not
...
[`max_seeks_for_key`](server-system-variables.html#sysvar_max_seeks_for_key) system
variable to ensure that MySQL prefers index lookups over table
scans. See [Section B.3.5, “Optimizer-Related Issues”](optimizer-issues.html "B.3.5 Optimizer-Related Issues").
...
* [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") with an
  `UPDATE HISTOGRAM` clause generates
  ...
  *`N`* must be an integer in the
  ...
  [Section 17.8.10.1, “Configuring Persistent Optimizer Statistics Parameters”](innodb-persistent-stats.html "17.8.10.1 Configuring Persistent Optimizer Statistics Parameters")), the
  [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") background statistics
  thread also updates the histogram. `MANUAL
  UPDATE` disables automatic updates, and is the
  default setting if not specified.

* [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") with a
  `DROP HISTOGRAM` clause removes histogram
  statistics for the named table columns from the data
  dictionary. Only one table name is permitted for this
  syntax.
...
[`COLUMN_STATISTICS`](information-schema-column-statistics-table.html "28.3.11 The INFORMATION_SCHEMA COLUMN_STATISTICS Table") table. Only one
...
* [`DROP TABLE`](drop-table.html "15.1.37 DROP TABLE Statement") removes
  histograms for columns in the dropped table.

* [`DROP DATABASE`](drop-database.html "15.1.28 DROP DATABASE Statement") removes
  histograms for any table in the dropped database because
  the statement drops all tables in the database.

* [`RENAME TABLE`](rename-table.html "15.1.41 RENAME TABLE Statement") does not
...
* [`ALTER TABLE
  ... CONVERT TO CHARACTER SET`](alter-table.html "15.1.11 ALTER TABLE Statement") removes histograms
...
[`COLUMN_STATISTICS`](information-schema-column-statistics-table.html "28.3.11 The INFORMATION_SCHEMA COLUMN_STATISTICS Table") table can be
...
The [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") storage engine
...
[`INNODB_METRICS`](information-schema-innodb-metrics-table.html "28.4.21 The INFORMATION_SCHEMA INNODB_METRICS Table") counters can be
used to monitor sampling of `InnoDB` data
pages. (For general
[`INNODB_METRICS`](information-schema-innodb-metrics-table.html "28.4.21 The INFORMATION_SCHEMA INNODB_METRICS Table") counter usage
information, see
...
`HISTOGRAM` column of the Information Schema
[`COLUMN_STATISTICS`](information-schema-column-statistics-table.html "28.3.11 The INFORMATION_SCHEMA COLUMN_STATISTICS Table") table.
...
[`INNODB_TABLESTATS`](information-schema-innodb-tablestats-table.html "28.4.26 The INFORMATION_SCHEMA INNODB_TABLESTATS View") table and sets
