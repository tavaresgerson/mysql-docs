...
library file (see [Plugin Data Structures](/doc/extending-mysql/9.5/en/plugin-data-structures.html)).
...
[`plugin_dir`](server-system-variables.html#sysvar_plugin_dir) is the
...
If the value of [`plugin_dir`](server-system-variables.html#sysvar_plugin_dir) is a
relative path name, it is taken to be relative to the MySQL base
directory (the value of the
[`basedir`](server-system-variables.html#sysvar_basedir) system variable).

[`INSTALL PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement") loads and
...
[`INSTALL PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement") also registers the
...
installed with [`INSTALL PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement")
...
PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement") statement. Each statement names a different
plugin, but all of them specify the same library name.

[`INSTALL PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement") causes the server
...
startup, see [Section 7.6.1, “Installing and Uninstalling Plugins”](plugin-loading.html "7.6.1 Installing and Uninstalling Plugins"). If you need to
...
the [`--plugin-load`](server-options.html#option_mysqld_plugin-load) option. See
...
PLUGIN`](uninstall-plugin.html "15.7.4.6 UNINSTALL PLUGIN Statement") statement.
...
[`SHOW PLUGINS`](show-plugins.html "15.7.7.28 SHOW PLUGINS Statement") statement or query
...
* Use [`UNINSTALL PLUGIN`](uninstall-plugin.html "15.7.4.6 UNINSTALL PLUGIN Statement") to
  uninstall all plugins in the library, install the new plugin
  library file in the plugin directory, and then use
  [`INSTALL PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement") to install all
  ...
  many [`INSTALL PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement") and
