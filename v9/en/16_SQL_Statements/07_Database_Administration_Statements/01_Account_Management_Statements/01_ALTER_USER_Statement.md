...
The [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statement modifies
...
In most cases, [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement")
requires the global [`CREATE USER`](privileges-provided.html#priv_create-user)
privilege, or the [`UPDATE`](privileges-provided.html#priv_update)
...
* Any client who connects to the server using a nonanonymous
  ...
  [`CURRENT_USER()`](information-functions.html#function_current-user) function:
...
* For `DEFAULT ROLE` syntax,
  [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") requires these
  privileges:

  + Setting the default roles for another user requires the
    global [`CREATE USER`](privileges-provided.html#priv_create-user)
    privilege, or the [`UPDATE`](privileges-provided.html#priv_update)
...
* Statements that modify secondary passwords require these
  ...
  + The
    ...
    USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statements that apply to your own
...
* The [`authentication_policy`](server-system-variables.html#sysvar_authentication_policy)
  ...
  USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statements may be used; for details, see the
...
USER`](alter-user.html "15.7.1.1 ALTER USER Statement") may be recorded in server logs or on the client
...
it, see [Section 8.1.2.3, “Passwords and Logging”](password-logging.html "8.1.2.3 Passwords and Logging"). For similar
...
USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statement, described under the following topics:
...
* [ALTER USER Multifactor Authentication Options](alter-user.html#alter-user-multifactor-authentication "ALTER USER Multifactor Authentication Options")
* [ALTER USER Registration Options](alter-user.html#alter-user-registration- "ALTER USER Registration Options")
* [ALTER USER Role Options](alter-user.html#alter-user-role "ALTER USER Role Options")
...
* [ALTER USER Comment and Attribute Options](alter-user.html#alter-user-comments-attributes "ALTER USER Comment and Attribute Options")
* [ALTER USER Account-Locking Options](alter-user.html#alter-user-account-locking "ALTER USER Account-Locking Options")
* [ALTER USER Binary Logging](alter-user.html#alter-user-binary-logging "ALTER USER Binary Logging")

##### ALTER USER Overview

For each affected account, [`ALTER
USER`](alter-user.html "15.7.1.1 ALTER USER Statement") modifies the corresponding row in the
...
[`USER()`](information-functions.html#function_user) function:
...
For [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") syntax that
...
user issuing the [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement")
...
* *`auth_plugin`* names an
  ...
  [Section 8.4.1, “Authentication Plugins”](authentication-plugins.html "8.4.1 Authentication Plugins").
...
* Credentials that are stored internally are stored in the
  ...
  + For syntax that uses `AS
    ...
    USER`](show-create-user.html "15.7.7.14 SHOW CREATE USER Statement") for password hashes containing
...
* The `RETAIN CURRENT PASSWORD` and
  ...
  + `RETAIN CURRENT PASSWORD` retains an
    ...
    [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statement is
...
[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") permits these
...
[Section 8.2.14, “Assigning Account Passwords”](assigning-passwords.html "8.2.14 Assigning Account Passwords"), and
...
[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") has
`ADD`, `MODIFY`, and
...
[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") multifactor
...
[`authentication_policy`](server-system-variables.html#sysvar_authentication_policy) setting
...
When [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") adds, modifies,
...
USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statement fails.
...
`MODIFY`, and `DROP`
examples, see
[Getting Started with Multifactor Authentication](multifactor-authentication.html#multifactor-authentication-getting-started "Getting Started with Multifactor Authentication").
...
[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") has clauses that
...
and the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client
...
[`--register-factor`](mysql-command-options.html#option_mysql_register-factor) option, used
for FIDO/FIDO2 device registration, causes the
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client to generate and execute
...
[`ALTER USER ...
DEFAULT ROLE`](alter-user.html "15.7.1.1 ALTER USER Statement") defines which roles become active when
the user connects to the server and authenticates, or when the
user executes the
[`SET ROLE
DEFAULT`](set-role.html "15.7.1.11 SET ROLE Statement") statement during a session.

[`ALTER USER ...
DEFAULT ROLE`](alter-user.html "15.7.1.1 ALTER USER Statement") is alternative syntax for
[`SET DEFAULT ROLE`](set-default-role.html "15.7.1.9 SET DEFAULT ROLE Statement") (see
[Section 15.7.1.9, “SET DEFAULT ROLE Statement”](set-default-role.html "15.7.1.9 SET DEFAULT ROLE Statement")). However,
[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") can set the default
for only a single user, whereas [`SET
DEFAULT ROLE`](set-default-role.html "15.7.1.9 SET DEFAULT ROLE Statement") can set the default for multiple users.
On the other hand, you can specify
`CURRENT_USER` as the user name for the
[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statement, whereas
...
* `role [,
  ...
  [`ALTER USER ...
  DEFAULT ROLE`](alter-user.html "15.7.1.1 ALTER USER Statement") is executed.
...
with MySQL, see [Section 8.3, “Using Encrypted Connections”](encrypted-connections.html "8.3 Using Encrypted Connections").
...
* `X509`
  ...
  must specify the [`--ssl-key`](connection-options.html#option_general_ssl-key)
  and [`--ssl-cert`](connection-options.html#option_general_ssl-cert) options to
  connect. (It is recommended but not required that
  [`--ssl-ca`](connection-options.html#option_general_ssl-ca) also be specified
...
* `ISSUER
  ...
  [`--ssl-key`](connection-options.html#option_general_ssl-key) and
  [`--ssl-cert`](connection-options.html#option_general_ssl-cert) options to
  connect. (It is recommended but not required that
  [`--ssl-ca`](connection-options.html#option_general_ssl-ca) also be specified
  so that the public certificate provided by the server can
  be verified.)
...
* `SUBJECT
  ...
  specify the [`--ssl-key`](connection-options.html#option_general_ssl-key) and
  [`--ssl-cert`](connection-options.html#option_general_ssl-cert) options to
  connect. (It is recommended but not required that
  [`--ssl-ca`](connection-options.html#option_general_ssl-ca) also be specified
  so that the public certificate provided by the server can
...
an account, as discussed in [Section 8.2.21, “Setting Account Resource Limits”](user-resources.html "8.2.21 Setting Account Resource Limits").
...
* `MAX_USER_CONNECTIONS
  ...
  *`count`* is `0`
  ...
  system variable. If
  [`max_user_connections`](server-system-variables.html#sysvar_max_user_connections) is
...
[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") supports several
...
[Section 8.2.16, “Server Handling of Expired Passwords”](expired-password-handling.html "8.2.16 Server Handling of Expired Passwords")). Operations
...
[Password Reuse Policy](password-management.html#password-reuse-policy "Password Reuse Policy").
...
* `PASSWORD HISTORY DEFAULT`
  ...
  specified by the
  [`password_history`](server-system-variables.html#sysvar_password_history) system
  variable.
...
* `PASSWORD_LOCK_TIME {N
  ...
  *`N`* must be a number from 0 to
  ...
  [Failed-Login Tracking and Temporary Account Locking](password-management.html#failed-login-tracking "Failed-Login Tracking and Temporary Account Locking").
...
MySQL 9.5 supports user comments and user
attributes, as described in [Section 15.7.1.3, “CREATE USER Statement”](create-user.html "15.7.1.3 CREATE USER Statement").
...
Information Schema
[`USER_ATTRIBUTES`](information-schema-user-attributes-table.html "28.3.51 The INFORMATION_SCHEMA USER_ATTRIBUTES Table") table as a JSON
...
[`JSON_MERGE_PATCH()`](json-modification-functions.html#function_json-merge-patch) function had
...
[`USER_ATTRIBUTES`](information-schema-user-attributes-table.html "28.3.51 The INFORMATION_SCHEMA USER_ATTRIBUTES Table") table; to remove
...
[`ALTER USER ...
ACCOUNT UNLOCK`](alter-user.html "15.7.1.1 ALTER USER Statement") unlocks any account named by the
statement that is temporarily locked due to too many failed
logins. See [Section 8.2.15, “Password Management”](password-management.html "8.2.15 Password Management").

##### ALTER USER Binary Logging

[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") is written to the
...
[`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statements with
