...
preceding tables in the same `FROM` clause. A
...
discussed in [Section 15.2.15.8, “Derived Tables”](derived-tables.html "15.2.15.8 Derived Tables"). The syntax for a
...
* In accordance with the SQL standard, MySQL always treats a
  ...
  [`JSON_TABLE()`](json-table-functions.html#function_json-table) as though
  `LATERAL` had been used. Since the
  `LATERAL` keyword is implicit, it is not
...
[`SELECT`](select.html "15.2.13 SELECT Statement") list could return multiple
columns:
