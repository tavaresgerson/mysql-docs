...
* [Common Table Expressions](with.html#common-table-expressions "Common Table Expressions")
* [Recursive Common Table Expressions](with.html#common-table-expressions-recursive "Recursive Common Table Expressions")
...
* [Recursive Common Table Expression Examples](with.html#common-table-expressions-recursive-examples "Recursive Common Table Expression Examples")
* [Common Table Expressions Compared to Similar Constructs](with.html#common-table-expressions-similar-constructs "Common Table Expressions Compared to Similar Constructs")
...
[`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause that has one or more
...
[`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause, and refers to them
in the top-level [`SELECT`](select.html "15.2.13 SELECT Statement") that
follows the [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause:
...
[`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause, each CTE name can be
...
[`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause:
...
containing the [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause.
...
included if any CTE in the [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)")
...
* Otherwise, the column names come from the select list of the
  first [`SELECT`](select.html "15.2.13 SELECT Statement") within the
  `AS (subquery)`
  part:
...
A [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause is permitted in
these contexts:

* At the beginning of [`SELECT`](select.html "15.2.13 SELECT Statement"),
...
* Immediately preceding [`SELECT`](select.html "15.2.13 SELECT Statement")
...
Only one [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause is permitted
at the same level. [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") followed
by [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") at the same level is not
...
[`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause that separates the
...
[`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clauses if they occur at
different levels:
...
A [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause can define one or
...
* A CTE can refer to CTEs defined earlier in the same
  [`WITH`](with.html "15.2.20 WITH (Common Table Expressions)") clause, but not those
  defined later.
...
see [Recursive Common Table Expressions](with.html#common-table-expressions-recursive "Recursive Common Table Expressions").
...
* The recursive CTE subquery has two parts, separated by
  ...
  or [`UNION
  [DISTINCT]`](union.html "15.2.18 UNION Clause"):
  ...
  The first [`SELECT`](select.html "15.2.13 SELECT Statement") produces the
  initial row or rows for the CTE and does not refer to the
  CTE name. The second [`SELECT`](select.html "15.2.13 SELECT Statement")
  ...
  [`SELECT`](select.html "15.2.13 SELECT Statement") part followed by a
  recursive [`SELECT`](select.html "15.2.13 SELECT Statement") part.

  Each [`SELECT`](select.html "15.2.13 SELECT Statement") part can itself
  be a union of multiple [`SELECT`](select.html "15.2.13 SELECT Statement")
  statements.

* The types of the CTE result columns are inferred from the
  column types of the nonrecursive
  [`SELECT`](select.html "15.2.13 SELECT Statement") part only, and the
  columns are all nullable. For type determination, the
  recursive [`SELECT`](select.html "15.2.13 SELECT Statement") part is
  ignored.

* If the nonrecursive and recursive parts are separated by
  [`UNION
  DISTINCT`](union.html "15.2.18 UNION Clause"), duplicate rows are eliminated. This is
...
At each iteration, that [`SELECT`](select.html "15.2.13 SELECT Statement")
...
[`SELECT`](select.html "15.2.13 SELECT Statement") determines the column
widths. Consequently, the wider `str` values
produced by the recursive [`SELECT`](select.html "15.2.13 SELECT Statement")
are truncated.
...
truncation or errors, use [`CAST()`](cast-functions.html#function_cast)
in the nonrecursive [`SELECT`](select.html "15.2.13 SELECT Statement") to
...
* The recursive [`SELECT`](select.html "15.2.13 SELECT Statement") part
  ...
  The recursive [`SELECT`](select.html "15.2.13 SELECT Statement") part of
  ...
  to [`UNION`](union.html "15.2.18 UNION Clause") members;
...
For recursive CTEs, [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement")
output rows for recursive [`SELECT`](select.html "15.2.13 SELECT Statement")
parts display `Recursive` in the
`Extra` column.

Cost estimates displayed by
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") represent cost per
...
performance; see [Section 10.4.4, “Internal Temporary Table Use in MySQL”](internal-temporary-tables.html "10.4.4 Internal Temporary Table Use in MySQL").

#### Limiting Common Table Expression Recursion

It is important for recursive CTEs that the recursive
[`SELECT`](select.html "15.2.13 SELECT Statement") part include a condition
...
* The [`cte_max_recursion_depth`](server-system-variables.html#sysvar_cte_max_recursion_depth)
...
* The [`max_execution_time`](server-system-variables.html#sysvar_max_execution_time)
  system variable enforces an execution timeout for
  [`SELECT`](select.html "15.2.13 SELECT Statement") statements executed
  within the current session.

* The [`MAX_EXECUTION_TIME`](optimizer-hints.html#optimizer-hints-execution-time "Statement Execution Time Optimizer Hints")
  optimizer hint enforces a per-query execution timeout for
  the [`SELECT`](select.html "15.2.13 SELECT Statement") statement in which
  it appears.
...
By default,
[`cte_max_recursion_depth`](server-system-variables.html#sysvar_cte_max_recursion_depth) has a
...
[`cte_max_recursion_depth`](server-system-variables.html#sysvar_cte_max_recursion_depth) value
...
outermost [`SELECT`](select.html "15.2.13 SELECT Statement"), for example:
...
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client"), typing **Control+C**
...
* [Date Series Generation](with.html#common-table-expressions-recursive-date-series "Date Series Generation")
* [Hierarchical Data Traversal](with.html#common-table-expressions-recursive-hierarchy-traversal "Hierarchical Data Traversal")
...
[`SELECT`](select.html "15.2.13 SELECT Statement") has access to the two
...
to the top-level [`SELECT`](select.html "15.2.13 SELECT Statement"). For
...
* The nonrecursive [`SELECT`](select.html "15.2.13 SELECT Statement")
  produces the lowest date in the date range spanned by the
  `sales` table.

* Each row produced by the recursive
  [`SELECT`](select.html "15.2.13 SELECT Statement") adds one day to the
...
* Are the queries inefficient, particularly the one with the
  [`MAX()`](aggregate-functions.html#function_max) subquery executed for
  each row in the recursive
  [`SELECT`](select.html "15.2.13 SELECT Statement")?
  [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") shows that the
  subquery containing [`MAX()`](aggregate-functions.html#function_max) is
  evaluated only once and the result is cached.

* The use of [`COALESCE()`](comparison-operators.html#function_coalesce) avoids
...
* The nonrecursive [`SELECT`](select.html "15.2.13 SELECT Statement")
  ...
  the recursive [`SELECT`](select.html "15.2.13 SELECT Statement").

* Each row produced by the recursive
  [`SELECT`](select.html "15.2.13 SELECT Statement") finds all employees
...
[`SELECT`](select.html "15.2.13 SELECT Statement"). For example, to display
the results for Tarek and Sarah, modify that
[`SELECT`](select.html "15.2.13 SELECT Statement") like this:
...
[`CREATE [TEMPORARY]
TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") but need not be defined or dropped explicitly.
