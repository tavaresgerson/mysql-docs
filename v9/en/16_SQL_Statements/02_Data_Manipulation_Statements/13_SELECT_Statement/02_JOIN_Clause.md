...
multiple-table [`DELETE`](delete.html "15.2.2 DELETE Statement") and
...
nested joins. See [Section 10.2.1.8, “Nested Join Optimization”](nested-join-optimization.html "10.2.1.8 Nested Join Optimization").

Index hints can be specified to affect how the MySQL optimizer
makes use of indexes. For more information, see
[Section 10.9.4, “Index Hints”](index-hints.html "10.9.4 Index Hints"). Optimizer hints and the
`optimizer_switch` system variable are other
ways to influence optimizer use of indexes. See
[Section 10.9.3, “Optimizer Hints”](optimizer-hints.html "10.9.3 Optimizer Hints"), and
...
* A *`table_subquery`* is also known as
  ...
  clause. See [Section 15.2.15.8, “Derived Tables”](derived-tables.html "15.2.15.8 Derived Tables"). Such
...
* Redundant columns of a `NATURAL` join do
  ...
  In the first [`SELECT`](select.html "15.2.13 SELECT Statement")
...
* A `USING` clause can be rewritten as an
  ...
  *` selects these values:
  ...
  b.c1)`](comparison-operators.html#function_coalesce) is the same as either
