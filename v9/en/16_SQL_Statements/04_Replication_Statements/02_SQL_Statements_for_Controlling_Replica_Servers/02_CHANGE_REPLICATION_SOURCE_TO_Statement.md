...
repositories (see [Section 19.2.4, “Relay Log and Replication Metadata Repositories”](replica-logs.html "19.2.4 Relay Log and Replication Metadata Repositories")).

[`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
requires the
[`REPLICATION_SLAVE_ADMIN`](privileges-provided.html#priv_replication-slave-admin) privilege
(or the deprecated [`SUPER`](privileges-provided.html#priv_super)
privilege).

Options that you do not specify on a [`CHANGE
REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement retain their value,
...
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement must name a channel using the
`FOR CHANNEL
channel` clause. See
[Section 19.2.2, “Replication Channels”](replication-channels.html "19.2.2 Replication Channels") for more information.

For some of the options of the [`CHANGE
REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement, you must issue a
[`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") statement prior to
issuing a [`CHANGE REPLICATION SOURCE
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement (and a [`START
REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement afterwards). Sometimes, you only
...
* Both the receiver thread and the applier thread must be
  ...
  REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement that employs
...
[`SHOW
REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement"). Note that the Group Replication
...
* If you are using statement-based replication and temporary
  ...
  REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement following a
  [`STOP
  REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") statement to leave behind temporary tables
  on the replica. A warning
  ([`ER_WARN_OPEN_TEMP_TABLES_MUST_BE_ZERO`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_warn_open_temp_tables_must_be_zero))
...
* [`ASSIGN_GTIDS_TO_ANONYMOUS_TRANSACTIONS
  = {OFF | LOCAL | uuid}`](change-replication-source-to.html#crs-opt-assign_gtids_to_anonymous_transactions)
  ...
  [`server_uuid`](replication-options.html#sysvar_server_uuid) setting).
  `uuid` assigns a
  GTID including the specified UUID, such as the
  [`server_uuid`](replication-options.html#sysvar_server_uuid) setting for the
  ...
  `uuid`, the
  replica must have
  [`gtid_mode=ON`](replication-options-gtids.html#sysvar_gtid_mode) set, and this
...
* [`GET_SOURCE_PUBLIC_KEY =
  {0|1}`](change-replication-source-to.html#crs-opt-get_source_public_key)
...
* [`GTID_ONLY = {0|1}`](change-replication-source-to.html#crs-opt-gtid_only)
  ...
  STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement") statements (where they are shown as being
  ...
  uses
  [`replica_parallel_workers=1`](replication-options-replica.html#sysvar_replica_parallel_workers)
...
* [`IGNORE_SERVER_IDS =
  (server_id_list)`](change-replication-source-to.html#crs-opt-ignore_server_ids)
  ...
  [`SHOW
  REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement") provide the list of servers that
  are currently ignored. For more information, see
  [Section 19.2.4.2, “Replication Metadata Repositories”](replica-logs-status.html "19.2.4.2 Replication Metadata Repositories"), and
  ...
  TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement is issued without
  ...
  [`RESET REPLICA
  ALL`](reset-replica.html "15.4.2.3 RESET REPLICA Statement") also clears
  `IGNORE_SERVER_IDS`.
  ...
  [`gtid_mode=ON`](replication-options-gtids.html#sysvar_gtid_mode). If
  `gtid_mode` is `ON`,
  ...
  [`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement"). In such
  cases, you can clear the list by issuing `CHANGE
  REPLICATION SOURCE TO` with an empty list of server
  IDs as shown previously.

* [`NETWORK_NAMESPACE =
  'namespace'`](change-replication-source-to.html#crs-opt-network_namespace)
  ...
  namespaces, see [Section 7.1.14, “Network Namespace Support”](network-namespace-support.html "7.1.14 Network Namespace Support").

* [`PRIVILEGE_CHECKS_USER = {NULL |
  'account'}`](change-replication-source-to.html#crs-opt-privilege_checks_user)
  ...
  the syntax described in [Section 8.2.4, “Specifying Account Names”](account-names.html "8.2.4 Specifying Account Names"), and
  ...
  [Section 19.3.3, “Replication Privilege Checks”](replication-privilege-checks.html "19.3.3 Replication Privilege Checks"). When you
...
* [`RELAY_LOG_FILE =
  'relay_log_file'`](change-replication-source-to.html#crs-opt-relay_log_file) ,
  [`RELAY_LOG_POS =
...
* [`REQUIRE_ROW_FORMAT = {0|1}`](change-replication-source-to.html#crs-opt-require_row_format)
...
* [`REQUIRE_TABLE_PRIMARY_KEY_CHECK =
  {STREAM | ON | OFF | GENERATE}`](change-replication-source-to.html#crs-opt-require_table_primary_key_check)
  ...
  + `ON`: The replica sets
    ...
    ON`](server-system-variables.html#sysvar_sql_require_primary_key); any replicated [`CREATE
    TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") or [`ALTER
    TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") statement must result in a table that
  ...
  + `GENERATE`: Causes the replica to
    ...
    [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") table that, as
    replicated, lacks a primary key. See
    [Section 15.1.24.11, “Generated Invisible Primary Keys”](create-table-gipks.html "15.1.24.11 Generated Invisible Primary Keys"), for more
    information.
...
* [`SOURCE_AUTO_POSITION =
  {0|1}`](change-replication-source-to.html#crs-opt-source_auto_position)
  ...
  ([`GTID_MODE=ON`](replication-options-gtids.html#sysvar_gtid_mode),
  `ON_PERMISSIVE,` or
  `OFF_PERMISSIVE` on the replica, and
  [`GTID_MODE=ON`](replication-options-gtids.html#sysvar_gtid_mode) on the
  ...
  [`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged) system variable
  by another method, the source sends the error
  [`ER_SOURCE_HAS_PURGED_REQUIRED_GTIDS`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_source_has_purged_required_gtids)
  ...
  [`ER_FOUND_MISSING_GTIDS`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_found_missing_gtids).
  ...
  [`ER_REPLICA_HAS_MORE_GTIDS_THAN_SOURCE`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_replica_has_more_gtids_than_source)
  ...
  [`replication_connection_status`](performance-schema-replication-connection-status-table.html "29.12.11.13 The replication_connection_status Table")
  table or the output of
  [`SHOW
  REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement"). Disabling the
  `SOURCE_AUTO_POSITION` option again makes
  the replica revert to file-based replication.

* [`SOURCE_BIND =
  'interface_name'`](change-replication-source-to.html#crs-opt-source_bind)
  ...
  output from
  [`SHOW
  REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement"). In the source metadata repository
...
* [`SOURCE_COMPRESSION_ALGORITHMS =
  'algorithm[,algorithm][,algorithm]'`](change-replication-source-to.html#crs-opt-source_compression_algorithms)
...
* [`SOURCE_CONNECT_RETRY =
  interval`](change-replication-source-to.html#crs-opt-source_connect_retry)
...
* [`SOURCE_CONNECTION_AUTO_FAILOVER =
  {0|1}`](change-replication-source-to.html#crs-opt-source_connection_auto_failover)
  ...
  functions
  [`asynchronous_connection_failover_add_source()`](replication-functions-async-failover.html#function_asynchronous-connection-failover-add-source)
  and
  ...
  and
  [`asynchronous_connection_failover_delete_managed()`](replication-functions-async-failover.html#function_asynchronous-connection-failover-delete-managed)
  ...
  3. When you set `SOURCE_CONNECTION_AUTO_FAILOVER
     ...
     TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement with the
     `SOURCE_USER` and
     `SOURCE_PASSWORD` options. For more
     information, see
     [Section 19.4.9, “Switching Sources and Replicas with Asynchronous Connection Failover”](replication-asynchronous-connection-failover.html "19.4.9 Switching Sources and Replicas with Asynchronous Connection Failover").

  4. When you set `SOURCE_CONNECTION_AUTO_FAILOVER
     ...
     the
     [`group_replication_disable_member_action()`](group-replication-functions-for-member-actions.html#function_group-replication-disable-member-action)
...
* [`SOURCE_DELAY =
  interval`](change-replication-source-to.html#crs-opt-source_delay)
...
* [`SOURCE_HEARTBEAT_PERIOD =
  interval`](change-replication-source-to.html#crs-opt-source_heartbeat_period)
  ...
  [`replica_net_timeout`](replication-options-replica.html#sysvar_replica_net_timeout) system
  variable. It is recorded in the source metadata repository
  and shown in the
  [`replication_connection_configuration`](performance-schema-replication-connection-configuration-table.html "29.12.11.12 The replication_connection_configuration Table")
  Performance Schema table.

  The [`replica_net_timeout`](replication-options-replica.html#sysvar_replica_net_timeout)
  ...
  TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") to adjust the heartbeat interval to an
...
* [`SOURCE_HOST =
  'host_name'`](change-replication-source-to.html#crs-opt-source_host)
...
* [`SOURCE_LOG_FILE =
  'source_log_name'`](change-replication-source-to.html#crs-opt-source_log_file),
  [`SOURCE_LOG_POS =
  source_log_pos`](change-replication-source-to.html#crs-opt-source_log_file)
...
* [`SOURCE_PASSWORD =
  'password'`](change-replication-source-to.html#crs-opt-source_password)
  ...
  PROCESSLIST`](show-processlist.html "15.7.7.32 SHOW PROCESSLIST Statement") statements.

* [`SOURCE_PORT =
  port_num`](change-replication-source-to.html#crs-opt-source_port)
...
* [`SOURCE_PUBLIC_KEY_PATH =
  'key_file_name'`](change-replication-source-to.html#crs-opt-source_public_key_path)
...
* [`SOURCE_RETRY_COUNT =
  count`](change-replication-source-to.html#crs-opt-source_retry_count)
  ...
  [`--master-retry-count`](replication-options-replica.html#option_mysqld_master-retry-count) server
  startup option.

* [`SOURCE_SSL = {0|1}`](change-replication-source-to.html#crs-opt-source_ssl)

  Specify whether the replica encrypts the replication
  connection. The default is 1, meaning you can configure the
  encryption using the
  `SOURCE_SSL_xxx`
  and
  `SOURCE_TLS_xxx`
  options. If you set `SOURCE_SSL=0`, the
  replica does not encrypt the replication connection.

  With the `SOURCE_SSL=1` setting for a
  replication connection, no further setting of
  `SOURCE_SSL_xxx`
  options corresponds to setting
  `--ssl-mode=REQUIRED` for the client, as
  described in [Command Options for Encrypted Connections](connection-options.html#encrypted-connection-options "Command Options for Encrypted Connections").
...
* [`SOURCE_SSL_xxx`](change-replication-source-to.html#crs-opt-source_ssl_xxx),
  [`SOURCE_TLS_xxx`](change-replication-source-to.html#crs-opt-source_ssl_xxx)
  ...
  [`SOURCE_TLS_CIPHERSUITES`](change-replication-source-to.html#crs-opt-source_ssl_xxx),
  ...
  [Command Options for Encrypted Connections](connection-options.html#encrypted-connection-options "Command Options for Encrypted Connections"). The
...
* [`SOURCE_USER =
  'user_name'`](change-replication-source-to.html#crs-opt-source_user)
  ...
  REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement or [`START
  GROUP_REPLICATION`](start-group-replication.html "15.4.3.1 START GROUP_REPLICATION Statement") statement that starts the
  ...
  [Section 19.3.1, “Setting Up Replication to Use Encrypted Connections”](replication-encrypted-connections.html "19.3.1 Setting Up Replication to Use Encrypted Connections"), or
...
* [`SOURCE_ZSTD_COMPRESSION_LEVEL =
  level`](change-replication-source-to.html#crs-opt-source_zstd_compression_level)
...
[`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") is
...
replication, see [Section 19.1.5, “MySQL Multi-Source Replication”](replication-multi-source.html "19.1.5 MySQL Multi-Source Replication").
...
[`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") to
...
[`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") or
another method, see
