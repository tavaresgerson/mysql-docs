...
Like [`START REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement"), this
...
[`rpl_stop_replica_timeout`](replication-options-replica.html#sysvar_rpl_stop_replica_timeout). This
...
Some [`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
...
[Section 15.4.2.2, “CHANGE REPLICATION SOURCE TO Statement”](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement"), and
...
Any gaps in the sequence of transactions executed from the relay
log are closed as part of stopping the worker threads. If the
replica is stopped unexpectedly (for example due to an error in
a worker thread, or another thread issuing
[`KILL`](kill.html "15.7.8.4 KILL Statement")) while a `STOP
REPLICA` statement is executing, the sequence of
executed transactions from the relay log may become
inconsistent. See
...
[`Replica_open_temp_tables`](server-status-variables.html#statvar_Replica_open_temp_tables). When
using statement-based replication, this value should be 0 before
executing [`CHANGE REPLICATION SOURCE
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement"). If there are any temporary tables open on the
replica, issuing a [`CHANGE REPLICATION
SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement after issuing a `STOP
REPLICA` causes an
