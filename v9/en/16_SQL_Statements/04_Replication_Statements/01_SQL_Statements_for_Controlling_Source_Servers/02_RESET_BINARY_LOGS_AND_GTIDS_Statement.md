...
the [`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged) system variable
...
[`RESET
REPLICA`](reset-replica.html "15.4.2.3 RESET REPLICA Statement") has no effect on the GTID execution history.
...
1. `RESET BINARY LOGS AND GTIDS` removes
...
2. `RESET BINARY LOGS AND GTIDS` is
   *not* intended to be used while any
   ...
   [`PURGE BINARY LOGS`](purge-binary-logs.html "15.4.1.1 PURGE BINARY LOGS Statement") may be
...
4. When replication is running correctly, issue
   [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") followed by
   [`RESET REPLICA`](reset-replica.html "15.4.2.3 RESET REPLICA Statement") (both on the
