...
[`CREATE OR REPLACE
VIEW`](create-view.html "15.1.27 CREATE VIEW Statement") is the same as [`CREATE
VIEW`](create-view.html "15.1.27 CREATE VIEW Statement"). If the view does exist,
[`CREATE OR REPLACE
VIEW`](create-view.html "15.1.27 CREATE VIEW Statement") replaces it.
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statement that provides the
definition of the view. (Selecting from the view selects, in
effect, using the [`SELECT`](select.html "15.2.13 SELECT Statement")
statement.) The *`select_statement`* can
select from base tables or from other views. The
[`SELECT`](select.html "15.2.13 SELECT Statement") statement can use a
[`VALUES`](values.html "15.2.19 VALUES Statement") statement as its source, or
can be replaced with a [`TABLE`](table.html "15.2.16 TABLE Statement")
statement, as with
[`CREATE TABLE
... SELECT`](create-table-select.html "15.1.24.4 CREATE TABLE ... SELECT Statement").

`IF NOT EXISTS` causes the view to be created if
it does not already exist. If the view already exists and
`IF NOT EXISTS` is specified, the statement is
succeeds with a warning rather than an error; in this case, the
view definition is not changed. For example:
...
`IF NOT EXISTS` and `OR REPLACE`
are mutually exclusive and cannot be used together in the same
`CREATE VIEW` statement. Attempting to do so
causes the statement to be rejected with a syntax error.
...
The [`CREATE VIEW`](create-view.html "15.1.27 CREATE VIEW Statement") statement requires
the [`CREATE VIEW`](privileges-provided.html#priv_create-view) privilege for the
view, and some privilege for each column selected by the
[`SELECT`](select.html "15.2.13 SELECT Statement") statement. For columns used
elsewhere in the [`SELECT`](select.html "15.2.13 SELECT Statement") statement,
you must have the [`SELECT`](privileges-provided.html#priv_select) privilege.
If the `OR REPLACE` clause is present, you must
also have the [`DROP`](privileges-provided.html#priv_drop) privilege for
the view. If the `DEFINER` clause is present, the
privileges required depend on the *`user`*
value, as discussed in [Section 27.8, “Stored Object Access Control”](stored-objects-security.html "27.8 Stored Object Access Control").
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statement are also
...
Columns retrieved by the [`SELECT`](select.html "15.2.13 SELECT Statement")
...
the [`SELECT`](select.html "15.2.13 SELECT Statement") statement are used for
...
* The [`SELECT`](select.html "15.2.13 SELECT Statement") statement cannot
  refer to system variables or user-defined variables.

* Within a stored program, the
  [`SELECT`](select.html "15.2.13 SELECT Statement") statement cannot refer
...
* Any table or view referred to in the definition must exist.
  ...
  use the [`CHECK TABLE`](check-table.html "15.7.3.2 CHECK TABLE Statement") statement.
...
* Aliases for column names in the
  [`SELECT`](select.html "15.2.13 SELECT Statement") statement are checked
...
[`SELECT`](select.html "15.2.13 SELECT Statement") keyword, and to clauses such
as `INTO`, `FOR UPDATE`,
...
VIEW`](create-view.html "15.1.27 CREATE VIEW Statement") statement. This is the same as specifying
...
characteristic, [`CURRENT_USER`](information-functions.html#function_current-user)
...
[`CURRENT_USER`](information-functions.html#function_current-user) returns the routine's
...
* At view definition time, the view creator must have the
  ...
  [`SELECT`](privileges-provided.html#priv_select) privilege for each
...
* The user who references a view must have appropriate
  privileges to access it ([`SELECT`](privileges-provided.html#priv_select)
  to select from it, [`INSERT`](privileges-provided.html#priv_insert) to
...
[`SELECT`](privileges-provided.html#priv_select)) and may grant them. MySQL
...
[Section 27.6.2, “View Processing Algorithms”](view-algorithms.html "27.6.2 View Processing Algorithms"), as well as
...
such as [`UPDATE`](update.html "15.2.17 UPDATE Statement"),
[`DELETE`](delete.html "15.2.2 DELETE Statement"), or
[`INSERT`](insert.html "15.2.7 INSERT Statement") to update the contents of
...
columns, see [Section 15.1.24.8, “CREATE TABLE and Generated Columns”](create-table-generated-columns.html "15.1.24.8 CREATE TABLE and Generated Columns").
...
[Section 27.6.3, “Updatable and Insertable Views”](view-updatability.html "27.6.3 Updatable and Insertable Views"), and
[Section 27.6.4, “The View WITH CHECK OPTION Clause”](view-check-option.html "27.6.4 The View WITH CHECK OPTION Clause").