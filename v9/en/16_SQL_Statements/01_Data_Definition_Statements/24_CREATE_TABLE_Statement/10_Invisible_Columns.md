...
MySQL 9.5 supports invisible columns. An invisible
...
* [DDL Statements and Invisible Columns](invisible-columns.html#invisible-column-ddl-statements "DDL Statements and Invisible Columns")
* [DML Statements and Invisible Columns](invisible-columns.html#invisible-column-dml-statements "DML Statements and Invisible Columns")
* [Invisible Column Metadata](invisible-columns.html#invisible-column-metadata "Invisible Column Metadata")
* [The Binary Log and Invisible Columns](invisible-columns.html#invisible-column-binary-logging "The Binary Log and Invisible Columns")
...
definition for [`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") or
...
[`CREATE
TABLE ... LIKE`](create-table-like.html "15.1.24.3 CREATE TABLE ... LIKE Statement") includes invisible columns, and they
are invisible in the new table.

[`CREATE
TABLE ... SELECT`](create-table-select.html "15.1.24.4 CREATE TABLE ... SELECT Statement") does not include invisible columns,
unless they are explicitly referenced in the
[`SELECT`](select.html "15.2.13 SELECT Statement") part. However, even if
...
TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") part of the
[`CREATE
TABLE ... SELECT`](create-table-select.html "15.1.24.4 CREATE TABLE ... SELECT Statement") statement:
...
For [`SELECT`](select.html "15.2.13 SELECT Statement") statements, an
...
The statement [`TABLE
t1`](table.html "15.2.16 TABLE Statement") produces the same output as the first
...
For [`INSERT`](insert.html "15.2.7 INSERT Statement") (and
[`REPLACE`](replace.html "15.2.12 REPLACE Statement"), for non-replaced
...
For the first two [`INSERT`](insert.html "15.2.7 INSERT Statement")
statements, the `VALUES()` list must provide
a value for each visible column and no invisible column. For
the third [`INSERT`](insert.html "15.2.7 INSERT Statement") statement, the
`VALUES()` list must provide the same number
of values as the number of named columns; the same is true
when you use [`VALUES
ROW()`](values.html "15.2.19 VALUES Statement") rather than `VALUES()`.

For [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") and
[`LOAD XML`](load-xml.html "15.2.10 LOAD XML Statement"), implicit default
...
[`REPLACE INTO ...
SELECT *`](replace.html "15.2.12 REPLACE Statement") do not include invisible columns because
...
* With the `IGNORE` modifier,
  ...
  [`LOAD XML`](load-xml.html "15.2.10 LOAD XML Statement") ignore the new
  row.

* [`REPLACE`](replace.html "15.2.12 REPLACE Statement") replaces the
  existing row with the new row. With the
  `REPLACE` modifier,
  [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") and
  [`LOAD XML`](load-xml.html "15.2.10 LOAD XML Statement") do the same.

* [`INSERT
  ... ON DUPLICATE KEY UPDATE`](insert-on-duplicate.html "15.2.7.2 INSERT ... ON DUPLICATE KEY UPDATE Statement") updates the existing
  row.

To update invisible columns for
[`UPDATE`](update.html "15.2.17 UPDATE Statement") statements, name them
...
or [`SHOW COLUMNS`](show-columns.html "15.7.7.6 SHOW COLUMNS Statement") output. For
example:
...
[**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") uses `SHOW CREATE
...
* Invisible columns are treated like visible columns in row
  ...
  [`binlog_row_image`](replication-options-binary-log.html#sysvar_binlog_row_image) system
  variable setting.
...
* The [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") command includes
