...
[`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") statement includes
the following:
...
* [Locking](create-table-foreign-keys.html#foreign-key-locking "Locking")
...
* If the `CONSTRAINT`
  ...
  *`symbol`* clause is not defined,
  ...
  [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engines ignore
  `FOREIGN_KEY
  index_name`.
...
* Creating a foreign key constraint requires the
  [`REFERENCES`](privileges-provided.html#priv_references) privilege on the
  parent table.
...
* Corresponding columns in the foreign key and the
  ...
  [`INTEGER`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") and
  [`DECIMAL`](fixed-point-types.html "13.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") must be the
...
* Previously, `InnoDB` allowed a foreign
  ...
  *first* columns in the same order.
  ...
  [Section 17.6.2.1, “Clustered and Secondary Indexes”](innodb-index-types.html "17.6.2.1 Clustered and Secondary Indexes")). You should expect
...
* Index prefixes on foreign key columns are not supported.
  Consequently, [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") and
  [`TEXT`](blob.html "13.3.4 The BLOB and TEXT Types") columns cannot be
  included in a foreign key because indexes on those columns
  must always include a prefix length.

* [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") does not currently
  ...
  [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables that are
...
When an [`UPDATE`](update.html "15.2.17 UPDATE Statement") or
[`DELETE`](delete.html "15.2.2 DELETE Statement") operation affects a key
...
* `NO ACTION`: A keyword from standard SQL.
  For [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine"), this is
  ...
  referenced table. [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5")
...
* `SET DEFAULT`: This action is recognized
  ...
  [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") and
  [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") reject table definitions
...
any [`INSERT`](insert.html "15.2.7 INSERT Statement") or
[`UPDATE`](update.html "15.2.17 UPDATE Statement") operation that attempts
...
[`SHOW CREATE TABLE`](show-create-table.html "15.7.7.12 SHOW CREATE TABLE Statement") output or in
tables dumped with [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program").
`RESTRICT`, which is an equivalent
non-default keyword, appears in [`SHOW
CREATE TABLE`](show-create-table.html "15.7.7.12 SHOW CREATE TABLE Statement") output and in tables dumped with
[**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program").

For [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables, `ON
...
[`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") types. (Bug #89511, Bug
#27484882)
...
MySQL 9.5 supports inline
`REFERENCE` clauses as well as implicit
parent table primary keys, so the second
[`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statement can be
...
using the following [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement")
syntax:
...
[`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement"), *remember
...
[`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") syntax:
...
CREATE TABLE`](show-create-table.html "15.7.7.12 SHOW CREATE TABLE Statement"):
...
supported for
[`ALTER TABLE ...
ALGORITHM=INPLACE`](alter-table.html "15.1.11 ALTER TABLE Statement"). It is not supported for
[`ALTER TABLE ...
...
[`foreign_key_checks`](server-system-variables.html#sysvar_foreign_key_checks) variable,
...
[`foreign_key_checks`](server-system-variables.html#sysvar_foreign_key_checks) variable
has the same effect on [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") tables
as it does for [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") tables.

The [`foreign_key_checks`](server-system-variables.html#sysvar_foreign_key_checks)
...
* Reloading tables in different order than required by their
  ...
  [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") produces correct definitions
  ...
  [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") automatically includes a
  statement in the dump output that disables
  [`foreign_key_checks`](server-system-variables.html#sysvar_foreign_key_checks). This
  ...
  [`foreign_key_checks`](server-system-variables.html#sysvar_foreign_key_checks) also
  speeds up the import operation by avoiding foreign key
  checks.

* Executing [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement")
  operations, to avoid foreign key checking.

* Performing an [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement")
  operation on a table that has a foreign key relationship.

When [`foreign_key_checks`](server-system-variables.html#sysvar_foreign_key_checks) is
...
Disabling [`foreign_key_checks`](server-system-variables.html#sysvar_foreign_key_checks)
...
* Enabling
  [`foreign_key_checks`](server-system-variables.html#sysvar_foreign_key_checks) does
...
TABLES`](lock-tables.html "15.3.6 LOCK TABLES and UNLOCK TABLES Statements"), any tables related by a foreign key
constraint are opened and locked implicitly. For foreign key
checks, a shared read-only lock
([`LOCK TABLES
READ`](lock-tables.html "15.3.6 LOCK TABLES and UNLOCK TABLES Statements")) is taken on related tables. For cascading
updates, a shared-nothing write lock
([`LOCK TABLES
WRITE`](lock-tables.html "15.3.6 LOCK TABLES and UNLOCK TABLES Statements")) is taken on related tables that are involved
in the operation.
...
Information Schema
[`KEY_COLUMN_USAGE`](information-schema-key-column-usage-table.html "28.3.16 The INFORMATION_SCHEMA KEY_COLUMN_USAGE Table") table. An
...
[`ER_NO_REFERENCED_ROW_2`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_no_referenced_row_2) and
[`ER_ROW_IS_REFERENCED_2`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_row_is_referenced_2) error
...
([`ER_NO_REFERENCED_ROW`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_no_referenced_row) and
[`ER_ROW_IS_REFERENCED`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_row_is_referenced)).
