...
The storage engine for `TEMPORARY` tables created with [`CREATE TEMPORARY TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") can be set separately from the engine for permanent tables by setting the `default_tmp_storage_engine`, either at startup or at runtime.
