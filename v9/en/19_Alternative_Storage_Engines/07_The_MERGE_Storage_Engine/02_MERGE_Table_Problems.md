...
* `MERGE` child tables are locked through the
  parent table. If the parent is temporary, it is not locked,
  and thus the child tables are also not locked; this means that
  parallel use of the underlying
  [`MyISAM`](myisam-storage-engine.html "18.2 The MyISAM Storage Engine") tables corrupts them.

* If you use [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") to
...
* A `MERGE` table cannot maintain uniqueness
  ...
  [`INSERT`](insert.html "15.2.7 INSERT Statement"), the data goes into the
...
* Because the `MERGE` engine cannot enforce
  ...
  [`REPLACE`](replace.html "15.2.12 REPLACE Statement") does not work as
  expected. The two key facts are:

  + [`REPLACE`](replace.html "15.2.12 REPLACE Statement") can detect unique
  ...
  + If [`REPLACE`](replace.html "15.2.12 REPLACE Statement") detects a unique
...
* You should not use [`ANALYZE
  TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement"), [`REPAIR TABLE`](repair-table.html "15.7.3.5 REPAIR TABLE Statement"),
  ...
  [`DELETE`](delete.html "15.2.2 DELETE Statement") without a
  `WHERE` clause, or
  [`TRUNCATE TABLE`](truncate-table.html "15.1.42 TRUNCATE TABLE Statement") on any of the
...
* [`DROP TABLE`](drop-table.html "15.1.37 DROP TABLE Statement") on a table that is
...
* The definition of the `MyISAM` tables and the
  ...
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statement). The checks
...
* The order of indexes in the `MERGE` table and
  ...
  [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") to add a
  `UNIQUE` index to a table used in a
  `MERGE` table, and then use
  [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") to add a nonunique
  ...
  [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") puts
...
* You cannot change the union list of a nontemporary
  ...
  TABLES`](lock-tables.html "15.3.6 LOCK TABLES and UNLOCK TABLES Statements") is in effect. The following does
  *not* work:
