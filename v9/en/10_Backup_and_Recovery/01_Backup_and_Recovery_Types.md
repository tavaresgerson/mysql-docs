...
structure ([`CREATE DATABASE`](create-database.html "15.1.14 CREATE DATABASE Statement"),
[`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statements) and
content ([`INSERT`](insert.html "15.2.7 INSERT Statement") statements or
...
* For restore:
  ...
  + [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "25.5.23 ndb_restore — Restore an NDB Cluster Backup") restores
...
* Logical backup tools include the [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program")
  program and the [`SELECT
  ... INTO OUTFILE`](select.html "15.2.13 SELECT Statement") statement. These work for any
  storage engine, even `MEMORY`.

* To restore logical backups, SQL-format dump files can be
  processed using the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client. To load
  delimited-text files, use the [`LOAD
  DATA`](load-data.html "15.2.9 LOAD DATA Statement") statement or the [**mysqlimport**](mysqlimport.html "6.5.5 mysqlimport — A Data Import Program")
  client.
...
* [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") can connect to local or remote
  servers. For SQL output (`CREATE` and
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statements), local or
...
* [`SELECT ... INTO
  OUTFILE`](select-into.html "15.2.13.1 SELECT ... INTO Statement") can be initiated from a local or remote
...
[`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") tables, this is not a typical
issue. For programs to check [`MyISAM`](myisam-storage-engine.html "18.2 The MyISAM Storage Engine")
