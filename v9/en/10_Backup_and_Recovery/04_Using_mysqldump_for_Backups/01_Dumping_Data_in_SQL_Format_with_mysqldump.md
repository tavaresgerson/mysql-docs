...
The difference between the two preceding commands is that without `--databases`, the dump output contains no [`CREATE DATABASE`](create-database.html "15.1.14 CREATE DATABASE Statement") or `USE` statements. This has several implications:
...
* Because the output contains no [`CREATE DATABASE`](create-database.html "15.1.14 CREATE DATABASE Statement") statement, the `--add-drop-database` option has no effect. If you use it, it produces no `DROP DATABASE` statement.
