...
Counts of several JavaScript library SQL statements are kept as status variables. These include `Com_create_library`, `Com_drop_library`, `Com_alter_library`, `Com_show_create_library`, and `Com_show_library_status`; these indicate, respectively, the numbers of `CREATE LIBRARY`, `DROP LIBRARY`, `ALTER LIBRARY`, `SHOW CREATE LIBRARY`, and `SHOW LIBRARY STATUS` statements executed. For more information, see Com_xxx Variables.
