...
* Previously, tables in the `mysql` system
  ...
  9.5, data dictionary tables are invisible and
...
* `INFORMATION_SCHEMA` tables in MySQL
  ...
  + Previously, `INFORMATION_SCHEMA` queries
    ...
    [`TABLES`](information-schema-tables-table.html "28.3.44 The INFORMATION_SCHEMA TABLES Table") tables retrieved
    statistics directly from storage engines. As of MySQL
    9.5, cached table statistics are used by
    default. The
  ...
  + [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") no longer dumps the
    `INFORMATION_SCHEMA` database, even if
    explicitly named on the command line.

  + [`CREATE
    TABLE dst_tbl LIKE
    src_tbl`](create-table-like.html "15.1.24.3 CREATE TABLE ... LIKE Statement") requires that
  ...
  + Previously, result set headers of columns selected from
    ...
    As of MySQL 9.5, these headers are
...
* The data directory affects how [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program")
  dumps information from the `mysql` system
  database:

  + [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") only dumps non-data
  ...
  + Previously, the
    ...
    MySQL 9.5, that table is not used;
