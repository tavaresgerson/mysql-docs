...
The list of databases displayed by the statement may be different on your machine; `SHOW DATABASES` does not show databases that you have no privileges for if you do not have the `SHOW DATABASES` privilege. See Section 15.7.7.16, “SHOW DATABASES Statement”.
...
`VARCHAR` is a good choice for the `name`, `owner`, and `species` columns because the column values vary in length. The lengths in those column definitions need not all be the same, and need not be `20`. You can normally pick any length from `1` to `65535`, whatever seems most reasonable to you. If you make a poor choice and it turns out later that you need a longer field, MySQL provides an [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") statement.
...
Once you have created a table, [`SHOW TABLES`](show-tables.html "15.7.7.40 SHOW TABLES Statement") should produce some output:
...
You can specify the column value separator and end of line marker explicitly in the [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") statement if you wish, but the defaults are tab and linefeed. These are sufficient for the statement to read the file `pet.txt` properly.
...
Given these considerations, the [`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statement for the `event` table might look like this:
