...
views into the firewall in-memory data cache. These tables are
available only if the appropriate firewall plugins are enabled. Like
the firewall plugin, these tables are deprecated, and subject to
removal in a future version of MySQL.

These `INFORMATION_SCHEMA` tables are
*not* used or supported by the MySQL Enterprise Firewall component.