...
Functionality remains the same and data format does not change in an LTS series, therefore in-place upgrades and downgrades are possible within the LTS series. For example, MySQL 8.4.0 can be upgraded to a later MySQL 8.4.x release. Additional upgrade and downgrade methods are available, such as [the clone plugin](clone-plugin.html "7.6.6 The Clone Plugin").

Upgrading to the next [LTS series](glossary.html#glos_lts_series "LTS Series") is supported, such as 8.4.x LTS to 9.7.x LTS, while skipping an LTS series is not supported. For example, you cannot skip from 8.4.x LTS directly to 10.7.x LTS.

**Innovation Notes**

An Innovation installation follows similar behavior in that an Innovation release upgrades to a more recent Innovation series release. For example, MySQL 9.1.0 Innovation would upgrade to MySQL 9.2.0.

The main difference is that you cannot directly upgrade between an [Innovation series](glossary.html#glos_innovation_series "Innovation Series") of different major versions, such as 8.3.0 to 9.0.0. Instead, first upgrade to the nearest LTS series and then upgrade to the following Innovation series. For example, upgrading 8.3.0 to 8.4.0, and then 8.4.0 to 9.0.0, is a valid upgrade path.
