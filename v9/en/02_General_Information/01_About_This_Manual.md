...
This is the Reference Manual for the MySQL Database System, version 9.5, through release 9.5.0. Differences between minor versions of MySQL 9.5 are noted in the present text with reference to release numbers (9.5.*`x`*). For license information, see the [Legal Notices](preface.html#legalnotice "Legal Notices").

This manual is not intended for use with older versions of the MySQL software due to the many functional and other differences between MySQL 9.5 and previous versions. If you are using an earlier release of the MySQL software, please refer to the appropriate manual. For example, the [MySQL 8.0 Reference Manual](/doc/refman/8.0/en/) covers the 8.0 bugfix series of MySQL software releases.
...
* `Text in this style` is used for SQL statements; database, table, and column names; program listings and source code; and environment variables. Example: “To reload the grant tables, use the [`FLUSH PRIVILEGES`](flush.html#flush-privileges) statement.”
...
An ellipsis (`...`) indicates the omission of a section of a statement, typically to provide a shorter version of more complex syntax. For example, [`SELECT ... INTO OUTFILE`](select.html "15.2.13 SELECT Statement") is shorthand for the form of `SELECT` statement that has an `INTO OUTFILE` clause following other parts of the statement.
