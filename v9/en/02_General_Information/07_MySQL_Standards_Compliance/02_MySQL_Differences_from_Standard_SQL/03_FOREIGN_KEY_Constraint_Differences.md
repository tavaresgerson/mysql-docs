...
* If there are several rows in the parent table with the
  ...
  [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") performs a foreign key
  ...
  any of the parent rows. This is shown in the following
  example:
...
* In an SQL statement that inserts, deletes, or updates many
  ...
  checks, [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") sets shared
...
* A `FOREIGN KEY` constraint that
  ...
  [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") extension that is now
  deprecated, and must be enabled by setting
  [`restrict_fk_on_non_standard_key`](server-system-variables.html#sysvar_restrict_fk_on_non_standard_key).
  You should expect support for use of nonstandard keys to
  be removed in a future version of MySQL, and migrate away
  from them now.

  The [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine
  requires an explicit unique key (or primary key) on any
  column referenced as a foreign key, as per the SQL
  standard.

* For storage engines that do not support foreign keys (such
  as [`MyISAM`](myisam-storage-engine.html "18.2 The MyISAM Storage Engine")), MySQL Server
  parses and ignores foreign key specifications.

* Previous versions of MySQL parsed but ignored
  “inline `REFERENCES`
  specifications” (as defined in the SQL standard)
  where the references were defined as part of the column
  specification. MySQL 9.5 accepts such
  `REFERENCES` clauses and enforces the
  foreign keys thus created. In addition, MySQL
  9.5 allows implicit references to the parent
  table's primary key. This means that the following
  syntax is valid:
  ...
  You can see that this works by checking the output of
  [`SHOW CREATE TABLE`](show-create-table.html "15.7.7.12 SHOW CREATE TABLE Statement") or
  [`DESCRIBE`](describe.html "15.8.1 DESCRIBE Statement"), like this:
