...
supported for use with [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") and
...
more information, see [Section 14.9.9, “MeCab Full-Text Parser Plugin”](fulltext-search-mecab.html "14.9.9 MeCab Full-Text Parser Plugin").
...
[Section 14.9, “Full-Text Search Functions”](fulltext-search.html "14.9 Full-Text Search Functions") applies to the ngram parser
...
[`ft_max_word_len`](server-system-variables.html#sysvar_ft_max_word_len)) are also
applicable.
...
[`ngram_token_size`](server-system-variables.html#sysvar_ngram_token_size) configuration
option, which has a minimum value of 1 and maximum value of 10.

Typically, [`ngram_token_size`](server-system-variables.html#sysvar_ngram_token_size) is
set to the size of the largest token that you want to search
for. If you only intend to search for single characters, set
[`ngram_token_size`](server-system-variables.html#sysvar_ngram_token_size) to 1. A
...
[`ngram_token_size`](server-system-variables.html#sysvar_ngram_token_size) accordingly.
...
as these, set [`ngram_token_size`](server-system-variables.html#sysvar_ngram_token_size)
to a value of 2 or higher.

As a read-only variable,
[`ngram_token_size`](server-system-variables.html#sysvar_ngram_token_size) may only be
...
[`ngram_token_size=2`](server-system-variables.html#sysvar_ngram_token_size), a document
...
[`ngram_token_size`](server-system-variables.html#sysvar_ngram_token_size) are ignored.
...
[`ngram_token_size=2`](server-system-variables.html#sysvar_ngram_token_size)) is
...
* If the prefix term of a wildcard search is shorter than
  ...
  example, assuming
  [`ngram_token_size=2`](server-system-variables.html#sysvar_ngram_token_size), a
...
* If the prefix term of a wildcard search is longer than ngram
  ...
  [`ngram_token_size=2`](server-system-variables.html#sysvar_ngram_token_size), an
