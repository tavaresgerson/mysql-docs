...
MODE` modifier, the
[`MATCH()`](fulltext-search.html#function_match) function performs a
...
When [`MATCH()`](fulltext-search.html#function_match) is used in a
...
the [`MATCH()`](fulltext-search.html#function_match) function must be the
same columns included in some `FULLTEXT` index
in your table. For the preceding query, note that the columns
named in the [`MATCH()`](fulltext-search.html#function_match) function
...
[Section 14.9.2, “Boolean Full-Text Searches”](fulltext-boolean.html "14.9.2 Boolean Full-Text Searches"), and
[Section 14.9.3, “Full-Text Searches with Query Expansion”](fulltext-query-expansion.html "14.9.3 Full-Text Searches with Query Expansion").

A full-text search that uses an index can name columns only from
a single table in the [`MATCH()`](fulltext-search.html#function_match)
...
use the [`MATCH()`](fulltext-search.html#function_match) function where
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statement includes neither
`WHERE` nor `ORDER BY`
clauses:
...
[`MATCH()`](fulltext-search.html#function_match) twice: once in the
[`SELECT`](select.html "15.2.13 SELECT Statement") list and once in the
`WHERE` clause. This causes no additional
overhead, because the MySQL optimizer notices that the two
[`MATCH()`](fulltext-search.html#function_match) calls are identical and
...
full-text parser. For details, see [The MySQL Plugin API](/doc/extending-mysql/9.5/en/plugin-api.html).
...
* Any word that is too short is ignored. The default minimum
  ...
  indexes, or [`ft_min_word_len`](server-system-variables.html#sysvar_ft_min_word_len)
  for `MyISAM`.
  ...
  the [`ngram_token_size`](server-system-variables.html#sysvar_ngram_token_size)
  option.
...
* Words in the stopword list are ignored. A stopword is a word
  ...
  [`ft_stopword_file`](server-system-variables.html#sysvar_ft_stopword_file) for
  `MyISAM` ones.

See [Section 14.9.4, “Full-Text Stopwords”](fulltext-stopwords.html "14.9.4 Full-Text Stopwords") to view default
...
mode explained in [Section 14.9.2, “Boolean Full-Text Searches”](fulltext-boolean.html "14.9.2 Boolean Full-Text Searches").