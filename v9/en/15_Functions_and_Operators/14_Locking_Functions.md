...
* [`GET_LOCK(str,timeout)`](locking-functions.html#function_get-lock)
  ...
  *`timeout`* value means infinite
  ...
  [**mysqladmin kill**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program")).
  ...
  [`GET_LOCK()`](locking-functions.html#function_get-lock) is implemented using
  the metadata locking (MDL) subsystem. Multiple simultaneous
  locks can be acquired and
  [`GET_LOCK()`](locking-functions.html#function_get-lock) does not release any
  ...
  The second [`GET_LOCK()`](locking-functions.html#function_get-lock) acquires
  a second lock and both
  [`RELEASE_LOCK()`](locking-functions.html#function_release-lock) calls return 1
  (success).
  ...
  [`GET_LOCK()`](locking-functions.html#function_get-lock) appear in the
  Performance Schema [`metadata_locks`](performance-schema-metadata-locks-table.html "29.12.13.3 The metadata_locks Table")
  ...
  *same* name, only the first lock for the
  ...
  [`metadata_locks`](performance-schema-metadata-locks-table.html "29.12.13.3 The metadata_locks Table") table. Subsequent
  locks for the name increment a counter in the lock but do not
  acquire additional metadata locks. The
  [`metadata_locks`](performance-schema-metadata-locks-table.html "29.12.13.3 The metadata_locks Table") row for the lock
  ...
  request with an
  [`ER_USER_LOCK_DEADLOCK`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_user_lock_deadlock) error.
  ...
  *`app_name.str`*.
  ...
  a single [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"), it is not suitable for use
  ...
  functions. For details, see [Section 7.6.8.1, “The Locking Service”](locking-service.html "7.6.8.1 The Locking Service").
...
* [`RELEASE_LOCK(str)`](locking-functions.html#function_release-lock)
  ...
  *`str`* that was obtained with
  ...
  The [`DO`](do.html "15.2.3 DO Statement") statement is convenient
