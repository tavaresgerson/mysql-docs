...
* [`ST_Distance(g1,
  ...
  *`unit`* argument if that is
  ...
  + If an intermediate or final result produces
    ...
    [`ER_GIS_INVALID_DATA`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_gis_invalid_data)
    error occurs.

  [`ST_Distance()`](spatial-relation-functions-object-shapes.html#function_st-distance) permits
  specifying the linear unit for the returned distance value
  with an optional *`unit`* argument
  which [`ST_Distance()`](spatial-relation-functions-object-shapes.html#function_st-distance) handles
