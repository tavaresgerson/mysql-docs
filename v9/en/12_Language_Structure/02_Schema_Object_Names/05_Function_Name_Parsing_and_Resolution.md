...
* [Built-In Function Name Parsing](function-resolution.html#function-name-parsing "Built-In Function Name Parsing")
...
[`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space) SQL mode.
...
9.5 that are affected by the
[`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space) setting and
...
* With [`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space)
...
* With [`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space)
  ...
  However, enabling
  [`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space) also has
  the side effect that the parser treats the affected
  function names as reserved words (see
  [Section 11.3, “Keywords and Reserved Words”](keywords.html "11.3 Keywords and Reserved Words")). This means that a space
...
To enable the [`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space)
...
[`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space) is also enabled
by certain other composite modes such as
[`ANSI`](sql-mode.html#sqlmode_ansi) that include it in
their value:
...
Check [Section 7.1.11, “Server SQL Modes”](sql-mode.html "7.1.11 Server SQL Modes"), to see which composite modes
enable [`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space).

To minimize the dependency of SQL code on the
[`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space) setting, use
these guidelines:
...
* Avoid using function names in nonexpression context. For
  ...
  [`IGNORE_SPACE`](sql-mode.html#sqlmode_ignore_space)), so they
...
* Built-in functions and loadable functions
  ...
  `IF NOT EXISTS` has no effect in such
  cases. See [Section 15.7.4.1, “CREATE FUNCTION Statement for Loadable Functions”](create-function-loadable.html "15.7.4.1 CREATE FUNCTION Statement for Loadable Functions"), for
  more information.
...
* Built-in functions and stored functions
  ...
  the server resolves [`PI()`](mathematical-functions.html#function_pi)
  ...
  `IF NOT EXISTS` has no effect in such
  cases; see [Section 15.1.21, “CREATE PROCEDURE and CREATE FUNCTION Statements”](create-procedure.html "15.1.21 CREATE PROCEDURE and CREATE FUNCTION Statements").
...
* Loadable functions and stored functions
  ...
  MySQL 9.5 supports `IF NOT
  EXISTS` with [`CREATE
  FUNCTION`](create-function.html "15.1.16 CREATE FUNCTION Statement") statements, but it has no effect in
  such cases.
...
* If you have already created a loadable function with a
  ...
  [`DROP FUNCTION`](drop-function.html "15.1.30 DROP FUNCTION Statement") to drop the
  loadable function and [`CREATE
  FUNCTION`](create-function.html "15.1.16 CREATE FUNCTION Statement") to re-create the loadable function with
