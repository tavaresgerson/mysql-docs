...
The version number uses the format *`Mmmrr`*, where *`M`* is a major version, *`mm`* is a two-digit minor version, and *`rr`* is a two-digit release number. For example: In a statement to be run only by a MySQL server version 9.5.0 or later, use `90500` in the comment.

In MySQL 9.5, the version number can also optionally be comprised of six digits in *`MMmmrr`* format, where *`MM`* is a two-digit major version, and *`mm`* and *`rr`* are the two-digit minor version and two-digit release numbers, respectively.
