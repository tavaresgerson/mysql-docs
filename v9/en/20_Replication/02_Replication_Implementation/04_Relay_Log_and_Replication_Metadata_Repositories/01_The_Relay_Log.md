...
be read using [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") (see
[Section 6.6.9, “mysqlbinlog — Utility for Processing Binary Log Files”](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files")). If binary log transaction
...
using the [`relay_log`](replication-options-replica.html#sysvar_relay_log) and
[`relay_log_index`](replication-options-replica.html#sysvar_relay_log_index) system
...
* When the logs are flushed (for example, with
  ...
  [**mysqladmin flush-logs**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program")).
...
* When the size of the current relay log file becomes too
  ...
  + If the value of
    [`max_relay_log_size`](replication-options-replica.html#sysvar_max_relay_log_size) is
    0, [`max_binlog_size`](replication-options-binary-log.html#sysvar_max_binlog_size)
...
doing so. However, [`FLUSH LOGS`](flush.html#flush-logs)
