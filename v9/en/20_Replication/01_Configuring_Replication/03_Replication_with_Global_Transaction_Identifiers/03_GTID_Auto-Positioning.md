...
enable the `SOURCE_AUTO_POSITION` option in the
[`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
statement. The alternative `SOURCE_LOG_FILE` and
`SOURCE_LOG_POS` options specify the name of the
...
The `SOURCE_AUTO_POSITION` option is disabled by
...
`GTID_ONLY=ON`, some positions may be marked as
invalid, in which case you must also specify both
`SOURCE_LOG_FILE` and
`SOURCE_LOG_POS` when disabling
...
`SOURCE_AUTO_POSITION` option enabled,
auto-positioning is activated for connection to the source. The
source must have [`GTID_MODE=ON`](replication-options-gtids.html#sysvar_gtid_mode) set
...
[`replication_connection_status`](performance-schema-replication-connection-status-table.html "29.12.11.13 The replication_connection_status Table") table
...
of GTIDs in the [`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged)
...
[`ER_FOUND_MISSING_GTIDS`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_found_missing_gtids). The
...
`SOURCE_AUTO_POSITION` option enabled only
...
[`ER_FOUND_MISSING_GTIDS`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_found_missing_gtids) message
...
([`binlog_expire_logs_seconds`](replication-options-binary-log.html#sysvar_binlog_expire_logs_seconds)) on
...
[`sync_binlog=1`](replication-options-binary-log.html#sysvar_sync_binlog) set experiences a
...
to the [`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed) set by the
