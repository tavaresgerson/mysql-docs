...
[Section 19.1.4, “Changing GTID Mode on Online Servers”](replication-mode-change-online.html "19.1.4 Changing GTID Mode on Online Servers"), use that
...
series, so MySQL 9.4 is the earliest supported
source for a MySQL 9.5 replica.

You can enable GTID assignment on a replication channel using the
`ASSIGN_GTIDS_TO_ANONYMOUS_TRANSACTIONS` option
of the [`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
...
[`gtid_mode=ON`](replication-options-gtids.html#sysvar_gtid_mode) set, and this cannot
be changed afterwards, unless you set
`ASSIGN_GTIDS_TO_ANONYMOUS_TRANSACTIONS=OFF`. If
the replica server is started without GTIDs enabled and with
`ASSIGN_GTIDS_TO_ANONYMOUS_TRANSACTIONS` set for
...
([`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed)) from a replica
set up with
`ASSIGN_GTIDS_TO_ANONYMOUS_TRANSACTIONS` should
not be transferred to another server or compared with another
server's [`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed) set. The
...
* The `UNTIL SQL_BEFORE_GTIDS` and
  ...
  [`START REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement cannot
...
* The function
  ...
  use the [`SOURCE_POS_WAIT()`](replication-functions-synchronization.html#function_source-pos-wait)
  function.
...
[`RESET REPLICA
ALL`](reset-replica.html "15.4.2.3 RESET REPLICA Statement") statement resets the
`ASSIGN_GTIDS_TO_ANONYMOUS_TRANSACTIONS` setting,
but a [`RESET REPLICA`](reset-replica.html "15.4.2.3 RESET REPLICA Statement") statement does
not.