...
[Chapter 27, *Stored Objects*](stored-objects.html "Chapter 27 Stored Objects")) which you can create using some
...
See [Section 14.18.2, “Functions Used with Global Transaction Identifiers (GTIDs)”](gtid-functions.html "14.18.2 Functions Used with Global Transaction Identifiers (GTIDs)"), more more information about
...
`server2` using [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program"),
then to restore this backup on `server1`.

Provided that [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") was run with
...
* Use `GTID_IS_EQUAL()` to verify that the
  ...
  [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") output, and store the GTID set
...
* Use `GTID_IS_DISJOINT()` to verify that the
  GTID set in the [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") output does not
...
configured with
[`log_replica_updates=ON`](replication-options-binary-log.html#sysvar_log_replica_updates), but it
