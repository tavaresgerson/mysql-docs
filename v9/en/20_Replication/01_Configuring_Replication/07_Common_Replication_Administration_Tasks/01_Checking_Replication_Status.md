...
`SOURCE_HEARTBEAT_PERIOD` setting on the source
(set by [`CHANGE REPLICATION SOURCE
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")) specifies the frequency of the heartbeat, which
defaults to half of the connection timeout interval for the
replica (specified by the system variable
[`replica_net_timeout`](replication-options-replica.html#sysvar_replica_net_timeout). The
...
You can use [`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement")
...
* `Seconds_Behind_Source`: The number of
  ...
  [`replica_net_timeout`](replication-options-replica.html#sysvar_replica_net_timeout) has not
  yet elapsed.
  ...
  execute [`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement")
...
using [`SHOW PROCESSLIST`](show-processlist.html "15.7.7.32 SHOW PROCESSLIST Statement") to examine
...
REPLICAS`](show-replicas.html "15.7.7.37 SHOW REPLICAS Statement") statement on the source shows basic
