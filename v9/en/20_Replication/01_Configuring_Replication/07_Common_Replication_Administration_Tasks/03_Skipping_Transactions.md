...
You can use [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") to retrieve and
display the events that were logged around the time of the
error. For instructions to do this, see
[Section 9.5, “Point-in-Time (Incremental) Recovery”](point-in-time-recovery.html "9.5 Point-in-Time (Incremental) Recovery"). Alternatively, you can
issue [`SHOW RELAYLOG EVENTS`](show-relaylog-events.html "15.7.7.35 SHOW RELAYLOG EVENTS Statement") on the
replica or [`SHOW BINLOG EVENTS`](show-binlog-events.html "15.7.7.3 SHOW BINLOG EVENTS Statement") on
the source.
...
* If you have enabled GTID assignment on a replication channel
  ...
  TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement, see
  [Section 19.1.7.3.2, “Skipping Transactions Without GTIDs”](replication-administration-skip.html#replication-administration-skip-nogtid "19.1.7.3.2 Skipping Transactions Without GTIDs").
  Using
...
[`START REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement"), with the
...
[`SHOW RELAYLOG EVENTS`](show-relaylog-events.html "15.7.7.35 SHOW RELAYLOG EVENTS Statement") on the
replica or [`SHOW BINLOG
EVENTS`](show-binlog-events.html "15.7.7.3 SHOW BINLOG EVENTS Statement") on the source, and search the output for a
...
REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement to restart replication, the
...
being phased in ([`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) is
`OFF`, `OFF_PERMISSIVE`, or
`ON_PERMISSIVE`), you can skip a specified
number of events by issuing `SET GLOBAL
sql_replica_skip_counter`. Alternatively, you can
...
While it is possible for a single transaction to contain
changes to both transactional and nontransactional tables,
support for such transactions is deprecated and you should
expect it to be removed in a future version of MySQL. See
[Section 19.5.1.36, “Replication and Transactions”](replication-features-transactions.html "19.5.1.36 Replication and Transactions"). For some
of the reasons for this deprecation, see
...
sql_replica_skip_counter` statement to skip events
...
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement does not have this function, so you
...
sql_replica_skip_counter`, and instead you can just
...
[`LAST_INSERT_ID()`](information-functions.html#function_last-insert-id) count as two
...
[`SET
GLOBAL sql_replica_skip_counter`](set-variable.html#set-variable-system-variables "System Variable Assignment"), the replica
...
[`gtid_mode=ON`](replication-options-gtids.html#sysvar_gtid_mode) is set, or if
...
[`START REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement for
...
[`START REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement also
automatically sets the value of the system variable back to

0. If the replica is a multi-source replica, when you issue
that [`START REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement,
...
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement as follows, where
...
This statement cannot be issued if
`SOURCE_AUTO_POSITION` is
`1`, or if the replication I/O (receiver)
and SQL (applier) threads are running. If you need to use
this method of skipping a transaction when
`SOURCE_AUTO_POSITION=1`, you can change
