...
<td>File name</td>
  ...
  In MySQL 9.5, binary logging is enabled by
  ...
  The [`--log-replica-updates`](replication-options-binary-log.html#sysvar_log_replica_updates) and
  [`--replica-preserve-commit-order`](replication-options-replica.html#sysvar_replica_preserve_commit_order)
  options require binary logging. If you disable binary
  logging, either omit these options, or specify
  `--log-replica-updates=OFF` and
  `--skip-replica-preserve-commit-order`. MySQL
  disables these options by default when
  [`--skip-log-bin`](replication-options-binary-log.html#option_mysqld_log-bin)
  or
  [`--disable-log-bin`](replication-options-binary-log.html#option_mysqld_log-bin)
  is specified. If you specify
  `--log-replica-updates` or
  `--replica-preserve-commit-order` together
  with `--skip-log-bin` or
  `--disable-log-bin`, a warning or error
  message is issued.
  ...
  setting the [`server_id`](replication-options.html#sysvar_server_id)
...
<td>String</td>
  ...
  TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") and [`ALTER
  TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement") are always logged as statements, without
...
<td><p class="valid-value"><code class="literal">NONE</code></p><p class="valid-value"><code class="literal">CRC32</code></p></td>
  ...
  Group Replication in MySQL 9.5 supports
...
<td>File name</td>
  ...
  instructions to do this, see [Section 8.4.5, “The MySQL Keyring”](keyring.html "8.4.5 The MySQL Keyring"). Any
...
<td>File name</td>
  ...
  the replicas. It is important to take into account both
  source and replica throughput when you are setting
  [`binlog_group_commit_sync_delay`](replication-options-binary-log.html#sysvar_binlog_group_commit_sync_delay).
...
<td>File name</td>
  ...
  [`replica_preserve_commit_order=1`](replication-options-replica.html#sysvar_replica_preserve_commit_order)
  on the replica.
...
<td>File name</td>
  ...
  [`SET`](set.html "13.3.7 The SET Type") statement.
...
<td>File name</td>
  ...
  If [`binlog_row_image`](replication-options-binary-log.html#sysvar_binlog_row_image) is
  set to `full` on the source and
  `minimal` on the replica, the replica's
  binary log event contains the full row after-image, even
  if only one column value changes.
...
<td>File name</td>
  ...
  server. When a MySQL 9.5 server instance has no
  ...
  [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5"). In addition, setting
...
<td>String</td>
  ...
  `--log-bin` option. Binary logging is enabled
  by default, with the `log_bin` system
  variable set to `ON`, whether or not you
  specify the `--log-bin` option. The exception
  is if you use [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") to initialize the
...
<td>String</td>
  ...
  See [Section 27.9, “Stored Program Binary Logging”](stored-programs-logging.html "27.9 Stored Program Binary Logging").
