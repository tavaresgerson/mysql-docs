...
2. Execute a [`CHANGE REPLICATION SOURCE
   TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement on the replica to set the source
   configuration. See
...
variable (default `ON`), so events that are
...
1. If you used MySQL Server's clone plugin to create a clone
   ...
   [Section 7.6.6.7, “Cloning for Replication”](clone-plugin-replication.html "7.6.6.7 Cloning for Replication")), the data is
...
2. Configure the replica with the replication coordinates
   ...
   SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement required, see
   [Section 19.1.2.7, “Setting the Source Configuration on the Replica”](replication-howto-slaveinit.html "19.1.2.7 Setting the Source Configuration on the Replica").

3. Start the replication threads by issuing a
   [`START REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement.
