...
* If the source has databases with a character set different
  ...
  TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statements so that they do not implicitly
  rely on the database default character set. A good
  workaround is to state the character set and collation
  explicitly in [`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement")
  statements.