...
On a multi-threaded replica, ensure that the system variable
...
`replica_pending_jobs_size_max`, 128M, is twice
the default setting for `max_allowed_packet`,
which is 64M. `max_allowed_packet` limits the
...
`max_allowed_packet` size, because the value of
`max_allowed_packet` only limits each column of
the table.
