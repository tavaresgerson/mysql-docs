...
[`sql_mode`](server-system-variables.html#sysvar_sql_mode) is also replicated
except for the
[`NO_DIR_IN_CREATE`](sql-mode.html#sqlmode_no_dir_in_create) mode; the
replica always preserves its own value for
[`NO_DIR_IN_CREATE`](sql-mode.html#sqlmode_no_dir_in_create), regardless
of changes to it on the source. This is true for all replication
formats.

However, when [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") parses a
`SET @@sql_mode =
mode` statement, the full
*`mode`* value, including
[`NO_DIR_IN_CREATE`](sql-mode.html#sqlmode_no_dir_in_create), is passed to
...
locking, and the [`SET PASSWORD`](set-password.html "15.7.1.10 SET PASSWORD Statement")
...
[`INSERT`](insert.html "15.2.7 INSERT Statement") statements on a
[`MEMORY`](memory-storage-engine.html "18.3 The MEMORY Storage Engine") table on the source that is
