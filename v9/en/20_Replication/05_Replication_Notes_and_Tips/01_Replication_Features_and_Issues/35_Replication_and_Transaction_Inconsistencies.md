...
5. [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") when using a
   multithreaded replica. After issuing
   [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement"), the replica
   ...
   [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") completes,
   either an error happens, or another thread issues
   [`KILL`](kill.html "15.7.8.4 KILL Statement"), or the server restarts.
   In these cases, [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement")
...
6. If the last transaction in the relay log is only
   ...
   then [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") waits up to
...
7. [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") when the ongoing
   transaction updates transactional tables only, in which case
   it is rolled back and [`STOP
   REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") stops immediately. If the ongoing
   transaction is mixed, [`STOP
   REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") waits up to 60 seconds for the transaction
...
[`rpl_stop_replica_timeout`](replication-options-replica.html#sysvar_rpl_stop_replica_timeout) is
unrelated to the process of stopping the replication threads. It
only makes the client that issues [`STOP
REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") return to the client, but the replication
...
2. The field `Exec_master_log_pos` in
   [`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement") is only a
...
3. [`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
...
5. If [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") is used with
   [`--dump-replica`](mysqldump.html#option_mysqldump_dump-replica), it does
   ...
   with `RELAY_LOG_POS` set to the
   “low-water mark” position in
   `Exec_master_log_pos`.
...
[`START REPLICA
[SQL_THREAD]`](start-replica.html "15.4.2.4 START REPLICA Statement") always consults this information so that
it applies only the correct transactions. This remains true even
if `START REPLICA` is used with
`UNTIL`. `START REPLICA UNTIL
SQL_AFTER_MTS_GAPS` applies only as many transactions
as needed in order to fill in the gaps. If `START
REPLICA` is used with `UNTIL` clauses
that tell it to stop before it has consumed all the gaps, then
it leaves remaining gaps.

Warning

[`RESET REPLICA`](reset-replica.html "15.4.2.3 RESET REPLICA Statement") removes the relay
logs and resets the replication position. Thus issuing
[`RESET REPLICA`](reset-replica.html "15.4.2.3 RESET REPLICA Statement") on a
...
REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement, the old relay logs
...
[`START REPLICA
UNTIL SQL_AFTER_MTS_GAPS`](start-replica.html "15.4.2.4 START REPLICA Statement") statement behaves
