...
* Verify that the source has binary logging enabled by issuing a
  ...
  statement. Binary logging is enabled by default. If binary
  logging is enabled, `Position` is nonzero. If
  binary logging is not enabled, verify that you are not running
  the source with any settings that disable binary logging, such
  as the
  [`--skip-log-bin`](replication-options-binary-log.html#option_mysqld_log-bin)
  option.

* Verify that the [`server_id`](replication-options.html#sysvar_server_id)
...
* Verify that the replica is running. Use
  [`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement") to check
  ...
  [`--skip-replica-start`](replication-options-replica.html#sysvar_skip_replica_start) prevents
  the replication threads from starting until you issue a
...
* If the replica is running, check whether it established a
  ...
  PROCESSLIST`](show-processlist.html "15.7.7.32 SHOW PROCESSLIST Statement"), find the I/O (receiver) and SQL
  (applier) threads and check their `State`
  column to see what they display. See
  [Section 19.2.3, “Replication Threads”](replication-threads.html "19.2.3 Replication Threads"). If the receiver thread
...
* If the replica was running previously but has stopped, the
  ...
  in [Section 19.5.1, “Replication Features and Issues”](replication-features.html "19.5.1 Replication Features and Issues"). If it is a bug, see
  [Section 19.5.5, “How to Report Replication Bugs or Problems”](replication-bugs.html "19.5.5 How to Report Replication Bugs or Problems"), for instructions on how to
  report it.
...
* If a statement that succeeded on the source refuses to run on
  ...
  3. If you decide that the replica can skip the next statement
     ...
     `AUTO_INCREMENT` or
     [`LAST_INSERT_ID()`](information-functions.html#function_last-insert-id) is that
