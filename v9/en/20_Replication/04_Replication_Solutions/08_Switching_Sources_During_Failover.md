...
[`--log-replica-updates=OFF`](replication-options-binary-log.html#sysvar_log_replica_updates) (logging
...
[`--log-replica-updates=OFF`](replication-options-binary-log.html#sysvar_log_replica_updates). Because
updates received by a replica from the source are not written to
the binary log when `--log-replica-updates=OFF` is
...
[`--log-replica-updates=OFF`](replication-options-binary-log.html#sysvar_log_replica_updates) is to
prevent replicas from receiving updates twice in case you cause
one of the replicas to become the new source. If `Replica
1` has `--log-replica-updates` enabled,
which is the default, it writes any updates that it receives from
`Source` in its own binary log. This means that,
when `Replica 2` changes from
...
[`SHOW PROCESSLIST`](show-processlist.html "15.7.7.32 SHOW PROCESSLIST Statement") until you see
...
source, issue [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") and
...
`Replica 3`, use [`STOP
REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement") and `CHANGE REPLICATION SOURCE TO
SOURCE_HOST='Replica1'` (where
`'Replica1'` represents the real host name of
`Replica 1`). To use [`CHANGE
REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement"), add all information about how to
...
REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") on `Replica 2` and
`Replica 3`.
...
[`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
statement as that issued on `Replica 2` and
`Replica 3` previously. `Source`
then becomes a replica of `Replica 1` and picks
up the `Web Client` writes that it missed while
it was offline.
...
[`RESET BINARY LOGS AND GTIDS`](reset-binary-logs-and-gtids.html "15.4.1.2 RESET BINARY LOGS AND GTIDS Statement") on
