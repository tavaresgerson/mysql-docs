...
plugins, see [Section 7.6.1, “Installing and Uninstalling Plugins”](plugin-loading.html "7.6.1 Installing and Uninstalling Plugins").
...
* The capability of installing plugins requires a MySQL server
  ...
  the value of the
  [`have_dynamic_loading`](server-system-variables.html#sysvar_have_dynamic_loading) system
  variable is `YES`. Binary distributions
  should support dynamic loading.

* Replication must already be working, see
  [Section 19.1, “Configuring Replication”](replication-configuration.html "19.1 Configuring Replication").
...
MySQL 9.5 supplies versions of the plugins that
implement semisynchronous replication—one for the source
server and one for the replica—which replace the terms
“master” and “slave” with
“source” and “replica” in system
variables and status variables; you must install these versions
instead of the old ones (which have now been removed).
Replication topologies that still use the old semisynchronous
plugins must be upgraded to use the new plugins. For more
information, see
[Upgrading Replication Topologies to Use New Semisynchronous Replication Plugins](replication-semisync-installation.html#replication-semisync-install-new-plugins "Upgrading Replication Topologies to Use New Semisynchronous Replication Plugins").
...
* Source server: `rpl_semi_sync_source`
  plugin (`semisync_source.so` or
  `semisync_source.dll` library)

* Replica: `rpl_semi_sync_replica` plugin
...
instructions. The [`INSTALL PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement"),
[`SET
GLOBAL`](set-variable.html "15.7.6.1 SET Syntax for Variable Assignment"), [`STOP REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement"),
...
PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement") statement on the source and on each replica
...
[`SET
GLOBAL`](set-variable.html "15.7.6.1 SET Syntax for Variable Assignment"), or at server startup on the command line or in
...
servers as follows. On the source:
...
In replication topologies that still use the old
semisynchronous plugins, the MySQL instances must first be
reconfigured to use the new semisynchronous replication
plugins. To reconfigure the MySQL instances:

1. Stop replication on each replica and source instances.
2. Uninstall the old `rpl_semi_sync_master`
   and `rpl_semi_sync_slave` plugins.

3. Install the new `rpl_semi_sync_source`
   and `rpl_semi_sync_source` plugins.

4. Enable semisynchronous replication again.

The replication topology upgrade must then be performed in the
standard manner, starting with replicas and then proceeding to
