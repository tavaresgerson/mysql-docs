...
in a [`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement")
statement for this channel. GTID auto-positioning must be in use
for the channel (`SOURCE_AUTO_POSITION = 1`).

Important

When the existing connection to a source fails, the replica
first retries the same connection the number of times
specified by the `SOURCE_RETRY_COUNT` option
of the [`CHANGE REPLICATION SOURCE
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement"). The interval between attempts is set by the
`SOURCE_CONNECT_RETRY` option. When these
...
`SOURCE_CONNECT_RETRY` to minimal numbers
...
`SOURCE_CONNECT_RETRY=10`, which make the
...
the
[`SOURCE_CONNECTION_AUTO_FAILOVER`](change-replication-source-to.html#crs-opt-source_connection_auto_failover)
option of the [`CHANGE REPLICATION SOURCE
TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement") statement, and the source list, are transferred to
