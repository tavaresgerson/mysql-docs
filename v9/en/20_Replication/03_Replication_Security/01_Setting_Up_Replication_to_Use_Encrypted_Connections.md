...
* To name the replica's certificate and SSL private key
  ...
  TO`](change-replication-source-to.html "15.4.2.2 CHANGE REPLICATION SOURCE TO Statement"), add the appropriate
  `SOURCE_SSL_xxx`
  options, like this:
...
* To activate host name identity verification, add the
  ...
  `SOURCE_SSL_VERIFY_SERVER_CERT=1` corresponds
...
* To specify lists of ciphers, ciphersuites, and encryption
  ...
  You can use the `SOURCE_TLS_CIPHERSUITES`
...
* After the source information has been updated, start the
  ...
  STATUS`](show-replica-status.html "15.7.7.36 SHOW REPLICA STATUS Statement") statement to confirm that an encrypted
