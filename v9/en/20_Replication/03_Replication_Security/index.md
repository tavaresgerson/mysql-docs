...
[Chapter 8, *Security*](security.html "Chapter 8 Security"). In addition, for servers in a
...
* Set up sources and replicas to use encrypted connections to
  ...
  statement, in addition to setting up the servers to support
  encrypted network connections. See
...
* Encrypt the binary log files and relay log files on sources and
  ...
  using the [`binlog_encryption`](replication-options-binary-log.html#sysvar_binlog_encryption)
  system variable. See
