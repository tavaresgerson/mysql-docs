...
[Section 26.2, “Partitioning Types”](partitioning-types.html "26.2 Partitioning Types"), for more information).

The [`DIV`](arithmetic-functions.html#operator_div) operator is also supported;
the [`/`](arithmetic-functions.html#operator_divide) operator is
not permitted.
...
[Section 7.1.11, “Server SQL Modes”](sql-mode.html "7.1.11 Server SQL Modes")), the results of many MySQL functions
...
[`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statement, which can
be executed successfully only if the
[`NO_UNSIGNED_SUBTRACTION`](sql-mode.html#sqlmode_no_unsigned_subtraction) mode is
in effect:
...
If you remove the
[`NO_UNSIGNED_SUBTRACTION`](sql-mode.html#sqlmode_no_unsigned_subtraction) server
...
See also [Section 7.1.11, “Server SQL Modes”](sql-mode.html "7.1.11 Server SQL Modes").
...
* **File system operations.**
  ...
  [`ALTER
  TABLE`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations") with `PARTITION BY ...`,
...
* **Table locks.**
  ...
  [`UPDATE`](update.html "15.2.17 UPDATE Statement") operations are
...
* **Indexes; partition pruning.**
  ...
  [Section 26.4, “Partition Pruning”](partitioning-pruning.html "26.4 Partition Pruning"), for more
  information.

  Index condition pushdown is supported for partitioned tables.
  See [Section 10.2.1.6, “Index Condition Pushdown Optimization”](index-condition-pushdown-optimization.html "10.2.1.6 Index Condition Pushdown Optimization").

* **Performance with LOAD DATA.**
  In MySQL 9.5, [`LOAD
  DATA`](load-data.html "15.2.9 LOAD DATA Statement") uses buffering to improve performance. You
...
using the [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") storage engine is
...
Partitioned tables using the [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine")
...
[`CREATE
TABLE`](create-table-foreign-keys.html "15.1.24.5 FOREIGN KEY Constraints") and [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement")
...
Once you perform
[`ALTER TABLE ... ADD
COLUMN ... ALGORITHM=INSTANT`](alter-table.html "15.1.11 ALTER TABLE Statement") on a partitioned table,
...
application uses [`REPLACE`](replace.html "15.2.12 REPLACE Statement")
statements and you do this, the results of these statements can
be drastically altered. See [Section 15.2.12, “REPLACE Statement”](replace.html "15.2.12 REPLACE Statement"), for more
...
[`ALTER
TABLE ... PARTITION BY ...`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations") statement on such a table
fails with an error.
...
[`ENUM`](enum.html "13.3.6 The ENUM Type") columns cannot be used. The
column or expression value may also be `NULL`;
see [Section 26.2.7, “How MySQL Partitioning Handles NULL”](partitioning-handling-nulls.html "26.2.7 How MySQL Partitioning Handles NULL").
...
1. When partitioning by [`LINEAR`]
   ...
   [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") as partitioning keys,
   because the internal key-hashing functions produce the correct
   data type from these types. For example, the following two
   [`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statements are
   valid:
...
2. When partitioning by `RANGE COLUMNS` or
   ...
   [`DATETIME`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types") columns. For example,
   each of the following [`CREATE
   TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statements is valid:
...
[`TEXT`](blob.html "13.3.4 The BLOB and TEXT Types") column types.
...
following [`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statement,
which has three [`VARCHAR`](char.html "13.3.2 The CHAR and VARCHAR Types") columns,
...
subpartitions of [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") tables. The
directory specified in a `DATA DIRECTORY`
clause must be known to [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine"). For
...
The statements [`CHECK TABLE`](check-table.html "15.7.3.2 CHECK TABLE Statement"),
...
[`REPAIR TABLE`](repair-table.html "15.7.3.5 REPAIR TABLE Statement") are supported for
partitioned tables.
...
[Section 15.1.11, “ALTER TABLE Statement”](alter-table.html "15.1.11 ALTER TABLE Statement"), for more information about these
two statements.
