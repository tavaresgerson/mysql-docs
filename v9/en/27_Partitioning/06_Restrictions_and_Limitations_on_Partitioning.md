...
* **Performance with LOAD DATA.** In MySQL 9.5, [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") uses buffering to improve performance. You should be aware that the buffer uses 130 KB memory per partition to achieve this.
...
The scope of the restrictions just listed includes all tables that use the `InnoDB` storage engine. [`CREATE TABLE`](create-table-foreign-keys.html "15.1.24.5 FOREIGN KEY Constraints") and `ALTER TABLE` statements that would result in tables violating these restrictions are not allowed.
...
**ADD COLUMN ... ALGORITHM=INSTANT.** Once you perform [`ALTER TABLE ... ADD COLUMN ... ALGORITHM=INSTANT`](alter-table.html "15.1.11 ALTER TABLE Statement") on a partitioned table, it is no longer possible to exchange partitions with this table.
...
**Log tables.** It is not possible to partition the log tables; an [`ALTER TABLE ... PARTITION BY ...`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations") statement on such a table fails with an error.
...
2. When partitioning by `RANGE COLUMNS` or `LIST COLUMNS`, it is possible to use string, `DATE`, and `DATETIME` columns. For example, each of the following [`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statements is valid:
...
In addition, you can use `ALTER TABLE ... REBUILD PARTITION` to rebuild one or more partitions of a partitioned table; `ALTER TABLE ... REORGANIZE PARTITION` also causes partitions to be rebuilt. See Section 15.1.11, “ALTER TABLE Statement”, for more information about these two statements.

`ANALYZE`, `CHECK`, `OPTIMIZE`, `REPAIR`, and `TRUNCATE` operations are supported with subpartitions. See Section 15.1.11.1, “ALTER TABLE Partition Operations”.
...
It is possible to add a primary key to `t_no_pk` using either of these [`ALTER TABLE`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations") statements:
...
These rules also apply to existing nonpartitioned tables that you wish to partition using [`ALTER TABLE ... PARTITION BY`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations"). Consider a table `np_pk` created as shown here:
...
The following [`ALTER TABLE`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations") statement fails with an error, because the `added` column is not part of any unique key in the table:
...
[`ALTER TABLE ... OPTIMIZE PARTITION`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations") does not work correctly with partitioned tables that use `InnoDB`. Use `ALTER TABLE ... REBUILD PARTITION` and `ALTER TABLE ... ANALYZE PARTITION`, instead, for such tables. For more information, see Section 15.1.11.1, “ALTER TABLE Partition Operations”.
...
`CREATE TABLE` and [`ALTER TABLE`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations") statements that would cause a user-partitioned `NDB` table not to meet either or both of the following two requirements are not permitted, and fail with an error:
