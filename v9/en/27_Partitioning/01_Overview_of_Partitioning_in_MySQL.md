...
limitations, see [Section 26.6, “Restrictions and Limitations on Partitioning”](partitioning-limitations.html "26.6 Restrictions and Limitations on Partitioning").
...
notion of a tablespace (see [Section 17.6.3, “Tablespaces”](innodb-tablespace.html "17.6.3 Tablespaces")),
...
[Section 10.12.2, “Using Symbolic Links”](symbolic-links.html "10.12.2 Using Symbolic Links"), for an explanation of how this
is done).
...
partitioning functions, see [Section 26.2, “Partitioning Types”](partitioning-types.html "26.2 Partitioning Types"),
as well as [Section 15.1.24, “CREATE TABLE Statement”](create-table.html "15.1.24 CREATE TABLE Statement"), which provides
...
that supports them. In MySQL 9.5, all partitions of
...
In MySQL 9.5, the only storage engines that support
partitioning are `InnoDB` and
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5"). Partitioning cannot be used with
...
KEY` is possible with [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5"),
but other types of user-defined partitioning are not supported for
tables using this storage engine. In addition, an
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table that employs user-defined
...
TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") or
[`ALTER
TABLE`](alter-table-partition-operations.html "15.1.11.1 ALTER TABLE Partition Operations") statement used to create or modify a
user-partitioned [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") table, then the
...
TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statement. This example shows how to create a
table that is partitioned by hash into 6 partitions and which uses
the `InnoDB` storage engine (regardless of the
value of [`default_storage_engine`](server-system-variables.html#sysvar_default_storage_engine)):
...
9.5 this has no effect.

Unless otherwise specified, the remaining examples in this
discussion assume that
[`default_storage_engine`](server-system-variables.html#sysvar_default_storage_engine) is
`InnoDB`.
...
[`CREATE TABLE`](create-table.html "15.1.24 CREATE TABLE Statement") statement used to
create the partitioned table.

Only the `DATA DIRECTORY` option is supported for
individual partitions and subpartitions of
[`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") tables. The directory
specified in a `DATA DIRECTORY` clause must be
...
each partition. See [Section 26.3, “Partition Management”](partitioning-management.html "26.3 Partition Management"), for
...
[`DataMemory`](mysql-cluster-ndbd-definition.html#ndbparam-ndbd-datamemory) data node
configuration parameter, as well as
[Section 25.2.2, “NDB Cluster Nodes, Node Groups, Fragment Replicas, and Partitions”](mysql-cluster-nodes-groups.html "25.2.2 NDB Cluster Nodes, Node Groups, Fragment Replicas, and Partitions"), for more
information.
...
* Some queries can be greatly optimized in virtue of the fact
  ...
  more information, see [Section 26.4, “Partition Pruning”](partitioning-pruning.html "26.4 Partition Pruning").

  In addition, MySQL supports explicit partition selection for
  queries. For example,
  [`SELECT * FROM t
  PARTITION (p0,p1) WHERE c < 5`](select.html "15.2.13 SELECT Statement") selects only those
  ...
  [`LOAD XML`](load-xml.html "15.2.10 LOAD XML Statement"). See the descriptions
