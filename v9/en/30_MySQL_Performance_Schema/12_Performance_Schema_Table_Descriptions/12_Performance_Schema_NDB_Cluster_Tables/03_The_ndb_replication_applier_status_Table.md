...
[`NDBCLUSTER`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 9.5") exposes its internal
...
* `CHANNEL_NAME`:
  ...
  The name of the replication channel. The default is an
  empty string (`""`).
...
* `API_WAIT_EXEC_COMPLETE_COUNT`:
  ...
  The number of times this thread has been blocked while
  waiting for an operation by this SQL node to complete.
...
* `API_WAIT_SCAN_RESULT_COUNT`:
  ...
  The number of times this thread has been blocked while
  waiting for a scan-based signal by this SQL node.
...
* `API_WAIT_META_REQUEST_COUNT`:
  ...
  The number of times this thread has been blocked waiting
  for a metadata-based signal by this SQL node.
...
* `API_WAIT_NANOS_COUNT`:
  ...
  Cumulative time in nanoseconds spent waiting for some type
  of signal from the data nodes by this SQL node.
...
* `API_BYTES_SENT_COUNT`:
  ...
  The number of bytes sent to the data nodes by this SQL
  node.
...
* `API_BYTES_RECEIVED_COUNT`:
  ...
  The number of bytes received from the data nodes by this
  SQL node.
...
* `API_TRANS_CLOSE_COUNT`:
  ...
  The number of transactions which have been closed by this
  SQL node; this value may be greater than the sum of
  `API_TRANS_COMMIT_COUNT` and
  `API_TRANS_ABORT_COUNT`.
...
* `API_PK_OP_COUNT`:
  ...
  The number of primary key operations which have been
  performed by this SQL node.
  [`Ndb_api_pk_op_count_replica`](mysql-cluster-options-variables.html#statvar_Ndb_api_pk_op_count_replica)
  shows this value for the default replication channel.

* `API_UK_OP_COUNT`:

  The number of unique key operations which have been
  performed by this SQL node.
...
* `API_TABLE_SCAN_COUNT`:
  ...
  The number of table scans which have been started by this
  SQL node. This includes scans of internal tables.
...
* `API_RANGE_SCAN_COUNT`:
  ...
  The number of range scans which have been started by this
  SQL node.
...
* `API_PRUNED_SCAN_COUNT`:
  ...
  The number of scans which have been pruned to a single
  partition by this SQL node.
...
* `API_SCAN_BATCH_COUNT`:
  ...
  The number of row batches which have been received by this
  SQL node.
...
* `API_READ_ROW_COUNT`:
  ...
  The total number of rows which have been read by this SQL
  node.
...
* `API_ADAPTIVE_SEND_FORCED_COUNT`:
  ...
  The number of adaptive sends which have been sent by this
  SQL node making use of forced send.
...
* `API_ADAPTIVE_SEND_UNFORCED_COUNT`:
  ...
  The number of adaptive sends which have been sent by this
  SQL node without using forced send.
...
* `API_ADAPTIVE_SEND_DEFERRED_COUNT`:
  ...
  The number of adaptive sends which were not actually sent
  by this SQL node.
...
* `CONFLICT_FN_MAX`:
  ...
  The number of times that NDB replication “greater
  timestamp wins” conflict resolution has been
...
* `CONFLICT_FN_OLD`:
  ...
  The number of times that NDB replication “same
  timestamp wins” conflict resolution has been
  applied.
...
* `CONFLICT_FN_MAX_DEL_WIN`:
  ...
  The number of times that NDB replication conflict
  resolution based on the result of
  `NDB$MAX_DELETE_WIN()` has been applied
  to update and delete operations.
...
* `CONFLICT_FN_MAX_DEL_WIN_INS`:
  ...
  The number of times that NDB replication conflict
  resolution based on the result of
  `NDB$MAX_DELETE_WIN_INS()` has been
  applied to update and delete operations.
...
* `CONFLICT_FN_EPOCH`:
  ...
  The number of rows which have been found in conflict by
  the `NDB$EPOCH()` NDB replication
...
* `CONFLICT_FN_EPOCH_TRANS`:
  ...
  The number of rows which have been found in conflict by
  the `NDB$EPOCH_TRANS()` NDB replication
  conflict detection function.
...
* `CONFLICT_FN_EPOCH2`:
  ...
  The number of rows which have been found in conflict by
  the `NDB$EPOCH2()` NDB replication
  conflict detection function.
...
* `CONFLICT_FN_EPOCH2_TRANS`:
  ...
  The number of rows which have been found in conflict by
  the `NDB$EPOCH2_TRANS()` NDB replication
  conflict detection function.
...
* `CONFLICT_TRANS_ROW_CONFLICT_COUNT`:
  ...
  The number of rows which have been found to be in conflict
  by a transactional conflict function, including any rows
  which were included in or dependent on conflicting
  transactions.
...
* `CONFLICT_TRANS_ROW_REJECT_COUNT`:
  ...
  The total number of rows which have been realigned after
  being found to be in conflict by a transactional conflict
  function, including
  `CONFLICT_TRANS_ROW_CONFLICT_COUNT` as
  well as any rows which were included in or dependent on
...
* `CONFLICT_TRANS_REJECT_COUNT`:
  ...
  The number of transactions which have been rejected after
  being found to be in conflict by a transactional conflict
  function.
...
* `CONFLICT_TRANS_DETECT_ITER_COUNT`:
  ...
  The number of internal iterations which have been required
  to commit epoch transactions. This value should be
  normally be slightly greater than or equal to the value of
...
* `CONFLICT_TRANS_CONFLICT_COMMIT_COUNT`:
  ...
  The number of epoch transactions which have been committed
...
* `CONFLICT_EPOCH_DELETE_DELETE_COUNT`:
  ...
  The number of delete-delete conflicts detected. A
  delete-delete conflict occurs when a delete operation is
  applied, but the row does not exist.
...
* `CONFLICT_REFLECTED_OP_PREPARE_COUNT`:
  ...
  The number of reflected operations which have been
  received and prepared for execution.
...
* `CONFLICT_REFLECTED_OP_DISCARD_COUNT`:
  ...
  The nmber of reflected operations which have not been
  applied due to errors during execution.
...
* `CONFLICT_LAST_CONFLICT_EPOCH`:
  ...
  The most recent NDB epoch on this replica during which a
...
* `CONFLICT_LAST_STABLE_EPOCH`:
  ...
  The most recent NDB epoch during which no conflicts were
  detected.
...
server status variables, as well as
