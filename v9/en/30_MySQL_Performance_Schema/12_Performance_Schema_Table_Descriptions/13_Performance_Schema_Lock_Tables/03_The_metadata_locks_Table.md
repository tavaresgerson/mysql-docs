...
* When a pending lock request is canceled by the deadlock
  ...
  ([`ER_LOCK_DEADLOCK`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_lock_deadlock)), its
  row status is updated from `PENDING` to
  `VICTIM`.

* When a pending lock request times out
  ([`ER_LOCK_WAIT_TIMEOUT`](/doc/mysql-errors/9.5/en/server-error-reference.html#error_er_lock_wait_timeout)),
