...
system variable. A multithreaded replica also has a
coordinator thread to manage the applier threads, and the
status of this thread is shown in the
...
[`START
REPLICA`](start-replica.html "15.4.2.4 START REPLICA Statement") statement, the columns beginning
...
* `WORKER_ID`
  ...
  [`STOP
  REPLICA`](stop-replica.html "15.4.2.5 STOP REPLICA Statement"), the `THREAD_ID` column
