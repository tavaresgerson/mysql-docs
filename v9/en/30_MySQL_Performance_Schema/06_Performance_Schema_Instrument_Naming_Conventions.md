...
Stage instruments have names of the form `stage/code_area/stage_name`, where *`code_area`* is a value such as `sql` or `myisam`, and *`stage_name`* indicates the stage of statement processing, such as `Sorting result` or `Sending data`. Stages correspond to the thread states displayed by [`SHOW PROCESSLIST`](show-processlist.html "15.7.7.32 SHOW PROCESSLIST Statement") or that are visible in the Information Schema `PROCESSLIST` table.
...
* `wait/synch`
  ...
  + `wait/synch/rwlock`
    ...
    A plain [read/write lock](glossary.html#glos_rw_lock "rw-lock") object used to lock a specific variable for access while preventing its use by other threads. A shared read lock can be acquired simultaneously by multiple threads. An exclusive write lock can be acquired by only one thread at a time.
