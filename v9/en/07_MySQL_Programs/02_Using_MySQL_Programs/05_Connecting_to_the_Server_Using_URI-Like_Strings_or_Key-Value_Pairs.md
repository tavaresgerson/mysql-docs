...
such as [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") or [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program"),
see [Section 6.2.4, “Connecting to the MySQL Server Using Command Options”](connecting.html "6.2.4 Connecting to the MySQL Server Using Command Options"). For additional information if
...
* Parameters specified with a URI-like string use a syntax such
  ...
  the full syntax, see [Connecting Using URI-Like Connection Strings](connecting-using-uri-or-key-value-pairs.html#connecting-using-uri "Connecting Using URI-Like Connection Strings").
...
* Parameters specified with key-value pairs use a syntax such as
  ...
  [Connecting Using Key-Value Pairs](connecting-using-uri-or-key-value-pairs.html#connecting-using-key-value-pairs "Connecting Using Key-Value Pairs").
...
* [Base Connection Parameters](connecting-using-uri-or-key-value-pairs.html#connection-parameters-base "Base Connection Parameters")
* [Additional Connection parameters](connecting-using-uri-or-key-value-pairs.html#connection-parameters-additional "Additional Connection parameters")
* [Connecting Using URI-Like Connection Strings](connecting-using-uri-or-key-value-pairs.html#connecting-using-uri "Connecting Using URI-Like Connection Strings")
* [Connecting Using Key-Value Pairs](connecting-using-uri-or-key-value-pairs.html#connecting-using-key-value-pairs "Connecting Using Key-Value Pairs")
...
URI-like syntax (see [Connecting Using URI-Like Connection Strings](connecting-using-uri-or-key-value-pairs.html#connecting-using-uri "Connecting Using URI-Like Connection Strings")), or
...
* *`scheme`*: The transport protocol to
  ...
  scheme (see [Connections Using DNS SRV Records](/doc/x-devapi-userguide/en/connecting-dns-srv.html)).
...
* `ssh`: The URI for connection to an SSH
  ...
  [Using an SSH Tunnel](/doc/mysql-shell/9.5/en/mysql-shell-connection-ssh.html).
...
* `compression`: This option requests or
  ...
  `required`, which requests compression and
  fails if the server does not support it;
  `preferred`, which requests compression and
  falls back to an uncompressed connection; and
  `disabled`, which requests an uncompressed
  connection and fails if the server does not permit those.
  `preferred` is the default for X Protocol
  connections, and `disabled` is the default
...
* `compression-algorithms` and
  ...
  `compression-algorithms` in place of
  `compression` to request compression for
...
[`--uri`](/doc/mysql-shell/9.5/en/mysqlsh.html#option_mysqlsh_uri) command option, the
...
described at [Base Connection Parameters](connecting-using-uri-or-key-value-pairs.html#connection-parameters-base "Base Connection Parameters").
