...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"), use
...
[`persisted_globals_load`](server-system-variables.html#sysvar_persisted_globals_load) system
...
* The `.mylogin.cnf` file that contains
  ...
  [Section 6.6.7, “mysql\_config\_editor — MySQL Configuration Utility”](mysql-config-editor.html "6.6.7 mysql_config_editor — MySQL Configuration Utility"). A “login
  ...
  the [`--login-path`](option-file-options.html#option_general_login-path) option.
  ...
  **mysql-test-run.pl** testing utility, but
  also is recognized by [**mysql\_config\_editor**](mysql-config-editor.html "6.6.7 mysql_config_editor — MySQL Configuration Utility")
  and by MySQL clients such as [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client"),
  [**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program"), and so forth.
...
* The `mysqld-auto.cnf` file in the data
  ...
  [`SET
  PERSIST_ONLY`](set-variable.html "15.7.6.1 SET Syntax for Variable Assignment") statements. See
  [Section 7.1.9.3, “Persisted System Variables”](persisted-system-variables.html "7.1.9.3 Persisted System Variables"). Management of
...
programs, see [Section 25.4, “Configuration of NDB Cluster”](mysql-cluster-configuration.html "25.4 Configuration of NDB Cluster").
...
installation directory. When MySQL 9.5 has been
...
can be changed by [`--datadir`](server-system-variables.html#sysvar_datadir)
...
server using the [**mysqld\_safe**](mysqld-safe.html "6.3.2 mysqld_safe — MySQL Server Startup Script") program,
[**mysqld\_safe**](mysqld-safe.html "6.3.2 mysqld_safe — MySQL Server Startup Script") sets it to
...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"), the *first*
...
[**mysql\_config\_editor**](mysql-config-editor.html "6.6.7 mysql_config_editor — MySQL Configuration Utility") and is encrypted, and
...
[Section 6.2.2.1, “Using Options on the Command Line”](command-line-options.html "6.2.2.1 Using Options on the Command Line")). However, in an option
...
9.5` in an option file. This can be
...
groups apply to the [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") server and the
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client program, respectively.

The `[client]` option group is read by all
client programs provided in MySQL distributions (but
*not* by [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")). To
understand how third-party client programs that use the C API
can use option files, see the C API documentation at
[mysql\_options()](/doc/c-api/9.5/en/mysql-options.html).
...
order `[client]`,
`[mysqldump]` enables
[**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program")-specific options to override
`[client]` options.
...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") servers from specific MySQL release
series, use groups with names of
`[mysqld-9.4]`,
`[mysqld-9.5]`, and so forth.
The following group indicates that the
[`sql_mode`](server-system-variables.html#sysvar_sql_mode) setting should be
used only by MySQL servers with 9.5.x version
numbers:
...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"), only the
`[mysqld]` group in
`/home/mydir/myopt.cnf` is used. If the
file is processed by [**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program"), only the
