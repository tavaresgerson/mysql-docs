...
* [Using CREATE TABLE ... TABLESPACE Syntax](innodb-create-table-external.html#innodb-create-table-external-tablespace-syntax "Using CREATE TABLE ... TABLESPACE Syntax")
* [Creating a Table in an External General Tablespace](innodb-create-table-external.html#innodb-create-table-external-tablespace "Creating a Table in an External General Tablespace")
...
directory using the `DATA DIRECTORY` clause are
restricted to directories known to `InnoDB`.
...
[Tablespace Discovery During Crash Recovery](innodb-recovery.html#innodb-recovery-tablespace-discovery "Tablespace Discovery During Crash Recovery")). Known
...
[`innodb_directories`](innodb-parameters.html#sysvar_innodb_directories) variables.
...
* MySQL initially holds the tablespace data file open,
  ...
  [data dictionary](/doc/refman/8.4/en/glossary.html#glos_data_dictionary).
...
* If using an LVM snapshot, file copy, or other file-based
  ...
  [`FLUSH
  TABLES ... FOR EXPORT`](flush.html#flush-tables-for-export-with-list) statement first to ensure
  that all changes buffered in memory are
  [flushed](glossary.html#glos_flush "flush") to disk before the
  backup occurs.

* Using the `DATA DIRECTORY` clause to create
  a table in an external directory is an alternative to using
  [symbolic links](symbolic-links.html "10.12.2 Using Symbolic Links"), which
...
[`CREATE TABLE ...
TABLESPACE`](create-table.html "15.1.24 CREATE TABLE Statement") syntax can be used in combination with the
