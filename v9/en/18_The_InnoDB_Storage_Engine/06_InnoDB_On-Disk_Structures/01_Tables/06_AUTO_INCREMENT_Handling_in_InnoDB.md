...
* [InnoDB AUTO\_INCREMENT Lock Mode Usage Implications](innodb-auto-increment-handling.html#innodb-auto-increment-lock-mode-usage-implications "InnoDB AUTO_INCREMENT Lock Mode Usage Implications")
...
* [Notes](innodb-auto-increment-handling.html#innodb-auto-increment-notes "Notes")
...
* “[`INSERT`](insert.html "15.2.7 INSERT Statement")-like”
  statements
  ...
  [`REPLACE ...
  SELECT`](replace.html "15.2.12 REPLACE Statement"), and [`LOAD
  DATA`](load-data.html "15.2.9 LOAD DATA Statement"). Includes “simple-inserts”,
...
* “Simple inserts”
  ...
  [`REPLACE`](replace.html "15.2.12 REPLACE Statement") statements that do
...
* “Bulk inserts”
  ...
  [`REPLACE ...
  SELECT`](replace.html "15.2.12 REPLACE Statement"), and [`LOAD
  DATA`](load-data.html "15.2.9 LOAD DATA Statement") statements, but not plain
...
* “Mixed-mode inserts”
  ...
  [`INSERT
  ... ON DUPLICATE KEY UPDATE`](insert-on-duplicate.html "15.2.7.2 INSERT ... ON DUPLICATE KEY UPDATE Statement"), which in the worst
  case is in effect an [`INSERT`](insert.html "15.2.7 INSERT Statement")
  followed by a [`UPDATE`](update.html "15.2.17 UPDATE Statement"), where
...
9.5 reflects the change from statement-based
...
* `innodb_autoinc_lock_mode = 0`
  ...
  sequence of [`INSERT`](insert.html "15.2.7 INSERT Statement")
  ...
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statements is
  deterministic, and the replica reproduces the same data as
  on the source. If auto-increment values generated by
  multiple [`INSERT`](insert.html "15.2.7 INSERT Statement") statements
  were interleaved, the result of two concurrent
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statements would be
  ...
  using an
  [`INSERT ...
  SELECT`](insert-select.html "15.2.7.1 INSERT ... SELECT Statement") statement that inserts 1000 rows, and
  another is using a simple
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statement that inserts
  one row:
  ...
  [`SELECT`](select.html "15.2.13 SELECT Statement") in the
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statement in Tx1, and
  ...
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statement referring to
  ...
  the Tx1
  [`INSERT ...
  SELECT`](insert-select.html "15.2.7.1 INSERT ... SELECT Statement") statement are consecutive, and the (single)
  auto-increment value used by the
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statement in Tx2 is
  ...
  [`INSERT`](insert.html "15.2.7 INSERT Statement") in Tx2 depends on
  precisely when the statement executes. If the
  [`INSERT`](insert.html "15.2.7 INSERT Statement") of Tx2 executes while
  the [`INSERT`](insert.html "15.2.7 INSERT Statement") of Tx1 is running
  (rather than before it starts or after it completes), the
  specific auto-increment values assigned by the two
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statements are
  nondeterministic, and may vary from run to run.

  Under the
  [consecutive](innodb-auto-increment-handling.html#innodb-auto-increment-lock-mode-consecutive)
  ...
  [interleaved](innodb-auto-increment-handling.html#innodb-auto-increment-lock-mode-interleaved)
...
* `innodb_autoinc_lock_mode = 1`
  ...
  [`REPLACE ...
  SELECT`](replace.html "15.2.12 REPLACE Statement"), and [`LOAD
  DATA`](load-data.html "15.2.9 LOAD DATA Statement") statements. Only one statement holding the
  ...
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statements where the
...
* Modifying `AUTO_INCREMENT` column values in
  ...
  auto-increment value, the new value is persisted, and
  subsequent [`INSERT`](insert.html "15.2.7 INSERT Statement") operations
...
The current maximum auto-increment counter value is written to
the redo log each time it changes and saved to the data
dictionary on each checkpoint; this makes the current maximum
...
[`UPDATE`](update.html "15.2.17 UPDATE Statement") operation, the new value
...
[importing a table](innodb-table-import.html "17.6.1.3 Importing InnoDB Tables")
...
[`ALTER TABLE ...
AUTO_INCREMENT = N`](alter-table.html "15.1.11 ALTER TABLE Statement") can only change the
auto-increment counter value to a value larger than the
current maximum.

The current maximum auto-increment value is persisted,
preventing the reuse of previously allocated values.

If a [`SHOW TABLE STATUS`](show-table-status.html "15.7.7.39 SHOW TABLE STATUS Statement") statement
...
The [`auto_increment_offset`](replication-options-source.html#sysvar_auto_increment_offset)
variable determines the starting point for the
`AUTO_INCREMENT` column value. The default
setting is 1.

The [`auto_increment_increment`](replication-options-source.html#sysvar_auto_increment_increment)
