...
when the [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") process dies.
...
the [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") process died may be beneficial
...
[`core_file`](server-system-variables.html#sysvar_core_file) variable is enabled
...
MADV\_DONTDUMP, otherwise it defaults to ON.

To generate core files with buffer pool pages, start the server
with the [`--core-file`](server-options.html#option_mysqld_core-file) and
[`--innodb-buffer-pool-in-core-file=ON`](innodb-parameters.html#sysvar_innodb_buffer_pool_in_core_file)
options.
...
The [`core_file`](server-system-variables.html#sysvar_core_file) variable is
read-only and disabled by default. It is enabled by specifying
the [`--core-file`](server-options.html#option_mysqld_core-file) option at
startup. The
[`innodb_buffer_pool_in_core_file`](innodb-parameters.html#sysvar_innodb_buffer_pool_in_core_file)
...
[`core_file`](server-system-variables.html#sysvar_core_file) variable is disabled
to prevent writing core files that unintentionally include
buffer pool pages. If the read-only
[`core_file`](server-system-variables.html#sysvar_core_file) variable becomes
...
<table summary="Core file configuration and         MADV_DONTDUMP support scenarios."><col style="width: 20%"/><col style="width: 20%"/><col style="width: 20%"/><col style="width: 40%"/><thead><tr>
...
<th scope="row">ON</th>
<td>ON (default on systems without <code class="literal">MADV_DONTDUMP</code> support)</td>
<td>Not relevant to outcome</td>
<td>Core file is generated with buffer pool pages</td>
</tr><tr>
<th scope="row">ON</th>
<td>OFF (default on systems with <code class="literal">MADV_DONTDUMP</code> support)</td>
<td>Yes</td>
...
<table summary="Core file size reduction examples for different pages sizes."><col style="width: 20%"/><col style="width: 20%"/><col style="width: 20%"/><thead><tr>
