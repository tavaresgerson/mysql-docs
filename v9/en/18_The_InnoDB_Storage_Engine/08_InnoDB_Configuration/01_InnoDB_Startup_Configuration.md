...
9.5\bin`, start the MySQL server like
this:
...
The `InnoDB` doublewrite buffer storage area
...
see [Configuring Redo Log Capacity](innodb-redo-log.html#innodb-modifying-redo-log-capacity "Configuring Redo Log Capacity").
...
variable. If `innodb_log_group_home_dir` was
...
defines the directory path to the `InnoDB` log
