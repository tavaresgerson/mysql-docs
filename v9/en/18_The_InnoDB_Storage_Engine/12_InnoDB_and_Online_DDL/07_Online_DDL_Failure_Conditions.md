...
* A timeout occurs while waiting for an
  [exclusive lock](glossary.html#glos_exclusive_lock "exclusive lock") on
  the table, which may be needed briefly during the initial and
  final phases of the DDL operation.

* The [`tmpdir`](server-system-variables.html#sysvar_tmpdir) or
  [`innodb_tmpdir`](innodb-parameters.html#sysvar_innodb_tmpdir) file system
...
* Concurrent DML makes changes to the table that are allowed
  ...
  [primary key](glossary.html#glos_primary_key "primary key") index on
  that column. The changes made by the concurrent DML take
  precedence, and the [`ALTER TABLE`](alter-table.html "15.1.11 ALTER TABLE Statement")
  operation is effectively [rolled
  back](glossary.html#glos_rollback "rollback").