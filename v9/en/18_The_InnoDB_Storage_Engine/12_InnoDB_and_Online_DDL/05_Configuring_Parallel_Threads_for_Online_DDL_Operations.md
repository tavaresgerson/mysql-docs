...
variable. The default setting is calculated by the number of
available logical processors on the system divided by 8, with a
minimum default value of 4. The maximum setting is 256, which is
the maximum number for all sessions. The actual number of threads
that scan the clustered index is the number defined by the
[`innodb_parallel_read_threads`](innodb-parameters.html#sysvar_innodb_parallel_read_threads)
...
controlled by the
[`innodb_ddl_threads`](innodb-parameters.html#sysvar_innodb_ddl_threads) variable. The
