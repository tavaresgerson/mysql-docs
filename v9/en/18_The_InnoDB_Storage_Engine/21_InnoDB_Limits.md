...
* The maximum row size, excluding any variable-length columns
  ...
  bytes. [`LONGBLOB`](blob.html "13.3.4 The BLOB and TEXT Types") and
  [`LONGTEXT`](blob.html "13.3.4 The BLOB and TEXT Types")
  columns must be less than 4GB, and the total row size,
  including [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") and
...
* The maximum table or tablespace size is impacted by the
  ...
  defined by `InnoDB`. For example, the
...
* For limits associated with concurrent read-write transactions,
  see [Section 17.6.6, “Undo Logs”](innodb-undo-logs.html "17.6.6 Undo Logs").