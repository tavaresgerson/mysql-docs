### 2.4.3 Instalação e Uso do Daemon de Lançamento do MySQL

O macOS utiliza demonstadores de lançamento para iniciar, parar e gerenciar automaticamente processos e aplicativos, como o MySQL.
...
Alguns usuários relatam que adicionar uma declaração de tipo de documento plist causa o falha da operação do launchd, apesar de passar na verificação de lint. Acreditamos que seja um erro de copiar e colar. O checksum md5 de um arquivo contendo o trecho acima é *d925f05f6d1b6ee5ce5451b596d6baed*.

:::

Para habilitar o serviço launchd, você pode:

* Abrir as preferências do sistema do macOS e selecionar o painel de preferências do MySQL, e então executar Iniciar o Servidor MySQL.
...
O processo de atualização substitui seu arquivo de launchd existente chamado `com.oracle.oss.mysql.mysqld.plist`.
...
* A seção **ProgramArguments** define as opções da linha de comando que são passadas para o programa, que, neste caso, é o binário `mysqld`.
...
Como a definição padrão do plist define várias **`ProgramArguments`**, você pode remover a maioria desses argumentos e, em vez disso, confiar no seu arquivo de configuração do MySQL `my.cnf` para defini-los. Por exemplo:
