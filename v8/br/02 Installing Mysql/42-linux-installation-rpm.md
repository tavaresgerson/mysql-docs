### 2.5.4 Instalar MySQL no Linux usando Pacotes RPM da Oracle
...
<table><thead><tr> <th>Nome da embalagem</th> <th>Resumo</th> </tr></thead><tbody><tr> <td>[[PH_HTML_CODE_<code>mysql-community-test</code>]</td> <td>Aplicativos e ferramentas de cliente MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-community-test</code>]</td> <td>Plugins compartilhados para aplicações cliente MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>debuginfo</code>]</td> <td>Arquivos comuns para bibliotecas de servidor e cliente</td> </tr><tr> <td>[[PH_HTML_CODE_<code>debuginfo</code>]</td> <td>Arquivos de cabeçalho de desenvolvimento e bibliotecas para aplicações cliente de banco de dados MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-community-client-debuginfo</code>]</td> <td>Servidor MySQL como uma biblioteca incorporada com compatibilidade para aplicações usando a versão 18 da biblioteca</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-community-libs-debuginfo</code>]</td> <td>Embalagem MySQL de arquivos de dados da UTI necessários para expressões regulares MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-community-server-debug-debuginfo</code>]</td> <td>Bibliotecas compartilhadas para aplicações cliente de banco de dados MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-community-server-debuginfo</code>]</td> <td>Bibliotecas de compatibilidade compartilhadas para instalações anteriores do MySQL; apenas presentes se as versões anteriores do MySQL forem suportadas pela plataforma</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-community-test-debuginfo</code>]</td> <td>Servidor de base de dados e ferramentas relacionadas</td> </tr><tr> <td>[[<code>mysql-community-server-debug</code>]]</td> <td>Servidor de depuração e binários de plugins</td> </tr><tr> <td>[[<code>mysql-community-test</code>]]</td> <td>Suíte de testes para o servidor MySQL</td> </tr><tr> <td>[[<code>mysql-community-client-plugins</code><code>mysql-community-test</code>]</td> <td>O código fonte RPM parece semelhante ao mysql-community-8.4.6-1.el7.src.rpm, dependendo do sistema operacional selecionado</td> </tr><tr> <td>RPMs adicionais</td> <td>Existem vários pacotes de [[<code>debuginfo</code>]]: [[<code>mysql-community-client-debuginfo</code>]], [[<code>mysql-community-libs-debuginfo</code>]] [[<code>mysql-community-server-debug-debuginfo</code>]] [[<code>mysql-community-server-debuginfo</code>]], e [[<code>mysql-community-test-debuginfo</code>]].</td> </tr></tbody></table>

**Tabela 2.11 Pacotes RPM para a Edição Empresarial do MySQL**

<table><thead><tr> <th>Nome da embalagem</th> <th>Resumo</th> </tr></thead><tbody><tr> <td>[[PH_HTML_CODE_<code>mysql-commercial-test</code>]</td> <td>Backup Empresarial do MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-commercial-test</code>]</td> <td>Aplicativos e ferramentas de cliente MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-commercial-client-debuginfo</code>]</td> <td>Plugins compartilhados para aplicações cliente MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-commercial-libs-debuginfo</code>]</td> <td>Arquivos comuns para bibliotecas de servidor e cliente</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-commercial-server-debug-debuginfo</code>]</td> <td>Arquivos de cabeçalho de desenvolvimento e bibliotecas para aplicações cliente de banco de dados MySQL</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-commercial-server-debuginfo</code>]</td> <td>Servidor MySQL como uma biblioteca incorporada com compatibilidade para aplicações usando a versão 18 da biblioteca</td> </tr><tr> <td>[[PH_HTML_CODE_<code>mysql-commercial-test-debuginfo</code>]</td> <td>Embalagem MySQL de arquivos de dados da UTI necessários para expressões regulares MySQL</td> </tr><tr> <td>[[<code>mysql-commercial-libs</code>]]</td> <td>Bibliotecas compartilhadas para aplicações cliente de banco de dados MySQL</td> </tr><tr> <td>[[<code>mysql-commercial-libs-compat</code>]]</td> <td>Bibliotecas de compatibilidade compartilhadas para instalações anteriores do MySQL; apenas presentes se as versões anteriores do MySQL forem suportadas pela plataforma. A versão das bibliotecas corresponde à versão das bibliotecas instaladas por padrão pela distribuição que você está usando.</td> </tr><tr> <td>[[<code>mysql-commercial-server</code>]]</td> <td>Servidor de base de dados e ferramentas relacionadas</td> </tr><tr> <td>[[<code>mysql-commercial-test</code>]]</td> <td>Suíte de testes para o servidor MySQL</td> </tr><tr> <td>RPMs adicionais *debuginfo*</td> <td>Existem vários pacotes de [[<code>mysql-commercial-client</code><code>mysql-commercial-test</code>]: [[<code>mysql-commercial-client-debuginfo</code>]], [[<code>mysql-commercial-libs-debuginfo</code>]] [[<code>mysql-commercial-server-debug-debuginfo</code>]] [[<code>mysql-commercial-server-debuginfo</code>]], e [[<code>mysql-commercial-test-debuginfo</code>]].</td> </tr></tbody></table>
...
<table><thead><tr> <th>Valor de distribuição</th> <th>Utilização prevista</th> </tr></thead><tbody><tr> <td>o<em>[[<code>{version}</code>]]</em>em que<em>[[<code>{version}</code>]]</em>é a principal versão do Enterprise Linux, como [[<code>el8</code>]]</td> <td>Plataformas baseadas em EL6 (8.0), EL7, EL8, EL9 e EL10 (por exemplo, as versões correspondentes de Oracle Linux, Red Hat Enterprise Linux e CentOS)</td> </tr><tr> <td>fc<em>[[<code>{version}</code>]]</em>em que<em>[[<code>{version}</code>]]</em>é a versão principal do Fedora, como [[<code>fc37</code>]]</td> <td>Fedora 41 e 42</td> </tr><tr> <td>[[<code>sl5</code>]]</td> <td>SUSE Linux Enterprise Server 15 (em inglês)</td> </tr></tbody></table>
...
Existem relações de dependência entre alguns dos pacotes. Se você planeja instalar muitos dos pacotes, você pode querer baixar o arquivo `tar` do pacote RPM, que contém todos os pacotes RPM listados acima, para que você não precise baixá-los separadamente.
...
Para instalar apenas os programas cliente, você pode ignorar `mysql-community-server` na sua lista de pacotes para instalar; emitir o seguinte comando:
...
Se o sistema operacional estiver habilitado, os comandos padrão `systemctl` (ou, alternativamente, `service` com os argumentos invertidos) como `stop`, `start`, `status`, e `restart` devem ser usados para gerenciar o serviço de servidor MySQL. O serviço `mysqld` está habilitado por padrão, e ele começa na reinicialização do sistema. Observe que certas coisas podem funcionar de forma diferente nas plataformas do systemd: por exemplo, mudar a localização do diretório de dados pode causar problemas.

Durante uma instalação de atualização usando pacotes RPM e DEB, se o servidor MySQL estiver em execução quando a atualização ocorrer, o servidor MySQL será interrompido, a atualização ocorrerá e o servidor MySQL será reiniciado.
...
Para algumas distribuições Linux, pode ser necessário aumentar o limite do número de descritores de arquivos disponíveis para `mysqld`.

É possível instalar várias versões de bibliotecas de clientes, como no caso de você querer manter a compatibilidade com aplicativos mais antigos ligados a bibliotecas anteriores. Para instalar uma biblioteca de clientes mais antiga, use a opção `--oldpackage` com `rpm`. Por exemplo, para instalar `mysql-community-libs-5.5` em um sistema EL6 que tem `libmysqlclient.21` do MySQL 8.0, use um comando como este:

```
$> rpm --oldpackage -ivh mysql-community-libs-5.5.50-2.el6.x86_64.rpm
```

\*\* Pacote de depuração. \*\* Uma variante especial do MySQL Server compilado com o pacote de depuração foi incluído nos pacotes RPM do servidor. Ele executa depuração e verifica a alocação de memória e produz um arquivo de rastreamento quando o servidor está em execução. Para usar essa versão de depuração, inicie o MySQL com `/usr/sbin/mysqld-debug`, em vez de iniciá-lo como um serviço ou com `/usr/sbin/mysqld`.
...
\*\* Reconstruir RPMs a partir de SRPMs de origem. \*\* Pacotes SRPM de código fonte para MySQL estão disponíveis para download. Eles podem ser usados como estão para reconstruir os RPMs de MySQL com a cadeia de ferramentas padrão `rpmbuild`.
