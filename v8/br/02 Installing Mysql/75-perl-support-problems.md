...
Se o Perl relata que não consegue encontrar o módulo `../mysql/mysql.so`, o problema é provavelmente que o Perl não consegue localizar a biblioteca compartilhada `libmysqlclient.so`. Você deve ser capaz de corrigir este problema por um dos seguintes métodos:
...
- Adicione o nome do caminho do diretório onde `libmysqlclient.so` está localizado para a variável de ambiente `LD_RUN_PATH`. Alguns sistemas usam `LD_LIBRARY_PATH` em vez disso.
...
Outra causa deste problema pode ser que Perl e MySQL não são ambos compilados com `gcc`. Neste caso, você pode resolver a incompatibilidade compilando ambos com `gcc`.
