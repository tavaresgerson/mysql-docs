...
Também é possível baixar as distribuições dos módulos na forma de arquivos `tar` comprimidos e construir os módulos manualmente. Por exemplo, para descompactar e construir uma distribuição do DBI, use um procedimento como este:
...
O comando `make test` é importante porque verifica se o módulo está funcionando. Note que, quando você executa esse comando durante a instalação do `DBD::mysql` para exercitar o código da interface, o servidor MySQL deve estar em execução ou o teste falha.
