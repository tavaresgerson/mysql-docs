...
Meios alternativos para realizar o processo descrito na presente secção:
...
- As instalações que usam pacotes Debian lhe dão a opção de atribuir uma senha.
...
A conta `'root'@'localhost'` também tem uma linha na tabela `mysql.proxies_priv` que permite conceder o privilégio `PROXY` para `''@''`, isto é, para todos os usuários e todos os hosts. Isto permite a `root` configurar usuários proxy, bem como delegar a outras contas a autoridade para configurar usuários proxy.

Para atribuir uma senha para a conta inicial do MySQL `root`, use o seguinte procedimento. Substitua `root-password` nos exemplos com a senha que você deseja usar.

Iniciar o servidor se ele não estiver em execução.

A conta inicial pode ou não ter uma senha.

- Se a conta `root` existe com uma senha aleatória inicial que expirou, conecte-se ao servidor como `root` usando essa senha e, em seguida, escolha uma nova senha. Este é o caso se o diretório de dados foi inicializado usando `mysqld --initialize`, manualmente ou usando um instalador que não lhe dá a opção de especificar uma senha durante a operação de instalação. Como a senha existe, você deve usá-la para se conectar ao servidor. Mas como a senha expirou, você não pode usar a conta para qualquer outro propósito além de escolher uma nova senha, até que você escolha uma.

  1. Se você não sabe a senha aleatória inicial, procure no registro de erros do servidor.
  ...
  3. Escolha uma nova senha para substituir a senha aleatória:
     ...
     ```
- Se a conta `root` existe, mas não tem senha, conecte-se ao servidor como `root` sem usar senha, em seguida, atribuir uma senha. Este é o caso se você inicializou o diretório de dados usando `mysqld --initialize-insecure`.
...
Depois de atribuir uma senha à conta PH, você deve fornecer essa senha sempre que se conectar ao servidor usando a conta. Por exemplo, para se conectar ao servidor usando o cliente PH, use este comando:
...
Para obter informações adicionais sobre a configuração de senhas, consulte a Seção 8.2.14, "Asignação de senhas de conta". Se você esquecer sua senha `root` depois de configurá-la, consulte a Seção B.3.3.2, "Como redefinir a senha raiz".

Para criar contas adicionais, ver Secção 8.2.8, "Adição de contas, atribuição de privilégios e eliminação de contas".
