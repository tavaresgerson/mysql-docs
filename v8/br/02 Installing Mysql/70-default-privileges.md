...
A tabela `mysql.user` define a conta de usuário inicial do MySQL e seus privilégios de acesso. A instalação do MySQL cria apenas uma conta de superusuário `'root'@'localhost'`, que possui todos os privilégios e pode fazer qualquer coisa. Se a conta `root` tiver uma senha vazia, sua instalação do MySQL não estará protegida: qualquer pessoa pode se conectar ao servidor MySQL como `root` *sem uma senha* e receber todos os privilégios.

A conta `'root'@'localhost'` também tem uma linha na tabela `mysql.proxies_priv` que permite conceder o privilégio `PROXY` para `''@''`, ou seja, para todos os usuários e todos os hosts. Isso permite que `root` configure usuários proxy, bem como delegar a outras contas a autoridade para configurar usuários proxy. Veja a Seção 8.2.19, “Usuários Proxy”.
...
Para obter informações adicionais sobre a configuração de senhas, consulte a Seção 8.2.14, “Atribuição de Senhas de Conta”. Se você esquecer a senha de `root` após tê-la configurado, consulte a Seção B.3.3.2, “Como Redefinir a Senha de Root”.
