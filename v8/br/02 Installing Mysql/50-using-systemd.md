...
Em plataformas para as quais o suporte do systemd para o MySQL está instalado, scripts como `mysqld_safe` e o script de inicialização `System V` são desnecessários e não são instalados. Por exemplo, `mysqld_safe` pode lidar com reinicializações do servidor, mas o systemd fornece a mesma capacidade, de maneira consistente com a gestão de outros serviços, em vez de usar um programa específico para a aplicação.
...
* `mysqld.service` (plataformas RPM), `mysql.service` (plataformas Debian): arquivo de configuração de unidade do serviço systemd, com detalhes sobre o serviço MySQL.
...
Para adicionar ou alterar opções do systemd para MySQL, estão disponíveis esses métodos:

* Use um arquivo de configuração systemd localizado.
* Organize para que o systemd defina variáveis de ambiente para o processo do servidor MySQL.
...
Sempre que você criar ou alterar `override.conf`, recarregue a configuração do systemd e, em seguida, diga ao systemd para reiniciar o serviço MySQL:
...
* Para alguns parâmetros, `override.conf` deve ser usado porque o próprio systemd deve conhecer seus valores e não pode ler arquivos de opções MySQL para obtê-los.
* Parâmetros que especificam valores que de outra forma podem ser definidos apenas usando opções conhecidas por `mysqld_safe` devem ser especificados usando o systemd porque não há um parâmetro correspondente `mysqld`.

Para obter informações adicionais sobre o uso do systemd em vez de `mysqld_safe`, consulte Migrando do mysqld_safe para o systemd.
...
* `NOTIFY_SOCKET`: Essa variável de ambiente especifica o socket que o `mysqld` usa para comunicar a conclusão da inicialização e a mudança do status do serviço com o systemd. Ela é definida pelo systemd quando o serviço `mysqld` é iniciado. O serviço `mysqld` lê o ajuste da variável e escreve no local definido.

No MySQL 8.4, o `mysqld` usa o tipo de inicialização do processo `Type=notify`. (`Type=forking` foi usado no MySQL 5.7.) Com `Type=notify`, o systemd configura automaticamente um arquivo de socket e exporta o caminho para a variável de ambiente `NOTIFY_SOCKET`.
* `TZ`: Defina essa variável para especificar a fusão de tempo padrão para o servidor.
...
* Use linhas `Environment` no arquivo `override.conf`. Para a sintaxe, consulte o exemplo na discussão anterior que descreve como usar esse arquivo.
* Especifique os valores no arquivo `/etc/sysconfig/mysql` (crie o arquivo se ele não existir). Atribua valores usando a seguinte sintaxe:
  ...
  Após modificar o `/etc/sysconfig/mysql`, reinicie o servidor para fazer as alterações entrarem em vigor:
...
Após modificar o ambiente do systemd, reinicie o servidor para fazer as alterações entrarem em vigor:
...
Para plataformas que usam systemd, o diretório de dados é inicializado se estiver vazio no início do servidor. Isso pode ser um problema se o diretório de montagem for um monte remoto que desapareceu temporariamente: o ponto de montagem parecerá ser um diretório de dados vazio, que então será inicializado como um novo diretório de dados. Para suprimir esse comportamento de inicialização automática, especifique a seguinte linha no arquivo `/etc/sysconfig/mysql` (crie o arquivo se ele não existir):

```
NO_INIT=true
```

#### Configurando Instâncias Múltiplas do MySQL Usando systemd
...
Para usar a capacidade de instâncias múltiplas, modifique o arquivo de opção `my.cnf` para incluir a configuração das opções-chave para cada instância. Esses locais de arquivo são típicos:
...
O uso de asteriscos também é suportado. Por exemplo, este comando exibe o status de todas as instâncias de replica:

```
systemctl status 'mysqld@replica*'
```

Para a gestão de múltiplas instâncias do MySQL na mesma máquina, o systemd usa automaticamente um arquivo de unidade diferente:
...
Como o `mysqld_safe` não está instalado em plataformas que usam o systemd para gerenciar o MySQL, as opções especificadas anteriormente para esse programa (por exemplo, em um grupo de opções `[mysqld_safe]` ou `[safe_mysqld]`) devem ser especificadas de outra forma:

* Algumas opções do `mysqld_safe` também são entendidas pelo `mysqld` e podem ser movidas do grupo de opções `[mysqld_safe]` ou `[safe_mysqld]` para o grupo `[mysqld]`. Isso não inclui `--pid-file`, `--open-files-limit` ou `--nice`. Para especificar essas opções, use o arquivo `override.conf` do systemd, descrito anteriormente.
...
* Para algumas opções do `mysqld_safe`, existem procedimentos alternativos do `mysqld`. Por exemplo, a opção `mysqld_safe` para habilitar o registro no `syslog` é `--syslog`, que está desatualizada. Para gravar a saída do log de erro no log do sistema, use as instruções na Seção 7.4.2.8, “Registro de Erros no Log do Sistema”.
* Opções do `mysqld_safe` que não são entendidas pelo `mysqld` podem ser especificadas no `override.conf` ou em variáveis de ambiente. Por exemplo, com `mysqld_safe`, se o servidor deve usar uma biblioteca específica de alocação de memória, isso é especificado usando a opção `--malloc-lib`. Para instalações que gerenciam o servidor com o systemd, organize-se para definir a variável de ambiente `LD_PRELOAD` em vez disso, conforme descrito anteriormente.