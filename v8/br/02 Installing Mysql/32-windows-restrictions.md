...
* **Nomes não sensíveis a maiúsculas e minúsculas**
  ...
  Os nomes de arquivos não são sensíveis a maiúsculas e minúsculas no Windows, portanto, os nomes de banco de dados e tabelas do MySQL também não são sensíveis a maiúsculas e minúsculas no Windows. A única restrição é que os nomes de banco de dados e tabelas devem ser especificados usando a mesma grafia ao longo de uma determinada instrução. Consulte a Seção 11.2.3, “Sensibilidade ao Caso dos Identificadores”.
* **Diretórios e nomes de arquivos**

  No Windows, o MySQL Server suporta apenas nomes de diretórios e arquivos que são compatíveis com as páginas de código ANSI atuais. Por exemplo, o seguinte nome de diretório japonês não funciona no local ocidental (página de código 1252):
