...
- \*\* Memória de processo \*\*
  ...
  Nas plataformas Windows de 32 bits, não é possível usar mais de 2 GB de RAM em um único processo, incluindo o MySQL. Isso ocorre porque o limite de endereço físico no Windows de 32 bits é de 4 GB e a configuração padrão no Windows é dividir o espaço de endereço virtual entre o kernel (2 GB) e o usuário / aplicativos (2 GB).

  Algumas versões do Windows têm uma configuração de tempo de inicialização para habilitar aplicações maiores, reduzindo o aplicativo do kernel.
- \*\* Aliases do sistema de ficheiros \*\*

  Ao usar tabelas `MyISAM`, você não pode usar aliases dentro do Windows para vincular os arquivos de dados em outro volume e, em seguida, vincular de volta ao local principal do MySQL `datadir`.

  Esta facilidade é frequentemente usada para mover os arquivos de dados e índice para um RAID ou outra solução rápida.
...
- **Nomes de diretórios e arquivos**
  ...
  No Windows, o MySQL Server suporta apenas diretórios e nomes de arquivos que são compatíveis com as páginas de código ANSI atuais. Por exemplo, o seguinte nome de diretório japonês não funciona na localização ocidental (página de código 1252):

  ```
  datadir="C:/私たちのプロジェクトのデータ"
  ```

  A mesma limitação aplica-se a diretórios e nomes de arquivos referidos em instruções SQL, como o nome do caminho de arquivo de dados em `LOAD DATA`.
- \*\* O caráter separador do nome de caminho `\`\*\*
