### 28.6.2 A tabela INFORMATION\_SCHEMA CONNECTION\_CONTROL\_FAILED\_LOGIN\_ATTEMPTS

Esta tabela fornece informações sobre o número atual de tentativas consecutivas de conexão falhas por conta (combinação de usuário/host).
...
- `FAILED_ATTEMPTS`
  ...
  O número atual de tentativas consecutivas de conexão falhas para o valor `USERHOST`. Este número conta todas as tentativas falhas, independentemente de elas terem sido atrasadas. O número de tentativas para as quais o servidor adicionou um atraso à sua resposta é a diferença entre o valor do sistema `FAILED_ATTEMPTS` e o valor da variável do sistema `connection_control_failed_connections_threshold`.

#### Notas

- O plugin `CONNECTION_CONTROL_FAILED_LOGIN_ATTEMPTS` deve estar ativado para que essa tabela esteja disponível, e o plugin `CONNECTION_CONTROL` deve estar ativado ou o conteúdo da tabela ficará sempre vazio. Veja a Seção 8.4.2, “Plugins de Controle de Conexão”.
