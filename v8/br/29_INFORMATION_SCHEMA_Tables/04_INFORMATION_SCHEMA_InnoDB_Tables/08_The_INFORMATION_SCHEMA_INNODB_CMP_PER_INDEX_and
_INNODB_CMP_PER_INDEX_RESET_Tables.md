...
Para uma tabela compactada `InnoDB`, tanto os dados da tabela quanto todos os índices secundários são compactados. Nesse contexto, os dados da tabela são tratados como apenas outro índice, aquele que contém todas as colunas: o índice agrupado.
...
- `COMPRESS_OPS`
  ...
  Número de operações de compressão realizadas. As páginas são comprimidas sempre que uma página vazia é criada ou quando o espaço para o log de modificação não comprimido esgota.
...
- `UNCOMPRESS_OPS`
  ...
  Número de operações de descompactação realizadas. As páginas compactadas `InnoDB` são descompactadas sempre que a compactação falhar ou na primeira vez que uma página compactada for acessada no pool de buffer e a página descompactada não existir.
