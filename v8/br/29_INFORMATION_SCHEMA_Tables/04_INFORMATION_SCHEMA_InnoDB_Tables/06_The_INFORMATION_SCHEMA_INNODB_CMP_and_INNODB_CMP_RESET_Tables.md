...
- `COMPRESS_OPS`
  ...
  O número de vezes que uma página de árvore B do tamanho `PAGE_SIZE` foi comprimida. As páginas são comprimidas sempre que uma página vazia é criada ou quando o espaço para o log de modificação não comprimido esgota.

- `COMPRESS_OPS_OK`

  Número de vezes que uma página de árvore B do tamanho `PAGE_SIZE` foi comprimida com sucesso. Esse número nunca deve exceder `COMPRESS_OPS`.
...
- `UNCOMPRESS_OPS`
  ...
  Número de vezes que uma página de árvore B do tamanho `PAGE_SIZE` foi descompactada. As páginas de árvore B são descompactadas sempre que a compressão falha ou na primeira vez de acesso, quando a página descompactada não existe no pool de buffer.
