...
As entradas na tabela `ST_SPATIAL_REFERENCE_SYSTEMS` são baseadas no conjunto de dados do European Petroleum Survey Group (EPSG), exceto para o SRID 0, que corresponde a um SRS especial usado no MySQL e que representa um plano cartesiano plano infinito sem unidades atribuídas aos seus eixos. Para obter informações adicionais sobre SRSs, consulte a Seção 13.4.5, “Suporte ao Sistema de Referência Espacial”.
...
- `ORGANIZATION_COORDSYS_ID`
  ...
  O ID numérico atribuído ao sistema de referência espacial pela organização que o definiu.

- `DEFINITION`

  Definição do sistema de referência espacial. Os valores `DEFINITION` são valores WKT, representados conforme especificado no documento do Open Geospatial Consortium OGC 12-063r5.
...
- `DESCRIPTION`
  ...
  Descrição do sistema de referência espacial.

#### Notas

- As colunas `SRS_NAME`, `ORGANIZATION`, `ORGANIZATION_COORDSYS_ID` e `DESCRIPTION` contêm informações que podem ser de interesse para os usuários, mas elas não são usadas pelo MySQL.
...
Esta entrada descreve o SRS usado para sistemas de GPS. Ele tem um nome (`SRS_NAME`) de WGS 84 e um ID (`SRS_ID`) de 4326, que é o ID usado pelo Grupo Europeu de Pesquisas Petrolíferas (EPSG).

Os valores `DEFINITION` para SRSs projetadas e geográficas começam com `PROJCS` e `GEOGCS`, respectivamente. A definição para SRID 0 é especial e tem um valor `DEFINITION` vazio. A seguinte consulta determina quantos registros na tabela `ST_SPATIAL_REFERENCE_SYSTEMS` correspondem a SRSs projetadas, geográficas e outras, com base nos valores `DEFINITION`:
