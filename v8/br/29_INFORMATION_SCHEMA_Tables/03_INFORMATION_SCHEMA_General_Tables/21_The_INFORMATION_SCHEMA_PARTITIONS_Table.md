...
A tabela `PARTITIONS` fornece informações sobre as partições de tabelas. Cada linha desta tabela corresponde a uma partição ou subpartição individual de uma tabela particionada. Para obter mais informações sobre a partição de tabelas, consulte o Capítulo 26, *Partição*.
...
- `PARTITION_ORDINAL_POSITION`
  ...
  Todas as partições são indexadas na mesma ordem em que são definidas, com `1` sendo o número atribuído à primeira partição. O índice pode mudar à medida que partições são adicionadas, removidas e reorganizadas; o número exibido neste campo reflete a ordem atual, levando em conta quaisquer alterações no índice.
...
- `SUBPARTITION_EXPRESSION`
  ...
  Isso funciona da mesma maneira para a expressão de subpartição que define a subpartição para uma tabela, da mesma forma que `PARTITION_EXPRESSION` faz com a expressão de partição usada para definir a partição de uma tabela.
...
- `PARTITION_DESCRIPTION`
  ...
  Para partições cujo `PARTITION_METHOD` é diferente de `RANGE` ou `LIST`, esta coluna é sempre `NULL`.

- `TABLE_ROWS`

  O número de linhas da tabela na partição.

  Para tabelas `InnoDB` particionadas, o número de linhas fornecido na coluna `TABLE_ROWS` é apenas um valor estimado usado na otimização do SQL e pode não ser sempre exato.
...
- Uma tabela `NDB` que não está explicitamente particionada tem uma linha na tabela `PARTITIONS` para cada nó de dados no cluster NDB. Para cada linha dessa tabela:
