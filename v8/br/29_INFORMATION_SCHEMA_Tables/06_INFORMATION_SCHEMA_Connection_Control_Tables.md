## 28.6 Tabelas de Controle de Conexão do SCHEMA_INFORMÁTICO

As seções a seguir descrevem as tabelas `INFORMATION_SCHEMA` associadas ao plugin `connection_control`.

### 28.6.1 Referências à Tabela de Controle de Conexão do SCHEMA_INFORMÁTICO
...
**Tabela 28.8 Tabelas de Controle de Conexão do SCHEMA_INFORMÁTICO**

<table frame="box" rules="all" summary="A reference that lists INFORMATION_SCHEMA connection control tables."><col style="width: 28%"/><col style="width: 71%"/><thead><tr><th>Table Name</th> <th>Descrição</th> </tr></thead><tbody><tr><td><code>CONNECTION_CONTROL_FAILED_LOGIN_ATTEMPTS</code></td> <td>Número atual de tentativas consecutivas de conexão falhadas por conta</td> </tr></tbody></table>

### 28.6.2 A tabela INFORMATION_SCHEMA CONNECTION_CONTROL_FAILED_LOGIN_ATTEMPTS

Esta tabela fornece informações sobre o número atual de tentativas de conexão consecutivas falhadas por conta (combinação de usuário/host).
...
* O plugin `CONNECTION_CONTROL_FAILED_LOGIN_ATTEMPTS` deve ser ativado para que esta tabela esteja disponível, e o plugin `CONNECTION_CONTROL` deve ser ativado ou o conteúdo da tabela ficará sempre vazio. Veja a Seção 8.4.2, “Plugins de Controle de Conexão”.
