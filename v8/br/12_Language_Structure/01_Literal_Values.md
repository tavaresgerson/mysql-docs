...
Uma cadeia é uma sequência de bytes ou caracteres, encerrada entre caracteres de aspas simples (`'`) ou de aspas duplas (`"`). Exemplos:
...
Para inserir dados binários em uma coluna de string (como uma coluna `BLOB`), você deve representar certos caracteres por sequências de escape. O barra invertida (`\`) e o caractere de citação usado para citar a string devem ser escapados. Em certos ambientes de cliente, também pode ser necessário escapar `NUL` ou Control+Z. O cliente **mysql** corta as strings citadas que contêm caracteres `NUL` se não forem escapados, e o Control+Z pode ser interpretado como FIM DE FICHAMENTO no Windows se não for escapado. Para as sequências de escape que representam cada um desses caracteres, consulte a Tabela 11.1, “Sequências de escape de caracteres especiais”.
...
* Processe a string com uma função que escape os caracteres especiais. Em um programa em C, você pode usar a função `mysql_real_escape_string_quote()` da API C para escapar caracteres. Veja mysql_real_escape_string_quote(). Dentro das declarações SQL que constroem outras declarações SQL, você pode usar a função `QUOTE()`. A interface Perl DBI fornece um método `quote` para converter caracteres especiais nas sequências de escape apropriadas. Veja Seção 31.9, “MySQL Perl API”. Outras interfaces de linguagem podem fornecer uma capacidade semelhante.
...
Esta seção descreve os formatos aceitáveis para datas e horários literais. Para mais informações sobre os tipos de dados temporais, como a faixa de valores permitidos, consulte a Seção 13.2, “Tipos de dados de data e hora”.
...
* Como uma cadeia em qualquer formato de `'YYYY-MM-DD'` ou `'YY-MM-DD'`. Uma sintaxe "relaxada" é permitida, mas é desaconselhada: qualquer caractere de pontuação pode ser usado como delimitador entre as partes da data. Por exemplo, `'2012-12-31'`, `'2012/12/31'`, `'2012^12^31'` e `'2012@12@31'` são equivalentes. A partir do MySQL 8.0.29, usar qualquer caractere diferente do hífen (`-`) como delimitador gera uma advertência, conforme mostrado aqui:
...
* Como uma string sem delimitadores em qualquer um dos formatos `'YYYYMMDD'` ou `'YYMMDD'`, desde que a string faça sentido como uma data. Por exemplo, `'20070523'` e `'070523'` são interpretados como `'2007-05-23'`, mas `'071332'` é ilegal (tem partes de mês e dia sem sentido) e se torna `'0000-00-00'`.
...
* Como uma cadeia em qualquer formato de `'YYYY-MM-DD hh:mm:ss'` ou `'YY-MM-DD hh:mm:ss'`. O MySQL também permite uma sintaxe "relaxada" aqui, embora essa seja desaconselhada: qualquer caractere de pontuação pode ser usado como delimitador entre partes de data ou partes de hora. Por exemplo, `'2012-12-31 11:30:45'`, `'2012^12^31 11+30+45'`, `'2012/12/31 11*30*45'` e `'2012@12@31 11^30^45'` são equivalentes. A partir do MySQL 8.0.29, o uso de qualquer caractere como delimitador em tais valores, exceto o travessão (`-`) para a parte de data e o colon (`:`) para a parte de hora, gera um aviso, conforme mostrado aqui:
...
Anteriormente, o MySQL suportava um número arbitrário de caracteres de espaço em branco no início e no fim de valores de data e hora, bem como entre as partes de data e hora dos valores de `DATETIME` e `TIMESTAMP`. No MySQL 8.0.29 e versões posteriores, esse comportamento é desaconselhado, e a presença de excessos de caracteres de espaço em branco aciona um aviso, conforme mostrado aqui:
...
Além disso, a partir do MySQL 8.0.29, um aviso é exibido quando são usados caracteres de espaço em branco, exceto o caractere de espaço, como este:
...
Apenas um aviso desse tipo é exibido por valor temporal, mesmo que existam vários problemas com delimitadores, espaços em branco ou ambos, conforme demonstrado na seguinte série de declarações:
...
* No formato *`hhmmss`*, desde que faça sentido como uma hora. Por exemplo, `101112` é entendido como `'10:11:12'`. Os seguintes formatos alternativos também são entendidos: *`ss`*, *`mmss`* ou *`hhmmss`*.
...
Para valores de `TIME` especificados como strings que incluem um delimitador de parte de tempo, não é necessário especificar dois dígitos para valores de horas, minutos ou segundos que são menores que `10`. `'8:3:2'` é o mesmo que `'08:03:02'`.

A partir do MySQL 8.0.19, você pode especificar um deslocamento de fuso horário ao inserir os valores `TIMESTAMP` e `DATETIME` em uma tabela. O deslocamento é anexado à parte de hora de uma literal datetime, sem espaços intertravados, e usa o mesmo formato usado para definir a variável de sistema `time_zone`, com as seguintes exceções:

* Para valores de hora menores que 10, é necessário um zero inicial.
* O valor `'-00:00'` é rejeitado.
* Nomes de fuso horário, como `'EET'` e `'Asia/Shanghai'` não podem ser usados; `'SYSTEM'` também não pode ser usado neste contexto.

O valor inserido não deve ter um zero na parte do mês, na parte do dia ou em ambas. Isso é exigido a partir do MySQL 8.0.22, independentemente da configuração do modo SQL do servidor.

Este exemplo ilustra a inserção de valores datetime com deslocamentos de fuso horário nas colunas `TIMESTAMP` e `DATETIME` usando diferentes configurações do `time_zone`, e depois recuperando-as:
...
O deslocamento não é exibido ao selecionar um valor de data e hora, mesmo que um tenha sido usado ao inseri-lo.

A faixa de valores de deslocamento suportados é `-13:59` a `+14:00`, inclusive.

Datas literais que incluem deslocamentos de fuso horário são aceitos como valores de parâmetro por declarações preparadas.
...
Os valores escritos usando a notação `X'val'` devem conter um número par de dígitos ou ocorrerá um erro de sintaxe. Para corrigir o problema, adicione um zero à frente do valor:
...
Um literal hexadecimal pode ter um introduzidor opcional de conjunto de caracteres e a cláusula `COLLATE`, para designá-lo como uma cadeia que utiliza um conjunto de caracteres e uma ordenação específicos:
...
Os exemplos utilizam a notação `X'val'`, mas a notação `0xval` também permite introdutores. Para informações sobre introdutores, consulte a Seção 12.3.8, “Introdutores de Conjunto de Caracteres”.
...
Os exemplos utilizam a notação `b'val'`, mas a notação `0bval` também permite introdutores. Para informações sobre introdutores, consulte a Seção 12.3.8, “Introdutores de Conjunto de Caracteres”.
...
Para literais de bits, as operações de bits são consideradas contexto numérico, mas as operações de bits permitem argumentos de cadeia numérica ou binária no MySQL 8.0 e versões posteriores. Para especificar explicitamente o contexto de cadeia binária para literais de bits, use um `_binary` para pelo menos um dos argumentos:
...
Para operações de importação ou exportação de arquivos de texto realizadas com `LOAD DATA` ou `SELECT ... INTO OUTFILE` (select-into.html "15.2.13.1 SELECT ... INTO Statement"), `NULL` é representado pela sequência `\N`. Veja a Seção 15.2.9, “Instrução LOAD DATA”.

Para a classificação com os valores de `ORDER BY`, `NULL`, os valores são classificados antes dos outros valores para classificações ascendentes e, após os outros valores para classificações descendentes.