...
Certos objetos dentro do MySQL, incluindo banco de dados, tabela, índice, coluna, alias, visão, procedimento armazenado, partição, tablespace, grupo de recursos e outros nomes de objetos são conhecidos como identificadores. Esta seção descreve a sintaxe permitida para identificadores no MySQL. A Seção 11.2.1, “Limites de comprimento do identificador”, indica o comprimento máximo de cada tipo de identificador. A Seção 11.2.3, “Sensibilidade ao caso dos identificadores”, descreve quais tipos de identificadores são sensíveis ao caso e sob quais condições.
...
* A partir do MySQL 8.0.32, o uso do sinal de dólar como o primeiro caractere no nome não citado de um banco de dados, tabela, visão, coluna, programa armazenado ou alias é desaconselhado e produz um aviso. Isso inclui nomes usados com qualificadores (ver Seção 11.2.2, “Qualificadores de Identificador”). O sinal de dólar ainda pode ser usado como o caractere inicial de tal identificador quando é citado de acordo com as regras dadas mais adiante nesta seção.
...
O modo `ANSI_QUOTES` faz com que o servidor interprete as cadeias de caracteres com aspas duplas como identificadores. Consequentemente, quando este modo é habilitado, as cadeias de caracteres literais devem ser encerradas entre aspas simples. Elas não podem ser encerradas entre aspas duplas. O modo SQL do servidor é controlado conforme descrito na Seção 7.1.11, “Modos SQL do servidor”.
...
Também é recomendado que você não use nomes de coluna que comecem com `!hidden!` para garantir que novos nomes não colidem com nomes usados por colunas ocultas existentes para índices funcionais.
...
Os caracteres especiais nos nomes de banco de dados e tabelas são codificados nos nomes correspondentes do sistema de arquivos, conforme descrito na Seção 11.2.4, “Mapeamento de Identificadores a Nomes de Arquivos”.

### 11.2.1 Limites de comprimento do identificador
...
Os alias para os nomes das colunas nas declarações `CREATE VIEW`(create-view.html "15.1.23 CREATE VIEW Statement") são verificados contra o comprimento máximo da coluna de 64 caracteres (e não o comprimento máximo do alias de 256 caracteres).

Para definições de restrição que não incluem nenhum nome de restrição, o servidor gera internamente um nome derivado do nome da tabela associada. Por exemplo, os nomes de chave estrangeira gerados internamente e as restrições `CHECK` consistem no nome da tabela mais `_ibfk_` ou `_chk_` e um número. Se o nome da tabela estiver próximo ao limite de comprimento para nomes de restrição, os caracteres adicionais necessários para o nome da restrição podem fazer com que o nome exceda o limite, resultando em um erro.

Os identificadores são armazenados usando Unicode (UTF-8). Isso se aplica a identificadores em definições de tabela e a identificadores armazenados nas tabelas de concessão no banco de dados `mysql`. Os tamanhos das colunas de cadeia de identificadores nas tabelas de concessão são medidos em caracteres. Você pode usar caracteres multibyte sem reduzir o número de caracteres permitidos para valores armazenados nessas colunas.

Antes do NDB 8.0.18, o NDB Cluster impunha um comprimento máximo de 63 caracteres para os nomes de bancos de dados e tabelas. A partir do NDB 8.0.18, essa limitação é removida. Veja a Seção 25.2.7.11, “Problemas anteriores do NDB Cluster resolvidos no NDB Cluster 8.0”.
...
### 11.2.2 Identificadores de qualificadores
...
* Um nome de coluna pode receber várias qualificações para indicar o contexto em declarações que a referenciam, conforme mostrado na tabela a seguir.
  ...
  <table summary="Column reference formats that can be used to refer to table columns."><col style="width: 35%"/><col style="width: 65%"/><thead><tr> <th>Column Reference</th> <th>Meaning</th> </tr></thead><tbody><tr> <td><em class="replaceable"><code>col_name</code></em></td> <td>Column <em class="replaceable"><code>col_name</code></em> from whichever table used in the statement contains a column of that name</td> </tr><tr> <td><em class="replaceable"><code>tbl_name.col_name</code></em></td> <td>Column <em class="replaceable"><code>col_name</code></em> from table <em class="replaceable"><code>tbl_name</code></em> of the default database</td> </tr><tr> <td><em class="replaceable"><code>db_name.tbl_name.col_name</code></em></td> <td>Column <em class="replaceable"><code>col_name</code></em> from table <em class="replaceable"><code>tbl_name</code></em> of the database <em class="replaceable"><code>db_name</code></em></td> </tr></tbody></table>
...
### 11.2.3 Sensibilidade do identificador à grafia de letras maiúsculas e minúsculas
...
Os nomes de partição, subpartição, coluna, índice, rotina armazenada, evento e grupo de recursos não são sensíveis ao caso em qualquer plataforma, assim como os aliases de coluna.
...
A forma como os nomes de tabela e banco de dados são armazenados no disco e utilizados no MySQL é afetada pela variável de sistema `lower_case_table_names`. `lower_case_table_names` pode assumir os valores mostrados na tabela a seguir. Esta variável *não* afeta a sensibilidade de caso dos identificadores de gatilho. No Unix, o valor padrão de `lower_case_table_names` é 0. No Windows, o valor padrão é 1. No macOS, o valor padrão é 2.

`lower_case_table_names` só pode ser configurado durante a inicialização do servidor. Alterar a configuração `lower_case_table_names` após a inicialização do servidor é proibido.

<table summary="Values for the lower_case_table_names system variable."><col style="width: 10%"/><col style="width: 90%"/><thead><tr> <th>Value</th> <th>Significado</th> </tr></thead><tbody><tr> <td><code>0</code></td> <td>Os nomes de tabela e banco de dados são armazenados no disco usando a letra maiúscula especificada no<a class="link" href="create-table.html" title="15.1.20 CREATE TABLE Statement"><code class="literal">CREATE TABLE</code></a>ou<a class="link" href="create-database.html" title="15.1.12 CREATE DATABASE Statement"><code class="literal">CREATE DATABASE</code></a>As comparações de nomes são sensíveis ao caso. Você não deve definir essa variável para 0 se estiver executando o MySQL em um sistema que tem nomes de arquivos insensíveis ao caso (como Windows ou macOS). Se você forçar essa variável para 0 com<code>--lower-case-table-names=0</code>sobre um sistema de arquivos sensível a maiúsculas e minúsculas e acesso<code>MyISAM</code>Se os nomes de tabela usarem diferentes maiúsculas e minúsculas, pode resultar corrupção de índice.</td> </tr><tr> <td><code>1</code></td> <td>Os nomes das tabelas são armazenados em minúsculas no disco e as comparações de nome não são sensíveis ao caso. O MySQL converte todos os nomes das tabelas para minúsculas no armazenamento e na pesquisa. Esse comportamento também se aplica aos nomes dos bancos de dados e aos aliases das tabelas.</td> </tr><tr> <td><code>2</code></td> <td>Os nomes de tabela e banco de dados são armazenados no disco usando a letra maiúscula especificada no<a class="link" href="create-table.html" title="15.1.20 CREATE TABLE Statement"><code class="literal">CREATE TABLE</code></a>ou<a class="link" href="create-database.html" title="15.1.12 CREATE DATABASE Statement"><code class="literal">CREATE DATABASE</code></a>A declaração, mas o MySQL as converte para minúsculas na pesquisa. As comparações de nome não são sensíveis ao caso. Isso funciona apenas em sistemas de arquivos que não são sensíveis ao caso!<code>InnoDB</code>os nomes de tabelas e de visualizações são armazenados em letras minúsculas, como no<code>lower_case_table_names=1</code>.</td> </tr></tbody></table>

Se você estiver usando MySQL em apenas uma plataforma, normalmente não precisa usar uma configuração `lower_case_table_names` além da padrão. No entanto, você pode encontrar dificuldades se quiser transferir tabelas entre plataformas que diferem na sensibilidade do sistema de arquivos. Por exemplo, no Unix, você pode ter duas tabelas diferentes com os nomes `my_table` e `MY_TABLE`, mas no Windows, esses dois nomes são considerados idênticos. Para evitar problemas de transferência de dados decorrentes da letra do caso das tabelas ou nomes de banco de dados, você tem duas opções:
...
**Exceção**: Se você está usando as tabelas `InnoDB` e está tentando evitar esses problemas de transferência de dados, você deve usar `lower_case_table_names=1` em todas as plataformas para forçar que os nomes sejam convertidos para minúsculas.
...
A sensibilidade ao caso de letra do sistema de arquivos pode afetar as pesquisas em colunas de string das tabelas `INFORMATION_SCHEMA`. Para mais informações, consulte a Seção 12.8.7, “Usando a Colaboração em Pesquisas do SCHEMA_INFORMACAO”.
...
Para os arquivos de dados e índices, a representação exata no disco é específica do mecanismo de armazenamento. Esses arquivos podem ser armazenados no diretório do banco de dados, ou as informações podem ser armazenadas em um arquivo separado. Os dados `InnoDB` são armazenados nos arquivos de dados do InnoDB. Se você estiver usando tablespaces com `InnoDB`, então os arquivos de tablespace específicos que você cria são usados em vez disso.
...
* Todos os caracteres que não são letras, exceto sublinhado (`_`), bem como letras de alfabetos que não têm mapeamento de maiúsculas/minúsculas (como o hebraico), são codificados usando representação hexadecimal, utilizando letras minúsculas para dígitos hexadecimais `a..f`:
...
Os seguintes nomes são reservados e anexados com `@@@` se forem usados em nomes de esquemas ou tabelas:
...
CLOCK$ também é um membro deste grupo de nomes reservados, mas não é anexado com `@@@`, mas sim `@0024` em vez disso. Isso significa que, se CLOCK$ for usado como nome de esquema ou tabela, ele é escrito no sistema de arquivos como `CLOCK@0024`. O mesmo vale para qualquer uso de $ (sinal de dólar) em um nome de esquema ou tabela; ele é substituído por `@0024` no sistema de arquivos.

Nota

Esses nomes também são escritos para `INNODB_TABLES` em suas formas anexadas, mas são escritos para `TABLES` em sua forma não anexada, conforme inserido pelo usuário.
...
A lista a seguir lista as funções no MySQL 8.0 que são afetadas pelo ajuste `IGNORE_SPACE` e listadas como especiais no arquivo fonte `sql/lex.h`. Você pode achar mais fácil tratar o requisito sem espaços em branco como aplicável a todas as chamadas de função.
...
No entanto, habilitar `IGNORE_SPACE` também tem o efeito colateral de que o analisador trata os nomes de funções afetadas como palavras reservadas (veja Seção 11.3, “Palavras-chave e Palavras Reservadas”). Isso significa que um espaço após o nome não mais significa seu uso como um identificador. O nome pode ser usado em chamadas de função com ou sem espaço em branco, mas causa um erro de sintaxe em contexto não expresso, a menos que seja citado. Por exemplo, com `IGNORE_SPACE` habilitado, ambas as seguintes declarações falham com um erro de sintaxe porque o analisador interpreta `count` como uma palavra reservada:
...
* Evite usar nomes de funções em contexto não expresso. Por exemplo, essas declarações usam `count` (um dos nomes de funções afetados afetados por `IGNORE_SPACE`), então elas falham com ou sem espaço em branco após o nome se `IGNORE_SPACE` estiver habilitado:
...
`IF NOT EXISTS` (disponível a partir do MySQL 8.0.29) não tem efeito em tais casos. Consulte a Seção 15.7.4.1, “Instrução CREATE FUNCTION para Funções Carregáveis”, para mais informações.

* Funções embutidas e funções armazenadas

É possível criar uma função armazenada com o mesmo nome que uma função embutida, mas para invocá-la, é necessário qualificá-la com o nome do esquema. Por exemplo, se você criar uma função armazenada com o nome `PI` no esquema `test`, invocá-la como `test.PI()`, porque o servidor resolve `PI()` sem um qualificador como referência à função embutida. O servidor gera um aviso se o nome da função armazenada colidir com o nome de uma função embutida. O aviso pode ser exibido com [[`SHOW WARNINGS`][(show-warnings.html "15.7.7.42 SHOW WARNINGS Statement")]].

`IF NOT EXISTS` (MySQL 8.0.29 e posterior) não tem efeito em tais casos; veja a Seção 15.1.17, “Instruções CREATE PROCEDURE e CREATE FUNCTION”.

* Funções carregáveis e funções armazenadas

É possível criar uma função armazenada com o mesmo nome que uma função carregável existente, ou vice-versa. O servidor gera um aviso se um nome de função armazenada proposto colidir com o nome de uma função carregável existente, ou se um nome de função carregável proposto seria o mesmo que o de uma função armazenada existente. Em qualquer caso, uma vez que ambas as funções existam, é necessário, posteriormente, qualificar a função armazenada com um nome de esquema ao invocá-la; o servidor assume, nesses casos, que o nome não qualificado se refere à função carregável.

Começando com o MySQL 8.0.29, `IF NOT EXISTS` é suportado com declarações `CREATE FUNCTION`, mas não tem efeito em tais casos.

Antes do MySQL 8.0.28, era possível criar uma função armazenada com o mesmo nome que uma função carregável existente, mas não o contrário (Bug #33301931).
...
* Se você já criou uma função carregável com um nome específico e atualizou o MySQL para uma versão que implementa uma nova função embutida com o mesmo nome, a função carregável se torna inacessível. Para corrigir isso, use `DROP FUNCTION` para descartar a função carregável e `CREATE FUNCTION`(create-function.html "15.1.14 CREATE FUNCTION Statement") para recriar a função carregável com um nome diferente e não conflitante. Em seguida, modifique qualquer código afetado para usar o novo nome.

* Se uma nova versão do MySQL implementar uma função embutida ou carregável com o mesmo nome que uma função armazenada existente, você tem duas opções: Renomear a função armazenada para usar um nome não conflitante ou alterar quaisquer chamadas à função que não o façam já para usar um qualificador de esquema (sintaxe `schema_name.func_name()`). Em qualquer caso, modifique o código afetado conforme necessário.