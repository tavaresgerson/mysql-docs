...
- As letras latinas básicas (`a..zA..Z`), os dígitos (`0..9`) e o sublinhado (`_`) são codificados como estão. Consequentemente, sua sensibilidade ao caso depende diretamente das características do sistema de arquivos.
...
- Todos os caracteres não alfanuméricos, exceto sublinhados (`_`), bem como letras de alfabetos que não têm mapeamento de maiúsculas/minúsculas (como o hebraico), são codificados usando representação hexadecimal, com letras minúsculas para algarismos hexadecimais `a..f`:
...
CLOCK$ também é um membro deste grupo de nomes reservados, mas não é anexado com `@@@`, mas sim `@0024`. Ou seja, se CLOCK$ for usado como nome de esquema ou tabela, ele será escrito no sistema de arquivos como `CLOCK@0024`. O mesmo vale para qualquer uso do $ (sinal de dólar) em um nome de esquema ou tabela; ele é substituído por `@0024` no sistema de arquivos.
