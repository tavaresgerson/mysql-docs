...
A parte mais visível de uma declaração SQL é o texto da declaração. A partir do MySQL 8.0.23, os clientes também podem definir atributos de consulta que se aplicam à próxima declaração enviada ao servidor para execução:

* Os atributos são definidos antes de enviar a declaração. * Os atributos existem até o término da execução da declaração, momento em que o conjunto de atributos é limpo.

* Embora os atributos existam, eles podem ser acessados no lado do servidor.

Exemplos das formas como os atributos de consulta podem ser utilizados:

* Uma aplicação web produz páginas que geram consultas de banco de dados, e para cada consulta, deve-se rastrear o URL da página que a gerou.

* Uma aplicação passa informações de processamento adicionais com cada consulta, para uso por um plugin, como um plugin de auditoria ou um plugin de reescrita de consulta.

O MySQL suporta essas capacidades sem o uso de soluções alternativas, como comentários especialmente formatados incluídos em strings de consulta. O restante desta seção descreve como usar o suporte ao atributo de consulta, incluindo os pré-requisitos que devem ser atendidos.

* Definindo e Acessando Atributos de Consulta
* Pré-requisitos para o Uso de Atributos de Consulta
* Funções Carregáveis de Atributo de Consulta

### Definindo e Acessando Atributos de Consulta

Aplicações que utilizam a API C do MySQL definem atributos de consulta chamando a função `mysql_bind_param()`. Veja mysql_bind_param(). Outros conectores MySQL também podem fornecer suporte a atributos de consulta. Veja a documentação dos conectores individuais.
...
Para acessar os atributos de consulta dentro das declarações SQL para as quais os atributos foram definidos, instale o componente `query_attributes` conforme descrito nos Pré-requisitos para Uso de Atributos de Consulta. O componente implementa uma função carregável `mysql_query_attribute_string()` que recebe um argumento de nome de atributo e retorna o valor do atributo como uma string, ou `NULL` se o atributo não existir. Veja Funções Carregáveis de Atributo de Consulta.

Os exemplos a seguir utilizam o comando do cliente **mysql** `query_attributes` para definir pares nome/valor de atributos, e a função `mysql_query_attribute_string()` para acessar os valores dos atributos por nome.
...
* Se várias operações de definição de atributos ocorrerem antes de enviar uma declaração para o servidor para execução, a operação de definição mais recente se aplica e substitui os atributos definidos em operações anteriores.
...
Para acessar os atributos de consulta dentro das declarações SQL para as quais os atributos foram definidos, o componente `query_attributes` deve ser instalado. Faça isso usando esta declaração:
...
O componente `query_attributes` acessa atributos de consulta para implementar uma função `mysql_query_attribute_string()`. Veja a Seção 7.5.4, “Componentes de Atributos de Consulta”.
...
### Carregamento de funções carregáveis de atributo de consulta

* `mysql_query_attribute_string(name)`

As aplicações podem definir atributos que se aplicam à próxima consulta enviada ao servidor. A função `mysql_query_attribute_string()`, disponível a partir do MySQL 8.0.23, retorna um valor de atributo como uma string, dado o nome do atributo. Esta função permite que uma consulta acesse e incorpore valores dos atributos que se aplicam a ela.
...
O exemplo a seguir usa o comando do cliente **mysql** `query_attributes` para definir atributos de consulta que podem ser recuperados por `mysql_query_attribute_string()`. O `SELECT` mostra que a recuperação de um atributo inexistente (`n3`) retorna `NULL`.
