...
* De uma sequência `--` até o final da linha. No MySQL, o estilo de comentário `--` (duas barras) exige que a segunda barra seja seguida por pelo menos um caractere de espaço ou controle, como um espaço ou uma tabulação. Essa sintaxe difere ligeiramente da sintaxe padrão de comentário SQL, conforme discutido na Seção 1.6.2.4, “'--' como o início de um comentário”.

* De uma sequência `/*` para a sequência `*/` a seguir, como na linguagem de programação C. Essa sintaxe permite que um comentário se estenda em várias linhas, pois o início e o fechamento das sequências não precisam estar na mesma linha.
...
Comentários aninhados não são suportados e são desaconselhados; espere que eles sejam removidos em um lançamento futuro do MySQL. (Em algumas condições, comentários aninhados podem ser permitidos, mas geralmente não são, e os usuários devem evitá-los.)
...
O número da versão utiliza o formato *`Mmmrr`*, onde *`M`* é uma versão principal, *`mm`* é uma versão menor de duas dígitos e *`rr`* é um número de lançamento de duas dígitos. Por exemplo: Em uma declaração que será executada apenas por uma versão do servidor MySQL 8.0.31 ou posterior, use `80031` no comentário.

O número da versão deve ser seguido por pelo menos um caractere de espaço em branco (ou o final do comentário). A partir do MySQL 8.0.34, se essa condição não for atendida, a declaração aciona um aviso; você deve esperar que esse requisito seja rigorosamente aplicado em uma versão futura do MySQL.

A sintaxe de comentário que acabou de ser descrita se aplica à forma como o servidor **mysqld** analisa as declarações SQL. O programa cliente **mysql** também realiza algum tipo de análise de declarações antes de enviá-las ao servidor. (Isso é feito para determinar os limites das declarações dentro de uma linha de entrada com várias declarações.) Para informações sobre as diferenças entre os analisadores do servidor e do cliente **mysql**, consulte a Seção 6.5.1.6, “Dicas do cliente **mysql” (em inglês).
