...
Se tiver problemas ou necessitar de informações específicas sobre RPM, consulte a secção 2.1.4.4, " Verificação de assinatura utilizando RPM " .
...
Isso é normal, pois dependem da sua configuração e configuração. Aqui estão as explicações para esses avisos:
...
- - AVISO: Esta chave não é certificada com uma assinatura confiável! Não há indicação de que a assinatura pertence ao proprietário. \*: Isso se refere ao seu nível de confiança em sua crença de que você possui nossa chave pública real. Esta é uma decisão pessoal. Idealmente, um desenvolvedor do MySQL lhe daria a chave pessoalmente, mas mais comumente, você a baixou. O download foi adulterado? Provavelmente não, mas essa decisão depende de você. Configurar uma rede de confiança é um método para confiar neles.
