...
- A verificação de Mostrar Opções Avançadas permite que opções de registro adicionais sejam definidas. Isso inclui a definição de caminhos de arquivo personalizados para o registro de erros, o registro geral, o registro de consulta lenta (incluindo a configuração de segundos necessários para executar uma consulta) e o registro binário.
...
- Conectividade
  ...
  - TCP/IP: Esta opção é selecionada por padrão. Você pode desativar TCP/IP Networking para permitir apenas conexões de host local. Com a opção de conexão TCP/IP selecionada, você pode modificar os seguintes itens:
    ...
    Se um número de porta já estiver em uso, você verá o ícone de erro (![](images/mi-info-symbol.png)) ao lado do valor padrão e o Próximo será desativado até que você forneça um novo número de porta.
...
- MySQL Enterprise Firewall (apenas Edição Enterprise)
...
- Senha da conta raiz
  ...
  A atribuição de uma senha de raiz é necessária e você será solicitado a ela quando reconfigurar com o MySQL Configurator no futuro. A força da senha é avaliada quando você repete a senha na caixa fornecida. Para informações descritivas sobre requisitos ou status de senha, mova o ponteiro do mouse sobre o ícone de informações (![](images/mi-info-symbol.png)) quando ele aparecer.
- Contas de utilizador MySQL (opcional)

  Clique em Adicionar usuário ou Editar usuário para criar ou modificar contas de usuário MySQL com funções predefinidas. Em seguida, insira as credenciais de conta necessárias:
  ...
  - Papel: Cada papel predefinido, como `DB Admin`, é configurado com seu próprio conjunto de privilégios. Por exemplo, o papel `DB Admin` tem mais privilégios do que o papel `DB Designer`. A lista suspensa de papéis contém uma descrição de cada papel.
  ...
  \*\* MySQL Configurator Commercial Release Only:\*\* MySQL Enterprise Edition for Windows, um produto comercial, também suporta um método de autenticação que executa autenticação externa no Windows.

  Para criar uma nova conta do MySQL que usa autenticação do Windows, digite o nome do usuário e, em seguida, selecione um valor para anfitrião e função. Clique em autenticação do Windows para ativar o plugin `authentication_windows`. Na área de Tokens de Segurança do Windows, digite um token para cada usuário do Windows (ou grupo) que pode autenticar com o nome de usuário do MySQL. As contas do MySQL podem incluir tokens de segurança para usuários locais do Windows e usuários do Windows que pertencem a um domínio. Vários tokens de segurança são separados pelo caractere ponto e vírgula (`;`) e usam o seguinte formato para contas locais e de domínio:

  - Conta local

    Insira o nome de usuário do Windows simples como token de segurança para cada usuário ou grupo local; por exemplo, `finley;jeffrey;admin`.
  - Conta de domínio
  ...
  A autenticação do Windows permite testar todos os tokens de segurança cada vez que você adiciona ou modifica um token. Clique em Test Security Tokens para validar (ou revalidar) cada token. Tokens inválidos geram uma mensagem de erro descritiva junto com um ícone vermelho `X` e texto de token vermelho. Quando todos os tokens são resolvidos como válidos (texto verde sem um ícone `X`), você pode clicar em OK para salvar as alterações.

##### 2.3.2.1.4 Serviço Windows

Na plataforma Windows, o servidor MySQL pode ser executado como um serviço nomeado gerenciado pelo sistema operacional e configurado para iniciar automaticamente quando o Windows é iniciado. Alternativamente, você pode configurar o servidor MySQL para ser executado como um programa executável que requer configuração manual.
...
- Faça com que o configurador do MySQL omita fazer alterações de permissão de arquivo durante a operação de configuração.
...
- Identificação do servidor
  ...
  Defina o identificador exclusivo usado em uma topologia de replicação. Se o registro binário estiver habilitado, você deve especificar um ID de servidor. O valor de ID padrão depende da versão do servidor. Para mais informações, consulte a descrição da variável de sistema `server_id`.
- Caso dos nomes das tabelas
  ...
  - Maiúscula e minúscula
    ...
    Define o valor da opção `lower_case_table_names` em 1 (padrão), no qual os nomes das tabelas são armazenados em minúsculas no disco e as comparações não são sensíveis a maiúsculas e minúsculas.
...
Opcionalmente instale bancos de dados de amostra que incluam dados de teste para ajudar a desenvolver aplicativos com o MySQL. As opções incluem os bancos de dados sakila e mundo.
