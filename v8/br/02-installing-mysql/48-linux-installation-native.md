...
Os pacotes nativos são muitas vezes várias versões atrás do lançamento atualmente disponível. Você também normalmente não é capaz de instalar lançamentos de marco de desenvolvimento (DMRs), uma vez que estes não são geralmente disponibilizados nos repositórios nativos. Antes de prosseguir, recomendamos que você confira as outras opções de instalação descritas na Seção 2.5, Instalar MySQL no Linux.

Instruções específicas de distribuição são mostradas abaixo:
...
- **Debian, Ubuntu, Kubuntu**
  ...
  O serviço é automaticamente adicionado aos níveis de execução 2, 3 e 4, com scripts de parada nos níveis de execução simples, de desligamento e de reinicialização.
