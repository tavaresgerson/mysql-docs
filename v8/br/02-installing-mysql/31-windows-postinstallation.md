...
Existem ferramentas GUI que executam a maioria das tarefas descritas nesta seção, incluindo:
...
Alternativamente, adicione o diretório `bin` à sua configuração de variável de ambiente `PATH`. Isso permite que seu interpretador de comandos encontre os programas do MySQL corretamente, para que você possa executar um programa digitando apenas seu nome, não seu nome de caminho. Veja Seção 2.3.3.7, Personalização do PATH para Ferramentas do MySQL.
