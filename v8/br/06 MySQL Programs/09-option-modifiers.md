#### 6.2.2.4 Modificadores de Opções de Programa

Algumas opções são `boolean` e controlam o comportamento que pode ser ativado ou desativado. Por exemplo, o cliente `mysql` suporta uma opção `--column-names` que determina se os nomes dos colunas devem ser exibidos no início dos resultados da consulta ou não. Por padrão, essa opção está ativada. No entanto, você pode querer desativá-la em algumas situações, como ao enviar a saída do `mysql` para outro programa que espera ver apenas dados e não uma linha de cabeçalho inicial.

Para desativar os nomes das colunas, você pode especificar a opção usando qualquer uma dessas formas:
...
Se uma opção for prefixada por `--loose`, um programa não sai com um erro se não reconhecer a opção, mas em vez disso emite apenas uma mensagem de aviso:
...
O prefixo `--maximum` está disponível apenas para `mysqld` e permite que um limite seja colocado sobre o tamanho máximo que os programas cliente podem definir as variáveis do sistema de sessão. Para fazer isso, use um prefixo `--maximum` com o nome da variável. Por exemplo, `--maximum-max_heap_table_size=32M` impede que qualquer cliente aumente o limite do tamanho da tabela heap para mais de 32M.
