...
* Ao usar `--recover` ou `--sort-recover` (mas não ao usar `--safe-recover`), você precisa de espaço em disco para a ordenação. Esse espaço é alocado no diretório temporário (especificado por `TMPDIR` ou `--tmpdir=dir_name`). A seguinte fórmula fornece a quantidade de espaço necessária:
