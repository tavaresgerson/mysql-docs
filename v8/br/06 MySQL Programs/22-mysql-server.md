...
As distribuições do MySQL em sistemas Unix e similares incluem um script chamado `mysql.server`, que inicia o servidor MySQL usando `mysqld_safe`. Ele pode ser usado em sistemas como Linux e Solaris que utilizam diretórios de execução de estilo System V para iniciar e parar serviços do sistema. Ele também é usado pelo Item de Inicialização do macOS para MySQL.
...
Para iniciar ou parar o servidor manualmente usando o script `mysql.server`, invólcá-lo a partir da linha de comando com os argumentos `start` ou `stop`:
...
Para iniciar e parar o MySQL automaticamente no seu servidor, você deve adicionar os comandos `start` e `stop` aos locais apropriados nos seus arquivos `/etc/rc*`:

* Se você usar o pacote RPM do servidor Linux (`MySQL-server-VERSION.rpm`) ou uma instalação de pacote nativo do Linux, o script `mysql.server` pode estar instalado no diretório `/etc/init.d` com o nome `mysqld` ou `mysql`. Consulte a Seção 2.5.4, “Instalando o MySQL no Linux Usando Pacotes RPM da Oracle”, para obter mais informações sobre os pacotes RPM do Linux.
...
* Como alternativa à configuração anterior, alguns sistemas operacionais também usam `/etc/rc.local` ou `/etc/init.d/boot.local` para iniciar serviços adicionais no início do sistema. Para iniciar o MySQL usando esse método, adicione um comando como o seguinte ao arquivo de inicialização apropriado:
...
* Para outros sistemas, consulte a documentação do seu sistema operacional para saber como instalar scripts de inicialização.
 ...
 `mysql.server` lê as opções dos seções `[mysql.server]` e `[mysqld]` dos arquivos de opções. Para compatibilidade com versões anteriores, também lê as seções `[mysql_server]`, mas para ser atual, você deve renomear essas seções para `[mysql.server]`.
...
<table><thead><tr><th>Nome da Opção</th> <th>Descrição</th> <th>Tipo</th> </tr></thead><tbody><tr><th><code>basedir</code></th> <td>Caminho para o diretório de instalação do MySQL</td> <td>Nome de diretório</td> </tr><tr><th><code>datadir</code></th> <td>Caminho para o diretório de dados do MySQL</td> <td>Nome de diretório</td> </tr><tr><th><code>pid-file</code></th> <td>Arquivo no qual o servidor deve escrever seu ID de processo</td> <td>Nome de arquivo</td> </tr><tr><th><code>service-startup-timeout</code></th> <td>Quanto tempo esperar para a inicialização do servidor</td> <td>Inteiro</td> </tr></tbody></table>
