...
Se a string de busca corresponder a vários itens, o `mysql` mostra uma lista dos tópicos correspondentes:
...
Use um tópico como string de busca para ver a entrada de ajuda para esse tópico:
...
A string de busca pode conter os caracteres curinga `%` e `_`. Esses têm o mesmo significado que as operações de correspondência de padrões realizadas com o operador `LIKE`. Por exemplo, `HELP rep%` retorna uma lista de tópicos que começam com `rep`:
