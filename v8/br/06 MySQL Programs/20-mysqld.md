...
Alguns pacotes de instalação contêm uma versão de depuração do servidor chamada `mysqld-debug`. Inicie essa versão em vez de `mysqld` para suporte de depuração, verificação de alocação de memória e suporte a arquivos de registro (consulte  Seção 7.9.1.2, “Criando Arquivos de Registro”).
