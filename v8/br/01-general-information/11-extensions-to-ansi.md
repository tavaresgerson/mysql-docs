...
Se você adicionar um número de versão após o caractere `!`, a sintaxe dentro do comentário é executada somente se a versão do MySQL for maior ou igual ao número de versão especificado. A cláusula `KEY_BLOCK_SIZE` no comentário seguinte é executada somente por servidores do MySQL 5.1.10 ou superior:
...
- Sintase da instrução SQL
  ...
  - As instruções `CREATE DATABASE`, `DROP DATABASE`, e `ALTER DATABASE`. Ver Secção 15.1.12, CRIAR INFORMAÇÃO DATABASE, Secção 15.1.24, DROP DATABASE, e Secção 15.1.2, ALTER DATABASE.
  - A instrução `DO`.
  - `EXPLAIN SELECT` para obter uma descrição de como as tabelas são processadas pelo optimizador de consulta.
  - As instruções `FLUSH` e `RESET`.
  - A instrução `SET`. Ver Seção 15.7.6.1, Sintaxe SET para Atribuição de Variáveis.
  ...
  - Utilização de `LOAD DATA`. Em muitos casos, esta sintaxe é compatível com o `LOAD DATA` da Oracle. Ver Seção 15.2.9, LOAD DATA Statement.
  - Utilização de `RENAME TABLE`. Ver Secção 15.1.36, RENAME TABLE Statement.
  ...
  - Utilização de nomes de índice, índices em um prefixo de uma coluna e utilização de `INDEX` ou `KEY` em instruções `CREATE TABLE`.
  - Utilização de `TEMPORARY` ou `IF NOT EXISTS` com `CREATE TABLE`.
  - Utilização de `IF EXISTS` com `DROP TABLE` e `DROP DATABASE`.
  - A capacidade de eliminar várias tabelas com uma única instrução `DROP TABLE`.
  ...
  - Utilização de `INTO OUTFILE` ou `INTO DUMPFILE` em instruções `SELECT`.
...
- Funções e operadores
  ...
  - Para facilitar a migração de usuários de outros ambientes SQL, o MySQL Server suporta aliases para muitas funções. Por exemplo, todas as funções string suportam tanto a sintaxe SQL padrão quanto a sintaxe ODBC.
