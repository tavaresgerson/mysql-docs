...
* Suporte para até 64 índices por tabela. Cada índice pode consistir de 1 a 16 colunas ou partes de colunas. A largura máxima do índice para tabelas `InnoDB` é de 767 bytes ou 3072 bytes. A largura máxima do índice para tabelas `MyISAM` é de 1000 bytes. Um índice pode usar um prefixo de uma coluna para tipos de coluna `CHAR`, `VARCHAR`, `BLOB` ou `TEXT`.
