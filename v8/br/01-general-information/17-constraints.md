...
Várias opções de modo SQL estão disponíveis para fornecer maior controle sobre o manuseio de valores de dados ruins e se deve continuar a execução de instruções ou abortar quando ocorrem erros. Usando essas opções, você pode configurar o MySQL Server para agir de uma forma mais tradicional que é como outros DBMSs que rejeitam entrada inadequada. O modo SQL pode ser definido globalmente na inicialização do servidor para afetar todos os clientes. Clientes individuais podem definir o modo SQL no tempo de execução, o que permite a cada cliente selecionar o comportamento mais apropriado para seus requisitos.
