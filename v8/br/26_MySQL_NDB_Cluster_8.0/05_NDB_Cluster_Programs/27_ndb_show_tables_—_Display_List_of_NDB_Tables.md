...
O comando **ndb\_show\_tables** exibe uma lista de todos os objetos do banco de dados `NDB` no cluster. Por padrão, isso inclui não apenas as tabelas criadas pelo usuário e as tabelas do sistema `NDB`, mas também os índices específicos do `NDB`, gatilhos internos e objetos de dados do disco do NDB Cluster.
