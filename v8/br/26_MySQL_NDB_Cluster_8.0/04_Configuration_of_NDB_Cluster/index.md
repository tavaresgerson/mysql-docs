...
Um servidor MySQL que faz parte de um NDB Cluster difere de um servidor MySQL normal (não agrupado) em um aspecto principal: ele utiliza o mecanismo de armazenamento `NDB`. Esse mecanismo também é chamado de `NDBCLUSTER`, embora `NDB` seja preferido.
...
Para obter informações gerais sobre a instalação do NDB Cluster, consulte a Seção 25.3, “Instalação do NDB Cluster”.
