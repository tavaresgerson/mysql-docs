...
- `SHOW ENGINE NDB STATUS`, `SHOW ENGINE NDBCLUSTER STATUS`
  ...
  Consulte a Seção 15.7.7.15, “Declaração SHOW ENGINE”, para obter um exemplo de uso e informações mais detalhadas.
...
- `SELECT * FROM INFORMATION_SCHEMA.ENGINES [WHERE ENGINE LIKE 'NDB%']`
  ...
  Isto é o equivalente a `SHOW ENGINES`, mas utiliza a tabela `ENGINES` do banco de dados `INFORMATION_SCHEMA`. Ao contrário do caso da instrução `SHOW ENGINES`, é possível filtrar os resultados usando uma cláusula `LIKE`, e selecionar colunas específicas para obter informações que podem ser úteis em scripts. Por exemplo, a seguinte consulta mostra se o servidor foi construído com suporte a `NDB` e, se sim, se ele está habilitado:
  ...
  Se o suporte ao `NDB` não estiver habilitado, a consulta anterior retornará um conjunto vazio. Consulte a Seção 28.3.13, “A Tabela INFORMATION\_SCHEMA ENGINES”, para obter mais informações.
...
- `SHOW VARIABLES LIKE 'NDB%'`
  ...
  Consulte a Seção 7.1.8, “Variáveis do Sistema do Servidor”, para obter mais informações.

- `SELECT * FROM performance_schema.global_variables WHERE VARIABLE_NAME LIKE 'NDB%'`

  Esta declaração é equivalente à declaração `SHOW VARIABLES` descrita no item anterior e fornece uma saída quase idêntica, conforme mostrado aqui:
  ...
  Para obter mais informações, consulte a Seção 29.12.15, “Tabelas de Variáveis de Status do Schema de Desempenho”, e a Seção 7.1.8, “Variáveis do Sistema do Servidor”.
...
- `SELECT * FROM performance_schema.global_status WHERE VARIABLE_NAME LIKE 'NDB%'`
  ...
  Esta declaração fornece uma saída semelhante à declaração `SHOW STATUS` discutida no item anterior. Ao contrário do caso com `SHOW STATUS`, é possível usar declarações `SELECT` para extrair valores em SQL para uso em scripts de monitoramento e automação.
...
- `SELECT * FROM INFORMATION_SCHEMA.PLUGINS WHERE PLUGIN_NAME LIKE 'NDB%'`
  ...
  Esta declaração exibe informações da tabela Schema de Informações `PLUGINS` sobre os plugins associados ao NDB Cluster, como versão, autor e licença, conforme mostrado aqui:
