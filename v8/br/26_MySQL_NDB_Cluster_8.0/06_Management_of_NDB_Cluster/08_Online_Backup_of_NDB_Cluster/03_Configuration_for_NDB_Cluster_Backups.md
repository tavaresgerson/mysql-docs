...
Além disso, `CompressedBackup` faz com que `NDB` use a compressão ao criar e escrever em arquivos de backup.
...
No NDB 8.0.22 e versões posteriores, você pode aplicar criptografia aos arquivos de backup habilitando `RequireEncryptedBackup`. Quando este parâmetro é definido para 1, os backups não podem ser criados sem especificar `ENCRYPT PASSWORD=password` como parte de um comando `START BACKUP`.
