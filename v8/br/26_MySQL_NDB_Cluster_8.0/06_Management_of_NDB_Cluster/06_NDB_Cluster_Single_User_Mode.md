...
O modo de usuário único permite que o administrador do banco de dados restrinja o acesso ao sistema do banco de dados a um único nó da API, como um servidor MySQL (nó SQL) ou uma instância do **ndb\_restore**. Ao entrar no modo de usuário único, as conexões a todos os outros nós da API são fechadas de forma suave e todas as transações em execução são abortadas. Nenhuma nova transação é permitida para começar.

Depois que o cluster entrar no modo de usuário único, apenas o nó da API designado terá acesso ao banco de dados.

Você pode usar o comando `ALL STATUS` no cliente **ndb\_mgm** para ver quando o clúster entrou no modo de usuário único. Você também pode verificar a coluna `status` da tabela `ndbinfo.nodes` (consulte a Seção 25.6.16.47, “A tabela de nós ndbinfo”, para obter mais informações).
...
O comando `EXIT SINGLE USER MODE` altera o estado dos nós de dados do cluster do modo de um único usuário para o modo normal. Os nós da API — como Servidores MySQL — que estão aguardando uma conexão (ou seja, aguardando que o cluster esteja pronto e disponível) podem se conectar novamente. O nó da API denominado como o nó de um único usuário continua funcionando (se ainda estiver conectado) durante e após a mudança de estado.
...
- Método 1:
  ...
  2. Emita o comando `EXIT SINGLE USER MODE`
