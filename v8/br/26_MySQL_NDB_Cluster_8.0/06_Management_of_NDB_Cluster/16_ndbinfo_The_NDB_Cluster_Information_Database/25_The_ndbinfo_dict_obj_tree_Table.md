#### 25.6.16.25 Tabela ndbinfo dict\_obj\_tree

A tabela `dict_obj_tree` oferece uma visualização baseada em árvore das informações da tabela `dict_obj_info`. Isso é destinado principalmente para uso em testes, mas pode ser útil na visualização de hierarquias de objetos de banco de dados `NDB`.

A tabela `dict_obj_tree` contém as seguintes colunas:

- `type`

  Tipo do objeto `DICT`; faça a junção em `dict_obj_types` para obter o nome do tipo de objeto
...
- `parent_type`
  ...
  O tipo de objeto `DICT` do objeto pai deste objeto; faça a junção em `dict_obj_types` para obter o nome do tipo de objeto
...
- `parent_name`
  ...
  Nome completo do objeto pai deste objeto; o mesmo que a coluna `fq_name` da tabela `dict_obj_info`
...
- `root_name`
  ...
  Nome completo do objeto raiz; o mesmo da coluna `fq_name` da tabela `dict_obj_info`
...
- `indented_name`
  ...
  O `name` precedido por uma seta para a direita (representado como `->`) com um número de espaços que o precedem, correspondendo à profundidade do objeto na hierarquia

A coluna `path` é útil para obter um caminho completo para um objeto de banco de dados `NDB` específico em uma única linha, enquanto a coluna `indented_name` pode ser usada para obter um layout semelhante a uma árvore de informações completas de hierarquia para um objeto desejado.

*Exemplo*: Supondo a existência de um banco de dados `test` e a ausência de uma tabela existente com o nome `t1` neste banco de dados, execute a seguinte instrução SQL:
...
Você pode ver os caminhos para todos os objetos dependentes desta tabela usando o caminho da tabela como nome raiz em uma consulta como esta:
...
Para obter uma visão hierárquica da tabela `t1` com todos os seus objetos dependentes, execute uma consulta semelhante à seguinte, que seleciona o nome indentável de cada objeto que tem `test/def/t1` como o nome do seu objeto raiz:
...
Agora você pode obter informações hierárquicas para a tabela, o espaço de armazenamento de tabelas e o grupo de arquivos de log da seguinte forma:
