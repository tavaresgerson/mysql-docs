...
A coluna `uptime` mostra o tempo em segundos que esse nó está em execução desde que foi iniciado ou reiniciado pela última vez. Esse é um valor `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT"). Esse número inclui o tempo realmente necessário para iniciar o nó; em outras palavras, esse contador começa a contar a partir do momento em que **ndbd** ou \*\*ndbmtd") é invocado pela primeira vez; assim, mesmo para um nó que ainda não terminou de iniciar, `uptime` pode mostrar um valor diferente de zero.

A coluna `status` mostra o status atual do nó. Isso é um dos seguintes: `NOTHING`, `CMVMI`, `STARTING`, `STARTED`, `SINGLEUSER`, `STOPPING_1`, `STOPPING_2`, `STOPPING_3` ou `STOPPING_4`. Quando o status é `STARTING`, você pode ver a fase de início atual na coluna `start_phase` (veja mais adiante nesta seção). `SINGLEUSER` é exibido na coluna `status` para todos os nós de dados quando o clúster está no modo de usuário único (veja a Seção 25.6.6, “Modo de Usuário Único do Clúster NDB”). Ver um dos estados `STOPPING` não significa necessariamente que o nó está desligando, mas pode significar que ele está entrando em um novo estado. Por exemplo, se você colocar o clúster no modo de usuário único, às vezes você pode ver nós de dados relatando seu estado brevemente como `STOPPING_2` antes que o status mude para `SINGLEUSER`.

A coluna `start_phase` utiliza a mesma faixa de valores que os usados na saída do comando do cliente do NDB Cluster **ndb\_mgm** `node_id STATUS` (consulte a Seção 25.6.1, “Comandos no Cliente de Gerenciamento do NDB Cluster”). Se o nó não estiver iniciando atualmente, essa coluna exibe `0`. Para uma lista das fases de início do NDB Cluster com descrições, consulte a Seção 25.6.4, “Resumo das Fases de Início do NDB Cluster”.

A coluna `config_generation` mostra qual versão da configuração do cluster está em vigor em cada nó de dados. Isso pode ser útil ao realizar um reinício contínuo do cluster para fazer alterações nos parâmetros de configuração. Por exemplo, a partir do resultado da seguinte declaração `SELECT`, você pode ver que o nó 3 ainda não está usando a versão mais recente da configuração do cluster (`6`) embora os nós 1, 2 e 4 estejam fazendo isso:
...
Se você desligar um dos nós, apenas os nós que ainda estão em execução serão representados na saída desta declaração `SELECT`, conforme mostrado aqui:
