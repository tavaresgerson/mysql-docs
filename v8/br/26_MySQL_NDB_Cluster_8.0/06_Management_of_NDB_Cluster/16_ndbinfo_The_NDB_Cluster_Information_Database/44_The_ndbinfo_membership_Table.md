#### 25.6.16.44 Tabela de associação ndbinfo
...
O ID do nó e o ID do grupo de nó são os mesmos relatados pelo **ndb\_mgm -e "SHOW"**.
...
**Figura 25.6. Disposição Circular dos Nodos do Agrupamento NDB**
...
O nó `president` é o nó que o nó atual considera responsável por definir um árbitro (veja as fases de início do NDB Cluster). Se o presidente falhar ou se desconectar, o nó atual espera que o nó cujo ID está exibido na coluna `successor` se torne o novo presidente. A coluna `succession_order` mostra o lugar na fila de sucessão que o nó atual considera que tem.

Em um cluster NDB normal, todos os nós de dados devem ver o mesmo nó como `president` e o mesmo nó (exceto o presidente) como seu `successor`. Além disso, o atual presidente deve se ver como `1` na ordem de sucessão, o nó `successor` deve se ver como `2`, e assim por diante.

Todos os nós devem exibir os mesmos valores de `arb_ticket` e os mesmos valores de `arb_state`. Os valores possíveis de `arb_state` são `ARBIT_NULL`, `ARBIT_INIT`, `ARBIT_FIND`, `ARBIT_PREP1`, `ARBIT_PREP2`, `ARBIT_START`, `ARBIT_RUN`, `ARBIT_CHOOSE`, `ARBIT_CRASH` e `UNKNOWN`.

`arb_connected` indica se este nó está conectado ao nó mostrado como `arbitrator` deste nó.
