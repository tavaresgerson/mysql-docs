...
A tabela `tc_time_track_stats` fornece informações de rastreamento de tempo obtidas das instâncias do bloco `DBTC` (TC) nos nós de dados, por meio do acesso aos nós API `NDB`. Cada instância de TC rastreia as latências para um conjunto de atividades que realiza em nome dos nós API ou de outros nós de dados; essas atividades incluem transações, erros de transação, leituras de chaves, escritas de chaves, operações de índice único, operações de chave falhas de qualquer tipo, varreduras, varreduras falhas, varreduras de fragmentação e varreduras de fragmento falhas.

Um conjunto de contadores é mantido para cada atividade, cada contador cobrindo uma faixa de latências de até um limite superior. Ao final de cada atividade, sua latência é determinada e o contador apropriado é incrementado. `tc_time_track_stats` apresenta essas informações como linhas, com uma linha para cada instância do seguinte:
...
- `transactions`
  ...
  Com base na duração das transações bem-sucedidas, desde o início até o envio do commit `ACK`, rastreadas contra as APIs ou nós de dados que as solicitam. As transações sem estado não são incluídas.
...
A coluna `block_instance` fornece o número da instância do bloco do kernel `DBTC`. Você pode usar isso junto com o nome do bloco para obter informações sobre threads específicas da tabela `threadblocks`.
