#### 25.6.16.54 Tabela servidor\_operações ndbinfo

A tabela `server_operations` contém entradas para todas as operações em andamento `NDB` nas quais o nó SQL atual (MySQL Server) está envolvido. Ela é, de fato, um subconjunto da tabela `cluster_operations`, na qual as operações para outros nós SQL e API não são exibidas.
...
O `mysql_connection_id` é o mesmo ID de conexão ou sessão exibido na saída do `SHOW PROCESSLIST`. Ele é obtido a partir da tabela `INFORMATION_SCHEMA` `NDB_TRANSID_MYSQL_CONNECTION_MAP`.
