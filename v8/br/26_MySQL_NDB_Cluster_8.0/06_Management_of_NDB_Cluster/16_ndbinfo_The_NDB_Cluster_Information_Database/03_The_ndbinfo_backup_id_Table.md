...
A tabela `backup_id` contém uma única coluna `id`, que corresponde a um ID de backup obtido usando o comando do cliente **ndb\_mgm** `START BACKUP`. Esta tabela contém uma única linha.

*Exemplo*: Suponha a seguinte sequência de comandos `START BACKUP` emitidos no cliente de gerenciamento do NDB, sem que outros backups tenham sido realizados desde que o clúster foi iniciado pela primeira vez:
...
Depois disso, a tabela `backup_id` contém a única linha mostrada aqui, usando o cliente **mysql**:
