...
A tabela `cluster_operations` fornece uma visão por operação (op de chave primária declarada) de toda a atividade no NDB Cluster do ponto de vista dos blocos de gerenciamento de dados locais (LQH) (veja o Bloco DBLQH).
...
As colunas `block_instance` e `tc_block_instance` fornecem, respectivamente, os números de instâncias do bloco `DBLQH` e `DBTC`. Você pode usar essas informações juntamente com os nomes dos blocos para obter informações sobre threads específicas da tabela `threadblocks`.
