...
1. Obtenha a hora do ponto de verificação global mais recente (GCP). Ou seja, você precisa determinar a época mais recente da tabela `ndb_apply_status` no clúster de replicação, que pode ser encontrada usando a seguinte consulta:
...
2. Usando as informações obtidas da consulta mostrada no Passo 1, obtenha os registros correspondentes da tabela `ndb_binlog_index` no cluster de origem.
   ...
   Estes são os registros salvos na fonte desde o falecimento do canal de replicação primário. Usamos uma variável de usuário `@latest` aqui para representar o valor obtido no Passo 1. Claro, não é possível que uma instância do **mysqld** acesse diretamente variáveis de usuário definidas em outra instância do servidor. Esses valores devem ser "conectados" à segunda consulta manualmente ou por uma aplicação.

   Importante

   Você deve garantir que a replicação do **mysqld** seja iniciada com `--slave-skip-errors=ddl_exist_errors` antes de executar `START REPLICA`. Caso contrário, a replicação pode parar com erros de DDL duplicados.
...
4. Agora você pode iniciar a replicação no canal secundário emitindo o comando apropriado no replica **mysqld**:
   ...
   No NDB 8.0.22 ou posterior, você também pode usar a seguinte declaração:
