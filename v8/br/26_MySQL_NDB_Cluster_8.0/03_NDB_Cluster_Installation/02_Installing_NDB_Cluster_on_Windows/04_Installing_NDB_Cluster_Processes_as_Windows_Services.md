#### 25.3.2.4 Instalar os processos do NDB Cluster como serviços do Windows
...
A opção `--install` deve ser usada primeiro, antes de qualquer outra opção que possa ser especificada para o **ndb\_mgmd.exe**. No entanto, é preferível especificar essas opções em um arquivo de opções em vez disso. Se o seu arquivo de opções não estiver em um dos locais padrão, conforme mostrado na saída do **ndb\_mgmd.exe** `--help`, você pode especificar a localização usando a opção `--config-file`.
...
Se você instalou o serviço usando um nome de serviço diferente do padrão, passe o nome do serviço como o valor da opção **ndb\_mgmd.exe** `--remove`, da seguinte forma:
...
A instalação de um processo de nó de dados de um cluster NDB Cluster como um serviço do Windows pode ser feita de maneira semelhante, usando a opção `--install` para **ndbd.exe** (ou **ndbmtd.exe**)), conforme mostrado aqui:
...
Assim como o **ndb\_mgmd.exe** (e o **mysqld.exe**), ao instalar o **ndbd.exe** como um serviço do Windows, você também pode especificar um nome para o serviço como o valor de `--install` e, em seguida, usá-lo ao iniciar ou parar o serviço, da seguinte forma:
...
Alternativamente, você pode passar o nome do serviço como o valor da opção `ndbd.exe` `--remove`, conforme mostrado aqui:
