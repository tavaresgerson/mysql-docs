...
- Cada nó de dados ou nó SQL requer um arquivo `my.cnf` que fornece duas informações: uma string de conexão que indica ao nó onde encontrar o nó de gerenciamento e uma linha que instrui o servidor MySQL neste host (a máquina que hospeda o nó de dados) a habilitar o mecanismo de armazenamento `NDBCLUSTER`.
  ...
  Para obter mais informações sobre as cadeias de conexão, consulte a Seção 25.4.3.3, “Cadeias de conexão do cluster NDB”.

- O nó de gerenciamento precisa de um arquivo `config.ini` que indique quantas réplicas de fragmentos devem ser mantidas, quanto memória deve ser alocada para dados e índices em cada nó de dados, onde encontrar os nós de dados, onde salvar os dados no disco em cada nó de dados e onde encontrar quaisquer nós SQL.
...
Para cada nó de dados e nó de SQL na configuração do nosso exemplo, `my.cnf` deve parecer assim:
...
Depois que todos os arquivos de configuração forem criados e essas opções mínimas forem especificadas, você estará pronto para iniciar o clúster e verificar se todos os processos estão em execução. Discutimos como isso é feito na Seção 25.3.4, “Inicialização do Clúster NDB”.
