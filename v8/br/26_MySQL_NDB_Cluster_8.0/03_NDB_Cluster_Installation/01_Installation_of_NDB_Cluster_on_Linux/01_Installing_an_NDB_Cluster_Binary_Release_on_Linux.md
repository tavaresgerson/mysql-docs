#### 25.3.1.1 Instalar uma versão binária de um cluster NDB no Linux
...
Para configurar um clúster usando binários pré-compilados, o primeiro passo no processo de instalação de cada host do clúster é baixar o arquivo binário da página de downloads do NDB Cluster. (Para a versão mais recente do NDB 8.0 de 64 bits, este é `mysql-cluster-gpl-8.0.43-linux-glibc2.12-x86_64.tar.gz`.) Suponhamos que você tenha colocado esse arquivo no diretório `/var/tmp` de cada máquina.

Se você precisar de um binário personalizado, consulte a Seção 2.8.5, “Instalando o MySQL usando uma árvore de fonte de desenvolvimento”.

Nota

Após concluir a instalação, ainda não inicie nenhum dos binários. Mostramos como fazer isso após a configuração dos nós (consulte a Seção 25.3.3, “Configuração Inicial do NDB Cluster”).
...
3. Altere a localização para o diretório `mysql` e configure as bases de dados do sistema usando **mysqld** `--initialize` conforme mostrado aqui:
...
**Nodos de dados.** A instalação dos nós de dados do NDB Cluster não requer o binário **mysqld**. É necessário apenas o executável do nó de dados do NDB Cluster **ndbd** (monotêcnico) ou **ndbmtd**") (multitêcnico). Esses binários também podem ser encontrados no arquivo `.tar.gz`. Novamente, assumimos que você colocou esse arquivo em `/var/tmp`.

Após o sistema `root` (ou seja, após usar **sudo**, **su root** ou o equivalente do seu sistema para assumir temporariamente os privilégios da conta de administrador do sistema), execute as etapas a seguir para instalar os binários do nó de dados nos hosts do nó de dados:
...
1. Altere a localização para o diretório `/var/tmp` e extraia os binários **ndbd** e **ndbmtd**") do arquivo para um diretório adequado, como `/usr/local/bin`:
   ...
   (Você pode excluir com segurança o diretório criado ao desempacotar o arquivo baixado e os arquivos que ele contém a partir de `/var/tmp` assim que **ndb\_mgm** e **ndb\_mgmd** forem copiados para o diretório de executaveis.)
...
Embora seja necessário apenas um dos executáveis do nó de dados para executar um nó de dados do NDB Cluster, mostramos como instalar tanto o **ndbd** quanto o **ndbmtd**") nas instruções anteriores. Recomendamos que você faça isso ao instalar ou atualizar o NDB Cluster, mesmo que planeje usar apenas um deles, pois isso economiza tempo e problemas no caso de você decidir mudar de um para o outro mais tarde.
...
Como sistema `root`, execute as etapas a seguir para instalar **ndb\_mgmd** e **ndb\_mgm** no host do nó de gerenciamento:

1. Altere a localização para o diretório `/var/tmp` e extraia os arquivos **ndb\_mgm** e **ndb\_mgmd** do arquivo para um diretório adequado, como `/usr/local/bin`:
