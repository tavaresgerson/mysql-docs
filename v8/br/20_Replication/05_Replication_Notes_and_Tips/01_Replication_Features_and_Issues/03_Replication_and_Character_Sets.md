...
- Se a fonte tiver bancos de dados com um conjunto de caracteres diferente do valor global `character_set_server`, você deve projetar suas instruções `CREATE TABLE` para que não dependam implicitamente do conjunto de caracteres padrão do banco de dados. Uma boa solução é declarar explicitamente o conjunto de caracteres e a collation nas instruções `CREATE TABLE`.
