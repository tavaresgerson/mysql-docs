...
No Unix, o MySQL usa o valor da variável de ambiente `TMPDIR` como o nome do diretório onde os arquivos temporários serão armazenados. Se `TMPDIR` não estiver definido, o MySQL usa o padrão do sistema, que geralmente é `/tmp`, `/var/tmp` ou `/usr/tmp`.
...
Para distribuir a carga de forma eficaz, esses caminhos devem estar localizados em diferentes *discos físicos*, não em diferentes partições do mesmo disco.
...
Operações DDL online podem usar arquivos de log temporários para registrar DML concorrente, arquivos de ordenação temporários ao criar um índice e arquivos de tabelas intermediárias temporárias ao reconstruir a tabela. Para mais informações, consulte  Seção 17.12.3, “Requisitos de Espaço DDL Online”.

As tabelas temporárias temporárias criadas pelo usuário `InnoDB` e as tabelas temporárias internas no disco são criadas em um arquivo de tabelas temporárias chamado `ibtmp1` no diretório de dados do MySQL. Para mais informações, consulte  Seção 17.6.3.5, “Tabelas Temporárias”.

Veja também  Seção 17.15.7, “Tabela de Informações Temporárias da Tabela `INFORMATION_SCHEMA` do `InnoDB`”.

O modificador opcional `EXTENDED` faz com que o `SHOW TABLES` liste tabelas ocultas criadas por instruções `ALTER TABLE` falhas. Veja  Seção 15.7.7.39, “Instrução SHOW TABLES”.