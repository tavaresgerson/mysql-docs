...
* Mensagens de erro do lado do cliente são geradas dentro da biblioteca de clientes MySQL, geralmente envolvendo problemas de comunicação com o servidor.

Exemplo de mensagens de erro do lado do servidor escritas no log de erro:
...
* Esta mensagem indica um problema que requer ação do DBA:
  ...
  ```
Exemplo de mensagem de erro do lado do servidor enviada para programas clientes, conforme exibida pelo cliente `mysql`:

```
mysql> SELECT * FROM no_such_table;
ERROR 1146 (42S02): Table 'test.no_such_table' doesn't exist
```
Exemplo de mensagem de erro do lado do cliente originada dentro da biblioteca de clientes, conforme exibida pelo cliente `mysql`:
...
Seja uma mensagem de erro originada dentro da biblioteca de clientes ou recebida do servidor, um programa cliente MySQL pode responder de maneiras variadas. Como ilustrado, o cliente pode exibir a mensagem de erro para que o usuário possa tomar medidas corretivas. O cliente pode, em vez disso, tentar internamente resolver ou repetir uma operação falha, ou tomar outras ações.
...
O conjunto de códigos de erro usado nas mensagens de erro é dividido em faixas distintas; veja Faixas de Códigos de Erro.

Os códigos de erro são estáveis em todas as versões de disponibilidade geral (GA) de uma série MySQL específica. Antes que uma série atinja o status GA, novos códigos ainda podem estar em desenvolvimento e estão sujeitos a alterações.
* Valor SQLSTATE: Esse valor é uma string de cinco caracteres (por exemplo, `'42S02'`). Os valores SQLSTATE são tirados do ANSI SQL e ODBC e são mais padronizados do que os códigos de erro numéricos. Os dois primeiros caracteres de um valor SQLSTATE indicam a classe do erro:
...
Para erros do lado do servidor, nem todos os números de erro do MySQL têm valores SQLSTATE correspondentes. Nesses casos, `'HY000'` (erro geral) é usado.

Para erros do lado do cliente, o valor SQLSTATE é sempre `'HY000'` (erro geral), então não é significativo para distinguir um erro do cliente de outro.
* String de mensagem: Essa string fornece uma descrição textual do erro.

### Faixas de Códigos de Erro

O conjunto de códigos de erro usado nas mensagens de erro é dividido em faixas distintas, cada uma com seu próprio propósito:

* 1 a 999: Códigos de erro globais. Esse intervalo de códigos de erro é chamado de “global” porque é um intervalo compartilhado que é usado pelo servidor e pelos clientes.

Quando um erro nesse intervalo ocorre no lado do servidor, o servidor o escreve no log de erro, preenchendo o código de erro com zeros à esquerda até seis dígitos e adicionando um prefixo de `MY-`.

Quando um erro nesse intervalo ocorre no lado do cliente, a biblioteca do cliente o disponibiliza ao programa cliente sem preenchimento com zeros ou prefixo.
...
O servidor trata os mensagens de erro escritas no log de erro de maneira diferente das mensagens de erro enviadas aos clientes:

* Quando o servidor escreve uma mensagem no log de erro, preenche o código de erro com zeros à esquerda até seis dígitos e adiciona um prefixo de `MY-` (exemplos:  `MY-000022`,  `MY-010048`).
* Quando o servidor envia uma mensagem para um programa cliente, não adiciona preenchimento com zeros ou prefixo ao código de erro (exemplos:  `1036`,  `3013`).