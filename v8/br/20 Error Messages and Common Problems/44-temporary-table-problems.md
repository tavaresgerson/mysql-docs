...
* Você não pode referenciar uma tabela `TEMPORARY` mais de uma vez na mesma consulta. Por exemplo, o seguinte não funciona:
  ...
  A instrução produz este erro:

  ```
  ERROR 1137: Can't reopen table: 'temp_table'
  ```

  Você pode contornar este problema se sua consulta permitir o uso de uma expressão de tabela comum (CTE) em vez de uma tabela `TEMPORARY`. Por exemplo, isso falha com o erro "Não é possível reabrir a tabela":
...
* Se uma `TEMPORARY` for criada com o mesmo nome de uma tabela existente não `TEMPORARY`, a tabela não `TEMPORARY` é oculta até que a tabela `TEMPORARY` seja eliminada, mesmo que as tabelas usem motores de armazenamento diferentes.
* Existem problemas conhecidos ao usar tabelas temporárias com replicação. Consulte a Seção 19.5.1.31, “Replicação e Tabelas Temporárias”, para obter mais informações.