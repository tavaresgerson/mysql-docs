...
O pacote maior possível que pode ser transmitido para ou a partir de um servidor ou cliente MySQL 8.4 é de 1 GB.
...
É seguro aumentar o valor dessa variável porque a memória extra é alocada apenas quando necessário. Por exemplo, o `mysqld` aloca mais memória apenas quando você emite uma consulta longa ou quando o `mysqld` deve retornar uma grande linha de resultado. O valor padrão pequeno da variável é uma precaução para capturar pacotes incorretos entre o cliente e o servidor e também para garantir que você não se esgote de memória usando pacotes grandes acidentalmente.
