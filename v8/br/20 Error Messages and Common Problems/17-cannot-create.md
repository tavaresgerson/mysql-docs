...
Uma solução é iniciar o `mysqld` com a opção `--tmpdir` ou adicionar a opção à seção `[mysqld]` do seu arquivo de opções. Por exemplo, para especificar um diretório de `C:\temp`, use estas linhas:
