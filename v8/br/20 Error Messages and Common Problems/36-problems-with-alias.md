...
Um alias pode ser usado em uma lista de seleção de consulta para dar a uma coluna um nome diferente. Você pode usar o alias em cláusulas `GROUP BY`, `ORDER BY` ou `HAVING` para referenciar a coluna:
...
A cláusula `WHERE` determina quais linhas devem ser incluídas na cláusula `GROUP BY`, mas ela refere-se ao alias de um valor de coluna que não é conhecido até que as linhas tenham sido selecionadas e agrupadas pela cláusula `GROUP BY`.
