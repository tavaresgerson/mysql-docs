...
Na maioria das versões do Unix, você pode proteger seu diretório `/tmp` para que arquivos só possam ser excluídos por seus proprietários ou pelo superusuário (`root`). Para fazer isso, defina o bit `sticky` no diretório `/tmp` iniciando sessão como `root` e usando o seguinte comando:
...
* Especifique o caminho em um arquivo de opção global ou local. Por exemplo, coloque as seguintes linhas em `/etc/my.cnf`:
  ...
  Veja  Seção 6.2.2.2, “Usando arquivos de opção”.
* Especifique a opção `--socket` na linha de comando para `mysqld_safe` e ao executar programas cliente.
* Defina a variável de ambiente `MYSQL_UNIX_PORT` para o caminho do arquivo de soquete Unix.
* Reconecte o MySQL a partir da fonte para usar uma localização de arquivo de soquete Unix padrão diferente. Defina o caminho do arquivo com a opção `MYSQL_UNIX_ADDR` ao executar `CMake`. Veja  Seção 2.8.7, “Opções de configuração de fonte do MySQL”.

Você pode testar se a nova localização de soquete funciona tentando se conectar ao servidor com este comando:
