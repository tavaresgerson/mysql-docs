...
O tipo de dados `YEAR(4)` usando uma largura de exibição explícita está desatualizado e você deve esperar que o suporte a ele seja removido em uma versão futura do MySQL. Em vez disso, use `YEAR` sem uma largura de exibição, que tem o mesmo significado.
