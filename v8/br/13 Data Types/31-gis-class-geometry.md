...
* Seu **SRID** (Identificador de Referência Espacial), ou sistema de referência espacial associado. Esse valor identifica o sistema de referência espacial associado à geometria que descreve o espaço de coordenadas no qual o objeto de geometria é definido.

  No MySQL, o valor SRID é um inteiro associado ao valor de geometria. O valor máximo utilizável de SRID é 232−1. Se um valor maior for fornecido, apenas os 32 bits inferiores são usados.
...
Cada geometria ocupa uma posição no espaço. O exterior de uma geometria é todo o espaço não ocupado pela geometria. O interior é o espaço ocupado pela geometria. A borda é a interface entre o interior e o exterior da geometria.
...
* Se o valor é **vazio** ou **não vazio**. Uma geometria é vazia se não tiver nenhum ponto. O exterior, interior e borda de uma geometria vazia não são definidos (ou seja, são representados por um valor `NULL`). Uma geometria vazia é definida como sempre simples e tem uma área de 0.
...
Os objetos `Point` têm uma dimensão de zero. Os objetos `LineString` têm uma dimensão de 1. Os objetos `Polygon` têm uma dimensão de 2. As dimensões dos objetos `MultiPoint`, `MultiLineString` e `MultiPolygon` são as mesmas das dimensões dos elementos de que são compostos.