...
O otimizador investiga se índices espaciais disponíveis podem ser envolvidos na busca por consultas que utilizam uma função como `MBRContains()` ou `MBRWithin()` na cláusula `WHERE`. A consulta a seguir encontra todos os objetos que estão no retângulo dado:
...
Executar a instrução `SELECT` sem o índice espacial produz o mesmo resultado, mas aumenta o tempo de execução de 0,00 segundos para 0,46 segundos:
