...
Os seguintes exemplos usam `DEFAULT 0`, um padrão que pode gerar avisos ou erros, dependendo se o modo SQL rigoroso ou o modo SQL `NO_ZERO_DATE` está habilitado. Tenha em mente que o modo SQL `TRADITIONAL` inclui o modo rigoroso e `NO_ZERO_DATE`. Consulte a Seção 7.1.11, “Modos SQL do Servidor”.


:::

As definições de colunas `TIMESTAMP` ou `DATETIME` podem especificar o timestamp atual tanto para o valor padrão quanto para o valor de atualização automática, para uma, mas não para a outra, ou para nenhuma. Diferentes colunas podem ter diferentes combinações de propriedades automáticas. As seguintes regras descrevem as possibilidades:
...
* Com `DEFAULT CURRENT_TIMESTAMP` e `ON UPDATE CURRENT_TIMESTAMP`, a coluna tem o timestamp atual para seu valor padrão e é automaticamente atualizado para o timestamp atual.
  ...
  ```
* Com uma cláusula `DEFAULT` mas sem uma cláusula `ON UPDATE CURRENT_TIMESTAMP`, a coluna tem o valor padrão dado e não é automaticamente atualizada para o timestamp atual.
...
* Com uma cláusula `ON UPDATE CURRENT_TIMESTAMP` e uma cláusula `DEFAULT` constante, a coluna é automaticamente atualizada para o timestamp atual e tem o valor padrão constante dado.
  ...
  ```
* Com uma cláusula `ON UPDATE CURRENT_TIMESTAMP` mas sem uma cláusula `DEFAULT`, a coluna é automaticamente atualizada para o timestamp atual, mas não tem o timestamp atual para seu valor padrão.
...
* Alternativamente, se `explicit_defaults_for_timestamp` estiver desativada, faça uma das seguintes ações:
  ...
  + Especifique o atributo `NULL`. Isso também faz com que a coluna permita valores `NULL`, o que significa que você não pode atribuir o timestamp atual configurando a coluna para `NULL`. Atribuir `NULL` configura a coluna para `NULL`, não o timestamp atual. Para atribuir o timestamp atual, configure a coluna para `CURRENT_TIMESTAMP` ou um sinônimo como `NOW()`.
...
Em outras palavras, uma coluna `TIMESTAMP` definida para permitir valores `NULL` se inicializa automaticamente apenas se sua definição incluir `DEFAULT CURRENT_TIMESTAMP`:
