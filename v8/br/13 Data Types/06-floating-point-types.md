...
`FLOAT(M,D)` e `DOUBLE(M,D)` são extensões não padrão do MySQL; e estão desatualizadas. Você deve esperar que o suporte para essas variantes seja removido em uma versão futura do MySQL.
...
Para a máxima portabilidade, o código que requer o armazenamento de valores de dados numéricos aproximados deve usar `FLOAT` ou `DOUBLE PRECISION` sem especificar precisão ou número de dígitos.