...
O InnoDB Cluster não oferece suporte para o MySQL NDB Cluster. Para obter mais informações sobre o MySQL NDB Cluster, consulte o Capítulo 25, *MySQL NDB Cluster 8.0* e a Seção 25.2.6, “MySQL Server Usando InnoDB Comparado com NDB Cluster”.
...
Construído sobre o MySQL Group Replication, oferece recursos como gerenciamento automático de membros, tolerância a falhas, failover automático, entre outros. Um InnoDB Cluster geralmente funciona em um modo de único primário, com uma única instância primária (leitura/escrita) e múltiplas instâncias secundárias (somente leitura). Usuários avançados também podem aproveitar um modo de múltiplos primários, onde todas as instâncias são primárias. Você pode até alterar a topologia do cluster enquanto o InnoDB Cluster estiver online, para garantir a maior disponibilidade possível.
