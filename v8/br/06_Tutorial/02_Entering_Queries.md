...
As declarações de várias linhas a partir deste ponto são escritas sem os prompts secundários (`->` ou outros), para facilitar a cópia e a inserção das declarações para que você possa experimentar por si mesmo.