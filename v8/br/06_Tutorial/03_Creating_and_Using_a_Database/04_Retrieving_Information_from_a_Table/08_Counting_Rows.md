...
Contar o número total de animais que você tem é a mesma pergunta que "Quantas linhas estão na tabela `pet`?", pois há um registro por animal. `COUNT(*)` conta o número de linhas, então a consulta para contar seus animais parece assim:
...
Se você nomear as colunas para seleção além do valor `COUNT()`, deve haver uma cláusula `GROUP BY` que nomeie essas mesmas colunas. Caso contrário, o seguinte ocorre:

- Se o modo SQL `ONLY_FULL_GROUP_BY` estiver ativado, ocorrerá um erro:
...
Veja também a Seção 14.19.3, “Tratamento do MySQL do GROUP BY”. Consulte a Seção 14.19.1, “Descrição das Funções Agregadas” para obter informações sobre o comportamento do `COUNT(expr)` e otimizações relacionadas.
