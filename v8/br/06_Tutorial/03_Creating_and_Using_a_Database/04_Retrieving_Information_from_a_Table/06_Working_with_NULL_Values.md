...
Você não pode usar operadores de comparação aritmética, como `=`, `<` ou `<>`, para testar `NULL`. Para demonstrar isso, tente a seguinte consulta:
...
Esse tratamento especial do `NULL` é a razão pela qual, na seção anterior, foi necessário determinar quais animais não estão mais vivos usando `death IS NOT NULL` em vez de `death <> NULL`.

Dois valores `NULL` são considerados iguais em um `GROUP BY`.

Ao realizar um `ORDER BY`, os valores `NULL` são apresentados primeiro se você fizer `ORDER BY ... ASC` e por último se você fizer `ORDER BY ... DESC`.
...
Portanto, é perfeitamente possível inserir um zero ou uma string vazia em uma coluna `NOT NULL`, pois essas são, na verdade, `NOT NULL`. Veja a Seção B.3.4.3, “Problemas com Valores NULL”.
