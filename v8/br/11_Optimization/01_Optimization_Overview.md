...
* Todos os [áreas de memória usadas para cache][(buffering-caching.html "10.10 Buffering and Caching")] estão dimensionados corretamente? Ou seja, grandes o suficiente para armazenar dados frequentemente acessados, mas não tão grandes que sobrecarreguem a memória física e causem paginação. As principais áreas de memória a serem configuradas são o pool de buffer `InnoDB` e o cache de chave `MyISAM`.
...
Para usar extensões de SQL orientadas ao desempenho em um programa MySQL portátil, você pode envolver palavras-chave específicas do MySQL em uma declaração dentro dos delimitadores de comentário `/*! */`. Outros servidores de SQL ignoram as palavras-chave comentadas. Para obter informações sobre a escrita de comentários, consulte a Seção 11.7, “Comentários”.