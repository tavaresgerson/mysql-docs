...
O valor `Open tables` de 12 pode ser um pouco confuso se você tiver menos de 12 tabelas.
...
- Quando o cache contém mais de `table_open_cache` entradas e uma tabela no cache não estiver sendo usada por nenhum dos threads.
...
Se o valor for muito grande ou aumentar rapidamente, mesmo quando você não emitiu muitas declarações `FLUSH TABLES`, aumente o valor `table_open_cache` ao iniciar o servidor.
