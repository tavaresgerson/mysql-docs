...
Para ajustar as consultas para as tabelas `InnoDB`, crie um conjunto apropriado de índices em cada tabela. Consulte a Seção 10.3.1, “Como o MySQL usa índices”, para obter detalhes. Siga estas diretrizes para os índices `InnoDB`:

- Como cada tabela `InnoDB` tem uma chave primária (se você solicitar uma ou não), especifique um conjunto de colunas chave primária para cada tabela, colunas que são usadas nas consultas mais importantes e críticas no tempo.
...
- Se uma coluna indexada não puder conter quaisquer valores de `NULL`, declare-a como `NOT NULL` ao criar a tabela. O otimizador pode determinar melhor qual índice é mais eficaz para usar em uma consulta, quando sabe se cada coluna contém valores de `NULL`.
