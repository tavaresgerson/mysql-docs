...
`InnoDB` estende automaticamente cada índice secundário, adicionando as colunas da chave primária. Considere esta definição de tabela:
...
Esta tabela define a chave primária nas colunas `(i1, i2)`. Também define um índice secundário `k_d` na coluna `(d)`, mas internamente `InnoDB` estende esse índice e o trata como colunas `(d, i1, i2)`.
...
O otimizador pode usar índices secundários extensos para o acesso ao índice `ref`, `range` e `index_merge`, para o acesso ao varredura de índice leve, para otimização de junção e ordenação, e para a otimização de `MIN()`/`MAX()`.
...
Quando o otimizador não considera extensões de índice, ele trata o índice `k_d` como apenas `(d)`. `EXPLAIN` para a consulta produz este resultado:
...
Em ambos os casos, `key` indica que o otimizador usa o índice secundário `k_d`, mas a saída `EXPLAIN` mostra essas melhorias ao usar o índice estendido:

- `key_len` passa de 4 bytes para 8 bytes, indicando que as pesquisas de chave usam as colunas `d` e `i1`, e não apenas `d`.

- O valor `ref` muda de `const` para `const,const` porque a busca por chave usa duas partes da chave, não uma.

- O número `rows` diminui de 5 para 1, indicando que `InnoDB` deve precisar examinar menos linhas para produzir o resultado.
