...
A chave primária de uma tabela representa a coluna ou conjunto de colunas que você usa em suas consultas mais importantes. Ela possui um índice associado, para melhorar o desempenho das consultas. O desempenho das consultas se beneficia da otimização do `NOT NULL`, porque não pode incluir quaisquer valores do `NULL`. Com o mecanismo de armazenamento `InnoDB`, os dados da tabela são organizados fisicamente para realizar consultas e ordenamentos ultra-rápidos com base na coluna ou colunas da chave primária.
