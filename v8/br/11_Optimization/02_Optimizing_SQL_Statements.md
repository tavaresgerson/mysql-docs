## 10.2 Otimizando declarações SQL
...
### 10.2.1 Otimizando declarações SELECT
...
Além das declarações `SELECT`, as técnicas de ajuste para consultas também se aplicam a construções como [[`CREATE TABLE...AS SELECT`]](create-table-select.html "15.1.20.4 CREATE TABLE ... SELECT Statement"), [[`INSERT INTO...SELECT`]](insert-select.html "15.2.7.1 INSERT ... SELECT Statement") e cláusulas `WHERE` em declarações `DELETE`. Essas declarações têm considerações de desempenho adicionais porque combinam operações de escrita com operações de consulta orientadas para leitura.
...
Os índices são especialmente importantes para consultas que fazem referência a diferentes tabelas, utilizando recursos como junções e chaves estrangeiras. Você pode usar a declaração `EXPLAIN` para determinar quais índices são usados para um `SELECT`. Veja a Seção 10.3.1, “Como o MySQL usa índices” e a Seção 10.8.1, “Otimizando consultas com EXPLAIN”.

* Isolate e ajuste qualquer parte da consulta, como uma chamada de função, que leva tempo excessivo. Dependendo da estrutura da consulta, uma função pode ser chamada uma vez para cada linha no conjunto de resultados, ou até mesmo uma vez para cada linha da tabela, aumentando significativamente qualquer ineficiência.
...
Em MySQL 8.0.14 e versões posteriores, isso ocorre durante a preparação, e não durante a fase de otimização, o que ajuda na simplificação das junções. Consulte a Seção 10.2.1.9, “Otimização de Junções Externas”, para obter mais informações e exemplos.
...
* `COUNT(*)` em uma única tabela sem uma `WHERE` é recuperado diretamente das informações da tabela para as tabelas `MyISAM` e `MEMORY`. Isso também é feito para qualquer expressão `NOT NULL` quando usada com apenas uma tabela.

* Detecção precoce de expressões constantes inválidas. O MySQL rapidamente detecta que algumas declarações `SELECT` são impossíveis e não retorna nenhuma linha.

* `HAVING` é fundido com `WHERE` se você não usar funções agregadas (`COUNT()`, `MIN()` e assim por diante).

* Para cada tabela em uma junção, é construído um `WHERE` mais simples para obter uma avaliação rápida do `WHERE` para a tabela e também para ignorar as linhas o mais rápido possível.
...
+ Uma tabela vazia ou uma tabela com uma única linha.  + Uma tabela que é usada com uma cláusula `WHERE` em um índice `PRIMARY KEY` ou `UNIQUE`, onde todas as partes do índice são comparadas a expressões constantes e são definidas como `NOT NULL`.
...
* Em alguns casos, o MySQL pode ler linhas do índice sem consultar o arquivo de dados. Se todas as colunas usadas do índice forem numéricas, apenas a árvore do índice é usada para resolver a consulta.

* Antes de cada linha ser exibida, as que não correspondem à cláusula `HAVING` são ignoradas.
...
As seguintes consultas utilizam indexação para recuperar as linhas em ordem ordenada, sem uma passagem de ordenação separada:
...
O método de acesso `range` utiliza um único índice para recuperar um subconjunto de linhas de tabela que estão contidas em um ou vários intervalos de valor de índice. Ele pode ser usado para índice de uma parte ou de múltiplas partes. As seções a seguir descrevem as condições sob as quais o otimizador utiliza o acesso de intervalo.

* Método de acesso de intervalo para índices de uma parte
* Método de acesso de intervalo para índices de várias partes
* Otimização de intervalo de igualdade de comparações de vários valores
* Método de acesso de intervalo de varredura de pular
* Otimização de intervalo de otimização de expressões do construtor de linha
...
* Para os índices `BTREE` e `HASH`, a comparação de uma parte chave com um valor constante é uma condição de intervalo ao usar os operadores `=`, `<=>`, `IN()`, [`IS NULL`](comparison-operators.html#operator_is-null), ou [`IS NOT NULL`(comparison-operators.html#operator_is-not-null)**.
...
* Uma constante da string de consulta * Uma coluna de uma tabela `const` ou `system` do mesmo join
...
2. Remova `nonkey = 4` e `key1 LIKE '%b'`, pois não podem ser usados para uma varredura de intervalo. A maneira correta de removê-los é substituí-los por `TRUE`, para que não percamos nenhuma linha correspondente ao fazer a varredura de intervalo. Substituí-los por `TRUE` resulta em:
...
Em geral (e como demonstrado pelo exemplo anterior), a condição usada para uma varredura de intervalo é menos restritiva do que a cláusula `WHERE`. O MySQL realiza uma verificação adicional para filtrar linhas que satisfazem a condição de intervalo, mas não a cláusula completa `WHERE`.
...
O MySQL não suporta a combinação de múltiplos intervalos para o método de acesso `range` para índices espaciais. Para contornar essa limitação, você pode usar um `UNION` com declarações `SELECT` idênticas, exceto que você coloca cada predicado espacial em um `SELECT` diferente.
...
Aqui, *`const1`*, *`const2`*, … são constantes, *`cmp`* é um dos operadores de comparação de `=`, `<=>`, ou `IS NULL`, e as condições cobrem todas as partes do índice. (Ou seja, existem condições *`N`*, uma para cada parte de um índice de *`N`*-parte.) Por exemplo, o seguinte é uma condição de intervalo para um índice de três partes de `HASH`:
...
* Para um índice `BTREE`, um intervalo pode ser útil para condições combinadas com `AND`, onde cada condição compara uma parte chave com um valor constante usando `=`, `<=>`, `IS NULL`, `>`, `<`, `>=`, `<=`, `!=`, `<>`, `BETWEEN` ou `LIKE 'pattern'`(string-comparison-functions.html#operator_like) (onde `'pattern'` não começa com um caractere wildcard). Um intervalo pode ser usado desde que seja possível determinar um único conjunto de chaves que contenha todas as linhas que correspondem à condição (ou dois intervalos se `<>` ou `!=` for usado).
...
Neste exemplo, o intervalo na primeira linha utiliza uma parte chave para o limite esquerdo e duas partes chave para o limite direito. O intervalo na segunda linha utiliza apenas uma parte chave. A coluna `key_len` no `EXPLAIN` de saída indica o comprimento máximo do prefixo chave utilizado.
...
Cada expressão é verdadeira se *`col_name`* for igual a qualquer um dos vários valores. Essas comparações são comparações de intervalo de igualdade (onde o "intervalo" é um único valor). O otimizador estima o custo de leitura de linhas qualificadas para comparações de intervalo de igualdade da seguinte forma:
...
Com mergulhos de índice, o otimizador faz um mergulho em cada extremidade de uma faixa e usa o número de linhas na faixa como a estimativa. Por exemplo, a expressão `col_name IN (10, 20, 30)` tem três faixas de igualdade e o otimizador faz dois mergulhos por faixa para gerar uma estimativa de linha. Cada par de mergulhos gera uma estimativa do número de linhas que têm o valor dado.

As mergulhas no índice fornecem estimativas precisas de linha, mas, à medida que o número de valores de comparação na expressão aumenta, o otimizador leva mais tempo para gerar uma estimativa de linha. O uso de estatísticas de índice é menos preciso do que as mergulhas no índice, mas permite uma estimativa de linha mais rápida para listas de valores grandes.

A variável de sistema `eq_range_index_dive_limit` permite que você configure o número de valores em que o otimizador muda de uma estratégia de estimativa de linha para outra. Para permitir o uso de mergulhos de índice para comparações de até *`N`* intervalos de igualdade, defina `eq_range_index_dive_limit` para *`N`* + 1. Para desabilitar o uso de estatísticas e usar sempre mergulhos de índice, independentemente de *`N`*, defina `eq_range_index_dive_limit` para 0.

Para atualizar as estatísticas do índice de tabela para as melhores estimativas, use `ANALYZE TABLE`.

Antes do MySQL 8.0, não há como ignorar o uso de mergulhos de índice para estimar a utilidade do índice, exceto usando a variável de sistema `eq_range_index_dive_limit`. No MySQL 8.0, o desvio de índice é possível para consultas que satisfazem todas essas condições:

* A consulta é para uma única tabela, não para uma junção em várias tabelas.

* Há uma dica de índice `FORCE INDEX` de índice único. A ideia é que, se o uso do índice for forçado, não há nada a ganhar com o sobrecarga adicional de realizar mergulhos no índice.
...
Para `EXPLAIN FOR CONNECTION`(explain.html "15.8.2 EXPLAIN Statement"), a saída muda da seguinte forma se as mergulhas de índice forem ignoradas:
...
Após a execução de uma consulta para a qual os mergulhos de índice são ignorados, a linha correspondente na tabela do esquema de informações `OPTIMIZER_TRACE` contém um valor de `index_dives_for_range_access`.

##### Método de acesso à faixa de varredura ignorada
...
Para executar essa consulta, o MySQL pode escolher uma varredura de índice para obter todas as linhas (o índice inclui todas as colunas a serem selecionadas), e, em seguida, aplicar a condição `f2 > 40` da cláusula `WHERE` para produzir o conjunto de resultados final.

Uma varredura de intervalo é mais eficiente do que uma varredura completa de índice, mas não pode ser usada neste caso, porque não há condição em `f1`, a primeira coluna do índice. No entanto, a partir do MySQL 8.0.13, o otimizador pode realizar múltiplas varreduras de intervalo, uma para cada valor de `f1`, usando um método chamado Skip Scan que é semelhante ao Índice Solto (consulte Seção 10.2.1.17, “Otimização de GROUP BY”):

1. Saltar entre valores distintos da primeira parte do índice, `f1` (o prefixo do índice).

2. Realize uma varredura de subintervalo em cada valor de prefixo distinto para a condição `f2 > 40` na parte restante do índice.
...
3. Realize uma varredura de intervalo. 4. Obtenha o próximo valor distinto da primeira parte da chave (`f1 = 2`).

5. Construa a faixa com base nas primeiras e segundas partes-chave (`f1 = 2 AND f2 > 40`).

6. Realize uma varredura de alcance.

Usar essa estratégia diminui o número de linhas acessadas, pois o MySQL ignora as linhas que não se qualificam para cada intervalo construído. Esse método de acesso de varredura de pular é aplicável nas seguintes condições:

* A tabela T tem pelo menos um índice composto com partes-chave do formulário ([A_1, ..., A_*`k`*,] B_1, ..., B_*`m`*, C [, D_1, ..., D_*`n`*]). As partes-chave A e D podem estar vazias, mas B e C devem ser não vazias.

* A consulta faz referência a apenas uma tabela. * A consulta não utiliza `GROUP BY` ou `DISTINCT`.

* A consulta faz referência apenas às colunas do índice. * Os predicados em A_1, ..., A_*`k`* devem ser predicados de igualdade e devem ser constantes. Isso inclui o operador `IN()`.

* A consulta deve ser uma consulta conjuntiva; ou seja, uma `AND` de condições `OR`: `(cond1(key_part1) OR cond2(key_part1)) AND (cond1(key_part2) OR ...) AND ...`

* Deve haver uma condição de intervalo em C. * As condições nas colunas D são permitidas. As condições em D devem estar em conjunto com a condição de intervalo em C.

O uso do Skip Scan é indicado no `EXPLAIN` conforme a seguinte descrição:

* `Using index for skip scan` na coluna `Extra` indica que o método de acesso ao índice solto Skip Scan é utilizado.

* Se o índice puder ser usado para Skip Scan, o índice deve ser visível na coluna `possible_keys`.
...
Você também pode ver um elemento `"best_skip_scan_summary"`. Se o Skip Scan for escolhido como a melhor variante de acesso à faixa, um elemento `"chosen_range_access_summary"` é escrito. Se o Skip Scan for escolhido como o método de acesso geral melhor, um elemento `"best_access_path"` está presente.

O uso do Skip Scan está sujeito ao valor da bandeira `skip_scan` da variável de sistema `optimizer_switch`. Veja a Seção 10.9.2, “Otimizações Desconectables”. Por padrão, esta bandeira é `on`. Para desabilitar, defina `skip_scan` para `off`.

Além de usar a variável de sistema `optimizer_switch` para controlar o uso do otimizador do Skip Scan em nível de sessão, o MySQL suporta dicas de otimizador para influenciar o otimizador em uma base por declaração. Veja a Seção 10.9.3, “Dicas de Otimizador”.

##### Otimização de faixa de expressão do construtor de linhas
...
* São utilizados apenas os predicados `IN()`, não `NOT IN()`.

* Do lado esquerdo do predicado `IN()`, o construtor de linha contém apenas referências de coluna.

* No lado direito do predicado `IN()`, os construtores de linha contêm apenas constantes de tempo de execução, que são literais ou referências de coluna local que são vinculadas a constantes durante a execução.

* Do lado direito do predicado `IN()`, há mais de um construtor de linha.
...
* Para as declarações `UPDATE` e `DELETE`, se o otimizador voltar a um varredura completa da tabela e a variável de sistema `sql_safe_updates` estiver habilitada, ocorre um erro em vez de um aviso, pois, na verdade, nenhuma chave é usada para determinar quais linhas devem ser modificadas. Para mais informações, consulte "Usando o modo de atualizações seguras (--safe-updates)".
...
* Se sua consulta tiver uma cláusula `WHERE` complexa com uma profundidade de `AND`/`OR` de encaixe e o MySQL não escolher o plano ótimo, tente distribuir os termos usando as seguintes transformações de identidade:
...
Na saída de `EXPLAIN`, o método de junção de índices aparece como `index_merge` na coluna `type`. Neste caso, a coluna `key` contém uma lista de índices utilizados e `key_len` contém uma lista das partes de chave mais longas para esses índices.
...
* Algoritmo de junção de índices com acesso de interseção
* Algoritmo de junção de índices com acesso de união
* Algoritmo de junção de índices com acesso de união
* Otimização de junção de índices com influência
...
O algoritmo de interseção de junção de índices realiza varreduras simultâneas em todos os índices usados e produz a interseção de sequências de linha que recebe dos varreduros de índice juncionado.

Se todas as colunas utilizadas na consulta estiverem cobertas pelos índices utilizados, as linhas inteiras da tabela não serão recuperadas (a saída `EXPLAIN` contém `Using index` no campo `Extra` neste caso). Aqui está um exemplo de tal consulta:

```
SELECT COUNT(*) FROM t1 WHERE key1 = 1 AND key2 = 1;
```

Se os índices utilizados não cobrem todas as colunas utilizadas na consulta, as linhas completas são recuperadas apenas quando as condições de intervalo para todas as chaves utilizadas são satisfeitas.

Se uma das condições unidas for uma condição sobre a chave primária de uma tabela `InnoDB`, ela não é usada para recuperação de linha, mas é usada para filtrar as linhas recuperadas usando outras condições.
...
A diferença entre o algoritmo de união por ordem e o algoritmo de união é que o algoritmo de união por ordem deve primeiro buscar os IDs de linha para todas as linhas e ordená-las antes de retornar qualquer linha.

##### Otimização da fusão do Índice de influência

O uso da junção de índices está sujeito ao valor das bandeiras `index_merge`, `index_merge_intersection`, `index_merge_union` e `index_merge_sort_union` da variável de sistema `optimizer_switch`. Veja a Seção 10.9.2, “Otimizações Desconectables”. Por padrão, todas essas bandeiras são `on`. Para habilitar apenas certos algoritmos, defina `index_merge` para `off`, e habilite apenas aqueles outros que devem ser permitidos.

Além de usar a variável de sistema `optimizer_switch` para controlar o uso do otimizador dos algoritmos de junção de índices em nível de sessão, o MySQL suporta dicas de otimizador para influenciar o otimizador em uma base por declaração. Veja a Seção 10.9.3, “Dicas de Otimizador”.

#### 10.2.1.4 Otimização do Join Hash

Por padrão, o MySQL (8.0.18 e versões posteriores) emprega junções hash sempre que possível. É possível controlar se as junções hash são empregadas usando uma das dicas de otimizador `BNL` e `NO_BNL`, ou definindo `block_nested_loop=on` ou `block_nested_loop=off` como parte do ajuste da variável de sistema servidor optimizer_switch.

Nota

O MySQL 8.0.18 suportava a definição de uma bandeira `hash_join` em `optimizer_switch`, bem como as dicas de otimizador `HASH_JOIN` e `NO_HASH_JOIN`. No MySQL 8.0.19 e versões posteriores, nenhuma dessas opções tem mais efeito.

A partir do MySQL 8.0.18, o MySQL emprega uma junção hash para qualquer consulta para a qual cada junção tenha uma condição de junção equi, e na qual não haja índices que possam ser aplicados a quaisquer condições de junção, como esta:
...
Uma junção hash também pode ser usada quando há um ou mais índices que podem ser usados para predicados de tabela única.

Uma junção hash é geralmente mais rápida e é destinada a ser usada em tais casos, em vez do algoritmo de laço aninhado de bloco (veja Algoritmo de Junção de Laço Aninhado de Bloco), empregado nas versões anteriores do MySQL. A partir do MySQL 8.0.20, o suporte para o laço aninhado de bloco é removido e o servidor emprega uma junção hash sempre que um laço aninhado de bloco teria sido usado anteriormente.

No exemplo que acabou de ser mostrado e nos exemplos restantes desta seção, assumimos que as três tabelas `t1`, `t2` e `t3` foram criadas usando as seguintes declarações:
...
`EXPLAIN ANALYZE` também exibe informações sobre as junções hash utilizadas.

A junção hash também é usada em consultas que envolvem múltiplas junções, desde que pelo menos uma condição de junção para cada par de tabelas seja uma junção equi, como a consulta mostrada aqui:
...
Em casos como o que foi mostrado, que faz uso de uma junção interna, quaisquer condições adicionais que não sejam junções equi são aplicadas como filtros após a execução da junção. (Para junções externas, como junções à esquerda, semijoins e antijunções, elas são impressas como parte da junção.) Isso pode ser visto aqui na saída de `EXPLAIN`:
...
Como também pode ser visto na saída mostrada acima, várias junções de hash podem ser (e são) usadas para junções com múltiplas condições de junção equi.

Antes do MySQL 8.0.20, uma junção hash não poderia ser usada se qualquer par de tabelas unidas não tivesse pelo menos uma condição de junção equi, e o algoritmo de laço aninhado de bloco mais lento fosse empregado. No MySQL 8.0.20 e versões posteriores, a junção hash é usada em tais casos, como mostrado aqui:
...
Uma junção hash também é aplicada para um produto cartesiano — ou seja, quando nenhuma condição de junção é especificada, como mostrado aqui:
...
Em MySQL 8.0.20 e versões posteriores, não é mais necessário que a junção contenha pelo menos uma condição de junção equi para que uma junção hash seja usada. Isso significa que os tipos de consultas que podem ser otimizados usando junções hash incluem as do seguinte (com exemplos):
...
* *Conjunção externa direita* (observe que o MySQL reescreve todas as junções externas à direita como junções externas à esquerda):
...
Por padrão, o MySQL 8.0.18 e versões posteriores empregam junções de hash sempre que possível. É possível controlar se as junções de hash são empregadas usando uma das dicas de otimizador `BNL` e `NO_BNL`.

(O MySQL 8.0.18 suporta `hash_join=on` ou `hash_join=off` como parte do ajuste para a variável do sistema do servidor `optimizer_switch`, bem como as dicas de otimização `HASH_JOIN` ou `NO_HASH_JOIN`. No MySQL 8.0.19 e versões posteriores, essas opções não têm mais efeito.)

O uso de memória por junções hash pode ser controlado usando a variável de sistema `join_buffer_size`; uma junção hash não pode usar mais memória do que esse valor. Quando a memória necessária para uma junção hash excede o valor disponível, o MySQL lida com isso usando arquivos no disco. Se isso acontecer, você deve estar ciente de que a junção pode não ser bem-sucedida se uma junção hash não caber na memória e criar mais arquivos do que o definido para `open_files_limit`. Para evitar tais problemas, faça uma das seguintes alterações:

* Aumente `join_buffer_size` para que a junção de hash não transborde para o disco.

* Aumente `open_files_limit`.

Começando com o MySQL 8.0.18, os buffers de junção para junções hash são alocados incrementalmente; assim, você pode definir `join_buffer_size` mais alto sem que consultas pequenas aloquem grandes quantidades de RAM, mas as junções externas alocam todo o buffer. No MySQL 8.0.20 e versões posteriores, as junções hash são usadas para junções externas (incluindo antijunções e semijoins) também, então isso não é mais um problema.

#### 10.2.1.5 Otimização da Depressão do Condição do Motor

Essa otimização melhora a eficiência das comparações diretas entre uma coluna não indexada e uma constante. Nesses casos, a condição é "empurrada" para o mecanismo de armazenamento para avaliação. Essa otimização só pode ser usada pelo mecanismo de armazenamento `NDB`.
...
No entanto, o pushdown da condição do motor *não pode* ser usado com a seguinte consulta:

```
SELECT a,b FROM t1 WHERE a = 10;
```

O pushdown de condição do motor não é aplicável aqui porque existe um índice na coluna `a`. (Um método de acesso a índice seria mais eficiente e, portanto, seria escolhido em preferência ao pushdown de condição.)
...
*`pattern`* deve ser uma literal de string que contém o padrão a ser correspondido; para sintaxe, consulte a Seção 14.8.1, “Funções e operadores de comparação de strings”.
...
O pushdown da condição do motor é ativado por padrão. Para desativá-lo na inicialização do servidor, defina a bandeira `engine_condition_pushdown` da variável de sistema `optimizer_switch` para `off`. Por exemplo, em um arquivo `my.cnf`, use essas linhas:
...
* Antes da NDB 8.0.18, as colunas podiam ser comparadas com constantes ou expressões que avaliam valores constantes apenas. Na NDB 8.0.18 e em versões posteriores, as colunas podem ser comparadas entre si, desde que sejam exatamente do mesmo tipo, incluindo a mesma sinalização, comprimento, conjunto de caracteres, precisão e escala, onde essas são aplicáveis.
...
* As junções não são diretamente suportadas; as condições que envolvem múltiplas tabelas são empurradas separadamente, sempre que possível. Use a saída `EXPLAIN` estendida para determinar quais condições são realmente empurradas. Veja a Seção 10.8.3, “Formato de saída de EXPLAIN estendida”.

Anteriormente, a exclusão de condições de motor era limitada a termos que se referissem a valores de coluna da mesma tabela para a qual a condição estava sendo excluída. A partir do NDB 8.0.16, os valores de coluna de tabelas mais antigas no plano de consulta também podem ser referenciados a partir de condições excluídas. Isso reduz o número de linhas que devem ser manipuladas pelo nó SQL durante o processamento de junção. O filtro também pode ser realizado em paralelo nos threads do LDM, em vez de em um único processo **mysqld**. Isso tem o potencial de melhorar significativamente o desempenho das consultas.

A partir do NDB 8.0.20, uma junção externa usando um varredura pode ser empurrada se não houver condições não empurráveis em qualquer tabela usada no mesmo ninho de junção, ou em qualquer tabela em ninhos de junção acima dela, da qual depende. Isso também é válido para uma junção semijoin, desde que a estratégia de otimização empregada seja `firstMatch` (consulte [Seção 10.2.2.1, “Otimizando Predicados de Subconsultas IN e EXISTS com Transformações de Junção Semijoin”][(semijoins.html "10.2.2.1 Optimizing IN and EXISTS Subquery Predicates with Semijoin Transformations")]).

Os algoritmos de junção não podem ser combinados com colunas de referência de tabelas anteriores nas seguintes duas situações:

1. Quando qualquer uma das tabelas referenciadas das tabelas anteriores estiver em um buffer de junção. Nesse caso, cada linha recuperada da tabela filtrada por varredura é correspondida a cada linha no buffer. Isso significa que não há uma única linha específica a partir da qual os valores da coluna podem ser obtidos ao gerar o filtro de varredura.

2. Quando a coluna provém de uma operação de filho em uma junção empurrada. Isso ocorre porque as linhas referenciadas a partir de operações ancestrais na junção ainda não foram recuperadas quando o filtro de varredura é gerado.

Começando com o NDB 8.0.27, as colunas das tabelas ancestrais em uma junção podem ser empurradas para baixo, desde que elas atendam aos requisitos listados anteriormente. Um exemplo de tal consulta, usando a tabela `t1` criada anteriormente, é mostrado aqui:
...
#### 10.2.1.6 Otimização da Depuração da Condição do Índice
...
* O ICP é utilizado para os métodos de acesso `range`, `ref`, `eq_ref` e `ref_or_null` quando é necessário acessar linhas completas da tabela.

* O ICP pode ser usado para as tabelas `InnoDB` e `MyISAM`, incluindo as tabelas particionadas `InnoDB` e `MyISAM`.

* Para as tabelas `InnoDB`, o ICP é usado apenas para índices secundários. O objetivo do ICP é reduzir o número de leituras completas de linha e, assim, reduzir as operações de E/S. Para índices agrupados `InnoDB`, o registro completo já é lido no buffer `InnoDB`. O uso do ICP nesse caso não reduz a E/S.
...
As condições desencadeadas não podem ser deslocadas. (Para informações sobre condições desencadeadas, consulte a Seção 10.2.2.3, “Otimizando subconsultas com a estratégia EXISTS”.)

* (*MySQL 8.0.30 e posterior*:) As condições não podem ser empurradas para tabelas derivadas que contenham referências a variáveis do sistema.
...
2. Teste a parte da condição `WHERE` que se aplica a esta tabela e que pode ser verificada usando apenas colunas de índice. Se a condição não for satisfeita, prossiga para o tuplo de índice para a próxima linha.
...
A saída `EXPLAIN` exibe `Using index condition` na coluna `Extra` quando o Index Condition Pushdown é usado. Não exibe `Using index`, porque isso não se aplica quando as linhas inteiras da tabela devem ser lidas.
...
O MySQL pode usar o índice para percorrer pessoas com `zipcode='95054'`. A segunda parte (`lastname LIKE '%etrunia%'`) não pode ser usada para limitar o número de linhas que devem ser percorridas, portanto, sem o Pushdown da Condição de Índice, esta consulta deve recuperar linhas completas da tabela para todas as pessoas que têm `zipcode='95054'`.

Com o Pushdown da Condição de Índice, o MySQL verifica a parte `lastname LIKE '%etrunia%'` antes de ler a linha completa da tabela. Isso evita a leitura de linhas completas correspondentes a tuplas de índice que correspondem à condição `zipcode`, mas não à condição `lastname`.
...
Veja a Seção 10.9.2, “Otimizações Desconectables”.

#### 10.2.1.7 Algoritmos de Conjunção com Loop Aninhado
...
Um algoritmo de junção em laço aninhado (NLJ) lê as linhas da primeira tabela em um laço, uma de cada vez, passando cada linha para um laço aninhado que processa a próxima tabela na junção. Esse processo é repetido tantas vezes quanto houver tabelas a serem juncionadas.
...
Um algoritmo de junção de laço aninhado (BNL) usa o buffer de linhas lidas em laços externos para reduzir o número de vezes que as tabelas em laços internos devem ser lidas. Por exemplo, se 10 linhas são lidas em um buffer e o buffer é passado para o próximo laço interno, cada linha lida no laço interno pode ser comparada com todas as 10 linhas no buffer. Isso reduz em uma ordem de magnitude o número de vezes que a tabela interna deve ser lida.

Antes do MySQL 8.0.18, este algoritmo era aplicado para junções equi quando não podiam ser usados índices; no MySQL 8.0.18 e versões posteriores, a otimização de junção hash é empregada nesses casos. A partir do MySQL 8.0.20, o loop aninhado de bloco não é mais usado pelo MySQL, e uma junção hash é empregada em todos os casos em que o loop aninhado de bloco foi usado anteriormente. Veja a Seção 10.2.1.4, “Otimização de Junção Hash”.

O bufferamento de junção no MySQL tem essas características:

* O join de buffer pode ser usado quando a junção é do tipo `ALL` ou `index` (ou seja, quando não podem ser usadas possíveis chaves e uma varredura completa é feita, seja nos dados ou nas linhas do índice, respectivamente), ou `range`. O uso de buffer também é aplicável a junções externas, conforme descrito na Seção 10.2.1.12, "Junções de Bloco de Busca e Acesso a Chave em Batel".
...
O número de varreduras de `t3` diminui à medida que o valor de `join_buffer_size` aumenta, até o ponto em que `join_buffer_size` é grande o suficiente para conter todas as combinações anteriores de linha. Nesse ponto, não se ganha velocidade ao torná-lo maior.

#### 10.2.1.8 Otimização de Conjunção Aninhada
...
Para as declarações da tabela fornecida, as duas expressões retornam conjuntos diferentes de linhas:
...
As consultas com junções externas aninhadas são executadas da mesma maneira que as consultas com junções internas. Mais precisamente, uma variação do algoritmo de junção de laço aninhado é explorada. Lembre-se do algoritmo pelo qual a junção de laço aninhado executa uma consulta (veja Seção 10.2.1.7, “Algoritmos de Junção de Laço Aninhado”). Suponha que uma consulta de junção sobre 3 tabelas `T1,T2,T3` tenha esta forma:
...
Aqui, `P1(T1,T2)` e `P2(T3,T3)` são algumas condições de junção (sobre expressões), enquanto `P(T1,T2,T3)` é uma condição sobre colunas de tabelas `T1,T2,T3`.
...
A notação `t1||t2||t3` indica uma linha construída concatenando as colunas das linhas `t1`, `t2` e `t3`. Em alguns dos exemplos a seguir, `NULL` onde aparece um nome de tabela significa uma linha na qual `NULL` é usado para cada coluna dessa tabela. Por exemplo, `t1||t2||NULL` indica uma linha construída concatenando as colunas das linhas `t1` e `t2`, e `NULL` para cada coluna de `t3`. Tal linha é dita `NULL`-completada.
...
Em geral, para qualquer laço aninhado na primeira tabela interna em uma operação de junção externa, é introduzida uma bandeira que é desligada antes do laço e verificada após o laço. A bandeira é ativada quando, para a linha atual da tabela externa, é encontrada uma correspondência na tabela que representa o operando interno. Se, no final do ciclo do laço, a bandeira ainda estiver desligada, não foi encontrada nenhuma correspondência para a linha atual da tabela externa. Neste caso, a linha é complementada com os valores `NULL` para as colunas das tabelas internas. A linha de resultado é passada para a verificação final para a saída ou para o próximo laço aninhado, mas apenas se a linha satisfazer a condição de junção de todas as junções externas incorporadas.
...
#### 10.2.1.9 Otimização de Conjunção Externa
...
* A condição `LEFT JOIN` é usada para decidir como recuperar as linhas da tabela *`B`*. (Em outras palavras, qualquer condição na cláusula `WHERE` não é usada.)
...
* Se houver uma linha em *`A`* que corresponda à cláusula `WHERE`, mas não houver uma linha em *`B`* que corresponda à condição `ON`, uma linha extra *`B`* é gerada com todas as colunas definidas como `NULL`.

* Se você usar `LEFT JOIN` para encontrar linhas que não existem em alguma tabela e você tiver o seguinte teste: `col_name IS NULL` na parte `WHERE`, onde *`col_name`* é uma coluna declarada como `NOT NULL`, o MySQL para de procurar mais linhas (para uma combinação de chave particular) depois de encontrar uma linha que corresponde à condição `LEFT JOIN`.

A implementação do `RIGHT JOIN` é análoga à do `LEFT JOIN`, com as tabelas trocadas de posição. As junções direitas são convertidas em junções equivalentes à esquerda, conforme descrito na Seção 10.2.1.10, “Simplificação de Junção Externa”.

Para um `LEFT JOIN`, se a condição `WHERE` sempre for falsa para a linha gerada `NULL`, o `LEFT JOIN` é alterado para uma junção interna. Por exemplo, a cláusula `WHERE` seria falsa na seguinte consulta se `t2.column1` fosse `NULL`:
...
Em MySQL 8.0.14 e versões posteriores, as condições triviais `WHERE` que surgem de expressões literais constantes são removidas durante a preparação, em vez de em uma etapa posterior na otimização, momento em que as junções já haviam sido simplificadas. A remoção anterior das condições triviais permite que o otimizador converta as junções externas em junções internas; isso pode resultar em planos aprimorados para consultas com junções externas que contêm condições triviais na cláusula `WHERE`, como esta:
...
Agora, o otimizador pode usar a tabela `t2` antes da tabela `t1` se isso resultar em um plano de consulta melhor. Para fornecer uma dica sobre a ordem de junção da tabela, use dicas do otimizador; veja Seção 10.9.3, “Dicas do Otimizador”. Alternativamente, use `STRAIGHT_JOIN`; veja Seção 15.2.13, “Instrução SELECT”. No entanto, `STRAIGHT_JOIN` pode impedir que índices sejam usados porque desativa transformações de junção parcial; veja [Seção 10.2.2.1, “Otimizando Predicados de Subconsultas IN e EXISTS com Transformações de Junção Parcial”](semijoins.html "10.2.2.1 Optimizing IN and EXISTS Subquery Predicates with Semijoin Transformations").
...
Todas as expressões de junção interna na forma `T1 INNER JOIN T2 ON P(T1,T2)` são substituídas pela lista `T1,T2`, sendo `P(T1,T2)` unido como conjunto à condição `WHERE` (ou à condição de junção da junção de incorporação, se houver).
...
Condições como essas são rejeitadas porque não podem ser verdadeiras para qualquer linha `NULL` (com as colunas `T2` definidas como `NULL`):
...
Condições como essas não são rejeitadas como nulas porque podem ser verdadeiras para uma linha `NULL` complementada:
...
#### 10.2.1.11 Otimização da leitura em várias faixas de faixa
#### 10.2.1.12 Otimização da leitura em várias faixas de faixa

Ler linhas usando uma varredura de intervalo em um índice secundário pode resultar em muitos acessos aleatórios ao disco na tabela base quando a tabela é grande e não está armazenada no cache do mecanismo de armazenamento. Com a otimização de Leitura Multi-Intervalo de Varredura em Disco (MRR), o MySQL tenta reduzir o número de acessos aleatórios ao disco para varreduras de intervalo, primeiro verificando o índice apenas e coletando as chaves para as linhas relevantes. Em seguida, as chaves são ordenadas e, finalmente, as linhas são recuperadas da tabela base usando a ordem da chave primária. A motivação do MRR de varredura em disco é reduzir o número de acessos aleatórios ao disco e, em vez disso, alcançar uma varredura mais sequencial dos dados da tabela base.
...
2. Os tuplos no buffer são ordenados por seu ID da linha de dados.  
3. As linhas de dados são acessadas de acordo com a sequência de tuplos do índice ordenada.
...
#### 10.2.1.12 Conjuntos de acesso a chave em lote e junções de laço aninhado em bloco
...
As seções a seguir discutem a gestão do buffer de junção que está por trás da extensão do algoritmo original BNL, o algoritmo BNL estendido e o algoritmo BKA. Para informações sobre estratégias de semijoin, consulte [Seção 10.2.2.1, “Otimizando predicados de subconsultas IN e EXISTS com transformações de semijoin”][(semijoins.html "10.2.2.1 Optimizing IN and EXISTS Subquery Predicates with Semijoin Transformations")]

* [Participe da Gerenciamento de Buffer para Algoritmos de Acesso a Chave em Bloco e Acesso a Chave em Batelagem](bnl-bka-optimization.html#join-buffer-management "Join Buffer Management for Block Nested-Loop and Batched Key Access Algorithms")
...
O MySQL pode empregar tampões de junção para executar não apenas junções internas sem acesso ao índice da tabela interna, mas também junções externas e semijoções que aparecem após o achatamento de subconsultas. Além disso, um tampão de junção pode ser efetivamente usado quando há acesso ao índice da tabela interna.

O código de gerenciamento do buffer de junção utiliza de forma um pouco mais eficiente o espaço do buffer de junção ao armazenar os valores das colunas das linhas de interesse: não são alocados bytes adicionais nos buffers para uma coluna de linha se seu valor for `NULL`, e o número mínimo de bytes é alocado para qualquer valor do tipo `VARCHAR`.
...
* Um buffer de junção regular contém colunas de cada operando de junção. Se `B2` é um buffer de junção regular, cada linha *`r`* inserida em `B2` é composta pelas colunas de uma linha *`r1`* de `B1` e pelas colunas de interesse de uma linha correspondente *`r2`* da tabela `t3`.

* Um buffer de junção incremental contém apenas as colunas das linhas da tabela produzida pelo segundo operador de junção. Isso significa que é incremental em relação a uma linha do buffer do primeiro operador. Se `B2` for um buffer de junção incremental, ele contém as colunas interessantes da linha *`r2`* juntamente com um link para a linha *`r1`* de `B1`.
...
Cada linha do buffer incremental utilizado para uma operação de junção contém apenas as colunas interessantes de uma linha da tabela que será juncionada. Essas colunas são aumentadas com uma referência às colunas interessantes da linha correspondente da tabela produzida pelo primeiro operador de junção. Várias linhas no buffer incremental podem se referir à mesma linha *`r`* cujas colunas são armazenadas nos buffers de junção anteriores, na medida em que todas essas linhas correspondem à linha *`r`*.

Os buffers incrementais permitem uma cópia menos frequente de colunas dos buffers utilizados para operações de junção anteriores. Isso proporciona uma economia de espaço no buffer, pois, no caso geral, uma linha produzida pelo primeiro operador de junção pode ser correspondida por várias linhas produzidas pelo segundo operador de junção. Não é necessário fazer várias cópias de uma linha do primeiro operador. Os buffers incrementais também proporcionam uma economia de tempo de processamento devido à redução do tempo de cópia.

Em MySQL 8.0, a bandeira `block_nested_loop` da variável de sistema `optimizer_switch` funciona da seguinte forma:

* Antes do MySQL 8.0.20, ele controla como o otimizador usa o algoritmo de junção Block Nested Loop.

* Em MySQL 8.0.18 e versões posteriores, também controla o uso de junções hash (consulte a Seção 10.2.1.4, “Otimização de junções hash”).

* A partir do MySQL 8.0.20, a bandeira controla apenas as junções hash, e o algoritmo de loop aninhado não é mais suportado.

A bandeira `batched_key_access` controla como o otimizador usa os algoritmos de junção de acesso por chave em lote.

Por padrão, `block_nested_loop` é `on` e `batched_key_access` é `off`. Veja a Seção 10.9.2, “Otimizações Desconectables”. Também podem ser aplicadas dicas de otimizador; veja Dicas de Otimizador para Algoritmos de Acesso a Chave em Bloco e de Acesso a Chave em Batel.

Para informações sobre estratégias de semijoin, consulte [Seção 10.2.2.1, “Otimizando predicados de subconsultas IN e EXISTS com transformações de semijoin”][(semijoins.html "10.2.2.1 Optimizing IN and EXISTS Subquery Predicates with Semijoin Transformations")]

##### Algoritmo de Bloco de Percurso Aninhado para Conjunções Externas e Semiconjunções

A implementação original do algoritmo BNL do MySQL foi estendida para suportar operações de junção externa e semijoin (e foi posteriormente substituída pelo algoritmo de junção hash; veja Seção 10.2.1.4, “Otimização da Junção Hash”).
...
Se uma operação de junção externa for executada usando um buffer de junção, cada linha da tabela produzida pelo segundo operador é verificada para uma correspondência com cada linha no buffer de junção. Quando uma correspondência é encontrada, uma nova linha estendida é formada (a linha original mais as colunas do segundo operador) e enviada para extensões adicionais pelas operações de junção restantes. Além disso, a bandeira de correspondência da linha correspondente no buffer é ativada. Após todas as linhas da tabela a ser juncionada terem sido examinadas, o buffer de junção é verificado. Cada linha do buffer que não tenha sua bandeira de correspondência ativada é estendida por complementos `NULL` (valores `NULL` para cada coluna no segundo operador) e enviada para extensões adicionais pelas operações de junção restantes.
...
Consulte a Seção 10.9.2, “Otimizações Desconectables”, para obter mais informações. Os indicadores de otimização também podem ser aplicados; consulte Indicadores de Otimização para Algoritmos de Acesso a Chave em Bloco e de Acesso a Chave em Massa.
...
O MySQL implementa um método de junção de tabelas chamado algoritmo de acesso em lote (BKA) de chave. O BKA pode ser aplicado quando há um acesso ao índice da tabela produzido pelo segundo operador de junção. Assim como o algoritmo de junção BNL, o algoritmo de junção BKA emprega um buffer de junção para acumular as colunas interessantes das linhas produzidas pelo primeiro operador da operação de junção. Em seguida, o algoritmo BKA constrói chaves para acessar a tabela a ser juncionada para todas as linhas no buffer e envia essas chaves em um lote para o motor do banco de dados para buscas de índice. As chaves são enviadas para o motor através da interface de leitura de várias faixas (MRR) (ver Seção 10.2.1.11, “Otimização de Leitura de Várias Faixas”). Após a submissão das chaves, as funções do motor MRR realizam buscas no índice de maneira otimizada, obtendo as linhas da tabela juncionada encontradas por essas chaves e começa a alimentar o algoritmo de junção BKA com linhas correspondentes. Cada linha correspondente é acoplada com uma referência a uma linha no buffer de junção.
...
* O primeiro cenário é utilizado para motores de armazenamento baseados em disco convencional, como `InnoDB` e `MyISAM`. Para esses motores, geralmente as chaves de todas as linhas do buffer de junção são enviadas para a interface MRR de uma vez. As funções MRR específicas do motor realizam pesquisas de índice para as chaves enviadas, obtêm IDs de linha (ou chaves primárias) a partir delas e, em seguida, buscam linhas para todos esses IDs de linha selecionados um a um por solicitação do algoritmo BKA. Cada linha é devolvida com uma referência de associação que permite o acesso à linha correspondente no buffer de junção. As linhas são obtidas pelas funções MRR de uma maneira otimizada: são obtidas na ordem do ID de linha (chave primária). Isso melhora o desempenho porque as leituras são em ordem de disco em vez de ordem aleatória.

* O segundo cenário é utilizado para motores de armazenamento remoto, como `NDB`. Um pacote de chaves para uma porção de linhas do buffer de junção, juntamente com suas associações, é enviado por um servidor MySQL (nó SQL) para os nós de dados do MySQL Cluster. Em troca, o nó SQL recebe um pacote (ou vários pacotes) de linhas correspondentes, juntamente com as associações correspondentes. O algoritmo de junção BKA toma essas linhas e constrói novas linhas juncionadas. Em seguida, um novo conjunto de chaves é enviado para os nós de dados e as linhas dos pacotes retornados são usadas para construir novas linhas juncionadas. O processo continua até que as últimas chaves do buffer de junção sejam enviadas para os nós de dados, e o nó SQL tenha recebido e juncionado todas as linhas que correspondem a essas chaves. Isso melhora o desempenho, pois menos pacotes que carregam chaves enviados pelo nó SQL para os nós de dados significa menos viagens entre ele e os nós de dados para realizar a operação de junção.

No primeiro cenário, uma parte do buffer de junção é reservada para armazenar IDs de linha (chaves primárias) selecionados por pesquisas de índice e passados como um parâmetro para as funções MRR.

Não há um buffer especial para armazenar chaves construídas para as linhas do buffer de junção. Em vez disso, uma função que constrói a chave para a próxima linha no buffer é passada como um parâmetro para as funções MRR.
...
Além de usar a variável de sistema `optimizer_switch` para controlar o uso do otimizador dos algoritmos BNL e BKA em nível de sessão, o MySQL suporta dicas de otimizador para influenciar o otimizador em uma base por declaração. Veja a Seção 10.9.3, “Dicas de Otimizador”.
...
No processamento de junção, as linhas prefixadas são aquelas linhas passadas de uma tabela em uma junção para a próxima. Em geral, o otimizador tenta colocar tabelas com baixo número de prefixos no início da ordem de junção para evitar que o número de combinações de linhas aumente rapidamente. Na medida em que o otimizador pode usar informações sobre condições em linhas selecionadas de uma tabela e passadas para a próxima, mais precisamente ele pode calcular estimativas de linhas e escolher o melhor plano de execução.

Sem filtragem condicional, o número de linhas do prefixo para uma tabela é baseado no número estimado de linhas selecionadas pela cláusula `WHERE`, de acordo com o método de acesso que o otimizador escolher. A filtragem condicional permite que o otimizador use outras condições relevantes na cláusula `WHERE` que não são levadas em conta pelo método de acesso, e assim melhore suas estimativas de número de linhas do prefixo. Por exemplo, embora possa haver um método de acesso baseado em índice que pode ser usado para selecionar linhas da tabela atual em uma junção, também pode haver condições adicionais para a tabela na cláusula `WHERE` que podem filtrar (restrição adicional) a estimativa para as linhas qualificadas passadas para a próxima tabela.
...
Na saída de `EXPLAIN`, a coluna `rows` indica a estimativa da linha para o método de acesso escolhido, e a coluna `filtered` reflete o efeito do filtro de condição. Os valores de `filtered` são expressos como porcentagem. O valor máximo é de 100, o que significa que não houve filtragem de linhas. Valores que diminuem de 100 indicam quantidades crescentes de filtragem.

O número de linhas prefixado (o número de linhas estimadas para serem passadas da tabela atual em uma junção para a próxima) é o produto dos valores de `rows` e `filtered`. Ou seja, o número de linhas prefixado é o número de linhas estimado, reduzido pelo efeito de filtragem estimado. Por exemplo, se `rows` é 1000 e `filtered` é 20%, o filtragem de condição reduz o número de linhas estimado de 1000 a um número de linhas prefixado de 1000 × 20% = 1000 × .2 = 200.
...
Para `employee`, o método de acesso no índice `name` coleta as 8 linhas que correspondem a um nome de `'John'`. Não há filtragem (`filtered` é 100%), portanto, todas as linhas são linhas prefixadas para a próxima tabela: O número de linhas prefixadas é `rows` × `filtered` = 8 × 100% = 8.
...
Normalmente, o otimizador não calcula o efeito de filtragem de condição (redução do número de linhas de pré-préfixo) para a última tabela associada, porque não há próxima tabela para passar as linhas. Uma exceção ocorre para `EXPLAIN`: Para fornecer mais informações, o efeito de filtragem é calculado para todas as tabelas associadas, incluindo a última.

Para controlar se o otimizador considera condições de filtragem adicionais, use a bandeira `condition_fanout_filter` da variável de sistema `optimizer_switch` (consulte Seção 10.9.2, “Otimizações Desconectables”). Essa bandeira é ativada por padrão, mas pode ser desativada para suprimir o filtragem das condições (por exemplo, se uma consulta específica for encontrada para produzir um melhor desempenho sem ela).
...
* Se uma coluna não estiver indexada, indexe-a para que o otimizador tenha alguma informação sobre a distribuição dos valores da coluna e possa melhorar suas estimativas de linha.

* Da mesma forma, se não houver informações sobre histograma de coluna disponíveis, gere um histograma (consulte a Seção 10.9.6, “Estatísticas do otimizador”).

* Altere a ordem de junção. As formas de realizar isso incluem dicas de otimização de ordem de junção (consulte Seção 10.9.3, “Dicas de otimizador”), `STRAIGHT_JOIN` imediatamente após o `SELECT`, e o operador de junção `STRAIGHT_JOIN`.
...
As comparações entre constantes e valores de coluna em que o valor constante está fora do intervalo ou do tipo errado em relação ao tipo da coluna são agora tratadas uma vez durante a otimização da consulta, em vez de ser tratada linha a linha durante a execução. As comparações que podem ser tratadas dessa maneira são `>`, `>=`, `<`, `<=`, `<>`/`!=`, `=` e `<=>`.
...
+ **Valor inteiro.** Se a constante estiver fora do intervalo para o tipo de coluna, a comparação é reduzida a `1` ou `IS NOT NULL`, conforme já mostrado.

Se a constante for um limite de intervalo, a comparação é preenchida com `=`. Por exemplo (usando a mesma tabela já definida):
...
+ Valor de ponto flutuante ou fixo. Se a constante for um dos tipos decimais (como `DECIMAL`, `REAL`, `DOUBLE` ou `FLOAT`) e tiver uma parte decimal não nula, não pode ser igual; dobre conforme necessário. Para outras comparações, arredonde para cima ou para baixo para um valor inteiro de acordo com o sinal, em seguida, realize uma verificação de intervalo e trate como já descrito para comparações inteiro-inteiro.

Um valor `REAL` que é muito pequeno para ser representado como `DECIMAL` é arredondado para .01 ou -.01, dependendo do sinal, e, em seguida, tratado como um `DECIMAL`.

+ **Tipos de cadeia de caracteres.** Tente interpretar o valor da cadeia de caracteres como um tipo de número inteiro, e, em seguida, realize a comparação como se fosse entre valores inteiros. Se isso falhar, tente tratar o valor como um `REAL`.

* **Coluna DECIMAL ou REAL.** Os tipos decimais são comparados com constantes dos seguintes tipos, conforme descrito aqui:

+ **Valor inteiro.** Realize uma verificação de intervalo contra a parte inteira do valor da coluna. Se não houver resultados de dobragem, converta a constante em `DECIMAL` com o mesmo número de casas decimais que o valor da coluna, em seguida, verifique-a como um `DECIMAL` (consulte a próxima).

+ **Valor DECIMAL ou REAL.** Verifique se há excesso (ou seja, se a constante tem mais dígitos em sua parte inteira do que o permitido para o tipo decimal da coluna). Se sim, feche.

Se a constante tiver mais dígitos fracionários significativos do que o tipo da coluna, trunque a constante. Se o operador de comparação for `=` ou `<>`, dobre. Se o operador for `>=` ou `<=`, ajuste o operador devido ao truncamento. Por exemplo, se o tipo da coluna for `DECIMAL(3,1)`, `SELECT * FROM t WHERE f >= 10.13` se torna `SELECT * FROM t WHERE f > 10.1`.

Se a constante tiver menos dígitos decimais do que o tipo da coluna, converta-a em uma constante com o mesmo número de dígitos. Para subfluxo de um valor de `REAL` (ou seja, com poucos dígitos fracionários para representá-lo), converta a constante para decimal 0.

+ **Valor de string.** Se o valor puder ser interpretado como um tipo de número inteiro, trate-o como tal. Caso contrário, tente tratá-lo como `REAL`.

* Coluna **FLOAT ou DOUBLE.** Os valores `FLOAT(m,n)` ou `DOUBLE(m,n)` em comparação com as constantes são tratados da seguinte forma:

Se o valor ultrapassar o intervalo da coluna, dobrar.

Se o valor tiver mais de *`n`* casas decimais, realize o corte, compensando durante o dobramento. Para comparações de `=` e `<>`, dobre para `TRUE`, `FALSE` ou `IS [NOT] NULL`, conforme descrito anteriormente; para outros operadores, ajuste o operador.

Se o valor tiver mais de `m` dígitos inteiros, dobre.

**Limitações.** Essa otimização não pode ser usada nos seguintes casos:

1. Com comparações usando `BETWEEN` ou `IN`.

2. Com colunas `BIT` ou colunas que utilizam tipos de data ou hora.

3. Durante a fase de preparação para uma declaração preparada, embora possa ser aplicada durante a fase de otimização quando a declaração preparada é executada. Isso ocorre porque, durante a preparação da declaração, o valor da constante ainda não é conhecido.

#### 10.2.1.15 Otimização de IS NULL

O MySQL pode realizar a mesma otimização no *`col_name`* [`IS NULL`](comparison-operators.html#operator_is-null) que ele pode usar para *`col_name`* `=` *`constant_value`*. Por exemplo, o MySQL pode usar índices e faixas para procurar `NULL` com [`IS NULL`](comparison-operators.html#operator_is-null).
...
Se uma cláusula `WHERE` incluir uma condição *`col_name`* [`IS NULL`](comparison-operators.html#operator_is-null) para uma coluna declarada como `NOT NULL`, essa expressão é otimizada. Essa otimização não ocorre em casos em que a coluna pode produzir `NULL` de qualquer maneira (por exemplo, se ela vem de uma tabela do lado direito de um `LEFT JOIN`).
...
Essa otimização pode lidar com um `IS NULL`(comparison-operators.html#operator_is-null) para qualquer peça chave.
...
A otimização pode lidar apenas com um nível de `IS NULL`(comparison-operators.html#operator_is-null). Na seguinte consulta, o MySQL usa buscas por chave apenas na expressão `(t1.a=t2.a AND t2.a IS NULL)` e não é capaz de usar a parte da chave em `b`:
...
No entanto, a consulta utiliza `SELECT *`, que pode selecionar mais colunas do que *`key_part1`* e *`key_part2`*. Nesse caso, digitalizar um índice inteiro e procurar linhas de tabela para encontrar colunas que não estão no índice pode ser mais caro do que digitalizar a tabela e ordenar os resultados. Se assim for, o otimizador provavelmente não usa o índice. Se `SELECT *` seleciona apenas as colunas do índice, o índice é usado e a ordenação é evitada.
...
Se uma consulta misturar `ASC` e `DESC`, o otimizador pode usar um índice nas colunas se o índice também usar colunas mistas correspondentes ascendentes e descendentes:
...
* Na próxima consulta, o `ORDER BY` não nomeia *`key_part1`*, mas todas as linhas selecionadas têm um valor constante *`key_part1`*, então o índice ainda pode ser usado:
...
* O índice usado para obter as linhas difere do usado no `ORDER BY`:

  ```
  SELECT * FROM t1 WHERE key2=constant ORDER BY key1;
  ```

* A consulta utiliza `ORDER BY` com uma expressão que inclui termos além do nome da coluna de índice:
...
* A consulta une muitas tabelas, e as colunas no `ORDER BY` não são todas da primeira tabela não constante que é usada para recuperar linhas. (Esta é a primeira tabela no `EXPLAIN` de saída que não tem um tipo de junção `const`.)
...
Anteriormente (MySQL 5.7 e versões inferiores), `GROUP BY` era ordenado implicitamente sob certas condições. No MySQL 8.0, isso não ocorre mais, portanto, especificar `ORDER BY NULL` no final para suprimir o ordenamento implícito (como era feito anteriormente) não é mais necessário. No entanto, os resultados das consultas podem diferir das versões anteriores do MySQL. Para produzir um determinado ordem de classificação, forneça uma cláusula `ORDER BY`.

##### Uso do filesort para satisfazer a cláusula ORDER BY

Se um índice não puder ser usado para satisfazer uma cláusula de `ORDER BY`, o MySQL realiza uma operação de `filesort` que lê as linhas da tabela e as ordena. Um `filesort` constitui uma fase de ordenação extra na execução da consulta.

Para obter memória para operações de `filesort`, a partir do MySQL 8.0.12, o otimizador aloca buffers de memória incrementalmente conforme necessário, até o tamanho indicado pela variável de sistema `sort_buffer_size`, em vez de alocar um montante fixo de `sort_buffer_size` bytes de antemão, como era feito antes do MySQL 8.0.12. Isso permite que os usuários definam `sort_buffer_size` para valores maiores para acelerar operações maiores, sem preocupação com o uso excessivo de memória para operações menores. (Esse benefício pode não ocorrer em múltiplos tipos de operações concorrentes em Windows, que tem um `malloc` multithread fraco).
...
Tais consultas são comuns em aplicativos da web que exibem apenas algumas linhas de um conjunto de resultados maior. Exemplos:
...
Para consultas lentas do `ORDER BY` para as quais o `filesort` não é usado, tente diminuir a variável de sistema `max_length_for_sort_data` para um valor apropriado para desencadear um `filesort`. (Um sintoma de definir o valor desta variável muito alto é uma combinação de alta atividade de disco e baixa atividade da CPU.) Esta técnica só se aplica antes do MySQL 8.0.20. A partir do 8.0.20, o `max_length_for_sort_data` é desatualizado devido a mudanças no otimizador que o tornam obsoleto e sem efeito.
...
* Aumente o valor da variável `sort_buffer_size`. Idealmente, o valor deve ser grande o suficiente para que todo o conjunto de resultados se encaixe no buffer de ordenação (para evitar gravações no disco e passes de junção).

Tenha em conta que o tamanho dos valores das colunas armazenados no buffer de ordenação é afetado pelo valor da variável de sistema `max_sort_length`. Por exemplo, se os tuplos armazenam valores de colunas de longa cadeia de caracteres e você aumenta o valor de `max_sort_length`, o tamanho dos tuplos do buffer de ordenação também aumenta e pode exigir que você aumente `sort_buffer_size`.
...
* Aumente o valor da variável `read_rnd_buffer_size` para que mais linhas sejam lidas de cada vez.
...
Com `EXPLAIN` (consulte a Seção 10.8.1, “Otimizando consultas com EXPLAIN”), você pode verificar se o MySQL pode usar índices para resolver uma cláusula `ORDER BY`:

* Se a coluna `Extra` do `EXPLAIN` de saída não contiver `Using filesort`, o índice é utilizado e não é realizada uma `filesort`.

* Se a coluna `Extra` do `EXPLAIN` de saída contiver `Using filesort`, o índice não é utilizado e é realizada uma `filesort`.
...
`peak_memory_used` indica a memória máxima usada em qualquer momento durante a ordenação. Esse é um valor que pode ser até, mas não necessariamente tão grande quanto o valor da variável de sistema `sort_buffer_size`. Antes do MySQL 8.0.12, a saída mostra `sort_buffer_size` em vez disso, indicando o valor de `sort_buffer_size`. (Antes do MySQL 8.0.12, o otimizador aloca sempre `sort_buffer_size` bytes para o buffer de ordenação. A partir do 8.0.12, o otimizador aloca memória de buffer de ordenação incrementalmente, começando com uma pequena quantidade e adicionando mais conforme necessário, até `sort_buffer_size` bytes.)
...
* Quaisquer outras partes do índice que não sejam as referenciadas no `GROUP BY` na consulta devem ser constantes (ou seja, devem ser referenciadas em igualdades com constantes), exceto o argumento das funções `MIN()` ou `MAX()`.
...
Se a consulta incluir `WHERE c3 = const`, pode ser usado o Scan de Índice Solto.
...
Devido a essa equivalência, as otimizações aplicáveis às consultas de `GROUP BY` também podem ser aplicadas a consultas com uma cláusula de `DISTINCT`. Assim, para mais detalhes sobre as possibilidades de otimização para consultas de `DISTINCT`, consulte a Seção 10.2.1.17, “Otimização por GROUP BY”.

Quando se combina `LIMIT row_count` com `DISTINCT`, o MySQL para assim que encontrar *`row_count`* linhas únicas.

Se você não usar colunas de todas as tabelas mencionadas em uma consulta, o MySQL para de analisar quaisquer tabelas não utilizadas assim que encontrar a primeira correspondência. No caso a seguir, assumindo que `t1` é usado antes de `t2` (que você pode verificar com `EXPLAIN`), o MySQL para de ler a partir de `t2` (para qualquer linha específica em `t1`) quando encontrar a primeira linha em `t2`:
...
Se você precisa apenas de um número especificado de linhas de um conjunto de resultados, use uma cláusula `LIMIT` na consulta, em vez de obter todo o conjunto de resultados e descartar os dados extras.

O MySQL, às vezes, otimiza uma consulta que tem uma cláusula `LIMIT row_count` e nenhuma cláusula `HAVING`:

* Se você selecionar apenas algumas linhas com `LIMIT`, o MySQL usa índices em alguns casos, quando normalmente preferiria realizar uma varredura completa da tabela.

* Se você combinar `LIMIT row_count` com `ORDER BY`, o MySQL para de ordenar assim que encontrar as primeiras linhas *`row_count`* do resultado ordenado, em vez de ordenar todo o resultado. Se a ordenação for feita usando um índice, isso é muito rápido. Se um filesort deve ser feito, todas as linhas que correspondem à consulta sem a cláusula `LIMIT` são selecionadas e a maioria ou todas elas são ordenadas, antes de serem encontradas as primeiras *`row_count`*. Após as primeiras linhas terem sido encontradas, o MySQL não ordena nenhum restante do conjunto de resultados.
...
* Se você combinar `LIMIT row_count` com `DISTINCT`, o MySQL para assim que encontrar *`row_count`* linhas únicas.
...
* Assim que o MySQL tiver enviado o número necessário de linhas ao cliente, ele interrompe a consulta, a menos que você esteja usando `SQL_CALC_FOUND_ROWS`. Nesse caso, o número de linhas pode ser recuperado com `SELECT FOUND_ROWS()`. Veja a Seção 14.15, “Funções de Informação”.

* `LIMIT 0` rapidamente retorna um conjunto vazio. Isso pode ser útil para verificar a validade de uma consulta. Também pode ser empregado para obter os tipos das colunas de resultado dentro de aplicativos que utilizam uma API MySQL que disponibiliza metadados do conjunto de resultados. Com o programa cliente **mysql**, você pode usar a opção `--column-type-info` para exibir os tipos das colunas de resultado.
...
* Se um índice não for usado para `ORDER BY`, mas também houver uma cláusula `LIMIT`, o otimizador pode evitar o uso de um arquivo de junção e ordenar as linhas na memória usando uma operação `filesort` de memória.

Se várias linhas tiverem valores idênticos nas colunas `ORDER BY`, o servidor pode retornar essas linhas em qualquer ordem, e pode fazer isso de maneira diferente, dependendo do plano de execução geral. Em outras palavras, o ordem de classificação dessas linhas é não determinística em relação às colunas não ordenadas.

Um fator que afeta o plano de execução é `LIMIT`, portanto, uma consulta `ORDER BY` com e sem `LIMIT` pode retornar linhas em ordens diferentes. Considere esta consulta, que é ordenada pela coluna `category`, mas não determinada em relação às colunas `id` e `rating`:
...
Para uma consulta com uma cláusula `ORDER BY` ou `GROUP BY` e uma cláusula `LIMIT`, o otimizador tenta escolher um índice ordenado por padrão, quando isso parecer acelerar a execução da consulta. Antes do MySQL 8.0.21, não havia como sobrepor esse comportamento, mesmo em casos em que o uso de outra otimização poderia ser mais rápido. A partir do MySQL 8.0.21, é possível desativar essa otimização, definindo a bandeira `prefer_ordering_index` da variável de sistema `optimizer_switch` como `off`.
...
Veja também a Seção 10.9.2, “Otimizações Desconectables”.
...
As funções não determinísticas podem afetar o desempenho das consultas. Por exemplo, algumas otimizações podem não estar disponíveis, ou pode ser necessário mais bloqueio. A discussão a seguir utiliza `RAND()`, mas se aplica também a outras funções não determinísticas.
...
* A segunda consulta contém uma expressão que utiliza a função não determinística `RAND()`, que não é constante na consulta, mas na verdade tem um novo valor para cada linha da tabela `t`. Consequentemente, a consulta lê todas as linhas da tabela, avalia o predicado para cada linha e emite todas as linhas para as quais a chave primária corresponde ao valor aleatório. Isso pode ser zero, um ou várias linhas, dependendo dos valores da coluna `id` e dos valores na sequência `RAND()`.

Os efeitos do não determinismo não se limitam às declarações `SELECT`. Esta declaração `UPDATE` utiliza uma função não determinística para selecionar as linhas que serão modificadas:

```
UPDATE t SET col_a = some_expr WHERE id = FLOOR(1 + RAND() * 49);
```

Presumivelmente, a intenção é atualizar no máximo uma única linha para a qual a chave primária corresponda à expressão. No entanto, ela pode atualizar zero, uma ou várias linhas, dependendo dos valores da coluna `id` e dos valores na sequência `RAND()`.
...
* `InnoDB` pode escalar para um bloqueio de chave de intervalo em vez de assumir um bloqueio de uma única linha para uma linha correspondente.
...
Se o otimizador puder usar `partial_key` para reduzir o conjunto de linhas selecionadas, `RAND()` é executado menos vezes, o que diminui o efeito do não determinismo na otimização.

#### 10.2.1.21 Otimização da função de janela

As funções de janela afetam as estratégias que o otimizador considera:

* A fusão de tabela derivada para uma subconsulta é desativada se a subconsulta tiver funções de janela. A subconsulta é sempre materializada.

* Os semijoins não são aplicáveis à otimização de funções de janela porque os semijoins se aplicam a subconsultas em `WHERE` e `JOIN ... ON`, que não podem conter funções de janela.

* O otimizador processa várias janelas que têm os mesmos requisitos de ordem em sequência, portanto, o ordenamento pode ser ignorado para as janelas que seguem a primeira.

* O otimizador não tenta combinar janelas que poderiam ser avaliadas em um único passo (por exemplo, quando várias cláusulas `OVER` contêm definições de janela idênticas). A solução é definir a janela em uma cláusula `WINDOW` e referenciar o nome da janela nas cláusulas `OVER`.

Uma função agregada que não é usada como uma função de janela é agregada na consulta mais externa possível. Por exemplo, nesta consulta, o MySQL percebe que `COUNT(t1.b)` é algo que não pode existir na consulta externa devido à sua colocação na cláusula `WHERE`:

```
SELECT * FROM t1 WHERE t1.a = (SELECT COUNT(t1.b) FROM t2);
```

Consequentemente, o MySQL agrega os dados internos da subconsulta, tratando `t1.b` como uma constante e retornando o número de linhas de `t2`.
...
O erro ocorre porque `COUNT(t1.b)` pode existir no `HAVING`, e, portanto, torna a consulta externa agregada.
...
Para alguns agregados de frames em movimento, a função agregada inversa pode ser aplicada para remover valores do agregado. Isso pode melhorar o desempenho, mas possivelmente com uma perda de precisão. Por exemplo, adicionar um valor muito pequeno em ponto flutuante a um valor muito grande faz com que o valor muito pequeno seja "escondido" pelo grande valor. Quando a inversão do grande valor é feita posteriormente, o efeito do pequeno valor é perdido.
...
* Para valores de ponto flutuante, a agregação inversa não é sempre segura e pode resultar na perda de precisão. O padrão é evitar a agregação inversa, que é mais lenta, mas preserva a precisão. Se for permitido sacrificar a segurança pela velocidade, `windowing_use_high_precision` pode ser desativado para permitir a agregação inversa.
...
* `windowing_use_high_precision` não tem efeito sobre `MIN()` e `MAX()`, que não utilizam agregação inversa em nenhum caso.

Para a avaliação das funções de variância `STDDEV_POP()`, `STDDEV_SAMP()`, `VAR_POP()`, `VAR_SAMP()` e seus sinônimos, a avaliação pode ocorrer no modo otimizado ou no modo padrão. O modo otimizado pode produzir resultados ligeiramente diferentes nas últimas casas decimais. Se tais diferenças forem permitidas, `windowing_use_high_precision` pode ser desativado para permitir o modo otimizado.

Para `EXPLAIN`, o plano de execução de janela é muito extenso para ser exibido no formato de saída tradicional. Para ver as informações de janela, use `EXPLAIN FORMAT=JSON`(explain.html "15.8.2 EXPLAIN Statement") e procure pelo elemento `windowing`.

#### 10.2.1.22 Otimização da expressão do construtor de linha
...
O otimizador é menos provável que use índices disponíveis se as colunas do construtor de linha não cobrem o prefixo de um índice. Considere a tabela a seguir, que tem uma chave primária em `(c1, c2, c3)`:
...
Nesses casos, a reescrita da expressão do construtor de linha usando uma expressão não-construtor equivalente pode resultar em um uso de índice mais completo. Para a consulta dada, as expressões do construtor de linha e não-construtor equivalente são:
...
Assim, para obter melhores resultados, evite misturar construtores de linha com expressões `AND`/`OR`. Use uma ou a outra.

Sob certas condições, o otimizador pode aplicar o método de acesso de intervalo às expressões `IN()` que possuem argumentos do construtor de linha. Veja Otimização de intervalo de expressões de construtor de linha.

#### 10.2.1.23 Evitando varreduras completas da tabela

A saída de `EXPLAIN` mostra `ALL` na coluna `type` quando o MySQL usa uma varredura completa da tabela para resolver uma consulta. Isso geralmente acontece nas seguintes condições:

* A tabela é tão pequena que é mais rápido realizar uma varredura da tabela do que se preocupar com uma busca de chave. Isso é comum para tabelas com menos de 10 linhas e um comprimento de linha curto.

* Não há restrições utilizáveis na cláusula `ON` ou `WHERE` para colunas indexadas.

* Você está comparando colunas indexadas com valores constantes e o MySQL calculou (com base na árvore de índice) que as constantes cobrem uma parte muito grande da tabela e que uma varredura da tabela seria mais rápida. Veja a Seção 10.2.1.1, “Otimização da cláusula WHERE”.

* Você está usando uma chave com baixa cardinalidade (muitas linhas correspondem ao valor da chave) através de outra coluna. Nesse caso, o MySQL assume que, ao usar a chave, provavelmente são necessários muitos procuros na chave e que uma varredura na tabela seria mais rápida.
...
* Use `ANALYZE TABLE tbl_name` para atualizar as distribuições de chave para a tabela digitalizada. Veja a Seção 15.7.3.1, “Declaração ANALYZE TABLE”.
...
* Inicie o **mysqld** com a opção `--max-seeks-for-key=1000` ou use `SET max_seeks_for_key=1000` para informar ao otimizador que nenhuma varredura de chave causa mais de 1.000 buscas de chave. Veja a Seção 7.1.8, “Variáveis do Sistema do Servidor”.

### 10.2.2 Otimizando subconsultas, tabelas derivadas, referências de visualização e expressões de tabela comum
...
Para uma tabela derivada, o otimizador tem essas opções (que também se aplicam a referências de visualização e expressões comuns de tabela):
...
Supondo que `class_num` seja uma chave primária na tabela `class`, a supressão de duplicatas é possível usando [`SELECT DISTINCT`](select.html "15.2.13 SELECT Statement"), mas é ineficiente gerar todas as linhas correspondentes primeiro e apenas eliminar as duplicatas posteriormente.
...
Em MySQL 8.0.16 e versões posteriores, qualquer declaração com um predicado de subconsulta `EXISTS` está sujeita às mesmas transformações de semijoin que uma declaração com um predicado de subconsulta equivalente `IN`.
...
Em suma, qualquer negação de uma subconsulta na forma de `IN (SELECT ... FROM ...)` ou `EXISTS (SELECT ... FROM ...)` é transformada em um antijoin.
...
Essa consulta é reescrita internamente como o antijoin `SELECT class_num, class_name FROM class ANTIJOIN roster ON class_num`, que retorna uma instância de cada linha em `class` que não seja correspondida por nenhuma linha em `roster`. Isso significa que, para cada linha em `class`, assim que uma correspondência for encontrada em `roster`, a linha em `class` pode ser descartada.

As transformações antijoin não podem, na maioria dos casos, ser aplicadas se as expressões que estão sendo comparadas forem nulos. Uma exceção a essa regra é que `(... NOT IN (SELECT ...)) IS NOT FALSE` e seu equivalente `(... IN (SELECT ...)) IS NOT TRUE` podem ser transformados em antijoin.

A sintaxe de junção externa e junção interna é permitida na especificação da consulta externa, e as referências de tabela podem ser tabelas base, tabelas derivadas, referências de visão ou expressões de tabela comuns.

Em MySQL, uma subconsulta deve satisfazer esses critérios para ser tratada como um semijoin (ou, em MySQL 8.0.17 e versões posteriores, um antijoin se `NOT` modifica a subconsulta):

* Deve fazer parte de um predicado `IN`, `= ANY` ou `EXISTS` que apareça no nível superior da cláusula `WHERE` ou `ON`, possivelmente como um termo em uma expressão `AND`. Por exemplo:
...
Em MySQL 8.0.17 e versões posteriores, a subconsulta também pode ser o argumento de uma expressão modificada por `NOT`, `IS [NOT] TRUE` ou `IS [NOT] FALSE`.

* Deve ser um único `SELECT` sem construções de `UNION`.

* Não deve conter uma cláusula `HAVING`. * Não deve conter funções agregadas (se explicitamente ou implicitamente agrupadas).

* Não deve ter uma cláusula `LIMIT`. * A declaração não deve usar o tipo de junção `STRAIGHT_JOIN` na consulta externa.
...
* A palavra-chave `DISTINCT` é permitida, mas ignorada. As estratégias de semijoin tratam automaticamente a remoção de duplicatas.

* Uma cláusula `GROUP BY` é permitida, mas ignorada, a menos que a subconsulta também contenha uma ou mais funções agregadas.

* Uma cláusula `ORDER BY` é permitida, mas ignorada, uma vez que a ordem não é relevante para a avaliação de estratégias de semijoin.

Se uma subconsulta atender aos critérios anteriores, o MySQL a converte em uma semijoin (ou, no MySQL 8.0.17 ou posterior, em um antijoin, se aplicável) e faz uma escolha baseada no custo dessas estratégias:
...
* *Duplo Weedout*: Execute o semijoin como se fosse uma junção e remova os registros duplicados usando uma tabela temporária.

* *FirstMatch*: Ao digitalizar as tabelas internas em busca de combinações de linhas e houver múltiplas instâncias de um grupo de valores específico, escolha uma em vez de devolvê-las todas. Esse "atalho" de digitalização elimina a produção de linhas desnecessárias.
...
* A bandeira `semijoin` controla se junções semijoias são usadas. A partir do MySQL 8.0.17, isso também se aplica a antijunções.
...
Essas bandeiras são ativadas por padrão. Veja a Seção 10.9.2, “Otimizações Desconectables”.
...
* Para saída `EXPLAIN` estendida, o texto exibido por um `SHOW WARNINGS` subsequente mostra a consulta reescrita, que exibe a estrutura semijoin. (Veja a Seção 10.8.3, “Formato de saída EXPLAIN estendida”.) Com isso, você pode ter uma ideia de quais tabelas foram extraídas da semijoin. Se uma subconsulta foi convertida em uma semijoin, você deve ver que o predicado da subconsulta desapareceu e suas tabelas e a cláusula `WHERE` foram mescladas na lista de junção da consulta externa e a cláusula `WHERE`.

* O uso de tabela temporária para Weedout Duplo é indicado por `Start temporary` e `End temporary` na coluna `Extra`. As tabelas que não foram extraídas e estão na faixa das linhas de saída `EXPLAIN` cobertas por `Start temporary` e `End temporary` têm seu `rowid` na tabela temporária.
...
No MySQL 8.0.21 e versões posteriores, uma transformação semijoin também pode ser aplicada a uma declaração `UPDATE` ou `DELETE` de tabela única que utiliza um predicado de subconsulta `[NOT] IN` ou `[NOT] EXISTS`, desde que a declaração não utilize `ORDER BY` ou `LIMIT`, e que as transformações semijoin sejam permitidas por uma dica de otimizador ou pelo ajuste `optimizer_switch`.

#### 10.2.2.2 Otimizando subconsultas com materialização
...
A materialização de subconsultas usa uma tabela temporária de memória quando possível, revertendo para armazenamento em disco se a tabela se tornar muito grande. Veja a Seção 10.4.4, “Uso de tabela temporária interna no MySQL”.
...
A materialização de subconsultas usando uma tabela temporária evita tais reescritas e permite executar a subconsulta apenas uma vez, em vez de uma vez por linha da consulta externa.

Para que a materialização de subconsulta seja usada no MySQL, a variável de sistema `optimizer_switch` `materialization` deve ser habilitada. (Veja a Seção 10.9.2, “Otimizações Alternativas”). Com a bandeira `materialization` habilitada, a materialização se aplica a predicados de subconsulta que aparecem em qualquer lugar (na lista de seleção, `WHERE`, `ON`, `GROUP BY`, `HAVING` ou `ORDER BY`), para predicados que se enquadram em qualquer um desses casos de uso:
...
* Comparado à execução de consulta que não utiliza materialização, `select_type` pode mudar de `DEPENDENT SUBQUERY` para `SUBQUERY`. Isso indica que, para uma subconsulta que seria executada uma vez por linha externa, a materialização permite que a subconsulta seja executada apenas uma vez.
...
No MySQL 8.0.21 e versões posteriores, o MySQL também pode aplicar a materialização de subconsulta a uma declaração de tabela única `UPDATE` ou `DELETE` que utiliza um predicado de subconsulta `[NOT] IN` ou `[NOT] EXISTS`, desde que a declaração não utilize `ORDER BY` ou `LIMIT`, e que a materialização de subconsulta seja permitida por uma dica de otimizador ou pelo ajuste `optimizer_switch`.
...
O MySQL avalia as consultas “de fora para dentro”. Ou seja, ele obtém primeiro o valor da expressão externa *`outer_expr`*, e depois executa a subconsulta e captura as linhas que ela produz.

Uma otimização muito útil é "informar" a subconsulta que as únicas linhas de interesse são aquelas onde a expressão interna *`inner_expr`* é igual a *`outer_expr`*. Isso é feito empurrando uma igualdade apropriada na cláusula `WHERE` da subconsulta para torná-la mais restritiva. A comparação convertida parece assim:
...
Mais genericamente, uma comparação de valores de *`N`* com uma subconsulta que retorna linhas com valores de *`N`* está sujeita à mesma conversão. Se *`oe_i`* e *`ie_i`* representam valores correspondentes de expressão externa e interna, respectivamente, essa comparação da subconsulta:
...
A estratégia de "empurrar para baixo" funciona apenas se uma dessas condições for verdadeira:

* *`outer_expr`* e *`inner_expr`* não podem ser `NULL`.

* Você não precisa distinguir os resultados da subconsulta `NULL` de `FALSE`. Se a subconsulta for parte de uma expressão `OR` ou `AND` na cláusula `WHERE`, o MySQL assume que você não se importa. Outra instância em que o otimizador percebe que os resultados da subconsulta `NULL` e `FALSE` não precisam ser distinguidos é este construtor:

  ```
  ... WHERE outer_expr IN (subquery)
  ```

Neste caso, a cláusula `WHERE` rejeita a linha se `IN (subquery)` retorna `NULL` ou `FALSE`.

Suponha que *`outer_expr`* seja conhecido como um valor não `NULL`, mas a subconsulta não produza uma linha de tal forma que *`outer_expr`* = *`inner_expr`*. Então, `outer_expr IN (SELECT ...)` é avaliado da seguinte forma:
...
Nessa situação, a abordagem de procurar linhas com `outer_expr = inner_expr` não é mais válida. É necessário procurar tais linhas, mas se nenhuma for encontrada, também procure linhas onde *`inner_expr`* é `NULL`. Grosso modo, a subconsulta pode ser convertida em algo como este:
...
A necessidade de avaliar a condição extra `IS NULL` é a razão pela qual o MySQL tem o método de acesso (comparison-operators.html#operator_is-null):
...
Para uma avaliação adequada, é necessário verificar se o `SELECT` produziu alguma linha, de modo que o `outer_expr = inner_expr` não possa ser empurrado para baixo na subconsulta. Esse é um problema, pois muitas subconsultas do mundo real tornam-se muito lentas, a menos que a igualdade possa ser empurrada para baixo.
...
* Se *`outer_expr`* for `NULL`, para avaliar a expressão a seguir, é necessário executar o `SELECT` para determinar se ela produz quaisquer linhas:

  ```
  NULL IN (SELECT inner_expr FROM ... WHERE subquery_where)
  ```

É necessário executar o original `SELECT` aqui, sem quaisquer igualdades empurradas do tipo mencionado anteriormente.
...
As funções de gatilho não são gatilhos do tipo que você cria com `CREATE TRIGGER` (create-trigger.html "15.1.22 CREATE TRIGGER Statement").
...
* Motores de execução de subconsultas com busca por índice: `trigcond(X=Y)` pode ser usado para construir acessos `unique_subquery` ou `index_subquery`.
...
A bandeira `subquery_materialization_cost_based` da variável de sistema `optimizer_switch` permite o controle sobre a escolha entre materialização de subconsulta e transformação de subconsulta de `IN` para `EXISTS`. Veja a Seção 10.9.2, “Otimizações Desconectables”.

#### 10.2.2.4 Otimizando tabelas derivadas, referências de visualização e expressões de tabela comuns com junção ou materialização

O otimizador pode lidar com referências a tabelas derivadas usando duas estratégias (que também se aplicam a referências de visualização e expressões comuns de tabela):
...
O otimizador trata tabelas derivadas, referências de visualização e expressões de tabela comuns da mesma maneira: ele evita a materialização desnecessária sempre que possível, o que permite a redução de condições da consulta externa para as tabelas derivadas e produz planos de execução mais eficientes. (Para um exemplo, consulte a Seção 10.2.2.2, “Otimizando subconsultas com materialização”.)
...
O otimizador propaga uma cláusula `ORDER BY` em uma referência de tabela ou visão derivada para o bloco de consulta externa se todas essas condições forem verdadeiras:
...
Os seguintes meios estão disponíveis para influenciar se o otimizador tenta combinar tabelas derivadas, referências de visão e expressões de tabela comuns no bloco de consulta externa:
...
* Da mesma forma, você pode usar a bandeira `derived_merge` da variável de sistema `optimizer_switch`. Veja a Seção 10.9.2, “Otimizações Desconectables”. Por padrão, a bandeira é habilitada para permitir a fusão. Desabilitar a bandeira impede a fusão e evita os erros de `ER_UPDATE_TABLE_USED`.
...
* É possível desabilitar a junção usando, na subconsulta, quaisquer construções que impeçam a junção, embora essas não sejam tão explícitas em seu efeito na materialização. As construções que impedem a junção são as mesmas para tabelas derivadas, expressões comuns de tabela e referências de visão:
...
+ `DISTINCT`
  ...
  + `HAVING`
  + `LIMIT`
  + `UNION` ou `UNION ALL`(union.html "15.2.18 UNION Clause")
...
* O otimizador adiou a materialização de tabelas derivadas até que seu conteúdo seja necessário durante a execução da consulta. Isso melhora o desempenho, pois o adiamento da materialização pode resultar em não precisar realizá-la. Considere uma consulta que une o resultado de uma tabela derivada a outra tabela: Se o otimizador processar essa outra tabela primeiro e descobrir que ela não retorna nenhuma linha, a união não precisa ser realizada e o otimizador pode pular completamente a materialização da tabela derivada.
...
O otimizador evita a materialização da tabela derivada, atrasando-a até que o resultado seja necessário durante a execução do `SELECT`. Neste caso, a consulta não é executada (porque ocorre em uma declaração `EXPLAIN`, portanto, o resultado nunca é necessário.
...
A cláusula `ALGORITHM` para `CREATE VIEW` não afeta a materialização para qualquer cláusula anterior à declaração `WITH` na definição da vista. Considere esta declaração:

```
CREATE ALGORITHM={TEMPTABLE|MERGE} VIEW v1 AS WITH ... SELECT ...
```

O valor `ALGORITHM` afeta apenas a materialização do `SELECT`, não do `WITH`")

Antes do MySQL 8.0.16, se `internal_tmp_disk_storage_engine=MYISAM`, ocorria um erro para qualquer tentativa de materializar um CTE usando uma tabela temporária em disco, pois, para CTEs, o mecanismo de armazenamento usado para tabelas temporárias internas em disco não podia ser `MyISAM`. A partir do MySQL 8.0.16, isso não é mais um problema, pois `TempTable` agora sempre usa `InnoDB` para tabelas temporárias internas em disco.

Como mencionado anteriormente, um CTE, se materializado, é materializado uma vez, mesmo se referenciado várias vezes. Para indicar uma materialização única, a saída do rastreamento do otimizador contém uma ocorrência de `creating_tmp_table` mais uma ou mais ocorrências de `reusing_tmp_table`.

As CTEs são semelhantes às tabelas derivadas, para as quais o nó `materialized_from_subquery` segue a referência. Isso é verdade para uma CTE que é referenciada várias vezes, portanto, não há duplicação dos nós `materialized_from_subquery` (o que daria a impressão de que a subconsulta é executada várias vezes e produziria uma saída desnecessariamente verbose). Apenas uma referência à CTE tem um nó `materialized_from_subquery` completo com a descrição do seu plano de subconsulta. Outras referências têm um nó `materialized_from_subquery` reduzido. A mesma ideia se aplica ao `EXPLAIN` de saída no formato `TRADITIONAL`: Subconsultas para outras referências não são mostradas.

#### 10.2.2.5 Otimização de empurrão de condição derivada

O MySQL 8.0.22 e versões posteriores suportam o empurrar de condições derivadas para subconsultas elegíveis. Para uma consulta como `SELECT * FROM (SELECT i, j FROM t1) AS dt WHERE i > constant`, em muitos casos é possível empurrar a condição externa `WHERE` para a tabela derivada, resultando, neste caso, em `SELECT * FROM (SELECT i, j FROM t1 WHERE i > constant) AS dt`. Quando uma tabela derivada não pode ser agregada à consulta externa (por exemplo, se a tabela derivada usa agregação), empurrar a condição externa `WHERE` para a tabela derivada deve diminuir o número de linhas que precisam ser processadas e, assim, acelerar a execução da consulta.

Nota

Antes do MySQL 8.0.22, se uma tabela derivada foi materializada, mas não foi mesclada, o MySQL materializou toda a tabela, e depois qualificou todas as linhas resultantes com a condição `WHERE`. Isso ainda é o caso se a empurrada de condição derivada não estiver habilitada, ou não puder ser empregada por algum outro motivo.

As condições externas `WHERE` podem ser reduzidas para tabelas materializadas derivadas nas seguintes circunstâncias:

* Quando a tabela derivada não utiliza funções agregadas ou funções de janela, a condição externa `WHERE` pode ser aplicada diretamente nela. Isso inclui condições `WHERE` que têm múltiplos predicados unidos com `AND`, `OR`, ou ambos.
...
* Quando a tabela derivada possui uma condição `GROUP BY` e não utiliza funções de janela, uma condição externa `WHERE` que faça referência a uma ou mais colunas que não fazem parte da `GROUP BY` pode ser empurrada para a tabela derivada como uma condição `HAVING`.

Por exemplo, `SELECT * FROM (SELECT i, j, SUM(k) AS sum FROM t1 GROUP BY i, j) AS dt WHERE sum > 100` é reescrito seguindo a empilhamento de condições derivadas como `SELECT * FROM (SELECT i, j, SUM(k) AS sum FROM t1 GROUP BY i, j HAVING sum > 100) AS dt`.

* Quando a tabela derivada utiliza um `GROUP BY` e as colunas na condição externa do `WHERE` são colunas `GROUP BY`, as condições `WHERE` que fazem referência a essas colunas podem ser empurradas diretamente para a tabela derivada.
...
Caso a condição externa `WHERE` tenha predicados que fazem referência a colunas que fazem parte do `GROUP BY`, bem como predicados que não fazem referência a colunas, os predicados do primeiro tipo são empurrados para baixo como condições de `WHERE`, enquanto os do segundo tipo são empurrados para baixo como condições de `HAVING`. Por exemplo, na consulta `SELECT * FROM (SELECT i, j, SUM(k) AS sum FROM t1 GROUP BY i,j) AS dt WHERE i > 10 AND sum > 100`, o predicado `i > 10` na cláusula externa `WHERE` faz referência a uma coluna de `GROUP BY`, enquanto o predicado `sum > 100` não faz referência a nenhuma coluna de `GROUP BY`. Assim, a otimização de empurrão de tabela derivada faz com que a consulta seja reescrita de uma maneira semelhante àquela mostrada aqui:
...
Para habilitar a otimização de projeção de condição derivada, a bandeira `derived_condition_pushdown` da variável de sistema `optimizer_switch` (adicionada nesta versão) deve ser definida como `on`, que é o ajuste padrão. Se essa otimização for desativada por `optimizer_switch`, você pode habilitá-la para uma consulta específica usando a dica de otimizador [[`DERIVED_CONDITION_PUSHDOWN`]. Para desativar a otimização para uma consulta específica, use a dica de otimizador [[`NO_DERIVED_CONDITION_PUSHDOWN`].

As seguintes restrições e limitações se aplicam à otimização de empurrar as condições de tabela derivada:

* A otimização não pode ser usada se a tabela derivada contiver `UNION`. Essa restrição é levantada no MySQL 8.0.29. Considere duas tabelas `t1` e `t2`, e uma visão `v` contendo sua união, criada conforme mostrado aqui:
...
Em MySQL 8.0.29 e versões posteriores, a otimização de empurrar a condição da tabela derivada pode ser empregada com consultas `UNION`, com as seguintes exceções:

+ O pushdown de condição não pode ser usado com uma consulta `UNION` se qualquer tabela derivada materializada que faz parte do `UNION` for uma expressão comum de tabela recursiva (consulte Expressões Comuns de Tabela Recursivas).

+ Condições que contêm expressões não determinísticas não podem ser reduzidas a uma tabela derivada.

* A tabela derivada não pode usar uma cláusula `LIMIT`.

* As condições que contêm subconsultas não podem ser empurradas para baixo. * A otimização não pode ser usada se a tabela derivada for uma tabela interna de uma junção externa.

* Se uma tabela derivada materializada for uma expressão de tabela comum, as condições não são aplicadas nela se ela for referenciada várias vezes.

* As condições que utilizam parâmetros podem ser empurradas para baixo se a condição tiver a forma `derived_column > ?`. Se uma coluna derivada em uma condição externa `WHERE` for uma expressão com um `?` na tabela derivada subjacente, essa condição não pode ser empurrada para baixo.

* Para uma consulta na qual a condição está nas tabelas de uma visão criada usando `ALGORITHM=TEMPTABLE` em vez da própria visão, a igualdade múltipla não é reconhecida na resolução, e, portanto, a condição não pode ser empurrada para baixo. Isso ocorre porque, ao otimizar uma consulta, a empurrada de condição ocorre durante a fase de resolução, enquanto a propagação da igualdade múltipla ocorre durante a otimização.

Isso não é um problema em tais casos para uma visão que usa `ALGORITHM=MERGE`, onde a igualdade pode ser propagada e a condição empurrada para baixo.
...
* Tente consultar apenas as tabelas `INFORMATION_SCHEMA` que são visualizações em tabelas do dicionário de dados.

* Tente consultar apenas metadados estáticos. Selecionar colunas ou usar condições de recuperação para metadados dinâmicos juntamente com metadados estáticos adiciona sobrecarga ao processar os metadados dinâmicos.

Nota

O comportamento de comparação para nomes de banco de dados e tabelas em consultas de `INFORMATION_SCHEMA` pode diferir do que você espera. Para obter detalhes, consulte a Seção 12.8.7, “Usando a Colaboração em Pesquisas do SCHEMA_INFORMACAO”.
...
Alguns tipos de valores, mesmo para uma tabela não visual `INFORMATION_SCHEMA`, são recuperados por consultas no dicionário de dados. Isso inclui valores como nomes de banco de dados e tabela, tipos de tabela e motores de armazenamento.

Algumas tabelas `INFORMATION_SCHEMA` contêm colunas que fornecem estatísticas de tabela:
...
Por padrão, o MySQL recupera valores armazenados em cache para essas colunas das tabelas de dicionário `mysql.index_stats` e `mysql.innodb_table_stats` quando as colunas são consultadas, o que é mais eficiente do que recuperar estatísticas diretamente do mecanismo de armazenamento. Se as estatísticas armazenadas em cache não estiverem disponíveis ou tiverem expirado, o MySQL recupera as estatísticas mais recentes do mecanismo de armazenamento e as armazena nas tabelas de dicionário `mysql.index_stats` e `mysql.innodb_table_stats`. Consultas subsequentes recuperam as estatísticas armazenadas em cache até que as estatísticas armazenadas em cache expirem. Um reinício do servidor ou a primeira abertura das tabelas `mysql.index_stats` e `mysql.innodb_table_stats` não atualizam automaticamente as estatísticas armazenadas em cache.

A variável de sessão `information_schema_stats_expiry` define o período de tempo antes que as estatísticas armazenadas expirem. O padrão é de 86400 segundos (24 horas), mas o período de tempo pode ser estendido por até um ano.

Para atualizar os valores armazenados em cache a qualquer momento para uma tabela específica, use `ANALYZE TABLE`.

A consulta de colunas de estatísticas não armazena ou atualiza estatísticas nas tabelas de dicionário `mysql.index_stats` e `mysql.innodb_table_stats` nessas circunstâncias:

* Quando as estatísticas armazenadas não expiraram. * Quando `information_schema_stats_expiry` está definido como 0.

* Quando o servidor estiver no modo `read_only`, `super_read_only`, `transaction_read_only` ou `innodb_read_only`.

* Quando a consulta também recupera dados do Gerador de Desempenho.

`information_schema_stats_expiry` é uma variável de sessão, e cada sessão do cliente pode definir seu próprio valor de expiração. Estatísticas que são recuperadas do motor de armazenamento e armazenadas em cache por uma sessão estão disponíveis para outras sessões.

Nota

Se a variável de sistema `innodb_read_only` estiver habilitada, (analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") pode falhar porque não consegue atualizar as tabelas de estatísticas no dicionário de dados, que utilizam `InnoDB`. Para operações de `ANALYZE TABLE` que atualizam a distribuição de chaves, a falha pode ocorrer mesmo que a operação atualize a própria tabela (por exemplo, se for uma tabela de `MyISAM`). Para obter as estatísticas de distribuição atualizadas, defina `information_schema_stats_expiry=0`.

Para as tabelas `INFORMATION_SCHEMA` implementadas como visualizações em tabelas do dicionário de dados, os índices nas tabelas subjacentes do dicionário de dados permitem que o otimizador construa planos de execução de consultas eficientes. Para ver as escolhas feitas pelo otimizador, use `EXPLAIN`. Para ver também a consulta usada pelo servidor para executar uma consulta `INFORMATION_SCHEMA`, use `SHOW WARNINGS` imediatamente após `EXPLAIN`.

Considere esta declaração, que identifica as codificações para o conjunto de caracteres `utf8mb4`:
...
Como indicado por `SHOW WARNINGS`, o servidor processa a consulta em `COLLATION_CHARACTER_SET_APPLICABILITY` como uma consulta nas tabelas do dicionário de dados `character_sets` e `collations` no banco de dados do sistema `mysql`.

### 10.2.4 Otimização de consultas do esquema de desempenho
### 10.2.5 Otimização de consultas de esquema de desempenho para consultas de alta performance

Aplicações que monitoram bancos de dados podem fazer uso frequente das tabelas do Gerador de Desempenho. Para escrever consultas para essas tabelas de forma mais eficiente, aproveite seus índices. Por exemplo, inclua uma cláusula `WHERE` que restringe as linhas recuperadas com base na comparação com valores específicos em uma coluna indexada.

A maioria das tabelas do Schema de Desempenho tem índices. As tabelas que não os possuem são aquelas que normalmente contêm poucas linhas ou são improváveis de serem consultadas frequentemente. Os índices do Schema de Desempenho dão ao otimizador acesso a planos de execução diferentes de varreduras completas de tabela. Esses índices também melhoram o desempenho para objetos relacionados, como as vistas do esquema `sys` que utilizam essas tabelas.

Para verificar se uma tabela de um esquema de desempenho específico tem índices e quais são eles, use `SHOW INDEX` ou `SHOW CREATE TABLE`:
...
Para ver o plano de execução de uma consulta do Performance Schema e se ela utiliza algum índice, use `EXPLAIN`:
...
A saída `EXPLAIN` indica que o otimizador utiliza a tabela `accounts` com índice `ACCOUNT` que compreende as colunas `USER` e `HOST`.

Os índices do Schema de desempenho são virtuais: são uma construção do motor de armazenamento do Schema de desempenho e não utilizam memória ou armazenamento em disco. O Schema de desempenho relata informações de índice ao otimizador para que ele possa construir planos de execução eficientes. O Schema de desempenho, por sua vez, utiliza informações do otimizador sobre o que procurar (por exemplo, um valor de chave particular), para que possa realizar pesquisas eficientes sem construir estruturas de índice reais. Essa implementação oferece dois benefícios importantes:

* Evita totalmente o custo de manutenção normalmente incorrido em mesas que passam por atualizações frequentes.

* Reduz, em uma fase inicial da execução da consulta, a quantidade de dados recuperados. Para condições em colunas indexadas, o Gerador de Desempenho de forma eficiente retorna apenas as linhas da tabela que satisfazem as condições da consulta. Sem um índice, o Gerador de Desempenho retornaria todas as linhas da tabela, exigindo que o otimizador avalie as condições posteriormente contra cada linha para produzir o resultado final.

Os índices do Schema de desempenho são pré-definidos e não podem ser excluídos, adicionados ou alterados.

Os índices de esquema de desempenho são semelhantes aos índices de hash. Por exemplo:

* Eles são usados apenas para comparações de igualdade que utilizam os operadores `=` ou `<=>`.

* Eles não estão ordenados. Se o resultado de uma consulta deve ter características específicas de ordem de linha, inclua uma cláusula `ORDER BY`.

Para informações adicionais sobre índices de hash, consulte a Seção 10.3.9, “Comparação entre B-Tree e índices de hash”.

### 10.2.5 Otimizando declarações de alteração de dados

Esta seção explica como acelerar as declarações de mudança de dados: `INSERT`, `UPDATE` e `DELETE`. Aplicações tradicionais OLTP e aplicações modernas na web geralmente realizam muitas operações pequenas de mudança de dados, onde a concorrência é vital. Aplicações de análise e relatórios de dados geralmente executam operações de mudança de dados que afetam muitas linhas de uma vez, onde as principais considerações são o I/O para escrever grandes quantidades de dados e manter os índices atualizados. Para inserir e atualizar grandes volumes de dados (conhecidos na indústria como ETL, para “extrair-transformar-carregar”), às vezes você usa outras declarações SQL ou comandos externos, que imitam os efeitos das declarações `INSERT`, `UPDATE` e `DELETE`.

#### 10.2.5.1 Otimizando instruções INSERT

Para otimizar a velocidade de inserção, combine muitas operações pequenas em uma única operação grande. Idealmente, faça uma única conexão, envie os dados para muitas novas linhas de uma só vez e adiar todas as atualizações de índice e verificação de consistência até o final.
...
* Se você está inserindo muitas linhas do mesmo cliente ao mesmo tempo, use as declarações `INSERT` com várias listas `VALUES` para inserir várias linhas de cada vez. Isso é consideravelmente mais rápido (muitas vezes mais rápido em alguns casos) do que usar declarações separadas de uma única linha `INSERT`. Se você está adicionando dados a uma tabela não vazia, pode ajustar a variável `bulk_insert_buffer_size` para tornar a inserção de dados ainda mais rápida. Veja a Seção 7.1.8, “Variáveis do Sistema do Servidor”.

* Ao carregar uma tabela a partir de um arquivo de texto, use `LOAD DATA`. Isso geralmente é 20 vezes mais rápido do que usar as instruções `INSERT`. Veja a Seção 15.2.9, “Instrução LOAD DATA”.
...
* Veja a Seção 10.5.5, “Carregamento de Dados em Massa para Tabelas InnoDB”, para dicas específicas para as tabelas `InnoDB`.

* Veja a Seção 10.6.2, “Carregamento de Dados em Massa para Tabelas MyISAM”, para dicas específicas para as tabelas `MyISAM`.

#### 10.2.5.2 Otimizando declarações UPDATE
...
Para uma tabela `MyISAM` que utiliza formato de linha dinâmico, atualizar uma linha para um comprimento total mais longo pode dividir a linha. Se você fizer isso com frequência, é muito importante usar ocasionalmente `OPTIMIZE TABLE`. Veja a Seção 15.7.3.4, “Instrução OPTIMIZE TABLE”.

#### 10.2.5.3 Otimizando declarações DELETE

O tempo necessário para excluir linhas individuais em uma tabela `MyISAM` é exatamente proporcional ao número de índices. Para excluir as linhas mais rapidamente, você pode aumentar o tamanho do cache de chave aumentando a variável de sistema `key_buffer_size`. Veja a Seção 7.1.1, “Configurando o servidor”.

Para excluir todas as linhas de uma tabela `MyISAM`, `TRUNCATE TABLE tbl_name` é mais rápido do que `DELETE FROM tbl_name`. As operações de truncar não são seguras em transação; ocorre um erro ao tentar uma delas durante uma transação ativa ou bloqueio de tabela ativo. Veja a Seção 15.1.37, “Instrução TRUNCATE TABLE”.
...
* Para corrigir quaisquer problemas de compressão que ocorram com as tabelas `ARCHIVE`, use `OPTIMIZE TABLE`. Veja a Seção 18.5, “O Motor de Armazenamento ARCHIVE”.
...
* A replicação pode proporcionar um benefício de desempenho para algumas operações. Você pode distribuir as recuperações dos clientes entre as réplicas para dividir a carga. Para evitar o atraso da fonte ao fazer backups, você pode fazer backups usando uma réplica. Veja o Capítulo 19, *Replicação*.