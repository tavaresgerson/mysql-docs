...
- Se você precisa de uma velocidade realmente alta, veja as interfaces de baixo nível do MySQL. Por exemplo, ao acessar diretamente os mecanismos de armazenamento `InnoDB` ou `MyISAM` do MySQL, você pode obter um aumento substancial na velocidade em comparação com o uso da interface SQL.

  Da mesma forma, para bancos de dados que utilizam o mecanismo de armazenamento `NDBCLUSTER`, você pode querer investigar o uso possível da API NDB (veja o Guia do Desenvolvedor da API MySQL NDB Cluster).

- A replicação pode proporcionar um benefício de desempenho para algumas operações. Você pode distribuir as recuperações dos clientes entre as réplicas para dividir a carga. Para evitar atrasar a fonte ao fazer backups, você pode fazer backups usando uma réplica. Veja o Capítulo 19, *Replicação*.
