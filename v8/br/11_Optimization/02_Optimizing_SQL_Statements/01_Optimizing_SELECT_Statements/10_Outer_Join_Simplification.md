...
Todas as expressões de junção interna do tipo `T1 INNER JOIN T2 ON P(T1,T2)` são substituídas pela lista `T1,T2`, sendo `P(T1,T2)` unida como conjunção à condição `WHERE` (ou à condição de junção da junção embutida, se houver).
...
Condições como essas são rejeitadas porque não podem ser verdadeiras para qualquer linha `NULL`-completada (com as colunas `T2` definidas como `NULL`):
...
Condições como essas não são rejeitadas como nulas porque podem ser verdadeiras para uma linha `NULL`-completada:
...
- É um predicado que contém uma referência a uma tabela interna que avalia para `UNKNOWN` quando um de seus argumentos é `NULL`
...
A operação de junção externa restante também pode ser substituída por uma junção interna porque a condição `T3.B=T2.B` é rejeitada como nula. Isso resulta em uma consulta sem junções externas:
