...
Antes do MySQL 8.0.18, esse algoritmo era aplicado para junções equi quando não podiam ser usados índices; no MySQL 8.0.18 e versões posteriores, a otimização da junção hash é empregada nesses casos. A partir do MySQL 8.0.20, o loop aninhado em blocos não é mais usado pelo MySQL, e uma junção hash é empregada em todos os casos em que o loop aninhado em blocos era usado anteriormente. Veja a Seção 10.2.1.4, “Otimização da Junção Hash”.

O bufferamento de junção no MySQL tem essas características:

- O uso do buffer pode ser feito quando a junção é do tipo `ALL` ou `index` (ou seja, quando não é possível usar nenhuma chave possível e uma varredura completa é realizada, seja nas linhas de dados ou nas linhas do índice, respectivamente), ou `range`. O uso do buffer também é aplicável a junções externas, conforme descrito na Seção 10.2.1.12, “Junções de Bloco de Loop Aninhado e Acesso a Chave em Batel”.
...
Se `S` é o tamanho de cada combinação armazenada de `t1` e `t2` no buffer de junção e `C` é o número de combinações no buffer, o número de vezes que a tabela `t3` é percorrida é:

```
(S * C)/join_buffer_size + 1
```

O número de varreduras `t3` diminui à medida que o valor de `join_buffer_size` aumenta, até o ponto em que `join_buffer_size` é grande o suficiente para armazenar todas as combinações de linhas anteriores. Nesse ponto, não há ganho de velocidade ao torná-lo maior.
