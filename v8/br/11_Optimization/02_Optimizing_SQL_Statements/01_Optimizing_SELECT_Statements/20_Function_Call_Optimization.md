...
- A primeira consulta sempre produz um máximo de uma linha porque `POW()` com argumentos constantes é um valor constante e é usado para pesquisa de índice.
...
Os efeitos do não determinismo não se limitam às declarações `SELECT`. Esta declaração `UPDATE` utiliza uma função não determinística para selecionar as linhas a serem modificadas:

```
UPDATE t SET col_a = some_expr WHERE id = FLOOR(1 + RAND() * 49);
```

Presumivelmente, a intenção é atualizar no máximo uma única linha cuja chave primária corresponda à expressão. No entanto, pode-se atualizar zero, uma ou várias linhas, dependendo dos valores da coluna `id` e dos valores na sequência `RAND()`.
...
- `InnoDB` pode ser escalonado para um bloqueio de chave de intervalo em vez de tomar um bloqueio de linha única para uma linha correspondente.
...
Se o otimizador puder usar `partial_key` para reduzir o conjunto de linhas selecionadas, `RAND()` será executado menos vezes, o que diminui o efeito do não determinismo na otimização.
