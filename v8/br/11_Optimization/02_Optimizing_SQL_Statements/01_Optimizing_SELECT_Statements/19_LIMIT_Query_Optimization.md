...
- Em alguns casos, um `GROUP BY` pode ser resolvido lendo o índice em ordem (ou fazendo uma ordenação no índice), depois calculando resumos até que o valor do índice mude. Nesse caso, o `LIMIT row_count` não calcula quaisquer valores desnecessários do `GROUP BY`.
...
- `LIMIT 0` retorna rapidamente um conjunto vazio. Isso pode ser útil para verificar a validade de uma consulta. Também pode ser empregado para obter os tipos das colunas do resultado em aplicativos que utilizam uma API MySQL que disponibiliza metadados do conjunto de resultados. Com o programa cliente **mysql**, você pode usar a opção `--column-type-info` para exibir os tipos das colunas do resultado.
...
- Se um índice não for usado para `ORDER BY`, mas uma cláusula `LIMIT` também estiver presente, o otimizador pode evitar o uso de um arquivo de junção e ordenar as linhas na memória usando uma operação `filesort` em memória.
...
Um fator que afeta o plano de execução é `LIMIT`, portanto, uma consulta `ORDER BY` com e sem `LIMIT` pode retornar linhas em ordens diferentes. Considere esta consulta, que é ordenada pela coluna `category`, mas não determinística em relação às colunas `id` e `rating`:
...
Para uma consulta com uma cláusula `ORDER BY` ou `GROUP BY` e uma cláusula `LIMIT`, o otimizador tenta escolher um índice ordenado por padrão, quando isso parecer acelerar a execução da consulta. Antes do MySQL 8.0.21, não havia como desativar esse comportamento, mesmo em casos em que a utilização de outra otimização poderia ser mais rápida. A partir do MySQL 8.0.21, é possível desativar essa otimização definindo o `prefer_ordering_index` da variável de sistema `optimizer_switch` para `off`.
...
Como a consulta a seguir contém uma cláusula `LIMIT`, esperamos que ela utilize um índice ordenado, se possível. Neste caso, como podemos ver no resultado `EXPLAIN`, ela utiliza a chave primária da tabela.
