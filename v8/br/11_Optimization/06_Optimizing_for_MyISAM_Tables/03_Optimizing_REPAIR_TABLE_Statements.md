### 10.6.3 Otimizando as declarações da Tabela de Reparo
...
- O **myisamchk** possui variáveis que controlam a alocação de memória. Você pode melhorar o desempenho configurando essas variáveis, conforme descrito na Seção 6.6.4.6, “Uso de Memória do myisamchk”.

- Para `REPAIR TABLE`, o mesmo princípio se aplica, mas, como a reparação é feita pelo servidor, você define variáveis do sistema do servidor em vez das variáveis **myisamchk**. Além disso, além de definir variáveis de alocação de memória, aumentar a variável do sistema `myisam_max_sort_file_size` aumenta a probabilidade de que a reparação use o método de filesort mais rápido e evite a reparação mais lenta pelo método de cache de chave. Defina a variável para o tamanho máximo do arquivo do seu sistema, após verificar se há espaço livre suficiente para conter uma cópia dos arquivos da tabela. O espaço livre deve estar disponível no sistema de arquivos que contém os arquivos originais da tabela.
...
Se você pretende alterar uma variável global, mas deseja fazer isso apenas durante a duração de uma operação `REPAIR TABLE` para afetar o mínimo possível outros usuários, salve seu valor em uma variável de usuário e restaure-o depois. Por exemplo:
...
As variáveis de sistema que afetam `REPAIR TABLE` podem ser definidas globalmente no início do servidor se você quiser que os valores sejam aplicados por padrão. Por exemplo, adicione essas linhas ao arquivo do servidor `my.cnf`:
