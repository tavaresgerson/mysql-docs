...
- Com um pouco mais de trabalho, é possível fazer o `LOAD DATA` rodar ainda mais rápido para uma tabela `MyISAM` quando a tabela tiver muitos índices. Use o procedimento a seguir:
  ...
  5. Recrie os índices com **myisamchk -rq `/path/to/db/tbl_name`**. Isso cria a árvore de índices na memória antes de gravá-la no disco, o que é muito mais rápido do que atualizar o índice durante `LOAD DATA` porque evita muitos acessos ao disco. A árvore de índices resultante também é perfeitamente equilibrada.

  6. Execute uma instrução `FLUSH TABLES` ou o comando **mysqladmin flush-tables**.

  `LOAD DATA` realiza a otimização anterior automaticamente se a tabela `MyISAM` na qual você insere dados estiver vazia. A principal diferença entre a otimização automática e o uso do procedimento explicitamente é que você pode permitir que o **myisamchk** aloque muito mais memória temporária para a criação do índice do que o servidor pode querer alocar para a recriação do índice quando ele executa a instrução `LOAD DATA`.
...
- Para acelerar as operações `INSERT` que são realizadas com múltiplas instruções para tabelas não transacionais, bloqueie suas tabelas:
  ...
  Isso beneficia o desempenho, pois o cache de índice é descarregado no disco apenas uma vez, após todas as instruções `INSERT` terem sido concluídas. Normalmente, haveria tantos descarregamentos do cache de índice quanto houver instruções `INSERT`. As instruções de bloqueio explícito não são necessárias se você puder inserir todas as linhas com uma única instrução `INSERT`.
