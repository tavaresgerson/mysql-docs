#### 10.12.3.3 Habilitar o suporte para páginas grandes
...
Antes que páginas grandes possam ser usadas no Linux, o kernel deve ser habilitado para suportá-las e é necessário configurar o pool de memória HugeTLB. Para referência, a API HugeTBL está documentada no arquivo `Documentation/vm/hugetlbpage.txt` dos seus arquivos de código-fonte do Linux.
...
1. Determine o número de páginas grandes necessárias. Esse é o tamanho do pool de buffers do InnoDB dividido pelo tamanho da página grande, que podemos calcular como `innodb_buffer_pool_size` / `Hugepagesize`. Considerando o valor padrão para o `innodb_buffer_pool_size` (128 MB) e usando o valor `Hugepagesize` obtido a partir de `/proc/meminfo` (2 MB), isso é 128 MB / 2 MB, ou 64 Páginas Gigantes. Chamamos esse valor de `P`.
...
3. Como raiz do sistema, execute o seguinte comando:
   ...
   Para verificar a configuração da página grande, verifique novamente `/proc/meminfo` conforme descrito anteriormente. Agora você deve ver alguns valores não nulos adicionais na saída, semelhantes a este:
...
5. Verifique quaisquer arquivos de configuração, como `my.cnf`, usados pelo servidor, e certifique-se de que `innodb_buffer_pool_chunk_size` esteja definido maior que o tamanho enorme da página. O valor padrão para essa variável é 128M.

6. O suporte a páginas grandes no servidor MySQL é desativado por padrão. Para ativá-lo, inicie o servidor com `--large-pages`. Você também pode fazer isso adicionando a seguinte linha à seção `[mysqld]` do arquivo do servidor `my.cnf`:

   ```
   large-pages=ON
   ```

   Com essa opção habilitada, o `InnoDB` usa páginas grandes automaticamente para seu pool de buffer e pool de memória adicional. Se o `InnoDB` não conseguir fazer isso, ele volta a usar a memória tradicional e escreve uma mensagem de aviso no log de erros: Aviso: Usando o pool de memória convencional.

Você pode verificar se o MySQL está usando páginas grandes verificando `/proc/meminfo` novamente após reiniciar o **mysqld**, da seguinte forma:
