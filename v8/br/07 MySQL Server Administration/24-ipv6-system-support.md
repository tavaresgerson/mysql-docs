...
Para produzir uma descrição das interfaces de rede do seu sistema, inicie `ifconfig -a` e procure por endereços IPv6 na saída.

Se o seu host não suportar IPv6, consulte a documentação do seu sistema para obter instruções sobre como habilitá-lo. Pode ser que você precise apenas reconfigurar uma interface de rede existente para adicionar um endereço IPv6. Ou pode ser necessário uma mudança mais extensa, como a reconstrução do kernel com opções IPv6 habilitadas.
