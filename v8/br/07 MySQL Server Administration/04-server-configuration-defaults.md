...
Para plataformas que não são o Windows, nenhum arquivo de opção padrão é criado durante a instalação do servidor ou o processo de inicialização do diretório de dados. Crie seu arquivo de opção seguindo as instruções da Seção 6.2.2.2, “Usando Arquivos de Opção”. Sem um arquivo de opção, o servidor apenas começa com suas configurações padrão—consulte a Seção 7.1.2, “Configurações Padrão do Servidor” para saber como verificar essas configurações.
