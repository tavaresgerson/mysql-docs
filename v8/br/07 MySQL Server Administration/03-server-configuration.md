...
O comando produz uma lista de todas as opções do `mysqld` e variáveis de sistema configuráveis. Sua saída inclui os valores padrão da opção e variável e parece algo assim:
...
Informações de monitoramento mais detalhadas estão disponíveis no Schema de Desempenho; consulte  Capítulo 29, *Schema de Desempenho MySQL*. Além disso, o esquema `sys` do MySQL é um conjunto de objetos que fornece acesso conveniente aos dados coletados pelo Schema de Desempenho; consulte  Capítulo 30, *Esquema sys MySQL*.

Se você especificar uma opção na linha de comando para `mysqld` ou `mysqld_safe`, ela permanecerá em vigor apenas para essa invocação do servidor. Para usar a opção toda vez que o servidor for executado, coloque-a em um arquivo de opção. Consulte  Seção 6.2.2.2, “Usando Arquivos de Opção”.

Usuários do Windows podem executar  Seção 2.3.2, “Configuração: Usando o Configurador MySQL” para ajudar a configurar uma instalação do servidor MySQL. Isso inclui tarefas como configurar usuários MySQL, arquivos de log, o nome do serviço do Windows e bancos de dados de exemplo.