...
* As instruções que alteram o `mysql` banco de dados indiretamente são registradas como instruções, independentemente do valor de `binlog_format`. Isso se aplica a instruções como `GRANT`, `REVOKE`, `SET PASSWORD`, `RENAME USER`, `CREATE` (todas as formas exceto `CREATE TABLE ... SELECT`), `ALTER` (todas as formas) e `DROP` (todas as formas).
