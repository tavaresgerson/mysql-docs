### 7.8.4 Usando programas de cliente em um ambiente de múltiplos servidores
...
* Inicie o cliente com `--protocol=TCP` para se conectar usando TCP/IP, `--protocol=SOCKET` para se conectar usando um arquivo de socket Unix, `--protocol=PIPE` para se conectar usando um tubo nomeado, ou `--protocol=MEMORY` para se conectar usando memória compartilhada. Para conexões TCP/IP, você também pode precisar especificar as opções `--host` e `--port`. Para os outros tipos de conexões, você pode precisar especificar uma opção `--socket` para especificar um arquivo de socket Unix ou um nome de tubo nomeado do Windows, ou uma opção `--shared-memory-base-name` para especificar o nome da memória compartilhada. Conexões de memória compartilhada são suportadas apenas no Windows.
...
* Em um programa C, você pode especificar os argumentos de arquivo de socket ou número de porta no chamado `mysql_real_connect()`. Você também pode fazer o programa ler arquivos de opções chamando `mysql_options()`. Veja Descrições básicas de funções da API C.
...
Outras interfaces de programação podem oferecer capacidades semelhantes para a leitura de arquivos de opção.