...
Os campos `[err_code]` e `[subsystem]` foram adicionados no MySQL 8.0, e, portanto, estão ausentes dos logs gerados por servidores mais antigos. Os analisadores de logs podem tratar esses campos como partes do texto da mensagem que está presente apenas para logs escritos por servidores recentes o suficiente para incluí-los. Os analisadores devem tratar a parte `err_code` dos indicadores `[err_code]` como um valor de string, e não como um número, porque valores como `MY-012487` e `MY-010051` contêm caracteres não numéricos.

##### Formato de Saída `log_sink_json`

O sink de log no formato JSON produz mensagens como objetos JSON que contêm pares chave-valor. Por exemplo:
...
A mensagem mostrada é reformatada para melhor legibilidade. Eventos escritos no log de erro aparecem uma mensagem por linha.

A chave `ts` (timestamp) é exclusiva do sink de log no formato JSON. O valor é um inteiro indicando milissegundos desde a época (`'1970-01-01 00:00:00'` UTC).
...
O sink de log de sistema produz saída que se conforma ao formato de log de sistema usado na plataforma local.

##### Formato de Saída de Log de Inicialização Precoce

O servidor gera alguns mensagens de log de erro antes que as opções de inicialização tenham sido processadas, e, portanto, antes que ele saiba os valores das variáveis de sistema `log_error_verbosity` e `log_timestamps`, e antes que ele saiba quais componentes de log devem ser usados. O servidor lida com mensagens de log de erro que são geradas no início do processo de inicialização da seguinte forma:

* O servidor armazena eventos de log (em vez de mensagens de log formatadas), o que lhe permite aplicar configurações a esses eventos retroativamente, após os ajustes serem conhecidos, com o resultado de que mensagens descarregadas usam as configurações configuradas, e não os padrões. Além disso, as mensagens são descarregadas para todos os sinks configurados, e não apenas para o sink padrão.
...
Os valores permitidos de `log_timestamps` são `UTC` (o padrão) e `SYSTEM` (a zona horária do sistema local). Os timestamps são escritos no formato ISO 8601 / RFC 3339: `YYYY-MM-DDThh:mm:ss.uuuuuu` mais um valor de cauda de `Z` que indica a hora Zulu (UTC) ou `±hh:mm` (um deslocamento que indica o ajuste da zona horária do sistema local em relação ao UTC). Por exemplo:
