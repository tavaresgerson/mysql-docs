...
Variáveis de sistema não persistidas não podem ser persistidas sob nenhuma circunstância. Variáveis de sistema com restrição de persistência podem ser persistidas com `SET PERSIST_ONLY`, mas apenas por usuários para os quais as seguintes condições sejam satisfeitas:

* A variável de sistema `persist_only_admin_x509_subject` é definida com um valor de Sujeito X.509 de certificado SSL.
* O usuário se conecta ao servidor usando uma conexão criptografada e fornece um certificado SSL com o valor de Sujeito designado.
* O usuário tem privilégios suficientes para usar `SET PERSIST_ONLY` (consulte a Seção 7.1.9.1, “Privilégios de variáveis de sistema”).

Por exemplo, `protocol_version` é de leitura apenas e definido apenas pelo servidor, então não pode ser persistido sob nenhuma circunstância. Por outro lado, `bind_address` é com restrição de persistência, então pode ser definido por usuários que satisfaçam as condições anteriores.
...
```
   ...
   O formato do valor de Sujeito é o mesmo usado para `CREATE USER ... REQUIRE SUBJECT`. Veja a Seção 15.7.1.3, “Instrução CREATE USER”.
...
Suponha que `myclient-cert.pem` seja o certificado SSL a ser usado pelos clientes que podem persistir variáveis de sistema restritas à persistência. Exiba o conteúdo do certificado usando o comando `openssl`:
...
Configure o arquivo `my.cnf` do servidor com o valor de Sujeito:
...
Para usar o X.509, os clientes devem especificar as opções `--ssl-key` e `--ssl-cert` para se conectar. É recomendado, mas não obrigatório, que a opção `--ssl-ca` também seja especificada para que o certificado público fornecido pelo servidor possa ser verificado. Por exemplo:
...
Supondo que o usuário tenha privilégios suficientes para usar `SET PERSIST_ONLY`, as variáveis de sistema restritas à persistência podem ser persistidas da seguinte forma:

```
$> mysql --ssl-key=myclient-key.pem --ssl-cert=myclient-cert.pem --ssl-ca=mycacert.pem
```

Se o servidor não estiver configurado para habilitar a persistência de variáveis de sistema restritas à persistência, ou o usuário não satisfaça as condições necessárias para essa capacidade, ocorrerá um erro:
