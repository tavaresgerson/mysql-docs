...
A partir do MySQL 8.0.24, o otimizador pode transformar uma subconsulta escalar correlacionada em uma tabela derivada quando a bandeira `subquery_to_derived` da variável `optimizer_switch` estiver habilitada. Considere a consulta mostrada aqui:
...
Na consulta reescrita, `REJECT_IF()` representa uma função interna que testa uma condição dada (aqui, a comparação `ct > 1`) e gera um erro específico (neste caso, `ER_SUBQUERY_NO_1_ROW`) se a condição for verdadeira. Isso reflete a verificação de cardinalidade que o otimizador realiza como parte da avaliação da cláusula `JOIN` ou `WHERE`, antes de avaliar qualquer predicado levantado, o que é feito apenas se a subconsulta não retornar mais de uma linha.
...
- Para ser elegível para transformação, um predicado da cláusula `WHERE` deve ser um predicado de igualdade em que cada operando deve ser uma referência simples de coluna. Outros predicados, incluindo outros predicados de comparação, não são elegíveis para transformação. O predicado deve empregar o operador de igualdade `=` para fazer a comparação; o operador `<=>` de segurança contra nulos não é suportado neste contexto.

- Um predicado da cláusula `WHERE` que contém apenas referências internas não é elegível para transformação, pois pode ser avaliado antes do agrupamento. Um predicado da cláusula `WHERE` que contém apenas referências externas é elegível para transformação, mesmo que possa ser elevado ao bloco de consulta externa. Isso é possível ao adicionar uma verificação de cardinalidade sem agrupamento na tabela derivada.
...
- A subconsulta não pode conter funções de janela e não pode conter nenhuma função agregada que agregue em um bloco de consulta externo à subconsulta. Uma função agregada `COUNT()` (se contida no elemento de lista `SELECT` da subconsulta) deve estar no nível mais alto e não pode fazer parte de uma expressão.
