...
Uma tabela derivada é uma expressão que gera uma tabela dentro do escopo de uma cláusula de consulta `FROM`. Por exemplo, uma subconsulta em uma cláusula `SELECT` de uma declaração `FROM` é uma tabela derivada:
...
Os nomes das colunas de uma tabela derivada vêm da lista de seleção:
...
Para fornecer nomes de colunas explicitamente, siga o nome da tabela derivada com uma lista entre parênteses dos nomes das colunas:
...
- Uma tabela derivada não pode conter referências a outras tabelas da mesma `SELECT` (use uma tabela derivada `LATERAL` para isso; veja a Seção 15.2.15.9, “Tabelas Derivadas Laterais”).

- Antes do MySQL 8.0.14, uma tabela derivada não pode conter referências externas. Essa é uma restrição do MySQL que foi removida no MySQL 8.0.14, e não uma restrição do padrão SQL. Por exemplo, a tabela derivada `dt` na seguinte consulta contém uma referência `t1.b` para a tabela `t1` na consulta externa:
  ...
  A consulta é válida no MySQL 8.0.14 e versões superiores. Antes do 8.0.14, ela produz um erro: `Unknown column 't1.b' in 'where clause'`

O otimizador determina informações sobre tabelas derivadas de tal forma que o `EXPLAIN` não precisa materializá-las. Veja a Seção 10.2.2.4, “Otimizando tabelas derivadas, referências de visualização e expressões de tabela comuns com junção ou materialização”.
...
Isso ocorre porque a declaração `SELECT` não fez referência a nenhuma tabela, como pode ser visto nas colunas `table` e `Extra` do resultado. Isso também é verdadeiro para os seguintes `SELECT` aninhados:
...
No entanto, se as referências externas do `SELECT` apontarem para quaisquer tabelas, o otimizador executa a instrução na subconsulta também, resultando na modificação do `t2`:
...
A otimização da tabela derivada também pode ser usada com muitas subconsultas (escalares) correlacionadas (MySQL 8.0.24 e versões posteriores). Para mais informações e exemplos, consulte a Seção 15.2.15.7, “Subconsultas Correlacionadas”.
