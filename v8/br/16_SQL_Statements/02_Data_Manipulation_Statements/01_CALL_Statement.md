...
`CALL` pode retornar valores para o chamador usando parâmetros declarados como parâmetros `OUT` ou `INOUT`. Quando o procedimento retornar, um programa cliente também pode obter o número de linhas afetadas pela instrução final executada dentro da rotina: No nível SQL, chame a função `ROW_COUNT()`; a partir da API C, chame a função `mysql_affected_rows()`.
...
Antes de chamar o procedimento, inicialize a variável que será passada como o parâmetro `INOUT`. Após chamar o procedimento, você pode ver que os valores das duas variáveis foram definidos ou modificados:
...
Em declarações preparadas `CALL` usadas com `PREPARE` e `EXECUTE`, podem ser usados marcadores para os parâmetros `IN`, `OUT` e `INOUT`. Esses tipos de parâmetros podem ser usados da seguinte forma:
...
Para escrever programas em C que utilizam a instrução SQL `CALL` para executar procedimentos armazenados que produzem conjuntos de resultados, a bandeira `CLIENT_MULTI_RESULTS` deve estar habilitada. Isso ocorre porque cada `CALL` retorna um resultado para indicar o status da chamada, além de quaisquer conjuntos de resultados que possam ser retornados por instruções executadas dentro do procedimento. `CLIENT_MULTI_RESULTS` também deve estar habilitado se `CALL` for usado para executar qualquer procedimento armazenado que contenha instruções preparadas. Não é possível determinar quando tal procedimento é carregado se essas instruções produzem conjuntos de resultados, então é necessário assumir que elas o fazem.

`CLIENT_MULTI_RESULTS` pode ser ativado quando você chama `mysql_real_connect()`, seja explicitamente passando a própria bandeira `CLIENT_MULTI_RESULTS`, ou implicitamente passando `CLIENT_MULTI_STATEMENTS` (que também ativa `CLIENT_MULTI_RESULTS`). `CLIENT_MULTI_RESULTS` é ativado por padrão.

Para processar o resultado de uma declaração `CALL` executada usando `mysql_query()` ou `mysql_real_query()`, use um loop que chame `mysql_next_result()` para determinar se há mais resultados. Para um exemplo, veja Suporte à Execução de Declarações Múltiplas.

Os programas em C podem usar a interface de declaração preparada para executar as instruções `CALL` e acessar os parâmetros `OUT` e `INOUT`. Isso é feito processando o resultado de uma instrução `CALL` usando um loop que chama `mysql_stmt_next_result()` para determinar se há mais resultados. Para um exemplo, consulte Suporte a Declaração CALL Preparada. Linguagens que fornecem uma interface MySQL podem usar declarações preparadas `CALL` para recuperar diretamente os parâmetros de procedimento `OUT` e `INOUT`.
