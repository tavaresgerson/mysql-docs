...
As variáveis locais são tratadas como parâmetros de rotina armazenados, no que diz respeito ao tipo de dados e à verificação de estouro. Consulte a Seção 15.1.17, “Instruções CREATE PROCEDURE e CREATE FUNCTION”.

As declarações de variáveis devem aparecer antes das declarações de cursor ou manipulador.

Os nomes de variáveis locais não são sensíveis ao maiúsculas e minúsculas. Os caracteres permitidos e as regras de citação são os mesmos que para outros identificadores, conforme descrito na Seção 11.2, “Nomes de Objetos de Esquema”.
