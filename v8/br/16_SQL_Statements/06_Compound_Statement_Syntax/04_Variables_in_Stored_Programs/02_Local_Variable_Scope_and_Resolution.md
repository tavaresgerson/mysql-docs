...
Da mesma forma, a definição do cursor no procedimento a seguir contém uma declaração `SELECT` que se refere a `xname`. O MySQL interpreta isso como uma referência à variável com esse nome, em vez de uma referência a uma coluna.
