...
`RESIGNAL` transmite as informações sobre a condição de erro disponíveis durante a execução de um manipulador de condição dentro de uma instrução composta dentro de um procedimento armazenado, função, gatilho ou evento. `RESIGNAL` pode alterar algumas ou todas as informações antes de transmiti-las. `RESIGNAL` está relacionado a `SIGNAL`, mas, em vez de originar uma condição como o faz `SIGNAL`, `RESIGNAL` retransmite as informações existentes sobre a condição, possivelmente após modificá-las.

`RESIGNAL` permite tanto lidar com um erro quanto retornar as informações do erro. Caso contrário, ao executar uma instrução SQL dentro do manipulador, as informações que causaram a ativação do manipulador são destruídas. `RESIGNAL` também pode tornar alguns procedimentos mais curtos se um manipulador específico puder lidar com parte de uma situação, então passar a condição "para cima da linha" para outro manipulador.

Não são necessários privilégios para executar a instrução `RESIGNAL`.
...
Para `condition_value` e `signal_information_item`, as definições e regras são as mesmas para `RESIGNAL` e `SIGNAL`. Por exemplo, o `condition_value` pode ser um valor de `SQLSTATE`, e o valor pode indicar erros, avisos ou “não encontrado”. Para obter informações adicionais, consulte a Seção 15.6.7.5, “Declaração SIGNAL”.

A declaração `RESIGNAL` recebe as cláusulas `condition_value` e `SET`, ambas opcionais. Isso leva a vários usos possíveis:
...
Dentro de um manipulador de condição que captura uma condição, um uso exclusivo do `RESIGNAL` é realizar outras ações e, em seguida, passar sem alterações as informações originais da condição (as informações que existiam antes da entrada no manipulador).
...
Neste ponto, se `@a = 0` e `RESIGNAL` empurram a pilha de área de diagnóstico, que agora parece assim:
...
Assim como com o `RESIGNAL` sozinho, a ideia é empurrar a pilha de área de diagnóstico para que a informação original saia. Ao contrário do `RESIGNAL` sozinho, qualquer coisa especificada na cláusula `SET` muda.
...
A declaração `RESIGNAL SET MYSQL_ERRNO = 5` resulta nesta pilha, que é o que o chamador vê:
...
`RESIGNAL` com um valor de condição significa "pushar uma condição na área de diagnóstico atual". Se a cláusula `SET` estiver presente, ela também altera as informações de erro.
...
Essa forma de `RESIGNAL` restaura a última área de diagnóstico e a torna a área de diagnóstico atual. Ou seja, ela "destaca" a pilha de áreas de diagnóstico, o que é o mesmo que uma simples `RESIGNAL` faria sozinha. No entanto, ela também altera a área de diagnóstico dependendo do valor da condição ou das informações do sinal.
...
Isso é semelhante ao exemplo anterior, e os efeitos são os mesmos, exceto que, se `RESIGNAL` acontecer, a área da condição atual parecerá diferente no final. (A razão pela qual a condição adiciona em vez de substituir a condição existente é o uso de um valor de condição.)

A declaração `RESIGNAL` inclui um valor de condição (`SQLSTATE '45000'`), portanto, adiciona uma nova área de condição, resultando em uma pilha de áreas de diagnóstico que parece assim:
