...
Em alguns casos, o MySQL altera silenciosamente as especificações das colunas das instruções `CREATE TABLE` ou `ALTER TABLE`. Essas alterações podem ser de tipo de dado, de atributos associados a um tipo de dado ou de especificação de índice.
...
- Colunas que fazem parte de um `PRIMARY KEY` são consideradas `NOT NULL` mesmo que não sejam declaradas dessa forma.

- Os espaços em branco finais são excluídos automaticamente dos valores dos membros `ENUM` e `SET` quando a tabela é criada.
...
- Se você incluir uma cláusula `USING` para especificar um tipo de índice que não é permitido para um determinado mecanismo de armazenamento, mas há outro tipo de índice disponível que o mecanismo pode usar sem afetar os resultados das consultas, o mecanismo usa o tipo disponível.
...
Certos outros tipos de alterações de dados podem ocorrer se você comprimir uma tabela usando **myisampack**. Veja a Seção 18.2.3.3, “Características da Tabela Compressa”.
