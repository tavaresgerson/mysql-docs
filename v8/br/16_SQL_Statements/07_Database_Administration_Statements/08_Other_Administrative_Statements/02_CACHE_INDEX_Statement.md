...
O cache de chaves mencionado em uma declaração `CACHE INDEX` pode ser criado definindo seu tamanho com uma declaração de configuração de parâmetros ou nas configurações de parâmetros do servidor. Por exemplo:
...
- Atribui o índice para a partição `p0` ao cache de chaves `kc_fast` e o índice para as partições `p1` e `p3` ao cache de chaves `kc_slow`; o índice para a partição restante (`p2`) usa o cache de chaves padrão do servidor.
