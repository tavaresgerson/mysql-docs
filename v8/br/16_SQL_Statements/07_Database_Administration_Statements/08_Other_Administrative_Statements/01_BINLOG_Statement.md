...
`BINLOG` é uma declaração de uso interno. É gerada pelo programa **mysqlbinlog** como a representação imprimível de certos eventos em arquivos de log binário. (Veja a Seção 6.6.9, “mysqlbinlog — Ferramenta para Processamento de Arquivos de Log Binário”.) O valor `'str'` é uma string codificada em base 64 que o servidor decodifica para determinar a mudança de dados indicada pelo evento correspondente.

Para executar as instruções `BINLOG` ao aplicar a saída do **mysqlbinlog**, uma conta de usuário precisa ter o privilégio `BINLOG_ADMIN` (ou o privilégio desatualizado `SUPER`), ou o privilégio `REPLICATION_APPLIER` mais os privilégios apropriados para executar cada evento de log.
