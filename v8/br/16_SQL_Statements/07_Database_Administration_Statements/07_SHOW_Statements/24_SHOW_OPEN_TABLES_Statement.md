#### 15.7.7.24. Mostrar mesas disponíveis
...
`SHOW OPEN TABLES` lista as tabelas não `TEMPORARY` que estão atualmente abertas no cache de tabelas. Veja a Seção 10.4.3.1, “Como o MySQL Abre e Fecha Tabelas”. A cláusula `FROM`, se presente, restringe as tabelas mostradas às presentes no banco de dados `db_name`. A cláusula `LIKE`, se presente, indica quais nomes de tabelas devem ser correspondidos. A cláusula `WHERE` pode ser usada para selecionar linhas com condições mais gerais, conforme discutido na Seção 28.8, “Extensões para Declarações SHOW”.
