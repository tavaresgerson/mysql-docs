...
* Sem `--tab`, o `mysqldump` escreve instruções SQL no saída padrão. Essa saída consiste em instruções `CREATE` para criar objetos dumpados (bancos de dados, tabelas, rotinas armazenadas, etc.) e instruções `INSERT` para carregar dados em tabelas. A saída pode ser salva em um arquivo e recarregada mais tarde usando o `mysql` para recriar os objetos dumpados. Opções estão disponíveis para modificar o formato das instruções SQL e para controlar quais objetos são dumpados.
* Com `--tab`, o `mysqldump` produz dois arquivos de saída para cada tabela dumpada. O servidor escreve um arquivo como texto delimitado por tabulação, uma linha por linha de tabela. Esse arquivo é nomeado `tbl_name.txt` no diretório de saída. O servidor também envia uma instrução `CREATE TABLE` para a tabela ao `mysqldump`, que a escreve como um arquivo nomeado `tbl_name.sql` no diretório de saída.