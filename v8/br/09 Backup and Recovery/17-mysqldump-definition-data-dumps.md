...
A opção `--no-data` instrui o `mysqldump` a não dumper os dados das tabelas, resultando em um arquivo de dump que contém apenas instruções para criar as tabelas. Por outro lado, a opção `--no-create-info` instrui o `mysqldump` a suprimir as instruções `CREATE` do output, de modo que o arquivo de dump contenha apenas os dados das tabelas.
