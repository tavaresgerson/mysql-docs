...
Você pode otimizar uma tabela da mesma maneira usando a instrução SQL `OPTIMIZE TABLE`. `OPTIMIZE TABLE` realiza uma reparação da tabela e uma análise de chaves, além de ordenar a árvore de índices para que as consultas por chave sejam mais rápidas. Não há também possibilidade de interação indesejada entre uma ferramenta e o servidor, porque o servidor realiza todo o trabalho quando você usa `OPTIMIZE TABLE`. Consulte a Seção 15.7.3.4, “Instrução OPTIMIZE TABLE”.
...
*  `--sort-records=index_num` ou `-R index_num`: Ordene as linhas de dados de acordo com um índice específico. Isso torna seus dados muito mais localizados e pode acelerar operações `SELECT` e `ORDER BY` baseadas em intervalo que usam esse índice.
