...
<table><thead><tr> <th>Plugin ou Arquivo</th> <th>Nome do Plugin ou Arquivo</th> </tr></thead><tbody><tr> <td>Plugin do lado do servidor</td> <td><code>test_plugin_server</code></td> </tr><tr> <td>Plugin do lado do cliente</td> <td><code>auth_test_plugin</code></td> </tr><tr> <td>Arquivo da biblioteca</td> <td><code>auth_test_plugin.so</code></td> </tr></tbody></table>

As seções a seguir fornecem informações de instalação e uso específicas para a autenticação conectada de teste:

*  Instalando a Autenticação Conectada de Teste
*  Desinstalando a Autenticação Conectada de Teste
*  Usando a Autenticação Conectada de Teste

Para informações gerais sobre autenticação conectada no MySQL, consulte  Seção 8.2.17, “Autenticação Conectada”.

##### Instalando a Autenticação Conectada de Teste

Esta seção descreve como instalar o plugin de autenticação de teste do lado do servidor. Para informações gerais sobre como instalar plugins, consulte  Seção 7.6.1, “Instalando e Desinstalando Plugins”.
...
Para associar contas MySQL ao plugin de autenticação test, consulte Usar Autenticação de Pluggable de Teste.

##### Desinstalação da Autenticação de Pluggable Test

O método usado para desinstalar o plugin de autenticação test depende de como ele foi instalado:
...
##### Usar Autenticação de Pluggable Test
...
O plugin obtém a senha recebida do cliente e a compara com o valor armazenado na coluna `authentication_string` da linha da conta na tabela de sistema `mysql.user`. Se os dois valores corresponderem, o plugin retorna o valor `authentication_string` como o novo ID de usuário efetivo.

Você pode verificar o log de erro do servidor em busca de uma mensagem que indique se a autenticação foi bem-sucedida (observe que a senha é relatada como o “usuário”):
