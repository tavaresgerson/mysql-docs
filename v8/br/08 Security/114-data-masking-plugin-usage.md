#### 8.5.3.2 Uso do Plugin de Máscara e Desidentificação de Dados do MySQL Enterprise

Antes de usar o Plugin de Máscara e Desidentificação de Dados do MySQL Enterprise, instale-o de acordo com as instruções fornecidas na Seção 8.5.3.1, “Instalação do Plugin de Máscara e Desidentificação de Dados do MySQL Enterprise”.

Para usar o Plugin de Máscara e Desidentificação de Dados do MySQL Enterprise em aplicativos, invocando as funções apropriadas para as operações que deseja realizar. Para descrições detalhadas das funções, consulte a Seção 8.5.3.4, “Descrições das Funções do Plugin de Máscara e Desidentificação de Dados do MySQL Enterprise”. Esta seção demonstra como usar as funções para realizar algumas tarefas representativas. Ela apresenta primeiro uma visão geral das funções disponíveis, seguida por alguns exemplos de como as funções podem ser usadas em contextos do mundo real:
...
##### Máscar Dados para Remover Características Identificadoras
...
*  `mask_inner()` mascara o interior de seu argumento de string, deixando as extremidades não mascaradas. Outros argumentos especificam os tamanhos das extremidades não mascaradas.
  ...
  +--------------------------------------+
  ```
*  `mask_outer()` faz o contrário, mascarando as extremidades de seu argumento de string, deixando o interior não mascarado. Outros argumentos especificam os tamanhos das extremidades mascaradas.
...
Por padrão, `mask_inner()` e `mask_outer()` usam `'X'` como o caractere de máscara, mas permitem um argumento opcional de caractere de máscara:
...
###### Funções de Máscara para Fins Específicos

Outras funções de máscara esperam um argumento de string representando um tipo específico de valor e o mascaram para remover características identificadoras.
...
**Máscara do Número da Conta Principal do Cartão de Pagamento.** As funções de máscara fornecem máscara rigorosa e relaxada dos Números da Conta Principal dos Cartões de Pagamento.
...
**Máscara do Número de Seguro Social dos EUA.** `mask_ssn()` mascara todos, exceto os últimos quatro dígitos do número:
...
`gen_rnd_pan()` retorna um Número da Conta Principal de Cartão de Pagamento aleatório:
...
`gen_rnd_ssn()` retorna um número de Seguro Social dos EUA aleatório com as primeiras e segundas partes escolhidas de um intervalo não usado para números legítimos:
...
`gen_rnd_us_phone()` retorna um número de telefone dos EUA aleatório no código de área 555 não usado para números legítimos:
...
O mascaramento e a desidentificação de dados do MySQL Enterprise permitem que dicionários sejam usados como fontes de valores aleatórios. Para usar um dicionário, ele deve ser carregado primeiro de um arquivo e receber um nome. Cada dicionário carregado se torna parte do registro de dicionários. Os itens podem então ser selecionados dos dicionários registrados e usados como valores aleatórios ou como substituições para outros valores.
...
A função `gen_blocklist()` permite que um termo de um dicionário seja substituído por um termo de outro dicionário, o que resulta em mascaramento por substituição. Seus argumentos são o termo a ser substituído, o dicionário em que o termo aparece e o dicionário a partir do qual escolherá uma substituição. Por exemplo, para substituir uma cidade dos EUA por uma cidade da Alemanha, ou vice-versa, use `gen_blocklist()` da seguinte maneira:
...
Nos centros de atendimento ao cliente, uma técnica comum de verificação de identidade é pedir aos clientes que forneçam seus últimos quatro dígitos do número do Seguro Social (SSN). Por exemplo, um cliente pode dizer que seu nome é Joanna Bond e que seus últimos quatro dígitos do SSN são `0007`.
...
* `ssn`: Número do Seguro Social do cliente.
...
Por que a função `CONVERT()` foi usada para o argumento de `mask_ssn()`? Porque `mask_ssn()` requer um argumento de comprimento 11. Assim, mesmo que `ssn` seja definido como `VARCHAR(11)`, se a coluna `ssn` tiver um conjunto de caracteres multibyte, ela pode parecer ser mais longa que 11 bytes quando passada para uma função carregável, e um erro ocorre. Converter o valor para uma string binária garante que a função veja um argumento de comprimento 11.
...
Se os dados mascarados de uma tabela forem usados em várias consultas, pode ser conveniente definir uma visualização que produza dados mascarados. Dessa forma, os aplicativos podem selecionar a visualização sem realizar o mascaramento em consultas individuais.
