...
* O cliente pode redefinir a senha da conta com `ALTER USER` ou `SET PASSWORD`. Após isso, o servidor restaura o acesso normal para a sessão, bem como para conexões subsequentes que utilizam a conta.
  ...
  Embora seja possível “redefinir” uma senha expirada definindo-a para seu valor atual, é preferível, por questão de boa prática, escolher uma senha diferente. Os DBA podem impor a não reutilização estabelecendo uma política apropriada de reutilização de senhas. Consulte Política de Reutilização de Senhas.
...
Isso é o que normalmente acontece para chamadas interativas do cliente `mysql`, pois, por padrão, tais chamadas são colocadas no modo sandbox. Para retomar o funcionamento normal, selecione uma nova senha.

Para chamadas não interativas do cliente `mysql` (por exemplo, em modo batch), o servidor normalmente desconecta o cliente se a senha expirar. Para permitir que chamadas `mysql` não interativas permaneçam conectadas para que a senha possa ser alterada (usando as instruções permitidas no modo sandbox), adicione a opção `--connect-expired-password` ao comando `mysql`.
...
Do lado do servidor, se um cliente indica que pode lidar com senhas expiradas, o servidor coloca-o no modo sandbox.

Se um cliente não indica que pode lidar com senhas expiradas (ou usa uma versão mais antiga da biblioteca de cliente que não pode indicar isso), a ação do servidor depende do valor da variável de sistema `disconnect_on_expired_password`:
...
* Se `disconnect_on_expired_password` estiver desabilitado, o servidor coloca o cliente no modo sandbox.