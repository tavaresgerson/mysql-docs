#### 8.4.5.10 Filtragem do Log de Auditoria no Modo Legado

::: info Nota

Esta seção descreve a filtragem do log de auditoria no modo legado, que se aplica se o plugin `audit_log` for instalado sem as tabelas e funções de auditoria que acompanham, necessárias para a filtragem baseada em regras.
...
Para filtrar eventos auditados com base na conta de origem, defina uma (não ambas) das seguintes variáveis de sistema no início ou durante o runtime do servidor. Essas variáveis desatualizadas aplicam-se apenas para a filtragem de log de auditoria legado.
...
As modificações em `audit_log_include_accounts` ou `audit_log_exclude_accounts` afetam apenas conexões criadas após a modificação, não conexões existentes.
...
Apenas uma das variáveis `audit_log_include_accounts` ou `audit_log_exclude_accounts` pode ser `NULL` de cada vez:
...
Se um nome de usuário ou nome de host exigir aspas porque contém uma vírgula, um espaço ou outro caractere especial, use aspas simples. Se o próprio valor da variável for citado com aspas simples, duplique cada aspa simples ou escape-a com uma barra invertida. As seguintes instruções habilitam o registro de auditoria para a conta local `root` e são equivalentes, embora os estilos de citação sejam diferentes:
...
A última instrução não funciona se o modo SQL `ANSI_QUOTES` estiver habilitado, porque, nesse modo, as aspas duplas significam citação de identificadores, não citação de strings.
...
Cada variável aceita um valor de `ALL` (registrar todos os eventos associados; esse é o padrão), `ERRORS` (registrar apenas eventos falhos) ou `NONE` (não registrar eventos). Por exemplo, para registrar todos os eventos de declaração, mas apenas eventos de conexão falhos, use essas configurações:
...
Em tempo de execução, é uma variável de leitura somente. Ela aceita um valor de `ALL` (registrar todos os eventos; este é o valor padrão), `LOGINS` (registrar eventos de conexão), `QUERIES` (registrar eventos de declaração) ou `NONE` (não registrar eventos). Para qualquer um desses valores, o plugin de log de auditoria registra todos os eventos selecionados sem distinção de sucesso ou falha. O uso de `audit_log_policy` no início funciona da seguinte forma:

* Se você não definir `audit_log_policy` ou definir como seu valor padrão de `ALL`, quaisquer configurações explícitas para `audit_log_connection_policy` ou `audit_log_statement_policy` se aplicam conforme especificado. Se não especificadas, elas têm o valor padrão de `ALL`.
