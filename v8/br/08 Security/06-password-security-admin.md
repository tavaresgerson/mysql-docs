...
Os administradores de banco de dados devem seguir as diretrizes abaixo para manter as senhas seguras.

O MySQL armazena as senhas dos usuários na tabela `mysql.user` do sistema. O acesso a essa tabela nunca deve ser concedido a contas não administrativas.
...
O plugin `validate_password` pode ser usado para impor uma política de senhas aceitáveis. Consulte a Seção 8.4.3, “O Componente de Validação de Senhas”.
