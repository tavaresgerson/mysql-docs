...
4. Desinstale o plugin `validate_password`:
   ...
   Se o plugin `validate_password` for carregado no início do servidor usando a opção `--plugin-load` ou `--plugin-load-add`, omita essa opção do procedimento de inicialização do servidor. Por exemplo, se a opção estiver listada em um arquivo de opções do servidor, remova-a do arquivo.
