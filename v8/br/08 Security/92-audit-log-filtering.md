...
* A filtragem de auditoria é baseada em regras:
  ...
  + Uma definição de filtro cria um conjunto de regras de auditoria. As definições podem ser configuradas para incluir ou excluir eventos para registro com base nas características descritas.
  + As regras de filtro têm a capacidade de bloquear (abortar) a execução de eventos qualificadores, além das capacidades existentes para o registro de eventos.
  + Vários filtros podem ser definidos, e qualquer filtro dado pode ser atribuído a qualquer número de contas de usuário.
...
A filtragem do log de auditoria é usada para implementar serviços de componentes. Para obter as estatísticas de consulta opcionais disponíveis nessa versão, você as configura como um filtro usando o componente de serviço, que implementa os serviços que escrevem as estatísticas no log de auditoria. Para instruções sobre como configurar esse filtro, consulte Adicionando Estatísticas de Consulta para Detecção de Anomalias.

Para obter informações sobre como escrever regras de filtragem, consulte a Seção 8.4.5.8, “Escrevendo Definições de Filtros de Log de Auditoria”.
...
* Dentro de uma sessão específica, o valor da variável de sistema `audit_log_filter_id` de leitura somente indica se um filtro está atribuído à sessão.


::: info Nota

Por padrão, a filtragem de log de auditoria com base em regras não registra eventos audíveis para nenhum usuário. Para registrar todos os eventos audíveis para todos os usuários, use as seguintes instruções, que criam um filtro simples para habilitar o registro e atribuí-lo à conta padrão:
...
*  `audit_log_filter_remove_user()`: Pare de filtrar uma conta de usuário.
*  `audit_log_filter_flush()`: Limpe as alterações manuais nas tabelas de filtro para afetar o filtro em andamento.

Para exemplos de uso e detalhes completos sobre as funções de filtragem, consulte Usando Funções de Filtragem de Log de Auditoria e Funções de Log de Auditoria.

##### Restrições para Funções de Filtragem de Log de Auditoria

As funções de filtragem de log de auditoria estão sujeitas a essas restrições:
...
* O plugin `audit_log` opera no modo legado se estiver instalado, mas as tabelas e funções de auditoria associadas não forem criadas. O plugin escreve essas mensagens no log de erro ao inicializar o servidor:
  ...
  No modo legado, que está desatualizado, a filtragem pode ser feita com base apenas na conta ou status do evento. Para detalhes, consulte a Seção 8.4.5.10, “Filtragem do Log de Auditoria no Modo Legado”.
* Teoricamente, é possível que um usuário com permissões suficientes crie acidentalmente um item “abort” no filtro de log de auditoria que impeça a si mesmo e outros administradores de acessar o sistema. O privilégio `AUDIT_ABORT_EXEMPT` está disponível para permitir que as consultas de uma conta de usuário sejam sempre executadas, mesmo que um item “abort” as bloqueie. Contas com esse privilégio podem, portanto, ser usadas para recuperar o acesso a um sistema após uma configuração incorreta de auditoria. A consulta ainda é registrada no log de auditoria, mas, em vez de ser rejeitada, é permitida devido ao privilégio.
...
##### Usando Funções de Filtragem de Log de Auditoria

Antes de usar as funções de log de auditoria, instale-as de acordo com as instruções fornecidas na Seção 8.4.5.2, “Instalando ou Desinstalando o MySQL Enterprise Audit”. O privilégio `AUDIT_ADMIN` ou `SUPER` é necessário para usar qualquer uma dessas funções.

As funções de filtragem de log de auditoria permitem o controle de filtragem fornecendo uma interface para criar, modificar e remover definições de filtro e atribuir filtros às contas de usuário.
...
Quando uma conexão chega, o plugin de log de auditoria determina qual filtro usar para a nova sessão, procurando o nome da conta de usuário nas atribuições de filtro atuais:

* Se um filtro for atribuído ao usuário, o log de auditoria usa esse filtro.
* Caso contrário, se não houver atribuição de filtro específica do usuário, mas houver um filtro atribuído à conta padrão (`%`), o log de auditoria usa o filtro padrão.
...
Para atribuir um filtro explicitamente a uma conta de usuário ou contas específicas, defina o filtro e atribua-o às contas relevantes:
...
* Para desvincular o filtro da conta de usuário:

  ```
  SELECT audit_log_filter_remove_user('user1@localhost');
  ```

  A filtragem das sessões atuais para a conta permanece inalterada. Conexões subsequentes da conta são filtradas usando o filtro de conta padrão, se houver, e não são registradas caso contrário.
* Para atribuir um filtro diferente à conta de usuário:
  ...
  A filtragem das sessões atuais para a conta permanece inalterada. Conexões subsequentes da conta são filtradas usando o novo filtro. Para o filtro mostrado aqui, isso significa que não há registro para novas conexões de `user1@localhost`.

Para a filtragem do log de auditoria, as comparações de nome de usuário e nome de host são case-sensitive. Isso difere das comparações para verificação de privilégios, para as quais as comparações de nome de host não são case-sensitive.

Para remover um filtro, faça isso:

```
SELECT audit_log_filter_remove_filter('log_nothing');
```

Remover um filtro também desvincula-o de quaisquer usuários a quem ele é atribuído, incluindo quaisquer sessões atuais para esses usuários.
...
Aviso
 ...
 `audit_log_filter_flush()` afeta todas as sessões atuais e as desconecta de seus filtros anteriores. As sessões atuais não são mais registradas, a menos que desconectem e se reconectem ou executem uma operação de mudança de usuário.

Para determinar se um filtro está atribuído à sessão atual, verifique o valor da variável de sistema `audit_log_filter_id` de leitura `audit_log_filter_id` para determinar se o valor é 0, nenhum filtro é atribuído. Um valor não nulo indica o ID mantido internamente do filtro atribuído:
