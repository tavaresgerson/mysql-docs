...
* Verifique se não há um firewall bloqueando o acesso ao MySQL. Seu firewall pode estar configurado com base na aplicação que está sendo executada ou no número de porta usado pelo MySQL para comunicação (3306 por padrão). Sob Linux ou Unix, verifique a configuração de suas tabelas de IP (ou similar) para garantir que a porta não tenha sido bloqueada. Sob Windows, aplicativos como o ZoneAlarm ou o Windows Firewall podem precisar ser configurados para não bloquear a porta MySQL.
...
* Após uma instalação recente, se você tentar fazer login no servidor como `root` sem usar uma senha, você pode receber a seguinte mensagem de erro.
  ...
  Isso significa que uma senha de root já foi atribuída durante a instalação e ela precisa ser fornecida. Veja a Seção 2.9.4, “Segurança da Conta Inicial do MySQL” sobre as diferentes maneiras em que a senha poderia ter sido atribuída e, em alguns casos, como encontrá-la. Se você precisar redefinir a senha de root, consulte as instruções na Seção B.3.3.2, “Como Redefinir a Senha de Root”. Após encontrar ou redefinir sua senha, faça login novamente como `root` usando a opção `--password` (ou `-p`):
...
* Se um programa cliente recebe o seguinte mensagem de erro quando tenta se conectar, isso significa que o servidor espera senhas em um formato mais recente do que o cliente é capaz de gerar:
  ...
  Enter password:
  ```
* Lembre-se de que os programas cliente usam parâmetros de conexão especificados em arquivos de opção ou variáveis de ambiente. Se um programa cliente parece estar enviando parâmetros de conexão padrão incorretos quando você não os especificou na linha de comando, verifique quaisquer arquivos de opção aplicáveis e seu ambiente. Por exemplo, se você receber `Access denied` ao executar um cliente sem nenhuma opção, certifique-se de que você não especificou uma senha antiga em nenhum de seus arquivos de opção!
...
* Se você receber o seguinte erro, isso significa que você está usando uma senha `root` incorreta:
...
Para obter informações sobre como alterar senhas, consulte a Seção 8.2.14, “Atribuição de Senhas de Conta”.
...
* Se você receber um erro “Acesso negado” ao tentar se conectar ao banco de dados com `mysql -u user_name`, você pode ter um problema com a tabela `user`. Verifique isso executando `mysql -u root mysql` e emitindo esta declaração SQL:
...
O resultado deve incluir uma linha com as colunas `Host` e `User` que correspondam ao nome do host do seu cliente e ao nome do usuário do seu MySQL.
* Se o seguinte erro ocorrer ao tentar se conectar de um host diferente do que contém o servidor MySQL, isso significa que não há uma linha na tabela `user` com um valor de `Host` que corresponda ao nome do host do cliente:

  ```
  SELECT * FROM user;
  ```

  Você pode corrigir isso configurando uma conta para a combinação do nome do host do cliente e do nome do usuário que você está usando ao tentar se conectar.

  Se você não souber o endereço IP ou o nome do host da máquina de onde está se conectando, você deve colocar uma linha com `'%'` como o valor da coluna `Host` na tabela `user`. Após tentar se conectar da máquina do cliente, use uma consulta `SELECT USER()` para ver como você realmente se conectou. Em seguida, mude o `'%'` na linha da tabela `user` para o nome do host real que aparece no log. Caso contrário, seu sistema fica inseguro porque permite conexões de qualquer host para o nome de usuário dado.

  No Linux, outra razão pela qual esse erro pode ocorrer é que você está usando uma versão binária do MySQL compilada com uma versão diferente da biblioteca `glibc` do que a que você está usando. Nesse caso, você deve atualizar seu sistema operacional ou a `glibc`, ou baixar uma distribuição de fonte da versão do MySQL e compilar manualmente. Uma RPM de fonte é normalmente trivial de compilar e instalar, então isso não é um grande problema.
* Se você especificar um nome de host ao tentar se conectar, mas receber uma mensagem de erro onde o nome do host não é mostrado ou é um endereço IP, isso significa que o servidor MySQL recebeu um erro ao tentar resolver o endereço IP do host do cliente para um nome:

  ```
  Host ... is not allowed to connect to this MySQL server
  ```

  Se você tentar se conectar como `root` e receber o seguinte erro, isso significa que você não tem uma linha na tabela `user` com um valor de coluna `User` de `'root'` e que o `mysqld` não consegue resolver o nome do host para o seu cliente:
...
+ Adicione uma entrada para o nome da máquina do cliente no `/etc/hosts` no Unix ou `\windows\hosts` no Windows.
...
+ No Windows, se você estiver executando o servidor e o cliente na mesma máquina e o servidor suportar conexões por meio de tubos nomeados, conecte-se ao nome do host `.` (ponto). As conexões a `.` usam um tubo nomeado em vez de TCP/IP.
* Se `mysql -u root` funciona, mas `mysql -h seu_hostname -u root` resulta em `Access denied` (onde *`seu_hostname`* é o nome real do host local), você pode não ter o nome correto para o seu host na tabela `user`. Um problema comum aqui é que o valor `Host` na linha da tabela `user` especifica um nome de host não qualificado, mas a rotina de resolução de nomes do seu sistema retorna um nome de domínio totalmente qualificado (ou vice-versa). Por exemplo, se você tiver uma linha com `'pluto'` na tabela `user`, mas o DNS informar ao MySQL que o nome do seu host é `'pluto.example.com'`, a linha não funciona. Tente adicionar uma linha à tabela `user` que contenha o endereço IP do seu host como o valor da coluna `Host`. (Alternativamente, você poderia adicionar uma linha à tabela `user` com um valor de `Host` que contenha um caractere de wildcard (por exemplo, `'pluto.%'`). No entanto, o uso de valores de `Host` terminados com `%` é *inseguro* e *não é recomendado*)
* Se `mysql -u user_name` funciona, mas `mysql -u user_name some_db` não funciona, você não deu acesso ao usuário especificado para o banco de dados chamado *`some_db`*.
* Se `mysql -u user_name` funciona quando executado no host do servidor, mas `mysql -h host_name -u user_name` não funciona quando executado em um host cliente remoto, você não habilitou o acesso ao servidor para o nome de usuário especificado do host remoto.
* Se você não conseguir descobrir por que está recebendo `Access denied`, remova da tabela `user` todas as linhas que têm valores de `Host` contendo wildcards (linhas que contêm caracteres `%'` ou `'_'`). Um erro muito comum é inserir uma nova linha com `Host=``%'` e `User=``some_user``, pensando que isso permite especificar `localhost` para se conectar da mesma máquina. A razão pela qual isso não funciona é que os privilégios padrão incluem uma linha com `Host=``localhost'` e `User=`''`. Como essa linha tem um valor de `Host` `'localhost'` que é mais específico que `'%``, ela é usada preferencialmente à nova linha quando se conecta de `localhost`! O procedimento correto é inserir uma segunda linha com `Host=``localhost'` e `User=``some_user``, ou excluir a linha com `Host=``localhost'` e `User=`''`. Após excluir a linha, lembre-se de emitir uma declaração `FLUSH PRIVILEGES` para recarregar as tabelas de concessão. Veja também a Seção 8.2.6, “Controle de Acesso, Etapa 1: Verificação de Conexão”.
* Se você conseguir se conectar ao servidor MySQL, mas receber uma mensagem de `Access denied` sempre que emitir uma declaração `SELECT ... INTO OUTFILE` ou `LOAD DATA`, sua linha na tabela `user` não tem o privilégio `FILE` habilitado.
* Se você alterar as tabelas de concessão diretamente (por exemplo, usando declarações `INSERT`, `UPDATE` ou `DELETE`) e suas alterações parecem ser ignoradas, lembre-se de que você deve executar uma declaração `FLUSH PRIV