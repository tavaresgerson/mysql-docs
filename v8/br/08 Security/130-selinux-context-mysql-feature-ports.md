...
Se você habilitar certos recursos do MySQL, pode ser necessário configurar o contexto de porta TCP do SELinux para portas adicionais usadas por esses recursos. Se as portas usadas por recursos do MySQL não tiverem o contexto correto do SELinux, os recursos podem não funcionar corretamente.

As seções a seguir descrevem como configurar os contextos de porta para recursos do MySQL. Geralmente, o mesmo método pode ser usado para configurar o contexto de porta para qualquer recurso do MySQL. Para obter informações sobre as portas usadas por recursos do MySQL, consulte o Referência de Portas do MySQL.

Para o MySQL 8.4, habilitar `mysql_connect_any` não é necessário nem recomendado.
...
Se o SELinux estiver habilitado, você deve configurar o contexto de porta para as portas de comunicação usadas pelo Roteador do MySQL. Assumindo que as portas de comunicação adicionais usadas pelo Roteador do MySQL são as portas padrão 6446, 6447, 64460 e 64470, em cada instância, configure o contexto de porta executando:
