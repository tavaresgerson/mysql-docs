...
* Quanto a um nome de conta, omitir a parte do host de um nome de papel resulta em uma parte do host de `'%'`. Mas, ao contrário de `'%'` em um nome de conta, uma parte do host de `'%'` em um nome de papel não tem propriedades de correspondência “qualquer host”. Por exemplo, para um nome `'me'@'%'` usado como um nome de papel, a parte do host (`'%'`) é apenas um valor literal; ele não tem nenhuma propriedade de correspondência “qualquer host”.
...
Por essa razão, os nomes de papéis são frequentemente especificados usando apenas a parte do nome do usuário e deixando a parte do nome do host implicitamente ser `'%'`. Especificar um papel com uma parte do host que não seja `'%'` pode ser útil se você pretende criar um nome que funcione tanto como um papel quanto como uma conta de usuário que é permitida para se conectar a partir do host dado.