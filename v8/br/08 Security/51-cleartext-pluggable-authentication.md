#### 8.4.1.4 Autenticação de Texto Claro Ativa (Cleartext Pluggable Authentication) no Lado do Cliente
...
**Tabela 8.17 Nomes de Plugins e Bibliotecas para Autenticação de Texto Claro**
...
* Os programas de cliente `mysql`, `mysqladmin`, `mysqlcheck`, `mysqldump`, `mysqlshow` e `mysqlslap` suportam uma opção `--enable-cleartext-plugin` que habilita o plugin de forma pontual.
* A função C `mysql_options()` suporta uma opção `MYSQL_ENABLE_CLEARTEXT_PLUGIN` que habilita o plugin de forma pontual. Além disso, qualquer programa que use `libmysqlclient` e leia arquivos de opção pode habilitar o plugin incluindo uma opção `enable-cleartext-plugin` em um grupo de opções lido pela biblioteca do cliente.