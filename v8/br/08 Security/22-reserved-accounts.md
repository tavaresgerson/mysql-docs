...
* `'mysql.infoschema'@'localhost'`: Usado como o `DEFINER` para vistas do esquema `INFORMATION_SCHEMA`. O uso da conta `mysql.infoschema` evita problemas que ocorrem se um DBA renomear ou remover a conta `root`. Esta conta está bloqueada para que não possa ser usada para conexões de clientes.