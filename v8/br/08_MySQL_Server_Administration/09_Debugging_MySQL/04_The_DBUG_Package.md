...
A maioria dos programas de clientes usa um valor padrão `debug_options` de `d:t:o,/tmp/program_name.trace`, independentemente da plataforma.
...
Para alterar o valor global `debug`, são necessários privilégios suficientes para definir variáveis de sistema globais. Para alterar o valor da sessão `debug`, são necessários privilégios suficientes para definir variáveis de sistema de sessão restritas. Consulte a Seção 7.1.9.1, “Privilégios de Variáveis de Sistema”.
...
Um valor de `+` ou `-` adiciona ou subtrai do valor atual de `d`:
