...
Os arquivos de rosca NPTL (a nova biblioteca de rosca no Linux) podem causar problemas ao executar o **mysqld** sob o **gdb**. Alguns sintomas são:

- O **mysqld** trava durante o início (antes de escrever `ready for connections`).
...
Ao executar o **mysqld** sob o **gdb**, você deve desabilitar a traça de pilha com `--skip-stack-trace` para poder capturar falhas de segmentação dentro do **gdb**.

Use a opção `--gdb` no **mysqld** para instalar um manipulador de interrupção para `SIGINT` (necessário para parar o **mysqld** com `^C` para definir pontos de interrupção) e desativar o rastreamento de pilha e o gerenciamento de arquivos de núcleo.
...
Se você quiser obter um dump do núcleo no Linux caso o **mysqld** morra com um sinal SIGSEGV, você pode iniciar o **mysqld** com a opção `--core-file`. Esse arquivo de núcleo pode ser usado para criar um backtrace que pode ajudar a descobrir por que o **mysqld** morreu:
...
Se você estiver usando a interface Perl `DBI`, pode ativar as informações de depuração usando o método `trace` ou configurando a variável de ambiente `DBI_TRACE`.
