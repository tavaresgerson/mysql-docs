...
Observe que, antes de iniciar o **mysqld** com o log de consulta geral habilitado, você deve verificar todas as suas tabelas com o **myisamchk**. Consulte o Capítulo 7, *Administração do Servidor MySQL*.
...
Se você usar o arquivo de log de consulta geral padrão, o log será armazenado no diretório do banco de dados como `host_name.log`. Na maioria dos casos, é a última consulta no arquivo de log que interrompeu o **mysqld**, mas, se possível, você deve verificar isso reiniciando o **mysqld** e executando a consulta encontrada a partir das ferramentas de linha de comando do **mysql**. Se isso funcionar, você também deve testar todas as consultas complicadas que não foram concluídas.

Você também pode tentar o comando `EXPLAIN` em todas as instruções `SELECT` que levam muito tempo para garantir que o **mysqld** esteja usando os índices corretamente. Veja a Seção 15.8.2, “Instrução EXPLAIN”.

Você pode encontrar as consultas que levam muito tempo para serem executadas iniciando o **mysqld** com o registro de consultas lentas habilitado. Veja a Seção 7.4.5, “O Registro de Consultas Lentas”.

Se você encontrar o texto `mysqld restarted` no log de erro (normalmente um arquivo chamado `host_name.err`), provavelmente encontrou uma consulta que faz com que o **mysqld** falhe. Se isso acontecer, você deve verificar todas as suas tabelas com o **myisamchk** (veja o Capítulo 7, *Administração do Servidor MySQL*), e testar as consultas nos arquivos de log do MySQL para ver se uma delas falha. Se você encontrar tal consulta, tente primeiro atualizar para a versão mais recente do MySQL. Se isso não ajudar, informe um erro, veja a Seção 1.5, “Como relatar erros ou problemas”.
