...
Antes de usar os Tokens de Versão, instale-os de acordo com as instruções fornecidas na Seção 7.6.6.2, “Instalando ou Desinstalando Tokens de Versão”.
...
A lista de tokens em cada caso é especificada como uma lista separada por ponto e vírgula de pares `name=value`. Os valores da lista de tokens resultantes resultam nestas atribuições de servidor:
...
Cada cliente deve cooperar com o Version Tokens enviando apenas declarações de acordo com a lista de tokens que ele registra em um servidor específico. Por exemplo, se um cliente registra uma lista de tokens de `'emp=read'`, não há nada no Version Tokens que impeça o cliente de enviar atualizações para o banco de dados `emp`. O próprio cliente deve se abster de fazer isso.

Para cada declaração recebida de um cliente, o servidor usa implicitamente o bloqueio, da seguinte forma:

- Pegue uma chave compartilhada para cada token nomeado na lista de tokens do cliente (ou seja, no valor `version_tokens_session`)
...
Se a lista de tokens contiver dois tokens, por que o `version_tokens_set()` retornou o valor `3 version tokens set`? Isso ocorreu porque a lista de tokens original continha duas definições para `tok1`, e a segunda definição substituiu a primeira.
...
- Os valores dos tokens não podem conter caracteres `;`. A extensão dos valores é limitada pelo valor da variável de sistema `max_allowed_packet`.
...
- `version_tokens_lock_exclusive()` obtém blocos de tokens de versão exclusiva. Ele recebe uma lista de um ou mais nomes de bloqueio e um valor de tempo de espera.

- `version_tokens_lock_shared()` obtém blocos de tokens de versão compartilhada. Ele recebe uma lista de um ou mais nomes de bloqueio e um valor de tempo de espera.

- `version_tokens_unlock()` libera blocos de tokens de versão (exclusivos e compartilhados). Não aceita argumentos.
