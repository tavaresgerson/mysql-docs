...
Esta seção descreve como instalar ou desinstalar Tokens de Versão, que é implementado em um arquivo de biblioteca de plugins que contém um plugin e funções carregáveis. Para informações gerais sobre como instalar ou desinstalar plugins e funções carregáveis, consulte a Seção 7.6.1, “Instalando e Desinstalando Plugins”, e a Seção 7.7.1, “Instalando e Desinstalando Funções Carregáveis”.
...
O nome de base do arquivo da biblioteca de plugins é `version_tokens`. O sufixo do nome do arquivo difere de acordo com a plataforma (por exemplo, `.so` para sistemas Unix e Unix-like, `.dll` para Windows).
