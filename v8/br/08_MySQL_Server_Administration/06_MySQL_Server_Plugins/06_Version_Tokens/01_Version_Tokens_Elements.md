...
- Um plugin do lado do servidor chamado `version_tokens` contém a lista de tokens de versão associados ao servidor e se inscreve para receber notificações sobre eventos de execução de declarações. O plugin `version_tokens` usa a API do plugin de auditoria para monitorar declarações recebidas de clientes e compara a lista de tokens de versão específicos de sessão de cada cliente com a lista de tokens de versão do servidor. Se houver uma correspondência, o plugin permite que a declaração seja processada e o servidor continua a processá-la. Caso contrário, o plugin retorna um erro ao cliente e a declaração falha.

- Um conjunto de funções carregáveis fornece uma API em nível SQL para manipular e inspecionar a lista de tokens de versão do servidor mantidos pelo plugin. O privilégio `VERSION_TOKEN_ADMIN` (ou o privilégio desatualizado `SUPER`) é necessário para chamar qualquer uma das funções de token de versão.
