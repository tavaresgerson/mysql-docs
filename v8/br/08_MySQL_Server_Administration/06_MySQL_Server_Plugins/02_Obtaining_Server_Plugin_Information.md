...
- A tabela `mysql.plugin` mostra quais plugins foram registrados com `INSTALL PLUGIN`. A tabela contém apenas nomes de plugins e nomes de arquivos de bibliotecas, portanto, não fornece tanta informação quanto a tabela `PLUGINS` ou a instrução `SHOW PLUGINS`.
