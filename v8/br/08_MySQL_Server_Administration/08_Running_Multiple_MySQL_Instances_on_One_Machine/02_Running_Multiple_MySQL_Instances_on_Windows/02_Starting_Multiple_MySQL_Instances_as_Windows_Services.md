...
No Windows, um servidor MySQL pode ser executado como um serviço do Windows. Os procedimentos para instalar, controlar e remover um único serviço MySQL são descritos na Seção 2.3.4.8, “Iniciar o MySQL como um serviço do Windows”.
...
Para as instruções a seguir, vamos supor que você deseja executar o servidor **mysqld** a partir de duas versões diferentes do MySQL instaladas em `C:\mysql-5.7.9` e `C:\mysql-8.0.44`, respectivamente. (Isso pode ser o caso se você estiver executando 5.7.9 como seu servidor de produção, mas também quiser realizar testes usando 8.0.44.)
...
- **Abordagem 1:** Especifique as opções para todos os serviços em um dos arquivos de opção padrão. Para fazer isso, use um nome de serviço diferente para cada servidor. Suponha que você queira executar o **mysqld** 5.7.9 usando o nome de serviço de `mysqld1` e o **mysqld** 8.0.44 usando o nome de serviço `mysqld2`. Neste caso, você pode usar o grupo `[mysqld1]` para 5.7.9 e o grupo `[mysqld2]` para 8.0.44. Por exemplo, você pode configurar `C:\my.cnf` da seguinte maneira:
...
- **Abordagem 2:** Especifique as opções para cada servidor em arquivos separados e use `--defaults-file` ao instalar os serviços para indicar a cada servidor qual arquivo usar. Nesse caso, cada arquivo deve listar as opções usando um grupo `[mysqld]`.
  ...
  Quando você instala um servidor MySQL como serviço e usa uma opção `--defaults-file`, o nome do serviço deve preceder a opção.

  Após instalar os serviços, inicie e pare-os da mesma maneira que no exemplo anterior.

Para remover vários serviços, use **SC DELETE `mysqld_service_name`** para cada um. Alternativamente, use **mysqld --remove** para cada um, especificando o nome do serviço após a opção `--remove`. Se o nome do serviço for o padrão (`MySQL`), você pode omiti-lo ao usar **mysqld --remove**.
