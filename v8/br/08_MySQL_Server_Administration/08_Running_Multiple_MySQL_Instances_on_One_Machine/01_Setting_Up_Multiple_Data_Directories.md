...
Com esse método, o diretório de dados está no mesmo estado em que estava quando você instalou o MySQL pela primeira vez, com o conjunto padrão de contas do MySQL e sem dados de usuário.

No Unix, inicie o diretório de dados. Consulte a Seção 2.9, “Configuração e teste pós-instalação”.

No Windows, o diretório de dados está incluído na distribuição do MySQL:

- As distribuições de arquivos compactados do MySQL para Windows contêm um diretório de dados não modificado. Você pode descompactar essa distribuição em um local temporário e, em seguida, copiá-la para o diretório `data` onde você está configurando a nova instância.

- Os instaladores de pacotes MSI do Windows criam e configuram o diretório de dados que o servidor instalado usa, mas também criam um diretório de dados "modelo" puro chamado `data` sob o diretório de instalação. Após uma instalação ter sido realizada usando um pacote MSI, o diretório de dados modelo pode ser copiado para configurar instâncias adicionais do MySQL.
