...
Como descrito na Seção 7.4, “Logs do MySQL Server”, o MySQL Server pode criar vários arquivos de log diferentes para ajudá-lo a ver quais atividades estão ocorrendo. No entanto, você deve limpar esses arquivos regularmente para garantir que os logs não ocupem muito espaço no disco.
...
Os arquivos de log binários são removidos automaticamente após o período de validade do log binário do servidor. A remoção dos arquivos pode ocorrer ao iniciar o servidor e quando o log binário é descarregado. O período padrão de validade do log binário é de 30 dias. Para especificar um período de validade alternativo, use a variável de sistema `binlog_expire_logs_seconds`. Se você estiver usando replicação, deve especificar um período de validade que não seja inferior ao tempo máximo em que suas réplicas podem ficar atrasadas em relação à fonte. Para remover logs binários sob demanda, use a instrução `PURGE BINARY LOGS` (consulte a Seção 15.4.1.1, “Instrução PURGE BINARY LOGS”).

Para forçar o MySQL a começar a usar novos arquivos de log, limpe os logs. O esvaziamento dos logs ocorre quando você executa uma instrução `FLUSH LOGS` ou o comando **mysqladmin flush-logs**, **mysqladmin refresh**, **mysqldump --flush-logs** ou **mysqldump --master-data**. Veja a Seção 15.7.8.3, “Instrução FLUSH”, a Seção 6.5.2, “mysqladmin — Um Programa de Administração do Servidor MySQL” e a Seção 6.5.4, “mysqldump — Um Programa de Backup de Bancos de Dados”. Além disso, o servidor esvazia o log binário automaticamente quando o tamanho atual do arquivo de log binário atinge o valor da variável de sistema `max_binlog_size`.
...
A execução de declarações ou comandos de esvaziamento de logs requer a conexão com o servidor usando uma conta que tenha o privilégio `RELOAD`. Em sistemas Unix e Unix-like, outra maneira de esvaziar os logs é enviar um sinal para o servidor, o que pode ser feito por `root` ou pela conta que possui o processo do servidor. (Veja a Seção 6.10, “Tratamento de Sinais Unix no MySQL”.) Os sinais permitem que o esvaziamento de logs seja realizado sem a necessidade de se conectar ao servidor:

- Um sinal `SIGHUP` esvazia todos os logs. No entanto, `SIGHUP` tem efeitos adicionais além da esvaziamento de logs que podem ser indesejáveis.

- A partir do MySQL 8.0.19, `SIGUSR1` faz com que o servidor limpe o log de erros, o log de consultas gerais e o log de consultas lentas. Se você estiver interessado em limpar apenas esses logs, `SIGUSR1` pode ser usado como um sinal mais "leve" que não tem os efeitos de `SIGHUP` que não estão relacionados aos logs.
...
Neste ponto, você pode fazer um backup de `mysql.log.old`, `mysql-slow.log.old` e `err.log.old` e, em seguida, removê-los do disco.
