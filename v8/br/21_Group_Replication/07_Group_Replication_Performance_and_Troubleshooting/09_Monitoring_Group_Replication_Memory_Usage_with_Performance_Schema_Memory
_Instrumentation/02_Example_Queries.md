...
Esta seção descreve consultas de exemplo usando os instrumentos e eventos para monitorar o uso da memória da replicação em grupo. As consultas recuperam dados da tabela `memory_summary_global_by_event_name`.
...
- Memória utilizada na certificação
- Memória utilizada na certificação
...
A alocação de memória para envio e recebimento de transações, certificação e todos os outros processos principais. Ela é calculada através da consulta a todos os eventos do grupo `memory/group_rpl/`. Por exemplo:
...
A alocação de memória para garantias de consistência de transações é a soma dos valores dos eventos `consistent_members_that_must_prepare_transaction`, `consistent_transactions`, `consistent_transactions_prepared`, `consistent_transactions_waiting` e `consistent_transactions_delayed_view_change`. Por exemplo:
...
A alocação de memória para o serviço de mensagem de entrega de replicação de grupo é a soma dos valores de evento `message_service_received_message` e `message_service_queue`. Por exemplo:
