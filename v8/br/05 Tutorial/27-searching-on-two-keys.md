...
Uma consulta `OR` usando uma única chave é bem otimizada, assim como o tratamento de `AND`.
...
Você também pode resolver o problema de forma eficiente usando uma `UNION` que combina a saída de duas instruções `SELECT` separadas.
