...
A opção `--no-data` informa ao **mysqldump** para não drenar os dados da tabela, resultando em um arquivo de dump que contém apenas instruções para criar as tabelas. Por outro lado, a opção `--no-create-info` informa ao **mysqldump** para suprimir as instruções `CREATE` da saída, de modo que o arquivo de dump contenha apenas os dados da tabela.
