...
- Não consigo encontrar o arquivo `tbl_name.MYI` (Erro: `nnn`)
...
Observe que os erros 135 (não há mais espaço no arquivo de registro) e 136 (não há mais espaço no arquivo de índice) não são erros que podem ser corrigidos por uma simples reparação. Nesse caso, você deve usar `ALTER TABLE` para aumentar os valores das opções da tabela `MAX_ROWS` e `AVG_ROW_LENGTH`:
...
Se você receber erros inesperados ao verificar (como erros `out of memory`), ou se o **myisamchk** falhar, vá para a Etapa 3.
...
Se você receber erros inesperados durante a reparação (como erros `out of memory`), ou se o **myisamchk** falhar, vá para a Etapa 3.
