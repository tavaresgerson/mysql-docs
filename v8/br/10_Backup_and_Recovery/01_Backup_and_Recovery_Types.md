...
- As ferramentas de backup físico incluem o **mysqlbackup** do MySQL Enterprise Backup para `InnoDB` ou quaisquer outras tabelas, ou comandos de nível de sistema de arquivos (como **cp**, **scp**, **tar**, **rsync**) para as tabelas `MyISAM`.

- Para restaurar:

  - O MySQL Enterprise Backup restaura `InnoDB` e outras tabelas que ele fez backup.

  - O comando **ndb\_restore** restaura as tabelas `NDB`.
...
- As ferramentas de backup lógicas incluem o programa **mysqldump** e a instrução `SELECT ... INTO OUTFILE`. Elas funcionam para qualquer mecanismo de armazenamento, até mesmo `MEMORY`.

- Para restaurar backups lógicos, os arquivos de dump no formato SQL podem ser processados usando o cliente **mysql**. Para carregar arquivos de texto delimitados, use a instrução `LOAD DATA` ou o cliente **mysqlimport**.
...
- O **mysqldump** pode se conectar a servidores locais ou remotos. Para saída SQL (as instruções `CREATE` e `INSERT`), podem ser feitas gravações locais ou remotas e geradas saídas no cliente. Para saída de texto delimitado (com a opção `--tab`), arquivos de dados são criados no host do servidor.

- O `SELECT ... INTO OUTFILE` pode ser iniciado a partir de um host de cliente local ou remoto, mas o arquivo de saída é criado no host do servidor.
...
A integridade dos dados pode ser comprometida se as tabelas ficarem corrompidas. Para as tabelas `InnoDB`, esse não é um problema típico. Para que os programas possam verificar as tabelas `MyISAM` e repará-las caso problemas sejam encontrados, consulte a Seção 9.6, “Manutenção e Recuperação de Falhas de Tabelas MyISAM”.
