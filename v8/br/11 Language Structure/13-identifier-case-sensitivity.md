...
No entanto, os nomes dos grupos de arquivos de log são sensíveis ao caso. Isso difere do SQL padrão.
...
<table><col style="width: 10%"/><col style="width: 90%"/><thead><tr> <th>Valor</th> <th>Significado</th> </tr></thead><tbody><tr> <td><code>0</code></td> <td>Os nomes da tabela e do banco de dados são armazenados no disco usando a letra maiúscula especificada na instrução <code>CREATE TABLE</code> ou <code>CREATE DATABASE</code>. As comparações de nomes são sensíveis ao maiúsculo. Você não deve <span><em>definir</em></span> essa variável para 0 se estiver executando o MySQL em um sistema que tem nomes de arquivos não sensíveis ao maiúsculo (como o Windows ou o macOS). Se você forçar essa variável para 0 com <code>--lower-case-table-names=0</code> em um sistema de arquivos não sensível ao maiúsculo e acessar os nomes de tabela <code>MyISAM</code> usando diferentes letras, pode ocorrer corrupção de índice.</td> </tr><tr> <td><code>1</code></td> <td>Os nomes das tabelas são armazenados em minúsculas no disco e as comparações de nomes não são sensíveis ao maiúsculo. O MySQL converte todos os nomes de tabela para minúsculas no armazenamento e na busca. Esse comportamento também se aplica aos nomes de banco de dados e aos aliases de tabela.</td> </tr><tr> <td><code>2</code></td> <td>Os nomes da tabela e do banco de dados são armazenados no disco usando a letra maiúscula especificada na instrução <code>CREATE TABLE</code> ou <code>CREATE DATABASE</code>, mas o MySQL converte-os para minúsculas na busca. As comparações de nomes não são sensíveis ao maiúsculo. Isso funciona <span><em>apenas</em></span> em sistemas de arquivos que não são sensíveis ao maiúsculo! Os nomes de tabela e os nomes de visualizações <code>InnoDB</code> são armazenados em minúsculas, como para <code>lower_case_table_names=1</code>.</td> </tr></tbody></table>
...
A sensibilidade ao caso de letras do sistema de arquivos pode afetar as pesquisas em colunas de texto das tabelas do `INFORMATION_SCHEMA`. Para mais informações, consulte a Seção 12.8.7, “Usando Ordenação em Pesquisas no `INFORMATION_SCHEMA`”.