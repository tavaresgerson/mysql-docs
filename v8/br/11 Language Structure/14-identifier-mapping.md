...
Para alguns blocos, como o cirílico, o segundo byte determina a maiúscula. Para outros blocos, como o Latin1 Supplement, o terceiro byte determina a maiúscula. Se dois bytes na sequência forem letras (como no grego estendido), o caractere da letra mais à esquerda representa a maiúscula. Todos os outros bytes de letras devem estar em minúsculas.
* Todos os caracteres não-letras, exceto sublinhado (`_`), bem como as letras de alfabetos que não têm mapeamento de maiúsculas/minúsculas (como o hebraico), são codificados usando representação hexadecimal com letras minúsculas para dígitos hexadecimais `a..f`:
...
Esses nomes também são escritos em `INNODB_TABLES` em suas formas anexadas, mas são escritos em `TABLES` em suas formas não anexadas, conforme inserido pelo usuário.