...
- Se qualquer argumento SRID se referir a um SRS não definido, ocorrerá um erro `ER_SRS_NOT_FOUND`.
...
- `ST_Dimension(g)`
  ...
  Retorna a dimensão inerente do valor de geometria `g`. A dimensão pode ser -1, 0, 1 ou 2. O significado desses valores está descrito na Seção 13.4.2.2, “Classe de Geometria”.
...
- `ST_Envelope(g)`
  ...
  Retorna o retângulo de contorno mínimo (MBR) para o valor de geometria `g`. O resultado é retornado como um valor `Polygon` definido pelos pontos de canto da caixa de contorno:
...
- `ST_GeometryType(g)`
  ...
  Retorna uma string binária que indica o nome do tipo de geometria do qual a instância de geometria `g` é membro. O nome corresponde a uma das subclasses `Geometry` instanciáveis.
...
- `ST_IsEmpty(g)`
  ...
  Essa função é um marcador que retorna 1 para um valor vazio da coleção de geometrias ou 0 caso contrário.
...
- `ST_IsSimple(g)`
  ...
  As descrições das classes geométricas instanciáveis fornecidas na Seção 13.4.2, “O Modelo de Geometria OpenGIS”, incluem as condições específicas que fazem com que as instâncias da classe sejam classificadas como não simples.
...
- `ST_SRID(g [, srid])`
  ...
  Com um único argumento que representa um objeto de geometria válido `g`, o `ST_SRID()` retorna um inteiro que indica o ID do sistema de referência espacial (SRS) associado a `g`.

  Com o segundo argumento opcional representando um valor válido de SRID, `ST_SRID()` retorna um objeto com o mesmo tipo que seu primeiro argumento, com um valor de SRID igual ao segundo argumento. Isso define apenas o valor de SRID do objeto; ele não realiza nenhuma transformação dos valores de coordenadas.

  `ST_SRID()` lida com seus argumentos conforme descrito na introdução desta seção, com esta exceção:

  - Para a sintaxe de um argumento, `ST_SRID()` retorna o SRID da geometria, mesmo que se refira a um SRS indefinido. Não ocorre um erro `ER_SRS_NOT_FOUND`.
  ...
  No entanto, esse método cria a geometria no SRID 0 e, em seguida, a converte para o SRID 4326 (WGS 84). Uma alternativa preferível é criar a geometria com o sistema de referência espacial correto desde o início. Por exemplo:
