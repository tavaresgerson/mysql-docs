...
As funções desta seção realizam operações de busca ou comparação em valores JSON para extrair dados deles, informar se os dados existem em uma localização dentro deles ou informar o caminho para os dados dentro deles. O operador `MEMBER OF()` também está documentado aqui.

- `JSON_CONTAINS(target, candidate[, path])`

  Indica, ao retornar 1 ou 0, se um dado documento JSON `candidate` está contido em um documento JSON `target` ou, se um argumento `path` foi fornecido, se o candidato é encontrado em um caminho específico dentro do alvo. Retorna `NULL` se qualquer argumento for `NULL`, ou se o argumento de caminho não identificar uma seção do documento alvo. Um erro ocorre se `target` ou `candidate` não for um documento JSON válido, ou se o argumento `path` não for uma expressão de caminho válida ou contiver um caractere curinga `*` ou `**`.
  ...
  - Um escalar candidato está contido em um escalar alvo se e somente se eles forem comparáveis e iguais. Dois valores escalares são comparáveis se tiverem os mesmos tipos `JSON_TYPE()`, com a exceção de que os valores dos tipos `INTEGER` e `DECIMAL` também são comparáveis entre si.
  ...
  A partir do MySQL 8.0.17, as consultas que usam `JSON_CONTAINS()` em tabelas `InnoDB` podem ser otimizadas usando índices de múltiplos valores; consulte Índices de Múltiplos Valores para obter mais informações.
...
- `JSON_CONTAINS_PATH(json_doc, one_or_all, path[, path] ...)`
  ...
  - `'all'`: 1 se todas as trajetórias existirem dentro do documento, 0 caso contrário.
...
- `JSON_EXTRACT(json_doc, path[, path] ...)`
  ...
  O MySQL suporta o operador `->` como abreviação para essa função, usado com 2 argumentos, onde o lado esquerdo é um identificador de coluna `JSON` (não uma expressão) e o lado direito é o caminho JSON a ser correspondido dentro da coluna.
...
- `column->path`
  ...
  Matrizes aninhadas são suportadas. Uma expressão que usa `->` é avaliada como `NULL` se nenhuma chave correspondente for encontrada no documento JSON de destino, como mostrado aqui:
  ...
  Esse é o mesmo comportamento observado em casos como esse ao usar `JSON_EXTRACT()`:
...
- `column->>path`
  ...
  Este é um operador de extração sem aspas aprimorado. Enquanto o operador `->` simplesmente extrai um valor, o operador `->>` também desasigna as aspas do resultado extraído. Em outras palavras, dado um valor da coluna `JSON` `column` e uma expressão de caminho `path` (uma literal de string), as seguintes três expressões retornam o mesmo valor:
...
- `JSON_OVERLAPS(json_doc1, json_doc2)`
  ...
  Compara dois documentos JSON. Retorna true (1) se os dois documentos tiverem quaisquer pares chave-valor ou elementos de array em comum. Se ambos os argumentos forem escalares, a função realiza um teste de igualdade simples. Se qualquer argumento for `NULL`, a função retorna `NULL`.

  Essa função serve como contraparte de `JSON_CONTAINS()`, que exige que todos os elementos da matriz pesquisada estejam presentes na matriz pesquisada. Assim, `JSON_CONTAINS()` realiza uma operação `AND` sobre as chaves de pesquisa, enquanto `JSON_OVERLAPS()` realiza uma operação `OR`.

  As consultas sobre colunas JSON das tabelas `InnoDB` usando `JSON_OVERLAPS()` na cláusula `WHERE` podem ser otimizadas usando índices de múltiplos valores. Índices de Múltiplos Valores fornece informações detalhadas e exemplos.

  Ao comparar dois arrays, `JSON_OVERLAPS()` retorna verdadeiro se eles compartilharem um ou mais elementos em comum e falso se não compartilharem:
  ...
  Jogos parciais são tratados como sem resultado, conforme mostrado aqui:
  ...
  Ao comparar um escalar com um array, o `JSON_OVERLAPS()` tenta tratar o escalar como um elemento do array. Neste exemplo, o segundo argumento `6` é interpretado como `[6]`, como mostrado aqui:
...
- `JSON_SEARCH(json_doc, one_or_all, search_str[, escape_char[, path] ...])`
  ...
  Dentro do argumento da string de busca `search_str`, os caracteres `%` e `_` funcionam como o operador `LIKE`: `%` corresponde a qualquer número de caracteres (incluindo zero caracteres), e `_` corresponde exatamente a um caractere.
  ...
  Para obter mais informações sobre a correspondência e o comportamento dos caracteres de escape, consulte a descrição de `LIKE` na Seção 14.8.1, “Funções e Operadores de Comparação de Strings”. Para o tratamento de caracteres de escape, uma diferença em relação ao comportamento de `LIKE` é que o caractere de escape para `JSON_SEARCH()` deve ser avaliado como uma constante no momento da compilação, e não apenas no momento da execução. Por exemplo, se `JSON_SEARCH()` for usado em uma instrução preparada e o argumento `escape_char` for fornecido usando um parâmetro `?`, o valor do parâmetro pode ser constante no momento da execução, mas não no momento da compilação.

  `search_str` e `path` são sempre interpretados como strings utf8mb4, independentemente de sua codificação real. Esse é um problema conhecido que foi corrigido no MySQL 8.0.24 (Bug #32449181).
...
- `JSON_VALUE(json_doc, path)`
  ...
  Extrai um valor de um documento JSON no caminho especificado no documento especificado e retorna o valor extraído, convertendo-o opcionalmente para o tipo desejado. A sintaxe completa é mostrada aqui:
  ...
  `path` é um caminho JSON que aponta para uma localização no documento. Isso deve ser um valor literal de string.
  ...
  - `YEAR` (MySQL 8.0.22 e versões posteriores)
  ...
  Os tipos listados acima são os mesmos tipos (não de array) suportados pela função `CAST()`.

  Se não for especificado por uma cláusula `RETURNING`, o tipo de retorno da função `JSON_VALUE()` é `VARCHAR(512)`. Quando não é especificado nenhum conjunto de caracteres para o tipo de retorno, o `JSON_VALUE()` usa `utf8mb4` com a ordenação binária, que é sensível ao caso; se `utf8mb4` for especificado como o conjunto de caracteres para o resultado, o servidor usa a ordenação padrão para este conjunto de caracteres, que não é sensível ao caso.

  Quando os dados no caminho especificado consistem em ou resolvem um literal JSON nulo, a função retorna SQL `NULL`.

  `on_empty`, se especificado, determina como o `JSON_VALUE()` se comporta quando não são encontrados dados no caminho especificado; esta cláusula aceita um dos seguintes valores:
  ...
  Se usado, `on_error` assume um dos seguintes valores com o resultado correspondente quando ocorre um erro, conforme listado aqui:

  - `NULL ON ERROR`: `JSON_VALUE()` retorna `NULL`; este é o comportamento padrão se nenhuma cláusula `ON ERROR` for usada.

  - `DEFAULT value ON ERROR`: Este é o valor retornado; seu valor deve corresponder ao do tipo de retorno.
  ...
  **Tratamento de erros.** Em geral, os erros são tratados pelo `JSON_VALUE()` da seguinte forma:
  ...
  - `ON ERROR` é acionado sempre que ocorrer qualquer um dos seguintes eventos:
    ...
    - Tentar extrair um objeto ou um array, como o resultado de um caminho que resolve em múltiplos locais dentro do documento JSON

    - Erros de conversão, como tentar converter `'asdf'` para um valor `UNSIGNED`

    - Truncação de valores

  - Um erro de conversão sempre aciona uma mensagem de alerta, mesmo que `NULL ON ERROR` ou `DEFAULT ... ON ERROR` seja especificado.
  ...
  **Exemplos.** Dois exemplos simples são mostrados aqui:
  ...
  A saída `EXPLAIN` a seguir mostra que uma consulta contra `t1` que utiliza a expressão de índice na cláusula `WHERE` usa o índice assim criado:
  ...
  *************************** 1. row ***************************
  ...
  Isso produz o mesmo efeito que criar uma tabela `t2` com um índice em uma coluna gerada (veja Criando um índice de coluna JSON para uma coluna gerada), como esta:
  ...
  A saída `EXPLAIN` para uma consulta nesta tabela, referenciando a coluna gerada, mostra que o índice é usado da mesma maneira que na consulta anterior à tabela `t1`:
...
- `value MEMBER OF(json_array)`
  ...
  As consultas que utilizam `MEMBER OF()` em colunas JSON de tabelas `InnoDB` na cláusula `WHERE` podem ser otimizadas usando índices de múltiplos valores. Consulte Índices de Múltiplos Valores para obter informações detalhadas e exemplos.

  Escalares simples são tratados como valores de matriz, como mostrado aqui:
  ...
  Os valores dos elementos da matriz de partidas parciais não correspondem:
  ...
  *************************** 1. row ***************************
  ...
  É também possível realizar o casting necessário usando a função `JSON_ARRAY()`, da seguinte forma:
