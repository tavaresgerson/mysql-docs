...
O valor máximo de 65 para *`M`* significa que os cálculos nos valores de `DECIMAL` - DECIMAL, NUMERIC") são precisos até 65 dígitos. Esse limite de 65 dígitos de precisão também se aplica a literais numéricos de valor exato, portanto, a faixa máxima desses literais difere da anterior. (Há também um limite sobre o comprimento do texto dos literais `DECIMAL` - DECIMAL, NUMERIC") e veja a Seção 14.24.3, “Tratamento de Expressões”).
...
### 14.24.3 Tratamento de Expressões
...
Se o valor tiver muitas casas decimais na parte fracionária, ocorre a arredondamento e uma nota é gerada. O arredondamento é feito conforme descrito na Seção 14.24.4, “Comportamento de Arredondamento”. A truncagem devido ao arredondamento da parte fracionária não é um erro, mesmo no modo estrito.
...
Antes do MySQL 8.0.31, para os literais `DECIMAL` - DECIMAL, NUMERIC") além do limite de precisão de 65 dígitos, há um limite para o comprimento do texto do literal. Se o valor exceder aproximadamente 80 caracteres, podem ocorrer resultados inesperados. Por exemplo:
...
A partir do MySQL 8.0.31, isso não deve mais ser um problema, conforme mostrado aqui:
...
### 14.24.4 Comportamento de arredondamento
...
A declaração `SHOW WARNINGS` exibe as notas que são geradas por truncação devido à arredondamento da parte fracionária. Tal truncação não é um erro, mesmo no modo SQL estrito (ver Seção 14.24.3, “Tratamento de Expressões”).

### 14.24.5 Exemplos de matemática de precisão

Esta seção fornece alguns exemplos que mostram os resultados de consultas de matemática de precisão no MySQL. Esses exemplos demonstram os princípios descritos na Seção 14.24.3, “Tratamento de Expressões”, e na Seção 14.24.4, “Comportamento de Arredondamento”.
