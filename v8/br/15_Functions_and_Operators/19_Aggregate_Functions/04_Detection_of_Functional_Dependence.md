...
Assim, as colunas `co.name` dependem funcionalmente das colunas `GROUP BY` e a consulta é válida.
...
E, como `co.Code` é chave primária de `co`:
...
Suponha que a integridade referencial tenha sido acidentalmente quebrada e exista uma linha de `countrylanguage` sem uma linha correspondente em `country`. Considere a mesma consulta do exemplo anterior, mas com um `LEFT JOIN`:
...
Se, no resultado da junção `co.Code` for complementado por `NULL`, `co.Name` também será. Se `co.Code` não for complementado por `NULL`, então, como `co.Code` é uma chave primária, ele determina `co.Name`. Portanto, em todos os casos:
...
Essa consulta exibe os países, quantas línguas oficiais diferentes eles têm e quantas cidades eles têm, unindo a visualização com a tabela `city`:
...
O MySQL consegue descobrir uma dependência funcional no resultado de uma visualização e usá-la para validar uma consulta que utiliza a visualização. O mesmo aconteceria se `country2` fosse uma tabela derivada (ou expressão de tabela comum), como em:
