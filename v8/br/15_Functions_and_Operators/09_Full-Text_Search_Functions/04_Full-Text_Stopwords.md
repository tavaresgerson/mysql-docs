### 14.9.4 Palavras-chave completas de stopwords

A lista de palavras-chave é carregada e pesquisada para consultas de texto completo usando o conjunto de caracteres e a ordenação do servidor (os valores das variáveis de sistema `character_set_server` e `collation_server`). Pode ocorrer falhas ou erros nos resultados de busca de palavras-chave se o arquivo de palavras-chave ou as colunas usadas para indexação ou buscas de texto completo tiverem um conjunto de caracteres ou ordenação diferente de `character_set_server` ou `collation_server`.

A sensibilidade à grafia maiúscula ou minúscula das consultas de palavras-chave depende da collation do servidor. Por exemplo, as consultas são não sensíveis à grafia maiúscula ou minúscula se a collation for `utf8mb4_0900_ai_ci`, enquanto as consultas são sensíveis à grafia maiúscula ou minúscula se a collation for `utf8mb4_0900_as_cs` ou `utf8mb4_bin`.
...
Para definir sua própria lista de palavras-chave para todas as tabelas `InnoDB`, defina uma tabela com a mesma estrutura da tabela `INNODB_FT_DEFAULT_STOPWORD`, preencha-a com palavras-chave e defina o valor da opção `innodb_ft_server_stopword_table` para um valor na forma `db_name/table_name` antes de criar o índice de texto completo. A tabela de palavras-chave deve ter uma única coluna `VARCHAR` com o nome `value`. O exemplo a seguir demonstra como criar e configurar uma nova tabela global de palavras-chave para `InnoDB`.
...
A lista a seguir mostra as palavras-chave padrão para os índices de pesquisa `MyISAM`. Em uma distribuição de fonte MySQL, você pode encontrar essa lista no arquivo `storage/myisam/ft_static.c`.
