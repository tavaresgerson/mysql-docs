...
Um ngram é uma sequência contínua de caracteres `n` de uma sequência dada de texto. O analisador de ngrams tokeniza uma sequência de texto em uma sequência contínua de caracteres `n`. Por exemplo, você pode tokenizar “abcd” para diferentes valores de `n` usando o analisador de texto completo de ngrams.
...
As seguintes opções de configuração de comprimento mínimo e máximo de palavras são ignoradas para índices `FULLTEXT` que utilizam o analisador de n-gramas: `innodb_ft_min_token_size`, `innodb_ft_max_token_size`, `ft_min_word_len` e `ft_max_word_len`.

#### Criando um índice FULLTEXT que usa o analisador ngram

Para criar um índice `FULLTEXT` que utilize o analisador de ngramas, especifique `WITH PARSER ngram` com `CREATE TABLE`, `ALTER TABLE` ou `CREATE INDEX`.

O exemplo a seguir demonstra como criar uma tabela com um índice `ngram` `FULLTEXT`, inserir dados de amostra (texto simplificado chinês) e visualizar dados tokenizados na tabela do esquema de informações `INNODB_FT_INDEX_CACHE`.
...
Como um índice ngram `FULLTEXT` contém apenas ngramas e não contém informações sobre o início dos termos, as pesquisas com caracteres curinga podem retornar resultados inesperados. Os seguintes comportamentos se aplicam às pesquisas com caracteres curinga usando índices de pesquisa ngram `FULLTEXT`:

- Se o termo prefixo de uma pesquisa com wildcard for menor que o tamanho do token ngram, a consulta retorna todas as linhas indexadas que contêm tokens ngram começando com o termo prefixo. Por exemplo, assumindo `ngram_token_size=2`, uma pesquisa com “a\*” retorna todas as linhas que começam com “a”.
