...
<table summary="Uma referência que lista as funções XML."><thead><tr><th>Nome</th> <th>Descrição</th> </tr></thead><tbody><tr><td>[[<code>ExtractValue()</code>]]</td> <td>Extrair um valor de uma string XML usando a notação XPath</td> </tr><tr><td>[[<code>UpdateXML()</code>]]</td> <td>Retorno substituiu o fragmento XML</td> </tr></tbody></table>

Esta seção discute o XML e as funcionalidades relacionadas no MySQL.

Nota

É possível obter saída formatada em XML do MySQL nos clientes **mysql** e **mysqldump** ao invocá-los com a opção `--xml`. Veja a Seção 6.5.1, “mysql — O Cliente de Linha de Comando do MySQL”, e a Seção 6.5.4, “mysqldump — Um Programa de Backup de Banco de Dados”.
...
Essas funções continuam em desenvolvimento. Continuamos a melhorar essas e outros aspectos da funcionalidade do XML e XPath no MySQL 8.0 e em versões posteriores. Você pode discutir essas funcionalidades, fazer perguntas sobre elas e obter ajuda de outros usuários no Fórum de Usuários do MySQL XML.
...
- **Variáveis de usuário (verificação fraca).** Variáveis que usam a sintaxe `$@variable_name` (ou seja, variáveis de usuário) não são verificadas. O servidor não emite avisos ou erros se uma variável tiver o tipo errado ou não tiver sido atribuído um valor anteriormente. Isso também significa que o usuário é totalmente responsável por quaisquer erros tipográficos, pois não são emitidos avisos se, por exemplo, `$@myvairable` for usado onde `$@myvariable` foi intencionado.
...
- **Variáveis em programas armazenados (verificação rigorosa).** Variáveis que utilizam a sintaxe `$variable_name` podem ser declaradas e usadas com essas funções quando chamadas dentro de programas armazenados. Essas variáveis são locais ao programa armazenado em que são definidas e são verificadas rigorosamente quanto ao tipo e ao valor.
...
- `ExtractValue(xml_frag, xpath_expr)`
  ...
  `ExtractValue()` recebe dois argumentos de string, um fragmento de marcação XML `xml_frag` e uma expressão XPath `xpath_expr` (também conhecida como localizador); ele retorna o texto (`CDATA`) do primeiro nó de texto que é um filho do(s) elemento(s) correspondido(s) pela expressão XPath.
  ...
  Se não for encontrado nenhum nó de texto correspondente à expressão (incluindo o `/text()` implícito), por qualquer motivo, desde que `xpath_expr` seja válido e `xml_frag` consista em elementos corretamente aninhados e fechados, uma string vazia é retornada. Não há distinção entre uma correspondência em um elemento vazio e nenhuma correspondência. Isso é por design.

  Se você precisar determinar se nenhum elemento correspondente foi encontrado em `xml_frag` ou se um elemento foi encontrado, mas não continha nós de texto filhos, você deve testar o resultado de uma expressão que usa a função XPath `count()`. Por exemplo, ambas as seguintes declarações retornam uma string vazia, conforme mostrado aqui:
...
- `UpdateXML(xml_target, xpath_expr, new_xml)`
  ...
  Se não for encontrado uma expressão que corresponda a `xpath_expr`, ou se forem encontrados múltiplos correspondentes, a função retorna o fragmento XML original de `xml_target`. Todos os três argumentos devem ser strings. Se qualquer um dos argumentos de `UpdateXML()` for `NULL`, a função retorna `NULL`.
...
- `/tag`
  ...
  Exemplo: `/a` tem uma correspondência em `<a><b/></a>` porque corresponde à tag mais externa (raiz). Não corresponde ao elemento interno `a` em `<b><a/></b>` porque, neste caso, é filho de outro elemento.

- `/tag1/tag2`

  Corresponda a `<tag2/>` se e somente se for um filho de `<tag1/>`, e `<tag1/>` é o elemento raiz.

  Exemplo: `/a/b` corresponde ao elemento `b` no fragmento XML `<a><b/></a>`, pois é um filho do elemento raiz `a`. Não há correspondência em `<b><a/></b>`, pois, neste caso, `b` é o elemento raiz (e, portanto, não tem filhos em nenhum outro elemento). Além disso, a expressão XPath não tem correspondência em `<a><c><b/></c></a>`; aqui, `b` é um descendente de `a`, mas não é realmente um filho de `a`.
...
- `//tag`
  ...
  Concorrerá a qualquer instância de `<tag>`.
  ...
  `//tag` é equivalente a `/descendant-or-self::*/tag`. Um erro comum é confundir isso com `/descendant-or-self::tag`, embora a expressão da última possa levar a resultados muito diferentes, como pode ser visto aqui:
...
- Também é possível combinar um elemento com base no valor de um ou mais de seus atributos. Isso é feito usando a sintaxe `tag[@attribute="value"]`. Por exemplo, a expressão `//b[@id="idB"]` combina o segundo elemento `b` no fragmento `<a><b id="idA"/><c/><b id="idB"/></a>`. Para combinar com *qualquer* elemento que tenha `attribute="value"`, use a expressão XPath `//*[attribute="value"]`.
  ...
  Para encontrar elementos para os quais o mesmo atributo corresponda a vários valores, você pode usar múltiplos locatores unidos pelo operador `|`. Por exemplo, para corresponder a todos os elementos `b` cujos atributos `c` tenham qualquer um dos valores 23 ou 17, use a expressão `//b[@c="23"]|//b[@c="17"]`. Você também pode usar o operador lógico `or` para esse propósito: `//b[@c="23" or @c="17"]`.
...
**Tratamento de erros.** Para os `ExtractValue()` e `UpdateXML()`, o localizador XPath utilizado deve ser válido e o XML a ser pesquisado deve conter elementos corretamente aninhados e fechados. Se o localizador for inválido, um erro será gerado:
...
Se `xml_frag` não contiver elementos corretamente aninhados e fechados, é retornado `NULL` e um aviso é gerado, conforme mostrado neste exemplo:
...
Não são realizados nenhum tipo de verificação na entrada. Isso significa que um usuário mal-intencionado pode "quebrar o circuito" do teste ao inserir `' or 1=1` tanto para o nome de login quanto para a senha, resultando em `$xpath` ser avaliado conforme mostrado aqui:

```
//user[login/text()='' or 1=1 and password/text()='' or 1=1]/attribute::id
```

Como a expressão dentro dos colchetes sempre avalia como `true`, ela é efetivamente a mesma que esta, que corresponde ao atributo `id` de cada elemento `user` no documento XML:
...
É importante também verificar a saída que está sendo enviada de volta ao cliente. Considere o que pode acontecer quando usamos a função MySQL `ExtractValue()`:
...
Como `ExtractValue()` retorna múltiplos resultados como uma única string delimitada por espaços, esse ataque de injeção fornece a cada usuário todas as IDs válidas contidas em `users.xml` como uma única linha de saída. Como medida de segurança adicional, você também deve testar a saída antes de devolvê-la ao usuário. Aqui está um exemplo simples:
