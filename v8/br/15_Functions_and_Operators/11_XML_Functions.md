...
É possível obter uma saída formatada em XML do MySQL nos clientes `--xml` do **mysql** e **mysqldump**, invocando-os com a opção `--xml`. Veja a Seção 6.5.1, “mysql — O Cliente de Linha de Comando MySQL”, e a Seção 6.5.4, “mysqldump — Um Programa de Backup de Banco de Dados”.
...
Usar essa função é equivalente a realizar uma correspondência usando o *`xpath_expr`* após anexar `/text()`. Em outras palavras, [`ExtractValue('<a><b>Sakila</b></a>', '/a/b')`](xml-functions.html#function_extractvalue) e [`ExtractValue('<a><b>Sakila</b></a>', '/a/b/text()')`](xml-functions.html#function_extractvalue) produzem o mesmo resultado. Se *`xml_frag`* ou *`xpath_expr`* é `NULL`, a função retorna `NULL`.
...
`NULL` é retornado se *`xml_frag`* contém elementos que não estão corretamente aninhados ou fechados, e um aviso é gerado, como mostrado neste exemplo:
...
Essa função substitui uma única porção de um fragmento dado de marcação XML *`xml_target`* por um novo fragmento XML *`new_xml`*, e então retorna o XML alterado. A porção de *`xml_target`* que é substituída corresponde a uma expressão XPath *`xpath_expr`* fornecida pelo usuário.

Se não for encontrada nenhuma expressão que corresponda a *`xpath_expr`*, ou se forem encontrados vários correspondentes, a função retorna o fragmento XML original de *`xml_target`*. Todos os três argumentos devem ser strings. Se qualquer um dos argumentos de `UpdateXML()` for `NULL`, a função retorna `NULL`.
...
Exemplo: `/a` tem uma correspondência em `<a><b/></a>`, pois corresponde à tag mais externa (raiz). Não corresponde ao elemento interno *`a`* em `<b><a/></b>`, porque, neste caso, é a criança de outro elemento.

* `/tag1/tag2`

Corresponda a `<tag2/>` se e somente se for um filho de `<tag1/>`, e `<tag1/>` é o elemento raiz.

Exemplo: `/a/b` corresponde ao elemento *`b`* no fragmento XML `<a><b/></a>`, pois é uma subcamada do elemento raiz *[[`a`]*. Não há correspondência em `<b><a/></b>`, pois, neste caso, *`b`* é o elemento raiz (e, portanto, a subcamada de nenhum outro elemento). Além disso, a expressão XPath não tem correspondência em `<a><c><b/></c></a>`; aqui, *`b`* é um descendente de *[[`a`]*, mas não é realmente uma subcamada de *[[`a`]*.

Esse construtor pode ser estendido para três ou mais elementos. Por exemplo, a expressão XPath `/a/b/c` corresponde ao elemento *`c`* no fragmento `<a><b><c/></b></a>`.
