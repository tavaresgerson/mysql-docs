...
É também possível converter um número em uma string explicitamente usando a função `CAST()`. A conversão ocorre implicitamente com a função `CONCAT()`, pois ela espera argumentos de string.
...
- Se um ou ambos os argumentos forem `NULL`, o resultado da comparação é `NULL`, exceto para o operador de comparação de igualdade `NULL`-seguro `<=>`. Para `NULL <=> NULL`, o resultado é verdadeiro. Não é necessária nenhuma conversão.
...
- Se um dos argumentos for uma coluna `TIMESTAMP` ou `DATETIME` e o outro argumento for uma constante, a constante é convertida em um timestamp antes que a comparação seja realizada. Isso é feito para ser mais compatível com o ODBC. Isso não é feito para os argumentos de `IN()`. Para garantir a segurança, sempre use strings de data, hora ou datetime completas ao fazer comparações. Por exemplo, para obter os melhores resultados ao usar `BETWEEN` com valores de data ou hora, use `CAST()` para converter explicitamente os valores para o tipo de dado desejado.
  ...
  Uma subconsulta de uma única linha de uma tabela ou tabelas não é considerada uma constante. Por exemplo, se uma subconsulta retornar um inteiro a ser comparado com um valor `DATETIME`, a comparação é feita como dois inteiros. O inteiro não é convertido em um valor temporal. Para comparar os operandos como valores `DATETIME`, use `CAST()` para converter explicitamente o valor da subconsulta em `DATETIME`.
...
Observe o resultado ao selecionar esta tabela e comparar `c3`, que é uma coluna `VARCHAR`, com o inteiro `0`:
...
Quando o `0` é citado, a declaração é bem-sucedida, mas a tabela criada não contém nenhuma linha porque não havia nenhuma que correspondesse ao `'0'`, como mostrado aqui:
...
Este é um problema conhecido, que ocorre devido ao fato de que o modo estrito não é aplicado ao processamento do `SELECT`. Veja também o Modo SQL Estrito.
...
Como as conversões produzidas por essa biblioteca diferem, em alguns casos, dos resultados não `dtoa` dos resultados anteriores, existe a possibilidade de incompatibilidades em aplicativos que dependem dos resultados anteriores. Por exemplo, aplicativos que dependem de um resultado exato específico de conversões anteriores podem precisar de ajustes para acomodar uma precisão adicional.
...
Para as conversões de `D` -> `F` -> `D`, uma condição suficiente para a perda de dados ser nula é que `D` use 15 dígitos ou menos de precisão, não seja um valor denormalizado, `-inf`, `+inf` ou `NaN`. Em alguns casos, a conversão é sem perda de dados mesmo que `D` tenha mais de 15 dígitos de precisão, mas isso nem sempre é o caso.

A conversão implícita de um valor numérico ou temporal para string produz um valor que tem um conjunto de caracteres e uma ordenação determinados pelas variáveis de sistema `character_set_connection` e `collation_connection`. (Essas variáveis são comumente definidas com `SET NAMES`. Para informações sobre conjuntos de caracteres de conexão, consulte a Seção 12.4, “Conjunto de caracteres de conexão e ordenação”).

Isso significa que essa conversão resulta em uma string de caractere (não binário) (um valor `CHAR`, `VARCHAR` ou `LONGTEXT`), exceto no caso em que o conjunto de caracteres de conexão está definido como `binary`. Nesse caso, o resultado da conversão é uma string binária (um valor `BINARY`, `VARBINARY` ou `LONGBLOB`).
...
Neste caso, a tabela na coluna resultante da expressão tem o tipo `INT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") ou `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") dependendo do comprimento da expressão inteira. Se o comprimento máximo da expressão não cabe em um `INT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT"), é usado em vez disso `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT"). O comprimento é obtido a partir do valor `max_length` do conjunto de metadados do conjunto de resultados `SELECT` (veja as estruturas básicas de dados da API C). Isso significa que você pode forçar um `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") em vez de `INT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") usando uma expressão suficientemente longa:
