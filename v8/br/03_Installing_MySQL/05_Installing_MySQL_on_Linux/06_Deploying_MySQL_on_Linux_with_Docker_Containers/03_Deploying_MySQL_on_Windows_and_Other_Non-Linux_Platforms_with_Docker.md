...
- Se você estiver montando em vínculo no diretório de dados MySQL do contêiner (consulte Persistência de Mudanças de Dados e Configurações para obter detalhes), você deve definir a localização do arquivo de soquete do servidor com a opção `--socket` para um local fora do diretório de dados MySQL; caso contrário, o servidor não consegue iniciar. Isso ocorre porque a maneira como o Docker para Windows lida com o montagem de arquivos não permite que um arquivo de host seja montado em vínculo no arquivo de soquete.
