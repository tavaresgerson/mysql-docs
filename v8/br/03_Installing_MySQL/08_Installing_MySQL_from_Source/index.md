...
Se você estiver interessado em construir o MySQL a partir de uma distribuição de código-fonte usando opções de compilação semelhantes às usadas pela Oracle para produzir distribuições binárias na sua plataforma, obtenha uma distribuição binária, descompacte-a e procure no arquivo `docs/INFO_BIN`, que contém informações sobre como essa distribuição do MySQL foi configurada e compilada.
