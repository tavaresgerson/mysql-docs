...
- Para definir quais compiladores C e C++ usar, você pode definir as variáveis de ambiente `CC` e `CXX`. Por exemplo:
  ...
  Para especificar suas próprias flags do compilador C e C++, use as opções `CMAKE_C_FLAGS` e `CMAKE_CXX_FLAGS` do CMake. Veja Fлагаs do Compilador.
...
- O arquivo `sql_yacc.cc` é gerado a partir de `sql_yacc.yy`. Normalmente, o processo de compilação não precisa criar `sql_yacc.cc`, pois o MySQL vem com uma cópia pré-gerada. No entanto, se você precisar recriá-lo, você pode encontrar esse erro:
