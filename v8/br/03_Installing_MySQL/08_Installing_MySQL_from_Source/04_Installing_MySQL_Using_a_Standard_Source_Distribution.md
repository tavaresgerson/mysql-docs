...
- Para desempacotar um arquivo **tar** compactado, o **tar** pode descompactar e desempacotar a distribuição se tiver suporte à opção `z`:
  ...
  Se o seu **tar** não tiver suporte para a opção `z`, use **gunzip** para descomprimir a distribuição e **tar** para descompac-la:
...
Construa fora da árvore de origem para manter a árvore limpa. Se o diretório de origem de nível superior estiver nomeado `mysql-src` sob seu diretório de trabalho atual, você pode construir em um diretório nomeado `build` no mesmo nível. Crie o diretório e vá até lá:
...
O diretório de compilação não precisa estar fora da árvore de código-fonte. Por exemplo, você pode compilar em um diretório chamado `build` sob a árvore de código-fonte de nível superior. Para fazer isso, começando com `mysql-src` como seu diretório de trabalho atual, crie o diretório `build` e vá até lá:
...
Antes de fazer uma solicitação no Slack da Comunidade MySQL, verifique os arquivos no diretório `CMakeFiles` para obter informações úteis sobre o erro. Para enviar um relatório de erro, use as instruções na Seção 1.5, “Como relatar erros ou problemas”.
...
Em Windows, crie o diretório de dados e, em seguida, crie um pacote de instalação de arquivo `.zip`:
