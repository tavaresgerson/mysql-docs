### 2.10.1 Instalar o Perl no Unix
...
`DBI` requer o módulo `Data::Dumper`. Ele pode estar instalado; se não estiver, você deve instalá-lo antes de instalar `DBI`.
...
É uma boa ideia reconstruir e reinstalar a distribuição `DBD::mysql` sempre que você instalar uma nova versão do MySQL. Isso garante que as versões mais recentes das bibliotecas do cliente do MySQL sejam instaladas corretamente.
