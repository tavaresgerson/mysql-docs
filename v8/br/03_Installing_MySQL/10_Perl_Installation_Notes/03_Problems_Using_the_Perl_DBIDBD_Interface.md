...
Observe que você também pode precisar modificar as opções `-L` se houver outras bibliotecas que o link não conseguir encontrar. Por exemplo, se o link não conseguir encontrar `libc` porque ele está em `/lib` e o comando de link especifica `-L/usr/lib`, mude a opção `-L` para `-L/lib` ou adicione `-L/lib` ao comando de link existente.
