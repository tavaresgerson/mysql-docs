## 2.1 Orientações Gerais de Instalação

As seções imediatamente subsequentes contêm as informações necessárias para escolher, baixar e verificar sua distribuição. As instruções nas seções posteriores do capítulo descrevem como instalar a distribuição que você escolheu. Para distribuições binárias, consulte as instruções na Seção 2.2, “Instalando MySQL em Unix/Linux Usando Binários Genericos” ou a seção correspondente para sua plataforma, se disponível. Para construir o MySQL a partir de fonte, use as instruções na Seção 2.8, “Instalando MySQL a partir de fonte”.

### 2.1.1 Plataformas suportadas

O suporte da plataforma MySQL evolui ao longo do tempo; consulte <https://www.mysql.com/support/supportedplatforms/database.html> para as atualizações mais recentes.

### 2.1.2 Qual versão e distribuição do MySQL a instalar

Ao preparar a instalação do MySQL, decida qual versão e formato de distribuição (binário ou fonte) utilizar.

Primeiro, decida se vai instalar a partir de uma série de correções de bugs, como o MySQL 8.4, ou usar uma versão de inovação, como o MySQL 9.5. Ambas as versões incluem correções de bugs, enquanto uma versão de inovação inclui as funcionalidades mais recentes. As versões de correções de bugs e de inovação são destinadas ao uso em produção.

O esquema de nomenclatura no MySQL 8.0 utiliza nomes de versão que consistem em três números e um sufixo opcional (por exemplo, **mysql-8.0.34**). Os números dentro do nome da versão são interpretados da seguinte forma:

* O primeiro número (**8**) é o número da versão principal.

* O segundo número (**0**) é o número de versão menor. Juntos, os números principais e menores constituem o número da série de lançamento. O número da série descreve o conjunto de recursos estáveis.

* O terceiro número (**34**) é o número da versão dentro da série de lançamento. Este número é incrementado para cada nova versão de correção de bugs; para uma versão inovadora, provavelmente sempre será 0. Para uma série de correção de bugs, como o MySQL 8.0, a versão mais recente dentro da série é a melhor escolha.

Depois de escolher qual versão do MySQL instalar, decida qual formato de distribuição instalar para o seu sistema operacional. Para a maioria dos casos de uso, uma distribuição binária é a escolha certa. Distribuições binárias estão disponíveis em formato nativo para muitas plataformas, como pacotes RPM para Linux ou pacotes DMG para macOS. As distribuições também estão disponíveis em formatos mais genéricos, como arquivos Zip ou arquivos **tar** comprimidos. Em Windows, você pode usar o [Instalação do MySQL][(mysql-installer.html "2.3.3 MySQL Installer for Windows")] para instalar uma distribuição binária.

Em algumas circunstâncias, pode ser preferível instalar o MySQL a partir de uma distribuição de fonte:

* Você deseja instalar o MySQL em um local explícito. As distribuições binárias padrão estão prontas para execução em qualquer local de instalação, mas você pode precisar de ainda mais flexibilidade para colocar os componentes do MySQL onde você deseja.

* Você deseja configurar o **mysqld** com recursos que talvez não estejam incluídos nas distribuições binárias padrão. Aqui está uma lista das opções extras mais comuns usadas para garantir a disponibilidade dos recursos:

+ `-DWITH_LIBWRAP=1` para suporte a wrappers TCP.

+ `-DWITH_ZLIB={system|bundled}` para recursos que dependem de compressão

+ `-DWITH_DEBUG=1` para suporte de depuração

Para informações adicionais, consulte a Seção 2.8.7, “Opções de Configuração de Fonte MySQL”.

* Você deseja configurar o **mysqld** sem alguns recursos que estão incluídos nas distribuições binárias padrão.

* Você quer ler ou modificar o código C e C++ que compõe o MySQL. Para isso, obtenha uma distribuição de código-fonte.

* As distribuições de fonte contêm mais testes e exemplos do que as distribuições binárias.

### 2.1.3 Como obter o MySQL

Consulte nossa página de downloads em <https://dev.mysql.com/downloads/> para obter informações sobre a versão atual do MySQL e instruções de download.

Para plataformas Linux baseadas em RPM que utilizam Yum como seu sistema de gerenciamento de pacotes, o MySQL pode ser instalado usando o [Repositório Yum de MySQL][(https://dev.mysql.com/downloads/repo/yum/)]. Consulte a Seção 2.5.1, “Instalando MySQL no Linux Usando o Repositório MySQL Yum”, para obter detalhes.

Para plataformas Linux baseadas no Debian, o MySQL pode ser instalado usando o [repositório APT do MySQL][(https://dev.mysql.com/downloads/repo/apt/)]. Consulte a Seção 2.5.2, “Instalando o MySQL no Linux usando o repositório APT do MySQL”, para obter detalhes.

Para as plataformas SUSE Linux Enterprise Server (SLES), o MySQL pode ser instalado usando o [Repositório SLES MySQL][(https://dev.mysql.com/downloads/repo/suse/)]. Consulte a Seção 2.5.3, “Instalando o MySQL no Linux usando o Repositório MySQL SLES”, para obter detalhes.

Para obter a fonte de desenvolvimento mais recente, consulte a Seção 2.8.5, “Instalando o MySQL usando uma árvore de fonte de desenvolvimento”.

### 2.1.4 Verificar a integridade do pacote usando verificações de checksums MD5 ou GnuPG

Depois de baixar o pacote MySQL que atende às suas necessidades e antes de tentar instalá-lo, certifique-se de que ele está intacto e não foi manipulado. Existem três meios de verificação de integridade:

* Checagens MD5
* Assinaturas criptográficas usando `GnuPG`, o GNU Privacy Guard

* Para pacotes RPM, o mecanismo de verificação de integridade RPM integrado

As seções a seguir descrevem como usar esses métodos.

Se você perceber que o checksum MD5 ou as assinaturas GPG não correspondem, tente primeiro baixar o pacote respectivo mais uma vez, talvez de outro site de espelhamento.

#### 2.1.4.1 Verificar o checksum MD5

Depois de ter baixado um pacote MySQL, você deve garantir que seu checksum MD5 corresponda ao fornecido nas páginas de download do MySQL. Cada pacote tem um checksum individual que você pode verificar contra o pacote que você baixou. O checksum MD5 correto está listado na página de downloads para cada produto MySQL; você deve compará-lo com o checksum MD5 do arquivo (produto) que você baixa.

Cada sistema operacional e configuração oferece sua própria versão de ferramentas para verificar o checksum MD5. Tipicamente, o comando é chamado de **md5sum**, ou pode ser chamado de **md5**, e alguns sistemas operacionais não o fornecem. No Linux, ele faz parte do pacote **GNU Text Utilities**, que está disponível para uma ampla gama de plataformas. Você também pode baixar o código-fonte de <http://www.gnu.org/software/textutils/>. Se você tiver o OpenSSL instalado, pode usar o comando **openssl md5 *`package_name`***. Uma implementação do utilitário de linha de comando **md5** do Windows está disponível em <http://www.fourmilab.ch/md5/>. **winMd5Sum** é uma ferramenta gráfica de verificação de MD5 que pode ser obtida em <http://www.nullriver.com/index/products/winmd5sum>. Nossos exemplos para Microsoft Windows assumem o nome **md5.exe**.

Exemplos de Linux e Microsoft Windows:

```
$> md5sum mysql-standard-8.0.44-linux-i686.tar.gz
aaab65abbec64d5e907dcd41b8699945  mysql-standard-8.0.44-linux-i686.tar.gz
```

```
$> md5.exe mysql-installer-community-8.0.44.msi
aaab65abbec64d5e907dcd41b8699945  mysql-installer-community-8.0.44.msi
```

Você deve verificar se o checksum resultante (a string de dígitos hexadecimais) corresponde ao exibido na página de download imediatamente abaixo do respectivo pacote.

Nota

Certifique-se de verificar o checksum do arquivo *de arquivo* (por exemplo, o arquivo `.zip`, `.tar.gz` ou `.msi`) e não dos arquivos que estão contidos dentro do arquivo. Em outras palavras, verifique o arquivo antes de extrair seu conteúdo.

#### 2.1.4.2 Verificação de assinatura usando GnuPG

Outro método para verificar a integridade e autenticidade de um pacote é o uso de assinaturas criptográficas. Isso é mais confiável do que usar [checksums MD5][(verifying-md5-checksum.html "2.1.4.1 Verifying the MD5 Checksum")], mas exige mais trabalho.

Assinamos pacotes de download do MySQL com **GnuPG** (GNU Privacy Guard). **GnuPG** é uma alternativa de código aberto à bem conhecida Pretty Good Privacy (**PGP**) do Phil Zimmermann. A maioria das distribuições Linux vem com **GnuPG** instalado por padrão. Caso contrário, consulte <http://www.gnupg.org/> para obter mais informações sobre **GnuPG** e como obtê-lo e instalá-lo.

Para verificar a assinatura de um pacote específico, você primeiro precisa obter uma cópia da nossa chave de compilação pública GPG, que você pode baixar em <http://pgp.mit.edu/>. A chave que você deseja obter é chamada `mysql-build@oss.oracle.com`. O keyID para pacotes MySQL 8.0.44 e superiores, MySQL 8.4.7 e superiores, e MySQL 9.5.0 e superiores é `B7B3B788A8D3785C`. Após obter essa chave, você deve compará-la com o valor seguinte antes de usá-la para verificar pacotes MySQL. Alternativamente, você pode copiar e colar a chave diretamente do texto abaixo.

Nota

A chave de compilação pública GPG para pacotes de versões anteriores do MySQL (keyID `A8D3785C`, `5072E1F5` ou `3A79BD29`) pode ser encontrada na Seção 2.1.4.5, “Chave de compilação pública GPG para pacotes arquivados”.

```
-----BEGIN PGP PUBLIC KEY BLOCK-----

udMv77etKsN/mPdhJotVLMUpzeespcu5G2qqc5zt
=6wRS
-----END PGP PUBLIC KEY BLOCK-----
```

Para importar a chave de construção em seu conjunto de chaves GPG público pessoal, use **gpg --import**. Por exemplo, se você salvou a chave em um arquivo chamado `mysql_pubkey.asc`, o comando de importação é o seguinte:

```
$> gpg --import mysql_pubkey.asc
gpg: key B7B3B788A8D3785C: public key "MySQL Release Engineering
<mysql-build@oss.oracle.com>" imported
gpg: Total number processed: 1
gpg:               imported: 1
```

Você também pode baixar a chave do servidor de chaves pública usando o ID da chave pública, `A8D3785C`:

```
$> gpg --recv-keys b7b3b788a8d3785c
gpg: requesting key B7B3B788A8D3785C from hkp server keys.gnupg.net
gpg: key B7B3B788A8D3785C: "MySQL Release Engineering <mysql-build@oss.oracle.com>"
1 new user ID
gpg: key B7B3B788A8D3785C: "MySQL Release Engineering <mysql-build@oss.oracle.com>"
53 new signatures
gpg: no ultimately trusted keys found
gpg: Total number processed: 1
gpg:           new user IDs: 1
gpg:         new signatures: 53
```

Se você deseja importar a chave na configuração RPM para validar os pacotes de instalação RPM, você deve ser capaz de importar a chave diretamente:

```
$> rpm --import mysql_pubkey.asc
```

Se você tiver problemas ou precisar de informações específicas sobre o RPM, consulte a Seção 2.1.4.4, “Verificação de assinatura usando RPM”.

Depois de ter baixado e importado a chave de compilação pública, baixe o pacote MySQL desejado e a assinatura correspondente, que também está disponível na página de download. O arquivo de assinatura tem o mesmo nome que o arquivo de distribuição com a extensão `.asc`, conforme mostrado pelos exemplos na tabela a seguir.

**Tabela 2.1 Arquivos de pacote e assinaturas do MySQL para arquivos de fonte**

<table><col style="width: 25%"/><col style="width: 75%"/><thead><tr> <th>File Type</th> <th>File Name</th> </tr></thead><tbody><tr> <td>Distribution file</td> <td><code>mysql-8.0.44-linux-glibc2.28-x86_64.tar.xz</code></td> </tr><tr> <td>Signature file</td> <td><code>mysql-8.0.44-linux-glibc2.28-x86_64.tar.xz.asc</code></td> </tr></tbody></table>

Certifique-se de que ambos os arquivos estejam armazenados no mesmo diretório e, em seguida, execute o seguinte comando para verificar a assinatura do arquivo de distribuição:

```
$> gpg --verify package_name.asc
```

Se o pacote baixado for válido, você deve ver uma mensagem `Good signature` semelhante à seguinte:

```
$> gpg --verify mysql-8.0.44-linux-glibc2.28-x86_64.tar.xz.asc
gpg: Signature made Fri 15 Dec 2023 06:55:13 AM EST
gpg:                using RSA key BCA43417C3B485DD128EC6D4B7B3B788A8D3785C
gpg: Good signature from "MySQL Release Engineering <mysql-build@oss.oracle.com>"
```

A mensagem `Good signature` indica que a assinatura do arquivo é válida, quando comparada à assinatura listada em nosso site. Mas você também pode ver avisos, como este:

```
$> gpg --verify mysql-8.0.44-linux-glibc2.28-x86_64.tar.xz.asc
gpg: Signature made Fri 15 Dec 2023 06:55:13 AM EST
gpg:                using RSA key BCA43417C3B485DD128EC6D4B7B3B788A8D3785C
gpg: Good signature from "MySQL Release Engineering <mysql-build@oss.oracle.com>"
gpg: WARNING: This key is not certified with a trusted signature!
gpg:          There is no indication that the signature belongs to the owner.
Primary key fingerprint: BCA4 3417 C3B4 85DD 128E  C6D4 B7B3 B788 A8D3 785C
```

Isso é normal, pois eles dependem da configuração e da configuração do seu sistema. Aqui estão as explicações para esses avisos:

* *gpg: não foram encontradas chaves confiáveis no final*: Isso significa que a chave específica não é "confिável no final" por você ou sua rede de confiança, o que está bem para os propósitos de verificação de assinaturas de arquivos.

* *AVISO: Esta chave não está certificada com uma assinatura confiável! Não há indicação de que a assinatura pertença ao proprietário.*: Isso se refere ao seu nível de confiança na crença de que você possui nossa chave pública real. Esta é uma decisão pessoal. Idealmente, um desenvolvedor do MySQL entregaria a chave pessoalmente, mas mais comumente, você a baixou. O download foi manipulado? Provavelmente não, mas esta decisão cabe a você. Configurar uma rede de confiança é um método para confiar neles.

Consulte a documentação do GPG para obter mais informações sobre como trabalhar com chaves públicas.

#### 2.1.4.3 Verificação de assinatura usando Gpg4win para Windows

A seção 2.1.4.2, “Verificação de assinatura usando GnuPG”, descreve como verificar os downloads do MySQL usando GPG. Esse guia também se aplica ao Microsoft Windows, mas outra opção é usar uma ferramenta gráfica como o Gpg4win. Você pode usar uma ferramenta diferente, mas nossos exemplos são baseados no Gpg4win e utilizam sua GUI integrada `Kleopatra`.

Baixe e instale o Gpg4win, e, em seguida, carregue o Kleopatra. O diálogo deve parecer semelhante a:

**Figura 2.1 Kleopatra: Tela Inicial**

![Shows the default Kleopatra screen. The top menu includes "File", "View", "Certificates", "Tools", "Settings", "Window", and "Help.". Underneath the top menu is a horizontal action bar with available buttons to "Import Certificates", "Redisplay", and "Lookup Certificates on Server". Greyed out buttons are "Export Certificates" and "Stop Operation". Underneath is a search box titled "Find". Underneath that are three tabs: "My Certificates", "Trusted Certificates", and "Other Certificates" with the "My Certificates" tab selected. "My Certificates" contains six columns: "Name", "E-Mail", "Valid From", "Valid Until", "Details", and "Key-ID". There are no example values.](images/gnupg-kleopatra-home.png)

Em seguida, adicione o certificado de Engenharia de Lançamento do MySQL. Faça isso clicando em Arquivo, Procure Certificados no servidor. Digite "Engenharia de Lançamento do MySQL" na caixa de pesquisa e pressione Pesquisar.

**Figura 2.2 Kleopatra: Assistente de Busca de Certificados no Servidor: Encontrando um Certificado**

![Shows a search input field titled "Find" with "mysql release engineering" entered. The one result contains the following values: Name=MySQL Release Engineering, E-Mail=mysql-build@oss.oracle.com, Valid From=2003-02-03, Valid Until="", Details=OpenPGP, Fingerprint=5072E1F5, and Key-ID=5072E1F5. Available action buttons are: Search, Select All, Deselect All, Details, Import, and Close.](images/gnupg-kleopatra-find-certificate.png)

Selecione o certificado "MySQL Release Engineering". O Fingerprint e Key-ID devem ser "3A79BD29" para o MySQL 8.0.28 e superior ou "5072E1F5" para o MySQL 8.0.27 e anterior, ou escolha Detalhes... para confirmar que o certificado é válido. Agora, importe-o clicando em Importar. Quando o diálogo de importação for exibido, escolha Okay, e este certificado deve agora estar listado na guia Certificados importados.

Em seguida, configure o nível de confiança para o nosso certificado. Selecione o nosso certificado e, a partir do menu principal, selecione Certificados, Alterar proprietário de confiança.... Sugerimos que você escolha "Verificações muito precisas" para o nosso certificado, pois, caso contrário, você pode não ser capaz de verificar nossa assinatura. Selecione "Verificações muito precisas" para habilitar a "confiança total" e, em seguida, pressione OK.

**Figura 2.3 Kleopatra: Altere o nível de confiança para o MySQL Release Engineering**

![A list of trust options are displayed, the options include "I don't know (unknown trust)", "I do NOT trust them (never trust)", "I believe checks are casual (marginal trust)", "I believe checks are very accurate (full trust)", and "This is my certificate (ultimate trust)". The "I believe checks are very accurate (full trust)" option is selected.](images/gnupg-kleopatra-change-trust.png)

Em seguida, verifique o arquivo do pacote MySQL baixado. Isso requer arquivos tanto para o arquivo embalado quanto para a assinatura. O arquivo de assinatura deve ter o mesmo nome que o arquivo embalado, mas com a extensão `.asc` anexada, conforme mostrado pelo exemplo na tabela a seguir. A assinatura está vinculada na página de downloads para cada produto MySQL. Você deve criar o arquivo `.asc` com essa assinatura.

**Tabela 2.2 Arquivos de pacote e assinaturas do MySQL para o instalador do MySQL para Microsoft Windows**

<table><col style="width: 25%"/><col style="width: 75%"/><thead><tr> <th>File Type</th> <th>File Name</th> </tr></thead><tbody><tr> <td>Distribution file</td> <td><code>mysql-installer-community-8.0.44.msi</code></td> </tr><tr> <td>Signature file</td> <td><code>mysql-installer-community-8.0.44.msi.asc</code></td> </tr></tbody></table>

Certifique-se de que ambos os arquivos estejam armazenados no mesmo diretório e, em seguida, execute o seguinte comando para verificar a assinatura do arquivo de distribuição. Arraste e solte o arquivo de assinatura (`.asc`) para o Kleopatra, ou carregue o diálogo a partir de Arquivo, Desencriptar/Verificar Arquivos..., e, em seguida, escolha o arquivo `.msi` ou `.asc`.

**Figura 2.4 Kleopatra: O diálogo para descriptografar e verificar arquivos**

![Shows available decrypt and verify options to perform. A MySQL Installer MSI file is used in the example where the .asc file is listed as "Input file" and the .msi file is listed under "Signed Data". The "Input file is detached signature" option's check box is checked. A "Input file is an archive; unpack with:" option is shown but greyed out. Below is the "Create all output files in a single folder" option check box that is checked, and an "Output folder" input field with "C:/docs" entered as an example. The available buttons are "Back" (greyed out), "Decrypt/Verify", and "Cancel."](images/gnupg-kleopatra-decrypt-load.png)

Clique em Decryptor/Verificar para verificar o arquivo. Os dois resultados mais comuns parecem da seguinte figura; embora o aviso amarelo possa parecer problemático, o que segue significa que a verificação do arquivo foi realizada com sucesso. Agora você pode executar este instalador.

**Figura 2.5 Kleopatra: o diálogo para descriptografar e verificar os resultados: todas as operações concluídas**

![Yellow portion of the results window shows "Not enough information to check signature validity" and "The validity of the signature cannot be verified." Also shown is key information, such as the KeyID and email address, the key's sign on date, and also displays the name of the ASC file..](images/gnupg-kleopatra-decrypt-okay-sig.png)

Ver um erro de assinatura vermelha significa que o arquivo é inválido. Não execute o arquivo MSI se você ver esse erro.

**Figura 2.6 Kleopatra: o diálogo para descriptografar e verificar os resultados: ruim**

![Red portion of the results window shows "Invalid signature", "Signed with unknown certificate", "The signature is bad", and also displays the name of the ASC file.](images/gnupg-kleopatra-decrypt-invalid-sig.png)

A seção 2.1.4.2, “Verificação de assinatura usando GnuPG”, explica por que você não vê um resultado verde `Good signature`.

#### 2.1.4.4 Verificação de assinatura usando RPM

Para pacotes RPM, não há uma assinatura separada. Os pacotes RPM possuem uma assinatura GPG integrada e um checksum MD5. Você pode verificar um pacote executando o seguinte comando:

```
$> rpm --checksig package_name.rpm
```

Exemplo:

```
$> rpm --checksig mysql-community-server-8.0.44-1.el8.x86_64.rpm
mysql-community-server-8.0.44-1.el8.x86_64.rpm: digests signatures OK
```

Nota

Se você está usando o RPM 4.1 e ele reclama sobre `(GPG) NOT OK (MISSING KEYS: GPG#3a79bd29)`, mesmo que você tenha importado a chave de construção pública do MySQL em seu próprio conjunto de chaves GPG, você precisa importar a chave no conjunto de chaves RPM primeiro. O RPM 4.1 não usa mais seu conjunto de chaves GPG pessoal (ou o GPG em si). Em vez disso, o RPM mantém um conjunto de chaves separado porque é uma aplicação de âmbito sistêmico e o conjunto de chaves públicas do GPG de um usuário é um arquivo específico do usuário. Para importar a chave pública do MySQL no conjunto de chaves RPM, obtenha primeiro a chave, em seguida, use **rpm --import** para importar a chave. Por exemplo:

```
$> gpg --export -a 3a79bd29 > 3a79bd29.asc
$> rpm --import 3a79bd29.asc
```

Como alternativa, o **rpm** também suporta o carregamento da chave diretamente de uma URL:

```
$> rpm --import https://repo.mysql.com/RPM-GPG-KEY-mysql-2023
```

Você também pode obter a chave pública do MySQL a partir desta página do manual: Seção 2.1.4.2, “Verificação de assinatura usando GnuPG”.

#### 2.1.4.5 Chave de Compilação Pública GPG para Pacotes Arquivados

A chave de compilação pública GPG a seguir (keyID `3A79BD29`) pode ser usada para verificar a autenticidade e a integridade das versões dos pacotes MySQL 8.0.28 a 8.0.35, 8.1.0 e 8.2.0. Para instruções de verificação de assinatura, consulte a Seção 2.1.4.2, “Verificação de assinatura usando GnuPG”. Ela expirou em 14 de dezembro de 2023.

Chave de compilação pública GPG para pacotes do MySQL 8.0.36 a 8.0.43 e 8.3.0 a 8.4.6 e 9.0.0 a 9.4.0

```
        -----BEGIN PGP PUBLIC KEY BLOCK-----
Version: SKS 1.1.6
Comment: Hostname: pgp.mit.edu


cAZUlaj3id3TxquAlud4lWDz
=h5nH
-----END PGP PUBLIC KEY BLOCK-----
```

Chave de compilação pública GPG para MySQL 8.0.28 a 8.0.35 e pacotes 8.1.0/8.2.0

```
-----BEGIN PGP PUBLIC KEY BLOCK-----


HAB5DGMkx92HUMidsTSKHe+QnnnoFmu4gnmDU31i
=Xqbo
-----END PGP PUBLIC KEY BLOCK-----
```

A seguinte chave de compilação pública do GPG (keyID `5072E1F5`) pode ser usada para verificar a autenticidade e a integridade dos pacotes do MySQL 8.0.27 e anteriores. Para instruções de verificação de assinatura, consulte a Seção 2.1.4.2, “Verificação de assinatura usando GnuPG”.

##### Chave de Compilação Pública GPG para Pacotes do MySQL 8.0.27 e Anterior

```
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: SKS 1.1.6
Comment: Hostname: pgp.mit.edu


=yia9
-----END PGP PUBLIC KEY BLOCK-----
```

### 2.1.5 Layouts de instalação

O layout de instalação difere para diferentes tipos de instalação (por exemplo, pacotes nativos, tarballs binários e tarballs de fonte), o que pode levar à confusão ao gerenciar diferentes sistemas ou usar diferentes fontes de instalação. Os layouts individuais são fornecidos no capítulo correspondente ao tipo de instalação ou plataforma, conforme descrito a seguir. Note que o layout das instalações de fornecedores que não são a Oracle pode diferir desses layouts.

* Seção 2.3.1, “Configuração da instalação do MySQL no Microsoft Windows”
* Seção 2.8.3, “Configuração do MySQL para instalação de fonte”
* Tabela 2.3, “Configuração da instalação do MySQL para pacotes binários genéricos Unix/Linux”
* Tabela 2.12, “Configuração da instalação do MySQL para pacotes RPM do Linux da Zona de Desenvolvimento do MySQL”
* Tabela 2.7, “Configuração da instalação do MySQL no macOS”

### 2.1.6 Características de compilação específicas do compilador

Em alguns casos, o compilador usado para construir o MySQL afeta as funcionalidades disponíveis para uso. As notas desta seção se aplicam a distribuições binárias fornecidas pela Oracle Corporation ou que você compilar sozinho a partir de fonte.

****icc** (Intel C++ Compiler) Builds**

Um servidor construído com **icc** tem essas características:

* O suporte SSL não está incluído.