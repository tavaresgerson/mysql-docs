...
Suponha que uma tabela `t` tenha uma coluna binária chamada `col1`, definida como `VARBINARY(50)`. Admitindo que as informações na coluna são codificadas usando um único conjunto de caracteres, você pode convertê-las para uma coluna não binária que tenha esse conjunto de caracteres. Por exemplo, se `col1` contém dados binários que representam caracteres no conjunto de caracteres `greek`, você pode convertê-los da seguinte forma:

```
ALTER TABLE t MODIFY col1 VARCHAR(50) CHARACTER SET greek;
```

Se a coluna original tiver um tipo de `BINARY(50)`, você pode convertê-la para `CHAR(50)`, mas os valores resultantes são preenchidos com `0x00` bytes no final, o que pode ser indesejável. Para remover esses bytes, use a função `TRIM()`:

```
UPDATE t SET col1 = TRIM(TRAILING 0x00 FROM col1);
```

Suponha que a tabela `t` tenha uma coluna não binária chamada `col1`, definida como `CHAR(50) CHARACTER SET latin1`, mas você deseja convertê-la para usar `utf8mb4` para que possa armazenar valores de várias línguas. A seguinte declaração realiza isso:
...
As declarações `ALTER TABLE` que realizam alterações nos conjuntos de caracteres ou nas colunas de tabelas ou colunas devem ser realizadas usando `ALGORITHM=COPY`. Para obter mais informações, consulte a Seção 17.12.1, “Operações DDL Online”.
