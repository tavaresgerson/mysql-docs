...
- Os elementos `<collation>` indicam a ordem de caracteres para comparação e ordenação, um elemento por cotação. As cotações binárias não precisam do elemento `<map>`, porque os próprios códigos de caracteres fornecem a ordem.

Para um conjunto de caracteres complexo, conforme implementado em um arquivo `ctype-MYSET.c` no diretório `strings`, existem arrays correspondentes: `ctype_MYSET[]`, `to_lower_MYSET[]` e assim por diante. Nem todos os conjuntos de caracteres complexos têm todos os arrays. Veja também os arquivos existentes `ctype-*.c` para exemplos. Veja o arquivo `CHARSET_INFO.txt` no diretório `strings` para informações adicionais.

A maioria dos arrays é indexada pelo valor do caractere e tem 256 elementos. O array `<ctype>` é indexado pelo valor do caractere + 1 e tem 257 elementos. Essa é uma convenção herdada para lidar com `EOF`.
...
Os valores da máscara de bits em `m_ctype.h` são valores octal, mas os elementos do array `<ctype>` em `MYSET.xml` devem ser escritos como valores hexadecimais.

Os arrays `<lower>` e `<upper>` armazenam os caracteres minúsculos e maiúsculos correspondentes a cada membro do conjunto de caracteres. Por exemplo:
