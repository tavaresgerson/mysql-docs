...
Se você deseja adicionar suporte para um novo conjunto de caracteres chamado `MYSET` que inclui caracteres multibyte, você deve usar funções de caracteres multibyte no arquivo de código `ctype-MYSET.c` no diretório `strings`.
