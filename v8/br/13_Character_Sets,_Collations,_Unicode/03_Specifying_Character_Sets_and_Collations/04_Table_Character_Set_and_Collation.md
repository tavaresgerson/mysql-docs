...
Cada tabela tem um conjunto de caracteres da tabela e uma ordenação da tabela. As instruções `CREATE TABLE` e `ALTER TABLE` têm cláusulas opcionais para especificar o conjunto de caracteres da tabela e a ordenação da tabela:
...
- Caso contrário (nem `CHARACTER SET` nem `COLLATE` sejam especificados), o conjunto de caracteres e a ordenação do banco de dados serão usados.
