## 12.5 Configurando o conjunto de caracteres e a codificação do aplicativo

Para aplicações que armazenam dados usando o conjunto de caracteres e a correção de texto padrão do MySQL (`utf8mb4`, `utf8mb4_0900_ai_ci`), não é necessário realizar nenhuma configuração especial. Se as aplicações exigem armazenamento de dados usando um conjunto de caracteres ou correção de texto diferente, você pode configurar as informações do conjunto de caracteres de várias maneiras:
...
Os exemplos mostrados aqui assumem o uso do conjunto de caracteres `latin1` e da ordenação `latin1_swedish_ci` em contextos específicos como uma alternativa aos padrões de `utf8mb4` e `utf8mb4_0900_ai_ci`.
...
As aplicações que utilizam o banco de dados também devem configurar sua conexão com o servidor a cada vez que se conectam. Isso pode ser feito executando uma declaração `SET NAMES 'latin1'` após a conexão. A declaração pode ser usada independentemente do método de conexão (o cliente **mysql**, scripts PHP, etc.).
...
Para obter mais informações sobre a configuração das conexões do cliente, consulte a Seção 12.4, “Conjunto de caracteres de conexão e codificações”.

Nota

Se você usar `ALTER DATABASE` para alterar o conjunto de caracteres ou a correção padrão do banco de dados, as rotinas armazenadas existentes no banco de dados que utilizam esses padrões devem ser excluídas e recriadas para que utilizem os novos padrões. (Em uma rotina armazenada, as variáveis com tipos de dados de caracteres utilizam os padrões do banco de dados se o conjunto de caracteres ou a correção não forem especificados explicitamente. Veja a Seção 15.1.17, “Instruções CREATE PROCEDURE e CREATE FUNCTION”).
...
Ainda é necessário que as aplicações configurem sua conexão usando `SET NAMES` ou equivalente após se conectarem, conforme descrito anteriormente. Você pode ser tentado a iniciar o servidor com a opção `--init_connect="SET NAMES 'latin1'"`(server-system-variables.html#sysvar_init_connect) para fazer com que `SET NAMES`(set-names.html "15.7.6.3 SET NAMES Statement") seja executado automaticamente para cada cliente que se conecta. No entanto, isso pode resultar em resultados inconsistentes, pois o valor `init_connect` não é executado para usuários que têm o privilégio `CONNECTION_ADMIN` (ou o privilégio descontinuado `SUPER`).
