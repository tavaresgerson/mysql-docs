...
- Todos os metadados devem estar no mesmo conjunto de caracteres. Caso contrário, nem as instruções `SHOW` nem as instruções `SELECT` para tabelas em `INFORMATION_SCHEMA` funcionariam corretamente, porque linhas diferentes na mesma coluna dos resultados dessas operações estariam em diferentes conjuntos de caracteres.
