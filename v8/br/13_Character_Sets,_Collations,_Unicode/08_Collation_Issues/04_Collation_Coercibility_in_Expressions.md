### 12.8.4 Coerção na Cotação de Expressões
...
No entanto, com múltiplos operadores, pode haver ambiguidade. Por exemplo, esta declaração realiza uma comparação entre a coluna `x` e a literal de string `'Y'`:

```
SELECT x FROM T WHERE x = 'Y';
```

Se `x` e `'Y'` tiverem a mesma ordem de comparação, não haverá ambiguidade sobre a ordem de comparação a ser usada. Mas se tiverem ordens de comparação diferentes, a comparação deve usar a ordem de comparação de `x` ou de `'Y'`? Ambos `x` e `'Y'` têm ordens de comparação, então qual ordem de comparação tem precedência?
...
- Uma cláusula explícita `COLLATE` tem uma coercibilidade de 0 (não é coercitiva).
...
- Se ambos os lados tiverem a mesma coercibilidade, então:
  ...
  - Se um dos lados tiver um conjunto de caracteres Unicode e o outro tiver um conjunto de caracteres não Unicode, o lado com o conjunto de caracteres Unicode vence, e a conversão automática de conjunto de caracteres é aplicada ao lado não Unicode. Por exemplo, a seguinte declaração não retorna um erro:
    ...
    Ele retorna um resultado que tem um conjunto de caracteres de `utf8mb4` e a mesma ordem de classificação que `utf8mb4_column`. Os valores de `latin1_column` são convertidos automaticamente para `utf8mb4` antes da concatenação.
