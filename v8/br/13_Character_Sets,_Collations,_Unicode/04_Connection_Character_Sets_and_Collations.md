...
- As variáveis de sistema `character_set_server` e `collation_server` indicam o conjunto de caracteres e a ordenação do servidor. Consulte a Seção 12.3.2, “Conjunto de caracteres e ordenação do servidor”.

- As variáveis de sistema `character_set_database` e `collation_database` indicam o conjunto de caracteres e a ordenação do banco de dados padrão. Consulte a Seção 12.3.3, “Conjunto de caracteres e ordenação do banco de dados”.
...
- Que conjunto de caracteres o servidor deve traduzir as declarações após recebê-las?
  ...
  - O servidor converte as declarações enviadas pelo cliente de `character_set_client` para `character_set_connection`. Exceção: Para as cadeias de caracteres que têm um introducer, como `_utf8mb4` ou `_latin2`, o introducer determina o conjunto de caracteres. Veja a Seção 12.3.8, “Introdutores de Conjunto de Caracteres”.

  - `collation_connection` é importante para comparações de strings literais. Para comparações de strings com valores de coluna, `collation_connection` não importa, pois as colunas têm sua própria collation, que tem precedência de collation mais alta (veja a Seção 12.8.4, “Coercibilidade de collation em expressões”).
...
- Qual conjunto de caracteres o servidor deve usar para traduzir os resultados da consulta antes de enviá-los de volta ao cliente?
  ...
  Para obter mais informações sobre conjuntos de caracteres e mensagens de erro, consulte a Seção 12.6, “Conjunto de caracteres de mensagens de erro”.
...
- A opção de comando `--default-character-set=charset_name` é usada por programas clientes do MySQL, como **mysql** e **mysqladmin**.
...
- Cada cliente pode autodetectar qual conjunto de caracteres usar com base na configuração do sistema operacional, como o valor da variável de ambiente `LANG` ou `LC_ALL` no Unix ou a configuração da página de código no sistema Windows. Para sistemas nos quais o local está disponível a partir do sistema operacional, o cliente usa-o para definir o conjunto de caracteres padrão em vez de usar o padrão embutido. Por exemplo, definir `LANG` para `ru_RU.KOI8-R` faz com que o conjunto de caracteres `koi8r` seja usado. Assim, os usuários podem configurar o local em seu ambiente para uso pelos clientes MySQL.

  O conjunto de caracteres do OS é mapeado para o conjunto de caracteres MySQL mais próximo, se não houver correspondência exata. Se o cliente não suportar o conjunto de caracteres de correspondência, ele usa o padrão predefinido compilado. Por exemplo, `utf8` e `utf-8` são mapeados para `utf8mb4`, e `ucs2` não é suportado como conjunto de caracteres de conexão, então ele é mapeado para o padrão predefinido compilado.

  As aplicações C podem usar a autodetecção de conjuntos de caracteres com base no ajuste do sistema operacional, invocando `mysql_options()` da seguinte forma antes de se conectar ao servidor:
...
Ao configurar programas cliente, você também deve considerar o ambiente em que eles serão executados. Consulte a Seção 12.5, “Configurando o Conjunto de Caracteres e a Codificação da Aplicação”.

### Instruções SQL para configuração do conjunto de caracteres de conexão

Após a conexão ser estabelecida, os clientes podem alterar as variáveis de conjunto de caracteres e sistema de ordenação para a sessão atual. Essas variáveis podem ser alteradas individualmente usando as instruções `SET`, mas duas instruções mais convenientes afetam as variáveis de sistema de conjunto de caracteres relacionadas à conexão como um grupo:
...
- `SET CHARACTER SET 'charset_name`'
  ...
  Definir `collation_connection` também define implicitamente `character_set_connection` para o conjunto de caracteres associado à colagem (equivalente a executar `SET character_set_connection = @@character_set_database`). Não é necessário definir explicitamente `character_set_connection`.
...
Exemplo: Suponha que `column1` seja definido como `CHAR(5) CHARACTER SET latin2`. Se você não disser `SET NAMES` ou `SET CHARACTER SET`, então para `SELECT column1 FROM t`, o servidor envia de volta todos os valores para `column1` usando o conjunto de caracteres especificado pelo cliente quando ele se conectou. Por outro lado, se você disser `SET NAMES 'latin1'` ou `SET CHARACTER SET 'latin1'` antes de emitir a declaração `SELECT`, o servidor converte os valores de `latin2` para `latin1` logo antes de enviar os resultados de volta. A conversão pode ser perda de dados para caracteres que não estão em ambos os conjuntos de caracteres.
...
- A collation padrão para `utf8mb4` difere entre MySQL 5.7 e 8.0 (`utf8mb4_general_ci` para 5.7, `utf8mb4_0900_ai_ci` para 8.0).
...
Nesse caso, o cliente ainda pode usar a collation `utf8mb4` ao emitir uma declaração `SET NAMES 'utf8mb4'` após a conexão. A collation resultante é a collation padrão `utf8mb4` 5.7; ou seja, `utf8mb4_general_ci`. Se o cliente também quiser uma collation de `utf8mb4_0900_ai_ci`, não conseguirá alcançá-la porque o servidor não reconhece essa collation. O cliente deve estar disposto a usar uma collation diferente de `utf8mb4`, ou se conectar a um servidor do MySQL 8.0 ou superior.

#### Tratamento de Erros em Tempo de Execução

Dentro de uma conexão estabelecida, o cliente pode solicitar uma alteração do conjunto de caracteres e da ordenação da conexão com `SET NAMES` ou `SET CHARACTER SET`.
