...
Por exemplo, “SHIFT JIS”, usado em ambientes Windows japoneses, é uma extensão da Microsoft de `Shift_JIS` e seu nome exato é `Microsoft Windows Codepage : 932` ou `cp932`. Além dos caracteres suportados por `Shift_JIS`, `cp932` suporta caracteres de extensão, como caracteres especiais NEC, caracteres selecionados da NEC, caracteres estendidos da IBM e caracteres selecionados da IBM.
...
O conjunto de caracteres `cp932` difere do conjunto de caracteres `sjis` das seguintes maneiras:

- O `cp932` suporta caracteres especiais NEC, caracteres selecionados NEC e caracteres extensos IBM e selecionados IBM.
...
- Alguns caracteres `cp932` têm dois pontos de código diferentes, ambos convertidos para o mesmo ponto de código Unicode. Ao converter do Unicode de volta para `cp932`, um dos pontos de código deve ser selecionado. Para essa “conversão de ida e volta”, a regra recomendada pela Microsoft é usada. (Veja <http://support.microsoft.com/kb/170559/EN-US/>).
  ...
  A tabela mostrada em <https://msdn.microsoft.com/en-us/goglobal/cc305152.aspx> fornece informações sobre os valores Unicode dos caracteres `cp932`. Para as entradas da tabela `cp932` com caracteres sob os quais aparece um número de quatro dígitos, o número representa a codificação Unicode correspondente (`ucs2`). Para entradas da tabela com um valor de dois dígitos sublinhado, há uma gama de valores de caracteres `cp932` que começam com esses dois dígitos. Ao clicar em uma entrada da tabela, você é direcionado para uma página que exibe o valor Unicode para cada um dos caracteres `cp932` que começam com esses dígitos.
  ...
  - NEC selecionada — caracteres estendidos da IBM (byte principal `0xED` e `0xEE`):
...
- `cp932` suporta a conversão de caracteres definidos pelo usuário em combinação com `eucjpms`, e resolve os problemas com a conversão de `sjis`/`ujis`. Para obter detalhes, consulte <http://www.sljfaq.org/afaq/encodings.html>.
