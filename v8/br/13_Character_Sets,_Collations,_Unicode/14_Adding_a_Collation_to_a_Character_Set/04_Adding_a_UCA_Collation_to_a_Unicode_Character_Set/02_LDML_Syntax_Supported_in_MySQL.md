...
Esta seção descreve a sintaxe LDML que o MySQL reconhece. Esta é uma subseção da sintaxe descrita na especificação LDML disponível em <http://www.unicode.org/reports/tr35/>, que deve ser consultada para obter mais informações. O MySQL reconhece um subconjunto suficientemente grande da sintaxe, de modo que, em muitos casos, é possível baixar uma definição de ordenação do Repositório Comum de Dados de Localização do Unicode e colar a parte relevante (ou seja, a parte entre as tags `<rules>` e `</rules>`) no arquivo `Index.xml` do MySQL. Todas as regras descritas aqui são suportadas, exceto que a ordenação de caracteres ocorre apenas no nível primário. Regras que especificam diferenças em níveis de ordenação secundários ou superiores são reconhecidas (e, portanto, podem ser incluídas em definições de ordenação) mas são tratadas como igualdade no nível primário.

O servidor MySQL gera diagnósticos quando encontra problemas ao analisar o arquivo `Index.xml`. Consulte a Seção 12.14.4.3, “Diagnósticos durante a análise do Index.xml”.

**Representação de Personagens**

Os caracteres nomeados nas regras LDML podem ser escritos literalmente ou no formato `\unnnn`, onde `nnnn` é o valor hexadecimal do ponto de código Unicode. Por exemplo, `A` e `á` podem ser escritos literalmente ou como `\u0041` e `\u00E1`. Dentro dos valores hexadecimais, os dígitos `A` a `F` não são sensíveis ao caso; `\u00E1` e `\u00e1` são equivalentes. Para as colorações UCA 4.0.0, a notação hexadecimal só pode ser usada para caracteres na Planilha Multilíngue Básica, não para caracteres fora da faixa do BMP de `0000` a `FFFF`. Para as colorações UCA 5.2.0, a notação hexadecimal pode ser usada para qualquer caractere.
...
- Uma regra `<reset>` não especifica qualquer ordem por si só. Em vez disso, ela “redefine” a ordem para as regras de deslocamento subsequentes, fazendo com que elas sejam aplicadas em relação a um caractere específico. Qualquer uma das seguintes regras redefine as regras de deslocamento subsequentes para serem aplicadas em relação à letra `'A'`:
...
- As regras de deslocamento `<p>`, `<s>` e `<t>` definem as diferenças primária, secundária e terciária de um caractere em relação a outro:
...
- Uma contração é uma regra de deslocamento que ordena uma sequência de vários caracteres. O MySQL suporta contrações de 2 a 6 caracteres. As seguintes regras colocam a sequência de três caracteres `'xyz'` no nível primário acima de `'a'`:
...
- As expansões longas e as contrações longas podem ser usadas juntas. Essas regras colocam a sequência de três caracteres `'xyz'` no nível primário maior do que a sequência de três caracteres `'abc'`:
...
- A sintaxe de expansão normal usa elementos `<x>` mais `<extend>` para especificar uma expansão. As seguintes regras colocam o caractere `'k'` no nível secundário acima da sequência `'ch'`. Isso significa que `'k'` se comporta como se expandisse para um caractere após `'c'` seguido por `'h'`:
  ...
  Essa sintaxe permite sequências longas. Essas regras classificam a sequência `'ccs'` em nível terciário como sendo maior do que a sequência `'cscs'`:
...
- O contexto anterior usa elementos `<x>` mais `<context>` para especificar que o contexto antes de um caractere afeta como ele é classificado. As seguintes regras colocam `'-'` em um nível secundário maior que `'a'`, mas apenas quando `'-'` ocorre após `'b'`:
...
- Uma regra de reinicialização pode nomear uma posição de reinicialização lógica em vez de um caractere literal:
  ...
  Essas regras atribuem maior `'z'` ao nível primário em relação a caracteres não ignoráveis que têm uma entrada na Tabela de Elementos de Cotação Unicode Padrão (DUCET) e que não são CJK:
  ...
  **Tabela 12.6. Pontos de código de posição de reinicialização lógica**
...
- O elemento `<collation>` permite um atributo `shift-after-method` que afeta o cálculo do peso dos caracteres para regras de deslocamento. O atributo tem esses valores permitidos:
  ...
  - `simple`: Calcule os pesos dos caracteres como para regras de redefinição que não têm o atributo `before`. Isso é o padrão se o atributo não for fornecido.
...
Uma extensão das regras do LDML permite que o elemento `<collation>` inclua um atributo opcional `version` nas tags `<collation>` para indicar a versão da UCA em que a ordenação é baseada. Se o atributo `version` for omitido, seu valor padrão é `4.0.0`. Por exemplo, esta especificação indica uma ordenação baseada na UCA 5.2.0:
