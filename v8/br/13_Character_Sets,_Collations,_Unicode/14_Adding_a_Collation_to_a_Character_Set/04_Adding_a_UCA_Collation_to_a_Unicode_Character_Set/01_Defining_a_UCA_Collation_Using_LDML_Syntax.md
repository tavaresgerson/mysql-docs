...
Para adicionar uma ordenação UCA para um conjunto de caracteres Unicode sem recompilar o MySQL, use o procedimento a seguir. Se você não estiver familiarizado com as regras LDML usadas para descrever as características de ordenação da ordenação, consulte a Seção 12.14.4.2, “Sintaxe LDML suportada no MySQL”.

O exemplo adiciona uma ordenação chamada `utf8mb4_phone_ci` ao conjunto de caracteres `utf8mb4`. A ordenação é projetada para um cenário envolvendo uma aplicação web para a qual os usuários postam seus nomes e números de telefone. Os números de telefone podem ser fornecidos em formatos muito diferentes:
...
1. Escolha um ID de collation, conforme mostrado na Seção 12.14.2, “Escolhendo um ID de collation”. Os passos seguintes usam um ID de 1029.
...
3. Escolha um nome para a ordenação e liste-o no arquivo `Index.xml`. Além disso, você deve fornecer as regras de ordenação da colagem. Encontre o elemento `<charset>` para o conjunto de caracteres ao qual a ordenação está sendo adicionada e adicione um elemento `<collation>` que indique o nome e o ID da ordenação, para associar o nome ao ID. Dentro do elemento `<collation>`, forneça um elemento `<rules>` contendo as regras de ordenação:
