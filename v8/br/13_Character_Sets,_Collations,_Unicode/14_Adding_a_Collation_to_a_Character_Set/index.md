## 12.14 Adicionando uma Codificação a um Conjunto de Caracteres

12.14.1 Tipos de implementação de colagem

12.14.2 Escolher um ID de collation

12.14.3 Adicionando uma Colagem Simples a um Conjunto de Caracteres de 8 Bits

12.14.4 Adicionando uma Codificação UCA a um Conjunto de Caracteres Unicode

Aviso

As colatações definidas pelo usuário estão desatualizadas; você deve esperar que o suporte a elas seja removido em uma versão futura do MySQL. A partir do MySQL 8.0.33, o servidor emite um aviso para qualquer uso de `COLLATE user_defined_collation` em uma instrução SQL; um aviso também é emitido quando o servidor é iniciado com `--collation-server` definido igual ao nome de uma colatação definida pelo usuário.
...
A redefinição de colunas incorporadas não é suportada e pode resultar em comportamento inesperado do servidor.
