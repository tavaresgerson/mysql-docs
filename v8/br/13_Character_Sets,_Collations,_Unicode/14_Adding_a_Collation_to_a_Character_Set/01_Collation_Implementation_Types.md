### 12.14.1 Tipos de implementação de cotações

O MySQL implementa vários tipos de colatações:

**Colagens simples para conjuntos de caracteres de 8 bits**

Esse tipo de ordenação é implementado usando um array de 256 pesos que define uma correspondência um-para-um entre códigos de caracteres e pesos. `latin1_swedish_ci` é um exemplo. É uma ordenação que não distingue maiúsculas de minúsculas, portanto, as versões maiúsculas e minúsculas de um caractere têm os mesmos pesos e são consideradas iguais.
...
Esse tipo de ordenação é implementado usando funções em um arquivo de código-fonte em C que definem como ordenar os caracteres, conforme descrito na Seção 12.13, “Adicionando um Conjunto de Caracteres”.

**Colagens para conjuntos de caracteres multibyte não Unicode**

Para este tipo de ordenação, os caracteres de 8 bits (um único byte) e os caracteres multibyte são tratados de maneira diferente. Para caracteres de 8 bits, os códigos de caracteres correspondem a pesos de forma insensível a maiúsculas e minúsculas. (Por exemplo, os caracteres de um único byte `'a'` e `'A'` têm o mesmo peso `0x41`.) Para caracteres multibyte, existem dois tipos de relação entre os códigos de caracteres e os pesos:

- Os pesos equivalem aos códigos de caracteres. `sjis_japanese_ci` é um exemplo desse tipo de ordenação. O caractere multibyte `'ぢ'` tem um código de caractere de `0x82C0`, e o peso também é `0x82C0`.
...
As collation não-UCA têm uma correspondência um-para-um entre o código de caracteres e o peso. No MySQL, essas collation são insensíveis ao caso e ao acento. `utf8mb4_general_ci` é um exemplo: `'a'`, `'A'`, `'À'` e `'á'` têm códigos de caracteres diferentes, mas todos têm um peso de `0x0041` e são comparados como iguais.
...
- Um caractere pode ter vários pesos. Isso é uma expansão. Exemplo: A letra alemã `'ß'` (ligatura SZ, ou SHARP S) tem um peso de `0x0FEA0FEA`.
...
Para obter instruções de implementação, para uma ordenação que não seja da UCA, consulte a Seção 12.13, “Adicionando um Conjunto de Caracteres”. Para uma ordenação da UCA, consulte a Seção 12.14.4, “Adicionando uma Ordenação da UCA a um Conjunto de Caracteres Unicode”.
