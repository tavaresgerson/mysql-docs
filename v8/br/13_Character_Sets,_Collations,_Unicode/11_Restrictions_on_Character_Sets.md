...
- Os identificadores são armazenados nas tabelas de banco de dados `mysql` (`user`, `db` e assim por diante) usando `utf8mb3`, mas os identificadores podem conter apenas caracteres no Plano Multilíngue Básico (BMP). Caracteres suplementares não são permitidos em identificadores.
...
- Os conjuntos de caracteres `ucs2`, `utf16`, `utf16le` e `utf32` têm as seguintes restrições:
  ...
  - Os índices `FULLTEXT` não podem ser criados em uma coluna que utilize qualquer um desses conjuntos de caracteres. No entanto, você pode realizar pesquisas `IN BOOLEAN MODE` na coluna sem um índice.
