#### 29.12.20.7 Tabelas de Resumo de Acesso e Gravação de Arquivos
...
- A coluna `file_summary_by_event_name` tem uma coluna `EVENT_NAME`. Cada linha resume os eventos para um nome de evento específico.

- A tabela `file_summary_by_instance` possui as colunas `FILE_NAME`, `EVENT_NAME` e `OBJECT_INSTANCE_BEGIN`. Cada linha resume os eventos para um arquivo e um nome de evento específicos.
...
As tabelas de resumo de I/O de arquivo têm esses índices:
...
`TRUNCATE TABLE` é permitido para tabelas de resumo de E/S de arquivos. Ele redefiniu as colunas de resumo para zero em vez de remover linhas.
