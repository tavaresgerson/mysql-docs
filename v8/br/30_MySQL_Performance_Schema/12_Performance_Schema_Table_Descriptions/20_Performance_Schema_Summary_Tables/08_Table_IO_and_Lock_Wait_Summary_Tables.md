#### 29.12.20.8 Tabelas de Resumo de Espera de Entrada/Saída e Bloqueio
...
- `table_lock_waits_summary_by_table`: Espera por bloqueio de tabela por tabela

##### 29.12.20.8.1 A tabela table\_io\_waits\_summary\_by\_table

A tabela `table_io_waits_summary_by_table` agrega todos os eventos de espera de I/O de tabela, conforme gerados pelo instrumento `wait/io/table/sql/handler`. O agrupamento é por tabela.
...
`TRUNCATE TABLE` é permitido para tabelas de resumo de I/O. Ele redefiniu as colunas de resumo para zero em vez de remover linhas. O truncamento desta tabela também trunca a tabela `table_io_waits_summary_by_index_usage`.

##### 29.12.20.8.2 A tabela table\_io\_waits\_summary\_by\_index\_usage
...
`TRUNCATE TABLE` é permitido para tabelas de resumo de I/O. Ele redefiniu as colunas de resumo para zero em vez de remover linhas. Esta tabela também é truncada pela truncagem da tabela `table_io_waits_summary_by_table`. Uma operação DDL que altera a estrutura do índice de uma tabela pode fazer com que as estatísticas por índice sejam redefinidas.

##### 29.12.20.8.3 A tabela table\_lock\_waits\_summary\_by\_table
...
`TRUNCATE TABLE` é permitido para tabelas de resumo de bloqueio de tabelas. Ele redefini o número de colunas de resumo para zero, em vez de remover linhas.
