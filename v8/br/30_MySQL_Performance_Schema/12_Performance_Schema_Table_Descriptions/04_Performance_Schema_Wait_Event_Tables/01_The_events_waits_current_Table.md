...
A tabela `events_waits_current` contém eventos de espera atuais. A tabela armazena uma linha por thread, mostrando o status atual do evento de espera mais recente monitorado da thread, portanto, não há uma variável de sistema para configurar o tamanho da tabela.
...
- `INDEX_NAME`
  ...
  O nome do índice utilizado. `PRIMARY` indica o índice primário da tabela. `NULL` significa que nenhum índice foi utilizado.
...
- `NUMBER_OF_BYTES`
  ...
  O MySQL executa junções usando uma implementação de loop aninhado. O trabalho do instrumento do Schema de Desempenho é fornecer o número de linhas e o tempo de execução acumulado por tabela na junção. Suponha uma consulta de junção da seguinte forma, que é executada usando uma ordem de junção de tabelas de `t1`, `t2`, `t3`:
