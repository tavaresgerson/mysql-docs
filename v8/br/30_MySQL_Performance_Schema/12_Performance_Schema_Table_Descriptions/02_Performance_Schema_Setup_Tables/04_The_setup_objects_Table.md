...
Quando o Schema de Desempenho verifica uma correspondência em `setup_objects`, ele tenta encontrar correspondências mais específicas primeiro. Por exemplo, com uma tabela `db1.t1`, ele procura uma correspondência para `'db1'` e `'t1'`, depois para `'db1'` e `'%'`, depois para `'%'` e `'%'`. A ordem em que a correspondência ocorre importa porque diferentes linhas de correspondência `setup_objects` podem ter diferentes valores de `ENABLED` e `TIMED`.
...
Para obter mais informações sobre o papel da tabela `setup_objects` no filtro de eventos, consulte a Seção 29.4.3, “Pré-filtro de eventos”.
...
- `OBJECT_TYPE`
  ...
  O filtro `TABLE` afeta os eventos de entrada/saída da tabela (instrumento `wait/io/table/sql/handler`) e os eventos de bloqueio da tabela (instrumento `wait/lock/table/sql/handler`).
...
- `TIMED`
  ...
  Se os eventos para o objeto estão cronometrados. Esta coluna pode ser modificada.
