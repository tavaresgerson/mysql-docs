...
- `WRITE_LOCKED_BY_THREAD_ID`
  ...
  Quando um fio atualmente tem um `rwlock` bloqueado no modo exclusivo (escrita), `WRITE_LOCKED_BY_THREAD_ID` é o `THREAD_ID` do fio de bloqueio, caso contrário, é `NULL`.
...
- Índice sobre (`NAME`)
- Índice sobre (`WRITE_LOCKED_BY_THREAD_ID`)
...
- `events_waits_current`, para ver o que `rwlock` uma thread está esperando
