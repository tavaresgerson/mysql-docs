...
- Para configurar o pré-filtro na fase de produção, várias tabelas podem ser usadas:
  ...
  - `setup_instruments` indica quais instrumentos estão disponíveis. Um instrumento desativado nesta tabela não produz eventos, independentemente do conteúdo das outras tabelas de configuração relacionadas à produção. Um instrumento ativado nesta tabela é permitido produzir eventos, sujeito ao conteúdo das outras tabelas.
...
- Para configurar o pré-filtro na etapa do consumidor, modifique a tabela `setup_consumers`. Isso determina os destinos para os quais os eventos são enviados. `setup_consumers` também afeta implicitamente a produção de eventos. Se um evento específico não for enviado para nenhum destino (ou seja, nunca for consumido), o Schema de Desempenho não o produz.

As modificações em qualquer uma dessas tabelas afetam o monitoramento imediatamente, com exceção de que as modificações na tabela `setup_actors` afetam apenas os threads em primeiro plano criados após a modificação, e não os threads existentes.
