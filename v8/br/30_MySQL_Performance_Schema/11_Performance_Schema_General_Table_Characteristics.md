...
- Para recuperar de tabelas `performance_schema`, você deve ter o privilégio `SELECT`.
...
Como apenas um conjunto limitado de privilégios se aplica às tabelas do Schema de Desempenho, as tentativas de usar `GRANT ALL` como abreviação para conceder privilégios no nível do banco de dados ou da tabela falham com um erro:
