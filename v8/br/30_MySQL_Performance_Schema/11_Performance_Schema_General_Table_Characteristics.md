## 29.11 Características Gerais da Tabela do Schema de Desempenho
...
As tabelas resumidas podem ser truncadas com `TRUNCATE TABLE`(truncate-table.html "15.1.37 TRUNCATE TABLE Statement"). Geralmente, o efeito é redefinir as colunas resumidas para 0 ou `NULL`, e não remover linhas. Isso permite que você limpe os valores coletados e reinicie a agregação. Isso pode ser útil, por exemplo, depois de ter feito uma alteração na configuração em tempo real. Exceções a esse comportamento de truncação são mencionadas nas seções individuais das tabelas resumidas.

Os privilégios são iguais aos de outras bases de dados e tabelas:

* Para recuperar das tabelas `performance_schema`, você deve ter o privilégio `SELECT`.
...
Como apenas um conjunto limitado de privilégios se aplica a tabelas do Performance Schema, as tentativas de usar `GRANT ALL` como abreviação para conceder privilégios no nível do banco de dados ou da tabela falham com um erro:
