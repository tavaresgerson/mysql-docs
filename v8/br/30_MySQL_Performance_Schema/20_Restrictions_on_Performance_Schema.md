...
O Schema de Desempenho evita o uso de mútues para coletar ou produzir dados, portanto, não há garantias de consistência e os resultados podem ser incorretos às vezes. Os valores de evento nas tabelas `performance_schema` são não determinísticos e não repetitivos.

Se você salvar as informações dos eventos em outra tabela, não deve assumir que os eventos originais permanecem disponíveis posteriormente. Por exemplo, se você selecionar eventos de uma tabela `performance_schema` para uma tabela temporária, com a intenção de juntá-la à tabela original posteriormente, pode não haver correspondências.
