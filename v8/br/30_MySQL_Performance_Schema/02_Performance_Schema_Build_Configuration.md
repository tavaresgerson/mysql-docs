...
Como o Schema de Desempenho é configurado no servidor durante a construção, uma linha para `PERFORMANCE_SCHEMA` aparece na saída do `SHOW ENGINES`. Isso significa que o Schema de Desempenho está disponível, não que ele está ativado. Para ativá-lo, você deve fazê-lo durante o início do servidor, conforme descrito na próxima seção.
