...
O método `createIndex()` cria um índice que você pode definir com um documento JSON que especifica quais campos usar. Esta seção é uma visão geral de alto nível do indexação. Para mais informações, consulte Coleções de indexação.

##### Adicionar um índice não único

Para criar um índice não único, passe o nome do índice e as informações do índice ao método `createIndex()`. Nomes de índices duplicados são proibidos.
...
Para excluir um índice, passe o nome do índice que deseja excluir para o método `dropIndex()`. Por exemplo, você pode excluir o índice “popul” da seguinte forma:
