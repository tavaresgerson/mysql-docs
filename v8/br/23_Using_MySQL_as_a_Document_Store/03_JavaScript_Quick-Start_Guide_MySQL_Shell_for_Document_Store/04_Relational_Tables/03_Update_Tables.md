...
Para substituir o nome de uma cidade na tabela de cidades, passe o novo nome da cidade ao método `set()`. Em seguida, passe o nome da cidade a ser localizada e substituída ao método `where()`. O exemplo a seguir substitui a cidade Peking por Beijing.

```
mysql-js> db.city.update().set("Name", "Beijing").where("Name = 'Peking'")
```

Use o método `select()` para verificar a alteração.
...
- Consulte TableUpdateFunction para obter a definição completa da sintaxe.
