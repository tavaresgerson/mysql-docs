...
Para especificar uma ordem dos resultados, adicione o método `orderBy()` ao método `select()`. Passe ao método `orderBy()` uma lista de uma ou mais colunas para ordenar e, opcionalmente, o atributo descendente (`desc`) ou ascendente (`asc`) conforme apropriado. A ordem ascendente é o tipo de ordem padrão.
