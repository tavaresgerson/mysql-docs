...
Esta seção descreve como atualizar instalações binárias e baseadas em pacotes do MySQL no Unix/Linux.
...
Uma atualização no local envolve o fechamento do antigo servidor MySQL, a substituição dos antigos binários ou pacotes MySQL pelos novos, o reinicio do MySQL no diretório de dados existente e a atualização de quaisquer partes restantes da instalação existente que necessitem de atualização.
...
Para realizar uma atualização no local:

1. Revise as informações na Secção 3. 1, " Antes de Começar " .
2. Assegure-se de que a sua instalação está pronta para a actualização, completando as verificações preliminares na secção 3.6, "Preparar a sua instalação para a actualização".
3. Se você usar transações XA com `InnoDB`, execute `XA RECOVER` antes da atualização para verificar se há transações XA não comprometidas. Se os resultados forem retornados, comite ou reverta as transações XA emitindo uma instrução `XA COMMIT` ou `XA ROLLBACK`.
...
5. Desligue o antigo servidor MySQL. Por exemplo:

   ```
   mysqladmin -u root -p shutdown
   ```
6. Atualize os binários ou pacotes do MySQL. Se atualizar uma instalação binária, desempaquete o novo pacote de distribuição binária do MySQL. Consulte Obter e Desempaquetar a Distribuição. Para instalações baseadas em pacotes, instale os novos pacotes.
7. Inicie o servidor MySQL 8.4, usando o diretório de dados existente. Por exemplo:
   ...
   Se houver tablespaces criptografados `InnoDB`, use a opção `--early-plugin-load` para carregar o plugin de chaveiro.
...
Uma atualização lógica envolve a exportação do SQL da antiga instância do MySQL usando um utilitário de backup ou exportação como `mysqldump`, instalar o novo servidor MySQL e aplicar o SQL à sua nova instância do MySQL.
...
Para identificar incompatibilidades antes de atualizar para a versão mais recente do MySQL 8.4, execute as etapas descritas na Seção 3.6, "Preparando sua instalação para atualização".

Para executar uma atualização lógica:
...
2. Exportar os dados existentes da instalação anterior do MySQL:
   ...
   Se você tem tabelas que contêm colunas geradas, use o utilitário `mysqldump` fornecido com o MySQL 5.7.9 ou superior para criar seus arquivos de despejo. O utilitário `mysqldump` fornecido em versões anteriores usa sintaxe incorreta para definições de colunas geradas (Bug # 20769542).

3. Desligue o antigo servidor MySQL. Por exemplo:
...
8. Carregar o arquivo de despejo criado anteriormente no novo servidor MySQL. Por exemplo:
   ...
   Não é recomendado carregar um arquivo de despejo quando os GTIDs estiverem habilitados no servidor (`gtid_mode=ON`), se o seu arquivo de despejo incluir tabelas de sistema. `mysqldump` emite instruções DML para as tabelas de sistema que usam o motor de armazenamento MyISAM não transacional, e esta combinação não é permitida quando os GTIDs estiverem habilitados.

   :::

9. Executar quaisquer operações de actualização restantes:
   ...
   Ao reiniciar com `--upgrade=FORCE`, o servidor faz todas as alterações necessárias no esquema do sistema `mysql` entre o MySQL 8.3 e o MySQL 8.4, para que você possa tirar proveito de novos privilégios ou recursos. Ele também atualiza o esquema de desempenho, `INFORMATION_SCHEMA` e `sys` para o MySQL 8.4, e examina todos os esquemas de usuário para incompatibilidades com a versão atual do MySQL.
...
As informações nesta seção são um adjunto ao procedimento de atualização no local descrito em In-Place Upgrade, para uso se você estiver atualizando o MySQL Cluster.
