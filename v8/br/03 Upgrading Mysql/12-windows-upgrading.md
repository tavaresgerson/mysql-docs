...
Baixe e execute o MSI mais recente. Embora a atualização entre as séries de versões não seja diretamente suportada, a opção "Custom Setup" permite definir um local de instalação, pois caso contrário, o MSI é instalado no local padrão, como `C:\Program Files\MySQL\MySQL Server 8.4\`.

Execute o configurador do MySQL para configurar a sua instalação.
...
2. Se o servidor estiver em execução, pare-o. Se o servidor estiver instalado como um serviço, pare o serviço com o seguinte comando do prompt de comandos:
   ...
   Em alternativa, use **NET STOP `mysqld_service_name`** .
...
4. Por exemplo, use o comando **SC START `mysqld_service_name`** ou **NET START `mysqld_service_name`** se você executar o MySQL como um serviço, ou invoque `mysqld` diretamente de outra forma.
