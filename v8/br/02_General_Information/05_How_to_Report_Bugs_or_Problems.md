...
* Se o **mysqld** morrer, você também deve relatar a declaração que fez o **mysqld** sair inesperadamente. Geralmente, você pode obter essas informações executando o **mysqld** com o registro de consulta habilitado e, em seguida, olhando o log após o **mysqld** sair. Veja a Seção 7.9, “Depuração do MySQL”.
...
* O modo SQL em vigor quando o problema ocorreu pode ser significativo, portanto, informe o valor da variável de sistema `sql_mode`. Para procedimentos armazenados, funções armazenadas e objetos de gatilho, o valor relevante `sql_mode` é o em vigor quando o objeto foi criado. Para um procedimento ou função armazenada, a declaração [`SHOW CREATE PROCEDURE`](show-create-procedure.html "15.7.7.9 SHOW CREATE PROCEDURE Statement") ou [`SHOW CREATE FUNCTION`](show-create-function.html "15.7.7.8 SHOW CREATE FUNCTION Statement") mostra o modo SQL relevante, ou você pode consultar `INFORMATION_SCHEMA` para obter as informações:
...
* Se um erro ou problema ocorrer durante a execução do **mysqld**, tente fornecer um script de entrada que reproduza a anomalia. O script deve incluir todos os arquivos de origem necessários. Quanto mais próximo o script puder reproduzir sua situação, melhor. Se você puder criar um caso de teste reproduzível, você deve enviá-lo para anexá-lo ao relatório do bug.

Se você não puder fornecer um script, deve, pelo menos, incluir a saída do [**mysqladmin variables extended-status processlist**][(mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program")] em seu relatório para fornecer alguma informação sobre o desempenho do seu sistema.
...
+ O mecanismo de recuperação de falha `InnoDB` lida com a limpeza quando o servidor é reiniciado após ser interrompido, portanto, na operação típica, não é necessário "reparar" as tabelas. Se você encontrar um erro com as tabelas `InnoDB`, reinicie o servidor e veja se o problema persiste, ou se o erro afetou apenas os dados armazenados em cache na memória. Se os dados estiverem corrompidos no disco, considere reiniciar com a opção `innodb_force_recovery` habilitada para que você possa descartar as tabelas afetadas.

+ Para tabelas não transacionais, tente repará-las com `REPAIR TABLE` ou com **myisamchk**. Veja o Capítulo 7, *Administração do MySQL Server*.
...
* Se você frequentemente recebe tabelas corrompidas, você deve tentar descobrir quando e por que isso acontece. Neste caso, o log de erro no diretório de dados do MySQL pode conter algumas informações sobre o que aconteceu. (Este é o arquivo com o sufixo `.err` no nome.) Veja a Seção 7.4.2, “O Log de Erro”. Inclua todas as informações relevantes deste arquivo em seu relatório de bug. Normalmente, o **mysqld** *nunca* deve corromper uma tabela se nada a matar no meio de uma atualização. Se você puder encontrar a causa da morte do **mysqld**, é muito mais fácil para nós fornecer uma solução para o problema. Veja a Seção B.3.1, “Como Determinar o que Está Causing um Problema”.
