...
<table><col style="width: 10%"/><col style="width: 90%"/><thead><tr> <th>Valor da Bandeira</th> <th>Significado</th> </tr></thead><tbody><tr> <td>0</td> <td>Sem opções. Este é o padrão se <em><code>options</code></em> não for especificado.</td> </tr><tr> <td>1</td> <td>Adicionar uma caixa de seleção à saída.</td> </tr><tr> <td>2</td> <td>Adicionar uma URN de CRS de formato curto à saída. O formato padrão é um formato curto (<code>EPSG:<em><code>srid</code></em></code>).</td> </tr><tr> <td>4</td> <td>Adicionar uma URN de CRS de formato longo (<code>urn:ogc:def:crs:EPSG::<em><code>srid</code></em></code>). Esta bandeira substitui a bandeira 2. Por exemplo, os valores de opção 5 e 7 significam a mesma coisa (adicionar uma caixa de seleção e uma URN de CRS de formato longo).</td> </tr></tbody></table>
  ...
  +-------------------------------------------------------------+
  ```
[`ST_GeomFromGeoJSON(str [, options [, srid]])`](https://developer.arcgis.com/en/sdk/arcgis-spatial-geojson-functions.html#function_st-geomfromgeojson)

  Analisa uma string *`str`* que representa um objeto GeoJSON e retorna uma geometria.

  Se qualquer argumento for `NULL`, o valor de retorno é `NULL`. Se qualquer argumento não `NULL` for inválido, ocorre um erro.

  *`options`*, se fornecido, descreve como lidar com documentos GeoJSON que contêm geometrias com dimensões de coordenadas superiores a 2. A tabela a seguir mostra os valores de *`options`* permitidos.

  <table><col style="width: 10%"/><col style="width: 90%"/><thead><tr> <th>Valor da Opção</th> <th>Significado</th> </tr></thead><tbody><tr> <td>1</td> <td>Rejeitar o documento e produzir um erro. Este é o padrão se <em><code>options</code></em> não for especificado.</td> </tr><tr> <td>2, 3, 4</td> <td>Aceitar o documento e remover as coordenadas para dimensões de coordenadas superiores.</td> </tr></tbody></table>

  Os valores de *`options`* 2, 3 e 4 atualmente produzem o mesmo efeito. Se geometrias com dimensões de coordenadas superiores a 2 forem suportadas no futuro, você pode esperar que esses valores produzam efeitos diferentes.
...
Se *`srid`* se refere a um sistema de referência espacial (SRS) indefinido, ocorre um erro `ER_SRS_NOT_FOUND`.
...
Conforme especificado na especificação GeoJSON, a análise é case-sensitive para o membro `type` da entrada GeoJSON (`Point`, `LineString`, e assim por diante). A especificação é silenciosa em relação à sensibilidade de caso para outras análises, o que, no MySQL, não é case-sensitive.
