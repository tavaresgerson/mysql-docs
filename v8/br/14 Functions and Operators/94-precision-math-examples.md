...
Esta seção fornece alguns exemplos que mostram os resultados das consultas de matemática de precisão no MySQL. Esses exemplos demonstram os princípios descritos na Seção 14.24.3, “Tratamento de Expressões”, e na Seção 14.24.4, “Comportamento de Arredondamento”.
...
* O arredondamento para colunas de valor exato ( `DECIMAL` - DECIMAL, NUMERIC") e inteiro) e números de valor exato usa a regra “arredonde para longe de zero”. Um valor com uma parte fracionária de .5 ou maior é arredondado para longe de zero para o inteiro mais próximo, como mostrado aqui:
  ...
  +------------+-------------+
  ```
* O arredondamento para valores de ponto flutuante usa a biblioteca C, que em muitos sistemas usa a regra “arredonde para o inteiro mais próximo”. Um valor com uma parte fracionária exatamente no meio entre dois inteiros é arredondado para o inteiro mais próximo:
...
**Exemplo 4**. No modo estrito, inserir um valor que está fora do intervalo para uma coluna causa um erro, em vez de truncar para um valor legal.
...
No entanto, ocorre um erro se o modo estrito estiver em vigor:
...
No modo não estrito, a divisão por zero tem um resultado de `NULL`:
