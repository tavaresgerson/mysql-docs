...
*  `JSON_QUOTE(string)`
  ...
  Cota uma string como um valor JSON, envolvendo-a com caracteres de aspas duplas e escapando as aspas internas e outros caracteres, e, em seguida, retorna o resultado como uma string `utf8mb4`. Retorna `NULL` se o argumento for `NULL`.
  ...
  Certos caracteres especiais são escapados com barras invertidas de acordo com as sequências de escape mostradas na Tabela 14.23, “Sequências de escape especiais JSON\_UNQUOTE()”.
...
Dois funções agregadas que geram valores JSON estão disponíveis. `JSON_ARRAYAGG()` retorna um conjunto de resultados como um único array JSON, e `JSON_OBJECTAGG()` retorna um conjunto de resultados como um único objeto JSON. Para mais informações, consulte a Seção 14.19, “Funções agregadas”.