...
Para inserções em uma coluna `DECIMAL` - DECIMAL, NUMERIC") ou inteiro, o alvo é um tipo de dados exato, então o arredondamento usa "arredondar para longe de zero", independentemente de o valor a ser inserido ser exato ou aproximado:
...
A declaração `SHOW WARNINGS` exibe as notas geradas por truncação devido ao arredondamento da parte fracionária. Tal truncação não é um erro, mesmo no modo SQL rigoroso (veja a Seção 14.24.3, “Tratamento de Expressões”).