...
Para funções simples que aceitam entrada de string e retornam uma string como resultado, o conjunto de caracteres e a ordenação da saída são os mesmos do valor principal de entrada. Por exemplo, `UPPER(X)` retorna uma string com a mesma sequência de caracteres e ordenação de *`X`*. O mesmo se aplica a `INSTR()`, `LCASE()`, `LOWER()`, `LTRIM()`, `MID()`, `REPEAT()`, `REPLACE()`, `REVERSE()`, `RIGHT()`, `RPAD()`, `RTRIM()`, `SOUNDEX()`, `SUBSTRING()`, `TRIM()`, `UCASE()` e `UPPER()`.

::: info Nota

A função `REPLACE()` , ao contrário de todas as outras funções, sempre ignora a ordenação da entrada de string e realiza uma comparação sensível ao caso.


:::

Se uma entrada de string ou resultado de função for uma string binária, a string tem o conjunto de caracteres `binary` e a ordenação `binary`. Isso pode ser verificado usando as funções `CHARSET()` e `COLLATION()`, ambas que retornam `binary` para um argumento de string binária:
