...
Entenda isso como “*`X`* determina *`Y`* de forma única”, o que também significa que *`Y`* é funcionalmente dependente de *`X`*.
...
* Dependências Funcionais Derivadas de Chaves Múltiplas Colunas e de Igualdades
...
E, devido a `co.Code` ser a chave primária de `co`:

```
{co.Code} -> {co.*}
```

As relações de “determina de forma única” são transitivas, portanto:
...
De fato, todas as linhas complementado com `NULL` feitas para `cl` são colocadas em um único grupo (elas têm ambas as colunas `GROUP BY` iguais a `NULL`), e dentro deste grupo o valor de `co.Name` pode variar. A consulta é inválida e o MySQL a rejeita.

A dependência funcional em junções externas está, portanto, ligada à questão de se as colunas determinantes pertencem ao lado esquerdo ou direito da `LEFT JOIN`. A determinação da dependência funcional torna-se mais complexa se houver junções externas aninhadas ou se a condição de junção não consistir inteiramente em comparações de igualdade.
...
O MySQL é capaz de descobrir uma dependência funcional no resultado de uma visualização e usar isso para validar uma consulta que utiliza a visualização. O mesmo seria verdadeiro se `country2` fosse uma tabela derivada (ou expressão de tabela comum), como:
