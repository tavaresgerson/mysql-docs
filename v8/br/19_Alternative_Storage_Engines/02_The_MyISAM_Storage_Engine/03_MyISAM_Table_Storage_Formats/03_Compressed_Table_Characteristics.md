...
Embora uma tabela compactada seja apenas de leitura, e você não possa, portanto, atualizar ou adicionar linhas na tabela, as operações de DDL (Linguagem de Definição de Dados) ainda são válidas. Por exemplo, você ainda pode usar `DROP` para excluir a tabela e `TRUNCATE TABLE` para esvaziar a tabela.
