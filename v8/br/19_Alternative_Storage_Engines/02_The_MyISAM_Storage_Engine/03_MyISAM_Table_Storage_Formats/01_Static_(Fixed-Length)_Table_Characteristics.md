#### 18.2.3.1 Características de uma tabela estática (com comprimento fixo)
...
A segurança é evidenciada se o seu computador falhar enquanto o servidor MySQL está escrevendo em um arquivo de formato fixo `MyISAM`. Nesse caso, o **myisamchk** pode facilmente determinar onde cada linha começa e termina, permitindo que ele recupere todas as linhas, exceto a parcialmente escrita. Os índices da tabela `MyISAM` sempre podem ser reconstruídos com base nas linhas de dados.

Nota

O formato de linha de comprimento fixo está disponível apenas para tabelas que não possuem colunas `BLOB` ou `TEXT`. Criar uma tabela com essas colunas com uma cláusula explícita `ROW_FORMAT` não gera um erro ou aviso; a especificação do formato é ignorada.

As tabelas em formato estático têm essas características:

- As colunas `CHAR` e `VARCHAR` são preenchidas com espaços até a largura da coluna especificada, embora o tipo da coluna não seja alterado. As colunas `BINARY` e `VARBINARY` são preenchidas com `0x00` bytes até a largura da coluna.

- As colunas `NULL` exigem espaço adicional na linha para registrar se seus valores são `NULL`. Cada coluna `NULL` exige um bit extra, arredondado para o byte mais próximo.
