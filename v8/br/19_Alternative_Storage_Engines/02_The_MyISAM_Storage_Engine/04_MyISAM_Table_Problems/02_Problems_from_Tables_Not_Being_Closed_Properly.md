#### 18.2.4.2 Problemas decorrentes da não fechamento adequado das tabelas
...
- Vários servidores **mysqld** estão usando a tabela e um servidor executou um `REPAIR TABLE` ou `CHECK TABLE` na tabela enquanto estava sendo usado por outro servidor. Nesse cenário, é seguro usar `CHECK TABLE`, embora você possa receber o aviso de outros servidores. No entanto, `REPAIR TABLE` deve ser evitado, pois, quando um servidor substitui o arquivo de dados por um novo, isso não é conhecido pelos outros servidores.
