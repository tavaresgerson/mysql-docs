...
Quando você cria uma tabela `CSV`, o servidor cria um arquivo de dados em texto simples com um nome que começa com o nome da tabela e tem uma extensão `.CSV`. Quando você armazena dados na tabela, o mecanismo de armazenamento os salva no arquivo de dados no formato de valores separados por vírgula.
...
Para reparar uma tabela, use `REPAIR TABLE`(repair-table.html "15.7.3.5 REPAIR TABLE Statement"), que copia quantas linhas válidas possíveis das dados existentes de `CSV` e, em seguida, substitui o arquivo existente de `CSV` com as linhas recuperadas. Quaisquer linhas além dos dados corrompidos são perdidas.
...
O motor de armazenamento `CSV` não suporta particionamento.
