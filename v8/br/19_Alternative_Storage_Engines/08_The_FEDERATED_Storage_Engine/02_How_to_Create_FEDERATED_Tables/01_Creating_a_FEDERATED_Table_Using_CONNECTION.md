...
A string `CONNECTION` contém as informações necessárias para se conectar ao servidor remoto que contém a tabela onde os dados estão fisicamente armazenados. A string de conexão especifica o nome do servidor, as credenciais de login, o número da porta e as informações da base de dados/tabela. No exemplo, a tabela remota está no servidor `remote_host`, usando a porta
...
- `scheme`: Um protocolo de conexão reconhecido. Apenas `mysql` é suportado como o valor `scheme` neste momento.
