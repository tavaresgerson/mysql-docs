...
Os espaços em branco finais são excluídos automaticamente dos valores do membro `SET` na definição da tabela quando uma tabela é criada.

Consulte os requisitos de armazenamento para o tipo de string `SET` para obter os requisitos de armazenamento.

Consulte a Seção 13.3.1, “Sintaxe do Tipo de Dados String”, para a sintaxe e os limites de comprimento do tipo `SET`.
...
<table summary="Valores decimais e binários para membros de uma coluna especificada como SET('a', 'b', 'c', 'd')."><thead><tr> <th scope="col">[[PH_HTML_CODE_<code>'d'</code>] Membro</th> <th scope="col">Valor decimal</th> <th scope="col">Valor Binário</th> </tr></thead><tbody><tr> <th>[[PH_HTML_CODE_<code>'d'</code>]</th> <td>[[PH_HTML_CODE_<code>1000</code>]</td> <td>[[<code>0001</code>]]</td> </tr><tr> <th>[[<code>'b'</code>]]</th> <td>[[<code>2</code>]]</td> <td>[[<code>0010</code>]]</td> </tr><tr> <th>[[<code>'c'</code>]]</th> <td>[[<code>4</code>]]</td> <td>[[<code>0100</code>]]</td> </tr><tr> <th>[[<code>'d'</code>]]</th> <td>[[<code>'a'</code><code>'d'</code>]</td> <td>[[<code>1000</code>]]</td> </tr></tbody></table>

Se você atribuir um valor de `9` a esta coluna, que é `1001` em binário, então os primeiros e os quatro membros dos valores `SET` `'a'` e `'d'` são selecionados e o valor resultante é `'a,d'`.

Para um valor que contém mais de um elemento `SET`, não importa a ordem em que os elementos estão listados ao inserir o valor. Também não importa quantas vezes um elemento específico está listado no valor. Quando o valor é recuperado posteriormente, cada elemento no valor aparece uma vez, com os elementos listados de acordo com a ordem em que foram especificados no momento da criação da tabela. Suponha que uma coluna seja especificada como `SET('a','b','c','d')`:
...
Se o modo SQL rigoroso estiver ativado, as tentativas de inserir valores inválidos de `SET` resultarão em um erro.

Os valores de `SET` são ordenados numericamente. Os valores de `NULL` são ordenados antes dos valores de `NULL` e `SET` que não são `NULL`.
...
Normalmente, você procura por valores de `SET` usando a função `FIND_IN_SET()` ou o operador `LIKE`:
...
A primeira afirmação encontra linhas onde `set_col` contém o membro do conjunto `value`. A segunda é semelhante, mas não a mesma: ela encontra linhas onde `set_col` contém `value` em qualquer lugar, mesmo como uma subcadeia de outro membro do conjunto.
...
Na API C, os valores de `SET` são retornados como strings. Para obter informações sobre como usar os metadados do conjunto de resultados para distingui-los de outras strings, consulte C API Basic Data Structures.
