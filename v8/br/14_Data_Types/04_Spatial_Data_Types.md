...
* Uma entidade. Por exemplo, uma montanha, um lago, uma cidade. * Um espaço. Por exemplo, um distrito da cidade, os trópicos. * Uma localização definível. Por exemplo, uma encruzilhada, como um lugar específico onde duas ruas se cruzam.
...
Esses padrões são importantes para a implementação de operações espaciais no MySQL:

* SQL/MM Parte 3: Espacial.
* O [Open Geospatial Consortium][(http://www.opengeospatial.org)] publica o *Padrão de Implementação OpenGIS® para Informações Geográficas*, um documento que propõe várias formas conceituais para estender um RDBMS SQL para suportar dados espaciais. Veja, em particular, Acesso a Recursos Simples - Parte 1: Arquitetura Comum e Acesso a Recursos Simples - Parte 2: Opção SQL. O Open Geospatial Consortium (OGC) mantém um site em <http://www.opengeospatial.org/>. A especificação está disponível lá em <http://www.opengeospatial.org/standards/sfs>. Ela contém informações adicionais relevantes para o material aqui.

* A gramática para as definições do sistema de referência espacial [(spatial-reference-systems.html "13.4.5 Spatial Reference System Support")] (SRS) é baseada na gramática definida em *Especificação de Implementação OpenGIS: Serviços de Transformação de Coordenadas*, Revisão 1.00, OGC 01-009, 12 de janeiro de 2001, Seção 7.2. Esta especificação está disponível em <http://www.opengeospatial.org/standards/ct>. Para diferenças dessa especificação nas definições do SRS conforme implementadas no MySQL, consulte a Seção 15.1.19, “Declaração CREATE SPATIAL REFERENCE SYSTEM”.
...
Colunas com um tipo de dados espaciais podem ter um atributo `SRID`, para indicar explicitamente o sistema de referência espacial (SRS) para os valores armazenados na coluna. Por exemplo:
...
O atributo `SRID` torna uma coluna espacial restrita ao SRID, o que tem essas implicações:

* A coluna pode conter apenas valores com o SRID especificado. Tentativas de inserir valores com um SRID diferente produzem um erro.
...
#### 13.4.2.1 A Hierarquia da Classe de Geometria
...
Em MySQL, o valor SRID é um número inteiro associado ao valor de geometria. O valor SRID máximo utilizável é 232-1. Se um valor maior for fornecido, apenas os 32 bits inferiores são utilizados.

SRID 0 representa um plano cartesiano plano infinito sem unidades atribuídas a seus eixos. Para garantir o comportamento do SRID 0, crie valores de geometria usando SRID 0. O SRID 0 é o padrão para novos valores de geometria se nenhum SRID for especificado.
...
* Um `Polygon` não tem anéis que se cruzam. Os anéis na borda de um `Polygon` podem se intersectam em um `Point`, mas apenas como uma tangente.

* Um `Polygon` não tem linhas, pontos ou perfurações.
...
`GeomCollection` e `GeometryCollection` são sinônimos, com `GeomCollection` sendo o nome preferido do tipo.

Todos os elementos de uma coleção de geometria devem estar no mesmo sistema de referência espacial (ou seja, no mesmo sistema de coordenadas). Não há outras restrições para os elementos de uma coleção de geometria, embora as subclasses de `GeomCollection` descritas nas seções a seguir possam restringir a adesão. As restrições podem ser baseadas em:
...
* Um `MultiPoint` é simples se nenhum dos seus valores de `Point` são iguais (têm valores de coordenadas idênticos).
...
* A fronteira de um `MultiCurve` fechado é sempre vazia.

#### 13.4.2.11 Classe MultiLineString
...
* Em um mapa da região, um `MultiPolygon` pode representar um sistema de lagos.
...
* Um `MultiPolygon` não pode ter linhas cortadas, picos ou perfurações. Um `MultiPolygon` é um conjunto de pontos regulares e fechados.
...
Existem funções disponíveis para converter entre diferentes formatos de dados; veja a Seção 14.16.6, “Funções de conversão de formatos de geometria”.
...
A representação de valores de geometria no texto bem conhecido (WKT) é projetada para a troca de dados de geometria em formato ASCII. A especificação OpenGIS fornece uma gramática Backus-Naur que especifica as regras de produção formal para a escrita de valores WKT (ver Seção 13.4, “Tipos de dados espaciais”).
...
Funções espaciais, como `ST_MPointFromText()` e `ST_GeomFromText()`, que aceitam representações de valores em formato WKT de `MultiPoint`, permitem que os pontos individuais dentro dos valores sejam rodeados por parênteses. Por exemplo, ambas as chamadas de função a seguir são válidas:
...
A representação binária bem conhecida (WKB) de valores geométricos é usada para a troca de dados geométricos como fluxos binários representados por valores `BLOB` que contêm informações WKB geométricas. Esse formato é definido pela especificação OpenGIS (veja a Seção 13.4, “Tipos de dados espaciais”). Também é definido no padrão ISO *SQL/MM Parte 3: Espacial*.
...
### 13.4.4 Geometria Bem Formada e Validade
...
* As linhas retas têm pelo menos dois pontos
...
É permitido inserir, selecionar e atualizar geometrias geometricamente inválidas, mas elas devem ser bem formadas sintaticamente. Devido ao custo computacional, o MySQL não verifica explicitamente a validade geométrica. Os cálculos espaciais podem detectar alguns casos de geometrias inválidas e gerar um erro, mas também podem retornar um resultado indefinido sem detectar a invalidade. Aplicações que exigem geometrias geometricamente válidas devem verificá-las usando a função `ST_IsValid()`.
...
Existem diferentes tipos de sistemas de referência espaciais:

* Um SRS projetado é uma projeção de um globo em uma superfície plana; ou seja, um mapa plano. Por exemplo, uma lâmpada dentro de um globo que brilha em um cilindro de papel que envolve o globo projeta um mapa no papel. O resultado é georreferenciado: Cada ponto mapeia a um lugar no globo. O sistema de coordenadas nesse plano é cartesiano, usando uma unidade de comprimento (metros, pés, etc.), em vez de graus de longitude e latitude.
...
Esta entrada descreve o SRS utilizado para sistemas GPS. Tem o nome (`SRS_NAME`) WGS 84 e o ID (`SRS_ID`) 4326, que é o ID utilizado pelo [Grupo Europeu de Pesquisa de Petróleo](http://epsg.org) (EPSG).

As definições de SRS na coluna `DEFINITION` são valores WKT, representados conforme especificado no documento do [Consórcio de Geoprocessamento Aberto][OGC 12-063r5][(http://docs.opengeospatial.org/is/12-063r5/12-063r5.html)].
...
A definição de SRS é analisada sob demanda quando as definições são necessárias por funções do SIG. As definições analisadas são armazenadas no cache do dicionário de dados para permitir a reutilização e evitar o sobrecarregamento da análise para cada declaração que precisa de informações do SRS.
...
* `DROP SPATIAL REFERENCE SYSTEM`(drop-spatial-reference-system.html "15.1.31 DROP SPATIAL REFERENCE SYSTEM Statement"): Veja a Seção 15.1.31, “Declaração de SISTEMA DE REFERÊNCIA ESPACIAL DROP”.

### 13.4.6 Criando Colunas Espaciais

O MySQL oferece uma maneira padrão de criar colunas espaciais para tipos de geometria, por exemplo, com `CREATE TABLE`(create-table.html "15.1.20 CREATE TABLE Statement") ou `ALTER TABLE`. Colunas espaciais são suportadas para as tabelas `MyISAM`, `InnoDB`, `NDB` e `ARCHIVE`. Consulte também as notas sobre índices espaciais na Seção 13.4.10, “Criando índices espaciais”.

Colunas com um tipo de dados espaciais podem ter um atributo SRID, para indicar explicitamente o sistema de referência espacial (SRS) para os valores armazenados na coluna. Para as implicações de uma coluna restrita por SRID, consulte a Seção 13.4.1, “Tipos de Dados Espaciais”.
...
### 13.4.8 Extraindo Dados Espaciais
...
### 13.4.9 Otimizando Análise Espacial
...
O MySQL utiliza **R-Trees com divisão quadrática** para índices `SPATIAL` em colunas espaciais. Um índice `SPATIAL` é construído usando o retângulo mínimo de delimitação (MBR) de uma geometria. Para a maioria das geometrias, o MBR é um retângulo mínimo que envolve as geometrias. Para uma linha horizontal ou vertical, o MBR é um retângulo degenerado na linha. Para um ponto, o MBR é um retângulo degenerado no ponto.
...
`MyISAM` e `InnoDB` suportam tanto índices `SPATIAL` quanto índices não `SPATIAL`. Outros motores de armazenamento suportam índices não `SPATIAL`, conforme descrito na Seção 15.1.15, "Declaração CREATE INDEX".
...
O otimizador pode usar índices espaciais definidos em colunas que são restritas por SRID. Para mais informações, consulte a Seção 13.4.1, “Tipos de dados espaciais”, e a Seção 10.3.3, “Otimização de índice espacial”.

Para mais informações sobre indexação de colunas espaciais, consulte a Seção 15.1.15, “Instrução CREATE INDEX”.

Para descartar índices espaciais, use `ALTER TABLE`(alter-table.html "15.1.9 ALTER TABLE Statement") ou `DROP INDEX`:
