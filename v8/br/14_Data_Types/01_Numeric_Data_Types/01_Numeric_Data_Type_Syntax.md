...
Para os tipos de dados inteiros, `M` indica a largura mínima de exibição. A largura máxima de exibição é de 255. A largura de exibição não está relacionada ao intervalo de valores que um tipo pode armazenar, conforme descrito na Seção 13.1.6, “Atributos de tipos numéricos”.

Para os tipos de dados de ponto flutuante e ponto fixo, `M` é o número total de dígitos que podem ser armazenados.

A partir do MySQL 8.0.17, o atributo width do display é descontinuado para os tipos de dados inteiros; você deve esperar que o suporte a ele seja removido em uma versão futura do MySQL.

Se você especificar `ZEROFILL` para uma coluna numérica, o MySQL adiciona automaticamente o atributo `UNSIGNED` à coluna.

A partir do MySQL 8.0.17, o atributo `ZEROFILL` é desatualizado para tipos de dados numéricos; você deve esperar que o suporte a ele seja removido em uma versão futura do MySQL. Considere usar um meio alternativo para produzir o efeito deste atributo. Por exemplo, as aplicações podem usar a função `LPAD()` para zero-padar números até a largura desejada, ou podem armazenar os números formatados nas colunas `CHAR`.
...
A partir do MySQL 8.0.17, o atributo `UNSIGNED` é desatualizado para colunas do tipo `FLOAT` - FLOAT, DOUBLE"), `DOUBLE` - FLOAT, DOUBLE"), e `DECIMAL` - DECIMAL, NUMERIC") (e quaisquer sinônimos); você deve esperar que o suporte a ele seja removido em uma versão futura do MySQL. Considere usar uma simples restrição `CHECK` em vez disso para tais colunas.
...
Quando você usa a subtração entre valores inteiros, onde um deles é do tipo `UNSIGNED`, o resultado é não assinado, a menos que o modo SQL `NO_UNSIGNED_SUBTRACTION` esteja habilitado. Veja a Seção 14.10, “Funções e Operadores de Cast”.
...
- `TINYINT[(M)] [UNSIGNED] [ZEROFILL]` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")
  ...
  Um número inteiro muito pequeno. A faixa de valores assinados é `-128` a `127`. A faixa de valores não assinados é `0` a `255`.
...
- `SMALLINT[(M)] [UNSIGNED] [ZEROFILL]` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")
  ...
  Um número inteiro pequeno. A faixa de valores assinados é `-32768` a `32767`. A faixa de valores não assinados é `0` a `65535`.

- `MEDIUMINT[(M)] [UNSIGNED] [ZEROFILL]` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")

  Um inteiro de tamanho médio. O intervalo assinado é `-8388608` a `8388607`. O intervalo não assinado é `0` a `16777215`.

- `INT[(M)] [UNSIGNED] [ZEROFILL]` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")

  Um inteiro de tamanho normal. A faixa de valores negativos é `-2147483648` a `2147483647`. A faixa de valores sem sinal é `0` a `4294967295`.
...
- `BIGINT[(M)] [UNSIGNED] [ZEROFILL]` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")
  ...
  Um número inteiro grande. A faixa de valores assinados é `-9223372036854775808` a `9223372036854775807`. A faixa de valores não assinados é `0` a `18446744073709551615`.

  `SERIAL` é um alias para `BIGINT UNSIGNED NOT NULL AUTO_INCREMENT UNIQUE`.

  Algumas coisas que você deve saber sobre as colunas `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")

  - Todas as operações aritméticas são realizadas usando valores `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") ou `DOUBLE` - FLOAT, DOUBLE"). Portanto, você não deve usar inteiros grandes não assinados maiores que `9223372036854775807` (63 bits), exceto com funções de bits! Se você fizer isso, alguns dos últimos dígitos no resultado podem estar errados devido a erros de arredondamento ao converter um valor `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") para um `DOUBLE` - FLOAT, DOUBLE").
    ...
    - Quando usar operadores (`+`, `-`, `*` e assim por diante) onde ambos os operandos são inteiros.
...
- `DECIMAL[(M[,D])] [UNSIGNED] [ZEROFILL]`
  ...
  Um número fixo "exato" preenchido. `M` é o número total de dígitos (a precisão) e `D` é o número de dígitos após a vírgula (a escala). O ponto decimal e (para números negativos) o sinal `-` não são contados em `M`. Se `D` for 0, os valores não têm ponto decimal ou parte fracionária. O número máximo de dígitos (`M`) para `DECIMAL` - DECIMAL, NUMERIC") é 65. O número máximo de casas decimais suportadas (`D`) é 30. Se `D` for omitido, o padrão é 0. Se `M` for omitido, o padrão é 10. (Há também um limite para a quantidade de texto que os literais de \*`DECIMAL` - DECIMAL, NUMERIC") podem ter; veja a Seção 14.24.3, “Tratamento de Expressões.”)

  `UNSIGNED`, se especificado, impede valores negativos. A partir do MySQL 8.0.17, o atributo `UNSIGNED` é desaconselhável para colunas do tipo `DECIMAL` - DECIMAL, NUMERIC") (e quaisquer sinônimos); você deve esperar que o suporte a ele seja removido em uma versão futura do MySQL. Considere usar uma simples restrição `CHECK` em vez disso para tais colunas.
...
- `FLOAT[(M,D)] [UNSIGNED] [ZEROFILL]` - FLOAT, DOUBLE")
  ...
  `M` é o número total de dígitos e `D` é o número de dígitos após o ponto decimal. Se `M` e `D` forem omitidos, os valores são armazenados nos limites permitidos pelo hardware. Um número de ponto flutuante de precisão simples tem precisão de aproximadamente 7 casas decimais.

  `FLOAT(M,D)` é uma extensão MySQL não padrão. A partir do MySQL 8.0.17, essa sintaxe está desatualizada e você deve esperar que o suporte a ela seja removido em uma versão futura do MySQL.

  `UNSIGNED`, se especificado, impede valores negativos. A partir do MySQL 8.0.17, o atributo `UNSIGNED` é desaconselhável para colunas do tipo `FLOAT` - FLOAT, DOUBLE") (e quaisquer sinônimos) e você deve esperar que o suporte para ele seja removido em uma versão futura do MySQL. Considere usar uma simples restrição `CHECK` em vez disso para tais colunas.

  O uso de `FLOAT` - FLOAT, DOUBLE") pode causar alguns problemas inesperados, pois todos os cálculos no MySQL são feitos com precisão dupla. Veja a Seção B.3.4.7, “Resolvendo Problemas com Nenhuma Linha de Correspondência”.

- `FLOAT(p) [UNSIGNED] [ZEROFILL]` - FLOAT, DOUBLE")

  Um número de ponto flutuante. `p` representa a precisão em bits, mas o MySQL usa esse valor apenas para determinar se deve usar `FLOAT` - FLOAT, DOUBLE") ou `DOUBLE` - FLOAT, DOUBLE") para o tipo de dado resultante. Se `p` estiver entre 0 e 24, o tipo de dado se torna `FLOAT` - FLOAT, DOUBLE") sem valores de `M` ou `D`. Se `p` estiver entre 25 e 53, o tipo de dado se torna `DOUBLE` - FLOAT, DOUBLE") sem valores de `M` ou `D`. A faixa da coluna resultante é a mesma que para os tipos de dados de ponto flutuante de precisão única `FLOAT` - FLOAT, DOUBLE") ou de precisão dupla `DOUBLE` - FLOAT, DOUBLE") descritos anteriormente nesta seção.
...
- `DOUBLE[(M,D)] [UNSIGNED] [ZEROFILL]` - FLOAT, DOUBLE")
  ...
  `M` é o número total de dígitos e `D` é o número de dígitos após o ponto decimal. Se `M` e `D` forem omitidos, os valores são armazenados nos limites permitidos pelo hardware. Um número de ponto flutuante de dupla precisão tem precisão de aproximadamente 15 casas decimais.

  `DOUBLE(M,D)` é uma extensão MySQL não padrão. A partir do MySQL 8.0.17, essa sintaxe está desatualizada e você deve esperar que o suporte a ela seja removido em uma versão futura do MySQL.

  `UNSIGNED`, se especificado, impede valores negativos. A partir do MySQL 8.0.17, o atributo `UNSIGNED` é desaconselhável para colunas do tipo `DOUBLE` - FLOAT, DOUBLE") (e quaisquer sinônimos) e você deve esperar que o suporte para ele seja removido em uma versão futura do MySQL. Considere usar uma simples restrição `CHECK` em vez disso para tais colunas.
