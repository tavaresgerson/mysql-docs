...
Para particionar uma tabela usando a particionamento `HASH`, é necessário anexar à declaração `CREATE TABLE` uma cláusula `PARTITION BY HASH (expr)`, onde `expr` é uma expressão que retorna um inteiro. Isso pode ser simplesmente o nome de uma coluna cujo tipo é um dos tipos inteiros do MySQL. Além disso, você provavelmente deseja seguir isso com `PARTITIONS num`, onde `num` é um inteiro positivo representando o número de particionações em que a tabela deve ser dividida.

Nota

Por simplicidade, as tabelas nos exemplos a seguir não usam nenhuma chave. Você deve estar ciente de que, se uma tabela tiver qualquer chave única, todas as colunas usadas na expressão de particionamento para essa tabela devem fazer parte de cada chave única, incluindo a chave primária. Consulte a Seção 26.6.1, “Chaves de Particionamento, Chaves Primárias e Chaves Únicas”, para obter mais informações.
...
Se você não incluir uma cláusula `PARTITIONS`, o número de partições será definido como `1`; o uso da palavra-chave `PARTITIONS` sem um número após ela resulta em um erro de sintaxe.
...
O MySQL 8.0 também suporta uma variante da partição `HASH` conhecida como hashing linear, que utiliza um algoritmo mais complexo para determinar o local onde as novas linhas inseridas na tabela particionada serão colocadas. Consulte a Seção 26.2.4.1, “Partição de Hash Linear”, para uma descrição desse algoritmo.
