## 26.4 **Rugosidade de Partição**

A otimização conhecida como poda de partição é baseada em um conceito relativamente simples, que pode ser descrito como “não escaneie partições onde não podem haver valores correspondentes”. Suponha que uma tabela dividida `t1` seja criada por meio da seguinte declaração:
...
É fácil perceber que nenhuma das linhas que deveriam ser devolvidas está em nenhuma das partições `p0` ou `p3`; ou seja, precisamos procurar apenas nas partições `p1` e `p2` para encontrar as linhas correspondentes. Ao limitar a pesquisa, é possível gastar muito menos tempo e esforço na busca de linhas correspondentes do que ao digitalizar todas as partições na tabela. Esse "retirada" de partições desnecessárias é conhecido como poda. Quando o otimizador pode fazer uso da poda de partições ao realizar essa consulta, a execução da consulta pode ser uma ordem de magnitude mais rápida do que a mesma consulta em uma tabela não particionada que contenha as mesmas definições de coluna e dados.
...
As declarações `SELECT`, `DELETE` e `UPDATE` suportam o corte de partições. Uma declaração `INSERT` também acessa apenas uma partição por linha inserida; isso é verdadeiro mesmo para uma tabela que é dividida por `HASH` ou `KEY`, embora isso não esteja atualmente mostrado na saída de `EXPLAIN`.
...
O otimizador também pode realizar poda para condições `WHERE` que envolvem comparações dos tipos anteriores em várias colunas para tabelas que utilizam `RANGE COLUMNS` ou `LIST COLUMNS` particionamento.

Esse tipo de otimização pode ser aplicado sempre que a expressão de particionamento consiste em uma igualdade ou em uma faixa que possa ser reduzida a um conjunto de igualdades, ou quando a expressão de particionamento representa uma relação crescente ou decrescente. A poda também pode ser aplicada para tabelas particionadas em uma coluna `DATE` ou `DATETIME`, quando a expressão de particionamento utiliza a função `YEAR()` ou `TO_DAYS()`. A poda também pode ser aplicada para tais tabelas quando a expressão de particionamento utiliza a função `TO_SECONDS()`.

Suponha que a tabela `t2`, particionada em uma coluna `DATE`, seja criada usando a declaração mostrada aqui:
...
Neste caso, isso significa que apenas as partições `d3`, `d4` e `d5` são analisadas. As demais partições podem ser ignoradas com segurança (e são ignoradas).
...
As tabelas que utilizam um mecanismo de armazenamento que oferece particionamento automático, como o mecanismo de armazenamento `NDB` utilizado pelo MySQL Cluster, podem ser reduzidas se estiverem explicitamente particionadas.