...
Um gatilho é definido para ser ativado quando uma declaração insere, atualiza ou exclui linhas na tabela associada. Essas operações de linha são eventos de gatilho. Por exemplo, as linhas podem ser inseridas por declarações `INSERT` ou [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") e um gatilho de inserção é ativado para cada linha inserida. Um gatilho pode ser configurado para ser ativado antes ou depois do evento do gatilho. Por exemplo, você pode ter um gatilho ativado antes de cada linha que é inserida em uma tabela ou depois de cada linha que é atualizada.
...
Os gatilhos não são ativados por alterações nas tabelas `INFORMATION_SCHEMA` ou `performance_schema`. Essas tabelas são, na verdade, vistas e os gatilhos não são permitidos em vistas.
...
* Para respostas a perguntas comumente feitas sobre gatilhos no MySQL, consulte a Seção A.5, “Perguntas frequentes do MySQL 8.0: gatilhos”.

* Há algumas restrições sobre o uso de gatilhos; veja a Seção 27.8, “Restrições sobre programas armazenados”.

* O registro binário para gatilhos ocorre conforme descrito na Seção 27.7, “Registro binário de programas armazenados”.

### 27.3.1 Sintaxe e exemplos de gatilho

Para criar um gatilho ou descartar um gatilho, use a declaração `CREATE TRIGGER` ou `DROP TRIGGER`, descrita na Seção 15.1.22, "Declaração CREATE TRIGGER", e na Seção 15.1.34, "Declaração DROP TRIGGER".
...
A declaração `CREATE TRIGGER` cria um gatilho chamado `ins_sum` que está associado à tabela `account`. Ela também inclui cláusulas que especificam o tempo da ação do gatilho, o evento que desencadeia e o que fazer quando o gatilho é ativado:
...
Para destruir o gatilho, use uma declaração `DROP TRIGGER` (drop-trigger.html "15.1.34 DROP TRIGGER Statement"). Você deve especificar o nome do esquema se o gatilho não estiver no esquema padrão:
...
Esse gatilho, `ins_transaction`, é semelhante ao `ins_sum`, mas acumula depósitos e retiradas separadamente. Ele possui uma cláusula `PRECEDES` que faz com que ele se ative antes do `ins_sum`; sem essa cláusula, ele se ativaria após o `ins_sum`, pois é criado após o `ins_sum`.
...
Usando o `BEGIN ... END` (begin-end.html "15.6.1 BEGIN ... END Compound Statement") construído, você pode definir um gatilho que executa múltiplos comandos. Dentro do bloco `BEGIN`, você também pode usar outra sintaxe que é permitida em rotinas armazenadas, como condicionais e loops. No entanto, assim como para rotinas armazenadas, se você usar o programa **mysql** para definir um gatilho que executa múltiplos comandos, é necessário redefinir o delimitador da declaração **mysql** para que você possa usar o delimitador da declaração `;` dentro da definição do gatilho. O exemplo a seguir ilustra esses pontos. Ele define um gatilho `UPDATE` que verifica o novo valor a ser usado para atualizar cada linha e modifica o valor para estar dentro da faixa de 0 a 100. Isso deve ser um gatilho `BEFORE` porque o valor deve ser verificado antes de ser usado para atualizar a linha:
...
* O gatilho não pode usar declarações que explicitamente ou implicitamente comecem ou terminem uma transação, como `START TRANSACTION`(commit.html "15.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements"), `COMMIT` ou `ROLLBACK`. (`ROLLBACK to SAVEPOINT`(commit.html "15.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements") é permitido porque não termina uma transação).
...
### 27.3.2 Metadados do gatilho

Para obter metadados sobre gatilhos:

* Consulte a tabela `TRIGGERS` do banco de dados `INFORMATION_SCHEMA`. Veja a Seção 28.3.45, “A tabela TRIGGERS do INFORMATION_SCHEMA”.

* Utilize a declaração `SHOW CREATE TRIGGER`. Veja a Seção 15.7.7.11, “Declaração SHOW CREATE TRIGGER”.

* Utilize a declaração `SHOW TRIGGERS`. Veja a Seção 15.7.7.40, “Declaração SHOW TRIGGERS”.