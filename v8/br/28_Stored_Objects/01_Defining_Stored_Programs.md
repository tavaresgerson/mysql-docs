...
Para redefinir o delimitador do **mysql**, use o comando `delimiter`. O exemplo a seguir mostra como fazer isso para o procedimento `dorepeat()` que foi mostrado anteriormente. O delimitador é alterado para `//` para permitir que toda a definição seja passada para o servidor como uma única declaração, e depois restaurado para `;` antes de invocar o procedimento. Isso permite que o delimitador `;` usado no corpo do procedimento seja passado para o servidor em vez de ser interpretado pelo próprio **mysql**.
