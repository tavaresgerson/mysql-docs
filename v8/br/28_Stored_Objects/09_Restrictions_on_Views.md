...
Existe um princípio geral de que não é possível modificar uma tabela e selecionar dados da mesma tabela em uma subconsulta. Veja a Seção 15.2.15.12, “Restrições em subconsultas”.
...
Uma solução para qualquer um desses problemas é modificar cada definição de visualização problemática para usar aliases que forneçam nomes de colunas mais curtos. Em seguida, a visualização se replica corretamente e pode ser descarregada e carregada novamente sem causar um erro. Para modificar a definição, arraste e crie a visualização novamente com [`DROP VIEW`](drop-view.html "15.1.35 DROP VIEW Statement") e `CREATE VIEW`, ou substitua a definição com [`CREATE OR REPLACE VIEW`](create-view.html "15.1.23 CREATE VIEW Statement").
