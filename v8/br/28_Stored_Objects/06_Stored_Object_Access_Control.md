...
Considere o seguinte procedimento armazenado, que é declarado com `SQL SECURITY DEFINER` para execução em contexto de segurança definido:
...
Qualquer usuário que tenha o privilégio `EXECUTE` para `p1` pode invocá-lo com uma declaração `CALL`. No entanto, quando `p1` é executado, ele é executado no contexto de segurança do definidor e, portanto, é executado com os privilégios de `'admin'@'localhost'`, a conta nomeada como seu atributo `DEFINER`. Essa conta deve ter o privilégio `EXECUTE` para `p1` e também o privilégio `UPDATE` para a tabela `t1` referenciada no corpo do objeto. Caso contrário, o procedimento falha.
...
Um objeto de orfanato é aquele para o qual o atributo `DEFINER` nomeia uma conta inexistente:
...
- Um objeto armazenado existente pode se tornar órfão devido à execução de uma instrução `DROP USER` que elimina a conta do objeto `DEFINER`, ou de uma instrução `RENAME USER` que renomeia a conta do objeto `DEFINER`.
...
- O objeto pode apresentar um risco de segurança se a conta inexistente `DEFINER` for posteriormente recriada para um propósito não relacionado ao objeto. Nesse caso, a conta “adquire” o objeto e, com os privilégios apropriados, é capaz de executá-lo, mesmo que isso não seja o objetivo.

A partir do MySQL 8.0.22, o servidor impõe verificações de segurança adicionais de gerenciamento de contas, projetadas para prevenir operações que (provavelmente inadvertidamente) causem objetos armazenados a ficarem órfãos ou que causem a adoção de objetos armazenados que estejam atualmente órfãos:

- `DROP USER` falha com um erro se qualquer conta a ser removida tiver o nome como o atributo `DEFINER` para qualquer objeto armazenado. (Ou seja, a declaração falha se a remoção de uma conta causar que um objeto armazenado se torne órfão.)

- `RENAME USER` falha com um erro se qualquer conta a ser renomeada tiver o nome como o atributo `DEFINER` para qualquer objeto armazenado. (Ou seja, a declaração falha se a renomeação de uma conta causar que um objeto armazenado se torne órfão.)

- `CREATE USER` falha com um erro se qualquer conta a ser criada tiver o nome como o atributo `DEFINER` para qualquer objeto armazenado. (Ou seja, a declaração falha se a criação de uma conta causar que a conta adote um objeto armazenado atualmente órfão.)

Em certas situações, pode ser necessário executar esses registros de gerenciamento de contas deliberadamente, mesmo quando eles falhariam de outra forma. Para tornar isso possível, se um usuário tiver o privilégio `SET_USER_ID`, esse privilégio substitui as verificações de segurança de objetos órfãos e os registros têm sucesso com um aviso em vez de falhar com um erro.

Para obter informações sobre as contas usadas como definidoras de objetos armazenados em uma instalação do MySQL, consulte o `INFORMATION_SCHEMA`.

Esta consulta identifica quais as tabelas `INFORMATION_SCHEMA` que descrevem objetos que possuem o atributo `DEFINER`:
...
O resultado indica quais tabelas consultar para descobrir quais valores do objeto armazenado `DEFINER` existem e quais objetos têm um valor específico `DEFINER`:
...
- Para identificar quais valores de `DEFINER` existem em cada tabela, use essas consultas:
  ...
  Para redefinir um objeto com um definidor diferente, você pode usar `ALTER EVENT` ou `ALTER VIEW` para modificar diretamente a conta `DEFINER` de eventos e visualizações. Para procedimentos armazenados, funções e gatilhos, você deve descartar o objeto e recriá-lo para atribuir uma conta diferente `DEFINER`

- Para identificar quais objetos têm uma conta com o `DEFINER` dado, use essas consultas, substituindo a conta de interesse por `user_name@host_name`:
...
- Não crie objetos armazenados órfãos; ou seja, objetos para os quais o atributo `DEFINER` nomeia uma conta inexistente. Não faça com que os objetos armazenados se tornem órfãos ao excluir ou renomear uma conta nomeada pelo atributo `DEFINER` de qualquer objeto existente.
...
- Se você criar objetos armazenados com o atributo de definidor de contexto enquanto estiver usando uma conta que tenha o privilégio `SET_USER_ID` (ou o privilégio desatualizado `SUPER`), especifique um atributo explícito `DEFINER` que nomeie uma conta que possua apenas os privilégios necessários para as operações realizadas pelo objeto. Especifique uma conta altamente privilegiada `DEFINER` apenas quando absolutamente necessário.

- Os administradores podem impedir que os usuários criem objetos armazenados que especifiquem contas altamente privilegiadas `DEFINER` ao não conceder a eles o privilégio `SET_USER_ID` (ou o privilégio desatualizado `SUPER`).
...
- Os objetos de definição de contexto devem ser escritos com a consciência de que eles podem ter acesso a dados para os quais o usuário que os invoca não tenha privilégios. Em alguns casos, você pode impedir referências a esses objetos ao não conceder privilégios específicos a usuários não autorizados:
  ...
  - Uma visualização não pode ser referenciada por um usuário que não tenha o privilégio apropriado para selecioná-la (`SELECT` para selecioná-la, `INSERT` para inseri-la, e assim por diante).
  ...
  Em ambos os casos, se a conta `DEFINER` tiver privilégios elevados, o objeto poderá realizar operações sensíveis ou perigosas. Isso continua válido mesmo se os privilégios necessários para criar o objeto forem revogados da conta do usuário que o criou. Os administradores devem ter especial cautela ao conceder privilégios de criação de objetos aos usuários.
