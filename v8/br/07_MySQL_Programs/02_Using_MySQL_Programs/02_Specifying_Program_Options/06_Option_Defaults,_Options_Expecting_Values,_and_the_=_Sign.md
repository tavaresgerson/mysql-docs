#### 6.2.2.6 Opções Padrão, Valores Esperados das Opções e o Sinal de Igual (=)
...
Devido a esse comportamento, problemas podem ocorrer ocasionalmente quando não é fornecido um valor para uma opção que espera um. Considere o seguinte exemplo, onde um usuário se conecta a um servidor MySQL em execução no host `tonfisk` como usuário `jon`:
...
Neste caso, o **mysql** não conseguiu encontrar um valor após a opção `--user`, porque nada veio depois dela na linha de comando. No entanto, se você omitir o valor para uma opção que *não* for a última opção a ser usada, você obterá um erro diferente que você pode não estar esperando:
...
Porque o **mysql** assume que qualquer string que siga `--host` na linha de comando é um nome de host, `--host` `--user` é interpretado como `--host=--user`, e o cliente tenta se conectar a um servidor MySQL que está rodando em um host chamado “--user”.

As opções com valores padrão sempre exigem um sinal de igual ao atribuir um valor; caso contrário, isso causará um erro. Por exemplo, a opção do servidor MySQL `--log-error` tem o valor padrão `host_name.err`, onde `host_name` é o nome do host em que o MySQL está sendo executado. Suponha que você esteja executando o MySQL em um computador cujo nome de host é “tonfisk”, e considere a seguinte invocação do **mysqld\_safe**:
...
O servidor tentou começar a usar `/usr/local/mysql/var/tonfisk.err` como o log de erro, mas depois desligou. Examinando as últimas linhas deste arquivo, você verá a razão:
...
No entanto, em arquivos de opção, um sinal de igual não é assumido. Suponha que o arquivo `my.cnf` seja como mostrado aqui:
