...
Esta seção descreve o uso de opções de linha de comando para especificar como estabelecer conexões com o servidor MySQL, para clientes como **mysql** ou **mysqldump**. Para obter informações sobre como estabelecer conexões usando cadeias de conexão semelhantes a URI ou pares chave-valor, para clientes como o MySQL Shell, consulte a Seção 6.2.5, “Conectando ao Servidor Usando Strings Semelhantes a URI ou Pares Chave-Valor”. Para obter informações adicionais se você não conseguir se conectar, consulte a Seção 8.2.22, “Resolvendo Problemas de Conexão com o MySQL”.
...
- Se você usar uma opção `--password` ou `-p` e especificar um valor de senha, não pode haver *espaço* entre `--password=` ou `-p` e a senha que segue.
...
Como mencionado, incluir o valor da senha na linha de comando é um risco à segurança. Para evitar esse risco, especifique a opção `--password` ou `-p` sem qualquer valor de senha subsequente:
...
A opção `--protocol` permite que você use um protocolo de transporte específico, mesmo quando outras opções normalmente resultam no uso de um protocolo diferente. Ou seja, `--protocol` especifica o protocolo de transporte explicitamente e substitui as regras anteriores, mesmo para `localhost`.
...
No Windows, para forçar um cliente MySQL a usar uma conexão por canal de nomeação, especifique a opção `--pipe` ou `--protocol=PIPE` ou especifique `.` (ponto) como o nome do host. Se o servidor não foi iniciado com a variável de sistema `named_pipe` habilitada para suportar conexões por canal de nomeação ou se o usuário que está fazendo a conexão não é membro do grupo do Windows especificado pela variável de sistema `named_pipe_full_access_group`, ocorre um erro. Use a opção `--socket` para especificar o nome do canal se você não quiser usar o nome padrão do canal.
...
- Especifique os parâmetros de conexão na seção `[client]` de um arquivo de opção. A seção relevante do arquivo pode parecer assim:
  ...
  Para obter mais informações, consulte a Seção 6.2.2.2, “Usando arquivos de opção”.
