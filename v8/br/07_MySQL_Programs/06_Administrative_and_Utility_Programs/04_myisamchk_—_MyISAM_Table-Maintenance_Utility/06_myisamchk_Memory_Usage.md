...
- Ao usar `--recover` ou `--sort-recover` (mas não quando usar `--safe-recover`), você precisa de espaço no disco para a classificação. Esse espaço é alocado no diretório temporário (especificado por `TMPDIR` ou `--tmpdir=dir_name`). A seguinte fórmula fornece a quantidade de espaço necessária:
