...
- `--backup`, `-B`
  ...
  <table summary="Propriedades para backup"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--backup</code>]]</td> </tr></tbody></table>
...
- `--correct-checksum`
  ...
  <table summary="Propriedades para verificação de checksum correta"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--correct-checksum</code>]]</td> </tr></tbody></table>

  Corrija as informações do checksum da tabela.

- `--data-file-length=len`, `-D len`

  <table summary="Propriedades para comprimento do arquivo de dados"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--data-file-length=len</code>]]</td> </tr><tr><th>Tipo</th> <td>Numérico</td> </tr></tbody></table>

  O comprimento máximo do arquivo de dados (quando o arquivo de dados é recriado quando está "cheio").

- `--extend-check`, `-e`

  <table summary="Propriedades para extend-check"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--extend-check</code>]]</td> </tr></tbody></table>
  ...
  Para uma descrição do formato de saída, consulte a Seção 6.6.4.5, “Obtendo informações da tabela com myisamchk”.

- `--force`, `-f`

  <table summary="Propriedades para força"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--force</code>]]</td> </tr></tbody></table>

  Sobrepor arquivos intermediários antigos (arquivos com nomes como `tbl_name.TMD`) em vez de abortar.

- `--keys-used=val`, `-k val`

  <table summary="Propriedades para chaves usadas"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--keys-used=val</code>]]</td> </tr><tr><th>Tipo</th> <td>Numérico</td> </tr></tbody></table>
...
- `--max-record-length=len`
  ...
  <table summary="Propriedades para max-record-length"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--max-record-length=len</code>]]</td> </tr><tr><th>Tipo</th> <td>Numérico</td> </tr></tbody></table>

  Pule linhas maiores que o comprimento especificado se o **myisamchk** não puder alocar memória para armazená-las.

- `--parallel-recover`, `-p`

  <table summary="Propriedades para recuperação paralela"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--parallel-recover</code>]]</td> </tr></tbody></table>

  Nota

  Esta opção foi descontinuada no MySQL 8.0.28 e removida no MySQL 8.0.30.
...
- `--quick`, `-q`
  ...
  <table summary="Propriedades para rápido"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--quick</code>]]</td> </tr></tbody></table>
...
- `--recover`, `-r`
  ...
  Faça uma reparação que possa corrigir quase qualquer problema, exceto chaves únicas que não são únicas (o que é um erro extremamente improvável com tabelas `MyISAM`). Se você quiser recuperar uma tabela, essa é a opção a tentar primeiro. Você deve tentar `--safe-recover` apenas se o **myisamchk** relatar que a tabela não pode ser recuperada usando `--recover`. (No caso improvável de que `--recover` falhe, o arquivo de dados permanece intacto.)
...
- `--tmpdir=dir_name`, `-t dir_name`
  ...
  O caminho do diretório a ser usado para armazenar arquivos temporários. Se este não for definido, o **myisamchk** usa o valor da variável de ambiente `TMPDIR`. `--tmpdir` pode ser definido como uma lista de caminhos de diretórios que são usados sucessivamente de forma round-robin para criar arquivos temporários. O caractere de separador entre os nomes dos diretórios é o colon (`:`) no Unix e o ponto e vírgula (`;`) no Windows.
