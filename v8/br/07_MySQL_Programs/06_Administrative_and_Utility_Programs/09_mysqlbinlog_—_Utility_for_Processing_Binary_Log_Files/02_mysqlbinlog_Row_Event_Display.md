...
Para ver os eventos da linha como comentários na forma de declarações de “pseudo-SQL”, execute o **mysqlbinlog** com a opção `--verbose` ou `-v`. Esse nível de saída também mostra as informações de partição da tabela, quando aplicável. A saída contém linhas que começam com `###`:
...
- As informações do conjunto de caracteres não estão disponíveis no log binário, o que afeta a exibição das colunas de texto:
  ...
  - Não há distinção feita entre os tipos de strings binárias e não binárias correspondentes (`BINARY` e `CHAR`, `VARBINARY` e `VARCHAR`, `BLOB` e `TEXT`). A saída utiliza um tipo de dado de `STRING` para strings de comprimento fixo e `VARSTRING` para strings de comprimento variável.

  - Para conjuntos de caracteres multibyte, o número máximo de bytes por caractere não está presente no log binário, portanto, o comprimento para os tipos de string é exibido em bytes, em vez de em caracteres. Por exemplo, `STRING(4)` é usado como tipo de dados para valores de qualquer um desses tipos de coluna:
