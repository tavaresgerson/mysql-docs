...
O **myisam\_ftdump** exibe informações sobre os índices `FULLTEXT` nas tabelas `MyISAM`. Ele lê o arquivo de índice `MyISAM` diretamente, portanto, ele deve ser executado no host do servidor onde a tabela está localizada. Antes de usar o **myisam\_ftdump**, certifique-se de emitir uma declaração `FLUSH TABLES` primeiro, se o servidor estiver em execução.
...
O índice em `id` é o índice 0 e o índice em `FULLTEXT` em `txt` é o índice 1. Se o diretório de trabalho for o diretório da base de dados `test`, invocando o **myisam\_ftdump** da seguinte forma:
...
- `--length`, `-l`
  ...
  <table summary="Propriedades para comprimento"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--length</code>]]</td> </tr></tbody></table>

  Relacione a distribuição do comprimento.

- `--stats`, `-s`

  <table summary="Propriedades para estatísticas"><tbody><tr><th>Formato de linha de comando</th> <td>[[<code>--stats</code>]]</td> </tr></tbody></table>
