...
- Se você tiver um dos seguintes sintomas, provavelmente é um problema de hardware (como memória, placa-mãe, CPU ou disco rígido) ou um problema no kernel:
  ...
  Nesse caso, você deve começar verificando todos os seus cabos e executar uma ferramenta de diagnóstico para verificar seu hardware! Você também deve verificar se há algum patch, atualização ou pacote de serviço para o seu sistema operacional que possa resolver o problema. Verifique também se todas as suas bibliotecas (como `glibc`) estão atualizadas.
...
- Por favor, examine seu arquivo de log do sistema (`/var/log/messages` ou semelhante) para encontrar as razões do seu problema. Se você acha que o problema está no MySQL, você também deve examinar os arquivos de log do MySQL. Veja a Seção 7.4, “Logs do Servidor MySQL”.
...
- O que um programa cliente diz quando você tenta se conectar ao servidor MySQL? (Tente com **mysql**, por exemplo.) O cliente travou? Você obtém algum tipo de saída do programa?
