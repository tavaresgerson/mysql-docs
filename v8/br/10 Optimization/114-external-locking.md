...
* Se você usar `myisamchk` para realizar operações de manutenção de tabelas em tabelas `MyISAM`, você deve garantir que o servidor não esteja em execução ou que o servidor tenha o bloqueio externo habilitado para que ele bloqueie os arquivos das tabelas conforme necessário para coordenar com `myisamchk` para o acesso às tabelas. O mesmo vale para o uso de `myisampack` para embalar tabelas `MyISAM`.
  ...
  Se você usar `myisamchk` para operações de escrita, como reparar ou otimizar tabelas, ou se usar `myisampack` para embalar tabelas, você *deve* sempre garantir que o servidor `mysqld` não esteja usando a tabela. Se você não parar o `mysqld`, pelo menos faça `mysqladmin flush-tables` antes de executar `myisamchk`. Seus tabelas *podem ficar corrompidas* se o servidor e o `myisamchk` acessarem as tabelas simultaneamente.

Com o bloqueio externo em vigor, cada processo que requer acesso a uma tabela adquire um bloqueio do sistema de arquivos para os arquivos das tabelas antes de prosseguir para acessar a tabela. Se todos os bloqueios necessários não puderem ser adquiridos, o processo é bloqueado de acessar a tabela até que os bloqueios possam ser obtidos (após o processo que atualmente detém os bloqueios liberá-los).
...
O bloqueio externo é desnecessário se você estiver executando um único servidor para acessar um diretório de dados específico (o que é o caso mais comum) e se nenhum outro programa, como `myisamchk`, precisar modificar tabelas enquanto o servidor estiver em execução. Se você estiver lendo apenas tabelas com outros programas, o bloqueio externo não é necessário, embora o `myisamchk` possa emitir avisos se o servidor alterar as tabelas enquanto o `myisamchk` as estiver lendo.

Com o bloqueio externo desativado, para usar o `myisamchk`, você deve interromper o servidor enquanto o `myisamchk` estiver sendo executado ou, caso contrário, bloquear e limpar as tabelas antes de executar o `myisamchk`. Para evitar essa exigência, use as instruções `CHECK TABLE` e `REPAIR TABLE` para verificar e reparar as tabelas `MyISAM`.
...
Se você usar a opção de bloqueio externo para permitir atualizações nas tabelas `MyISAM` de muitos processos MySQL, não inicie o servidor com a variável de sistema `delay_key_write` definida como `ALL` ou use a opção de tabela `DELAY_KEY_WRITE=1` para quaisquer tabelas compartilhadas. Caso contrário, pode ocorrer corrupção de índices.

A maneira mais fácil de atender a essa condição é sempre usar `--external-locking` junto com `--delay-key-write=OFF`. (Isso não é feito por padrão porque, em muitas configurações, é útil ter uma mistura das opções anteriores.)