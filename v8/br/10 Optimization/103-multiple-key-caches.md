#### 10.10.2.2 Caches de Chave Múltiplas

::: info Nota

A partir do MySQL 8.4, a sintaxe de variável estruturada com parte composta discutida aqui para se referir a múltiplas caches de chave `MyISAM` é desatualizada.
...
As variáveis de cache de chave são variáveis de sistema estruturadas que têm um nome e componentes. Para `keycache1.key_buffer_size`, `keycache1` é o nome da variável de cache e `key_buffer_size` é o componente de cache. Consulte  Seção 7.1.9.5, “Variáveis de Sistema Estruturadas”, para uma descrição da sintaxe usada para se referir a variáveis de sistema de cache de chave estruturadas.

Por padrão, os índices de tabela são atribuídos ao cache de chave principal (padrão) criado na inicialização do servidor. Quando um cache de chave é destruído, todos os índices atribuídos a ele são reatribuídos ao cache de chave padrão.
...
* Um cache de chave "frio" que ocupa 20% do espaço alocado para todos os caches de chave. Use este cache para tabelas de tamanho médio e que são modificadas intensamente, como tabelas temporárias.
* Um cache de chave "quente" que ocupa 60% do espaço do cache de chave. Empregue este como o cache de chave padrão, para ser usado por padrão para todas as outras tabelas.
...
* Uma operação de atualização executada com mais frequência para tabelas temporárias é realizada muito mais rápido quando o nó atualizado está no cache e não precisa ser lido do disco primeiro. Se o tamanho dos índices das tabelas temporárias for comparável ao tamanho do cache de chave fria, a probabilidade é muito alta de que o nó atualizado esteja no cache.

A declaração `CACHE INDEX` estabelece uma associação entre uma tabela e um cache de chave, mas essa associação é perdida toda vez que o servidor é reiniciado. Se você deseja que a associação seja efetiva a cada início do servidor, uma maneira de realizar isso é usar um arquivo de opções: Inclua configurações de variáveis que configurem seus caches de chave e uma variável de sistema `init_file` que nomeia um arquivo contendo as declarações `CACHE INDEX` a serem executadas. Por exemplo:
...
As declarações em `mysqld_init.sql` são executadas a cada vez que o servidor é iniciado. O arquivo deve conter uma declaração SQL por linha. O exemplo a seguir atribui várias tabelas a cada um dos `hot_cache` e `cold_cache`:
