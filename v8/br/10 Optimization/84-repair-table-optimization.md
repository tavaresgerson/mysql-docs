...
O `REPARE TÁBLIA` para tabelas `MyISAM` é semelhante ao uso do `myisamchk` para operações de reparo, e algumas das mesmas otimizações de desempenho se aplicam:
...
As variáveis do sistema que afetam `REPAIR TABLE` podem ser definidas globalmente no início do servidor se você quiser que os valores sejam efetivos por padrão. Por exemplo, adicione essas linhas ao arquivo `my.cnf` do servidor:
