...
Quando o otimizador avalia os planos para operações de junção externa, ele considera apenas planos onde, para cada operação desse tipo, as tabelas externas são acessadas antes das tabelas internas. As escolhas do otimizador são limitadas porque apenas tais planos permitem a execução de junções externas usando o algoritmo de laço aninhado.
...
Condições como essas são rejeitadas por `NULL` porque não podem ser verdadeiras para qualquer linha completada por `NULL` (com as colunas `T2` definidas como `NULL`):
...
Condições como essas não são rejeitadas por `NULL` porque podem ser verdadeiras para uma linha completada por `NULL`:
...
* É um predicado que contém uma referência a uma tabela interna e avalia como `UNKNOWN` quando um de seus argumentos é `NULL`
...
Que pode ser reescrita apenas na forma que ainda contém a operação de junção externa embutida:
...
Consequentemente, a consulta pode ser convertida para:
