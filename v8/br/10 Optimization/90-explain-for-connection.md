...
Se a conexão nomeada estiver executando uma instrução que não é explicável, ocorre um erro. Por exemplo, você não pode nomear o identificador da conexão para sua sessão atual porque `EXPLAIN` não é explicável:
