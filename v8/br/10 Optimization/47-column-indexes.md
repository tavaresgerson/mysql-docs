...
Para obter informações adicionais sobre índices de prefixo, consulte a Seção 15.1.15, “Instrução CREATE INDEX”.
...
As otimizações são aplicadas a certos tipos de consultas `FULLTEXT` em tabelas `InnoDB` individuais. As consultas com essas características são particularmente eficientes:
...
#### Índices no Moedor de Armazenamento `MEMORY`

O moedor de armazenamento `MEMORY` usa índices `HASH` por padrão, mas também suporta índices `BTREE`.