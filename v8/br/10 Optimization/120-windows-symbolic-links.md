...
5. Se você ainda não estiver usando o prompt de comando, abra-o e mude a localização para o diretório de dados, assim:
   ...
   Se a sua instalação do MySQL estiver no local padrão, você pode usar:

   ```
   C:\> cd C:\ProgramData\MySQL\MySQL Server 8.4\Data
   ```
6. No diretório de dados, crie um symlink chamado `mydb` que aponte para a localização do diretório de banco de dados:
...
Depois disso, todas as tabelas criadas no banco de dados `mydb` são criadas em `D:\data\mydb`.
...
O suporte para links simbólicos de banco de dados no Windows usando arquivos `.sym` está habilitado por padrão. Se você não precisar de links simbólicos de arquivos `.sym`, pode desabilitar o suporte para eles iniciando o `mysqld` com a opção `--skip-symbolic-links`. Para determinar se o seu sistema suporta links simbólicos de arquivos `.sym`, verifique o valor da variável de sistema `have_symlink` usando esta instrução:
