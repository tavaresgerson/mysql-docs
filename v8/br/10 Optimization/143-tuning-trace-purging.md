...
*`offset`* é um inteiro assinado (padrão `-1`); *`limit`* é um inteiro positivo (padrão `1`). Uma instrução `SET` desse tipo tem os seguintes efeitos:

* Todos os rastros previamente armazenados são apagados da memória.
* Uma subsequente `SELECT` da tabela `OPTIMIZER_TRACE` retorna os primeiros *`limit`* rastros dos rastros armazenados mais antigos de *`offset`* (se *`offset`* >= 0), ou os primeiros *`limit`* rastros dos rastros armazenados mais novos de *`-offset`* (se *`offset`* < 0).
...
Um valor positivo para *`offset`* pode ser útil quando se sabe que as subinstruções interessantes são as primeiras em uma rotina armazenada.
...
Usar `optimizer_trace_offset` e `optimizer_trace_limit`, que são restrições no nível do produtor do rastreamento, oferece melhor (maior) velocidade e (menos) uso de memória do que definir offsets ou limites no nível do consumidor do rastreamento (SQL) com `SELECT * FROM OPTIMIZER_TRACE LIMIT limit OFFSET offset`, o que não economiza quase nada.