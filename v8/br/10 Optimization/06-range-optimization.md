...
* Método de Acesso de Faixa de Escaneamento Saltador
* Otimização de Faixa de Expressões de Construtores de Linhas
* Limitação do Uso de Memória para Otimização de Faixa

##### Método de Acesso de Faixa para Índices de Uma Parte

Para um índice de uma parte, os intervalos de valores de índice podem ser convenientemente representados por condições correspondentes na cláusula `WHERE`, denotados como condições de faixa em vez de “intervalos”.
...
Aqui estão alguns exemplos de consultas com condições de faixa na cláusula `WHERE`:
...
1. Comece com a cláusula `WHERE` original:
   ...
   (key1 < 'uux' AND key1 > 'z')
   ```
2. Remova `nonkey = 4` e `key1 LIKE '%b'` porque não podem ser usados para uma varredura de intervalo. A maneira correta de removê-las é substituí-las por `TRUE`, para que não percamos nenhuma linha que corresponda ao fazer a varredura de intervalo. Substituí-las por `TRUE` resulta em:
...
3. Reduza as condições que são sempre verdadeiras ou falsas:
   ...
   * `(key1 < 'uux' AND key1 > 'z')` é sempre falso
   ...
   Removendo as constantes `TRUE` e `FALSE` desnecessárias, resulta em:
...
O algoritmo de extração de condições de intervalo pode lidar com construções `AND`/`OR` aninhadas de profundidade arbitrária, e sua saída não depende da ordem em que as condições aparecem na cláusula `WHERE`.

O MySQL não suporta a junção de múltiplos intervalos para o método de acesso `range` para índices espaciais. Para contornar essa limitação, você pode usar uma `UNION` com declarações `SELECT` idênticas, exceto que coloque cada predicado espacial em uma `SELECT` diferente.

##### Método de Acesso de Intervalo para Índices de Múltiplas Partes
...
Em contraste, a condição `key_part3 = 'abc'` não define um único intervalo e não pode ser usada pelo método de acesso de intervalo.
...
* Se condições que cobrem conjuntos de linhas contidas dentro de intervalos forem combinadas com `OR`, elas formam uma condição que cobre um conjunto de linhas contidas dentro da união de seus intervalos. Se as condições forem combinadas com `AND`, elas formam uma condição que cobre um conjunto de linhas contidas dentro da interseção de seus intervalos. Por exemplo, para esta condição em um índice de duas partes:
...
Neste exemplo, o intervalo na primeira linha usa uma parte chave para o limite esquerdo e duas partes chave para o limite direito. O intervalo na segunda linha usa apenas uma parte chave. A coluna `key_len` na saída `EXPLAIN` indica o comprimento máximo do prefixo de chave usado.
...
Cada expressão é verdadeira se *`col_name`* é igual a vários valores. Essas comparações são comparações de intervalo de igualdade (onde o “intervalo” é um único valor). O otimizador estima o custo de ler as linhas qualificadoras para comparações de intervalo de igualdade da seguinte forma:
...
A variável de sistema `eq_range_index_dive_limit` permite que você configure o número de valores em que o otimizador troca de uma estratégia de estimativa de linha para outra. Para permitir o uso de mergulhas no índice para comparações de até *`N`* faixas de igualdade, defina `eq_range_index_dive_limit` para *`N`* + 1. Para desabilitar o uso de estatísticas e sempre usar mergulhas no índice, independentemente de *`N`*, defina `eq_range_index_dive_limit` para 0.

Para atualizar as estatísticas do índice da tabela para as melhores estimativas, use `ANALYZE TABLE`.

Antes do MySQL 8.4, não há maneira de pular o uso de mergulhas no índice para estimar a utilidade do índice, exceto usando a variável de sistema `eq_range_index_dive_limit`. No MySQL 8.4, o pular mergulhas no índice é possível para consultas que satisfazem todas essas condições:

* A consulta é para uma única tabela, não para uma junção em múltiplas tabelas.
* Há uma dica de índice `FORCE INDEX` de um único índice. A ideia é que, se o uso do índice for forçado, não há nada a ganhar com o overhead adicional de realizar mergulhas no índice.
* O índice não é único e não é um índice `FULLTEXT`.
* Não há subconsulta.
* Não há cláusula `DISTINCT`, `GROUP BY` ou `ORDER BY`.

Para `EXPLAIN FOR CONNECTION`, a saída muda da seguinte forma se as mergulhas no índice forem pular:
...
Sem `FOR CONNECTION`, a saída do `EXPLAIN` não muda quando as mergulhas no índice são pular.

Após a execução de uma consulta para a qual os saltos de índice são ignorados, a linha correspondente na tabela do Schema de Informações `OPTIMIZER_TRACE` contém um valor `index_dives_for_range_access` de `skipped_due_to_force_index`.

##### Método de Acesso por Escaneio de Faixa
...
Um escaneamento por faixa é mais eficiente do que um escaneamento completo de índice, mas não pode ser usado neste caso porque não há condição no `f1`, a primeira coluna do índice. O otimizador pode realizar múltiplos escaneamentos por faixa, um para cada valor de `f1`, usando um método chamado Escaneamento por Saltos que é semelhante ao Escaneamento de Índice Solto (veja a Seção 10.2.1.17, “Otimização por GROUP BY”):

1. Saltar entre os valores distintos da primeira parte do índice, `f1` (o prefixo do índice).
2. Realizar um escaneamento sub-faixa em cada valor distinto do prefixo para a condição `f2 > 40` na parte restante do índice.

Para o conjunto de dados mostrado anteriormente, o algoritmo opera da seguinte forma:

1. Obtenha o primeiro valor distinto da primeira parte da chave (`f1 = 1`).
2. Construa a faixa com base nas primeiras e segundas partes da chave (`f1 = 1 e f2 > 40`).
3. Realize um escaneamento por faixa.
4. Obtenha o próximo valor distinto da primeira parte da chave (`f1 = 2`).
5. Construa a faixa com base nas primeiras e segundas partes da chave (`f1 = 2 e f2 > 40`).
6. Realize um escaneamento por faixa.
...
* Os predicados em `A_1, ..., A_k` devem ser predicados de igualdade e devem ser constantes. Isso inclui o operador `IN()`.
...
* Condições em colunas D são permitidas. As condições em D devem estar em conjunto com a condição de intervalo em C.

O uso do Scan Descartável é indicado na saída do `EXPLAIN` da seguinte forma:

* `Usando índice para o método de acesso descartável` na coluna `Extra` indica que o método de acesso descartável do índice livre é usado.
* Se o índice puder ser usado para o Scan Descartável, o índice deve ser visível na coluna `possible_keys`.

O uso do Scan Descartável é indicado na saída da traça do otimizador por um elemento de `"`skip scan"`` dessa forma:
...
Você também pode ver um elemento `"best_skip_scan_summary"`. Se o Scan Descartável for escolhido como a melhor variante de acesso de intervalo, um `"chosen_range_access_summary"` é escrito. Se o Scan Descartável for escolhido como o método de acesso geral melhor, um elemento `"best_access_path"` está presente.

O uso do Scan Descartável está sujeito ao valor da bandeira `skip_scan` da variável de sistema `optimizer_switch`. Veja a Seção 10.9.2, “Otimizações Switchables”. Por padrão, essa bandeira é `on`. Para desabilitá-la, defina `skip_scan` para `off`.

Além de usar a variável de sistema `optimizer_switch` para controlar o uso do Scan Descartável pelo otimizador em nível de sessão, o MySQL suporta dicas do otimizador para influenciar o otimizador em nível de declaração. Veja a Seção 10.9.3, “Dicas do Otimizador”.

##### Otimização de Intervalo de Expressões de Construtores de Linhas


O otimizador é capaz de aplicar o método de varredura de intervalo para consultas dessa forma:

```
SELECT ... FROM t1 WHERE ( col_1, col_2 ) IN (( 'a', 'b' ), ( 'c', 'd' ));
```

Anteriormente, para que as varreduras de intervalo fossem usadas, era necessário escrever a consulta da seguinte forma:
...
* São usadas apenas preditivas `IN()`, não `NOT IN()`.
* No lado esquerdo da preditiva `IN()`, o construtor de linha contém apenas referências de coluna.
* No lado direito da preditiva `IN()`, os construtores de linha contêm apenas constantes de tempo de execução, que são literais ou referências de coluna locais que são vinculadas a constantes durante a execução.
* No lado direito da preditiva `IN()`, há mais de um construtor de linha.

Para obter mais informações sobre o otimizador e os construtores de linha, consulte a Seção 10.2.1.22, “Otimização da Expressão do Construtor de Linha”

##### Limitar o Uso de Memória para a Otimização de Intervalo
...
* Com um valor maior que 0, o otimizador acompanha a memória consumida ao considerar o método de acesso de intervalo. Se o limite especificado estiver prestes a ser excedido, o método de acesso de intervalo é abandonado e outros métodos, incluindo uma varredura completa da tabela, são considerados em vez disso. Isso pode ser menos ótimo. Se isso acontecer, o seguinte aviso ocorre (onde *`N`* é o valor atual de `range_optimizer_max_mem_size`):
...
* Para uma consulta simples, como a seguinte, onde há uma chave candidata para o método de acesso ao intervalo, cada predicado combinado com `OR` usa aproximadamente 230 bytes:
