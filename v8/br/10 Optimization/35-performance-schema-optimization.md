...
Para ver o plano de execução de uma consulta do Schema de Desempenho e se ela usa algum índice, use `EXPLAIN`:
...
* Evita totalmente o custo de manutenção normalmente incorrido em tabelas que passam por atualizações frequentes.
* Reduz, desde o início da execução da consulta, a quantidade de dados recuperados. Para condições nas colunas indexadas, o Schema de Desempenho retorna eficientemente apenas as linhas da tabela que satisfazem as condições da consulta. Sem um índice, o Schema de Desempenho retornaria todas as linhas da tabela, exigindo que o otimizador avaliasse as condições contra cada linha para produzir o resultado final.
...
Para obter informações adicionais sobre índices de hash, consulte a Seção 10.3.9, “Comparação de Índices B-Tree e de Hash”.