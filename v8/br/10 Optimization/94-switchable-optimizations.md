...
A variável de sistema `optimizer_switch` permite controlar o comportamento do otimizador. Seu valor é um conjunto de flags, cada um dos quais tem um valor de `on` ou `off` para indicar se o comportamento correspondente do otimizador está habilitado ou desabilitado. Esta variável tem valores globais e de sessão e pode ser alterada em tempo de execução. O valor padrão global pode ser definido no início do servidor.

Para ver o conjunto atual de flags de otimização, selecione o valor da variável:
...
<table><thead><tr> <th>Sintaxe do Comando</th> <th>Significado</th> </tr></thead><tbody><tr> <td><code>default</code></td> <td>Reinicie toda otimização para seu valor padrão</td> </tr><tr> <td><code><em><code>opt_name</code></em>=default</code></td> <td>Defina a otimização nomeada para seu valor padrão</td> </tr><tr> <td><code><em><code>opt_name</code></em>=off</code></td> <td>Desabilite a otimização nomeada</td> </tr><tr> <td><code><em><code>opt_name</code></em>=on</code></td> <td>Habilite a otimização nomeada</td> </tr></tbody></table>

A ordem dos comandos no valor não importa, embora o comando `default` seja executado primeiro, se presente. Definir uma *`opt_name`* como `default` a define para o valor `on` ou `off` que é seu valor padrão. Especificar qualquer *`opt_name`* mais de uma vez no valor não é permitido e causa um erro. Quaisquer erros no valor fazem com que a atribuição falhe com um erro, deixando o valor de `optimizer_switch` inalterado.

A lista a seguir descreve os nomes de flags de *`opt_name`* permitidos, agrupados por estratégia de otimização:

* Flags de Acesso a Chave em Bateladas
...
Para mais informações, consulte a Seção 10.2.1.12, “Conjuntos de Acesso a Chave em Bloco Aninhado e Acesso a Chave em Bloco”.
* Bandeiras de Acesso a Chave em Bloco Aninhado
...
* Bandeiras de Empurrão de Condição Derivada

  +  `derived_condition_pushdown` (padrão `on`)

    Controla o empurrão de condição derivada.
...
* Bandeiras de Fusão de Tabelas Derivadas
  ...
  +  `derived_merge` (padrão `on`)
    ...
    Controla a fusão de tabelas e vistas derivadas no bloco da consulta externa.

  A bandeira `derived_merge` controla se o otimizador tenta fusar tabelas derivadas, referências de vistas e expressões de tabela comuns no bloco da consulta externa, assumindo que nenhuma outra regra impeça a fusão; por exemplo, uma diretiva `ALGORITHM` para uma vista tem precedência sobre o ajuste `derived_merge`. Por padrão, a bandeira está ativada para permitir a fusão.

  Para mais informações, consulte a Seção 10.2.2.4, “Otimização de Tabelas, Referências de Vistas e Expressões de Tabela Derivadas com Fusão ou Materialização”.
* Bandeiras de Empurrão de Condição do Motor

  +  `engine_condition_pushdown` (padrão `on`)

    Controla o empurrão de condição do motor.

  Para mais informações, consulte a Seção 10.2.1.5, “Otimização de Empurrão de Condição do Motor”.
* Bandeiras de Junção Hash

  +  `hash_join` (padrão `on`)

    Não tem efeito no MySQL 8.4. Use a bandeira `block_nested_loop` em vez disso.

Para obter mais informações, consulte a Seção 10.2.1.4, “Otimização da Conjunção de Índices”.
* Finais de Pushdown de Condição de Índex

  +  `index_condition_pushdown` (padrão `on`)

    Controla a otimização de pushdown de condição de índice.

  Para obter mais informações, consulte a Seção 10.2.1.6, “Otimização da Otimização de Pushdown de Condição de Índex”.
* Finais de Extensão de Índex

  +  `use_index_extensions` (padrão `on`)

    Controla o uso de extensões de índice.

  Para obter mais informações, consulte a Seção 10.3.10, “Uso de Extensões de Índex”.
* Finais de Fusão de Índex

  +  `index_merge` (padrão `on`)

    Controla todas as otimizações de fusão de índice.
  +  `index_merge_intersection` (padrão `on`)

    Controla a otimização de acesso de fusão de índice interseção.
  +  `index_merge_sort_union` (padrão `on`)

    Controla a otimização de acesso de fusão de índice ordenação-união.
  +  `index_merge_union` (padrão `on`)

    Controla a otimização de acesso de fusão de índice união.

  Para obter mais informações, consulte a Seção 10.2.1.3, “Otimização de Fusão de Índex”.
* Finais de Visibilidade de Índex

  +  `use_invisible_indexes` (padrão `off`)

    Controla o uso de índices invisíveis.

  Para obter mais informações, consulte a Seção 10.3.12, “Índices Invisíveis”.
* Finais de Otimização de Limite
...
Para obter mais informações e exemplos, consulte a Seção 10.2.1.19, “Otimização da Consulta de Limite”.
* Finais de Operações de Conjunto
...
Para mais informações, consulte a Seção 10.2.1.11, “Otimização da Leitura de Múltiplos Intervalos”.
* Finais de Join Semijoin

  +  `duplicateweedout` (padrão `on`)

    Controla a estratégia de Join Semijoin Duplo.
  +  `firstmatch` (padrão `on`)

    Controla a estratégia de Join Semijoin Primeiro Match.
  +  `loosescan` (padrão `on`)

    Controla a estratégia de Join Semijoin LooseScan (não confundir com Loose Index Scan para `GROUP BY`).
  +  `semijoin` (padrão `on`)

    Controla todas as estratégias de Join Semijoin.

Isso também se aplica à otimização de antijoin.

Os sinais `semijoin`, `firstmatch`, `loosescan` e `duplicateweedout` permitem o controle sobre as estratégias de Join Semijoin. O sinal `semijoin` controla se os Join Semijoin são usados. Se estiver definido como `on`, os sinais `firstmatch` e `loosescan` permitem um controle mais fino sobre as estratégias de Join Semijoin permitidas.

Se a estratégia de Join Semijoin `duplicateweedout` for desativada, ela não será usada, a menos que todas as outras estratégias aplicáveis também sejam desativadas.

Se `semijoin` e `materialização` estiverem ambos `on`, os Join Semijoin também usam materialização quando aplicável. Esses sinais estão `on` por padrão.

Para mais informações, consulte a Otimização de Predicados de Subconsultas IN e EXISTS com Transformações de Join Semijoin.
* Finais de Operações de Conjunto
...
* Marcadores de varredura omitidos

  +  `skip_scan` (padrão `on`)

    Controla o uso do método de acesso de varredura omitida.
  Para mais informações, consulte o método de acesso de intervalo de varredura omitida.
* Marcadores de materialização de subconsultas

  +  `materialization` (padrão `on`)

    Controla a materialização (incluindo materialização de junção parcial).
  +  `subquery_materialization_cost_based` (padrão `on`)

    Use a escolha de materialização baseada no custo.
  O sinalizador `materialization` controla se a materialização de subconsultas é usada. Se `semijoin` e `materialization` estiverem ambos `on`, as junções parciais também usam materialização quando aplicável. Esses sinais são `on` por padrão.
  O sinalizador `subquery_materialization_cost_based` permite o controle sobre a escolha entre a materialização de subconsultas e a transformação de subconsultas `IN` para `EXISTS`. Se o sinalizador estiver `on` (o padrão), o otimizador realiza uma escolha baseada no custo entre a materialização de subconsultas e a transformação de subconsultas `IN` para `EXISTS` se qualquer um dos métodos puder ser usado. Se o sinalizador estiver `off`, o otimizador escolhe a materialização de subconsultas em detrimento da transformação de subconsultas `IN` para `EXISTS`.
...
- A consulta pai não define uma variável de usuário, pois reescrevê-la pode afetar a ordem de execução, o que pode levar a resultados inesperados se a variável for acessada mais de uma vez na mesma consulta.
- A subconsulta não deve ser correlacionada, ou seja, não deve referenciar uma coluna de uma tabela na consulta externa, nem conter um agregado que seja avaliado na consulta externa.
...
O valor padrão para essa flag é `off`, pois, na maioria dos casos, habilitar essa otimização não produz nenhuma melhoria perceptível no desempenho (e, em muitos casos, pode até fazer com que as consultas funcionem mais lentamente), mas você pode habilitar a otimização definindo a flag `subquery_to_derived` para `on`. Ela é principalmente destinada ao uso em testes.
...
Verificar e simplificar o resultado de `SHOW WARNINGS` após executar `EXPLAIN` nesta consulta mostra que, quando a flag `subquery_to_derived` é habilitada, `SELECT * FROM t1 WHERE t1.b < 0 OR t1.a IN (SELECT t2.a + 1 FROM t2)` é reescrita de uma forma semelhante à que é mostrada aqui:
...
Se executarmos `SHOW WARNINGS` após executarmos `EXPLAIN` na consulta `SELECT * FROM t1 WHERE t1.b < 0 OR EXISTS(SELECT * FROM t2 WHERE t2.a = t1.a + 1)` quando `subquery_to_derived` estiver habilitado, e simplificarmos a segunda linha do resultado, vemos que ela foi reescrita de uma forma que se assemelha a:
