...
* `key_len` passa de 4 bytes para 8 bytes, indicando que as consultas de chave usam as colunas `d` e `i1`, e não apenas `d`.
* O valor `ref` muda de `const` para `const,const` porque a consulta de chave usa duas partes da chave, e não uma.
* O número de `rows` diminui de 5 para 1, indicando que o `InnoDB` deve examinar menos linhas para produzir o resultado.
* O valor `Extra` muda de `Using where; Using index` para `Using index`. Isso significa que as linhas podem ser lidas usando apenas o índice, sem consultar as colunas da linha de dados.
...
As declarações anteriores incluem `FLUSH TABLES` e  `FLUSH STATUS` para limpar o cache da tabela e os contadores de status.
...
A  fala `use_index_extensions` da variável de sistema `optimizer_switch` permite controlar se o otimizador leva em consideração as colunas da chave primária ao determinar como usar os índices secundários de uma tabela `InnoDB`. Por padrão, `use_index_extensions` está habilitado. Para verificar se a desabilitação do uso de extensões de índice pode melhorar o desempenho, use esta declaração:
