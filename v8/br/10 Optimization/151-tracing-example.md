...
Para referência, o registro completo é mostrado ininterrupto no final desta seção.


:::

Agora podemos examinar o registro, cuja primeira coluna (`QUERY`), contendo a declaração original a ser rastreada, é mostrada aqui:
...
Isso pode ser útil quando vários registros são armazenados.
...
Isso é seguido pela preparação da junção para o primeiro (e único) `SELECT` na declaração sendo rastreada, como mostrado aqui:
...
A saída mostrada agora exibe a consulta como ela é usada para preparar a junção; todas as colunas (campos) foram resolvidas para suas bases de dados e tabelas, e cada `SELECT` é anotado com um número de sequência, o que pode ser útil ao estudar subconsultas.

A próxima parte do registro mostra como a junção é otimizada, começando com o processamento de condições:
...
Um acesso `ref` que rejeita `NULL` foi identificado: nenhum `NULL` em `test.alias1.col_int_key` pode ter uma correspondência. (Observe que poderia ter uma correspondência, se o operador fosse um igual `=>` seguro para nulos).

Em seguida, para cada tabela na consulta, estimamos o custo e o número de registros retornados por uma varredura de tabela ou um acesso de intervalo.
...
Como mostrado na segunda parte da análise de intervalo, não é possível usar `GROUP_MIN_MAX` porque aceita apenas uma tabela, e temos duas na junção. Isso significa que nenhum acesso de intervalo é possível.

O otimizador estima que a leitura da primeira tabela e a aplicação de quaisquer condições necessárias a ela gerem 20 linhas:
...
Para `alias2`, escolhemos o acesso `ref` à chave primária em vez de uma varredura da tabela, porque o número de registros retornados por esta última (75) é muito maior do que o retornado pelo acesso `ref` (1), como mostrado aqui:
...
Agora que a ordem das tabelas está fixa, podemos dividir a condição `WHERE` em partes que podem ser testadas precocemente (empilhamento das condições na árvore de junção):
...
Esta condição pode ser testada em linhas de `alias1` sem ler linhas de `alias2`.
...
Como a cláusula `WHERE` contém `alias2.pk=alias1.col_int_key`, a ordenação por ambas as colunas é desnecessária; podemos ordenar apenas pela primeira coluna, uma vez que a segunda coluna é sempre igual a ela.
...
A cláusula `ORDER BY` mais curta (que não é visível na saída de `EXPLAIN`) pode ser implementada como uma varredura de índice, pois usa apenas uma coluna de uma tabela.
...
Todos os traços têm a mesma estrutura básica. Se uma instrução usa subconsultas, pode haver múltiplas preparações, otimizações e execuções, além de transformações específicas de subconsultas.

O traço completo é mostrado aqui:
