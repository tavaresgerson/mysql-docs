...
17.16.1 Monitoramento do progresso da alteração de tabelas do InnoDB usando o Gerenciador de Desempenho

17.16.2 Monitoramento de Espera de Mutex InnoDB Usando o Schema de Desempenho
...
- Você deve ter uma noção geral de como usar o recurso do Schema de Desempenho. Por exemplo, você deve saber como habilitar instrumentos e consumidores e como consultar as tabelas `performance_schema` para recuperar dados. Para uma visão geral introdutória, consulte a Seção 29.1, “Início Rápido do Schema de Desempenho”.
...
- Você deve estar familiarizado com os instrumentos do Schema de Desempenho disponíveis para `InnoDB`. Para visualizar os instrumentos relacionados a `InnoDB`, você pode consultar a tabela `setup_instruments` para nomes de instrumentos que contenham '`innodb`'.
  ...
  Para obter informações adicionais sobre os objetos instrumentados `InnoDB`, você pode consultar as tabelas de instâncias do Schema de Desempenho, que fornecem informações adicionais sobre os objetos instrumentados. As tabelas de instâncias relevantes para `InnoDB` incluem:
  ...
  Os mútuos e bloqueios de escrita relacionados ao conjunto de buffers `InnoDB` não estão incluídos nesta cobertura; o mesmo se aplica ao resultado do comando `SHOW ENGINE INNODB MUTEX`.

  Por exemplo, para visualizar informações sobre os objetos de arquivo `InnoDB` instrumentados vistos pelo Schema de Desempenho ao executar a instrumentação de E/S de arquivo, você pode emitir a seguinte consulta:
...
- Você deve estar familiarizado com as tabelas `performance_schema` que armazenam dados de eventos `InnoDB`. As tabelas relevantes para eventos relacionados a `InnoDB` incluem:
  ...
  - Tabelas de eventos de estágio, que armazenam dados de eventos para as operações de `InnoDB` `ALTER TABLE` e carga do pool de buffers. Para mais informações, consulte a Seção 17.16.1, “Monitoramento do progresso da alteração da tabela para tabelas InnoDB usando o Gerenciador de Desempenho”, e Monitoramento do progresso da carga do pool de buffers usando o Gerenciador de Desempenho.
