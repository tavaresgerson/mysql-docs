...
- Para bloqueios de leituras (`SELECT` com `FOR UPDATE` ou `FOR SHARE`), as declarações `UPDATE` e `DELETE` dependem dos bloqueios que são tomados, dependendo se a declaração usa um índice único com uma condição de pesquisa única ou uma condição de pesquisa de tipo intervalo.
  ...
  - Para outras condições de pesquisa e para índices não únicos, `InnoDB` bloqueia o intervalo do índice pesquisado, usando bloqueios de lacuna ou bloqueios de próxima chave para bloquear inserções por outras sessões nos intervalos cobertos pelo intervalo. Para informações sobre bloqueios de lacuna e bloqueios de próxima chave, consulte a Seção 17.7.1, “Bloqueio InnoDB”.

- Para registros de índice que encontrem `SELECT ... FOR UPDATE`, os blocos `SELECT ... FOR SHARE` impedem que outras sessões façam `SELECT ... FOR SHARE` ou leiam em certos níveis de isolamento de transação. Leitura consistente ignora quaisquer bloqueios definidos nos registros que existem na visualização de leitura.
...
- `DELETE FROM ... WHERE ...` define um bloqueio de próxima chave exclusivo em cada registro que a pesquisa encontra. No entanto, apenas um bloqueio de registro de índice é necessário para declarações que bloqueiam linhas usando um índice único para buscar uma linha única.
...
- `INSERT INTO T SELECT ... FROM S WHERE ...` define um bloqueio de registro de índice exclusivo (sem bloqueio de lacuna) em cada linha inserida em `T`. Se o nível de isolamento de transação for `READ COMMITTED`, `InnoDB` faz a busca em `S` como uma leitura consistente (sem bloqueios). Caso contrário, `InnoDB` define bloqueios compartilhados de próxima chave em linhas de `S`. `InnoDB` deve definir bloqueios no último caso: Durante a recuperação de avanço progressivo usando um log binário baseado em instruções, cada instrução SQL deve ser executada exatamente da mesma maneira que foi feita originalmente.

  O `CREATE TABLE ... SELECT ...` executa o `SELECT` com bloqueios de chave compartilhada ou como leitura consistente, como no caso do `INSERT ... SELECT`.

  Quando um `SELECT` é usado nas construções `REPLACE INTO t SELECT ... FROM s WHERE ...` ou `UPDATE t ... WHERE col IN (SELECT ... FROM s ...)`, o `InnoDB` define bloqueios de chave compartilhada para as linhas da tabela `s`.
...
- `InnoDB` define um bloqueio exclusivo no final do índice associado à coluna `AUTO_INCREMENT` ao inicializar uma coluna `AUTO_INCREMENT` especificada anteriormente em uma tabela.
  ...
  Com `innodb_autoinc_lock_mode=0`, `InnoDB` usa um modo especial de bloqueio da tabela `AUTO-INC` onde o bloqueio é obtido e mantido até o final da declaração SQL atual (e não até o final de toda a transação) ao acessar o contador de autoincremento. Outros clientes não podem inserir na tabela enquanto o bloqueio da tabela `AUTO-INC` estiver sendo mantido. O mesmo comportamento ocorre para as inserções em lote com `innodb_autoinc_lock_mode=1`. Os bloqueios de nível de tabela `AUTO-INC` não são usados com `innodb_autoinc_lock_mode=2`. Para mais informações, consulte a Seção 17.6.1.6, “Tratamento de AUTO\_INCREMENT no InnoDB”.

  `InnoDB` recupera o valor de uma coluna `AUTO_INCREMENT` previamente inicializada sem definir nenhum bloqueio.

- Se uma restrição `FOREIGN KEY` for definida em uma tabela, qualquer inserção, atualização ou exclusão que exija que a condição da restrição seja verificada estabelece bloqueios compartilhados em nível de registro nos registros que ela examina para verificar a restrição. `InnoDB` também estabelece esses bloqueios no caso de a restrição falhar.

- `LOCK TABLES` define bloqueios de tabela, mas é a camada MySQL superior à `InnoDB` que define esses bloqueios. `InnoDB` está ciente dos bloqueios de tabela se `innodb_table_locks = 1` (o padrão) e `autocommit = 0` estiverem ativados, e a camada MySQL acima de `InnoDB` sabe sobre bloqueios em nível de linha.

  Caso contrário, a detecção automática de deadlock do `InnoDB` não consegue detectar deadlocks envolvendo tais bloqueios de tabela. Além disso, como, neste caso, a camada superior do MySQL não conhece os bloqueios de nível de linha, é possível obter um bloqueio de tabela em uma tabela onde outra sessão atualmente tem bloqueios de nível de linha. No entanto, isso não coloca em risco a integridade da transação, conforme discutido na Seção 17.7.5.2, “Detecção de Deadlock”.

- `LOCK TABLES` adquire dois bloqueios em cada tabela se `innodb_table_locks=1` (o padrão). Além de um bloqueio de tabela na camada MySQL, ele também adquire um bloqueio de tabela `InnoDB`. Para evitar adquirir bloqueios de tabela `InnoDB`, configure `innodb_table_locks=0`. Se nenhum bloqueio de tabela `InnoDB` for adquirido, `LOCK TABLES` será concluído mesmo que alguns registros das tabelas estejam sendo bloqueados por outras transações.

  No MySQL 8.0, `innodb_table_locks=0` não tem efeito para tabelas bloqueadas explicitamente com `LOCK TABLES ... WRITE`. Ele tem efeito para tabelas bloqueadas para leitura ou escrita por `LOCK TABLES ... WRITE` implicitamente (por exemplo, por meio de gatilhos) ou por `LOCK TABLES ... READ`.

- Todas as `InnoDB` bloqueadas por uma transação são liberadas quando a transação é confirmada ou abortada. Portanto, não faz muito sentido invocar `LOCK TABLES` em tabelas `InnoDB` no modo `autocommit=1` porque os bloqueados `InnoDB` da tabela adquirida seriam liberados imediatamente.

- Você não pode bloquear tabelas adicionais no meio de uma transação porque `LOCK TABLES` executa um `COMMIT` e `UNLOCK TABLES` implícitos.
