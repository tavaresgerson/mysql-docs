...
O status do InnoDB contém detalhes do último deadlock. Para deadlocks frequentes, habilite a variável global `innodb_print_all_deadlocks`. Isso adiciona informações sobre deadlocks ao log de erros.
...
Em seguida, o cliente B inicia uma transação e seleciona uma linha em Aves em modo de compartilhamento:
...
O Schema de Desempenho mostra as bloqueadas após as duas instruções SELECT:
...
O InnoDB usa apenas IDs de transação sequenciais quando uma transação tenta modificar o banco de dados. Portanto, o ID de transação anterior de leitura somente muda de 421291106148352 para 43260.
...
O InnoDB desfaz a transação que causou o impasse. A primeira atualização, do Cliente B, agora pode prosseguir.

O esquema de informações contém o número de bloqueios:
...
O status do InnoDB contém as seguintes informações sobre o impasse e as transações. Também mostra que o ID de transação somente de leitura 421291106147544 muda para o ID de transação sequencial 43261.
