...
Com o nível de isolamento `READ COMMITTED`, cada leitura consistente dentro de uma transação cria e lê seu próprio instantâneo fresco.
...
Suponha que você esteja executando o nível de isolamento padrão `REPEATABLE READ`. Quando você emite uma leitura consistente (ou seja, uma declaração comum `SELECT`), `InnoDB` atribui um ponto de tempo para sua transação de acordo com o qual sua consulta vê o banco de dados. Se outra transação excluir uma linha e confirmar após o seu ponto de tempo ter sido atribuído, você não verá que a linha foi excluída. Inserções e atualizações são tratadas de maneira semelhante.

Nota

O instantâneo do estado do banco de dados se aplica a declarações `SELECT` dentro de uma transação, não necessariamente a declarações DML. Se você inserir ou modificar algumas linhas e então confirmar essa transação, uma declaração `DELETE` ou `UPDATE` emitida de outra transação `REPEATABLE READ` concorrente poderia afetar aquelas linhas que foram confirmadas recentemente, mesmo que a sessão não possa consultá-las. Se uma transação atualizar ou excluir linhas confirmadas por outra transação, essas alterações se tornam visíveis para a transação atual. Por exemplo, você pode encontrar uma situação como a seguinte:
...
Você pode avançar seu ponto de tempo ao confirmar sua transação e, em seguida, fazer outro `SELECT` ou `START TRANSACTION WITH CONSISTENT SNAPSHOT`.
...
Com o nível de isolamento `READ COMMITTED`, cada leitura consistente dentro de uma transação cria e lê seu próprio instantâneo fresco. Com `FOR SHARE`, ocorre uma leitura de bloqueio: um `SELECT` bloqueia até que a transação que contém as linhas mais recentes termine (veja a Seção 17.7.2.4, “Leitura de Bloqueio”).
...
- Por padrão, o `InnoDB` usa trancas mais fortes para essas instruções e a parte `SELECT` funciona como `READ COMMITTED`, onde cada leitura consistente, mesmo dentro da mesma transação, define e lê seu próprio instantâneo fresco.

- Para realizar uma leitura não bloqueante nesses casos, defina o nível de isolamento da transação para `READ UNCOMMITTED` ou `READ COMMITTED` para evitar a definição de bloqueios nas linhas lidas da tabela selecionada.
