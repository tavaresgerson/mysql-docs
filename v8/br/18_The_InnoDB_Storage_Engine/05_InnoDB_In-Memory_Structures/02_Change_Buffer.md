...
A variável `innodb_change_buffering` controla a extensão em que o `InnoDB` realiza o bufferamento de alterações. Você pode habilitar ou desabilitar o bufferamento para inserções, operações de exclusão (quando os registros do índice são marcados inicialmente para exclusão) e operações de purga (quando os registros do índice são excluídos fisicamente). Uma operação de atualização é uma combinação de uma inserção e uma exclusão. O valor padrão do `innodb_change_buffering` é `all`.
...
- `purges`
  ...
  Ajuste as operações de exclusão física que ocorrem em segundo plano.
...
- `InnoDB` A saída padrão do monitor inclui informações sobre o status do buffer de alterações. Para visualizar os dados do monitor, execute a instrução `SHOW ENGINE INNODB STATUS`.

  ```
  mysql> SHOW ENGINE INNODB STATUS\G
  ```

  As informações de status do buffer estão localizadas sob o título `INSERT BUFFER AND ADAPTIVE HASH INDEX` e aparecem de forma semelhante ao seguinte:
  ...
  Para obter mais informações, consulte a Seção 17.17.3, “Saída do Monitor Padrão InnoDB e do Monitor de Bloqueio”.

- A tabela Schema de Informações `INNODB_METRICS` fornece a maioria dos pontos de dados encontrados na saída do Monitor Padrão `InnoDB`, além de outros pontos de dados. Para visualizar as métricas do buffer de alterações e uma descrição de cada uma, execute a seguinte consulta:

  ```
  mysql> SELECT NAME, COMMENT FROM INFORMATION_SCHEMA.INNODB_METRICS WHERE NAME LIKE '%ibuf%'\G
  ```

  Consulte a Seção 17.15.6, “Tabela de métricas do InnoDB INFORMATION\_SCHEMA”.

- A tabela Schema de Informações `INNODB_BUFFER_PAGE` fornece metadados sobre cada página no pool de buffer, incluindo páginas do índice do buffer de alterações e páginas do mapa de bits do buffer de alterações. As páginas do buffer de alterações são identificadas por `PAGE_TYPE`. `IBUF_INDEX` é o tipo de página para as páginas do índice do buffer de alterações, e `IBUF_BITMAP` é o tipo de página para as páginas do mapa de bits do buffer de alterações.
  ...
  Por exemplo, você pode consultar a tabela `INNODB_BUFFER_PAGE` para determinar o número aproximado de páginas `IBUF_INDEX` e `IBUF_BITMAP` como uma porcentagem das páginas do pool de buffer total.
  ...
  Para obter informações sobre outros dados fornecidos pela tabela `INNODB_BUFFER_PAGE`, consulte a Seção 28.4.2, “A tabela INFORMATION\_SCHEMA INNODB\_BUFFER\_PAGE”. Para informações sobre o uso relacionado, consulte a Seção 17.15.5, “Tabelas de Pool de Buffer do INFORMATION\_SCHEMA InnoDB”.
...
- O Schema de Desempenho fornece instrumentação de espera por mutex de buffer de alterações para monitoramento avançado de desempenho. Para visualizar a instrumentação do buffer de alterações, execute a seguinte consulta:
  ...
  Para obter informações sobre o monitoramento das espera dos mutses `InnoDB`, consulte a Seção 17.16.2, “Monitoramento das Espera dos Mutses InnoDB Usando o Schema de Desempenho”.
