...
- Os conjuntos de operações DML relacionadas são agrupados em transações ao serem delimitados com as instruções `START TRANSACTION` e `COMMIT`. Embora você não queira comprometer com muita frequência, também não quer emitir grandes lotes de instruções `INSERT`, `UPDATE` ou `DELETE` que executem por horas sem comprometer.

- Não use as instruções `LOCK TABLES`. O `InnoDB` pode lidar com múltiplas sessões lendo e escrevendo na mesma tabela ao mesmo tempo, sem comprometer a confiabilidade ou o alto desempenho. Para obter acesso exclusivo de escrita a um conjunto de linhas, use a sintaxe `SELECT ... FOR UPDATE` para bloquear apenas as linhas que você deseja atualizar.

- Ative a variável `innodb_file_per_table` ou use tabelas gerais para colocar os dados e índices das tabelas em arquivos separados, em vez do espaço de tabelas do sistema. A variável `innodb_file_per_table` é ativada por padrão.

- Avalie se seus padrões de dados e acesso se beneficiam das funcionalidades de compressão de tabelas ou páginas `InnoDB`. Você pode comprimir tabelas `InnoDB` sem sacrificar a capacidade de leitura/escrita.
