#### 17.8.3.1 Configurando o tamanho do pool de buffers do InnoDB

Você pode configurar o tamanho do pool de tampão `InnoDB` offline ou enquanto o servidor estiver em execução. O comportamento descrito nesta seção se aplica a ambos os métodos. Para obter informações adicionais sobre a configuração do tamanho do pool de tampão online, consulte Configurando o Tamanho do Pool de Tampão do InnoDB Online.

Ao aumentar ou diminuir `innodb_buffer_pool_size`, a operação é realizada em lotes. O tamanho do lote é definido pela opção de configuração `innodb_buffer_pool_chunk_size`, que tem um valor padrão de `128M`. Para mais informações, consulte Configurando o tamanho do lote do buffer do InnoDB.

O tamanho do pool de tampão deve sempre ser igual a ou múltiplo de `innodb_buffer_pool_chunk_size` \* `innodb_buffer_pool_instances`. Se você configurar `innodb_buffer_pool_size` para um valor que não é igual a ou múltiplo de `innodb_buffer_pool_chunk_size` \* `innodb_buffer_pool_instances`, o tamanho do pool de tampão será ajustado automaticamente para um valor que é igual a ou múltiplo de `innodb_buffer_pool_chunk_size` \* `innodb_buffer_pool_instances`.
...
`innodb_buffer_pool_chunk_size` pode ser aumentado ou diminuído em unidades de 1 MB (1048576 bytes), mas só pode ser modificado ao iniciar, em uma string de linha de comando ou em um arquivo de configuração do MySQL.
...
- Se o novo valor de `innodb_buffer_pool_chunk_size` \* `innodb_buffer_pool_instances` for maior que o tamanho atual do pool de buffers quando o pool de buffers for inicializado, `innodb_buffer_pool_chunk_size` será truncado para `innodb_buffer_pool_size` / `innodb_buffer_pool_instances`.

  Por exemplo, se o pool de buffers for inicializado com um tamanho de `2GB` (2147483648 bytes), instâncias de pool de buffers `4` e um tamanho de bloco de `1GB` (1073741824 bytes), o tamanho do bloco é truncado para um valor igual a `innodb_buffer_pool_size` / `innodb_buffer_pool_instances`, conforme mostrado abaixo:
...
A opção de configuração `innodb_buffer_pool_size` pode ser definida dinamicamente usando uma declaração `SET`, permitindo que você redimensione o pool de buffers sem precisar reiniciar o servidor. Por exemplo:
...
O tamanho do pool de tampão deve ser igual a ou múltiplo de `innodb_buffer_pool_chunk_size` \* `innodb_buffer_pool_instances`. Para alterar essas configurações de variáveis, é necessário reiniciar o servidor.

As transações e operações ativas realizadas por meio das APIs `InnoDB` devem ser concluídas antes de redimensionar o pool de buffers. Ao iniciar uma operação de redimensionamento, a operação só começa quando todas as transações ativas são concluídas. Uma vez que a operação de redimensionamento estiver em andamento, novas transações e operações que exigem acesso ao pool de buffers devem esperar até que a operação de redimensionamento termine. A exceção à regra é que o acesso concorrente ao pool de buffers é permitido enquanto o pool de buffers estiver sendo desfragmentado e páginas forem retiradas quando o tamanho do pool de buffers for reduzido. Uma desvantagem de permitir o acesso concorrente é que isso pode resultar em uma escassez temporária de páginas disponíveis enquanto as páginas estão sendo retiradas.
...
A variável `Innodb_buffer_pool_resize_status` reporta um valor de string que indica o progresso da redimensionamento do pool de buffers; por exemplo:
...
A partir do MyQL 8.0.31, você também pode monitorar uma operação de redimensionamento do pool de buffers online usando as variáveis de status `Innodb_buffer_pool_resize_status_code` e `Innodb_buffer_pool_resize_status_progress`, que retornam valores numéricos, ideais para monitoramento programático.

A variável de status `Innodb_buffer_pool_resize_status_code` reporta um código de status que indica o estágio de uma operação de redimensionamento do pool de tampão online. Os códigos de status incluem:
...
A variável de status `Innodb_buffer_pool_resize_status_progress` reporta um valor percentual que indica o progresso de cada estágio. O valor percentual é atualizado após cada instância do pool de buffers ser processada. À medida que o status (relatado por `Innodb_buffer_pool_resize_status_code`) muda de um status para outro, o valor percentual é redefinido para 0.

A consulta a seguir retorna um valor de string que indica o progresso da redimensionamento do pool de buffers, um código que indica a fase atual da operação e o progresso atual dessa fase, expresso como um valor percentual:
...
O progresso da redimensionamento do pool de buffers também é visível no log de erros do servidor. Este exemplo mostra as notas registradas ao aumentar o tamanho do pool de buffers:
...
A partir do MySQL 8.0.31, ao iniciar o servidor com `--log-error-verbosity=3`, são registrados dados adicionais no log de erros durante uma operação de redimensionamento do pool de buffers online. Os dados adicionais incluem os códigos de status reportados por `Innodb_buffer_pool_resize_status_code` e o valor de progresso percentual reportado por `Innodb_buffer_pool_resize_status_progress`.
