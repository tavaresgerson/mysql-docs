...
O modelo ACID é um conjunto de princípios de design de banco de dados que enfatizam aspectos da confiabilidade que são importantes para dados de negócios e aplicações críticas à missão. O MySQL inclui componentes como o motor de armazenamento `InnoDB` que aderem de perto ao modelo ACID, para que os dados não sejam corrompidos e os resultados não sejam distorcidos por condições excepcionais, como falhas de software e mau funcionamento do hardware. Quando você confia em recursos compatíveis com ACID, não precisa reinventar a roda da verificação de consistência e dos mecanismos de recuperação de falhas. Em casos em que você tem proteções de software adicionais, hardware ultra-confiável ou uma aplicação que pode tolerar uma pequena quantidade de perda de dados ou inconsistência, você pode ajustar as configurações do MySQL para trocar parte da confiabilidade ACID por maior desempenho ou capacidade de processamento.
...
O aspecto de **atomicity** do modelo ACID envolve principalmente as transações `InnoDB`. As funcionalidades relacionadas do MySQL incluem:
...
O aspecto de **consistência** do modelo ACID envolve principalmente o processamento interno `InnoDB` para proteger os dados contra falhas. As funcionalidades relacionadas do MySQL incluem:

- O buffer de dupla gravação `InnoDB`. Veja a Seção 17.6.4, “Buffer de Dupla Gravação”.

- `InnoDB` recuperação de falhas. Veja Recuperação de Falhas do InnoDB.

### Isolamento

O aspecto de **isolamento** do modelo ACID envolve principalmente as transações `InnoDB`, em particular o nível de isolamento que se aplica a cada transação. As funcionalidades relacionadas do MySQL incluem:

- A configuração `autocommit`.

- Níveis de isolamento de transações e a declaração `SET TRANSACTION`. Veja a Seção 17.7.2.1, “Níveis de Isolamento de Transações”.

- Os detalhes de nível baixo do bloqueio `InnoDB`. Os detalhes podem ser visualizados nas tabelas `INFORMATION_SCHEMA` (consulte a Seção 17.15.2, “Transações e Informações de Bloqueio do Schema de Informações InnoDB”) e nas tabelas do Schema de Desempenho `data_locks` e `data_lock_waits`.
