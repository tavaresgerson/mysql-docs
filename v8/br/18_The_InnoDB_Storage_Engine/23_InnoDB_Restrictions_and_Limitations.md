...
Para informações sobre como o `InnoDB` processa as declarações do `SELECT COUNT(*)`, consulte a descrição do `COUNT()` na Seção 14.19.1, “Descritores de função agregada”.
...
* Para as limitações associadas à importação de tabelas usando o recurso *Transportable Tablespaces*, consulte [Limitações de importação de tabelas][(innodb-statistics-estimation.html "17.8.10.2 Configuring Non-Persistent Optimizer Statistics Parameters")].
