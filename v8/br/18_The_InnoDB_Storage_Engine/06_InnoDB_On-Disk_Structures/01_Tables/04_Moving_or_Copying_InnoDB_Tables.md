#### 17.6.1.4 Movimentar ou Copiar Tabelas InnoDB
...
No Windows, `InnoDB` armazena sempre os nomes de bancos de dados e tabelas internamente em minúsculas. Para mover bancos de dados em formato binário do Unix para o Windows ou do Windows para o Unix, crie todos os bancos de dados e tabelas usando nomes em minúsculas. Uma maneira conveniente de realizar isso é adicionar a seguinte linha à seção `[mysqld]` do seu arquivo `my.cnf` ou `my.ini` antes de criar quaisquer bancos de dados ou tabelas:
...
Uma tabela que reside em um espaço de tabela por arquivo pode ser importada de outra instância do servidor MySQL ou de um backup usando o recurso *Espaço de tabela transportable*. Veja a Seção 17.6.1.3, “Importando tabelas InnoDB”.
...
2. Emita a declaração `ALTER TABLE` para excluir o arquivo atual `.ibd`:
...
4. Emita a declaração `ALTER TABLE` para dizer ao `InnoDB` que use o novo arquivo `.ibd` para a tabela:
...
Neste contexto, um backup de arquivo `.ibd` "limpo" é aquele para o qual são atendidas as seguintes exigências:
...
- O **mysqld** limpou todas as páginas modificadas do arquivo `.ibd` do pool de buffer para o arquivo.
...
2. Aguarde até que `SHOW ENGINE INNODB STATUS` mostre que não há transações ativas no banco de dados e que o estado da thread principal de `InnoDB` é `Waiting for server activity`. Em seguida, você pode fazer uma cópia do arquivo `.ibd`.
...
Para melhorar o desempenho deste método, desative `autocommit` ao importar dados. Realize um commit apenas após importar uma tabela inteira ou um segmento de uma tabela.
