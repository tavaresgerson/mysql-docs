...
Os clientes do **memcached** só conseguem acessar as tabelas `InnoDB` que estão registradas na tabela `innodb_memcache.containers`. Embora um DBA possa colocar restrições de acesso a essas tabelas, o acesso através das aplicações **memcached** não pode ser controlado. Por essa razão, o suporte SASL é fornecido para controlar o acesso às tabelas `InnoDB` associadas ao plugin `daemon_memcached`.
...
2. Construa as bibliotecas de compartilhamento do plugin `daemon_memcached` com capacidade SASL adicionando `ENABLE_MEMCACHED_SASL=1` às suas opções de **cmake**. O **memcached** também oferece suporte simples a senhas em texto claro, o que facilita o teste. Para habilitar o suporte simples a senhas em texto claro, especifique a opção de **cmake** `ENABLE_MEMCACHED_SASL_PWDB=1`.
...
3. Instale o plugin `daemon_memcached`, conforme descrito na Seção 17.20.3, “Configurando o Plugin InnoDB memcached”.
...
4. Configure um arquivo de nome de usuário e senha. (Este exemplo usa o suporte simples de senha em texto claro do **memcached**.)
   ...
   1. Crie um arquivo com um usuário chamado `testname` e defina a senha como `testpasswd`:
   ...
   3. Informe `memcached` que uma senha em texto claro é usada:
...
6. Para testar a configuração, use um cliente habilitado para SASL, como o libmemcached habilitado para SASL.
   ...
   Se você especificar um nome de usuário ou senha incorreto, a operação será rejeitada com uma mensagem `memcache error AUTHENTICATION FAILURE`. Nesse caso, examine a senha em texto claro definida no arquivo `memcached-sasl-db` para verificar se as credenciais fornecidas estão corretas.
