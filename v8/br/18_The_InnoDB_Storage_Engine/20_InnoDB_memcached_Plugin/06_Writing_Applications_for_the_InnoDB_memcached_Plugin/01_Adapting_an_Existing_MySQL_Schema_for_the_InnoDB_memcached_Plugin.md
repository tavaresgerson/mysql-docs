#### 17.20.6.1 Adaptando um Schema Existente do MySQL para o Plugin memcached do InnoDB
...
- O protocolo **memcached** passa valores numéricos como strings. Para armazenar valores numéricos na tabela subjacente `InnoDB`, para implementar contadores que podem ser usados em funções SQL, como `SUM()` ou `AVG()`, por exemplo:
  ...
  - Em qualquer consulta que realize operações aritméticas com valores de coluna, use a função `CAST()` para converter os valores de string para inteiro ou para outro tipo numérico. Por exemplo:
    ...
    Qualquer valor alfabético no conjunto de resultados é convertido em 0 pela chamada para `CAST()`. Ao usar funções como `AVG()`, que dependem do número de linhas no conjunto de resultados, inclua cláusulas `WHERE` para filtrar valores não numéricos.
...
- Para usar uma tabela existente com o plugin `daemon_memcached`, defina uma entrada para ela na tabela `innodb_memcache.containers`. Para tornar essa tabela a padrão para todas as solicitações do **memcached**, especifique um valor de `default` na coluna `name`, e então reinicie o servidor MySQL para que a mudança entre em vigor. Se você usar várias tabelas para diferentes classes de dados do **memcached**, configure várias entradas na tabela `innodb_memcache.containers` com valores de `name` de sua escolha, e então faça uma solicitação do **memcached** na forma de `get @@name` ou `set @@name` dentro do aplicativo para especificar a tabela a ser usada para solicitações subsequentes do **memcached**.

  Para um exemplo de uso de uma tabela diferente da tabela predefinida `test.demo_test`, consulte o Exemplo 17.13, “Usando sua própria tabela com um aplicativo InnoDB memcached”. Para o layout da tabela necessário, consulte a Seção 17.20.8, “Interna do Plugin InnoDB memcached”.

- Para usar múltiplos valores de coluna da tabela `InnoDB` com pares de chaves e valores do **memcached**, especifique os nomes das colunas separados por vírgula, ponto e vírgula, espaço ou caracteres de barril no campo `value_columns` da entrada `innodb_memcache.containers` para a tabela `InnoDB`. Por exemplo, especifique `col1,col2,col3` ou `col1|col2|col3` no campo `value_columns`.

  Concatenar os valores da coluna em uma única string usando o caractere de barramento como separador antes de passar a string para as chamadas **memcached** `add` ou `set`. A string é despacota automaticamente na coluna correta. Cada chamada `get` retorna uma única string contendo os valores da coluna, também delimitada pelo caractere de barramento. Você pode despacotar os valores usando a sintaxe da linguagem de aplicação apropriada.

**Exemplo 17.13: Usando sua própria tabela com um aplicativo InnoDB memcached**

Este exemplo mostra como usar sua própria tabela com um aplicativo Python de amostra que usa `memcached` para manipulação de dados.

O exemplo assume que o plugin `daemon_memcached` está instalado conforme descrito na Seção 17.20.3, “Configurando o Plugin InnoDB memcached”. Também assume que o sistema está configurado para executar um script Python que utiliza o módulo `python-memcache`.
...
2. Insira um registro na tabela `innodb_memcache.containers` para que o plugin `daemon_memcached` possa acessar a tabela `multicol`.
   ...
   - O registro `innodb_memcache.containers` da tabela `multicol` especifica um valor `name` de `'bbb'`, que é o identificador da tabela.
   ...
   - `key_columns` é definido como a coluna única `country`. A coluna `country` é definida como a chave primária na definição da tabela `multicol`.

   - Em vez de uma única coluna da tabela `InnoDB` para armazenar um valor de dados composto, os dados são divididos entre três colunas da tabela (`population`, `area_sq_km` e `drive_side`). Para acomodar múltiplas colunas de valores, uma lista separada por vírgula de colunas é especificada no campo `value_columns`. As colunas definidas no campo `value_columns` são as colunas usadas ao armazenar ou recuperar valores.

   - Os valores dos campos `flags`, `expire_time` e `cas_column` são baseados em valores usados na tabela de amostra `demo.test`. Esses campos geralmente não são significativos em aplicações que usam o plugin `daemon_memcached`, porque o MySQL mantém os dados sincronizados e não há necessidade de se preocupar com dados que expiram ou ficam desatualizados.

   - O campo `unique_idx_name_on_key` está definido como `PRIMARY`, que se refere ao índice primário definido na coluna única `country` na tabela `multicol`.
...
3. Copie o aplicativo de exemplo em Python em um arquivo. Neste exemplo, o script de exemplo é copiado para um arquivo chamado `multicol.py`.
   ...
   - Para garantir que o aplicativo use a tabela `multicol`, a função `switch_table()` é chamada, que realiza um pedido `get` ou `set` fictício usando a notação `@@`. O valor `name` na solicitação é `bbb`, que é o identificador da tabela `multicol` definido no campo `innodb_memcache.containers.name`.

     Um valor `name` mais descritivo pode ser usado em uma aplicação real. Este exemplo ilustra simplesmente que, nas solicitações `get @@...`, é especificado um identificador de tabela em vez do nome da tabela.

   - As funções de utilitário usadas para inserir e consultar dados demonstram como transformar uma estrutura de dados Python em valores separados por vírgula para enviar dados para o MySQL com solicitações `add` ou `set` e como desempacotar os valores separados por vírgula retornados por solicitações `get`. Esse processamento adicional é necessário apenas ao mapear um único valor do **memcached** para múltiplas colunas de tabelas do MySQL.
...
6. Consulte a tabela `multicol` para visualizar os dados inseridos pelo aplicativo Python de amostra. Os dados estão disponíveis para consultas MySQL, o que demonstra como os mesmos dados podem ser acessados usando SQL ou através de aplicativos (usando o conector MySQL ou API apropriados).
