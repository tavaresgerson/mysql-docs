...
17.20.6.2 Adaptando uma aplicação memcached para o plugin memcached do InnoDB
...
17.20.6.6 Executando declarações DML e DDL na tabela subjacente InnoDB
...
- Com o plugin `daemon_memcached`, em vez de muitos servidores tradicionais de **memcached** rodando em máquinas de baixo poder de processamento, você tem o mesmo número de servidores de **memcached** que servidores MySQL, rodando em máquinas relativamente de alto poder de processamento com armazenamento de disco e memória substanciais. Você pode reutilizar algum código existente que funciona com a API de **memcached**, mas é provável que seja necessária uma adaptação devido à configuração diferente do servidor.

- Os dados armazenados através do plugin `daemon_memcached` vão para as colunas `VARCHAR`, `TEXT` ou `BLOB` e precisam ser convertidos para realizar operações numéricas. Você pode realizar a conversão no lado do aplicativo ou usando a função `CAST()` nas consultas.
