### 17.15.5 Tabelas do Banco de Armazenamento do Banco de Dados do Schema de Informação InnoDB

As tabelas de pools de buffers `InnoDB` e `INFORMATION_SCHEMA` fornecem informações de status do pool de buffers e metadados sobre as páginas dentro do pool de buffers `InnoDB`.

As tabelas de pools de buffers `InnoDB` e `INFORMATION_SCHEMA` incluem as listadas abaixo:
...
- `INNODB_BUFFER_PAGE`: Armazena informações sobre cada página no pool de buffer `InnoDB`.

- `INNODB_BUFFER_PAGE_LRU`: Armazena informações sobre as páginas no pool de buffer `InnoDB`, em particular, como elas estão ordenadas na lista LRU que determina quais páginas devem ser removidas do pool de buffer quando ele ficar cheio. A tabela `INNODB_BUFFER_PAGE_LRU` tem as mesmas colunas que a tabela `INNODB_BUFFER_PAGE`, exceto que a tabela `INNODB_BUFFER_PAGE_LRU` tem uma coluna `LRU_POSITION` em vez de uma coluna `BLOCK_ID`.

- `INNODB_BUFFER_POOL_STATS`: Fornece informações sobre o estado do pool de buffers. Grande parte das mesmas informações é fornecida pelo `SHOW ENGINE INNODB STATUS` de saída, ou pode ser obtida usando as variáveis de status do servidor do pool de buffers `InnoDB`.
...
**Exemplo 17.6: Consultando Dados do Sistema na Tabela INNODB\_BUFFER\_PAGE**

Essa consulta fornece uma contagem aproximada de páginas que contêm dados do sistema, excluindo páginas onde o valor `TABLE_NAME` é `NULL` ou inclui uma barra `/` ou ponto `.` no nome da tabela, o que indica uma tabela definida pelo usuário.
...
O tipo de dados do sistema no pool de buffer pode ser determinado consultando o valor `PAGE_TYPE`. Por exemplo, a seguinte consulta retorna oito valores distintos de `PAGE_TYPE` entre as páginas que contêm dados do sistema:
...
**Exemplo 17.7: Consultando Dados do Usuário na Tabela INNODB\_BUFFER\_PAGE**

Essa consulta fornece uma contagem aproximada de páginas que contêm dados do usuário, contando páginas onde o valor `TABLE_NAME` é `NOT NULL` e `NOT LIKE '%INNODB_TABLES%'`.
...
**Exemplo 17.8: Consultando dados do índice na tabela INNODB\_BUFFER\_PAGE**
...
Esta consulta retorna o número de páginas e o tamanho total dos dados das páginas para todos os índices definidos na tabela `employees.salaries`:
...
**Exemplo 17.9: Consultando dados LRU\_POSITION na tabela INNODB\_BUFFER\_PAGE\_LRU**

A tabela `INNODB_BUFFER_PAGE_LRU` contém informações sobre as páginas no pool de buffer `InnoDB`, em particular, como elas são ordenadas, o que determina quais páginas devem ser expulsas do pool de buffer quando ele ficar cheio. A definição para essa página é a mesma que para `INNODB_BUFFER_PAGE`, exceto que essa tabela tem uma coluna `LRU_POSITION` em vez de uma coluna `BLOCK_ID`.
...
A tabela `INNODB_BUFFER_POOL_STATS` fornece informações semelhantes às variáveis de estado do pool de buffers `SHOW ENGINE INNODB STATUS` e `InnoDB`.
...
Para comparação, as saídas de `SHOW ENGINE INNODB STATUS` e a saída da variável de status do pool de buffers `InnoDB` são mostradas abaixo, com base no mesmo conjunto de dados.

Para obter mais informações sobre a saída do `SHOW ENGINE INNODB STATUS`, consulte a Seção 17.17.3, “Saída do Monitor Padrão InnoDB e do Monitor de Bloqueio”.
