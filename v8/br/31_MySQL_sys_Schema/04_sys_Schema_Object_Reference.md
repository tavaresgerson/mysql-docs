## 30.4 Objeto de esquema do sistema de referência ##
...
Esta tabela contém as opções de configuração do esquema `sys`, uma linha por opção. As alterações de configuração feitas ao atualizar esta tabela persistem em todas as sessões do cliente e reinício do servidor.
...
O horário de data da modificação mais recente da linha.
...
O número máximo de linhas a serem retornadas para visualizações que não possuem um limite embutido. (Por exemplo, a visualização `statements_with_runtimes_in_95th_percentile` tem um limite embutido no sentido de que ela retorna apenas declarações com tempo de execução médio no percentil 95. O padrão é 100.
...
#### 30.4.2.2 O gatilho sys_config_insert_set_user

Para as linhas adicionadas à tabela `sys_config` por declarações do `INSERT`, o gatilho `sys_config_insert_set_user` define a coluna `set_by` para o usuário atual.

#### 30.4.2.3 O gatilho sys_config_update_set_user
...
#### 30.4.3.1 As visualizações host_summary e x$host_summary
...
O host a partir do qual o cliente se conectou. As linhas para as quais a coluna `HOST` na tabela subjacente do Schema de Desempenho é `NULL` são assumidas para serem para threads de fundo e são relatadas com um nome de host de `background`.
...
Esses pontos de vista resumem o I/O de arquivos, agrupados por host. Por padrão, as linhas são ordenadas por latência total de I/O de arquivos em ordem decrescente.
...
Esses pontos resumem o I/O de arquivos, agrupados por host e tipo de evento. Por padrão, as linhas são ordenadas por host e latência total de I/O em ordem decrescente.
...
#### 30.4.3.4 As visualizações host_summary_by_stages e x$host_summary_by_stages

Esses pontos de vista resumem as etapas da declaração, agrupadas por host. Por padrão, as linhas são ordenadas por host e latência total decrescente.
...
#### 30.4.3.5 As visualizações host_summary_by_statement_latency e x$host_summary_by_statement_latency

Esses pontos de vista resumem as estatísticas de declaração geral, agrupadas por host. Por padrão, as linhas são ordenadas por latência total decrescente.
...
#### 30.4.3.6 As visualizações host_summary_by_statement_type e x$host_summary_by_statement_type

Esses pontos resumem informações sobre declarações executadas, agrupadas por host e tipo de declaração. Por padrão, as linhas são ordenadas por host e latência total decrescente.
...
O número total de linhas devolvidas por ocorrências do evento de declaração para o host.
...
#### 30.4.3.7 As vistas innodb_buffer_stats_by_schema e x$innodb_buffer_stats_by_schema

Esses pontos resumem as informações na tabela `INFORMATION_SCHEMA` `INNODB_BUFFER_PAGE`, agrupada por esquema. Por padrão, as linhas são ordenadas por tamanho de buffer descendente.
...
#### 30.4.3.8 As vistas innodb_buffer_stats_by_table e x$innodb_buffer_stats_by_table

Esses pontos resumem as informações na tabela `INFORMATION_SCHEMA` `INNODB_BUFFER_PAGE`, agrupada por esquema e tabela. Por padrão, as linhas são ordenadas em ordem decrescente de tamanho do buffer.
...
#### 30.4.3.9 As vistas innodb_lock_waits e x$innodb_lock_waits

Esses pontos resumem as `InnoDB` de bloqueio pelas quais as transações estão esperando. Por padrão, as linhas são ordenadas por idade de bloqueio descendente.
...
O número de linhas bloqueadas pela transação em espera.

* `waiting_trx_rows_modified`

O número de linhas modificadas pela transação em espera.
...
Esses pontos resumem os consumidores de E/S para exibir o tempo de espera por E/S, agrupados por thread. Por padrão, as linhas são ordenadas em ordem decrescente de latência total de E/S.
...
Esses pontos de vista resumem os consumidores globais de E/S para exibir o volume de E/S, agrupado por arquivo. Por padrão, as linhas são ordenadas em ordem decrescente de total de E/S (bytes lidos e escritos).
...
#### 30.4.3.12 As vistas io_global_by_file_by_latency e x$io_global_by_file_by_latency

Esses pontos de vista resumem os consumidores globais de E/S para exibir o tempo de espera por E/S, agrupados por arquivo. Por padrão, as linhas são ordenadas por latência total decrescente.
...
#### 30.4.3.13 As visualizações io_global_by_wait_by_bytes e x$io_global_by_wait_by_bytes

Esses pontos resumem os consumidores globais de E/S para exibir o volume de E/S e o tempo de espera para E/S, agrupados por evento. Por padrão, as linhas são ordenadas em ordem decrescente de total de E/S (bytes lidos e escritos).
...
#### 30.4.3.14 As vistas io_global_by_wait_by_latency e x$io_global_by_wait_by_latency

Esses pontos de vista resumem os consumidores globais de E/S para exibir o volume de E/S e o tempo de espera para E/S, agrupados por evento. Por padrão, as linhas são ordenadas por latência total decrescente.
...
Esses pontos de vista resumem a atividade de E/S de arquivos, agrupados por arquivo e thread. Por padrão, as linhas são ordenadas com a I/O mais recente primeiro.
...
#### 30.4.3.17 As visualizações memory_by_thread_by_current_bytes e x$memory_by_thread_by_current_bytes
...
O usuário do fio ou o nome do fio.
...
#### 30.4.3.18 Memória_por_usuário_por_bytes_curto e x$Memória_por_usuário_por_bytes_curto Visitas
...
O nome do usuário do cliente. As linhas para as quais a coluna `USER` na tabela subjacente do Schema de Desempenho é `NULL` são assumidas para serem para threads de fundo e são relatadas com um nome de host de `background`.
...
#### 30.4.3.19 As vistas memory_global_by_current_bytes e x$memory_global_by_current_bytes

Esses pontos resumem o uso da memória, agrupados por tipo de alocação (ou seja, por evento). Por padrão, as linhas são ordenadas por quantidade de memória usada em ordem decrescente.
...
#### 30.4.3.20 Memória_global_total e x$Memória_global_total Visualizações
...
#### 30.4.3.21 Metricas de visualização

Essa visão resume as métricas do servidor MySQL para mostrar os nomes das variáveis, valores, tipos e se elas estão habilitadas. Por padrão, as linhas são ordenadas por tipo e nome de variável.

A visão `metrics` inclui essas informações:
...
A porcentagem de trabalho concluído para etapas que suportam relatórios de progresso. Veja a Seção 30.3, “Relatório de progresso do esquema sys”.
...
O número de linhas lidas dos motores de armazenamento pelo comando atual.

* `rows_sent`

O número de linhas devolvidas pelo comando atual.
...
Essa visão indica quais tabelas possuem as colunas `AUTO_INCREMENT` e fornece informações sobre essas colunas, como os valores atuais e máximos das colunas e a proporção de uso (proporção de valores usados em relação aos possíveis). Por padrão, as linhas são ordenadas por proporção de uso decrescente e valor máximo da coluna.
...
#### 30.4.3.25 As vistas schema_index_statistics e x$schema_index_statistics

Esses pontos de vista fornecem estatísticas de índice. Por padrão, as linhas são ordenadas por latência total de índice descendente.
...
#### 30.4.3.26 A visão do schema_object_overview View

Essa visão resume os tipos de objetos dentro de cada esquema. Por padrão, as linhas são ordenadas por esquema e tipo de objeto.
...
#### 30.4.3.27 Os esquemas schema_redundant_indexes e x$schema_flattened_keys Views
...
#### 30.4.3.28 As vistas schema_table_lock_waits e x$schema_table_lock_waits

Essas visualizações mostram quais sessões estão bloqueadas aguardando bloqueios de metadados e o que as está bloqueando.

As descrições das colunas aqui são breves. Para informações adicionais, consulte a descrição da tabela do Schema de desempenho `metadata_locks` na Seção 29.12.13.3, “A tabela metadata_locks”.
...
O número de linhas lidas dos motores de armazenamento pelo comando.
...
Esses pontos de vista resumem as estatísticas da tabela. Por padrão, as linhas são ordenadas por tempo de espera total descendente (as tabelas com mais contenção primeiro).
...
Esses pontos resumem as estatísticas da tabela, incluindo as estatísticas do pool de tampão `InnoDB`. Por padrão, as linhas são ordenadas por tempo de espera total descendente (as tabelas com mais concorrência primeiro).
...
#### 30.4.3.31 Os esquemas_tables_with_full_table_scans e as vistas x$schema_tables_with_full_table_scans

Essas visualizações mostram quais tabelas estão sendo acessadas com varreduras completas da tabela. Por padrão, as linhas são ordenadas por varreduras descendentes.
...
O número total de linhas que são varridas por varreduras completas da tabela.
...
#### 30.4.3.33 A sessão e x$session Views

Essas visualizações são semelhantes a `processlist` e `x$processlist`, mas elas filtram os processos de fundo para exibir apenas as sessões do usuário. Para descrições das colunas, consulte a Seção 30.4.3.22, “As visualizações processlist e x$processlist”.
...
#### 30.4.3.35 Análise de declaração e x$Análise de declaração de visualização

Essas visualizações listam declarações normalizadas com estatísticas agregadas. O conteúdo imita a visualização Análise de consulta do MySQL Enterprise Monitor. Por padrão, as linhas são ordenadas por latência total decrescente.
...
O valor máximo de memória controlada (bytes) utilizado pelo comando.

Esta coluna foi adicionada no MySQL 8.0.31

* `max_total_memory`

O valor máximo de memória (bytes) utilizado pelo comando.
...
#### 30.4.3.36 Relatórios com erros ou avisos e x$Relatórios com erros ou avisos Visões

Essas visualizações exibem declarações normalizadas que produziram erros ou avisos. Por padrão, as linhas são ordenadas por contagem de erros e avisos em ordem decrescente.
...
#### 30.4.3.37 As declarações com varreduras completas da tabela e as vistas x$declaraciones_con_varreduras_completos_de_tabla

Essas visualizações exibem declarações normalizadas que realizaram varreduras completas da tabela. Por padrão, as linhas são ordenadas por porcentagem descendente de tempo em que uma varredura completa foi realizada e latência total descendente.
...
#### 30.4.3.38 As declarações com tempos de execução no 95º percentil e x$ declarações com tempos de execução no 95º percentil Visitas

Essas visualizações listam declarações com tempos de execução no percentil 95. Por padrão, as linhas são ordenadas por latência média descendente.
...
#### 30.4.3.39 As declarações_com_classificação e as visualizações x$declarativas_com_classificação

Essas visualizações listam declarações normalizadas que realizaram classificações. Por padrão, as linhas são classificadas por latência total decrescente.
...
#### 30.4.3.40 As declarações com tabelas temporárias e as vistas x$declarativas_com_tabelas_temporárias

Essas visualizações listam declarações normalizadas que utilizaram tabelas temporárias. Por padrão, as linhas são ordenadas por número descendente de tabelas temporárias no disco utilizadas e número descendente de tabelas temporárias em memória utilizadas.
...
#### 30.4.3.41 Resumo do usuário e x$Resumo do usuário Visualizações

Esses pontos resumem a atividade de declaração, a entrada/saída de arquivos e as conexões, agrupados por usuário. Por padrão, as linhas são ordenadas por latência total decrescente.
...
#### 30.4.3.42 As visualizações user_summary_by_file_io e x$user_summary_by_file_io do usuário

Esses pontos resumem o I/O de arquivos, agrupados por usuário. Por padrão, as linhas são ordenadas em ordem decrescente de latência total de I/O de arquivos.
...
#### 30.4.3.43 Resumo do usuário por tipo de IO de arquivo e x$Resumo do usuário por tipo de IO de arquivo

Esses pontos resumem o I/O de arquivos, agrupados por usuário e tipo de evento. Por padrão, as linhas são ordenadas por usuário e latência total decrescente.
...
#### 30.4.3.44 O usuário_resumo_por_etapas e x$ usuário_resumo_por_etapas Visualizações

Esses pontos de vista resumem as etapas, agrupadas por usuário. Por padrão, as linhas são ordenadas por usuário e latência total de etapa em ordem decrescente.
...
#### 30.4.3.45 As visualizações user_summary_by_statement_latency e x$user_summary_by_statement_latency do usuário

Esses pontos de vista resumem as estatísticas de declaração geral, agrupadas por usuário. Por padrão, as linhas são ordenadas por latência total decrescente.
...
#### 30.4.3.46 As visualizações user_summary_by_statement_type e x$user_summary_by_statement_type

Esses pontos resumem informações sobre declarações executadas, agrupadas por usuário e tipo de declaração. Por padrão, as linhas são ordenadas por usuário e latência total decrescente.
...
O número total de linhas devolvidas por ocorrências do evento de declaração para o usuário.
...
#### 30.4.3.47 A versão View

Essa visão fornece o esquema atual do `sys` e as versões do servidor MySQL.

Nota

A partir do MySQL 8.0.18, essa visão é desatualizada e está sujeita à remoção em uma versão futura do MySQL. Aplicações que a utilizam devem ser migradas para usar uma alternativa. Por exemplo, use a função `VERSION()` para recuperar a versão do servidor MySQL.
...
Esses pontos mostram as médias de latência do tipo de espera, agrupadas por classe de evento. Por padrão, as linhas são ordenadas em ordem decrescente de latência média. Eventos ociosos são ignorados.
...
Esses pontos resumem as latências totais da classe de espera, agrupadas por classe de evento. Por padrão, as linhas são ordenadas em ordem decrescente de latência total. Eventos ociosos são ignorados.
...
Esses pontos de vista resumem eventos de espera, agrupados por host e evento. Por padrão, as linhas são ordenadas por host e latência total decrescente. Eventos ociosos são ignorados.
...
Esses pontos de vista resumem eventos de espera, agrupados por usuário e evento. Por padrão, as linhas são ordenadas por usuário e latência total decrescente. Eventos ociosos são ignorados.
...
Esses pontos de vista resumem eventos de espera, agrupados por evento. Por padrão, as linhas são ordenadas por latência total descendente. Eventos ociosos são ignorados.
...
### 30.4.4 Procedimentos armazenados do esquema sys
...
Esse procedimento desabilita o registro binário durante sua execução, manipulando o valor da sessão da variável do sistema `sql_log_bin`. Esse é um procedimento restrito, portanto, requer privilégios suficientes para definir variáveis de sessão restritas. Veja a Seção 7.1.9.1, “Privilégios da Variável do Sistema”.

Os dados coletados para `diagnostics()` Procedimento ") incluem essas informações:
...
`diagnostics()` O procedimento de operação pode ser modificado usando as seguintes opções de configuração ou suas variáveis definidas pelo usuário correspondentes (consulte a Seção 30.4.2.1, "A tabela sys_config"):
...
Para capturar a saída do procedimento `diagnostics()` em um arquivo enquanto ele está sendo executado, use os comandos do cliente **mysql** `tee filename` e `notee` (consulte Seção 6.5.1.2, “Comandos do Cliente mysql”):
...
* `in_query LONGTEXT CHARACTER SET utf8mb3`: A string de declaração a ser executada.

Opções de configuração

`execute_prepared_stmt()` O procedimento de operação pode ser modificado usando as seguintes opções de configuração ou suas variáveis definidas pelo usuário correspondentes (consulte a Seção 30.4.2.1, "A Tabela sys_config"):
...
* `in_pattern VARCHAR(128)`: O valor utilizado para corresponder aos nomes dos instrumentos, que são identificados usando `%in_pattern%` como um operador para uma correspondência de padrão `LIKE`.
...
#### 30.4.4.11 O procedimento ps_setup_enable_thread()
...
#### 30.4.4.13 O procedimento ps_setup_reset_to_default()
...
Para evitar outras chamadas simultâneas para salvar a configuração, o `ps_setup_save()` "Procedimento") adquire um bloqueio de aconselhamento denominado `sys.ps_setup_save` ao chamar a função `GET_LOCK()`. O `ps_setup_save()` "Procedimento") leva um parâmetro de tempo de espera para indicar quantos segundos esperar se o bloqueio já existir (o que indica que alguma outra sessão tem uma configuração salva pendente). Se o tempo de espera expirar sem obter o bloqueio, o `ps_setup_save()` "Procedimento") falha.

Pretende-se que você chame o procedimento `ps_setup_reload_saved()`") mais tarde, durante a *mesma* sessão como o procedimento [[`ps_setup_save()`]") porque a configuração é salva nas tabelas `TEMPORARY`". O procedimento [[`ps_setup_save()`]") elimina as tabelas temporárias e libera o bloqueio. Se você encerrar a sessão sem invocar o procedimento [[`ps_setup_save()`]")", as tabelas e o bloqueio desaparecerão automaticamente.
...
* `in_show_threads BOOLEAN`: Se deve exibir os tópicos desabilitados.
...
#### 30.4.4.19 O procedimento ps_setup_show_enabled_consumers()
...
Descarrega todos os dados do Schema de Desempenho de um fio instrumentado em um arquivo de gráfico formatado `.dot` (para a linguagem de descrição de gráficos DOT). Cada conjunto de resultados retornado pelo procedimento deve ser usado para um gráfico completo.
...
* `in_interval DECIMAL(20,2)`: O número de segundos (que pode ser fracionário) para dormir entre as coletas de dados. Use `NULL` para dormir por um segundo padrão.

* `in_start_fresh BOOLEAN`: Se deve reiniciar todos os dados do Schema de Desempenho antes da análise.

* `in_auto_setup BOOLEAN`: Se deseja desabilitar todos os outros fios e habilitar todos os instrumentos e consumidores. Isso também redefre os ajustes no final da execução.
...
* `in_verbose BOOLEAN`: Se deve exibir cada declaração `TRUNCATE TABLE` (truncate-table.html "15.1.37 TRUNCATE TABLE Statement") antes de executá-la.
...
#### 30.4.4.25 O procedimento statement_performance_analyzer()
...
+ `delta`: Gerar uma análise delta. O delta é calculado entre a tabela de referência especificada por `in_table` e o instantâneo, que deve existir. Esta ação utiliza a tabela temporária do esquema `tmp_digests_delta` `sys`. O parâmetro `in_views` e a opção de configuração `statement_performance_analyzer.limit` afetam o funcionamento deste procedimento.
...
`statement_performance_analyzer()` operação") pode ser modificada usando as seguintes opções de configuração ou suas variáveis definidas pelo usuário correspondentes (consulte Seção 30.4.2.1, "A tabela sys_config"):
...
#### 30.4.4.26 O procedimento table_exists()
...
### 30.4.5 Funções Armazenadas no Schema sys Schema
...
A partir do MySQL 8.0.16, a função `format_bytes()`") é descontinuada e sujeita à remoção em uma versão futura do MySQL. As aplicações que a utilizam devem ser migradas para usar a função embutida `FORMAT_BYTES()` em vez disso. Veja a Seção 14.21, "Funções do Schema de Desempenho"
...
Um valor de `VARCHAR(512) CHARACTER SET utf8mb3`.
...
A operação `format_statement()` (Função) pode ser modificada usando as seguintes opções de configuração ou suas variáveis definidas pelo usuário correspondentes (consulte a Seção 30.4.2.1, “A Tabela sys_config”):
...
A partir do MySQL 8.0.16, a função `format_time()`") é descontinuada e sujeita à remoção em uma versão futura do MySQL. As aplicações que a utilizam devem ser migradas para usar a função embutida `FORMAT_PICO_TIME()` em vez disso. Veja a Seção 14.21, "Funções do Schema de Desempenho"
...
Retorna `YES` ou `NO` para indicar se um determinado consumidor do Schema de Desempenho está habilitado, ou `NULL` se o argumento for `NULL`. Se o argumento não for um nome de consumidor válido, ocorre um erro. (Antes do MySQL 8.0.18, a função retorna `NULL` se o argumento não for um nome de consumidor válido.)

Essa função leva em conta a hierarquia do consumidor, portanto, um consumidor não é considerado habilitado a menos que todos os consumidores dos quais depende também estejam habilitados. Para informações sobre a hierarquia do consumidor, consulte a Seção 29.4.7, “Pré-filtragem por Consumidor”.
...
* `in_connection_id BIGINT UNSIGNED`: O ID de conexão. Este é um valor do tipo dado na coluna `PROCESSLIST_ID` da tabela do Gerador de Desempenho `threads` ou na coluna `Id` da saída `SHOW PROCESSLIST`.
...
#### 30.4.5.14 A função ps_thread_account()

Dado um ID de fio do Schema de Desempenho, retorna a conta `user_name@host_name` associada ao fio.

##### Parâmetros

* `in_thread_id BIGINT UNSIGNED`: O ID do fio para o qual deve ser retornado a conta. O valor deve corresponder à coluna `THREAD_ID` de algumas linhas da tabela do Schema de Desempenho `threads`.
...
A partir do MySQL 8.0.16, a função `ps_thread_id()`") é descontinuada e sujeita à remoção em uma versão futura do MySQL. As aplicações que a utilizam devem ser migradas para usar as funções embutidas `PS_THREAD_ID()` e `PS_CURRENT_THREAD_ID()` em vez disso. Veja a Seção 14.21, "Funções do Schema de Desempenho"

Retorna o ID do fio do Schema de Desempenho atribuído a um ID de conexão dado, ou o ID do fio para a conexão atual se o ID de conexão for `NULL`.

##### Parâmetros

* `in_connection_id BIGINT UNSIGNED`: O ID da conexão para a qual deve ser retornado o ID do fio. Este é um valor do tipo dado na coluna `PROCESSLIST_ID` da tabela do Schema de Desempenho `threads` ou na coluna `Id` da saída `SHOW PROCESSLIST`.
...
* `in_thread_id BIGINT`: O ID do fio a ser rastreado. O valor deve corresponder à coluna `THREAD_ID` de alguma linha da tabela do Schema de Desempenho `threads`.
...
#### 30.4.5.17 A função ps_thread_trx_info()
...
* `in_thread_id BIGINT UNSIGNED`: O ID do fio para o qual retornar informações sobre a transação. O valor deve corresponder à coluna `THREAD_ID` de algumas linhas da tabela do Schema de Desempenho `threads`.

Opções de configuração

A operação `ps_thread_trx_info()` (Função) pode ser modificada usando as seguintes opções de configuração ou suas variáveis definidas pelo usuário correspondentes (consulte a Seção 30.4.2.1, “A Tabela sys_config”):
...
Dado um argumento de string, essa função produz um identificador citado adequado para inclusão em declarações SQL. Isso é útil quando um valor a ser usado como um identificador é uma palavra reservada ou contém caracteres de barra tensa (`` ` ``).
...
#### 30.4.5.19 A função sys_get_config()
...
Exemplo em uma linha: Verifique se a opção já está definida; se não estiver, atribua o resultado do `IFNULL(...)` (usando o valor da tabela `sys_config`):
...
#### 30.4.5.21 A função version_minor()
