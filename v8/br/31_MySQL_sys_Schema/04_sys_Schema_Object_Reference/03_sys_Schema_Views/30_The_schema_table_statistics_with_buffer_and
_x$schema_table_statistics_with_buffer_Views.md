...
Esses pontos de vista resumem as estatísticas da tabela, incluindo as estatísticas do pool de buffers `InnoDB`. Por padrão, as linhas são ordenadas por tempo de espera total decrescente (as tabelas com mais concorrência primeiro).

Essas visualizações utilizam uma visualização auxiliar, `x$ps_schema_table_statistics_io`.

As visualizações `schema_table_statistics_with_buffer` e `x$schema_table_statistics_with_buffer` possuem essas colunas:
...
- `innodb_buffer_pages_hashed`
  ...
  O número total de páginas `InnoDB` hašeadas alocadas para a tabela.
