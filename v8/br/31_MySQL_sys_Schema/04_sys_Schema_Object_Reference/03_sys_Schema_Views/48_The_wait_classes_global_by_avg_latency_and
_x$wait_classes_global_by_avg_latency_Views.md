...
Uma classe de evento é determinada removendo do nome do evento tudo o que vem após os três primeiros componentes. Por exemplo, a classe para `wait/io/file/sql/slow_log` é `wait/io/file`.

As visualizações `wait_classes_global_by_avg_latency` e `x$wait_classes_global_by_avg_latency` possuem essas colunas:
