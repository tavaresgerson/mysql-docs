### 30.4.5 Funções Armazenadas no Schema sys

30.4.5.1 A função extract\_schema\_from\_file\_name()

30.4.5.2 A função extract\_table\_from\_file\_name()

30.4.5.3 A função format\_bytes()

30.4.5.4 A função format\_path()

30.4.5.5 A função format\_statement()

30.4.5.6 A função format\_time()

30.4.5.7 A função list\_add()

30.4.5.8 A função list\_drop()

30.4.5.9 A função ps\_is\_account\_enabled()

30.4.5.10 A função ps\_is\_consumer\_enabled()

30.4.5.11 A função ps\_is\_instrument\_default\_enabled()

30.4.5.12 A função ps\_is\_instrument\_default\_timed()

30.4.5.13 A função ps\_is\_thread\_instrumented()

30.4.5.14 A função ps\_thread\_account()

30.4.5.15 A função ps\_thread\_id()

30.4.5.16 A função ps\_thread\_stack()

30.4.5.17 A função ps\_thread\_trx\_info()

30.4.5.18 A função quote\_identifier()

30.4.5.19 A função sys\_get\_config()

30.4.5.20 A função version\_major()

30.4.5.21 A função version\_minor()

30.4.5.22 A função version\_patch()

As seções a seguir descrevem as funções de esquema `sys` armazenadas.
