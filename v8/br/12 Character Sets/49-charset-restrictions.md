...
* Os conjuntos de caracteres `ucs2`, `utf16`, `utf16le` e `utf32` têm as seguintes restrições:
  ...
  + Atualmente, não é possível usar `LOAD DATA` para carregar arquivos de dados que usem esses conjuntos de caracteres.
...
* Os operadores `REGEXP` e `RLIKE` funcionam de forma byte a byte, portanto, não são seguros para multibytes e podem produzir resultados inesperados com conjuntos de caracteres multibytes. Além disso, esses operadores comparam caracteres por seus valores de byte e caracteres acentuados podem não ser comparados como iguais, mesmo que uma determinada collation os trate como iguais.