...
**Colágias complexas para conjuntos de caracteres de 8 bits**
...
**Colágias para conjuntos de caracteres multibyte Unicode**

Algumas dessas colágias são baseadas no Algoritmo de Colagem Unicode (UCA), outras não.

As collation não baseadas em UCA têm uma correspondência um-para-um entre o código de caracteres e o peso. No MySQL, tais collation são insensíveis ao caso e à acentuação. `utf8mb4_general_ci` é um exemplo: `'a'`, `'A'`, `'À'`, e `'á'` têm códigos de caracteres diferentes, mas todos têm um peso de `0x0041` e são comparados como iguais.
...
As collation baseadas em UCA no MySQL têm essas propriedades:
...
* Um caractere pode ter muitos pesos. Esta é uma expansão. Exemplo: A letra alemã `'ß'` (ligatura SZ, ou SHARP S) tem um peso de `0x0FEA0FEA`.
  ...
  ```
* Muitos caracteres podem ter um peso. Esta é uma contração. Exemplo: `'ch'` é uma única letra em tcheco e tem um peso de `0x0EE2`.
...
Uma correspondência de muitos caracteres para muitos pesos também é possível (esta é uma contração com expansão), mas não é suportada pelo MySQL.

Para instruções de implementação, para uma collation não baseada em UCA, consulte a Seção 12.13, “Adicionando um Conjunto de Caracteres”. Para uma collation baseada em UCA, consulte a Seção 12.14.4, “Adicionando uma Collation UCA a um Conjunto de Caracteres Unicode”.

**Collation Múltiplas**

Há também algumas collation que não se encaixam em nenhuma das categorias anteriores.