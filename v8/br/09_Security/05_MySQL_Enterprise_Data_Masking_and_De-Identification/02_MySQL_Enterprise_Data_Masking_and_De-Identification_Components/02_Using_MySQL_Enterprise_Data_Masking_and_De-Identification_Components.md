...
O MySQL oferece funções de componentes de mascaramento de propósito geral que mascaram strings arbitrárias e funções de mascaramento de propósito específico que mascaram tipos específicos de valores.

###### Funções do componente de máscara de uso geral

`mask_inner()` e `mask_outer()` são funções de propósito geral que mascaram partes de strings arbitrárias com base na posição dentro da string. Ambas as funções suportam uma string de entrada que está codificada em qualquer conjunto de caracteres:

- `mask_inner()` mascara o interior do argumento de string, deixando as extremidades não mascaradas. Outros argumentos especificam os tamanhos das extremidades não mascaradas.
...
Por padrão, `mask_inner()` e `mask_outer()` usam `'X'` como caractere de mascaramento, mas permitem um argumento opcional para o caractere de mascaramento:
...
###### Funções do componente de máscara de propósito especial
...
**Mascagem do Número da Conta Principal do Cartão de Pagamento.** As funções de mascaramento oferecem uma mascaragem rigorosa e relaxada dos números das contas principais.
...
**Mascaramento do Número de Segurança Social dos EUA.** `mask_ssn()` mascara todos os dígitos, exceto os últimos quatro:
...
**Mascaramento do Número de Seguro Social do Canadá.** `mask_canada_sin()` mascara os dígitos significativos do número:
...
**Número de Seguro Social do Reino Unido com mascaramento.** `mask_uk_nin()` mascara todos os dígitos, exceto os dois primeiros:
...
`gen_rnd_iban()` retorna um número escolhido de uma faixa não utilizada para números legítimos:
...
`gen_rnd_pan()` retorna um Número Principal da Conta (NPC) aleatório do cartão de pagamento.

Como não pode ser garantido que o número gerado não seja atribuído a uma conta de pagamento legítima, o resultado de `gen_rnd_pan()` nunca deve ser exibido, exceto para fins de teste. Para exibição em aplicativos, sempre utilize uma função de mascaramento, como `mask_pan()` ou `mask_pan_relaxed()`. Mostramos esse uso da última função com `gen_rnd_pan()` aqui:
...
O mascaramento de dados e a desidentificação do MySQL Enterprise permitem que os dicionários sejam usados como fontes de valores aleatórios chamados *termos*. Para usar um dicionário, ele deve primeiro ser adicionado à tabela `masking_dictionaries` do sistema e receber um nome. Os dicionários são lidos da tabela e carregados na cache durante a inicialização dos componentes (ao iniciar o servidor). Os termos podem então ser adicionados, removidos e selecionados dos dicionários e usados como valores aleatórios ou como substituições para outros valores.

Nota

Sempre edite os dicionários usando as funções de administração de dicionários, em vez de modificar a tabela diretamente. Se você manipular a tabela manualmente, o cache do dicionário fica inconsistente com a tabela.
...
A função `gen_blocklist()` permite que um termo de um dicionário seja substituído por um termo de outro dicionário, o que resulta em um mascaramento por substituição. Seus argumentos são o termo a ser substituído, o dicionário no qual o termo aparece e o dicionário a partir do qual será escolhido um substituto. Por exemplo, para substituir uma cidade dos EUA por uma cidade alemã, ou vice-versa, use `gen_blocklist()` da seguinte maneira:
...
Nos centros de atendimento ao cliente, uma técnica comum de verificação de identidade é pedir aos clientes que forneçam seus últimos quatro dígitos do Número de Seguridade Social (NSS). Por exemplo, um cliente pode dizer que seu nome é Joanna Bond e que seus últimos quatro dígitos do NSS são `0007`.
...
Por que a função `CONVERT()` foi usada para o argumento de `mask_ssn()`? Porque `mask_ssn()` requer um argumento de comprimento 11. Assim, mesmo que `ssn` seja definido como `VARCHAR(11)`, se a coluna `ssn` tiver um conjunto de caracteres multibyte, ela pode parecer mais longa que 11 bytes quando passada para uma função carregável e retornar `NULL` ao registrar o erro. Converter o valor em uma string binária garante que a função veja um argumento de comprimento 11.
