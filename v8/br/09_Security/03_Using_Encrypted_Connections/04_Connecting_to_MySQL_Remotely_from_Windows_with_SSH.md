...
2. Inicie o cliente SSH do Windows. Defina `Host_Name = yourmysqlserver_URL_or_IP`. Defina `userid=your_userid` para fazer login no seu servidor. Este valor `userid` pode não ser o mesmo do nome de usuário da sua conta MySQL.
...
7. Crie um novo arquivo no Windows e faça o link para o MySQL usando o driver ODBC da mesma maneira que você normalmente faz, exceto que digite `localhost` para o servidor do host MySQL, não `yourmysqlservername`.
