...
As instruções que se seguem utilizam o código binário `semanage` para gerenciar o contexto do arquivo; no RHEL, faz parte do pacote `policycoreutils-python-utils`:
...
O local padrão para o soquete de domínio Unix é `/var/lib/mysql/mysql.sock`; e o tipo de contexto SELinux usado é `mysqld_var_run_t`.
...
#### Definindo o contexto de diretório secure\_file\_priv
...
Se você ativou o uso deste diretório configurando `secure_file_priv`, você pode precisar definir o contexto da seguinte forma:
