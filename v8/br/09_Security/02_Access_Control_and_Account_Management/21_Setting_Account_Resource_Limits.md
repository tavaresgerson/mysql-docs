...
A opção `--old-style-user-limits` é desaconselhada no MySQL 8.0.30 e está sujeita à remoção em uma futura versão do MySQL. O uso dessa opção na linha de comando ou em um arquivo de opções no MySQL 8.0.30 ou posterior faz com que o servidor emita uma mensagem de alerta.
...
Para remover um limite, defina seu valor para zero. Por exemplo, para remover o limite de quantas vezes por hora o `francis` pode se conectar, use esta declaração:

```
mysql> ALTER USER 'francis'@'localhost' WITH MAX_CONNECTIONS_PER_HOUR 0;
```

Como mencionado anteriormente, o limite de conexão simultânea para uma conta é determinado a partir do limite `MAX_USER_CONNECTIONS` e da variável de sistema `max_user_connections`. Suponha que o valor global `max_user_connections` seja 10 e que três contas tenham limites de recursos individuais especificados da seguinte forma:
...
- Para redefinir as contagens atuais para zero em todas as contas, execute uma declaração `FLUSH USER_RESOURCES`. As contagens também podem ser redefinidas reiniciando as tabelas de concessão (por exemplo, com uma declaração `FLUSH PRIVILEGES` ou o comando **mysqladmin reload**).
