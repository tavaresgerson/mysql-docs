...
Os certificados gerados por essas instruções são autoassinados, o que pode não ser muito seguro. Depois de ganhar experiência usando esses arquivos, considere obter o material de certificado/chave de uma autoridade de certificação registrada.

1. Prepare as chaves do servidor HashiCorp Vault e da empresa.
   ...
   Os comandos geram arquivos que contêm a chave privada da empresa (`company.key`) e a chave privada do servidor Vault (`vault.key`). As chaves são chaves RSA geradas aleatoriamente de 4.096 e 2.048 bits, respectivamente.

   Cada comando solicita uma senha. Para fins de teste, a senha não é necessária. Para desabilitar, omita o argumento `-aes256`.

   Os arquivos chave contêm informações sensíveis e devem ser armazenados em um local seguro. A senha (também sensível) será necessária mais tarde, então anote-a e armazene-a em um local seguro.
...
2. Crie o certificado da empresa CA.
   ...
   Use o seguinte comando para criar um arquivo de certificado CA da empresa com o nome `company.crt` que seja válido por 365 dias (insira o comando em uma única linha):
   ...
   Se você usou o argumento `-aes256` para realizar a criptografia de chave durante a geração de chaves, você será solicitado a fornecer a senha da chave da empresa durante a criação do certificado CA. Você também será solicitado a fornecer informações sobre o titular do certificado (ou seja, você ou sua empresa), conforme mostrado aqui:
...
3. Crie um pedido de assinatura de certificado.
   ...
   Para criar um certificado do servidor HashiCorp Vault, uma solicitação de assinatura de certificado (CSR) deve ser preparada para a chave do servidor recém-criada. Crie um arquivo de configuração chamado `request.conf` contendo as seguintes linhas. Se o servidor HashiCorp Vault não estiver rodando no host local, substitua os valores apropriados de CN e IP e faça quaisquer outras alterações necessárias.
...
4. Crie o certificado do servidor HashiCorp Vault.
   ...
   Assine as informações combinadas da chave do servidor HashiCorp Vault (`vault.key`) e do CSR (`request.csr`) com o certificado da empresa (`company.crt`) para criar o certificado do servidor HashiCorp Vault (`vault.crt`). Use o seguinte comando para fazer isso (insira o comando em uma única linha):
   ...
   Para tornar o certificado do servidor `vault.crt` útil, adicione o conteúdo do certificado da empresa `company.crt` a ele. Isso é necessário para que o certificado da empresa seja entregue junto com o certificado do servidor nas solicitações.
...
3. Inicie o servidor HashiCorp Vault.
   ...
   Para iniciar o servidor do Vault, use o seguinte comando, onde a opção `-config` especifica o caminho para o arquivo de configuração recém-criado:

   ```
   vault server -config=config.hcl
   ```

   Durante esta etapa, você pode ser solicitado a fornecer uma senha para a chave privada do servidor do Vault armazenada no arquivo `vault.key`.
...
4. Inicialize o servidor HashiCorp Vault.

   Nota

   As operações descritas nesta etapa são necessárias apenas ao iniciar o Vault pela primeira vez, para obter a chave de desbloqueio e o token raiz. Os reinícios subsequentes da instância do Vault exigem apenas o desbloqueio usando a chave de desbloqueio.
   ...
   O segundo comando cria uma única chave de desaseamento com a exigência de que uma única chave de desaseamento esteja presente para a desaseamento. (Para uso em produção, uma instância teria várias chaves de desaseamento, com até tantas chaves sendo necessárias para desaseá-la. As chaves de desaseamento devem ser entregues aos guardiões das chaves dentro da empresa. O uso de uma única chave pode ser considerado um problema de segurança, pois isso permite que o cofre seja desaseado por um único guardião das chaves.)
...
5. Descongele o servidor HashiCorp Vault.
   ...
   Use este comando para destrancar o servidor do Vault:

   ```
   vault operator unseal
   ```

   Quando solicitado para inserir a chave de desbloqueio, use a chave obtida anteriormente durante a inicialização do Vault.

   O Vault deve gerar uma saída indicando que a configuração está completa e o cofre está aberto.
...
8. Adicione uma política de segurança AppRole.
   ...
   Prepare uma política que permita que o papel criado anteriormente acesse os segredos apropriados. Crie um novo arquivo chamado `mysql.hcl` com o seguinte conteúdo:
   ...
   `kv/mysql/` neste exemplo pode precisar de ajustes de acordo com as políticas de instalação e requisitos de segurança da sua instalação local. Se for o caso, faça o mesmo ajuste em qualquer outro lugar onde `kv/mysql/` aparece nessas instruções.

   Importe o arquivo de política para o servidor do Vault para criar uma política com o nome `mysql-policy`, e, em seguida, atribua a política ao novo papel:
   ...
   Gerencie uma ID secreta para o papel e armazene-a em um local seguro:

   ```
   vault write -f auth/approle/role/mysql/secret-id
   ```

   Após a geração dessas credenciais de ID de papel e ID secreta do AppRole, espera-se que elas permaneçam válidas indefinidamente. Elas não precisam ser geradas novamente e o plugin `keyring_hashicorp` pode ser configurado com elas para uso contínuo. Para mais informações sobre a autenticação AuthRole, visite <https://www.vaultproject.io/docs/auth/approle.html>.

##### configuração keyring\_hashicorp

O arquivo da biblioteca de plugins contém o plugin `keyring_hashicorp` e uma função carregável, `keyring_hashicorp_update_config()`. Quando o plugin se inicializa e termina, ele carrega e descarrega automaticamente a função. Não é necessário carregar e descarregar a função manualmente.
...
De acordo com a documentação da HashiCorp, todas as rotas da API são prefixadas com uma versão do protocolo (que você pode ver no exemplo anterior como `/v1/` nos valores `keyring_hashicorp_store_path` e `keyring_hashicorp_auth_path`). Se a HashiCorp desenvolver novas versões de protocolo, pode ser necessário alterar `/v1/` para outra coisa na sua configuração.

O MySQL Server autentica-se contra o HashiCorp Vault usando a autenticação AppRole. A autenticação bem-sucedida requer que dois segredos sejam fornecidos ao Vault, um ID de papel e um ID de segredo, que são semelhantes ao nome de usuário e senha. Os valores do ID de papel e do ID de segredo a serem usados são aqueles obtidos durante o procedimento de configuração do HashiCorp Vault realizado anteriormente. Para especificar os dois IDs, atribua seus respectivos valores às variáveis de sistema `keyring_hashicorp_role_id` e `keyring_hashicorp_secret_id`. O procedimento de configuração também resulta em um caminho de armazenamento de `/v1/kv/mysql`, que é o valor a ser atribuído a `keyring_hashicorp_commit_store_path`.
...
1. Use as instruções `SET` para atribuir os novos valores desejados às variáveis do sistema de configuração mostradas na tabela anterior. Essas atribuições, por si só, não têm efeito na operação contínua do plugin.
...
3. Se a conexão for bem-sucedida, o plugin armazena os valores de configuração atualizados em variáveis de sistema correspondentes que têm `_commit_` em seu nome.
...
Se o plugin não conseguir se conectar ao HashiCorp Vault durante a inicialização ou reconfiguração e não houvesse uma conexão existente, as variáveis de sistema `_commit_` são definidas como `'Not committed'` para variáveis de valor de string e `OFF` para variáveis de valor de booleano. Se o plugin não conseguir se conectar, mas houvesse uma conexão existente, essa conexão permanece ativa e as variáveis `_commit_` refletem os valores usados para ela.
...
Para obter informações adicionais sobre as variáveis de sistema específicas do plugin `keyring_hashicorp` e as funções, consulte a Seção 8.4.4.19, “Variáveis do Sistema de Carteira de Chaves”, e a Seção 8.4.4.16, “Funções de Gerenciamento de Chaves da Carteira de Plugins”.
