...
### 8.7.1 Verifique se o SELinux está habilitado
...
#### 8.7.5.1 Configurando o contexto do porto TCP para mysqld

A porta TCP padrão para o **mysqld** é `3306`; e o tipo de contexto SELinux utilizado é `mysqld_port_t`.

Se você configurar o **mysqld** para usar um TCP diferente `port`, você pode precisar definir o contexto para a nova porta. Por exemplo, para definir o contexto SELinux para uma porta não padrão, como a porta 3307:
...
#### 8.7.5.2 Configuração do contexto de porta TCP para recursos do MySQL
...
De MySQL 8.0.14 a MySQL 8.0.17, o booleano `mysql_connect_any` SELinux deve ser definido como `ON`. A partir do MySQL 8.0.18, a ativação de `mysql_connect_any` não é necessária ou recomendada.
...
Se o SELinux estiver habilitado, você deve definir o contexto de porta para a porta de comunicação usada pelo X Plugin, que é definida pela variável `mysqlx_port`. O **mysqld** deve ser capaz de se ligar à porta de comunicação do X Plugin e ouvir lá.
...
##### Definindo o contexto do porto TCP para o roteador MySQL

Se o SELinux estiver habilitado, você deve definir o contexto de porta para as portas de comunicação usadas pelo MySQL Router. Supondo que as portas de comunicação adicionais usadas pelo MySQL Router sejam as padrão 6446, 6447, 64460 e 64470, em cada instância, defina o contexto de porta emitindo:
...
Para obter informações sobre a configuração do contexto SELinux adequado para diretórios e arquivos do MySQL, consulte a Seção 8.7.4, “Contexto de arquivo SELinux”.
...
Para obter informações sobre a configuração do contexto do porto SELinux adequado para o MySQL, consulte a Seção 8.7.5, “Contexto de porto TCP SELinux”. Problemas semelhantes de acesso à porta podem ocorrer ao habilitar recursos do MySQL que utilizam portas que não são definidas com o contexto necessário. Para mais informações, consulte a Seção 8.7.5.2, “Definindo o contexto do porto TCP para recursos do MySQL”.
