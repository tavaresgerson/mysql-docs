...
`mysql` sends each SQL st nt that you issue to the server to be executed. There is also a set of commands that `mysql` itself interprets. For a list of these commands, type `help` or `\h` at the `mysql>` prompt:
...
If `mysql` is invoked with the `--binary-mode` option, all `mysql` commands are disabled except `charset` and `delimiter` in noninteractive mode (for input piped to `mysql` or loaded using the `source` command). Beginning with MySQL 8.4.6, the `--commands` option can be used to enable or disable all commands except `/C`, `delimiter`, and `use`.

Each command has both a long and short form. The long form is not case-sensitive; the short form is. The long form can be followed by an optional colon terminator, but the short form should not.
...
* `charset charset_name`, `\C charset_name`
  ...
  Change the default character set and issue a `SET NAMES` st nt. This enables the character set to in synchronized on the client and server if `mysql` is run with auto-reconnect enabled (which is not recommended), because the specified character set is used for reconnects.
* `clear`, `\c`

  Clear the current input. Use this if you change your mind about executing the st nt that you are entering.
...
* `delimiter str`, `\d str`
  ...
  Change the string that `mysql` interprets as the separator between SQL st nts. The default is the colon character (`;`).
  ...
  `mysql` interprets instances of the delimiter string as a st nt delimiter anywhere it occurs, except within quoted strings. Be careful about defining a delimiter that might occur within other words. For example, if you define the delimiter as `X`, it is not possible to use the word `INDEX` in st nts. `mysql` interprets this as `INDE` followed by the delimiter `X`.

  When the delimiter recognized by `mysql` is set to something other than the default of `;`, instances of that character are sent to the server without interpretation. However, the server itself still interprets `;` as a st nt delimiter and processes st nts accordingly. This behavior on the server side comes into play for multiple-st nt execution (see Multiple St nt Execution Support), and for parsing the body of stored procedures and functions, triggers, and events (see  Section 27.1, “Defining Stored Programs”).
* `edit`, `\e`

  Edit the current input st nt. `mysql` checks the values of the `EDITOR` and `VISUAL` environment variables to determine which editor to use. The default editor is `vi` if neither variable is set.

  The `edit` command works only in Unix.
* `ego`, `\G`

  Send the current st nt to the server to be executed and display the result using vertical format.
* `exit`, `\q`

  Exit `mysql`.
* `go`, `\g`

  Send the current st nt to the server to be executed.
* `nopager`, `\n`
...
* `nowarning`, `\w`
  ...
  Disable display of warnings after each st nt.
...
* `print`, `\p`
  ...
  Print the current input st nt without executing it.
* `prompt [str]`, `\R [str]`
...
* `query_attributes name value [name value ...]`
  ...
  + If multiple attributes are defined with the same name, a ts to retrieve the attribute value have an undefined result.
  + An attribute defined with a ty name cannot be retrieved by name.
...
* `rehash`, `\#`
  ...
  Rebuild the completion hash that enables database, table, and column name completion while you are entering st nts. (See the description for the `--auto-rehash` option.)
* `resetconnection`, `\x`
...
* `source file_name`, `\. file_name`
  ...
  Read the named file and executes the st nts contained therein. On Windows, specify path name separators as `/` or `\\`.
...
* `ssl_session_data_print [file_name]`
  ...
  If the MySQL session is configured for reuse, session data from the file is deserialized and supplied to the `connect` command to reconnect. When the session is reused successfully, the `status` command contains a row showing `SSL session reused: true` while the client ins reconnected to the server.
* `status`, `\s`
...
* `sy command`, `\! command`
  ...
  In MySQL 8.4.3 and later, this command can be disabled by starting the client with `--sy command=OFF` or `--skip-sy command`.
* `tee [file_name]`, `\T [file_name]`

  By using the `--tee` option when you invoke `mysql`, you can log st nts and their output. All the data displayed on the screen is appended into a given file. This can be very useful for debugging purposes also. `mysql` flushes results to the file after each st nt, just before it prints its next prompt. Tee functionality works only in interactive mode.
...
* `warnings`, `\W`
  ...
  Enable display of warnings after each st nt (if there are any).
...
* You can specify very complex pager commands for handling query output:
  ...
  In this example, the command would send query results to two files in two different directories on two different file sy  mounted on `/dr1` and `/dr2`, yet still display the results onscreen using `less`.
