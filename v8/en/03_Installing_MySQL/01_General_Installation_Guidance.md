...
First, decide whether to install from a bugfix series like MySQL 8.4, or use an innovation release like MySQL 9.5. Both tracks include bug fixes while an innovation release includes the newest features. Both bugfix and innovation releases are meant for production use.
...
For SUSE Linux Enterprise Server (SLES) platforms, MySQL can be installed using the [MySQL SLES Repository](https://dev.mysql.com/downloads/repo/suse/). See Section 2.5.3, “Installing MySQL on Linux Using the MySQL SLES Repository” for details.
...
Another method of verifying the integrity and authenticity of a package is to use cryptographic signatures. This is more reliable than using [MD5 checksums](verifying-md5-checksum.html "2.1.4.1 Verifying the MD5 Checksum"), but requires more work.
