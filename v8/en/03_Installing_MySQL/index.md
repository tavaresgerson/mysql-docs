...
3. **Choose which track to install.**
   ...
   MySQL offers a bugfix track (such as MySQL 8.4), and an innovation track (today it's MySQL 9.5) and each track addresses different use cases. Both tracks are considered production-ready and include bug fixes, while innovation releases also include new features and potential for modified behavior.

   A bugfix track upgrade includes point releases, such as MySQL 8.4.*`x`* upgrading to 8.4.*`y`*, while innovation track releases typically only have minor releases, such as MySQL 9.5.0 upgrading to 9.6.0. However, an innovation track does have the occasional point release.
