...
properly, see [Section 2.9.3, “Testing the Server”](testing-server.html "2.9.3 Testing the Server").
...
* [Data Directory Initialization Overview](data-directory-initialization.html#data-directory-initialization-overview "Data Directory Initialization Overview")
* [Data Directory Initialization Procedure](data-directory-initialization.html#data-directory-initialization-procedure "Data Directory Initialization Procedure")
* [Server Actions During Data Directory Initialization](data-directory-initialization.html#data-directory-initialization-server-actions "Server Actions During Data Directory Initialization")
...
[Create a mysql User and Group](binary-installation.html#binary-installation-createsysuser "Create a mysql User and Group")), or
...
2. The [`secure_file_priv`](server-system-variables.html#sysvar_secure_file_priv) system
...
3. Use the server to initialize the data directory, including
   ...
   [Chapter 3, *Upgrading MySQL*](upgrading.html "Chapter 3 Upgrading MySQL").) However, the command that
...
4. If you want to deploy the server with automatic support for
   ...
   [**mysql\_ssl\_rsa\_setup**](mysql-ssl-rsa-setup.html "6.4.3 mysql_ssl_rsa_setup — Create SSL/RSA Files") utility to create
   ...
   The [**mysql\_ssl\_rsa\_setup**](mysql-ssl-rsa-setup.html "6.4.3 mysql_ssl_rsa_setup — Create SSL/RSA Files") utility is
   deprecated as of MySQL 8.0.34.

5. In the absence of any option files, the server starts with
   its default settings. (See
   [Section 7.1.2, “Server Configuration Defaults”](server-configuration-defaults.html "7.1.2 Server Configuration Defaults").) To
   ...
   [Section 6.2.2.2, “Using Option Files”](option-files.html "6.2.2.2 Using Option Files").) For example, you can use an
...
6. To arrange for MySQL to start without manual intervention at
   system boot time, see [Section 2.9.5, “Starting and Stopping MySQL Automatically”](automatic-start.html "2.9.5 Starting and Stopping MySQL Automatically").
...
* Use [`--initialize`](server-options.html#option_mysqld_initialize) for
...
* With [`--initialize-insecure`](server-options.html#option_mysqld_initialize-insecure),
...
where it is located, see [Section 7.4.2, “The Error Log”](error-log.html "7.4.2 The Error Log").

On Windows, use the [`--console`](server-options.html#option_mysqld_console)
...
this, start [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") from the system
`root` account and include the
[`--user`](server-options.html#option_mysqld_user) option as shown here:
...
Alternatively, execute [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") while logged in
as `mysql`, in which case you can omit the
[`--user`](server-options.html#option_mysqld_user) option from the command.
...
[`--basedir`](server-system-variables.html#sysvar_basedir) or
[`--datadir`](server-system-variables.html#sysvar_datadir) if
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") cannot identify the correct locations
...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"). For Unix and Unix-like systems,
...
Then invoke [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") as follows (enter the
command on a single line, with the
[`--defaults-file`](option-file-options.html#option_general_defaults-file) option first):
...
Then invoke [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") as follows (again, you
...
locations such as [`--basedir`](server-system-variables.html#sysvar_basedir) or
[`--datadir`](server-system-variables.html#sysvar_datadir), and the
[`--user`](server-options.html#option_mysqld_user) option if needed.
...
[`--initialize`](server-options.html#option_mysqld_initialize) option for further
information.
...
[**mysql\_secure\_installation**](mysql-secure-installation.html "6.4.2 mysql_secure_installation — Improve MySQL Installation Security") and
[**mysql\_ssl\_rsa\_setup**](mysql-ssl-rsa-setup.html "6.4.3 mysql_ssl_rsa_setup — Create SSL/RSA Files"). See
[Section 6.4.2, “mysql\_secure\_installation — Improve MySQL Installation Security”](mysql-secure-installation.html "6.4.2 mysql_secure_installation — Improve MySQL Installation Security"), and
[Section 6.4.3, “mysql\_ssl\_rsa\_setup — Create SSL/RSA Files”](mysql-ssl-rsa-setup.html "6.4.3 mysql_ssl_rsa_setup — Create SSL/RSA Files").

When invoked with the
[`--initialize`](server-options.html#option_mysqld_initialize) or
[`--initialize-insecure`](server-options.html#option_mysqld_initialize-insecure) option,
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") performs the following actions during
...
3. The server initializes the
   ...
   manage [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") tables.

   Note

   After [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") sets up the
   `InnoDB`
   [system
   tablespace](glossary.html#glos_system_tablespace "system tablespace"), certain changes to tablespace
   characteristics require setting up a whole new
   [instance](glossary.html#glos_instance "instance"). Qualifying
   ...
   [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"). Also make sure to specify as
...
4. The server creates a `'root'@'localhost'`
   ...
   [Section 8.2.9, “Reserved Accounts”](reserved-accounts.html "8.2.9 Reserved Accounts")). Some reserved accounts
   ...
   * With [`--initialize`](server-options.html#option_mysqld_initialize) but not
   ...
   * With
     ...
     [`--initialize`](server-options.html#option_mysqld_initialize) because
     [`--initialize-insecure`](server-options.html#option_mysqld_initialize-insecure)
     implies [`--initialize`](server-options.html#option_mysqld_initialize)),
...
5. The server populates the server-side help tables used for
   the [`HELP`](help.html "15.8.3 HELP Statement") statement (see
   [Section 15.8.3, “HELP Statement”](help.html "15.8.3 HELP Statement")). The server does not populate the
   time zone tables. To do so manually, see
   [Section 7.1.15, “MySQL Server Time Zone Support”](time-zone-support.html "7.1.15 MySQL Server Time Zone Support").

6. If the [`init_file`](server-system-variables.html#sysvar_init_file) system
   ...
   USER`](create-user.html "15.7.1.3 CREATE USER Statement") or [`GRANT`](grant.html "15.7.1.6 GRANT Statement")),
...
[`--initialize-insecure`](server-options.html#option_mysqld_initialize-insecure), start the
...
2. Connect to the server:
   ...
   * If you used [`--initialize`](server-options.html#option_mysqld_initialize)
     but not
     [`--initialize-insecure`](server-options.html#option_mysqld_initialize-insecure) to
...
3. After connecting, use an [`ALTER
   USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statement to assign a new
   `root` password:
...
See also [Section 2.9.4, “Securing the Initial MySQL Account”](default-privileges.html "2.9.4 Securing the Initial MySQL Account").
...
executed using the [`init_file`](server-system-variables.html#sysvar_init_file)
