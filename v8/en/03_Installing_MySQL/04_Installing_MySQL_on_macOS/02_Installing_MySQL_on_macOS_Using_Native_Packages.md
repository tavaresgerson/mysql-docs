...
[**mysqladmin shutdown**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") on the command line.
...
1. Download the disk image (`.dmg`) file (the
   ...
   For example, for MySQL server 8.0.44 it might be
   named
   `mysql-8.0.44-macos-10.13-x86_64.pkg`.
...
6. The default MySQL 8.0 password mechanism is
   ...
   **Figure 2.15 MySQL Package Installer Wizard: Choose a Password Encryption Type**
   ...
   launchd file to set
   [`--default_authentication_plugin=mysql_native_password`](server-system-variables.html#sysvar_default_authentication_plugin)
...
information, see [Section 2.4.3, “Installing and Using the MySQL Launch Daemon”](macos-installation-launchd.html "2.4.3 Installing and Using the MySQL Launch Daemon"), and
[Section 2.4.4, “Installing and Using the MySQL Preference Pane”](macos-installation-prefpane.html "2.4.4 Installing and Using the MySQL Preference Pane"). Use the MySQL
...
`mysql-8.0.44-macos10.15-x86_64.dmg`
installs MySQL into
`/usr/local/mysql-8.0.44-macos10.15-x86_64/` with a symlink to
...
<td>Miscellaneous support files, including error messages,
            <code class="filename">dictionary.txt</code>, and rewriter SQL</td>
</tr><tr>
<td><code class="filename">support-files</code></td>
<td>Support scripts, such as <code class="filename">mysqld_multi.server</code>,
            <code class="filename">mysql.server</code>, and
            <code class="filename">mysql-log-rotate</code>.</td>
</tr><tr>
