...
* From the MySQL software repositories:
  ...
  + The MySQL Yum repository (see
    [Section 2.5.1, “Installing MySQL on Linux Using the MySQL Yum Repository”](linux-installation-yum-repo.html "2.5.1 Installing MySQL on Linux Using the MySQL Yum Repository") for
    details).

  + The MySQL SLES repository (see
    [Section 2.5.3, “Installing MySQL on Linux Using the MySQL SLES Repository”](linux-installation-sles-repo.html "2.5.3 Installing MySQL on Linux Using the MySQL SLES Repository") for
    details).
...
<table frame="all"><col style="width: 35%"/><col style="width: 25%"/><thead><tr>
<th>Package Name</th>
<th>Summary</th>
</tr></thead><tbody><tr>
...
<td><code class="literal">mysql-community-client-plugins</code></td>
<td>Shared plugins for MySQL client applications</td>
</tr><tr>
...
<td><code class="literal">mysql-community-icu-data-files</code></td>
<td>MySQL packaging of ICU data files needed by MySQL regular expressions</td>
</tr><tr>
...
<td>Shared compatibility libraries for previous MySQL installations; only
...
<td>Debug server and plugin binaries</td>
</tr><tr>
...
<td>The source code RPM looks similar to
            mysql-community-8.0.44-1.el7.src.rpm, depending
            on selected OS</td>
</tr><tr>
<td>Additional *debuginfo* RPMs</td>
<td>There are several <code class="literal">debuginfo</code> packages:
            ...
            mysql-community-test-debuginfo.</td>
</tr></tbody></table>
...
<td><code class="literal">mysql-commercial-client-plugins</code></td>
<td>Shared plugins for MySQL client applications</td>
</tr><tr>
...
<td><code class="literal">mysql-commercial-icu-data-files</code></td>
<td>MySQL packaging of ICU data files needed by MySQL regular expressions</td>
</tr><tr>
...
<td>There are several <code class="literal">debuginfo</code> packages:
            ...
            mysql-commercial-test-debuginfo.</td>
</tr></tbody></table>
...
<td>el<em class="replaceable"><code>{version}</code></em> where
            <em class="replaceable"><code>{version}</code></em> is the major Enterprise
            Linux version, such as <code class="literal">el8</code></td>
...
<td>fc<em class="replaceable"><code>{version}</code></em> where
            <em class="replaceable"><code>{version}</code></em> is the major Fedora
            version, such as <code class="literal">fc37</code></td>
<td>Fedora 41 and 42</td>
</tr><tr>
...
<table><col style="width: 55%"/><col style="width: 45%"/><thead><tr>
...
<td><a class="link" href="mysqld.html" title="6.3.1 mysqld — The MySQL Server"><span class="command"><strong>mysqld</strong></span></a> server</td>
...
<td>Value of <a class="link" href="server-system-variables.html#sysvar_secure_file_priv"><code class="literal">secure_file_priv</code></a></td>
...
<td>Miscellaneous support files (for example, error messages, and character
            set files)</td>
...
* The `mysql` user is created using the
  ...
  [Creating
  the mysql User and Group](/doc/mysql-secure-deployment-guide/8.0/en/secure-deployment-install.html#secure-deployment-mysql-user) for details). To switch to
...
[Section 2.5.9, “Managing MySQL Server with systemd”](using-systemd.html "2.5.9 Managing MySQL Server with systemd") for additional information.
...
* [`validate_password`](validate-password.html "8.4.3 The Password Validation Component")
...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"). See
...
[debug package](dbug-package.html "7.9.4 The DBUG Package") has been
...
See [Section 7.9.4, “The DBUG Package”](dbug-package.html "7.9.4 The DBUG Package") for the debug options you can
use.
...
[`plugin_dir`](server-system-variables.html#sysvar_plugin_dir) to
