...
* `INNODB_BUFFER_POOL_STATS`: Provides buffer pool status information. Much of the same information is provided by [`SHOW ENGINE INNODB STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement") output, or may be obtained using `InnoDB` buffer pool server status variables.
...
The `INNODB_BUFFER_POOL_STATS` table provides information similar to [`SHOW ENGINE INNODB STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement") and `InnoDB` buffer pool status variables.
...
For comparison, [`SHOW ENGINE INNODB STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement") output and `InnoDB` buffer pool status variable output is shown below, based on the same data set.

For more information about [`SHOW ENGINE INNODB STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement") output, see Section 17.17.3, “InnoDB Standard Monitor and Lock Monitor Output”.
...
The counters enabled by default correspond to those shown in [`SHOW ENGINE INNODB STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement") output. Counters shown in [`SHOW ENGINE INNODB STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement") output are always enabled at a system level but can be disable for the `INNODB_METRICS` table. Counter status is not persistent. Unless configured otherwise, counters revert to their default enabled or disabled status when the server is restarted.
...
The Information Schema `FILES` table provides metadata about all `InnoDB` tablespace types including [file-per-table tablespaces](glossary.html#glos_file_per_table "file-per-table"), [general tablespaces](glossary.html#glos_general_tablespace "general tablespace"), the system tablespace, [temporary table tablespaces](glossary.html#glos_temporary_tablespace "temporary tablespace"), and [undo tablespaces](glossary.html#glos_undo_tablespace "undo tablespace") (if present).
