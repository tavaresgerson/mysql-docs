...
* If the `daemon_memcached` plugin causes MySQL
  ...
  PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement") statement before running the
...
* When defining your own `InnoDB` table for use
  ...
  [`INSERT`](insert.html "15.2.7 INSERT Statement") statement for the
  ...
  [`INSERT`](insert.html "15.2.7 INSERT Statement") to fail, which may only
...
* As the length of the **memcached** key and
  ...
  + `InnoDB` table limits may be encountered
    ...
    [Section 17.22, “InnoDB Limits”](innodb-limits.html "17.22 InnoDB Limits") for details.
