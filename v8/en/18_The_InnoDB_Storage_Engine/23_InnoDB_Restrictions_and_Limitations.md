...
* `InnoDB` does not keep an internal count of
  ...
  [`COUNT()`](aggregate-functions.html#function_count) description in
...
* A MySQL instance using a particular `InnoDB`
  page size ([`innodb_page_size`](innodb-parameters.html#sysvar_innodb_page_size))
...
* For limitations associated with importing tables using the
  *Transportable Tablespaces* feature, see
  [Table Import
  Limitations](innodb-statistics-estimation.html "17.8.10.2 Configuring Non-Persistent Optimizer Statistics Parameters").
