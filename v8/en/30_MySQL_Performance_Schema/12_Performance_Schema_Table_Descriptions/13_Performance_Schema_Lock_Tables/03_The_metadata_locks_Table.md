...
[Section 10.11.4, “Metadata Locking”](metadata-locking.html "10.11.4 Metadata Locking"). Metadata locking applies
...
through the [`metadata_locks`](performance-schema-metadata-locks-table.html "29.12.13.3 The metadata_locks Table") table:
...
The [`metadata_locks`](performance-schema-metadata-locks-table.html "29.12.13.3 The metadata_locks Table") table is read
...
update the [`setup_instruments`](performance-schema-setup-instruments-table.html "29.12.2.3 The setup_instruments Table")
table:
...
[`metadata_locks`](performance-schema-metadata-locks-table.html "29.12.13.3 The metadata_locks Table") table content as
...
* When a pending lock request is canceled by the deadlock
  ...
  ([`ER_LOCK_DEADLOCK`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_lock_deadlock)), its
  row status is updated from `PENDING` to
  `VICTIM`.

* When a pending lock request times out
  ([`ER_LOCK_WAIT_TIMEOUT`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_lock_wait_timeout)),
...
The [`metadata_locks`](performance-schema-metadata-locks-table.html "29.12.13.3 The metadata_locks Table") table has
these columns:
...
for the [`metadata_locks`](performance-schema-metadata-locks-table.html "29.12.13.3 The metadata_locks Table") table.