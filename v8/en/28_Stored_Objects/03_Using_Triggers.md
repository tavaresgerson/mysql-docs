...
* There are some restrictions on the use of triggers; see Section 27.8, “Restrictions on Stored Programs”.

* Binary logging for triggers takes place as described in Section 27.7, “Stored Program Binary Logging”.
...
To destroy the trigger, use a [`DROP TRIGGER`](drop-trigger.html "15.1.34 DROP TRIGGER Statement") statement. You must specify the schema name if the trigger is not in the default schema:
...
* Query the `TRIGGERS` table of the `INFORMATION_SCHEMA` database. See Section 28.3.45, “The INFORMATION_SCHEMA TRIGGERS Table”.
