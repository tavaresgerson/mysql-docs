...
* `INFORMATION_SCHEMA` tables in MySQL 8.0 are closely tied to the data dictionary, resulting in several usage differences:
  ...
  + [`CREATE TABLE dst_tbl LIKE src_tbl`](create-table-like.html "15.1.20.3 CREATE TABLE ... LIKE Statement") requires that *`src_tbl`* be a base table and fails if it is an `INFORMATION_SCHEMA` table that is a view on data dictionary tables.
