...
The preceding improvements also apply to `SHOW` statements that display information corresponding to the `INFORMATION_SCHEMA` tables that are views on data dictionary tables. For example, [`SHOW DATABASES`](show-databases.html "15.7.7.14 SHOW DATABASES Statement") displays the same information as the `SCHEMATA` table.
...
`INFORMATION_SCHEMA` tables in MySQL 8.0 are closely tied to the data dictionary, resulting in several usage differences. See Section 16.7, “Data Dictionary Usage Differences”.