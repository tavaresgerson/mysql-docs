...
* X Plugin enables MySQL Server to communicate with clients
  using X Protocol. See [Section 22.5, “X Plugin”](x-plugin.html "22.5 X Plugin").
...
are relevant. (See [Section 7.1.8, “Server System Variables”](server-system-variables.html "7.1.8 Server System Variables"),
and [Section 7.1.10, “Server Status Variables”](server-status-variables.html "7.1.10 Server Status Variables").)

* The [`thread_cache_size`](server-system-variables.html#sysvar_thread_cache_size)
  ...
  [`thread_cache_size`](server-system-variables.html#sysvar_thread_cache_size) to
...
* To monitor the number of threads in the cache and how many
  ...
  [`Threads_cached`](server-status-variables.html#statvar_Threads_cached) and
...
* When the thread stack is too small, this limits the
  ...
  *`N`* bytes for each thread, start
  the server with
  [`thread_stack`](server-system-variables.html#sysvar_thread_stack) set to
  *`N`*.
...
to increase [`max_connections`](server-system-variables.html#sysvar_max_connections)
if more clients attempt to connect simultaneously then the
server is configured to handle (see
[Section B.3.2.5, “Too many connections”](too-many-connections.html "B.3.2.5 Too many connections")). If the server refuses
a connection because the
[`max_connections`](server-system-variables.html#sysvar_max_connections) limit is
...
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") actually permits
...
PROCESSLIST`](show-processlist.html "15.7.7.29 SHOW PROCESSLIST Statement") to diagnose problems even if the maximum
...
[`max_connections`](server-system-variables.html#sysvar_max_connections) server system
variable. In those releases, if the server has reached the
[`max_connections`](server-system-variables.html#sysvar_max_connections) limit when
...
[`max_connections`](server-system-variables.html#sysvar_max_connections) limit and are
...
[`max_connections`](server-system-variables.html#sysvar_max_connections) can be set)
...
Increasing the
[`max_connections`](server-system-variables.html#sysvar_max_connections) value
increases the number of file descriptors that
[**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") requires. If the required number of
descriptors are not available, the server reduces the value of
[`max_connections`](server-system-variables.html#sysvar_max_connections). For comments
on file descriptor limits, see [Section 10.4.3.1, “How MySQL Opens and Closes Tables”](table-cache.html "10.4.3.1 How MySQL Opens and Closes Tables").
...
to do so. See also [Section B.3.2.16, “File Not Found and Similar Errors”](not-enough-file-handles.html "B.3.2.16 File Not Found and Similar Errors").