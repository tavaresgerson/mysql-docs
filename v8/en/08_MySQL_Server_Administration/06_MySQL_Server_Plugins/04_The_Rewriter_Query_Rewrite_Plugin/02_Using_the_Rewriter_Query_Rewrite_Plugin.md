...
[`rewriter_enabled`](rewriter-query-rewrite-plugin-reference.html#sysvar_rewriter_enabled) system
variable. By default, the `Rewriter` plugin is
enabled when you install it (see
[Section 7.6.4.1, “Installing or Uninstalling the Rewriter Query Rewrite Plugin”](rewriter-query-rewrite-plugin-installation.html "7.6.4.1 Installing or Uninstalling the Rewriter Query Rewrite Plugin")).
...
* As of MySQL 8.0.12: [`SELECT`](select.html "15.2.13 SELECT Statement"),
...
* Prior to MySQL 8.0.12: [`SELECT`](select.html "15.2.13 SELECT Statement")
  only.
...
[`SKIP_QUERY_REWRITE`](privileges-provided.html#priv_skip-query-rewrite) privilege are
...
programs including [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files"),
[**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program"), [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program"), and
[**mysqlpump**](mysqlpump.html "6.5.6 mysqlpump — A Database Backup Program"); for this reason, you should grant
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statements to match, and
...
[Section 15.5.1, “PREPARE Statement”](prepare.html "15.5.1 PREPARE Statement")). Within a pattern template,
...
(as defined in [Section 11.1, “Literal Values”](literals.html "11.1 Literal Values")), so you can put a
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statement, but does for
...
[`DELETE`](delete.html "15.2.2 DELETE Statement") statements to
...
[`INSERT`](insert.html "15.2.7 INSERT Statement") statements needed look
like this:
...
[`Rewriter_reload_error`](rewriter-query-rewrite-plugin-reference.html#statvar_Rewriter_reload_error) status
variable to `ON`.
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statements and whether
the rule pattern matches them.

<table summary="How the Rewriter plugin matches prepared statements against the pattern SELECT ?,3."><col style="width: 50%"/><col style="width: 50%"/><thead><tr>
...
[`character_set_client`](server-system-variables.html#sysvar_character_set_client) value is
