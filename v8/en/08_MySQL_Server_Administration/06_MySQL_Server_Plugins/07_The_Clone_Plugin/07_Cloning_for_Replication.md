...
[Section 19.2.4.2, “Replication Metadata Repositories”](replica-logs-status.html "19.2.4.2 Replication Metadata Repositories"). Note that if the settings
[`master_info_repository=FILE`](replication-options-replica.html#sysvar_master_info_repository) and
...
2. For a replica in a source/replica MySQL replication
   ...
   instructions, see [Section 7.6.7.3, “Cloning Remote Data”](clone-plugin-remote.html "7.6.7.3 Cloning Remote Data").
...
3. After the cloning operation completes successfully, if you
   ...
   * For binary log file position based replication, if the
     ...
     [`clone_status`](performance-schema-clone-status-table.html "29.12.19.1 The clone_status Table") table.
...
4. If you need to set up cloned replication channels manually,
   ...
   * To add a recipient MySQL server instance to a MySQL
     ...
     [`CHANGE MASTER TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement")
