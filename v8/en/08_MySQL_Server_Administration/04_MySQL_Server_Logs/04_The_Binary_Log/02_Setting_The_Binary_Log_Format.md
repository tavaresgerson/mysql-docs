...
* If the [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 8.0") storage engine is
  enabled.
...
If you are using [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") tables and
...
[`--binlog-row-event-max-size`](replication-options-binary-log.html#option_mysqld_binlog-row-event-max-size) set a
