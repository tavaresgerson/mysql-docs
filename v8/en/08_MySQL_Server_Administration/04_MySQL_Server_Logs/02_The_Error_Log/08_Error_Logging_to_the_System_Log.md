...
It is possible to have [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") write the error
...
* Event Log entries have a source of `MySQL`
  (or `MySQL-tag`
  if [`syseventlog.tag`](server-system-variables.html#sysvar_syseventlog.tag) is
  defined as *`tag`*).
...
* [`syseventlog.facility`](server-system-variables.html#sysvar_syseventlog.facility): The
...
* [`syseventlog.tag`](server-system-variables.html#sysvar_syseventlog.tag): This
...
[`log_syslog_include_pid`](server-system-variables.html#sysvar_log_syslog_include_pid), and
