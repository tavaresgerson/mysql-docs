...
3. Set up the replica to use the source. Using the
   [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client, this can be accomplished with
   the [`CHANGE REPLICATION SOURCE
   TO`](change-replication-source-to.html "15.4.2.3 CHANGE REPLICATION SOURCE TO Statement") statement (beginning with NDB 8.0.23) or
   ...
   *`replica_password`* is the password
   ...
   statement, see [Section 15.4.2.1, “CHANGE MASTER TO Statement”](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement").

   To provide replication backup capability, you also need to add
   an [`--ndb-connectstring`](mysql-cluster-options-variables.html#option_mysqld_ndb-connectstring) option
   to the replica's `my.cnf` file prior
   to starting the replication process. See
   [Section 25.7.9, “NDB Cluster Backups With NDB Cluster Replication”](mysql-cluster-replication-backups.html "25.7.9 NDB Cluster Backups With NDB Cluster Replication"), for
   details.
...
4. If the source cluster is already in use, you can create a
   ...
   [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client to import the data from the
   dumpfile into the replica database as shown here, where
   *`dump_file`* is the name of the file
   that was generated using [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") on the
   ...
   [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program"), see [Section 6.5.4, “mysqldump — A Database Backup Program”](mysqldump.html "6.5.4 mysqldump — A Database Backup Program").
   ...
   [`--skip-slave-start`](replication-options-replica.html#option_mysqld_skip-slave-start) option on
   ...
   [`skip_replica_start`](replication-options-replica.html#sysvar_skip_replica_start) instead.
...
5. Ensure that each MySQL server acting as a replication source
   ...
   [Section 19.2.1, “Replication Formats”](replication-formats.html "19.2.1 Replication Formats").) In addition, we
   strongly recommend enabling the
   [`replica_allow_batching`](mysql-cluster-options-variables.html#sysvar_replica_allow_batching) system
   ...
   [`--ndb-replica-batch-size`](mysql-cluster-options-variables.html#sysvar_ndb_replica_batch_size) to set
   the batch size used for writes on the replica instead of
   `--ndb-batch-size`, and
   [`--ndb-replica-blob-write-batch-bytes`](mysql-cluster-options-variables.html#sysvar_ndb_replica_blob_write_batch_bytes)
   ...
   [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") process. See
   [Section 25.7.6, “Starting NDB Cluster Replication (Single Replication Channel)”](mysql-cluster-replication-starting.html "25.7.6 Starting NDB Cluster Replication (Single Replication Channel)"), for more
   information.