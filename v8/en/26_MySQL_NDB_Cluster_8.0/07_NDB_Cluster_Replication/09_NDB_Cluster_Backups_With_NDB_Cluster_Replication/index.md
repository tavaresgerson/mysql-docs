...
[Section 25.7.5, “Preparing the NDB Cluster for Replication”](mysql-cluster-replication-preparation.html "25.7.5 Preparing the NDB Cluster for Replication"), and the
...
1. There are two different methods by which the backup may be
   ...
   * **Method A.**
     ...
     *`management_host`* is the IP
     ...
     [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 8.0") management server for
   ...
   * **Method B.**
     ...
     [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 8.0") management client as
     part of the [`START
     BACKUP`](mysql-cluster-backup-using-management-client.html "25.6.8.2 Using The NDB Cluster Management Client to Create a Backup") command. This can be done as shown
...
2. Copy the cluster backup files to the replica that is being
   ...
   [**ndbd**](mysql-cluster-programs-ndbd.html "25.5.1 ndbd — The NDB Cluster Data Node Daemon") process for the source cluster has
...
3. Create any databases on the replica cluster that are present
   ...
   A [`CREATE DATABASE`](create-database.html "15.1.12 CREATE DATABASE Statement") (or
   [`CREATE
   SCHEMA`](create-database.html "15.1.12 CREATE DATABASE Statement")) statement corresponding to each database
...
5. You can now start the cluster restoration process on the
   replica using the [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "25.5.23 ndb_restore — Restore an NDB Cluster Backup") command for
   ...
   *`dir`* is the path to the directory
   ...
   the [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "25.5.23 ndb_restore — Restore an NDB Cluster Backup") commands corresponding to
   the remaining backup files, the `-m` option
   should *not* be used.

   For restoring from a source cluster with four data nodes (as
   shown in the figure in
   [Section 25.7, “NDB Cluster Replication”](mysql-cluster-replication.html "25.7 NDB Cluster Replication")) where the backup
   ...
   The `-e` (or
   [`--restore-epoch`](mysql-cluster-programs-ndb-restore.html#option_ndb_restore_restore-epoch)) option
   in the final invocation of [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "25.5.23 ndb_restore — Restore an NDB Cluster Backup") in
...
7. Using `@latest` as the epoch value obtained
   ...
   [`SHOW MASTER STATUS`](show-master-status.html "15.7.7.23 SHOW MASTER STATUS Statement") on the
...
8. Using the values obtained in the previous step, you can now
   ...
   [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client. In NDB 8.0.23 and later, use
