...
1. Start the MySQL replication source server by issuing this
   ...
   This starts the server's [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
   ...
   [`--ndb-log-bin`](mysql-cluster-options-variables.html#sysvar_ndb_log_bin) option; this is a
   ...
   [`--binlog-format=MIXED`](replication-options-binary-log.html#sysvar_binlog_format), in
...
2. Start the MySQL replica server as shown here:
   ...
   [`START REPLICA`](start-replica.html "15.4.2.6 START REPLICA Statement") statement has
   ...
   [`--skip-replica-start`](replication-options-replica.html#option_mysqld_skip-replica-start) and
...
3. It is necessary to synchronize the replica server with the
   ...
   [Section 25.7.8, “Implementing Failover with NDB Cluster Replication”](mysql-cluster-replication-failover.html "25.7.8 Implementing Failover with NDB Cluster Replication"), for
...
4. Finally, instruct the replica to begin applying replication by
   issuing this command from the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client
   on the replica:
...
[`replica_allow_batching`](mysql-cluster-options-variables.html#sysvar_replica_allow_batching) (NDB
8.0.26 and later) or
[`slave_allow_batching`](mysql-cluster-options-variables.html#sysvar_slave_allow_batching) (prior to
NDB 8.0.26) on the replicas' [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
...
an appropriate [`SHOW VARIABLES`](show-variables.html "15.7.7.41 SHOW VARIABLES Statement")
