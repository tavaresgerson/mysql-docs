...
instance of [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "25.5.23 ndb_restore — Restore an NDB Cluster Backup"). When entering single
...
[**ndb\_mgm**](mysql-cluster-programs-ndb-mgm.html "25.5.5 ndb_mgm — The NDB Cluster Management Client") client to see when the cluster has
entered single user mode. You can also check the
`status` column of the
[`ndbinfo.nodes`](mysql-cluster-ndbinfo-nodes.html "25.6.16.47 The ndbinfo nodes Table") table (see
[Section 25.6.16.47, “The ndbinfo nodes Table”](mysql-cluster-ndbinfo-nodes.html "25.6.16.47 The ndbinfo nodes Table"), for more
information).
