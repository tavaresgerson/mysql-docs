...
You can see in the **mysql** client whether a MySQL server is connected to the cluster using [`SHOW PROCESSLIST`](show-processlist.html "15.7.7.29 SHOW PROCESSLIST Statement"). If the MySQL server is connected to the cluster, and you have the `PROCESS` privilege, then the first row of the output is as shown here:
