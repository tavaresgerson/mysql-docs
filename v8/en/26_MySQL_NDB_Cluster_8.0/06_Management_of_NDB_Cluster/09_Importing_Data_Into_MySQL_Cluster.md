...
If a data import operation does not complete successfully, for whatever reason, you should be prepared to perform any necessary cleanup including possibly one or more [`DROP TABLE`](drop-table.html "15.1.32 DROP TABLE Statement") statements, [`DROP DATABASE`](drop-database.html "15.1.24 DROP DATABASE Statement") statements, or both. Failing to do so may leave the database in an inconsistent state.