...
* [`SHOW ENGINE NDB STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement"), [`SHOW ENGINE NDBCLUSTER STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement")
  ...
  See Section 15.7.7.15, “SHOW ENGINE Statement”, for a usage example and more detailed information.
...
* `SELECT * FROM INFORMATION_SCHEMA.ENGINES [WHERE ENGINE LIKE 'NDB%']`
  ...
  This is the equivalent of [`SHOW ENGINES`](show-engines.html "15.7.7.16 SHOW ENGINES Statement"), but uses the `ENGINES` table of the `INFORMATION_SCHEMA` database. Unlike the case with the `SHOW ENGINES` statement, it is possible to filter the results using a `LIKE` clause, and to select specific columns to obtain information that may be of use in scripts. For example, the following query shows whether the server was built with `NDB` support and, if so, whether it is enabled:
...
* `SELECT * FROM performance_schema.global_variables WHERE VARIABLE_NAME LIKE 'NDB%'`
  ...
  Unlike the case with the [`SHOW VARIABLES`](show-variables.html "15.7.7.41 SHOW VARIABLES Statement") statement, it is possible to select individual columns. For example:
...
* `SELECT * FROM INFORMATION_SCHEMA.PLUGINS WHERE PLUGIN_NAME LIKE 'NDB%'`
  ...
  *************************** 3. row ***************************
  ...
  You can also use the [`SHOW PLUGINS`](show-plugins.html "15.7.7.25 SHOW PLUGINS Statement") statement to display this information, but the output from that statement cannot easily be filtered. See also The MySQL Plugin API, which describes where and how the information in the `PLUGINS` table is obtained.

You can also query the tables in the `ndbinfo` information database for real-time data about many NDB Cluster operations. See Section 25.6.16, “ndbinfo: The NDB Cluster Information Database”.