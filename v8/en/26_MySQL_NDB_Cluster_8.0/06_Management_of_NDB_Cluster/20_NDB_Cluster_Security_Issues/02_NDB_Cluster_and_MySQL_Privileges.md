...
* Execute metadata statements such as `SHOW DATABASES` statement (to obtain a list of all `NDB` databases on the server) or [`SHOW TABLES FROM some_ndb_database`](show-tables.html "15.7.7.39 SHOW TABLES Statement") statement to obtain a list of all `NDB` tables in a given database
