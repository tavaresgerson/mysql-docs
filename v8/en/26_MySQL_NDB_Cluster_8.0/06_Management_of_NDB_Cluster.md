...
To force creation of a single-threaded backup that can easily be imported by **ndb_restore** from an NDB release prior to 8.0, you can set [`EnableMultithreadedBackup = 0`](mysql-cluster-ndbd-definition.html#ndbparam-ndbmtd-enablemultithreadedbackup) for all data nodes (you can do this by setting the parameter in the `[ndbd default]` section of the `config.ini` global configuration file). It is also possible to restore a parallel backup to a cluster running an older version of `NDB`. See Section 25.5.23.1.1, “Restoring an NDB backup to a previous version of NDB Cluster”, for more information.
