...
* `delimiter str`,
  ...
  [Multiple Statement Execution Support](/doc/c-api/8.0/en/c-api-multiple-queries.html)), and for parsing
...
* `resetconnection`, `\x`
  ...
  [`mysql_change_user()`](/doc/c-api/8.0/en/mysql-change-user.html) or an
  auto-reconnect except that the connection is not closed and
  reopened, and re-authentication is not done. See
  [mysql\_change\_user()](/doc/c-api/8.0/en/mysql-change-user.html), and
...
<td>Print an asterisk (<code class="literal">*</code>) if the current session is
              inside a transaction block (from MySQL 8.0.28)</td>
</tr><tr>
