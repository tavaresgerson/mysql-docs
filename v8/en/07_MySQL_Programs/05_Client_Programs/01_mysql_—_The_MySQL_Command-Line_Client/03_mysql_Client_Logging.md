...
The [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client can do these types of
logging for statements executed interactively:

* On Unix, [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") writes the statements to a
...
* On all platforms, if the `--syslog` option is
  given, [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") writes the statements to the
...
* [How Logging Occurs](mysql-logging.html#mysql-logging-how-logging-occurs "How Logging Occurs")
* [Controlling the History File](mysql-logging.html#mysql-logging-history-file "Controlling the History File")
* [syslog Logging Characteristics](mysql-logging.html#mysql-logging-syslog "syslog Logging Characteristics")
...
* Statements are logged only when executed interactively.
  ...
  logging by using the [`--batch`](mysql-command-options.html#option_mysql_batch)
...
* [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") logs each nonignored, nonempty
  statement line individually.

* If a nonignored statement spans multiple lines (not
  including the terminating delimiter),
  [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") concatenates the lines to form the
...
In this case, [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") logs the
“SELECT”, “'Today is'”,
“,”, “CURDATE()”, and “;”
...
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") ignores for logging purposes statements
...
[`--histignore`](mysql-command-options.html#option_mysql_histignore) option or set the
...
invoke [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") like this:
...
contain passwords. See [Section 8.1.2.1, “End-User Guidelines for Password Security”](password-security-user.html "8.1.2.1 End-User Guidelines for Password Security").
Statements in the file are accessible from the
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client when the
...
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") writes interactive statements to the
...
* `MYSQL_USER`
  ...
  [`--user`](mysql-command-options.html#option_mysql_user) option) or
  `--` if the user is unknown.
