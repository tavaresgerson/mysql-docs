...
[1.6.1 MySQL Extensions to Standard SQL](extensions-to-ansi.html)

[1.6.2 MySQL Differences from Standard SQL](differences-from-ansi.html)

[1.6.3 How MySQL Deals with Constraints](constraints.html)
...
of this strategy. See [Section 15.2.5, “HANDLER Statement”](handler.html "15.2.5 HANDLER Statement").
...
[Chapter 25, *MySQL NDB Cluster 8.0*](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 8.0").

We implement XML functionality which supports most of the W3C
XPath standard. See [Section 14.11, “XML Functions”](xml-functions.html "14.11 XML Functions").

MySQL supports a native JSON data type as defined by RFC 7159, and
based on the ECMAScript standard (ECMA-262). See
[Section 13.5, “The JSON Data Type”](json.html "13.5 The JSON Data Type"). MySQL also implements a subset of the
SQL/JSON functions specified by a pre-publication draft of the
SQL:2016 standard; see [Section 14.17, “JSON Functions”](json-functions.html "14.17 JSON Functions"), for more
information.
...
To run MySQL Server in ANSI mode, start [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
with the [`--ansi`](server-options.html#option_mysqld_ansi) option. Running the
...
[`sql_mode`](server-system-variables.html#sysvar_sql_mode) system variable to
...
[`--ansi`](server-options.html#option_mysqld_ansi) is not quite the same as
setting the SQL mode to `'ANSI'` because the
[`--ansi`](server-options.html#option_mysqld_ansi) option also sets the
transaction isolation level.

See [Section 7.1.7, “Server Command Options”](server-options.html "7.1.7 Server Command Options").