...
To repair a table, use [`REPAIR TABLE`](repair-table.html "15.7.3.5 REPAIR TABLE Statement"), which copies as many valid rows from the existing `CSV` data as possible, and then replaces the existing `CSV` file with the recovered rows. Any rows beyond the corrupted data are lost.
