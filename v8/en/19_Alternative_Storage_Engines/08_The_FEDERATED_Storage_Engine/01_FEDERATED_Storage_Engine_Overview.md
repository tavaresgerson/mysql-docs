...
[`mysql_real_query()`](/doc/c-api/8.0/en/mysql-real-query.html) to send the
statement. To read a result set, it uses
[`mysql_store_result()`](/doc/c-api/8.0/en/mysql-store-result.html) and fetches
rows one at a time using
[`mysql_fetch_row()`](/doc/c-api/8.0/en/mysql-fetch-row.html).