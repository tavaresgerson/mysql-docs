...
[B-trees](glossary.html#glos_b_tree "B-tree"). Exceptions: Indexes
on spatial data types use R-trees; `MEMORY`
tables also support [hash
indexes](glossary.html#glos_hash_index "hash index"); `InnoDB` uses inverted lists
...
* To eliminate rows from consideration. If there is a choice
  ...
  [selective](glossary.html#glos_selectivity "selectivity") index).
...
* To retrieve rows from other tables when performing joins.
  ...
  [`CHAR`](char.html "13.3.2 The CHAR and VARCHAR Types") are considered the same
...
* To find the [`MIN()`](aggregate-functions.html#function_min) or
  [`MAX()`](aggregate-functions.html#function_max) value for a specific
  ...
  for each [`MIN()`](aggregate-functions.html#function_min) or
  [`MAX()`](aggregate-functions.html#function_max) expression and replaces
...
* To sort or group a table if the sorting or grouping is done
  ...
  [Section 10.2.1.17, “GROUP BY Optimization”](group-by-optimization.html "10.2.1.17 GROUP BY Optimization"), and
...
* In some cases, a query can be optimized to retrieve values
  ...
  [covering index](glossary.html#glos_covering_index "covering index").)
...
for the query. See [Section 10.2.1.23, “Avoiding Full Table Scans”](table-scan-avoidance.html "10.2.1.23 Avoiding Full Table Scans") for
details.