...
as discussed in [Section 10.2.1.19, “LIMIT Query Optimization”](limit-optimization.html "10.2.1.19 LIMIT Query Optimization").
...
* Two columns in an `ORDER BY` can sort
  ...
  *`key_part2`* is ascending. It
  ...
  descending. See [Section 10.3.13, “Descending Indexes”](descending-indexes.html "10.3.13 Descending Indexes").
...
* The query joins many tables, and the columns in the
  ...
  [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") output that does
...
* There is an index on only a prefix of a column named in
  ...
  [`CHAR(20)`](char.html "13.3.2 The CHAR and VARCHAR Types") column are
...
to set [`sort_buffer_size`](server-system-variables.html#sysvar_sort_buffer_size) to
...
[`max_length_for_sort_data`](server-system-variables.html#sysvar_max_length_for_sort_data) is
...
(see [Section 10.8.1, “Optimizing Queries with EXPLAIN”](using-explain.html "10.8.1 Optimizing Queries with EXPLAIN")), you can check whether
...
value of [`sort_buffer_size`](server-system-variables.html#sysvar_sort_buffer_size).
(Prior to MySQL 8.0.12, the optimizer always allocates
[`sort_buffer_size`](server-system-variables.html#sysvar_sort_buffer_size) bytes for
...
[`sort_buffer_size`](server-system-variables.html#sysvar_sort_buffer_size) bytes.)
...
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") does not distinguish
