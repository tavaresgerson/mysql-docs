...
* [Nested-Loop Join Algorithm](nested-loop-joins.html#nested-loop-join-algorithm "Nested-Loop Join Algorithm")
...
See [Section 10.2.1.4, “Hash Join Optimization”](hash-joins.html "10.2.1.4 Hash Join Optimization").
...
* Join buffering can be used when the join is of type
  ...
  [`index`](explain-output.html#jointype_index) (in other
  words, when no possible keys can be used, and a full
  scan is done, of either the data or index rows,
  respectively), or
  [`range`](explain-output.html#jointype_range). Use of
  buffering is also applicable to outer joins, as
  described in [Section 10.2.1.12, “Block Nested-Loop and Batched Key Access Joins”](bnl-bka-optimization.html "10.2.1.12 Block Nested-Loop and Batched Key Access Joins").
...
* The [`join_buffer_size`](server-system-variables.html#sysvar_join_buffer_size)
...
value of [`join_buffer_size`](server-system-variables.html#sysvar_join_buffer_size)
increases, up to the point when
[`join_buffer_size`](server-system-variables.html#sysvar_join_buffer_size) is large
