...
by other programs, see [Section 10.11.5, “External Locking”](external-locking.html "10.11.5 External Locking").
...
heavy [`SELECT`](select.html "15.2.13 SELECT Statement") activity for the
table. However, if there are many updates for a table,
[`SELECT`](select.html "15.2.13 SELECT Statement") statements wait until
there are no more updates.

For information on altering the priority of reads and writes,
see [Section 10.11.2, “Table Locking Issues”](table-locking.html "10.11.2 Table Locking Issues").
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statements for a
...
variable. See [Section 10.11.3, “Concurrent Inserts”](concurrent-inserts.html "10.11.3 Concurrent Inserts").

If you acquire a table lock explicitly with
[`LOCK TABLES`](lock-tables.html "15.3.6 LOCK TABLES and UNLOCK TABLES Statements"), you can request a
...
To perform many [`INSERT`](insert.html "15.2.7 INSERT Statement") and
[`SELECT`](select.html "15.2.13 SELECT Statement") operations on a table
...
* [`SELECT`](select.html "15.2.13 SELECT Statement") combined with
  concurrent [`INSERT`](insert.html "15.2.7 INSERT Statement")
