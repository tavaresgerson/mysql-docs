...
As of MySQL 8.0.13, for XA transactions in `PREPARED` state, metadata locks are maintained across client disconnects and server restarts, until an [`XA COMMIT`](xa.html "15.3.8 XA Transactions") or [`XA ROLLBACK`](xa.html "15.3.8 XA Transactions") is executed.