...
Because the statement displayed by [`SHOW WARNINGS`](show-warnings.html "15.7.7.42 SHOW WARNINGS Statement") may contain special markers to provide information about query rewriting or optimizer actions, the statement is not necessarily valid SQL and is not intended to be executed. The output may also include rows with `Message` values that provide additional non-SQL explanatory notes about actions taken by the optimizer.

The following list describes special markers that can appear in the extended output displayed by [`SHOW WARNINGS`](show-warnings.html "15.7.7.42 SHOW WARNINGS Statement"):
...
* `outer_tables semi join (inner_tables)`
  ...
  A semijoin operation. *`inner_tables`* shows the tables that were not pulled out. See [Section 10.2.2.1, “Optimizing IN and EXISTS Subquery Predicates with Semijoin Transformations”](semijoins.html "10.2.2.1 Optimizing IN and EXISTS Subquery Predicates with Semijoin Transformations").
