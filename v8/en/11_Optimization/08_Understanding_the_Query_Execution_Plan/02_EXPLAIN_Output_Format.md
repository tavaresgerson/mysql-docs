...
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") returns a row of
information for each table used in the
[`SELECT`](select.html "15.2.13 SELECT Statement") statement. It lists the
...
* [EXPLAIN Output Columns](explain-output.html#explain-output-columns "EXPLAIN Output Columns")
* [EXPLAIN Join Types](explain-output.html#explain-join-types "EXPLAIN Join Types")
* [EXPLAIN Extra Information](explain-output.html#explain-extra-information "EXPLAIN Extra Information")
* [EXPLAIN Output Interpretation](explain-output.html#explain-output-interpretation "EXPLAIN Output Interpretation")

#### EXPLAIN Output Columns

This section describes the output columns produced by
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement"). Later sections provide
additional information about the
[`type`](explain-output.html#explain-join-types "EXPLAIN Join Types")
and
[`Extra`](explain-output.html#explain-extra-information "EXPLAIN Extra Information")
columns.

Each output row from [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement")
provides information about one table. Each row contains the
values summarized in
[Table 10.1, “EXPLAIN Output Columns”](explain-output.html#explain-output-column-table "Table 10.1 EXPLAIN Output Columns"), and described
...
<table summary="Output columns produced by the EXPLAIN statement."><col style="width: 25%"/><col style="width: 25%"/><col style="width: 50%"/><thead><tr>
...
* `id` (JSON name:
  ...
  The [`SELECT`](select.html "15.2.13 SELECT Statement") identifier. This
  is the sequential number of the
  [`SELECT`](select.html "15.2.13 SELECT Statement") within the query.
...
* `select_type` (JSON name: none)
  ...
  The type of [`SELECT`](select.html "15.2.13 SELECT Statement"), which
  ...
  <table summary="select_type values and the meaning of each value."><col style="width: 25%"/><col style="width: 25%"/><col style="width: 50%"/><thead><tr>
...
<td>None</td>
<td>Simple <a class="link" href="select.html" title="15.2.13 SELECT Statement"><code class="literal">SELECT</code></a> (not using
                    <a class="link" href="union.html" title="15.2.18 UNION Clause"><code class="literal">UNION</code></a> or subqueries)</td>
</tr><tr>
<th scope="row"><code class="literal">PRIMARY</code></th>
<td>None</td>
<td>Outermost <a class="link" href="select.html" title="15.2.13 SELECT Statement"><code class="literal">SELECT</code></a></td>
</tr><tr>
<th scope="row"><a class="link" href="union.html" title="15.2.18 UNION Clause"><code class="literal">UNION</code></a></th>
<td>None</td>
<td>Second or later <a class="link" href="select.html" title="15.2.13 SELECT Statement"><code class="literal">SELECT</code></a> statement in a
                    <a class="link" href="union.html" title="15.2.18 UNION Clause"><code class="literal">UNION</code></a></td>
</tr><tr>
...
<td>Second or later <a class="link" href="select.html" title="15.2.13 SELECT Statement"><code class="literal">SELECT</code></a> statement in a
                    <a class="link" href="union.html" title="15.2.18 UNION Clause"><code class="literal">UNION</code></a>, dependent on
                    outer query</td>
</tr><tr>
<th scope="row"><code class="literal">UNION RESULT</code></th>
<td><code class="literal">union_result</code></td>
<td>Result of a <a class="link" href="union.html" title="15.2.18 UNION Clause"><code class="literal">UNION</code></a>.</td>
</tr><tr>
...
<td>None</td>
<td>First <a class="link" href="select.html" title="15.2.13 SELECT Statement"><code class="literal">SELECT</code></a> in subquery</td>
</tr><tr>
<th scope="row"><code class="literal">DEPENDENT SUBQUERY</code></th>
<td><code class="literal">dependent</code> (<code class="literal">true</code>)</td>
<td>First <a class="link" href="select.html" title="15.2.13 SELECT Statement"><code class="literal">SELECT</code></a> in subquery, dependent on
                    outer query</td>
</tr><tr>
...
<td><code class="literal">cacheable</code> (<code class="literal">false</code>)</td>
<td>The second or later select in a <a class="link" href="union.html" title="15.2.18 UNION Clause"><code class="literal">UNION</code></a>
  ...
  non-[`SELECT`](select.html "15.2.13 SELECT Statement") statements
...
* `type` (JSON name:
  ...
  see
  [`EXPLAIN`
  Join Types](explain-output.html#explain-join-types "EXPLAIN Join Types").
...
* `possible_keys` (JSON name:
  ...
  [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement"). That means that
  ...
  [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") again. See
...
* `key` (JSON name: `key`)
  ...
  your query. See [Section 10.9.4, “Index Hints”](index-hints.html "10.9.4 Index Hints").

  For `MyISAM` tables, running
  [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") helps the
  optimizer choose better indexes. For
  `MyISAM` tables, [**myisamchk
  --analyze**](myisamchk.html "6.6.4 myisamchk — MyISAM Table-Maintenance Utility") does the same. See
  [Section 15.7.3.1, “ANALYZE TABLE Statement”](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement"), and
...
* `ref` (JSON name: `ref`)
  ...
  [`SHOW WARNINGS`](show-warnings.html "15.7.7.42 SHOW WARNINGS Statement") following
  [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") to see the extended
  [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") output. The
...
* `rows` (JSON name:
  ...
  For [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") tables, this
...
* `Extra` (JSON name: none)
  ...
  [`EXPLAIN`
  Extra Information](explain-output.html#explain-extra-information "EXPLAIN Extra Information").
...
The `type` column of
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") output describes how
...
* [`system`](explain-output.html#jointype_system)

  The table has only one row (= system table). This is a
  special case of the
  [`const`](explain-output.html#jointype_const) join type.

* [`const`](explain-output.html#jointype_const)
  ...
  [`const`](explain-output.html#jointype_const) tables are very
  fast because they are read only once.

  [`const`](explain-output.html#jointype_const) is used when you
  compare all parts of a `PRIMARY KEY` or
  `UNIQUE` index to constant values. In the
  following queries, *`tbl_name`* can
  be used as a [`const`](explain-output.html#jointype_const)
  table:
...
* [`eq_ref`](explain-output.html#jointype_eq_ref)
  ...
  [`system`](explain-output.html#jointype_system) and
  [`const`](explain-output.html#jointype_const) types, this is
  ...
  [`eq_ref`](explain-output.html#jointype_eq_ref) can be used for
  ...
  [`eq_ref`](explain-output.html#jointype_eq_ref) join to process
  *`ref_table`*:
...
* [`ref`](explain-output.html#jointype_ref)
  ...
  tables. [`ref`](explain-output.html#jointype_ref) is used if
  ...
  [`ref`](explain-output.html#jointype_ref) can be used for
  indexed columns that are compared using the
  `=` or `<=>`
  operator. In the following examples, MySQL can use a
  [`ref`](explain-output.html#jointype_ref) join to process
  *`ref_table`*:
...
* [`fulltext`](explain-output.html#jointype_fulltext)

  The join is performed using a `FULLTEXT`
  index.

* [`ref_or_null`](explain-output.html#jointype_ref_or_null)

  This join type is like
  [`ref`](explain-output.html#jointype_ref), but with the
  ...
  [`ref_or_null`](explain-output.html#jointype_ref_or_null) join to
  process *`ref_table`*:
  ...
  See [Section 10.2.1.15, “IS NULL Optimization”](is-null-optimization.html "10.2.1.15 IS NULL Optimization").

* [`index_merge`](explain-output.html#jointype_index_merge)
...
* [`unique_subquery`](explain-output.html#jointype_unique_subquery)

  This type replaces
  [`eq_ref`](explain-output.html#jointype_eq_ref) for some
  ...
  [`unique_subquery`](explain-output.html#jointype_unique_subquery) is just
  an index lookup function that replaces the subquery
  completely for better efficiency.

* [`index_subquery`](explain-output.html#jointype_index_subquery)

  This join type is similar to
  [`unique_subquery`](explain-output.html#jointype_unique_subquery). It
...
* [`range`](explain-output.html#jointype_range)
  ...
  [`range`](explain-output.html#jointype_range) can be used when
  ...
  [`BETWEEN`](comparison-operators.html#operator_between),
  [`LIKE`](string-comparison-functions.html#operator_like), or
  [`IN()`](comparison-operators.html#operator_in) operators:
...
* [`index`](explain-output.html#jointype_index)

  The `index` join type is the same as
  [`ALL`](explain-output.html#jointype_all), except that the
  ...
  + If the index is a covering index for the queries and
    ...
    [`ALL`](explain-output.html#jointype_all) because the
...
* [`ALL`](explain-output.html#jointype_all)
  ...
  [`const`](explain-output.html#jointype_const), and usually
  *very* bad in all other cases.
  Normally, you can avoid
  [`ALL`](explain-output.html#jointype_all) by adding indexes
...
* `Child of 'table'
  ...
  *`table`* in a join that can be
  ...
  description of the
  [`ndb_join_pushdown`](mysql-cluster-options-variables.html#sysvar_ndb_join_pushdown) server
...
* `Deleting all rows` (JSON property:
  ...
  For [`DELETE`](delete.html "15.2.2 DELETE Statement"), some storage
  engines (such as [`MyISAM`](myisam-storage-engine.html "18.2 The MyISAM Storage Engine"))
...
* `Impossible WHERE noticed after reading const
  ...
  MySQL has read all
  [`const`](explain-output.html#jointype_const) (and
  [`system`](explain-output.html#jointype_system)) tables and
...
* `No matching rows after partition
  ...
  For [`DELETE`](delete.html "15.2.2 DELETE Statement") or
  [`UPDATE`](update.html "15.2.17 UPDATE Statement"), the optimizer found
  nothing to delete or update after partition pruning. It is
  similar in meaning to `Impossible WHERE`
  for [`SELECT`](select.html "15.2.13 SELECT Statement") statements.
...
* `No tables used` (JSON property:
  ...
  For [`INSERT`](insert.html "15.2.7 INSERT Statement") or
  [`REPLACE`](replace.html "15.2.12 REPLACE Statement") statements,
  [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") displays this value
  when there is no [`SELECT`](select.html "15.2.13 SELECT Statement")
...
* `Plan isn't ready yet` (JSON property:
  ...
  CONNECTION`](explain-for-connection.html "10.8.4 Obtaining Execution Plan Information for a Named Connection") when the optimizer has not finished
...
* `Range checked for each record (index map:
  ...
  use a [`range`](explain-output.html#jointype_range) or
  [`index_merge`](explain-output.html#jointype_index_merge) access
  ...
  [Section 10.2.1.2, “Range Optimization”](range-optimization.html "10.2.1.2 Range Optimization"), and
  [Section 10.2.1.3, “Index Merge Optimization”](index-merge-optimization.html "10.2.1.3 Index Merge Optimization"), with the
  ...
  as shown by [`SHOW INDEX`](show-index.html "15.7.7.22 SHOW INDEX Statement") for
...
* `Recursive` (JSON property:
  ...
  common table expression. See [Section 15.2.20, “WITH (Common Table Expressions)”](with.html "15.2.20 WITH (Common Table Expressions)").
...
* `Scanned N
  ...
  in [Section 10.2.3, “Optimizing INFORMATION\_SCHEMA Queries”](information-schema-optimization.html "10.2.3 Optimizing INFORMATION_SCHEMA Queries"). The
...
* `Using index` (JSON property:
  ...
  `type` is
  [`index`](explain-output.html#jointype_index) and
  `key` is `PRIMARY`.
...
* `Using join buffer (Block Nested Loop)`,
  ...
  [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") output are
  ...
  optimizations, see [Section 10.2.1.4, “Hash Join Optimization”](hash-joins.html "10.2.1.4 Hash Join Optimization"), and
  [Block Nested-Loop Join Algorithm](nested-loop-joins.html#block-nested-loop-join-algorithm "Block Nested-Loop Join Algorithm").

  See [Batched Key Access Joins](bnl-bka-optimization.html#bka-optimization "Batched Key Access Joins"), for information
...
* `Using MRR` (JSON property:
  ...
  strategy. See [Section 10.2.1.11, “Multi-Range Read Optimization”](mrr-optimization.html "10.2.1.11 Multi-Range Read Optimization").
...
* `Using sort_union(...)`, `Using
  ...
  [`index_merge`](explain-output.html#jointype_index_merge) join type.
  See [Section 10.2.1.3, “Index Merge Optimization”](index-merge-optimization.html "10.2.1.3 Index Merge Optimization").
...
* `Using where` (JSON property:
  ...
  [`ALL`](explain-output.html#jointype_all) or
  [`index`](explain-output.html#jointype_index).
...
* `Using where with pushed condition` (JSON
  ...
  This item applies to [`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 8.0")
...
column of the [`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") output.
This should tell you roughly how many rows MySQL must examine
to execute the query. If you restrict queries with the
[`max_join_size`](server-system-variables.html#sysvar_max_join_size) system
variable, this row product also is used to determine which
multiple-table [`SELECT`](select.html "15.2.13 SELECT Statement")
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statement shown here and
...
* The columns being compared have been declared as follows.
  ...
  <table summary="Table names, column names, and data types for the columns being compared in the EXPLAIN example described in the preceding text."><col style="width: 10%"/><col style="width: 25%"/><col style="width: 25%"/><thead><tr>
...
* The tables have the following indexes.
  ...
  <table summary="Indexes for each of the tables that are part of the EXPLAIN example described in the preceding text."><col style="width: 10%"/><col style="width: 40%"/><thead><tr>
<th>Table</th>
<th>Index</th>
</tr></thead><tbody><tr>
...
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") statement produces the
...
Because `type` is
[`ALL`](explain-output.html#jointype_all) for each table, this
...
this context, [`VARCHAR`](char.html "13.3.2 The CHAR and VARCHAR Types") and
[`CHAR`](char.html "13.3.2 The CHAR and VARCHAR Types") are considered the same if
...
[`ALTER TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement") to lengthen
...
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") statement again
...
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") produces the output
shown here:
...
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") produces this result:
...
[`EXPLAIN`](explain.html "15.8.2 EXPLAIN Statement") is an educated guess
...
[`SELECT`](select.html "15.2.13 SELECT Statement") statement and trying to
...
data when [`EXPLAIN
SELECT`](explain.html "15.8.2 EXPLAIN Statement") is used with a subquery; for more
information, see [Section 15.2.15.8, “Derived Tables”](derived-tables.html "15.2.15.8 Derived Tables").