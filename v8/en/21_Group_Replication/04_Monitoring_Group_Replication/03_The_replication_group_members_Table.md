...
[`server_uuid`](replication-options.html#sysvar_server_uuid), as well as the
member's host name and port number, which clients use to connect
to it. The `MEMBER_STATE` column shows one of the
[Section 20.4.2, “Group Replication Server States”](group-replication-server-states.html "20.4.2 Group Replication Server States"), in this case it
