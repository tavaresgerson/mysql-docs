...
1. `name FOR
   ...
   *`name`* is a counter whose type is
   ...
   `AUTO_INCREMENT` in a
   [`CREATE TABLE`](create-table.html "15.1.20 CREATE TABLE Statement") statement, and can
...
query raises [`ER_PARSE_ERROR`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_parse_error).