...
* `CONNECTION_ID()`
  ...
  The value returned by `CONNECTION_ID()` is the same type of value as displayed in the `ID` column of the Information Schema `PROCESSLIST` table, the `Id` column of [`SHOW PROCESSLIST`](show-processlist.html "15.7.7.29 SHOW PROCESSLIST Statement") output, and the `PROCESSLIST_ID` column of the Performance Schema `threads` table.
...
* `ROW_COUNT()`
  ...
  + DML statements other than `SELECT`: The number of affected rows. This applies to statements such as `UPDATE`, `INSERT`, or `DELETE` (as before), but now also to statements such as [`ALTER TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement") and [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement").
