...
[ngram](fulltext-search-ngram.html "14.9.8 ngram Full-Text Parser") full-text
...
[Section 14.9, “Full-Text Search Functions”](fulltext-search.html "14.9 Full-Text Search Functions") applies to the MeCab parser
...
Binary Distribution (Optional)](fulltext-search-mecab.html#install-mecab-binary "Installing MeCab From a Binary Distribution (Optional)"). If you want to build
`mecab` and `mecab-ipadic`
from source, see
[Building MeCab From
Source (Optional)](fulltext-search-mecab.html#build-mecab-from-source "Installing MeCab From Source (Optional)").
...
1. In the MySQL configuration file, set the
   [`mecab_rc_file`](server-system-variables.html#sysvar_mecab_rc_file) configuration
   ...
   [option modifier](option-modifiers.html "6.2.2.4 Program Option Modifiers"). The
   [`mecab_rc_file`](server-system-variables.html#sysvar_mecab_rc_file) option is not
   ...
   location, see [Section 6.2.2.2, “Using Option Files”](option-files.html "6.2.2.2 Using Option Files").
...
2. Also in the MySQL configuration file, set the minimum token
   ...
   defined by [`ft_min_word_len`](server-system-variables.html#sysvar_ft_min_word_len),
...
3. Modify the `mecabrc` configuration file
   ...
   the [`mecab_charset`](server-status-variables.html#statvar_mecab_charset) status
   ...
   * The `ipadic_utf-8` dictionary
   ...
   [`mecab_charset`](server-status-variables.html#statvar_mecab_charset) only
   reports the first supported character set. For example,
   the `ipadic_utf-8` dictionary supports
   both `utf8mb3` and
   `utf8mb4`.
   [`mecab_charset`](server-status-variables.html#statvar_mecab_charset) always
...
5. Install the MeCab parser plugin:
   ...
   [`INSTALL PLUGIN`](install-plugin.html "15.7.4.4 INSTALL PLUGIN Statement"). The plugin
...
6. Verify that the MeCab parser plugin is loaded using the
   [`SHOW PLUGINS`](show-plugins.html "15.7.7.25 SHOW PLUGINS Statement") statement.
...
Information Schema
[`INNODB_FT_INDEX_CACHE`](information-schema-innodb-ft-index-cache-table.html "28.4.18 The INFORMATION_SCHEMA INNODB_FT_INDEX_CACHE Table") table:
...
you can use [`ALTER TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement") or
[`CREATE INDEX`](create-index.html "15.1.15 CREATE INDEX Statement"). For example:
...
4. Compile MySQL using the
   [`WITH_MECAB`](source-configuration-options.html#option_cmake_with_mecab) CMake option. Set
   the [`WITH_MECAB`](source-configuration-options.html#option_cmake_with_mecab) option to
   ...
   [`WITH_MECAB`](source-configuration-options.html#option_cmake_with_mecab) to the custom
