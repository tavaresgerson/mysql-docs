...
* Full-text searches are supported for
  ...
  [`MyISAM`](myisam-storage-engine.html "18.2 The MyISAM Storage Engine") tables only.

* Full-text searches are not supported for partitioned tables.
  See [Section 26.6, “Restrictions and Limitations on Partitioning”](partitioning-limitations.html "26.6 Restrictions and Limitations on Partitioning").
...
* The [`MATCH()`](fulltext-search.html#function_match) column list must
  match exactly the column list in some
  `FULLTEXT` index definition for the table,
  unless this [`MATCH()`](fulltext-search.html#function_match) is
