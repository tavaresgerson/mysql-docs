...
<table frame="box" rules="all" summary="A reference that lists cast functions and operators."><col style="width: 25%"/><col style="width: 62%"/><col style="width: 12%"/><thead><tr><th>Name</th>
<th>Description</th>
<th>Deprecated</th>
</tr></thead><tbody><tr><th scope="row"><a class="link" href="cast-functions.html#operator_binary"><code class="literal">BINARY</code></a></th>
<td>
...
* [Cast Function and Operator Descriptions](cast-functions.html#cast-function-descriptions "Cast Function and Operator Descriptions")
* [Character Set Conversions](cast-functions.html#cast-character-set-conversions "Character Set Conversions")
* [Character Set Conversions for String Comparisons](cast-functions.html#cast-string-comparisons "Character Set Conversions for String Comparisons")
* [Cast Operations on Spatial Types](cast-functions.html#cast-spatial-types "Cast Operations on Spatial Types")
* [Other Uses for Cast Operations](cast-functions.html#cast-other-uses "Other Uses for Cast Operations")

### Cast Function and Operator Descriptions

* [`BINARY`](cast-functions.html#operator_binary)
  *`expr`*

  The [`BINARY`](cast-functions.html#operator_binary) operator converts
  the expression to a binary string (a string that has the
  `binary` character set and
  `binary` collation). A common use for
  [`BINARY`](cast-functions.html#operator_binary) is to force a character
  string comparison to be done byte by byte using numeric byte
  values rather than character by character. The
  [`BINARY`](cast-functions.html#operator_binary) operator also causes
  ...
  sets, see [Section 12.8.5, “The binary Collation Compared to \_bin Collations”](charset-binary-collations.html "12.8.5 The binary Collation Compared to _bin Collations").
  ...
  BINARY)`](cast-functions.html#function_cast) instead.
  ...
  In a comparison, [`BINARY`](cast-functions.html#operator_binary) affects
  ...
  The [`BINARY`](cast-functions.html#operator_binary) operator in
  ...
  definition of a [`CHAR`](char.html "13.3.2 The CHAR and VARCHAR Types"),
  [`VARCHAR`](char.html "13.3.2 The CHAR and VARCHAR Types"), or
  [`TEXT`](blob.html "13.3.4 The BLOB and TEXT Types") column causes the column
  ...
  If [`BINARY`](cast-functions.html#operator_binary) is invoked from
  ...
  of the [`--binary-as-hex`](mysql-command-options.html#option_mysql_binary-as-hex). For
...
* [`CAST(expr AS
  type [ARRAY])`](cast-functions.html#function_cast)

  [`CAST(timestamp_value
  AT TIME ZONE timezone_specifier
  AS
  DATETIME[(precision)])`](cast-functions.html#function_cast)
  ...
  *`timezone_specifier`*: [INTERVAL]
  ...
  With
  [`CAST(expr AS
  type`](cast-functions.html#function_cast) syntax, the
  [`CAST()`](cast-functions.html#function_cast) function takes an
  expression of any type and produces a result value of the
  specified type. This operation may also be expressed as
  [`CONVERT(expr,
  type)`](cast-functions.html#function_convert), which is
  ...
  + `CHAR[(N)]
    ...
    Produces a string with the
    [`VARCHAR`](char.html "13.3.2 The CHAR and VARCHAR Types") data type, unless
  ...
  + `DATE`
    ...
    Produces a [`DATE`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types") value.

  + `DATETIME[(M)]`

    Produces a [`DATETIME`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types")
  ...
  + `DECIMAL[(M[,D])]`
    ...
    Produces a [`DECIMAL`](fixed-point-types.html "13.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") value.
  ...
  + `DOUBLE`
    ...
    Produces a [`DOUBLE`](floating-point-types.html "13.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") result.
  ...
  + `FLOAT[(p)]`
    ...
    [`FLOAT`](floating-point-types.html "13.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE"). If
    *`p`* is provided and 0 <=
    < *`p`* <= 24, the result
    is of type `FLOAT`. If 25 <=
    *`p`* <= 53, the result is of
    type [`DOUBLE`](floating-point-types.html "13.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE"). If
    *`p`* < 0 or
    *`p`* > 53, an error is
  ...
  + `JSON`
    ...
    Produces a [`JSON`](json.html "13.5 The JSON Data Type") value.
    For details on the rules for conversion of values
    between [`JSON`](json.html "13.5 The JSON Data Type") and other
    types, see [Comparison and Ordering of JSON Values](json.html#json-comparison "Comparison and Ordering of JSON Values").

  + `NCHAR[(N)]`

    Like `CHAR`, but produces a string with
    the national character set. See
    [Section 12.3.7, “The National Character Set”](charset-national.html "12.3.7 The National Character Set").
  ...
  + `REAL`
    ...
    [`REAL`](floating-point-types.html "13.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE"). This is actually
    `FLOAT` if the
    [`REAL_AS_FLOAT`](sql-mode.html#sqlmode_real_as_float) SQL mode
    is enabled; otherwise the result is of type
    `DOUBLE`.

  + `SIGNED [INTEGER]`

    Produces a signed [`BIGINT`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT")
    value.
  ...
  + *`spatial_type`*
    ...
    [`CONVERT()`](cast-functions.html#function_convert) support casting
  ...
  + `TIME[(M)]`
    ...
    Produces a [`TIME`](time.html "13.2.3 The TIME Type") value. If
  ...
  + `YEAR`
    ...
    Produces a [`YEAR`](year.html "13.2.4 The YEAR Type") value.
    ...
    - For a [`DATE`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types"),
      [`DATETIME`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types"), or
      [`TIMESTAMP`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types") value,
      return the `YEAR` portion of the
      value. For a [`TIME`](time.html "13.2.3 The TIME Type")
    ...
    - If the argument is of type
      ...
      [`DECIMAL`](fixed-point-types.html "13.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC"), or
      [`REAL`](floating-point-types.html "13.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE"), round the value
    ...
    - An argument of type
      [`GEOMETRY`](spatial-type-overview.html "13.4.1 Spatial Data Types") cannot be
      converted to [`YEAR`](year.html "13.2.4 The YEAR Type").
  ...
  [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") allows the use of an
  ...
  INDEX`](create-index.html "15.1.15 CREATE INDEX Statement"), [`CREATE
  TABLE`](create-table.html "15.1.20 CREATE TABLE Statement"), and [`ALTER
  TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement") statements. `ARRAY` is not
  ...
  *`type`* following the
  ...
  [`CONVERT()`](cast-functions.html#function_convert), unlike
  [`CAST()`](cast-functions.html#function_cast), does
  *not* support multi-valued index
  creation or the `ARRAY` keyword.

  Beginning with MySQL 8.0.22, `CAST()`
  supports retrieval of a
  [`TIMESTAMP`](datetime.html "13.2.2 The DATE, DATETIME, and TIMESTAMP Types") value as being in
...
* [`CONVERT(expr
  USING transcoding_name)`](cast-functions.html#function_convert)

  [`CONVERT(expr,type)`](cast-functions.html#function_convert)

  [`CONVERT(expr
  USING transcoding_name)`](cast-functions.html#function_convert)
  is standard SQL syntax. The non-`USING`
  form of [`CONVERT()`](cast-functions.html#function_convert) is ODBC
  ...
  [`CONVERT(expr,
  type)`](cast-functions.html#function_convert) syntax (without
  ...
  *`type`* value specifying a result
  ...
  [`CAST(expr AS
  type)`](cast-functions.html#function_cast), which is
  ...
  [`BINARY`](binary-varbinary.html "13.3.3 The BINARY and VARBINARY Types") values to a
  ...
  [SQL Changes](upgrading-from-previous-series.html#upgrade-sql-changes "SQL Changes"), for information
  about how to handle this situation.

### Character Set Conversions

[`CONVERT()`](cast-functions.html#function_convert) with a
...
`USING`), or
[`CAST(expr AS
type)`](cast-functions.html#function_cast), which is equivalent:
...
and collation (see [Section 12.4, “Connection Character Sets and Collations”](charset-connection.html "12.4 Connection Character Sets and Collations")).

A `COLLATE` clause is not permitted within a
[`CONVERT()`](cast-functions.html#function_convert) or
[`CAST()`](cast-functions.html#function_cast) call, but you can apply it
...
[`CAST()`](cast-functions.html#function_cast), or
[`CONVERT()`](cast-functions.html#function_convert), which convert a string
...
[`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") value or other binary string
...
[`CONVERT()`](cast-functions.html#function_convert) or
[`CAST()`](cast-functions.html#function_cast) function to convert the
...
[`LIKE`](string-comparison-functions.html#operator_like) operation is not
...
[`CONVERT()`](cast-functions.html#function_convert) call:
...
[`CONVERT()`](cast-functions.html#function_convert) and
[`CAST()`](cast-functions.html#function_cast) can be used more generally
...
Be aware that if you apply [`BINARY`](cast-functions.html#operator_binary),
[`CAST()`](cast-functions.html#function_cast), or
[`CONVERT()`](cast-functions.html#function_convert) to an indexed column,
MySQL may not be able to use the index efficiently.

### Cast Operations on Spatial Types

As of MySQL 8.0.24, [`CAST()`](cast-functions.html#function_cast) and
[`CONVERT()`](cast-functions.html#function_convert) support casting
...
standard](spatial-types.html "13.4 Spatial Data Types"):
...
* [General Conditions for Spatial Casts](cast-functions.html#spatial-cast-general-conditions "General Conditions for Spatial Casts")
* [Conditions for Casts to Point](cast-functions.html#spatial-cast-point-conditions "Conditions for Casts to Point")
* [Conditions for Casts to LineString](cast-functions.html#spatial-cast-linestring-conditions "Conditions for Casts to LineString")
* [Conditions for Casts to Polygon](cast-functions.html#spatial-cast-polygon-conditions "Conditions for Casts to Polygon")
* [Conditions for Casts to MultiPoint](cast-functions.html#spatial-cast-multipoint-conditions "Conditions for Casts to MultiPoint")
* [Conditions for Casts to MultiLineString](cast-functions.html#spatial-cast-multilinestring-conditions "Conditions for Casts to MultiLineString")
* [Conditions for Casts to MultiPolygon](cast-functions.html#spatial-cast-multipolygon-conditions "Conditions for Casts to MultiPolygon")
* [Conditions for Casts to GeometryCollection](cast-functions.html#spatial-cast-geometrycollection-conditions "Conditions for Casts to GeometryCollection")
...
* Casting to spatial types using the
  [`JSON_VALUE()`](json-search-functions.html#function_json-value) function with a
...
* If the expression to cast is a well-formed geometry of type
  ...
  than one `Point`, an
  [`ER_INVALID_CAST_TO_GEOMETRY`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_invalid_cast_to_geometry)
  error occurs.
...
* If the expression to cast is a well-formed geometry of type
  ...
  [`ER_INVALID_CAST_POLYGON_RING_DIRECTION`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_invalid_cast_polygon_ring_direction)
  error occurs.
...
* If the expression to cast is a well-formed geometry of type
  ...
  [`ER_WRONG_PARAMETERS_TO_STORED_FCT`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_wrong_parameters_to_stored_fct)
  error occurs.
...
specific type in a
[`CREATE TABLE ...
SELECT`](create-table.html "15.1.20 CREATE TABLE Statement") statement:
...
[`ENUM`](enum.html "13.3.5 The ENUM Type") columns in lexical order.
Normally, sorting of [`ENUM`](enum.html "13.3.5 The ENUM Type") columns
occurs using the internal numeric values. Casting the values to
[`CHAR`](char.html "13.3.2 The CHAR and VARCHAR Types") results in a lexical sort:
...
[`CAST()`](cast-functions.html#function_cast) also changes the result if
you use it as part of a more complex expression such as
[`CONCAT('Date: ',CAST(NOW() AS
DATE))`](string-functions.html#function_concat).

For temporal values, there is little need to use
[`CAST()`](cast-functions.html#function_cast) to extract data in
different formats. Instead, use a function such as
[`EXTRACT()`](date-and-time-functions.html#function_extract),
[`DATE_FORMAT()`](date-and-time-functions.html#function_date-format), or
[`TIME_FORMAT()`](date-and-time-functions.html#function_time-format). See
...
see [Section 14.3, “Type Conversion in Expression Evaluation”](type-conversion.html "14.3 Type Conversion in Expression Evaluation").

MySQL supports arithmetic with both signed and unsigned 64-bit
values. For numeric operators (such as
[`+`](arithmetic-functions.html#operator_plus) or
[`-`](arithmetic-functions.html#operator_minus)) where one of
the operands is an unsigned integer, the result is unsigned by
default (see [Section 14.6.1, “Arithmetic Operators”](arithmetic-functions.html "14.6.1 Arithmetic Operators")). To
...
(In this context, [`DECIMAL`](fixed-point-types.html "13.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") column
...
[Section 7.1.11, “Server SQL Modes”](sql-mode.html "7.1.11 Server SQL Modes")). Examples:

* For conversion of a “zero” date string to a
  date, [`CONVERT()`](cast-functions.html#function_convert) and
  [`CAST()`](cast-functions.html#function_cast) return
