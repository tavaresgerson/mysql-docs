...
example, [`GROUPING(year)`](miscellaneous-functions.html#function_grouping) returns 1
when `NULL` in the `year`
column occurs in a super-aggregate row, and 0 otherwise.
Similarly, [`GROUPING(country)`](miscellaneous-functions.html#function_grouping) and
...
[`GROUPING()`](miscellaneous-functions.html#function_grouping) to achieve a
...
an argument to [`MATCH()`](fulltext-search.html#function_match) (and is
rejected with an error) except when called in a
`WHERE` clause. See
[Section 14.9, “Full-Text Search Functions”](fulltext-search.html "14.9 Full-Text Search Functions"), for more information.