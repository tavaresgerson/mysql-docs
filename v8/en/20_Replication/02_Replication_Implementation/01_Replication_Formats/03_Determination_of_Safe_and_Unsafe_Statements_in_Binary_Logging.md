...
[Nondeterministic functions not considered unsafe](replication-rbr-safe-unsafe.html#replication-rbr-safe-unsafe-not "Nondeterministic functions not considered unsafe"), later in this
...
[Section 19.5.1.12, “Replication and Floating-Point Values”](replication-features-floatvalues.html "19.5.1.12 Replication and Floating-Point Values")).
...
most recent
[`ER_BINLOG_UNSAFE_STATEMENT`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_binlog_unsafe_statement)
...
For more information, see [Section 19.2.1, “Replication Formats”](replication-formats.html "19.2.1 Replication Formats").
...
* **Statements containing system functions that may return a different value
  ...
  [`UUID()`](miscellaneous-functions.html#function_uuid), and
  ...
  **Nondeterministic functions not considered unsafe.**
  ...
  [`UTC_TIME()`](date-and-time-functions.html#function_utc-time), and
...
* **References to system variables.**
  ...
  [Section 19.5.1.39, “Replication and Variables”](replication-features-variables.html "19.5.1.39 Replication and Variables"). For
  exceptions, see [Section 7.4.4.3, “Mixed Binary Logging Format”](binary-log-mixed.html "7.4.4.3 Mixed Binary Logging Format").
...
* **Trigger or stored program updates a table having an AUTO\_INCREMENT
  ...
  In addition, an [`INSERT`](insert.html "15.2.7 INSERT Statement") into a
...
* **INSERT ... ON DUPLICATE KEY UPDATE statements on tables with multiple
  ...
  An
  [`INSERT
  ... ON DUPLICATE KEY UPDATE`](insert-on-duplicate.html "15.2.7.2 INSERT ... ON DUPLICATE KEY UPDATE Statement") statement against a
...
* **LOAD DATA statements.**
  [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") is treated as
  unsafe and when
  ...
  [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") does not generate
...
* **XA transactions.**
  ...
  [`binlog_format=MIXED`](replication-options-binary-log.html#sysvar_binlog_format) or
...
* **`DEFAULT` clause that refers to a nondeterministic
  ...
  [`ALTER TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement"). Unlike most
  ...
  log. When [`binlog_format`](replication-options-binary-log.html#sysvar_binlog_format)
  is set to `MIXED` or
