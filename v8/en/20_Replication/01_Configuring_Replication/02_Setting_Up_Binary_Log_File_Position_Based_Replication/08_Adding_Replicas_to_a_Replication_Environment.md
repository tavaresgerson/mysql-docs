...
1. Stop the existing replica and record the replica status
   ...
   STATUS`](show-replica-status.html "15.7.7.35 SHOW REPLICA STATUS Statement") as follows:
...
3. Copy the data directory from the existing replica to the new
   ...
   * After copying, delete the
   ...
   1. If you have not already done so, issue
      [`STOP REPLICA`](stop-replica.html "15.4.2.8 STOP REPLICA Statement") on the new
      replica.

      If you have already started the existing replica again,
      issue [`STOP REPLICA`](stop-replica.html "15.4.2.8 STOP REPLICA Statement") on the
...
6. Start the new replica server, ensuring that replication does
   ...
   issue [`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.35 SHOW REPLICA STATUS Statement") to
