...
such a replica is therefore to use [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") to
create an appropriate dump file on each source, then use the
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client to import the dump file on the
replica.

If you are using GTID-based replication, you need to pay attention
to the `SET @@GLOBAL.gtid_purged` statement that
[**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") places in the dump output. This
...
[**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") documentation.

The behavior of the `SET @@GLOBAL.gtid_purged`
statement written by [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") is different in
releases from MySQL 8.0 compared to MySQL 5.6 and 5.7. In MySQL
5.6 and 5.7, the statement replaces the value of
[`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged) on the replica, and
...
[`RESET MASTER`](reset-master.html "15.4.1.2 RESET MASTER Statement") on the replica, but if
...
in [Section 19.1.3.5, “Using GTIDs for Failover and Scaleout”](replication-gtids-failover.html "19.1.3.5 Using GTIDs for Failover and Scaleout").

From MySQL 8.0, the `SET @@GLOBAL.gtid_purged`
statement adds the GTID set from the dump file to the existing
[`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged) set on the replica.
...
1. To create dump files for a database named
   ...
   `source2`, run [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program")
   ...
   Then run [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") for
   `source2` as follows:
...
2. Record the [`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged) value
   that [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") added to each of the dump
...
4. Use the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client to import each edited
...
5. On the replica, issue [`RESET
   MASTER`](reset-master.html "15.4.1.2 RESET MASTER Statement") to clear the GTID execution history
   ...
   [Section 19.1.3.8, “Stored Function Examples to Manipulate GTIDs”](replication-gtids-functions.html "19.1.3.8 Stored Function Examples to Manipulate GTIDs") to check this
