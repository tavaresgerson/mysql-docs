...
[`replica_parallel_workers > 0`](replication-options-replica.html#sysvar_replica_parallel_workers) or
...
REPLICA`](stop-replica.html "15.4.2.8 STOP REPLICA Statement") statement, ongoing transactions are applied so
...
* A multi-table [`DROP TABLE`](drop-table.html "15.1.32 DROP TABLE Statement")
...
* A
  [`CREATE
  TABLE ... SELECT`](create-table-select.html "15.1.20.4 CREATE TABLE ... SELECT Statement") statement is issued when
  ...
  TABLE`](create-table.html "15.1.20 CREATE TABLE Statement") action and one GTID is generated for the
  row-insert actions.
...
[`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged)) was empty. From
...
[`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged). For details of how
...
the [`RESET MASTER`](reset-master.html "15.4.1.2 RESET MASTER Statement") statement. For
...
Use [`RESET MASTER`](reset-master.html "15.4.1.2 RESET MASTER Statement") with caution
to avoid losing any wanted GTID execution history and binary
log files.

Before issuing [`RESET MASTER`](reset-master.html "15.4.1.2 RESET MASTER Statement"),
...
When you issue [`RESET MASTER`](reset-master.html "15.4.1.2 RESET MASTER Statement"), the
...
Note that [`RESET MASTER`](reset-master.html "15.4.1.2 RESET MASTER Statement") is the
method to reset the GTID execution history even if the server is
a replica where binary logging is disabled.
[`RESET REPLICA`](reset-replica.html "15.4.2.4 RESET REPLICA Statement") has no effect on
