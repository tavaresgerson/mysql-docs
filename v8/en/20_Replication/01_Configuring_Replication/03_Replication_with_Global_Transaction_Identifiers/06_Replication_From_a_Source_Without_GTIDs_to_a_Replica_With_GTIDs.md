...
of the [`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.3 CHANGE REPLICATION SOURCE TO Statement")
...
* GTIDs are assigned to the replicated transactions when they
  ...
  [`slave-preserve-commit-order=1`](replication-options-replica.html#sysvar_slave_preserve_commit_order)
  is set.
...
* The `UNTIL SQL_BEFORE_GTIDS` and
  ...
  [`START REPLICA`](start-replica.html "15.4.2.6 START REPLICA Statement") statement cannot
...
[`RESET REPLICA
ALL`](reset-replica.html "15.4.2.4 RESET REPLICA Statement") statement resets the
`ASSIGN_GTIDS_TO_ANONYMOUS_TRANSACTIONS` setting,
but a [`RESET REPLICA`](reset-replica.html "15.4.2.4 RESET REPLICA Statement") statement does
not.