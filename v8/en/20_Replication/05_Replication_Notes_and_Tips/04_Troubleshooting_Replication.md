...
* Verify that the replica is running. Use
  [`SHOW REPLICA STATUS`](show-replica-status.html "15.7.7.35 SHOW REPLICA STATUS Statement") to check
  ...
  [`--skip-slave-start`](replication-options-replica.html#option_mysqld_skip-slave-start) command line
  ...
  you issue a [`START REPLICA`](start-replica.html "15.4.2.6 START REPLICA Statement")
  statement.

* If the replica is running, check whether it established a
  connection to the source. Use [`SHOW
  PROCESSLIST`](show-processlist.html "15.7.7.29 SHOW PROCESSLIST Statement"), find the I/O (receiver) and SQL
