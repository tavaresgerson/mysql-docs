...
MASTER TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") statement (prior to MySQL 8.0.23).
...
* To name the replica's certificate and SSL private key
  ...
  TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement")), add the appropriate
  `SOURCE_SSL_xxx`
  (`MASTER_SSL_xxx`)
  options, like this:
...
* To specify lists of ciphers, ciphersuites, and encryption
  ...
  + The `SOURCE_TLS_VERSION` option specifies
    ...
    [`tls_version`](server-system-variables.html#sysvar_tls_version) server system
...
* After the source information has been updated, start the
  ...
  STATUS`](show-replica-status.html "15.7.7.35 SHOW REPLICA STATUS Statement") (prior to MySQL 8.0.22,
  [`SHOW SLAVE STATUS`](show-slave-status.html "15.7.7.36 SHOW SLAVE | REPLICA STATUS Statement")) statement to
