...
REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.3 CHANGE REPLICATION SOURCE TO Statement") statement (from MySQL 8.0.23) or
[`CHANGE MASTER TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") statement (before
...
[**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") output, for example to recover from
...
* The `REQUIRE_ROW_FORMAT` option (available
  ...
  ([`binlog_format=ROW`](replication-options-binary-log.html#sysvar_binlog_format)) on the
...
* The `REQUIRE_TABLE_PRIMARY_KEY_CHECK` option
  ...
  [`sql_require_primary_key`](server-system-variables.html#sysvar_sql_require_primary_key). It
  also normalizes the behavior across replication channels for
  different sources.

You grant the [`REPLICATION_APPLIER`](privileges-provided.html#priv_replication-applier)
...
syntax described in [Section 8.2.4, “Specifying Account Names”](account-names.html "8.2.4 Specifying Account Names"), and the user
must not be an anonymous user (with a blank user name) or the
`CURRENT_USER`. To create a new account, use
[`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement"). To grant this account
the [`REPLICATION_APPLIER`](privileges-provided.html#priv_replication-applier) privilege,
use the [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statement. For
...
details, see
[Section 8.4.1.2, “Caching SHA-2 Pluggable Authentication”](caching-sha2-pluggable-authentication.html "8.4.1.2 Caching SHA-2 Pluggable Authentication")). To
...
described in
[Section 19.3.1, “Setting Up Replication to Use Encrypted Connections”](replication-encrypted-connections.html "19.3.1 Setting Up Replication to Use Encrypted Connections"), or enable
the unencrypted connection to support password exchange using an
RSA key pair.

After setting up the user account, use the
[`GRANT`](grant.html "15.7.1.6 GRANT Statement") statement to grant additional
...
[Section 19.3.3.1, “Privileges For The Replication PRIVILEGE\_CHECKS\_USER Account”](replication-privilege-checks-account.html "19.3.3.1 Privileges For The Replication PRIVILEGE_CHECKS_USER Account") explains
what additional privileges the account needs. For example, to
grant the `priv_repl` user account the
[`INSERT`](privileges-provided.html#priv_insert) privilege to add rows to the
...
[`STOP REPLICA`](stop-replica.html "15.4.2.8 STOP REPLICA Statement") (or before MySQL
8.0.22, [`STOP SLAVE`](stop-slave.html "15.4.2.9 STOP SLAVE Statement")) before the
[`CHANGE MASTER TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") statement, and
[`START REPLICA`](start-replica.html "15.4.2.6 START REPLICA Statement") after it. The use of
...
[`FLUSH PRIVILEGES`](flush.html#flush-privileges) statement or by
executing a [**mysqladmin flush-privileges**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") or
[**mysqladmin reload**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") command.
...
[`SKIP_QUERY_REWRITE`](privileges-provided.html#priv_skip-query-rewrite) privilege. This
prevents statements issued by this user from being rewritten. See
[Section 7.6.4, “The Rewriter Query Rewrite Plugin”](rewriter-query-rewrite-plugin.html "7.6.4 The Rewriter Query Rewrite Plugin"), for more
information.
...
[`pseudo_thread_id`](server-system-variables.html#sysvar_pseudo_thread_id) session system
...
use [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files"), to enforce row-based
replication events in [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") output.
...
[`activate_all_roles_on_login`](server-system-variables.html#sysvar_activate_all_roles_on_login) is
set to `ON`.
...
the [`INSERT`](privileges-provided.html#priv_insert) privilege for the
...
[`REPLICATION_APPLIER`](privileges-provided.html#priv_replication-applier) privilege. The
...
**Limitation.**
In MySQL 8.0.18 only, if the replica [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
is restarted immediately after issuing a
[`RESET REPLICA`](reset-replica.html "15.4.2.4 RESET REPLICA Statement") statement (due to
