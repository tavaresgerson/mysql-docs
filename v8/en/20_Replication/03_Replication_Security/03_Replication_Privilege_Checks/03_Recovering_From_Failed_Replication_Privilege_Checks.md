...
1. Identify the replicated event that caused the error and
   ...
   trusted source. You can use [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files")
...
3. If the `PRIVILEGE_CHECKS_USER` account
   ...
   account, use a [`FLUSH
   PRIVILEGES`](flush.html#flush-privileges) statement or execute a
   [**mysqladmin flush-privileges**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") or
   [**mysqladmin reload**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") command to reload the
...
5. If the transaction is an administrative action that should
   ...
   [`START REPLICA`](start-replica.html "15.4.2.6 START REPLICA Statement") to restart
