...
[`CHANGE MASTER TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") statement as the
...
explained in [Section 19.3.3, “Replication Privilege Checks”](replication-privilege-checks.html "19.3.3 Replication Privilege Checks"), the
...
([`binlog_format=ROW`](replication-options-binary-log.html#sysvar_binlog_format)) is strongly
...
* Setting the value of the system variables
  ...
  and [`pseudo_replica_mode`](server-system-variables.html#sysvar_pseudo_replica_mode) or
  [`pseudo_slave_mode`](server-system-variables.html#sysvar_pseudo_slave_mode), to apply
  appropriate metadata and behaviors when executing
  transactions.

* Executing internal-use [`BINLOG`](binlog.html "15.7.8.1 BINLOG Statement")
  statements to apply [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") output,
...
[`sql_require_primary_key`](server-system-variables.html#sysvar_sql_require_primary_key) system
variable for the duration of a session to match the setting
replicated from the source. The
[`SESSION_VARIABLES_ADMIN`](privileges-provided.html#priv_session-variables-admin) privilege
gives the account this capability. This privilege also allows
the account to apply [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") output that
was created using the
[`--disable-log-bin`](mysqlbinlog.html#option_mysqlbinlog_disable-log-bin) option. If
...
needs the [`TABLE_ENCRYPTION_ADMIN`](privileges-provided.html#priv_table-encryption-admin)
...
relay log, or transactions from [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files")
...
* For a row insertion logged in row format (which are logged
  ...
  [`INSERT`](privileges-provided.html#priv_insert) privilege on the
  relevant table.

* For a row update logged in row format (which are logged as
  an `Update_rows_log_event`), the
  [`UPDATE`](privileges-provided.html#priv_update) privilege on the
  relevant table.

* For a row deletion logged in row format (which are logged as
  a `Delete_rows_log_event`), the
  [`DELETE`](privileges-provided.html#priv_delete) privilege on the
  relevant table.
...
If [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") operations need to
be carried out on the replication channel, use row-based binary
logging ([`binlog_format=ROW`](replication-options-binary-log.html#sysvar_binlog_format)).
With this logging format, the
[`FILE`](privileges-provided.html#priv_file) privilege is not needed to
...
DATA`](load-data.html "15.2.9 LOAD DATA Statement") events, is processed without privilege checks.
For more information, see
[Section 19.5.1.19, “Replication and LOAD DATA”](replication-features-load-data.html "19.5.1.19 Replication and LOAD DATA").

If the [`init_replica`](replication-options-replica.html#sysvar_init_replica) or
[`init_slave`](replication-options-replica.html#sysvar_init_slave) system variable is
...
[`DROP ROLE`](privileges-provided.html#priv_drop-role), and
[`GRANT OPTION`](privileges-provided.html#priv_grant-option), and do not permit
