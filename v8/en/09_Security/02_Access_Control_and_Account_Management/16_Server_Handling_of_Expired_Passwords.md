...
The [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") statement enables
...
[`ER_MUST_CHANGE_PASSWORD_LOGIN`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_must_change_password_login)
error:
...
* The client can reset the account password with
  ...
  [`SET PASSWORD`](set-password.html "15.7.1.10 SET PASSWORD Statement"). After that has
...
returns an [`ER_MUST_CHANGE_PASSWORD`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_must_change_password)
error:
...
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client because by default such
invocations are put in sandbox mode. To resume normal functioning,
select a new password.

For noninteractive invocations of the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client")
client (for example, in batch mode), the server normally
disconnects the client if the password is expired. To permit
noninteractive [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") invocations to stay
...
* Pass the
  ...
  to [`mysql_options()`](/doc/c-api/8.0/en/mysql-options.html) prior to
  connecting:
  ...
  This is the technique used within the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client")
...
* Pass the
  ...
  [`mysql_real_connect()`](/doc/c-api/8.0/en/mysql-real-connect.html) at
  connect time:
