...
[`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) system variable
...
* [Using Partial Revokes](partial-revokes.html#partial-revokes-usage "Using Partial Revokes")
* [Partial Revokes Versus Explicit Schema Grants](partial-revokes.html#partial-revokes-versus-schema-grants "Partial Revokes Versus Explicit Schema Grants")
* [Disabling Partial Revokes](partial-revokes.html#partial-revokes-disabling "Disabling Partial Revokes")
* [Partial Revokes and Replication](partial-revokes.html#partial-revokes-replication "Partial Revokes and Replication")

Note

For brevity, [`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement")
statements shown here do not include passwords. For production
use, always assign account passwords.

#### Using Partial Revokes

The [`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) system
variable controls whether privilege restrictions can be placed
on accounts. By default,
[`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) is disabled and
...
To permit the [`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") operation,
...
With [`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) enabled,
...
[`SHOW GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement") lists partial revokes
as [`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") statements in its
output. The result indicates that `u1` has
global [`SELECT`](privileges-provided.html#priv_select) and
[`INSERT`](privileges-provided.html#priv_insert) privileges, except that
[`INSERT`](privileges-provided.html#priv_insert) cannot be exercised for
...
See [Protecting System Accounts Against Manipulation by Regular Accounts](account-categories.html#protecting-system-accounts "Protecting System Accounts Against Manipulation by Regular Accounts").
...
* Partial revokes apply at the schema level only. You cannot
  ...
  (such as [`FILE`](privileges-provided.html#priv_file) or
  [`BINLOG_ADMIN`](privileges-provided.html#priv_binlog-admin)), or for table,
...
privileges appear in [`SHOW GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement")
output as [`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") statements. This
differs from how [`SHOW GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement")
represents “plain” schema-level privileges:

* When granted, schema-level privileges are represented by
  their own [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statements in
  the output:
...
If `admin` grants
[`SELECT`](privileges-provided.html#priv_select) globally to
...
* If `admin` grants
  [`SELECT`](privileges-provided.html#priv_select) globally to
  `u1`, who has no
  [`SELECT`](privileges-provided.html#priv_select) privilege to begin
...
* On the other hand, `u2` already holds a
  ...
  without restriction. [`GRANT`](grant.html "15.7.1.6 GRANT Statement")
...
If a [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statement includes an
`AS user` clause,
...
`AS` clause, see [Section 15.7.1.6, “GRANT Statement”](grant.html "15.7.1.6 GRANT Statement").
...
For example, to remove the [`INSERT`](privileges-provided.html#priv_insert)
restriction:
...
example, to remove the [`UPDATE`](privileges-provided.html#priv_update)
restriction:
...
[`DELETE`](privileges-provided.html#priv_delete) restriction (at the cost
of removing all [`DELETE`](privileges-provided.html#priv_delete) access):
...
privileges, where [`INSERT`](privileges-provided.html#priv_insert) is held
...
Revoking [`INSERT`](privileges-provided.html#priv_insert) on
`world` revokes the schema-level privilege
([`SHOW GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement") no longer displays
the schema-level [`GRANT`](grant.html "15.7.1.6 GRANT Statement")
statement):
...
global privilege ([`SHOW GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement") now
includes a schema-level [`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement")
statement):
...
* For attempts to disable
  [`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) at startup,
  the server logs an error message and enables
  [`partial_revokes`](server-system-variables.html#sysvar_partial_revokes).

* For attempts to disable
  [`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) at runtime,
  an error occurs and the
  [`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) value
  remains unchanged.

To disable [`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) when
...
[`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) is enabled on
any host, it must be enabled on all hosts. Otherwise,
[`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") statements to partially
...
When [`partial_revokes`](server-system-variables.html#sysvar_partial_revokes) is
enabled, an extended syntax is recorded in the binary log for
[`GRANT`](grant.html "15.7.1.6 GRANT Statement") statements, including the
...
the [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statement
with an error. Ensure that all user accounts that issue or might
issue [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statements on the
