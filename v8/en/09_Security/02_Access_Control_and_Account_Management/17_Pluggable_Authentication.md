...
* Plugins that perform authentication using SHA-256 password
  ...
  [Section 8.4.1.2, “Caching SHA-2 Pluggable Authentication”](caching-sha2-pluggable-authentication.html "8.4.1.2 Caching SHA-2 Pluggable Authentication"), and
...
plugins, see [Writing Authentication Plugins](/doc/extending-mysql/8.0/en/writing-authentication-plugins.html).
...
* Factor 1: If
  ...
  [`authentication_policy`](server-system-variables.html#sysvar_authentication_policy) or
  [`default_authentication_plugin`](server-system-variables.html#sysvar_default_authentication_plugin)
  setting:
  ...
  You can use [`SHOW CREATE USER`](show-create-user.html "15.7.7.12 SHOW CREATE USER Statement")
...
* In MySQL 5.7, `libmysqlclient` uses as its
  ...
  option for [`mysql_options()`](/doc/c-api/8.0/en/mysql-options.html).
...
* If a connector should be able to load client-side plugins
  ...
  [C API Client Plugin Interface](/doc/c-api/8.0/en/c-api-plugin-interface.html).
