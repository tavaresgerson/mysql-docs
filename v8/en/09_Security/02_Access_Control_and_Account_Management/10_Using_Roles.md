...
* [`CREATE ROLE`](create-role.html "15.7.1.2 CREATE ROLE Statement") and
  [`DROP ROLE`](drop-role.html "15.7.1.4 DROP ROLE Statement") create and remove
  roles.

* [`GRANT`](grant.html "15.7.1.6 GRANT Statement") and
  [`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") assign privileges to
  revoke privileges from user accounts and roles.

* [`SHOW GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement") displays privilege
  and role assignments for user accounts and roles.

* [`SET DEFAULT ROLE`](set-default-role.html "15.7.1.9 SET DEFAULT ROLE Statement") specifies
  which account roles are active by default.

* [`SET ROLE`](set-role.html "15.7.1.11 SET ROLE Statement") changes the active
...
* The [`mandatory_roles`](server-system-variables.html#sysvar_mandatory_roles) and
...
[Section 15.7.1, “Account Management Statements”](account-management-statements.html "15.7.1 Account Management Statements"). The following
...
* [Creating Roles and Granting Privileges to Them](roles.html#roles-creating-granting "Creating Roles and Granting Privileges to Them")
* [Defining Mandatory Roles](roles.html#mandatory-roles "Defining Mandatory Roles")
* [Checking Role Privileges](roles.html#roles-checking "Checking Role Privileges")
* [Activating Roles](roles.html#roles-activating "Activating Roles")
* [Revoking Roles or Role Privileges](roles.html#roles-revoking "Revoking Roles or Role Privileges")
* [Dropping Roles](roles.html#roles-dropping "Dropping Roles")
...
ROLE`](create-role.html "15.7.1.2 CREATE ROLE Statement") statement:
...
additional information, see [Section 8.2.5, “Specifying Role Names”](role-names.html "8.2.5 Specifying Role Names").

To assign privileges to the roles, execute
[`GRANT`](grant.html "15.7.1.6 GRANT Statement") statements using the same
...
[`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") to create the
accounts:
...
use [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statements of the same
form as just shown, but that requires enumerating individual
privileges for each user. Instead, use an alternative
[`GRANT`](grant.html "15.7.1.6 GRANT Statement") syntax that permits
...
The [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statement for the
`rw_user1` account grants the read and write
roles, which combine to provide the required read and write
privileges.

The [`GRANT`](grant.html "15.7.1.6 GRANT Statement") syntax for granting
...
you must use separate [`GRANT`](grant.html "15.7.1.6 GRANT Statement")
...
[`mandatory_roles`](server-system-variables.html#sysvar_mandatory_roles) in your server
`my.cnf` file:
...
To set and persist
[`mandatory_roles`](server-system-variables.html#sysvar_mandatory_roles) at runtime, use
...
`PERSIST`. See [Section 15.7.6.1, “SET Syntax for Variable Assignment”](set-variable.html "15.7.6.1 SET Syntax for Variable Assignment").

Setting [`mandatory_roles`](server-system-variables.html#sysvar_mandatory_roles)
requires the [`ROLE_ADMIN`](privileges-provided.html#priv_role-admin)
...
effect until activated (see [Activating Roles](roles.html#roles-activating "Activating Roles")).
At login time, role activation occurs for all granted roles if
the [`activate_all_roles_on_login`](server-system-variables.html#sysvar_activate_all_roles_on_login)
system variable is enabled, or for roles that are set as default
roles otherwise. At runtime, [`SET
ROLE`](set-role.html "15.7.1.11 SET ROLE Statement") activates roles.
...
a role that has the [`SYSTEM_USER`](privileges-provided.html#priv_system-user)
...
[`SYSTEM_USER`](privileges-provided.html#priv_system-user) privilege through a
role in order to guard against the possibility of privilege
escalation.

If a role named in
[`mandatory_roles`](server-system-variables.html#sysvar_mandatory_roles) is not present
...
PRIVILEGES`](flush.html#flush-privileges) may be necessary to cause the server to
...
[`SHOW GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement"). For example:
...
[`CURRENT_ROLE()`](information-functions.html#function_current-role) function.
...
[`CURRENT_ROLE()`](information-functions.html#function_current-role) function, the
...
[`SET DEFAULT ROLE`](set-default-role.html "15.7.1.9 SET DEFAULT ROLE Statement"). To set the
...
value of [`CURRENT_ROLE()`](information-functions.html#function_current-role) reflects
...
ROLE`](set-role.html "15.7.1.11 SET ROLE Statement") to change the set of active roles. For example,
for `rw_user1`:
...
The first [`SET ROLE`](set-role.html "15.7.1.11 SET ROLE Statement") statement
...
* Stored program and view objects that execute in definer
  ...
  execute [`SET
  ROLE`](set-role.html "15.7.1.11 SET ROLE Statement") to activate the required roles. This must be
...
[`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") can also be applied to a
...
only. To do this, use [`REVOKE`](revoke.html "15.7.1.8 REVOKE Statement") to
...
as can be seen using [`SHOW GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement")
...
privileges ([`INSERT`](privileges-provided.html#priv_insert),
[`UPDATE`](privileges-provided.html#priv_update), and
[`DELETE`](privileges-provided.html#priv_delete) are no longer present):
...
To drop roles, use [`DROP ROLE`](drop-role.html "15.7.1.4 DROP ROLE Statement"):
...
GRANTS`](show-grants.html "15.7.7.21 SHOW GRANTS Statement"), which displays grants for user accounts or
roles, accounts and roles can be used interchangeably.

One difference between roles and users is that
[`CREATE ROLE`](create-role.html "15.7.1.2 CREATE ROLE Statement") creates an
authorization identifier that is locked by default, whereas
[`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") creates an
...
* The [`CREATE ROLE`](privileges-provided.html#priv_create-role) and
  [`DROP ROLE`](privileges-provided.html#priv_drop-role) privileges enable
  only use of the [`CREATE ROLE`](create-role.html "15.7.1.2 CREATE ROLE Statement")
  and [`DROP ROLE`](drop-role.html "15.7.1.4 DROP ROLE Statement") statements,
  respectively.

* The [`CREATE USER`](privileges-provided.html#priv_create-user) privilege
...
Thus, the [`CREATE ROLE`](privileges-provided.html#priv_create-role) and
[`DROP ROLE`](privileges-provided.html#priv_drop-role) privileges are not as
powerful as [`CREATE USER`](privileges-provided.html#priv_create-user) and may
