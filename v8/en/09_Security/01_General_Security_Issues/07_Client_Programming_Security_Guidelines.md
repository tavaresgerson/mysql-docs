...
* MySQL C API: Use the
  ...
  placeholders; see
  [C API Prepared Statement Interface](/doc/c-api/8.0/en/c-api-prepared-statement-interface.html).
...
* PHP: Use either the `mysqli` or
  ...
  See also [MySQL and PHP](/doc/apis-php/en/).

  If the older `ext/mysql` extension must be
  used, then for escaping use the
  [`mysql_real_escape_string_quote()`](/doc/c-api/8.0/en/mysql-real-escape-string-quote.html)
  function and not
  [`mysql_escape_string()`](/doc/c-api/8.0/en/mysql-escape-string.html) or
...
debug a common 10-way join [`SELECT`](select.html "15.2.13 SELECT Statement")
