...
[Section 8.3, “Using Encrypted Connections”](encrypted-connections.html "8.3 Using Encrypted Connections"). Alternatively, use SSH to
...
* Require all MySQL accounts to have a password. A client
  ...
  example, anyone can use the [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") program
...
* Make sure that the only Unix user account with read or write
  ...
  used for running [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server").

* Never run the MySQL server as the Unix `root`
  user. This is extremely dangerous, because any user with the
  [`FILE`](privileges-provided.html#priv_file) privilege is able to cause
  the server to create files as `root` (for
  example, `~root/.bashrc`). To prevent this,
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") refuses to run as
  `root` unless that is specified explicitly
  using the [`--user=root`](server-options.html#option_mysqld_user) option.

  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") can (and should) be run as an
  ...
  MySQL. To start [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") as a different Unix
  ...
  [**mysqld\_safe**](mysqld-safe.html "6.3.2 mysqld_safe — MySQL Server Startup Script") or
  [**mysql.server**](mysql-server.html "6.3.3 mysql.server — MySQL Server Startup Script"). For more details, see
  [Section 8.1.5, “How to Run MySQL as a Normal User”](changing-mysql-user.html "8.1.5 How to Run MySQL as a Normal User").

  Running [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") as a Unix user other than
...
* Do not grant the [`FILE`](privileges-provided.html#priv_file) privilege
  to nonadministrative users. Any user that has this privilege
  can write a file anywhere in the file system with the
  privileges of the [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") daemon. This
  includes the server's data directory containing the files that
  implement the privilege tables. To make
  [`FILE`](privileges-provided.html#priv_file)-privilege operations a bit
  safer, files generated with
  [`SELECT ... INTO
  OUTFILE`](select-into.html "15.2.13.1 SELECT ... INTO Statement") do not overwrite existing files and are
  writable by everyone.

  The [`FILE`](privileges-provided.html#priv_file) privilege may also be
  ...
  DATA`](load-data.html "15.2.9 LOAD DATA Statement") to load `/etc/passwd` into a
  ...
  set the [`secure_file_priv`](server-system-variables.html#sysvar_secure_file_priv)
...
* Do not grant the [`PROCESS`](privileges-provided.html#priv_process) or
  [`SUPER`](privileges-provided.html#priv_super) privilege to
  nonadministrative users. The output of [**mysqladmin
  processlist**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") and [`SHOW
  PROCESSLIST`](show-processlist.html "15.7.7.29 SHOW PROCESSLIST Statement") shows the text of any statements
  ...
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") reserves an extra connection for
  users who have the
  [`CONNECTION_ADMIN`](privileges-provided.html#priv_connection-admin) or
  [`SUPER`](privileges-provided.html#priv_super) privilege, so that a
  MySQL `root` user can log in and check server
  activity even if all normal connections are in use.

  The [`SUPER`](privileges-provided.html#priv_super) privilege can be used
...
* Do not permit the use of symlinks to tables. (This capability
  ...
  [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") as `root`, because
...
* If you want to restrict the number of connections permitted to
  ...
  [`max_user_connections`](server-system-variables.html#sysvar_max_user_connections) variable
  in [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server"). The [`CREATE
  ...
  [Section 15.7.1.3, “CREATE USER Statement”](create-user.html "15.7.1.3 CREATE USER Statement"), and
