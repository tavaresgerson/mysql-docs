...
[Section 8.4.4.3, “Keyring Plugin Installation”](keyring-plugin-installation.html "8.4.4.3 Keyring Plugin Installation"), together with the
...
<table summary="keyring_oci configuration parameters and corresponding system variables."><col style="width: 35%"/><col style="width: 50%"/><col style="width: 15%"/><thead><tr>
...
[`--early-plugin-load`](server-options.html#option_mysqld_early-plugin-load) option. As
...
* Oracle Cloud Infrastructure uses Oracle Cloud IDs (OCIDs) extensively to designate
  ...
  + A user for connecting to Oracle Cloud Infrastructure must exist. Create the
    ...
    [`keyring_oci_user`](keyring-system-variables.html#sysvar_keyring_oci_user) system
    variable.
...
* The Oracle Cloud Infrastructure API uses an RSA private/public key pair for
  ...
  [`keyring_oci_key_file`](keyring-system-variables.html#sysvar_keyring_oci_key_file) and
...
see [Section 8.4.4.19, “Keyring System Variables”](keyring-system-variables.html "8.4.4.19 Keyring System Variables").
...
* Modify parameter settings in the `my.cnf`
  file, or use
  [`SET
  PERSIST_ONLY`](set-variable.html "15.7.6.1 SET Syntax for Variable Assignment") for parameters that are persisted to
  `mysqld-auto.conf`.
