...
* For connections by accounts that authenticate with
  ...
  + For programs that use the C API, call
    [`mysql_options()`](/doc/c-api/8.0/en/mysql-options.html) to
  ...
  + For replicas, use the [`CHANGE
    ...
    TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") statement (before MySQL 8.0.23) with the
