...
with [`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement") or
...
`stefanie` invokes [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client")
...
* Name both users at account-creation time, one following
  [`CREATE USER`](create-user.html "15.7.1.3 CREATE USER Statement"), and the other
...
* If you have already used [`CREATE
  USER`](create-user.html "15.7.1.3 CREATE USER Statement") to create the account for a single user,
  use [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") to add the
  second user:
