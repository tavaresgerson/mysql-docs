...
[`tls_channel_status`](performance-schema-tls-channel-status-table.html "29.12.21.9 The tls_channel_status Table") table exposes
...
example, the [`--ssl`](server-options.html#option_mysqld_ssl) and
[`--admin-ssl`](server-options.html#option_mysqld_admin-ssl) options affect only
...
use [`--ssl=OFF`](server-options.html#option_mysqld_ssl) to start the server
with encrypted connections disabled on the main interface, then
reconfigure TLS and execute [`ALTER INSTANCE
RELOAD TLS`](alter-instance.html#alter-instance-reload-tls) to enable encrypted connections at runtime.
