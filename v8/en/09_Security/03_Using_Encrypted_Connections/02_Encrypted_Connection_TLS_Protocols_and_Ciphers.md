...
* [Supported TLS Protocols](encrypted-connection-protocols-ciphers.html#encrypted-connection-supported-protocols "Supported TLS Protocols")
...
<th>MySQL Server Release</th>
<th>TLS Protocols Supported</th>
</tr></thead><tbody><tr>
<td>MySQL 8.0.15 and below</td>
<td>TLSv1, TLSv1.1, TLSv1.2</td>
</tr><tr>
<td>MySQL 8.0.16 and MySQL 8.0.17</td>
<td>TLSv1, TLSv1.1, TLSv1.2, TLSv1.3 (except Group Replication)</td>
</tr><tr>
<td>MySQL 8.0.18 through MySQL 8.0.25</td>
<td>TLSv1, TLSv1.1, TLSv1.2, TLSv1.3 (including Group Replication)</td>
</tr><tr>
<td>MySQL 8.0.26 and MySQL 8.0.27</td>
<td>TLSv1 (deprecated), TLSv1.1 (deprecated), TLSv1.2, TLSv1.3</td>
</tr><tr>
<td>MySQL 8.0.28 and above</td>
...
:   If the SSL library does not support a particular protocol,
    ...
    [`admin_tls_version`](server-system-variables.html#sysvar_admin_tls_version), and
...
* The default values of the server’s
  [`tls_version`](server-system-variables.html#sysvar_tls_version) and
  [`admin_tls_version`](server-system-variables.html#sysvar_admin_tls_version) system
...
values of the [`tls_version`](server-system-variables.html#sysvar_tls_version) or
[`admin_tls_version`](server-system-variables.html#sysvar_admin_tls_version) system
...
TLS 1.0 and 1.1?](faqs-security.html#faq-mysql-tls-versions "A.9.7.")

#### Connection TLS Protocol Configuration

On the server side, the value of the
[`tls_version`](server-system-variables.html#sysvar_tls_version) system variable
determines which TLS protocols a MySQL server permits for
encrypted connections. The
[`tls_version`](server-system-variables.html#sysvar_tls_version) value applies to
...
[`admin_tls_version`](server-system-variables.html#sysvar_admin_tls_version) system
variable (see
[Section 7.1.12.2, “Administrative Connection Management”](administrative-connection-interface.html "7.1.12.2 Administrative Connection Management")). This
discussion applies to
[`admin_tls_version`](server-system-variables.html#sysvar_admin_tls_version) as well.

The [`tls_version`](server-system-variables.html#sysvar_tls_version) value is a list
...
therefore as shown in [Table 8.14, “MySQL Server TLS Protocol Default Settings”](encrypted-connection-protocols-ciphers.html#tls-support-defaults "Table 8.14 MySQL Server TLS Protocol Default Settings").

**Table 8.14 MySQL Server TLS Protocol Default Settings**

<table><col style="width: 25%"/><col style="width: 75%"/><thead><tr>
<th>MySQL Server Release</th>
<th><code class="literal">tls_version</code> Default Setting</th>
</tr></thead><tbody><tr>
<td>MySQL 8.0.15 and below</td>
<td><p>
<code class="literal">TLSv1,TLSv1.1,TLSv1.2</code>
</p></td>
</tr><tr>
<td>MySQL 8.0.16 and MySQL 8.0.17</td>
<td><p>
<code class="literal">TLSv1,TLSv1.1,TLSv1.2,TLSv1.3 (with OpenSSL
                1.1.1)</code>
</p><p>
<code class="literal"> TLSv1,TLSv1.1,TLSv1.2 (otherwise) </code>
</p><p>
...
<code class="literal"> TLSv1,TLSv1.1,TLSv1.2 (otherwise) </code>
                ...
                Group Replication supports TLSv1.3
              </p></td>
</tr><tr>
<td>MySQL 8.0.26 and MySQL 8.0.27</td>
<td><p>
...
<td>MySQL 8.0.28 and above</td>
<td><p>
<code class="literal">TLSv1.2,TLSv1.3</code>
</p></td>
</tr></tbody></table>

To determine the value of
[`tls_version`](server-system-variables.html#sysvar_tls_version) at runtime, use
this statement:
...
[`tls_version`](server-system-variables.html#sysvar_tls_version), set it at server
...
set [`tls_version`](server-system-variables.html#sysvar_tls_version) like this:
...
[`--tls-version`](connection-options.html#option_general_tls-version) option specifies
...
MASTER TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") statement (before MySQL 8.0.23) specifies
...
[`tls_version`](server-system-variables.html#sysvar_tls_version) value. For example,
a server that acts as a replica can be configured with
[`tls_version`](server-system-variables.html#sysvar_tls_version) set to TLSv1.3 to
...
the server [`tls_version`](server-system-variables.html#sysvar_tls_version) value,
...
* [`tls_version`](server-system-variables.html#sysvar_tls_version): The server
  does not permit encrypted incoming connections.

* [`--tls-version`](connection-options.html#option_general_tls-version): The client
...
* Set the [`ssl_cipher`](server-system-variables.html#sysvar_ssl_cipher) system
  variable on the server side, and use the
  [`--ssl-cipher`](connection-options.html#option_general_ssl-cipher) option for
  client programs.
...
* For regular source/replica replication connections, where
  ...
  [`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.3 CHANGE REPLICATION SOURCE TO Statement")
  statement (from MySQL 8.0.23) or [`CHANGE
  MASTER TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") statement (before MySQL 8.0.23). See
  [Section 19.3.1, “Setting Up Replication to Use Encrypted Connections”](replication-encrypted-connections.html "19.3.1 Setting Up Replication to Use Encrypted Connections").
...
[`--tls-ciphersuites`](server-system-variables.html#sysvar_tls_ciphersuites) or
[`--admin-tls-ciphersuites`](server-system-variables.html#sysvar_admin_tls_ciphersuites).
...
* On the client side, use the
  [`--tls-ciphersuites`](connection-options.html#option_general_tls-ciphersuites) option.
...
[`CHANGE REPLICATION SOURCE TO`](change-replication-source-to.html "15.4.2.3 CHANGE REPLICATION SOURCE TO Statement")
statement (from MySQL 8.0.23) or [`CHANGE
MASTER TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") statement (before MySQL 8.0.23) are not
...
[`Ssl_cipher_list`](server-status-variables.html#statvar_Ssl_cipher_list) status
variable:
...
The [`Ssl_cipher_list`](server-status-variables.html#statvar_Ssl_cipher_list) status
...
* As of MySQL 8.0.35, the following ciphers are deprecated and
  ...
  [`--ssl-cipher`](server-system-variables.html#sysvar_ssl_cipher) and
  [`--admin-ssl-cipher`](server-system-variables.html#sysvar_admin_ssl_cipher):
...
[`ssl_cert`](server-system-variables.html#sysvar_ssl_cert) system variable set to
...
* If TLSv1.3 is available, it is used if possible. (This means
  ...
  [`tls_version`](server-system-variables.html#sysvar_tls_version) has a value of
...
* TLSv1.2 does not work with all ciphers that have a key size
  ...
  [`--ssl-cipher`](connection-options.html#option_general_ssl-cipher) client option
...
* If the server is configured with
  [`tls_version=TLSv1.1,TLSv1.2`](server-system-variables.html#sysvar_tls_version):

  + Connection attempts fail for clients invoked with
    [`--tls-version=TLSv1`](connection-options.html#option_general_tls-version), and
...
* If the server is configured with
  [`tls_version=TLSv1`](server-system-variables.html#sysvar_tls_version) or is an
  older server that supports only TLSv1:

  + Connection attempts fail for clients invoked with
    [`--tls-version=TLSv1.1,TLSv1.2`](connection-options.html#option_general_tls-version).
...
[`Ssl_version`](server-status-variables.html#statvar_Ssl_version) and
[`Ssl_cipher`](server-status-variables.html#statvar_Ssl_cipher) status variables:
