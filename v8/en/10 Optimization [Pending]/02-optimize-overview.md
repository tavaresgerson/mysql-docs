--- title: MySQL 8.4 Reference Manual :: 10.1 Optimization Overview url: https://dev.mysql.com/doc/refman/8.4/en/optimize-overview.html order: 2 ---
...
* Are all [memory areas used for caching](buffering-caching.html "10.10 Buffering and Caching") sized correctly? That is, large enough to hold frequently accessed data, but not so large that they overload physical memory and cause paging. The main memory areas to configure are the `InnoDB` buffer pool and the `MyISAM` key cache.
