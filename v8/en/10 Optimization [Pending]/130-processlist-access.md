--- title: MySQL 8.4 Reference Manual :: 10.14.1 Accessing the Process List url: https://dev.mysql.com/doc/refman/8.4/en/processlist-access.html order: 130 ---
...
* The `INFORMATION_SCHEMA` `PROCESSLIST` table: Section 28.3.23, “The INFORMATION\_SCHEMA PROCESSLIST Table”
...
* `Info` indicates the statement the thread is executing, or `NULL` if it is executing no statement. For [`SHOW PROCESSLIST`](show-processlist.html "15.7.7.31 SHOW PROCESSLIST Statement"), this value contains only the first 100 characters of the statement. To see complete statements, use [`SHOW FULL PROCESSLIST`](show-processlist.html "15.7.7.31 SHOW PROCESSLIST Statement") (or query a different process information source).
