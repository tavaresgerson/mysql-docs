...
In some cases, MySQL may change a string column to a type different from that given in a [`CREATE TABLE`](create-table.html "15.1.20 CREATE TABLE Statement") or `ALTER TABLE` statement. See Section 15.1.20.7, “Silent Column Specification Changes”.
...
To determine all possible values for an `ENUM` column, use [`SHOW COLUMNS FROM tbl_name LIKE 'enum_col'`](show-columns.html "15.7.7.5 SHOW COLUMNS Statement") and parse the `ENUM` definition in the `Type` column of the output.
