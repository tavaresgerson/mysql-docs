...
[Section 2.1, “General Installation Guidance”](general-installation-issues.html "2.1 General Installation Guidance"), and
...
* Even if you use `lower_case_table_names=2`
  ...
  [`DATABASE()`](information-functions.html#function_database) or within the
...
* [`REPLACE`](replace.html "15.2.12 REPLACE Statement") (and
  [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") with the
  [`REPLACE`](replace.html "15.2.12 REPLACE Statement") option) does not
  trigger `ON DELETE CASCADE`.

* `DISTINCT` with `ORDER BY`
  does not work inside
  [`GROUP_CONCAT()`](aggregate-functions.html#function_group-concat) if you do not
...
* With statement-based binary logging, the source server
  ...
  + [`CREATE
    TABLE ... SELECT`](create-table-select.html "15.1.20.4 CREATE TABLE ... SELECT Statement") or
    [`INSERT
    ... SELECT`](insert-select.html "15.2.7.1 INSERT ... SELECT Statement") statements that insert zero or
    `NULL` values into an
    `AUTO_INCREMENT` column.

  + [`DELETE`](delete.html "15.2.2 DELETE Statement") if you are
    deleting rows from a table that has foreign keys with
    `ON DELETE CASCADE` properties.

  + [`REPLACE ...
    SELECT`](replace.html "15.2.12 REPLACE Statement"), `INSERT IGNORE ...
  ...
  **If and only if the preceding queries
  ...
  For example, for
  [`INSERT ...
  SELECT`](insert-select.html "15.2.7.1 INSERT ... SELECT Statement") with no `ORDER BY`, the
  [`SELECT`](select.html "15.2.13 SELECT Statement") may return rows in a
  ...
  + MySQL buffer sizes
    ([`key_buffer_size`](server-system-variables.html#sysvar_key_buffer_size), and
...
* Log file names are based on the server host name if you do
  ...
  [`--log-bin=old_host_name-bin`](replication-options-binary-log.html#option_mysqld_log-bin).
  See [Section 7.1.7, “Server Command Options”](server-options.html "7.1.7 Server Command Options"). Alternatively, rename
...
* [**mysqlbinlog**](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files") does not delete temporary
  files left after a [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement")
  statement. See [Section 6.6.9, “mysqlbinlog — Utility for Processing Binary Log Files”](mysqlbinlog.html "6.6.9 mysqlbinlog — Utility for Processing Binary Log Files").
...
* The server uses only the first
  [`max_sort_length`](server-system-variables.html#sysvar_max_sort_length) bytes when
  ...
  [`max_sort_length`](server-system-variables.html#sysvar_max_sort_length) bytes. To
  work around this, increase the variable value. The default
  value of [`max_sort_length`](server-system-variables.html#sysvar_max_sort_length) is
...
* Numeric calculations are done with
  ...
  [`DOUBLE`](floating-point-types.html "13.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") (both are normally 64
  bits long). Which precision you get depends on the function.
  The general rule is that bit functions are performed with
  [`BIGINT`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") precision,
  [`IF()`](flow-control-functions.html#function_if) and
  [`ELT()`](string-functions.html#function_elt) with
  [`BIGINT`](integer-types.html "13.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") or
  [`DOUBLE`](floating-point-types.html "13.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") precision, and the
  rest with [`DOUBLE`](floating-point-types.html "13.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") precision.
...
* You can have up to 255 [`ENUM`](enum.html "13.3.5 The ENUM Type")
  and [`SET`](set.html "13.3.6 The SET Type") columns in one table.

* In [`MIN()`](aggregate-functions.html#function_min),
  [`MAX()`](aggregate-functions.html#function_max), and other aggregate
  functions, MySQL currently compares
  [`ENUM`](enum.html "13.3.5 The ENUM Type") and
  [`SET`](set.html "13.3.6 The SET Type") columns by their string
  value rather than by the string's relative position in the
  set.

* In an [`UPDATE`](update.html "15.2.17 UPDATE Statement") statement,
...
* If you use [`ALTER TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement") to add
  ...
  because [`ALTER TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement") puts
  `UNIQUE` indexes before normal indexes to
  be able to detect duplicate keys as early as possible.

* An [`UPDATE`](update.html "15.2.17 UPDATE Statement") statement involving
  ...
  + With read-only mode enabled (by using `SET
    GLOBAL`[`read_only`](server-system-variables.html#sysvar_read_only)`= 1`).

  + With the transaction level set to
    `READ_ONLY` (that is, using
    [`SET
    GLOBAL TRANSACTION READ ONLY`](set-transaction.html "15.3.7 SET TRANSACTION Statement") or `SET
