...
* Error code: This value is numeric. It is MySQL-specific and
  ...
  + The symbol for server error number
    `1146` is
    [`ER_NO_SUCH_TABLE`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_no_such_table).

  + The symbol for client error number
    `2005` is
    [`CR_UNKNOWN_HOST`](/doc/mysql-errors/8.0/en/client-error-reference.html#error_cr_unknown_host).
...
* When the server writes a message to the error log, it pads
  ...
  [`MY-000022`](/doc/mysql-errors/8.0/en/global-error-reference.html#error_ee_unknown_charset),
...
* When the server sends a message to a client program, it adds
  ...
  [`1036`](/doc/mysql-errors/8.0/en/server-error-reference.html#error_er_open_as_readonly),
