...
[Section B.1, “Error Message Sources and Elements”](error-message-elements.html "B.1 Error Message Sources and Elements"). For lists of
server-side, client-side, and global (shared between server and
clients) errors, see [MySQL 8.0 Error Message Reference](/doc/mysql-errors/8.0/en/).
...
strings; see [Section 12.12, “Setting the Error Message Language”](error-message-language.html "12.12 Setting the Error Message Language").
...
writes the log, see [Section 7.4.2, “The Error Log”](error-log.html "7.4.2 The Error Log").
...
[Error Code Ranges](error-message-elements.html#error-code-ranges "Error Code Ranges").
...
* SQL statement warning and error information is available
  through the [`SHOW WARNINGS`](show-warnings.html "15.7.7.42 SHOW WARNINGS Statement") and
  [`SHOW ERRORS`](show-errors.html "15.7.7.17 SHOW ERRORS Statement") statements. The
  ...
  [`sql_notes`](server-system-variables.html#sysvar_sql_notes) system variable
  is disabled). The
...
* The [`GET DIAGNOSTICS`](get-diagnostics.html "15.6.7.3 GET DIAGNOSTICS Statement") statement
  may be used to inspect the diagnostic information in the
  diagnostics area. See [Section 15.6.7.3, “GET DIAGNOSTICS Statement”](get-diagnostics.html "15.6.7.3 GET DIAGNOSTICS Statement").

* [`SHOW SLAVE STATUS`](show-slave-status.html "15.7.7.36 SHOW SLAVE | REPLICA STATUS Statement") statement
  output includes information about replication errors
  occurring on replica servers.

* [`SHOW ENGINE
  INNODB STATUS`](show-engine.html "15.7.7.15 SHOW ENGINE Statement") statement output includes
  information about the most recent foreign key error if a
  [`CREATE TABLE`](create-table.html "15.1.20 CREATE TABLE Statement") statement for an
  [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine") table fails.
...
these ranges, see [Error Code Ranges](error-message-elements.html#error-code-ranges "Error Code Ranges").
...
* [`mysql_sqlstate()`](/doc/c-api/8.0/en/mysql-sqlstate.html) returns the
  SQLSTATE value.

* [`mysql_error()`](/doc/c-api/8.0/en/mysql-error.html) returns the
  message string.

* [`mysql_stmt_errno()`](/doc/c-api/8.0/en/mysql-stmt-errno.html),
  [`mysql_stmt_sqlstate()`](/doc/c-api/8.0/en/mysql-stmt-sqlstate.html), and
  [`mysql_stmt_error()`](/doc/c-api/8.0/en/mysql-stmt-error.html) are the
  corresponding error functions for prepared statements.

* [`mysql_warning_count()`](/doc/c-api/8.0/en/mysql-warning-count.html)
  returns the number of errors, warnings, and notes for the
  most recent statement.

For descriptions of the client library error functions, see
[MySQL 8.0 C API Developer Guide](/doc/c-api/8.0/en/).
...
[**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") client), a failure to connect to the
...
The [**perror**](perror.html "6.8.2 perror — Display MySQL Error Message Information") program provides information from
...
For MySQL NDB Cluster errors, use [**ndb\_perror**](mysql-cluster-programs-ndb-perror.html "25.5.16 ndb_perror — Obtain NDB Error Message Information").
See [Section 25.5.16, “ndb\_perror — Obtain NDB Error Message Information”](mysql-cluster-programs-ndb-perror.html "25.5.16 ndb_perror — Obtain NDB Error Message Information").
