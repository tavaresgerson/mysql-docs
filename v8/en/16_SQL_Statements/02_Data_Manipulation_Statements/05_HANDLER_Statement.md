...
interface instead of normal [`SELECT`](select.html "15.2.13 SELECT Statement")
statements:
...
* `HANDLER` is faster than
  ...
  + The handler interface does not have to provide a
    ...
    that [`SELECT`](select.html "15.2.13 SELECT Statement") does not
    normally permit.

* `HANDLER` makes it easier to port to MySQL
  applications that use a low-level `ISAM`-like
  interface. (See [Section 17.20, “InnoDB memcached Plugin”](innodb-memcached.html "17.20 InnoDB memcached Plugin") for an
...
* `HANDLER` enables you to traverse a database
  ...
  accomplish with [`SELECT`](select.html "15.2.13 SELECT Statement"). The
