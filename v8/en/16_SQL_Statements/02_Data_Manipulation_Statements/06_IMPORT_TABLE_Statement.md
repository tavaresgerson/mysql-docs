...
The [`IMPORT TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement") statement imports
`MyISAM` tables based on information contained in
`.sdi` (serialized dictionary information)
metadata files. [`IMPORT TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement")
requires the [`FILE`](privileges-provided.html#priv_file) privilege to read
the `.sdi` and table content files, and the
[`CREATE`](privileges-provided.html#priv_create) privilege for the table to
be created.

Tables can be exported from one server using
[**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") to write a file of SQL statements and
imported into another server using [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") to
process the dump file. [`IMPORT TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement")
...
named by the [`secure_file_priv`](server-system-variables.html#sysvar_secure_file_priv)
system variable, or (if
[`secure_file_priv`](server-system-variables.html#sysvar_secure_file_priv) is empty) in a
...
2. At the file system level, copy the `.sdi`
   ...
   [`secure_file_priv`](server-system-variables.html#sysvar_secure_file_priv) directory,
...
3. Import the tables by executing an [`IMPORT
   TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement") statement that names the
   `.sdi` files:
...
[`secure_file_priv`](server-system-variables.html#sysvar_secure_file_priv) system variable
...
TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement") statement could have been written like this
...
[`IMPORT TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement") as the server-side
rules for [`LOAD DATA`](load-data.html "15.2.9 LOAD DATA Statement") (that is, the
non-`LOCAL` rules). See
[Section 15.2.9, “LOAD DATA Statement”](load-data.html "15.2.9 LOAD DATA Statement"), paying particular attention to the
rules used to interpret relative path names.

[`IMPORT TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement") fails if the
...
reported issues, use [`REPAIR TABLE`](repair-table.html "15.7.3.5 REPAIR TABLE Statement").

[`IMPORT TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement") is not written to the
binary log.

#### Restrictions and Limitations

[`IMPORT TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement") applies only to
non-`TEMPORARY` `MyISAM`
tables. It does not apply to tables created with a transactional
storage engine, tables created with
[`CREATE TEMPORARY
TABLE`](create-table.html "15.1.20 CREATE TABLE Statement"), or views.
...
TABLE`](import-table.html "15.2.6 IMPORT TABLE Statement") statement, whereas others are problematic or may
