...
releases, this is always an [`InnoDB`](innodb-storage-engine.html "Chapter 17 The InnoDB Storage Engine")
...
[`CREATE TABLESPACE`](create-tablespace.html "15.1.21 CREATE TABLESPACE Statement") syntax is used to
...
[`ALTER TABLE
tbl_name TABLESPACE [=]
tablespace_name`](alter-table.html "15.1.9 ALTER TABLE Statement") to add tables
...
Disk Data tables (see [Section 25.6.11, “NDB Cluster Disk Data Tables”](mysql-cluster-disk-data.html "25.6.11 NDB Cluster Disk Data Tables")).
...
using the [`ALTER TABLESPACE`](alter-tablespace.html "15.1.10 ALTER TABLESPACE Statement")
statement (see [Section 15.1.10, “ALTER TABLESPACE Statement”](alter-tablespace.html "15.1.10 ALTER TABLESPACE Statement")).
...
GROUP`](create-logfile-group.html "15.1.16 CREATE LOGFILE GROUP Statement") (see [Section 15.1.16, “CREATE LOGFILE GROUP Statement”](create-logfile-group.html "15.1.16 CREATE LOGFILE GROUP Statement")).
...
[`NDB`](mysql-cluster.html "Chapter 25 MySQL NDB Cluster 8.0") reserves 4% of a tablespace for
...
[`INFORMATION_SCHEMA.FILES`](information-schema-files-table.html "28.3.15 The INFORMATION_SCHEMA FILES Table") column
...
When [`CREATE TABLESPACE`](create-tablespace.html "15.1.21 CREATE TABLESPACE Statement") is used with
...
[`FILES`](information-schema-files-table.html "28.3.15 The INFORMATION_SCHEMA FILES Table") table. (See the example later
in this section.)

(See [Section 28.3.15, “The INFORMATION\_SCHEMA FILES Table”](information-schema-files-table.html "28.3.15 The INFORMATION_SCHEMA FILES Table").)
...
* `ADD DATAFILE`: Defines the name of a
  ...
  **InnoDB data files.**
  ...
  [`innodb_directories`](innodb-parameters.html#sysvar_innodb_directories) value or
  ...
  [`innodb_directories`](innodb-parameters.html#sysvar_innodb_directories) is a
  ...
  **NDB data files.**
  ...
  TABLESPACE`](alter-tablespace.html "15.1.10 ALTER TABLESPACE Statement") statement.
  ...
  ([`DataDir`](mysql-cluster-ndbd-definition.html#ndbparam-ndbd-datadir)), where
  *`nodeid`* is the data node's
  [`NodeId`](mysql-cluster-ndbd-definition.html#ndbparam-ndbd-nodeid). To place the
...
* `FILE_BLOCK_SIZE`: This option—which is
  ...
  [`innodb_page_size`](innodb-parameters.html#sysvar_innodb_page_size) value. Also,
  the physical page size of the compressed table
  (`KEY_BLOCK_SIZE`) must be equal to
  `FILE_BLOCK_SIZE/1024`. For example, if
  [`innodb_page_size=16K`](innodb-parameters.html#sysvar_innodb_page_size), and
  `FILE_BLOCK_SIZE=8K`, the
  `KEY_BLOCK_SIZE` of the table must be 8. For
  more information, see [Section 17.6.3.3, “General Tablespaces”](general-tablespaces.html "17.6.3.3 General Tablespaces").

* `USE LOGFILE GROUP`: Required for
  `NDB`, this is the name of a log file group
  previously created using [`CREATE LOGFILE
  GROUP`](create-logfile-group.html "15.1.16 CREATE LOGFILE GROUP Statement"). Not supported for `InnoDB`,
...
* `EXTENT_SIZE`: This option is specific to
  ...
  by querying the Information Schema
  [`FILES`](information-schema-files-table.html "28.3.15 The INFORMATION_SCHEMA FILES Table") table, and so derive an
...
* `INITIAL_SIZE`: This option is specific to
  ...
  [`ALTER
  TABLESPACE ... ADD DATAFILE`](alter-tablespace.html "15.1.10 ALTER TABLESPACE Statement").
...
* For the rules covering the naming of MySQL tablespaces, see
  [Section 11.2, “Schema Object Names”](identifiers.html "11.2 Schema Object Names"). In addition to these rules, the
...
* The `TABLESPACE` option may be used with
  ...
  [`ALTER TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement") to assign an
...
* General tablespaces support the addition of tables of any row
  format using
  [`CREATE TABLE ...
  TABLESPACE`](create-table.html "15.1.20 CREATE TABLE Statement").
  [`innodb_file_per_table`](innodb-parameters.html#sysvar_innodb_file_per_table) does
  not need to be enabled.

* [`innodb_strict_mode`](innodb-parameters.html#sysvar_innodb_strict_mode) is not
  applicable to general tablespaces. Tablespace management rules
  are strictly enforced independently of
  [`innodb_strict_mode`](innodb-parameters.html#sysvar_innodb_strict_mode). If
  `CREATE TABLESPACE` parameters are incorrect
  or incompatible, the operation fails regardless of the
  [`innodb_strict_mode`](innodb-parameters.html#sysvar_innodb_strict_mode) setting.
  When a table is added to a general tablespace using
  [`CREATE TABLE ...
  TABLESPACE`](create-table.html "15.1.20 CREATE TABLE Statement") or
  [`ALTER TABLE ...
  TABLESPACE`](alter-table.html "15.1.9 ALTER TABLE Statement"),
  [`innodb_strict_mode`](innodb-parameters.html#sysvar_innodb_strict_mode) is ignored
...
* Use `DROP TABLESPACE` to remove a tablespace.
  ...
  [`DROP TABLE`](drop-table.html "15.1.32 DROP TABLE Statement") prior to dropping
  the tablespace. Before dropping an NDB Cluster tablespace you
  must also remove all its data files using one or more
  [`ALTER
  TABLESPACE ... DROP DATATFILE`](alter-tablespace.html "15.1.10 ALTER TABLESPACE Statement") statements. See
...
* All parts of an `InnoDB` table added to an
  ...
  [`BLOB`](blob.html "13.3.4 The BLOB and TEXT Types") pages.
...
* Similar to the system tablespace, truncating or dropping
  ...
  [.ibd data file](glossary.html#glos_ibd_file ".ibd file") which can
...
* [`ALTER TABLE ...
  ...
  [`ALTER TABLE
  ...IMPORT TABLESPACE`](alter-table.html "15.1.9 ALTER TABLE Statement") are not supported for tables
...
* [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") and [**mysqlpump**](mysqlpump.html "6.5.6 mysqlpump — A Database Backup Program")
  do not dump `InnoDB`
  [`CREATE TABLESPACE`](create-tablespace.html "15.1.21 CREATE TABLESPACE Statement") statements.
...
[`innodb_page_size`](innodb-parameters.html#sysvar_innodb_page_size) value of 16K.
...
[`CREATE LOGFILE GROUP`](create-logfile-group.html "15.1.16 CREATE LOGFILE GROUP Statement") statement
shown here:
...
[`CREATE TABLE`](create-table.html "15.1.20 CREATE TABLE Statement") statement with the
...
[`FILES`](information-schema-files-table.html "28.3.15 The INFORMATION_SCHEMA FILES Table") table, as shown here:
