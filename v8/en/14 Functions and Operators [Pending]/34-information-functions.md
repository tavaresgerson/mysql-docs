--- title: MySQL 8.4 Reference Manual :: 14.15 Information Functions url: https://dev.mysql.com/doc/refman/8.4/en/information-functions.html order: 34 ---
...
*  `CONNECTION_ID()`
  ...
  The value returned by `CONNECTION_ID()` is the same type of value as displayed in the `ID` column of the Information Schema `PROCESSLIST` table, the `Id` column of [`SHOW PROCESSLIST`](show-processlist.html "15.7.7.31 SHOW PROCESSLIST Statement") output, and the `PROCESSLIST_ID` column of the Performance Schema  `threads` table.
