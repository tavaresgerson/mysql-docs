...
* The `mysql` system schema, which contains
  ...
  it runs (see [Section 7.3, “The mysql System Schema”](system-schema.html "7.3 The mysql System Schema")).
...
* Other schemas, some of which are built in and may be
  ...
  + The [`performance_schema`](performance-schema.html "Chapter 29 MySQL Performance Schema"),
    [`INFORMATION_SCHEMA`](information-schema.html "Chapter 28 INFORMATION_SCHEMA Tables"),
    [`ndbinfo`](mysql-cluster-ndbinfo.html "25.6.16 ndbinfo: The NDB Cluster Information Database"), and
    [`sys`](sys-schema.html "Chapter 30 MySQL sys Schema") schemas.
...
is [`--upgrade=NONE`](server-options.html#option_mysqld_upgrade) as of MySQL
8.0.16, [`--no-dd-upgrade`](server-options.html#option_mysqld_no-dd-upgrade) prior to
MySQL 8.0.16.
...
* Prior to MySQL 8.0.16, [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables")
  upgrades the Performance Schema, the
  `INFORMATION_SCHEMA`, and the objects
  described in step 2. The DBA is expected to invoke
  [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") manually after starting the
  server.

* As of MySQL 8.0.16, the server performs all tasks previously
  handled by [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables"). Although
...
instructions in [In-Place Upgrade](upgrade-binary-package.html#upgrade-procedure-inplace "In-Place Upgrade") and
[Logical Upgrade](upgrade-binary-package.html#upgrade-procedure-logical "Logical Upgrade") indicate whether the
server performs all upgrade tasks or whether you must also invoke
[**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") after server startup.

Note

Because the server upgrades the Performance Schema,
`INFORMATION_SCHEMA`, and the objects described
in step 2 as of MySQL 8.0.16, [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables")
...
As of MySQL 8.0.16, the [`--upgrade`](server-options.html#option_mysqld_upgrade)
server option controls whether and how the server performs an
automatic upgrade at startup:

* With no option or with
  [`--upgrade=AUTO`](server-options.html#option_mysqld_upgrade), the server
  upgrades anything it determines to be out of date (steps 1 and
  2).

* With [`--upgrade=NONE`](server-options.html#option_mysqld_upgrade), the server
...
* With [`--upgrade=MINIMAL`](server-options.html#option_mysqld_upgrade), the
...
* With [`--upgrade=FORCE`](server-options.html#option_mysqld_upgrade), the
  server upgrades the data dictionary, the Performance Schema,
  and the `INFORMATION_SCHEMA`, if necessary
...
* Perform a normal upgrade (steps 1 and 2 as necessary):
  ...
  + Prior to MySQL 8.0.16: [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") followed
    by [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables")

  + As of MySQL 8.0.16: [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server")
* Perform only step 1 as necessary:
  ...
  + Prior to MySQL 8.0.16: It is not possible to perform all
    ...
    [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") followed by
    [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") with the
  ...
  + As of MySQL 8.0.16: [**mysqld
    --upgrade=MINIMAL**](mysqld.html "6.3.1 mysqld — The MySQL Server")

* Perform step 1 as necessary, and force step 2:

  + Prior to MySQL 8.0.16: [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") followed
    by [**mysql\_upgrade --force**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables")

  + As of MySQL 8.0.16: [**mysqld
    --upgrade=FORCE**](mysqld.html "6.3.1 mysqld — The MySQL Server")

Prior to MySQL 8.0.16, certain [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables")
...
<table summary="Server --upgrade option value equivalents to certain mysql_upgrade options."><col style="width: 50%"/><col style="width: 50%"/><thead><tr>
...
* Step 2 upgrades the system tables to ensure that they have the
  ...
  [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") performs the step. With
  respect to the content of the help tables and time zone
  tables, [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") does not load either
...
* From MySQL 8.0.30, when Step 2 is upgrading the system tables
  ...
  [`DROP USER`](drop-user.html "15.7.1.5 DROP USER Statement"), and
  [`RENAME USER`](rename-user.html "15.7.1.7 RENAME USER Statement") statements, and for
...
* Step 2 processes all tables in all user schemas as necessary.
  ...
  [`CHECK TABLE`](check-table.html "15.7.3.2 CHECK TABLE Statement") statement. For
  ...
  + Prior to MySQL 8.0.16: Invoke
    [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") with the
    [`--force`](mysql-upgrade.html#option_mysql_upgrade_force) option.
