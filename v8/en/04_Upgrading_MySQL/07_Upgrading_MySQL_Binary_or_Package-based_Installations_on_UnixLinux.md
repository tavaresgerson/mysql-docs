...
9. In the previous step, the server upgrades the data
   ...
   * Prior to MySQL 8.0.16, the server upgrades only the data
     ...
     [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") to perform the
     ...
     [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") has been run
...
If the upgrade process uses [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables")
...
see [Section 3.4, “What the MySQL Upgrade Process Upgrades”](upgrading-what-is-upgraded.html "3.4 What the MySQL Upgrade Process Upgrades").
...
9. Perform any remaining upgrade operations:
   ...
   * In MySQL 8.0.16 and higher, shut down the server, then
     ...
     Upon restart with
     [`--upgrade=FORCE`](server-options.html#option_mysqld_upgrade), the
...
2. Upgrade the system tables.
   ...
   [**mysql\_upgrade**](mysql-upgrade.html "6.4.5 mysql_upgrade — Check and Upgrade MySQL Tables") client to upgrade the
   system tables. As of MySQL 8.0.16, the server performs this
   action: To upgrade the system tables, restart each
   individual [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") without the
