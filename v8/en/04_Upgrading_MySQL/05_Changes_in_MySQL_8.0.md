...
* [Data Dictionary Changes](upgrading-from-previous-series.html#upgrade-data-dictionary-changes "Data Dictionary Changes")
...
* [Configuration Changes](upgrading-from-previous-series.html#upgrade-configuration-changes "Configuration Changes")
* [Server Changes](upgrading-from-previous-series.html#upgrade-server-changes "Server Changes")
* [InnoDB Changes](upgrading-from-previous-series.html#upgrade-innodb-changes "InnoDB Changes")
* [SQL Changes](upgrading-from-previous-series.html#upgrade-sql-changes "SQL Changes")
* [Changed Server Defaults](upgrading-from-previous-series.html#upgrade-server-defaults "Changed Server Defaults")
* [Valid Performance Regressions](upgrading-from-previous-series.html#upgrade-performance-regressions "Valid Performance Regressions")
...
[Section 3.6, “Preparing Your Installation for Upgrade”](upgrade-prerequisites.html "3.6 Preparing Your Installation for Upgrade"). A data
...
* [caching\_sha2\_password Compatibility Issues and Solutions](upgrading-from-previous-series.html#upgrade-caching-sha2-password-compatibility-issues "caching_sha2_password Compatibility Issues and Solutions")
* [caching\_sha2\_password-Compatible Clients and Connectors](upgrading-from-previous-series.html#upgrade-caching-sha2-password-compatible-connectors "caching_sha2_password-Compatible Clients and Connectors")
* [caching\_sha2\_password and the root Administrative Account](upgrading-from-previous-series.html#upgrade-caching-sha2-password-root-account "caching_sha2_password and the root Administrative Account")
* [caching\_sha2\_password and Replication](upgrading-from-previous-series.html#upgrade-caching-sha2-password-replication "caching_sha2_password and Replication")
...
* Clients and connectors that have not been updated to know
  ...
  [Authentication Plugin Connector-Writing Considerations](pluggable-authentication.html#pluggable-authentication-connector-writing "Authentication Plugin Connector-Writing Considerations").
...
* Clients and connectors that have not been updated to know
  ...
  *cannot* connect to accounts that
  ...
  [Authentication Plugin Client/Server Compatibility](pluggable-authentication.html#pluggable-authentication-compatibility "Authentication Plugin Client/Server Compatibility").)
...
* Because `caching_sha2_password` is also now
  ...
  invoked with a
  [`--default-auth=mysql_native_password`](connection-options.html#option_general_default-auth)
  option.
...
* Standard MySQL clients such as [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") and
  [**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") are
...
* Some MySQL clients support a
  [`--default-auth`](connection-options.html#option_general_default-auth) option.
  (Standard MySQL clients such as [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") and
  [**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program") support this option but can
...
* Programs that use the `libmysqlclient` C
  API can call the
  [`mysql_options()`](/doc/c-api/8.0/en/mysql-options.html) function
...
* The `libmysqlclient` client library in
  ...
  [**mysql**](mysql.html "6.5.1 mysql — The MySQL Command-Line Client") and [**mysqladmin**](mysqladmin.html "6.5.2 mysqladmin — A MySQL Server Administration Program")
...
[Section 2.9.1, “Initializing the Data Directory”](data-directory-initialization.html "2.9.1 Initializing the Data Directory")), the
...
use [`ALTER USER`](alter-user.html "15.7.1.1 ALTER USER Statement") as follows to
...
* Supply a
  [`--default-authentication-plugin=mysql_native_password`](server-system-variables.html#sysvar_default_authentication_plugin)
  option along with
...
* Set
  ...
  [`--defaults-file`](server-options.html#option_mysqld_defaults-file) option along
  with [`--initialize`](server-options.html#option_mysqld_initialize) or
  [`--initialize-insecure`](server-options.html#option_mysqld_initialize-insecure). (In
...
* Use any of the following [`CHANGE MASTER
  TO`](change-master-to.html "15.4.2.1 CHANGE MASTER TO Statement") options:
...
* **Incompatible change**: A
  ...
  [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") into a MySQL 8.0
  ...
  [Section 3.6, “Preparing Your Installation for Upgrade”](upgrade-prerequisites.html "3.6 Preparing Your Installation for Upgrade"), describes how to
...
* **Incompatible change**:
  ...
  [Features Removed in MySQL 8.0](mysql-nutshell.html#mysql-nutshell-removals "Features Removed in MySQL 8.0")). Applications
...
* **Important change**: The
  ...
  + The default value of the
    [`collation_server`](server-system-variables.html#sysvar_collation_server) and
...
* In MySQL 8.0.11, several deprecated features related to
  ...
  [`GRANT`](grant.html "15.7.1.6 GRANT Statement") statement to modify
  ...
  described in [Features Removed in MySQL 8.0](mysql-nutshell.html#mysql-nutshell-removals "Features Removed in MySQL 8.0").

  To avoid a startup failure on MySQL 8.0, remove any instance
  of `NO_AUTO_CREATE_USER` from
  [`sql_mode`](server-system-variables.html#sysvar_sql_mode) system variable
  ...
  [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program") removes
...
* In MySQL 8.0.11, these deprecated compatibility SQL modes
  ...
  [`ALTER TABLE`](alter-table.html "15.1.9 ALTER TABLE Statement") is no longer
  ...
  [`sql_mode`](server-system-variables.html#sysvar_sql_mode) value includes any
...
* As of MySQL 8.0.3, spatial data types permit an
  ...
  column. See [Section 13.4.1, “Spatial Data Types”](spatial-type-overview.html "13.4.1 Spatial Data Types").
  ...
  [Section 10.3.3, “SPATIAL Index Optimization”](spatial-index-optimization.html "10.3.3 SPATIAL Index Optimization"). If you want
...
* Several spatial functions were removed in MySQL 8.0.0 due to
  ...
  --check-upgrade**](mysqlcheck.html "6.5.3 mysqlcheck — A Table Maintenance Program") for removed spatial functions and
...
* From MySQL 8.0.13, because of differences between row-based
  ...
  + As of MySQL 8.0.27, the clone plugin permits
    ...
    [`clone_block_ddl`](clone-plugin-options-variables.html#sysvar_clone_block_ddl)
    variable. See
...
* From MySQL 8.0.30, error log components listed in the
  [`log_error_services`](server-system-variables.html#sysvar_log_error_services) value at
  ...
  COMPONENT`](install-component.html "15.7.4.3 INSTALL COMPONENT Statement") and you list those components in a
  [`log_error_services`](server-system-variables.html#sysvar_log_error_services) setting
  ...
  [Error Log Configuration Methods](error-log-configuration.html#error-log-configuration-methods "Error Log Configuration Methods").
...
* [`INFORMATION_SCHEMA`](information-schema.html "Chapter 28 INFORMATION_SCHEMA Tables") views based
  ...
  **Table 3.1 Renamed InnoDB Information Schema Views**
  ...
  <table summary="InnoDB Information Schema views that were renamed in MySQL 8.0."><col style="width: 50%"/><col style="width: 50%"/><thead><tr>
<th>Old Name</th>
<th>New Name</th>
</tr></thead><tbody><tr>
...
* The [zlib library](http://www.zlib.net/)
  ...
  tables. As a result,
  [`CREATE TABLE
  ... ROW_FORMAT=COMPRESSED`](create-table.html "15.1.20 CREATE TABLE Statement"),
  [`INSERT`](insert.html "15.2.7 INSERT Statement"), and
  [`UPDATE`](update.html "15.2.17 UPDATE Statement") operations with row
...
* With the introduction of the
  [`--innodb-directories`](innodb-parameters.html#sysvar_innodb_directories)
  ...
  the [`innodb_directories`](innodb-parameters.html#sysvar_innodb_directories)
...
* Undo logs can no longer reside in the system tablespace. In
  ...
  [`ALTER UNDO
  TABLESPACE`](alter-tablespace.html "15.1.10 ALTER TABLESPACE Statement") and
  [`DROP UNDO
  TABLESPACE`](drop-tablespace.html "15.1.33 DROP TABLESPACE Statement") syntax, respectively, after upgrading.
...
* **Incompatible change**: As of
  MySQL 8.0.17, the
  [`CREATE
  TABLESPACE ... ADD DATAFILE`](create-tablespace.html "15.1.21 CREATE TABLESPACE Statement") clause does not permit
...
* Due to a regression introduced in MySQL 8.0.14, in-place
  ...
  [`lower_case_table_names=1`](server-system-variables.html#sysvar_lower_case_table_names).
  ...
  1. Restart the server with
     [`--upgrade=force`](server-options.html#option_mysqld_upgrade) to force
  ...
  5. Run [`ANALYZE TABLE`](analyze-table.html "15.7.3.1 ANALYZE TABLE Statement") on each
...
* As of MySQL 8.0.21, a warning is written to the error log at
  ...
  [`innodb_directories`](innodb-parameters.html#sysvar_innodb_directories) setting.
...
* Some keywords may be reserved in MySQL 8.0 that
  ...
  [Section 11.3, “Keywords and Reserved Words”](keywords.html "11.3 Keywords and Reserved Words"). This can cause words previously
...
* The handling of system variable values by MySQL flow control
  functions such as [`IFNULL()`](flow-control-functions.html#function_ifnull)
  and [`CASE()`](flow-control-functions.html#operator_case) changed in MySQL
...
* **Incompatible change**: MySQL
  ...
  [`BINARY`](binary-varbinary.html "13.3.3 The BINARY and VARBINARY Types") values to nonbinary
  ...
  2. Drop and then re-create the index.
     ...
     [`ALTER TABLE
     table FORCE`](alter-table.html "15.1.9 ALTER TABLE Statement"),
     instead.
...
<table summary="Summary of which MySQL Server defaults changed in this release."><col style="width: 33%"/><col style="width: 33%"/><col style="width: 33%"/><thead><tr>
...
<td><a class="link" href="group-replication-system-variables.html#sysvar_group_replication_autorejoin_tries"><code class="literal">group_replication_autorejoin_tries</code></a></td>
<td>0</td>
<td>3</td>
</tr><tr>
<td><a class="link" href="group-replication-system-variables.html#sysvar_group_replication_exit_state_action"><code class="literal">group_replication_exit_state_action</code></a></td>
<td>ABORT_SERVER</td>
<td>READ_ONLY</td>
</tr><tr>
<td><a class="link" href="group-replication-system-variables.html#sysvar_group_replication_member_expel_timeout"><code class="literal">group_replication_member_expel_timeout</code></a></td>
<td>0</td>
<td>5</td>
</tr></tbody></table>

For more information about options or variables which have been
added, see [Option and Variable Changes for MySQL 8.0](/doc/mysqld-version-reference/en/optvar-changes-8-0.html), in the
...
* The default value of the
  [`character_set_server`](server-system-variables.html#sysvar_character_set_server) system
...
* The default value of the
  ...
  [`--collation-server`](server-system-variables.html#sysvar_collation_server) changed
...
* The autosize algorithm for the
  [`back_log`](server-system-variables.html#sysvar_back_log) system variable
  has changed.  The value for autosize (-1) is now set to the
  value of [`max_connections`](server-system-variables.html#sysvar_max_connections),
  ...
  [`max_connections`](server-system-variables.html#sysvar_max_connections) number of
...
* The default value of the
  ...
  as the largest [Section 13.3.4, “The BLOB and TEXT Types”](blob.html "13.3.4 The BLOB and TEXT Types") you want to use. To
  revert to the previous behavior, set
  `max_allowed_packet=4194304`.

* The default value of the
  [`max_error_count`](server-system-variables.html#sysvar_max_error_count) system
...
* The default value of the
  [`event_scheduler`](server-system-variables.html#sysvar_event_scheduler) system
...
* **Incompatible change** The
  ...
  for [`innodb_undo_tablespaces`](innodb-parameters.html#sysvar_innodb_undo_tablespaces)
  ...
  such as a [**mysqldump**](mysqldump.html "6.5.4 mysqldump — A Database Backup Program").
...
* The default value of the
  [`innodb_flush_method`](innodb-parameters.html#sysvar_innodb_flush_method) system
  ...
  [`innodb_flush_method`](innodb-parameters.html#sysvar_innodb_flush_method) default
  `NULL` meant
  `async_unbuffered` in 5.7, and is replaced
  by default `unbuffered` in 8.0, which in
  combination with the existing default
  [`innodb_use_native_aio=ON`](innodb-parameters.html#sysvar_innodb_use_native_aio)
  has the same effect.
...
* The default value of the
  [`log_bin`](replication-options-binary-log.html#sysvar_log_bin) system variable
  changed from `OFF` to
  ...
  later requires a [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") restart. Enabling
  it by default also provides better test coverage and it
  becomes easier to spot performance regressions. Remember to
  also set [`server_id`](replication-options.html#sysvar_server_id) (see
...
* The default value of the
  [`server_id`](replication-options.html#sysvar_server_id) system
  variable changed from `0` to
  `1` (combines with the change to
  [`log_bin=ON`](replication-options-binary-log.html#sysvar_log_bin)). The server can
  be started with this default ID, but in practice you must
  set the [`server-id`](replication-options.html#sysvar_server_id) according
  to the replication infrastructure being deployed, to avoid
  having duplicate server ids.

* The default value of the
  [`log-slave-updates`](replication-options-binary-log.html#sysvar_log_slave_updates) system
  variable changed from `OFF` to
...
* The default value of the
  [`expire_logs_days`](replication-options-binary-log.html#sysvar_expire_logs_days) system
  variable changed from `0` to
  `30`. The new default `30`
  causes [**mysqld**](mysqld.html "6.3.1 mysqld — The MySQL Server") to periodically purge
...
* The default value of the
  [`master_info_repository`](replication-options-replica.html#sysvar_master_info_repository) and
...
* The default value of the
  [`transaction-write-set-extraction`](replication-options-binary-log.html#sysvar_transaction_write_set_extraction)
  system variable changed from `OFF` to
...
[`--innodb-dedicated-server`](innodb-parameters.html#option_mysqld_innodb-dedicated-server) option,
...
[`--innodb-dedicated-server`](innodb-parameters.html#option_mysqld_innodb-dedicated-server), which
...
[`innodb_flush_method`](innodb-parameters.html#sysvar_innodb_flush_method). See
...
[`variables_info`](performance-schema-variables-info-table.html "29.12.14.2 Performance Schema variables_info Table") table, which shows
...
* [Binary logs](binary-log.html "7.4.4 The Binary Log") are disabled
...
[Transactional Data
Dictionary](data-dictionary.html "Chapter 16 MySQL Data Dictionary") (atomic DDL) was introduced in 8.0.
...
The more modern [TLS
ciphers/algorithms](encrypted-connections.html "8.3 Using Encrypted Connections") used as of 5.7.28 has an effect when
...
[Performance Schema
(PFS)](performance-schema.html "Chapter 29 MySQL Performance Schema") instrumentation is much wider in 8.0 than in 5.7:
