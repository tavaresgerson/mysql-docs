...
No entanto, a cascata não se propaga na replica porque, na replica, a `DELETE` para `fc1` não exclui nenhuma linha de `fc2`. A cópia da replica de `fc2` ainda contém todas as linhas que foram originalmente inseridas:
...
Essa diferença ocorre porque as exclusões em cascata são gerenciadas internamente pelo mecanismo de armazenamento `InnoDB`, o que significa que nenhuma das alterações é registrada.
