...
<table frame="all" summary="Colunas da tabela config_options."><col style="width: 20%"/><col style="width: 80%"/><thead><tr> <th>Coluna</th> <th>Descrição</th> </tr></thead><tbody><tr> <td><code>Nome</code></td> <td>Nome da opção de configuração relacionada ao <span><strong>memcached</strong></span>. As seguintes opções de configuração são suportadas pela tabela <code>config_options</code>: <div class="itemizedlist"> <ul class="itemizedlist" style="list-style-type: disc; "><li class="listitem"><p> <code>separator</code>: Usado para separar os valores de uma string longa em valores separados quando há várias <code>value_columns</code> definidas. Por padrão, o <code>separator</code> é um caractere <code>|</code>. Por exemplo, se você definir `col1`, `col2` como colunas de valor e definir `|` como separador, poderá executar o seguinte comando `memcached` para inserir valores em `col1` e `col2`, respectivamente: `set keyx 10 0 19 valuecolx|valuecoly` ... Por exemplo, <code>@@t1.some_key</code> e <code>@@t2.some_key</code> têm o mesmo valor de chave, mas são armazenados em tabelas diferentes. </p></li></ul> </div> </td> </tr><tr> <td><code>Valor</code></td> <td>O valor atribuído à opção de configuração relacionada ao <span><strong>memcached</strong></span>.</td> </tr></tbody></table>
