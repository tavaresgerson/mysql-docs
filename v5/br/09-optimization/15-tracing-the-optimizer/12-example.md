...
Agora podemos examinar a traça, cuja primeira coluna (`QUERY`), contendo a declaração original a ser traçada, é mostrada aqui:
...
A coluna `TRACE` começa mostrando que a execução da instrução é composta por etapas discretas, como esta:
...
Isso é seguido pela preparação da junção para o primeiro (e único) `SELECT` na instrução que está sendo rastreada, como mostrado aqui:
...
A saída mostrada agora exibe a consulta conforme ela é usada para preparar a junção; todas as colunas (campos) foram resolvidas em suas bases de dados e tabelas, e cada `SELECT` é anotado com um número de sequência, o que pode ser útil ao estudar subconsultas.

A próxima parte da traça mostra como a junção é otimizada, começando com o processamento da condição:
...
Foi identificado um acesso `ref` que rejeita `NULL`: nenhum `NULL` em `test.alias1.col_int_key` pode ter uma correspondência. (Observe que poderia ter uma correspondência, se o operador fosse um igual `<=>` seguro para nulos).
...
Precisamos encontrar uma ordem ótima para as tabelas. Normalmente, a busca gananciosa é usada, mas, como a declaração utiliza uma junção direta, apenas a ordem solicitada é explorada, e um ou mais métodos de acesso são selecionados. Como mostrado nesta parte da traça, podemos escolher uma varredura de tabela:
...
Como foi demonstrado na segunda parte da análise de intervalo, não é possível usar `GROUP_MIN_MAX` porque ele aceita apenas uma tabela, e temos duas na junção. Isso significa que nenhum acesso de intervalo é possível.
...
Para `alias2`, escolhemos o acesso `ref` na chave primária em vez de uma varredura da tabela, porque o número de registros retornados por esta última (75) é muito maior do que o retornado pelo acesso `ref` (1), como mostrado aqui:
...
Agora que a ordem das tabelas está definida, podemos dividir a condição `WHERE` em partes que podem ser testadas precocemente (empilhamento das condições na árvore de junção):
...
Agora, tentamos simplificar a `ORDER BY`:
...
Como a cláusula `WHERE` contém `alias2.pk=alias1.col_int_key`, a ordenação por ambas as colunas é desnecessária; podemos ordenar apenas pela primeira coluna, uma vez que a segunda coluna sempre é igual a ela.
...
A cláusula `ORDER BY` mais curta (que não é visível na saída do `EXPLAIN`) pode ser implementada como uma varredura de índice, pois usa apenas uma única coluna de uma tabela.
...
Todos os rastreamentos têm a mesma estrutura básica. Se uma declaração usa subconsultas, podem haver múltiplas preparações, otimizações e execuções, além de transformações específicas para subconsultas.
