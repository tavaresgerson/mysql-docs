...
Para auxiliar no tratamento de `NULL`, você pode usar os operadores `IS NULL` e `IS NOT NULL` e a função `IFNULL()`.
...
Para procurar por valores `NULL`, você deve usar o teste `IS NULL`. As seguintes instruções mostram como encontrar o número de telefone `NULL` e o número de telefone vazio:
...
Veja Seção 3.3.4.6, “Trabalhando com Valores NULL”, para informações e exemplos adicionais.
...
Ao ler dados com `LOAD DATA`, colunas vazias ou ausentes são atualizadas com `''`. Para carregar um valor `NULL` em uma coluna, use `\N` no arquivo de dados. A palavra literal `NULL` também pode ser usada sob certas circunstâncias. Veja Seção 13.2.6, “LOAD DATA Statement”.
