...
`CREATE TABLE` e `ALTER TABLE` que fariam com que uma tabela `NDB` particionada pelo usuário não atendesse a um ou ambos os requisitos a seguir não são permitidos e falham com um erro:
...
**Exceção.** Se uma tabela `NDB` particionada pelo usuário for criada usando uma lista de colunas vazia (ou seja, usando `PARTITION BY KEY()` ou `PARTITION BY LINEAR KEY()`), então nenhuma Primary Key explícita é necessária.

**Seleção de Partition (Partition selection).** A seleção de Partition não é suportada para tabelas `NDB`. Consulte Section 22.5, “Partition Selection”, para mais informações.

**Upgrade de tabelas particionadas.** Ao realizar um upgrade, tabelas particionadas por `KEY` e que usam qualquer storage engine diferente de `NDB` devem ser despejadas (dumped) e recarregadas (reloaded).
