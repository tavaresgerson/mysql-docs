...
A instrução `CREATE TABLE` falha porque tanto `col1` quanto `col3` estão incluídas na Chave de Partitioning proposta, mas nenhuma dessas colunas faz parte de ambas as Unique Keys na tabela. Isto mostra uma possível correção para a definição de tabela inválida:
...
É possível adicionar uma Primary Key a `t_no_pk` usando qualquer uma destas instruções `ALTER TABLE`:
...
Estas regras também se aplicam a tabelas existentes não particionadas que você deseja particionar usando `ALTER TABLE ... PARTITION BY`. Considere uma tabela `np_pk` criada conforme mostrado aqui:
...
A seguinte instrução `ALTER TABLE` falha com um erro, porque a coluna `added` não faz parte de nenhuma Unique Key na tabela:
