...
O MySQL 5.7 oferece suporte ao particionamento `LIST COLUMNS`. Esta é uma variante do particionamento `LIST` que permite o uso de múltiplas colunas como chaves de Partition, e para que colunas de tipos de dados que não sejam inteiros sejam usadas como colunas de Partitioning; você pode usar tipos string, colunas `DATE` e `DATETIME`. (Para mais informações sobre tipos de dados permitidos para colunas de particionamento `COLUMNS`, consulte Section 22.2.3, “COLUMNS Partitioning”.)
...
Consulte Section 22.2.3.1, “RANGE COLUMNS partitioning”, para mais informações.

Além disso (assim como no particionamento `RANGE COLUMNS`), você pode usar múltiplas colunas na cláusula `COLUMNS()`.

Consulte Section 13.1.18, “CREATE TABLE Statement”, para informações adicionais sobre a sintaxe `PARTITION BY LIST COLUMNS()`.