...
Você também pode usar uma opção `PARTITION` na porção `SELECT` de uma instrução `INSERT ... SELECT`, conforme mostrado aqui:
...
Para informações gerais sobre *joins* no MySQL, consulte Section 13.2.9.2, “JOIN Clause”.

Quando a opção `PARTITION` é usada com instruções `DELETE`, apenas aquelas partições (e subpartições, se houver) listadas com a opção são verificadas em busca de linhas a serem excluídas. Quaisquer outras partições são ignoradas, conforme mostrado aqui:
...
Apenas as duas linhas nas partições `p0` e `p1` que correspondiam à condição `WHERE` foram excluídas. Como você pode ver pelo resultado quando a instrução `SELECT` é executada uma segunda vez, resta uma linha na tabela que corresponde à condição `WHERE`, mas que reside em uma partição diferente (`p2`).

Instruções `UPDATE` que usam seleção explícita de partições se comportam da mesma maneira; apenas as linhas nas partições referenciadas pela opção `PARTITION` são consideradas ao determinar as linhas a serem atualizadas, como pode ser visto pela execução das seguintes instruções:
...
Da mesma forma, quando `PARTITION` é usado com `DELETE`, apenas as linhas na partição ou partições nomeadas na lista de partições são verificadas para exclusão.

Para instruções que inserem linhas, o comportamento difere, pois a falha em encontrar uma partição adequada faz com que a instrução falhe. Isso é verdade tanto para instruções `INSERT` quanto para `REPLACE`, conforme mostrado aqui:
...
O precedente é verdadeiro tanto para instruções `INSERT` quanto para instruções `REPLACE` que gravam múltiplas linhas.
