...
O Group Replication é construído sobre replicação baseada em GTID; portanto, você também deve estar ciente da Seção 16.1.3.6, “Restrictions on Replication with GTIDs”.

* **Gap Locks.** O processo de certificação do Group Replication para transações concorrentes não leva em consideração os [gap locks], pois as informações sobre gap locks não estão disponíveis fora do `InnoDB`. Veja Gap Locks para mais informações.
...
* **Table Locks and Named Locks.** O processo de certificação não leva em consideração os table locks (veja Seção 13.3.5, “LOCK TABLES and UNLOCK TABLES Statements”) ou named locks (veja `GET_LOCK()`).

* **Replication Event Checksums.** Devido a uma limitação de design dos checksums de eventos de replicação, o Group Replication atualmente não pode utilizá-los. Portanto, defina `--binlog-checksum=NONE`.
