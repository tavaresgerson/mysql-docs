...
As seções imediatamente subsequentes contêm as informações necessárias para escolher, baixar e verificar sua distribuição. As instruções nas seções posteriores do capítulo descrevem como instalar a distribuição que você escolheu. Para distribuições binárias, consulte as instruções na Seção 2.2, “Instalando MySQL em Unix/Linux Usando Binários Genericos” ou a seção correspondente para sua plataforma, se disponível. Para construir o MySQL a partir de fonte, use as instruções na Seção 2.8, “Instalando MySQL a partir de Fonte”. ### 2.1.1 Plataformas Suporteadas

O suporte da plataforma MySQL evolui ao longo do tempo; consulte <https://www.mysql.com/support/supportedplatforms/database.html> para as atualizações mais recentes.

### 2.1.2 Qual versão e distribuição do MySQL a instalar
...
O esquema de nomenclatura no MySQL 5.7 utiliza nomes de versão que consistem em três números e um sufixo opcional; por exemplo, **mysql-5.7.1-m1**. Os números dentro do nome da versão são interpretados da seguinte forma:

* O primeiro número (**5**) é o número da versão principal.

* O segundo número (**7**) é o número de versão menor. Juntos, os números principais e menores constituem o número da série de lançamento. O número da série descreve o conjunto de recursos estáveis.

* O terceiro número (**1**) é o número da versão dentro da série de lançamento. Este é incrementado para cada nova versão de correção de bugs. Na maioria dos casos, a versão mais recente dentro de uma série é a melhor escolha.

Os nomes das versões também podem incluir um sufixo para indicar o nível de estabilidade da versão. As versões dentro de uma série progridem através de um conjunto de sufixos para indicar como o nível de estabilidade melhora. Os sufixos possíveis são:

* **m*N*** (por exemplo, **m1**, **m2**, **m3**, ...) indica um número de marco. O desenvolvimento do MySQL utiliza um modelo de marco, no qual cada marco introduz um pequeno subconjunto de funcionalidades testadas minuciosamente. De um marco para o próximo, as interfaces das funcionalidades podem mudar ou até mesmo ser removidas, com base no feedback fornecido pelos membros da comunidade que experimentam essas versões iniciais. As funcionalidades dentro das versões de marco podem ser consideradas de qualidade de pré-produção.

* **rc** indica um candidato de lançamento (RC). Acredita-se que os candidatos de lançamento sejam estáveis, tendo passado todos os testes internos do MySQL. Novos recursos ainda podem ser introduzidos em lançamentos RC, mas o foco muda para corrigir bugs para estabilizar recursos introduzidos anteriormente na série.

* A ausência de um sufixo indica uma disponibilidade geral (GA) ou lançamento de produção. Os lançamentos GA são estáveis, tendo passado com sucesso pelas etapas de lançamento anteriores, e acredita-se que sejam confiáveis, livres de bugs graves e adequados para uso em sistemas de produção.

O desenvolvimento de uma série começa com lançamentos de marco, seguidos por lançamentos RC e, finalmente, atinge lançamentos com status GA.

Depois de escolher qual versão do MySQL instalar, decida qual formato de distribuição instalar para o seu sistema operacional. Para a maioria dos casos de uso, uma distribuição binária é a escolha certa. Distribuições binárias estão disponíveis em formato nativo para muitas plataformas, como pacotes RPM para Linux ou pacotes DMG para macOS. As distribuições também estão disponíveis em formatos mais genéricos, como arquivos Zip ou arquivos **tar** comprimidos. Em Windows, você pode usar o [Instalação do MySQL][(mysql-installer.html "2.3.3 MySQL Installer for Windows")] para instalar uma distribuição binária.

Em algumas circunstâncias, pode ser preferível instalar o MySQL a partir de uma distribuição de fonte:

* Você deseja instalar o MySQL em um local explícito. As distribuições binárias padrão estão prontas para execução em qualquer local de instalação, mas você pode precisar de ainda mais flexibilidade para colocar os componentes do MySQL onde você deseja.

* Você deseja configurar `mysqld` com recursos que talvez não estejam incluídos nas distribuições binárias padrão. Aqui está uma lista das opções extras mais comuns usadas para garantir a disponibilidade dos recursos:

+ `-DWITH_LIBWRAP=1` para suporte a wrappers TCP.

+ `-DWITH_ZLIB={system|bundled}` para recursos que dependem de compressão

+ `-DWITH_DEBUG=1` para suporte de depuração
(source-configuration-options.html#option_cmake_with_debug)

Para informações adicionais, consulte [Seção 2.8.7, “Opções de Configuração de Fonte MySQL”][(source-configuration-options.html "2.8.7 MySQL Source-Configuration Options")].

* Você deseja configurar `mysqld` (mysqld.html "4.3.1 mysqld — The MySQL Server") sem algumas funcionalidades que estão incluídas nas distribuições binárias padrão. Por exemplo, as distribuições são normalmente compiladas com suporte para todos os conjuntos de caracteres. Se você deseja um servidor MySQL menor, pode recompilar com suporte apenas para os conjuntos de caracteres que você precisa.

* Você quer ler ou modificar o código C e C++ que compõe o MySQL. Para isso, obtenha uma distribuição de código-fonte.

* As distribuições de fonte contêm mais testes e exemplos do que as distribuições binárias.

### 2.1.3 Como obter o MySQL

Consulte nossa página de downloads em <https://dev.mysql.com/downloads/> para obter informações sobre a versão atual do MySQL e instruções de download.

Para plataformas Linux baseadas em RPM que utilizam Yum como seu sistema de gerenciamento de pacotes, o MySQL pode ser instalado usando o [Repositório Yum de MySQL][(https://dev.mysql.com/downloads/repo/yum/)]. Consulte a Seção 2.5.1, “Instalando MySQL no Linux Usando o Repositório MySQL Yum”, para obter detalhes.

Para plataformas Linux baseadas no Debian, o MySQL pode ser instalado usando o [repositório APT do MySQL][(https://dev.mysql.com/downloads/repo/apt/)]. Consulte a Seção 2.5.3, “Instalando o MySQL no Linux usando o repositório APT do MySQL”, para obter detalhes.

Para as plataformas SUSE Linux Enterprise Server (SLES), o MySQL pode ser instalado usando o [Repositório SLES MySQL][(https://dev.mysql.com/downloads/repo/suse/)]. Consulte a Seção 2.5.4, “Instalando MySQL no Linux usando o Repositório MySQL SLES”, para obter detalhes.

Para obter a fonte de desenvolvimento mais recente, consulte a Seção 2.8.5, “Instalando o MySQL usando uma árvore de fonte de desenvolvimento”.

### 2.1.4 Verificar a integridade do pacote usando verificações de checksums MD5 ou GnuPG

Depois de baixar o pacote MySQL que atende às suas necessidades e antes de tentar instalá-lo, certifique-se de que ele está intacto e não foi manipulado. Existem três meios de verificação de integridade:

* Checagens MD5
* Assinaturas criptográficas usando `GnuPG`, o GNU Privacy Guard
* Para pacotes RPM, o mecanismo de verificação de integridade RPM integrado

As seções a seguir descrevem como usar esses métodos.

Se você perceber que o checksum MD5 ou as assinaturas GPG não correspondem, tente primeiro baixar o pacote respectivo mais uma vez, talvez de outro site de espelhamento.

#### 2.1.4.1 Verificar o checksum MD5

Depois de baixar um pacote MySQL, você deve garantir que seu checksum MD5 corresponda ao fornecido nas páginas de download do MySQL. Cada pacote tem um checksum individual que você pode verificar contra o pacote que você baixou. O checksum MD5 correto está listado na página de downloads para cada produto MySQL; compare-o com o checksum MD5 do arquivo (produto) que você baixa.

Cada sistema operacional e configuração oferece sua própria versão de ferramentas para verificar o checksum MD5. Tipicamente, o comando é chamado de **md5sum**, ou pode ser chamado de **md5**, e alguns sistemas operacionais não o fornecem. No Linux, ele faz parte do pacote **GNU Text Utilities**, que está disponível para uma ampla gama de plataformas. Você também pode baixar o código-fonte de <http://www.gnu.org/software/textutils/>. Se você tiver o OpenSSL instalado, pode usar o comando **openssl md5 *`package_name`***. Uma implementação do utilitário de string de comando **md5** do Windows está disponível em <http://www.fourmilab.ch/md5/>. **winMd5Sum** é uma ferramenta gráfica de verificação MD5 que pode ser obtida em <http://www.nullriver.com/index/products/winmd5sum>. Nossos exemplos para Microsoft Windows assumem o nome **md5.exe**.

Exemplos de Linux e Microsoft Windows:
...
Certifique-se de verificar o checksum do arquivo *de arquivo* (por exemplo, o arquivo `.zip`, `.tar.gz` ou `.msi`) e não dos arquivos que estão contidos dentro do arquivo. Em outras palavras, verifique o arquivo antes de extrair seu conteúdo.
...
Assinamos pacotes de download do MySQL com **GnuPG** (GNU Privacy Guard). **GnuPG** é uma alternativa de código aberto à bem conhecida Pretty Good Privacy (**PGP**) do Phil Zimmermann. A maioria das distribuições Linux vem com **GnuPG** instalado por padrão. Caso contrário, consulte <http://www.gnupg.org/> para obter mais informações sobre **GnuPG** e como obtê-lo e instalá-lo.

Para verificar a assinatura de um pacote específico, você primeiro precisa obter uma cópia da nossa chave de compilação pública GPG, que você pode baixar em <http://pgp.mit.edu/>. A chave que você deseja obter é chamada `mysql-build@oss.oracle.com`. O keyID para pacotes MySQL 5.7.37 e superiores é `3A79BD29`. Após obter essa chave, você deve compará-la com a chave mostrada a seguir, antes de usá-la para verificar pacotes MySQL. Alternativamente, você pode copiar e colar a chave diretamente do texto abaixo.
...
Para importar a chave de construção em seu conjunto de chaves GPG público pessoal, use **gpg --import**. Por exemplo, se você salvou a chave em um arquivo chamado `mysql_pubkey.asc`, o comando de importação é o seguinte:
...
Você também pode baixar a chave do servidor de chaves pública usando o ID da chave pública, `3A79BD29`:
...
Se você deseja importar a chave na configuração RPM para validar os pacotes de instalação RPM, você deve ser capaz de importar a chave diretamente:
...
**Tabela 2.1 Arquivos de pacote e assinaturas do MySQL para arquivos de fonte**
...
<td><code>mysql-standard-5.7.44-linux-i686.tar.gz</code></td>
</tr>
<tr>
<td>Signature file</td>
<td><code>mysql-standard-5.7.44-linux-i686.tar.gz.asc</code></td>
</tr>
</tbody>
</table>
...
Se o pacote baixado for válido, você deve ver uma mensagem `Good signature` semelhante à seguinte:
...
A mensagem `Good signature` indica que a assinatura do arquivo é válida, quando comparada à assinatura listada em nosso site. Mas você também pode ver avisos, como este:
...
Isso é normal, pois eles dependem da configuração e da configuração do seu sistema. Aqui estão as explicações para esses avisos:

* *gpg: não foram encontradas chaves confiáveis no final*: Isso significa que a chave específica não é "confिável no final" por você ou sua rede de confiança, o que está bem para os propósitos de verificação de assinaturas de arquivos.

* *AVISO: Esta chave não está certificada com uma assinatura confiável! Não há indicação de que a assinatura pertença ao proprietário.*: Isso se refere ao seu nível de confiança na crença de que você possui nossa chave pública real. Esta é uma decisão pessoal. Idealmente, um desenvolvedor do MySQL entregaria a chave pessoalmente, mas mais comumente, você a baixou. O download foi manipulado? Provavelmente não, mas esta decisão cabe a você. Configurar uma rede de confiança é um método para confiar neles.
...
A seção 2.1.4.2, “Verificação de assinatura usando GnuPG”, descreve como verificar os downloads do MySQL usando GPG. Esse guia também se aplica ao Microsoft Windows, mas outra opção é usar uma ferramenta gráfica como o Gpg4win. Você pode usar uma ferramenta diferente, mas nossos exemplos são baseados no Gpg4win e utilizam sua GUI integrada `Kleopatra`.

Baixe e instale o Gpg4win, e, em seguida, carregue o Kleopatra. O diálogo deve parecer semelhante a:
...
**Figura 2.2 Kleopatra: Assistente de Busca de Certificados no Servidor: Encontrando um Certificado**
...
Selecione o certificado "Engenharia de Lançamento do MySQL". O Fingerprint e o Key-ID devem ser "3A79BD29" para o MySQL 5.7.37 e superior ou "5072E1F5" para o MySQL 5.7.36 e anterior, ou escolha Detalhes... para confirmar que o certificado é válido. Agora, importe-o clicando em Importar. Uma caixa de diálogo de importação é exibida; escolha Okay, e este certificado deve estar listado agora na guia Certificados Importado.

Em seguida, configure o nível de confiança para o nosso certificado. Selecione o nosso certificado e, a partir do menu principal, selecione Certificados, Alterar proprietário de confiança.... Sugerimos que você escolha "Verificações muito precisas" para o nosso certificado, pois, caso contrário, você pode não ser capaz de verificar nossa assinatura. Selecione "Verificações muito precisas" para habilitar a "confiança total" e, em seguida, pressione OK.

**Figura 2.3 Kleopatra: Altere o nível de confiança para o MySQL Release Engineering**
...
Em seguida, verifique o arquivo do pacote MySQL baixado. Isso requer arquivos tanto para o arquivo embalado quanto para a assinatura. O arquivo de assinatura deve ter o mesmo nome que o arquivo embalado, mas com a extensão `.asc` anexada, conforme mostrado pelo exemplo na tabela a seguir. A assinatura está vinculada na página de downloads para cada produto MySQL. Você deve criar o arquivo `.asc` com essa assinatura.

**Tabela 2.2 Arquivos de pacote e assinaturas do MySQL para o instalador do MySQL para Microsoft Windows**
...
<tr>
<td>Distribution file</td>
<td><code>mysql-installer-community-5.7.44.msi</code></td>
</tr>
<tr>
<td>Signature file</td>
<td><code>mysql-installer-community-5.7.44.msi.asc</code></td>
</tr>
</tbody>
</table>
...
**Figura 2.4 Kleopatra: O diálogo para descriptografar e verificar arquivos**
...
Clique em Decryptor/Verificar para verificar o arquivo. Os dois resultados mais comuns são os seguintes, e, embora o aviso amarelo pareça problemático, o seguinte significa que a verificação do arquivo foi realizada com sucesso. Agora, você pode executar este instalador.
...
Ver um erro vermelho "A assinatura é inválida" significa que o arquivo é inválido. Não execute o arquivo MSI se você ver esse erro.
...
A seção 2.1.4.2, “Verificação de assinatura usando GnuPG”, explica por que você provavelmente não vê um resultado verde `Good signature`.

#### 2.1.4.4 Verificação de assinatura usando RPM

Para pacotes RPM, não há uma assinatura separada. Os pacotes RPM possuem uma assinatura GPG integrada e um checksum MD5. Você pode verificar um pacote executando o seguinte comando:
...
Se você está usando o RPM 4.1 e ele reclama sobre `(GPG) NOT OK (MISSING KEYS: GPG#3a79bd29)`, mesmo que você tenha importado a chave de construção pública do MySQL em seu próprio conjunto de chaves GPG, você precisa importar a chave no conjunto de chaves RPM primeiro. O RPM 4.1 não usa mais seu conjunto de chaves GPG pessoal (ou o GPG em si). Em vez disso, o RPM mantém um conjunto de chaves separado porque é uma aplicação de âmbito sistêmico e o conjunto de chaves públicas do GPG de um usuário é um arquivo específico do usuário. Para importar a chave pública do MySQL no conjunto de chaves RPM, obtenha primeiro a chave, em seguida, use **rpm --import** para importar a chave. Por exemplo:
...
#### 2.1.4.5 Chave de Compilação Pública GPG para Pacotes Arquivados

A seguinte chave de compilação pública do GPG (keyID `5072E1F5`) pode ser usada para verificar a autenticidade e a integridade dos pacotes do MySQL 5.7.36 e anteriores. Para instruções de verificação de assinatura, consulte a Seção 2.1.4.2, “Verificação de assinatura usando GnuPG”.

##### Chave de Compilação Pública GPG para Pacotes do MySQL 5.7.36 e Anterior
...
### 2.1.5 Planos de instalação

O layout de instalação difere para diferentes tipos de instalação (por exemplo, pacotes nativos, tarballs binários e tarballs de fonte), o que pode levar à confusão ao gerenciar diferentes sistemas ou usar diferentes fontes de instalação. Os layouts individuais são fornecidos no capítulo correspondente ao tipo de instalação ou plataforma, conforme descrito a seguir. Note que o layout das instalações de fornecedores que não são a Oracle pode diferir desses layouts.

* Seção 2.3.1, “Configuração da instalação do MySQL no Microsoft Windows” * Seção 2.8.3, “Configuração do MySQL para instalação de fonte” * Tabela 2.3, “Configuração da instalação do MySQL para pacotes binários genéricos Unix/Linux” * Tabela 2.12, “Configuração da instalação do MySQL para pacotes RPM do Linux da Zona de Desenvolvimento do MySQL”

* Tabela 2.7, “Layout de instalação do MySQL no macOS”

### 2.1.6 Características de compilação específicas do compilador

Em alguns casos, o compilador usado para construir o MySQL afeta as funcionalidades disponíveis para uso. As notas desta seção se aplicam a distribuições binárias fornecidas pela Oracle Corporation ou que você compilar sozinho a partir de fonte.

****icc** (Intel C++ Compiler) Builds**

Um servidor construído com **icc** tem essas características:
