...
Modificações na maioria das linhas da tabela `setup_instruments` afetam o monitoramento imediatamente. Para alguns instrumentos, as modificações são efetivas apenas na inicialização do server; alterá-los em runtime não tem efeito. Isso afeta principalmente mutexes, conditions e rwlocks no server, embora possa haver outros instrumentos para os quais isso seja verdade.

A tabela `setup_instruments` fornece a forma mais básica de controle sobre a produção de eventos. Para refinar ainda mais a produção de eventos com base no tipo de objeto ou Thread monitorado, outras tabelas podem ser usadas conforme descrito na Seção 25.4.3, “Event Pre-Filtering”.

Os exemplos a seguir demonstram operações possíveis na tabela `setup_instruments`. Essas alterações, assim como outras operações de pré-filtragem, afetam todos os usuários. Algumas dessas Querys usam o operador `LIKE` e um padrão para corresponder aos nomes dos instrumentos. Para informações adicionais sobre a especificação de padrões para selecionar instrumentos, consulte a Seção 25.4.9, “Naming Instruments or Consumers for Filtering Operations”.
