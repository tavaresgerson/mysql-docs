...
Para um evento de `I/O` de tabela, geralmente há duas linhas em `events_waits_current`, não apenas uma. Por exemplo, uma busca por linha (`row fetch`) pode resultar em linhas como estas:
...
Isso ocorre porque, diferentemente de outros eventos de espera “atômicos” (como para `mutexes` ou `I/O` de arquivo), os eventos de `I/O` de tabela são “moleculares” e incluem (se sobrepõem a) outros eventos. Em `events_waits_current`, o evento de `I/O` de tabela geralmente tem duas linhas:
