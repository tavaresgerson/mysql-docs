...
**mysqldump** e `BACKUP DATABASE` ignoram tabelas no *Database* `performance_schema`.
...
Os tipos de *timers* podem variar por plataforma. A tabela `performance_timers` mostra quais *event timers* estão disponíveis. Se os valores nesta tabela para um determinado nome de *timer* forem `NULL`, esse *timer* não é suportado na sua plataforma.
