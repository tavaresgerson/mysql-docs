...
Para uma réplica *multithreaded*, a réplica usa múltiplos *worker threads* e um *coordinator thread* para gerenciá-los, e esta tabela mostra o status do *coordinator thread*. Para uma réplica *single-threaded*, esta tabela está vazia. Para uma réplica *multithreaded*, a tabela `replication_applier_status_by_worker` mostra o status dos *worker threads*.

A tabela `replication_applier_status_by_coordinator` possui as seguintes colunas:
...
* `LAST_ERROR_NUMBER`, `LAST_ERROR_MESSAGE`
  ...
  A emissão de `RESET MASTER` ou `RESET SLAVE` redefine (resets) os valores exibidos nestas colunas.
