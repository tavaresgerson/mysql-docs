...
* `MEMBER_HOST`
  ...
  Endereço de rede deste membro (host name ou endereço IP). Obtido da variável `hostname` do membro.

* `MEMBER_PORT`

  Port na qual o Server está ouvindo. Obtido da variável `port` do membro.
...
O comando `TRUNCATE TABLE` não é permitido para a tabela `replication_group_members`.