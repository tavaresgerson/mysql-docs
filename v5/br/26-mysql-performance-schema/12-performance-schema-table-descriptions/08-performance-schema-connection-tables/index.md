...
* `accounts`: Estatísticas de Connection por account do cliente

* `hosts`: Estatísticas de Connection por nome de host do cliente

* `users`: Estatísticas de Connection por nome de user do cliente
...
Cada tabela de Connection possui as colunas `CURRENT_CONNECTIONS` e `TOTAL_CONNECTIONS` para rastrear o número atual e total de Connections por "valor de rastreamento" (tracking value) no qual suas estatísticas são baseadas. As tabelas diferem no que usam para o valor de rastreamento. A tabela `accounts` possui as colunas `USER` e `HOST` para rastrear Connections por combinação de user e host. As tabelas `users` e `hosts` possuem uma coluna `USER` e `HOST`, respectivamente, para rastrear Connections por nome de user e nome de host.
...
* A tabela `accounts` tem quatro linhas, para os valores de account `user1`/`hosta`, `user1`/`hostb`, `user2`/`hosta` e `user2`/`hostb`, com cada linha contando uma Connection por account.

* A tabela `hosts` tem duas linhas, para `hosta` e `hostb`, com cada linha contando duas Connections por nome de host.

* A tabela `users` tem duas linhas, para `user1` e `user2`, com cada linha contando duas Connections por nome de user.
...
`TRUNCATE TABLE` é permitido para tabelas de Connection. Ele tem os seguintes efeitos:
...
Truncar uma summary table `_summary_global` também trunca implicitamente suas summary tables de Connection e Thread correspondentes. Por exemplo, truncar `events_waits_summary_global_by_event_name` trunca implicitamente as summary tables de eventos Wait que são agregadas por account, host, user ou Thread.