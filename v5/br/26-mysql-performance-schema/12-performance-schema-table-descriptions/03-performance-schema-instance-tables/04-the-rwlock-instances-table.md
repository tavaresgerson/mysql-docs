...
A tabela `rwlock_instances` possui estas colunas:
...
* `events_waits_current`, para ver por qual `rwlock` um thread está esperando

* `rwlock_instances`, para ver qual outro thread atualmente possui um `rwlock`

Há uma limitação: A tabela `rwlock_instances` pode ser usada apenas para identificar o thread que detém um write lock, mas não os threads que detêm um read lock.