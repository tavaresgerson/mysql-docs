...
A tabela `performance_timers` mostra quais *event timers* estão disponíveis:
...
Se os valores associados a um determinado nome de *timer* forem `NULL`, esse *timer* não é suportado em sua plataforma. As linhas que não contêm `NULL` indicam quais *timers* você pode usar em `setup_timers`. Para uma explicação sobre como o *event timing* ocorre, veja Seção 25.4.1, “Performance Schema Event Timing”.

Nota

A partir do MySQL 5.7.21, a tabela `setup_timers` do Performance Schema está depreciada e foi removida no MySQL 8.0, assim como a linha `TICKS` na tabela `performance_timers`.

A tabela `performance_timers` possui estas colunas:

* `TIMER_NAME`

  O nome pelo qual o *timer* deve ser referenciado ao configurar a tabela `setup_timers`.
...
O `TRUNCATE TABLE` não é permitido para a tabela `performance_timers`.