...
A lista de processos do MySQL indica as operações atualmente em execução pelo conjunto de Threads que estão rodando no servidor. A tabela `processlist` é uma das fontes de informação sobre processos. Para uma comparação desta tabela com outras fontes, consulte Sources of Process Information.
...
* `USER`
  ...
  Um valor `USER` de `system user` é distinto do privilégio `SYSTEM_USER`. O primeiro designa Threads internos. O último distingue as categorias de conta de usuário de sistema e usuário regular (consulte Account Categories).
