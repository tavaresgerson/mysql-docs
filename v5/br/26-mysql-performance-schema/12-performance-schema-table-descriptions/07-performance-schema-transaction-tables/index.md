...
* `events_transactions_current`: O transaction event atual para cada Thread.

* `events_transactions_history`: Os transaction events mais recentes que terminaram por Thread.

* `events_transactions_history_long`: Os transaction events mais recentes que terminaram globalmente (em todas as Threads).

As seções a seguir descrevem as tabelas de transaction event. Existem também tabelas de resumo que agregam informações sobre transaction events; veja Section 25.12.15.4, “Transaction Summary Tables”.

Para mais informações sobre o relacionamento entre as três tabelas de transaction event, veja Section 25.9, “Performance Schema Tables for Current and Historical Events”.
...
* A tabela `setup_instruments` contém um Instrument chamado `transaction`. Use este Instrument para habilitar ou desabilitar a coleta de classes individuais de transaction event.

* A tabela `setup_consumers` contém valores de Consumer com nomes que correspondem aos nomes das tabelas de transaction event atuais e históricas. Use estes Consumers para filtrar a coleta de transaction events.
...
Para controlar a coleta de transaction events em tempo de execução (runtime), atualize as tabelas `setup_instruments` e `setup_consumers`:
...
A tabela `setup_timers` contém uma linha com um valor `NAME` de `transaction` que indica a unidade para o timing (cronometragem) de transaction event. A unidade padrão é `NANOSECOND`:
...
Para informações adicionais sobre a configuração da coleta de Eventos, veja Section 25.3, “Performance Schema Startup Configuration” e Section 25.4, “Performance Schema Runtime Configuration”.
...
Transactions também começam implicitamente. Por exemplo, quando a variável de sistema `autocommit` está habilitada, o início de cada Statement inicia uma nova Transaction.

Quando `autocommit` está desabilitada, a primeira Statement após uma Transaction committed marca o início de uma nova Transaction. As Statements subsequentes fazem parte da Transaction até que ela seja committed.
...
* Para uma Transaction iniciada explicitamente, o transaction event começa durante o processamento da Statement `START TRANSACTION`.
...
* Para qualquer Transaction, seja ela terminada explícita ou implicitamente, o transaction event termina quando o servidor faz a transição para fora do estado de Transaction ativo durante o processamento de `COMMIT` ou `ROLLBACK`.
...
* Nível de Isolation (`SERIALIZABLE`, `REPEATABLE READ` e assim por diante)

* Implícita (`autocommit` habilitada) ou explícita (`autocommit` desabilitada)
...
O Parent (Pai) de um transaction event é o Event que iniciou a Transaction. Para uma Transaction iniciada explicitamente, isso inclui as Statements `START TRANSACTION` e `COMMIT AND CHAIN`. Para uma Transaction iniciada implicitamente, é a primeira Statement que usa um engine transacional após o término da Transaction anterior.

Em geral, uma Transaction é o Parent de nível superior para todos os Events iniciados durante a Transaction, incluindo Statements que encerram explicitamente a Transaction, como `COMMIT` e `ROLLBACK`. As exceções são Statements que encerram uma Transaction implicitamente, como Statements DDL, caso em que a Transaction atual deve ser committed antes que a nova Statement seja executada.
...
Statements de Savepoint são registradas como statement events separados. Transaction events incluem contadores separados para Statements `SAVEPOINT`, `ROLLBACK TO SAVEPOINT` e `RELEASE SAVEPOINT` emitidas durante a Transaction.
