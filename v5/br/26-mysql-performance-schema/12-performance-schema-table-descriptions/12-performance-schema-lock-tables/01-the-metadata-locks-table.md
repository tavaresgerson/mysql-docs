...
O MySQL usa *metadata locking* para gerenciar o acesso concorrente a objetos de *Database* e garantir a consistência dos dados; consulte Seção 8.11.4, “Metadata Locking”. O *metadata locking* aplica-se não apenas a tabelas, mas também a *schemas*, programas armazenados (*procedures*, *functions*, *triggers*, eventos agendados), *tablespaces*, *user locks* adquiridos com a função `GET_LOCK()` (consulte Seção 12.14, “Funções de Locking”), e *locks* adquiridos com o serviço de *locking* descrito em Seção 5.5.6.1, “O Serviço de Locking”.
