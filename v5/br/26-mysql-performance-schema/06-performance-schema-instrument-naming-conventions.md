...
O Instrument `idle` é usado para `idle events`, que o Performance Schema gera conforme discutido na descrição da coluna `socket_instances.STATE` na Seção 25.12.3.5, “A Tabela socket_instances”.

### Elementos do Instrumento Memory

A maior parte da instrumentação de memória está desativada por padrão e pode ser ativada ou desativada na inicialização, ou dinamicamente em tempo de execução, atualizando a coluna `ENABLED` dos Instruments relevantes na tabela `setup_instruments`. Os Instruments de memória têm nomes no formato `memory/code_area/instrument_name`, onde *`code_area`* é um valor como `sql` ou `myisam`, e *`instrument_name`* é o detalhe do Instrument.

Instruments nomeados com o prefixo `memory/performance_schema/` expõem quanta memória é alocada para buffers internos no Performance Schema. Os Instruments `memory/performance_schema/` são integrados (`built-in`), estão sempre ativados e não podem ser desativados na inicialização ou em tempo de execução. Instruments de memória integrados são exibidos apenas na tabela `memory_summary_global_by_event_name`. Para mais informações, consulte a Seção 25.17, “O Modelo de Alocação de Memória do Performance Schema”.

### Elementos do Instrumento Stage

Os Stage Instruments têm nomes no formato `stage/code_area/stage_name`, onde *`code_area`* é um valor como `sql` ou `myisam`, e *`stage_name`* indica o estágio do processamento de Statement, como `Sorting result` ou `Sending data`. Stages correspondem aos estados de Thread exibidos por `SHOW PROCESSLIST` ou que são visíveis na tabela `PROCESSLIST`.

### Elementos do Instrumento Statement

* `statement/abstract/*`: Um Instrument abstrato para operações de Statement. Instruments abstratos são usados durante os estágios iniciais da classificação de Statement, antes que o tipo exato de Statement seja conhecido, sendo então alterados para um Statement Instrument mais específico quando o tipo é determinado. Para uma descrição deste processo, consulte a Seção 25.12.6, “Tabelas de Statement Event do Performance Schema”.
...
* `statement/sql`: Uma operação de SQL Statement instrumentada. Por exemplo, os Instruments `statement/sql/create_db` e `statement/sql/select` são usados para Statements `CREATE DATABASE` e `SELECT`.
...
* `wait/io`
  ...
  + `wait/io/table`
    ...
    Diferente da maioria dos `waits`, um `wait` de I/O de tabela pode incluir outros `waits`. Por exemplo, o I/O de tabela pode incluir I/O de arquivo ou operações de memória. Assim, `events_waits_current` para um `wait` de I/O de tabela geralmente possui duas linhas. Para mais informações, consulte a Seção 25.8, “Eventos Atom e Molecule do Performance Schema”.
...
* `wait/synch`
  ...
  + `wait/synch/rwlock`
    ...
    Um objeto read/write lock (rwlock) usado para bloquear uma variável específica para acesso, impedindo seu uso por outros Threads. Um `shared read lock` pode ser adquirido simultaneamente por múltiplos Threads. Um `exclusive write lock` pode ser adquirido por apenas um Thread por vez.

  + `wait/synch/sxlock`

    Um `shared-exclusive (SX) lock` é um tipo de objeto rwlock que fornece acesso de escrita a um recurso comum, ao mesmo tempo que permite leituras inconsistentes por outros Threads. Os `sxlocks` otimizam a concorrência e melhoram a escalabilidade para cargas de trabalho de leitura e escrita.
