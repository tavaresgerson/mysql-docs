...
Na Seção 25.4.3, “Event Pre-Filtering”, um exemplo mostrou como aplicar o Pre-filtering para *file instruments*. Se as *event tables* contiverem informações tanto de arquivo quanto não relacionadas a arquivo, o Post-filtering é outra maneira de visualizar informações apenas para *file events*. Adicione uma cláusula `WHERE` às Queries para restringir a seleção de eventos de forma apropriada:
