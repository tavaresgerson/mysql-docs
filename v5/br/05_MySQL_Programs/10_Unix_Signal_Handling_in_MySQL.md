...
* `SIGHUP` faz com que o servidor recarregue as tabelas de concessão e limpe as tabelas, os registros, o cache de threads e o cache do host. Essas ações são como várias formas da declaração `FLUSH`. Enviar o sinal permite que as operações de limpeza sejam realizadas sem precisar se conectar ao servidor, o que requer uma conta do MySQL que tenha privilégios suficientes para essas operações. O servidor também escreve um relatório de status no log de erro que tem este formato:
...
* `SIGINT` normalmente é ignorado pelo servidor. Iniciar o servidor com a opção `--gdb` instala um manipulador de interrupção para `SIGINT` para fins de depuração. Veja a Seção 5.8.1.4, “Depuração do mysqld sob o gdb”.
