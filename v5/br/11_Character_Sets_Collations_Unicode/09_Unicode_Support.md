...
Os caracteres fora do BMP são comparados como CARACTERE DE REPLICA e convertidos em `'?'` quando convertidos para um conjunto de caracteres Unicode que suporta apenas caracteres do BMP (`utf8mb3` ou `ucs2`).
...
Os aplicativos do cliente que se comunicam com o servidor usando Unicode devem definir o conjunto de caracteres do cliente de acordo (por exemplo, emitindo uma declaração `SET NAMES 'utf8mb4'`). Alguns conjuntos de caracteres não podem ser usados como conjunto de caracteres do cliente. Tentar usá-los com [`SET NAMES`](set-names.html "13.7.4.3 SET NAMES Statement") ou [`SET CHARACTER SET`](set-character-set.html "13.7.4.2 SET CHARACTER SET Statement") produz um erro. Veja Conjuntos de caracteres do cliente impermissíveis.
...
`utf8mb4` contrasta com o conjunto de caracteres `utf8mb3`, que suporta apenas caracteres BMP e utiliza no máximo três bytes por caractere:
...
* Para um caractere suplementar, `utf8mb4` requer quatro bytes para armazená-lo, enquanto `utf8mb3` não pode armazenar o caractere de forma alguma. Ao converter colunas de `utf8mb3` para `utf8mb4`, você não precisa se preocupar em converter caracteres suplementares, pois não há nenhum.
...
### 10.9.2 O conjunto de caracteres utf8mb3 (codificação Unicode UTF-8 de 3 bytes)
...
Aplicações que utilizam dados UTF-8, mas que exigem suporte para caracteres adicionais, devem usar `utf8mb4` em vez de `utf8mb3` (consulte a Seção 10.9.1, “O conjunto de caracteres utf8mb4 (codificação Unicode de UTF-8 de 4 bytes”)”).
...
O MySQL imediatamente converte instâncias de `utf8mb3` em declarações para `utf8`, portanto, em declarações como `SHOW CREATE TABLE` ou `SELECT CHARACTER_SET_NAME FROM INFORMATION_SCHEMA.COLUMNS` ou `SELECT COLLATION_NAME FROM INFORMATION_SCHEMA.COLUMNS`, os usuários veem o nome `utf8` ou `utf8_collation_substring`.

`utf8mb3` também é válido em contextos que não são cláusulas `CHARACTER SET`. Por exemplo:
...
`utf8` é um alias para o conjunto de caracteres `utf8mb3`. Para mais informações, consulte a Seção 10.9.2, “O conjunto de caracteres utf8mb3 (codificação Unicode UTF-8 de 3 bytes)”).
...
Essa discussão se refere aos nomes dos conjuntos de caracteres `utf8mb3` e `utf8mb4` para ser explícito ao se referir a dados de conjuntos de caracteres UTF-8 de 3 bytes e 4 bytes. A exceção é que, nas definições de tabela, `utf8` é usado porque o MySQL converte instâncias de `utf8mb3` especificadas nessas definições para `utf8`, que é um alias para `utf8mb3`.
...
* Para os tipos de dados de caracteres de comprimento variável (os tipos `VARCHAR` e `TEXT`), o comprimento máximo permitido em caracteres é menor para as colunas `utf8mb4` do que para as colunas `utf8mb3`.
...
As tabelas podem ser convertidas de `utf8mb3` para `utf8mb4` usando [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement"). Suponha que uma tabela tenha esta definição:
...
O problema ao converter de `utf8mb3` para `utf8mb4` é que o comprimento máximo de uma coluna ou chave de índice não muda em termos de *bytes*. Portanto, é menor em termos de *caracteres* porque o comprimento máximo de um caractere é de quatro bytes em vez de três. Para os tipos de dados `CHAR`, `VARCHAR` e `TEXT`, fique atento a esses problemas ao converter suas tabelas do MySQL:

* Verifique todas as definições das colunas de `utf8mb3` e certifique-se de que elas não excedam o comprimento máximo para o motor de armazenamento.
...
Se as condições anteriores se aplicarem, você deve reduzir o comprimento definido das colunas ou índices, ou continuar a usar `utf8mb3` em vez de `utf8mb4`.
...
* Para definições de objeto que se referem ao conjunto de caracteres `utf8mb4`, você pode descartá-las com **mysqldump** antes de fazer o downgrade, edite o arquivo de dump para alterar as instâncias de `utf8mb4` para `utf8` e recarregue o arquivo no servidor mais antigo, desde que não haja caracteres de 4 bytes nos dados. O servidor mais antigo vê `utf8` no objeto de definição de dump e cria novos objetos que usam o conjunto de caracteres (3-byte) `utf8`.