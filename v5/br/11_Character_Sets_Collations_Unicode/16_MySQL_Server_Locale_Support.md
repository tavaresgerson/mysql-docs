## 10.16 Suporte ao Locale do MySQL Server
...
Os nomes dos locais têm subtags de idioma e região listadas pela IANA (<http://www.iana.org/assignments/language-subtag-registry>) como `'ja_JP'` ou `'pt_BR'`. O valor padrão é `'en_US'`, independentemente da configuração do idioma do sistema, mas você pode definir o valor na inicialização do servidor ou definir o valor de `GLOBAL` no runtime, se você tiver privilégios suficientes para definir variáveis de sistema globais; consulte a Seção 5.1.8.1, “Privilégios de Variáveis de Sistema”. Qualquer cliente pode examinar o valor de `lc_time_names` ou definir seu valor de `SESSION` para afetar o idioma para sua própria conexão.
...
O nome do dia ou mês para cada uma das funções afetadas é convertido de `utf8` para o conjunto de caracteres indicado pela variável do sistema `character_set_connection`.
