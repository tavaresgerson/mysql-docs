## 7.3 Exemplo de estratégia de backup e recuperação
...
Para fazer backups incrementais, precisamos salvar as alterações incrementais. No MySQL, essas alterações são representadas no log binário, então o servidor MySQL deve ser iniciado sempre com a opção `--log-bin` para habilitar esse log. Com o registro binário habilitado, o servidor escreve cada alteração de dados em um arquivo enquanto atualiza os dados. Olhando para o diretório de dados de um servidor MySQL que foi iniciado com a opção `--log-bin` e que tem andado por alguns dias, encontramos esses arquivos de log binário MySQL:
...
Excluir os logs binários do MySQL com **mysqldump --delete-master-logs** pode ser perigoso se o seu servidor for um servidor de fonte de replicação, porque os servidores replicados ainda podem não ter processado completamente o conteúdo do log binário. A descrição para a declaração `PURGE BINARY LOGS` explica o que deve ser verificado antes de excluir os logs binários do MySQL. Veja a Seção 13.4.1.1, “Declaração PURGE BINARY LOGS”.

### 7.3.2 Usando backups para recuperação
...
Para mais informações sobre o uso do **mysqlbinlog** para processar arquivos de log binário, consulte a Seção 7.5, “Recuperação Ponto no Tempo (Incremental) (Recuperação)”.
...
* Sempre execute o servidor MySQL com a opção `--log-bin`, ou até mesmo `--log-bin=log_name`, onde o nome do arquivo de registro está localizado em algum meio seguro diferente do disco em que o diretório de dados está localizado. Se você tiver esse meio seguro, essa técnica também pode ser boa para o equilíbrio de carga do disco (o que resulta em uma melhoria de desempenho).

* Faça backups completos periódicos, usando o comando **mysqldump** mostrado anteriormente na Seção 7.3.1, “Estabelecimento de uma Política de Backup”, que faz um backup online, não bloqueando.
