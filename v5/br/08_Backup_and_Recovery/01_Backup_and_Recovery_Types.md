...
+ **ndb\_restore** restaura as tabelas `NDB`.
...
* As ferramentas de backup lógicas incluem o programa **mysqldump** e a declaração `SELECT ... INTO OUTFILE`. Elas funcionam para qualquer mecanismo de armazenamento, até mesmo `MEMORY`.
...
* Os clientes podem ser afetados negativamente porque o servidor não está disponível durante o backup. Por essa razão, esses backups são frequentemente feitos a partir de um servidor replica que pode ser desativado sem prejudicar a disponibilidade.
...
A integridade dos dados pode ser comprometida se as tabelas se tornarem corrompidas. Para as tabelas do `InnoDB`, esse não é um problema típico. Para que os programas possam verificar as tabelas do `MyISAM` e repará-las se problemas forem encontrados, consulte a Seção 7.6, “Manutenção e Recuperação em caso de Falha de Tabela MyISAM”.

### Agendamento, compressão e criptografia de backup

A programação de backups é valiosa para automatizar os procedimentos de backup. A compressão dos backups reduz os requisitos de espaço, e a criptografia dos resultados fornece uma melhor segurança contra o acesso não autorizado dos dados protegidos. O próprio MySQL não oferece essas capacidades. O produto MySQL Enterprise Backup pode comprimir backups `InnoDB`, e a compressão ou criptografia dos resultados dos backups pode ser realizada usando utilitários do sistema de arquivos. Outras soluções de terceiros podem estar disponíveis.