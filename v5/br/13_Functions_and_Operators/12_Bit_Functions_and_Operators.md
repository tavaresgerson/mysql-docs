...
As funções e operadores de bits compreendem `BIT_COUNT()`, `BIT_AND()`, `BIT_OR()`, `BIT_XOR()`, `&`, `|`, `^`, `~`, `<<` e `>>`. (As funções `BIT_AND()`, `BIT_OR()` e `BIT_XOR()` são funções agregadas descritas na Seção 12.19.1, “Descrição de Função Agregada”). Atualmente, as funções e operadores de bits requerem `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") (inteiro de 64 bits) e retornam valores de `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") e, portanto, têm um alcance máximo de 64 bits. Os argumentos de outros tipos são convertidos em `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") e pode ocorrer uma truncagem.

Uma extensão para o MySQL 8.0 altera o comportamento do tipo cast-to-`BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT"): as funções e operadores de bit permitem argumentos de tipo de string binária (os tipos `BINARY`, `VARBINARY` e `BLOB`) e permitem que eles recebam argumentos e produzam valores de retorno maiores que 64 bits. Consequentemente, as operações de bit em argumentos binários no MySQL 5.7 podem produzir resultados diferentes no MySQL 8.0. Para fornecer um aviso sobre essa possível mudança de comportamento, o servidor produz avisos a partir do MySQL 5.7.11 para operações de bit para as quais os argumentos binários não são convertidos para inteiro no MySQL 8.0. Esses avisos oferecem uma oportunidade de reescrever as declarações afetadas. Para produzir o comportamento do MySQL 5.7 explicitamente de uma maneira que não mude após uma atualização para 8.0, cast-ar argumentos binários de operações de bit para convertê-los para inteiro.
...
Para evitar que uma declaração afetada produza um resultado diferente após uma atualização para o MySQL 8.0, reescreva-a para que não gere avisos de operação de bits. Para isso, realize pelo menos um cast binário para `BIGINT` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") com `CAST(... AS UNSIGNED)`. Isso torna o cast binário implícito do MySQL 5.7 explícito:
