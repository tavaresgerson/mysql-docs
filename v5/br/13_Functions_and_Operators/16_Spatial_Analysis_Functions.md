...
Se o argumento for um ponto ou um segmento de string vertical ou horizontal, `ST_Envelope()` retorna o ponto ou o segmento de string como seu MBR, em vez de retornar um polígono inválido:
...
Caso contrário, se `(pt1, pt2)` seja um segmento de string vertical ou horizontal, o resultado é o segmento de string `(pt1, pt2)`.
...
O argumento *`max_distance`* é a distância (em unidades das coordenadas de entrada) de um vértice para outros segmentos a serem removidos. Os vértices dentro desta distância das strings simplificadas são removidos. Se o argumento *`max_distance`* não for positivo ou for `NaN`, ocorre um erro `ER_WRONG_ARGUMENTS`.
