...
A precedência de `=` depende de ser usado como operador de comparação (`=`) ou como operador de atribuição (`=`). Quando usado como operador de comparação, tem a mesma precedência que `<=>`, `>=`, `>`, `<=`, `<`, `<>`, `!=`, `IS`, `LIKE`, `REGEXP` e `IN()`. Quando usado como operador de atribuição, tem a mesma precedência que `:=`. A Seção 13.7.4.1, “Sintaxe de definição de variáveis”, e a Seção 9.4, “Variáveis definidas pelo usuário”, explicam como o MySQL determina qual interpretação de `=` deve ser aplicada.
...
Para converter um valor em um tipo específico para fins de comparação, você pode usar a função `CAST()`. Os valores de string podem ser convertidos para um conjunto de caracteres diferente usando `CONVERT()`. Veja a Seção 12.10, “Funções e Operadores de Conversão”.

Por padrão, as comparações de strings não são sensíveis ao caso e utilizam o conjunto de caracteres atual. O padrão é `latin1` (cp1252 da Europa Ocidental), que também funciona bem para o inglês.
...
igual seguro para `NULL`. Este operador realiza uma comparação de igualdade como o operador `=`, mas retorna `1` em vez de `NULL` se ambos os operandos forem `NULL`, e `0` em vez de `NULL` se um dos operandos for `NULL`.
...
A sintaxe do `IN()` também pode ser usada para escrever certos tipos de subconsultas. Veja a Seção 13.2.10.3, “Subconsultas com ANY, IN ou SOME”.
...
Retorna `0` se *`N`* ≤ *`N1`*, `1` se *`N`* ≤ *`N2`* e assim por diante, ou `-1` se *`N`* é `NULL`. Todos os argumentos são tratados como inteiros. É necessário que *`N1`* ≤ *`N2`* ≤ *`N3`* ≤ `...` ≤ *`Nn`* para que essa função funcione corretamente. Isso ocorre porque uma busca binária é usada (muito rápida).
...
O MySQL avalia qualquer valor não nulo, não `NULL`, para `TRUE`. Por exemplo, as seguintes declarações todas são avaliadas para `TRUE`:
...
NOT lógico. Avalia `1` se o operando for `0`, a `0` se o operando for não nulo e retorna `NULL` se `NOT NULL`.
...
E lógico. Avalia `1` se todos os operandos forem não nulos e não `NULL`, para `0` se um ou mais operandos forem `0`, caso contrário, `NULL` é retornado.
...
XOR lógico. Retorna `NULL` se qualquer dos operandos for `NULL`. Para operandos que não são `NULL`, avalia-se como `1` se houver um número ímpar de operandos não nulos, caso contrário, `0` é retornado.
...
Embora seja também possível definir e ler o valor da mesma variável em uma única declaração SQL usando o operador `:=`, isso não é recomendado. A Seção 9.4, “Variáveis Definidas pelo Usuário”, explica por que você deve evitar fazer isso.
...
Para mais informações, consulte a Seção 13.7.4.1, “Sintaxe SET para Atribuição de Variáveis”, a Seção 13.2.11, “Instrução UPDATE”, e a Seção 13.2.10, “Subconsultas”.