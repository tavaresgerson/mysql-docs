...
Esta seção descreve as funções que podem ser usadas para manipular valores temporais. Consulte a Seção 11.2, “Tipos de dados de data e hora”, para uma descrição da faixa de valores que cada tipo de data e hora tem e dos formatos válidos nos quais os valores podem ser especificados.
...
Aqui está um exemplo que usa funções de data. A consulta a seguir seleciona todas as strings com um valor *`date_col`* nos últimos 30 dias:
...
A consulta também seleciona strings com datas que estão no futuro.
...
* `ADDDATE(date,INTERVAL expr unit)`(date-and-time-functions.html#function_adddate), `ADDDATE(expr,days)`
...
Se o valor sair do intervalo suportado do tipo `TIMESTAMP` quando convertido de *`from_tz`* para UTC, não ocorre nenhuma conversão. A faixa `TIMESTAMP` é descrita na Seção 11.2.1, “Sintaxe do Tipo de Dados de Data e Hora”.
...
Extrai a parte de data da expressão de data ou datetime *`expr`*.
...
Para mais informações sobre a sintaxe de intervalo temporal, incluindo uma lista completa dos especificadores *`unit`*, a forma esperada do argumento *`expr`* para cada valor *`unit`* e as regras para a interpretação do operador em aritmética temporal, consulte Intervalos Temporais.
...
+ `DATETIME` se o primeiro argumento for um valor de `DATETIME` (ou `TIMESTAMP`) ou se o primeiro argumento for um valor de `DATE` e o *`unit`* usa `HOURS`, `MINUTES` ou `SECONDS`.
...
Formata o valor *`date`* de acordo com a string *`format`*.
...
A linguagem usada para os nomes de dia e mês e abreviações é controlada pelo valor da variável do sistema `lc_time_names` (Seção 10.16, “Suporte de localização do servidor MySQL”).
...
`DATE_FORMAT()` retorna uma string com um conjunto de caracteres e uma ordenação determinados por `character_set_connection` e `collation_connection`, para que possa retornar nomes de mês e dia da semana que contenham caracteres não ASCII.
...
Retorna o dia do mês para *`date`*, na faixa de `1` a `31`, ou `0` para datas como `'0000-00-00'` ou `'2008-00-00'` que têm uma parte do dia em zero.
...
Retorna o índice do dia da semana para *`date`* (`1` = domingo, `2` = segunda-feira, etc., `7` = sábado). Esses valores de índice correspondem ao padrão ODBC.
...
Retorna o dia do ano para *`date`*, na faixa de `1` a `366`.
...
A função `EXTRACT()` utiliza os mesmos tipos de especificadores *`unit`* que `DATE_ADD()` ou `DATE_SUB()`, mas extrai partes da data em vez de realizar cálculos de data. Para informações sobre o argumento *`unit`*, consulte Intervalos Temporais.
...
Dado um número de dia *`N`*, retorna um valor de `DATE`.
...
Use `FROM_DAYS()` com cautela em datas antigas. Não é destinado para uso com valores que precedem o advento do calendário gregoriano (1582). Veja a Seção 11.2.8, “Que calendário é usado pelo MySQL?”.

* `FROM_UNIXTIME(unix_timestamp[,format])`

Retorna uma representação de *`unix_timestamp`* como um valor de data e hora ou uma cadeia de caracteres. O valor retornado é expresso usando o fuso horário da sessão. (Os clientes podem definir o fuso horário da sessão conforme descrito na Seção 5.1.13, “Suporte ao Fuso Horário do Servidor MySQL”.) *`unix_timestamp`* é um valor de marca-passo interno que representa segundos desde `'1970-01-01 00:00:00'` UTC, como produzido pela função `UNIX_TIMESTAMP()`.

Se *`format`* for omitido, essa função retorna um valor de `DATETIME`.

Se *`unix_timestamp`* for um inteiro, a precisão de segundos fracionários do `DATETIME` é zero. Quando *`unix_timestamp`* é um valor decimal, a precisão de segundos fracionários do `DATETIME` é a mesma que a precisão do valor decimal, até um máximo de 6. Quando *`unix_timestamp`* é um número em ponto flutuante, a precisão de segundos fracionários do datetime é 6.
...
Retorna a hora para *`time`*. A faixa do valor de retorno é `0` a `23` para valores de hora do dia. No entanto, a faixa dos valores de `TIME` é realmente muito maior, então `HOUR` pode retornar valores maiores que `23`.
...
Retorna uma data, dados o ano e o dia do ano. *`dayofyear`* deve ser maior que 0 ou o resultado é `NULL`.
...
Retorna um valor de tempo calculado a partir dos argumentos *`hour`*, *`minute`* e *`second`*.
...
Retorna os microsegundos do tempo ou expressão datetime *`expr`* como um número no intervalo de `0` a `999999`.
...
Retorna o minuto para *`time`*, na faixa de `0` a `59`.
...
Retorna o mês para *`date`*, na faixa de `1` a `12` para janeiro a dezembro, ou `0` para datas como `'0000-00-00'` ou `'2008-00-00'` que têm uma parte de mês zero.
...
Retorna o trimestre do ano para *`date`*, na faixa de `1` a `4`.
...
A verificação de intervalo das partes dos valores de data é conforme descrito na Seção 11.2.2, “Os tipos DATE, DATETIME e TIMESTAMP”. Isso significa, por exemplo, que datas “zero” ou datas com valores de parte de 0 são permitidas, a menos que o modo SQL seja configurado para não permitir tais valores.
...
Antes do MySQL 5.7.44, era possível passar uma string de data inválida, como `'2021-11-31'`, para essa função. No MySQL 5.7.44 e versões posteriores, `STR_TO_DATE()` realiza verificação completa da faixa e levanta um erro se a data após a conversão for inválida.
...
A segunda forma permite o uso de um valor inteiro para *`days`*. Nesses casos, é interpretado como o número de dias a serem subtraídos da data ou expressão datetime *`expr`*.
...
Como o `SYSDATE()` pode retornar diferentes valores mesmo dentro da mesma declaração e não é afetado pelo `SET TIMESTAMP`, ele é não determinístico e, portanto, inseguro para replicação se o registro binário baseado em declaração for usado. Se isso for um problema, você pode usar o registro baseado em string.

Como alternativa, você pode usar a opção `--sysdate-is-now` para fazer com que `SYSDATE()` seja um alias para `NOW()`. Isso funciona se a opção for usada tanto na fonte quanto na replica.
...
`TIMEDIFF()` retorna *`expr1`* − *`expr2`* expresso como um valor de tempo. *`expr1`* e *`expr2`* são cadeias de caracteres que são convertidas em expressões de `TIME` ou `DATETIME`; essas devem ser do mesmo tipo após a conversão.
...
Com um único argumento, essa função retorna a expressão de data ou datetime *`expr`* como um valor de datetime. Com dois argumentos, ela adiciona a expressão de hora *`expr2`* à expressão de data ou datetime *`expr1`* e retorna o resultado como um valor de datetime.
...
O valor *`unit`* pode ser especificado usando um dos termos conforme mostrado, ou com um prefixo de `SQL_TSI_`. Por exemplo, `DAY` e `SQL_TSI_DAY` são ambos legais.
...
Quando adicionar um intervalo `MONTH` a um valor `DATE` ou `DATETIME`, e a data resultante incluir um dia que não existe no mês dado, o dia é ajustado para o último dia do mês, conforme mostrado aqui:
...
Isso é usado como a função `DATE_FORMAT()`, mas a string *`format`* pode conter especificadores de formato apenas para horas, minutos, segundos e microsegundos. Outros especificadores produzem um valor `NULL` ou `0`.
...
Dado uma data *`date`*, retorna um número de dias (o número de dias desde o ano 0).
...
`TO_DAYS()` não é destinado para uso com valores que precedem o advento do calendário gregoriano (1582), porque ele não leva em conta os dias que foram perdidos quando o calendário foi alterado. Para datas anteriores a 1582 (e possivelmente um ano posterior em outros locais), os resultados desta função não são confiáveis. Consulte a Seção 11.2.8, “Que calendário é usado pelo MySQL?”, para detalhes.

Lembre-se de que o MySQL converte valores de ano de duas casas decimais em datas para formato de quatro casas decimais usando as regras da Seção 11.2, “Tipos de dados de data e hora”. Por exemplo, `'2008-10-07'` e `'08-10-07'` são vistos como datas idênticas:
...
Assim como `TO_DAYS()`, `TO_SECONDS()`, converte valores de ano de duas dígitos em datas para formato de quatro dígitos usando as regras na Seção 11.2, “Tipos de dados de data e hora”.
...
A faixa válida de valores de argumento é a mesma que para o tipo de dados `TIMESTAMP`: UTC de `'1970-01-01 00:00:01.000000'` a UTC de `'2038-01-19 03:14:07.999999'`. Se você passar uma data fora da faixa para `UNIX_TIMESTAMP()`, ele retornará `0`.
...
Se você quiser subtrair as colunas `UNIX_TIMESTAMP()`, talvez queira convertê-las em inteiros assinados. Veja a Seção 12.10, “Funções e Operadores de Conversão”.
...
Essa função retorna o número da semana para *`date`*. A forma de dois argumentos de `WEEK()` permite que você especifique se a semana começa no domingo ou no domingo e se o valor de retorno deve estar na faixa de `0` a `53` ou de `1` a `53`. Se o argumento *`mode`* for omitido, o valor da variável do sistema `default_week_format` é usado. Veja a Seção 5.1.7, “Variáveis do Sistema do Servidor”.
...
Para os valores de *`mode`* com significado de “com 4 ou mais dias este ano”, as semanas são numeradas de acordo com a ISO 8601:1988:
...
Retorna a semana do calendário da data como um número no intervalo de `1` a `53`. `WEEKOFYEAR()` é uma função de compatibilidade que é equivalente a `WEEK(date,3)`.
...
Retorna o ano para *`date`*, na faixa de `1000` a `9999`, ou `0` para a data “zero”.
...
Retorna ano e semana para uma data. O ano no resultado pode ser diferente do ano no argumento de data para a primeira e a última semana do ano.
