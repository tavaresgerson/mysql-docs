...
Você pode parar e iniciar a Replication na Replica usando as instruções `STOP SLAVE` e `START SLAVE`.

Para interromper o processamento do Binary Log da Source, use `STOP SLAVE`:
...
Para iniciar a execução novamente, use a instrução `START SLAVE`:
