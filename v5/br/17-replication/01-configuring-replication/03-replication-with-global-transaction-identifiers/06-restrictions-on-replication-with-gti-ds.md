...
Modo GTID e mysql\_upgrade. Quando o servidor estiver em execução com identificadores de transações globais (GTIDs) habilitados (`gtid_mode=ON`), não habilite o registro binário pelo **mysql\_upgrade** (a opção `--write-binlog` (mysql-upgrade.html#option\_mysql\_upgrade\_write-binlog)).
