...
Esta seção descreve como os canais podem ser usados em uma topologia de replicação e o impacto que eles têm na replicação single-source. Para instruções sobre como configurar sources e réplicas para replicação multi-source, iniciar, parar e redefinir réplicas multi-source, e monitorar a replicação multi-source, consulte Seção 16.1.5, “MySQL Multi-Source Replication”.

O número máximo de canais que podem ser criados em uma réplica em uma topologia de replicação multi-source é 256. Cada canal de replicação deve ter um nome exclusivo (não vazio), conforme explicado em Seção 16.2.2.4, “Replication Channel Naming Conventions”. Os códigos de erro e mensagens emitidas quando a replicação multi-source está habilitada especificam o canal que gerou o erro.
