...
A Replication baseada em Statement (Statement-based replication) de cláusulas `LIMIT` em comandos `DELETE`, `UPDATE` e `INSERT ... SELECT` é insegura (unsafe), pois a ordem das linhas afetadas não está definida. (Tais comandos só podem ser replicados corretamente com Statement-based replication se também contiverem uma cláusula `ORDER BY`.) Quando tal comando é encontrado:
