...
O MySQL 5.7 permite segundos fracionários para valores de `TIME`, `DATETIME` e `TIMESTAMP`, com precisão de até microssegundos (6 dígitos). Consulte Seção 11.2.7, “Segundos Fracionários em Valores de Tempo”.
...
* Para `CREATE TABLE` statements que contêm colunas com um valor *`fsp`* (precisão de segundos fracionários) maior que 0, a Replication falha devido a erros do Parser.
