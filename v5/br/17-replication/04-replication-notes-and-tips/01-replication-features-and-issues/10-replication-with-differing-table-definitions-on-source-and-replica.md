...
**Modos de conversão de tipos (variável slave\_type\_conversions).** A configuração da variável global do servidor `slave_type_conversions` controla o modo de conversão de tipos usado na replica. Essa variável aceita um conjunto de valores da seguinte tabela, que mostra os efeitos de cada modo no comportamento de conversão de tipos da replica:

<table summary="Modos de conversão de tipo para a variável global slave_type_conversions e os efeitos de cada modo no comportamento de conversão de tipo do escravo."><col style="width: 35%"/><col style="width: 65%"/><thead><tr> <th>Modo</th> <th>Efeito</th> </tr></thead><tbody><tr> <td>[[PH_HTML_CODE_<code class="literal">TINYINT</code>]</td> <td><p>Nesse modo, as conversões de tipos que significam perda de informações são permitidas.</p><p>Isso não implica que as conversões sem perda sejam permitidas, apenas que apenas os casos que exigem conversões com perda ou nenhuma conversão são permitidos; por exemplo, habilitar<span class="emphasis"><em>apenas</em></span>Esse modo permite que uma coluna [[PH_HTML_CODE_<code class="literal">TINYINT</code>] seja convertida para [[PH_HTML_CODE_<code class="literal">VARCHAR(20)</code>] (uma conversão com perda de dados), mas não uma coluna [[PH_HTML_CODE_<code class="literal">ALL_LOSSY,ALL_NON_LOSSY</code>] para uma coluna [[PH_HTML_CODE_<code class="literal">ALL_SIGNED</code>] (sem perda de dados). Tentar a conversão deste último caso causaria a parada da replicação com um erro na replica.</p></td> </tr><tr> <td>[[PH_HTML_CODE_<code class="literal">ALL_UNSIGNED</code>]</td> <td><p>Esse modo permite conversões que não exigem truncação ou outro tratamento especial do valor de origem; ou seja, permite conversões em que o tipo de destino tem uma faixa mais ampla do que o tipo de origem.</p><p>Definir este modo não afeta se as conversões com perda de dados são permitidas; isso é controlado com o modo [[PH_HTML_CODE_<code class="literal">ALL_SIGNED,ALL_UNSIGNED</code>]. Se apenas [[PH_HTML_CODE_<code class="literal">slave_type_conversions</code>] for definido, mas não [[<code class="literal">ALL_LOSSY</code>]], então tentar uma conversão que resultaria na perda de dados (como [[<code class="literal">INT</code>]] para [[<code class="literal">TINYINT</code>]], ou [[<code class="literal">INT</code><code class="literal">TINYINT</code>] para [[<code class="literal">VARCHAR(20)</code>]]) faz com que a replica pare com um erro.</p></td> </tr><tr> <td>[[<code class="literal">ALL_LOSSY,ALL_NON_LOSSY</code>]]</td> <td><p>Quando este modo estiver definido, todas as conversões de tipo suportadas serão permitidas, independentemente de serem ou não conversões com perda de dados.</p></td> </tr><tr> <td>[[<code class="literal">ALL_SIGNED</code>]]</td> <td><p>Trate os tipos inteiros promovidos como valores assinados (o comportamento padrão).</p></td> </tr><tr> <td>[[<code class="literal">ALL_UNSIGNED</code>]]</td> <td><p>Trate os tipos inteiros promovidos como valores não assinados.</p></td> </tr><tr> <td>[[<code class="literal">ALL_SIGNED,ALL_UNSIGNED</code>]]</td> <td><p>Trate os tipos inteiros promovidos como assinados, se possível, caso contrário, como não assinados.</p></td> </tr><tr> <td>[<span class="emphasis"><em>vazio</em></span>]</td> <td><p>Quando [[<code class="literal">slave_type_conversions</code>]] não estiver definido, nenhuma promoção ou redução de atributo é permitida; isso significa que todas as colunas nas tabelas de origem e destino devem ser do mesmo tipo.</p><p>Esse modo é o padrão.</p></td> </tr></tbody></table>
