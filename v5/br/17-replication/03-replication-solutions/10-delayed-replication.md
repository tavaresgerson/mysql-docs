...
Quando o fio de replicação SQL está aguardando o término do atraso antes de executar um evento, o `SHOW PROCESSLIST` exibe seu valor `State` como `Aguardando até os segundos `MASTER\_DELAY` após o evento ser executado pelo mestre`.
