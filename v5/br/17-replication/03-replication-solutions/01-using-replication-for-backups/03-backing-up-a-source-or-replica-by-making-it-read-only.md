...
É possível fazer backup de servidores Source ou Replica em uma configuração de replicação adquirindo um global read lock e manipulando a variável de sistema `read_only` para alterar o estado read-only do servidor onde o backup será realizado:
...
Em ambos os cenários, as instruções para adquirir o global read lock e manipular a variável `read_only` são executadas no servidor onde o backup será realizado e não se propagam para quaisquer Replicas desse servidor.
