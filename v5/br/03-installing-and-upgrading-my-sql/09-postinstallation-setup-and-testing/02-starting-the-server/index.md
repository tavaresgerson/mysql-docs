...
::: Nota
Para sistemas Linux nos quais o MySQL é instalado usando pacotes RPM, o início e o desligamento do servidor são gerenciados usando o systemd, e não o **mysqld_safe**, e o **mysqld_safe** não é instalado. Veja a Seção 2.5.10, “Gerenciamento do servidor MySQL com o systemd”.
:::
...
É importante que o servidor MySQL seja executado usando uma conta de login não privilegiada (não `root`). Para garantir isso, execute o **mysqld_safe** como `root` e inclua a opção `--user`, conforme mostrado. Caso contrário, você deve executar o programa enquanto estiver logado como `mysql`, nesse caso, você pode omitir a opção `--user` do comando.
...
Se você tiver outros problemas para iniciar o servidor, consulte a Seção 2.9.2.1, “Soluções de problemas para iniciar o servidor MySQL”. Para mais informações sobre o **mysqld_safe**, consulte a Seção 4.3.2, “mysqld_safe — Script de inicialização do servidor MySQL”. Para mais informações sobre o suporte ao systemd, consulte a Seção 2.5.10, “Gerenciamento do servidor MySQL com o systemd”.
