...
Esta seção descreve como fazer uma atualização para uma versão anterior das instalações binárias e baseadas em pacotes do MySQL no Unix/Linux. Métodos de atualização local e lógico são descritos.
...
6. Downgrade o binário ou pacote MySQL no local, substituindo os binários ou pacotes mais recentes pelos mais antigos.
...
8. Execute **mysql_upgrade**. Por exemplo:

   ```sh
   mysql_upgrade -u root -p
   ```

   O **mysql_upgrade** analisa todas as tabelas em todos os bancos de dados quanto à incompatibilidade com a versão atual do MySQL e tenta reparar as tabelas se problemas forem encontrados.
...
As downgrades lógicas são suportadas para downgrades entre versões dentro da mesma série de lançamento e para downgrades para o nível de versão anterior. Apenas as downgrades entre versões de Disponibilidade Geral (GA) são suportadas. Antes de prosseguir, revise a Seção 2.11.1, “Antes de Começar”.
...
7. Execute **mysql_upgrade**. Por exemplo:
