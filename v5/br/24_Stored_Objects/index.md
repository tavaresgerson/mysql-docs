...
* Trigger: Um objeto criado com `CREATE TRIGGER` que está associado a uma tabela. O trigger é ativado quando um evento específico ocorre para a tabela, como uma inserção ou atualização.
...
* Para cada tipo de objeto, existem as declarações `CREATE`, `ALTER` e `DROP` que controlam quais objetos existem e como são definidos. Veja a Seção 13.1, “Declarações de Definição de Dados”.

* A declaração `CALL` é usada para invocar procedimentos armazenados. Veja a Seção 13.2.1, “Declaração CALL”.

* As definições de programas armazenados incluem um corpo que pode usar instruções compostas, laços, condicionais e variáveis declaradas. Veja a Seção 13.6, “Instruções compostas”.
