...
A instrução `PREPARE` prepara uma instrução SQL e atribui a ela um nome, *`stmt_name`*, pelo qual a instrução será referenciada posteriormente. A instrução preparada é executada com `EXECUTE` e liberada com `DEALLOCATE PREPARE`. Para exemplos, veja Seção 13.5, “Prepared Statements”.
...
*   Quando uma *session* termina, seja de forma normal ou anormal, suas instruções preparadas deixam de existir. Se o *auto-reconnect* estiver habilitado, o cliente não é notificado de que a conexão foi perdida. Por este motivo, os clientes podem querer desabilitar o *auto-reconnect*. Veja Controle de Reconexão Automática.
