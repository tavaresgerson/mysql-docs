...
A instrução `XA RECOVER` retorna informações para aquelas transações XA no servidor MySQL que estão no estado `PREPARED`. (Veja Seção 13.3.7.2, “Estados da Transação XA”). A saída inclui uma linha para cada transação XA no servidor, independentemente de qual cliente a iniciou.

As linhas de saída de `XA RECOVER` se parecem com isto (para um valor *`xid`* de exemplo consistindo nas partes `'abc'`, `'def'` e `7`):
