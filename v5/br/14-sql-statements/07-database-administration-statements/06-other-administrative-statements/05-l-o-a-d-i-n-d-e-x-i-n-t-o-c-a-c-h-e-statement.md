...
O `LOAD INDEX INTO CACHE` pré-carrega um Index de tabela no key cache ao qual ele foi atribuído por uma declaração `CACHE INDEX` explícita, ou no default key cache, caso contrário.

O `LOAD INDEX INTO CACHE` aplica-se apenas a tabelas `MyISAM`, incluindo tabelas `MyISAM` particionadas. Além disso, Indexes em tabelas particionadas podem ser pré-carregados para uma, várias ou todas as partições.
...
A sintaxe do `LOAD INDEX INTO CACHE` permite que você especifique que apenas Indexes específicos de uma tabela devem ser pré-carregados. No entanto, a implementação pré-carrega todos os Indexes da tabela no cache, portanto, não há razão para especificar nada além do nome da tabela.
...
O `LOAD INDEX INTO CACHE ... IGNORE LEAVES` falha a menos que todos os Indexes em uma tabela tenham o mesmo block size. Para determinar os block sizes de Index para uma tabela, use **myisamchk -dv** e verifique a coluna `Blocksize`.