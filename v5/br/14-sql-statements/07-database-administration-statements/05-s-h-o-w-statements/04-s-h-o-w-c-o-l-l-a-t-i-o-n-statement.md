...
Esta instrução lista as collations suportadas pelo servidor. Por padrão, a saída de `SHOW COLLATION` inclui todas as collations disponíveis. A cláusula `LIKE`, se presente, indica quais nomes de collation devem ser correspondidos. A cláusula `WHERE` pode ser fornecida para selecionar linhas usando condições mais gerais, conforme discutido na Seção 24.8, “Extensões às Instruções SHOW”. Por exemplo:
...
A saída de `SHOW COLLATION` possui estas colunas:
...
As informações de collation também estão disponíveis na tabela `COLLATIONS` do `INFORMATION_SCHEMA`. Consulte a Seção 24.3.3, “A Tabela COLLATIONS do INFORMATION_SCHEMA”.