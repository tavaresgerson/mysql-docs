...
`SHOW ENGINE` exibe informações operacionais sobre um storage engine. Requer o privilégio `PROCESS`. A instrução possui estas variantes:
...
`SHOW ENGINE INNODB STATUS` exibe informações extensas do Monitor `InnoDB` padrão sobre o estado do storage engine `InnoDB`. Para obter informações sobre o monitor padrão e outros `InnoDB` Monitors que fornecem informações sobre o processamento `InnoDB`, consulte Seção 14.18, “Monitores InnoDB”.

`SHOW ENGINE INNODB MUTEX` exibe estatísticas de mutex e rw-lock do `InnoDB`.

Nota

Os mutexes e rw-locks do `InnoDB` também podem ser monitorados usando tabelas do Performance Schema. Consulte Seção 14.17.2, “Monitorando Esperas de Mutex do InnoDB Usando Performance Schema”.

A saída de `SHOW ENGINE INNODB MUTEX` foi removida no MySQL 5.7.2. Ela foi revisada e reintroduzida no MySQL 5.7.8.
...
A coleta de estatísticas de mutex para `SHOW ENGINE INNODB MUTEX` também pode ser habilitada configurando `innodb_monitor_enable='all'`, ou desabilitada configurando `innodb_monitor_disable='all'`.

A saída de `SHOW ENGINE INNODB MUTEX` possui estas colunas:
...
* `Status`
  ...
  Antes do MySQL 5.7.8, o campo `Status` exibe vários valores se `WITH_DEBUG` foi definido no momento da compilação do MySQL. Se `WITH_DEBUG` não foi definido, a instrução exibe apenas o valor `os_waits`. Neste último caso (sem `WITH_DEBUG`), a informação na qual a saída se baseia é insuficiente para distinguir mutexes regulares e mutexes que protegem rwlocks (que permitem múltiplos leitores ou um único escritor). Consequentemente, a saída pode parecer conter múltiplas linhas para o mesmo mutex. Os valores do campo `Status` anteriores ao MySQL 5.7.8 incluem:
...
Consulte Seção 21.7, “Replicação NDB Cluster”, para obter mais informações.
...
* `NdbTransaction`: O número e o tamanho dos objetos `NdbTransaction` que foram criados. Um `NdbTransaction` é criado toda vez que uma operação de schema de tabela (como `CREATE TABLE` ou `ALTER TABLE`) é realizada em uma tabela `NDB`.
...
* `NdbBlob`: O número e o tamanho dos objetos `NdbBlob` que foram criados. Um `NdbBlob` é criado para cada nova operação envolvendo uma coluna `BLOB` em uma tabela `NDB`.
...
`SHOW ENGINE NDB STATUS` retorna um resultado vazio se nenhuma operação envolvendo tabelas `NDB` tiver sido realizada durante a sessão atual pelo cliente MySQL que está acessando o SQL node no qual esta instrução foi executada.