...
A instrução `SHOW COLUMNS` exibe informações sobre as colunas em uma determinada tabela. Também funciona para views. `SHOW COLUMNS` exibe informações apenas para as colunas nas quais você possui algum privilégio.
...
A cláusula `LIKE`, se presente, indica quais nomes de coluna devem ser correspondidos. A cláusula `WHERE` pode ser fornecida para selecionar linhas usando condições mais gerais, conforme discutido na Seção 24.8, “Extensões para Instruções SHOW”.

Os tipos de dados podem diferir do que você espera com base em uma instrução `CREATE TABLE` porque o MySQL às vezes altera os tipos de dados quando você cria ou altera uma tabela. As condições sob as quais isso ocorre são descritas na Seção 13.1.18.6, “Alterações Silenciosas na Especificação de Coluna”.
...
Informações sobre as colunas da tabela também estão disponíveis na tabela `COLUMNS` do `INFORMATION_SCHEMA`. Consulte Seção 24.3.5, “A Tabela COLUMNS do INFORMATION_SCHEMA”.

Você pode listar as colunas de uma tabela com o comando **mysqlshow *`db_name`* *`tbl_name`***.

A instrução `DESCRIBE` fornece informações semelhantes a `SHOW COLUMNS`. Consulte Seção 13.8.1, “Instrução DESCRIBE”.
