...
Esta instrução é uma extensão do MySQL que está disponível apenas para servidores que foram compilados com suporte a *debugging*. Ela exibe uma representação da implementação interna da `stored procedure` nomeada. Uma instrução semelhante, `SHOW FUNCTION CODE`, exibe informações sobre *stored functions* (consulte Seção 13.7.5.19, “SHOW FUNCTION CODE Statement”).
...
Neste exemplo, as instruções não executáveis `BEGIN` e `END` desapareceram, e para a instrução `DECLARE variable_name`, apenas a parte executável aparece (a parte onde o valor *default* é atribuído). Para cada instrução que é retirada da fonte, há uma palavra-chave `stmt` seguida por um tipo (9 significa `DROP`, 5 significa `INSERT`, e assim por diante). A linha final contém uma instrução `jump 2`, significando `GOTO instruction #2`.
