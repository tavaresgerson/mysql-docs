...
A declaração `DROP USER` remove uma ou mais contas MySQL e seus privilégios. Ela remove as linhas de privilégio da conta de todas as *grant tables*.

Para usar `DROP USER`, você deve ter o privilégio global `CREATE USER`, ou o privilégio `DELETE` para o *system database* `mysql`. Quando a *system variable* `read_only` estiver habilitada, `DROP USER` requer adicionalmente o privilégio `SUPER`.
...
Cada nome de conta usa o formato descrito na Seção 6.2.4, “Specifying Account Names”. Por exemplo:
