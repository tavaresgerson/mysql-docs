...
O uso de múltiplas declarações exige que o cliente seja capaz de enviar strings de declaração contendo o delimitador de declaração `;`. No cliente de linha de comando **mysql**, isso é tratado com o comando `delimiter`. Alterar o delimitador de fim de declaração `;` (por exemplo, para `//`) permite que `;` seja usado no corpo de um programa. Para um exemplo, consulte Seção 23.1, “Definindo Stored Programs”.

Um bloco `BEGIN ... END` pode ser rotulado. Consulte Seção 13.6.2, “Statement Labels”.
