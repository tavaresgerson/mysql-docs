...
A declaração `IF` para stored programs implementa um construto condicional básico.

Nota

Há também uma *função* `IF()`, que difere da *declaração* `IF` descrita aqui. Consulte a Seção 12.5, “Funções de Controle de Fluxo”. A declaração `IF` pode ter cláusulas `THEN`, `ELSE` e `ELSEIF`, e é encerrada com `END IF`.
...
Neste exemplo, o `IF` interno é avaliado apenas se `n` for diferente de `m`.