...
Para alterar a coluna `a` de `INTEGER` - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") para `TINYINT NOT NULL` (mantendo o mesmo nome), e para alterar a coluna `b` de `CHAR(10)` para `CHAR(20)`, renomeando-a também de `b` para `c`:
...
Para tabelas `NDB`, também é possível alterar o tipo de Storage usado para uma tabela ou coluna. Por exemplo, considere uma tabela `NDB` criada conforme mostrado aqui:
...
Para converter esta tabela para Storage baseado em disco, você pode usar a seguinte instrução `ALTER TABLE`:
...
Para alterar o tipo de Storage de uma coluna individual, você pode usar `ALTER TABLE ... MODIFY [COLUMN]`. Por exemplo, suponha que você crie uma tabela NDB Cluster Disk Data com duas colunas, usando esta instrução `CREATE TABLE`:
...
A coluna `c1` usa Storage baseado em disco, pois este é o default para a tabela (determinado pela cláusula `STORAGE DISK` em nível de tabela na instrução `CREATE TABLE`). No entanto, a coluna `c2` usa Storage in-memory, como pode ser visto aqui na saída do SHOW `CREATE TABLE`:
...
Ao adicionar uma coluna `AUTO_INCREMENT`, os valores da coluna são preenchidos automaticamente com números de sequência. Para tabelas `MyISAM`, você pode definir o primeiro número de sequência executando `SET INSERT_ID=value` antes de `ALTER TABLE` ou usando a opção de tabela `AUTO_INCREMENT=value`.
