...
Para uma tabela `InnoDB` criada em um tablespace file-per-table ou general tablespace, os dados da tabela e os Indexes associados são armazenados em um .ibd file no diretório do Database. Quando uma tabela `InnoDB` é criada no system tablespace, os dados da tabela e os Indexes são armazenados nos ibdata\* files que representam o system tablespace. A opção `innodb_file_per_table` controla se as tabelas são criadas em tablespaces file-per-table ou no system tablespace, por padrão. A opção `TABLESPACE` pode ser usada para colocar uma tabela em um tablespace file-per-table, general tablespace, ou no system tablespace, independentemente da configuração de `innodb_file_per_table`.
...
Capítulo 15, *Alternative Storage Engines*, descreve quais arquivos cada Storage Engine cria para representar tabelas. Se um nome de tabela contiver caracteres especiais, os nomes dos arquivos da tabela contêm versões codificadas desses caracteres, conforme descrito na Seção 9.2.4, “Mapeamento de Identificadores para Nomes de Arquivos”.
...
* `int_length` está relacionado à lista de valores para colunas `ENUM` e `SET`. Neste contexto, “int” não significa “integer”. Significa “intervalo” (interval), um termo que se refere coletivamente às colunas `ENUM` e `SET`.
...
* Usar nomes de coluna longos pode reduzir o número máximo de colunas, assim como a inclusão de colunas `ENUM` ou `SET`, ou o uso de comentários de coluna.

* Uma tabela não pode ter mais de 255 definições únicas de `ENUM` e `SET`. Colunas com listas de elementos idênticas são consideradas iguais contra este limite. Por exemplo, se uma tabela contiver estas duas colunas, elas contam como uma (não duas) para este limite porque as definições são idênticas:
...
* A soma do comprimento dos nomes dos elementos nas definições únicas de `ENUM` e `SET` é contabilizada no limite de 64KB, então, embora o limite teórico no número de elementos em uma determinada coluna `ENUM` seja 65.535, o limite prático é inferior a 3.000.
