...
Esta declaração cria a definição de um `SERVER` para uso com a `Storage Engine` `FEDERATED`. A declaração `CREATE SERVER` cria uma nova linha na tabela `servers` no `Database` `mysql`. Esta declaração requer o privilégio `SUPER`.
...
A declaração `CREATE SERVER` cria uma entrada na tabela `mysql.servers` que pode ser usada posteriormente com a declaração `CREATE TABLE` ao criar uma tabela `FEDERATED`. As `options` que você especifica são usadas para preencher as colunas na tabela `mysql.servers`. As colunas da tabela são `Server_name`, `Host`, `Db`, `Username`, `Password`, `Port` e `Socket`.
...
Para mais informações, consulte Seção 15.8, “The FEDERATED Storage Engine”.

`CREATE SERVER` causa um `COMMIT` implícito. Consulte Seção 13.3.3, “Statements That Cause an Implicit Commit”.
