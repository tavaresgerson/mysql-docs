...
Diferentemente do caso em que se usa `PARTITION` com uma declaração `INSERT` ou `REPLACE`, uma declaração `UPDATE ... PARTITION` que seja válida é considerada bem-sucedida mesmo que nenhuma linha nas partitions (ou subpartitions) listadas corresponda à *`where_condition`*.

Para mais informações e exemplos, consulte Seção 22.5, “Seleção de Partition”.

*`where_condition`* é uma expression que avalia como true para cada linha a ser atualizada. Para a sintaxe de expression, consulte Seção 9.5, “Expressions”.

*`table_references`* e *`where_condition`* são especificados conforme descrito na Seção 13.2.9, “Declaração SELECT”.

Você precisa do privilégio `UPDATE` apenas para colunas referenciadas em um [`UPDATE`] que são efetivamente atualizadas. Você precisa apenas do privilégio `SELECT` para quaisquer colunas que são lidas, mas não modificadas.

A declaração `UPDATE` suporta os seguintes modificadores:

* Com o modificador `LOW_PRIORITY`, a execução do `UPDATE` é adiada até que nenhum outro client esteja lendo a tabela. Isso afeta apenas storage engines que usam somente locking de nível de tabela (como `MyISAM`, `MEMORY` e `MERGE`).
...
Declarações `UPDATE IGNORE`, incluindo aquelas que possuem uma cláusula `ORDER BY`, são sinalizadas como inseguras para replication baseada em declaração (statement-based replication). (Isso ocorre porque a ordem em que as linhas são atualizadas determina quais linhas são ignoradas.) Tais declarações produzem um warning no error log ao usar o modo statement-based e são escritas no binary log usando o formato row-based quando se usa o modo `MIXED`. (Bug #11758262, Bug #50439) Consulte Seção 16.2.1.3, “Determinação de Declarações Seguras e Inseguras no Binary Logging”, para mais informações.

Se você acessar uma coluna da tabela a ser atualizada em uma expression, `UPDATE` usa o valor current da coluna. Por exemplo, a seguinte declaração define `col1` para um a mais do que seu valor current:
...
Atribuições de `UPDATE` em tabela única são geralmente avaliadas da esquerda para a direita. Para updates de múltiplas tabelas, não há garantia de que as atribuições sejam realizadas em qualquer ordem específica.

Se você definir uma coluna para o valor que ela atualmente possui, o MySQL nota isso e não a atualiza.

Se você atualizar uma coluna que foi declarada `NOT NULL` definindo-a como `NULL`, um erro ocorre se o strict SQL mode estiver habilitado; caso contrário, a coluna é definida para o valor default implícito para o data type da coluna e a contagem de warnings é incrementada. O valor default implícito é `0` para tipos numéricos, a string vazia (`''`) para tipos string e o valor “zero” para tipos date e time. Consulte Seção 11.6, “Valores Default de Tipos de Dados”.

Se uma generated column for explicitamente atualizada, o único valor permitido é `DEFAULT`. Para informações sobre generated columns, consulte Seção 13.1.18.7, “CREATE TABLE e Generated Columns”.

`UPDATE` retorna o número de linhas que foram efetivamente alteradas. A função C API `mysql_info()` retorna o número de linhas que foram correspondidas e atualizadas e o número de warnings que ocorreram durante o `UPDATE`.

Você pode usar `LIMIT row_count` para restringir o escopo do `UPDATE`. Uma cláusula `LIMIT` é uma restrição de linhas correspondidas. A declaração para assim que encontrar *`row_count`* linhas que satisfaçam a cláusula `WHERE`, independentemente de terem sido realmente alteradas ou não.
...
Você também pode realizar operações `UPDATE` cobrindo múltiplas tabelas. No entanto, você não pode usar `ORDER BY` ou `LIMIT` com um `UPDATE` de múltiplas tabelas. A cláusula *`table_references`* lista as tabelas envolvidas no join. Sua sintaxe é descrita na Seção 13.2.9.2, “Cláusula JOIN”. Aqui está um exemplo:
...
Como o optimizer tenta por default mesclar a derived table `discounted` no query block mais externo, isso só funciona se você forçar a materialization da tabela derivada. Você pode fazer isso definindo o flag `derived_merge` da variável de sistema `optimizer_switch` como `off` antes de executar o update, ou usando o optimizer hint `NO_MERGE`, conforme mostrado aqui:
