...
Se você especificar uma cláusula `ON DUPLICATE KEY UPDATE` e uma linha a ser inserida causaria um valor duplicado em um `UNIQUE` index ou `PRIMARY KEY`, ocorre um `UPDATE` da linha antiga. Por exemplo, se a coluna `a` for declarada como `UNIQUE` e contiver o valor `1`, as duas instruções a seguir têm efeito semelhante:
...
Se a coluna `b` também for unique, o `INSERT` é equivalente à seguinte instrução `UPDATE`:
...
Com `ON DUPLICATE KEY UPDATE`, o valor de affected-rows por linha é 1 se a linha for inserida como uma nova linha, 2 se uma linha existente for atualizada e 0 se uma linha existente for definida com seus valores atuais. Se você especificar a flag `CLIENT_FOUND_ROWS` para a função C API `mysql_real_connect()` ao se conectar ao **mysqld**, o valor de affected-rows é 1 (não 0) se uma linha existente for definida com seus valores atuais.

Se uma tabela contiver uma coluna `AUTO_INCREMENT` e `INSERT ... ON DUPLICATE KEY UPDATE` inserir ou atualizar uma linha, a função `LAST_INSERT_ID()` retorna o valor `AUTO_INCREMENT`.
...
Para instruções `INSERT ... SELECT`, estas regras se aplicam em relação às formas aceitáveis de expressões de `SELECT` query às quais você pode se referir em uma cláusula `ON DUPLICATE KEY UPDATE`:
...
Referências a colunas de um `UNION` não funcionam de forma confiável. Para contornar essa restrição, reescreva o `UNION` como uma derived table para que suas linhas possam ser tratadas como um result set de tabela única. Por exemplo, esta instrução pode produzir resultados incorretos:
...
Em vez disso, use uma instrução equivalente que reescreve o `UNION` como uma derived table:
...
Como os resultados das instruções `INSERT ... SELECT` dependem da ordenação das linhas do `SELECT` e essa ordem nem sempre pode ser garantida, é possível que, ao logar instruções `INSERT ... SELECT ON DUPLICATE KEY UPDATE`, a source e a replica divergirem. Assim, as instruções `INSERT ... SELECT ON DUPLICATE KEY UPDATE` são sinalizadas como inseguras (unsafe) para replication baseada em statement (statement-based replication). Tais instruções produzem um warning no error log ao usar o modo statement-based e são escritas no binary log usando o formato row-based ao usar o modo `MIXED`. Uma instrução `INSERT ... ON DUPLICATE KEY UPDATE` contra uma tabela que tenha mais de um unique ou primary key também é marcada como insegura (unsafe). (Bug #11765650, Bug #58637)

Veja também Seção 16.2.1.1, “Vantagens e Desvantagens da Replication Baseada em Statement e Baseada em Row”.

Um `INSERT ... ON DUPLICATE KEY UPDATE` em uma partitioned table que usa um storage engine como `MyISAM` e que emprega locks de nível de tabela (table-level locks) bloqueia quaisquer partitions da tabela nas quais uma coluna de partitioning key é atualizada. (Isso não ocorre com tabelas que usam storage engines como `InnoDB` que empregam row-level locking.) Para mais informações, veja Section 22.6.4, “Partitioning and Locking”.