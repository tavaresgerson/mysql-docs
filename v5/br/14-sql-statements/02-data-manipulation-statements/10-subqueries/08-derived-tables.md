...
Uma *derived table* (tabela derivada) é uma expressão que gera uma tabela dentro do escopo de uma cláusula `FROM` de uma Query. Por exemplo, uma subquery na cláusula `FROM` de uma instrução `SELECT` é uma *derived table*:
...
* Uma *derived table* não pode conter referências a outras tabelas da mesma instrução `SELECT`.
...
O Optimizer determina informações sobre *derived tables* de tal forma que o `EXPLAIN` não precisa materializá-las. Consulte Seção 8.2.2.4, “Otimizando Tabelas Derivadas e Referências de View com Mesclagem ou Materialização”.

É possível, sob certas circunstâncias, que o uso de `EXPLAIN SELECT` modifique os dados da tabela. Isso pode ocorrer se a Query externa acessar quaisquer tabelas e uma Query interna invocar uma stored function que altere uma ou mais linhas de uma tabela. Suponha que existam duas tabelas `t1` e `t2` no Database `d1`, e uma stored function `f1` que modifica `t2`, criada conforme mostrado aqui:
...
Referenciar a função diretamente em um `EXPLAIN SELECT` não tem efeito em `t2`, conforme mostrado aqui:
...
Isso ocorre porque a instrução `SELECT` não referenciou nenhuma tabela, como pode ser visto nas colunas `table` e `Extra` da saída. Isto também é verdade para a seguinte instrução `SELECT` aninhada:
...
No entanto, se a instrução `SELECT` externa referenciar quaisquer tabelas, o Optimizer também executa a instrução na subquery, resultando na modificação de `t2`:
