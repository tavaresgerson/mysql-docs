...
Tradicionalmente, uma subquery `EXISTS` começa com `SELECT *`, mas poderia começar com `SELECT 5` ou `SELECT column1` ou qualquer outra coisa. O MySQL ignora a lista `SELECT` em tal subquery, portanto, não faz diferença.
