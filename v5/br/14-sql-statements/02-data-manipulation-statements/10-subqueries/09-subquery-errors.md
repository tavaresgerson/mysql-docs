...
* Número incorreto de Columns de uma subquery:
  ...
  Você pode usar uma subquery que retorna múltiplas columns, se o propósito for a comparação de Row. Em outros contextos, a subquery deve ser um operando escalar. Consulte Section 13.2.10.5, “Row Subqueries”.
...
* Uso incorreto de table na subquery:
  ...
  Você pode usar uma subquery para atribuição dentro de um comando `UPDATE` porque subqueries são permitidas em comandos `UPDATE` e `DELETE`, assim como em comandos `SELECT`. No entanto, você não pode usar a mesma table (neste caso, a table `t1`) tanto para a cláusula `FROM` da subquery quanto para o alvo do UPDATE.
