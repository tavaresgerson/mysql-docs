...
`IN` e `= ANY` não são sinônimos quando usados com uma lista de expressões. `IN` pode receber uma lista de expressões, mas `= ANY` não. Consulte Section 12.4.2, “Comparison Functions and Operators”.

`NOT IN` não é um alias para `<> ANY`, mas sim para `<> ALL`. Consulte Section 13.2.10.4, “Subqueries with ALL”.
