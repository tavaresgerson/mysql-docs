...
A instrução `LOAD XML` lê dados de um arquivo XML para uma table. O *`file_name`* deve ser fornecido como uma string literal. O *`tagname`* na cláusula opcional `ROWS IDENTIFIED BY` também deve ser fornecido como uma string literal e deve ser cercado por colchetes angulares (`<` e `>`).

`LOAD XML` atua como o complemento da execução do **mysql** client no modo de saída XML (ou seja, iniciando o client com a opção `--xml`). Para escrever dados de uma table para um arquivo XML, você pode invocar o **mysql** client com as opções `--xml` e `-e` a partir do shell do sistema, conforme mostrado aqui:

```sql
$> mysql --xml -e 'SELECT * FROM mydb.mytable' > file.xml
```

Para ler o arquivo de volta para uma table, use `LOAD XML`. Por padrão, o elemento `<row>` é considerado o equivalente a uma linha de table de Database; isso pode ser alterado usando a cláusula `ROWS IDENTIFIED BY`.
...
* Nomes de coluna são os atributos `name` de tags `<field>`, e os valores são o conteúdo dessas tags:
  ...
  Este é o formato usado por outras ferramentas MySQL, como o **mysqldump**.
...
As seguintes cláusulas funcionam essencialmente da mesma forma para `LOAD XML` como funcionam para `LOAD DATA`:
...
Consulte Seção 13.2.6, “LOAD DATA Statement”, para obter mais informações sobre essas cláusulas.
...
A cláusula `IGNORE number LINES` ou `IGNORE number ROWS` faz com que as primeiras *`number`* linhas no arquivo XML sejam ignoradas. É análoga à cláusula `IGNORE ... LINES` da instrução `LOAD DATA`.
...
Como pode ser visto pela resposta do Server, 8 linhas foram importadas para a table `test.person`. Isso pode ser verificado por meio de uma instrução `SELECT` simples:
...
Isso mostra, conforme declarado anteriormente nesta seção, que qualquer um ou todos os 3 formatos XML permitidos podem aparecer em um único arquivo e ser lidos usando `LOAD XML`.

O inverso da operação de importação mostrada — isto é, fazer o Dump de dados da table MySQL para um arquivo XML — pode ser realizado usando o **mysql** client a partir do shell do sistema, conforme mostrado aqui:
...
A opção `--xml` faz com que o **mysql** client use formatação XML para sua saída; a opção `-e` faz com que o client execute a instrução SQL imediatamente após a opção. Consulte Seção 4.5.1, “mysql — The MySQL Command-Line Client”.
...
A ordem em que os campos são fornecidos dentro de cada linha do arquivo XML não afeta a operação do `LOAD XML`; a ordem dos campos pode variar de linha para linha e não é obrigatório que seja a mesma ordem das colunas correspondentes na table.
...
Agora, crie uma table `address` no Database `test` usando a seguinte instrução `CREATE TABLE`:
...
Para importar os dados do arquivo XML para a table `person`, execute a seguinte instrução `LOAD XML`, que especifica que as linhas devem ser especificadas pelo elemento `<person>`, conforme mostrado aqui:
...
Você pode verificar se os registros foram importados usando uma instrução `SELECT`:
...
Para importar os dados dos elementos `<address>` para a table `address`, use a instrução `LOAD XML` mostrada aqui:
...
**Considerações de Segurança.** Assim como na instrução `LOAD DATA`, a transferência do arquivo XML do host do Client para o host do Server é iniciada pelo MySQL Server. Em teoria, um Server modificado poderia ser construído para instruir o programa Client a transferir um arquivo de escolha do Server, em vez do arquivo nomeado pelo Client na instrução `LOAD XML`. Tal Server poderia acessar qualquer arquivo no host do Client ao qual o usuário do Client tenha acesso de leitura.

Em um ambiente Web, os Clients geralmente se conectam ao MySQL a partir de um Web Server. Um usuário que pode executar qualquer comando contra o MySQL Server pode usar `LOAD XML LOCAL` para ler quaisquer arquivos aos quais o processo do Web Server tenha acesso de leitura. Neste ambiente, o Client em relação ao MySQL Server é, na verdade, o Web Server, e não o programa remoto sendo executado pelo usuário que se conecta ao Web Server.

Você pode desabilitar o carregamento de arquivos XML de Clients iniciando o Server com `--local-infile=0` ou `--local-infile=OFF`. Esta opção também pode ser usada ao iniciar o **mysql** client para desabilitar o `LOAD XML` durante a sessão do client.

Para impedir que um client carregue arquivos XML do Server, não conceda o privilégio `FILE` à conta de usuário MySQL correspondente, ou revogue este privilégio se a conta de usuário do client já o possuir.

Importante

Revogar o privilégio `FILE` (ou não concedê-lo em primeiro lugar) impede o usuário apenas de executar a instrução `LOAD XML` (bem como a função `LOAD_FILE()`); isso *não* impede o usuário de executar `LOAD XML LOCAL`. Para desabilitar esta instrução, você deve iniciar o Server ou o Client com `--local-infile=OFF`.

Em outras palavras, o privilégio `FILE` afeta apenas se o client pode ler arquivos no Server; ele não tem relação com o fato de o client poder ler arquivos no sistema de arquivos local.

Para tables particionadas que usam storage engines que empregam table locks, como `MyISAM`, quaisquer Locks causados por `LOAD XML` aplicam Locks em todas as partitions da table. Isso não se aplica a tables que usam storage engines que empregam row-level locking, como `InnoDB`. Para mais informações, consulte Seção 22.6.4, “Partitioning and Locking”.