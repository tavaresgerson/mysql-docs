...
A declaração `HELP` retorna informações online do Manual de Referência do MySQL. Seu funcionamento adequado requer que as tabelas de ajuda no `database` `mysql` sejam inicializadas com informações de tópicos de ajuda (consulte Section 5.1.14, “Server-Side Help Support”).

A declaração `HELP` pesquisa as tabelas de ajuda pela `search string` fornecida e exibe o resultado da pesquisa. A `search string` não diferencia maiúsculas de minúsculas (`case-sensitive`).

A `search string` pode conter os caracteres `wildcard` `%` e `_`. Estes têm o mesmo significado que nas operações de correspondência de padrão realizadas com o operador `LIKE`. Por exemplo, `HELP 'rep%'` retorna uma lista de tópicos que começam com `rep`.
...
* Para ajuda sobre um tópico de ajuda específico, como a `function` `ASCII()` ou a declaração `CREATE TABLE`, use a `keyword` ou `keywords` associadas:
