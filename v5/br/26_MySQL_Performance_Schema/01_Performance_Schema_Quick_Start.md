...
O motor de armazenamento `PERFORMANCE_SCHEMA` opera em tabelas no banco de dados `performance_schema`. Você pode tornar o banco de dados `performance_schema` o padrão, para que as referências a suas tabelas não precisem ser qualificadas com o nome do banco de dados:

```sql
mysql> USE performance_schema;
```

As tabelas do Schema de desempenho são armazenadas no banco de dados `performance_schema`. Informações sobre a estrutura desse banco de dados e suas tabelas podem ser obtidas, como para qualquer outro banco de dados, selecionando-se a partir do banco de dados `INFORMATION_SCHEMA` ou usando instruções `SHOW`. Por exemplo, use uma dessas instruções para ver quais tabelas do Schema de desempenho existem:
...
A estrutura de tabela também está disponível ao selecionar tabelas como `INFORMATION_SCHEMA.COLUMNS` ou ao usar declarações como `SHOW COLUMNS`.
...
Para entender como interpretar os nomes dos instrumentos, consulte a Seção 25.6, “Convenções de Nomenclatura de Instrumentos do Schema de Desempenho”.
...
Para obter mais informações sobre as tabelas de configuração e como usá-las para controlar a coleta de eventos, consulte a Seção 25.4.2, “Filtragem de Eventos do Schema de Desempenho”.

Existem algumas tabelas diversas que não se encaixam em nenhum dos grupos anteriores. Por exemplo, `performance_timers` lista os temporizadores de evento disponíveis e suas características. Para informações sobre temporizadores, consulte a Seção 25.4.1, “Temporização de Eventos do Schema de Desempenho”.