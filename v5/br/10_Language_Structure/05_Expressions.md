...
Para a precedência do operador, consulte a Seção 12.4.1, “Precedência do operador”. A precedência e o significado de alguns operadores dependem do modo SQL:
...
Para a sintaxe de identificadores, consulte a Seção 9.2, “Nomes de Objetos do Esquema”.
...
* Variáveis locais de programa armazenado: Seção 13.6.4.1, “Declaração de Variável Local DECLARE”

* Parâmetros de programa armazenado: Seção 13.1.16, “Estruturas de declaração CREATE PROCEDURE e CREATE FUNCTION”

*`param_marker`* é `?` conforme utilizado em declarações preparadas para marcadores. Veja a Seção 13.5.1, “Declaração PREPARE”.

`(subquery)` indica uma subconsulta que retorna um único valor; ou seja, uma subconsulta escalar. Veja a Seção 13.2.10.1, “A subconsulta como operador escalar”.
...
*`match_expr`* indica uma expressão `MATCH`. Veja a Seção 12.9, “Funções de Pesquisa de Texto Completo”.
...
Os intervalos temporais podem ser utilizados em declarações de `CREATE EVENT`:
...
Para garantir a interpretação do valor do intervalo conforme o esperado, pode ser usada uma operação `CAST()`. Para tratar `6/4` como 1 hora e 5 minutos, transforme-o em um valor de `DECIMAL` - DECIMAL, NUMERIC") com um único dígito fracionário:
