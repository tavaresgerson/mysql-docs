...
É também possível atribuir um valor a uma variável de usuário em declarações que não sejam `SET`. (Essa funcionalidade é descontinuada no MySQL 8.0 e está sujeita à remoção em uma versão subsequente.) Ao fazer uma atribuição dessa forma, o operador de atribuição deve ser `:=` e não `=`, pois este último é tratado como o operador de comparação `=` em declarações que não sejam `SET`:
...
Como regra geral, exceto nas declarações de `SET`, você nunca deve atribuir um valor a uma variável de usuário e ler o valor na mesma declaração. Por exemplo, para incrementar uma variável, isso está bem:
...
Em uma declaração `SELECT`, cada expressão selecionada é avaliada apenas quando enviada ao cliente. Isso significa que em uma cláusula `HAVING`, `GROUP BY` ou `ORDER BY`, referenciando uma variável que recebe um valor na lista de expressão selecionada *não* funciona como esperado:
