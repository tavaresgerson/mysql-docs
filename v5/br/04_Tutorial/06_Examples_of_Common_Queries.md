...
### 3.6.3 Máximo de Colunas por Grupo
...
O exemplo anterior usa uma subconsulta correlacionada, que pode ser ineficiente (consulte a Seção 13.2.10.7, “Subconsultas correlacionadas”). Outras possibilidades para resolver o problema são usar uma subconsulta não correlacionada na cláusula `FROM` ou uma `LEFT JOIN`.
...
O `LEFT JOIN` funciona com base no princípio de que, quando o `s1.price` está em seu valor máximo, não há `s2.price` com um valor maior e, portanto, o valor correspondente ao `s2.article` é `NULL`. Veja a Seção 13.2.9.2, “Cláusula JOIN”.

### 3.6.5 Usando variáveis definidas pelo usuário

Você pode utilizar variáveis de usuário do MySQL para lembrar resultados sem precisar armazená-los em variáveis temporárias no cliente. (Veja a Seção 9.4, “Variáveis Definidas pelo Usuário”.)
...
Para demonstrar a ação referencial `ON DELETE CASCADE`, exclua os registros da tabela principal onde `parent_id = 2`; isso exclui todos os registros na tabela principal.
...
Este caso é otimizado. Veja a Seção 8.2.1.3, “Otimização da Mesclagem de Índices”.

Você também pode resolver o problema de forma eficiente usando um `UNION` que combina a saída de duas declarações separadas `SELECT`. Veja a Seção 13.2.9.3, “Cláusula UNION”.
...
### 3.6.8 Calcular visitas por dia
...
Atualizar um valor existente na coluna `AUTO_INCREMENT` em uma tabela `InnoDB` não redefinirá a sequência `AUTO_INCREMENT`, como acontece com as tabelas `MyISAM` e `NDB`.
