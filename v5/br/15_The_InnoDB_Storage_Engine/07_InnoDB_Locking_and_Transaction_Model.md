...
* A seção 14.7.4, “Strings Fantasma”, descreve como o `InnoDB` usa o bloqueio de próxima chave para evitar strings fantasmas.
...
`InnoDB` implementa o bloqueio padrão em nível de string, onde existem dois tipos de bloqueios, bloqueios compartilhados (`S`) e bloqueios exclusivos (`X`).

* Um bloqueio compartilhado (`S`) permite que a transação que detém o bloqueio leia uma string.

* Uma chave exclusiva (`X`) permite a transação que mantém a chave para atualizar ou excluir uma string.

Se a transação `T1` possuir um bloqueio compartilhado (`S`) na string `r`, então os pedidos de algumas transações distintas `T2` para um bloqueio na string `r` são tratados da seguinte forma:
...
Se uma transação `T1` possuir um bloqueio exclusivo (`X`) na string `r`, um pedido de alguma transação `T2` para um bloqueio de qualquer tipo na `r` não pode ser concedido imediatamente. Em vez disso, a transação `T2` tem que esperar que a transação `T1` libere seu bloqueio na string `r`.

#### Fechaduras de Intenção

`InnoDB` suporta *bloqueio de múltiplas granularidades*, que permite a coexistência de bloqueios de string e bloqueios de tabela. Por exemplo, uma declaração como `LOCK TABLES ... WRITE` obtém um bloqueio exclusivo (um bloqueio `X` na tabela especificada). Para tornar o bloqueio em múltiplos níveis de granularidade prático, `InnoDB` usa bloqueios de intenção. Bloqueios de intenção são bloqueios de nível de tabela que indicam que tipo de bloqueio (compartilhado ou exclusivo) uma transação requer posteriormente para uma string em uma tabela. Existem dois tipos de bloqueios de intenção:

* Uma chave de bloqueio compartilhada (`IS`) indica que uma transação pretende definir um *bloqueio* compartilhado em strings individuais de uma tabela.

* Um bloqueio de intenção exclusiva (`IX`) indica que uma transação pretende definir um bloqueio exclusivo em strings individuais de uma tabela.
...
* Antes que uma transação possa adquirir um bloqueio compartilhado em uma string em uma tabela, ela deve primeiro adquirir um bloqueio `IS` ou mais forte na tabela.

* Antes que uma transação possa adquirir um bloqueio exclusivo em uma string em uma tabela, ela deve primeiro adquirir um bloqueio `IX` na tabela.
...
Os bloqueios de intenção não bloqueiam nada, exceto solicitações de tabela completa (por exemplo, `LOCK TABLES ... WRITE`). O principal propósito dos bloqueios de intenção é mostrar que alguém está bloqueando uma string ou vai bloquear uma string na tabela.
...
Um bloqueio de registro é um bloqueio em um registro de índice. Por exemplo, `SELECT c1 FROM t WHERE c1 = 10 FOR UPDATE;` impede que qualquer outra transação insira, atualize ou exclua strings onde o valor de `t.c1` é `10`.
...
O bloqueio de lacuna não é necessário para declarações que bloqueiam strings usando um índice único para pesquisar uma string única. (Isso não inclui o caso em que a condição de pesquisa inclui apenas algumas colunas de um índice único de múltiplas colunas; nesse caso, o bloqueio de lacuna ocorre.) Por exemplo, se a coluna `id` tiver um índice único, a seguinte declaração usa apenas um bloqueio de registro de índice para a string com o valor `id` 100 e não importa se outras sessões inserem strings no espaço anterior:
...
`InnoDB` realiza o bloqueio de nível de string de tal forma que, quando ele pesquisa ou examina um índice de tabela, ele define bloqueios compartilhados ou exclusivos nos registros do índice que ele encontra. Assim, os bloqueios de nível de string são, na verdade, bloqueios de registro de índice. Um bloqueio de próxima chave em um registro de índice também afeta o "espaço" antes desse registro de índice. Isso significa que um bloqueio de próxima chave é um bloqueio de registro de índice mais um bloqueio de espaço sobre o espaço que precede o registro de índice. Se uma sessão tiver um bloqueio compartilhado ou exclusivo no registro `R` em um índice, outra sessão não pode inserir um novo registro de índice no espaço imediatamente antes de `R` na ordem do índice.
...
Por padrão, `InnoDB` opera no nível de isolamento de transação `REPEATABLE READ`. Neste caso, `InnoDB` utiliza bloqueios de próxima chave para pesquisas e varreduras de índice, o que previne strings fantasmas (ver Seção 14.7.4, “Strings fantasmas”).
...
Um bloqueio de inserção de intenção é um tipo de bloqueio de lacuna definido pelas operações de `INSERT` antes da inserção de string. Esse bloqueio sinaliza a intenção de inserir de tal forma que várias transações que inserem no mesmo intervalo de índice não precisam esperar uma da outra se não estiverem inserindo na mesma posição dentro da lacuna. Suponha que haja registros de índice com valores de 4 e 7. Transações separadas que tentam inserir valores de 5 e 6, respectivamente, bloqueiam a lacuna entre 4 e 7 com bloqueios de intenção de inserção antes de obter o bloqueio exclusivo da string inserida, mas não se bloqueiam uma à outra porque as strings não são conflitantes.
...
Para permitir o suporte a níveis de isolamento para tabelas com índices `SPATIAL`, o `InnoDB` utiliza bloqueios de predicado. Um índice `SPATIAL` contém valores de retângulo mínimo de delimitação (MBR), portanto, o `InnoDB` impõe uma leitura consistente no índice, definindo um bloqueio de predicado no valor MBR usado para uma consulta. Outras transações não podem inserir ou modificar uma string que corresponderia à condição da consulta.

### 14.7.2 Modelo de Transação InnoDB

O modelo de transação `InnoDB` visa combinar as melhores propriedades de um banco de dados de múltiplas versões com o bloqueio tradicional de duas fases. `InnoDB` realiza o bloqueio no nível da string e executa consultas como leituras consistentes não bloqueantes por padrão, no estilo do Oracle. As informações de bloqueio em `InnoDB` são armazenadas de forma eficiente em termos de espaço, de modo que a escalada de bloqueio não seja necessária. Tipicamente, vários usuários são autorizados a bloquear cada string em tabelas de `InnoDB`, ou qualquer subconjunto aleatório das strings, sem causar o esgotamento da memória de `InnoDB`.
...
Um usuário pode alterar o nível de isolamento para uma única sessão ou para todas as conexões subsequentes com a declaração `SET TRANSACTION`. Para definir o nível de isolamento padrão do servidor para todas as conexões, use a opção `--transaction-isolation` na string de comando ou em um arquivo de opções. Para informações detalhadas sobre os níveis de isolamento e a sintaxe de definição de nível, consulte a Seção 13.3.6, “Declaração SET TRANSACTION”.
...
Como o bloqueio de lacunas está desativado, podem ocorrer problemas com strings fantasmas, pois outras sessões podem inserir novas strings nas lacunas. Para informações sobre strings fantasmas, consulte a Seção 14.7.4, “Strings fantasmas”.

Apenas o registro binário baseado em string é suportado com o nível de isolamento `READ COMMITTED`. Se você usar `READ COMMITTED` com `binlog_format=MIXED`, o servidor usa automaticamente o registro baseado em string.
...
À medida que o `InnoDB` executa cada `UPDATE`, ele primeiro adquire um bloqueio exclusivo para cada string que lê, e depois determina se a modificará. Se o `InnoDB` não modifica a string, ele libera o bloqueio. Caso contrário, o `InnoDB` retém o bloqueio até o final da transação. Isso afeta o processamento da transação da seguinte forma.

Ao usar o nível de isolamento padrão `REPEATABLE READ`, o primeiro `UPDATE` adquire um x-lock em cada string que lê e não libera nenhuma delas:
...
O segundo bloco `UPDATE` bloqueia assim que tenta adquirir qualquer bloqueio (porque a primeira atualização retivou os bloqueios em todas as strings), e não prossegue até que o primeiro `UPDATE` commit ou rollback:

  ```sql
  x-lock(1,2); block and wait for first UPDATE to commit or roll back
  ```

Se for usado `READ COMMITTED` em vez disso, o primeiro `UPDATE` adquire um x-lock em cada string que lê e libera essas strings que não modifica:
...
Para o segundo `UPDATE`, o `InnoDB` realiza uma leitura “semi-consistente”, retornando a versão mais recente comprometida de cada string que lê para o MySQL, para que o MySQL possa determinar se a string corresponde à condição do `WHERE` do `UPDATE`:
...
No entanto, se a condição `WHERE` incluir uma coluna indexada e o `InnoDB` utilizar o índice, apenas a coluna indexada é considerada ao realizar e manter bloqueios de registro. No exemplo a seguir, o primeiro `UPDATE` realiza e mantém um bloqueio x em cada string onde b = 2. O segundo `UPDATE` bloqueia quando tenta adquirir blocos x nos mesmos registros, pois também utiliza o índice definido na coluna b.
...
Suponha que você esteja executando o nível de isolamento padrão `REPEATABLE READ`. Quando você emite uma leitura consistente (ou seja, uma declaração comum `SELECT`, `InnoDB` atribui a sua transação um ponto de tempo de acordo com o qual sua consulta vê o banco de dados. Se outra transação excluir uma string e confirmar após seu ponto de tempo ter sido atribuído, você não verá que a string foi excluída. Inserções e atualizações são tratadas de forma semelhante.

Nota

O instantâneo do estado do banco de dados se aplica às declarações `SELECT` dentro de uma transação, não necessariamente às declarações DML. Se você inserir ou modificar algumas strings e então confirmar essa transação, uma declaração `DELETE` ou `UPDATE` emitida a partir de outra transação `REPEATABLE READ` concorrente pode afetar essas strings que foram confirmadas recentemente, mesmo que a sessão não as possa consultar. Se uma transação atualiza ou exclui strings confirmadas por outra transação, essas alterações se tornam visíveis para a transação atual. Por exemplo, você pode encontrar uma situação como a seguinte:
...
No exemplo a seguir, a sessão A vê a string inserida por B apenas quando B comprometeu a inserção e A também comprometeu, de modo que o ponto de tempo é avançado além do comprometimento de B.
...
* A leitura consistente não funciona em operações `ALTER TABLE` que fazem uma cópia temporária da tabela original e excluem a tabela original quando a cópia temporária é construída. Quando você reemite uma leitura consistente dentro de uma transação, as strings na nova tabela não são visíveis porque essas strings não existiam quando o instantâneo da transação foi tomado. Neste caso, a transação retorna um erro: `ER_TABLE_DEF_CHANGED`, “A definição da tabela mudou, tente novamente a transação”.
...
* Para realizar uma leitura não bloqueável nesses casos, habilite a opção `innodb_locks_unsafe_for_binlog` e defina o nível de isolamento da transação para `READ UNCOMMITTED`, `READ COMMITTED` ou `REPEATABLE READ` para evitar definir bloqueios em strings lidas da tabela selecionada.

#### 14.7.2.4 Leitura de bloqueio

Se você fizer uma consulta de dados e, em seguida, inserir ou atualizar dados relacionados na mesma transação, a declaração regular `SELECT` não oferece proteção suficiente. Outras transações podem atualizar ou excluir as mesmas strings que você acabou de consultar. `InnoDB` suporta dois tipos de leituras de bloqueio que oferecem segurança extra:

* `SELECT ... LOCK IN SHARE MODE`

Define um bloqueio de modo compartilhado em quaisquer strings que são lidas. Outras sessões podem ler as strings, mas não as podem modificar até que sua transação seja confirmada. Se alguma dessas strings foi alterada por outra transação que ainda não foi confirmada, sua consulta espera até que essa transação termine e, em seguida, usa os valores mais recentes.

* `SELECT ... FOR UPDATE`

Para registros de índice, a pesquisa encontra, bloqueia as strings e quaisquer entradas de índice associadas, da mesma forma como se emitisse uma declaração `UPDATE` para essas strings. Outras transações são bloqueadas para atualizar essas strings, para fazer `SELECT ... LOCK IN SHARE MODE`, ou para ler os dados em certos níveis de isolamento de transação. Leitura consistente ignora quaisquer bloqueios definidos nos registros que existem na visão de leitura. (Versões antigas de um registro não podem ser bloqueadas; elas são reconstruídas aplicando registros de desfazer em uma cópia de memória do registro.)
...
Uma cláusula de leitura com bloqueio em uma declaração externa não bloqueia as strings de uma tabela em uma subconsulta aninhada, a menos que uma cláusula de leitura com bloqueio também seja especificada na subconsulta. Por exemplo, a seguinte declaração não bloqueia as strings da tabela `t2`.

```sql
SELECT * FROM t1 WHERE c1 = (SELECT c1 FROM t2) FOR UPDATE;
```

Para bloquear strings na tabela `t2`, adicione uma cláusula de leitura de bloqueio à subconsulta:
...
Suponha que você queira inserir uma nova string em uma tabela `child`, e certifique-se de que a string secundária tenha uma string principal na tabela `parent`. Seu código de aplicação pode garantir a integridade referencial ao longo dessa sequência de operações.

Primeiro, use uma leitura consistente para consultar a tabela `PARENT` e verifique se a string pai existe. Você pode inserir com segurança a string filho na tabela `CHILD`? Não, porque outra sessão pode excluir a string pai no momento entre seus `SELECT` e seus `INSERT`, sem que você saiba disso.
...
Um `SELECT ... FOR UPDATE` lê os dados disponíveis mais recentes, definindo bloqueios exclusivos em cada string que lê. Assim, ele define os mesmos bloqueios que um `UPDATE` SQL pesquisado definiria nas strings.
...
Se você não tiver índices adequados para sua declaração e o MySQL precisar percorrer toda a tabela para processar a declaração, cada string da tabela é bloqueada, o que, por sua vez, bloqueia todas as inserções feitas por outros usuários na tabela. É importante criar bons índices para que suas consultas não percorram mais strings do que o necessário.

`InnoDB` define os tipos específicos de fechaduras da seguinte forma.

* `SELECT ... FROM` é uma leitura consistente, que lê um instantâneo do banco de dados e não define bloqueios, a menos que o nível de isolamento de transação seja definido como `SERIALIZABLE`. Para o nível `SERIALIZABLE`, a pesquisa define blocos compartilhados de próxima chave nos registros do índice que ela encontra. No entanto, apenas um bloqueio de registro de índice é necessário para declarações que bloqueiam strings usando um índice único para pesquisar uma string única.

* Para `SELECT ... FOR UPDATE` ou `SELECT ... LOCK IN SHARE MODE`, as chaves são adquiridas para as strings digitalizadas e esperadas para serem liberadas para as strings que não se qualificam para inclusão no conjunto de resultados (por exemplo, se não atenderem aos critérios dados na cláusula `WHERE`). No entanto, em alguns casos, as strings podem não ser desbloqueadas imediatamente porque a relação entre uma string de resultado e sua fonte original é perdida durante a execução da consulta. Por exemplo, em um `UNION`, strings digitalizadas (e bloqueadas) de uma tabela podem ser inseridas em uma tabela temporária antes de avaliar se se qualificam para o conjunto de resultados. Nessa circunstância, a relação das strings na tabela temporária com as strings na tabela original é perdida e as últimas strings não são desbloqueadas até o final da execução da consulta.

* `SELECT ... LOCK IN SHARE MODE` define bloqueios de próxima chave compartilhada em todos os registros de índice que a pesquisa encontra. No entanto, apenas um bloqueio de registro de índice é necessário para declarações que bloqueiam strings usando um índice único para pesquisar uma string única.

* `SELECT ... FOR UPDATE` define um bloqueio exclusivo de próxima chave em cada registro que o busca encontra. No entanto, apenas um bloqueio de registro de índice é necessário para declarações que bloqueiam strings usando um índice único para buscar uma string única.
...
* `UPDATE ... WHERE ...` define um bloqueio exclusivo de próxima chave em cada registro que o busca encontra. No entanto, apenas um bloqueio de registro de índice é necessário para declarações que bloqueiam strings usando um índice único para buscar uma string única.
...
* `DELETE FROM ... WHERE ...` define um bloqueio de próxima chave exclusivo em cada registro que o busca encontra. No entanto, apenas um bloqueio de registro de índice é necessário para declarações que bloqueiam strings usando um índice único para buscar uma string única.

* `INSERT` estabelece um bloqueio exclusivo na string inserida. Esse bloqueio é um bloqueio de registro de índice, não um bloqueio de próxima chave (ou seja, não há bloqueio de lacuna) e não impede que outras sessões insiram na lacuna antes da string inserida.

Antes de inserir a string, um tipo de bloqueio de lacuna chamado bloqueio de lacuna de intenção de inserção é definido. Esse bloqueio sinaliza a intenção de inserir de tal forma que várias transações que inserem no mesmo intervalo de índice não precisam esperar uma da outra se não estiverem inserindo na mesma posição dentro da lacuna. Suponha que haja registros de índice com valores de 4 e 7. Transações separadas que tentam inserir valores de 5 e 6 cada uma bloqueiam a lacuna entre 4 e 7 com bloqueios de intenção de inserção antes de obter o bloqueio exclusivo da string inserida, mas não se bloqueiam uma à outra porque as strings não são conflitantes.

Se ocorrer um erro de chave duplicada, um bloqueio compartilhado no registro do índice duplicado é definido. Esse uso de um bloqueio compartilhado pode resultar em um impasse se houver várias sessões tentando inserir a mesma string se outra sessão já tiver um bloqueio exclusivo. Isso pode ocorrer se outra sessão excluir a string. Suponha que uma tabela `InnoDB` `t1` tenha a seguinte estrutura:
...
A primeira operação da sessão 1 adquire um bloqueio exclusivo para a string. As operações das sessões 2 e 3 resultam em um erro de chave duplicada e ambas solicitam um bloqueio compartilhado para a string. Quando a sessão 1 retorna, libera seu bloqueio exclusivo na string e os pedidos de bloqueio compartilhado em fila para as sessões 2 e 3 são concedidos. Neste ponto, as sessões 2 e 3 entram em deadlock: nenhuma delas pode adquirir um bloqueio exclusivo para a string devido ao bloqueio compartilhado mantido pela outra.

Uma situação semelhante ocorre se a tabela já contiver uma string com o valor da chave 1 e três sessões realizarem as seguintes operações em ordem:
...
A primeira operação da sessão 1 adquire um bloqueio exclusivo para a string. As operações das sessões 2 e 3 resultam em um erro de chave duplicada e ambas solicitam um bloqueio compartilhado para a string. Quando a sessão 1 confirma, libera seu bloqueio exclusivo na string e os pedidos de bloqueio compartilhado em fila para as sessões 2 e 3 são concedidos. Neste ponto, as sessões 2 e 3 entram em deadlock: nenhuma delas pode adquirir um bloqueio exclusivo para a string devido ao bloqueio compartilhado mantido pela outra.

* `INSERT ... ON DUPLICATE KEY UPDATE` difere de um simples `INSERT` porque uma chave exclusiva é colocada na string a ser atualizada quando ocorre um erro de chave duplicada. Um bloqueio exclusivo de registro de índice é tomado para um valor de chave primária duplicada. Um bloqueio exclusivo de próxima chave é tomado para um valor de chave única duplicado.

* `REPLACE` é feito como um `INSERT` se não houver colisão em uma chave única. Caso contrário, uma trava exclusiva de próxima chave é colocada na string a ser substituída.

* `INSERT INTO T SELECT ... FROM S WHERE ...` define um registro de índice exclusivo com bloqueio (sem bloqueio de lacuna) em cada string inserida em `T`. Se o nível de isolamento de transação é `READ COMMITTED`, ou `innodb_locks_unsafe_for_binlog` está habilitado e o nível de isolamento de transação não é `SERIALIZABLE`, `InnoDB` faz a pesquisa em `S` como uma leitura consistente (sem bloqueios). Caso contrário, `InnoDB` define bloqueios compartilhados de próxima chave em strings de `S`. `InnoDB` tem que definir bloqueios no último caso: Durante a recuperação de rolagem usando um log binário baseado em declaração, cada declaração SQL deve ser executada exatamente da mesma maneira que foi feita originalmente.
...
Quando um `SELECT` é usado nos construtos `REPLACE INTO t SELECT ... FROM s WHERE ...` ou `UPDATE t ... WHERE col IN (SELECT ... FROM s ...)`, o `InnoDB` define bloqueios compartilhados de próxima chave em strings da tabela `s`.
...
* `LOCK TABLES` define bloqueios de tabela, mas é a camada MySQL superior à `InnoDB` que define esses bloqueios. `InnoDB` é consciente dos bloqueios de tabela se `innodb_table_locks = 1` (o padrão) e `autocommit = 0`, e a camada MySQL acima de `InnoDB` sabe sobre bloqueios de nível de string.

Caso contrário, o detector automático de bloqueio `InnoDB` não consegue detectar bloqueios onde tais bloqueios de tabela estão envolvidos. Além disso, porque, neste caso, a camada superior do MySQL não sabe sobre bloqueios de nível de string, é possível obter um bloqueio de tabela em uma tabela onde outra sessão atualmente tem bloqueios de nível de string. No entanto, isso não coloca em risco a integridade da transação, conforme discutido na Seção 14.7.5.2, "Detecção de Bloqueio".
...
### 14.7.4 Strings Fantasma

O chamado problema fantasma ocorre em uma transação quando a mesma consulta produz diferentes conjuntos de strings em diferentes momentos. Por exemplo, se um `SELECT` é executado duas vezes, mas retorna uma string na segunda vez que não foi retornada na primeira, a string é uma string "fantasma".

Suponha que haja um índice na coluna `id` da tabela `child` e que você queira ler e bloquear todas as strings da tabela que tenham um valor de identificador maior que 100, com a intenção de atualizar alguma coluna nas strings selecionadas mais tarde:

```sql
SELECT * FROM child WHERE id > 100 FOR UPDATE;
```

A consulta examina o índice a partir do primeiro registro onde `id` é maior que 100. Deixe a tabela conter strings com valores de `id` de 90 e 102. Se as chaves definidas nos registros do índice no intervalo examinado não bloqueiam inserções feitas nos intervalos (neste caso, o intervalo entre 90 e 102), outra sessão pode inserir uma nova string na tabela com um `id` de 101. Se você executasse o mesmo `SELECT` dentro da mesma transação, veria uma nova string com um `id` de 101 (um “fantasma”) no conjunto de resultados retornado pela consulta. Se considerarmos um conjunto de strings como um item de dados, a nova criança fantasma violaria o princípio de isolamento das transações, que uma transação deve ser capaz de executar para que os dados que ela leu não mudem durante a transação.
...
Você pode usar o bloqueio de próxima chave para implementar uma verificação de unicidade em sua aplicação: Se você ler seus dados no modo de compartilhamento e não ver um duplicado para uma string que você vai inserir, então você pode inserir com segurança sua string e saber que o bloqueio de próxima chave definido no sucessor de sua string durante a leitura impede que alguém, entretanto, insira um duplicado para sua string. Assim, o bloqueio de próxima chave permite que você "bloqueie" a não existência de algo em sua tabela.

O bloqueio de lacunas pode ser desativado conforme discutido na Seção 14.7.1, "Bloqueio InnoDB". Isso pode causar problemas fantásticos, pois outras sessões podem inserir novas strings nas lacunas quando o bloqueio de lacunas é desativado.
...
Primeiro, o cliente A cria uma tabela contendo uma única string e, em seguida, inicia uma transação. Dentro da transação, A obtém um bloqueio `S` na string selecionando-a no modo de compartilhamento:
...
Em seguida, o cliente B inicia uma transação e tenta excluir a string da tabela:
...
A operação de exclusão requer um bloqueio `X`. O bloqueio não pode ser concedido porque é incompatível com o bloqueio `S` que o cliente A possui, então o pedido fica na fila de solicitações de bloqueio para a string e o cliente B bloqueia.

Por fim, o cliente A também tenta excluir a string da tabela:

```sql
mysql> DELETE FROM t WHERE i = 1;
```

O bloqueio ocorre aqui porque o cliente A precisa de um bloqueio `X` para excluir a string. No entanto, esse pedido de bloqueio não pode ser concedido porque o cliente B já tem um pedido para um bloqueio `X` e está esperando o cliente A liberar seu bloqueio `S`. Além disso, o bloqueio `S` mantido por A não pode ser atualizado para um bloqueio `X` devido ao pedido prévio do B para um bloqueio `X`. Como resultado, `InnoDB` gera um erro para um dos clientes e libera seus bloqueios. O cliente retorna esse erro:
...
`InnoDB` utiliza bloqueio automático em nível de string. Você pode obter deadlocks mesmo no caso de transações que apenas inserem ou excluem uma única string. Isso ocorre porque essas operações não são realmente “atómicas”; elas automaticamente definem bloqueios nos (possíveis vários) registros de índice da string inserida ou excluída.
...
* Outra maneira de serializar transações é criar uma tabela auxiliar de "sinalização" que contenha apenas uma única string. Faça com que cada transação atualize essa string antes de acessar outras tabelas. Dessa forma, todas as transações ocorrem de forma serializada. Observe que o algoritmo de detecção de deadlock instantâneo `InnoDB` também funciona nesse caso, porque o bloqueio de serialização é um bloqueio de nível de string. Com bloqueios de nível de tabela no MySQL, o método de tempo limite deve ser usado para resolver deadlocks.