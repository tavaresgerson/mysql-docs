...
* As solicitações subsequentes pelos mesmos dados são atendidas a partir do pool de buffer `InnoDB`. O pool de buffer gerencia o cache de memória. Você pode ajustar o desempenho das operações intensivas em dados usando as opções de configuração `InnoDB`.
...
### 14.21.2 Arquitetura do InnoDB memcached
...
* Acesso direto às tabelas do `InnoDB`, ignorando o analisador SQL, o otimizador e até mesmo a camada de API do Handler.
...
Antes de configurar o plugin `daemon_memcached`, consulte a Seção 14.21.4, “Considerações de segurança para o plugin InnoDB memcached”, para entender os procedimentos de segurança necessários para impedir o acesso não autorizado.
...
Para mais informações sobre as tabelas `innodb_memcache` e a tabela de amostra `demo_test`, consulte a Seção 14.21.7, “Interiores do Plugin memcached do InnoDB”.
...
* `set` é o comando para armazenar um valor
   * `BB` é a chave
   * `10` é uma bandeira para a operação; ignorada por **memcached**, mas pode ser usada pelo cliente para indicar qualquer tipo de informação; especifique `0` se não for usada
...
2. Adicione uma entrada na tabela `innodb_memcache.containers` para que o plugin `daemon_memcached` saiba como acessar a tabela `InnoDB`. A entrada deve satisfazer a definição da tabela `innodb_memcache.containers`. Para uma descrição de cada campo, consulte a Seção 14.21.7, “Interiores do Plugin memcached InnoDB”.
...
* `daemon_memcached_engine_lib_path`: O caminho do diretório que contém a biblioteca compartilhada que implementa o plugin `InnoDB` **memcached**. O padrão é NULL, representando o diretório do plugin.
...
Os parâmetros de configuração do plugin `daemon_memcached` podem ser especificados no arquivo de configuração do MySQL ou em uma cadeia de inicialização `mysqld`. Eles entram em vigor quando você carrega o plugin `daemon_memcached`.
...
Para obter informações adicionais sobre a habilitação e desabilitação de plugins, consulte a Seção 5.5.1, “Instalando e Desinstalando Plugins”.
...
3. Instale o plugin `daemon_memcached`, conforme descrito na Seção 14.21.3, “Configurando o plugin memcached InnoDB”.
...
### 14.21.5 Aplicativos de escrita para o plugin memcached do InnoDB
...
#### 14.21.5.1 Adaptando um esquema existente do MySQL para o plugin memcached do InnoDB
...
O exemplo assume que o plugin `daemon_memcached` está instalado conforme descrito na Seção 14.21.3, “Configurando o Plugin memcached InnoDB”. Também assume que seu sistema está configurado para executar um script Python que utiliza o módulo `python-memcache`.
...
* O registro `innodb_memcache.containers` para a tabela `multicol` especifica um valor `name`, que é o identificador da tabela.
...
* `key_columns` é definido como a coluna única `country`. A coluna `country` é definida como chave primária na definição da tabela `multicol`.
...
6. Consulte a tabela `multicol` para visualizar os dados inseridos pelo aplicativo Python de amostra. Os dados estão disponíveis para consultas MySQL, o que demonstra como os mesmos dados podem ser acessados usando SQL ou através de aplicativos (usando o conector ou API apropriada MySQL).
...
#### 14.21.5.2 Adaptando um aplicativo memcached para o plugin memcached do InnoDB
...
`InnoDB` possui vários ajustes que permitem escolher o equilíbrio entre alta confiabilidade, em caso de falha, e a quantidade de overhead de E/S durante cargas de trabalho de escrita alta. Por exemplo, considere definir os `innodb_doublewrite` para `0` e `innodb_flush_log_at_trx_commit` para `2`. Meça o desempenho com diferentes configurações de `innodb_flush_method`.

Nota

`innodb_support_xa` é desatualizado; espere que ele seja removido em uma versão futura. A partir do MySQL 5.7.10, o suporte `InnoDB` para commit de duas fases em transações XA está sempre habilitado e a desativação de `innodb_support_xa` não é permitida mais.

Para outras maneiras de reduzir ou ajustar o I/O para operações de tabela, consulte a Seção 8.5.8, “Otimizando o I/O de disco do InnoDB”.
...
#### 14.21.5.4 Controlar o comportamento transacional do plugin memcached do InnoDB
...
#### 14.21.5.5 Adaptando declarações DML a operações memcached
...
#### 14.21.5.6 Realizando declarações DML e DDL nas tabelas subjacentes InnoDB
...
Como o plugin `daemon_memcached` suporta o log binário do MySQL, as atualizações feitas em um servidor fonte através da interface **memcached** podem ser replicadas para backup, equilíbrio de cargas de leitura intensivas e alta disponibilidade. Todos os comandos **memcached** são suportados com registro binário.
...
As seções a seguir mostram como usar a capacidade de registro binário ao usar o plugin `daemon_memcached` com replicação do MySQL. Assume-se que você tenha completado a configuração descrita na Seção 14.21.3, “Configurando o Plugin InnoDB memcached”.
...
2. Configure o servidor de origem e o servidor de replicação, conforme descrito na Seção 16.1.2, “Configuração da replicação com base na posição do arquivo de registro binário”.
...
4. No servidor de origem, emita `SHOW MASTER STATUS` para obter as coordenadas do log binário de origem.

   ```sql
   mysql> SHOW MASTER STATUS;
   ```

5. No servidor replica, use uma declaração `CHANGE MASTER TO` para configurar um servidor replica usando as coordenadas do log binário de origem.
...
10. Verifique para garantir que a operação `flush_all` foi replicada no servidor de replicação.
...
* O valor padrão de `daemon_memcached_w_batch_size` é `1`, o que significa que cada operação de escrita do **memcached** é comprometida imediatamente. Esta configuração padrão incorre em uma certa sobrecarga de desempenho para evitar inconsistências nos dados que são visíveis nos servidores fonte e réplica. Os registros replicados estão sempre disponíveis imediatamente no servidor réplica. Se você definir `daemon_memcached_w_batch_size` para um valor maior que `1`, os registros inseridos ou atualizados através do **memcached** não serão imediatamente visíveis no servidor fonte; para visualizar os registros no servidor fonte antes de serem comprometidos, execute `SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED`.

### 14.21.7 Interiores do Plugin memcached do InnoDB

#### API InnoDB para o Plugin memcached InnoDB

O motor `InnoDB` **memcached** acessa `InnoDB` através das APIs `InnoDB`, a maioria das quais é diretamente adotada do embutido `InnoDB`. As funções das APIs `InnoDB` são passadas para o motor `InnoDB` **memcached** como funções de callback. As funções das APIs `InnoDB` acessam as tabelas `InnoDB` diretamente e são, na maioria, operações DML, com exceção de `TRUNCATE TABLE`.
...
**Tabela 14.23 Colunas de opções de configuração**

<table frame="all" summary="Columns of the config_options table."><col style="width: 20%"/><col style="width: 80%"/><thead><tr> <th>Column</th> <th>Descrição</th> </tr></thead><tbody><tr> <td><code>Name</code></td> <td>Nome do<strong>memcached</strong>- opção de configuração relacionada. As seguintes opções de configuração são suportadas pelo<code>config_options</code>tabela:<div class="itemizedlist"> <ul class="itemizedlist" style="list-style-type: disc; "><li class="listitem"><p> <code>separator</code>: Usado para separar valores de uma longa cadeia em valores separados quando há vários<code>value_columns</code>definida. Por padrão, o<code>separator</code>é um<code>|</code>característica. Por exemplo, se você definir<code>col1, col2</code>como colunas de valor, e você define<code>|</code>como separador, você pode emitir o seguinte<strong>memcached</strong>comando para inserir valores em<code>col1</code>e<code>col2</code>, respectivamente:</p><pre class="programlisting copytoclipboard language-terminal"><code class="language-terminal">set keyx 10 0 19 valuecolx|valuecoly</code></pre><p> <code>valuecol1x</code>são armazenados em<code>col1</code>e<code>valuecoly</code>são armazenados em<code>col2</code>. </p></li><li class="listitem"><p> <code>table_map_delimiter</code>: O caractere que separa o nome do esquema e o nome da tabela quando você usa o<code>@@</code>A notação em um nome de chave para acessar uma chave em uma tabela específica. Por exemplo,<code>@@t1.some_key</code>e<code>@@t2.some_key</code>têm o mesmo valor chave, mas são armazenados em tabelas diferentes.</p></li></ul> </div> </td> </tr><tr> <td><code>Value</code></td> <td>O valor atribuído ao<strong>memcached</strong>- opção de configuração relacionada.</td> </tr></tbody></table>
...
* Você deve fornecer um valor para `db_schema`, `db_name`, `key_columns`, `value_columns` e `unique_idx_name_on_key`. Especifique `0` para `flags`, `cas_column` e `expire_time_column` se eles estiverem inutilizados. Não fazer isso pode fazer com que sua configuração falhe.
...
### 14.21.8 Solucionando problemas do plugin InnoDB memcached
...
* Para solucionar problemas em que o daemon **memcached** não consegue armazenar ou recuperar os dados da tabela `InnoDB`, codifique a opção **memcached** `-vvv` no parâmetro de configuração `daemon_memcached_option` no arquivo de configuração do MySQL. Examine o log de erro do MySQL para obter saída de depuração relacionada às operações do **memcached**.
...
Por exemplo, se você executar a declaração `INSTALL PLUGIN` antes de executar o script de configuração `innodb_memcached_config.sql` para configurar o banco de dados e as tabelas necessárias, o servidor pode sair inesperadamente e não iniciar. O servidor também pode não iniciar se você configurar incorretamente uma entrada na tabela `innodb_memcache.containers`.
