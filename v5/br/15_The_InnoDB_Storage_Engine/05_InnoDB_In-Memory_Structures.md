## 14.5 Estruturas de Memória Dinâmica InnoDB
...
* Idealmente, você define o tamanho do pool de buffer para um valor tão grande quanto possível, deixando memória suficiente para outros processos no servidor para funcionar sem paginação excessiva. Quanto maior o pool de buffer, mais o `InnoDB` age como um banco de dados em memória, lendo dados do disco uma vez e, em seguida, acessando os dados da memória durante leituras subsequentes. Veja a Seção 14.8.3.1, “Configurando o Tamanho do Pool de Buffer InnoDB”.

* Em sistemas de 64 bits com memória suficiente, você pode dividir o pool de buffers em várias partes para minimizar a disputa por estruturas de memória entre operações concorrentes. Para detalhes, consulte a Seção 14.8.3.2, “Configurando múltiplas instâncias de pool de buffers”.

* Você pode manter dados frequentemente acessados na memória, independentemente de picos repentinos de atividade de operações que levem grandes quantidades de dados raramente acessados ao pool de buffer. Para mais detalhes, consulte a Seção 14.8.3.3, “Tornando o Scan do Pool de Buffer Resistente”.

* Você pode controlar como e quando realizar solicitações de leitura antecipada para pré-carregar páginas no pool de buffer de forma assíncrona, antecipando que as páginas serão necessárias em breve. Para mais detalhes, consulte a Seção 14.8.3.4, “Configurando a pré-visualização do pool de buffer InnoDB (Leitura antecipada)”).

* Você pode controlar quando o esvaziamento de fundo ocorre e se a taxa de esvaziamento é ajustada dinamicamente com base na carga de trabalho. Para detalhes, consulte a Seção 14.8.3.5, “Configurando o esvaziamento do Pool de Buffer”.

* Você pode configurar como o `InnoDB` preserva o estado atual do pool de buffer para evitar um longo período de aquecimento após o reinício do servidor. Para detalhes, consulte a Seção 14.8.3.6, “Salvar e restaurar o estado do pool de buffer”.

#### Monitoramento do Pool de Buffer usando o Monitor Padrão InnoDB

A saída padrão do Monitor `InnoDB` pode ser acessada usando `SHOW ENGINE INNODB STATUS`, e fornece métricas sobre o funcionamento do pool de buffers. As métricas do pool de buffers estão localizadas na seção `BUFFER POOL AND MEMORY` da saída padrão do Monitor `InnoDB`:
...
**Tabela 14.2: Métricas do Banco de Armazenamento de Buffer InnoDB**
...
As variáveis de status do buffer pool e a tabela `INNODB_BUFFER_POOL_STATS` fornecem muitas das mesmas métricas do buffer pool encontradas na saída do Monitor Padrão `InnoDB`. Para mais informações, consulte o Exemplo 14.10, “Consultando a tabela INNODB_BUFFER_POOL_STATS”.
...
Para respostas a perguntas frequentes sobre o buffer de alterações, consulte a Seção A.16, “Perguntas frequentes sobre o MySQL 5.7: Buffer de alterações InnoDB”.
...
Os valores permitidos de `innodb_change_buffering` incluem:
...
Bloquear operações de exclusão física que ocorrem no segundo plano.
...
* `InnoDB` A saída padrão do monitor inclui informações sobre o estado do buffer de alteração. Para visualizar os dados do monitor, execute a declaração `SHOW ENGINE INNODB STATUS`.
...
Para mais informações, consulte a Seção 14.18.3, “Saída do Monitor Padrão InnoDB e do Monitor de Bloqueio”.
...
Para informações sobre o uso da tabela `INNODB_METRICS`, consulte a Seção 14.16.6, “Tabela de métricas do InnoDB INFORMATION_SCHEMA”.
...
Para informações sobre outros dados fornecidos pela tabela `INNODB_BUFFER_PAGE`, consulte a Seção 24.4.2, “A tabela INFORMATION_SCHEMA INNODB_BUFFER_PAGE”. Para informações sobre o uso relacionado, consulte a Seção 14.16.5, “Tabelas de buffer do pool de informações do InnoDB”.
...
Para obter informações sobre o monitoramento das esperas dos mutexes `InnoDB`, consulte a Seção 14.17.2, “Monitoramento das esperas dos mutexes InnoDB usando o Gerador de Desempenho”.

### 14.5.3 Índice Hash Adaptativo

O índice de hash adaptável permite que `InnoDB` funcionar mais como um banco de dados em memória em sistemas com combinações apropriadas de carga de trabalho e memória suficiente para o pool de buffers, sem sacrificar as características transacionais ou a confiabilidade. O índice de hash adaptável é ativado pela variável `innodb_adaptive_hash_index`, ou desativado na inicialização do servidor por `--skip-innodb-adaptive-hash-index`.
...
Para informações relacionadas, consulte Configuração de memória e Seção 8.5.4, “Otimização do registro InnoDB Redo”.