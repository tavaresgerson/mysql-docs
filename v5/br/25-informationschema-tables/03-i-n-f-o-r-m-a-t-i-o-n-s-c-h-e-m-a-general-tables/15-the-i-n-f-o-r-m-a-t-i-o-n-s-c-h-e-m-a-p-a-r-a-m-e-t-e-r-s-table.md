...
A tabela `PARAMETERS` fornece informações sobre *parameters* para *stored routines* (*stored procedures* e *stored functions*), e sobre valores de retorno para *stored functions*. A tabela `PARAMETERS` não inclui *built-in* (nativas) ou *loadable functions*. As informações do *parameter* são semelhantes ao conteúdo da coluna `param_list` na tabela `mysql.proc`.

A tabela `PARAMETERS` possui as seguintes colunas:
