...
A tabela `COLLATIONS` fornece informações sobre collations para cada character set.

A tabela `COLLATIONS` possui estas colunas:
...
Informações sobre Collation também estão disponíveis na instrução `SHOW COLLATION`. Consulte a Seção 13.7.5.4, “SHOW COLLATION Statement”. As seguintes instruções são equivalentes:
