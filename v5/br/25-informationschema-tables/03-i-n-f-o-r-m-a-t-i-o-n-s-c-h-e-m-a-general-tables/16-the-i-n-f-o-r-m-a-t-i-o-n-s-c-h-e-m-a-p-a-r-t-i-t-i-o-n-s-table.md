...
A tabela `PARTITIONS` fornece informações sobre as PARTITIONS de tabelas. Cada linha nesta tabela corresponde a uma PARTITION individual ou SUBPARTITION de uma tabela particionada. Para mais informações sobre particionamento de tabelas, consulte Capítulo 22, *Partitioning*.

A tabela `PARTITIONS` possui estas colunas:
...
* `SUBPARTITION_NAME`
  ...
  Se a linha da tabela `PARTITIONS` representar uma SUBPARTITION, o nome da SUBPARTITION; caso contrário, `NULL`.
...
* `PARTITION_METHOD`
  ...
  Um dos valores `RANGE`, `LIST`, `HASH`, `LINEAR HASH`, `KEY`, ou `LINEAR KEY`; ou seja, um dos tipos de Partitioning disponíveis, conforme discutido na Seção 22.2, “Partitioning Types”.

* `SUBPARTITION_METHOD`

  Um dos valores `HASH`, `LINEAR HASH`, `KEY`, ou `LINEAR KEY`; ou seja, um dos tipos de Subpartitioning disponíveis, conforme discutido na Seção 22.2.6, “Subpartitioning”.

* `PARTITION_EXPRESSION`

  A expressão para a função de Partitioning usada na instrução `CREATE TABLE` ou `ALTER TABLE` que criou o esquema de Partitioning atual da tabela.
  ...
  A coluna `PARTITION_EXPRESSION` em uma linha da tabela `PARTITIONS` para uma PARTITION desta tabela exibe `c1 + c2`, conforme mostrado aqui:
  ...
  Para uma tabela `NDB` que não está explicitamente particionada, esta coluna fica vazia. Para tabelas que utilizam outros Storage Engines e que não estão particionadas, esta coluna é `NULL`.
...
* `TABLE_ROWS`
  ...
  Para tabelas particionadas `InnoDB`, a contagem de linhas fornecida na coluna `TABLE_ROWS` é apenas um valor estimado usado na otimização de SQL, e pode nem sempre ser exato.

  Para tabelas `NDB`, você também pode obter esta informação usando o utilitário **ndb_desc**.
...
* `INDEX_LENGTH`
  ...
  Para PARTITIONS de tabelas `NDB`, independentemente de as tabelas usarem Partitioning implícito ou explícito, o valor da coluna `INDEX_LENGTH` é sempre 0. No entanto, você pode obter informações equivalentes usando o utilitário **ndb_desc**.
...
* `CHECK_TIME`
  ...
  Para tabelas particionadas `InnoDB`, o valor é sempre `NULL`.
...
* `NODEGROUP`
  ...
  Este é o nodegroup ao qual a PARTITION pertence. Para tabelas NDB Cluster, este é sempre `default`. Para tabelas particionadas usando Storage Engines diferentes de `NDB`, o valor também é `default`. Caso contrário, esta coluna fica vazia.
