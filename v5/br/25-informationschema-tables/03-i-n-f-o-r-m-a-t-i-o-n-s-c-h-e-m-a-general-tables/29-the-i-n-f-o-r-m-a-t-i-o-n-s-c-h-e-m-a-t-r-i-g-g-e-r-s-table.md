...
A tabela `TRIGGERS` fornece informações sobre triggers. Para visualizar informações sobre os triggers de uma tabela, você deve ter o privilégio `TRIGGER` para essa tabela.

A tabela `TRIGGERS` possui as seguintes colunas:
...
* `EVENT_OBJECT_CATALOG`, `EVENT_OBJECT_SCHEMA`, e `EVENT_OBJECT_TABLE`
  ...
  Como observado na Seção 23.3, “Using Triggers”, todo trigger está associado a exatamente uma tabela. Estas colunas indicam o catalog e o schema (Database) onde esta tabela ocorre, e o nome da tabela, respectivamente. O valor de `EVENT_OBJECT_CATALOG` é sempre `def`.
...
* `SQL_MODE`
  ...
  O SQL mode em vigor quando o trigger foi criado, e sob o qual o trigger é executado. Para os valores permitidos, consulte a Seção 5.1.10, “Server SQL Modes”.
...
O exemplo a seguir usa o trigger `ins_sum` definido na Seção 23.3, “Using Triggers”:
...
Informações sobre triggers também estão disponíveis através da instrução `SHOW TRIGGERS`. Consulte a Seção 13.7.5.38, “SHOW TRIGGERS Statement”.