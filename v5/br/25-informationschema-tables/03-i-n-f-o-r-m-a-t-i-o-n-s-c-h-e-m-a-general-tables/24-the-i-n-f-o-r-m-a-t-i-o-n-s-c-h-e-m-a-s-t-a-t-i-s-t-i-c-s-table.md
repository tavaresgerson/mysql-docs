...
A tabela `STATISTICS` fornece informações sobre Indexes de tabela.

A tabela `STATISTICS` possui as seguintes colunas:
...
* `SUB_PART`
  ...
  **Nota**
  ...
  Os *limites* de prefixo são medidos em bytes. No entanto, os *comprimentos* de prefixo para especificações de Index nas instruções `CREATE TABLE`, `ALTER TABLE` e `CREATE INDEX` são interpretados como o número de caracteres para tipos de string não binários (`CHAR`, `VARCHAR`, `TEXT`) e número de bytes para tipos de string binários (`BINARY`, `VARBINARY`, `BLOB`). Leve isso em consideração ao especificar um comprimento de prefixo para uma coluna de string não binária que usa um conjunto de caracteres multibyte.

  Para obter informações adicionais sobre prefixos de Index, consulte Section 8.3.4, “Column Indexes” e Section 13.1.14, “CREATE INDEX Statement”.
...
Informações sobre Indexes de tabela também estão disponíveis na instrução `SHOW INDEX`. Consulte Section 13.7.5.22, “SHOW INDEX Statement”. As seguintes instruções são equivalentes:
