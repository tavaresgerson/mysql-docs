...
A tabela `REFERENTIAL_CONSTRAINTS` fornece informações sobre Foreign Keys.

A tabela `REFERENTIAL_CONSTRAINTS` possui as seguintes colunas:
...
* `TABLE_NAME`
  ...
  O nome da Table. Este valor é o mesmo que na tabela `TABLE_CONSTRAINTS`.
