...
A tabela `EVENTS` fornece informações sobre os eventos do Event Manager, que são discutidos na Seção 23.4, “Utilizando o Event Scheduler”.
...
* `SQL_MODE`
  ...
  O SQL mode em vigor quando o evento foi criado ou alterado, e sob o qual o evento é executado. Para os valores permitidos, consulte Seção 5.1.10, “Server SQL Modes”.
...
* Os horários na tabela `EVENTS` são exibidos usando o fuso horário do evento, o fuso horário da sessão atual ou UTC, conforme descrito em Seção 23.4.4, “Metadados do Evento”.

* Para mais informações sobre `SLAVESIDE_DISABLED` e a coluna `ORIGINATOR`, consulte Seção 16.4.1.16, “Replication of Invoked Features”.
...
Informações do evento também estão disponíveis a partir da instrução `SHOW EVENTS`. Consulte Seção 13.7.5.18, “SHOW EVENTS Statement”. As seguintes instruções são equivalentes:
