...
A tabela `INNODB_BUFFER_PAGE` fornece informações sobre cada page no buffer pool do `InnoDB`.

Para informações de uso e exemplos relacionados, consulte Seção 14.16.5, “Tabelas do Buffer Pool do INFORMATION_SCHEMA do InnoDB”.

Aviso

Fazer Querys na tabela `INNODB_BUFFER_PAGE` pode afetar a performance. Não consulte esta tabela em um sistema de produção a menos que você esteja ciente do impacto na performance e o tenha determinado como aceitável. Para evitar impactar a performance em um sistema de produção, reproduza o problema que deseja investigar e consulte as estatísticas do buffer pool em uma instância de teste.

A tabela `INNODB_BUFFER_PAGE` possui as seguintes colunas:
