...
A tabela \`TP_THREAD_GROUP_STATE\` possui uma linha por Thread Group no Thread Pool. Cada linha fornece informações sobre o estado atual de um grupo.

A tabela \`TP_THREAD_GROUP_STATE\` possui as seguintes colunas:
...
* `STALL_LIMIT`
  ...
  O valor da variável de sistema `thread_pool_stall_limit` para o Thread Group. Este valor é o mesmo para todos os Thread Groups.

* `PRIO_KICKUP_TIMER`

  O valor da variável de sistema `thread_pool_prio_kickup_timer` para o Thread Group. Este valor é o mesmo para todos os Thread Groups.

* `ALGORITHM`

  O valor da variável de sistema `thread_pool_algorithm` para o Thread Group. Este valor é o mesmo para todos os Thread Groups.
...
* `MAX_THREAD_IDS_IN_GROUP`
  ...
  O ID máximo de Thread das Threads no grupo. Este é o mesmo que `MAX(TP_THREAD_NUMBER)` para as Threads quando selecionadas da tabela `TP_THREAD_STATE`. Ou seja, estas duas Queries são equivalentes:
