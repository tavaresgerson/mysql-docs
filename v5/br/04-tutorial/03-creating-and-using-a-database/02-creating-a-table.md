...
Vários tipos de valores podem ser escolhidos para representar o sexo nos registros de animais, como `'m'` e `'f'`, ou talvez `'masculino'` e `'feminino'`. É mais simples usar os caracteres únicos `'m'` e `'f'`.

O uso do tipo de dado `DATE` para as colunas `birth` e `death` é uma escolha bastante óbvia.

Depois de criar uma tabela, o comando `SHOW TABLES` deve exibir algum resultado:
...
Para obter mais informações sobre os tipos de dados do MySQL, consulte Capítulo 11, *Tipos de Dados*.
