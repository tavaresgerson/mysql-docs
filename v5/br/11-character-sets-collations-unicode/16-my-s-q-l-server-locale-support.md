...
O *locale* indicado pela variável de sistema `lc_time_names` controla o idioma usado para exibir nomes e abreviações de dias e meses. Esta variável afeta a saída das funções `DATE_FORMAT()`, `DAYNAME()` e `MONTHNAME()`.

`lc_time_names` não afeta as funções `STR_TO_DATE()` ou `GET_FORMAT()`.

O valor de `lc_time_names` não afeta o resultado de `FORMAT()`, mas esta função aceita um terceiro parâmetro opcional que permite que um *locale* seja especificado para ser usado no ponto decimal, separador de milhares e agrupamento entre separadores do número resultante. Os valores de *locale* permitidos são os mesmos que os valores válidos para a variável de sistema `lc_time_names`.

Nomes de *locale* possuem sub*tags* de idioma e região listadas pela IANA (<http://www.iana.org/assignments/language-subtag-registry>), como `'ja_JP'` ou `'pt_BR'`. O valor padrão é `'en_US'`, independentemente da configuração de *locale* do seu sistema, mas você pode definir o valor na inicialização do server, ou definir o valor `GLOBAL` em tempo de execução se você tiver privilégios suficientes para definir variáveis de sistema globais; consulte a Seção 5.1.8.1, “Privilégios de Variável de Sistema”. Qualquer cliente pode examinar o valor de `lc_time_names` ou definir seu valor `SESSION` para afetar o *locale* de sua própria conexão.
...
`lc_time_names` pode ser definida para qualquer um dos seguintes valores de *locale*. O conjunto de *locales* suportados pelo MySQL pode ser diferente daqueles suportados pelo seu sistema operacional.
