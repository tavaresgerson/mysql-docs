...
Veja a Seção 8.15.10, “Selecionando recursos do otimizador para rastrear”, para mais informações sobre os efeitos desses.

* `optimizer_trace_max_mem_size`: Quantidade máxima de memória que pode ser usada para armazenar todos os rastros.

* `optimizer_trace_limit`: O número máximo de rastros do otimizador a serem exibidos. Consulte a Seção 8.15.4, “Ajuste da Purga de Rastros”, para mais informações.
...
### 8.15.4  Ajuste da Purga de Rastras
...
* Um subsequente `SELECT` da tabela `OPTIMIZER_TRACE` retorna os primeiros rastros *`limit`* dos rastros mais antigos do *`offset`* (se *`offset`* ≥ 0), ou os primeiros rastros *`limit`* dos rastros mais recentes do *`-offset`* (se *`offset`* < 0).
...
Os valores negativos para *`offset`* podem, portanto, ser úteis quando os sub-instruções de interesse são os últimos em uma rotina armazenada. Por exemplo:
...
Usando `optimizer_trace_offset` e `optimizer_trace_limit`, que são restrições no nível do produtor de traços, oferece melhor (maior) velocidade e (menos) uso de memória do que definir desvios ou limites no nível do consumidor de traços (SQL) com `SELECT * FROM OPTIMIZER_TRACE LIMIT limit OFFSET offset`, que não economiza quase nada.
...
### 8.15.10 Selecionando recursos do otimizador para rastreamento
...
### 8.15.15 Rastreador do otimizador de testes
