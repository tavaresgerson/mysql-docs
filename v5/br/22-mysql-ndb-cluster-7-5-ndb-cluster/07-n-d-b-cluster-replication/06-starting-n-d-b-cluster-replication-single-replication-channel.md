...
1. Inicie o servidor Source de Replication MySQL emitindo este comando, onde *`id`* é o ID exclusivo deste servidor (veja Seção 21.7.2, “Requisitos Gerais para Replication do NDB Cluster”):
   ...
   Isso inicia o processo **mysqld** do servidor com o Binary Log ativado, usando o formato de logging apropriado.

   Note

   Você também pode iniciar o Source com `--binlog-format=MIXED`, caso em que a Replication baseada em linha (row-based replication) é usada automaticamente ao replicar entre Clusters. Statement-based binary logging não é suportado para Replication do NDB Cluster (veja Seção 21.7.2, “Requisitos Gerais para Replication do NDB Cluster”).
...
2. Inicie o servidor Replica MySQL conforme mostrado aqui:
   ...
   Você deve usar a opção `--skip-slave-start` com este comando ou deve incluir `skip-slave-start` no arquivo `my.cnf` do servidor Replica, a menos que você queira que a Replication comece imediatamente. Com o uso desta opção, o início da Replication é atrasado até que a instrução `START SLAVE` apropriada tenha sido emitida, conforme explicado na Etapa 4 abaixo.
...
4. Finalmente, instrua o Replica a começar a aplicar a Replication, emitindo este comando a partir do cliente **mysql** no Replica:
...
Também é possível usar dois canais de replication, de forma semelhante ao procedimento descrito na próxima seção; as diferenças entre este e o uso de um canal único de replication são abordadas em Seção 21.7.7, “Usando Dois Canais de Replication para Replication do NDB Cluster”.

Também é possível melhorar o performance da Replication do Cluster habilitando Batched Updates. Isso pode ser realizado configurando a variável de sistema `slave_allow_batching` nos processos **mysqld** dos Replicas. Normalmente, os Updates são aplicados assim que são recebidos. No entanto, o uso de Batching faz com que os Updates sejam aplicados em Batches de 32 KB cada; isso pode resultar em Throughput mais alto e menor uso de CPU, particularmente onde os Updates individuais são relativamente pequenos.
...
Você pode verificar se o Batching está sendo usado atualmente por meio de uma instrução `SHOW VARIABLES` apropriada, como esta:
