...
Configurar a replicação com dois canais não é radicalmente diferente de configurar um único canal de replicação. Primeiro, os processos **mysqld** para os servidores source de replicação primária e secundária devem ser iniciados, seguidos pelos das replicas primárias e secundárias. Os processos de replicação podem ser iniciados emitindo o Statement `START SLAVE` em cada uma das replicas. Os comandos e a ordem em que devem ser emitidos são mostrados aqui:
...
5. Finalmente, inicie a replicação no canal primário executando o Statement `START SLAVE` na replica primária, conforme mostrado aqui:
