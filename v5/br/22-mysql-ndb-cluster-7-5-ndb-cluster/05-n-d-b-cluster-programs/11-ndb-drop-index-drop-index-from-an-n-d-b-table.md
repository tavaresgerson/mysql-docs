...
**ndb_drop_index** remove o Index especificado de uma Table `NDB`. *É recomendado que você use este utilitário apenas como um exemplo para escrever aplicações NDB API* — veja o Aviso (Warning) mais adiante nesta seção para detalhes.
...
As opções que podem ser usadas com **ndb_drop_index** são mostradas na tabela a seguir. Descrições adicionais seguem a tabela.
...
Nesses casos, sua *única* opção para tornar a Table disponível ao MySQL novamente é remover a Table e recriá-la. Você pode usar tanto a instrução SQL `DROP TABLE` quanto o utilitário **ndb_drop_table** (veja Section 21.5.12, “ndb_drop_table — Remove uma Table NDB”) para remover a Table.