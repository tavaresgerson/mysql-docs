...
* Valores mínimos para os parâmetros de configuração `DataMemory`, `IndexMemory`, `MaxNoOfTables`, `MaxNoOfAttributes`, `MaxNoOfOrderedIndexes` e `MaxNoOfTriggers` necessários para acomodar as Tables analisadas.

* Requisitos de memória para todas as Tables, attributes, Ordered Indexes e Unique Hash Indexes definidos no Database.

* O `IndexMemory` e `DataMemory` exigidos por Table e por linha de Table (table row).