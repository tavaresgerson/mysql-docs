...
**ndb_waiter** imprime repetidamente (a cada 100 milissegundos) o status de todos os Data Nodes do cluster até que o cluster alcance um status determinado ou o limite de `--timeout` seja excedido, e então é encerrado. Por padrão, ele aguarda que o cluster atinja o status `STARTED`, no qual todos os Nodes foram iniciados e se conectaram ao cluster. Isso pode ser sobrescrito usando as opções `--no-contact` e `--not-started`.

Os estados de Node relatados por esta utilidade são os seguintes:

* `NO_CONTACT`: O Node não pode ser contatado.
* `UNKNOWN`: O Node pode ser contatado, mas seu status ainda é desconhecido. Geralmente, isso significa que o Node recebeu um comando `START` ou `RESTART` do Management Server, mas ainda não agiu sobre ele.
...
* `STARTING`: O processo **ndbd** do Node foi iniciado, mas o Node ainda não se juntou ao cluster.
...
As opções que podem ser usadas com **ndb_waiter** são mostradas na tabela a seguir. Descrições adicionais seguem a tabela.
...
* `--no-contact`, `-n`
  ...
  Em vez de aguardar o estado `STARTED`, **ndb_waiter** continua em execução até que o cluster atinja o status `NO_CONTACT` antes de ser encerrado.
...
* `--not-started`
  ...
  Em vez de aguardar o estado `STARTED`, **ndb_waiter** continua em execução até que o cluster atinja o status `NOT_STARTED` antes de ser encerrado.

* `--nowait-nodes=list`

  Quando esta opção é usada, **ndb_waiter** não aguarda pelos Nodes cujos IDs estão listados. A lista é delimitada por vírgulas; *ranges* podem ser indicados por hífens, conforme mostrado aqui:
...
* `--wait-nodes=list`, `-w list`
  ...
  Quando esta opção é usada, **ndb_waiter** aguarda apenas pelos Nodes cujos IDs estão listados. A lista é delimitada por vírgulas; *ranges* podem ser indicados por hífens, conforme mostrado aqui:
...
**Exemplo de Saída.** É mostrada aqui a saída de **ndb_waiter** quando executado contra um cluster de 4 Nodes no qual dois Nodes foram encerrados e depois iniciados novamente de forma manual. Relatórios duplicados (indicados por `...`) são omitidos.
...
Se nenhuma Connect String for especificada, então **ndb_waiter** tenta se conectar a um Management Server em `localhost` e relata `Connecting to mgmsrv at (null)`.
