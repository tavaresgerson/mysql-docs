...
**ndb_perror** exibe informações sobre um erro NDB, dado o seu Error Code. Isso inclui a mensagem de erro, o tipo de erro e se o erro é permanente ou temporário. Adicionado à distribuição do MySQL NDB Cluster no NDB 7.6, ele se destina a ser um substituto direto (drop-in replacement) para **perror** `--ndb`.
...
**ndb_perror** não precisa acessar um NDB Cluster em execução, nem quaisquer nodes (incluindo nodes SQL). Para visualizar informações sobre um determinado erro NDB, invoque o programa, usando o Error Code como argumento, assim:
...
Para exibir apenas a mensagem de erro, invoque **ndb_perror** com a Option `--silent` (forma abreviada `-s`), conforme mostrado aqui:
...
Assim como **perror**, **ndb_perror** aceita múltiplos Error Codes:
...
Options de programa adicionais para **ndb_perror** são descritas mais adiante nesta seção.

**ndb_perror** substitui **perror** `--ndb`, que está deprecated no NDB 7.6 e sujeito à remoção em um futuro release do MySQL NDB Cluster. Para facilitar a substituição em scripts e outros aplicativos que possam depender de **perror** para obter informações de erro NDB, **ndb_perror** suporta sua própria Option "dummy" (fictícia) `--ndb`, que não faz nada.

A tabela a seguir inclui todas as options que são específicas do programa NDB Cluster **ndb_perror**. Descrições adicionais seguem a tabela.
...
* `--ndb`
  ...
  Para compatibilidade com aplicativos que dependem de versões antigas do **perror** que usam a Option `--ndb` desse programa. A Option, quando usada com **ndb_perror**, não faz nada e é ignorada por ele.
...
* `--verbose`, `-v`
  ...
  Verbose output; desabilite com `--silent`.