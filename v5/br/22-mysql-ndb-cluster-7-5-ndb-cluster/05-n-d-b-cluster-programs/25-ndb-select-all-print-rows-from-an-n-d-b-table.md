...
**ndb_select_all** imprime todas as linhas de uma tabela `NDB` para o `stdout`.
...
As opções que podem ser usadas com **ndb_select_all** são mostradas na tabela a seguir. Descrições adicionais seguem a tabela.
...
| `--lock=#`, ` -l # ` | Tipo de Lock | (Suportado em todas as versões NDB baseadas no MySQL 5.7) |
...
| `--parallelism=#`, ` -p # ` | Grau de paralelismo (Degree of parallelism) | (Suportado em todas as versões NDB baseadas no MySQL 5.7) |
...
* `--descending`, `-z`
  ...
  Ordena a saída (output) em ordem decrescente. Esta opção só pode ser usada em conjunto com a opção `-o` (`--order`).
...
* `--gci`
  ...
  Adiciona uma coluna `GCI` à saída (output) mostrando o ponto de verificação global (global checkpoint) no qual cada linha foi atualizada pela última vez. Consulte Section 21.2, “NDB Cluster Overview”, e Section 21.6.3.2, “NDB Cluster Log Events”, para mais informações sobre pontos de verificação (checkpoints).
...
* `--usage`
  ...
  | Formato de Linha de Comando | ` --character-sets-dir=path ` |
  |---|---|
  Exibe o texto de ajuda (help) e sai; o mesmo que `--help`.
...
Saída (Output) de uma instrução MySQL `SELECT`:
...
Saída (Output) da invocação equivalente de **ndb_select_all**:
...
Todos os valores de string são delimitados por colchetes (`[`...`]`) na saída de **ndb_select_all**. Para outro exemplo, considere a tabela criada e preenchida conforme mostrado aqui:
