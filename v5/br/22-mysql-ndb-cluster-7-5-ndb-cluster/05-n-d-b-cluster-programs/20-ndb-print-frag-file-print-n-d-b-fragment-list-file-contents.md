...
**ndb_print_frag_file** obtém informações de um arquivo de lista de fragmentos do Cluster. Ele é destinado a auxiliar no diagnóstico de problemas com reinicializações de data node.
...
Assim como **ndb_print_backup_file**, **ndb_print_sys_file**, e **ndb_print_schema_file** (e, diferentemente da maioria dos outros utilitários `NDB` que se destinam a serem executados em um host de management server ou a se conectar a um management server), **ndb_print_frag_file** deve ser executado em um data node do Cluster, uma vez que acessa o file system do data node diretamente. Como ele não utiliza o management server, este utilitário pode ser usado quando o management server não está em execução, e mesmo quando o Cluster foi completamente encerrado.
