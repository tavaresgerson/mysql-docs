...
**ndb_drop_table** remove a table `NDB` especificada. (Se você tentar usar isso em uma table criada com um storage engine diferente de `NDB`, a tentativa falhará com o erro 723: No such table exists.) Esta operação é extremamente rápida; em alguns casos, pode ser uma ordem de magnitude mais rápida do que usar uma instrução MySQL `DROP TABLE` em uma table `NDB`.
...
As Options que podem ser usadas com **ndb_drop_table** são mostradas na tabela a seguir. Descrições adicionais seguem a tabela.
