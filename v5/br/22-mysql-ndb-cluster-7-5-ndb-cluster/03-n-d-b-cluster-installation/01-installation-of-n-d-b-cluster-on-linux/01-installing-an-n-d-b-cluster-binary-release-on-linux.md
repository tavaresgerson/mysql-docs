...
Se você precisar de um **binary** personalizado, consulte Seção 2.8.5, “Instalando o MySQL Usando uma Source Tree de Desenvolvimento”.

Nota

Após completar a instalação, não inicie nenhum dos **binaries** ainda. Mostraremos como fazer isso após a configuração dos **nodes** (consulte Seção 21.3.3, “Configuração Inicial do NDB Cluster”).
...
3. Mude o local para o diretório `mysql` e configure os **databases** do sistema usando **mysqld** `--initialize` conforme mostrado aqui:
   ...
   Isso gera uma senha aleatória para a conta `root` do MySQL. Se você *não* quiser que a senha aleatória seja gerada, você pode substituir a opção `--initialize` pela opção `--initialize-insecure`. Em ambos os casos, você deve revisar Seção 2.9.1, “Inicializando o Data Directory”, para obter informações adicionais antes de executar esta etapa. Consulte também Seção 4.4.4, “mysql_secure_installation — Melhore a Segurança da Instalação do MySQL”.
...
**Data nodes.** A instalação dos **data nodes** não requer o **binary** **mysqld**. Apenas o executável do **data node** do NDB **Cluster** **ndbd** (**single-threaded**) ou **ndbmtd**") (**multithreaded**) é requerido. Estes **binaries** também podem ser encontrados no arquivo `.tar.gz`. Novamente, assumimos que você colocou este arquivo em `/var/tmp`.
...
1. Mude o local para o diretório `/var/tmp` e extraia os **binaries** **ndbd** e **ndbmtd**") do arquivo para um diretório adequado, como `/usr/local/bin`:
   ...
   (Você pode excluir com segurança o diretório criado ao desempacotar o arquivo baixado, e os arquivos que ele contém, de `/var/tmp` assim que **ndb_mgm** e **ndb_mgmd** tiverem sido copiados para o diretório de executáveis.)
...
Embora apenas um dos executáveis do **data node** seja necessário para executar um **data node** do NDB **Cluster**, mostramos como instalar tanto **ndbd** quanto **ndbmtd**") nas instruções precedentes. Recomendamos que você faça isso ao instalar ou atualizar o NDB **Cluster**, mesmo que planeje usar apenas um deles, pois isso deve economizar tempo e problemas caso decida mudar de um para o outro posteriormente.

Nota

O **data directory** em cada máquina que hospeda um **data node** é `/usr/local/mysql/data`. Esta informação é essencial ao configurar o **management node**. (Consulte Seção 21.3.3, “Configuração Inicial do NDB Cluster”.)

**Management nodes.** A instalação do **management node** não requer o **binary** **mysqld**. Apenas o **management server** do NDB **Cluster** (**ndb_mgmd**) é requerido; você provavelmente também desejará instalar o **management client** (**ndb_mgm**). Ambos os **binaries** também podem ser encontrados no arquivo `.tar.gz`. Novamente, assumimos que você colocou este arquivo em `/var/tmp`.

Como `root` do sistema, execute os seguintes passos para instalar **ndb_mgmd** e **ndb_mgm** no **host** do **management node**:

1. Mude o local para o diretório `/var/tmp` e extraia **ndb_mgm** e **ndb_mgmd** do arquivo para um diretório adequado, como `/usr/local/bin`:
...
Em Seção 21.3.3, “Configuração Inicial do NDB Cluster”, criamos arquivos de configuração para todos os **nodes** em nosso exemplo de NDB **Cluster**.