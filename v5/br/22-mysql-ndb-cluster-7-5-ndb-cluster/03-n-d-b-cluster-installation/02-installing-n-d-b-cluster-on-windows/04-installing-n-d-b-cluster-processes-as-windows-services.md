...
Para instalar o management node como um Service no Windows, invoque **ndb_mgmd.exe** a partir da Command Line na máquina que hospeda o management node, usando a opção `--install`, conforme mostrado aqui:
...
A opção `--install` deve ser usada primeiro, antes de quaisquer outras opções que possam ser especificadas para **ndb_mgmd.exe**. No entanto, é preferível especificar tais opções em um options file. Se o seu options file não estiver em um dos locais padrão, conforme mostrado na saída de **ndb_mgmd.exe** `--help`, você pode especificar o local usando a opção `--config-file`.
...
Alternativamente, invoque **ndb_mgmd.exe** com a opção `--remove`, conforme mostrado aqui:
...
Se você instalou o Service usando um nome de Service diferente do padrão, passe o nome do Service como valor da opção `--remove` de **ndb_mgmd.exe**, desta forma:
...
A instalação de um processo data node do NDB Cluster como um Windows Service pode ser feita de maneira semelhante, usando a opção `--install` para **ndbd.exe** (ou **ndbmtd.exe**")), conforme mostrado aqui:
...
Assim como com **ndb_mgmd.exe** (e **mysqld.exe**), ao instalar **ndbd.exe** como um Windows Service, você também pode especificar um nome para o Service como valor de `--install` e então usá-lo ao iniciar ou parar o Service, desta forma:
...
Alternativamente, você pode passar o nome do Service como valor da opção `--remove` de `ndbd.exe`, conforme mostrado aqui:
...
A instalação do SQL node como um Windows Service, iniciar o Service, parar o Service e remover o Service são feitos de maneira semelhante, usando **mysqld** `--install`, **SC START**, **SC STOP** e **SC DELETE** (ou **mysqld** `--remove`). Comandos **NET** também podem ser usados para iniciar ou parar um Service. Para informações adicionais, consulte Section 2.3.4.8, “Starting MySQL as a Windows Service”.