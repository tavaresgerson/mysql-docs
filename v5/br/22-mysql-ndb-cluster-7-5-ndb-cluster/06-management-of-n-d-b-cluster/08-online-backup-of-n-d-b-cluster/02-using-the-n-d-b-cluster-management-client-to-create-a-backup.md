...
Antes de iniciar um backup, certifique-se de que o cluster esteja configurado corretamente para realizar um. (Consulte Seção 21.6.8.3, “Configuração para Backups do NDB Cluster”.)
...
Se você usar a opção `SNAPSHOTSTART` com `START BACKUP`, e o parâmetro `CompressedBackup` estiver habilitado, apenas os arquivos de data e control são compactados — o log file não é compactado.
...
1. Inicie o management client (**ndb_mgm**), se ainda não estiver em execução.
...
Também é possível realizar um backup a partir do system shell invocando **ndb_mgm** com a opção `-e` ou `--execute`, conforme mostrado neste exemplo:
...
Backups do Cluster são criados por padrão no subdiretório `BACKUP` do `DataDir` em cada data node. Isso pode ser sobrescrito para um ou mais data nodes individualmente, ou para todos os data nodes do cluster no arquivo `config.ini` usando o parâmetro de configuração `BackupDataDir`. Os arquivos de backup criados para um backup com um determinado *`backup_id`* são armazenados em um subdiretório chamado `BACKUP-backup_id` no diretório de backup.
