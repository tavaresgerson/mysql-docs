...
A tabela `dict_obj_info` fornece informações sobre objetos do dicionário de dados do `NDB` (`DICT`), como tables e indexes. (A tabela `dict_obj_types` pode ser consultada (Query) para obter uma lista de todos os types.) Esta informação inclui o type, o state, o parent object (se houver) e o nome totalmente qualificado do objeto.
