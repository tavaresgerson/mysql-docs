...
Consultar esta tabela fornece informações semelhantes às fornecidas pelo comando `ALL REPORT MemoryUsage` no cliente **ndb_mgm**, ou registradas por `ALL DUMP 1000`.
...
Suponha também que o valor do parâmetro de configuração `LongMessageBuffer` possa assumir seu valor padrão (64 MB).
...
Neste caso, os valores da coluna `total` para index memory são ligeiramente superiores ao valor definido em `IndexMemory` devido a arredondamentos internos.

Para as colunas `used_pages` e `total_pages`, os recursos são medidos em pages, que têm um tamanho de 32K para `DataMemory` e 8K para `IndexMemory`. Para long message buffer memory, o tamanho da page é de 256 bytes.