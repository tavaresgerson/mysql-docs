...
A coluna `mysql_connection_id` mostra o ID de Connection ou Thread do MySQL, conforme exibido por `SHOW PROCESSLIST`.
...
O `tableid` é atribuído à Table pelo `NDB`; o mesmo ID é usado para esta Table em outras tabelas `ndbinfo`, bem como na saída de **ndb_show_tables**.
