...
A tabela `locks_per_fragment` fornece informações sobre a contagem de solicitações de reivindicação de Lock e os resultados dessas solicitações por fragmento (per-fragment basis), servindo como uma tabela complementar a `operations_per_fragment` e `memory_per_fragment`. Esta tabela também mostra o tempo total gasto esperando por Locks com sucesso e sem sucesso desde a criação do fragmento ou da tabela, ou desde o reinício mais recente.
...
`table_id` é o ID interno da tabela gerado pelo `NDB`. Este é o mesmo ID interno da tabela mostrado em outras tabelas `ndbinfo`; também está visível na saída de **ndb_show_tables**.
