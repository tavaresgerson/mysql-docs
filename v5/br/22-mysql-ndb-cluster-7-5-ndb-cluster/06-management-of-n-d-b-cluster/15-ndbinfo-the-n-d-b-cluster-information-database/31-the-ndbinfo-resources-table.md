...
* `FILE_BUFFERS`: Usado pelo redo log handler no Kernel Block \`DBLQH\`; não pode ser overallocated. O tamanho é \`NoOfFragmentLogParts\` \* \`RedoBuffer\`, mais 1 MB por log file part.

* `TRANSPORTER_BUFFERS`: Usado para send buffers por **ndbmtd**"); é a soma de \`TotalSendBufferMemory\` e \`ExtraSendBufferMemory\`. Este Resource pode ser overallocated em até 25 por cento. O \`TotalSendBufferMemory\` é calculado somando a send buffer memory por node, cujo valor default é 2 MB. Assim, em um sistema com quatro data nodes e oito API nodes, os data nodes têm 12 * 2 MB de send buffer memory. O \`ExtraSendBufferMemory\` é usado por **ndbmtd**") e equivale a 2 MB de Memory extra por Thread. Assim, com 4 LDM threads, 2 TC threads, 1 main thread, 1 replication thread e 2 receive threads, o \`ExtraSendBufferMemory\` é 10 * 2 MB. A overallocation deste Resource pode ser realizada configurando o parâmetro de configuração do data node \`SharedGlobalMemory\`.

* `DISK_PAGE_BUFFER`: Usado para o disk page buffer; determinado pelo parâmetro de configuração \`DiskPageBufferMemory\`. Não pode ser overallocated.

* `QUERY_MEMORY`: Usado pelo Kernel Block \`DBSPJ\`.
