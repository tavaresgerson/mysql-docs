...
O valor de `block_name` nesta tabela é um dos valores encontrados na coluna `block_name` ao selecionar da tabela `ndbinfo.blocks`. Embora a lista de valores possíveis seja estática para um determinado *release* do NDB Cluster, a lista pode variar entre *releases*.
