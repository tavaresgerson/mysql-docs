...
O ID do `node` e o ID do grupo de `node` são os mesmos reportados por **ndb_mgm -e "SHOW"**.
...
O `president` node é o `node` visto pelo `node` atual como responsável por definir um `arbitrator` (consulte NDB Cluster Start Phases). Se o `president` falhar ou for desconectado, o `node` atual espera que o `node` cujo ID é mostrado na coluna `successor` se torne o novo `president`. A coluna `succession_order` mostra o lugar que o `node` atual se vê ocupando na fila de sucessão.
...
As colunas `connected_rank1_arbs` e `connected_rank2_arbs` exibem, cada uma, uma lista de 0 ou mais `arbitrators` que possuem um `ArbitrationRank` igual a 1, ou a 2, respectivamente.
