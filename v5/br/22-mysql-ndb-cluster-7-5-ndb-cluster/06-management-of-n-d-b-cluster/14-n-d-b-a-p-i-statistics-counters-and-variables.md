...
Uma série de tipos de contadores estatísticos relacionados a ações executadas por ou que afetam objetos `Ndb` estão disponíveis. Tais ações incluem iniciar e fechar (ou abortar) Transactions; operações de Primary Key e Unique Key; Table, Range e Pruned Scans; Threads bloqueadas enquanto aguardam a conclusão de várias operações; e dados e Events enviados e recebidos pelo `NDBCLUSTER`. Os contadores são incrementados dentro do kernel NDB sempre que chamadas da API NDB são feitas ou dados são enviados ou recebidos pelos Data Nodes. O **mysqld** expõe esses contadores como System Status Variables; seus valores podem ser lidos na saída do `SHOW STATUS`, ou consultando as tabelas `SESSION_STATUS` ou `GLOBAL_STATUS` do Information Schema. Comparando os valores antes e depois de Statements que operam em tabelas `NDB`, você pode observar as ações correspondentes realizadas no nível da API e, portanto, o custo de execução do Statement.

Você pode listar todas essas Status Variables usando o seguinte Statement `SHOW STATUS`:
...
Essas Status Variables também estão disponíveis nas tabelas `SESSION_STATUS` e `GLOBAL_STATUS` do Database `INFORMATION_SCHEMA`, conforme mostrado aqui:
...
Cada objeto `Ndb` possui seus próprios contadores. Aplicações da API NDB podem ler os valores dos contadores para uso em otimização ou monitoramento. Para Clientes multithreaded que usam mais de um objeto `Ndb` concorrentemente, também é possível obter uma visualização somada dos contadores de todos os objetos `Ndb` pertencentes a uma determinada `Ndb_cluster_connection`.

Quatro conjuntos desses contadores são expostos. Um conjunto se aplica apenas à Session atual; os outros 3 são Global. *Isto ocorre apesar do fato de que seus valores podem ser obtidos como Status Variables de Session ou Global no Cliente **mysql***. Isso significa que especificar a palavra-chave `SESSION` ou `GLOBAL` com `SHOW STATUS` não tem efeito sobre os valores reportados para as Status Variables de estatísticas da API NDB, e o valor para cada uma dessas variáveis é o mesmo, seja o valor obtido da coluna equivalente da tabela `SESSION_STATUS` ou da tabela `GLOBAL_STATUS`.

* *Session counters (específicos da Session)*

  Contadores de Session se relacionam aos objetos `Ndb` em uso (apenas) pela Session atual. O uso de tais objetos por outros Clientes MySQL não influencia essas contagens.
...
* *Replica counters (Global)*
  ...
  Este conjunto de contadores se relaciona aos objetos `Ndb` usados pelo Thread SQL do Replica, se houver. Se este **mysqld** não atua como um Replica, ou não usa tabelas `NDB`, todos esses contadores são 0.
...
* *Injector counters (Global)*
  ...
  Contadores do Injector se relacionam ao objeto `Ndb` usado para escutar Events do Cluster pelo Thread Binary Log Injector. Mesmo quando não está escrevendo um Binary Log, processos **mysqld** anexados a um NDB Cluster continuam a escutar alguns Events, como mudanças de Schema.
...
* *Server (Global) counters (Global)*
  ...
  Este conjunto de contadores se relaciona a todos os objetos `Ndb` atualmente usados por este **mysqld**. Isso inclui todas as aplicações Cliente MySQL, o Thread SQL do Replica (se houver), o binlog injector e o Thread de utilidade `NDB`.

  Nós nos referimos às Status Variables que correspondem a esses contadores como “variáveis Global” ou “variáveis de nível **mysqld**”.
...
Para obter uma listagem das Status Variables de nível **mysqld** da API NDB, filtre por nomes de variáveis que começam com `ndb_api` e terminam em `_count`, assim:
...
Nem todos os contadores são refletidos nos 4 conjuntos de Status Variables. Para os contadores de Event `DataEventsRecvdCount`, `NondataEventsRecvdCount` e `EventBytesRecvdCount`, apenas as Status Variables da API NDB `_injector` e de nível **mysqld** estão disponíveis:
...
Para ver todas as contagens de Transactions committed — ou seja, todas as Status Variables do contador `TransCommitCount` — você pode filtrar os resultados de `SHOW STATUS` pela substring `trans_commit_count`, assim:
...
Com isso, você pode determinar que 1 Transaction foi committed na Session atual do Cliente **mysql**, e 2 Transactions foram committed neste **mysqld** desde a última vez que foi reiniciado.

Você pode ver como vários contadores da API NDB são incrementados por um determinado Statement SQL, comparando os valores das Status Variables `_session` correspondentes imediatamente antes e depois de executar o Statement. Neste exemplo, depois de obter os valores iniciais de `SHOW STATUS`, criamos no Database `test` uma tabela `NDB`, chamada `t`, que possui uma única coluna:
...
Agora você pode executar um novo Statement `SHOW STATUS` e observar as mudanças, conforme mostrado aqui (com as linhas alteradas destacadas na saída):
...
Da mesma forma, você pode ver as alterações nos contadores de estatísticas da API NDB causadas pela inserção de uma linha em `t`: Insira a linha e, em seguida, execute o mesmo Statement `SHOW STATUS` usado no exemplo anterior, conforme mostrado aqui:
...
* Embora tenhamos criado `t` sem uma Primary Key explícita, 5 operações de Primary Key foram executadas ao fazê-lo (a diferença nos valores “antes” e “depois” de `Ndb_api_pk_op_count_session`, ou 6 menos 1). Isso reflete a criação da Primary Key oculta que é uma característica de todas as tabelas que usam o Storage Engine `NDB`.

* Comparando valores sucessivos para `Ndb_api_wait_nanos_count_session`, podemos ver que as operações da API NDB que implementam o Statement `CREATE TABLE` esperaram muito mais tempo (706871709 - 820705 = 706051004 nanosegundos, ou aproximadamente 0,7 segundo) por respostas dos Data Nodes do que aquelas executadas pelo `INSERT` (707370418 - 706871709 = 498709 ns ou aproximadamente 0,0005 segundo). Os tempos de execução relatados para esses Statements no Cliente **mysql** correlacionam-se aproximadamente com esses números.

  Em plataformas sem resolução de tempo (nanosegundo) suficiente, pequenas mudanças no valor do contador NDB API `WaitNanosCount` devido a Statements SQL que são executados muito rapidamente podem nem sempre ser visíveis nos valores de `Ndb_api_wait_nanos_count_session`, `Ndb_api_wait_nanos_count_slave`, ou `Ndb_api_wait_nanos_count`.

* O Statement `INSERT` incrementou ambos os contadores de estatísticas da API NDB `ReadRowCount` e `TransLocalReadRowCount`, conforme refletido pelos valores aumentados de `Ndb_api_read_row_count_session` e `Ndb_api_trans_local_read_row_count_session`.
