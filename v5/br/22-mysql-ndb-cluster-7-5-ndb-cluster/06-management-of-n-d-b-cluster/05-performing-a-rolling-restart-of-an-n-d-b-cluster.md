...
*   Iniciar cada processo de *data node* (**ndbd** ou possivelmente **ndbmtd**")) com a opção `--initial`, o que força o *data node* a limpar seu sistema de arquivos e recarregar todos os *data* e *metadata* do NDB Cluster dos outros *data nodes*.

*   Criar um *backup* usando o comando `START BACKUP` do cliente **ndb_mgm** antes de realizar o *restart*. Após o *upgrade*, restaurar o *node* ou *nodes* usando **ndb_restore**.

    Veja Section 21.6.8, “Online Backup of NDB Cluster”, e Section 21.5.24, “ndb_restore — Restore an NDB Cluster Backup”, para mais informações.

*   Usar **mysqldump** para criar um *backup* antes do *upgrade*; posteriormente, restaurar o *dump* usando `LOAD DATA`.

**Recuperação de Recursos (*Resource Recovery*).** Para liberar a memória previamente alocada a uma *table* por sucessivas operações `INSERT` e `DELETE`, para reutilização por outras *tables* do NDB Cluster.

O processo para realizar um *rolling restart* pode ser generalizado da seguinte forma:

1.  Parar todos os *management nodes* do *Cluster* (processos **ndb_mgmd**), reconfigurá-los e, em seguida, reiniciá-los. (Veja Rolling restarts with multiple management servers.)

2.  Parar, reconfigurar e, em seguida, reiniciar cada *data node* do *Cluster* (processo **ndbd**) em sequência.

    Alguns parâmetros de *configuration* do *node* podem ser atualizados emitindo `RESTART` para cada um dos *data nodes* no cliente **ndb_mgm** após a etapa anterior. Outros parâmetros exigem que o *data node* seja parado completamente usando o comando `STOP` do cliente de gerenciamento, e depois iniciado novamente a partir de um *shell* do sistema, invocando o executável **ndbd** ou **ndbmtd**") conforme apropriado. (Um comando de *shell* como **kill** também pode ser usado na maioria dos sistemas Unix para parar um processo de *data node*, mas o comando `STOP` é preferível e geralmente mais simples.)

    Note

    No Windows, você também pode usar os comandos **SC STOP** e **SC START**, os comandos `NET STOP` e `NET START`, ou o Windows Service Manager para parar e iniciar *nodes* que foram instalados como *Windows services* (veja Section 21.3.2.4, “Installing NDB Cluster Processes as Windows Services”).

    O tipo de *restart* necessário é indicado na documentação para cada parâmetro de *configuration* do *node*. Veja Section 21.4.3, “NDB Cluster Configuration Files”.

3.  Parar, reconfigurar e, em seguida, reiniciar cada *SQL node* do *Cluster* (processo **mysqld**) em sequência.
...
Ao realizar um *rolling restart* de um NDB Cluster com múltiplos *management nodes*, você deve ter em mente que **ndb_mgmd** verifica se algum outro *management node* está em execução e, se estiver, tenta usar os dados de *configuration* desse *node*. Para evitar que isso ocorra, e para forçar o **ndb_mgmd** a reler seu arquivo de *configuration*, execute as seguintes etapas:

1.  Parar todos os processos **ndb_mgmd** do NDB Cluster.
...
4.  Se você iniciou o primeiro **ndb_mgmd** com a opção `--initial`, você também deve iniciar todos os processos **ndb_mgmd** restantes usando `--initial`.

    Independentemente de quaisquer outras opções usadas ao iniciar o primeiro **ndb_mgmd**, você não deve iniciar nenhum processo **ndb_mgmd** restante após o primeiro usando `--reload`.

5.  Concluir os *rolling restarts* dos *data nodes* e *API nodes* normalmente.

Ao realizar um *rolling restart* para atualizar a *configuration* do *Cluster*, você pode usar a coluna `config_generation` da *table* `ndbinfo.nodes` para monitorar quais *data nodes* foram reiniciados com sucesso com a nova *configuration*. Veja Section 21.6.15.28, “The ndbinfo nodes Table”.
