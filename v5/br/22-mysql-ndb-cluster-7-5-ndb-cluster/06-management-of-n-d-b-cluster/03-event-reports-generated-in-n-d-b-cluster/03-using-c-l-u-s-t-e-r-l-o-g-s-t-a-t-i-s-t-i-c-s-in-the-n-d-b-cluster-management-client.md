...
Você pode determinar qual método de seleção de TC é usado para transações iniciadas a partir de um dado *SQL node* usando a variável de sistema `ndb_optimized_node_selection`.
...
* **Local reads.** Este é o número de operações de Primary Key read realizadas usando um *transaction coordinator* em um *node* que também contém a réplica de fragmento primário do registro. Essa contagem também pode ser obtida a partir do contador `LOCAL_READS` na tabela `ndbinfo.counters`.

* **Local writes.** Isto contém o número de operações de Primary Key read que foram realizadas usando um *transaction coordinator* em um *node* que também contém a réplica de fragmento primário do registro. Essa contagem também pode ser obtida a partir do contador `LOCAL_WRITES` na tabela `ndbinfo.counters`.
...
Além das estatísticas relatadas pelo *transaction coordinator* e *local query handler*, cada processo **ndbd** possui um *scheduler* que também fornece métricas úteis relacionadas ao desempenho de um NDB Cluster. Este *scheduler* é executado em um loop infinito; durante cada loop, o *scheduler* realiza as seguintes tarefas:
...
Para fazer com que todas as estatísticas do *cluster log* sejam registradas, você pode usar o seguinte comando no cliente de gerenciamento `NDB`:
...
Para mais informações sobre os comandos do cliente de gerenciamento NDB Cluster relacionados a logging e relatórios, consulte Section 21.6.3.1, “NDB Cluster Logging Management Commands”.