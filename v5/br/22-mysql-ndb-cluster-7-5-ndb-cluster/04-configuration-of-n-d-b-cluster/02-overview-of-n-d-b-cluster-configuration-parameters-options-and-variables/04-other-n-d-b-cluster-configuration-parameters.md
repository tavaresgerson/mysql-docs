...
As listas nesta seção fornecem informações sobre os parâmetros usados nas seções `[computer]`, `[tcp]` e `[shm]` de um arquivo `config.ini` para configurar o NDB Cluster. Para descrições detalhadas e informações adicionais sobre parâmetros individuais, consulte Section 21.4.3.10, “NDB Cluster TCP/IP Connections”, ou Section 21.4.3.12, “NDB Cluster Shared Memory Connections”, conforme apropriado.
