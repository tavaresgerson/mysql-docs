...
Agora você pode iniciar o Management Server **ndb_mgmd**. Por padrão, ele tenta ler o arquivo `config.ini` em seu Current Working Directory, então mude a localização para o diretório onde o arquivo está e invoque **ndb_mgmd**:
...
Em seguida, inicie um Data Node único executando **ndbd**:

```sql
$> ndbd
```

Por padrão, **ndbd** procura o Management Server em `localhost` na porta 1186.

Nota

Se você instalou o MySQL a partir de um binary tarball, você deve especificar o path dos servidores **ndb_mgmd** e **ndbd** explicitamente. (Normalmente, eles podem ser encontrados em `/usr/local/mysql/bin`.)
...
Se tudo correu bem até agora, você pode começar a usar o cluster. Conecte-se ao server e verifique se a Storage Engine `NDBCLUSTER` está habilitada:
...
Tente criar uma table `NDBCLUSTER`:
