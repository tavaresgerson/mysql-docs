...
O arquivo de configuração global do NDB Cluster é, por convenção, chamado `config.ini` (mas isso não é obrigatório). Se necessário, ele é lido pelo **ndb_mgmd** no momento do início e pode ser colocado em qualquer local que possa ser lido por ele. A localização e o nome da configuração são especificados usando `--config-file=caminho_nome` com **ndb_mgmd** na linha de comando. Esta opção não tem valor padrão e é ignorada se **ndb_mgmd** usar o cache de configuração.
