...
* `Id`
  ...
  O uso do parâmetro `Id` para identificar Nodes de gerenciamento está obsoleto (deprecated) em favor de `NodeId`. Embora `Id` continue a ser suportado para compatibilidade retroativa, ele agora gera um aviso (warning) e está sujeito à remoção em uma versão futura do NDB Cluster.
...
* `NodeId`
  ...
  `NodeId` é o nome de parâmetro preferido a ser usado ao identificar Nodes de gerenciamento. Embora o `Id` mais antigo continue a ser suportado para compatibilidade retroativa, ele agora está obsoleto (deprecated) e gera um aviso (warning) quando usado; ele também está sujeito à remoção em um futuro lançamento do NDB Cluster.
...
* `ExecuteOnComputer`
  ...
  Este parâmetro está obsoleto (deprecated) a partir do NDB 7.5.0 e está sujeito à remoção em um lançamento futuro. Use o parâmetro `HostName` em vez disso.
...
* `DataDir`
  ...
  O valor Default para este parâmetro é o diretório onde **ndb_mgmd** está localizado.
...
* `ExtraSendBufferMemory`
  ...
  Este parâmetro especifica a quantidade de memória do Send Buffer do Transporter a ser alocada além de qualquer valor que tenha sido definido usando `TotalSendBufferMemory`, `SendBufferMemory`, ou ambos.
...
* `TotalSendBufferMemory`
  ...
  Se este parâmetro for definido, seu valor mínimo permitido é 256KB; 0 indica que o parâmetro não foi definido. Para informações mais detalhadas, consulte Section 21.4.3.13, “Configuring NDB Cluster Send Buffer Parameters”.
