...
A seção `[system]` é usada para parâmetros aplicáveis ao cluster como um todo. O parâmetro de sistema `Name` é usado com o MySQL Enterprise Monitor; `ConfigGenerationNumber` e `PrimaryMGMNode` não são usados em ambientes de produção. Exceto ao usar o NDB Cluster com o MySQL Enterprise Monitor, não é necessário ter uma seção `[system]` no arquivo `config.ini`.
...
* `Name`
  ...
  Você pode obter o valor deste parâmetro verificando a variável de status `Ndb_system_name`. Em aplicações da NDB API, você também pode recuperá-lo usando `get_system_name()`.
