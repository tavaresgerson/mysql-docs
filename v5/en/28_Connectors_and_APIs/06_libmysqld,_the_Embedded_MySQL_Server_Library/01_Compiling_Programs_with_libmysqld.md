...
[`-DWITH_EMBEDDED_SERVER=1`](source-configuration-options.html#option_cmake_with_embedded_server) option. See
...
get the full list of libraries by executing [**mysql\_config
--libmysqld-libs**](mysql-config.html "4.7.1 mysql_config — Display Options for Compiling Clients").
...
compiler needs to know where to find various files and needs
...
assuming that you are using **gcc**, use the GNU C
compiler:
...
[**mysql\_config**](mysql-config.html "4.7.1 mysql_config — Display Options for Compiling Clients") command is contained in backticks,
not single quotation marks.

On some non-**gcc** platforms, the embedded library
