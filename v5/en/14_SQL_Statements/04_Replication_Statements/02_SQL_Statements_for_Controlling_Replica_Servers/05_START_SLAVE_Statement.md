...
[`SUPER`](privileges-provided.html#priv_super) privilege. `START
SLAVE` causes an implicit commit of an ongoing
transaction (see [Section 13.3.3, “Statements That Cause an Implicit Commit”](implicit-commit.html "13.3.3 Statements That Cause an Implicit Commit")).
...
* `START SLAVE` with no thread type options
  starts all of the replication threads, and so does
  `START SLAVE` with both of the thread type
  options.
...
`START SLAVE` sends an acknowledgment to the
...
SLAVE` does not continue to monitor the threads after
...
[`SHOW SLAVE STATUS`](show-slave-status.html "13.7.5.34 SHOW SLAVE STATUS Statement"). A successful
`START SLAVE` statement causes
[`SHOW SLAVE STATUS`](show-slave-status.html "13.7.5.34 SHOW SLAVE STATUS Statement") to show
`Slave_SQL_Running=Yes`, but it might or
might not show `Slave_IO_Running=Yes`,
because `Slave_IO_Running=Yes` is only shown
...
`group_replication_applier` channel. This
...
`START SLAVE` cannot be used at all with the
`group_replication_recovery` channel.

`START SLAVE` supports pluggable user-password
authentication (see [Section 6.2.13, “Pluggable Authentication”](pluggable-authentication.html "6.2.13 Pluggable Authentication"))
...
PROCESSLIST`](show-processlist.html "13.7.5.29 SHOW PROCESSLIST Statement") statements. However, it is sent in plain
...
[Section 16.1.7.3, “Skipping Transactions”](replication-administration-skip.html "16.1.7.3 Skipping Transactions"). To identify a
transaction, you can use [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") with the
source's binary log or the replica's relay log, or use a
[`SHOW BINLOG EVENTS`](show-binlog-events.html "13.7.5.2 SHOW BINLOG EVENTS Statement") statement.
...
:   This option makes the replication applier start processing
    ...
    with multi-threaded slaves. If this option is used with a
    multi-threaded slave, a warning is raised, and the slave
    switches to single-threaded mode. Depending on the use
    case, it may be possible to to use `START SLAVE
    UNTIL MASTER_LOG_POS` or `START SLAVE
    UNTIL SQL_BEFORE_GTIDS` instead. You can also use
    ...
    does not stop the slave thread.
