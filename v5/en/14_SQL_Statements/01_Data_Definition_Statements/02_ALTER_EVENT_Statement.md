...
The [`ALTER EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") statement changes
...
`ENABLE` / `DISABLE`, and
[`DO`](do.html "13.2.3 DO Statement") clauses is exactly the same as
when used with [`CREATE EVENT`](create-event.html "13.1.12 CREATE EVENT Statement"). (See
...
user has the [`EVENT`](privileges-provided.html#priv_event) privilege. When
a user executes a successful [`ALTER
EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") statement, that user becomes the definer for the
affected event.

[`ALTER EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") works only with an
existing event:
...
[`ALTER EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") statement is run.
...
EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") statement only for those characteristics that you
want to change; omitted options keep their existing values. This
includes any default values for [`CREATE
EVENT`](create-event.html "13.1.12 CREATE EVENT Statement") such as `ENABLE`.

To disable `myevent`, use this
[`ALTER EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") statement:
...
[`ALTER EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") and
[`CREATE EVENT`](create-event.html "13.1.12 CREATE EVENT Statement") statements. References
...
Although an [`ALTER EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") statement
that contains another [`ALTER EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement")
statement in its [`DO`](do.html "13.2.3 DO Statement") clause appears
...
EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") statement's `RENAME TO` clause.
...
the [`EVENT`](privileges-provided.html#priv_event) privilege on both the
...
`DISABLE ON SLAVE` is set automatically as
