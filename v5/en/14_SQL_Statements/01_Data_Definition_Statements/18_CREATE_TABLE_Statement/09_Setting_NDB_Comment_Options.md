...
[`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") table. Table-level options for
...
This works for [`BLOB`](blob.html "11.3.4 The BLOB and TEXT Types"),
`MEDIUMBLOB`, `LONGBLOB`,
...
TABLE` or [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement")
statement can also be used to specify an
`NDB_COLUMN` option. NDB 7.5 and 7.6 support a
single column comment option
`MAX_BLOB_PART_SIZE`; syntax for this option is
shown here:
...
The `=` sign and the value following it are
...
[`BLOB`](blob.html "11.3.4 The BLOB and TEXT Types") column to the maximum number
...
`MAX_BLOB_PART_SIZE` represents column size in
...
SQL statement in the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client to create a
...
From the system shell, run the [**ndb\_desc**](mysql-cluster-programs-ndb-desc.html "21.5.10 ndb_desc — Describe NDB Tables")
...
The output of [**ndb\_desc**](mysql-cluster-programs-ndb-desc.html "21.5.10 ndb_desc — Describe NDB Tables") shows that the blob
...
For more information about how [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6")
...
output of [`SHOW CREATE TABLE`](show-create-table.html "13.7.5.10 SHOW CREATE TABLE Statement") to
...
`READ_BACKUP` for an existing table online (Bug
...
per table. Using `MAX_ROWS` to force the number
...
Information Schema [`TABLES`](information-schema-tables-table.html "24.3.25 The INFORMATION_SCHEMA TABLES Table") table, as
in this example:
...
[**ndb\_desc**](mysql-cluster-programs-ndb-desc.html "21.5.10 ndb_desc — Describe NDB Tables"). [**ndb\_desc**](mysql-cluster-programs-ndb-desc.html "21.5.10 ndb_desc — Describe NDB Tables") also
