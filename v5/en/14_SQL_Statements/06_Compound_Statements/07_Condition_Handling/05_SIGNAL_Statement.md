...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") is the way to
“return” an error.
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") provides error information
...
value, message). Without [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement"),
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement.

To retrieve information from the diagnostics area, use the
[`GET DIAGNOSTICS`](get-diagnostics.html "13.6.7.3 GET DIAGNOSTICS Statement") statement (see
[Section 13.6.7.3, “GET DIAGNOSTICS Statement”](get-diagnostics.html "13.6.7.3 GET DIAGNOSTICS Statement")). For information about the
diagnostics area, see [Section 13.6.7.7, “The MySQL Diagnostics Area”](diagnostics-area.html "13.6.7.7 The MySQL Diagnostics Area").
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement indicates the
...
[`DECLARE ...
CONDITION`](declare-condition.html "13.6.7.1 DECLARE ... CONDITION Statement") (see [Section 13.6.7.1, “DECLARE ... CONDITION Statement”](declare-condition.html "13.6.7.1 DECLARE ... CONDITION Statement")).
...
in [Signal Condition Information Items](signal.html#signal-condition-information-items "Signal Condition Information Items"). Some
signal values cause statement termination; see
[Effect of Signals on Handlers, Cursors, and Statements](signal.html#signal-effects "Effect of Signals on Handlers, Cursors, and Statements").

The `SQLSTATE` value for a
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement should not
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement or in a named
...
The [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement optionally
...
with [`DECLARE`](declare.html "13.6.3 DECLARE Statement"), user-defined
...
WARNINGS`](show-warnings.html "13.7.5.40 SHOW WARNINGS Statement") after the procedure returns.
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") is typically used within
stored programs, but it is a MySQL extension that it is
permitted outside handler context. For example, if you invoke
the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client program, you can enter any
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") executes according to
the following rules:

If the [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement
...
If the [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement uses a
named condition, the condition must be declared in some scope
that applies to the [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement")
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement, an
`Undefined CONDITION` error occurs.

If [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") refers to a named
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement executes. The
...
condition occurs, see [Section 13.6.7.6, “Scope Rules for Handlers”](handler-scope.html "13.6.7.6 Scope Rules for Handlers").
...
`CALL p()` reaches the
[`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") statement. There is
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") (or
[`RESIGNAL`](resignal.html "13.6.7.4 RESIGNAL Statement")) statement. All items
are standard SQL except `MYSQL_ERRNO`, which
is a MySQL extension. For more information about these items
see [Section 13.6.7.7, “The MySQL Diagnostics Area”](diagnostics-area.html "13.6.7.7 The MySQL Diagnostics Area").
...
information item in a [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement")
statement.

A [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement always
...
* Class = `'00'` (success)
  ...
  valid for [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement").
...
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") executes are the
`SQLSTATE` value raised by the
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") statement and the
`MESSAGE_TEXT` and
`MYSQL_ERRNO` items. These values are
available from the C API:

* [`mysql_sqlstate()`](/doc/c-api/5.7/en/mysql-sqlstate.html) returns
  the `SQLSTATE` value.

* [`mysql_errno()`](/doc/c-api/5.7/en/mysql-errno.html) returns the
  `MYSQL_ERRNO` value.

* [`mysql_error()`](/doc/c-api/5.7/en/mysql-error.html) returns the
  `MESSAGE_TEXT` value.

At the SQL level, the output from [`SHOW
WARNINGS`](show-warnings.html "13.7.5.40 SHOW WARNINGS Statement") and [`SHOW
ERRORS`](show-errors.html "13.7.5.17 SHOW ERRORS Statement") indicates the `MYSQL_ERRNO`
...
[`sql_mode`](server-system-variables.html#sysvar_sql_mode) system variable; in
particular, strict SQL mode does not matter. MySQL also
ignores `IGNORE`: The intent of
[`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement") is to raise a
...
[`DECLARE ...
HANDLER`](declare-handler.html "13.6.7.2 DECLARE ... HANDLER Statement").
...
* Class = `'01'` (warning)
  ...
  The value of the
  [`warning_count`](server-system-variables.html#sysvar_warning_count) system
  variable goes up. [`SHOW
  WARNINGS`](show-warnings.html "13.7.5.40 SHOW WARNINGS Statement") shows the signal.
  `SQLWARNING` handlers catch the signal.

  Warnings cannot be returned from stored functions because
  the [`RETURN`](return.html "13.6.5.7 RETURN Statement") statement that
  ...
  [`warning_count`](server-system-variables.html#sysvar_warning_count) to 0).
