...
[`SELECT`](select.html "13.2.9 SELECT Statement") statement that retrieves
the rows to be traversed by the cursor. To fetch the rows later,
use a [`FETCH`](fetch.html "13.6.6.3 Cursor FETCH Statement") statement. The number
of columns retrieved by the
[`SELECT`](select.html "13.2.9 SELECT Statement") statement must match the
number of output variables specified in the
[`FETCH`](fetch.html "13.6.6.3 Cursor FETCH Statement") statement.

The [`SELECT`](select.html "13.2.9 SELECT Statement") statement cannot have
an `INTO` clause.
...
For an example, see [Section 13.6.6, “Cursors”](cursors.html "13.6.6 Cursors").

For information available through
[`SHOW`](show.html "13.7.5 SHOW Statements") statements, it is possible
