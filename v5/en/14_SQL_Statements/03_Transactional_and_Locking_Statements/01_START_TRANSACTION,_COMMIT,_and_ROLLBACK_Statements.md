...
* The `WITH CONSISTENT SNAPSHOT` modifier
  ...
  followed by a [`SELECT`](select.html "13.2.9 SELECT Statement") from any
  `InnoDB` table. See
  [Section 14.7.2.3, “Consistent Nonlocking Reads”](innodb-consistent-read.html "14.7.2.3 Consistent Nonlocking Reads"). The `WITH
...
* The `READ WRITE` and `READ
  ...
  [Section 8.5.3, “Optimizing InnoDB Read-Only Transactions”](innodb-performance-ro-txn.html "8.5.3 Optimizing InnoDB Read-Only Transactions") for more
  information.
...
See [Chapter 27, *Connectors and APIs*](connectors-apis.html "Chapter 27 Connectors and APIs"), or the documentation for
...
[`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6")) are not made permanent
immediately. You must use [`COMMIT`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements") to
...
[`BEGIN ... END`](begin-end.html "13.6.1 BEGIN ... END Compound Statement")
...
[`BEGIN ...
END`](begin-end.html "13.6.1 BEGIN ... END Compound Statement") block. Begin a transaction in this context with
[`START
TRANSACTION`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements") instead.
...
committed. See [Section 13.3.3, “Statements That Cause an Implicit Commit”](implicit-commit.html "13.3.3 Statements That Cause an Implicit Commit"), for more
information.

Beginning a transaction also causes table locks acquired with
[`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") to be released, as
though you had executed
[`UNLOCK
TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements"). Beginning a transaction does not release a
...
* If you use tables from more than one transaction-safe storage
  ...
  [`SET
  TRANSACTION ISOLATION LEVEL`](set-transaction.html "13.3.6 SET TRANSACTION Statement") to set the isolation
...
* If you issue a
  ...
  transaction, an
  [`ER_WARNING_NOT_COMPLETE_ROLLBACK`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_warning_not_complete_rollback)
...
[`COMMIT`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements"). Transactions that are
...
with the [`SET TRANSACTION`](set-transaction.html "13.3.6 SET TRANSACTION Statement") statement.
...
PROCESSLIST`](show-processlist.html "13.7.5.29 SHOW PROCESSLIST Statement") displays `Rolling back` in
...
If a [`SELECT`](select.html "13.2.9 SELECT Statement") statement within a
