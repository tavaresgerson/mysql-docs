...
[`SELECT`](select.html "13.2.9 SELECT Statement") statements and
multiple-table [`DELETE`](delete.html "13.2.2 DELETE Statement") and
...
nested joins. See [Section 8.2.1.7, “Nested Join Optimization”](nested-join-optimization.html "8.2.1.7 Nested Join Optimization").

Index hints can be specified to affect how the MySQL optimizer
makes use of indexes. For more information, see
[Section 8.9.4, “Index Hints”](index-hints.html "8.9.4 Index Hints"). Optimizer hints and the
`optimizer_switch` system variable are other
ways to influence optimizer use of indexes. See
[Section 8.9.3, “Optimizer Hints”](optimizer-hints.html "8.9.3 Optimizer Hints"), and
...
* A *`table_subquery`* is also known as
  ...
  clause. See [Section 13.2.10.8, “Derived Tables”](derived-tables.html "13.2.10.8 Derived Tables"). Such
...
* If there is no matching row for the right table in the
  ...
  `right_tbl`). See
  [Section 8.2.1.8, “Outer Join Optimization”](outer-join-optimization.html "8.2.1.8 Outer Join Optimization").
...
* Redundant columns of a `NATURAL` join do
  ...
  In the first [`SELECT`](select.html "13.2.9 SELECT Statement")
  ...
  common columns have the same value, so it does not matter
