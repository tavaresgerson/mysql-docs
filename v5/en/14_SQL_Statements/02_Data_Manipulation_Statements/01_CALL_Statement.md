...
The [`CALL`](call.html "13.2.1 CALL Statement") statement invokes a stored
...
In prepared [`CALL`](call.html "13.2.1 CALL Statement") statements used
with [`PREPARE`](prepare.html "13.5.1 PREPARE Statement") and
[`EXECUTE`](execute.html "13.5.2 EXECUTE Statement"), placeholders can be used
...
[`CALL`](call.html "13.2.1 CALL Statement") SQL statement to execute
stored procedures that produce result sets, the
`CLIENT_MULTI_RESULTS` flag must be enabled. This
is because each [`CALL`](call.html "13.2.1 CALL Statement") returns a
...
enabled if [`CALL`](call.html "13.2.1 CALL Statement") is used to execute
...
call [`mysql_real_connect()`](/doc/c-api/5.7/en/mysql-real-connect.html), either
...
To process the result of a [`CALL`](call.html "13.2.1 CALL Statement")
statement executed using
[`mysql_query()`](/doc/c-api/5.7/en/mysql-query.html) or
[`mysql_real_query()`](/doc/c-api/5.7/en/mysql-real-query.html), use a loop
that calls [`mysql_next_result()`](/doc/c-api/5.7/en/mysql-next-result.html) to
...
[`CALL`](call.html "13.2.1 CALL Statement") statements and access
`OUT` and `INOUT` parameters.
This is done by processing the result of a
[`CALL`](call.html "13.2.1 CALL Statement") statement using a loop that
calls [`mysql_stmt_next_result()`](/doc/c-api/5.7/en/mysql-stmt-next-result.html) to
determine whether there are more results. For an example, see
[Prepared CALL Statement Support](/doc/c-api/5.7/en/c-api-prepared-call-statements.html). Languages that
provide a MySQL interface can use prepared
[`CALL`](call.html "13.2.1 CALL Statement") statements to directly
