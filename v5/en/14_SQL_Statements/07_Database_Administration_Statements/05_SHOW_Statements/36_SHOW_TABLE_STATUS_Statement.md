...
[`SHOW TABLE STATUS`](show-table-status.html "13.7.5.36 SHOW TABLE STATUS Statement") works likes
[`SHOW TABLES`](show-tables.html "13.7.5.37 SHOW TABLES Statement"), but provides a lot
of information about each non-`TEMPORARY`
table. You can also get this list using the [**mysqlshow
--status *`db_name`***](mysqlshow.html "4.5.7 mysqlshow — Display Database, Table, and Column Information") command.
The [`LIKE`](string-comparison-functions.html#operator_like) clause, if present,
...
[`SHOW TABLE STATUS`](show-table-status.html "13.7.5.36 SHOW TABLE STATUS Statement") output has
these columns:
...
* `Row_format`
  ...
  [**myisamchk -dvv**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") reports as
  `Packed`. `InnoDB` table
...
* `Rows`
  ...
  For [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") tables, the row
  count is only a rough estimate used in SQL optimization.
  (This is also true if the
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") table is partitioned.)
...
* `Update_time`
  ...
  [system
  tablespace](glossary.html#glos_system_tablespace "system tablespace") and the data file timestamp does not
  apply. Even with
  [file-per-table](glossary.html#glos_file_per_table "file-per-table")
  mode with each `InnoDB` table in a separate
  `.ibd` file,
  [change
  buffering](glossary.html#glos_change_buffering "change buffering") can delay the write to the data file, so
  ...
  for the last [`UPDATE`](update.html "13.2.11 UPDATE Statement"),
...
* `Create_options`
  ...
  `ENCRYPTION` option specified when creating
  or altering a file-per-table tablespace.

  When creating a table with
  [strict mode](glossary.html#glos_strict_mode "strict mode")
...
* For `InnoDB` tables,
  [`SHOW TABLE STATUS`](show-table-status.html "13.7.5.36 SHOW TABLE STATUS Statement") does not
...
* For [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") tables, the output of
  ...
  that [`BLOB`](blob.html "11.3.4 The BLOB and TEXT Types") columns are not
  taken into account.
...
* For views, all columns displayed by
  [`SHOW TABLE STATUS`](show-table-status.html "13.7.5.36 SHOW TABLE STATUS Statement") are
...
`INFORMATION_SCHEMA`
[`TABLES`](information-schema-tables-table.html "24.3.25 The INFORMATION_SCHEMA TABLES Table") table. See
