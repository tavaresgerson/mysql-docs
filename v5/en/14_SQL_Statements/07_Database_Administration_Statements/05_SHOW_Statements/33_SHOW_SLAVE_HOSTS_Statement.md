...
server that acts as a replication source. `SHOW SLAVE
HOSTS` requires the [`REPLICATION
SLAVE`](privileges-provided.html#priv_replication-slave) privilege. The statement displays information
about servers that are or have been connected as replicas, with
each row of the result corresponding to one replica server, as
shown here:
...
* `Host`: The host name of the replica server
  as specified on the replica with the
