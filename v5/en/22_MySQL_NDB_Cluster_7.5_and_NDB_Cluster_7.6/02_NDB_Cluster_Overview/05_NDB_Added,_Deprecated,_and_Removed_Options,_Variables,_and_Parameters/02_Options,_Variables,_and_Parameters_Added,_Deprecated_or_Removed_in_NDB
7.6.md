...
have been added to, deprecated in, or removed from NDB 7.6.
...
* `EnablePartialLcp`:
  ...
  LCPs write full checkpoints. Added in NDB 7.6.4.

* `EnableRedoControl`:
  Enable adaptive checkpointing speed for controlling redo log
  usage. Added in NDB 7.6.7.

* `InsertRecoveryWork`:
  Percentage of RecoveryWork used for inserted rows; has no
  effect unless partial local checkpoints are in use. Added in
  NDB 7.6.5.

* `LocationDomainId`:
  Assign this API node to specific availability domain or
  zone. 0 (default) leaves this unset. Added in NDB 7.6.4.

* `LocationDomainId`:
  Assign this management node to specific availability domain
  or zone. 0 (default) leaves this unset. Added in NDB 7.6.4.
...
* `MaxFKBuildBatchSize`:
  ...
  but impacts ongoing traffic as well. Added in NDB 7.6.4.
...
* `BackupDataBufferSize`:
  Default size of databuffer for backup (in bytes). Deprecated
  in NDB 7.6.4.

* `BackupMaxWriteSize`:
  Maximum size of file system writes made by backup (in
  bytes). Deprecated in NDB 7.6.4.
...
* `IndexMemory`:
  ...
  DataMemory. Deprecated in NDB 7.6.2.

* `Signum`:
  Signal number to be used for signalling. Deprecated in NDB
  7.6.6.
...
options have been added in NDB 7.6.

* `Ndb_system_name`:
  Configured cluster system name; empty if server not
  connected to NDB. Added in NDB 5.7.18-ndb-7.6.2.

* `ndb-log-fail-terminate`:
  Terminate mysqld process if complete logging of all found
  row events is not possible. Added in NDB 5.7.29-ndb-7.6.14.
...
* `ndb_row_checksum`:
  When enabled, set row checksums; enabled by default. Added
  in NDB 5.7.23-ndb-7.6.8.

##### Options and Variables Deprecated in NDB 7.6

No system variables, status variables, or server options have
been deprecated in NDB 7.6.
