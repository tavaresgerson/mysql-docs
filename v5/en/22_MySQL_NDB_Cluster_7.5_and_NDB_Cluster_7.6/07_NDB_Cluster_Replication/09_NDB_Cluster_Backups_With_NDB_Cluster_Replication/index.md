...
[Section 21.7.5, “Preparing the NDB Cluster for Replication”](mysql-cluster-replication-preparation.html "21.7.5 Preparing the NDB Cluster for Replication"), and the
...
1. There are two different methods by which the backup may be
   ...
   * **Method A.**
     ...
     *`management_host`* is the IP
     ...
     [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") management server for
   ...
   * **Method B.**
     ...
     [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") management client as
     part of the [`START
     BACKUP`](mysql-cluster-backup-using-management-client.html "21.6.8.2 Using The NDB Cluster Management Client to Create a Backup") command. This can be done as shown
...
2. Copy the cluster backup files to the replica that is being
   ...
   [**ndbd**](mysql-cluster-programs-ndbd.html "21.5.1 ndbd — The NDB Cluster Data Node Daemon") process for the source cluster has
   ...
   *all* of these files must be copied to the
   ...
   same number of [**ndbd**](mysql-cluster-programs-ndbd.html "21.5.1 ndbd — The NDB Cluster Data Node Daemon") processes (data nodes)
   as the source, it is highly recommended this number be the
   same. It *is* necessary that the replica be
   started with the
   [`--skip-slave-start`](replication-options-replica.html#option_mysqld_skip-slave-start) option, to
...
3. Create any databases on the replica cluster that are present
   ...
   A [`CREATE DATABASE`](create-database.html "13.1.11 CREATE DATABASE Statement") (or
   [`CREATE
   SCHEMA`](create-database.html "13.1.11 CREATE DATABASE Statement")) statement corresponding to each database
...
5. You can now start the cluster restoration process on the
   replica using the [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") command for
   ...
   *`dir`* is the path to the directory
   ...
   the [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") commands corresponding to
   the remaining backup files, the `-m` option
   should *not* be used.

   For restoring from a source cluster with four data nodes (as
   shown in the figure in
   [Section 21.7, “NDB Cluster Replication”](mysql-cluster-replication.html "21.7 NDB Cluster Replication")) where the backup
   ...
   in the final invocation of [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") in
...
7. Using `@latest` as the epoch value obtained
   ...
   `next_position` and
   `next_file` columns from the last epoch
   applied before the logical restore position:
   ...
   [`SHOW MASTER STATUS`](show-master-status.html "13.7.5.23 SHOW MASTER STATUS Statement") on the
...
8. Using the values obtained in the previous step, you can now
   ...
   TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") statement in the replica's
   [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client:
