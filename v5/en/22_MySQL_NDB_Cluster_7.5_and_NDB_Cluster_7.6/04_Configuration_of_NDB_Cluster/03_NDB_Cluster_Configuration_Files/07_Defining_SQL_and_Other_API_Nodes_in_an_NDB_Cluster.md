...
applications other than [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") processes
...
[Section 21.4.3.9.1, “MySQL Server Options for NDB Cluster”](mysql-cluster-options-variables.html#mysql-cluster-program-options-mysqld "21.4.3.9.1 MySQL Server Options for NDB Cluster"). For
...
<span class="bold"><strong>Node Restart:
  ...
  any of three possible values (0, 240, 3840).

  The original intended use for this parameter was to
  facilitate upgrades and especially downgrades to and from
  very old releases with differing default hash map sizes.
  This is not an issue when upgrading from NDB Cluster 7.3 (or
  later) to later versions.
...
STATUS`](show-status.html "13.7.5.35 SHOW STATUS Statement") in the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client, as
shown here:
