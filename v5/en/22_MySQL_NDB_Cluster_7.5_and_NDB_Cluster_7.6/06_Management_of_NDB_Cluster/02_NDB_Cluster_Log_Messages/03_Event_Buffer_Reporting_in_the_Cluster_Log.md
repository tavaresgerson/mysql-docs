...
for each [`Ndb`](/doc/ndbapi/en/ndb-ndb.html) object subscribing
to table events, which means that there are usually two buffers
for each [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") performing binary logging
...
them were changed significantly in NDB 7.5.1, with the addition
of the `NDB_LE_EventBufferStatus2` event type
and the `ndb_logevent_EventBufferStatus2` data
structure (see [The Ndb\_logevent\_type Type](/doc/ndbapi/en/mgm-types.html#mgm-ndb-logevent-type)). The
...
* *`object_id`*: ID of the
  [`Ndb`](/doc/ndbapi/en/ndb-ndb.html) object where the report
  originated.
...
* *`latest_consumed_epoch`*: The epoch
  ...
  [`nextEvent()`](/doc/ndbapi/en/ndb-ndb.html#ndb-ndb-nextevent).)
...
The `latest_consumed_epoch` and
`latest_buffered_epoch` fields correspond,
respectively, to the `apply_gci` and
`latest_gci` fields of the old-style event
buffer logging messages used prior to NDB 7.5.1.
...
* `BUFFERED_EPOCHS_OVER_THRESHOLD`: Whether
  ...
  applications, this is done by calling
  [`nextEvent()`](/doc/ndbapi/en/ndb-ndb.html#ndb-ndb-nextevent) or
  [`nextEvent2()`](/doc/ndbapi/en/ndb-ndb.html#ndb-ndb-nextevent2)). The
  ...
  [`setEventBufferQueueEmptyEpoch()`](/doc/ndbapi/en/ndb-ndb.html#ndb-ndb-seteventbufferqueueemptyepoch).
...
* `PARTIALLY_BUFFERING`: The buffer free
  ...
  which can be set in the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client
  using the
  ...
  [`set_eventbuffer_free_percent()`](/doc/ndbapi/en/ndb-ndb.html#ndb-ndb-set-eventbuffer-free-percent).
