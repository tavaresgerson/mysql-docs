...
NDB Cluster supports distribution of MySQL users and privileges
across all SQL nodes in an NDB Cluster. This support is not
...
[`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine") storage engine, which means
...
An SQL file `ndb_dist_priv.sql` provided with
...
After connecting to the SQL node in the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
...
* A set of local copies that use the
  [`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine") storage engine. Their
  names are generated by adding the suffix
  `_backup` to the original privilege table
  names.

* A set of distributed copies that use the
  [`NDBCLUSTER`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") storage engine. These
...
the [`ALTER TABLE ...
ENGINE = NDB`](alter-table.html "13.1.8 ALTER TABLE Statement") statements that convert the mysql system
tables to [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6").
...
proceeding. You can do this using [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") in
...
node using the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client (again, as the
...
PRIVILEGES`](flush.html#flush-privileges) on those SQL nodes, or to disconnect and
...
mysql.procs_priv`](drop-table.html "13.1.29 DROP TABLE Statement"). This causes the SQL node to use the
...
[`--initial`](mysql-cluster-programs-ndbd.html#option_ndbd_initial)), the shared privilege
...
file created with [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program"). If you need to use
a new MySQL Server to perform the restoration, you should start it
with [`--skip-grant-tables`](server-options.html#option_mysqld_skip-grant-tables) when
...
BACKUP`](mysql-cluster-backup-using-management-client.html "21.6.8.2 Using The NDB Cluster Management Client to Create a Backup") in the [**ndb\_mgm**](mysql-cluster-programs-ndb-mgm.html "21.5.5 ndb_mgm — The NDB Cluster Management Client") client. (The
[`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine") tables created by
`mysql_cluster_move_privileges` are not backed up
by the `START BACKUP` command.)
[**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") does not restore the privilege
...
1. If copies of the `mysql.ndb_*_backup` tables
...
just as they can any other [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6")
