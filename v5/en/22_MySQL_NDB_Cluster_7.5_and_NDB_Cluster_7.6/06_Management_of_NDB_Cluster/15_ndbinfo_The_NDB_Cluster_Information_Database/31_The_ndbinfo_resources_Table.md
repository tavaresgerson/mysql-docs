...
* `max`
  ...
  that is available to this data node. 0 in this column
  indicates that the resource is unlimited, which means the
  effective maximum is 4294967295
  (232-1).
...
* `TRANSACTION_MEMORY`: Memory allocated for
  transactions on this data node.
...
* `DISK_OPERATIONS`: If a log file group is
  ...
  GROUP`](create-logfile-group.html "13.1.15 CREATE LOGFILE GROUP Statement").
...
* `FILE_BUFFERS`: Used by the redo log
  handler in the [`DBLQH`](/doc/ndb-internals/en/ndb-internals-kernel-blocks-dblqh.html)
  ...
  \* [`RedoBuffer`](mysql-cluster-ndbd-definition.html#ndbparam-ndbd-redobuffer), plus 1
  MB per log file part.

* `TRANSPORTER_BUFFERS`: Used for send
  buffers by [**ndbmtd**](mysql-cluster-programs-ndbmtd.html "21.5.3 ndbmtd — The NDB Cluster Data Node Daemon (Multi-Threaded)"); the sum of
  ...
  used by [**ndbmtd**](mysql-cluster-programs-ndbmtd.html "21.5.3 ndbmtd — The NDB Cluster Data Node Daemon (Multi-Threaded)") and amounts to 2 MB extra
