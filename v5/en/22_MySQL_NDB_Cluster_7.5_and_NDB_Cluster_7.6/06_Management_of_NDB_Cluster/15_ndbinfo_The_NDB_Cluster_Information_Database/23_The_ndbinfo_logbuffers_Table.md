...
* `log_type`
  ...
  Type of log. In NDB 7.5, one of: `REDO` or
  `DD-UNDO`. In NDB 7.6, one of:
  `REDO`, `DD-UNDO`,
  `BACKUP-DATA`, or
  `BACKUP-LOG`.
...
* `log_id`
  ...
  [`logspaces`](mysql-cluster-ndbinfo-logspaces.html "21.6.15.24 The ndbinfo logspaces Table") table
...
NDB 7.6.6 makes available `logbuffers` table
rows reflecting two additional log types when performing an NDB
...
currently being performed. (Bug #25822988)