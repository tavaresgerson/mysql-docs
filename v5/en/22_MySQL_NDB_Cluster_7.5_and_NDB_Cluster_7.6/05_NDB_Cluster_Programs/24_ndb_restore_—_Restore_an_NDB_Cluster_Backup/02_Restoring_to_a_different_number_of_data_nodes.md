...
1. The management server for the original cluster is on
   ...
   with [**ndbmtd**](mysql-cluster-programs-ndbmtd.html "21.5.3 ndbmtd — The NDB Cluster Data Node Daemon (Multi-Threaded)")
...
3. The files created by the backup on each data node are
   ...
   * `BACKUP-B.N.log`
   ...
   [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup")). It is simplest to copy
...
4. The management server for the target cluster is on host
   ...
   `host3` and `host5`
   should be started with [**ndbmtd**](mysql-cluster-programs-ndbmtd.html "21.5.3 ndbmtd — The NDB Cluster Data Node Daemon (Multi-Threaded)")
   `-c host20`
...
6. On each of the two target data nodes, you must restore
   ...
   [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") on
   `host3` as shown here:
   ...
   invoking [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") on
   `host5`, like this:
...
[**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") command is that of the node
in the original backup and not that of the data node to
restore it to. When performing a backup using the method
described in this section, [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup")
...
from backup created using [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program").

1. *Create the backup of the data*. You
   can do this by invoking the [**ndb\_mgm**](mysql-cluster-programs-ndb-mgm.html "21.5.5 ndb_mgm — The NDB Cluster Management Client")
...
2. Create a backup of the schema. In NDB 7.5.2 and later,
   this step is necessary only if the total number of LCP
   threads or LQH threads per node group is changed.
   ...
   native backup using [**ndb\_mgm**](mysql-cluster-programs-ndb-mgm.html "21.5.5 ndb_mgm — The NDB Cluster Management Client"), you
...
1. *Restore the schema*.
   ...
   * If you created a separate schema backup file using
     [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program"), import this file using
     the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client, similar to what
     is shown here:
     ...
     in order for the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client to
   ...
   * If you did *not* need to create a
     ...
     [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") must be able to
...
3. *Rebuild the indexes*. These were
   ...
   [`--ndb-connectstring`](mysql-cluster-programs-ndb-restore.html#option_ndb_restore_ndb-connectstring)
   option, so that [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") can
