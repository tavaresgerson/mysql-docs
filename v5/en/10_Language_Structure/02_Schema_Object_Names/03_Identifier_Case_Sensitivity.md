...
[Section 1.6.1, “MySQL Extensions to Standard SQL”](extensions-to-ansi.html "1.6.1 MySQL Extensions to Standard SQL"). The
...
[`lower_case_table_names`](server-system-variables.html#sysvar_lower_case_table_names) can take
...
[`lower_case_table_names`](server-system-variables.html#sysvar_lower_case_table_names) is 0. On
Windows, the default value is 1. On macOS, the default value is
2.

<table summary="Values for the lower_case_table_names system variable."><col style="width: 10%"/><col style="width: 90%"/><thead><tr>
<th>Value</th>
<th>Meaning</th>
</tr></thead><tbody><tr>
<td><code>0</code></td>
<td>Table and database names are stored on disk using the lettercase
              specified in the <a class="link" href="create-table.html" title="13.1.18 CREATE TABLE Statement"><code>CREATE
              TABLE</code></a> or <a class="link" href="create-database.html" title="13.1.11 CREATE DATABASE Statement"><code>CREATE
              DATABASE</code></a> statement. Name comparisons are
              case-sensitive. You should <em>not</em> set
...
<td>Table and database names are stored on disk using the lettercase
              ...
              case-sensitive. This works <em>only</em> on
              file systems that are not case-sensitive!
              <code>InnoDB</code> table names and view names are
              stored in lowercase, as for
              <code>lower_case_table_names=1</code>.</td>
</tr></tbody></table>

If you are using MySQL on only one platform, you do not normally
have to change the
[`lower_case_table_names`](server-system-variables.html#sysvar_lower_case_table_names) variable
...
* Use `lower_case_table_names=1` on all
  ...
  use [`SHOW TABLES`](show-tables.html "13.7.5.37 SHOW TABLES Statement") or
  [`SHOW DATABASES`](show-databases.html "13.7.5.14 SHOW DATABASES Statement"), you do not
...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") and restarting it with the new
variable setting. To do this for an individual table, use
...
setting [`lower_case_table_names`](server-system-variables.html#sysvar_lower_case_table_names),
then drop the databases, and reload them after setting
[`lower_case_table_names`](server-system-variables.html#sysvar_lower_case_table_names):

1. Use [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") to dump each database:
...
3. Stop the server, set
   [`lower_case_table_names`](server-system-variables.html#sysvar_lower_case_table_names), and
   restart the server.
...
4. Reload the dump file for each database. Because
   ...
   set, each database and table name is converted to lowercase
   as it is re-created:
...
[`PREPARE`](prepare.html "13.5.1 PREPARE Statement"), tables, triggers, users,
