...
[`user_variables_by_thread`](performance-schema-user-variable-tables.html "25.12.10 Performance Schema User-Defined Variable Tables") table can
...
For [`SET`](set-variable.html "13.7.4.1 SET Syntax for Variable Assignment"),
either [`=`](assignment-operators.html#operator_assign-equal) or
[`:=`](assignment-operators.html#operator_assign-value) can be
...
value having the [`JSON`](json.html "11.5 The JSON Data Type") data type is
...
add 0 or use [`CAST(... AS UNSIGNED)`](cast-functions.html#function_cast):
...
`LIMIT` clause of a
[`SELECT`](select.html "13.2.9 SELECT Statement") statement, or the
`IGNORE N LINES`
clause of a [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") statement.

It is also possible to assign a value to a user variable in
statements other than
[`SET`](set-variable.html "13.7.4.1 SET Syntax for Variable Assignment"). (This
...
[`:=`](assignment-operators.html#operator_assign-value) and not
[`=`](assignment-operators.html#operator_assign-equal) because
the latter is treated as the comparison operator
[`=`](comparison-operators.html#operator_equal) in statements
other than
[`SET`](set-variable.html "13.7.4.1 SET Syntax for Variable Assignment"):
...
[`SELECT`](select.html "13.2.9 SELECT Statement"), you might get the results
you expect, but this is not guaranteed. In the following
statement, you might think that MySQL evaluates
`@a` first and then does an assignment second:
...
non-[`SET`](set-variable.html "13.7.4.1 SET Syntax for Variable Assignment")
...
For this [`SELECT`](select.html "13.2.9 SELECT Statement") statement, MySQL
...
[`SELECT`](select.html "13.2.9 SELECT Statement") statement executes,
...
In a [`SELECT`](select.html "13.2.9 SELECT Statement") statement, each select
...
[`SELECT`](select.html "13.2.9 SELECT Statement"). This is true even if the
...
See [Section 13.5, “Prepared Statements”](sql-prepared-statements.html "13.5 Prepared Statements"), for more
information.
