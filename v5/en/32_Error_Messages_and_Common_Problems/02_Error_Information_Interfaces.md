...
Error messages can originate on the server side or the client side, and each error message includes an error code, SQLSTATE value, and message string, as described in Section B.1, “Error Message Sources and Elements”. For lists of server-side, client-side, and global (shared between server and clients) errors, see MySQL 5.7 Error Message Reference.
...
For descriptions of the client library error functions, see MySQL 5.7 C API Developer Guide.
...
For MySQL NDB Cluster errors, use **ndb\_perror**. See Section 21.5.17, “ndb\_perror — Obtain NDB Error Message Information”.
