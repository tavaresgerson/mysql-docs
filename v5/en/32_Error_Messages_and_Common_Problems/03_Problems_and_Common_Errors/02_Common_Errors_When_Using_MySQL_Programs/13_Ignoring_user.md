...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") was started or when it reloaded the
...
* You may be running a new version of
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") with an old
  `user` table. Check whether the
  `Password` column of that table is
  shorter than 16 characters. If so, correct this condition
  by running [`mysqld_upgrade`](mysql-upgrade.html "4.4.7 mysql_upgrade — Check and Upgrade MySQL Tables").
...
* You have specified a password in the
  ...
  [`PASSWORD()`](encryption-functions.html#function_password) function. Use
  [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") to update the account in the
  `user` table with a new password, making
  sure to use the [`PASSWORD()`](encryption-functions.html#function_password)
  function:
