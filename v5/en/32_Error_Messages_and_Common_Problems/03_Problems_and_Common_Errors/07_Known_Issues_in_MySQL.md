...
* [`REPLACE`](replace.html "13.2.8 REPLACE Statement") (and
  [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") with the
  [`REPLACE`](replace.html "13.2.8 REPLACE Statement") option) does not
...
* With statement-based binary logging, the source server
  ...
  + [`CREATE
    TABLE ... SELECT`](create-table-select.html "13.1.18.4 CREATE TABLE ... SELECT Statement") or
    [`INSERT
    ... SELECT`](insert-select.html "13.2.5.1 INSERT ... SELECT Statement") statements that insert zero or
    `NULL` values into an
    `AUTO_INCREMENT` column.

  + [`DELETE`](delete.html "13.2.2 DELETE Statement") if you are
    deleting rows from a table that has foreign keys with
    `ON DELETE CASCADE` properties.

  + [`REPLACE ...
    SELECT`](replace.html "13.2.8 REPLACE Statement"), `INSERT IGNORE ...
  ...
  **If and only if the preceding queries
  ...
  For example, for
  [`INSERT ...
  SELECT`](insert-select.html "13.2.5.1 INSERT ... SELECT Statement") with no `ORDER BY`, the
  [`SELECT`](select.html "13.2.9 SELECT Statement") may return rows in a
...
* Log file names are based on the server host name if you do
  ...
  See [Section 5.1.6, “Server Command Options”](server-options.html "5.1.6 Server Command Options"). Alternatively, rename
...
* [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") does not delete temporary
  files left after a [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement")
  statement. See [Section 4.6.7, “mysqlbinlog — Utility for Processing Binary Log Files”](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files").
...
* Numeric calculations are done with
  ...
  [`DOUBLE`](floating-point-types.html "11.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") (both are normally 64
  ...
  [`BIGINT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") or
  [`DOUBLE`](floating-point-types.html "11.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") precision, and the
  rest with [`DOUBLE`](floating-point-types.html "11.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") precision.
...
* You can have up to 255 [`ENUM`](enum.html "11.3.5 The ENUM Type")
  and [`SET`](set.html "11.3.6 The SET Type") columns in one table.
...
* In an [`UPDATE`](update.html "13.2.11 UPDATE Statement") statement,
...
* If you use [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to add
  ...
  because [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") puts
  `UNIQUE` indexes before normal indexes to
  be able to detect duplicate keys as early as possible.

* An [`UPDATE`](update.html "13.2.11 UPDATE Statement") statement involving
  ...
  + With the transaction level set to
    ...
    [`SET
    GLOBAL TRANSACTION READ ONLY`](set-transaction.html "13.3.6 SET TRANSACTION Statement") or `SET
