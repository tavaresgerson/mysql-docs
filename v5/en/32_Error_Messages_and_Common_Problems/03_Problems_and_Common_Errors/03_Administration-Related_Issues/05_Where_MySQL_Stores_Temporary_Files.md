...
too small, you can use the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server")
...
DATA`](load-data.html "13.2.6 LOAD DATA Statement") statements. This directory should be in a
...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") is terminated. On platforms that
...
large files associated with [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server").)
...
Some [`SELECT`](select.html "13.2.9 SELECT Statement") queries creates
...
`ibtmp1` in the MySQL data directory. For
...
[Orphan Temporary Tables](innodb-troubleshooting-datadict.html#innodb-orphan-temporary-tables "Orphan Temporary Tables").