...
change it, see [Section 13.7.1.1, “ALTER USER Statement”](alter-user.html "13.7.1.1 ALTER USER Statement"), and
...
6. Start the MySQL server with the
   ...
   the [`init_file`](server-system-variables.html#sysvar_init_file) system
   ...
   [`--console`](server-options.html#option_mysqld_console) option to the
   [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") command.

   If you installed MySQL using the MySQL Installation
   Wizard, you may need to specify a
   [`--defaults-file`](option-file-options.html#option_general_defaults-file) option.
   For example:
   ...
   The appropriate
   [`--defaults-file`](option-file-options.html#option_general_defaults-file) setting
...
If the [`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement") statement
...
[`--user=mysql`](server-options.html#option_mysqld_user) option. If you
start the server as `root` without using
[`--user=mysql`](server-options.html#option_mysqld_user), the server may
...
2. Stop the MySQL server if it is running. Locate the
   ...
   `kill` (not `kill -9`)
   to the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") process. Use the actual
...
5. Start the MySQL server with the
   ...
   [`--defaults-file`](option-file-options.html#option_general_defaults-file) may be
   needed before the
   [`init_file`](server-system-variables.html#sysvar_init_file) argument.
...
password using the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client (but this
...
1. Stop the MySQL server if necessary, then restart it with
   ...
   [`SET PASSWORD`](set-password.html "13.7.1.7 SET PASSWORD Statement"). Because
   ...
   variable to prevent remote clients from connecting. On
   Windows platforms, if you enable
   `skip_networking`, you must also enable
   [`shared_memory`](server-system-variables.html#sysvar_shared_memory) or
   [`named_pipe`](server-system-variables.html#sysvar_named_pipe); otherwise
   the server cannot start.

2. Connect to the MySQL server using the
   [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client; no password is
