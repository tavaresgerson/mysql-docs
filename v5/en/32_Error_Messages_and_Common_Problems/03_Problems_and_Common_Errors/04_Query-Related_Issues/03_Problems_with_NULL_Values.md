...
[`IS NULL`](comparison-operators.html#operator_is-null) and [`IS
NOT NULL`](comparison-operators.html#operator_is-not-null) operators and the
[`IFNULL()`](flow-control-functions.html#function_ifnull) function.
...
[`IS NULL`](comparison-operators.html#operator_is-null) test. The following
...
See [Section 3.3.4.6, “Working with NULL Values”](working-with-null.html "3.3.4.6 Working with NULL Values"), for additional
...
DATA`](load-data.html "13.2.6 LOAD DATA Statement"), empty or missing columns are updated with
`''`. To load a `NULL` value
into a column, use `\N` in the data file. The
literal word `NULL` may also be used under
some circumstances. See [Section 13.2.6, “LOAD DATA Statement”](load-data.html "13.2.6 LOAD DATA Statement").
...
[`SUM()`](aggregate-functions.html#function_sum) ignore
`NULL` values. The exception to this is
[`COUNT(*)`](aggregate-functions.html#function_count), which counts rows and
...
values specially. For example, if you insert
...
a [`TIMESTAMP`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") column, the current
date and time is inserted; this behavior depends in part on
the server SQL mode (see [Section 5.1.10, “Server SQL Modes”](sql-mode.html "5.1.10 Server SQL Modes")) as well
as the value of the
