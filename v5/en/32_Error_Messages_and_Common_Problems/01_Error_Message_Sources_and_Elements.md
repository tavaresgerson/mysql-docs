...
displayed by the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client:
...
client library, as displayed by the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
client:
...
* Error code: This value is numeric. It is MySQL-specific and
  ...
  + The symbol for server error number
    `1146` is
    [`ER_NO_SUCH_TABLE`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_no_such_table).

  + The symbol for client error number
    `2005` is
    [`CR_UNKNOWN_HOST`](/doc/mysql-errors/5.7/en/client-error-reference.html#error_cr_unknown_host).
