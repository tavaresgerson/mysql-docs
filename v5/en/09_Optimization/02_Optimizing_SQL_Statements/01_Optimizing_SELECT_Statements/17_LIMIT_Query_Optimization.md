...
* `LIMIT 0` quickly returns an empty set.
  ...
  [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client program, you can use the
...
execution. Prior to MySQL 5.7.33, there was no way to override
this behavior, even in cases where using some other
optimization might be faster. Beginning with MySQL 5.7.33, it
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output, it uses the
table's primary key.
...
See also [Section 8.9.2, “Switchable Optimizations”](switchable-optimizations.html "8.9.2 Switchable Optimizations").