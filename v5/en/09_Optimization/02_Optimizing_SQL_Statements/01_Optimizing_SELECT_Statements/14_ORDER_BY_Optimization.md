...
as discussed in [Section 8.2.1.17, “LIMIT Query Optimization”](limit-optimization.html "8.2.1.17 LIMIT Query Optimization").

* [Use of Indexes to Satisfy ORDER BY](order-by-optimization.html#order-by-index-use "Use of Indexes to Satisfy ORDER BY")
* [Use of filesort to Satisfy ORDER BY](order-by-optimization.html#order-by-filesort "Use of filesort to Satisfy ORDER BY")
* [Influencing ORDER BY Optimization](order-by-optimization.html#order-by-optimizer-control "Influencing ORDER BY Optimization")
* [ORDER BY Execution Plan Information Available](order-by-optimization.html#order-by-diagnostic-information "ORDER BY Execution Plan Information Available")
...
* In this query, the index on
  ...
  *`key_part2`*. In that case,
  ...
  optimizer is not likely to use the index. If
...
* The query joins many tables, and the columns in the
  ...
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output that does
  not have a [`const`](explain-output.html#jointype_const) join
  type.)
...
* There is an index on only a prefix of a column named in
  ...
  [`CHAR(20)`](char.html "11.3.2 The CHAR and VARCHAR Types") column are
...
col1,
col2, ...` queries as if
you also included `ORDER BY
col1,
col2, ...` in the query.
If you include an explicit `ORDER BY`
...
`GROUP BY` implicitly sorts by default
(that is, in the absence of `ASC` or
`DESC` designators for `GROUP
BY` columns). However, relying on implicit
`GROUP BY` sorting (that is, sorting in
the absence of `ASC` or
`DESC` designators) or explicit sorting
for `GROUP BY` (that is, by using
explicit `ASC` or `DESC`
designators for `GROUP BY` columns) is
...
[`sort_buffer_size`](server-system-variables.html#sysvar_sort_buffer_size) bytes up
...
* Increase the
  ...
  [`max_sort_length`](server-system-variables.html#sysvar_max_sort_length), the
  size of sort buffer tuples increases as well and may
  require you to increase
  [`sort_buffer_size`](server-system-variables.html#sysvar_sort_buffer_size). For
  column values calculated as a result of string
  expressions (such as those that invoke a string-valued
  function), the `filesort` algorithm
...
* Change the [`tmpdir`](server-system-variables.html#sysvar_tmpdir)
...
(see [Section 8.8.1, “Optimizing Queries with EXPLAIN”](using-explain.html "8.8.1 Optimizing Queries with EXPLAIN")), you can check whether
MySQL can use indexes to resolve an `ORDER
BY` clause:

* If the `Extra` column of
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output does not
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") does not distinguish
