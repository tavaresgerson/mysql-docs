...
* `NULL`, if the
  [`SELECT`](select.html "13.2.9 SELECT Statement") produces any row
  where *`inner_expr`* is
  `NULL`

* `FALSE`, if the
  [`SELECT`](select.html "13.2.9 SELECT Statement") produces only
...
* `NULL`, if the
  [`SELECT`](select.html "13.2.9 SELECT Statement") produces any rows

* `FALSE`, if the
  [`SELECT`](select.html "13.2.9 SELECT Statement") produces no rows

For proper evaluation, it is necessary to be able to check
whether the [`SELECT`](select.html "13.2.9 SELECT Statement") has produced
any rows at all, so
`outer_expr =
inner_expr` cannot be
...
* If *`outer_expr`* is
  ...
  [`SELECT`](select.html "13.2.9 SELECT Statement") to determine whether
  ...
  [`SELECT`](select.html "13.2.9 SELECT Statement") here, without any
...
TRIGGER`](create-trigger.html "13.1.20 CREATE TRIGGER Statement").
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output, the fallback
...
If you run [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") followed by
[`SHOW WARNINGS`](show-warnings.html "13.7.5.40 SHOW WARNINGS Statement"), you can see the
triggered condition:
