...
* The [`derived_merge`](switchable-optimizations.html#optflag_derived-merge) flag
  ...
  system variable can be used, assuming that no other rule
  prevents merging. See
  [Section 8.9.2, “Switchable Optimizations”](switchable-optimizations.html "8.9.2 Switchable Optimizations"). By default,
  the flag is enabled to permit merging. Disabling the flag
  prevents merging and avoids
  [`ER_UPDATE_TABLE_USED`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_update_table_used)
  errors.

  The [`derived_merge`](switchable-optimizations.html#optflag_derived-merge) flag
  also applies to views that contain no
  `ALGORITHM` clause. Thus, if an
  [`ER_UPDATE_TABLE_USED`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_update_table_used) error
...
* It is possible to disable merging by using in the subquery
  ...
  + [`UNION`](union.html "13.2.9.3 UNION Clause") or
    [`UNION
    ALL`](union.html "13.2.9.3 UNION Clause")
...
The [`derived_merge`](switchable-optimizations.html#optflag_derived-merge) flag also
...
Consider the following [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement")
statement, for a [`SELECT`](select.html "13.2.9 SELECT Statement") query
...
[`SELECT`](select.html "13.2.9 SELECT Statement") execution. In this case,
the query is not executed (because it occurs in an
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") statement), so the
