...
the `State` column for a replica server SQL
thread:
...
* `Making temporary file (append) before replaying
  ...
  DATA`](load-data.html "13.2.6 LOAD DATA Statement") statement and is appending the data to a
...
* `Making temporary file (create) before replaying
  ...
  The thread is executing a [`LOAD
  DATA`](load-data.html "13.2.6 LOAD DATA Statement") statement and is creating a temporary file
  containing the data from which the replica reads rows. This
  state can only be encountered if the original
  [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") statement was
