...
The [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") statement provides
information about how MySQL executes statements.
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") works with
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") returns a row of
information for each table used in the
[`SELECT`](select.html "13.2.9 SELECT Statement") statement. It lists the
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output includes partition
information. Also, for [`SELECT`](select.html "13.2.9 SELECT Statement")
statements, [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") generates
extended information that can be displayed with
[`SHOW WARNINGS`](show-warnings.html "13.7.5.40 SHOW WARNINGS Statement") following the
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") (see
...
was produced using
[`EXPLAIN
PARTITIONS`](explain.html "13.8.2 EXPLAIN Statement") and
[`EXPLAIN
EXTENDED`](explain.html "13.8.2 EXPLAIN Statement"). Those syntaxes are still recognized for
backward compatibility but partition and extended output is
now enabled by default, so the `PARTITIONS`
and `EXTENDED` keywords are superfluous and
deprecated. Their use results in a warning; expect them to be
removed from [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") syntax in
a future MySQL release.

You cannot use the deprecated `PARTITIONS`
and `EXTENDED` keywords together in the same
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") statement. In addition,
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output. See
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement"). Later sections provide
...
Each output row from [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement")
provides information about one table. Each row contains the
values summarized in
[Table 8.1, “EXPLAIN Output Columns”](explain-output.html#explain-output-column-table "Table 8.1 EXPLAIN Output Columns"), and described
...
* `id` (JSON name:
  ...
  The [`SELECT`](select.html "13.2.9 SELECT Statement") identifier. This
  is the sequential number of the
  [`SELECT`](select.html "13.2.9 SELECT Statement") within the query.
...
* `select_type` (JSON name: none)
  ...
  The type of [`SELECT`](select.html "13.2.9 SELECT Statement"), which
...
<td>None</td>
<td>Simple <code>SELECT</code> (not using
                    <code>UNION</code> or subqueries)</td>
</tr><tr>
<th><code>PRIMARY</code></th>
<td>None</td>
<td>Outermost <code>SELECT</code></td>
</tr><tr>
<th><code>UNION</code></th>
<td>None</td>
<td>Second or later <code>SELECT</code> statement in a
                    <code>UNION</code></td>
</tr><tr>
<th><code>DEPENDENT UNION</code></th>
<td><code>dependent</code> (<code>true</code>)</td>
<td>Second or later <code>SELECT</code> statement in a
                    <code>UNION</code>, dependent on
                    outer query</td>
</tr><tr>
<th><code>UNION RESULT</code></th>
<td><code>union_result</code></td>
<td>Result of a <code>UNION</code>.</td>
</tr><tr>
<th><code>SUBQUERY</code></th>
<td>None</td>
<td>First <code>SELECT</code> in subquery</td>
</tr><tr>
<th><code>DEPENDENT SUBQUERY</code></th>
<td><code>dependent</code> (<code>true</code>)</td>
<td>First <code>SELECT</code> in subquery, dependent on
                    outer query</td>
</tr><tr>
...
<td><code>cacheable</code> (<code>false</code>)</td>
<td>The second or later select in a <code>UNION</code>
                    that belongs to an uncacheable subquery (see
                    <code>UNCACHEABLE SUBQUERY</code>)</td>
</tr></tbody></table>

  `DEPENDENT` typically signifies the use
  of a correlated subquery. See
  [Section 13.2.10.7, “Correlated Subqueries”](correlated-subqueries.html "13.2.10.7 Correlated Subqueries").
  ...
  [Section 8.10.3.1, “How the Query Cache Operates”](query-cache-operation.html "8.10.3.1 How the Query Cache Operates")). Subquery caching
  ...
  non-[`SELECT`](select.html "13.2.9 SELECT Statement") statements
...
* `possible_keys` (JSON name:
  ...
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement"). That means that
  ...
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") again. See
...
* `key` (JSON name: `key`)
  ...
  It is possible for `key` to name an index
  ...
  your query. See [Section 8.9.4, “Index Hints”](index-hints.html "8.9.4 Index Hints").

  For `MyISAM` tables, running
  [`ANALYZE TABLE`](analyze-table.html "13.7.2.1 ANALYZE TABLE Statement") helps the
  optimizer choose better indexes. For
  `MyISAM` tables, [**myisamchk
  --analyze**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") does the same. See
  [Section 13.7.2.1, “ANALYZE TABLE Statement”](analyze-table.html "13.7.2.1 ANALYZE TABLE Statement"), and
...
* `ref` (JSON name: `ref`)
  ...
  [`SHOW WARNINGS`](show-warnings.html "13.7.5.40 SHOW WARNINGS Statement") following
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") to see the extended
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output. The
...
* `rows` (JSON name:
  ...
  For [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") tables, this
...
* `filtered` (JSON name:
  ...
  estimated percentage of table rows filtered by the table
  condition. The maximum value is 100, which means no
  ...
  examined and `rows` ×
  `filtered` shows the number of rows
  joined with the following table. For example, if
  `rows` is 1000 and
...
The `type` column of
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output describes how
...
* [`ref_or_null`](explain-output.html#jointype_ref_or_null)
  ...
  See [Section 8.2.1.13, “IS NULL Optimization”](is-null-optimization.html "8.2.1.13 IS NULL Optimization").
...
The `Extra` column of
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output contains
...
* `Deleting all rows` (JSON property:
  ...
  For [`DELETE`](delete.html "13.2.2 DELETE Statement"), some storage
  engines (such as [`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine"))
...
* `No matching rows after partition
  ...
  For [`DELETE`](delete.html "13.2.2 DELETE Statement") or
  [`UPDATE`](update.html "13.2.11 UPDATE Statement"), the optimizer found
  nothing to delete or update after partition pruning. It is
  similar in meaning to `Impossible WHERE`
  for [`SELECT`](select.html "13.2.9 SELECT Statement") statements.
...
* `No tables used` (JSON property:
  ...
  For [`INSERT`](insert.html "13.2.5 INSERT Statement") or
  [`REPLACE`](replace.html "13.2.8 REPLACE Statement") statements,
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") displays this value
  when there is no [`SELECT`](select.html "13.2.9 SELECT Statement")
...
* `Plan isn't ready yet` (JSON property:
  ...
  CONNECTION`](explain-for-connection.html "8.8.4 Obtaining Execution Plan Information for a Named Connection") when the optimizer has not finished
...
* `Range checked for each record (index map:
  ...
  [Section 8.2.1.2, “Range Optimization”](range-optimization.html "8.2.1.2 Range Optimization"), and
  [Section 8.2.1.3, “Index Merge Optimization”](index-merge-optimization.html "8.2.1.3 Index Merge Optimization"), with the
  ...
  as shown by [`SHOW INDEX`](show-index.html "13.7.5.22 SHOW INDEX Statement") for
...
* `Scanned N
  ...
  in [Section 8.2.3, “Optimizing INFORMATION\_SCHEMA Queries”](information-schema-optimization.html "8.2.3 Optimizing INFORMATION_SCHEMA Queries"). The
...
* `Skip_open_table`,
  ...
  [Section 8.2.3, “Optimizing INFORMATION\_SCHEMA Queries”](information-schema-optimization.html "8.2.3 Optimizing INFORMATION_SCHEMA Queries").
  ...
  + `Open_frm_only`: Only the
    table's `.frm` file need be
    opened.
...
* `Using join buffer (Block Nested Loop)`,
  ...
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output are
  ...
  `using_join_buffer` is always either one
  of `Block Nested Loop` or
  `Batched Key Access`.

  For more information about these algorithms, see
  [Block Nested-Loop Join Algorithm](nested-loop-joins.html#block-nested-loop-join-algorithm "Block Nested-Loop Join Algorithm"), and
...
* `Using MRR` (JSON property:
  ...
  strategy. See [Section 8.2.1.10, “Multi-Range Read Optimization”](mrr-optimization.html "8.2.1.10 Multi-Range Read Optimization").
...
* `Using sort_union(...)`, `Using
  ...
  See [Section 8.2.1.3, “Index Merge Optimization”](index-merge-optimization.html "8.2.1.3 Index Merge Optimization").
...
* `Using where with pushed condition` (JSON
  ...
  This item applies to [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6")
...
column of the [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") output.
...
multiple-table [`SELECT`](select.html "13.2.9 SELECT Statement")
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement").

Suppose that you have the
[`SELECT`](select.html "13.2.9 SELECT Statement") statement shown here and
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") statement produces the
...
this context, [`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") and
[`CHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") are considered the same if
...
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to lengthen
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") statement again
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") produces the output
shown here:
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") produces this result:
...
[`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") is an educated guess
...
[`SELECT`](select.html "13.2.9 SELECT Statement") statement and trying to
...
data when [`EXPLAIN
SELECT`](explain.html "13.8.2 EXPLAIN Statement") is used with a subquery; for more
information, see [Section 13.2.10.8, “Derived Tables”](derived-tables.html "13.2.10.8 Derived Tables").