...
* For a `MyISAM` table, you can use
  ...
  file. See [Section 8.11.3, “Concurrent Inserts”](concurrent-inserts.html "8.11.3 Concurrent Inserts").

* With some extra work, it is possible to make
  [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") run even faster for
  ...
  1. Execute a [`FLUSH TABLES`](flush.html#flush-tables)
     ...
     flush-tables**](mysqladmin.html "4.5.2 mysqladmin — A MySQL Server Administration Program") command.

  2. Use [**myisamchk --keys-used=0 -rq
     *`/path/to/db/tbl_name`***](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility")
     to remove all use of indexes for the table.

  3. Insert data into the table with
     [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement"). This does not
     update any indexes and therefore is very fast.

  4. If you intend only to read from the table in the future,
     use [**myisampack**](myisampack.html "4.6.5 myisampack — Generate Compressed, Read-Only MyISAM Tables") to compress it. See
  ...
  5. Re-create the indexes with [**myisamchk -rq
     *`/path/to/db/tbl_name`***](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility").
     ...
     during [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") because
  ...
  [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") performs the
  ...
  [**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") allocate much more temporary
  ...
  statements rather than [**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility"). If you
...
* To speed up [`INSERT`](insert.html "13.2.5 INSERT Statement") operations
  ...
  [`INSERT`](insert.html "13.2.5 INSERT Statement") statements have
  completed. Normally, there would be as many index buffer
  flushes as there are [`INSERT`](insert.html "13.2.5 INSERT Statement")
  ...
  [`DELETE`](delete.html "13.2.2 DELETE Statement") operations are very
  ...
  TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") followed by an
  [`UNLOCK
  TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") once in a while (each 1,000 rows or so) to
  permit other threads to access table. This would still
  result in a nice performance gain.

  [`INSERT`](insert.html "13.2.5 INSERT Statement") is still much slower
  for loading data than [`LOAD
  DATA`](load-data.html "13.2.6 LOAD DATA Statement"), even when using the strategies just
  outlined.

* To increase performance for `MyISAM`
  tables, for both [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement")
  and [`INSERT`](insert.html "13.2.5 INSERT Statement"), enlarge the key
