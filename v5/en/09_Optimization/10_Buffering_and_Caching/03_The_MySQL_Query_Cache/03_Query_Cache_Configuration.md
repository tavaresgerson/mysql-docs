...
The [`have_query_cache`](server-system-variables.html#sysvar_have_query_cache) server
...
starting [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server"). The query cache system
variables all have names that begin with
`query_cache_`. They are described briefly in
[Section 5.1.7, “Server System Variables”](server-system-variables.html "5.1.7 Server System Variables"), with additional
configuration information given here.

To set the size of the query cache, set the
[`query_cache_size`](server-system-variables.html#sysvar_query_cache_size) system
variable. Setting it to 0 disables the query cache, as does
setting [`query_cache_type=0`](server-system-variables.html#sysvar_query_cache_type).
By default, the query cache is disabled. This is achieved
using a default size of 1M, with a default for
`query_cache_type` of 0.

To reduce overhead significantly, start the server with
[`query_cache_type=0`](server-system-variables.html#sysvar_query_cache_type) if you do
not intend to use the query cache.

Note

When using the Windows Configuration Wizard to install or
configure MySQL, the default value for
[`query_cache_size`](server-system-variables.html#sysvar_query_cache_size) is
...
setting of the
[`query_cache_type`](server-system-variables.html#sysvar_query_cache_type) variable.
Check the values of these variables as set in your
`my.ini` file after configuration has
taken place.

When you set [`query_cache_size`](server-system-variables.html#sysvar_query_cache_size)
...
The [`query_cache_size`](server-system-variables.html#sysvar_query_cache_size) value is
...
If [`query_cache_size`](server-system-variables.html#sysvar_query_cache_size) is 0, you
should also set
[`query_cache_type`](server-system-variables.html#sysvar_query_cache_type) variable to
...
Setting the `GLOBAL`
[`query_cache_type`](server-system-variables.html#sysvar_query_cache_type) value
...
`SESSION`
[`query_cache_type`](server-system-variables.html#sysvar_query_cache_type) value. For
...
If you set [`query_cache_type`](server-system-variables.html#sysvar_query_cache_type)
...
[`query_cache_limit`](server-system-variables.html#sysvar_query_cache_limit) system
...
* If you have a lot of queries with small results, the
  ...
  [`Qcache_free_blocks`](server-status-variables.html#statvar_Qcache_free_blocks) and
...
* If most of your queries have large results (check the
  [`Qcache_total_blocks`](server-status-variables.html#statvar_Qcache_total_blocks) and
