...
memory with the [`FLUSH QUERY
CACHE`](flush.html#flush-query-cache) statement. The statement does not remove any
queries from the cache.

The `RESET QUERY CACHE` statement removes all
query results from the query cache. The
[`FLUSH TABLES`](flush.html#flush-tables) statement also
does this.

To monitor query cache performance, use
[`SHOW STATUS`](show-status.html "13.7.5.35 SHOW STATUS Statement") to view the cache
status variables:
...
[Section 5.1.9, “Server Status Variables”](server-status-variables.html "5.1.9 Server Status Variables"). Some uses for them
are described here.

The total number of [`SELECT`](select.html "13.2.9 SELECT Statement")
...
[`Qcache_free_blocks`](server-status-variables.html#statvar_Qcache_free_blocks) may
indicate query cache memory fragmentation. After
[`FLUSH QUERY CACHE`](flush.html#flush-query-cache), only a
...
[`Qcache_lowmem_prunes`](server-status-variables.html#statvar_Qcache_lowmem_prunes) status
