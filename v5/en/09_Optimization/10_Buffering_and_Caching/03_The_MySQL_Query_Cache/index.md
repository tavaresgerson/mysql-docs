...
[8.10.3.1 How the Query Cache Operates](query-cache-operation.html)

[8.10.3.2 Query Cache SELECT Options](query-cache-in-select.html)

[8.10.3.3 Query Cache Configuration](query-cache-configuration.html)

[8.10.3.4 Query Cache Status and Maintenance](query-cache-status-and-maintenance.html)
...
[`SELECT`](select.html "13.2.9 SELECT Statement") statement together with
...
multiple [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") servers updating the same
`MyISAM` tables.

The query cache is used for prepared statements under the
conditions described in [Section 8.10.3.1, “How the Query Cache Operates”](query-cache-operation.html "8.10.3.1 How the Query Cache Operates").
...
improvement, but do not assume that it does so under all
...
* Server workload has a significant effect on query cache
  ...
  fixed set of [`SELECT`](select.html "13.2.9 SELECT Statement")
  statements is much more likely to benefit from enabling the
  cache than a mix in which frequent
  [`INSERT`](insert.html "13.2.5 INSERT Statement") statements cause
  ...
  [`SELECT`](select.html "13.2.9 SELECT Statement") statements that use
