...
* Prepared statements, both those processed at the SQL level
  (using the [`PREPARE`](prepare.html "13.5.1 PREPARE Statement") statement)
  and those processed using the binary client/server protocol
  (using the
  [`mysql_stmt_prepare()`](/doc/c-api/5.7/en/mysql-stmt-prepare.html) C API
  function). The
...
* Stored programs (stored procedures and functions, triggers,
  ...
  [`stored_program_cache`](server-system-variables.html#sysvar_stored_program_cache) system
...
example, with [`INSERT`](insert.html "13.2.5 INSERT Statement") or
[`UPDATE`](update.html "13.2.11 UPDATE Statement")) do not change metadata,
nor do [`SELECT`](select.html "13.2.9 SELECT Statement") statements.
...
[`IF`](if.html "13.6.5.2 IF Statement"),
[`CASE`](case.html "13.6.5.1 CASE Statement"), and
[`RETURN`](return.html "13.6.5.7 RETURN Statement").
...
* When a statement is affected, the server reparses it only
  ...
  [`CASE`](case.html "13.6.5.1 CASE Statement") statement:
...
[`Com_stmt_reprepare`](server-status-variables.html#statvar_Com_xxx)
