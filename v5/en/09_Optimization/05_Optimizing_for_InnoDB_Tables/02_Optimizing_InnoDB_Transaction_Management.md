...
* Alternatively, for transactions that consist only of a
  single [`SELECT`](select.html "13.2.9 SELECT Statement") statement,
  turning on `AUTOCOMMIT` helps
  `InnoDB` to recognize read-only
  transactions and optimize them. See
  [Section 8.5.3, “Optimizing InnoDB Read-Only Transactions”](innodb-performance-ro-txn.html "8.5.3 Optimizing InnoDB Read-Only Transactions") for
  requirements.
...
* Avoid performing rollbacks after inserting, updating, or
  ...
  + Increase the size of the
    [buffer pool](glossary.html#glos_buffer_pool "buffer pool") so
    that all the data change changes can be cached rather
    than immediately written to disk.

  + Set
    [`innodb_change_buffering=all`](innodb-parameters.html#sysvar_innodb_change_buffering)
  ...
  [`innodb_force_recovery=3`](innodb-parameters.html#sysvar_innodb_force_recovery), as
  explained in [Section 14.19.2, “InnoDB Recovery”](innodb-recovery.html "14.19.2 InnoDB Recovery").
...
* If you can afford the loss of some of the latest committed
  ...
  [`innodb_support_xa`](innodb-parameters.html#sysvar_innodb_support_xa) to 0,
  which reduces the number of disk flushes due to
  ...
  As of MySQL 5.7.10, `InnoDB` support for
...
* When rows are modified or deleted, the rows and associated
  [undo logs](glossary.html#glos_undo_log "undo log") are not
...
* When a long-running transaction modifies a table, queries
  ...
  of the [covering
  index](glossary.html#glos_covering_index "covering index") technique. Queries that normally could retrieve
