...
* Make your redo log files big, even as big as the
  [buffer pool](glossary.html#glos_buffer_pool "buffer pool"). When
  `InnoDB` has written the redo log files
  full, it must write the modified contents of the buffer pool
  to disk in a
  [checkpoint](glossary.html#glos_checkpoint "checkpoint"). Small
  ...
  [Changing the Number or Size of InnoDB Redo Log Files](innodb-redo-log.html#innodb-redo-log-file-reconfigure "Changing the Number or Size of InnoDB Redo Log Files").
