...
2. If the database does not already exist, issue
   ...
   [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client to create it.
...
Because support for `.sym` files is
redundant with native symlink support available using
**mklink**, use of `.sym`
files is deprecated; expect support for them to be removed
...
* If you run [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") as a service, you
