...
* `MERGE` child tables are locked through the
  ...
  [`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine") tables corrupts them.

* If you use [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to
...
* A `MERGE` table cannot maintain uniqueness
  ...
  [`INSERT`](insert.html "13.2.5 INSERT Statement"), the data goes into the
...
* Because the `MERGE` engine cannot enforce
  ...
  [`REPLACE`](replace.html "13.2.8 REPLACE Statement") does not work as
  expected. The two key facts are:

  + [`REPLACE`](replace.html "13.2.8 REPLACE Statement") can detect unique
  ...
  + If [`REPLACE`](replace.html "13.2.8 REPLACE Statement") detects a unique
...
* You should not use [`ANALYZE
  TABLE`](analyze-table.html "13.7.2.1 ANALYZE TABLE Statement"), [`REPAIR TABLE`](repair-table.html "13.7.2.5 REPAIR TABLE Statement"),
  ...
  [`DELETE`](delete.html "13.2.2 DELETE Statement") without a
  `WHERE` clause, or
  [`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") on any of the
  ...
  [`FLUSH TABLES`](flush.html#flush-tables) statement prior to
  ...
  [`FLUSH TABLES`](flush.html#flush-tables) statement after
  modifying the `MyISAM` tables.

* [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") on a table that is
  ...
  [`FLUSH TABLES`](flush.html#flush-tables)) or drop the
...
* The definition of the `MyISAM` tables and the
  ...
  [`INSERT`](insert.html "13.2.5 INSERT Statement") statement). The checks
  ...
  alterations causes the statement to fail.

* The order of indexes in the `MERGE` table and
  its underlying tables should be the same. If you use
  [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to add a
  `UNIQUE` index to a table used in a
  `MERGE` table, and then use
  [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to add a nonunique
  ...
  [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") puts
...
* You cannot change the union list of a nontemporary
  ...
  TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") is in effect. The following does
  *not* work:
