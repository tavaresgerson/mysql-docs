...
* Support for variable-length data types (including
  ...
  [`TEXT`](blob.html "11.3.4 The BLOB and TEXT Types")) not supported by
  `MEMORY`.
...
[`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") tables on a busy server, for
...
* `MEMORY` tables use a fixed-length
  ...
  [`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") are stored using a
  fixed length.
...
stored in `.frm` files on disk, but they are
...
[`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement"), or a subsequent
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") or
[`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement"), is the value used
...
[`INSERT INTO ...
SELECT`](insert-select.html "13.2.5.1 INSERT ... SELECT Statement") or [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") into
...
[Section 5.1.7, “Server System Variables”](server-system-variables.html "5.1.7 Server System Variables"), and
...
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables become empty. To
replicate this effect to replicas, the first time that the source
uses a given [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") table after
startup, it logs an event that notifies replicas that the table
must be emptied by writing a [`DELETE`](delete.html "13.2.2 DELETE Statement")
or (from MySQL 5.7.32) [`TRUNCATE
TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") statement for that table to the binary log. When a
replica server shuts down and restarts, its
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables also become empty, and
it writes a [`DELETE`](delete.html "13.2.2 DELETE Statement") or (from MySQL
5.7.32) [`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") statement to
its own binary log, which is passed on to any downstream replicas.

When you use [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables in a
replication topology, in some situations, the table on the source
and the table on the replica may differ. For information on
handling each of these situations to prevent stale reads or
errors, see [Section 16.4.1.20, “Replication and MEMORY Tables”](replication-features-memory.html "16.4.1.20 Replication and MEMORY Tables").
...
[`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") to remove all rows,
or remove the table altogether using [`DROP
TABLE`](drop-table.html "13.1.29 DROP TABLE Statement"). To free up the memory used by deleted rows, use
...
[`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statements for
