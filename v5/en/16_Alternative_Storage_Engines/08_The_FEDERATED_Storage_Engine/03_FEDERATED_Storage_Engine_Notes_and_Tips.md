...
* A `FEDERATED` table does not support indexes
  ...
  this [`SELECT`](select.html "13.2.9 SELECT Statement") statement; these
...
* Care should be taken when creating a
  ...
  `FEDERATED` table with an index prefix fails
  for [`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types"),
  [`TEXT`](blob.html "11.3.4 The BLOB and TEXT Types") or
  [`BLOB`](blob.html "11.3.4 The BLOB and TEXT Types") columns. The following
...
* Internally, the implementation uses
  ...
  [`DELETE`](delete.html "13.2.2 DELETE Statement"), but not
  [`HANDLER`](handler.html "13.2.4 HANDLER Statement").
...
* The `FEDERATED` storage engine supports
  ...
  [`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement"), and indexes. It
  does not support [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement"),
  or any Data Definition Language statements that directly
  affect the structure of the table, other than
  [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement"). The current
  implementation does not use prepared statements.

* `FEDERATED` accepts
  [`INSERT
  ... ON DUPLICATE KEY UPDATE`](insert-on-duplicate.html "13.2.5.2 INSERT ... ON DUPLICATE KEY UPDATE Statement") statements, but if a
...
* When using a `CONNECTION` string, you cannot
  ...
  SERVER`](create-server.html "13.1.17 CREATE SERVER Statement") statement to create a server connection.

* The [`insert_id`](server-system-variables.html#sysvar_insert_id) and
  [`timestamp`](server-system-variables.html#sysvar_timestamp) options are not
  propagated to the data provider.

* Any [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") statement issued
