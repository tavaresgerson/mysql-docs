...
If a `MERGE` table cannot be opened or used because of a problem with an underlying table, [`CHECK TABLE`](check-table.html "13.7.2.2 CHECK TABLE Statement") displays information about which table caused the problem.
...
* Because the `MERGE` engine cannot enforce uniqueness over the set of underlying tables, `REPLACE` does not work as expected. The two key facts are:
  ...
  Similar considerations apply for [`INSERT ... ON DUPLICATE KEY UPDATE`](insert-on-duplicate.html "13.2.5.2 INSERT ... ON DUPLICATE KEY UPDATE Statement").
...
* You cannot change the union list of a nontemporary `MERGE` table when [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") is in effect. The following does *not* work:
