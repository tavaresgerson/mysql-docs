...
[`CHECK TABLE`](check-table.html "13.7.2.2 CHECK TABLE Statement") or
[**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility"), it means that this counter has
gone out of sync:
...
This warning does not necessarily mean that the table is
...
* When the last instance of a table is closed (because a
  [`FLUSH TABLES`](flush.html#flush-tables) operation was
...
* A `MyISAM` table is copied without first
  issuing [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") and
...
* A table was modified by [**myisamchk
  --recover**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") or [**myisamchk
  --update-state**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") at the same time that it was in use
  by [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server").

* Multiple [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") servers are using the
  table and one server performed a [`REPAIR
  TABLE`](repair-table.html "13.7.2.5 REPAIR TABLE Statement") or [`CHECK
  TABLE`](check-table.html "13.7.2.2 CHECK TABLE Statement") on the table while it was in use by another
  server. In this setup, it is safe to use
  [`CHECK TABLE`](check-table.html "13.7.2.2 CHECK TABLE Statement"), although you
  might get the warning from other servers. However,
  [`REPAIR TABLE`](repair-table.html "13.7.2.5 REPAIR TABLE Statement") should be
  ...
  multiple servers. See [Section 5.7, “Running Multiple MySQL Instances on One Machine”](multiple-servers.html "5.7 Running Multiple MySQL Instances on One Machine"),
