...
[`-DWITH_BLACKHOLE_STORAGE_ENGINE`](source-configuration-options.html#option_cmake_storage_engine_options "Storage Engine Options")
option.
...
ENGINES`](show-engines.html "13.7.5.16 SHOW ENGINES Statement") statement.
...
source host a “dummy” replica process whose default
...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") process acts as a replica, applying the
desired combination of `replicate-do-*` and
`replicate-ignore-*` rules, and writes a new,
filtered binary log of its own. (See
[Section 16.1.6, “Replication and Binary Logging Options and Variables”](replication-options.html "16.1.6 Replication and Binary Logging Options and Variables").) This filtered log is
...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") process on the replication source host.
This type of setup can be repeated with additional replication
replicas.

[`INSERT`](insert.html "13.2.5 INSERT Statement") triggers for
`BLACKHOLE` tables work as expected. However,
because the `BLACKHOLE` table does not actually
store any data, [`UPDATE`](update.html "13.2.11 UPDATE Statement") and
[`DELETE`](delete.html "13.2.2 DELETE Statement") triggers are not activated:
...
a table using BLACKHOLE have no effect. This should be borne in mind
when considering the behavior of primary key columns that auto
increment. The engine does not automatically increment field values,
and does not retain auto increment column state. This has important
...
([`binlog_format=ROW`](replication-options-binary-log.html#sysvar_binlog_format)), a replica
where the last columns are missing from a table is supported, as
described in the section
[Section 16.4.1.10, “Replication with Differing Table Definitions on Source and Replica”](replication-features-differing-tables.html "16.4.1.10 Replication with Differing Table Definitions on Source and Replica").
...
[`--replicate-do-table`](replication-options-replica.html#option_mysqld_replicate-do-table) or
[`--replicate-ignore-table`](replication-options-replica.html#option_mysqld_replicate-ignore-table) option.
