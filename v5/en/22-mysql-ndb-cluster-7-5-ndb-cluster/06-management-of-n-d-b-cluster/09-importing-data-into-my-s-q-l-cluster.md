...
* A CSV file produced by [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") or other export program. Such files can be imported into `NDB` using `LOAD DATA INFILE` in the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client, or with the [**ndb\_import**](mysql-cluster-programs-ndb-import.html "21.5.14 ndb_import — Import CSV Data Into NDB") utility provided with the NDB Cluster distribution. For more information about the latter, see [Section 21.5.14, “ndb\_import — Import CSV Data Into NDB”](mysql-cluster-programs-ndb-import.html "21.5.14 ndb_import — Import CSV Data Into NDB").

* A native `NDB` backup produced using [`START BACKUP`](mysql-cluster-backup-using-management-client.html "21.6.8.2 Using The NDB Cluster Management Client to Create a Backup") in the `NDB` management client. To import a native backup, you must use the [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") program that comes as part of NDB Cluster. See [Section 21.5.24, “ndb\_restore — Restore an NDB Cluster Backup”](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup"), for more about using this program.
