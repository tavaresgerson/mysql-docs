...
[`INSERT`](insert.html "13.2.5 INSERT Statement") statements are useful for
this.
...
<table summary="Example of pet records mentioned in the preceding text."><col style="width: 10%"/><col style="width: 10%"/><col style="width: 10%"/><col style="width: 05%"/><col style="width: 20%"/><col style="width: 20%"/><thead><tr>
<th>name</th>
<th>owner</th>
<th>species</th>
<th>sex</th>
...
DATA`](load-data.html "13.2.6 LOAD DATA Statement") statement if you wish, but the defaults are tab
...
default. See [Section 6.1.6, “Security Considerations for LOAD DATA LOCAL”](load-data-local-security.html "6.1.6 Security Considerations for LOAD DATA LOCAL"), for
information on how to change this.

When you want to add new records one at a time, the
[`INSERT`](insert.html "13.2.5 INSERT Statement") statement is useful. In
its simplest form, you supply values for each column, in the
order in which the columns were listed in the
[`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement. Suppose
that Diane gets a new hamster named “Puffball.” You
could add a new record using an
[`INSERT`](insert.html "13.2.5 INSERT Statement") statement like this:
...
Also, with [`INSERT`](insert.html "13.2.5 INSERT Statement"), you can insert
...
several [`INSERT`](insert.html "13.2.5 INSERT Statement") statements rather
than a single [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement")
statement.