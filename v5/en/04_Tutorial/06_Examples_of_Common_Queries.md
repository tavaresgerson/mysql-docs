...
For information about `AUTO_INCREMENT` usage specific to `InnoDB`, see Section 14.6.1.6, “AUTO\_INCREMENT Handling in InnoDB”.
...
* For `MyISAM` tables, you can specify `AUTO_INCREMENT` on a secondary column in a multiple-column index. In this case, the generated value for the `AUTO_INCREMENT` column is calculated as `MAX(auto_increment_column)`
  ...
  + `WHERE prefix=given-prefix`. This is useful when you want to put data into ordered groups.
...
* `AUTO_INCREMENT` and replication: Section 16.4.1.1, “Replication and AUTO\_INCREMENT”.
