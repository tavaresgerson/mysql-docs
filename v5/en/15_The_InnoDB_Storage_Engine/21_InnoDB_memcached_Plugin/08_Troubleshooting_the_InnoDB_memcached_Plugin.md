...
* If the `daemon_memcached` plugin causes MySQL
  ...
  PLUGIN`](install-plugin.html "13.7.3.3 INSTALL PLUGIN Statement") statement before running the
...
* When defining your own `InnoDB` table for use
  ...
  [`INSERT`](insert.html "13.2.5 INSERT Statement") statement for the
  ...
  [`INSERT`](insert.html "13.2.5 INSERT Statement") to fail, which may only
...
* As the length of the **memcached** key and
  ...
  + `InnoDB` table limits may be encountered
    ...
    [Section 14.23, “InnoDB Limits”](innodb-limits.html "14.23 InnoDB Limits") for details.
