...
updates made on a source server through the
**memcached** interface can be replicated for
backup, balancing intensive read workloads, and high availability.
All **memcached** commands are supported with
binary logging.
...
described in [Section 14.21.3, “Setting Up the InnoDB memcached Plugin”](innodb-memcached-setup.html "14.21.3 Setting Up the InnoDB memcached Plugin").
...
1. To use the `daemon_memcached` plugin with
   ...
   [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") command line.
...
3. Use [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") to create a source data
...
4. On the source server, issue [`SHOW MASTER
   STATUS`](show-master-status.html "13.7.5.23 SHOW MASTER STATUS Statement") to obtain the source binary log
   coordinates.
...
5. On the replica server, use a [`CHANGE
   MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") statement to set up a replica server
...
* The **memcached**
  ...
  [`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") command. Since
...
* The default value of
  ...
  **memcached** write operation is committed
  ...
  issue [`SET
  TRANSACTION ISOLATION LEVEL READ UNCOMMITTED`](set-transaction.html "13.3.6 SET TRANSACTION Statement").