...
* When querying a table that is also accessed through the
  ...
  **memcached** operations can be configured to
  ...
  `1`, use [`READ
  UNCOMMITTED`](innodb-transaction-isolation-levels.html#isolevel_read-uncommitted) queries to find rows that were just
  inserted.
...
* The `InnoDB` table is either IS (intention
  ...
  [DDL](glossary.html#glos_ddl "DDL") statements on the table.