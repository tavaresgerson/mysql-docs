...
plugin, consult [Section 14.21.4, “Security Considerations for the InnoDB memcached Plugin”](innodb-memcached-security.html "14.21.4 Security Considerations for the InnoDB memcached Plugin") to
...
* When building MySQL from source, you must build with
  [`-DWITH_INNODB_MEMCACHED=ON`](source-configuration-options.html#option_cmake_with_innodb_memcached).
  This build option generates two shared libraries in the
  MySQL plugin directory
  ([`plugin_dir`](server-system-variables.html#sysvar_plugin_dir)) that are
...
* `libevent` must be installed.
  ...
  + If you installed MySQL from a source code release,
    ...
    the [`-DWITH_LIBEVENT`](source-configuration-options.html#option_cmake_with_libevent) build
...
2. Activate the `daemon_memcached` plugin by
   running the [`INSTALL PLUGIN`](install-plugin.html "13.7.3.3 INSTALL PLUGIN Statement")
   statement:
...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") startup string, encoded in the
argument of the
...
[`daemon_memcached_option`](innodb-parameters.html#sysvar_daemon_memcached_option) string.
...
* [`daemon_memcached_r_batch_size`](innodb-parameters.html#sysvar_daemon_memcached_r_batch_size):
  ...
  **memcached** read operations after which a
  [commit](glossary.html#glos_commit "commit") occurs.
  [`daemon_memcached_r_batch_size`](innodb-parameters.html#sysvar_daemon_memcached_r_batch_size)
...
in a [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") startup string. They take effect
