...
the `LOCK` clause specifies a less restrictive
...
* `LOCK=DEFAULT`:
  ...
  of the DDL statement does not cause availability problems
  for the table.
...
The session 1 [`SELECT`](select.html "13.2.9 SELECT Statement") statement
...
The [`SELECT`](select.html "13.2.9 SELECT Statement") statement issued in
session 3 is blocked waiting for the exclusive metadata lock
requested by the [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement")
operation in session 2 to be granted.

You can use
[`SHOW FULL
PROCESSLIST`](show-processlist.html "13.7.5.29 SHOW PROCESSLIST Statement") to determine if transactions are waiting
for a metadata lock.
...
Performance Schema [`metadata_locks`](performance-schema-metadata-locks-table.html "25.12.12.1 The metadata_locks Table")
...
compare results using `ALGORITHM=INPLACE` with
results using `ALGORITHM=COPY`. Alternatively,
you can compare results with
[`old_alter_table`](server-system-variables.html#sysvar_old_alter_table) disabled and
enabled.
...
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") progress. See
