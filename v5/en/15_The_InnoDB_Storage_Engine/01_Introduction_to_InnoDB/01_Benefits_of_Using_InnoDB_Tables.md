...
* The `InnoDB` storage engine maintains its own
  ...
  See [Section 14.5.1, “Buffer Pool”](innodb-buffer-pool.html "14.5.1 Buffer Pool").
...
* When you design a database with appropriate primary key
  ...
  [`WHERE`](select.html "13.2.9 SELECT Statement")
  clauses, [`ORDER
  BY`](select.html "13.2.9 SELECT Statement") clauses,
  [`GROUP BY`](select.html "13.2.9 SELECT Statement")
...
* Performance benefits are not limited to large tables with
  ...
  table. See [Section 14.5.3, “Adaptive Hash Index”](innodb-adaptive-hash.html "14.5.3 Adaptive Hash Index").
...
* The storage layout for table data is more efficient for
  [`BLOB`](blob.html "11.3.4 The BLOB and TEXT Types") and long text fields, with
...
* You can mix `InnoDB` tables with tables from
  ...
  `InnoDB` and
  [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables in a single query.
...
[Section 8.5, “Optimizing for InnoDB Tables”](optimizing-innodb.html "8.5 Optimizing for InnoDB Tables").