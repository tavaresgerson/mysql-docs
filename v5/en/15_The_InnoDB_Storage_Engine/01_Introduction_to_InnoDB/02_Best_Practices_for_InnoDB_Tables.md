...
* Group sets of related DML operations into transactions by
  ...
  [`DELETE`](delete.html "13.2.2 DELETE Statement") statements that run for
  hours without committing.

* Do not use [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements")
  ...
  [`SELECT
  ... FOR UPDATE`](innodb-locking-reads.html "14.7.2.4 Locking Reads") syntax to lock just the rows you
  intend to update.
...
* Run the server with the
  [`--sql_mode=NO_ENGINE_SUBSTITUTION`](server-system-variables.html#sysvar_sql_mode)
