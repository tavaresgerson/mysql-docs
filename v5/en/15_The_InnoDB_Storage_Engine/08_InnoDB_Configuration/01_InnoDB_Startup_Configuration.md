...
* [Specifying Options in a MySQL Configuration File](innodb-init-startup-configuration.html#innodb-startup-mysql-configuration-file "Specifying Options in a MySQL Configuration File")
...
* [Temporary Tablespace Configuration](innodb-init-startup-configuration.html#innodb-startup-temporary-tablespace "Temporary Tablespace Configuration")
...
You can place `InnoDB` settings in the
`[mysqld]` group of any option file that your
server reads when it starts. The locations of MySQL option files
are described in [Section 4.2.2.2, “Using Option Files”](option-files.html "4.2.2.2 Using Option Files").

To make sure that [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") reads options only
from a specific file, use the
[`--defaults-file`](option-file-options.html#option_general_defaults-file) option as the
...
during startup, start [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") from a command
prompt, which prints initialization information to the console.

For example, on Windows, if [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") is located
in `C:\Program Files\MySQL\MySQL Server
5.7\bin`, start the MySQL server like
this:
...
On Unix-like systems, [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") is located in
...
* `InnoDB` is a transaction-safe (ACID
  ...
  **However, it cannot do so** if
  ...
  the database because write operation have been reordered. If
  data integrity is important to you, perform
...
* With regard to `InnoDB` recovery
  ...
  doublewrite buffer, see [Section 14.12.1, “InnoDB Disk I/O”](innodb-disk-io.html "14.12.1 InnoDB Disk I/O").
...
* Running MySQL server on a 4K sector hard drive on Windows is
  not supported with
  [`innodb_flush_method=async_unbuffered`](innodb-parameters.html#sysvar_innodb_flush_method),
...
If you specify
[`innodb_data_home_dir`](innodb-parameters.html#sysvar_innodb_data_home_dir) as an
empty string, you can specify absolute paths for data files
listed in the
[`innodb_data_file_path`](innodb-parameters.html#sysvar_innodb_data_file_path) value.
...
`InnoDB` creates two 5MB redo log files named
`ib_logfile0` and
...
* [`innodb_log_group_home_dir`](innodb-parameters.html#sysvar_innodb_log_group_home_dir)
  ...
  files. If this option is not configured,
  `InnoDB` log files are created in the MySQL
  data directory ([`datadir`](server-system-variables.html#sysvar_datadir)).
...
Undo logs are part of the system tablespace by default. However,
you can choose to store undo logs in one or more separate undo
...
A single auto-extending temporary tablespace data file named
`ibtmp1` is created in the
...
The default temporary tablespace data file configuration can be
modified at startup using the
...
temporary tablespace data files. The full directory path is
formed by concatenating
[`innodb_data_home_dir`](innodb-parameters.html#sysvar_innodb_data_home_dir) to the
path specified by
[`innodb_temp_data_file_path`](innodb-parameters.html#sysvar_innodb_temp_data_file_path).
File size is specified in KB, MB, or GB (1024MB) by appending K,
M, or G to the size value. The file size or combined file size
must be slightly larger than 12MB.

The [`innodb_data_home_dir`](innodb-parameters.html#sysvar_innodb_data_home_dir)
...
see
[Managing Temporary Tablespace Data File Size](innodb-temporary-tablespace.html#innodb-temporary-tablespace-data-file-size "Managing Temporary Tablespace Data File Size").
...
* [`innodb_buffer_pool_size`](innodb-parameters.html#sysvar_innodb_buffer_pool_size)
  ...
  [Section 8.12.4.1, “How MySQL Uses Memory”](memory-use.html "8.12.4.1 How MySQL Uses Memory"). For information about how to
  configure `InnoDB` buffer pool size, see
  [Section 14.8.3.1, “Configuring InnoDB Buffer Pool Size”](innodb-buffer-pool-resize.html "14.8.3.1 Configuring InnoDB Buffer Pool Size"). Buffer pool
...
if the memory allocated to the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server")
...
see [Section 8.12.4.1, “How MySQL Uses Memory”](memory-use.html "8.12.4.1 How MySQL Uses Memory").
...
for its buffer pool. See [Section 8.12.4.3, “Enabling Large Page Support”](large-page-support.html "8.12.4.3 Enabling Large Page Support").