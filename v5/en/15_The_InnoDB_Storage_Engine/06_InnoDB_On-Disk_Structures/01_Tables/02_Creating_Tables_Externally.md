...
`DATA DIRECTORY` clause cannot be used with
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to change the
location later.
...
links](symbolic-links.html "8.12.3 Using Symbolic Links") are not supported for use with
`InnoDB` data files.)
...
* MySQL initially holds the tablespace data file open,
  ...
  `.isl` file from the schema directory.
  After restarting, drop the table to remove the
  `.frm` file and the information about the
  table from the [data
...
* Using the `DATA DIRECTORY` clause to create
  ...
  [symbolic links](symbolic-links.html "8.12.3 Using Symbolic Links"), which
...
[`CREATE TABLE ...
TABLESPACE`](create-table.html "13.1.18 CREATE TABLE Statement") syntax can be used in combination with the
