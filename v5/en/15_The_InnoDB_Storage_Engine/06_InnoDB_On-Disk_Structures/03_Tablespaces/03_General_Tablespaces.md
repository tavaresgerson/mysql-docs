...
TABLESPACE`](create-tablespace.html "13.1.19 CREATE TABLESPACE Statement") syntax. General tablespace capabilities and
...
* General tablespaces have a potential memory advantage over
  [file-per-table
  tablespaces](innodb-file-per-table-tablespaces.html "14.6.3.2 File-Per-Table Tablespaces"). The server keeps tablespace metadata in
...
* General tablespace data files can be placed in a directory
  ...
  [file-per-table
  tablespaces](innodb-file-per-table-tablespaces.html "14.6.3.2 File-Per-Table Tablespaces"). As with file-per-table tablespaces, the
...
* General tablespaces support both Antelope and Barracuda file
  ...
  [`innodb_file_format`](innodb-parameters.html#sysvar_innodb_file_format) or
...
* The `TABLESPACE` option can be used with
  [`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") to create tables
  in a general tablespaces, file-per-table tablespace, or in
  the system tablespace.

* The `TABLESPACE` option can be used with
  [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to move tables
...
[`CREATE TABLESPACE`](create-tablespace.html "13.1.19 CREATE TABLESPACE Statement") syntax.
...
An [.isl](glossary.html#glos_isl_file ".isl file") file is created in
...
TABLESPACE`](create-tablespace.html "13.1.19 CREATE TABLESPACE Statement") statement, or `InnoDB`
...
[`ALTER TABLE
tbl_name TABLESPACE [=]
tablespace_name`](alter-table.html "13.1.8 ALTER TABLE Statement") statements
...
TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") and [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement").
...
**Table 14.3 Permitted Page Size, FILE\_BLOCK\_SIZE, and KEY\_BLOCK\_SIZE Combinations
for Compressed Tables**

<table frame="all"><col style="width: 33%"/><col style="width: 33%"/><col style="width: 34%"/><thead><tr>
<th>InnoDB Page Size (innodb_page_size)</th>
<th>Permitted FILE_BLOCK_SIZE Value</th>
<th>Permitted KEY_BLOCK_SIZE Value</th>
</tr></thead><tbody><tr>
<th>64KB</th>
<td>64K (65536)</td>
...
<th>16KB</th>
<td>16K (16384)</td>
<td>None. If <code>innodb_page_size</code> is equal to
              <code>FILE_BLOCK_SIZE</code>, the tablespace cannot
...
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") with the
...
[`ALTER TABLESPACE`](alter-tablespace.html "13.1.9 ALTER TABLESPACE Statement") for more
information.
...
The [`DROP TABLESPACE`](drop-tablespace.html "13.1.30 DROP TABLESPACE Statement") statement is
used to drop an `InnoDB` general tablespace.

All tables must be dropped from the tablespace prior to a
[`DROP TABLESPACE`](drop-tablespace.html "13.1.30 DROP TABLESPACE Statement") operation. If the
tablespace is not empty, [`DROP
TABLESPACE`](drop-tablespace.html "13.1.30 DROP TABLESPACE Statement") returns an error.
...
If a [`DROP TABLESPACE`](drop-tablespace.html "13.1.30 DROP TABLESPACE Statement") operation on
...
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") operation that was
...
[`DROP TABLESPACE
tablespace_name`](drop-tablespace.html "13.1.30 DROP TABLESPACE Statement").

A general tablespace does not belong to any particular database.
A [`DROP DATABASE`](drop-database.html "13.1.22 DROP DATABASE Statement") operation can
drop tables that belong to a general tablespace but it cannot
drop the tablespace, even if the [`DROP
DATABASE`](drop-database.html "13.1.22 DROP DATABASE Statement") operation drops all tables that belong to the
tablespace.
...
is deleted during a [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement")
operation.
...
* Similar to the system tablespace, truncating or dropping
  ...
  TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") operation on table that resides in a shared
  ...
  the table-copying [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement")
...
* Support for placing table partitions in general tablespaces
  was deprecated in MySQL 5.7.24; expect it to be removed in a
