...
`InnoDB` data dictionary, the doublewrite buffer,
...
1. Use [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") to dump all of your
   `InnoDB` tables, including
   `InnoDB` tables located in the
   `mysql` schema. Identify
   `InnoDB` tables in the
   `mysql` schema using the following query:
...
4. Remove any `.frm` files for
   `InnoDB` tables.

5. Configure the data files for the new system tablespace. See
   [System Tablespace Data File Configuration](innodb-init-startup-configuration.html#innodb-startup-data-file-configuration "System Tablespace Data File Configuration").
...
server, remove all databases and `InnoDB` log
...
[Section 14.6.3.2, “File-Per-Table Tablespaces”](innodb-file-per-table-tablespaces.html "14.6.3.2 File-Per-Table Tablespaces"). General
...
For additional information, see [Section 4.2.2.2, “Using Option Files”](option-files.html "4.2.2.2 Using Option Files").
