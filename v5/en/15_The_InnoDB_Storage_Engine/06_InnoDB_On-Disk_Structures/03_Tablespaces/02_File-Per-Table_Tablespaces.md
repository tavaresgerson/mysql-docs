...
* [File-Per-Table Tablespace Configuration](innodb-file-per-table-tablespaces.html#innodb-file-per-table-configuration "File-Per-Table Tablespace Configuration")
* [File-Per-Table Tablespace Data Files](innodb-file-per-table-tablespaces.html#innodb-file-per-table-data-files "File-Per-Table Tablespace Data Files")
* [File-Per-Table Tablespace Advantages](innodb-file-per-table-tablespaces.html#innodb-file-per-table-advantages "File-Per-Table Tablespace Advantages")
* [File-Per-Table Tablespace Disadvantages](innodb-file-per-table-tablespaces.html#innodb-file-per-table-disadvantages "File-Per-Table Tablespace Disadvantages")
...
Disabling [`innodb_file_per_table`](innodb-parameters.html#sysvar_innodb_file_per_table)
causes `InnoDB` to create tables in the system
tablespace.

An [`innodb_file_per_table`](innodb-parameters.html#sysvar_innodb_file_per_table)
setting can be specified in an option file or configured at
runtime using a
[`SET
GLOBAL`](set-variable.html "13.7.4.1 SET Syntax for Variable Assignment") statement. Changing the setting at runtime
requires privileges sufficient to set global system variables.
See [Section 5.1.8.1, “System Variable Privileges”](system-variable-privileges.html "5.1.8.1 System Variable Privileges").
...
Using [`SET
GLOBAL`](set-variable.html "13.7.4.1 SET Syntax for Variable Assignment") at runtime:
...
TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") operations from implicitly moving a table that
resides in the system tablespace to a file-per-table
tablespace. A table-copying [`ALTER
TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") operation recreates the table using the
current [`innodb_file_per_table`](innodb-parameters.html#sysvar_innodb_file_per_table)
setting. This behavior does not apply when adding or dropping
secondary indexes, nor does it apply to
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") operations that use
the `INPLACE` algorithm, or to tables added
to the system tablespace using
[`CREATE TABLE ...
TABLESPACE`](create-table.html "13.1.18 CREATE TABLE Statement") or
[`ALTER TABLE ...
TABLESPACE`](alter-table.html "13.1.8 ALTER TABLE Statement") syntax.
...
[`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement to
...
* A table-copying [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement")
...
* [`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") performance is
...
* You can import a table that resides in a file-per-table
...
* Tables created in file-per-table tablespaces use the
  ...
  [Section 14.10, “InnoDB File-Format Management”](innodb-file-format.html "14.10 InnoDB File-Format Management"). The Barracuda file
...
* You can backup or restore tables created in file-per-table
  tablespaces quickly using MySQL Enterprise Backup, without interrupting the
  ...
  [Making a Partial Backup](/doc/mysql-enterprise-backup/4.1/en/partial.html) for details.
...
* Common Linux file systems do not permit concurrent writes to
  ...
  [`innodb_flush_method`](innodb-parameters.html#sysvar_innodb_flush_method) is set
...
* [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") must keep an open file handle for
...
* There is potential for more fragmentation, which can impede
  [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") and table scan
