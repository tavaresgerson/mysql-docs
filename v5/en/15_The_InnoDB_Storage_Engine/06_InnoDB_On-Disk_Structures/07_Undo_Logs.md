...
to a [clustered index](glossary.html#glos_clustered_index "clustered index")
...
[undo log segments](glossary.html#glos_undo_log_segment "undo log segment"),
...
in [undo tablespaces](glossary.html#glos_undo_tablespace "undo tablespace"),
and in the [temporary
tablespace](glossary.html#glos_temporary_tablespace "temporary tablespace").
...
`InnoDB` supports a maximum of 128 rollback
...
[`innodb_rollback_segments`](innodb-parameters.html#sysvar_innodb_rollback_segments) variable
...
<table summary="Number of undo slots in a rollback segment for each InnoDB page size"><col style="width: 50%"/><col style="width: 50%"/><thead><tr>
...
<td><code>32768 (32KB)</code></td>
<td><code>2048</code></td>
</tr><tr>
<td><code>65536 (64KB)</code></td>
...
1. [`INSERT`](insert.html "13.2.5 INSERT Statement") operations on
   user-defined tables

2. [`UPDATE`](update.html "13.2.11 UPDATE Statement") and
   [`DELETE`](delete.html "13.2.2 DELETE Statement") operations on
   user-defined tables

3. [`INSERT`](insert.html "13.2.5 INSERT Statement") operations on
   user-defined temporary tables

4. [`UPDATE`](update.html "13.2.11 UPDATE Statement") and
   [`DELETE`](delete.html "13.2.2 DELETE Statement") operations on
   user-defined temporary tables

Undo logs are assigned as needed. For example, a transaction that
performs [`INSERT`](insert.html "13.2.5 INSERT Statement"),
[`UPDATE`](update.html "13.2.11 UPDATE Statement"), and
[`DELETE`](delete.html "13.2.2 DELETE Statement") operations on regular and
temporary tables requires a full assignment of four undo logs. A
transaction that performs only
[`INSERT`](insert.html "13.2.5 INSERT Statement") operations on regular tables
...
a transaction for an [`INSERT`](insert.html "13.2.5 INSERT Statement")
...
* If each transaction performs either an
  ...
  **or** an
  ...
  [`DELETE`](delete.html "13.2.2 DELETE Statement") operation, the number of
...
* If each transaction performs an
  [`INSERT`](insert.html "13.2.5 INSERT Statement") operation on a temporary
...
* If each transaction performs an
  ...
  **and** an
  [`UPDATE`](update.html "13.2.11 UPDATE Statement") or
  [`DELETE`](delete.html "13.2.2 DELETE Statement") operation on a temporary
  table, the number of concurrent read-write transactions that
  `InnoDB` is capable of supporting is:
