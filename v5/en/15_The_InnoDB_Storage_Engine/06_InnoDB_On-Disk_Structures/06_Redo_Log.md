...
by an ever-increasing [LSN](glossary.html#glos_lsn "LSN") value.
...
`InnoDB` [redo
log](glossary.html#glos_redo_log "redo log") files, perform the following steps:

1. Stop the MySQL server and make sure that it shuts down
   without errors.

2. Edit `my.cnf` to change the log file
