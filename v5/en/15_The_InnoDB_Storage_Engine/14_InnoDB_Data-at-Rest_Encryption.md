...
* [Enabling File-Per-Table Tablespace Encryption](innodb-data-encryption.html#innodb-data-encryption-enabling-disabling "Enabling File-Per-Table Tablespace Encryption")
...
* [Identifying Encrypted Tablespaces](innodb-data-encryption.html#innodb-data-encryption-identifying "Identifying Encrypted Tablespaces")
...
or authenticated user wants to access encrypted data,
...
The data-at-rest encryption feature relies on a keyring plugin for
...
feature, see [Section A.17, “MySQL 5.7 FAQ: InnoDB Data-at-Rest Encryption”](faqs-tablespace-encryption.html "A.17 MySQL 5.7 FAQ: InnoDB Data-at-Rest Encryption").

### Encryption Prerequisites

* A keyring plugin must be installed and configured. Keyring
  plugin installation is performed at startup using the
  [`early-plugin-load`](server-options.html#option_mysqld_early-plugin-load) option. Early
  loading ensures that the plugin is available prior to
  initialization of the `InnoDB` storage
  engine. For keyring plugin installation and configuration
  instructions, see [Section 6.4.4, “The MySQL Keyring”](keyring.html "6.4.4 The MySQL Keyring").
  ...
  the keyring plugin that was loaded when creating the
  encrypted tablespace must continue to be loaded at startup
  using the [`early-plugin-load`](server-options.html#option_mysqld_early-plugin-load)
  option. Failing to do so results in errors when starting the
  server and during `InnoDB` recovery.

  To verify that a keyring plugin is active, use the
  [`SHOW PLUGINS`](show-plugins.html "13.7.5.25 SHOW PLUGINS Statement") statement or query
  the Information Schema [`PLUGINS`](information-schema-plugins-table.html "24.3.17 The INFORMATION_SCHEMA PLUGINS Table")
  table. For example:
...
To enable encryption for a new file-per-table tablespace, specify
the `ENCRYPTION` option in a
[`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement. The
following example assumes that
[`innodb_file_per_table`](innodb-parameters.html#sysvar_innodb_file_per_table) is enabled.
...
specify the `ENCRYPTION` option in an
...
[`ALTER INSTANCE
ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") supports concurrent DML.
However, it cannot be run concurrently with tablespace encryption
operations, and locks are taken to prevent conflicts that could
arise from concurrent execution. If an
[`ALTER INSTANCE
ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") operation is running, it must
...
* The [`ALTER
  INSTANCE ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") statement is only
  supported in replication environments where the source and
  replicas run a version of MySQL that supports at-rest data
  encryption.

* Successful
  [`ALTER INSTANCE
  ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") statements are written to
  the binary log for replication on replicas.

* If an [`ALTER
  INSTANCE ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") statement fails,
  it is not logged to the binary log and is not replicated on
  replicas.

* Replication of an
  [`ALTER INSTANCE
  ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") operation fails if the
  keyring plugin is installed on the source but not on the
  replica.
...
* If the `keyring_file` or
  ...
  a keyring data file, the replicated
  [`ALTER INSTANCE
  ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") statement creates the
  keyring data file on the replica, assuming the keyring file
  data is not cached in memory.
  [`ALTER INSTANCE
  ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") uses keyring file data that
  is cached in memory, if available.
...
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") statement, it is
recorded in the `CREATE_OPTIONS` column of the
Information Schema [`TABLES`](information-schema-tables-table.html "24.3.25 The INFORMATION_SCHEMA TABLES Table") table. This
...
* Plan appropriately when altering an existing tablespace with
  the `ENCRYPTION` option. The table is rebuilt
  using the `COPY` algorithm. The
  `INPLACE` algorithm is not supported.
...
* Master key rotation re-encrypts tablespaces keys but does not
  ...
  you must disable and re-enable encryption, which is an
  `ALGORITHM=COPY` operation that rebuilds the
  table.

* If a table is created with both the
  [`COMPRESSION`](create-table.html "13.1.18 CREATE TABLE Statement")
  and
  [`ENCRYPTION`](create-table.html "13.1.18 CREATE TABLE Statement")
...
* If a keyring data file (the file named by
  ...
  [`ALTER INSTANCE
  ROTATE INNODB MASTER KEY`](alter-instance.html "13.1.4 ALTER INSTANCE Statement") creates a master encryption
  key.

* Uninstalling the `keyring_file` or
...
* Advanced Encryption Standard (AES) is the only supported
  encryption algorithm. `InnoDB` data-at-rest
...
* Altering the `ENCRYPTION` attribute of a
  table is performed using the `COPY`
  algorithm. The `INPLACE` algorithm is not
  supported.

* Encryption is only supported for
  [file-per-table](glossary.html#glos_file_per_table "file-per-table")
  tablespaces. Encryption is not supported for other tablespace
  types including
  [general
  tablespaces](glossary.html#glos_general_tablespace "general tablespace") and the
  [system
...
* Encryption only applies to data in the tablespace. Data is not
  encrypted in the redo log, undo log, or binary log.

* It is not permitted to change the storage engine of a table
  that resides in, or previously resided in, an encrypted
  tablespace.

* Encryption is not supported for the `InnoDB`
