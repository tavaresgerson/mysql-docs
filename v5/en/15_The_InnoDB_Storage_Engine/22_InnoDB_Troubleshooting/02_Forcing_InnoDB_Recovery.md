...
[`SELECT ... INTO
OUTFILE`](select-into.html "13.2.9.1 SELECT ... INTO Statement"). Usually, most of the data obtained in this way
...
Only set [`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery)
...
[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) setting
...
[`innodb_force_recovery=1`](innodb-parameters.html#sysvar_innodb_force_recovery) and
only increase the value incrementally, as necessary.

[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) is 0 by
default (normal startup without forced recovery). The permissible
nonzero values for
[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) are 1 to 6.
...
[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) value of 3
...
more corruption into [B-trees](glossary.html#glos_b_tree "B-tree")
...
[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) is greater
than 0. An [`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery)
...
* `1`
  ...
  [page](glossary.html#glos_page "page"). Tries to make
  `SELECT * FROM
  tbl_name` jump over
...
* `2`
  ...
  Prevents the [master
  thread](glossary.html#glos_master_thread "master thread") and any [purge
  threads](glossary.html#glos_purge_thread "purge thread") from running. If an unexpected exit would occur
  during the [purge](glossary.html#glos_purge "purge") operation,
...
* `3`
  ...
  [rollbacks](glossary.html#glos_rollback "rollback") after
...
* `4`
  ...
  Prevents [insert
  buffer](glossary.html#glos_insert_buffer "insert buffer") merge operations. If they would cause a crash,
  does not do them. Does not calculate table
  [statistics](glossary.html#glos_statistics "statistics"). This value
...
* `5`
  ...
  logs](glossary.html#glos_undo_log "undo log") when starting the database:
...
* `6`
  ...
  Does not do the [redo log](glossary.html#glos_redo_log "redo log")
...
You can [`SELECT`](select.html "13.2.9 SELECT Statement") from tables to dump
them. With an
[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) value of 3
or less you can `DROP` or
`CREATE` tables. [`DROP
TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") is also supported with an
[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) value
greater than 3, up to MySQL 5.7.17. As of MySQL 5.7.18,
[`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") is not permitted with an
[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) value
greater than 4.
...
TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement"), you can kill the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server")
process and set
[`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery) to
...
If a high [`innodb_force_recovery`](innodb-parameters.html#sysvar_innodb_force_recovery)
