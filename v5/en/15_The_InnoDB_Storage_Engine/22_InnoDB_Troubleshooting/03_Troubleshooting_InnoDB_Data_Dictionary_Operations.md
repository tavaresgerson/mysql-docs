...
[data dictionary](glossary.html#glos_data_dictionary "data dictionary"). If
...
[Section 14.22.2, “Forcing InnoDB Recovery”](forcing-innodb-recovery.html "14.22.2 Forcing InnoDB Recovery") for information about
manual recovery.

#### CREATE TABLE Failure Due to Orphan Table

A symptom of an out-of-sync data dictionary is that a
[`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement fails. If
...
[`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") successfully, the
problem may be due to name completion in the
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client. To work around this problem,
start the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client with the
[`--skip-auto-rehash`](mysql-command-options.html#option_mysql_auto-rehash)
option and try [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") again.
(With name completion on, [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") tries to
...
TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") statement to remove data about the missing table
...
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") operation
...
[general
tablespace](glossary.html#glos_general_tablespace "general tablespace") prevents you from dropping the general
...
[`INNODB_SYS_TABLES`](information-schema-innodb-sys-tables-table.html "24.4.23 The INFORMATION_SCHEMA INNODB_SYS_TABLES Table") table. Look for
...
1. In the database directory, rename the
...
2. Drop the orphan intermediate table by issuing a
   [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") statement,
...
If MySQL exits in the middle of an in-place
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") operation on a
...
1. In a separate instance of the same MySQL version, create a
   non-partitioned table with the same schema name and
   columns as the partitioned table.

2. Copy the `.frm` file of the
   non-partitioned table to the database directory with the
   orphan intermediate tables.

3. Make a copy of the `.frm` file for each
   table, and rename the `.frm` files to
   match names of the orphan intermediate tables (as
   described above).

4. Perform a [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement")
...
[`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") statement, prefixing
...
1. Create a matching `.frm` file in some
   other database directory and copy it to the database
   directory where the orphan table is located.

2. Issue [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") for the
...
[Section 14.6.1.4, “Moving or Copying InnoDB Tables”](innodb-migration.html "14.6.1.4 Moving or Copying InnoDB Tables") for information about
creating clean backups.

Table import limitations outlined in
[Section 14.6.1.3, “Importing InnoDB Tables”](innodb-table-import.html "14.6.1.3 Importing InnoDB Tables") are applicable to this
procedure.
