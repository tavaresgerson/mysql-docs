...
* [Shared and Exclusive Locks](innodb-locking.html#innodb-shared-exclusive-locks "Shared and Exclusive Locks")
* [Intention Locks](innodb-locking.html#innodb-intention-locks "Intention Locks")
* [Record Locks](innodb-locking.html#innodb-record-locks "Record Locks")
* [Gap Locks](innodb-locking.html#innodb-gap-locks "Gap Locks")
* [Next-Key Locks](innodb-locking.html#innodb-next-key-locks "Next-Key Locks")
* [Insert Intention Locks](innodb-locking.html#innodb-insert-intention-locks "Insert Intention Locks")
* [AUTO-INC Locks](innodb-locking.html#innodb-auto-inc-locks "AUTO-INC Locks")
* [Predicate Locks for Spatial Indexes](innodb-locking.html#innodb-predicate-locks "Predicate Locks for Spatial Indexes")
...
[shared (`S`)
locks](glossary.html#glos_shared_lock "shared lock") and [exclusive
(`X`) locks](glossary.html#glos_exclusive_lock "exclusive lock").

* A [shared
  (`S`) lock](glossary.html#glos_shared_lock "shared lock") permits the transaction
  that holds the lock to read a row.

* An [exclusive
  (`X`) lock](glossary.html#glos_exclusive_lock "exclusive lock") permits the transaction
...
[`LOCK TABLES ...
WRITE`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") takes an exclusive lock (an `X`
lock) on the specified table. To make locking at multiple
granularity levels practical, `InnoDB` uses
[intention locks](glossary.html#glos_intention_lock "intention lock").
...
* An [intention
  shared lock](glossary.html#glos_intention_shared_lock "intention shared lock") (`IS`) indicates that a
  transaction intends to set a *shared*
  lock on individual rows in a table.

* An [intention
  exclusive lock](glossary.html#glos_intention_exclusive_lock "intention exclusive lock") (`IX`) indicates that
  a transaction intends to set an exclusive lock on individual
  rows in a table.

For example, [`SELECT ...
LOCK IN SHARE MODE`](select.html "13.2.9 SELECT Statement") sets an `IS` lock,
and [`SELECT ... FOR
UPDATE`](select.html "13.2.9 SELECT Statement") sets an `IX` lock.
...
<table summary='A matrix showing table-level lock type compatibility. Each cell in the matrix is marked as either "Compatible" or "Conflict".'><col style="width: 20%"/><col style="width: 20%"/><col style="width: 20%"/><col style="width: 20%"/><col style="width: 20%"/><thead><tr>
<th></th>
<th><code>X</code></th>
<th><code>IX</code></th>
<th><code>S</code></th>
...
[deadlock](glossary.html#glos_deadlock "deadlock"), an error occurs.

Intention locks do not block anything except full table requests
(for example, [`LOCK
TABLES ... WRITE`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements")). The main purpose of intention locks
...
following in [`SHOW
ENGINE INNODB STATUS`](show-engine.html "13.7.5.15 SHOW ENGINE Statement") and
[InnoDB monitor](innodb-standard-monitor.html "14.18.3 InnoDB Standard Monitor and Lock Monitor Output")
output:
...
[`READ COMMITTED`](innodb-transaction-isolation-levels.html#isolevel_read-committed) or enable the
...
[`READ COMMITTED`](innodb-transaction-isolation-levels.html#isolevel_read-committed) isolation
level or enabling
[`innodb_locks_unsafe_for_binlog`](innodb-parameters.html#sysvar_innodb_locks_unsafe_for_binlog).
...
[`REPEATABLE READ`](innodb-transaction-isolation-levels.html#isolevel_repeatable-read) transaction
isolation level. In this case, `InnoDB` uses
next-key locks for searches and index scans, which prevents
phantom rows (see [Section 14.7.4, “Phantom Rows”](innodb-next-key-locking.html "14.7.4 Phantom Rows")).
...
[`INSERT`](insert.html "13.2.5 INSERT Statement") operations prior to row
...
the following in
[`SHOW ENGINE INNODB
STATUS`](show-engine.html "13.7.5.15 SHOW ENGINE Statement") and
[InnoDB monitor](innodb-standard-monitor.html "14.18.3 InnoDB Standard Monitor and Lock Monitor Output")
output:
...
[Section 11.4.8, “Optimizing Spatial Analysis”](optimizing-spatial-analysis.html "11.4.8 Optimizing Spatial Analysis")).

To handle locking for operations involving
`SPATIAL` indexes, next-key locking does not
work well to support [`REPEATABLE
READ`](innodb-transaction-isolation-levels.html#isolevel_repeatable-read) or
[`SERIALIZABLE`](innodb-transaction-isolation-levels.html#isolevel_serializable) transaction
