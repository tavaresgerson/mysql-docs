...
deadlocks in [Section 14.7.5.2, “Deadlock Detection”](innodb-deadlock-detection.html "14.7.5.2 Deadlock Detection"). It
explains how to organize database operations to minimize
deadlocks and the subsequent error handling required in
applications.

[Deadlocks](glossary.html#glos_deadlock "deadlock") are a classic
...
* At any time, issue
  [`SHOW ENGINE
  INNODB STATUS`](show-engine.html "13.7.5.15 SHOW ENGINE Statement") to determine the cause of the most
...
* If frequent deadlock warnings cause concern, collect more
  ...
  [error log](glossary.html#glos_error_log "error log"). Disable
...
* Commit transactions immediately after making a set of
  ...
  [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") session open for a long time with
  an uncommitted transaction.

* If you use [locking
  reads](glossary.html#glos_locking_read "locking read") ([`SELECT
  ... FOR UPDATE`](select.html "13.2.9 SELECT Statement") or
  [`SELECT ... LOCK IN
  SHARE MODE`](select.html "13.2.9 SELECT Statement")), try using a lower isolation level
  such as [`READ COMMITTED`](innodb-transaction-isolation-levels.html#isolevel_read-committed).
...
* Add well-chosen indexes to your tables so that your queries
  ...
  [`EXPLAIN
  SELECT`](explain.html "13.8.2 EXPLAIN Statement") to determine which indexes the MySQL server
  regards as the most appropriate for your queries.

* Use less locking. If you can afford to permit a
  [`SELECT`](select.html "13.2.9 SELECT Statement") to return data from an
  old snapshot, do not add a `FOR UPDATE` or
  `LOCK IN SHARE MODE` clause to it. Using
  the [`READ COMMITTED`](innodb-transaction-isolation-levels.html#isolevel_read-committed)
...
* If nothing else helps, serialize your transactions with
  ...
  [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") with
