...
[14.7.5.1 An InnoDB Deadlock Example](innodb-deadlock-example.html)

[14.7.5.2 Deadlock Detection](innodb-deadlock-detection.html)

[14.7.5.3 How to Minimize and Handle Deadlocks](innodb-deadlocks-handling.html)
...
[`SELECT ... FOR
UPDATE`](select.html "13.2.9 SELECT Statement")), but in the opposite order. A deadlock can also
...
[Section 14.7.5.1, “An InnoDB Deadlock Example”](innodb-deadlock-example.html "14.7.5.1 An InnoDB Deadlock Example").

To reduce the possibility of deadlocks, use transactions rather
than [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") statements; keep
...
[`SELECT ... FOR
UPDATE`](select.html "13.2.9 SELECT Statement")) in each transaction; create indexes on the
columns used in [`SELECT ...
FOR UPDATE`](select.html "13.2.9 SELECT Statement") and
[`UPDATE ... WHERE`](update.html "13.2.11 UPDATE Statement")
...
[`innodb_deadlock_detect`](innodb-parameters.html#sysvar_innodb_deadlock_detect) variable,
...
[`SHOW ENGINE INNODB
STATUS`](show-engine.html "13.7.5.15 SHOW ENGINE Statement"). If frequent deadlocks highlight a problem with
transaction structure or application error handling, enable
[`innodb_print_all_deadlocks`](innodb-parameters.html#sysvar_innodb_print_all_deadlocks) to
print information about all deadlocks to the
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") error log. For more information about
