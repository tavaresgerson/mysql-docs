...
[`UPDATE`](update.html "13.2.11 UPDATE Statement"), or a
[`DELETE`](delete.html "13.2.2 DELETE Statement") generally set record locks
...
[Section 14.7.1, “InnoDB Locking”](innodb-locking.html "14.7.1 InnoDB Locking"). The transaction isolation level
...
* [`SELECT ...
  FROM`](select.html "13.2.9 SELECT Statement") is a consistent read, reading a snapshot of the
...
* For [`SELECT ... FOR
  UPDATE`](select.html "13.2.9 SELECT Statement") or
  [`SELECT ... LOCK IN
  SHARE MODE`](select.html "13.2.9 SELECT Statement"), locks are acquired for scanned rows, and
  ...
  [`UNION`](union.html "13.2.9.3 UNION Clause"), scanned (and locked)
...
* [`SELECT ... LOCK IN
  SHARE MODE`](select.html "13.2.9 SELECT Statement") sets shared next-key locks on all index
...
* [`SELECT ... FOR
  UPDATE`](select.html "13.2.9 SELECT Statement") sets an exclusive next-key lock on every
  ...
  [`SELECT ... FOR
  UPDATE`](select.html "13.2.9 SELECT Statement") blocks other sessions from doing
  [`SELECT ... LOCK IN
  SHARE MODE`](select.html "13.2.9 SELECT Statement") or from reading in certain transaction
  isolation levels. Consistent reads ignore any locks set on the
  records that exist in the read view.

* [`UPDATE ... WHERE
  ...`](update.html "13.2.11 UPDATE Statement") sets an exclusive next-key lock on every record
...
* When [`UPDATE`](update.html "13.2.11 UPDATE Statement") modifies a
  clustered index record, implicit locks are taken on affected
  secondary index records. The
  [`UPDATE`](update.html "13.2.11 UPDATE Statement") operation also takes
...
* [`DELETE FROM ... WHERE
  ...`](delete.html "13.2.2 DELETE Statement") sets an exclusive next-key lock on every record
  the search encounters. However, only an index record lock is
  required for statements that lock rows using a unique index to
  search for a unique row.

* [`INSERT`](insert.html "13.2.5 INSERT Statement") sets an exclusive lock
...
* [`INSERT
  ... ON DUPLICATE KEY UPDATE`](insert-on-duplicate.html "13.2.5.2 INSERT ... ON DUPLICATE KEY UPDATE Statement") differs from a simple
  [`INSERT`](insert.html "13.2.5 INSERT Statement") in that an exclusive
...
* [`REPLACE`](replace.html "13.2.8 REPLACE Statement") is done like an
  [`INSERT`](insert.html "13.2.5 INSERT Statement") if there is no collision
...
* `INSERT INTO T SELECT ... FROM S WHERE ...`
  ...
  COMMITTED`](innodb-transaction-isolation-levels.html#isolevel_read-committed), or
  ...
  [`CREATE TABLE ...
  SELECT ...`](create-table.html "13.1.18 CREATE TABLE Statement") performs the
  [`SELECT`](select.html "13.2.9 SELECT Statement") with shared next-key
...
* [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") sets table locks,
...
* [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") acquires two locks
  ...
  [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") completes even if
...
* All `InnoDB` locks held by a transaction are
  ...
  [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") on
  `InnoDB` tables in
...
* You cannot lock additional tables in the middle of a
  transaction because [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements")
  performs an implicit [`COMMIT`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements") and
  [`UNLOCK
