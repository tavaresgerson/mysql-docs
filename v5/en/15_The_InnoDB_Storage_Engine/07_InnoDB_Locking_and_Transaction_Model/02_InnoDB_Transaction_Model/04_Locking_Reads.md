...
* [`SELECT ... LOCK IN
  SHARE MODE`](select.html "13.2.9 SELECT Statement")
...
* [`SELECT ... FOR
  UPDATE`](select.html "13.2.9 SELECT Statement")
  ...
  doing `SELECT ... LOCK IN SHARE MODE`, or
...
[`START
TRANSACTION`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements") or by setting
...
[`SELECT`](select.html "13.2.9 SELECT Statement") using `LOCK IN
SHARE MODE`:
...
A [`SELECT ... FOR
UPDATE`](select.html "13.2.9 SELECT Statement") reads the latest available data, setting
exclusive locks on each row it reads. Thus, it sets the same
locks a searched SQL [`UPDATE`](update.html "13.2.11 UPDATE Statement")
would set on the rows.

The preceding description is merely an example of how
[`SELECT ... FOR
UPDATE`](select.html "13.2.9 SELECT Statement") works. In MySQL, the specific task of
...
The [`SELECT`](select.html "13.2.9 SELECT Statement") statement merely
