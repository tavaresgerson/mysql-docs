...
[`SELECT`](select.html "13.2.9 SELECT Statement") sees the latest version of
...
[`SELECT`](select.html "13.2.9 SELECT Statement") statements within a
...
[`UPDATE`](update.html "13.2.11 UPDATE Statement") statement issued from
...
and then doing another [`SELECT`](select.html "13.2.9 SELECT Statement") or
[`START TRANSACTION WITH
CONSISTENT SNAPSHOT`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements").
...
* Consistent read does not work over [`DROP
  TABLE`](drop-table.html "13.1.29 DROP TABLE Statement"), because MySQL cannot use a table that has
...
* Consistent read does not work over
  ...
  [`ER_TABLE_DEF_CHANGED`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_table_def_changed),
  “Table definition has changed, please retry
  transaction”.

The type of read varies for selects in clauses like
[`INSERT INTO ...
SELECT`](insert.html "13.2.5 INSERT Statement"), [`UPDATE
... (SELECT)`](update.html "13.2.11 UPDATE Statement"), and
[`CREATE TABLE ...
SELECT`](create-table.html "13.1.18 CREATE TABLE Statement") that do not specify `FOR
UPDATE` or `LOCK IN SHARE MODE`:

* By default, `InnoDB` uses stronger locks in
  those statements and the
  [`SELECT`](select.html "13.2.9 SELECT Statement") part acts like
...
* To perform a nonlocking read in such cases, enable the
  ...
  [`READ COMMITTED`](innodb-transaction-isolation-levels.html#isolevel_read-committed), or
  [`REPEATABLE READ`](innodb-transaction-isolation-levels.html#isolevel_repeatable-read) to avoid
