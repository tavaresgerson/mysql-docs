...
candidates for [eviction](glossary.html#glos_eviction "eviction").
...
* When `InnoDB` reads a page into the buffer
  ...
  [read-ahead](glossary.html#glos_read_ahead "read-ahead") operation
...
scan, performed for a [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") operation or a
...
[Section 14.8.3.3, “Making the Buffer Pool Scan Resistant”](innodb-performance-midpoint_insertion.html "14.8.3.3 Making the Buffer Pool Scan Resistant"), and
...
see [Monitoring the Buffer Pool Using the InnoDB Standard Monitor](innodb-buffer-pool.html#innodb-buffer-pool-monitoring "Monitoring the Buffer Pool Using the InnoDB Standard Monitor").
...
* On 64-bit systems with sufficient memory, you can split the
  ...
  see [Section 14.8.3.2, “Configuring Multiple Buffer Pool Instances”](innodb-multiple-buffer-pools.html "14.8.3.2 Configuring Multiple Buffer Pool Instances").
...
* You can control how and when to perform read-ahead requests to
  ...
  anticipation that the pages are needed soon. For details, see
  [Section 14.8.3.4, “Configuring InnoDB Buffer Pool Prefetching (Read-Ahead)”](innodb-performance-read_ahead.html "14.8.3.4 Configuring InnoDB Buffer Pool Prefetching (Read-Ahead)").
...
accessed using
[`SHOW
ENGINE INNODB STATUS`](innodb-standard-monitor.html "14.18.3 InnoDB Standard Monitor and Lock Monitor Output"), provides metrics regarding
...
<table summary="InnoDB buffer pool metrics reported by the InnoDB Standard Monitor."><col style="width: 35%"/><col style="width: 65%"/><thead><tr>
<th>Name</th>
<th>Description</th>
</tr></thead><tbody><tr>
...
<td>The total memory allocated for the <code>InnoDB</code> data
...
* `not (young-making rate)` is the average hit
  ...
  [`innodb_old_blocks_time`](innodb-parameters.html#sysvar_innodb_old_blocks_time) not
...
variables](server-status-variables.html "5.1.9 Server Status Variables") and the
[`INNODB_BUFFER_POOL_STATS`](information-schema-innodb-buffer-pool-stats-table.html "24.4.4 The INFORMATION_SCHEMA INNODB_BUFFER_POOL_STATS Table") table
provide many of the same buffer pool metrics found in
`InnoDB` Standard Monitor output. For more
information, see
[Example 14.10, “Querying the INNODB\_BUFFER\_POOL\_STATS Table”](innodb-information-schema-buffer-pool-tables.html#innodb-information-schema-buffer-pool-stats-example "Example 14.10 Querying the INNODB_BUFFER_POOL_STATS Table").