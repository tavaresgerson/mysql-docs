...
MySQL client programs prompt for a password when invoked with a [`--password`](connection-options.html#option_general_password) or `-p` option that has no following password value:
