...
<table summary="Names for the plugins and library file used for socket peer-credential password authentication."><thead><tr> <th>Plugin or File</th> <th>Plugin or File Name</th> </tr></thead><tbody><tr> <td>Server-side plugin</td> <td><code>auth_socket</code></td> </tr><tr> <td>Client-side plugin</td> <td>None, see discussion</td> </tr><tr> <td>Library file</td> <td><code class="filename">auth_socket.so</code></td> </tr></tbody></table>

The following sections provide installation and usage information specific to socket pluggable authentication:

* [Installing Socket Pluggable Authentication](socket-pluggable-authentication.html#socket-pluggable-authentication-installation "Installing Socket Pluggable Authentication")
* [Uninstalling Socket Pluggable Authentication](socket-pluggable-authentication.html#socket-pluggable-authentication-uninstallation "Uninstalling Socket Pluggable Authentication")
* [Using Socket Pluggable Authentication](socket-pluggable-authentication.html#socket-pluggable-authentication-usage "Using Socket Pluggable Authentication")
...
This section describes how to install the socket authentication plugin. For general information about installing plugins, see [Section 5.5.1, “Installing and Uninstalling Plugins”](plugin-loading.html "5.5.1 Installing and Uninstalling Plugins").
...
To associate MySQL accounts with the socket plugin, see [Using Socket Pluggable Authentication](socket-pluggable-authentication.html#socket-pluggable-authentication-usage "Using Socket Pluggable Authentication").
...
* Name both users at account-creation time, one following [`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement"), and the other in the authentication string:

  ```sql
  CREATE USER 'valerie'@'localhost' IDENTIFIED WITH auth_socket AS 'stephanie';
  ```

* If you have already used [`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement") to create the account for a single user, use [`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement") to add the second user:
