...
Because `SHOW` is familiar and widely used, the `SHOW` statements remain as an alternative. In fact, along with the implementation of `INFORMATION_SCHEMA`, there are enhancements to `SHOW` as described in Section 24.8, “Extensions to SHOW Statements”.
...
* information about `INFORMATION_SCHEMA` tables specific to the `InnoDB` storage engine: Section 24.4, “INFORMATION\_SCHEMA InnoDB Tables”

* information about `INFORMATION_SCHEMA` tables specific to the thread pool plugin: Section 24.5, “INFORMATION\_SCHEMA Thread Pool Tables”

* information about `INFORMATION_SCHEMA` tables specific to the `CONNECTION_CONTROL` plugin: Section 24.6, “INFORMATION\_SCHEMA Connection Control Tables”

* Answers to questions that are often asked concerning the `INFORMATION_SCHEMA` database: Section A.7, “MySQL 5.7 FAQ: INFORMATION\_SCHEMA”
