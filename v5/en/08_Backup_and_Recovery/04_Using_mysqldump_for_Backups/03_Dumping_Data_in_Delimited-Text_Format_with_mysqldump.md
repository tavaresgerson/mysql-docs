...
If you invoke [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") with the
...
`t1.txt`. The `.sql` file
contains a [`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement
...
[`SELECT ... INTO
OUTFILE`](select-into.html "13.2.9.1 SELECT ... INTO Statement") to write the files, so you must have the
...
dumped tables to [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program"), which writes them
to `.sql` files. These files therefore are
owned by the user who executes [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program").
...
[**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") in the local directory (on the
client host).

For [**mysqldump --tab**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program"), the server by default
...
[**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") supports these options:
...
[**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") to quote column values within
...
data, you must specify the same format when you reload data
