...
The last section,
[Section 7.5.1, “Point-in-Time Recovery Using Binary Log”](point-in-time-recovery-binlog.html "7.5.1 Point-in-Time Recovery Using Binary Log"), explains the
...
As an example, suppose that around 13:00:00 on May 27, 2020, an
SQL statement was executed that deleted a table. You can perform
a point-in-time recovery to restore the server up to its state
right before the table deletion. These are some sample steps to
achieve that:

1. Restore the last full backup created before the
   point-in-time of interest (call it
   `tp`, which is
   13:00:00 on May 27, 2020 in our example). When finished,
   ...
   example, if you are using [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files")
   ...
   [Point-in-Time Recovery](/doc/mysql-enterprise-backup/4.1/en/advanced.point.html).
...
2. Find the precise binary log event position corresponding to
   ...
   time using the [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") utility. Use
   ...
   From the output of [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files"), the
   `` DROP TABLE `pets`.`cats` `` statement can be
   found in the segment of the binary log between the line
   `# at 1868` and `# at
   1985`, which means the statement takes place
   *after* the log position 1868, and the
   log is at position 1985 after the `DROP
   TABLE` statement.
...
3. Apply the events in binary log file to the server, starting
   ...
   1006) and ending at the position you have found in step 2
   that is *before* your point-in-time of
   interest (which is 1868):
   ...
   output of [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") includes
...
4. Beyond the point-in-time recovery that has been finished, if
   ...
   *after* your point-in-time of interest,
   use [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") again to apply all the
   events after `tp` to
   the server. We noted in step 2 that after the statement we
   wanted to skip, the log is at position 1985; we can use it
