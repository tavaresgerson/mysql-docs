...
[**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") on `MyISAM` tables
(extensions `.MYI` and
`.MYD`).
...
[**perror**](perror.html "4.8.2 perror — Display MySQL Error Message Information") *`nnn`*, where
*`nnn`* is the error number. The
following example shows how to use [**perror**](perror.html "4.8.2 perror — Display MySQL Error Message Information") to
...
[`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to increase the
`MAX_ROWS` and
...
[**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") can usually detect and fix most
problems that occur.
...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") runs as (and to you, because you need
...
those described in [Section 7.6.2, “How to Check MyISAM Tables for Errors”](myisam-check.html "7.6.2 How to Check MyISAM Tables for Errors")), or you want
to use the extended features that [**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility")
provides.

The [**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") options used for table
maintenance with are described in [Section 4.6.3, “myisamchk — MyISAM Table-Maintenance Utility”](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility").
[**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") also has variables that you can set
...
must first stop the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") server. Note that
when you do [**mysqladmin shutdown**](mysqladmin.html "4.5.2 mysqladmin — A MySQL Server Administration Program") on a remote
server, the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") server is still available
for a while after [**mysqladmin**](mysqladmin.html "4.5.2 mysqladmin — A MySQL Server Administration Program") returns, until
all statement-processing has stopped and all index changes have
been flushed to disk.

**Stage 1: Checking your tables**

Run [**myisamchk \*.MYI**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") or [**myisamchk -e
\*.MYI**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") if you have more time. Use the
`-s` (silent) option to suppress unnecessary
information.

If the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") server is stopped, you should
use the [`--update-state`](myisamchk-check-options.html#option_myisamchk_update-state) option
to tell [**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") to mark the table as
“checked.”

You have to repair only those tables for which
[**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") announces an error. For such
tables, proceed to Stage 2.

If you get unexpected errors when checking (such as `out
of memory` errors), or if [**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility")
crashes, go to Stage 3.

**Stage 2: Easy safe repair**

First, try [**myisamchk -r -q
*`tbl_name`***](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") (`-r
...
2. Use [**myisamchk -r
   *`tbl_name`***](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility")
...
3. If the preceding step fails, use [**myisamchk
   --safe-recover
   *`tbl_name`***](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility"). Safe recovery
...
[`sort_buffer_size`](server-system-variables.html#sysvar_sort_buffer_size) and
[`key_buffer_size`](server-system-variables.html#sysvar_key_buffer_size) variables
...
[**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") crashes, go to Stage 3.
...
Go back to Stage 2. [**myisamchk -r -q**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility") should
...
when you use [`REPAIR TABLE`](repair-table.html "13.7.2.5 REPAIR TABLE Statement"). See
[Section 13.7.2.5, “REPAIR TABLE Statement”](repair-table.html "13.7.2.5 REPAIR TABLE Statement").
...
1. Restore the description file from a backup and go back to
   Stage 3. You can also restore the index file and go back to
   Stage 2. In the latter case, you should start with
   [**myisamchk -r**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility").

2. If you do not have a backup but know exactly how the table
