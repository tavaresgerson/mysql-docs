...
structure ([`CREATE DATABASE`](create-database.html "13.1.11 CREATE DATABASE Statement"),
[`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statements) and
content ([`INSERT`](insert.html "13.2.5 INSERT Statement") statements or
...
* For restore:
  ...
  + [**ndb\_restore**](mysql-cluster-programs-ndb-restore.html "21.5.24 ndb_restore — Restore an NDB Cluster Backup") restores
...
* Logical backup tools include the [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program")
  program and the [`SELECT
  ... INTO OUTFILE`](select.html "13.2.9 SELECT Statement") statement. These work for any
  storage engine, even `MEMORY`.

* To restore logical backups, SQL-format dump files can be
  processed using the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client. To load
  delimited-text files, use the [`LOAD
  DATA`](load-data.html "13.2.6 LOAD DATA Statement") statement or the [**mysqlimport**](mysqlimport.html "4.5.5 mysqlimport — A Data Import Program")
  client.
...
* Clients can be affected adversely because the server is
  ...
  often taken from a replica server that can be taken offline
  without harming availability.
...
it is more likely for clients to be affected for online recovery
than for online backup because recovery requires stronger locking.
...
* [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") can connect to local or remote
  servers. For SQL output (`CREATE` and
  [`INSERT`](insert.html "13.2.5 INSERT Statement") statements), local or
  remote dumps can be done and generate output on the client.
  For delimited-text output (with the
  [`--tab`](mysqldump.html#option_mysqldump_tab) option), data files
  are created on the server host.

* [`SELECT ... INTO
  OUTFILE`](select-into.html "13.2.9.1 SELECT ... INTO Statement") can be initiated from a local or remote
...
[`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") tables, this is not a typical
issue. For programs to check [`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine")
