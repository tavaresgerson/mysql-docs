...
* Between source and replica servers. See
...
* By client programs that are based on the MySQL C API. See
  [Support for Encrypted Connections](/doc/c-api/5.7/en/c-api-encrypted-connections.html).

Instructions for creating any required certificate and key files
are available in [Section 6.3.3, “Creating SSL and RSA Certificates and Keys”](creating-ssl-rsa-files.html "6.3.3 Creating SSL and RSA Certificates and Keys").

#### Server-Side Startup Configuration for Encrypted Connections

On the server side, the [`--ssl`](server-options.html#option_mysqld_ssl)
...
key files, see [Section 6.3.3, “Creating SSL and RSA Certificates and Keys”](creating-ssl-rsa-files.html "6.3.3 Creating SSL and RSA Certificates and Keys"). MySQL
...
[`--ssl`](server-options.html#option_mysqld_ssl) (possibly along with
[`ssl_cipher`](server-system-variables.html#sysvar_ssl_cipher)) to configure
...
* [`tls_version`](server-system-variables.html#sysvar_tls_version): Which
  encryption protocols the server permits for encrypted
  connections; see
...
[Section 6.3.3.1, “Creating SSL and RSA Certificates and Keys using MySQL”](creating-ssl-rsa-files-using-mysql.html "6.3.3.1 Creating SSL and RSA Certificates and Keys using MySQL")). Such
self-signed certificates do not contain the server name as the
Common Name value.

Prior to MySQL 5.7.23, host name identity verification also
...
* [`--tls-version`](connection-options.html#option_general_tls-version): The permitted
...
clause, see [Section 13.7.1.2, “CREATE USER Statement”](create-user.html "13.7.1.2 CREATE USER Statement").
...
in [Section 6.3.3.2, “Creating SSL Certificates and Keys Using openssl”](creating-ssl-files-using-openssl.html "6.3.3.2 Creating SSL Certificates and Keys Using openssl"). If you
...
For the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client, an alternative is to
use the `STATUS` or `\s`
...
requirements, use the [`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement")
statement.