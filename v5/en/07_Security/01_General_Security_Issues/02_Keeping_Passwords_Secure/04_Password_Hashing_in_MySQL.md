...
and the `OLD_PASSWORD()` function. Also,
[`secure_auth`](server-system-variables.html#sysvar_secure_auth) cannot be
disabled, and [`old_passwords`](server-system-variables.html#sysvar_old_passwords)
cannot be set to 1.
...
* After the client connects, it can (if it has sufficient
  ...
  [`PASSWORD()`](encryption-functions.html#function_password) function to
  generate a password hash, or by using a password-generating
  statement ([`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement"),
...
[`PASSWORD()`](encryption-functions.html#function_password) function or uses a
...
result from the [`PASSWORD()`](encryption-functions.html#function_password)
...
* Different format of password values produced by the
  [`PASSWORD()`](encryption-functions.html#function_password) function
...
* In MySQL 4.1.1, the hashing method was modified to produce a
  ...
  + The `OLD_PASSWORD()` function was
    added, which returns hash values in the 16-byte format.

  + For compatibility purposes, the
    [`old_passwords`](server-system-variables.html#sysvar_old_passwords) system
    variable was added, to enable DBAs and applications
    control over the hashing method. The default
    [`old_passwords`](server-system-variables.html#sysvar_old_passwords) value of
    0 causes hashing to use the 4.1 method (41-byte hash
    values), but setting
    [`old_passwords=1`](server-system-variables.html#sysvar_old_passwords) causes
    hashing to use the pre-4.1 method. In this case,
    [`PASSWORD()`](encryption-functions.html#function_password) produces
    16-byte values and is equivalent to
    `OLD_PASSWORD()`

  To permit DBAs control over how clients are permitted to
  connect, the [`secure_auth`](server-system-variables.html#sysvar_secure_auth)
  ...
  Before MySQL 5.6.5,
  [`secure_auth`](server-system-variables.html#sysvar_secure_auth) is disabled by
  default. As of 5.6.5,
  [`secure_auth`](server-system-variables.html#sysvar_secure_auth) is enabled by
  ...
  In addition, the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client supports a
  [`--secure-auth`](mysql-command-options.html#option_mysql_secure-auth) option that is
  analogous to [`secure_auth`](server-system-variables.html#sysvar_secure_auth),
...
difficulties. For example, a 4.0 [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client
...
[`old_passwords`](server-system-variables.html#sysvar_old_passwords) system variable.
...
[`old_passwords`](server-system-variables.html#sysvar_old_passwords) must not be set
to 1.
...
* The `Password` column must be wide enough
  ...
  [`PASSWORD()`](encryption-functions.html#function_password) function or a
  ...
  [`mysqld_upgrade`](mysql-upgrade.html "4.4.7 mysql_upgrade — Check and Upgrade MySQL Tables") program to widen the
  `Password` column.

* If the `Password` column is wide, it can
  store either short or long password hashes. In this case,
  the [`PASSWORD()`](encryption-functions.html#function_password) function and
...
[`old_passwords`](server-system-variables.html#sysvar_old_passwords) disabled, the
...
* The client changes its own password. With
  [`old_passwords`](server-system-variables.html#sysvar_old_passwords) disabled,
...
without [`old_passwords`](server-system-variables.html#sysvar_old_passwords) set to 1.
By running the server with
[`old_passwords=1`](server-system-variables.html#sysvar_old_passwords),
...
The downside of [`old_passwords=1`](server-system-variables.html#sysvar_old_passwords)
...
[`old_passwords`](server-system-variables.html#sysvar_old_passwords) set to 0 while
...
[`old_passwords`](server-system-variables.html#sysvar_old_passwords) enabled or
disabled.
...
* For connected clients, password hash-generating operations
  involving the [`PASSWORD()`](encryption-functions.html#function_password)
...
* The value of [`old_passwords`](server-system-variables.html#sysvar_old_passwords)
...
upgraded to 4.1 or later but [`mysqld_upgrade`](mysql-upgrade.html "4.4.7 mysql_upgrade — Check and Upgrade MySQL Tables")
...
hashes because [`old_passwords=1`](server-system-variables.html#sysvar_old_passwords)
prevents generation of long hashes. Also, if you create an
account with a long hash before setting
[`old_passwords`](server-system-variables.html#sysvar_old_passwords) to 1, changing
the account's password while
[`old_passwords=1`](server-system-variables.html#sysvar_old_passwords) results in the
...
[`old_passwords=0`](server-system-variables.html#sysvar_old_passwords) and
[`secure_auth=1`](server-system-variables.html#sysvar_secure_auth).

**Scenario 3:** Long
`Password` column; server started with
[`old_passwords=0`](server-system-variables.html#sysvar_old_passwords):
...
PASSWORD`](set-password.html "13.7.1.7 SET PASSWORD Statement") as follows to change an account password:
...
In scenario 2, [`old_passwords=1`](server-system-variables.html#sysvar_old_passwords)
...
session value of [`old_passwords`](server-system-variables.html#sysvar_old_passwords)
to 0 first.
