...
* Use the [**mysql\_config\_editor**](mysql-config-editor.html "4.6.6 mysql_config_editor — MySQL Configuration Utility") utility,
...
* Use the [`--password`](connection-options.html#option_general_password) or
...
* Store your password in an option file. For example, on Unix,
  ...
  [Section 4.2.2.2, “Using Option Files”](option-files.html "4.2.2.2 Using Option Files"), discusses option files in
  more detail.
...
On Unix, the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client writes a record of
executed statements to a history file (see
[Section 4.5.1.3, “mysql Client Logging”](mysql-logging.html "4.5.1.3 mysql Client Logging")). By default, this file is named
`.mysql_history` and is created in your home
directory. Passwords can be written as plain text in SQL
statements such as [`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement")
and [`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement"), so if you use
...
any file in which the commands are saved contains MySQL
