...
The [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") statement loads a
...
DATA`](load-data.html "13.2.6 LOAD DATA Statement") has two potential security issues:

* Because [`LOAD DATA
  LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") is an SQL statement, parsing occurs on the
  ...
  [`LOAD DATA
  LOCAL`](load-data.html "13.2.6 LOAD DATA Statement"), so a more fundamental issue is that clients
...
* In a Web environment where the clients are connecting from a
  ...
  [`LOAD DATA
  LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") to read any files that the Web server process
...
To avoid [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") issues, clients
should avoid using `LOCAL`.

Adminstrators and applications can configure whether to permit
...
* On the server side:
  ...
  + By default, [`local_infile`](server-system-variables.html#sysvar_local_infile)
    is enabled. To cause the server to refuse or permit
    [`LOAD DATA
    LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") statements explicitly (regardless of how
    client programs and libraries are configured at build time
    or runtime), start [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") with
    [`local_infile`](server-system-variables.html#sysvar_local_infile) disabled or
    enabled. [`local_infile`](server-system-variables.html#sysvar_local_infile) can
...
* On the client side:
  ...
  + For client programs that use the C API, local data loading
    ...
    invoke the [`mysql_options()`](/doc/c-api/5.7/en/mysql-options.html)
  ...
  + For the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client, local data
  ...
  + For the [**mysqlimport**](mysqlimport.html "4.5.5 mysqlimport — A Data Import Program") client, local data
  ...
  + If you use [`LOAD
    DATA LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") in Perl scripts or other programs
    ...
    [`loose-`](option-modifiers.html "4.2.2.4 Program Option Modifiers")
    prefix:
...
[`LOAD DATA
LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") statement receives the following error message:
...
[`LOAD DATA LOCAL
INFILE`](load-data.html "13.2.6 LOAD DATA Statement") statements to upload data, so the
