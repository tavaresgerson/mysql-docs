...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") should be started by the local
...
, the MySQL server [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") can be started and
run by any user. However, you should avoid running the server as
the Unix `root` user for security reasons. To
change [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") to run as a normal unprivileged
...
2. Change the database directories and files so that
   *`user_name`* has privileges to read
   ...
   If you do not do this, the server is unable to access
...
3. Start the server as user *`user_name`*.
   Another alternative is to start [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") as
   the Unix `root` user and use the
   [`--user=user_name`](server-options.html#option_mysqld_user)
   option. [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") starts, then switches to run
...
machine can run the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client with a
[`--user=root`](mysql-command-options.html#option_mysql_user) option and perform any
