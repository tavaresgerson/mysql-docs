...
crack the password. (See [Section 6.1.2.4, “Password Hashing in MySQL”](password-hashing.html "6.1.2.4 Password Hashing in MySQL"), for a
...
[Section 6.3, “Using Encrypted Connections”](encrypted-connections.html "6.3 Using Encrypted Connections"). Alternatively, use SSH to
...
* Require all MySQL accounts to have a password. A client
  ...
  example, anyone can use the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") program
...
* Make sure that the only Unix user account with read or write
  ...
  used for running [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server").
...
* Never run the MySQL server as the Unix `root`
  ...
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") refuses to run as
  `root` unless that is specified explicitly
  using the [`--user=root`](server-options.html#option_mysqld_user) option.

  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") can (and should) be run as an
  ...
  MySQL. To start [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") as a different Unix
  ...
  [`mysqld_safe`](mysqld-safe.html "4.3.2 mysqld_safe — MySQL Server Startup Script") or
  [**mysql.server**](mysql-server.html "4.3.3 mysql.server — MySQL Server Startup Script"). For more details, see
  [Section 6.1.5, “How to Run MySQL as a Normal User”](changing-mysql-user.html "6.1.5 How to Run MySQL as a Normal User").

  Running [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") as a Unix user other than
...
* Do not grant the [`FILE`](privileges-provided.html#priv_file) privilege
  ...
  privileges of the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") daemon. This
  ...
  [`SELECT ... INTO
  OUTFILE`](select-into.html "13.2.9.1 SELECT ... INTO Statement") do not overwrite existing files and are
  ...
  DATA`](load-data.html "13.2.6 LOAD DATA Statement") to load `/etc/passwd` into a
...
* Do not grant the [`PROCESS`](privileges-provided.html#priv_process) or
  ...
  processlist**](mysqladmin.html "4.5.2 mysqladmin — A MySQL Server Administration Program") and [`SHOW
  PROCESSLIST`](show-processlist.html "13.7.5.29 SHOW PROCESSLIST Statement") shows the text of any statements
  ...
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") reserves an extra connection for
...
* Do not permit the use of symlinks to tables. (This capability
  ...
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") as `root`, because
...
* If you want to restrict the number of connections permitted to
  ...
  in [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server"). The [`CREATE
  USER`](create-user.html "13.7.1.2 CREATE USER Statement") and [`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement")
  statements also support resource control options for limiting
  the extent of server use permitted to an account. See
  [Section 13.7.1.2, “CREATE USER Statement”](create-user.html "13.7.1.2 CREATE USER Statement"), and
