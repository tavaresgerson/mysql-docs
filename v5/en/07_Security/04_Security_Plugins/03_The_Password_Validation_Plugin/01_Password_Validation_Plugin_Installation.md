...
by Oracle](linux-installation-rpm.html "2.5.5 Installing MySQL on Linux Using RPM Packages from Oracle"), `validate_password` is
...
[`INSTALL PLUGIN`](install-plugin.html "13.7.3.3 INSTALL PLUGIN Statement") or is loaded with
...
[`--validate-password`](validate-password-options-variables.html#option_mysqld_validate-password) with a value
of `FORCE` or
