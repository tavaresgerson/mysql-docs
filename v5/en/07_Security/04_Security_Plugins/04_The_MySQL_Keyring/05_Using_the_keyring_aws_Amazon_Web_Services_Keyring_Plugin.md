...
[Section 6.4.4.1, “Keyring Plugin Installation”](keyring-plugin-installation.html "6.4.4.1 Keyring Plugin Installation"), together with
...
3. Use the AWS KMS account to create a customer master key
   ...
   [`SET
   GLOBAL`](set-variable.html "13.7.4.1 SET Syntax for Variable Assignment").)
...
3. Reinitialize the `keyring_aws` plugin so
   ...
   * Reinitialize the plugin without restarting the server
     ...
     [`INSTALL PLUGIN`](install-plugin.html "13.7.3.3 INSTALL PLUGIN Statement") has
     ...
     execute [`UNINSTALL
     PLUGIN`](uninstall-plugin.html "13.7.3.4 UNINSTALL PLUGIN Statement") again to unregister the plugin in
