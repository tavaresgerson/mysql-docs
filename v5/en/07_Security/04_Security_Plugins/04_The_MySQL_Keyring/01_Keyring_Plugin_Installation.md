...
Keyring service consumers require that a keyring plugin be
installed. This section describes how to install the keyring
plugin of your choosing. Also, for general information about
...
If you intend to use keyring functions in conjunction with the
chosen keyring plugin, install the functions after installing
that plugin, using the instructions in
...
* `keyring_file`: Stores keyring data in a
  file local to the server host. Available in MySQL Community Edition and MySQL Enterprise Edition
  distributions.

* `keyring_encrypted_file`: Stores keyring
  data in an encrypted, password-protected file local to the
...
Schema [`PLUGINS`](information-schema-plugins-table.html "24.3.17 The INFORMATION_SCHEMA PLUGINS Table") table or use the
...
* Plugins installed using [`INSTALL
  PLUGIN`](install-plugin.html "13.7.3.3 INSTALL PLUGIN Statement") are registered in the
