...
see [Section 6.4.1.4, “Caching SHA-2 Pluggable Authentication”](caching-sha2-pluggable-authentication.html "6.4.1.4 Caching SHA-2 Pluggable Authentication").
...
`sha256_password` plugin uses MySQL'
...
[`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement") syntax can be used.
...
* For connections by accounts that authenticate using
  ...
  + For replicas, RSA key pair-based password exchange
    cannot be used to connect to source servers for
    accounts that authenticate with the
    `sha256_password` plugin. For such
...
3. Restart the server, then connect to it and check the
   ...
   variable value. The actual value differs from that shown
...
[`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement") time). The plugin
