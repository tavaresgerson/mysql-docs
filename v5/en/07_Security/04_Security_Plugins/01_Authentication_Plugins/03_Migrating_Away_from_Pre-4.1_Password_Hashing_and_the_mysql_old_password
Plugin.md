...
account listed in the `mysql.user` system table
using the authentication plugin named in the
`plugin` column. If the
`plugin` column is empty, the server
...
`mysql_old_password` plugin are deprecated in
...
removal of `mysql_old_password` support, DBAs
...
<table summary="Characteristics of MySQL accounts and what must be done to upgrade them."><col style="width: 30%"/><col style="width: 20%"/><col style="width: 30%"/><col style="width: 20%"/><thead><tr>
...
USER`](alter-user.html "13.7.1.1 ALTER USER Statement") to expire empty account passwords).
...
[`mysqld_upgrade`](mysql-upgrade.html "4.4.7 mysql_upgrade — Check and Upgrade MySQL Tables"), which performs the same
...
[`secure_auth=0`](server-system-variables.html#sysvar_secure_auth).
...
The client-side [`--secure-auth`](mysql-command-options.html#option_mysql_secure-auth)
option is enabled by default, so remind users to disable it;
otherwise, they cannot connect:
...
* Before MySQL 5.7, you must modify the
  `mysql.user` system table directly using
  these statements:
