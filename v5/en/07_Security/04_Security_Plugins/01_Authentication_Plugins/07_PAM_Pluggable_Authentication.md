...
<table summary="Names for the plugins and library file used for PAM password authentication."><col style="width: 30%"/><col style="width: 70%"/><thead><tr>
<th>Plugin or File</th>
<th>Plugin or File Name</th>
</tr></thead><tbody><tr>
<td>Server-side plugin</td>
<td><code>authentication_pam</code></td>
</tr><tr>
<td>Client-side plugin</td>
<td><code>mysql_clear_password</code></td>
</tr><tr>
<td>Library file</td>
<td><code>authentication_pam.so</code></td>
</tr></tbody></table>
...
see [Section 6.2.13, “Pluggable Authentication”](pluggable-authentication.html "6.2.13 Pluggable Authentication"). For information
about the `mysql_clear_password` plugin, see
[Section 6.4.1.6, “Client-Side Cleartext Pluggable Authentication”](cleartext-pluggable-authentication.html "6.4.1.6 Client-Side Cleartext Pluggable Authentication"). For proxy
user information, see [Section 6.2.14, “Proxy Users”](proxy-users.html "6.2.14 Proxy Users").
...
installing plugins, see [Section 5.5.1, “Installing and Uninstalling Plugins”](plugin-loading.html "5.5.1 Installing and Uninstalling Plugins").
...
`authentication_pam`. The file name suffix
...
[`INSTALL PLUGIN`](install-plugin.html "13.7.3.3 INSTALL PLUGIN Statement") loads the plugin
...
[`PLUGINS`](information-schema-plugins-table.html "24.3.17 The INFORMATION_SCHEMA PLUGINS Table") table or use the
[`SHOW PLUGINS`](show-plugins.html "13.7.5.25 SHOW PLUGINS Statement") statement (see
[Section 5.5.2, “Obtaining Server Plugin Information”](obtaining-plugin-information.html "5.5.2 Obtaining Server Plugin Information")). For example:
...
* If you installed the plugin at runtime using an
  [`INSTALL PLUGIN`](install-plugin.html "13.7.3.3 INSTALL PLUGIN Statement") statement,
  it remains installed across server restarts. To uninstall
  it, use [`UNINSTALL PLUGIN`](uninstall-plugin.html "13.7.3.4 UNINSTALL PLUGIN Statement"):
...
[`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement") statement, use the
...
* No proxy users. This uses PAM only to check login names
  ...
  *`user_name`* must be the external
  ...
  to 32 characters (see [Section 6.2.3, “Grant Tables”](grant-tables.html "6.2.3 Grant Tables")),
...
* The PAM service name corresponds to the authentication
  ...
  [`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement") statement for
...
4. Use the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") command-line client to
...
4. Create a default proxy user (`''@''`)
   ...
   it, see [Default Proxy User and Anonymous User Conflicts](proxy-users.html#proxy-users-conflicts "Default Proxy User and Anonymous User Conflicts").
...
5. Create the proxied accounts and grant to each one the
   ...
   who authenticate using PAM use the
   `developer` or
   `data_entry` account by proxy based on
   their PAM group. (This assumes that the plugin is
   installed. For instructions, see
   [Section 6.4.1.10, “No-Login Pluggable Authentication”](no-login-pluggable-authentication.html "6.4.1.10 No-Login Pluggable Authentication").) For
...
7. Use the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") command-line client to
   connect to the MySQL server as `antonio`.
...
The PAM authentication plugin checks at initialization time
whether the `AUTHENTICATION_PAM_LOG`
environment value is set. In MySQL 5.7, and in MySQL NDB
Cluster rrior to NDB 7.5.33 and NDB 7.6.29, the value does not
matter. The plugin enables logging of diagnostic messages to
the standard output, including passwords. These messages may
be helpful for debugging PAM-related issues that occur when
the plugin performs authentication.

In MySQL NDB Cluster, beginning with versions 7.5.33 and
7.6.29, passwords are *not* included if you
set `AUTHENTICATION_PAM_LOG=1` (or some other
arbitrary value); you can enable logging of debugging
messages, passwords included, by setting
