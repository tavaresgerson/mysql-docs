...
* The row indicates that the account is unlocked. Locking state
  ...
  [`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement") statement.
...
traveling over the connection. See [Section 6.2.1, “Account User Names and Passwords”](user-names.html "6.2.1 Account User Names and Passwords").
...
<table summary="How various combinations of User and Host values in the user table apply to incoming connections to a MySQL server."><col style="width: 15%"/><col style="width: 35%"/><col style="width: 50%"/><thead><tr>
<th><code>User</code> Value</th>
...
<td><code>fred</code>, connecting from any host in the
            <code>example.net</code> domain</td>
</tr><tr>
<th><code>'fred'</code></th>
<td><code>'x.example.%'</code></td>
<td><code>fred</code>, connecting from
            <code>x.example.net</code>,
            <code>x.example.com</code>,
            <code>x.example.edu</code>, and so on; this is
            probably not useful</td>
</tr><tr>
<th><code>'fred'</code></th>
<td><code>'198.51.100.177'</code></td>
<td><code>fred</code>, connecting from the host with IP address
            <code>198.51.100.177</code></td>
</tr><tr>
<th><code>'fred'</code></th>
<td><code>'198.51.100.%'</code></td>
<td><code>fred</code>, connecting from any host in the
            <code>198.51.100</code> class C subnet</td>
</tr><tr>
<th><code>'fred'</code></th>
...
* The specificity of a literal IP address is not affected by
  whether it has a netmask, so `198.51.100.13`
  and `198.51.100.0/255.255.255.0` are
  considered equally specific.
...
[`CURRENT_USER()`](information-functions.html#function_current-user) function. (See
[Section 12.15, “Information Functions”](information-functions.html "12.15 Information Functions").) It returns a value in
`user_name@host_name`
