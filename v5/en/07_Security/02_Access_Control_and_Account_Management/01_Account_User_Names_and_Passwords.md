...
methods. See [Section 6.2.13, “Pluggable Authentication”](pluggable-authentication.html "6.2.13 Pluggable Authentication").
...
* MySQL user names are up to 32 characters long. Operating
  ...
  [Section 2.10, “Upgrading MySQL”](upgrading.html "2.10 Upgrading MySQL"). Attempting to redefine the
...
* If the user name and password contain only ASCII characters,
  ...
  [`mysql_options()`](/doc/c-api/5.7/en/mysql-options.html) C API function
  ...
  [`mysql_options()`](/doc/c-api/5.7/en/mysql-options.html) to be called
  as just described. In addition, character set autodetection is
  supported as described in
  [Section 10.4, “Connection Character Sets and Collations”](charset-connection.html "10.4 Connection Character Sets and Collations"). For programs that use a
  connector that is not based on the C API, the connector may
  provide an equivalent to
  [`mysql_options()`](/doc/c-api/5.7/en/mysql-options.html) that can be
...
[`REVOKE`](revoke.html "13.7.1.6 REVOKE Statement"). See
[Section 6.2.7, “Adding Accounts, Assigning Privileges, and Dropping Accounts”](creating-accounts.html "6.2.7 Adding Accounts, Assigning Privileges, and Dropping Accounts"), and
...
insecure. See [Section 6.1.2.1, “End-User Guidelines for Password Security”](password-security-user.html "6.1.2.1 End-User Guidelines for Password Security"). To avoid
giving the password on the command line, use an option file or a
login path file. See [Section 4.2.2.2, “Using Option Files”](option-files.html "4.2.2.2 Using Option Files"), and
...
and other connection parameters, see [Section 4.2.4, “Connecting to the MySQL Server Using Command Options”](connecting.html "4.2.4 Connecting to the MySQL Server Using Command Options").