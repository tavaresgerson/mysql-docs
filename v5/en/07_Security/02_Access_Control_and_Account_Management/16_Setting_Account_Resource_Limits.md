...
time, use the [`CREATE USER`](create-user.html "13.7.1.2 CREATE USER Statement")
statement. To modify the limits for an existing account, use
[`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement"). Provide a
...
[`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement") statement. The following
...
* To reset the current counts to zero for all accounts, issue a
  ...
  reload**](mysqladmin.html "4.5.2 mysqladmin — A MySQL Server Administration Program") command).
...
([`ER_TOO_MANY_USER_CONNECTIONS`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_too_many_user_connections) or
[`ER_USER_LIMIT_REACHED`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_user_limit_reached)) if the
