...
The [`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement") statement enables
...
[`ER_MUST_CHANGE_PASSWORD_LOGIN`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_must_change_password_login)
error:
...
* The client can reset the account password with
  ...
  [`SET PASSWORD`](set-password.html "13.7.1.7 SET PASSWORD Statement"). After that has
...
* The client can use the
  ...
  [`SET PASSWORD`](set-password.html "13.7.1.7 SET PASSWORD Statement") must be used
  instead of [`ALTER USER`](alter-user.html "13.7.1.1 ALTER USER Statement") and the
...
returns an [`ER_MUST_CHANGE_PASSWORD`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_must_change_password)
error:
...
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client because by default such
invocations are put in sandbox mode. To resume normal functioning,
select a new password.

For noninteractive invocations of the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
client (for example, in batch mode), the server normally
disconnects the client if the password is expired. To permit
noninteractive [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") invocations to stay
connected so that the password can be changed (using the
statements permitted in sandbox mode), add the
[`--connect-expired-password`](mysql-command-options.html#option_mysql_connect-expired-password) option to
the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") command.
...
* Pass the
  ...
  to [`mysql_options()`](/doc/c-api/5.7/en/mysql-options.html) prior to
  connecting:
  ...
  This is the technique used within the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
