...
* Replicated statements executed on a replica are processed by
  the replica SQL thread, which has full privileges. It is
  possible for a procedure to follow different execution paths
  on source and replica servers, so a user can write a routine
  containing a dangerous statement that executes only on the
  replica where it is processed by a thread that has full
  privileges.
...
[Section 5.4.4, “The Binary Log”](binary-log.html "5.4.4 The Binary Log").) If the binary log is not enabled,
replication is not possible, nor is the binary log available for
data recovery. In MySQL 5.7, binary logging is not enabled by
default, and you enable it using the
...
[`CALL`](call.html "13.2.1 CALL Statement") statement is not logged. For
...
* To create or alter a stored function, you must have the
  ...
  the [`CREATE ROUTINE`](privileges-provided.html#priv_create-routine) or
  [`ALTER ROUTINE`](privileges-provided.html#priv_alter-routine) privilege that is
  normally required. (Depending on the
  `DEFINER` value in the function definition,
  [`SUPER`](privileges-provided.html#priv_super) might be required
...
* When you create a stored function, you must declare either
  ...
  FUNCTION`](create-function.html "13.1.13 CREATE FUNCTION Statement") statement to be accepted, at least one of
...
* For information about built-in functions that may be unsafe
  ...
  [Section 16.4.1, “Replication Features and Issues”](replication-features.html "16.4.1 Replication Features and Issues").
...
exception: [`CREATE TRIGGER`](create-trigger.html "13.1.20 CREATE TRIGGER Statement") does not
...
TRIGGER`](create-trigger.html "13.1.20 CREATE TRIGGER Statement") if you do not have the required privileges. On
...
* The server writes [`CREATE EVENT`](create-event.html "13.1.12 CREATE EVENT Statement"),
  ...
  [`ALTER PROCEDURE`](alter-procedure.html "13.1.6 ALTER PROCEDURE Statement"),
  [`ALTER FUNCTION`](alter-function.html "13.1.3 ALTER FUNCTION Statement"),
  [`DROP EVENT`](drop-event.html "13.1.23 DROP EVENT Statement"),
  [`DROP PROCEDURE`](drop-procedure.html "13.1.27 DROP PROCEDURE and DROP FUNCTION Statements"), and
  [`DROP FUNCTION`](drop-function.html "13.1.24 DROP FUNCTION Statement") statements to the
  binary log.

* A stored function invocation is logged as a
  [`SELECT`](select.html "13.2.9 SELECT Statement") statement if the
  ...
  statements. For example, [`SELECT`](select.html "13.2.9 SELECT Statement")
  statements are not written to the binary log, but a
  [`SELECT`](select.html "13.2.9 SELECT Statement") might invoke a stored
  ...
  When the [`SELECT`](select.html "13.2.9 SELECT Statement") statement
  executes, the function `f1()` is invoked
  three times. Two of those invocations insert a row, and MySQL
  logs a [`SELECT`](select.html "13.2.9 SELECT Statement") statement for
  ...
  The server also logs a [`SELECT`](select.html "13.2.9 SELECT Statement")
  statement for a stored function invocation when the function
  invokes a stored procedure that causes an error. In this case,
  the server writes the [`SELECT`](select.html "13.2.9 SELECT Statement")
...
* Logging stored function invocations rather than the statements
  ...
  + Statements executed on a replica are processed by the
    replica SQL thread which has full privileges.
  ...
  The [`CREATE FUNCTION`](create-function.html "13.1.13 CREATE FUNCTION Statement") and
  [`INSERT`](insert.html "13.2.5 INSERT Statement") statements are written
  to the binary log, so the replica executes them. Because the
  replica SQL thread has full privileges, it executes the
  ...
  [`ALTER FUNCTION`](alter-function.html "13.1.3 ALTER FUNCTION Statement"), you must have
...
* Stored procedure calls are logged at the statement level
  rather than at the [`CALL`](call.html "13.2.1 CALL Statement") level.
  That is, the server does not log the
  [`CALL`](call.html "13.2.1 CALL Statement") statement, it logs those
  statements within the procedure that actually execute. As a
  result, the same changes that occur on the source server are
  observed on replicas. This prevents problems that could result
  ...
  + A statement to be logged might contain references to local
    ...
    *`var_name`*. Thus, if you invoke
    ...
    [`CREATE TABLE
    ... SELECT`](create-table.html "13.1.18 CREATE TABLE Statement") statements when the source column
  ...
  + Procedure calls can occur within a committed or
    ...
    [`COMMIT`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements"), and
    ...
    transaction,
    [`BEGIN`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements")
    and [`COMMIT`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements") statements are
...
* A stored procedure call is *not* written to
  ...
  [`DO`](do.html "13.2.3 DO Statement") statement (if it occurs
