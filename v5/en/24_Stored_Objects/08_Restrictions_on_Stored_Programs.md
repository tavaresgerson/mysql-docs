...
[`DO`](do.html "13.2.3 DO Statement") clause of Event Scheduler event
...
* SQL prepared statements
  ...
  [`EXECUTE`](execute.html "13.5.2 EXECUTE Statement"),
  [`DEALLOCATE PREPARE`](deallocate-prepare.html "13.5.3 DEALLOCATE PREPARE Statement")) can be
...
* Generally, statements not permitted in SQL prepared
  ...
  [Section 13.5, “Prepared Statements”](sql-prepared-statements.html "13.5 Prepared Statements"). Exceptions are
  [`SIGNAL`](signal.html "13.6.7.5 SIGNAL Statement"),
  [`RESIGNAL`](resignal.html "13.6.7.4 RESIGNAL Statement"), and
  [`GET DIAGNOSTICS`](get-diagnostics.html "13.6.7.3 GET DIAGNOSTICS Statement"), which are
...
* Within all stored programs (stored procedures and functions,
  ...
  [`BEGIN
  [WORK]`](commit.html "13.3.1 START TRANSACTION, COMMIT, and ROLLBACK Statements") as the beginning of a
  [`BEGIN ...
  END`](begin-end.html "13.6.1 BEGIN ... END Compound Statement") block.
...
use [`FLUSH`](flush.html "13.7.6.3 FLUSH Statement") in a stored procedure,
...
* Statements that return a result set. This includes
  [`SELECT`](select.html "13.2.9 SELECT Statement") statements that do not
  have an `INTO
  var_list` clause and
  other statements such as
  [`SHOW`](show.html "13.7.5 SHOW Statements"),
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement"), and
  [`CHECK TABLE`](check-table.html "13.7.2.2 CHECK TABLE Statement"). A function can
  process a result set either with
  [`SELECT ... INTO
  var_list`](select-into.html "13.2.9.1 SELECT ... INTO Statement") or by using a
  cursor and [`FETCH`](fetch.html "13.6.6.3 Cursor FETCH Statement") statements.
  See [Section 13.2.9.1, “SELECT ... INTO Statement”](select-into.html "13.2.9.1 SELECT ... INTO Statement"), and
  [Section 13.6.6, “Cursors”](cursors.html "13.6.6 Cursors").
...
* [`HANDLER ...
  READ`](handler.html "13.2.4 HANDLER Statement") statements that invoke stored functions can
...
* The [`RETURN`](return.html "13.6.5.7 RETURN Statement") statement is not
  permitted in triggers, which cannot return a value. To exit
  a trigger immediately, use the
  [`LEAVE`](leave.html "13.6.5.4 LEAVE Statement") statement.
...
* Event timings using the intervals `YEAR`,
  ...
  Information Schema [`EVENTS`](information-schema-events-table.html "24.3.8 The INFORMATION_SCHEMA EVENTS Table")
...
* Each execution of the statements contained in the body of an
  ...
  [`SHOW STATUS`](show-status.html "13.7.5.35 SHOW STATUS Statement"). However, such
...
* References to stored functions, loadable functions, and
  ...
  [`ALTER EVENT`](alter-event.html "13.1.2 ALTER EVENT Statement") statements are
...
[`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") storage engine, you must keep
...
* The `.TRN` and `.TRG`
  ...
  [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") storage engine, and are not
...
[`CREATE FUNCTION`](create-function.html "13.1.13 CREATE FUNCTION Statement"), or
[`CREATE TRIGGER`](create-trigger.html "13.1.20 CREATE TRIGGER Statement") statements on each
