...
[`MD5()`](encryption-functions.html#function_md5) for this purpose; for
...
as [`PASSWORD()`](encryption-functions.html#function_password).
...
* `KEY` takes only a list of zero or more
  ...
  [`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement is
  valid in MySQL 5.7:
  ...
  TABLE`](show-create-table.html "13.7.5.10 SHOW CREATE TABLE Statement") or in the
  ...
  TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement is valid:
  ...
  [`VARBINARY`](binary-varbinary.html "11.3.3 The BINARY and VARBINARY Types") columns can be used
  ...
  [`TEXT`](blob.html "11.3.4 The BLOB and TEXT Types") columns in index
  definitions, it is not possible to use columns of these two
  types in partitioning keys. In MySQL 5.7,
  columns using prefixes are permitted when creating,
  altering, or upgrading partitioned tables, even though they
  are not included in the table's partitioning key. This
  is a known issue in MySQL 5.7 which is
  addressed in MySQL 8.0, where this permissive behavior is
  deprecated, and the server displays appropriate warnings or
  errors when attempting to use such columns in these cases.
  See [Column index prefixes not supported for key partitioning](partitioning-limitations.html#partitioning-limitations-prefixes "Column index prefixes not supported for key partitioning"), for
  more information and examples.

  Note

  Tables using the [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") storage
  ...
  key generated by the [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6")
  storage engine for each NDB Cluster table is used as the
  partitioning key.

  If you define an explicit partitioning scheme for an
  [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") table, the table must
  ...
  [**ndb\_desc**](mysql-cluster-programs-ndb-desc.html "21.5.10 ndb_desc — Describe NDB Tables") utility (with the
  `-p` option).
  ...
  partitioning key. See [Chapter 21, *MySQL NDB Cluster 7.5 and NDB Cluster 7.6*](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6").
...
arithmetic. See [Section 22.2.4.1, “LINEAR HASH Partitioning”](partitioning-linear-hash.html "22.2.4.1 LINEAR HASH Partitioning"), for
