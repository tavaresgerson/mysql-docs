...
[`-`](arithmetic-functions.html#operator_minus), and
[`*`](arithmetic-functions.html#operator_times) is permitted in
...
[Section 22.2, “Partitioning Types”](partitioning-types.html "22.2 Partitioning Types"), for more information).

The [`DIV`](arithmetic-functions.html#operator_div) operator is also supported,
and the [`/`](arithmetic-functions.html#operator_divide) operator
is not permitted. (Bug #30188, Bug #33182)

The bit operators
[`|`](bit-functions.html#operator_bitwise-or),
[`&`](bit-functions.html#operator_bitwise-and),
[`^`](bit-functions.html#operator_bitwise-xor),
[`<<`](bit-functions.html#operator_left-shift),
[`>>`](bit-functions.html#operator_right-shift), and
[`~`](bit-functions.html#operator_bitwise-invert) are not
permitted in partitioning expressions.

**HANDLER statements.**
Previously, the [`HANDLER`](handler.html "13.2.4 HANDLER Statement") statement
...
discussed in [Section 5.1.10, “Server SQL Modes”](sql-mode.html "5.1.10 Server SQL Modes"), the results of many
...
1. **Error handling.**
   ...
   the same [`INSERT`](insert.html "13.2.5 INSERT Statement") statement to
   fail, as shown here:
...
2. **Table accessibility.**
   ...
   [`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement can be
...
* **File system operations.**
  ...
  [`ALTER
  TABLE`](alter-table-partition-operations.html "13.1.8.1 ALTER TABLE Partition Operations") with `PARTITION BY ...`,
  ...
  [`large_files_support`](server-system-variables.html#sysvar_large_files_support) is
  enabled and that
  [`open_files_limit`](server-system-variables.html#sysvar_open_files_limit) is set
  ...
  See also
  [Maximum number of partitions](partitioning-limitations.html#partitioning-limitations-max-partitions "Maximum number of partitions").

* **MyISAM and partition file descriptor usage.**
  For a partitioned [`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine") table,
  ...
  [`ALTER
  TABLE`](alter-table-partition-operations.html "13.1.8.1 ALTER TABLE Partition Operations") operations.
  ...
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") are not affected by this
  issue.
  ...
  operations, you should make sure that the
  [`open_files_limit`](server-system-variables.html#sysvar_open_files_limit) system
...
* **Table locks.**
  ...
  [`UPDATE`](update.html "13.2.11 UPDATE Statement") operations are
...
* **Indexes; partition pruning.**
  ...
  [Section 22.4, “Partition Pruning”](partitioning-pruning.html "22.4 Partition Pruning"), for more
  information.

  Previously, index condition pushdown was not supported for
  partitioned tables. This limitation was removed in MySQL
  5.7.3. See
...
* **Performance with LOAD DATA.**
  In MySQL 5.7, [`LOAD
  DATA`](load-data.html "13.2.6 LOAD DATA Statement") uses buffering to improve performance. You
...
using the [`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") storage engine is
...
[NDB and user-defined partitioning](mysql-cluster-nodes-groups.html#mysql-cluster-nodes-groups-user-partitioning "NDB and user-defined partitioning"),
...
[`open_files_limit`](server-system-variables.html#sysvar_open_files_limit) system variable.
However, this is dependent on the operating system, and may not be
possible or advisable on all platforms; see
[Section B.3.2.16, “File Not Found and Similar Errors”](not-enough-file-handles.html "B.3.2.16 File Not Found and Similar Errors"), for more information.
...
See also
[File system operations](partitioning-limitations.html#partitioning-limitations-file-system-ops "File system operations").
...
partitioned [`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine") tables, using
the [`CACHE INDEX`](cache-index.html "13.7.6.2 CACHE INDEX Statement") and
[`LOAD INDEX INTO
CACHE`](load-index.html "13.7.6.5 LOAD INDEX INTO CACHE Statement") statements. Key caches may be defined for one,
...
Partitioned tables using the [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine")
...
[`CREATE
TABLE`](create-table-foreign-keys.html "13.1.18.5 FOREIGN KEY Constraints") and [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement")
...
application uses [`REPLACE`](replace.html "13.2.8 REPLACE Statement")
statements and you do this, the results of these statements can
be drastically altered. See [Section 13.2.8, “REPLACE Statement”](replace.html "13.2.8 REPLACE Statement"), for more
...
[`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine") storage engine.
...
[`ALTER
TABLE ... PARTITION BY ...`](alter-table-partition-operations.html "13.1.8.1 ALTER TABLE Partition Operations") statement on such a table
fails with an error.
...
[`ENUM`](enum.html "11.3.5 The ENUM Type") columns cannot be used. The
column or expression value may also be `NULL`.
(See [Section 22.2.7, “How MySQL Partitioning Handles NULL”](partitioning-handling-nulls.html "22.2.7 How MySQL Partitioning Handles NULL").)
...
1. When partitioning by [`LINEAR`]
   ...
   [`BLOB`](blob.html "11.3.4 The BLOB and TEXT Types") as partitioning keys,
   because MySQL's internal key-hashing functions produce
   the correct data type from these types. For example, the
   following two [`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement")
...
2. When partitioning by `RANGE COLUMNS` or
   ...
   [`DATETIME`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") columns. For example,
   each of the following [`CREATE
   TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statements is valid:
...
[`TEXT`](blob.html "11.3.4 The BLOB and TEXT Types") column types.
...
[`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement, which has
three [`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") columns, and whose
...
No warning is issued or any other indication provided that this
has occurred, except in the event that all columns specified for
the partitioning key use prefixes, in which case the statement
fails with the error message shown here:
...
This is a known issue which is addressed in MySQL 8.0 by
deprecating the permissive behavior; in MYSQL 8.0, if any columns
using prefixes are included in a table's partitioning
function, the server logs an appropriate warning for each such
column, or raises a descriptive error if necessary. (Allowing the
use of columns with prefixes in partitioning keys is subject to
removal altogether in a future version of MySQL.)
...
* On Windows, the `DATA DIRECTORY` and
  ...
  [`MyISAM`](myisam-storage-engine.html "15.2 The MyISAM Storage Engine") tables. However, you can
  use `DATA DIRECTORY` for individual
  partitions or subpartitions of
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") tables.

**Repairing and rebuilding partitioned tables.**
The statements [`CHECK TABLE`](check-table.html "13.7.2.2 CHECK TABLE Statement"),
...
[`REPAIR TABLE`](repair-table.html "13.7.2.5 REPAIR TABLE Statement") are supported for
partitioned tables.
...
[Section 13.1.8, “ALTER TABLE Statement”](alter-table.html "13.1.8 ALTER TABLE Statement"), for more information about these
two statements.
...
`REBUILD` was also accepted syntax prior to MySQL
...
[**mysqlcheck**](mysqlcheck.html "4.5.3 mysqlcheck — A Table Maintenance Program"), [**myisamchk**](myisamchk.html "4.6.3 myisamchk — MyISAM Table-Maintenance Utility"), and
[**myisampack**](myisampack.html "4.6.5 myisampack — Generate Compressed, Read-Only MyISAM Tables") are not supported with partitioned
tables.

**FOR EXPORT option (FLUSH TABLES).**
The
[`FLUSH
TABLES`](flush.html#flush-tables-for-export-with-list) statement's `FOR EXPORT`
option is not supported for partitioned
`InnoDB` tables in MySQL 5.7.4 and earlier.
(Bug #16943907)
