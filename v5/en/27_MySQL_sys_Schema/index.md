...
For upgrades, `mysqld_upgrade` installs the `sys` schema if it is not installed, and upgrades it to the current version otherwise. To permit this behavior to be suppressed, `mysqld_upgrade` has a `--skip-sys-schema` option.

`mysqld_upgrade` returns an error if a `sys` schema exists but has no `version` view, on the assumption that absence of this view indicates a user-created `sys` schema. To upgrade in this case, remove or rename the existing `sys` schema first.
