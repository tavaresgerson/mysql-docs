...
[Chapter 21, *MySQL NDB Cluster 7.5 and NDB Cluster 7.6*](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") and
...
* [MySQL Shell](/doc/mysql-shell/8.0/en/), which is an
  advanced client and code editor for MySQL.

* MySQL Server, and [Group
  Replication](group-replication.html "Chapter 17 Group Replication"), which enables a set of MySQL instances to
  provide high-availability. InnoDB Cluster provides an
  alternative, easy to use programmatic way to work with Group
  Replication.

* [MySQL Router](/doc/mysql-router/8.0/en/), a lightweight
...
Replication](group-replication.html "Chapter 17 Group Replication"), provides features such as automatic membership
...
[multi-primary](group-replication-multi-primary-mode.html "17.5.1.2 Multi-Primary Mode")
mode, where all instances are primaries.

You work with InnoDB Cluster using the
[AdminAPI](/doc/mysql-shell/8.0/en/admin-api-overview.html), provided as
...
and [install](/doc/mysql-shell/8.0/en/mysql-shell-install.html) MySQL Shell.
You need some hosts with MySQL Server instances
[installed](installing.html "Chapter 2 Installing and Upgrading MySQL"), and you can also
[install](/doc/mysql-router/8.0/en/mysql-router-installation.html) MySQL Router.

InnoDB Cluster supports [MySQL
Clone](/doc/refman/8.0/en/clone-plugin.html), which enables you to provision instances simply. In
...
instance](/doc/mysql-shell/8.0/en/add-instances-cluster.html) to the cluster and it is automatically provisioned.

Similarly, InnoDB Cluster is tightly integrated with
[MySQL Router](/doc/mysql-router/8.0/en/), and you can use
AdminAPI to [work with](/doc/mysql-shell/8.0/en/registered-routers.html)
them together. MySQL Router can automatically configure itself based on
an InnoDB Cluster, in a process called
[bootstrapping](/doc/mysql-shell/8.0/en/admin-api-bootstrapping-router.html),
...
users](/doc/mysql-shell/8.0/en/configuring-router-user.html) at the cluster level, to work with the MySQL Routers
...
[Using Instances Running MySQL
5.7](/doc/mysql-shell/8.0/en/using-version-5-7.html). For the best experience using AdminAPI and
InnoDB Cluster [upgrade](upgrading.html "2.10 Upgrading MySQL") to MySQL
8.0.
...
[Connectors and APIs](/doc/index-connectors.html).