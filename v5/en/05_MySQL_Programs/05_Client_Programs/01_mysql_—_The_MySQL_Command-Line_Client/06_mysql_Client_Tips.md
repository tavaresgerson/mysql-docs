...
effective use of [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") and about
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") operational behavior.

* [Input-Line Editing](mysql-tips.html#mysql-input-editing "Input-Line Editing")
* [Disabling Interactive History](mysql-tips.html#mysql-history "Disabling Interactive History")
* [Unicode Support on Windows](mysql-tips.html#windows-unicode-support "Unicode Support on Windows")
* [Displaying Query Results Vertically](mysql-tips.html#vertical-query-results "Displaying Query Results Vertically")
* [Using Safe-Updates Mode (--safe-updates)](mysql-tips.html#safe-updates "Using Safe-Updates Mode (--safe-updates)")
* [Disabling mysql Auto-Reconnect](mysql-tips.html#mysql-reconnect "Disabling mysql Auto-Reconnect")
* [mysql Client Parser Versus Server Parser](mysql-tips.html#mysql-parsers "mysql Client Parser Versus Server Parser")

##### Input-Line Editing

[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") supports input-line editing, which
...
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") (for example, the
`libedit` or `readline`
library).
...
current cursor position. If [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") was built
...
`.editrc`. Then [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
...
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") supports disabling the interactive
...
option was used to start [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client"). Closing the
...
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") with the
`--histignore="*"` option to ignore all new
input lines. To re-enable the recall (and logging) behavior,
restart [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") without the option.

If you prevent the `.mysql_history` file from
being created (see [Controlling the History File](mysql-logging.html#mysql-logging-history-file "Controlling the History File"))
and use `--histignore="*"` to start the
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client, the interactive history recall
facility is disabled fully. Alternatively, if you omit the
[`--histignore`](mysql-command-options.html#option_mysql_histignore) option, you can
...
writing to the console; the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client for
...
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client with properties set to
communicate through the console to the MySQL server using
Unicode.

To take advantage of this support manually, run
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") within a console that uses a compatible
...
3. Execute [**mysql.exe**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") with the
   [`--default-character-set=utf8`](mysql-command-options.html#option_mysql_default-character-set)
   (or `utf8mb4`) option. This option is
...
With those changes, [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") can use the Windows
...
To avoid those steps each time you run [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client"),
you can create a shortcut that invokes
[**mysql.exe**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client"). The shortcut should set the
console font to Lucida Console or some other compatible Unicode
font, and pass the
[`--default-character-set=utf8`](mysql-command-options.html#option_mysql_default-character-set) (or
`utf8mb4`) option to
[**mysql.exe**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client").
...
[`--safe-updates`](mysql-command-options.html#option_mysql_safe-updates) (or
...
[`DELETE`](delete.html "13.2.2 DELETE Statement") statement but forgotten
the `WHERE` clause indicating which rows to
modify. Normally, such statements update or delete all rows in
the table. With [`--safe-updates`](mysql-command-options.html#option_mysql_safe-updates),
...
[`SELECT`](select.html "13.2.9 SELECT Statement") statements that produce
(or are estimated to produce) very large result sets.

The [`--safe-updates`](mysql-command-options.html#option_mysql_safe-updates) option causes
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") to execute the following statement when
it connects to the MySQL server, to set the session values of
the [`sql_safe_updates`](server-system-variables.html#sysvar_sql_safe_updates),
...
* Enabling [`sql_safe_updates`](server-system-variables.html#sysvar_sql_safe_updates)
  causes [`UPDATE`](update.html "13.2.11 UPDATE Statement") and
  [`DELETE`](delete.html "13.2.2 DELETE Statement") statements to produce
...
* Setting [`sql_select_limit`](server-system-variables.html#sysvar_sql_select_limit) to
  1,000 causes the server to limit all
  [`SELECT`](select.html "13.2.9 SELECT Statement") result sets to 1,000
  rows unless the statement includes a
  `LIMIT` clause.

* Setting [`max_join_size`](server-system-variables.html#sysvar_max_join_size) to
  1,000,000 causes multiple-table
  [`SELECT`](select.html "13.2.9 SELECT Statement") statements to produce
...
[`--select-limit`](mysql-command-options.html#option_mysql_select-limit) and
[`--max-join-size`](mysql-command-options.html#option_mysql_max-join-size) options when you
invoke [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client"):

```sql
mysql --safe-updates --select-limit=500 --max-join-size=10000
```

It is possible for [`UPDATE`](update.html "13.2.11 UPDATE Statement") and
[`DELETE`](delete.html "13.2.2 DELETE Statement") statements to produce an
...
* Range access on the index cannot be used if memory usage
  ...
  scan. See [Limiting Memory Use for Range Optimization](range-optimization.html#range-optimization-memory-use "Limiting Memory Use for Range Optimization").

* If key comparisons require type conversion, the index may
  not be used (see [Section 8.3.1, “How MySQL Uses Indexes”](mysql-indexes.html "8.3.1 How MySQL Uses Indexes")). Suppose
  ...
  numerically (see [Section 12.3, “Type Conversion in Expression Evaluation”](type-conversion.html "12.3 Type Conversion in Expression Evaluation")),
...
* [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") with
  [`UPDATE`](update.html "13.2.11 UPDATE Statement") and
  [`DELETE`](delete.html "13.2.2 DELETE Statement") statements does not
  produce safe-updates errors. This enables use of
  [`EXPLAIN`](explain.html "13.8.2 EXPLAIN Statement") plus
  [`SHOW WARNINGS`](show-warnings.html "13.7.5.40 SHOW WARNINGS Statement") to see why an
...
If the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client loses its connection to
the server while sending a statement, it immediately and
automatically tries to reconnect once to the server and send the
statement again. However, even if [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
...
important to have [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") terminate with an
error if the connection has been lost, you can start the
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client with the
...
The [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client uses a parser on the client
side that is not a duplicate of the complete parser used by the
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") server on the server side. This can
...
* The server parser treats strings delimited by
  ...
  The [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client parser does not take the
  `ANSI_QUOTES` SQL mode into account. It
  treats strings delimited by `"`,
  `'`, and `` ` `` characters
  the same, regardless of whether
  [`ANSI_QUOTES`](sql-mode.html#sqlmode_ansi_quotes) is enabled.

* Within `/*! ... */` comments, the
  [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client parser interprets short-form
  [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") commands. The server parser does
  not interpret them because these commands have no meaning on
  the server side.

  If it is desirable for [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") not to
  interpret short-form commands within comments, a partial
  workaround is to use the
  [`--binary-mode`](mysql-command-options.html#option_mysql_binary-mode) option, which
  causes all [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") commands to be disabled
  except `\C` and `\d` in
  noninteractive mode (for input piped to
  [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") or loaded using the
  `source` command).