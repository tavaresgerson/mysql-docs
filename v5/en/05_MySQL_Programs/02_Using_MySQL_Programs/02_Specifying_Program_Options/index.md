...
[4.2.2.1 Using Options on the Command Line](command-line-options.html)

[4.2.2.2 Using Option Files](option-files.html)

[4.2.2.3 Command-Line Options that Affect Option-File Handling](option-file-options.html)

[4.2.2.4 Program Option Modifiers](option-modifiers.html)

[4.2.2.5 Using Options to Set Program Variables](program-variables.html)

[4.2.2.6 Option Defaults, Options Expecting Values, and the = Sign](option-defaults-equals.html)
...
* List the options in environment variables (see
  [Section 4.2.7, “Setting Environment Variables”](setting-environment-variables.html "4.2.7 Setting Environment Variables")). This method
  is useful for options that you want to apply each time the
  program runs. In practice, option files are used more commonly
  for this purpose, but [Section 5.7.3, “Running Multiple MySQL Instances on Unix”](multiple-unix-servers.html "5.7.3 Running Multiple MySQL Instances on Unix"),
...
following command causes [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") to connect to
...
There is one exception: For [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server"), the
*first* instance of the
[`--user`](server-options.html#option_mysqld_user) option is used as a security
...
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") in “no column names” mode:
