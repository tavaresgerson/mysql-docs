...
[4.6.7.1 mysqlbinlog Hex Dump Format](mysqlbinlog-hexdump.html)

[4.6.7.2 mysqlbinlog Row Event Display](mysqlbinlog-row-events.html)

[4.6.7.3 Using mysqlbinlog to Back Up Binary Log Files](mysqlbinlog-backup.html)

[4.6.7.4 Specifying the mysqlbinlog Server ID](mysqlbinlog-server-id.html)
...
[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") utility. You can also use
[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to display the contents of relay
...
[Section 5.4.4, “The Binary Log”](binary-log.html "5.4.4 The Binary Log"), and
...
Invoke [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") like this:
...
an SQL statement. See [Section 16.2.1, “Replication Formats”](replication-formats.html "16.2.1 Replication Formats"), for
...
The output from [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") can be
re-executed (for example, by using it as input to
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")) to redo the statements in the log.
This is useful for recovery operations after an unexpected
server exit. For other usage examples, see the discussion later
in this section and in [Section 7.5, “Point-in-Time (Incremental) Recovery”](point-in-time-recovery.html "7.5 Point-in-Time (Incremental) Recovery").

You can use [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to read binary log
...
When running [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") against a large
binary log, be careful that the filesystem has enough space for
the resulting files. To configure the directory that
[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") uses for temporary files, use the
`TMPDIR` environment variable.

[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") sets the value of
...
affects the handling of XA transactions.

[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") supports the following options,
...
<table frame="box" rules="all" summary="Command-line options available for mysqlbinlog."><col style="width: 27%"/><col style="width: 50%"/><col style="width: 11%"/><col style="width: 11%"/><thead><tr><th>Option Name</th>
<th>Description</th>
<th>Introduced</th>
<th>Deprecated</th>
</tr></thead><tbody><tr><th>--base64-output</th>
...
<td></td>
<td></td>
</tr><tr><th>--binlog-row-event-max-size</th>
...
<td></td>
<td></td>
</tr><tr><th>--verify-binlog-checksum</th>
...
<td><code>--base64-output=value</code></td>
</tr><tr><th>Type</th>
<td>String</td>
</tr><tr><th>Default Value</th>
<td><code>AUTO</code></td>
</tr><tr><th>Valid Values</th>
<td><p class="valid-value"><code>AUTO</code></p><p class="valid-value"><code>NEVER</code></p><p class="valid-value"><code>DECODE-ROWS</code></p></td>
</tr></tbody></table>

  This option determines when events should be displayed
  encoded as base-64 strings using
  [`BINLOG`](binlog.html "13.7.6.1 BINLOG Statement") statements. The option
  has these permissible values (not case-sensitive):

  + `AUTO` ("automatic") or
    `UNSPEC` ("unspecified") displays
    [`BINLOG`](binlog.html "13.7.6.1 BINLOG Statement") statements
    ...
    Automatic [`BINLOG`](binlog.html "13.7.6.1 BINLOG Statement")
    display is the only safe behavior if you intend to use
    the output of [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to
  ...
  + `NEVER` causes
    [`BINLOG`](binlog.html "13.7.6.1 BINLOG Statement") statements not to
    be displayed. [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") exits with
    an error if a row event is found that must be displayed
    using [`BINLOG`](binlog.html "13.7.6.1 BINLOG Statement").

  + `DECODE-ROWS` specifies to
    [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") that you intend for row
    ...
    [`BINLOG`](binlog.html "13.7.6.1 BINLOG Statement") statements, but
  ...
  output, see [Section 4.6.7.2, “mysqlbinlog Row Event Display”](mysqlbinlog-row-events.html "4.6.7.2 mysqlbinlog Row Event Display").
...
<td><code>4294967295</code></td>
  ...
  depend on whether [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") is run in
  blocking mode or non-blocking mode. When
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") is run in blocking mode, the
...
<td>String</td>
  ...
  This option causes [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to output
  entries from the binary log (local log only) that occur
  while *`db_name`* is been selected as
  the default database by [`USE`](use.html "13.8.4 USE Statement").

  The [`--database`](mysqlbinlog.html#option_mysqlbinlog_database) option
  for [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") is similar to the
  [`--binlog-do-db`](replication-options-binary-log.html#option_mysqld_binlog-do-db) option for
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server"), but can be used to specify only
  one database. If
  ...
  **Statement-based logging.**
  ...
  + There is an exception for [`CREATE
    DATABASE`](create-database.html "13.1.11 CREATE DATABASE Statement"), [`ALTER
    DATABASE`](alter-database.html "13.1.1 ALTER DATABASE Statement"), and [`DROP
    DATABASE`](drop-database.html "13.1.22 DROP DATABASE Statement"). The database being
  ...
  [**mysqlbinlog --database=test**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") does not
  output the first two [`INSERT`](insert.html "13.2.5 INSERT Statement")
  statements because there is no default database. It outputs
  the three [`INSERT`](insert.html "13.2.5 INSERT Statement") statements
  following [`USE
  test`](use.html "13.8.4 USE Statement"), but not the three
  [`INSERT`](insert.html "13.2.5 INSERT Statement") statements following
  [`USE db2`](use.html "13.8.4 USE Statement").

  [**mysqlbinlog --database=db2**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") does not
  output the first two [`INSERT`](insert.html "13.2.5 INSERT Statement")
  statements because there is no default database. It does not
  output the three [`INSERT`](insert.html "13.2.5 INSERT Statement")
  statements following
  [`USE test`](use.html "13.8.4 USE Statement"), but
  does output the three [`INSERT`](insert.html "13.2.5 INSERT Statement")
  statements following
  [`USE db2`](use.html "13.8.4 USE Statement").

  **Row-based logging.**
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") outputs only entries that
  ...
  *`db_name`*. The default database
  ...
  --database=test**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") outputs only those entries that
  modify `t1` in the test database,
  regardless of whether [`USE`](use.html "13.8.4 USE Statement")
  ...
  use [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") with the
  [`--database`](mysqlbinlog.html#option_mysqlbinlog_database) option, you
  must ensure that tables that are modified are in the
  database selected by [`USE`](use.html "13.8.4 USE Statement"). (In
...
<td><code>d:t:o,/tmp/mysqlbinlog.trace</code></td>
</tr></tbody></table>
...
<td><code>--help</code></td>
  ...
  *`str`*. For example,
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") normally reads the
  `[client]` and
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") also reads the
  `[client_other]` and
...
<td><code>--help</code></td>
  ...
  This option causes [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to include
  a [`SET
  sql_log_bin = 0`](set-sql-log-bin.html "13.4.1.3 SET sql_log_bin Statement") statement in its output to disable
...
<td><code>--help</code></td>
  ...
  With this option, if [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") reads a
  binary log event that it does not recognize, it prints a
  warning, ignores the event, and continues. Without this
  option, [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") stops if it reads
  such an event.
...
<td><p class="valid-value"><code>AUTO</code></p><p class="valid-value"><code>NEVER</code></p><p class="valid-value"><code>DECODE-ROWS</code></p></td>
  ...
  [Section 4.6.7.1, “mysqlbinlog Hex Dump Format”](mysqlbinlog-hexdump.html "4.6.7.1 mysqlbinlog Hex Dump Format"). The hex output can be
...
<td><p class="valid-value"><code>AUTO</code></p><p class="valid-value"><code>NEVER</code></p><p class="valid-value"><code>DECODE-ROWS</code></p></td>
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") client and session only.
...
<td><p class="valid-value"><code>AUTO</code></p><p class="valid-value"><code>NEVER</code></p><p class="valid-value"><code>DECODE-ROWS</code></p></td>
  ...
  [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") statements,
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") extracts the files from the
  binary log events, writes them as temporary files to the
  local file system, and writes
  [`LOAD DATA
  LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") statements to cause the files to be loaded.
  By default, [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") writes these
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") should prepare local
  temporary files.

  Important

  These temporary files are not automatically removed by
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") or any other MySQL program.
...
<td><p class="valid-value"><code>AUTO</code></p><p class="valid-value"><code>NEVER</code></p><p class="valid-value"><code>DECODE-ROWS</code></p></td>
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") prompts for one. If given,
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") should not prompt for one,
  use the
...
<td><code>--bind-address=ip_address</code></td>
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") does not find it. See
...
<td><code>--bind-address=ip_address</code></td>
  ...
  By default, [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") reads binary log
  files and writes events in text format. The
  [`--raw`](mysqlbinlog.html#option_mysqlbinlog_raw) option tells
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to write them in their
  original binary format. Its use requires that
  [`--read-from-remote-server`](mysqlbinlog.html#option_mysqlbinlog_read-from-remote-server)
  also be used because the files are requested from a server.
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") writes one output file for
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") stays connected to the
...
<td><code>--bind-address=ip_address</code></td>
  ...
  remote server, not relay log files, and takes only the
  binary log file name (including the numeric suffix) as its
  argument, while ignoring any path.
...
<td><code>--bind-address=ip_address</code></td>
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") writes text output. With
  [`--raw`](mysqlbinlog.html#option_mysqlbinlog_raw),
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") writes one binary output file
...
<td><code>--bind-address=ip_address</code></td>
  ...
  *`to_name`*. Rewriting is done on the
  ...
  [`USE`](use.html "13.8.4 USE Statement") clauses, for
  statement-based logs. In MySQL versions prior to 5.7.8, this
  option was only for use when restoring tables logged using
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") is started with
  ...
  `--rewrite-db='mydb->yourdb'
  --database=mydb`, then
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") outputs no statements at all:
...
* [`--secure-auth`](mysqlbinlog.html#option_mysqlbinlog_secure-auth)
...
<td><code>--bind-address=ip_address</code></td>
  ...
  ([`--skip-secure-auth`](mysqlbinlog.html#option_mysqlbinlog_secure-auth),
  [`--secure-auth=0`](mysqlbinlog.html#option_mysqlbinlog_secure-auth))
...
<td><code>18446744073709547520</code></td>
  ...
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") with server-id-bits set to less
  than 32 and user data stored in the most significant bit,
  running [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") with
  `--server-id-bits` set to 32 enables this
  data to be seen.

  This option is supported only by the version of
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") supplied with the NDB Cluster
...
<td><code>18446744073709547520</code></td>
  ...
  The
  [`--server-public-key-path`](mysqlbinlog.html#option_mysqlbinlog_server-public-key-path)
  option was added in MySQL 5.7.23.
...
<td><code>18446744073709547520</code></td>
  ...
  Add a [`SET NAMES
  charset_name`](set-names.html "13.7.4.3 SET NAMES Statement") statement
...
<td><code>18446744073709547520</code></td>
  ...
  *`datetime`* value is relative to the
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files"). The value should be in a
  format accepted for the
  [`DATETIME`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") or
  [`TIMESTAMP`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") data types. For
  example:
...
<td>Directory name</td>
  ...
  [`--read-from-remote-server`](mysqlbinlog.html#option_mysqlbinlog_read-from-remote-server).
  It tells [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to remain connected
  to the server. Otherwise [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files")
...
<td>Directory name</td>
  ...
  With [`--stop-never`](mysqlbinlog.html#option_mysqlbinlog_stop-never),
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") reports a server ID of 65535
  when it connects to the server.
  [`--stop-never-slave-server-id`](mysqlbinlog.html#option_mysqlbinlog_stop-never-slave-server-id)
  explicitly specifies the server ID to report. It can be used
  to avoid a conflict with the ID of a replica server or
  another [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") process. See
...
<td>Directory name</td>
  ...
  In MySQL 5.7, the version number shown by
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") when using this option is
  3.4.

You can pipe the output of [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") into
the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client to execute the events
contained in the binary log. This technique is used to recover
from an unexpected exit when you have an old backup (see
[Section 7.5, “Point-in-Time (Incremental) Recovery”](point-in-time-recovery.html "7.5 Point-in-Time (Incremental) Recovery")). For example:
...
If the statements produced by [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") may
contain [`BLOB`](blob.html "11.3.4 The BLOB and TEXT Types") values, these may
cause problems when [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") processes them. In
this case, invoke [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") with the
[`--binary-mode`](mysql-command-options.html#option_mysql_binary-mode) option.

You can also redirect the output of
[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to a text file instead, if you
...
by using it as input to the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") program:
...
When [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") is invoked with the
...
[`CREATE TEMPORARY
TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") statement and the second log contains a
statement that uses the temporary table. When the first
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") process terminates, the server drops
the temporary table. When the second [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
process attempts to use the table, the server reports
“unknown table.”

To avoid problems like this, use a *single*
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") process to execute the contents of all
...
[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") can produce output that
reproduces a [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") operation
without the original data file. [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files")
copies the data to a temporary file and writes a
[`LOAD DATA
LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") statement that refers to the file. The default
...
Because [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") converts
[`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") statements to
[`LOAD DATA
LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") statements (that is, it adds
...
[`LOAD DATA
LOCAL`](load-data.html "13.2.6 LOAD DATA Statement") statements are *not*
