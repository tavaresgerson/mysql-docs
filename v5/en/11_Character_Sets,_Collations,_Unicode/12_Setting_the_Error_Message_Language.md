...
By default, [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") produces error messages in
...
language), see [Section 10.6, “Error Message Character Set”](charset-errors.html "10.6 Error Message Character Set"). For general
...
* It looks for the file in a directory constructed from two
  ...
  [`lc_messages_dir`](server-system-variables.html#sysvar_lc_messages_dir) and
  [`lc_messages`](server-system-variables.html#sysvar_lc_messages), with the latter
  ...
  In this case, [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") maps the locale
...
* If the message file cannot be found in the directory
  ...
  [`lc_messages`](server-system-variables.html#sysvar_lc_messages) value and uses
  only the [`lc_messages_dir`](server-system-variables.html#sysvar_lc_messages_dir)
  value as the location in which to look.

The [`lc_messages_dir`](server-system-variables.html#sysvar_lc_messages_dir) system
variable can be set only at server startup and has only a global
read-only value at runtime.
[`lc_messages`](server-system-variables.html#sysvar_lc_messages) can be set at server
...
[`lc_messages`](server-system-variables.html#sysvar_lc_messages) value to the desired
