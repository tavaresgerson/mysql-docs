...
`utf8` is used because MySQL converts
instances of `utf8mb3` specified in such
definitions to `utf8`, which is an alias for
`utf8mb3`.
...
* For the variable-length character data types
  ...
  [`TEXT`](blob.html "11.3.4 The BLOB and TEXT Types") types), the maximum
...
TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement"). Suppose that a table has this definition:
...
[`TEXT`](blob.html "11.3.4 The BLOB and TEXT Types") data types, watch for these
...
* A [`TINYTEXT`](blob.html "11.3.4 The BLOB and TEXT Types") column can hold up
  to 255 bytes, so it can hold up to 85 3-byte or 63 4-byte
  characters. Suppose that you have a
  [`TINYTEXT`](blob.html "11.3.4 The BLOB and TEXT Types") column that uses
  ...
  [`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") column may need to be
  changed to one of the longer
  [`TEXT`](blob.html "11.3.4 The BLOB and TEXT Types") types if you want to
...
* `InnoDB` has a maximum index length of 767
  ...
  key prefixes](glossary.html#glos_index_prefix "index prefix") longer than 767 bytes (up to 3072
  bytes). Creating such tables also requires the option
  values
  [`innodb_file_format=barracuda`](innodb-parameters.html#sysvar_innodb_file_format)
  and
  [`innodb_file_per_table=true`](innodb-parameters.html#sysvar_innodb_file_per_table).)
  In this case, enabling the
  [`innodb_large_prefix`](innodb-parameters.html#sysvar_innodb_large_prefix)
  option enables you to index a maximum of 1024 or 768
  characters for `utf8mb3` or
  `utf8mb4` columns, respectively. For
  related information, see [Section 14.23, “InnoDB Limits”](innodb-limits.html "14.23 InnoDB Limits").
...
problems, using [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") as
...
* For object definitions that refer to the
  ...
  with [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") prior to downgrading, edit
  ...
  and creates new objects that use the (3-byte)
  `utf8` character set.