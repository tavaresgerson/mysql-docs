...
* [`START SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement") starts replication
  threads for all channels, except the
  `group_replication_recovery` and
  `group_replication_applier` channels.

* [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") stops replication
  threads for all channels, except the
  `group_replication_recovery` and
  `group_replication_applier` channels.

* [`SHOW SLAVE STATUS`](show-slave-status.html "13.7.5.34 SHOW SLAVE STATUS Statement") reports the
  status for all channels, except the
  `group_replication_applier` channel.

* [`FLUSH RELAY
  LOGS`](flush.html "13.7.6.3 FLUSH Statement") flushes the relay logs for all channels, except
  the `group_replication_applier` channel.

* [`RESET
  SLAVE`](reset-slave.html "13.4.2.3 RESET SLAVE Statement") resets all channels.
