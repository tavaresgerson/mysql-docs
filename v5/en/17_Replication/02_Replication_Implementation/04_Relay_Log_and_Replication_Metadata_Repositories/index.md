...
* The *relay log*, which is written by the
  replication I/O thread, contains the transactions read from
  the replication source server's binary log. The transactions
  in the relay log are applied on the replica by the replication
  SQL thread. For information about the relay log, see
...
* The replica's *connection metadata
  ...
  the `mysql.slave_master_info` table or to a
  file.
...
* The replica's *applier metadata repository*
  ...
  `mysql.slave_relay_log_info` table or to a
  file.

The connection metadata repository and applier metadata repository
are collectively known as the replication metadata repositories.
...
[`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine"). Updates to the replica's
