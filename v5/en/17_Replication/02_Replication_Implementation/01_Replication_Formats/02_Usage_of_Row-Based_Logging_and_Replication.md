...
[Section 16.2.1, “Replication Formats”](replication-formats.html "16.2.1 Replication Formats"), and
[Section 16.2.1.1, “Advantages and Disadvantages of Statement-Based and Row-Based
Replication”](replication-sbr-rbr.html "16.2.1.1 Advantages and Disadvantages of Statement-Based and Row-Based Replication").
...
* **Row-based logging of temporary tables.**
  ...
  “safe” statements involving temporary tables
  are logged using statement-based format. For more
  information, see [Section 16.2.1.1, “Advantages and Disadvantages of Statement-Based and Row-Based
  Replication”](replication-sbr-rbr.html "16.2.1.1 Advantages and Disadvantages of Statement-Based and Row-Based Replication").
  ...
  logs a `DROP TEMPORARY TABLE IF EXISTS`
  ...
  table was created, the `DROP TEMPORARY TABLE IF
  EXISTS` statement is not logged. In previous
  releases, the `DROP TEMPORARY TABLE IF
  EXISTS` statement was logged regardless of the
...
* **Reading the binary log.**
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") displays row-based events
  in the binary log using the `BINLOG`
  statement (see [Section 13.7.6.1, “BINLOG Statement”](binlog.html "13.7.6.1 BINLOG Statement")). This statement
  ...
  [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") formats the contents of the
  ...
  information, see [Section 4.6.7.2, “mysqlbinlog Row Event Display”](mysqlbinlog-row-events.html "4.6.7.2 mysqlbinlog Row Event Display").
...
* **Binary log execution errors and replica execution mode.**
  ...
  When [`slave_exec_mode`](replication-options-replica.html#sysvar_slave_exec_mode) is
  ...
  about [`slave_exec_mode`](replication-options-replica.html#sysvar_slave_exec_mode),
...
* **Filtering based on server ID not supported.**
  ...
  some replicas is to use a `WHERE` clause
  that includes the relation `@@server_id <>
  id_value` clause with
  [`UPDATE`](update.html "13.2.11 UPDATE Statement") and
  [`DELETE`](delete.html "13.2.2 DELETE Statement") statements. For
...
* **RBL, nontransactional tables, and stopped replicas.**
  ...
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") for all tables
  replicated using the row-based format. Use of
  [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") or
  [`STOP SLAVE
  SQL_THREAD`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") prior to shutting down the replica
  server helps prevent issues from occurring, and is always
  recommended regardless of the logging format or storage
  engine you use.