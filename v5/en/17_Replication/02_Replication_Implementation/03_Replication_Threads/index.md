...
* **Binary log dump thread.**
  ...
  PROCESSLIST`](show-processlist.html "13.7.5.29 SHOW PROCESSLIST Statement") on the source as the `Binlog
  Dump` thread.
...
* **Replication I/O thread.**
  When a [`START SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement") statement
  is issued on a replica server, the replica creates an I/O
  thread, which connects to the source and asks it to send the
...
[`slave_parallel_workers`](replication-options-replica.html#sysvar_slave_parallel_workers) set to a
...
See [Section 21.7.3, “Known Issues in NDB Cluster Replication”](mysql-cluster-replication-issues.html "21.7.3 Known Issues in NDB Cluster Replication") for more
information.