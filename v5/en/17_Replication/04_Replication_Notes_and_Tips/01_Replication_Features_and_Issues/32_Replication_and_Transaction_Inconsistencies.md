...
* *Gaps*. A gap is a transaction that has
  ...
  and that [`log-bin`](replication-options-binary-log.html#sysvar_log_bin) and
  [`log-slave-updates`](replication-options-binary-log.html#sysvar_log_slave_updates) are also
  enabled. Note that
  [`slave_preserve_commit_order=1`](replication-options-replica.html#sysvar_slave_preserve_commit_order)
...
2. [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") shuts down. Both clean and unclean
   shutdown abort ongoing transactions and may leave gaps and
   half-applied transactions.

3. [`KILL`](kill.html "13.7.6.4 KILL Statement") of replication threads
...
5. [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") when using a
   multithreaded replica. After issuing
   [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement"), the replica waits
   ...
   [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") completes, either
   an error happens, or another thread issues
   [`KILL`](kill.html "13.7.6.4 KILL Statement"), or the server restarts.
   In these cases, [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement")
...
6. If the last transaction in the relay log is only
   ...
   [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") waits up to 60
...
7. [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") when the ongoing
   transaction updates transactional tables only, in which case
   it is rolled back and [`STOP
   SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") stops immediately. If the ongoing
   transaction is mixed, [`STOP
   SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") waits up to 60 seconds for the transaction
...
SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") return to the client, but the replication
...
2. The field `Exec_master_log_pos` in
   [`SHOW SLAVE STATUS`](show-slave-status.html "13.7.5.34 SHOW SLAVE STATUS Statement") is only a
...
3. [`CHANGE MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") statements
   for that channel fail with an error, unless the applier
   threads are running and the [`CHANGE
   MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") statement only sets receiver options.

4. If [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") is started with
...
5. If [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") is used with
   [`--dump-slave`](mysqldump.html#option_mysqldump_dump-slave), it does not
   record the existence of gaps; thus it prints
   [`CHANGE MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") with
...
[`START SLAVE
[SQL_THREAD]`](start-slave.html "13.4.2.5 START SLAVE Statement") always consults this information so that
it applies only the correct transactions. This remains true even
if [`slave_parallel_workers`](replication-options-replica.html#sysvar_slave_parallel_workers) has
been changed to 0 before [`START
SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement"), and even if [`START
SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement") is used with `UNTIL` clauses.
[`START SLAVE UNTIL
SQL_AFTER_MTS_GAPS`](start-slave.html "13.4.2.5 START SLAVE Statement") only applies as many transactions
as needed in order to fill in the gaps. If
[`START SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement") is used with
`UNTIL` clauses that tell it to stop before it
has consumed all the gaps, then it leaves remaining gaps.

Warning

[`RESET SLAVE`](reset-slave.html "13.4.2.3 RESET SLAVE Statement") removes the relay
logs and resets the replication position. Thus issuing
[`RESET SLAVE`](reset-slave.html "13.4.2.3 RESET SLAVE Statement") on a replica with
