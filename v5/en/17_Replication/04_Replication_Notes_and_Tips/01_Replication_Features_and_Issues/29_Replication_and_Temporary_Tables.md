...
[`binlog_format=ROW`](replication-options-binary-log.html#sysvar_binlog_format) because, in
...
stop the replica server (not just the replication threads) and
you have replicated temporary tables that are open for use in
updates that have not yet been executed on the replica. If you
stop the replica server, the temporary tables needed by those
...
2. Use [`SHOW STATUS`](show-status.html "13.7.5.35 SHOW STATUS Statement") to check the
   value of the
...
3. If the value is not 0, restart the replication SQL thread
   with `START SLAVE SQL_THREAD` and repeat
   the procedure later.

4. When the value is 0, issue a [**mysqladmin
   shutdown**](mysqladmin.html "4.5.2 mysqladmin — A MySQL Server Administration Program") command to stop the replica.
