...
However, when [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") parses a
`SET @@sql_mode =
...
locking, and the [`SET PASSWORD`](set-password.html "13.7.1.7 SET PASSWORD Statement")
...
[`INSERT`](insert.html "13.2.5 INSERT Statement") statements on a
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") table on the source that is
...
For example, the following sequence of statements do not insert
the same data on the source and the replica:
