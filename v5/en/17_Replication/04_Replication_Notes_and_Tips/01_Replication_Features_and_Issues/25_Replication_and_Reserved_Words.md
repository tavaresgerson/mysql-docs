...
running on the replica. An example of this is using a table
column named `virtual` on a 5.6 source that is
replicating to a 5.7 or higher replica because
`VIRTUAL` is a reserved word beginning in MySQL
...
* Use one or more [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement")
...
[Keywords and Reserved Words in MySQL 5.7](/doc/mysqld-version-reference/en/keywords-5-7.html), in the *MySQL Server
