...
source uses a given [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") table
after startup, it logs an event that notifies replicas that the
table must be emptied by writing a
[`DELETE`](delete.html "13.2.2 DELETE Statement") or (from MySQL 5.7.32)
[`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") statement for that
...
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") table during the interval
...
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables become empty. This
...
* Statements such as
  [`INSERT INTO
  ... SELECT FROM
  memory_table`](insert-select.html "13.2.5.1 INSERT ... SELECT Statement") may insert
  a different set of rows on the source and replica.

The replica also writes a [`DELETE`](delete.html "13.2.2 DELETE Statement")
or (from MySQL 5.7.32) [`TRUNCATE
TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") statement to its own binary log, which is passed
...
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables is to first drop or
delete all rows from the [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine")
...
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables still differ from
those on the source. This is acceptable if the application logic
is such that the contents of [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine")
tables can be safely lost (for example, if the
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables are used for
caching).
[`slave_exec_mode=IDEMPOTENT`](replication-options-replica.html#sysvar_slave_exec_mode)
applies globally to all tables, so it may hide other replication
errors in non-[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables.

(The method just described is not applicable in NDB Cluster,
where [`slave_exec_mode`](replication-options-replica.html#sysvar_slave_exec_mode) is always
`IDEMPOTENT`, and cannot be changed.)

The size of [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables is
...
[Section 16.4.1.37, “Replication and Variables”](replication-features-variables.html "16.4.1.37 Replication and Variables")). A change in
`max_heap_table_size` takes effect for
`MEMORY` tables that are created or updated
using [`ALTER TABLE
... ENGINE = MEMORY`](alter-table.html "13.1.8 ALTER TABLE Statement") or [`TRUNCATE
TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") following the change, or for all
[`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables following a server
...
See [Section 15.3, “The MEMORY Storage Engine”](memory-storage-engine.html "15.3 The MEMORY Storage Engine"), for more
information about [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") tables.