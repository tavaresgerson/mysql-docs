...
(see [Section 16.4.1.10, “Replication with Differing Table Definitions on Source and Replica”](replication-features-differing-tables.html "16.4.1.10 Replication with Differing Table Definitions on Source and Replica")).

Replication between tables having different partitioning is
generally not supported. This because statements (such as
[`ALTER
TABLE ... DROP PARTITION`](alter-table-partition-operations.html "13.1.8.1 ALTER TABLE Partition Operations")) acting directly on
...
results on source and replica), we recommend that insure that
the partitioning of any tables to be replicated from the source
