...
When a replica loses its connection to the source, the replica
tries to reconnect immediately and retries periodically if that
fails. The default is to retry every 60 seconds. This may be
changed with the [`CHANGE MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement")
statement. A replica also is able to deal with network
connectivity outages. However, the replica notices the network
outage only after receiving no data from the source for
[`slave_net_timeout`](replication-options-replica.html#sysvar_slave_net_timeout) seconds. If
your outages are short, you may want to decrease
[`slave_net_timeout`](replication-options-replica.html#sysvar_slave_net_timeout). See
...
back up. Setting [`sync_binlog=1`](replication-options-binary-log.html#sysvar_sync_binlog)
in the source's `my.cnf` file helps to
...
tell [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") that the flush-to-disk operation
...
[Section 16.4.1.29, “Replication and Temporary Tables”](replication-features-temptables.html "16.4.1.29 Replication and Temporary Tables"). Unclean
...
* A similar problem can occur if the replica updates
  ...
  [`sync_relay_log_info=1`](replication-options-replica.html#sysvar_sync_relay_log_info) in
  the replica `my.cnf` file. Setting
  [`sync_relay_log_info`](replication-options-replica.html#sysvar_sync_relay_log_info) to 0
