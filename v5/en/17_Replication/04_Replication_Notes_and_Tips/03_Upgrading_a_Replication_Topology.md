...
instance, see [Section 2.10, “Upgrading MySQL”](upgrading.html "2.10 Upgrading MySQL").

As explained in
[Section 16.4.2, “Replication Compatibility Between MySQL Versions”](replication-compatibility.html "16.4.2 Replication Compatibility Between MySQL Versions"), MySQL supports
...
5.7.22, MySQL 5.7.24, and MySQL
5.7.28 concurrently in such a setup, although you
...
[Upgrade Checker Utility](/doc/mysql-shell/8.0/en/mysql-shell-utilities-upgrade.html) for
...
[Section 16.1.3.4, “Setting Up Replication Using GTIDs”](replication-gtids-howto.html "16.1.3.4 Setting Up Replication Using GTIDs") for information about
...
[`STRICT_ALL_TABLES`](sql-mode.html#sqlmode_strict_all_tables)) may result
...
[Section 2.10, “Upgrading MySQL”](upgrading.html "2.10 Upgrading MySQL") for each individual MySQL Server
...
1. Upgrade the replicas first. On each replica instance:
   ...
   * Carry out the preliminary checks and steps described in
     [Preparing Your Installation for Upgrade](/doc/refman/8.0/en/upgrade-prerequisites.html).
   ...
   * If you have upgraded to a release earlier than MySQL
     8.0.16, invoke [`mysqld_upgrade`](mysql-upgrade.html "4.4.7 mysql_upgrade — Check and Upgrade MySQL Tables") manually
     ...
     [`mysqld_upgrade`](mysql-upgrade.html "4.4.7 mysql_upgrade — Check and Upgrade MySQL Tables") (so do not use the
   ...
   * If you have upgraded to MySQL 8.0.16 or later, do not
     invoke [`mysqld_upgrade`](mysql-upgrade.html "4.4.7 mysql_upgrade — Check and Upgrade MySQL Tables"). From that
   ...
   * Restart replication using a [`START
     REPLICA`](/doc/refman/8.0/en/start-replica.html) or [`START
     SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement") statement.
...
2. When all the replicas have been upgraded, follow the same
   ...
   REPLICA`](/doc/refman/8.0/en/start-replica.html) or [`START
   SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement") statement. If you made a temporary change to
...
[Section 2.10.3, “Changes in MySQL 5.7”](upgrading-from-previous-series.html "2.10.3 Changes in MySQL 5.7"). It is
...
1. Stop all the replicas and upgrade the binaries or packages.
   ...
   [`--skip-slave-start`](replication-options-replica.html#option_mysqld_skip-slave-start) option, or
   from MySQL 8.0.24, the
   [`skip_slave_start`](/doc/refman/8.0/en/replication-options-replica.html#sysvar_skip_slave_start)
...
4. Re-enable the binary log on the source. If you set
   [`sql_log_bin`](replication-options-binary-log.html#sysvar_sql_log_bin) to
...
5. Restart the replicas, this time without the
   [`--skip-slave-start`](replication-options-replica.html#option_mysqld_skip-slave-start) option or
   [`skip_slave_start`](/doc/refman/8.0/en/replication-options-replica.html#sysvar_skip_slave_start) system
   variable.