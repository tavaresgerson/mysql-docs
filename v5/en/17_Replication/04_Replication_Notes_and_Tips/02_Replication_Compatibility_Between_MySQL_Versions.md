...
on. However, you may encounter difficulties when replicating from
an older source to a newer replica if the source uses statements
or relies on behavior no longer supported in the version of MySQL
used on the replica. For example, foreign key names longer than 64
...
Replication from newer sources to older replicas may be possible,
...
* **Binary log format changes.**
  ...
  [Section 16.4.3, “Upgrading a Replication Topology”](replication-upgrade.html "16.4.3 Upgrading a Replication Topology"), for more information.
