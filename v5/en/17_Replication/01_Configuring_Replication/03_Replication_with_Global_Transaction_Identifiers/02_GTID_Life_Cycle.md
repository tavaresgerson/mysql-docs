...
1. A transaction is executed and committed on the replication
   source server. This client transaction is assigned a GTID
   composed of the source's UUID and the smallest nonzero
   transaction sequence number not yet used on this server. The
   GTID is written to the source's binary log (immediately
   preceding the transaction itself in the log). If a client
   transaction is not written to the binary log (for example,
   because the transaction was filtered out, or the transaction
...
4. After the binary log data is transmitted to the replica and
   ...
   [Section 16.2, “Replication Implementation”](replication-implementation.html "16.2 Replication Implementation"), for details),
...
8. If binary logging is disabled on the replica, the GTID is
   ...
   record of the transactions applied on the replica. Note that
   in MySQL 5.7, the operation to insert the GTID into the table
   is atomic for DML statements, but not for DDL statements, so
   if the server exits unexpectedly after a transaction involving
...
[`slave_parallel_workers > 0`](replication-options-replica.html#sysvar_slave_parallel_workers) ),
transactions can be applied in parallel, so replicated
transactions can commit out of order (unless
[`slave_preserve_commit_order=1`](replication-options-replica.html#sysvar_slave_preserve_commit_order) is
...
SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") statement, ongoing transactions are applied so
that the gaps are filled in. In the event of a shutdown such as a
server failure or the use of the
[`KILL`](kill.html "13.7.6.4 KILL Statement") statement to stop replication
...
that use the [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") storage engine
...
* A multi-table [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement")
  statement drops tables of different types.

* A
  [`CREATE
  TABLE ... SELECT`](create-table-select.html "13.1.18.4 CREATE TABLE ... SELECT Statement") statement is issued when
  ...
  TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") action and one GTID is generated for the
  row-insert actions.
...
[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to generate a dump of the binary
...
[`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged) when
[`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed) (and therefore
[`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged)) is empty. For
...
computed for [`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed) and
[`gtid_purged`](replication-options-gtids.html#sysvar_gtid_purged), and they remain
...
the [`RESET MASTER`](reset-master.html "13.4.1.2 RESET MASTER Statement") statement. For
...
Use [`RESET MASTER`](reset-master.html "13.4.1.2 RESET MASTER Statement") with caution
to avoid losing any wanted GTID execution history and binary
log files.

Before issuing [`RESET MASTER`](reset-master.html "13.4.1.2 RESET MASTER Statement"),
...
GTID set, use [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files") to examine the
...
When you issue [`RESET MASTER`](reset-master.html "13.4.1.2 RESET MASTER Statement"), the
...
Note that [`RESET MASTER`](reset-master.html "13.4.1.2 RESET MASTER Statement") is the
method to reset the GTID execution history even if the server is
a replica where binary logging is disabled.
[`RESET SLAVE`](reset-slave.html "13.4.2.3 RESET SLAVE Statement") has no effect on the
