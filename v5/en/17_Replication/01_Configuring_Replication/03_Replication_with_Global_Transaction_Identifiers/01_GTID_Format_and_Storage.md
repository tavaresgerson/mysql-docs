...
[**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files"), and it is used to identify an
...
are GTID sets. The [`START SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement")
...
upgraded, using a [`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement")
...
MASTER`](reset-master.html "13.4.1.2 RESET MASTER Statement").
...
* If binary logging is disabled (`log_bin` is
  ...
  transaction together with the transaction in the buffer when
  the transaction is committed, and the background thread adds
  the contents of the buffer periodically as one or more
  entries to the `mysql.gtid_executed` table.
  In addition, the table is compressed periodically at a
  ...
  replica where binary logging or replica update logging is
  disabled. It does not apply on a replication source server,
  because on the source, binary logging must be enabled for
...
* If binary logging is enabled (`log_bin` is
  ...
  situation applies on a replication source server, or a
  replica where binary logging is enabled.
  ...
  `mysql.gtid_executed` table does not hold a
  ...
  variable. Always use
  `@@GLOBAL.gtid_executed`, which is updated
...
PROCESSLIST`](show-processlist.html "13.7.5.29 SHOW PROCESSLIST Statement"), but it can be viewed as a row in the
[`threads`](performance-schema-threads-table.html "25.12.16.4 The threads Table") table, as shown here:
