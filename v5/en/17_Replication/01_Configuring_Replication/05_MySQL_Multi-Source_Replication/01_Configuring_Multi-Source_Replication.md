...
replication. See [Section 16.1.3.4, “Setting Up Replication Using GTIDs”](replication-gtids-howto.html "16.1.3.4 Setting Up Replication Using GTIDs") for how
to configure a source using GTID-based replication. See
[Section 16.1.2.1, “Setting the Replication Source Configuration”](replication-howto-masterbaseconfig.html "16.1.2.1 Setting the Replication Source Configuration") for how to
configure a source using file position based replication.

Replicas in a multi-source replication topology require
`TABLE` repositories for the connection metadata
...
`FILE` repositories for the replication metadata
repositories to use `TABLE` repositories, you can
convert the existing repositories dynamically by using the
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") client to issue the following statements
on the replica:
...
SLAVE`](privileges-provided.html#priv_replication-slave) privilege. For example, to set up a new user,
`ted`, that can connect from the replica
`replicahost`, use the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
