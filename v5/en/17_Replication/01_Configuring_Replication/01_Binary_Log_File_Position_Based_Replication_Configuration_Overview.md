...
operating as the source (where the database changes originate)
...
(using the [`server_id`](replication-options.html#sysvar_server_id) system
...
within a MySQL session using the [`CHANGE
MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") statement on the replica. The details are
