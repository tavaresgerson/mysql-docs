...
1. Execute the following on each replica, and if you using
   multi-source replication, do it for each channel and include
   the `FOR CHANNEL` channel clause:
...
4. On each server, wait until the variable @@GLOBAL.GTID\_OWNED is
...
5. Wait for all transactions that currently exist in any binary
   ...
   [Section 16.1.4.4, “Verifying Replication of Anonymous Transactions”](replication-mode-change-online-verify-transactions.html "16.1.4.4 Verifying Replication of Anonymous Transactions")
   for one method of checking that all anonymous transactions
   have replicated to all servers.

6. If you use binary logs for anything else than replication, for
   ...
   [`FLUSH LOGS`](flush.html#flush-logs) on the server where
