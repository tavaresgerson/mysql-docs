...
* GTID transactions are identified by a global transaction
  ...
  `Gtid_log_event`. GTID transactions can be
...
* Anonymous transactions do not have a GTID assigned, and MySQL
  ...
  by an `Anonymous_gtid_log_event`. In previous
...
[`WAIT_FOR_EXECUTED_GTID_SET()`](gtid-functions.html#function_wait-for-executed-gtid-set),
[`session_track_gtids`](server-system-variables.html#sysvar_session_track_gtids), and monitor
replicated transactions using Performance Schema tables. With
GTIDs enabled you cannot use
[`sql_slave_skip_counter`](replication-options-replica.html#sysvar_sql_slave_skip_counter), instead
...
the [`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) and
...
[Section 5.1.8.1, “System Variable Privileges”](system-variable-privileges.html "5.1.8.1 System Variable Privileges"). In previous
...
[`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) could be set to
`ON` or `OFF`, which
corresponded to whether GTIDs were used to identify transactions
or not. When [`gtid_mode=ON`](replication-options-gtids.html#sysvar_gtid_mode) it is
not possible to replicate anonymous transactions, and when
[`gtid_mode=OFF`](replication-options-gtids.html#sysvar_gtid_mode) only anonymous
transactions can be replicated. As of MySQL 5.7.6, the
[`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) variable has two
additional states, `OFF_PERMISSIVE` and
`ON_PERMISSIVE`. When
[`gtid_mode=OFF_PERMISSIVE`](replication-options-gtids.html#sysvar_gtid_mode) then
*new* transactions are anonymous while
permitting replicated transactions to be either GTID or anonymous
transactions. When
[`gtid_mode=ON_PERMISSIVE`](replication-options-gtids.html#sysvar_gtid_mode) then
...
[`gtid_mode=ON`](replication-options-gtids.html#sysvar_gtid_mode) could be replicating
to a replica with
[`gtid_mode=ON_PERMISSIVE`](replication-options-gtids.html#sysvar_gtid_mode). The
valid values for [`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) are as
...
[`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) can only be changed by
...
`OFF` or `ON_PERMISSIVE` but not
to `ON`. This is to ensure that the process of
...
[`gtid_mode=OFF`](replication-options-gtids.html#sysvar_gtid_mode), the GTID state (in
other words the value of
[`gtid_executed`](replication-options-gtids.html#sysvar_gtid_executed)) is persistent.
...
[`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode). This means that fields
...
`RECEIVED_TRANSACTION_SET` in the
...
[`SHOW SLAVE STATUS`](show-slave-status.html "13.7.5.34 SHOW SLAVE STATUS Statement"), now return the
empty string when there are no GTIDs present. Fields that display
a single GTID, such as `CURRENT_TRANSACTION` in
...
table, now display `ANONYMOUS` when GTID
transactions are not being used.

Replication from a source using
[`gtid_mode=ON`](replication-options-gtids.html#sysvar_gtid_mode) provides the ability
to use auto-positioning, configured using the `CHANGE
MASTER TO MASTER_AUTO_POSITION = 1;` statement. The
...
[Section 16.1.4.2, “Enabling GTID Transactions Online”](replication-mode-change-online-enable-gtids.html "16.1.4.2 Enabling GTID Transactions Online"). The
valid combinations of [`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode)
and auto-positioning on source and replica are shown in the
following table, where the source's
[`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) is shown on the
horizontal and the replica's
[`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) is on the vertical:

**Table 16.1 Valid Combinations of Source and Replica gtid\_mode**

<table width="708"><col style="width: 2%"/><col style="width: 1%"/><col style="width: 2%"/><col style="width: 21%"/><col style="width: 17%"/><thead><tr>
<th><p>
<code>gtid_mode</code>
</p></th>
<th><p>
              Source <code>OFF</code>
</p></th>
<th><p>
              Source <code>OFF_PERMISSIVE</code>
</p></th>
<th><p>
              Source <code>ON_PERMISSIVE</code>
</p></th>
<th><p>
              Source <code>ON</code>
</p></th>
...
<th><p>
              Replica <code>OFF_PERMISSIVE</code>
</p></th>
<td><p>
              Y
            </p></td>
...
<th><p>
              Replica <code>ON_PERMISSIVE</code>
</p></th>
<td><p>
              Y
            </p></td>
...
* `Y`: the
  [`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode) of source and
...
The currently selected [`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode)
also impacts on the [`gtid_next`](replication-options-gtids.html#sysvar_gtid_next)
variable. The following table shows the behavior of the server for
the different values of [`gtid_mode`](replication-options-gtids.html#sysvar_gtid_mode)
and [`gtid_next`](replication-options-gtids.html#sysvar_gtid_next).

**Table 16.2 Valid Combinations of gtid\_mode and gtid\_next**

<table><col style="width: 2.03%"/><col style="width: 1%"/><col style="width: 2.01%"/><col style="width: 1.92%"/><col style="width: 1.04%"/><thead><tr>
<th><p>
<code>gtid_next</code>
</p></th>
<th><p>
...
* `Error`: generate an error and fail to
...
[`gtid_next`](replication-options-gtids.html#sysvar_gtid_next) is set to
`AUTOMATIC`, then no GTID is generated. This is
