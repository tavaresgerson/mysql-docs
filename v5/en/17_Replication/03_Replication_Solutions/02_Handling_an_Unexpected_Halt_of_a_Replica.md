...
instead use an [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") table named
`mysql.slave_relay_log_info` to store the applier
...
[`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") table, set the system variable
[`relay_log_info_repository`](replication-options-replica.html#sysvar_relay_log_info_repository) to
`TABLE`. For more information on the applier
...
replication setup that uses [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine")
with transactions. However, other settings specific to
[`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") or to other storage engines,
...
* When GTID-based replication is in use
  ([`gtid_mode=ON`](replication-options-gtids.html#sysvar_gtid_mode)), set
  ...
  TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") statement. If the replica has multiple
  replication channels, you need to set this option for each
  channel individually. For details of how GTID auto-positioning
  works, see
  [Section 16.1.3.3, “GTID Auto-Positioning”](replication-gtids-auto-positioning.html "16.1.3.3 GTID Auto-Positioning"). When
...
* Set [`sync_relay_log=1`](replication-options-replica.html#sysvar_sync_relay_log), which
...
* Set
  [`innodb_flush_log_at_trx_commit=1`](innodb-parameters.html#sysvar_innodb_flush_log_at_trx_commit),
  which synchronizes the [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine")
  logs to disk before each transaction is committed. This
  setting, which is the default, ensures that
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") tables and the
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") logs are saved on disk so
  that there is no longer a requirement for the information in
  the relay log regarding the transaction. Combined with the
  setting [`sync_relay_log=1`](replication-options-replica.html#sysvar_sync_relay_log),
  this setting further ensures that the content of the
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") tables and the
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") logs is consistent with
...
* Set [`relay_log_info_repository =
  TABLE`](replication-options-replica.html#sysvar_relay_log_info_repository), which stores the replication SQL thread
  position in the [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") table
  ...
  `FILE` setting is used, the information is
  ...
  FILE`](replication-options-replica.html#sysvar_relay_log_info_repository), recovery is not guaranteed.

* Set [`relay_log_recovery = ON`](replication-options-replica.html#sysvar_relay_log_recovery),
  ...
  [`--relay-log-recovery`](replication-options-replica.html#sysvar_relay_log_recovery) option at
...
method as the [`START
SLAVE UNTIL SQL_AFTER_MTS_GAPS`](start-slave.html "13.4.2.5 START SLAVE Statement") statement would. When the
...
[`relay_log_recovery = OFF`](replication-options-replica.html#sysvar_relay_log_recovery),
starting the replica with
[`START SLAVE UNTIL
SQL_AFTER_MTS_GAPS`](start-slave.html "13.4.2.5 START SLAVE Statement") to fix any transaction
inconsistencies, and then restarting the replica with
[`relay_log_recovery = ON`](replication-options-replica.html#sysvar_relay_log_recovery). When
