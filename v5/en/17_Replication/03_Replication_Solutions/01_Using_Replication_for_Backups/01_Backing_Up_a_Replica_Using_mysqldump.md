...
Using [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") to create a copy of a
...
[Section 4.5.4, “mysqldump — A Database Backup Program”](mysqldump.html "4.5.4 mysqldump — A Database Backup Program")). Because the format of the
...
your data set is very large, [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") may be
impractical.

When using [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program"), you should stop
...
1. Stop the replica from processing requests. You can stop
   ...
   relay logs using the I/O thread, but prevents the replica
   from executing these events and changing its data. Within
   busy replication environments, permitting the I/O thread to
   run during backup may speed up the catch-up process when you
   restart the replication SQL thread.

2. Run [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") to dump your databases. You
...
source. See [Section 16.1.7.1, “Checking Replication Status”](replication-administration-status.html "16.1.7.1 Checking Replication Status").
