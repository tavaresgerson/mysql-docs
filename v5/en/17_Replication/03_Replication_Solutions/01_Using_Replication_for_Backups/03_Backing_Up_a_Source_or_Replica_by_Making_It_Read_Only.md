...
manipulating the [`read_only`](server-system-variables.html#sysvar_read_only)
...
data from the server, such as [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program")
(see [Section 4.5.4, “mysqldump — A Database Backup Program”](mysqldump.html "4.5.4 mysqldump — A Database Backup Program")). You should not attempt to
...
server and for a replica server. For both scenarios discussed
...
[`read_only`](server-system-variables.html#sysvar_read_only) variable are
...
use [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program").
...
This strategy applies to backing up a source server in a
...
updates from the binary log of the source.