...
of the replica), then in addition to the replica's data, you
should also back up the replica status files, the replication
metadata repositories, and the relay log files. These files are
needed to resume replication after you restore the
replica's data.
...
[`CHANGE MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") with the
`MASTER_LOG_FILE` and
...
DATA`](load-data.html "13.2.6 LOAD DATA Statement") statements, you should also back up any
...
[`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement") operations. The
