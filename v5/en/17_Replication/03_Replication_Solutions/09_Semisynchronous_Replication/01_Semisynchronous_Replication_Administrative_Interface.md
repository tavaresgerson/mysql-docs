...
* System variables control plugin behavior. Some examples:
  ...
  + [`rpl_semi_sync_master_enabled`](replication-options-source.html#sysvar_rpl_semi_sync_master_enabled)
    ...
    on the source. To enable or disable the plugin, set this
...
* From MySQL 5.7.33, you can improve the performance of
  ...
  performance. Semisynchronous replication source servers can
  also get performance benefits from enabling these system
  variables, because they use the same locking mechanisms as
  the replicas.
...
* Status variables enable semisynchronous replication
  ...
  + [`Rpl_semi_sync_master_status`](server-status-variables.html#statvar_Rpl_semi_sync_master_status)
    ...
    operational on the source. The value is 1 if the plugin
    has been enabled and a commit acknowledgment has not
    occurred. It is 0 if the plugin is not enabled or the
    source has fallen back to asynchronous replication due
