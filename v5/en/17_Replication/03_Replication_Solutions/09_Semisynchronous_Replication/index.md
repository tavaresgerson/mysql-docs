...
transaction, all replicas must also have committed the transaction
...
not be reused as the replication source server, and should be
...
[`SET autocommit =
0`](set-variable.html "13.7.4.1 SET Syntax for Variable Assignment")), autocommit is enabled and each statement commits
...
replication source waits for replica acknowledgment of transaction
receipt before returning a status to the client that committed the
transaction. These values are permitted:

* `AFTER_SYNC` (the default): The source writes
...
* With `AFTER_COMMIT`, the client issuing the
  ...
  exit and failover to the replica, it is possible that such
  clients see a loss of data relative to what they saw on the
  source.