...
[`CHANGE MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") statement. The
...
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server"). Assume that you have the structure
shown in [Figure 16.4, “Redundancy Using Replication, Initial Structure”](replication-solutions-switch.html#figure_replication-redundancy-before "Figure 16.4 Redundancy Using Replication, Initial Structure").
...
structure, see [Section 16.3.4, “Using Replication for Scale-Out”](replication-solutions-scaleout.html "16.3.4 Using Replication for Scale-Out").
...
[`SHOW PROCESSLIST`](show-processlist.html "13.7.5.29 SHOW PROCESSLIST Statement") until you see
...
source, issue [`STOP SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") and
[`RESET MASTER`](reset-master.html "13.4.1.2 RESET MASTER Statement").

On the other replicas `Replica 2` and
`Replica 3`, use [`STOP
SLAVE`](stop-slave.html "13.4.2.6 STOP SLAVE Statement") and `CHANGE MASTER TO
MASTER_HOST='Replica1'` (where
`'Replica1'` represents the real host name of
`Replica 1`). To use `CHANGE MASTER
TO`, add all information about how to connect to
`Replica 1` from `Replica 2` or
`Replica 3` (*`user`*,
*`password`*,
...
SLAVE`](start-slave.html "13.4.2.5 START SLAVE Statement") on `Replica 2` and
`Replica 3`.
...
[Figure 16.5, “Redundancy Using Replication, After Source Failure”](replication-solutions-switch.html#figure_replication-redundancy-after "Figure 16.5 Redundancy Using Replication, After Source Failure").
...
[`CHANGE MASTER TO`](change-master-to.html "13.4.2.1 CHANGE MASTER TO Statement") statement as that
...
[`RESET MASTER`](reset-master.html "13.4.1.2 RESET MASTER Statement") on
