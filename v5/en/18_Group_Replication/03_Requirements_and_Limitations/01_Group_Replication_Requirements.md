...
* **InnoDB Storage Engine.**
  ...
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") transactional storage
  ...
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") provides some
  ...
  [`MEMORY`](memory-storage-engine.html "15.3 The MEMORY Storage Engine") storage engine, might
  cause errors in Group Replication. Convert any tables in
  other storage engines to use
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") before using the
...
* **Unique Server Identifier.**
  ...
  as required for all servers in replication topologies.
  With the default server ID of 0, servers in a replication
  topology cannot connect to each other. The server ID must
  be a positive integer between 1 and
...
* **Replica Updates Logged.**
  Set [`--log-slave-updates`](replication-options-binary-log.html#sysvar_log_slave_updates).
...
* **Binary Log Row Format.**
  Set [`--binlog-format=row`](replication-options-binary-log.html#sysvar_binlog_format).
...
* **Binary Log Checksums Off.**
  Set
  [`--binlog-checksum=NONE`](replication-options-binary-log.html#sysvar_binlog_checksum).
...
* **Replication Information Repositories.**
  ...
  The replication applier needs to have the source and
  replica metadata written to the
...
* **Transaction Write Set Extraction.**
  Set
  [`--transaction-write-set-extraction=XXHASH64`](replication-options-binary-log.html#sysvar_transaction_write_set_extraction)
...
* **Lower Case Table Names.**
  Set
  [`--lower-case-table-names`](server-system-variables.html#sysvar_lower_case_table_names)
  to the same value on all group members. A setting of 1 is
  correct for the use of the
  [`InnoDB`](innodb-storage-engine.html "Chapter 14 The InnoDB Storage Engine") storage engine, which
...
* **Multithreaded Appliers.**
  ...
  1024 parallel applier threads can be specified. If you do
  this, the following settings are also required:
  ...
  the [`slave_parallel_type`](replication-options-replica.html#sysvar_slave_parallel_type) and
