...
for [`INSERT`](insert.html "13.2.5 INSERT Statement") into a column with an
exact data type ([`DECIMAL`](fixed-point-types.html "11.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") or
...
truncation for [`INSERT`](insert.html "13.2.5 INSERT Statement") is
permissible.)
...
* If no approximate values are present, the expression contains
  ...
  evaluated using [`DECIMAL`](fixed-point-types.html "11.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") exact
...
* Otherwise, the expression contains only integer values. The
  ...
  [`BIGINT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") (64 bits).
...
is controlled by the [`sql_mode`](server-system-variables.html#sysvar_sql_mode)
system variable. (See [Section 5.1.10, “Server SQL Modes”](sql-mode.html "5.1.10 Server SQL Modes").) The following
discussion mentions strict mode (selected by the
[`STRICT_ALL_TABLES`](sql-mode.html#sqlmode_strict_all_tables) or
[`STRICT_TRANS_TABLES`](sql-mode.html#sqlmode_strict_trans_tables) mode values)
...
[`TRADITIONAL`](sql-mode.html#sqlmode_traditional) mode, which includes
...
([`DECIMAL`](fixed-point-types.html "11.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") or integer), it is
...
[Section 12.21.4, “Rounding Behavior”](precision-math-rounding.html "12.21.4 Rounding Behavior"). Truncation due to
...
For [`DECIMAL`](fixed-point-types.html "11.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") literals, in addition
...
With the
[`ERROR_FOR_DIVISION_BY_ZERO`](sql-mode.html#sqlmode_error_for_division_by_zero) SQL
...
requires
[`ERROR_FOR_DIVISION_BY_ZERO`](sql-mode.html#sqlmode_error_for_division_by_zero) in
...
<table summary="What happens for combinations of strict and ERROR_FOR_DIVISION_BY_ZERO modes."><col style="width: 50%"/><col style="width: 50%"/><thead><tr>
<th><code>sql_mode</code> Value</th>
<th>Result</th>
</tr></thead><tbody><tr>
<td><code>''</code> (Default)</td>
<td>No warning, no error; <code>i</code> is set to
            <code>NULL</code>.</td>
</tr><tr>
<td>strict</td>
<td>No warning, no error; <code>i</code> is set to
...
<td>Warning, no error; <code>i</code> is set to
            <code>NULL</code>.</td>
</tr><tr>
<td>strict,<code>ERROR_FOR_DIVISION_BY_ZERO</code></td>
