...
</tr><tr><td><code>UUID()</code></td>
<td>
      Return a Universal Unique Identifier (UUID)
    </td>
</tr><tr><td><code>UUID_SHORT()</code></td>
<td>
      Return an integer-valued universal identifier
    </td>
</tr><tr><td><code>VALUES()</code></td>
<td>
...
* [`INET_ATON(expr)`](miscellaneous-functions.html#function_inet-aton)
  ...
  [`INT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT"), which is signed. If you
...
* [`INET6_ATON(expr)`](miscellaneous-functions.html#function_inet6-aton)
  ...
  [`VARBINARY(16)`](binary-varbinary.html "11.3.3 The BINARY and VARBINARY Types") for IPv6
  addresses and [`VARBINARY(4)`](binary-varbinary.html "11.3.3 The BINARY and VARBINARY Types") for
  ...
  [`INT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") value to an IPv6 address
  represented in numeric form as a
  [`VARBINARY`](binary-varbinary.html "11.3.3 The BINARY and VARBINARY Types") value, use this
  expression:
...
* [`IS_IPV4(expr)`](miscellaneous-functions.html#function_is-ipv4)
  ...
  [`INET6_ATON()`](miscellaneous-functions.html#function_inet6-aton)) returns a value
  that is not `NULL`. The converse statement is
  not true: In some cases,
  [`INET_ATON()`](miscellaneous-functions.html#function_inet-aton) returns a value
  other than `NULL` when
...
* [`MASTER_POS_WAIT(log_name,log_pos[,timeout][,channel])`](miscellaneous-functions.html#function_master-pos-wait)

  This function is useful for control of source-replica
  synchronization. It blocks until the replica has read and
  applied all updates up to the specified position in the source
  log. The return value is the number of log events the replica
  had to wait for to advance to the specified position. The
  function returns `NULL` if the replica SQL
  ...
  exceeded. If the replica SQL thread stops while
  [`MASTER_POS_WAIT()`](miscellaneous-functions.html#function_master-pos-wait) is waiting,
  ...
  [`slave_checkpoint_group`](replication-options-replica.html#sysvar_slave_checkpoint_group) or
  ...
  [`MASTER_POS_WAIT()`](miscellaneous-functions.html#function_master-pos-wait) stops waiting
  when *`timeout`* seconds have elapsed.
  *`timeout`* must be greater than or
  equal to 0. (As of MySQL 5.7.18, when the server is running in
  strict SQL mode, a negative *`timeout`*
  value is immediately rejected with
  [`ER_WRONG_ARGUMENTS`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_wrong_arguments); otherwise
  the function returns **`NULL`**, and raises a
  warning.)

  The optional *`channel`* value enables
  you to name which replication channel the function applies to.
  See [Section 16.2.2, “Replication Channels”](replication-channels.html "16.2.2 Replication Channels") for more
  information.
...
* [`NAME_CONST(name,value)`](miscellaneous-functions.html#function_name-const)
  ...
  [Section 23.7, “Stored Program Binary Logging”](stored-programs-logging.html "23.7 Stored Program Binary Logging"). You might see this
  function in the output from [**mysqlbinlog**](mysqlbinlog.html "4.6.7 mysqlbinlog — Utility for Processing Binary Log Files").
  ...
  See [Section 13.2.9, “SELECT Statement”](select.html "13.2.9 SELECT Statement"), for more information about
  column aliases.
...
* [`SLEEP(duration)`](miscellaneous-functions.html#function_sleep)
  ...
  *`duration`* argument, then returns 0.
  ...
  + This statement is interrupted using
    [`KILL QUERY`](kill.html "13.7.6.4 KILL Statement")
...
* [`UUID_SHORT()`](miscellaneous-functions.html#function_uuid-short)
  ...
  + You do not set back the system time for your server host
    between [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") restarts
  ...
  + You invoke [`UUID_SHORT()`](miscellaneous-functions.html#function_uuid-short) on
    ...
    [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") restarts
...
* [`VALUES(col_name)`](miscellaneous-functions.html#function_values)
  ...
  In an
  [`INSERT
  ... ON DUPLICATE KEY UPDATE`](insert-on-duplicate.html "13.2.5.2 INSERT ... ON DUPLICATE KEY UPDATE Statement") statement, you can use
  the
  `VALUES(col_name)`
  function in the [`UPDATE`](update.html "13.2.11 UPDATE Statement") clause
  to refer to column values from the
  [`INSERT`](insert.html "13.2.5 INSERT Statement") portion of the
  statement. In other words,
  `VALUES(col_name)`
  in the [`UPDATE`](update.html "13.2.11 UPDATE Statement") clause refers to
  ...
  of [`INSERT`](insert.html "13.2.5 INSERT Statement") statements and
