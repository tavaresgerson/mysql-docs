...
[`GROUPING()`](/doc/refman/8.0/en/miscellaneous-functions.html#function_grouping) function to test the
distinction.
...
`ORDER BY` clause to sort the results. In
other words, `ROLLUP` and `ORDER
BY` are mutually exclusive in MySQL. However, you
still have some control over sort order. To work around the
restriction that prevents using `ROLLUP` with
`ORDER BY` and achieve a specific sort order
of grouped results, generate the grouped result set as a
derived table and apply `ORDER BY` to it. For
example:
...
`GROUP BY`, see
[Section 12.19.3, “MySQL Handling of GROUP BY”](group-by-handling.html "12.19.3 MySQL Handling of GROUP BY").) In this case, the server
