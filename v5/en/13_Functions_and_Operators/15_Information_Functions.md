...
* `CONNECTION_ID()`
  ...
  The value returned by `CONNECTION_ID()` is the same type of value as displayed in the `ID` column of the Information Schema `PROCESSLIST` table, the `Id` column of [`SHOW PROCESSLIST`](show-processlist.html "13.7.5.29 SHOW PROCESSLIST Statement") output, and the `PROCESSLIST_ID` column of the Performance Schema `threads` table.
...
* `LAST_INSERT_ID()`, `LAST_INSERT_ID(expr)`
  ...
  + For stored functions and triggers that change the value, the value is restored when the function or trigger ends, so statements following them do not see a changed value.
...
* `ROW_COUNT()`
  ...
  + DML statements other than `SELECT`: The number of affected rows. This applies to statements such as `UPDATE`, `INSERT`, or `DELETE` (as before), but now also to statements such as [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") and [`LOAD DATA`](load-data.html "13.2.6 LOAD DATA Statement").
