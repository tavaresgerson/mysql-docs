...
* If one of the arguments is a
  [`TIMESTAMP`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") or
  [`DATETIME`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") column and the other
  ...
  be compared to a [`DATETIME`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types")
  value, the comparison is done as two integers. The integer is
  not converted to a temporal value. To compare the operands as
  [`DATETIME`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") values, use
  [`CAST()`](cast-functions.html#function_cast) to explicitly convert
  the subquery value to [`DATETIME`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types").
...
to another, see [Section 11.2.9, “Conversion Between Date and Time Types”](date-and-time-type-conversion.html "11.2.9 Conversion Between Date and Time Types").
...
`c3`, which is a
[`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") column, with integer
`0`:
...
[`SELECT`](select.html "13.2.9 SELECT Statement") is part of a data definition
statement such as
[`CREATE TABLE
... SELECT`](create-table-select.html "13.1.18.4 CREATE TABLE ... SELECT Statement"); in strict mode, the statement fails due to
...
[`DECIMAL`](fixed-point-types.html "11.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") values and
approximate-value
([`FLOAT`](floating-point-types.html "11.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE")/[`DOUBLE`](floating-point-types.html "11.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE"))
numbers:
...
value with a [`DECIMAL`](fixed-point-types.html "11.1.3 Fixed-Point Types (Exact Value) - DECIMAL, NUMERIC") or string
...
[`SET NAMES`](set-names.html "13.7.4.3 SET NAMES Statement"). For information about
...
(nonbinary) string (a [`CHAR`](char.html "11.3.2 The CHAR and VARCHAR Types"),
[`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types"), or
[`LONGTEXT`](blob.html "11.3.4 The BLOB and TEXT Types") value), except in the case
that the connection character set is set to
`binary`. In that case, the conversion result is
a binary string (a [`BINARY`](binary-varbinary.html "11.3.3 The BINARY and VARBINARY Types"),
[`VARBINARY`](binary-varbinary.html "11.3.3 The BINARY and VARBINARY Types"), or
[`LONGBLOB`](blob.html "11.3.4 The BLOB and TEXT Types") value).
...
expression has type [`INT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") or
[`BIGINT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") depending on the length of
the integer expression. If the maximum length of the expression
does not fit in an [`INT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT"),
[`BIGINT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") is used instead. The length
is taken from the `max_length` value of the
[`SELECT`](select.html "13.2.9 SELECT Statement") result set metadata (see
[C API Basic Data Structures](/doc/c-api/5.7/en/c-api-data-structures.html)). This means that you can
force a [`BIGINT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") rather than
[`INT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") by use of a sufficiently long
expression:
