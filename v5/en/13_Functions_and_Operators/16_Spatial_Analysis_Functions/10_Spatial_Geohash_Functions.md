...
* [`ST_GeoHash(longitude,
  ...
  is `NULL`. If any argument is invalid, an
  error occurs.
...
* [`ST_LatFromGeoHash(geohash_str)`](spatial-geohash-functions.html#function_st-latfromgeohash)
  ...
  [`DOUBLE`](floating-point-types.html "11.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") value in the range
  [−90, 90].

  If the argument is `NULL`, the return value
  is `NULL`. If the argument is invalid, an
  error occurs.
...
* [`ST_PointFromGeoHash(geohash_str,
  ...
  The *`srid`* argument is an unsigned
  32-bit integer.
