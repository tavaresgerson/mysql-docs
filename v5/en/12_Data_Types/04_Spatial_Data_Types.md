...
MySQL provides a standard way of creating spatial columns for geometry types, for example, with [`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement") or `ALTER TABLE`. Spatial columns are supported for `MyISAM`, `InnoDB`, `NDB`, and `ARCHIVE` tables. See also the notes about spatial indexes under Section 11.4.9, “Creating Spatial Indexes”.
...
To drop spatial indexes, use [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") or `DROP INDEX`:
