...
(JavaScript Object Notation) data type defined by
[RFC
...
roughly the same as for [`LONGBLOB`](blob.html "11.3.4 The BLOB and TEXT Types") or
[`LONGTEXT`](blob.html "11.3.4 The BLOB and TEXT Types"); see
[Section 11.7, “Data Type Storage Requirements”](storage-requirements.html "11.7 Data Type Storage Requirements"), for more information. It
...
functions, see [Section 12.17, “JSON Functions”](json-functions.html "12.17 JSON Functions").

A set of spatial functions for operating on GeoJSON values is also
available. See [Section 12.16.11, “Spatial GeoJSON Functions”](spatial-geojson-functions.html "12.16.11 Spatial GeoJSON Functions").
...
`JSON` columns and MySQL JSON functions,
including creation of an index on a column generated from a
`JSON` column as a workaround for being unable to
index a `JSON` column. A maximum of 3
`JSON` columns per
[`NDB`](mysql-cluster.html "Chapter 21 MySQL NDB Cluster 7.5 and NDB Cluster 7.6") table is supported.
...
[Section 12.17.2, “Functions That Create JSON Values”](json-creation-functions.html "12.17.2 Functions That Create JSON Values")) as well as by casting
...
[`JSON_MERGE()`](json-modification-functions.html#function_json-merge) takes two or more
...
doing a simple [`SELECT`](select.html "13.2.9 SELECT Statement"), like this:
...
This “first key wins” handling of duplicate keys
is not consistent with
[RFC
7159](https://tools.ietf.org/html/rfc7159). This is a known issue in MySQL 5.7, which is
...
[Section 12.17.2, “Functions That Create JSON Values”](json-creation-functions.html "12.17.2 Functions That Create JSON Values")) always return
normalized values.

To make lookups more efficient, it also sorts the keys of a JSON
object. *You should be aware that the result of this
ordering is subject to change and not guaranteed to be
...
[`JSON_MERGE()`](json-modification-functions.html#function_json-merge) merges its arguments
into a single array:
...
[Section 12.17.3, “Functions That Search JSON Values”](json-search-functions.html "12.17.3 Functions That Search JSON Values"), for more information.
...
elsewhere in this Manual (see [Section 12.17, “JSON Functions”](json-functions.html "12.17 JSON Functions"))
...
* `INTEGER`, `DOUBLE`
  ...
  types of numbers, see [Section 9.1.2, “Numeric Literals”](number-literals.html "9.1.2 Numeric Literals").

  The rules for comparing native MySQL numeric types are
  discussed in [Section 12.3, “Type Conversion in Expression Evaluation”](type-conversion.html "12.3 Type Conversion in Expression Evaluation"), but the
  rules for comparing numbers within JSON values differ
  somewhat:

  + In a comparison between two columns that use the native
    MySQL [`INT`](integer-types.html "11.1.2 Integer Types (Exact Value) - INTEGER, INT, SMALLINT, TINYINT, MEDIUMINT, BIGINT") and
    [`DOUBLE`](floating-point-types.html "11.1.4 Floating-Point Types (Approximate Value) - FLOAT, DOUBLE") numeric types,
...
<th>other type</th>
<th>CAST(other type AS JSON)</th>
<th>CAST(JSON AS other type)</th>
</tr></thead><tbody><tr>
<th>JSON</th>
<td>No change</td>
<td>No change</td>
</tr><tr>
<th>utf8 character type (<code>utf8mb4</code>,
              <code>utf8</code>, <code>ascii</code>)</th>
<td>The string is parsed into a JSON value.</td>
<td>The JSON value is serialized into a <code>utf8mb4</code> string.</td>
</tr><tr>
<th>Other character types</th>
<td>Other character encodings are implicitly converted to
              <code>utf8mb4</code> and treated as described for
              utf8 character type.</td>
<td>The JSON value is serialized into a <code>utf8mb4</code> string,
              then cast to the other character encoding. The result may
              not be meaningful.</td>
</tr><tr>
<th><code>NULL</code></th>
<td>Results in a <code>NULL</code> value of type JSON.</td>
...
<td>Succeeds if the JSON document consists of a single scalar value of the
              ...
              target type. Otherwise, returns <code>NULL</code>
