...
In MySQL 5.7.9 and later, you can use `column->path` with a JSON column identifier and JSON path expression as a synonym for [`JSON_EXTRACT(column, path)`](json-search-functions.html#function_json-extract). See Section 12.17.3, “Functions That Search JSON Values”, for more information. See also Indexing a Generated Column to Provide a JSON Column Index.
