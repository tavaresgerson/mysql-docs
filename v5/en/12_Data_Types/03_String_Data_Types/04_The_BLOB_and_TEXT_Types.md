...
into [`TEXT`](blob.html "11.3.4 The BLOB and TEXT Types") columns always
...
column as a [`VARBINARY`](binary-varbinary.html "11.3.3 The BINARY and VARBINARY Types") column that
can be as large as you like. Similarly, you can regard a
`TEXT` column as a
[`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") column.
`BLOB` and `TEXT` differ from
[`VARBINARY`](binary-varbinary.html "11.3.3 The BINARY and VARBINARY Types") and
[`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") in the following ways:

* For indexes on `BLOB` and
  `TEXT` columns, you must specify an index
  prefix length. For [`CHAR`](char.html "11.3.2 The CHAR and VARCHAR Types") and
  [`VARCHAR`](char.html "11.3.2 The CHAR and VARCHAR Types"), a prefix length is
  optional. See [Section 8.3.4, “Column Indexes”](column-indexes.html "8.3.4 Column Indexes").
...
* Instances of `BLOB` or
  ...
  [Section 8.4.4, “Internal Temporary Table Use in MySQL”](internal-temporary-tables.html "8.4.4 Internal Temporary Table Use in MySQL")). Use of disk
...
* The maximum size of a `BLOB` or
  ...
  client program. For example, both [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client")
  and [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") enable you to change the
  client-side
  [`max_allowed_packet`](server-system-variables.html#sysvar_max_allowed_packet) value.
  See [Section 5.1.1, “Configuring the Server”](server-configuration.html "5.1.1 Configuring the Server"),
  [Section 4.5.1, “mysql — The MySQL Command-Line Client”](mysql.html "4.5.1 mysql — The MySQL Command-Line Client"), and [Section 4.5.4, “mysqldump — A Database Backup Program”](mysqldump.html "4.5.4 mysqldump — A Database Backup Program").
  You may also want to compare the packet sizes and the size
  of the data objects you are storing with the storage
  requirements, see [Section 11.7, “Data Type Storage Requirements”](storage-requirements.html "11.7 Data Type Storage Requirements")
...
[Section 12.8, “String Functions and Operators”](string-functions.html "12.8 String Functions and Operators"). For security and other
