...
normalized form (the statement digest) and computes an MD5 hash
...
the server is executing and how often they occur. This section
describes how statement digesting occurs and how it can be useful.

Digesting occurs in the parser regardless of whether the
Performance Schema is available, so that other server components
such as MySQL Enterprise Firewall and query rewrite plugins have access to statement
digests.
...
Setting the [`max_digest_length`](server-system-variables.html#sysvar_max_digest_length)
system variable to zero disables digest production, which also
disables server functionality that requires digests.

After the normalized statement has been computed, an MD5 hash
...
* If the Performance Schema has digest instrumentation
  ...
  [`performance_schema_max_digest_length`](performance-schema-system-variables.html#sysvar_performance_schema_max_digest_length)
  is less than
  [`max_digest_length`](server-system-variables.html#sysvar_max_digest_length), the copy
...
[`max_digest_length`](server-system-variables.html#sysvar_max_digest_length)), truncation
...
* A `statements_digest` consumer in the
  [`setup_consumers`](performance-schema-setup-consumers-table.html "25.12.2.2 The setup_consumers Table") table controls
...
is 1024 bytes by default. To change this value, set the
...
[`max_digest_length`](server-system-variables.html#sysvar_max_digest_length) enables
...
[`max_digest_length`](server-system-variables.html#sysvar_max_digest_length) causes the
...
[`max_digest_length`](server-system-variables.html#sysvar_max_digest_length) and
[`performance_schema_max_digest_length`](performance-schema-system-variables.html#sysvar_performance_schema_max_digest_length):

* If [`max_digest_length`](server-system-variables.html#sysvar_max_digest_length) is
  less than
  [`performance_schema_max_digest_length`](performance-schema-system-variables.html#sysvar_performance_schema_max_digest_length):
...
* If [`max_digest_length`](server-system-variables.html#sysvar_max_digest_length) equals
  [`performance_schema_max_digest_length`](performance-schema-system-variables.html#sysvar_performance_schema_max_digest_length):
...
[`events_statements_history`](performance-schema-events-statements-history-table.html "25.12.6.2 The events_statements_history Table") table. A
...
consume *`N`* times the memory indicated
by the
[`performance_schema_max_digest_length`](performance-schema-system-variables.html#sysvar_performance_schema_max_digest_length)
value.
...
[`SHOW ENGINE
PERFORMANCE_SCHEMA STATUS`](show-engine.html "13.7.5.15 SHOW ENGINE Statement") statement, or monitor these
instruments:
