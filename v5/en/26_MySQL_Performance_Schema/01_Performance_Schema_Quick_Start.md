...
This section briefly introduces the Performance Schema with examples that show how to use it. For additional examples, see Section 25.19, “Using the Performance Schema to Diagnose Problems”.
...
The Performance Schema is implemented as a storage engine. If this engine is available (which you should already have checked earlier), you should see it listed with a `SUPPORT` value of `YES` in the output from the Information Schema `ENGINES` table or the `SHOW ENGINES` statement:
