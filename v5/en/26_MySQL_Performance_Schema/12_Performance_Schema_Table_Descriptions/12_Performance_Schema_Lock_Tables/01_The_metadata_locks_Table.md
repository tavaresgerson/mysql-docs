...
[Section 8.11.4, “Metadata Locking”](metadata-locking.html "8.11.4 Metadata Locking"). Metadata locking applies
...
through the [`metadata_locks`](performance-schema-metadata-locks-table.html "25.12.12.1 The metadata_locks Table") table:
...
The [`metadata_locks`](performance-schema-metadata-locks-table.html "25.12.12.1 The metadata_locks Table") table is read
...
update the [`setup_instruments`](performance-schema-setup-instruments-table.html "25.12.2.3 The setup_instruments Table")
table:
...
[`metadata_locks`](performance-schema-metadata-locks-table.html "25.12.12.1 The metadata_locks Table") table content as
...
* When a pending lock request is canceled by the deadlock
  ...
  ([`ER_LOCK_DEADLOCK`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_lock_deadlock)), its
  row status is updated from `PENDING` to
  `VICTIM`.

* When a pending lock request times out
  ([`ER_LOCK_WAIT_TIMEOUT`](/doc/mysql-errors/5.7/en/server-error-reference.html#error_er_lock_wait_timeout)),
...
The [`metadata_locks`](performance-schema-metadata-locks-table.html "25.12.12.1 The metadata_locks Table") table has
these columns:
...
* `OBJECT_TYPE`
  ...
  `EVENT`, `COMMIT`,
  `USER LEVEL LOCK`,
  `TABLESPACE`, or `LOCKING
  SERVICE`.

  A value of `USER LEVEL LOCK` indicates a
  lock acquired with
  [`GET_LOCK()`](locking-functions.html#function_get-lock). A value of
...
for the [`metadata_locks`](performance-schema-metadata-locks-table.html "25.12.12.1 The metadata_locks Table") table.