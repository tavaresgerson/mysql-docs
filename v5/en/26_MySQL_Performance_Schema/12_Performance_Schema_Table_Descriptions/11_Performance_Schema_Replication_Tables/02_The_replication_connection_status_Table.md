...
This table shows the current status of the replication I/O
thread that handles the replica's connection to the
source.
...
* `SOURCE_UUID`
  ...
  The [`server_uuid`](replication-options.html#sysvar_server_uuid) value
  from the source.
...
<table summary="Correspondence between replication_connection_status columns and SHOW SLAVE STATUS columns"><col style="width: 60%"/><col style="width: 40%"/><thead><tr>
<th><code>replication_connection_status</code> Column</th>
...
<td><code>Slave_IO_Running</code></td>
</tr><tr>
