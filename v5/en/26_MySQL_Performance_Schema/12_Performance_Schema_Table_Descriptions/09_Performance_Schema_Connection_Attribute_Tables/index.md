...
[`mysql_options()`](/doc/c-api/5.7/en/mysql-options.html) and
[`mysql_options4()`](/doc/c-api/5.7/en/mysql-options4.html) functions
...
* [`session_account_connect_attrs`](performance-schema-session-account-connect-attrs-table.html "25.12.9.1 The session_account_connect_attrs Table"):
  Connection attributes for the current session, and other
  sessions associated with the session account

* [`session_connect_attrs`](performance-schema-session-connect-attrs-table.html "25.12.9.2 The session_connect_attrs Table"):
...
[**mysqladmin**](mysqladmin.html "4.5.2 mysqladmin — A MySQL Server Administration Program") and [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program")
set `program_name` to
...
* [`FEDERATED`](federated-storage-engine.html "15.8 The FEDERATED Storage Engine") storage engine
  connections:
...
[`mysql_options()`](/doc/c-api/5.7/en/mysql-options.html) that cause this
limit to be exceeded produce a
[`CR_INVALID_PARAMETER_NO`](/doc/mysql-errors/5.7/en/client-error-reference.html#error_cr_invalid_parameter_no) error.
...
* The server imposes a limit of 64KB on the aggregate size of
  connection attribute data it can accept. If a client
