...
see [Section 16.4.1.15, “Replication and System Functions”](replication-features-functions.html "16.4.1.15 Replication and System Functions") and
...
* The server system time zone. When the server starts, it
  ...
  variable before you start [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server"). If you
  start the server using [`mysqld_safe`](mysqld-safe.html "4.3.2 mysqld_safe — MySQL Server Startup Script"), its
...
* The server current time zone. The global
  ...
  If you have the [`SUPER`](privileges-provided.html#priv_super)
...
and retrieved from [`TIMESTAMP`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types")
columns. Values for [`TIMESTAMP`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types")
...
[`TIME`](time.html "11.2.3 The TIME Type"), or
[`DATETIME`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") columns. Nor are values
in those data types stored in UTC; the time zone applies for
them only when converting from
[`TIMESTAMP`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") values. If you want
locale-specific arithmetic for
[`DATE`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types"),
[`TIME`](time.html "11.2.3 The TIME Type"), or
[`DATETIME`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") values, convert them to
...
* As a string indicating an offset from UTC of the form
  ...
  retriving the value in such cases. MySQL converts
  `'-00:00'` or `'-0:00'` to
  `'+00:00'`.
...
[Section 5.3, “The mysql System Database”](system-schema.html "5.3 The mysql System Database")). The MySQL installation
...
[**mysql\_tzinfo\_to\_sql**](mysql-tzinfo-to-sql.html "4.4.6 mysql_tzinfo_to_sql — Load the Time Zone Tables") program to load the time
...
[**mysql\_tzinfo\_to\_sql**](mysql-tzinfo-to-sql.html "4.4.6 mysql_tzinfo_to_sql — Load the Time Zone Tables") and send the output into
the [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") program. For example:

```sql
mysql_tzinfo_to_sql /usr/share/zoneinfo | mysql -u root -p mysql
```

The [**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") command shown here assumes that you
connect to the server using an account such as
`root` that has privileges for modifying tables
in the `mysql` system database. Adjust the
connection parameters as required.

[**mysql\_tzinfo\_to\_sql**](mysql-tzinfo-to-sql.html "4.4.6 mysql_tzinfo_to_sql — Load the Time Zone Tables") reads your system's time
zone files and generates SQL statements from them.
[**mysql**](mysql.html "4.5.1 mysql — The MySQL Command-Line Client") processes those statements to load the
time zone tables.

[**mysql\_tzinfo\_to\_sql**](mysql-tzinfo-to-sql.html "4.4.6 mysql_tzinfo_to_sql — Load the Time Zone Tables") also can be used to load
...
* To load a single time zone file
  *`tz_file`* that corresponds to a
  ...
  [**mysql\_tzinfo\_to\_sql**](mysql-tzinfo-to-sql.html "4.4.6 mysql_tzinfo_to_sql — Load the Time Zone Tables") like this:
...
After running [**mysql\_tzinfo\_to\_sql**](mysql-tzinfo-to-sql.html "4.4.6 mysql_tzinfo_to_sql — Load the Time Zone Tables"), restart
...
[**mysql\_tzinfo\_to\_sql**](mysql-tzinfo-to-sql.html "4.4.6 mysql_tzinfo_to_sql — Load the Time Zone Tables") utility instead.
...
is intended for older MySQL versions. MySQL 5.7 and higher
uses `InnoDB` for the time zone tables.
Trying to replace them with `MyISAM` tables
causes problems.
...
* If you replace the system's
  ...
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") startup, restart
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") so that it uses the updated rules.
  Otherwise, [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") might not notice when
...
* If you use named time zones with MySQL, make sure that the
  ...
  [Populating the Time Zone Tables](time-zone-support.html#time-zone-installation "Populating the Time Zone Tables").
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") caches time zone information that
  it looks up, so after updating the time zone tables, restart
  [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") to make sure that it does not
...
time zones obtain correct query results.
...
[`TIMESTAMP`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") values one second
...
[`TIMESTAMP`](datetime.html "11.2.2 The DATE, DATETIME, and TIMESTAMP Types") values (which are in
