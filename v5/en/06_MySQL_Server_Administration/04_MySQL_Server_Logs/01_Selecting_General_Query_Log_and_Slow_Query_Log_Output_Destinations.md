...
The [`log_output`](server-system-variables.html#sysvar_log_output) system variable
specifies the destination for log output. Setting this variable
does not in itself enable the logs; they must be enabled
separately.

* If [`log_output`](server-system-variables.html#sysvar_log_output) is not
  specified at startup, the default logging destination is
  `FILE`.

* If [`log_output`](server-system-variables.html#sysvar_log_output) is specified
...
The [`general_log`](server-system-variables.html#sysvar_general_log) system variable
controls logging to the general query log for the selected log
destinations. If specified at server startup,
[`general_log`](server-system-variables.html#sysvar_general_log) takes an optional
argument of 1 or 0 to enable or disable the log. To specify a
file name other than the default for file logging, set the
[`general_log_file`](server-system-variables.html#sysvar_general_log_file) variable.
Similarly, the [`slow_query_log`](server-system-variables.html#sysvar_slow_query_log)
variable controls logging to the slow query log for the selected
destinations and setting
[`slow_query_log_file`](server-system-variables.html#sysvar_slow_query_log_file) specifies a
...
* To write general query log entries to the log table and the
  log file, use
  [`--log_output=TABLE,FILE`](server-system-variables.html#sysvar_log_output) to
  select both log destinations and
  [`--general_log`](server-system-variables.html#sysvar_general_log) to enable the
  general query log.

* To write general and slow query log entries only to the log
  tables, use
  [`--log_output=TABLE`](server-system-variables.html#sysvar_log_output) to select
  tables as the log destination and
  [`--general_log`](server-system-variables.html#sysvar_general_log) and
  [`--slow_query_log`](server-system-variables.html#sysvar_slow_query_log) to enable
  both logs.

* To write slow query log entries only to the log file, use
  [`--log_output=FILE`](server-system-variables.html#sysvar_log_output) to select
  ...
  [`log_output`](server-system-variables.html#sysvar_log_output) setting.
...
* The [`log_output`](server-system-variables.html#sysvar_log_output) variable
  indicates the current logging destination. It can be
  modified at runtime to change the destination.

* The [`general_log`](server-system-variables.html#sysvar_general_log) and
  [`slow_query_log`](server-system-variables.html#sysvar_slow_query_log) variables
...
* The [`general_log_file`](server-system-variables.html#sysvar_general_log_file) and
...
* To disable or enable general query logging for the current
  ...
  [`sql_log_off`](server-system-variables.html#sysvar_sql_log_off) variable to
  `ON` or `OFF`. (This
...
* [`CREATE TABLE`](create-table.html "13.1.18 CREATE TABLE Statement"),
  [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement"), and
  [`DROP TABLE`](drop-table.html "13.1.29 DROP TABLE Statement") are valid
  operations on a log table. For [`ALTER
  TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") and [`DROP
  TABLE`](drop-table.html "13.1.29 DROP TABLE Statement"), the log table cannot be in use and must be
...
* By default, the log tables use the `CSV`
  ...
  [`ALTER TABLE`](alter-table.html "13.1.8 ALTER TABLE Statement") to alter a log
  ...
  **Log Tables and “Too many open files” Errors.**
  ...
  [`FLUSH
  TABLES`](flush.html "13.7.6.3 FLUSH Statement") or ensure that the value of
  [`open_files_limit`](server-system-variables.html#sysvar_open_files_limit) is
...
* To disable logging so that you can alter (or drop) a log
  ...
  and [`slow_query_log`](server-system-variables.html#sysvar_slow_query_log) system
  variable.
...
* [`TRUNCATE TABLE`](truncate-table.html "13.1.34 TRUNCATE TABLE Statement") is a valid
  operation on a log table. It can be used to expire log
  entries.

* [`RENAME TABLE`](rename-table.html "13.1.33 RENAME TABLE Statement") is a valid
...
* [`CHECK TABLE`](check-table.html "13.7.2.2 CHECK TABLE Statement") is a valid
  operation on a log table.

* [`LOCK TABLES`](lock-tables.html "13.3.5 LOCK TABLES and UNLOCK TABLES Statements") cannot be used on
  a log table.

* [`INSERT`](insert.html "13.2.5 INSERT Statement"),
  [`DELETE`](delete.html "13.2.2 DELETE Statement"), and
  [`UPDATE`](update.html "13.2.11 UPDATE Statement") cannot be used on a
...
* [`FLUSH TABLES WITH READ LOCK`](flush.html#flush-tables-with-read-lock)
  ...
  [`read_only`](server-system-variables.html#sysvar_read_only) system variable
...
* To flush the log tables or log files, use
  ...
  [`FLUSH LOGS`](flush.html#flush-logs), respectively.

* Partitioning of log tables is not permitted.
* A [**mysqldump**](mysqldump.html "4.5.4 mysqldump — A Database Backup Program") dump includes statements to
