...
the binary log. The exact format employed depends on the version
of MySQL being used. There are three logging formats:
...
* In *row-based logging*, the source writes
  events to the binary log that indicate how individual table
  rows are affected. It is important therefore that tables
  always use a primary key to ensure rows can be efficiently
  identified. You can cause the server to use row-based
  logging by starting it with
  [`--binlog-format=ROW`](replication-options-binary-log.html#sysvar_binlog_format).
...
* A third option is also available: *mixed
  ...
  by starting [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") with the option
  [`--binlog-format=MIXED`](replication-options-binary-log.html#sysvar_binlog_format).
