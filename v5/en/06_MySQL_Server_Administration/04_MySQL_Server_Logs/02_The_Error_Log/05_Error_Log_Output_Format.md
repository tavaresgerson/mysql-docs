...
The ID included in error log messages is that of the thread
within [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") responsible for writing the
message. This indicates which part of the server produced the
...
Permitted [`log_timestamps`](server-system-variables.html#sysvar_log_timestamps) values
are `UTC` (the default) and
