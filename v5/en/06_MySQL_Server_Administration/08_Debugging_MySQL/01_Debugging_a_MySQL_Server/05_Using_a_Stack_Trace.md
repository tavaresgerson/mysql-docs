...
if [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") dies unexpectedly. You can use this
to find out where (and maybe why) [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server")
died. See [Section 5.4.2, “The Error Log”](error-log.html "5.4.2 The Error Log"). To get a stack trace, you
must not compile [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") with the
...
[**resolve\_stack\_dump**](resolve-stack-dump.html "4.7.3 resolve_stack_dump — Resolve Numeric Stack Trace Dump to Symbols") utility to determine where
[`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") died by using the following procedure:

1. Copy the numbers from the stack trace to a file, for example
   `mysqld.stack`. The numbers should not
...
2. Make a symbol file for the [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") server:
   ...
   If [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") is not linked statically, use
   ...
   **nm** does not have this option, you must
   use the **c++filt** command after the stack
...
3. Execute the following command:
   ...
   If you were not able to include demangled C++ names in your
   symbol file, process the
   [**resolve\_stack\_dump**](resolve-stack-dump.html "4.7.3 resolve_stack_dump — Resolve Numeric Stack Trace Dump to Symbols") output using
   **c++filt**:
   ...
   This prints out where [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") died. If
   that does not help you find out why
   [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") died, you should create a bug
   ...
   to locate the bug or provide a workaround, in most cases we
   need to know the statement that killed
   [`mysqld`](mysqld.html "4.3.1 mysqld — The MySQL Server") and preferably a test case so that
