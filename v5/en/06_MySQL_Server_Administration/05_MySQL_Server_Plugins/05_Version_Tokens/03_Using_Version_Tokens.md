...
by calling functions. The [`SUPER`](privileges-provided.html#priv_super)
privilege is required to call any of the Version Token
functions, so token list modification is expected to be done by
a management or administrative application that has that
privilege.
...
service described at [Section 5.5.6.1, “The Locking Service”](locking-service.html "5.5.6.1 The Locking Service"), and
